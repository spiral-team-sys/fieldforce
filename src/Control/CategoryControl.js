...
export const CategoryControl = React.forwardRef((props, refParent) => {
  ...
  const onLoad = async () => {
    const f1 =
      seleted.division !== undefined ? seleted.division.code || null : null;
    const f2 =
      seleted.category !== undefined ? seleted.category.code || null : null;
    const f3 =
      seleted.subcate !== undefined ? seleted.subcate.code || null : null;
    const f4 =
      seleted.segment !== undefined ? seleted.segment.code || null : null;
    const f5 =
      seleted.subsegment !== undefined ? seleted.subsegment.code || null : null;
  ...
  const fillter = async (tagFilter, nextKey) => {
    const f1 =
      tagFilter.division !== undefined ? tagFilter.division.code || null : null;
    const f2 =
      tagFilter.category !== undefined ? tagFilter.category.code || null : null;
    const f3 =
      tagFilter.subcate !== undefined ? tagFilter.subcate.code || null : null;
    const f4 =
      tagFilter.segment !== undefined ? tagFilter.segment.code || null : null;
    const f5 =
      tagFilter.subsegment !== undefined
        ? tagFilter.subsegment.code || null
        : null;
    // console.log(nextKey)
    switch (nextKey) {
      case 'division':
  ...
  return (
    <View style={{ height: deviceHeight * 0.6 }}>
      <View
        style={{
          display: listDivision.length > 0 ? 'flex' : 'none',
          width: '100%',
          borderRadius: 12,
      ...
      <View
        style={{
          display: listCategory.length > 0 ? 'flex' : 'none',
          width: '100%',
          paddingLeft: 7,
          paddingEnd: 7,
          paddingTop: 7,
      ...
      >
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => {
            return (
              <TouchableOpacity
                onPress={() => onSeletedItem(item, 'category')}
                key={'D2' + index}
                style={{ padding: 3, marginBottom: 7 }}
              >
                <View
      ...
      <View
        style={{
          display: listSubCate.length > 0 ? 'flex' : 'none',
          width: '100%',
          paddingLeft: 7,
          paddingEnd: 7,
          paddingTop: 7,
      ...
      >
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <TouchableOpacity
              onPress={() => onSeletedItem(item, 'subcate')}
              key={'D3' + index}
              style={{ padding: 3, marginBottom: 7 }}
            >
              <View
                style={{
                  backgroundColor:
                    item.selected === true ? appcolor.primary : appcolor.light,
                  borderWidth: appcolor.surface,
                  flexDirection: 'row',
                  borderRadius: 40,
                  borderWidth: 1,
      ...
      <View
        style={{
          display: listSegment.length > 0 ? 'flex' : 'none',
          width: '100%',
          paddingLeft: 7,
          paddingEnd: 7,
          paddingTop: 7,
      ...
      >
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <TouchableOpacity
              onPress={() => onSeletedItem(item, 'segment')}
              key={'D4' + index}
              style={{ padding: 3, marginBottom: 7 }}
            >
              <View
      ...
      <View
        style={{
          display: listSubsegment.length > 0 ? 'flex' : 'none',
          width: '100%',
          paddingLeft: 7,
          paddingEnd: 7,
          paddingTop: 7,
      ...
      >
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <TouchableOpacity
              onPress={() => onSeletedItem(item, 'subsegment')}
              key={'D5' + index}
              style={{ padding: 3, marginBottom: 7 }}
            >
              <View
      ...
      >
        <FormGroup
          editable
          title={'Tổng số sản phẩm ' + listProduct.length || 0}
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <TouchableOpacity
              onPress={() => onSeletedItem(item, 'product')}
              style={{
                borderRadius: 12,
                marginBottom: 7,
                flexGrow: 1,
