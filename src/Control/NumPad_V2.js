...
export const NumPad_V2 = React.forwardRef((props, refNumPad) => {
    ...
    const { item, key, value, index, totalRow, placeholderText, handerNumberChange, upload, editable, containerStyle, inputStyle, iconSize, iconColor, iconStyle, showIcon = true, reloadNum } = props
        ...
        <View style={{ ...containerStyle, flexDirection: 'row', alignItems: 'center' }}>
            {
                ...
                <TouchableOpacity onPress={downAction} disabled={upload} style={{ paddingVertical: 3, paddingHorizontal: 5 }}>
                    <Icon
                        color={iconColor ? iconColor : appcolor.light}
                        style={{ ...iconStyle, backgroundColor: appcolor.dark, padding: 3, width: iconSize ? iconSize + 7 : 25, height: iconSize ? iconSize + 7 : 25, alignItems: 'center', justifyContent: 'center', borderRadius: 50 }}
            ...
            <View style={{ flexGrow: 1, minWidth: 40, alignItems: 'center', marginHorizontal: 3 }}>
                <TextInput keyboardType="number-pad"
                    ...
                    key={key}
                    maxlength={8}
                    value={value === null || isNaN(value) ? '' : value.toLocaleString("en-US")}
                    blurOnSubmit={false}
                    autoCorrect={false}
                    ...
                    style={{
                        ...
                        padding: 8, width: '100%', borderRadius: 5,
                        fontSize: scaleSize(12), color: appcolor.dark, textAlign: 'center',
                        ...inputStyle
                    }} />
            </View>
            {
                ...
                <TouchableOpacity onPress={upAction} disabled={upload} style={{ paddingVertical: 3, paddingHorizontal: 5 }} >
                    <Icon
                        name="plus"
                        color={iconColor ? iconColor : appcolor.light}
                        style={{ ...iconStyle, backgroundColor: appcolor.dark, padding: 3, width: iconSize ? iconSize + 7 : 25, height: iconSize ? iconSize + 7 : 25, alignItems: 'center', justifyContent: 'center', borderRadius: 50 }}
                        size={iconSize ? iconSize : 13} type="font-awesome-5"
                    />
                </TouchableOpacity>
                // </View>
            }
