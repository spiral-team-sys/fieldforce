...
import { imageSize, insets } from '../../Core/Utility';
...
const CameraReportAction = forwardRef((props, _ref) => {
    ...
    const takePicture = async () => {
        try {
            ...
            const imageResult = await resizeImage(photo.path, photo);
            if (imageResult) {
    ...
    const styles = StyleSheet.create({
        ...
        actionChangeWide: { padding: 8, borderRadius: 32, top: 14, end: 14, position: 'absolute', backgroundColor: appcolor.light },
        contentCameras: { position: 'absolute', top: insets().top, end: 8, zIndex: 20 },
        titleCamera: { fontSize: 14, margin: 8, fontWeight: fontWeightBold, color: appcolor.dark },
    ...
    const renderItemCamera = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity key={index} style={styles.itemMainCamera} onPress={onPress}>
                <View style={{ ...styles.opacityView, backgroundColor: appcolor.white, opacity: 0.6 }} />
