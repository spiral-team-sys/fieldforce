...
const CameraPage = ({ callBackData }) => {
  ...
  const normalPhotoInfoRef = useRef(null);
  // Handler
  const handlerSavePhoto = async (uri, time = null, isFaceVerify = false) => {
    ...
    try {
      ...
      const photoinfo = {
        ...
        photoTime: time
          ? moment(time, 'YYYY-MM-DD HH:mm:ss').format('YYYYMMDDHHmmss')
      ...
      if (isFaceVerify) {
        ...
        await ATTENDANT_API.UploadDataAttendance(
          photoinfo,
          () => {
        ...
        let normalPhotoInfo = normalPhotoInfoRef.current;
        const isNewPhoto =
          !normalPhotoInfo || normalPhotoInfo.photoPath !== uri;

        if (isNewPhoto) {
          await InsertPhotosItem(photoinfo);
          normalPhotoInfoRef.current = photoinfo;
          normalPhotoInfo = photoinfo;
        }

        const result = await ATTENDANT_API.UploadAttendance(
          normalPhotoInfo,
          'TAKE',
        );
        if (result) {
          DeviceEventEmitter.emit('RELOAD_ATTENDANCE', normalPhotoInfo);
          callBackData(cameraInfo.photoType);
        }
      }
    } catch (e) {
  ...
  const handlerReject = () => {
    normalPhotoInfoRef.current = null;
    setPhotoUri(null);
  };
  // View
  if (photoUri) {
