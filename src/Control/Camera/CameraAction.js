...
import { Camera, useCameraDevices, useCameraFormat, useFrameProcessor, VisionCameraProxy, runAtTargetFps } from 'react-native-vision-camera';
...
const CameraAction = forwardRef((props, _ref) => {
    ...
    const { appcolor, shopinfo, userinfo } = useSelector(state => state.GAppState)
    ...
    const [inlineMessage, setInlineMessage] = useState({ text: '', type: 'info' });
    ...
    const showMessage = useCallback((text, type = 'info') => {
        ...
        inlineMsgTimerRef.current = setTimeout(() => setInlineMessage({ text: '', type: 'info' }), 3500);
    }, []);
    const updateFaceData = useMemo(() => Worklets.createRunOnJS((count, avgEyeOpen) => {
        ...
        const canRunBlink = isFaceDetectEnabled || (isFaceVerifyEnabled && faceVerifyStatusRef.current === 'verified');
        ...
        runAtTargetFps(5, () => {
            ...
            try {
                ...
                const avgEyeOpen = first
                    ? ((first.leftEyeOpenProbability ?? -1) + (first.rightEyeOpenProbability ?? -1)) / 2
                    : -1;
    ...
    const handlerCapture = async () => {
        if (isFaceDetectEnabled && !isFaceVerifyEnabled && (faceCount !== 1 || !livenessVerified)) {
            showMessage(faceCount === 0
        ...
        if (isFaceVerifyEnabled) {
            if (faceCount !== 1) {
                showMessage('Không phát hiện khuôn mặt. Vui lòng đưa khuôn mặt vào khung hình.');
                return;
            }
            if (faceVerifyStatus !== 'verified') {
                showMessage('Bước 1/3: Vui lòng chờ hệ thống xác minh danh tính hoàn tất.');
                return;
            }
            if (!livenessVerified) {
        ...
        if (isFaceDetectEnabled) {
            ...
            if (!faceStable) {
                ...
                alertWarning('Vui lòng giữ nguyên khuôn mặt trong khung hình trong khi chụp ảnh. Hệ thống cần thời gian để xác nhận khuôn mặt ổn định trước khi chụp. Vui lòng thử lại.');
                return;
            }
        }
        //
    ...
    const takePhoto = async () => {
        try {
            ...
            const imageResult = await resizeImage(photo.path, photo, cameraPosition == frontCameraId);
            if (imageResult) {
            ...
            toastInfo('Thông báo', `Không thể chụp ảnh. Vui lòng thử lại sau (${error})`);
        } finally {
            setWaitingPhoto(false);
        }
    };
    ...
    useEffect(() => {
        ...
        const isVerifySuccess = isFaceVerifyEnabled && faceVerifyStatus === 'verified' && livenessVerified;
        const isDetectSuccess = isFaceDetectEnabled && !isFaceVerifyEnabled && faceCount === 1 && livenessVerified;
        ...
        if (!livenessVerified || (isFaceVerifyEnabled && faceVerifyStatus !== 'verified')) {
            autoCaptureFiredRef.current = false;
            verifyTimeRef.current = null;
        }
    }, [faceVerifyStatus, livenessVerified, faceCount, waitingPhoto, isCheckingDistance]);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            ...
            if (faceVerifyStatusRef.current === 'verifying' && verifyingStartRef.current != null && Date.now() - verifyingStartRef.current > 15000) {
                faceVerifyStatusRef.current = 'idle';
                verifyingStartRef.current = null;
                setFaceVerifyStatus('idle');
            }
            ...
            try {
                ...
                const result = await AI.compareFaces(imageResult, userinfo?.employeeId, info);
                ...
                if (result?.success) {
                    ...
                    showMessage(`${result?.message || 'Khuôn mặt không khớp, đang thử lại...'}`, 'error');
                    blinkPhaseRef.current = 'idle';
                    setLivenessVerified(false);
                    setTimeout(() => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            LOCATION_INFO.getCurrentLocation(async (locationinfo) => {
                ...
                //
                await VALID_LOCATION.checkDistance(shopinfo, locationinfo, 6, handlerMessage, handlerGoback);
                if (!isCancelled) {
    ...
    const styles = StyleSheet.create({
        ...
        loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: appcolor.light },
        waitingView: { position: 'absolute', top: 0, bottom: 0, start: 0, end: 0, alignItems: 'center', justifyContent: 'center' },
        verifyingOverlay: { position: 'absolute', top: 0, bottom: 0, start: 0, end: 0, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.55)' },
        ...
        actionCapture: { borderWidth: 3, borderRadius: 54, borderColor: appcolor.white, alignSelf: 'center' },
        actionFlash: { padding: 8, borderRadius: 32 },
        actionChangeCamera: { padding: 8, borderRadius: 32 },
        opacityView: { backgroundColor: appcolor.dark, opacity: 0.4, position: 'absolute', top: 0, bottom: 0, start: 0, end: 0, justifyContent: 'center', alignItems: 'center', borderRadius: 50 },
        ...
        messageContainer: { position: 'absolute', alignItems: 'center', justifyContent: 'center', bottom: 150, width: deviceWidth / 1.2 },
        textMessage: { color: appcolor.white, fontSize: 16, },
        faceHint: { position: 'absolute', top: FACE_OVAL.bottom + 14, alignSelf: 'center', backgroundColor: 'rgba(0,0,0,0.60)', paddingHorizontal: 18, paddingVertical: 8, borderRadius: 20 },
        faceHintText: { color: appcolor.white, fontSize: 14, textAlign: 'center', fontWeight: '500' },
        captureDisabled: { opacity: 0.4 },
        inlineMsg: { position: 'absolute', alignSelf: 'center', bottom: 130, maxWidth: deviceWidth * 0.85, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, alignItems: 'center' },
    ...
    if (device == null)
        return (
            <View style={styles.loadingContainer}>
                <LoadingDefault isLoading={true} color={appcolor.primary} title='Đang khởi động máy ảnh' />
            </View>
        )
        ...
        <View style={styles.cameraContainer}>
            {(isFocused || waitingPhoto) && (
                <Camera
                    ...
                    frameProcessor={(isFaceDetectEnabled || isFaceVerifyEnabled) && !isCheckingDistance ? frameProcessor : undefined}
                />
            )}
            ...
            {(isFaceDetectEnabled || isFaceVerifyEnabled) && message == '' && !isCheckingDistance && (
                ...
                <View style={styles.actionButtonItem}>
                    {cameraConfig?.isFlipCamera &&
                        <TouchableOpacity style={styles.actionChangeCamera} disabled={waitingPhoto} onPress={toggleCamera}>
                            ...
                            <Icon type='ionicon' name='sync' color={appcolor.white} size={32} />
                        </TouchableOpacity>
                    }
                </View>

                {waitingPhoto && <ActivityIndicator size='small' color={appcolor.white} style={styles.waitingView} />}
                {message == '' &&
                    <View style={styles.actionButtonItem}>
                        {(cameraConfig?.timeAutoTake || 0) > 0 ?
                            <CountTimeAutoTake
                                ...
                                disabled={waitingPhoto || isCheckingDistance || (isFaceDetectEnabled && !isFaceVerifyEnabled && (faceCount !== 1 || !livenessVerified)) || (isFaceVerifyEnabled && (faceVerifyStatus !== 'verified' || !livenessVerified))}
                            />
                            :
                            !isFaceDetectEnabled &&
                            <TouchableOpacity
                                style={[styles.actionCapture, (isCheckingDistance || (isFaceDetectEnabled && !isFaceVerifyEnabled && (faceCount !== 1 || !livenessVerified)) || (isFaceVerifyEnabled && (faceVerifyStatus !== 'verified' || !livenessVerified))) && styles.captureDisabled]}
                                disabled={waitingPhoto || isCheckingDistance || (isFaceDetectEnabled && !isFaceVerifyEnabled && (faceCount !== 1 || !livenessVerified)) || (isFaceVerifyEnabled && (faceVerifyStatus !== 'verified' || !livenessVerified))}
                                onPress={handlerCapture}
                            >
                                <View style={styles.opacityView} />
                                <Icon type='ionicon' name='ellipse' color={appcolor.white} size={54} />
                                {waitingPhoto && <ActivityIndicator size='small' color={appcolor.greydark} style={styles.waitingView} />}
                            </TouchableOpacity>
                        }
                    </View>
                }
                <View style={styles.actionButtonItem}>
                    {cameraPosition === backCameraId && device?.hasTorch &&
                        <TouchableOpacity style={styles.actionFlash} disabled={waitingPhoto} onPress={toggleFlash}>
                            <View style={styles.opacityView} />
                            <Icon type='ionicon' name={flash === 'on' ? 'flash' : 'flash-off'} color={appcolor.white} size={32} />
                        </TouchableOpacity>
                    }
                </View>
                {(isCheckingDistance || message !== '') && (
                    <View style={styles.messageContainer}>
                        <LoadingDefault isLoading={true} color={appcolor.white} title={message !== '' ? message : 'Đang xác định vị trí...'} colorTitle={appcolor.white} />
                    </View>
                )}
            </View>
            {inlineMessage.text !== '' && (
                <View style={[styles.inlineMsg, inlineMessage.type === 'error' ? styles.inlineMsgError : styles.inlineMsgInfo]}>
