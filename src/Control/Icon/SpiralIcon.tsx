...
function mapName(type: IconType, name: string) {
  if (type === 'font-awesome-5' || type === 'font-awesome-6') {
...
function getIconStyle(type: IconType, name: string) {
  ...
  if (type === 'font-awesome-5' || type === 'font-awesome-6') {
    if (reg.solid?.[name]) return 'solid';

    if (reg.regular?.[name]) return 'regular';

    if (reg.brand?.[name]) return 'brand';

    if (reg.brands?.[name]) return 'brand';
...
export default function SpiralIcon({
  ...
  const Component = ICON_COMPONENT[iconType] ?? MaterialIcons;

  const finalName = mapName(iconType, name);

  const style = getIconStyle(iconType, finalName);

  if (!style) {
    console.warn(`[SpiralIcon] ${iconType}:${finalName} not found`);

    return <MaterialIcons name="help-outline" size={size} color={color} />;
  }

  if (iconType === 'font-awesome-5' || iconType === 'font-awesome-6') {
    return (
      <Component
        {...props}
  ...
  return (
    <Component {...props} name={finalName as any} size={size} color={color} />
  );
}
