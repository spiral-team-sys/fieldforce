...
const MultipleShowVideo = ({ }) => {
    ...
    const styles = StyleSheet.create({
        ...
        contentSheet: { width: '100%', height: '100%', backgroundColor: appcolor.black },
        itemContainer: { width: deviceWidth, height: deviceHeight, justifyContent: 'center', alignItems: 'center' },
        contentHeader: { width: '100%', alignContent: 'center', zIndex: 10, position: 'absolute', top: 0 },
        titleCount: { color: appcolor.light, fontSize: 14, fontWeight: '700', textAlign: 'center', position: 'absolute', top: 8, alignSelf: 'center', margin: 16 },
        titleHeaderClose: { color: appcolor.light, fontSize: 13, fontWeight: '700', textAlign: 'center' },
        viewHeader: { width: 80, borderWidth: 0.5, borderColor: appcolor.light, backgroundColor: appcolor.dark, padding: 8, borderRadius: 5, margin: 8, position: 'absolute', end: 8, top: 8 },
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View style={styles.itemContainer}>
                ...
                <TouchableOpacity style={styles.buttonVideoAction} onPress={onPlayVideo}>
                    <Icon type="ionicon" name={itemVideo.isView ? 'stop' : 'play'} size={42} color={appcolor.white} />
                </TouchableOpacity>
            </View>
        );
    };
    ...
    return (
        <ActionSheet id="media-video-show" onBeforeShow={setDataVideo} containerStyle={styles.contentSheet}>
            <SafeAreaView style={styles.mainContainer}>
                <View style={styles.contentHeader}>
                    {dataVideo.length > 0 && (
                        <Text style={styles.titleCount}>{`${currentIndex + 1} / ${dataVideo.length}`}</Text>
                    )}
