...
const TimeChoose = ({ visible = true, dateValue, title, stepTime, keyValue, onChooseTime }) => {
    ...
    const styles = StyleSheet.create({
        ...
        titleView: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.primary, padding: 8, textAlign: 'center' },
    ...
    return (
        <View style={styles.containerStyle}>
            <TouchableOpacity onPress={handlerChange} style={styles.mainContainer} >
                <Text style={styles.titleView}>{title} {timeValue ? moment(timeValue).format('HH:mm') : ''}</Text>
            </TouchableOpacity>
            {isOpen &&
                <DatePicker
                    mode='time'
