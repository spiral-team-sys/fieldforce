...
export const MutipleItemSelected = ({
    titleName, iconName, defaultValue, defaultValueDistrict, defaultValueMulti = [], dataItems, containerStyle, mulipleChoose = false,
    isHorizontal = true, onItemChoose, isRequire = false, typeItem, isFilter = false, placeholder = null, paddingBottom = 0, signleAnimation = false,
    isUploaded = false, isUseEatDay = false, isViewMulti = false, maxMultiSelect = 0, isSelectDistrict = 0, itemMain = null, filterStyle, focusSelected = false
}) => {
    ...
    const styles = StyleSheet.create({
        ...
        itemContent: { flexGrow: 1, backgroundColor: appcolor.light, borderRadius: 5, padding: 8, margin: 5, borderWidth: 0.5, borderColor: appcolor.grayLight },
        selectedItemContent: { borderWidth: 1.2, backgroundColor: appcolor.surface, shadowColor: appcolor.dark, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.14, shadowRadius: 4, elevation: 2 },
    ...
    const normalizeText = (t) => removeVietnameseTones((t || '').toString()).toLowerCase().trim()
    const getSearchText = (item = {}) => {
        const keys = [
            'itemName', 'name', 'Name', 'NameVN', 'label', 'title',
            'itemCode', 'code', 'Code', 'value', 'Value', 'itemValue',
            'id', 'Id', 'dealerName', 'dealerCode', 'shopName', 'shopCode',
            'provinceName', 'districtName', 'townName', 'district',
            'description', 'desc', 'note'
        ]
        return normalizeText(keys.map(key => item?.[key]).filter(Boolean).join(' '))
    }
    ...
    const getSelectedIndex = (list = [], selectedValue) => {
        ...
        return (list || []).findIndex(item => normalizeText(item.itemName || item.name) === selected)
    }
    ...
    const setDataItem = async () => {
        ...
        if (defaultValueDistrict && defaultValue) {
            const listDistrict = _.filter(dataItems, (i) =>
                (i.itemName || i.name) == defaultValue
            )
        ...
        if (focusSelected && defaultValue && didFocusSelectedRef.current !== defaultValue) {
    ...
    const singleSelected = (item, _index) => {
        ...
        if (isViewMulti == true && typeof defaultValue == 'object') {
            ...
            if (dataSelectVehcle?.some(it => (it.code || it.level1_id) == (item.code || item.level1_id))) {
                dataSelectVehcle = dataSelectVehcle.filter(it => (it.code || it.level1_id) != (item.code || item.level1_id))
            } else {
                dataSelectVehcle.push(item)
            }
            defaultValue = (dataSelectVehcle?.length || 0) > maxMultiSelect ? [item] : dataSelectVehcle
        } else {
        ...
        signleAnimation && scrollRef.current?.scrollTo({ x: 0, y: 0, animated: true })
        setMutate(e => !e)
    ...
    const clearFilterItem = () => {
        ...
        defaultValue ? scrollToSelectedIndex(dataItems, defaultValue) : scrollRef.current?.scrollTo({ x: 0, y: 0, animated: false })
    }
    ...
    const renderItem = (item, index) => {
        const onPress = () => {
            mulipleChoose ? mutipleSelected(item, 'PROVINCE') : singleSelected(item, index)
        }
        const colorSelected = item.isColor || appcolor.primary
        const isSelected = (isViewMulti == true && typeof defaultValue == 'object')
            ? defaultValue.length > 0 && defaultValue?.findIndex(it => it.code == item.code) !== -1
            : ((item.itemName || item.name) == defaultValue) || (mulipleChoose && (itemSelect.itemName || itemSelect.name) == (item.itemName || item.name))
        const styleView = isSelected ? [styles.itemContent, styles.selectedItemContent, { borderColor: colorSelected }] : styles.itemContent

        const styleTitle = isSelected ? [styles.itemName, styles.selectedItemName, { color: colorSelected }] : styles.itemName
    ...
    const renderItemDistrict = (item, index) => {
        const onPress = () => {
            mulipleChoose ? mutipleSelected(item, 'DISTRICT') : handlerSelectDistrict(item, index)
        }
        const colorSelected = item.isColor || appcolor.primary
        const styleView = ((item.itemName || item.name) == defaultValueDistrict ? { ...styles.itemContent, borderWidth: 1, borderColor: colorSelected } : styles.itemContent)
        const styleTitle = ((item.itemName || item.name) == defaultValueDistrict ? { ...styles.itemName, fontWeight: '700', color: colorSelected } : styles.itemName)
        return (
            <TouchableOpacity key={`${typeItem}_District_${index}`} style={styleView} onPress={onPress}>
                <Text style={styleTitle}>{item.itemName || item.name}</Text>
    ...
    const renderItemValue = (item, index) => {
        ...
        if (!item.districtCode && isSelectDistrict == 1)
            return

        return (
            <View key={`${typeItem}__${index}`} style={{ width: '90%', flexDirection: 'row', alignItems: 'center', alignSelf: 'center', justifyContent: 'center', padding: 5 }}>
                <TouchableOpacity onPress={onDelete}>
                    <Icon
                        type="font-awesome-5"
                        style={{ textAlign: 'center', marginEnd: 8 }}
                        name={"minus-circle"} size={20}
                        color={appcolor.red}
                    />
                </TouchableOpacity>
                <View style={{ width: isUseEatDay == true ? '50%' : '70%' }}>
                    <Text style={{ width: '100%', color: appcolor.info, textAlign: 'left', fontWeight: '700', }}>{`${index + 1}. ${item.itemName || item.name}`}</Text>
                    {
                        isSelectDistrict == 1 && item.district && <Text style={{ width: '100%', color: appcolor.info, textAlign: 'left', fontWeight: '700', }}>{`  ${item.district}`}</Text>
                    }
                </View>
                <FormGroup
                    selectTextOnFocus={true}
                    keyboardType="numeric"
                    containerStyle={{ width: isUseEatDay == true ? '25%' : '30%', borderRadius: 8, marginBottom: 0, padding: 0, marginStart: 8, backgroundColor: 'transparent' }}
                ...
                />
                {
                    ...
                    <FormGroup
                        selectTextOnFocus={true}
                        keyboardType="numeric"
                        containerStyle={{ width: '25%', borderRadius: 8, marginBottom: 0, padding: 0, marginStart: 8, backgroundColor: 'transparent' }}
                        inputStyle={{ textAlign: 'center', fontSize: 12, backgroundColor: 'transparent' }}
                        editable
    ...
    const dataDistrictView = isDistrictSearching ? dataDistrictFilter : dataDistrict

    return (
        <View style={[styles.mainContainer, containerStyle]}>
            <View style={{ width: '100%', flexDirection: 'row', marginBottom: isFilter ? 5 : 0 }}>
            ...
            {(placeholder !== null && placeholder.length > 0) && <Text style={styles.placeholderHeader}>{`${placeholder} `}</Text>}
            {isFilter &&
                <FormGroup
                    editable
            ...
            <ScrollView
                ...
                ref={scrollRef}
                contentContainerStyle={{ flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', paddingBottom: paddingBottom }}
            ...
            {
                (defaultValue || defaultValueMulti) && (dataDistrictView?.length > 0 || isDistrictSearching) && isSelectDistrict == 1 &&
