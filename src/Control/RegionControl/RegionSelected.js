...
export const RegionSelected = ({
  ...
  const setDataItem = async () => {
    ...
    setTimeout(() => {
      const indexView = _.findIndex(dataItems, i => {
        return i.itemName == defaultValue;
      });
  ...
  const filterItem = text => {
    const dataFilter = dataItems.filter(
      i =>
        i.itemName.toLowerCase().match(text.toLowerCase()) ||
        i.itemNameEn.toLowerCase().match(text.toLowerCase()),
    );
  ...
  const renderItem = ({ item, index }) => {
    ...
    const styleView =
      item.itemName == defaultValue
        ? {
            ...styles.itemContent,
            borderWidth: 1,
            borderColor: itemColor || appcolor.primary,
          }
        : styles.itemContent;
    const styleTitle =
      item.itemName == defaultValue
