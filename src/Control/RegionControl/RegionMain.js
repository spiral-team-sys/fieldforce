...
export const RegionMain = ({
  ...
  const LoadData = async () => {
    ...
    await GetDataAllRegion(async mData => {
      if (regionId) {
        ...
        const dataDistrict = mData.table2?.filter(
          it => it.districtCode !== null,
        );
        const dataTown = mData.table3?.filter(it => it.townCode !== null);
        await setData({
          region: mData.table,
          province: mData.table1,
          district: dataDistrict,
          town: dataTown,
          regionAll: mData.table4,
        });
        await setDataMain({
          region: mData.table,
          province: mData.table1,
          district: mData.table2,
          town: mData.table3,
          regionAll: mData.table4,
        });
      }
  ...
  const loadDataByRegionId = async (mData, regionIdSelect) => {
    const itemByRegionId =
      mData?.table4?.find(it => it.regionID == regionIdSelect) || {};
    const itemByProvinceCode =
      mData?.table1?.find(
        it => it.provinceCode == itemByRegionId.provinceCode,
      ) || {};
    const itemByDistrictCode =
      mData?.table2?.find(
        it =>
          it.districtCode == itemByRegionId.districtCode &&
          itemByRegionId.districtCode !== null,
      ) || {};
    const itemByTownCode =
      mData?.table3?.find(
        it =>
          it.townCode == itemByRegionId.townCode &&
          itemByRegionId.townCode !== null,
      ) || {};
    const itemByRegionArea =
      mData.table.find(it => it.area == itemByProvinceCode.area) || {};
  ...
  const handlerItemChoose = (item, type) => {
    ...
    switch (type) {
      case TYPE.REGION:
        ...
        //
        const lstProvince = dataMain.province.filter(
          i => i.area == item.itemName && i.provinceCode !== null,
        );
      ...
      case TYPE.PROVINCE:
        ...
        //
        const lstDistrict = dataMain.district.filter(
          i => i.provinceCode == item.provinceCode && i.districtCode !== null,
        );
      ...
      case TYPE.DISTRICT:
        ...
        if (!itemRegion.regionName || !itemRegion.provinceCode) {
          itemProvince = data.province.find(
            it => it.provinceCode == item.provinceCode,
          );
        }
        itemChange = {
          ...itemRegion,
          regionName: itemRegion.regionName
            ? itemRegion.regionName
            : itemProvince.area,
          districtName: item.districtName,
          districtCode: item.districtCode,
          provinceName: itemRegion.provinceCode
            ? itemRegion.provinceName
            : itemProvince.provinceName,
          provinceCode: itemRegion.provinceCode
            ? itemRegion.provinceCode
            : itemProvince.provinceCode,
          townCode: null,
          townName: null,
        };
        //
    ...
    if (
      (itemChange.districtCode == null ||
        itemChange.provinceCode == null ||
        itemChange.regionName == null) &&
      type == TYPE.TOWN
    ) {
      const dataClone = {
        table: dataMain.region,
        table1: dataMain.province,
        table2: dataMain.district,
        table3: dataMain.town,
        table4: dataMain.regionAll,
      };
  ...
  const loadRegionId = item => {
    ...
    if (item.townCode != null && item.townCode > 0) {
      itemRegionByCode = dataMain.regionAll.find(
        it =>
          it.townCode == item.townCode &&
          it.districtCode == item.districtCode &&
          it.provinceCode == item.provinceCode,
      );
