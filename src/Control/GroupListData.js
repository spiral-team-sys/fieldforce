...
export const GroupListData = ({ dataMain, keyCountData, keyValue, keyName, handlerChange, handlerCloseTag, isMultiple = false }) => {
    ...
    const styles = StyleSheet.create({
        mainContainer: { width: '100%', paddingVertical: 8 },
    ...
    const renderItem = ({ item, index }) => {
        ...
        const sumByTag = keyCountData ? item[keyCountData] : _.filter(dataMain, e => e[keyValue] == item[keyValue]).length
        ...
        return (
            <TouchableOpacity key={`igi_${index}`} style={styleDefault} onPress={pressItem}>
                ...
                <Text style={[styles.titleView, { color: colorText }]}>{item[keyName] || 'Tất cả'}</Text>
            </TouchableOpacity>
        )
    }
    return (
