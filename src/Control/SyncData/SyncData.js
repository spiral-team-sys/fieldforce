...
const SyncData = forwardRef((props, ref) => {
    ...
    const handlerDownloadData = async () => {
        ...
        try {
            ...
            if (!isUpdateNewApp) {
                try {
                    const isDownloaded = await DOWNLOADAPI.downloadAll((message) => toastInfo('Thông báo', message))
                    if (!isDownloaded) {
    ...
    //
    useEffect(() => {
        const reload_data = DeviceEventEmitter.addListener('REDOWNLOAD_DATA', handlerDownloadData)
        let isMounted = true
        if (!isMounted)
            return
        handlerDownloadData()
    ...
    const styles = StyleSheet.create({
        mainContainer: { width: '100%', height: '100%', position: 'absolute', zIndex: 1, backgroundColor: appcolor.primary, start: 0 },
        contentMain: { flex: 1, flexDirection: 'row', padding: 8, alignItems: 'center', justifyContent: 'center' },
        titleView: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.light, marginStart: 8 }
    })
