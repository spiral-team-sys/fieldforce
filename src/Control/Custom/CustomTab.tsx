...
import type { CollapsibleRef, OnTabChangeCallback } from "react-native-collapsible-tab-view";
...
const CustomTab = ({
    ...
    const tabCountByName = useMemo(() => {
        if (!Array.isArray(dataMain) || dataMain.length === 0) return {} as Record<string, number>
    ...
    const styles = useMemo(() => StyleSheet.create({
        ...
        contentDataMain: { width: deviceWidth, height: '100%', paddingTop: 45, zIndex: 1 },
        labelStyle: { fontSize: 12, fontWeight: fontWeightBold },
        indicatorStyle: { backgroundColor: appcolor.primary },
        tabStyle: { backgroundColor: appcolor.light, minWidth: minWidthTab(dataTab), height: 42 }
    }), [appcolor, dataTab])
    ...
    const renderItemTab = () => {
        return dataTab.map((item, index) => {
            ...
            const titleTab = dataMain.length > 0 ? `${tabName} (${itemCount})` : tabName
            return (
    ...
    if (!hasData)
        return <View style={{ justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ color: appcolor.dark, fontSize: 13 }}>Không có dữ liệu</Text>
        </View>
