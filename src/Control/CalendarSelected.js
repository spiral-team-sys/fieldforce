...
export const CalendarSelected = ({
  ...
  const calendarConfig = useMemo(() => {
    const base = fromDate || defaultDate || moment().format('YYYY-MM-DD');
        ...
        minDate || maxDate
          ? candidates
              .filter(date => {
                const isAfterMin =
                  !minDate ||
                  date.isSameOrAfter(moment(minDate).startOf('month'), 'day');
                const isBeforeMax =
                  !maxDate ||
                  date.isSameOrBefore(moment(maxDate).endOf('month'), 'day');
                return isAfterMin && isBeforeMax;
              })
              .sort(
                (a, b) =>
                  Math.abs(a.diff(baseMoment, 'months')) -
                  Math.abs(b.diff(baseMoment, 'months')),
              )[0]
          : null;
  ...
  const handlerBetweenDay = ({ dateString }) => {
    ...
    if (
      lockOutMonth &&
      moment(dataCalendar.startDate).format('MM') !==
