...
export const MapApp = ({ navigation, route, slist }) => {
	...
	useEffect(() => {
		...
		const checkSelectedItem = () => {
			const isSelectedItem = itemSelect && provinceShopMain?.slice(0, 5).findIndex((item) => item.province === itemSelect.province) === -1;
			if (isMounted) {
	...
	const AddMarker = () => {
		...
		if (shoplist !== null) {
			shoplist?.forEach((mk) => {
				if (mk.latitude !== undefined && mk.longitude !== undefined && mk.latitude !== null && mk.longitude !== null && isDecimal(mk.latitude) && isDecimal(mk.longitude)) {
					mkList.push(<Marker key={mk.shopCode} onPress={() => onClickMarker(mk)} coordinate={{ latitude: mk.latitude, longitude: mk.longitude }} pinColor={appcolor[mk.colorMarker] || 'tomato'} title={mk.shopName} description={mk.address}></Marker>);
				}
			});
		}
	...
	const onSearchData = (text) => {
		...
		const listUpdate = _.filter(provinceShopSearch, (e) => {
			const normalizedProvince = removeVietnameseTones(e.province).toLowerCase();
	...
	const handlerOpenDetailsDistrict = async (item, key) => {
		...
		const listUpdate = _.map(districtShop, (e) => {
			return item.district == e.district ? { ...e, isOpen: _isOpen } : { ...e, isOpen: false };
		});
	...
	const handlerSelectTag = (item, index, key) => {
		try {
			refRegionList?.current?.scrollToIndex({ index: index > 4 && key === 'provinceTag' ? 4 : index, animated: true });
		} catch (e) {
		...
		selectedStore(item, key);
		//
		const _listUpdate = _.map(search.text.length > 0 ? provinceShop : provinceShopMain, (e) => {
			return { ...e, isOpen: item.province == e.province ? !(item.isOpen || false) : false };
		});
	...
	const styles = StyleSheet.create({
		...
		buttonProvince: { padding: 8, marginVertical: 4, borderRadius: 20, backgroundColor: appcolor.light, marginHorizontal: 5 },
		textProvinces: { color: appcolor.dark, fontWeight: '500' },
		moveCurrentLocation: { height: 50, width: 50, backgroundColor: appcolor.white, borderRadius: 40, position: 'absolute', zIndex: 5, bottom: 20, right: Platform.OS === 'android' ? deviceWidth - 60 : 20, justifyContent: 'center' },
		onSearchShop: { shadowOpacity: 0.7, shadowColor: appcolor.dark, shadowRadius: 10, borderColor: appcolor.dark, borderWidth: 0.1, top: 20, left: 20, right: 20, borderRadius: 30, opacity: 0.8, position: 'absolute', color: appcolor.dark, backgroundColor: appcolor.white },
		iconMarker: { left: 10, position: 'absolute' },
		viewProvinceTag: { position: 'absolute', top: 70, height: '10%', left: 8, right: 8 },
		viewButtonClose: { alignSelf: 'center', position: 'absolute', bottom: Platform.OS == 'android' ? 16 : deviceHeight / 12 },
		addressText: { color: appcolor.greylight, fontSize: scaleSize(12), fontWeight: '500' },
		shopNameText: { color: appcolor.dark, fontSize: scaleSize(14), fontWeight: '700' },
		viewProvinceShop: { height: deviceHeight / 2, width: '100%' },
		titleFilter: { padding: 7, textAlign: 'center', fontSize: 16, fontWeight: '700', color: appcolor.dark },
	...
	const DistrictShop = ({ item }) => {
		...
		return (
			<View>
				<TouchableOpacity style={[styles.itemViewMain, { marginLeft: 20, backgroundColor: item.isOpen ? appcolor.primary : appcolor.light }]} onPress={onPress}>
					<View>
						<Text style={[styles.contentTitle, { color: item.isOpen ? appcolor.light : appcolor.dark }]}>{item.district}</Text>
					</View>
				</TouchableOpacity>
			</View>
		);
		...
		return (
			<View key={`it_group_${index}`} style={styles.mainView}>
				<TouchableOpacity style={styles.itemViewMain} onPress={onPress}>
					{item.isOpen && <Icon type="ionicon" name="checkmark-circle" size={18} color={appcolor.primary} style={{ marginStart: 8 }} />}
					<View>
						<Text style={styles.contentTitle}>{item.province}</Text>
					</View>
				</TouchableOpacity>
				{item.isOpen ? districtShop.map((item, index) => <DistrictShop key={index} item={item} />) : <View />}
			</View>
		);
	};
	const renderItemTag = ({ item, index }) => {
		...
		return (
			<View key={`ma-${index}`}>
				<TouchableOpacity onPress={onPress} style={[styles.buttonProvince, { backgroundColor: item.isOpen ? appcolor.primary : appcolor.light }]}>
					<Text style={[styles.textProvinces, { color: item.isOpen ? appcolor.light : appcolor.dark }]}>{item.province}</Text>
				</TouchableOpacity>
			</View>
		);
	};
	...
	return (
		<View style={styles.viewContainer}>
			...
			<TouchableOpacity onPress={moveCurrentLocation} style={styles.moveCurrentLocation}>
				<Icon color={appcolor.info} name="location-arrow" type="font-awesome" size={24} />
			</TouchableOpacity>
			<Button title="Tìm cửa hàng..." onPress={onSearch} icon={<Icon name="map-marker" containerStyle={styles.iconMarker} color={appcolor.info} type="font-awesome" size={24} />} type="raise" color={appcolor.dark} containerStyle={styles.onSearchShop} />
			<View style={styles.viewProvinceTag}>
				<CustomListView
					...
					endView={<TouchableOpacity onPress={handleShowRegion} style={[styles.buttonProvince, { backgroundColor: isSelectedItem ? appcolor.primary : appcolor.light }]}>
						<Text style={[styles.textProvinces, { color: isSelectedItem ? appcolor.light : appcolor.dark }]}>{'Thêm...'}</Text>
					</TouchableOpacity>}
				/>
			</View>
			<ActionSheet
			...
			>
				<ScrollView style={styles.scrollView}>
					<View style={styles.viewContainerText}>
						<Text style={styles.shopNameText}>Cửa hàng: {shopinfo?.shopName || ''}</Text>
						<Text style={styles.addressText}>Địa chỉ: {shopinfo?.address || ''}</Text>
					</View>
					<ShopProfile navigation={navigation} />
				</ScrollView>
			</ActionSheet>

			<ActionSheet id="viewSheetRegion" nestedScrollEnabled>
				...
				<View style={styles.viewButtonClose}>
					<ButtonAction iconName="close" iconSize={24} iconColor={appcolor.light} backgroundColor={appcolor.blacklight} onPress={handleHideRegion} />
				</View>
			</ActionSheet>
