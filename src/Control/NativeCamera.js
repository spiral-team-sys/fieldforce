...
import { InsertPhotosItem, UpdatePhotosDataStatus, UpdatePhotosFileStatus, UpdateUrlPhotosShop, uploadAllDataPhoto, uploadPhotoData } from '../Controller/PhotoController';
...
const cameraStart = async (iphotoinfo, fResult, customOption) => {
    ...
    if (allow) {
        ...
        const options = (customOption !== undefined && customOption !== null && Object.keys(customOption || null)?.length > 0) ? customOption : { ...imageOptions, quality: 0.8 }
        return ImagePicker.launchCamera(options, (res) => actionAfterTakeImage('CAMERA', res, iphotoinfo, fResult));
    }
}
const videoStart = async (ivideoinfo, fResult, customOption) => {
    try {
        ...
        if (allow) {
            ...
            const options = (customOption && Object.keys(customOption).length > 0) ? customOption : videoOptions;
            return ImagePicker.launchCamera(options, (res) => actionAfterTakeVideo('VIDEO', res, ivideoinfo, fResult));
        }
    } catch (e) {
...
const imageGalleryLaunch = async (iphotoinfo, payload, customOption) => {
    try {
        ...
        //
        imageOptions.saveToPhotos = settings.exportPhotoReport || false
        const options = (customOption !== undefined && customOption !== null && Object.keys(customOption)?.length > 0) ? customOption : imageOptions
...
const videoGalleryLaunch = async (iphotoinfo, payload, customOption) => {
    try {
        ...
        //
        videoOptions.saveToPhotos = settings.exportPhotoReport || false
        const options = (customOption !== undefined && customOption !== null && Object.keys(customOption)?.length > 0) ? customOption : videoOptions
        ImagePicker.launchImageLibrary(options, (res) =>
            res.assets ? actionAfterTakeVideo('GALLERY', res, iphotoinfo, payload) : actionAfterTakeVideo('GALLERY', null, iphotoinfo, payload)
        )
    } catch (e) { console.log(e) }
...
const resizeImage = async (uri) => {
    try {
        let maxWidth = 1280, maxHeight = 1280, quality = 80;

        let infoImageConvert;
        const path = `${RNFS.DocumentDirectoryPath ? RNFS.DocumentDirectoryPath : RNFS.LibraryDirectoryPath}/Photo/`;
        // Tạo thư mục nếu nó không tồn tại
        const dirExists = await RNFS.exists(path);
        if (!dirExists) {
            await RNFS.mkdir(path);
        }
        return await ImageResizer.createResizedImage(uri, maxWidth, maxHeight, "JPEG", quality, 0, path)
    } catch (error) {
...
const changeImageInfo = async (arrImage) => {
    ...
    for (let i = 0; i < (arrImage.length || 0); i++) {
        ...
        const newItem = { ...item, fileName: newImageUrl.name, photoPath: newImageUrl.uri, uri: newImageUrl.uri }
...
const actionAfterTakeImage = async (type, ressult, iphotoinfo, fResult) => {
    ...
    if (assets !== undefined) {
        ...
        await newArrImage?.forEach(async res => {
            let timePhotoInsert = await new Date().getTime() + (Math.floor(Math.random() * 100) + 1)
            ...
            if (fileInfo.reportId === -1) {
                ...
                if (resUploadData) {
                    await UpdatePhotosDataStatus(fileInfo.shopId, fileInfo.photoDate, fileInfo.photoType);
                    await uploadAllDataPhoto([fileInfo], async () => {
                        await UpdatePhotosFileStatus(fileInfo.shopId, fileInfo.photoDate, fileInfo.photoType);
                        ...
                        if (shopContent.latitude == 0 || shopContent.longitude == 0) {
                            await UpdateUrlPhotosShop(fileInfo.shopId, fileInfo.photoPath, fileInfo.latitude, fileInfo.longitude)
                            shopContent.latitude = fileInfo.latitude
                            shopContent.longitude = fileInfo.longitude
                        } else {
                            await UpdateUrlPhotosShop(fileInfo.shopId, fileInfo.photoPath, shopContent.latitude, shopContent.longitude)
                        }
                    ...
                    ToastError('Gửi hình ảnh photo overview không thành công. Vui lòng bấm nút refresh trên tấm hình để gửi lại.');
                    return;
                }
            } else {
                await InsertPhotosItem(fileInfo);
                await fResult({ statusId: 200, "messager": "success", data: await newArrImage || res, fileInfo: fileInfo })
            }
            // await InsertPhotosItem(fileInfo);
        ...
        const fileInfo = await { ...iphotoinfo, photoPath: await newImageUrl.uri || await ressult.uri }
        if (res.uri === undefined)
            return;
        await InsertPhotosItem(fileInfo);
        await fResult({ statusId: 200, messager: "success", data: await assets || ressult, fileInfo: fileInfo })
    }
}
const actionAfterTakeVideo = async (type, ressult, ivideoinfo, fResult) => {
    ...
    if (assets !== undefined) {
        await assets?.forEach(async res => {
            let timeVideoInsert = await new Date().getTime() + (Math.floor(Math.random() * 100) + 1)
        ...
        await fResult({ statusId: 200, "messager": "success", data: await assets || ressult })
    }
};

const NativeCamera = { cameraStart, videoStart, imageGalleryLaunch, videoGalleryLaunch, actionAfterTakeImage, resizeImage };
