...
export const YearMonthSelected = ({
  option,
  contanerStyle,
  numMonth,
  onYearMonth,
  ...
  useEffect(() => {
    ...
    if (option.year !== undefined) setSelected(option);
    else
      setSelected({
        month: _currentM,
        monthname: `Tháng ${_currentM}`,
        year: _currentY,
        yearname: `Năm ${_currentY}`,
      });
    _year = _year.sort((a, b) => {
      return a > b;
    });
    setYear(_year);
    return () => false;
      ...
      <View style={styles.monthWrap}>
        {listMonth.map((item, i) => {
          return (
            <View
              key={`${i}jkao`}
              style={[styles.monthItem, { flexBasis: `${100 / monthColumn}%` }]}
            >
              <TouchableOpacity
