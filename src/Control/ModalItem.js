...
const ModalItem = ({
  ...
  const renderItemModal = ({ item, index }) => {
    ...
    return (
      <View>
        {item.isParent && sub2Value && (
          <Text style={styles.groupName}>{sub2Value}</Text>
        )}
        <TouchableOpacity
          style={[styles.itemModal, { backgroundColor }]}
          onPress={item.isSelect == 1 ? actionClearItem : chooseItem}
        >
          <View style={styles.headerTitle}>
            <Text style={[styles.titleName, { color: textColor }]}>
              {index + 1}. {value}
            </Text>
            {(item?.numAttendant || null) !== null && (
              <Text
                style={[styles.subTitleName, { color: textColor }]}
              >{`${item.numAttendant}`}</Text>
            )}
          </View>
          {subValue && (
            <Text
              style={[styles.subTitleName, { color: subTextColor }]}
            >{`Code: ${subValue}`}</Text>
          )}
          {address && (
            <Text
  ...
  return (
    ...
    >
      <SafeAreaProvider>
        ...
        >
          <Text
            style={[styles.titleName, { textAlign: 'center' }]}
          >{`Tìm kiếm`}</Text>
          <FilterSheet groupType={groupType} filterDataBy={filterDataBy} />
          <SearchData
