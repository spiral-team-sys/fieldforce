...
export const SellOutSummary = ({ appcolor, dataSellOut, navigation }) => {
  ...
  const data = {
    dataSets: [
      {
        values: [
          { value: dataSellOut?.actualPercent || 0 },
          { value: target || 0 },
        ],
        config: {
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() =>
        dataSellOut?.pageName != null
          ? navigation.navigate(dataSellOut?.pageName)
          : null
      }
    >
      <View style={{ width: '100%', height: '100%', padding: 8 }}>
        ...
        >
          {dataSellOut?.cname || 'Báo cáo số bán'}
        </Text>
