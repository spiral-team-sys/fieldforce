...
export const SearchStore = forwardRef((props, ref) => {
  ...
  const filterOptions = [
    {
      ...
      text: 'Khoảng cách tăng dần',
    },
    {
      key: 'DISTANCE_DOWN',
      icon: 'sort-numeric-down-alt',
      text: 'Khoảng cách giảm dần',
    },
  ...
  const styles = StyleSheet.create({
    ...
    safeAreaContainer: {
      ...
      paddingTop: Platform.OS == 'android' ? 56 : 45,
    },
  ...
  const renderitem = ({ item, index }) => {
    ...
    return (
      ...
      >
        ...
        {item.shopFormat && (
          <Text style={styles.itemText}>{item.shopFormat}</Text>
        )}
        {item.dealerName && (
          <Text style={styles.itemText}>{item.dealerName}</Text>
        )}
        <NumericFormat
          key={item.shopId + '-fe'}
          displayType="text"
  ...
  return (
    <SafeAreaView style={styles.safeAreaContainer}>
      ...
      <CustomListView key="shopId" data={shops} renderItem={renderitem} />
      <ActionSheet
        id={'sheetSearchStore'}
                  ...
                  style={{
                    backgroundColor:
                      textSelect === item.key
                        ? appcolor.light
                        : appcolor.surface,
                    borderWidth: textSelect === item.key ? 0.5 : 0,
