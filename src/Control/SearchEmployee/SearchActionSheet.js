...
export const SearchActionSheet = ({ data, actionResult }) => {
  ...
  const onFilterEmployee = text => {
    ...
    const dataFilter = _.filter(dataEmployeeMain, e => {
      return (
        removeVietnameseTones(e.employeeCode).toLowerCase().match(value) ||
  ...
  const onSelectedEmployee = employeeId => {
    const byEmployee = dataEmployeeMain.map(item =>
    ...
    const itemEmployee = (item, index) => {
      ...
      return (
        ...
        >
          <Text
            style={
              !isSelected ? styles.titleEmployee : styles.titleEmployeeSelected
            }
          >{`${index + 1}. ${item.employeeCode} - ${item.employeeName}`}</Text>
        </TouchableOpacity>
      );
    };
    return (
    ...
    >
      <View style={{ width: '100%', height: deviceHeight }}>
        ...
        {/* <TouchableOpacity onPress={handlerResultByEmployee}>
						<SpiralIcon name="arrow-alt-circle-right" solid type="font-awesome-5" size={28} />
					</TouchableOpacity> */}
        {/* </View> */}
