...
export const ScreenCreate = ({ type, itemMain = {} }) => {
  ...
  const LoadConfig = async () => {
    const params = {
      shopId: shopinfo.shopId || itemMain.shopId,
      reportId: kpiinfo.id,
    };
  ...
  const onSaveIssue = async () => {
    ...
    if (_valid) {
      alertConfirm(
        'Tạo vấn đề',
        `Bạn có muốn ${
          type == 'PLUS' ? 'Tạo' : 'Cập nhật'
        } dữ liệu đã khai báo như bên dưới không ?`,
        async () => {
          const result = await REPORT.UploadDataRaw_Realtime(
            { ...itemIssue, typeAction: type },
            shopinfo,
            kpiinfo.id,
          );
  ...
  const onCancelIssue = async () => {
    if (type == 'PLUS' && itemIssue.imageIssueLength > 0) {
      alertConfirm(
        'Chú ý',
        'Dữ liệu khởi tạo vấn đề của bạn hiện tại sẽ bị xóa, Bạn có muốn đóng không ?',
        () => {
  ...
  const validData = async () => {
    ...
    // Create Issue
    if (type == 'PLUS') {
      if (itemIssue.groupIssueId == 0) strError += `- Chưa chọn loại vấn đề\n`;
    ...
    // Update Issue
    if (type == 'UPDATE') {
      if (itemIssue.statusIssueId == 0) strError += `- Chưa chọn trạng thái\n`;
      if (itemIssue.groupIssueId == 0) strError += `- Chưa chọn loại vấn đề\n`;
    }
    //
    ...
    return (
      ...
      >
        {dataList.map((it, idx) => {
          return (
            <InputByCode
              key={'InputByCode_' + idx}
              itemInput={it}
              indexInput={idx}
              typeInput={it.typeInput}
  ...
  return (
    <View style={styles.mainContainer}>
      <View style={styles.contentAction}>
        ...
        <ButtonAction type="CANCEL" title="Đóng" handlerPress={onButtonPress} />
        <ButtonAction
          type="CREATE"
      ...
      <View style={styles.contentMain}>
        ...
        <View style={styles.itemMain}>
          ...
          >{`Hình ảnh (${itemIssue.imageIssueLength} Tấm)`}</Text>
          <View style={styles.contentImage}>
            <PhotoList
              typeMain={type}
              itemIssue={itemIssue}
              callBackData={handlerCountImage}
            />
          </View>
        </View>
        {/* // */}
