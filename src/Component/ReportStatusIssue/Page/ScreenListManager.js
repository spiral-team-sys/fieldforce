...
export const ScreenListManager = ({}) => {
  ...
  const onSearchData = text => {
    ...
    const listFilter = _.filter(
      dataMain,
      e =>
        removeVietnameseTones(e.groupIssueName || '')
          .toLowerCase()
          .match(textValue) ||
        removeVietnameseTones(e.issueStatusName || '')
          .toLowerCase()
          .match(textValue) ||
        removeVietnameseTones(e.noteIssue || '')
          .toLowerCase()
          .match(textValue) ||
        removeVietnameseTones(e.issueComments || '')
          .toLowerCase()
          .match(textValue) ||
  ...
  const onSendUpdate = async () => {
    ...
    if (listReudpate !== null && listReudpate.length > 0) {
      ...
      listReudpate.forEach(item => {
        ...
        if (info.match(item.shopName) == null) info += `${item.shopName}, `;
        //
        const itemUpdate = {
      ...
      ToastError(
        `Vui lòng chọn tối thiểu 1 vấn đề để yêu cầu cập nhật lại`,
        'Dữ liệu cập nhật',
        'top',
      );
    }
  ...
  useEffect(() => {
    ...
    const _reloadData = DeviceEventEmitter.addListener(
      'RELOAD_DATA_ISSUE',
      LoadData,
    );
    return () => {
  ...
  const styles = StyleSheet.create({
    ...
    groupViewHead: {
      ...
      borderBottomColor:
        data !== null && data.length > 0 ? DEFAULT_LIGHT_COLOR : appcolor.light,
      backgroundColor: appcolor.light,
    },
  ...
  const renderItem = ({ item, index }) => {
    ...
    const onPress = () => {
      openReupdate.isVisible
        ...
        : item.isUpdate == 1 && handlerUpdateItem(item);
    };
    const onLongPress = () => {
      handlerReupdateStatus(item);
    };
    const iconNameChoose =
      item.isReupdateStatus == 1
        ? item.reupdate == 1
          ? 'close-circle'
          : 'checkmark-circle'
        : 'ellipse-outline';
        ...
        <View style={styles.headerContentView}>
          {openReupdate.isVisible ? (
            <SpiralIcon
              ...
              name={iconNameChoose}
              size={21}
              color={item.reupdate == 1 ? appcolor.red : appcolor.success}
            />
          ) : (
            <SpiralIcon
              type="ionicon"
          ...
          <View style={styles.contentDataItem}>
            <Text
              style={{ ...styles.titleContent, fontWeight: fontWeightBold }}
            >{`Cửa hàng: ${item.shopName}`}</Text>
            <Text
              style={{ ...styles.titleContent, fontStyle: 'italic' }}
            >{`${item.groupIssueName}`}</Text>
            <Text style={styles.titleHead}>{`${item.noteIssue}`}</Text>
            {item.issueComments && (
              <Text
                style={styles.titleHead}
              >{`Ghi chú: ${item.issueComments}`}</Text>
            )}
          </View>
        </View>
              ...
              >
                <View style={styles.contentMain}>
                  <FlatList
                    ...
                    ListFooterComponent={
                      <View style={{ paddingBottom: deviceHeight / 4.5 }} />
                    }
                    refreshControl={
