...
import { deviceWidth, minWidthTab } from "../../../Core/Utility";
...
import { ToastError, ToastSuccess, removeVietnameseTones } from "../../../Core/Helper";
...
export const ScreenListManager = ({ }) => {
    ...
    const [openReupdate, _setOpenReupdate] = useState({ isVisible: false, countItem: 0 })
    ...
    const LoadData = async () => {
        openReupdate.isVisible = false
        openReupdate.countItem = 0
        //
        await setLoading(true)
        const params = { shopId: 0, reportId: kpiinfo.id }
    ...
    // Handler
    const handlerSearchByGroup = async (item, _type, isMultiple) => {
    ...
    const onSearchData = (text) => {
        ...
        const listFilter = _.filter(dataMain, (e) => removeVietnameseTones(e.groupIssueName || '').toLowerCase().match(textValue) ||
            ...
            removeVietnameseTones(e.shopName || '').toLowerCase().match(textValue) ||
    ...
    const handlerReupdateStatus = (item) => {
        const value = (item.isReupdateStatus || 0) == 1 ? 0 : 1
        if (!openReupdate.isVisible) {
            openReupdate.isVisible = true
            openReupdate.countItem = 1
        } else {
            openReupdate.countItem = openReupdate.countItem + (value == 1 ? 1 : -1)
        }

        item.isReupdateStatus = value
        setMutate(e => !e)
    }
    const onCloseSendUpdate = () => {
        openReupdate.isVisible = false
        openReupdate.countItem = 0
        const listUpdate = _.map(data, (it, _idx) => {
            return { ...it, isReupdateStatus: 0 }
        })
        setData(listUpdate)
    }
    const onSendUpdate = async () => {
        const listReudpate = _.filter(dataMain, (e) => e.isReupdateStatus == 1)
        if (listReudpate !== null && listReudpate.length > 0) {
            let dataUpdate = []
            let employeeList = ''
            let info = ''
            listReudpate.forEach(item => {
                // EmployeeId Info
                if (employeeList == '') employeeList = `${item.employeeId},`
                if (employeeList.match(item.employeeId) == null)
                    employeeList += `${item.employeeId},`
                // Shop Name Info
                if (info == '') info = `${item.shopName}, `
                if (info.match(item.shopName) == null)
                    info += `${item.shopName}, `
                // 
                const itemUpdate = {
                    issueId: item.issueId,
                    issueStatusUpdate: item.issueStatus,
                    jsonUpdate: JSON.stringify(item)
                }
                dataUpdate.push(itemUpdate)
            });
            const itemUpdate = {
                jsonData: JSON.stringify(dataUpdate),
                messager: `Quản lý yêu cầu cập nhật lại vấn đề của cửa hàng: ${info} Vui lòng kiểm tra lại `,
                employeeList: `${employeeList}0`
            }
            // 
            await ISSUEAPI.sendReupdateData(itemUpdate, async (result) => {
            ...
            ToastError(`Vui lòng chọn tối thiểu 1 vấn đề để yêu cầu cập nhật lại`, 'Dữ liệu cập nhật', 'top')
        }
    }
    //
    useEffect(() => {
    ...
    const styles = StyleSheet.create({
        ...
        titleContent: { width: '100%', fontSize: 14, color: appcolor.greylight, paddingHorizontal: 8, fontWeight: '500' },
        ...
        groupViewHead: { width: '100%', borderBottomWidth: 0.5, borderBottomColor: data !== null && data.length > 0 ? DEFAULT_LIGHT_COLOR : appcolor.light, backgroundColor: appcolor.light },
        ...
        itemViewAction: { flexDirection: 'row', alignItems: 'center' },
        viewActionSend: { width: '100%', padding: 8, position: 'absolute', bottom: (deviceHeight / 8) + 8, flexDirection: 'row', justifyContent: 'center' }
    })
    const renderItem = ({ item, index }) => {
        const photos = JSON.parse(item.imageIssues || [])
        const onPress = () => {
            openReupdate.isVisible ? handlerReupdateStatus(item) : (item.isUpdate == 1 && handlerUpdateItem(item))
        }
        const onLongPress = () => {
            handlerReupdateStatus(item)
        }
        const iconNameChoose = item.isReupdateStatus == 1 ? (item.reupdate == 1 ? "close-circle" : "checkmark-circle") : 'ellipse-outline'
        return (
            <TouchableOpacity
                key={`slism_${index}`}
            ...
            >
                <View style={styles.headerContentView}>
                    {openReupdate.isVisible ?
                        <Icon type="ionicon" name={iconNameChoose} size={21} color={item.reupdate == 1 ? appcolor.red : appcolor.success} />
                        :
                        <Icon type="ionicon" name={item.iconStatus} size={21} color={appcolor[item.colorStatus]} />
                    }
                    <View style={styles.contentDataItem}>
                        <Text style={{ ...styles.titleContent, fontWeight: fontWeightBold }}>{`Cửa hàng: ${item.shopName}`}</Text>
                        <Text style={{ ...styles.titleContent, fontStyle: 'italic' }}>{`${item.groupIssueName}`}</Text>
        ...
        <View style={styles.mainContainer}>
                            ...
                            <Tabs.Tab key={`tabiis_${index}`} label={titleHead} name={titleHead} >
                                <View style={styles.contentMain}>
                                    <FlatList
                                        ...
                                        ListFooterComponent={<View style={{ paddingBottom: deviceHeight / 4.5 }} />}
            ...
            {openReupdate.isVisible &&
                <FloatActionButton
                    ...
                    titleAction={`Gửi ${openReupdate.countItem} yêu cầu`}
                    onAction={onSendUpdate}
                    onClose={onCloseSendUpdate}
                />
            }
        </View>
    )
