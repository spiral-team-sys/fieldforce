...
export const GroupList = ({ defaultValue, dataMain, handlerChange, isEnableChoose = true }) => {
    ...
    const onScrollToIndex = () => {
        if (dataGroup !== null && dataGroup.length > 0) {
            const _index = _.findIndex(dataGroup, (e) => e.GroupIssueId == defaultValue, 0)
            if (_index > 0)
    ...
    const styles = StyleSheet.create({
        ...
        itemChoose: { minWidth: 80, padding: 8, margin: 8, marginEnd: 1, borderRadius: 20, borderWidth: 0.5, borderColor: appcolor.primary, backgroundColor: appcolor.primary },
    ...
    const renderItem = ({ item, index }) => {
        ...
        const styleDefault = { ...styles.itemMain, marginEnd: (index + 1) == dataGroup.length ? 8 : 1 }
        const styleChoose = { ...styles.itemChoose, marginEnd: (index + 1) == dataGroup.length ? 8 : 1 }
        return (
            <TouchableOpacity key={`igi_${index}`} style={isChoose ? styleChoose : styleDefault} onPress={isEnableChoose ? pressItem : null}>
