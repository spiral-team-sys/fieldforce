...
import { ActivityIndicator, FlatList, Modal, Platform, ScrollView, StyleSheet, TouchableOpacity, View } from "react-native";
...
export const PhotoView = ({ photos, indexMain }) => {
    ...
    const [itemShowImage, setItemShowImage] = useState({ visible: false, photos: [], index: 0 })
    ...
    // View
    const sizeWidthImage = Math.round(dataPhoto.length < 5 ? (deviceWidth / dataPhoto.length) : (deviceWidth / 5))
    const styles = StyleSheet.create({
        mainContainer: { borderRadius: 5, overflow: 'hidden' },
        itemMain: { width: sizeWidthImage, marginEnd: 5, borderRadius: 5, overflow: 'hidden' },
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity key={`igsi_${index}_${indexMain}`} style={styles.itemMain} onPress={onPress}>
                <Image
