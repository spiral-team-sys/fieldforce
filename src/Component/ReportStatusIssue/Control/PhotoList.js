...
export const PhotoList = ({ typeMain, itemIssue, callBackData }) => {
    ...
    const LoadData = async () => {
        const _photoList = await GetPhotosIssue(shopinfo.shopId || itemIssue.shopId, TODAY, kpiinfo.id, itemIssue.guid, typeMain == 'UPDATE')
        switch (typeMain) {
            case "PLUS":
                ...
                break
            case "UPDATE":
                const mergeList = _.unionWith(_photoList, itemIssue.imageIssueList, _.isEqual)
    ...
    const handlerChooseImage = (item) => {
        ...
        if (valueDelete)
            dataImageRemove.push(item)
        else
            _.pullAll(dataImageRemove, [item])
        if (dataImageRemove == null || dataImageRemove.length == 0)
            setRemoveImage(false)
        setMutate(e => !e)
    }
    ...
    const styles = StyleSheet.create({
        ...
        itemMain: { minWidth: deviceWidth / 2.5, margin: 8, marginEnd: 1, borderRadius: 5, borderWidth: 1.5, borderColor: appcolor.grayLight, overflow: 'hidden' },
        ...
        iconRemoveView: { position: 'absolute', top: 0, end: 0, zIndex: 1000, padding: 8 },
        opacityView: { width: '100%', height: '100%', backgroundColor: "#000", opacity: 0.6, zIndex: 100, position: 'absolute' }
    })
    const renderItem = ({ item, index }) => {
        ...
        const photoPath = item.photoPath !== null && item?.photoPath?.indexOf('file://') > -1 ? item.photoPath : `${URLDEFAULT}${item.photoPath}`
        return (
            <TouchableOpacity key={`igi_${index}`} style={styles.itemMain} onPress={onPress} onLongPress={onLongPress} >
                {isRemoveImage && <View style={styles.opacityView} />}
                {isRemoveImage && (item.isDelete || false) &&
                    <View style={styles.iconRemoveView}>
                        <Icon type="ionicon" name="trash" size={21} color={appcolor.red} />
                    </View>
                }
                <Image
        ...
        <View style={styles.mainContainer}>
            <View style={styles.viewOptionCamera}>
                ...
                {isRemoveImage &&
                    <ButtonAction
                        type='REMOVE_IMAGE'
                        title={dataImageRemove.length > 0 ? `Xóa ${dataImageRemove.length} hình` : `Hủy`}
                        iconName='trash'
