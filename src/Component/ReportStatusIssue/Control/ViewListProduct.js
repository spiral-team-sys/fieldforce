...
export const ViewListProduct = ({ dataProducts, modalConfig, handleCloseModal, styles, type, handleSelectChange }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    const [_mutate, setMutate] = useState(false)
    const [itemSelect, setItemSelect] = useState({})
    const [data, setData] = useState({ dataView: [], dataMain: [], dataSelect: [] })
    ...
    const loadData = () => {
        ...
        setItemSelect(itemS)
        setData({ dataView: updatedLargeArray, dataMain: updatedLargeArray, dataSelect: dataS })
        setLoading(false)
    }
    ...
    const onContentChangeText = (item, text) => {
        const dataSelectNew = data.dataSelect.map(it => ({
            ...it,
            [itemSelect.subInfo]: (it.Id == item.Id ? text : (it[itemSelect.subInfo] || null))
        }))
    ...
    const renderItem = ({ item, index }) => {
        const itemValue = item.isSelect == 1 ? (data.dataSelect.filter(it => it.Id == item.Id)[0] || null) : null
            ...
            <View style={{ width: '100%' }}>
                {item.isParent && item.CategoryId !== undefined &&
                    <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: appcolor.primary, padding: 8, borderRadius: 5, marginVertical: 8 }}>
                        <Text style={{ flex: 1, fontSize: 14, fontWeight: 'bold', color: appcolor.white }}> {item.Category}</Text>
                    </View>
                }
                <View style={{ width: '100%', padding: 4, borderBottomColor: appcolor.darklight, borderBottomWidth: 1 }}>
                    <TouchableOpacity
                        onPress={() => handleSelectProduct(item)}
                        style={{ justifyContent: 'center', backgroundColor: item.isSelect == 1 ? DEFAULT_LIGHT_COLOR : appcolor.transparent, padding: 8, borderRadius: 8 }}>
                        <Text style={{ fontSize: 14, fontWeight: 'bold', color: appcolor.dark }}> {item.ProductName}</Text>
                    </TouchableOpacity>
                    {
    ...
    return (
        <View style={{ width: deviceWidth, height: deviceHeight, backgroundColor: appcolor.light }}>
            <Modal
                animationType="slide" onRequestClose={() => { handleCloseModal() }}
                style={{ flex: 1 }} visible={modalConfig.visible} >

                <View style={{ flex: 1 }}>
                    <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center', padding: 8, borderRadius: 5, marginVertical: 16 }}>
                        <Text style={{ fontSize: 22, fontWeight: 'bold', color: appcolor.dark }}>Danh sách sản phẩm</Text>
                    </View>

                    {
                        ...
                        <FormGroup
                            ...
                            containerStyle={search.isSearch ? { margin: 8, padding: Platform.OS == 'android' ? 3 : 5, paddingHorizontal: 8, borderRadius: 20, backgroundColor: appcolor.primary, borderWidth: 0.5 } : { margin: 8, padding: Platform.OS == 'android' ? 3 : 5, paddingHorizontal: 8, borderRadius: 20, backgroundColor: appcolor.light, borderWidth: 0.5, borderColor: appcolor.primary }}
                            inputStyle={search.isSearch ? { fontSize: 13, color: appcolor.light, fontWeight: '500' } : { fontSize: 13, color: appcolor.primary }}
                ...
                <TouchableOpacity onPress={handleCloseModal} style={{ position: 'absolute', right: 20, top: 20, zIndex: 1000 }}>
                    <Icon size={18} name="close" containerStyle={{ opacity: 0.8 }} reverse />
                </TouchableOpacity>
            </Modal>
        </View>
    )
