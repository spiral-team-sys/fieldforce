...
export const getSummaryRowsFromConfig = ({
    ...
    return configItemsSorted.map((item, index) => {
        ...
        if (mapInfo?.field === "reportDate") {
            return { label: item.ItemName || "Ngay bao cao", value: reportDateText || "-" };
        }
        if (mapInfo?.field === "productName") {
            return { label: item.ItemName || "San pham", value: selectedProduct?.productName || "Chua chon" };
        }
        if (mapInfo?.field === "quantity") {
            return { label: item.ItemName || "So luong", value: qtyValue > 0 ? `${qtyValue}` : "0" };
        }
        if (mapInfo?.field === "price") {
            return { label: item.ItemName || "Gia", value: priceValue > 0 ? `${toCurrency(priceValue)} VND` : "0 VND" };
        }
        if (mapInfo?.field === "serial") {
            return { label: item.ItemName || "SERIAL/IMEI", value: formValues.serial || "-" };
        }
        if (mapInfo?.field === "customer") {
            return { label: item.ItemName || "Khach hang", value: formValues.customer || "-" };
        }
        if (mapInfo?.field === "address") {
            return { label: item.ItemName || "Dia chi", value: formValues.address || "-" };
        }
        if (mapInfo?.field === "phone") {
            return { label: item.ItemName || "So dien thoai", value: formValues.phone || "-" };
        }
        ...
        return {
            ...
            value: dynamicValue !== undefined && dynamicValue !== null && `${dynamicValue}`.trim().length > 0 ? `${dynamicValue}` : "-",
        };
    });
};
