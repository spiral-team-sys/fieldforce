...
const CompetitorSection = ({
  ...
  const sectionStyles = StyleSheet.create({
    ...
    competitorChoiceActive: {
      borderColor: appcolor.primary,
      backgroundColor: appcolor.primary,
    },
    competitorChoiceNegativeActive: {
      borderColor: appcolor.primary,
      backgroundColor: appcolor.surface,
    },
    competitorChoiceText: {
      fontSize: 12,
      color: appcolor.dark,
      fontWeight: '700',
    },
  ...
  const otherCompetitorItem =
    competitorDataList.find(it => Number(it?.Level) === 2) ||
    competitorDataList.find(it => Number(it?.Id) === 100);
  const selectedYesIds = Array.isArray(payload?.selectedId)
    ? payload.selectedId.map(id => Number(id))
    : [];
  ...
  useEffect(() => {
    setChoiceMap(prev => {
      ...
      Object.keys(nextMap).forEach(key => {
        if (
          !selectedYesIds.some(id => Number(id) === Number(key)) &&
          Number(nextMap[key]) === 1
        ) {
          delete nextMap[key];
        }
      });
  ...
  const isChooseYesOtherCompetitor =
    !!otherCompetitorItem &&
    (Number(choiceMap[`${otherCompetitorItem?.Id}`]) === 1 ||
      selectedYesIds.some(
        id => Number(id) === Number(otherCompetitorItem?.Id),
      ));
      ...
      {groupDataList.length > 0 && (
        <View style={sectionStyles.competitorQuestionWrap}>
          {competitorDataList.map((child, childIndex) => {
            ...
            const isYes =
              selectedValue === 1 ||
              selectedYesIds.some(id => Number(id) === Number(child?.Id));
            return (
              <View
                key={`competitor_${item?.Id || index}_${
                  child?.Id || childIndex
                }`}
                style={sectionStyles.competitorQuestionRow}
              >
                <Text style={sectionStyles.competitorName}>
                  {child?.CompetitorName || child?.KPIName || ''}
                </Text>
                <View style={sectionStyles.competitorActionRow}>
                  {yesNoOptions.map((option, optionIndex) => {
                    const active =
                      Number(option?.itemValue) === 1
                        ? isYes
                        : isUploaded == 1
                        ? !isYes
                        : selectedValue === 0;
                    ...
                    return (
                      <React.Fragment
                        key={`competitor_yesno_${item?.Id || index}_${
                          child?.Id || childIndex
                        }_${optionIndex}`}
                      >
                        {optionIndex > 0 && (
                          <View style={sectionStyles.competitorChoiceGap} />
                        )}
                        <TouchableOpacity
                          activeOpacity={0.8}
                          ...
                          style={[
                            ...
                            active &&
                              (isYesOption
                                ? sectionStyles.competitorChoiceActive
                                : sectionStyles.competitorChoiceNegativeActive),
                          ]}
                        >
                          <Text
                            style={[
                              ...
                              active &&
                                (isYesOption
                                  ? sectionStyles.competitorChoiceTextActive
        ...
        <View style={sectionStyles.inputWrap}>
          <FormGroup
            ...
            handleChangeForm={value =>
              onChangePayloadValue(index, 'otherCompetitor', value)
            }
            onClearTextAndroid={() =>
              onChangePayloadValue(index, 'otherCompetitor', '')
            }
          />
        </View>
      )}
