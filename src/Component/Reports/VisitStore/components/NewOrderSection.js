...
const NewOrderSection = ({
  ...
  const otherData = (groupDataList || []).find(child => {
    const childName = `${child?.CompetitorName || child?.KPIName || ''}`
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .toLowerCase();
      ...
      <View style={sectionStyles.optionListWrap}>
        {list.map((child, childIndex) => {
          const isActive =
            ...
            selectedIds.some(id => Number(id) === Number(child?.Id));
          if (typeof onPress !== 'function') {
            return (
              <View
                key={`${keyPrefix}_${child?.Id || childIndex}`}
                style={sectionStyles.optionItem}
              >
                <Text style={sectionStyles.optionItemText}>
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                sectionStyles.optionItem,
                isActive && sectionStyles.optionItemNegativeActive,
              ]}
            >
              <Text
                style={[
                  sectionStyles.optionItemText,
                  isActive && sectionStyles.optionItemTextNegativeActive,
                ]}
              >
                {child?.CompetitorName || child?.KPIName || ''}
              </Text>
    ...
    <>
        ...
        <View style={sectionStyles.inputWrap}>
          <FormGroup
            ...
            handleChangeForm={value =>
              onChangePayloadValue(index, 'price', formatNumber(value, '.'))
            }
      ...
      {showOtherDataInput && (
        <View style={sectionStyles.inputWrap}>
          <FormGroup
            ...
            handleChangeForm={value =>
              onChangePayloadValue(index, 'otherProduct', value)
            }
            onClearTextAndroid={() =>
              onChangePayloadValue(index, 'otherProduct', '')
            }
          />
        </View>
      )}
      {isShowReasonList && (
        <Text style={sectionStyles.sectionTitle}>{'Lý do'}</Text>
      )}
      {isShowReasonList && (
        <View style={sectionStyles.optionListWrap}>
          {reasonOptions.map(reason => {
            ...
            return (
              <TouchableOpacity
                ...
                disabled={isUploaded == 1}
                onPress={() =>
                  isUploaded != 1 && onChooseReason(index, item, reason?.id)
                }
                style={[
                  sectionStyles.optionItem,
                  active && sectionStyles.optionItemNegativeActive,
                ]}
              >
                <Text
                  style={[
                    sectionStyles.optionItemText,
                    active && sectionStyles.optionItemTextNegativeActive,
                  ]}
                >
                  {reason?.name || ''}
                </Text>
      ...
      {isShowReasonList &&
        (Number(payload?.reasonId) === 100 ||
          Number(payload?.reasonId) === Number(otherReason?.id)) && (
          <View style={sectionStyles.inputWrap}>
            <FormGroup
              editable={isUploaded != 1}
              useClearAndroid={true}
              ...
              handleChangeForm={value =>
                onChangePayloadValue(index, 'otherReason', value)
              }
              onClearTextAndroid={() =>
                onChangePayloadValue(index, 'otherReason', '')
              }
            />
          </View>
        )}
      ...
      {item?.IsNewOrder === 1 && isChooseNo && (
        <View style={sectionStyles.inputWrap}>
          <FormGroup
            ...
            handleChangeForm={value =>
              onChangePayloadValue(index, 'detailReason', value)
            }
            onClearTextAndroid={() =>
              onChangePayloadValue(index, 'detailReason', '')
            }
          />
        </View>
      )}
