...
import { MessageAcept, formatNumber, groupDataByKey } from "../../../Core/Helper";
import { alertConfirm, TODAY } from "../../../Core/Utility";
import { itemUploaded, saveJsonData } from "../../../Controller/ReportController";
...
const SalesPromoterScreen = ({ navigation }) => {
    ...
    const reasonOptions = Array.isArray(config?.reasonList) ? config.reasonList : []
    ...
    const handlerSaveLocalData = async (nextData) => {
        await saveJsonData(shopinfo.shopId || 0, kpiinfo.id, shopinfo.auditDate, nextData || [])
    }
    ...
    const handlerGetPayloadFromItemValue = (rawValue) => {
        const handlerNormalizePayload = (payload) => {
            ...
            const { selectedIds, orderValue, reasonInput, competitorOtherText, ...restPayload } = nextPayload
            ...
            return {
                ...
                detailReason: nextPayload?.detailReason ?? (Number(nextPayload?.itemValue) === 0 ? reasonInput ?? '' : ''),
                otherReason: nextPayload?.otherReason ?? '',
                otherCompetitor: nextPayload?.otherCompetitor ?? competitorOtherText ?? ''
            }
        }
    ...
    const handlerIsItemCompletedForUpload = (item) => {
        ...
        if (Number(item?.isSingleChoice) === 1 && Number(item?.IsNewOrder) === 1) {
            ...
            if (selectedValue === 0) {
                const otherReasonId = (reasonOptions || []).find((reason) => `${reason?.name || ''}`.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().includes('khac'))?.id
                if (!handlerHasInputValue(payload?.reasonId) || !handlerHasInputValue(payload?.detailReason)) return false
                if ((Number(payload?.reasonId) === 100 || Number(payload?.reasonId) === Number(otherReasonId)) && !handlerHasInputValue(payload?.otherReason)) return false
                return true
            }

            if (selectedValue === 1) {
                if (!handlerHasInputValue(payload?.price) || !handlerHasInputValue(payload?.selectedId)) return false
                if ((payload?.selectedId || []).some((id) => Number(id) === 0) && !handlerHasInputValue(payload?.otherProduct)) return false
                return true
            }
        ...
        if (Number(item?.isSingleChoice) === 1 && Number(item?.IsCompetitor) === 1) {
            const payload = handlerGetPayloadFromItemValue(item?.ItemValue)
    ...
    const handlerValidateData = () => {
        ...
        const missingRequiredItems = (dataMain || []).filter((item) => !handlerIsItemCompletedForUpload(item))
        if (missingRequiredItems.length > 0) {
            const previewNames = missingRequiredItems
                .map((item, idx) => `${idx + 1}. ${item?.KPIName || 'KPI chưa có tên'}`)
                .join('\n')
            MessageAcept('Thiếu dữ liệu bắt buộc', `Vui lòng nhập đầy đủ các mục bắt buộc trước khi gửi:\n${previewNames}`, () => { })
            return false
        }
    ...
    const onChooseYesNo = (index, item, optionValue, newOrderFlow = false) => {
        if (!newOrderFlow) {
            handlerUpdateItemValue(index, dataMain[index]?.ItemValue === optionValue ? null : optionValue)
            return
        }

        const currentPayload = handlerGetPayloadFromItemValue(dataMain[index]?.ItemValue)
        if (currentPayload?.itemValue === optionValue) {
            handlerUpdateItemValue(index, null)
            return
        }

        handlerUpdateItemValue(index, JSON.stringify({
            ...
            otherProduct: optionValue === 1 ? currentPayload?.otherProduct ?? '' : '',
            detailReason: optionValue === 0 ? currentPayload?.detailReason ?? '' : '',
    ...
    const onToggleDataListValue = (index, childId) => {
        ...
        const selectedId = Array.isArray(currentPayload?.selectedId) ? [...currentPayload.selectedId] : []
        ...
        handlerUpdateItemValue(index, JSON.stringify({
            ...currentPayload,
            selectedId,
            otherProduct: selectedId.some((id) => Number(id) === 0) ? currentPayload?.otherProduct ?? '' : ''
        }))
    }
    ...
    const onChooseCompetitorValue = (index, childId, optionValue) => {
        ...
        handlerUpdateItemValue(index, JSON.stringify({
            ...
            selectedId,
            otherCompetitor: selectedId.some((id) => Number(id) === 0) ? currentPayload?.otherCompetitor ?? '' : ''
        }))
    }
    ...
    const onUploadData = async () => {
        ...
        alertConfirm('Gửi dữ liệu', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, bạn có chắc chắn không?', async () => {
            ...
            if (result?.statusId == 200) {
                setIsUploaded(1)
                MessageAcept('Thông báo', result?.messager || 'Gửi dữ liệu thành công', () => navigation.goBack())
            } else {
                MessageAcept('Lỗi dữ liệu', result?.messager || 'Gửi dữ liệu thất bại', () => { })
            }
            setLoading(false)
        })
    }

    const totalGroups = _.uniqBy(dataMain || [], (it) => `${it?.GroupId ?? ''}`).length
    const reportDate = shopinfo?.auditDate || TODAY

    const styles = StyleSheet.create({
        mainContainer: { flex: 1, backgroundColor: appcolor.light },
        contentContainer: { paddingHorizontal: 12, paddingVertical: 8, backgroundColor: appcolor.light },
        listHeaderWrap: { borderWidth: 1, borderColor: appcolor.grayLight, borderRadius: 10, backgroundColor: appcolor.surface, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 8 },
        ...
        listHeaderInfoWrap: { marginTop: 8, borderTopWidth: 1, borderTopColor: appcolor.grayLight, paddingTop: 8 },
        listHeaderInfoRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 },
        listHeaderInfoLabel: { fontSize: 12, color: appcolor.greydark, fontWeight: '600' },
        listHeaderInfoValue: { fontSize: 12, color: appcolor.dark, fontWeight: '700', flexShrink: 1, textAlign: 'right' },
        listHeaderStatus: { marginTop: 2, alignSelf: 'flex-start', borderWidth: 1, borderColor: isUploaded == 1 ? appcolor.primary : appcolor.grayLight, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 4, backgroundColor: appcolor.light },
        listHeaderStatusText: { fontSize: 11, color: isUploaded == 1 ? appcolor.primary : appcolor.greydark, fontWeight: '700' },
        groupHeader: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, paddingHorizontal: 10, backgroundColor: appcolor.primary, borderRadius: 10, marginTop: 8, marginBottom: 6 },
        groupHeaderText: { flex: 1, marginLeft: 8, marginRight: 8, fontSize: 13, lineHeight: 18, color: appcolor.light, fontWeight: '700' },
        itemCard: { borderRadius: 12, borderWidth: 1, borderColor: appcolor.grayLight, backgroundColor: appcolor.surface, padding: 12, marginBottom: 8 },
        itemName: { flex: 1, fontSize: 13, color: appcolor.dark, fontWeight: '700', paddingRight: 6 },
        itemNameInline: { flex: 1, fontSize: 13, color: appcolor.dark, fontWeight: '700', paddingRight: 8 },
        ...
        yesNoButtonInline: { flex: 1, borderWidth: 1, borderColor: appcolor.grayLight, borderRadius: 8, paddingVertical: 6, alignItems: 'center', justifyContent: 'center', backgroundColor: appcolor.light },
        yesNoButtonActive: { borderColor: appcolor.primary, backgroundColor: appcolor.primary },
        yesNoButtonNegativeActive: { borderColor: appcolor.primary, backgroundColor: appcolor.surface },
        ...
        salesWrap: { marginTop: 8, borderWidth: 1, borderColor: appcolor.grayLight, borderRadius: 10, backgroundColor: appcolor.light, paddingHorizontal: 10, paddingVertical: 8 },
        salesRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 2 },
        ...
        emptyText: { fontSize: 13, color: appcolor.placeholderText, textAlign: 'center' },
        reloadButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderRadius: 10, paddingVertical: 8, marginTop: 10 },
        reloadText: { fontSize: 13, color: appcolor.primary, fontWeight: '700', marginLeft: 6 },
    })
        ...
        return (
            <View style={styles.yesNoRowInline}>
                {yesNoOptions.map((option, optionIndex) => {
                    const isActive = hasSelectedValue && Number(selectedValue) === Number(option?.itemValue)
                    const isYesOption = Number(option?.itemValue) === 1
                    return (
                        <React.Fragment key={`choice_inline_${optionIndex}_${option?.itemValue}`}>
                            ...
                            <TouchableOpacity
                                ...
                                style={[
                                    ...
                                    isActive && (isYesOption ? styles.yesNoButtonActive : styles.yesNoButtonNegativeActive)
                                ]}
                            >
                                <Text
                                    style={[
                                        styles.yesNoTextInline,
                                        isActive && (isYesOption ? styles.yesNoTextActive : styles.yesNoTextNegativeActive)
                                    ]}
                                >
                                    {option?.itemName || ''}
                                </Text>
        ...
        return (
            <View style={styles.optionListWrap}>
                {list.map((child, childIndex) => (
                    <View key={`${keyPrefix}_${child?.Id || childIndex}`} style={styles.optionItem}>
                        <Text style={styles.optionItemText}>{child?.CompetitorName || child?.KPIName || ''}</Text>
                    </View>
                ))}
            </View>
        )
    ...
    const renderHeader = (
        <View style={styles.listHeaderWrap}>
            ...
            <Text style={styles.listHeaderSub}>Mở từng nhóm để nhập dữ liệu và nhấn gửi khi hoàn tất.</Text>
            <View style={styles.listHeaderInfoWrap}>
                <View style={styles.listHeaderInfoRow}>
                    ...
                    <Text style={styles.listHeaderInfoValue}>{shopinfo?.shopName || 'Chưa có thông tin'}</Text>
                </View>
                <View style={styles.listHeaderInfoRow}>
                ...
                <View style={styles.listHeaderInfoRow}>
                    ...
                    <Text style={styles.listHeaderInfoValue}>{`${totalGroups} nhóm`}</Text>
                </View>
                <View style={styles.listHeaderStatus}>
                    <Text style={styles.listHeaderStatusText}>{isUploaded == 1 ? 'Trạng thái: Đã gửi dữ liệu' : 'Trạng thái: Chưa gửi dữ liệu'}</Text>
                </View>
            </View>
        </View>
    )
    ...
    const renderItem = ({ item, index }) => {
        ...
        const payload = handlerGetPayloadFromItemValue(item?.ItemValue)
        const hasItemValue = payload?.itemValue !== null && payload?.itemValue !== undefined && payload?.itemValue !== ''
        const isChooseYes = hasItemValue && Number(payload?.itemValue) === 1
        const isChooseNo = hasItemValue && Number(payload?.itemValue) === 0
        const isShowReasonList = Number(item?.isSingleChoice) === 1 && item.IsNewOrder == 1 && isChooseNo
        ...
        return (
            <View>
                {item?.isParent && item?.GroupName !== undefined && (
                    <TouchableOpacity
                        activeOpacity={0.8}
                        onPress={() => setCollapsedGroups(prev => ({ ...prev, [groupKey]: !prev[groupKey] }))}
                        style={styles.groupHeader}
                    >
                        <Icon name='layers' type='feather' size={16} color={appcolor.light} />
                        <Text style={styles.groupHeaderText}>{item?.GroupName || `Nhóm ${index + 1}`}</Text>
                        <Icon name={isCollapsed ? 'chevron-down' : 'chevron-up'} type='feather' size={16} color={appcolor.light} />
                    </TouchableOpacity>
                )}

                {!isCollapsed && (
                    <View style={styles.itemCard}>
                        {Number(item?.isSingleChoice) === 1 && Number(item?.IsCompetitor) !== 1 ? (
                            <View style={styles.itemTitleRow}>
                                <Text style={styles.itemNameInline}>{item?.KPIName || 'Chưa có tên KPI'}</Text>
                                {renderChoiceButtonsInline(payload?.itemValue, (value) => onChooseYesNo(index, item, value, item.IsNewOrder === 1))}
                            </View>
                        ) : (
                            <Text style={styles.itemName}>{item?.KPIName || 'Chưa có tên KPI'}</Text>
                        )}
                            ...
                            <View style={styles.salesWrap}>
                                {item.SalesLastMonth !== undefined && (
                                    <View style={styles.salesRow}>
                                        ...
                                        <Text style={styles.salesValue}>{formatNumber(item?.SalesLastMonth ?? 0, '.')}</Text>
                                    </View>
                                )}
                                {item.SalesToDate !== undefined && (
                                    <View style={styles.salesRow}>
                                        <Text style={styles.salesLabel}>Doanh số tới hiện tại</Text>
                                        <Text style={styles.salesValue}>{formatNumber(item?.SalesToDate ?? 0, '.')}</Text>
                                    </View>
                                )}
                            </View>
                        )}

                        {!(item?.IsNewOrder === 1) && Number(item?.IsCompetitor) !== 1 && groupDataList.length > 0 && item?.isParent && (
                            renderChipList({ list: groupDataList, keyPrefix: `readonly_${item?.Id || index}` })
                        )}
                        ...
                        {Number(item?.Note) === 1 && Number(item?.isSingleChoice) !== 1 && (
                            <View style={styles.inputWrap}>
                                <FormGroup
                                    ...
                                    title={'Nhập nội dung'}
                                    placeholder={item.GroupId == 1 ? 'Nhập chi tiết tên sản phẩm, cách nhau bởi dấu ","' : 'Nhập nội dung'}
                        ...
                        {Number(item?.NumberWarning) === 1 && Number(item?.isSingleChoice) !== 1 && (
                            <View style={styles.inputWrap}>
                                <FormGroup
                                    editable={isUploaded != 1}
                                    useClearAndroid={true}
                                    ...
                                    handleChangeForm={(value) => onChangeItemValue(index, formatNumber(value, '.'))}
                                    onClearTextAndroid={() => onChangeItemValue(index, '')}
                                />
                            </View>
                        )}
    ...
    const renderEmpty = (
        <View style={styles.emptyWrap}>
            ...
            <TouchableOpacity activeOpacity={0.8} onPress={LoadData} style={styles.reloadButton}>
                <Icon name='refresh-ccw' type='feather' size={16} color={appcolor.primary} />
    ...
    if (loading) {
        return <LoadingDefault isLoading={loading} title={'Đang tải dữ liệu KPI...'} />
    }
