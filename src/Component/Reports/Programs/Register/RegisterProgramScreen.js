...
const RegisterProgramScreen = ({ navigation, route }) => {
  ...
  const LoadData = async () => {
    ...
    //
    let guidRegister = await AsyncStorage.getItem(
      `GUID_REGISTER_${programs.id}`,
    );
  ...
  const UploadData = async () => {
    ...
    alertConfirm(
      'Thông báo',
      'Bạn có muốn gửi thông tin đăng kí chương trình trên không?',
      async () => {
        ...
        if (result.statusId == 200) {
          ...
          await AsyncStorage.removeItem(
            `GUID_REGISTER_${dataRegister.ProgramId}`,
          );
  ...
  const validData = async () => {
    ...
    for (let index = 0; index < dataValid.length; index++) {
      ...
      if (keyValue == 'CCCDInfo') {
        ...
        if (
          ...
          dataRegister[keyValue] == undefined ||
    ...
    // Photo
    const photos = await getDataPhotoByGUID(
      kpiinfo.id,
      shopinfo.shopId,
      dataRegister.GuidRegister,
    );
    if (photos.length < (config?.totalPicture || 0)) {
      strError += ` - Hình ảnh thỏa thuận cửa hàng (${photos.length}/${
        config?.totalPicture || 0
      })`;
    }
    //
  ...
  const handlerSaveItem = async dataUpdate => {
    await saveJsonData(shopinfo.shopId, kpiinfo.id, shopinfo.auditDate, [
      dataUpdate,
    ]);
  };
  const handlerSearchTaxCode = async value => {
    if (value) {
      ...
      if (checkData.length > 0) {
        const shops = _.map(checkData, e => {
          return `${e.shopName} - ${e.dealerName} - ${e.employeeName}`;
        }).join('\n');
        MessageAction2(
          `Thông tin mã số thuế đã được đăng kí cho:\n${shops}.Bạn có muốn đăng ký không?`,
          async () => {
  ...
  //
  useEffect(() => {
    const update_item = DeviceEventEmitter.addListener(
      'UPDATE_ITEM_PROGRAM',
      handlerSaveItem,
    );
    LoadData();
  ...
  if (isLoading)
    return (
      <ActivityIndicator color={appcolor.primary} style={styles.loadingView} />
    );
  return (
    <View style={styles.mainContainer}>
      ...
      >
        ...
        <View style={styles.viewGroup}>
          <ItemCapture dataRegister={dataRegister} />
        </View>
      ...
      <View style={styles.viewActionRegister}>
        ...
        >
          {isUpload ? (
            ...
            <Text style={[styles.titleName, { color: appcolor.light }]}>
              Đăng kí tham gia
            </Text>
          )}
        </TouchableOpacity>
