...
const ItemChoose = ({ dataRegister, type, title, data = [], keyValue, isEdit = true }) => {
    ...
    const onPress = () => {
        SheetManager.show('choose-item-sheet', { payload: { type, title, data, dataMain: data } })
    }
    ...
    const styles = StyleSheet.create({
        ...
        actionContainer: { padding: 12, borderWidth: 0.5, borderColor: dataRegister[keyValue] ? appcolor.grayLight : appcolor.red, borderRadius: 8, marginVertical: 8 },
        titleAction: { fontSize: 12, color: appcolor.dark },
        titleName: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.dark },
        subTitleName: { fontSize: 11, fontWeight: '500', color: appcolor.placeholderText },
        ...
        if (type == 'PROGRAM') {
            ...
            return (
                <View>
                    ...
                    <Text style={styles.subTitleName}>{itemInfo.AwardTypeName == '%' ? `${formatNumber(itemInfo.TargetAmount, ',')} VNĐ` : itemInfo.AwardName}</Text>
                    <Text style={styles.subTitleName}>{`Thời gian tham gia: ${itemInfo.FromDate} - ${itemInfo.ToDate}`}</Text>
                </View>
            )
        }
        return null
    ...
    return (
        <View style={styles.viewItemChoose}>
            ...
            <TouchableOpacity style={styles.actionContainer} onPress={onPress} disabled={!isEdit}>
                {dataRegister[keyValue] ?
                    <InfomationView />
                    :
                    <Text style={styles.titleAction}>{`Chọn ${title.toLowerCase()}`}</Text>
                }
            </TouchableOpacity>
        </View>
    )
