...
const ItemScanID = ({ navigation, dataRegister, title, keyValue, enabled = false }) => {
    ...
    const styles = StyleSheet.create({
        ...
        actionContainer: { padding: 12, borderWidth: 0.5, borderColor: Object.keys(dataRegister[keyValue] || {}).length > 0 ? appcolor.grayLight : appcolor.red, borderRadius: 8, margin: 8 },
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <TouchableOpacity style={styles.actionContainer} onPress={onScanId} activeOpacity={0.6}>
                {dataRegister?.CCCDInfo?.identityCardNumber ?
                    <View>
                        <Text style={styles.titleName}>{`${dataRegister.CCCDInfo.identityCardNumber}`}</Text>
                        <Text style={styles.subTitleName}>{`${dataRegister.CCCDInfo.fullName}`}</Text>
                        <Text style={styles.subTitleName}>{`${dataRegister.CCCDInfo.birthday}`}</Text>
                        <Text style={styles.subTitleName}>{`${dataRegister.CCCDInfo.permanentAddress}`}</Text>
                    </View>
                    :
                    <View>
                        <Icon type="ionicon" name="qr-code-outline" color={appcolor.placeholderText} size={24} />
                        <Text style={[styles.subTitleName, { textAlign: 'center', marginTop: 4 }]}>{`Quét thông tin CCCD`}</Text>
                    </View>
                }
            </TouchableOpacity>
        </View>
