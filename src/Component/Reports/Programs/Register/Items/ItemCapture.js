...
const ItemCapture = ({ dataRegister }) => {
  ...
  const onDeletePictures = async () => {
    alertConfirm(
      'Xác nhận',
      `Bạn có chắc chắn muốn xóa ${modeView.data.length} hình đã chọn không?`,
      async () => {
    ...
    <View style={styles.mainContainer}>
      <Button
        ...
        icon={
          <SpiralIcon
            type="ionicon"
            name={modeView.mode === 'delete' ? 'trash' : 'camera'}
            size={24}
            color={modeView.mode === 'delete' ? appcolor.red : appcolor.primary}
          />
        }
        ...
        buttonStyle={[
          ...
          {
            borderColor:
              modeView.mode === 'delete' ? appcolor.red : appcolor.primary,
          },
        ]}
