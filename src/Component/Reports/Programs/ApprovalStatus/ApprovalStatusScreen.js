...
const ApprovalStatusScreen = ({ navigation }) => {
  ...
  const creatorSuggestions = useMemo(() => {
    const createBySearch = removeVietnameseTones(search.createByKeyword || '')
      .toLowerCase()
      .trim();
    ...
    return _.filter(creatorList, name =>
      removeVietnameseTones(name).toLowerCase().includes(createBySearch),
    ).slice(0, 12);
  }, [search.createByKeyword, uniqueCreators]);
  const dealerSuggestions = useMemo(() => {
    const dealerSearch = removeVietnameseTones(search.dealerKeyword || '')
      .toLowerCase()
      .trim();
    ...
    return _.filter(dealerList, name =>
      removeVietnameseTones(name).toLowerCase().includes(dealerSearch),
    ).slice(0, 12);
  ...
  const onApplyFilter = () => {
    ...
    const dataShow = _.map(dataUpdate, e => {
      return { ...e, isShowStore: search.type ? true : false };
    });
    setData(dataShow);
  ...
  const onCancelRegister = item => {
    alertConfirm(
      'Thông báo',
      'Bạn có muốn hủy yêu cầu đăng kí gói trưng bày của cửa hàng này không',
      async () => {
  ...
  const _searchData = ({
    ...
    const normalizedCreateByList = _.map(createByList || [], name =>
      removeVietnameseTones(name || '')
        .toLowerCase()
        .trim(),
    );
    const normalizedDealerList = _.map(dealerList || [], name =>
      removeVietnameseTones(name || '')
        .toLowerCase()
        .trim(),
    );
    ...
    return _.filter(data, e => {
      const matchSearch =
        !valueSearch ||
        removeVietnameseTones(e.programName)
          .toLowerCase()
          .includes(valueSearch) ||
        removeVietnameseTones(e.shopName).toLowerCase().includes(valueSearch) ||
        removeVietnameseTones(e.dealerName)
          .toLowerCase()
          .includes(valueSearch) ||
        removeVietnameseTones(e.address).toLowerCase().includes(valueSearch) ||
        removeVietnameseTones(e.addressDealer)
          .toLowerCase()
          .includes(valueSearch);

      if (!matchSearch) return false;

      const createByValue = removeVietnameseTones(e.createBy || '')
        .toLowerCase()
        .trim();
      const matchCreateBy =
      ...
      const dealerValue = removeVietnameseTones(e.dealerName || '')
        .toLowerCase()
        .trim();
      const matchDealer =
  ...
  useEffect(() => {
    const _updateLocked = DeviceEventEmitter.addListener(
      'UPDATE_LOCKED_DELIVERY_SLIP',
      handlerUpdateLocked,
    );
    LoadData();
    return () => {
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <View
        style={[styles.card, { shadowColor: appcolor.light, borderWidth: 1 }]}
      >
        <TouchableOpacity
          activeOpacity={0.8}
        ...
        >
          ...
          <View style={styles.viewMore}>
            <SpiralIcon
              type="ionicon"
              name={!isShowDetail ? 'chevron-down' : 'chevron-up'}
              size={16}
              color={appcolor.gray}
            />
          </View>
        </TouchableOpacity>
        <ProgramDetails isShowDetails={isShowDetail} item={item} />
        <Divider color={appcolor.gray} style={{ marginBottom: 8 }} />
        <View style={styles.createDate}>
          <Text style={styles.subValue}>{`Tạo bởi: ${item.createBy} - ${moment(
            item.createdDate,
          ).fromNow()}`}</Text>
          {item.confirm !== 3 && (
            <Text
              style={[styles.subValue, { color: appcolor[item.confirmColor] }]}
            >{`Xác nhận từ ${item.fullName} - ${moment(
              item.confirmDate,
            ).fromNow()}`}</Text>
          )}
          {item.confirmNote && (
    ...
    return (
      <View style={styles.itemContentVerify}>
        <Text
          style={[
            styles.subValue,
            { textDecorationLine: item.isDone === 1 ? 'line-through' : 'none' },
          ]}
        >
          {index + 1}. {item.name}
        </Text>
        {item.isDone !== undefined && (
          <SpiralIcon
            type="ionicon"
            name={item.isDone === 1 ? 'checkmark' : 'close'}
            size={16}
            color={item.isDone === 1 ? appcolor.success : appcolor.danger}
          />
        )}
      </View>
    );
  ...
  const renderItemGroup = ({ item }) => {
    ...
    const dataDetails = _.filter(
      data,
      e =>
        e.confirmStatus == item.confirmStatus && e.programId == item.programId,
    );
        ...
        <TouchableOpacity onPress={onPress}>
          <View style={styles.headerContent}>
            <View style={{ width: '95%' }}>
              ...
              <Text
                style={[styles.subValue, { fontSize: 12 }]}
              >{`Tổng ${dataDetails.length} cửa hàng tham gia: `}</Text>
            </View>
            <SpiralIcon
              type="ionicon"
              name={!isShowStore ? 'chevron-down' : 'chevron-up'}
              size={16}
              color={appcolor.gray}
            />
          </View>
          ...
          >
            <View style={styles.itemContentProgram}>
              <Text
                style={styles.subConfirmValue}
              >{`Loại chương trình: `}</Text>
            ...
            {item.targetAmount ? (
              <View style={styles.itemContentProgram}>
                ...
                <Text style={styles.value}>
                  {formatNumber(item.targetAmount, ',')} VNĐ
                </Text>
              </View>
            ) : null}
            {item.totalAwardValue ? (
              <View style={styles.itemContentProgram}>
                ...
                <Text style={styles.value}>
                  {formatNumber(item.totalAwardValue, ',')} VNĐ
                </Text>
              </View>
            ) : null}
            {item.awardName && (
  ...
  const renderTabItem = tabItem => {
    const dataByStatus = _.filter(
      data,
      e => e.confirmStatus == tabItem.confirmStatus,
    );
    ...
    <View style={styles.mainContainer}>
      ...
      <ActionSheet
        ...
        containerStyle={StyleSheet.flatten([
          [styles.actionSheetContainer, { backgroundColor: appcolor.light }],
