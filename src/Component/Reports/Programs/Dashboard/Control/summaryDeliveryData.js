...
export const applyDeliveryFilterSort = (
  payload,
  filterSort = DEFAULT_DELIVERY_FILTER_SORT,
) => {
  const rows = getDeliveryRows(payload).map((row, index) => {
    const distributorName = pickFirstText(row, [
      'dealerName',
      'DealerName',
      'distributorName',
      'DistributorName',
    ]);
    const distributorCode = pickFirstText(row, [
      'dealerCode',
      'DealerCode',
      'distributorCode',
      'DistributorCode',
...
export const getDeliveryMetricData = payload => {
  ...
  rows.forEach((row, index) => {
    ...
    const awardType = pickFirstText(row, [
      'awardType',
      'AwardType',
      'award_type',
      'typeAward',
    ...
    awardDetail.forEach(awardItem => {
      const awardName = pickFirstText(awardItem, [
        'name',
        'Name',
        'awardName',
        'AwardName',
  ...
  const totalAwardQty = rows.reduce((sum, row) => {
    ...
    const detailQty = awardDetail.reduce((awardSum, awardItem) => {
      const quantity = pickFirstNumber(awardItem, [
        'quantity',
        'Quantity',
        'qty',
        'Qty',
  ...
  const directShopTotal = pickMetricFromRows(rows, [
    'shopTotal',
    'ShopTotal',
    'totalStore',
    'TotalStore',
  ...
  const directPassShop = pickMetricFromRows(rows, [
    'metTargetCount',
    'MetTargetCount',
    'shopPass',
    'ShopPass',
    'l_delivery_shop_pass',
    'L_DELIVERY_SHOP_PASS',
  ]);
  const directFailShop = pickMetricFromRows(rows, [
    'notMetTargetCount',
    'NotMetTargetCount',
    'shopFail',
    'ShopFail',
  ...
  const directPassCount = pickMetricFromRows(rows, [
    'execPassCount',
    'ExecPassCount',
    'passCount',
    'PassCount',
    'l_delivery_pass_count',
    'L_DELIVERY_PASS_COUNT',
  ]);
  const directTurnCount = pickMetricFromRows(rows, [
    'execTotal',
    'ExecTotal',
    'turnCount',
    'TurnCount',
  ...
  const directAwardType = pickMetricFromRows(rows, [
    'awardTypeCount',
    'AwardTypeCount',
    'l_total_award_type',
    'L_TOTAL_AWARD_TYPE',
  ]);
  const directAwardQty = pickMetricFromRows(rows, [
    'totalAwardQty',
    'TotalAwardQty',
    'l_total_award_qty',
    'L_TOTAL_AWARD_QTY',
  ...
  const normalizedPassShop =
    directPassShop > 0 ? directPassShop : metTargetCount;
  const normalizedFailShop =
    directFailShop > 0
      ? directFailShop
      : Math.max(normalizedShopTotal - normalizedPassShop, 0);
  const normalizedExecTotal = directTurnCount > 0 ? directTurnCount : execTotal;
  const normalizedExecPassCount =
    directPassCount > 0 ? directPassCount : execPassCount;
  const normalizedExecFailCount = Math.max(
    normalizedExecTotal - normalizedExecPassCount,
    0,
  );
  const normalizedAwardTypeCount =
    directAwardType > 0 ? directAwardType : awardTypeCount;
  const normalizedAwardQty =
    directAwardQty > 0 ? directAwardQty : totalAwardQty;
  const normalizedPassRate =
    normalizedShopTotal > 0
      ? (normalizedPassShop / normalizedShopTotal) * 100
      : 0;
  const normalizedNotMetRate =
    normalizedShopTotal > 0
      ? (normalizedFailShop / normalizedShopTotal) * 100
      : 0;
  const normalizedExecPassRate =
    normalizedExecTotal > 0
      ? (normalizedExecPassCount / normalizedExecTotal) * 100
      : 0;
