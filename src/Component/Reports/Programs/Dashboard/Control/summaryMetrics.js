...
export const getMetricData = (payload) => {
    ...
    const registerPassKeys = [
        'registerPass', 'totalRegisterPass', 'registeredProgram', 'totalRegistered', 'daDangKy', 'dangKyDat'
    ]
    const registerFailKeys = [
        'registerFail', 'totalRegisterFail', 'unRegisteredProgram', 'totalUnregistered', 'chuaDangKy', 'dangKyRot'
    ]
    const registerTotalKeys = [
        'registerTotal', 'totalRegister', 'totalProgramRegister', 'tongDangKy', 'totalProgram'
    ]

    const billPassKeys = [
        'billPass', 'totalBillPass', 'invoicePass', 'totalInvoicePass', 'hoaDonDat', 'hoadonPass'
    ]
    const billFailKeys = [
        'billFail', 'totalBillFail', 'invoiceFail', 'totalInvoiceFail', 'hoaDonRot', 'hoadonFail'
    ]

    const resultPassKeys = [
        'resultPass', 'totalResultPass', 'programPass', 'totalProgramPass', 'cuaHangDat', 'statusResultPass'
    ]
    const resultFailKeys = [
        'resultFail', 'totalResultFail', 'programFail', 'totalProgramFail', 'cuaHangRot', 'statusResultFail'
    ]
    ...
    if (!billPass && !billFail) {
        billPass = countByPredicate(rows, (row) => {
            ...
            const statusName = (row?.billStatusName || row?.billConfirmName || '').toString().toLowerCase()
            return status === 1 || /pass|success|dat|duyet/.test(statusName)
        })
        billFail = countByPredicate(rows, (row) => {
            const status = Number(row?.billStatus)
            const statusName = (row?.billStatusName || row?.billConfirmName || '').toString().toLowerCase()
            return status === 0 || status === 2 || /fail|reject|rot|khongdat/.test(statusName)
        })
    }

    if (!registerPass && !registerFail) {
        registerPass = countByPredicate(rows, (row) => {
            const val = row?.confirmStatus || row?.registerStatus || row?.statusName || ''
    ...
    if (!registerTotal) {
        const uniqueProgram = new Set(rows.map((row) => row?.programId).filter((v) => v !== null && v !== undefined))
        registerTotal = uniqueProgram.size > 0 ? uniqueProgram.size : (registerPass + registerFail)
    }
...
export const getSummaryListData = (payload, sortBy = SUMMARY_SORT_OPTIONS.EMPLOYEE) => {
    const rows = getRows(payload)

    const list = rows.map((row, index) => {
        const employeeName = pickFirst(row, ['employeeName', 'staffName', 'fullName', 'userName', 'employee']) || 'Chưa có NV'
        const shopName = pickFirst(row, ['shopName', 'storeName', 'outletName', 'shop']) || 'Chưa có shop'
        const programName = pickFirst(row, ['programName', 'displayName', 'programTypeName', 'program']) || 'Chưa có chương trình'
        ...
        const status = pickFirst(row, ['confirmStatus', 'statusName', 'billStatusName', 'billConfirmName'])
    ...
    const sortMap = {
        [SUMMARY_SORT_OPTIONS.EMPLOYEE]: (item) => `${item.employeeName} ${item.shopName} ${item.programName}`,
        [SUMMARY_SORT_OPTIONS.SHOP]: (item) => `${item.shopName} ${item.employeeName} ${item.programName}`,
        [SUMMARY_SORT_OPTIONS.PROGRAM]: (item) => `${item.programName} ${item.shopName} ${item.employeeName}`,
    }

    const sorter = sortMap[sortBy] || sortMap[SUMMARY_SORT_OPTIONS.EMPLOYEE]

    return [...list].sort((a, b) => sorter(a).localeCompare(sorter(b), 'vi', { sensitivity: 'base' }))
}
