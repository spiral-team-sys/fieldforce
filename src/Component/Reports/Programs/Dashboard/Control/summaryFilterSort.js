...
const normalizeSummaryRow = (row, index) => {
  ...
  const shopName = pickFirst(row, [
    'shopName',
    'storeName',
    'outletName',
    'shop',
  ]);
  const programName = pickFirst(row, [
    'programName',
    'displayName',
    'programTypeName',
    'program',
  ...
  const statusText = pickFirst(row, [
    'confirmStatus',
    'statusName',
    'billStatusName',
    'billConfirmName',
...
const isPassRow = row => {
  ...
  return (
    statusResult === 1 ||
    billStatus === 1 ||
    /pass|success|dat|duyet/.test(statusText)
  );
};
...
const isFailRow = row => {
  ...
  return (
    ...
    billStatus === 0 ||
    billStatus === 2 ||
    /fail|reject|rot|khongdat/.test(statusText)
  );
};

export const applySummaryFilterSort = (
  payload,
  filterSort = DEFAULT_SUMMARY_FILTER_SORT,
) => {
  ...
  const sortMap = {
    [SUMMARY_SORT_OPTIONS.EMPLOYEE]: row =>
      `${row._employeeName} ${row._shopName} ${row._programName}`,
    [SUMMARY_SORT_OPTIONS.SHOP]: row =>
      `${row._shopName} ${row._employeeName} ${row._programName}`,
    [SUMMARY_SORT_OPTIONS.PROGRAM]: row =>
      `${row._programName} ${row._shopName} ${row._employeeName}`,
  };
