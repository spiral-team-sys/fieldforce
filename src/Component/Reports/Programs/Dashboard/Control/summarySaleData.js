...
export const getTypeDataRows = (payload, typeData = '') => {
    ...
    const normalizedTypes = types.map((item) => normalizeTypeData(item)).filter(Boolean)

    if (!normalizedTypes.length) return rows

    return rows.filter((row) => normalizedTypes.includes(normalizeTypeData(row?.typeData || row?.TypeData || row?.type || row?.Type)))
}
...
export const isSalePassRow = (row) => {
    ...
    const completionPercent = actualSale > 0 && targetSale > 0 ? (actualSale / targetSale) * 100 : 0
...
export const applySaleFilterSort = (payload, filterSort = DEFAULT_SALE_FILTER_SORT) => {
    ...
    const sortMap = {
        [SALE_SORT_OPTIONS.SHOP]: (row) => `${row._shopName} ${row._programName} ${row._distributorName}`,
        [SALE_SORT_OPTIONS.DISTRIBUTOR]: (row) => `${row._distributorName} ${row._shopName} ${row._programName}`,
        [SALE_SORT_OPTIONS.PROGRAM]: (row) => `${row._programName} ${row._shopName} ${row._distributorName}`,
    }
...
export const getTypeDataMetricData = (payload, typeData = 'SALE') => {
    ...
    const actualSaleTotal = shopList.reduce((sum, item) => sum + item.actualSale, 0)
    const targetSaleTotal = shopList.reduce((sum, item) => sum + item.targetSale, 0)
    ...
    return {
        ...
        completionRate: targetSaleTotal > 0 ? (actualSaleTotal / targetSaleTotal) * 100 : 0,
        actualSaleTotal,
        targetSaleTotal,
    }
}
