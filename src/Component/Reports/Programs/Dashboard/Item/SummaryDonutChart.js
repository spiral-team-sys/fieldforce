...
const SummaryDonutChart = ({ appcolor, segments = [], centerLabel = 'Tổng' }) => {
    const chartSize = 128
    ...
    const showSegments = total > 0 ? segments.filter((item) => (item?.value || 0) > 0) : []
    ...
    const styles = StyleSheet.create({
        chartWrap: { marginTop: 4, marginBottom: 6, flexDirection: 'row', alignItems: 'center' },
        donutHolder: { width: 140, height: 140, alignItems: 'center', justifyContent: 'center' },
        donutCenter: {
            position: 'absolute',
            alignItems: 'center',
        ...
        donutCenterLabel: { fontSize: 10, color: appcolor.placeholderText || appcolor.dark, fontWeight: '700', marginTop: 2 },
        ...
        legendPercent: { fontSize: 11, color: appcolor.placeholderText || appcolor.dark, fontWeight: '600', marginStart: 6 }
    })
        ...
        <View style={styles.chartWrap}>
            <View style={styles.donutHolder}>
                ...
                <View style={styles.donutCenter}>
                    <Text style={styles.donutCenterValue}>{renderMetricValue(total)}</Text>
                ...
                {segments.map((segment) => (
                    <View key={`legend-${segment.key}`} style={styles.legendRow}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <Text style={styles.legendValue}>{renderMetricValue(segment.value)}</Text>
                            <Text style={styles.legendPercent}>{`${getPercent(segment.value)}%`}</Text>
                        </View>
                    </View>
                ))}
            </View>
