...
const SummarySortedList = ({ appcolor, data = [], sortBy, onChangeSort }) => {
    ...
    const styles = StyleSheet.create({
        ...
        title: { fontSize: 14, fontWeight: '700', color: appcolor.primary, marginBottom: 10 },
        ...
        label: { fontSize: 11, color: appcolor.placeholderText || appcolor.dark, fontWeight: '600' },
        value: { fontSize: 12, color: appcolor.dark, fontWeight: '700', maxWidth: '72%', textAlign: 'right' },
        emptyText: { fontSize: 12, color: appcolor.placeholderText || appcolor.dark, textAlign: 'center', marginVertical: 10 }
    })
                ...
                data.map((item) => (
                    <View key={item.id} style={styles.itemRow}>
                        <View style={styles.line}>
                            ...
                            <Text style={styles.value}>{item.employeeCode ? `${item.employeeName} (${item.employeeCode})` : item.employeeName}</Text>
                        </View>
                        <View style={styles.line}>
                            <Text style={styles.label}>Shop</Text>
                            <Text style={styles.value}>{item.shopCode ? `${item.shopName} (${item.shopCode})` : item.shopName}</Text>
                        </View>
                        <View style={styles.line}>
                        ...
                        {item.status ?
                            <View style={[styles.line, { marginTop: 2 }]}>
                                ...
                                <Text style={[styles.value, { color: appcolor.primary }]}>{item.status}</Text>
                            </View>
                            : null}
                    </View>
                ))
