...
import { applySummaryFilterSort, DEFAULT_SUMMARY_FILTER_SORT } from "./Control/summaryFilterSort";
...
const SummaryProgramScreen = ({ navigation, route }) => {
    ...
    const buildApiParams = useCallback((filterInput = {}) => {
        const normalizeYMD = (value, fallback) => {
            if (value === null || value === undefined || value === '') return fallback
            const text = `${value}`.trim()
            if (/^\d{8}$/.test(text)) return text

            const parsed = moment(text, ['YYYY-MM-DD', 'YYYY/MM/DD', 'DD/MM/YYYY', 'YYYYMMDD'], true)
    ...
    const LoadData = useCallback(async (isPullRefresh = false) => {
        ...
        const routeFilter = route?.params?.params || route?.params?.filter || route?.params || {}
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <View style={styles.contentContainer}>
                <LoadingView isLoading={isLoading} title={'Đang cập nhật thống kê'} styles={{ paddingTop: 8 }} />
                {!isLoading &&
