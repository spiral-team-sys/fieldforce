...
import { getTypeDataMetricData, getTypeDataRows } from "../Control/summarySaleData";
import { getDisplayMetricData, getDisplayRows } from "../Control/summaryDisplayData";
import { getDeliveryMetricData, getDeliveryRows } from "../Control/summaryDeliveryData";
import SummarySaleCard from "../Item/SummarySaleCard";

const SummaryDashboardPage = ({ appcolor, metrics, rawData = [], filterSort, onChangeFilterSort, onCardPress, errorMessage, isRefreshing, onRefresh }) => {
    const failColor = appcolor.danger || appcolor.red || '#C62828'
    const filterSheetId = 'summary-filter-sort-sheet'

    const currentStatusLabel = filterSort?.status === 'PASS'
        ? 'Pass'
        : filterSort?.status === 'FAIL'
            ? 'Fail'
            : 'Tất cả'

    const currentSortLabel = filterSort?.sortBy === 'SHOP'
        ? 'Shop'
    ...
    const styles = StyleSheet.create({
        ...
        filterActionTitle: { fontSize: 14, fontWeight: '800', color: appcolor.dark },
        filterActionSubtitle: { fontSize: 11, fontWeight: '600', color: appcolor.placeholderText || appcolor.dark },
        ...
        filterMetaText: { fontSize: 11, fontWeight: '700', color: appcolor.primary },
    })
    ...
    const programMetrics = programRows.length > 0 ? getMetricData(programRows) : metrics
        ...
        <>
            ...
            >
                ...
                <TouchableOpacity style={styles.filterAction} onPress={openFilterSheet}>
                    <View style={styles.filterActionTop}>
                        <View>
                            ...
                            <Text style={styles.filterActionSubtitle}>Nhấn để thay đổi trạng thái, từ khóa và cách sắp xếp</Text>
                        </View>
                        <Icon type="ionicon" name="options-outline" color={appcolor.primary} size={20} />
                    </View>
                    <View style={styles.filterMetaRow}>
                        <View style={styles.filterMetaChip}>
                            <Text style={styles.filterMetaText}>{`Trạng thái: ${currentStatusLabel}`}</Text>
                        </View>
                        <View style={styles.filterMetaChip}>
                            <Text style={styles.filterMetaText}>{`Sắp xếp: ${currentSortLabel}`}</Text>
                        </View>
                        {filterSort?.keyword ?
                            <View style={styles.filterMetaChip}>
                                <Text style={styles.filterMetaText}>{`Từ khóa: ${filterSort.keyword}`}</Text>
                            </View>
                            : null}
                    </View>
                </TouchableOpacity>
                ...
                <SummaryProgramCard
                    ...
                    onPress={() => onCardPress?.(programRows.length > 0 ? programRows : rawData, 'PROGRAM')}
                />
