...
const UploadDeliverySlipScreen = ({ navigation, route }) => {
  ...
  const UploadData = async () => {
    ...
    alertConfirm(
      'Xác nhận',
      'Bạn có chắc chắn muốn gửi phiếu xuất không?',
      async () => {
        ...
        if (result.statusId == 200) {
          ...
          //
          const mapDataVerify = JSON.parse(data.verifyList || '[]').map(
            item => {
              if (item.id === 2) {
          ...
          const dataUpdate = {
            ...data,
            verifyList: JSON.stringify(mapDataVerify),
          };
          DeviceEventEmitter.emit(
            'UPDATE_LOCKED_DELIVERY_SLIP',
            { ...dataUpdate, isDoneDelivery: 1 },
            dataSend,
          );
          onBack();
        } else {
  ...
  const validData = () => {
    const photoList = dataPhoto.filter(
      it => it.photoType === `DELIVERY_SLIP_${data?.programId}`,
    );
    ...
    for (const item of selectedProducts) {
      if ((item.Price || 0) <= 0) {
        toastError(
          'Thông báo',
          `Sản phẩm "${item.ProductName}" có giá bằng 0, vui lòng kiểm tra lại`,
        );
        return false;
      }
      if (!item.Quantity || item.Quantity < 1) {
        toastError(
          'Thông báo',
          `Vui lòng nhập số lượng cho sản phẩm "${item.ProductName}"`,
        );
        return false;
      }
    }
  ...
  const handlerButtonCamera = () => {
    if (deleteMode.isDelete) {
      alertConfirm(
        'Xóa hình ảnh',
        `Bạn có chắc chắn muốn xóa ${deleteMode.dataDelete.length} hình ảnh không ?`,
        async () => {
  ...
  const onChangeQuantityForProduct = async (item, text) => {
    ...
    if (config.maxAmount) {
      const otherProductsAmount =
        summaryData.totalAmount - (item.Price || 0) * oldQuantity;
      ...
      if (newTotalAmount > config.maxAmount) {
        ...
        const maxQuantityAllowed = Math.floor(
          remainingAmount / (item.Price || 1),
        );

        toastError(
          'Thông báo',
          `Thành tiền không được vượt quá ${formatNumber(
            config.maxAmount,
            ',',
  ...
  const onSelectModel = async item => {
    ...
    if (config.maxAmount) {
      ...
      if (newAmount > config.maxAmount) {
        toastError(
          'Thông báo',
          `Không thể thêm sản phẩm. Thành tiền sẽ vượt quá ${formatNumber(
            config.maxAmount,
            ',',
          )} VNĐ`,
        );
  ...
  const onSearchData = text => {
    ...
    const dataSearch = _.filter(dataMain, product => {
      return (
        removeVietnameseTones(product.ProductName)
          .toLowerCase()
          .includes(valueSearch) ||
        removeVietnameseTones(product.ProductCode)
          .toLowerCase()
          .includes(valueSearch)
      );
    });
      ...
      <View key={index} style={styles.selectedItem}>
        <View style={styles.selectedItemInfo}>
          ...
          <Text style={styles.selectedItemPrice}>
            {formatNumber(item.Price || 0, ',')} VNĐ/sp
          </Text>
          <View style={styles.quantityContainer}>
            <Text style={styles.quantityLabel}>SL:</Text>
            <FormGroup
      ...
      <View style={styles.summaryContainer}>
        <View style={styles.summaryRow}>
          ...
          <Text style={styles.summaryValue}>
            {summaryData.productCount} sản phẩm
          </Text>
        </View>
        <View style={styles.summaryRow}>
        ...
        >
          <Text
            style={[
              styles.summaryLabel,
              { fontWeight: fontWeightBold, color: appcolor.dark },
            ]}
          >
            Tổng thành tiền:
          </Text>
          <Text style={styles.amountValue}>
            {formatNumber(summaryData.totalAmount, ',')} VNĐ
          </Text>
        </View>
      </View>
    );
            ...
            >
              <Text style={styles.selectButtonText}>
                {summaryData.productCount > 0
                  ? `Đã chọn ${summaryData.productCount} sản phẩm`
            ...
            <View style={styles.photoSection}>
              ...
              <View style={styles.photoRow}>
                <TouchableOpacity
                  ...
                  onPress={handlerButtonCamera}
                >
                  <View style={styles.captureIconWrap}>
                    <View style={styles.badgeContainer}>
                      <Text style={styles.badgeText}>
                        {deleteMode.isDelete
                          ? deleteMode.dataDelete.length
                          : dataPhoto?.length || 0}
                      </Text>
                    </View>
                    <SpiralIcon
                      name={deleteMode.isDelete ? 'trash' : 'camera'}
                      type={'ionicon'}
                      size={32}
                      color={appcolor.primary}
                    />
      ...
      >
        ...
        >
          ...
          <View style={styles.containerListProducts}>
            ...
            <CustomListView data={dataModal} renderItem={renderProductItem} />
          </View>
