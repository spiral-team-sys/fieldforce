...
import { checkLinkType, getStore, removeStore, saveStore } from '../../../../Core/Helper'
...
const EvaluateDetailScreen = ({ navigation, route }) => {
    ...
    const LoadData = async () => {
        const { dataItem, itemVerify } = route?.params
        await getDataPhotoByGUID(kpiinfo.id, shopinfo.shopId, dataItem.guid, setDataPhoto)
        const dataVerify = JSON.parse(dataItem?.verifyList || '[]')?.filter(it => it.id === itemVerify.id)
    ...
    const onSubmit = async () => {
        ...
        alertConfirm('Gửi đánh giá', 'Bạn có chắc chắn muốn gửi đánh giá không ?', async () => {
            await setLoading(true)
            const result = await REPORT.UploadDataRaw_Realtime(data, shopinfo, kpiinfo.id)
    ...
    const validData = async () => {
        if (formId == FORM_ID.REVIEW) {
            if (data.inputPassed == null || data.inputPassed == undefined) {
                toastError('Thông báo', 'Vui lòng chọn trạng thái đánh giá trưng bày trước khi gửi đánh giá')
                return false
            }
            if (data.inputNote == null || data.inputNote == undefined) {
                toastError('Thông báo', 'Vui lòng nhập ghi chú đánh giá trước khi gửi đánh giá')
                return false
            }
        ...
        for (const item of dataMain) {
            const photoList = dataPhoto.filter(it => it.photoType === `PROGRAM_REVIEW_${data.programId}_${item.id}`)
            if (photoList.length == 0) {
                toastError('Thông báo', `Vui lòng chụp hình trưng bày cho ${item.name} trước khi gửi đánh giá`)
                return false
            }
        }
        return true
    ...
    const handlerButtonCamera = (photoType, isDelete, photoList) => {
        ...
        if (isDelete) {
            alertConfirm('Xóa hình ảnh', `Bạn có chắc chắn muốn xóa ${dataDelete?.length} hình ảnh không ?`, async () => {
    ...
    const onCapture = async (photoType) => {
        ...
        await NativeCamera.cameraStart(templateInfo, async (result) => {
            if (result?.statusId === 200) {
                await getDataPhotoByGUID(kpiinfo.id, shopinfo.shopId, data.guid, setDataPhoto)
            } else {
        ...
        switch (formId) {
            case FORM_ID.REVIEW:
                return <ReviewDisplay
                    data={data}
                    onChangeData={onChangeReviewData}
                />
            default:
                return <View />
        }
    ...
    const styles = StyleSheet.create({
        ...
        sectionTitle: { fontSize: 13, fontWeight: fontWeightBold, color: appcolor.dark, fontStyle: 'italic', marginTop: 8 },
        photoRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 16, paddingVertical: 8 },
        ...
        photoItem: { width: 86, height: 86, marginRight: 8, borderRadius: 86, overflow: 'hidden', alignItems: 'center', justifyContent: 'center', backgroundColor: appcolor.surface, borderWidth: 0.5, borderColor: appcolor.grayLight },
    ...
    const renderItem = (item, index, photoList) => {
        ...
        return (
            <TouchableOpacity onLongPress={data.isLocked == 1 ? null : onLongPress} onPress={onPress} activeOpacity={0.8} style={styles.photoItem}>
                {item.isDelete && (
                    <View style={styles.deleteContainer}>
                        <Icon name='checkmark-circle' type='ionicon' size={24} color={appcolor.primary} />
                    </View>
                )}
                <Image source={{ uri: checkLinkType(item.photoPath) }} style={styles.photoImage} resizeMode="cover" />
            </TouchableOpacity>
        )
    }
        ...
        return (
            <View key={index} style={{ flex: 1 }}>
                ...
                <View style={styles.photoRow}>
                    <TouchableOpacity style={styles.captureCardHorizontal}
                        ...
                        <View style={styles.captureIconWrap}>
                            ...
                            <Icon name='camera' type='ionicon' size={32} color={appcolor.primary} />
                        </View>
                    </TouchableOpacity>
                    {isDelete && data.isLocked !== 1 &&
                        <TouchableOpacity style={styles.captureCardHorizontal}
                            onPress={onPress}>
                            <View style={styles.captureIconWrap}>
                                <Icon name='trash' type='ionicon' size={32} color={appcolor.primary} />
                            </View>
                        </TouchableOpacity>
                    }
                    <FlatList
                        horizontal
                        data={photoList}
                        keyExtractor={(photoItem, photoIndex) => `${photoItem?.guid || photoItem?.photoPath || 'photo'}_${photoIndex}`}
                        renderItem={({ item: photoItem, index: photoIndex }) => renderItem(photoItem, photoIndex, photoList)}
    ...
    if (loading) return <LoadingDefault isLoading={loading} title={'Đang gửi đánh giá...'} />

    return (
        <View style={styles.mainContainer}>
            <HeaderCustom title={data.programName}
