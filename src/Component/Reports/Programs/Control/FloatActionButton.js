...
export const FloatActionButton = ({ visible = true, info, showMenu, handlerChange }) => {
    const _fadeInDown = FadeInDown.duration(120).withInitialValues({ transform: [{ translateY: 120 }] })
    const _fadeOutDown = FadeOutDown.duration(120).withInitialValues({ transform: [{ translateY: 120 }] })
    // 
    useEffect(() => {
        return () => false
    }, [info])
    ...
    const styles = StyleSheet.create({
        mainContainer: { alignItems: 'flex-end', position: 'absolute', bottom: 24, end: 16, zIndex: 1000 },
