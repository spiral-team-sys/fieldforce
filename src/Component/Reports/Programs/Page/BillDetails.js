...
const BillDetails = ({ item }) => {
    ...
    <View style={styles.itemContainer}>
      ...
      >
        <View style={styles.columnTitle}>
          ...
          {item.fromDate && (
            <Text style={styles.titleName}>{`Trong kì CT: `}</Text>
          )}
  ...
  const renderProductHeader = (
    <View style={styles.viewHeader}>
      <Text
        style={[styles.titleNameTable, { textAlign: 'left', width: '55%' }]}
      >{`Sản phẩm`}</Text>
      ...
      <View style={styles.itemProductContainer}>
        <View style={styles.viewHeader}>
          <View style={{ width: '55%' }}>
            <Text
              style={[
                styles.valueNameTable,
                { textAlign: 'left', width: '100%', marginHorizontal: 0 },
              ]}
            >
              {`${item.detail_item_name} `}
              <SpiralIcon
                type="ionicon"
                name={
                  item.detail_status == 1 ? 'checkmark-circle' : 'close-circle'
                }
                size={13}
                color={appcolor[item.status_color]}
              />
            </Text>
            <Text
              style={[
            ...
            >{`${item.confirm_name} `}</Text>
          </View>
          <Text
            style={[styles.valueNameTable, { width: '10%' }]}
          >{`${item.detail_quantity}`}</Text>
          <Text style={styles.valueNameTable}>{`${formatNumber(
            item.detail_unit_price,
            '.',
          )}`}</Text>
          <Text style={styles.valueNameTable}>{`${formatNumber(
            item.detail_line_amount,
            '.',
          )}`}</Text>
        </View>
      </View>
    ...
    <>
      <View style={styles.viewFooter}>
        <Text
          style={[
            styles.titleName,
            { margin: 8, marginBottom: 0, fontSize: 14 },
          ]}
        >
          {`Tổng tiền: `}
          <Text
            style={[styles.subTitleValue, { fontSize: 16 }]}
          >{`${formatNumber(item.total_amount, '.')} VNĐ`}</Text>
        </Text>
        <Text
          style={[styles.titleName, { margin: 8, marginTop: 0, fontSize: 14 }]}
        >
          {`Tổng tiền hợp lệ: `}
          <Text
            style={[styles.subTitleValue, { fontSize: 16 }]}
          >{`${formatNumber(totalAmountApproved, '.')} VNĐ`}</Text>
        </Text>
      </View>
    </>
  );
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <Text
        style={[styles.subTitleName, { textAlign: 'center', marginBottom: 8 }]}
      >{`${item.guuid}`}</Text>
      <CustomListView
        data={['MAIN']}
