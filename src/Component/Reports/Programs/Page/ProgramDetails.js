...
const ProgramDetails = ({ item, isShowDetails = true }) => {
  ...
  const [uriDocument, setUriDocument] = useState({
    uri: null,
    pageName: null,
    isShow: false,
  });
  const productContent = (
    item?.models && item.models.trim().length > 0
      ? item.models
      : item?.modelList || ''
  ).replace(/,/g, '\n');
  //
  ...
  const onShowDocument = files => {
    const linkView =
      Platform.OS == 'android'
        ? `https://docs.google.com/gview?embedded=true&url=${URL_WEB}${files.link}`
        : `${URL_WEB}${files.link}`;
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <ScrollView
        style={[styles.cardContent, { height: 200 }]}
        nestedScrollEnabled
      >
