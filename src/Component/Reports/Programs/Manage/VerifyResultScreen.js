...
const VerifyResultScreen = ({ navigation }) => {
  ...
  const [itemViewFile, setItemViewFile] = useState({
    visible: false,
    uri: null,
    pageName: null,
  });
  ...
  const onShowFile = item => {
    const linkView =
      Platform.OS == 'android'
        ? `https://docs.google.com/gview?embedded=true&url=${item.url}`
        : `${item.url}`;
  ...
  const renderTab = item => {
    const dataDetail = _.filter(
      data,
      e => item.billStatusName == e.billStatusName,
    );
  ...
  const renderItem = ({ item }) => {
    ...
    const detailBill = (
      <CustomListView
        ...
        renderItem={info =>
          renderBillDetail({ item: info.item, itemMain: item })
        }
          ...
          <TouchableOpacity onPress={onPressShow} activeOpacity={0.8}>
            <View style={styles.viewItemHeader}>
              ...
              <View style={{ width: '84%' }}>
                ...
                >{`Mã CH: ${item.shopCode}`}</Text>
                <Text
                  style={styles.subTitleName}
                >{`ĐC: ${item.storeAddress}`}</Text>
                <Text
                  style={styles.subTitleName}
            ...
            <View style={styles.viewSummary}></View>
            <Divider />
          </TouchableOpacity>
        )}
        {/* Bill Info */}
            ...
            <View style={styles.itemContainer}>
              <View style={styles.viewItemStatus}>
                <TouchableOpacity onPress={onPressFile}>
                  ...
                  >{`Bấm để xem chi tiết File`}</Text>
                </TouchableOpacity>
                <View
                  style={[
                    styles.viewConfirmStatus,
                    { backgroundColor: appcolor[item.billConfirmColor] },
                  ]}
                >
                  <Text
                    style={[styles.subTitleName, { color: appcolor.white }]}
                  >{`${item.billConfirmName}`}</Text>
                </View>
              </View>
    ...
    //
    return item.billStatus == 1 ? itemSuccess : itemFail;
  };
      ...
      <TouchableOpacity onPress={onPressBill}>
        <View style={styles.viewBillInfo}>
          <View style={styles.itemInvoice}>
            <Text
              style={[styles.titleName, { marginBottom: 2 }]}
            >{`Thông tin hóa đơn`}</Text>
            <Text
              style={styles.subTitleValue}
            >{`Số HĐ: ${item.invoice_series}`}</Text>
            <Text
              style={styles.subTitleValue}
            >{`Ngày HĐ: ${item.invoice_issue_date}`}</Text>
            <Text
              style={styles.subTitleValue}
            >{`MST: ${item.buyer_tax_code}`}</Text>
            <Text style={styles.subTitleValue}>{`Tổng tiền: ${formatNumber(
              item.total_amount,
              '.',
            )} VNĐ`}</Text>
            <Text
              style={styles.subTitleValue}
            >{`Tổng tiền hợp lệ: ${formatNumber(
              total_amount_valid,
              '.',
            )} VNĐ`}</Text>
          </View>
          <View style={[styles.itemInvoice, { marginStart: 8 }]}>
            <Text
              style={[styles.titleName, { marginBottom: 2 }]}
            >{`Tổng SKU: ${item.total_product} sản phẩm`}</Text>
            <Text
              style={styles.subTitleValue}
            >{`Thuộc CT: ${item.total_product_accept}`}</Text>
            <Text
              style={styles.subTitleValue}
            >{`Không thuộc CT: ${item.total_product_reject}`}</Text>
            <Text
              style={styles.subTitleValue}
            >{`Không nhận dạng: ${item.total_product_none}`}</Text>
          </View>
        </View>
        <Text
          style={[styles.subTitleAction, { textAlign: 'right' }]}
        >{`Xem chi tiết hóa đơn -->`}</Text>
      </TouchableOpacity>
    );
  };
  return (
    <View style={styles.mainContainer}>
      <HeaderCustom
        ...
        leftFunc={onBack}
      />
      <SearchData placeholder="Tìm kiếm hóa đơn" onSearchData={onSearchData} />
