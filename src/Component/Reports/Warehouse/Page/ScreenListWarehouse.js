...
export const ScreenListWarehouse = ({ isReload }) => {
    ...
    const handlerSearchByGroup = async (item, keyValue, isMultiple) => {
        ...
        await setData(_dataByGroup !== null && _dataByGroup.length > 0 ? _dataByGroup : listChooseGroup)
    }
    ...
    const _searchData = (filterList) => {
        ...
        //
        const searchData = _.filter(filterByTag !== null && filterByTag.length > 0 ? filterByTag : filterList,
            (e) => (
    ...
    const styles = StyleSheet.create({
        ...
        titleGroupName: { width: '42%', fontWeight: '700', color: appcolor.primary, zIndex: 2, margin: 8, fontSize: 14 },
    ...
    const renderItem = ({ item, index }) => {
        ...
        return <ItemWarehouse key={`islw-${indexGroup.groupId}_${index}`} item={item} index={index} onChange={onChange} />
    }
    return (
                ...
                {dataTab !== undefined && dataTab !== null && dataTab.length > 0 &&
                    <Tabs.Container
                        ...
                        {dataTab.map((item, index) => {
                            const _dataWarehouse = _.filter(data, (e) => e[configPage.keyTab] == item[configPage.keyTab])
                            const titleHead = `${item[configPage.keyTabName]}${_dataWarehouse.length > 0 ? ` (${_dataWarehouse.length})` : ``}`
                            const isDataView = _dataWarehouse !== null && _dataWarehouse.length > 0 && item.DistrictName
                            return (
                                    ...
                                    <View style={styles.contentMain}>
                                        {isDataView &&
                                            <View style={styles.headViewMain}>
                                                ...
                                                <Text style={styles.titleGroupName}>{indexTab.subTitle || item.DistrictName}</Text>
                                            </View>
                                        }
                                        <FlashList
                                            ref={listRef}
