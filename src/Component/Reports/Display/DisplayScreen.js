...
import { checkRawReport, removeRawReport } from '../../../Controller/ReportController';
...
const DisplayScreen = ({ navigation }) => {
	const { appcolor, kpiinfo, shopinfo, isEdit } = useSelector((state) => state.GAppState);
	...
	const LoadDataDisplay = async (type) => {
		try {
			...
			if (parseStored?.isEdit) {
				...
				const itemFilter = { shopId: shopinfo.shopId, reportId: kpiinfo.id, formMode, typeReport: 'PRODUCT' };
				await DISPLAY.GetDataDisplayByShop(itemFilter, async (mData, message) => {
	...
	const onValidData = async () => {
		...
		const _isData = _.filter(dataUpload, (e) => (e.Display || e.Price) !== null || e.DisplayArea !== null);
		if (_isData.length === 0) {
	...
	const generateGroupedMessage = (data) => {
		...
		const result = Object.entries(groupData).map(([competitor, itemsByCompetitor]) => {
			const categories = _.groupBy(itemsByCompetitor, 'CategoryName');
			const categoryStrings = Object.entries(categories).map(([category, itemsByCategory]) => {
				const products = itemsByCategory.map(item => ` - ${item.ProductName}`).join('\n');
	...
	return (
		<View style={styles.mainContainer}>
			<HeaderCustom
				...
				disabled={isLoading}
			/>
			<ScreenChecking key={`checking`} navigation={navigation} onLockField={setLockField} />
			{/* // Product Field */}
