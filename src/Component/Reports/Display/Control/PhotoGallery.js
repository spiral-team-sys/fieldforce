...
import { DeviceEventEmitter, Modal, Platform, SafeAreaView, StyleSheet, TouchableOpacity, View } from "react-native";
...
export const PhotoGallery = ({ data, keyValue }) => {
    ...
    const onDeletePhoto = () => {
        if (!isRemoveImage) {
            let options = dataPhoto.length > 1 ?
                [{ text: 'Đóng' }, {
                    text: 'Xóa từng tấm', onPress: () => {
                        actionDeletePhoto('ONLY')
                    }
                }, { text: `Xóa tất cả ${dataPhoto.length} tấm`, onPress: () => { actionDeletePhoto('ALL') } }]
                :
                [{ text: 'Đóng' }, { text: `Đồng ý`, onPress: () => { actionDeletePhoto('ALL') } }]
            optionConfirm('Thông báo', `Bạn có muốn xóa hình ảnh ${itemHead.photoDesc} không ?`, options)
        } else {
    ...
    const styles = StyleSheet.create({
        mainContainer: { width: '100%', height: deviceHeight / (dataPhoto.length > 3 ? 1 : 1.6), backgroundColor: appcolor.light },
        itemMain: { margin: 8 },
        titleHead: { width: '60%', fontSize: 15, fontWeight: fontWeightBold, alignSelf: 'center', color: appcolor.primary },
        viewPhoto: { borderRadius: 5, overflow: 'hidden' },
        viewPosition: { width: 80, position: 'absolute', justifyContent: 'center', top: 0, end: 0, padding: 8, backgroundColor: appcolor.dark, opacity: 0.8, zIndex: 10, borderBottomStartRadius: 5 },
        titleTimeView: { fontSize: 13, fontWeight: fontWeightBold, color: appcolor.light, textAlign: 'center' },
        buttonCloseView: { padding: 8, paddingTop: 0 },
        contentAction: { padding: 8, width: '100%', flexDirection: 'row', justifyContent: 'flex-end', borderBottomWidth: 0.5, borderBottomColor: appcolor.grayLight, marginTop: 15 },
    ...
    const headeView = () => {
        return (
            <View style={styles.contentAction}>
                <Text style={styles.titleHead}>{`Hình ảnh ${itemHead.photoDesc} - Tổng ${dataPhoto.length} tấm`}</Text>
                <ButtonAction
                    type='CANCEL'
                    title='Đóng'
                    handlerPress={onCloseView} />
                <ButtonAction
                    type='REMOVE_IMAGE'
            ...
            <View key={`pg_item_${index}`} style={styles.itemMain}>
                <TouchableOpacity style={styles.viewPhoto} onPress={onPress}>
                    <View style={styles.viewPosition} >
                        <Text style={styles.titleTimeView}>{moment(item.photoFullTime).format('HH:mm:ss')}</Text>
                    </View>
                    <Image
                ...
                {isRemoveImage &&
                    <TouchableOpacity style={styles.actionRemoveView} onPress={onRemovePhoto}>
                        <View style={styles.viewOpacityRemove} />
                        <Icon type='ionicon' name="trash-bin" color={appcolor.red} size={28} />
                    </TouchableOpacity>
                }
            </View>
        )
