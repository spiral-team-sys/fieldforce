...
export const FloatActionField = forwardRef((props, ref) => {
    const { type, item, keyValue, index, onEditing, handlerEndEditing, bottom } = props
    ...
    const styles = StyleSheet.create({
        mainContainer: { width: '100%', padding: 8, position: 'absolute', bottom: bottom || (Platform.OS === 'ios' ? 96 : 72), backgroundColor: appcolor.light, zIndex: 10000 },
    ...
    return (
        typeAction !== null && typeAction.length > 0 &&
        <View key={`${type}_${index}_${itemUpdate.ProductId}`} style={styles.mainContainer}>
            <FormGroup
                inputRefFull={ref}
                editable
                defaultValue={(itemUpdate[keyValue] !== undefined && itemUpdate[keyValue] !== null) ? (itemUpdate[keyValue]).toString() : ''}
                placeholder={typeAction == "NOTE" ? `Ghi chú` : `${itemUpdate?.ProductName || ''}`}
