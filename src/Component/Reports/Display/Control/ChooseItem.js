...
const ChooseItem = ({ isShow = false, item, dataItem, handlerChange }) => {
  ...
  const handlerChoose = useCallback(
    itemChild => {
      const _value =
        item[item.keyValue] == itemChild.ItemName ? null : itemChild.ItemName;
  ...
  const renderItem = useCallback(
    (it, index) => {
      ...
      const backgroundColorChoose = isSelected
        ? appcolor.primary
        : appcolor.light;
      ...
      return (
        <View key={`ci-${index}`} style={styles.itemMain}>
          ...
          >
            <Text style={{ ...styles.titleName, color: colorChoose }}>
              {it.ItemName}
            </Text>
          </TouchableOpacity>
        </View>
