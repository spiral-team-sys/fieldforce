...
const InputItem = ({ isShow = false, item, dataItem, handlerChange }) => {
  ...
  const handlerChangeValue = useCallback(
    (text, itemChild) => {
      ...
      if (value == 0) {
        alertWarning(
          'Số lượng trưng bày theo vị trí phải lớn hơn 0, Nếu bằng 0 vui lòng chọn "Sản phẩm không trưng bày"',
        );
        return;
      }
      if (value >= 2) {
        alertWarning(
          'Số lượng trưng bày theo vị trí lớn hơn 2, Bạn có chắc nhập đúng không?',
        );
      }
      ...
      const summaryDisplay =
        validItems.length > 0 ? _.sumBy(validItems, 'ItemQuantity') : null;
  ...
  const styles = StyleSheet.create({
    ...
    titleNoDisplay: {
      fontSize: 13,
      color: item.Display == 0 ? appcolor.white : appcolor.black,
      fontWeight: '500',
    },
    buttonNoDisplay: {
      borderWidth: 1,
    ...
    containerNoDisplay: {
      backgroundColor: item.Display == 0 ? appcolor.redgray : appcolor.light,
      marginTop: 8,
      elevation: 3,
  ...
  const renderItem = useCallback(
    params => {
      ...
      const displayValue = `${
        item.ItemQuantity !== null && item.ItemQuantity !== undefined
          ? item.ItemQuantity
          : ''
      }`;
      return (
