...
export const HeaderAction = forwardRef((props, _ref) => {
    const { item, keyGroup, keyValue, keyName, isNote = false, isCapture = true, handlerNote, isShow = true, isEdit } = props
    ...
    const LoadData = async () => {
        try {
            const photos = await dataPhotoReport(shopinfo, kpiinfo.id, `${keyGroup}_${keyValue}`);
    ...
    // Handler   
    const onShowPhoto = () => {
        dataPhoto !== null && dataPhoto.length > 0 && SheetManager.show(`photodisplay_${keyValue}`)
    }
    const onActionNote = () => {
    ...
    useEffect(() => {
        ...
        if (!isMounted) return
        LoadData();
        const _reloadPhoto = DeviceEventEmitter.addListener('RELOAD_PHOTO_DISPLAY', LoadData);
        return () => {
            isMounted = false;
            _reloadPhoto.remove();
        };
    ...
    const styles = StyleSheet.create({
        mainContainer: { flexDirection: 'row', position: 'absolute', end: 8, alignSelf: 'center', justifyContent: 'center', zIndex: 100 },
        cameraView: { minWidth: 60, height: 32, alignItems: 'center', justifyContent: 'center', backgroundColor: appcolor.light, borderRadius: 50, marginStart: 8, zIndex: 5 },
        badgeLengthView: { backgroundColor: appcolor.light, elevation: 3, paddingHorizontal: 5, borderRadius: 20, position: 'absolute', top: -5, end: 0 },
        titleBadge: { color: appcolor.dark, padding: 3, fontSize: 12, fontWeight: fontWeightBold },
    })
        ...
        <View key={`${keyGroup}_${keyValue}`} style={styles.mainContainer}>
            {isCapture &&
                <TouchableOpacity style={styles.cameraView} onPress={onActionCamera}>
                    <Icon type="ionicon" name="camera" size={24} color={appcolor.primary} />
                </TouchableOpacity>
            }
            {isCapture &&
                <TouchableOpacity style={styles.cameraView} onPress={onShowPhoto}>
                    <Icon type="ionicon" name="images" size={20} color={appcolor.primary} />
            ...
            {isNote &&
                <TouchableOpacity style={styles.cameraView} onPress={onActionNote}>
                    <Icon type="ionicon" name="chatbubble" size={20} color={appcolor.primary} />
                </TouchableOpacity>
            }
            <ActionSheet id={`photodisplay_${keyValue}`} drawUnderStatusBar={Platform.OS == 'ios'}>
