...
export const GroupList = ({ dataMain = [], listValue = [], keyValue, keyName, handlerChange, handlerCloseTag, isMultiple = false }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    const refList = useRef()
    const dataGroup = useMemo(() => _.uniqBy(dataMain, keyValue), [dataMain, keyValue])
    const isCloseTag = useMemo(() => _.some(dataGroup, (e) => e.isChooseTag == 1), [dataGroup])
    const countAll = useMemo(() => _.countBy(dataMain, keyValue), [dataMain, keyValue])
    const countSelected = useMemo(() => _.countBy(listValue, keyValue), [listValue, keyValue])
    ...
    const renderItem = ({ item, index }) => {
        ...
        const groupCount = item.isChooseTag == 1 ? (countSelected[groupKey] || 0) : (countAll[groupKey] || 0)
        ...
        return (
            <TouchableOpacity key={`igi_${index}`} style={styleDefault} onPress={pressItem}>
                <View style={{ ...styles.viewSumValue, backgroundColor: bgColorView, borderColor: borderColorView }}>
                    <Text style={{ ...styles.titleSumValue, color: colorValue, paddingHorizontal: groupCount > 10 ? 8 : 0 }}>{groupCount}</Text>
                </View>
