...
import { ToastError, formatNumber, removeVietnameseTones } from "../../../../Core/Helper";
import { deviceHeight, deviceWidth, minWidthTab, optionConfirm } from "../../../../Core/Utility";
...
import { SET_IsStartDisplay } from "../../../../Redux/action";
...
const DisplayRow = React.memo(({
    ...
    const isEditPrice = inputType == 'PRICE' && inputProductId == item.ProductId
    const isEditDisplay = inputType == 'DISPLAY' && inputProductId == item.ProductId
    const priceValue = item.Price || null
    const displayValue = (item.Display !== undefined && item.Display !== null && item.Display !== '') ? item.Display : null
    const checkAreaValue = item.checkAreaValue || false
    const iconCheckArea = checkAreaValue ? 'checkbox' : 'square-outline'
    const infoWidth = item.isPrice == 1 ? '55%' : (item.isCheckArea == 1 ? '70%' : '75%')
    const resultAreaList = (item.DisplayAreaList || []).filter(e => e.ItemQuantity !== null && e.ItemQuantity !== undefined).map(e => `${e.ItemName}: ${e.ItemQuantity}`).join('\n')
    ...
    return (
        <View key={`it_${indexGroupId}_${index}`} style={styles.itemMain}>
            ...
            {item.isCheckArea == 1 &&
                <TouchableOpacity style={styles.actionCheckArea} disabled={!isEdit || !!inputIndex} onPress={onCheckArea}>
                    <Icon type='ionicon' name={iconCheckArea} size={21} color={checkAreaValue ? appcolor.primary : appcolor.greylight} />
                </TouchableOpacity>
            }
            <View style={{ ...styles.infoView, width: infoWidth }}>
                <Text style={styles.titleHead}>{`${index + 1}. ${item.ProductName}`}</Text>
                {item.ProductCode && <Text style={styles.titleContent}>{`Code: ${item.ProductCode}`}</Text>}
                {!menuIsOpenCamera &&
                    <View style={styles.contentChooseItem}>
                        {resultAreaList !== '' && <Text style={styles.displayAreaText}>{`${resultAreaList || ''}`}</Text>}
                        {item.isPosition == 1 && (displayValue || 0) > 0 && !configPage.isInputArea &&
                            <TouchableOpacity style={styles.displayAreaButton} disabled={!isEdit || !!inputIndex} onPress={() => onShowAction(item, index, 'jsonPosition', 'DisplayArea')}>
                                <Icon type="ionicon" name="cube" size={18} color={appcolor.placeholderText} />
                                <Text style={styles.displayAreaText}>{`Vị trí: ${item.DisplayArea || ''}`}</Text>
                            </TouchableOpacity>
                        }
                        {item.noteNoDisplay == 1 && displayValue == 0 &&
                            <TouchableOpacity style={styles.displayAreaButton} disabled={!isEdit || !!inputIndex} onPress={() => onShowAction(item, index, 'jsonReason', 'NoteProduct')}>
                                <Icon type="ionicon" name="chatbubble" size={18} color={appcolor.placeholderText} />
                                <Text style={styles.displayAreaText}>{`Ghi chú: ${item.NoteProduct || ''}`}</Text>
                            </TouchableOpacity>
                        }
                    </View>
                }
            ...
            {item.isDisplay == 1 && !menuIsOpenCamera &&
                <TouchableOpacity style={{ ...styles.inputView, width: item.isPrice == 1 ? '15%' : '25%' }} disabled={!isEdit || !!inputIndex} onPress={onInputDisplay}>
                    <Text style={styles.labelInput}>{configPage.isInputArea ? 'Tổng vị trí' : 'Số lượng'}</Text>
                    {displayValue !== null &&
                        <Text style={{ ...styles.valueInput, color: isEditDisplay ? appcolor.danger : appcolor.dark }}>{displayValue}</Text>
                    }
                </TouchableOpacity>
            }
            {item.isPrice == 1 && !menuIsOpenCamera &&
                <TouchableOpacity style={styles.inputView} onPress={onInputPrice}>
                    ...
                    {priceValue !== null &&
                        <Text style={{ ...styles.valueInput, color: isEditPrice ? appcolor.danger : appcolor.dark }}>{formatNumber(priceValue, ',')}</Text>
                    }
                </TouchableOpacity>
            }
        </View>
...
export const ScreenProductDisplay = ({ }) => {
    const { appcolor, shopinfo, kpiinfo, isEdit, isLoading, isStartDisplay } = useSelector(state => state.GAppState)
    ...
    const [menu, setMenu] = useState({ isOpenCamera: false, isOpen: false, type: null, title: null })
    const [indexGroup, setIndexGroup] = useState({ groupId: 0, groupName: null })
    const [indexTab, setIndexTab] = useState({ index: 0, tabId: 0, tabName: null, subTitle: null })
    const [itemChooseAction, setItemChooseAction] = useState({})
    const [checkClose, setCheckClose] = useState(false)
    const [itemDataChoose, setItemDataChoose] = useState({ jsonPosition: [], jsonReason: [] })
    ...
    //
    const persistData = useCallback((nextData, immediate = false) => {
        ...
        saveTimeoutRef.current = setTimeout(() => {
            if (latestDataRef.current) {
                saveJsonData(shopinfo.shopId, kpiinfo.id, shopinfo.auditDate, latestDataRef.current)
            }
            saveTimeoutRef.current = null
        }, 250)
    }, [kpiinfo.id, shopinfo.auditDate, shopinfo.shopId])

    const _searchData = useCallback((filterList, isDataInput = false, isDataNonInput = false, searchText = search.text) => {
        ...
        const searchData = _.filter(filterList, (e) => e.isChooseTag == 1 &&
            (
                ...
                removeVietnameseTones(e.CategoryCode).toLowerCase().match(valueSearch) ||
                removeVietnameseTones(e.CompetitorName).toLowerCase().match(valueSearch) ||
                removeVietnameseTones(e.SubCategoryCode).toLowerCase().match(valueSearch) ||
                removeVietnameseTones(e.SubCategoryName).toLowerCase().match(valueSearch)
            )
        )
        if (isDataInput) {
            return _.filter(searchData, (e) => (
                (e.Price !== undefined && e.Price !== null && e.Price !== '') ||
                (e.Display !== undefined && e.Display !== null && e.Display !== '' && e.Display > 0)
            ))
        }
        if (isDataNonInput) {
            return _.filter(searchData, (e) => (e.Display !== null && e.Display == 0 && e.Display !== ''))
        }
        return searchData
    }, [search.text])
    ...
    const LoadData = async () => {
        ...
        //
        const itemFilter = { reportId: kpiinfo.id, shopId: shopinfo.shopId, typeReport: 'PRODUCT' }
        //  
        if (!isEdit) {
            await removeRawReport(itemFilter.shopId, itemFilter.reportId)
        }
        //
        await REPORT.GetDataConfigReport(itemFilter, (mData) => { setItemDataChoose(mData[0] || {}) })
        await REPORT.GetDataReportByShop(itemFilter, async (mData, message) => {
            ...
            await setCheckClose(tabList?.find(item => item.CompetitorName === 'LG') !== undefined)
        })
    ...
    const handlerSearchByGroup = async (item, keyValue, isMultiple) => {
        try {
            ...
            // 
            const _productByGroup = _searchData(listChooseGroup, menu.type == 'SORT', menu.type == 'SORT_NONE')
            ...
            await setData(_productByGroup !== null && _productByGroup.length > 0 ? _productByGroup : listChooseGroup)
            await setCheckClose(_tabList?.find(item => item.CompetitorName === 'LG') !== undefined)
        } catch (e) {
            console.log(e);
        }
    }
    const handlerInputValue = useCallback((type, item, index) => {
        switch (type) {
            case 'AREA':
                ...
                const listUpdate = _searchData(updateData, menu.type == 'SORT', menu.type == 'SORT_NONE')
    ...
    const handlerEndEditing = () => {
        ...
        if (input.type === 'DISPLAY' && input.item.Display > 0) {
            const listPosition = _.filter(JSON.parse(itemDataChoose.jsonPosition || '[]'), (e) => (
                e.Ref_Id == input.item.CompetitorId || (input.item.CompetitorId !== _competitorId && e.Ref_Id !== _competitorId)
            ))
        ...
        if (input?.item?.noteNoDisplay == 1) {
            if (input.type == 'DISPLAY' && input.item.Display == 0 && input.item.Display != '') {
            ...
            if (input.type == 'DISPLAY' && input.item.Display >= 2) {
                alertConfirm(_item, input, index)
            }
        }
    }
    const handlerChangeFAB = async (type, titleAction) => {
        ...
        switch (type) {
            ...
            case "SORT":
                ...
                break
            case "DELETE":
                optionReset = [{ text: 'Hủy' }, { text: 'Xác nhận', onPress: () => onResetData('ONLY') }]
                optionConfirm(titleAction, `Dữ liệu ngành hàng ${indexTab.tabName} - ${indexGroup.groupName} sẽ được làm mới sau khi bạn "Xác nhận", Bạn có muốn xóa dữ liệu không ?`, optionReset)
                break
            case "RESET_DATA":
                optionConfirm(titleAction, 'Dữ liệu sẽ được làm mới sau khi bạn "Xác nhận", Bạn có muốn xóa tất cả dữ liệu không ?', optionReset)
                break
            case "CAMERA":
                //
                setMenu((prev) => ({
    ...
    const handlerChangePosition = async (itemUpdate) => {
        const isDisplayAreaList = configPage.isInputArea && itemChooseAction?.item?.keyValue == 'DisplayArea'
        console.log('[handlerChangePosition] isDisplayAreaList:', isDisplayAreaList)
        console.log('[handlerChangePosition] keyValue:', itemChooseAction?.item?.keyValue)
        console.log('[handlerChangePosition] itemUpdate.DisplayArea:', itemUpdate.DisplayArea)
        console.log('[handlerChangePosition] itemUpdate.NoteProduct:', itemUpdate.NoteProduct)
        console.log('[handlerChangePosition] itemUpdate.Display:', itemUpdate.Display)
        ...
        //
        const updatedItem = dataUpdate.find(e => e.ProductId == itemUpdate.ProductId)
        console.log('[handlerChangePosition] result DisplayArea:', updatedItem?.DisplayArea)
        console.log('[handlerChangePosition] result NoteProduct:', updatedItem?.NoteProduct)
        console.log('[handlerChangePosition] result Display:', updatedItem?.Display)
        ...
        if (isDisplayAreaList && itemUpdate.Display == 0) {
            ...
            setTimeout(() => { onChoosePosition(itemUpdate, 0, 'jsonReason', 'NoteProduct') }, 200)
        } else {
    ...
    // Aciton
    const alertConfirm = async (_item, input, index) => {
        Alert.alert('Thông báo', 'Số lượng trưng bày lớn hơn 1, bạn có muốn tiếp tục lưu?', [
            {
                text: 'Không',
                onPress: () => {
                    const updatedData = dataMain.map((dataItem, idx) => {
                        if (idx === index) {
                            return { ...dataItem, Display: null, NoteProduct: null, DisplayArea: null };
                        } else {
                            return dataItem;
                        }
                    });
    ...
    const onSearchData = (text) => {
        ...
        const listUpdate = _searchData(dataMain, menu.type == 'SORT', menu.type == 'SORT_NONE', text)
        setData(listUpdate)
        setDataGroup(listUpdate)
    }
    const onEditing = async (item) => {
        ...
        //
        if (nextInput.type == 'DISPLAY' && nextInput.item.Display == null && nextInput.item.Display !== 0) {
            ...
            const updateData = _.map(dataMain, (e) => e.ProductId === item.ProductId ? { ...e, ...item } : e)
            const dataInTabUpdate = _.map(data, (it) => it.ProductId === item.ProductId ? { ...it, ...item } : it)
            setDataMain(updateData)
            setData(dataInTabUpdate)
            await persistData(updateData)
        }
    ...
    const onActionMenuFAB = async () => {
        switch (menu.type) {
            case 'SORT_NONE':
                if (!menu.isOpen) {
                    await setLoading(true)
                    setMenu((prev) => ({ ...prev, isOpen: false, type: null, title: null }))
                    //
            ...
            case 'CAMERA':
                if (!menu.isOpen) {
                    setMenu({ isOpenCamera: false, isOpen: false, type: null, title: null })
                }
                break
            default:
    ...
    const onChoosePosition = useCallback((item, index, keyJson, keyValue) => {
        let listChoose = []
        if (keyJson == 'jsonPosition') {
            const dataJsonPosition = item.DisplayAreaList || JSON.parse(itemDataChoose.jsonPosition || '[]')
            listChoose = _.filter(dataJsonPosition, (e) => (
                e.Ref_Id == item.CompetitorId || (item.CompetitorId !== _competitorId && e.Ref_Id !== _competitorId)
            ))
        } else {
    ...
    const onResetData = async (type = 'ALL') => {
        ...
        const _listReset = _.map(dataMain, (e) => {
            if (type == 'ONLY') {
                if (e[configPage.keyGroup] == indexGroup.groupId && e[configPage.keyTab] == indexTab.tabId) {
    ...
    const onScroll = (e) => {
        ...
        try {
            ...
            const _dataInTab = _.filter(data, (e) => e.isChooseTag == 1 && e[configPage.keyTab] == _keyTab)
            //
    ...
    useEffect(() => {
        ...
        if (!isMounted) return
        //
        if (isStartDisplay && menu.type !== null && !isEdit) {
            onActionMenuFAB()
        }
        dispatch(SET_IsStartDisplay(false));
        return () => { isMounted = false }
    }, [isStartDisplay, menu.type])
    ...
    const styles = useMemo(() => StyleSheet.create({
        ...
        contentMain: { width: deviceWidth, height: deviceHeight, paddingTop: 40, zIndex: 1 },
        itemMain: { width: '100%', paddingVertical: 8, paddingHorizontal: 4, flexDirection: 'row', borderBottomWidth: 0.5, borderBottomColor: appcolor.grayLight, alignItems: 'center' },
        titleHead: { width: '100%', fontSize: 13, fontWeight: fontWeightBold, color: appcolor.dark, paddingLeft: 5 },
        titleContent: { width: '100%', fontSize: 13, fontWeight: '500', color: appcolor.greylight, paddingLeft: 5 },
        infoView: { width: '60%', },
        cameraView: { width: '20%', alignItems: 'center', alignSelf: 'center' },
        inputView: { width: '25%', backgroundColor: appcolor.light, marginEnd: 8, alignItems: 'center', alignSelf: 'center' },
        labelInput: { fontSize: 12, fontWeight: '500', color: appcolor.greylight, fontStyle: 'italic' },
        valueInput: { fontSize: 13, fontWeight: fontWeightBold, color: appcolor.greylight, fontStyle: 'italic' },
        ...
        headViewMain: { margin: 8, borderRadius: 3, borderBottomEndRadius: 20, overflow: 'hidden', flexDirection: 'row', alignItems: 'center' },
        subViewPosition: { width: '100%', height: '100%', backgroundColor: appcolor.primary, opacity: 0.1, position: 'absolute', zIndex: 1 },
        titleGroupName: { width: '42%', padding: 5, fontWeight: '700', color: appcolor.primary, zIndex: 2, margin: 8, fontSize: 14 },
        titleNote: { width: '100%', padding: 5, fontWeight: '500', color: appcolor.primary, fontSize: 13, fontStyle: 'italic', paddingHorizontal: 16 },
        loadingView: { position: 'absolute', top: 0, bottom: deviceWidth / 3, start: 0, end: 0, justifyContent: 'center', zIndex: 1000 },
        actionCheckArea: { padding: 10, paddingStart: 0, paddingVertical: 3 },
        selectedItem: { backgroundColor: appcolor.primary },
        displayAreaButton: { width: '100%', flexDirection: 'row', borderRadius: 5, alignItems: 'center', paddingHorizontal: 5, paddingTop: 8 },
        displayAreaText: { width: '95%', fontSize: 13, fontWeight: '600', color: appcolor.greylight, paddingStart: 5, },
        contentChooseItem: { width: '100%' },
        contentActionItem: { width: '100%' }
    }), [appcolor])
    const bottomViewStyle = useMemo(() => ({ paddingBottom: deviceHeight / (input.index !== null ? 1.5 : 2.8) }), [input.index])
    ...
    }), [dataTab.length, input.index, input?.item?.ProductId, input.type, menu.isOpenCamera])

    const dataByTab = useMemo(() => {
        return _.groupBy(_.filter(data, (e) => e.isChooseTag == 1), configPage.keyTab)
    ...
    }, [appcolor, configPage, handlerInputValue, indexGroup.groupId, input.index, input?.item?.ProductId, input.type, isEdit, menu.isOpenCamera, onChoosePosition, styles])
    if (loading || isLoading)
        return <ActivityIndicator style={styles.loadingView} color={appcolor.primary} />
    return (
        <View style={{ flex: 1 }}>
                    ...
                    <View style={styles.contentView}>
                        ...
                        {dataTab !== null && dataTab.length > 0 ?
                            <Tabs.Container
                                ...
                                {dataTab.length == 0 ? null : dataTab.map((item, index) => {
                                    const _dataProduct = dataByTab[item[configPage.keyTab]] || []
                                    const titleHead = `${item[configPage.keyTabName]} (${Array.isArray(_dataProduct) ? _dataProduct.length : 0})`;
                                    const isDataView = _dataProduct !== null && _dataProduct.length > 0
                                            ...
                                            <View style={styles.contentMain}>
                                                {isDataView && isEdit &&
                                                    <View style={styles.headViewMain}>
                                                        ...
                                                        <Text style={styles.titleGroupName}>{item.SubCategoryName}</Text>
                                                        <HeaderAction
                                                ...
                                                {isDataView && noteValue && <Text style={styles.titleNote}>{`Ghi chú: ${noteValue}`}</Text>}
                                                <FlashList
                                                    ref={listRef}
                                                    key={`${indexGroup.groupId}_${item[configPage.keyTabName]}_${index}`}
                                                    ...
                                                    ListFooterComponent={<View style={bottomViewStyle} />}
                        ...
                        {/* // */}
                        <ActionSheet id="action-item-sheet"
                            ...
                            <View style={styles.contentActionItem}>
                                {configPage.isInputArea && itemChooseAction?.item?.keyValue == 'DisplayArea' ?
                                    <InputItem
                                        isShow
            ...
            <FloatActionButton
                ...
                tabInfo={indexTab}
                containerStyle={{ bottom: deviceHeight / 7 }}
