...
const RequestUpdateScreen = ({ navigation }) => {
  ...
  const LoadData = async () => {
    ...
    await REPORT.GetDataReportByShop_RealTime(params, (mData, message) => {
      ...
      //
      const grouplist = _.unionBy(mData, 'weekNumber').map(item => {
        return {
          ...item,
          weekName: `Tuần ${item.weekNumber} ${item.timeWeek}`,
        };
      });
      setData(mData);
      setDataMain(mData);
  ...
  const validData = () => {
    const { item } = modalConfirm;
    if (
      item.reasonValue == null ||
      item.reasonValue == '' ||
      item.reasonValue == undefined
    ) {
      ...
      if (!isChoose) {
        setModalConfirm({
          ...modalConfirm,
          error: 'Vui lòng chọn lý do yêu cầu cập nhật',
        });
        return false;
      }
    }
    if (!note || note.trim() === '') {
      setModalConfirm({
        ...modalConfirm,
        error: 'Vui lòng nhập lý do yêu cầu cập nhật chi tiết',
      });
      return false;
    } else {
      if (note.trim().length < 10) {
        setModalConfirm({
          ...modalConfirm,
          error: 'Lý do yêu cầu cập nhật chi tiết phải có ít nhất 10 ký tự',
        });
        return false;
      }
    }
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
                ...
                dataReason.map((reason, index) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.actionReason,
                      {
                        backgroundColor: reason.isChoose
                          ? appcolor.primary
                          : appcolor.surface,
                      },
                    ]}
                    onPress={() => onChooseReason(item, reason)}
                  >
                    <Text
                      style={[
                        styles.subTitleName,
                        {
                          fontWeight: fontWeightBold,
                          color: reason.isChoose
                            ? appcolor.light
                            : appcolor.placeholderText,
                        },
                      ]}
                    >{`${reason.ItemName}`}</Text>
                  </TouchableOpacity>
                ))
              ) : (
            ...
            <View style={styles.modalActions}>
              ...
              >
                <Text
                  style={[styles.modalBtnText, { color: appcolor.greylight }]}
                >
                  Hủy
                </Text>
              </TouchableOpacity>
              ...
              >
                {isSubmitting ? (
                  ...
                  <Text
                    style={[styles.modalBtnText, { color: appcolor.light }]}
                  >
                    Gửi yêu cầu
                  </Text>
                )}
  ...
  const renderItem = ({ item }) => {
    return (
      ...
      >
        {item.confirmStatus && (
          <Text
            style={[
              styles.subTitleName,
              {
                color: getStatusColor(item.confirm),
                fontWeight: fontWeightBold,
              },
            ]}
          >{`${item.confirmStatus}`}</Text>
        )}
        {/* // Shop Info */}
        <Text
        ...
        >{`Nhân viên thực hiện: ${item.employeeName}`}</Text>
        <Text
          style={styles.subTitleName}
        >{`Code NV: ${item.employeeCode}`}</Text>
        {item.reasonValue && (
          <Text style={styles.subTitleName}>{`${item.reasonValue}`}</Text>
        )}
        {item.requestNote && (
          <Text
            style={styles.subTitleName}
        ...
        {item.confirmByName && (
          <>
            ...
            <Text style={styles.subTitleName}>
              {moment(item.confirmDate).fromNow()}
            </Text>
          </>
        )}
        {/* // Time */}
  ...
  return (
    <View style={styles.mainContainer}>
      <HeaderCustom
        ...
        leftFunc={onBack}
      />
      <SearchData placeholder="Tìm kiếm cửa hàng" onSearchData={onSearchData} />
