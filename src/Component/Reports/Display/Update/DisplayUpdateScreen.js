...
import { alertConfirm, alertNotify, deviceHeight, deviceWidth, minWidthTab, optionConfirm } from "../../../../Core/Utility";
import { FloatActionField } from "../Control/FloatActionField";
import { checkRawReport, removeRawReport, saveJsonData } from "../../../../Controller/ReportController";
...
const DisplayRow = React.memo(({
    ...
    return (
        <View key={`it_${indexGroupId}_${index}`} style={styles.itemMain}>
            ...
            {item.isCheckArea == 1 &&
                <TouchableOpacity style={styles.actionCheckArea} disabled={!!inputIndex} onPress={onCheckArea}>
                ...
                {!menuIsOpenCamera &&
                    <View style={styles.contentChooseItem}>
                        ...
                        {item.isPosition == 1 && (displayValue || 0) > 0 && !configPage.isInputArea &&
                            <TouchableOpacity style={styles.displayAreaButton} disabled={!!inputIndex} onPress={() => onShowAction(item, index, 'jsonPosition', 'DisplayArea')}>
                        ...
                        {item.noteNoDisplay == 1 && displayValue == 0 &&
                            <TouchableOpacity style={styles.displayAreaButton} disabled={!!inputIndex} onPress={() => onShowAction(item, index, 'jsonReason', 'NoteProduct')}>
            ...
            {item.isDisplay == 1 && !menuIsOpenCamera &&
                <TouchableOpacity style={{ ...styles.inputView, width: item.isPrice == 1 ? '15%' : '25%' }} disabled={!!inputIndex} onPress={onInputDisplay}>
...
const DisplayUpdateScreen = ({ navigation, route }) => {
    const { appcolor, shopinfo, kpiinfo, isLoading } = useSelector(state => state.GAppState)
    ...
    const LoadData = async () => {
        ...
        await REPORT.GetDataReportByShop(itemFilter, async (mData, message) => {
            ...
            if (mData == null || mData.length == 0) {
                ...
                optionConfirm('Thông báo', 'Dữ liệu cập nhật trống, Vui lòng kiểm tra lại các yêu cầu cập nhật', options)
            } else {
    ...
    const UploadData = async () => {
        ...
        if (!isValid) return;
        //
        alertConfirm('Gửi dữ liệu', message || 'Sau khi gửi dữ liệu bạn sẽ thoát chế độ "Chỉnh sửa", Bạn có chắc chắn không ?', async () => {
            await setLoading(true);
    ...
    const onValidData = async () => {
        ...
        const _isData = _.filter(dataUpload, (e) => (e.Display || e.Price) !== null || (e.DisplayArea !== null && e.DisplayArea !== ''));
        if (_isData.length === 0) {
            toastError('Dữ liệu trống', 'Vui lòng nhập dữ liệu đầy đủ trước khi gửi dữ liệu lên hệ thống');
            return false;
        }
        // Check Full Data 
        let checkData = {
        ...
        if (dataUpload !== null && dataUpload.length > 0) {
            for (let index = 0; index < dataUpload.length; index++) {
                ...
                if (configPage.isInputArea) {
                    ...
                    const checkListArea = _.filter(dataAreaList, (e) => e.ItemQuantity !== null && e.ItemQuantity !== undefined)
                    if (item.Display > 0 && checkListArea.length == 0) {
                        if (configPage?.checkByCompetitor) {
                            const isIncluded = configPage?.checkByCompetitor.split(',').includes(item.CompetitorName);
                            if (isIncluded)
                    ...
                    if (item.Display == 0) {
                        if (item.CompetitorName == 'LG' && (item.NoteProduct || null) == null) {
                            checkData.displayNoteEmpty.push(item)
                        }
                    } else {
                        const checkAreaValues = _.filter(dataAreaList, (e) => e.ItemQuantity >= 2)
                    ...
                    if ((item.Display || null) !== null) {
                        if (item.Display > 0) {
                            if (item.CompetitorName == 'LG' && (item.DisplayArea == null || item.DisplayArea === '')) {
        ...
        if (configPage.isInputArea) {
            ...
            if (checkData.displayAreaValues.length > 0) {
                return { isValid: true, message: `Chú ý: Dữ liệu trưng bày có sản phẩm nhập vị trí lớn hơn 2 sản phẩm.\n\nSau khi gửi dữ liệu bạn sẽ thoát chế độ "Chỉnh sửa", Bạn có muốn gửi dữ liệu không?` }
            }
        } else {
            ...
            if (checkData.displayValues.length > 0) {
                return { isValid: true, message: `Dữ liệu trưng bày có sản phẩm trưng bày lớn hơn 2 sản phẩm, Bạn có muốn gửi dữ liệu không?` }
            }
        }
        //
    ...
    const styles = useMemo(() => StyleSheet.create({
        ...
        loadingView: { position: 'absolute', top: 0, bottom: 0, start: 0, end: 0, justifyContent: 'center', zIndex: 1000 },
    ...
    }, [appcolor, configPage, handlerInputValue, indexGroup.groupId, input.index, input?.item?.ProductId, input.type, menu.isOpenCamera, onChoosePosition, styles])
