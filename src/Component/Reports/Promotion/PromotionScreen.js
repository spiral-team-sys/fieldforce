...
import { getPhotosReportByGuiId, PromotionGetList } from "../../../Controller/WorkController";
...
const PromotionScreen = ({ navigation }) => {
    const { workinfo, kpiinfo, appcolor, shopinfo } = useSelector(state => state.GAppState)
    ...
    const mapItem = useCallback(async (item) => {
        const lst = await getPhotosReportByGuiId(kpiinfo.kpiId, item.guiId, workinfo.shopId, workinfo.workDate);
        setSellouts(prevSellouts => prevSellouts.map(itemS => (
            itemS.guiId === item.guiId ? { ...itemS, numPhoto: lst.length } : itemS
        )))
    ...
    const upload = async () => {
        ...
        if (res != null && res.length > 0) {
            alertConfirm("Gửi báo cáo", "Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không?", async () => {
                ...
                await UploadController.DataPromotion(workTem, async () => {
    ...
    const styles = StyleSheet.create({
        ...
        progress: { position: 'absolute', alignSelf: "center", marginTop: Dimensions.get('window').height / 2 },
        addButton: { position: 'absolute', zIndex: 10, right: 20, bottom: 40, maxHeight: 40 }
    })
        ...
        <View style={styles.mainContainer}>
            <View style={styles.listContainer}>
                <HeaderCustom
                    ...
                    leftFunc={onBack}
                    rightFunc={() => !isLockReport ? (status ? null : upload()) : ToastSuccess('Bạn đã hoàn thành chấm công nên không thể gửi dữ liệu báo cáo')}
                />
                ...
                {
                    showProgress === true && <Progress.Circle color={DEFAULT_COLOR} thickness={5} size={90} indeterminate={true} style={styles.progress} />
                }
                <Icon
