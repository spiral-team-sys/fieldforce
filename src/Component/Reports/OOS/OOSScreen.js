...
import { checkRawReport, getDataPhotoByReport, removeRawReport } from "../../../Controller/ReportController";
...
const OOSScreen = ({ navigation }) => {
    ...
    const onValidData = async () => {
        ...
        const _isData = _.filter(dataUpload, (e) => e.OOS !== undefined && e.OOS !== null)
        // 
        if (_isData == null || _isData.length == 0) {
            ...
            const photoReport = await getDataPhotoByReport(kpiinfo.id, shopinfo.shopId)
            ...
            for (let index = 0; index < _isData.length; index++) {
                ...
                if (e.OOS == 1) {
                    ...
                    const _checkPhoto = _.filter(photoReport, (it) => it.photoType == _photoType)
                    if (_checkPhoto.length < (e.photoLimit || 0)) {
                        strError += `Chụp hình sản phẩm ${e.ProductName} (${_checkPhoto.length}/${e.photoLimit || 0})\n`
                    }
                }
            }
