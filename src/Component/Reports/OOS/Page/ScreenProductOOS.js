...
export const ScreenProductOOS = ({ isActionType }) => {
    ...
    const [configPage, setConfigPage] = useState({ keyGroup: "CompetitorId", keyGroupName: "CompetitorName", keyTab: "CategoryId", keyTabName: "CategoryName", keySubTabName: "SubCategoryName" })
    ...
    const onCheckAllOOS = async () => {
        closeMenu()
        //
        const _dataUpdate = _.map(dataMain, (e) => { return { ...e, OOS: 0 } })
    ...
    const styles = StyleSheet.create({
        ...
        titleHead: { width: '100%', fontSize: 14, fontWeight: fontWeightBold, color: appcolor.dark },
        titleContent: { width: '100%', fontSize: 13, fontWeight: '500', color: appcolor.greylight },
        infoView: { width: '50%', },
        ...
        bottomView: { paddingBottom: deviceHeight / (input.index !== null ? 1.5 : 2.8) },
        overflowView: { position: 'absolute', top: 0, bottom: 0, start: 0, end: 0, zIndex: 900, elevation: 10, backgroundColor: appcolor.dark, opacity: 0.65, justifyContent: 'center' },
        actionContainer: { position: 'absolute', top: 0, bottom: 0, start: 0, end: 0, zIndex: 2000, elevation: 50 },
        ...
        titleGroupName: { width: '42%', padding: 5, fontWeight: '700', color: appcolor.primary, zIndex: 2, margin: 5, fontSize: 14 },
        titleNote: { width: '100%', padding: 5, fontWeight: '500', color: appcolor.primary, fontSize: 13, fontStyle: 'italic', paddingHorizontal: 16 },
        loadingView: { position: 'absolute', top: 56, start: 0, end: 0, justifyContent: 'center', zIndex: 1000 },
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <View style={styles.contentView}>
                ...
                {isLoading && <ActivityIndicator style={styles.loadingView} color={appcolor.primary} />}
                {/* // Content Products */}
                {dataTab !== undefined && dataTab !== null && dataTab.length > 0 &&
                    <Tabs.Container
                        ...
                        {dataTab.map((item, index) => {
                            const _dataProduct = _.filter(data, (e) => e.isChooseTag == 1 && e[configPage.keyTab] == item[configPage.keyTab])
                            const titleHead = `${item[configPage.keyTabName]}${_dataProduct.length > 0 ? ` (${_dataProduct.length})` : ``}`
                            const isDataView = _dataProduct !== null && _dataProduct.length > 0
                            const noteValue = item.Note || null
                            return (
                                <Tabs.Tab key={`tabioos_${index}`} label={titleHead} name={titleHead} >
                                    <View style={styles.contentMain}>
                                        {isDataView &&
                                            <View style={styles.headViewMain}>
                                                <View style={styles.subViewPosition} />
                                                <Text style={styles.titleGroupName}>{indexTab.subTitle || item.SubCategoryName}</Text>
                                            </View>
                                        }
            ...
            <View pointerEvents='box-none' style={styles.actionContainer}>
                ...
                <FloatActionButton
                    ...
                    containerStyle={{ bottom: deviceHeight / (isActionType ? 4.5 : 7), zIndex: 1000, elevation: 60 }}
                />
            </View>
        </View>
    )
