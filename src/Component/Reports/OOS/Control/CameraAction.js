...
export const CameraAction = forwardRef((props, _ref) => {
    ...
    const onActionCamera = () => {
        const options = [{ text: "Đóng" }, { text: "Chụp hình", onPress: handlerCameraAction }, { text: "Thư viện", onPress: handlerGalleryAction }]
        optionConfirm('', `Hình ảnh ${keyName || ''}`, options)
    }
    const onShowPhoto = () => {
        dataPhoto !== null && dataPhoto.length > 0 && SheetManager.show(`photooos_${keyValue}`)
    }
    ...
    useEffect(() => {
        ...
        if (!isMounted)
            return
        LoadData();
        const _reloadPhoto = DeviceEventEmitter.addListener('RELOAD_PHOTO_OOS', LoadData);
        return () => {
            isMounted = false;
            _reloadPhoto.remove();
        };
    ...
    const styles = StyleSheet.create({
        mainContainer: { width: 100, flexDirection: 'row', alignSelf: 'center', justifyContent: 'center', zIndex: 100, marginEnd: 10 },
        cameraView: { width: 50, height: 32, alignItems: 'center', justifyContent: 'center', zIndex: 5, marginEnd: 0 },
        badgeLengthView: { backgroundColor: appcolor.light, elevation: 3, shadowColor: appcolor.dark, shadowOffset: { width: 3, height: 0 }, shadowOpacity: 0.3, elevation: 3, paddingHorizontal: 5, borderRadius: 20, position: 'absolute', top: -5, end: 0 },
        ...
        enable ?
            <View key={`${keyGroup}_${keyValue}`} style={styles.mainContainer}>
                <TouchableOpacity style={styles.cameraView} onPress={onShowPhoto}>
                    <Icon type="ionicon" name="images" size={21} color={appcolor.greylight} />
                ...
                <TouchableOpacity style={styles.cameraView} onPress={onActionCamera}>
                    <Icon type="ionicon" name="camera" size={24} color={appcolor.greylight} />
                </TouchableOpacity>
                <ActionSheet id={`photooos_${keyValue}`} drawUnderStatusBar={Platform.OS == 'ios'}>
