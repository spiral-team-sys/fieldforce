...
export const PhotoGallery = ({ data, keyValue }) => {
    ...
    const onCloseView = () => {
        SheetManager.hide(`photooos_${keyValue}`)
    }
    const onDeletePhoto = () => {
        if (!isRemoveImage) {
            let options = dataPhoto.length > 1 ?
                [{ text: 'Đóng' }, { text: 'Xóa từng tấm', onPress: () => { actionDeletePhoto('ONLY') } }, { text: `Xóa tất cả ${dataPhoto.length} tấm`, onPress: () => { actionDeletePhoto('ALL') } }]
                :
    ...
    const actionDeletePhoto = async (type) => {
        if (type == 'ALL') {
            ...
            await DeviceEventEmitter.emit('RELOAD_PHOTO_OOS')
        }
        if (type == 'ONLY')
            setRemoveImage(true)
    }
    ...
    const headeView = () => {
        return (
            <View style={styles.contentAction}>
                <Text style={styles.titleHead}>{`${itemHead.photoDesc} - Tổng ${dataPhoto.length} tấm`}</Text>
                <ButtonAction
                    type='CANCEL'
                    title='Đóng'
