...
export const ActionItem = ({ isMain = false, typeAction, title, iconName, onPress }) => {
    ...
    const styles = StyleSheet.create({
        ...
        titleName: { fontSize: 13, color: isMain ? appcolor.red : appcolor.dark, fontWeight: Platform.OS == 'android' ? '700' : '600' }
    })
    return (
        <View style={styles.mainContainer}>
            ...
            <TouchableOpacity style={styles.viewActionMain} onPress={onPressMenu}>
                <Icon type='ionicon' name={iconName || ''} size={24} color={isMain && title ? appcolor.red : appcolor.primary} />
            </TouchableOpacity>
        </View>
    )
}