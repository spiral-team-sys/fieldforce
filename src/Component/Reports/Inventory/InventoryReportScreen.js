...
const InventoryReportScreen = ({ navigation }) => {
  ...
  const bindDataReport = async (dataReport = [], itemUpdate = {}) => {
    ...
    await setNote(
      dataReport[0]?.InventoryComment || dataReport[0]?.displayComment || '',
    );
  ...
  const validData = async () => {
    ...
    const invalidItems = dataMain.filter(item => {
      ...
      return (
        ...
        value !== undefined &&
        value !== '' &&
        (isNotInteger(value) || parseInt(value) < 0)
      );
    });
    ...
    const itemsUpload = dataMain.filter(
      item =>
    ...
    if (itemsUpload.length === 0) {
      ToastError(
        'Vui lòng nhập dữ liệu trước khi gửi dữ liệu lên hệ thống',
        'Dữ liệu trống',
        'top',
      );
      return false;
  ...
  const onChangeQuantity = async (value, item) => {
    const quantityValue =
      value !== null && value !== undefined && value !== ''
        ...
        : null;
    const nextValue =
      quantityValue !== null && !isNotInteger(quantityValue)
        ? parseInt(quantityValue)
        : quantityValue;
    const dataUpdate = _.map(dataMain, e => {
      const isCurrentItem =
        `${e.GroupName || ''}_${e.ProductId || e.ProductCode}` ==
        `${item.GroupName || ''}_${item.ProductId || item.ProductCode}`;
    ...
    await setDataGroup(
      _.unionBy(search.text?.length > 0 ? dataUpdate : dataFilter, 'GroupName'),
    );
    await saveJsonData(
      shopinfo.shopId,
      reportId,
  ...
  const handleEndChangeNote = async () => {
    ...
    await setData(
      search.text?.length > 0
        ? _searchData(search.text, dataUpdate)
        : dataUpdate,
    );
    await saveJsonData(
      shopinfo.shopId,
  ...
  const styles = StyleSheet.create({
    ...
    mainView: { flex: 1 },
    tabContainer: { width: '100%', height: '100%', padding: 8 },
    itemContainer: {
      width: '100%',
      flexDirection: 'row',
      alignItems: 'center',
      ...
      <View style={styles.itemContainer}>
        ...
        <View style={styles.viewInput}>
          <FormGroup
            ...
            containerStyle={[
              styles.inputMain,
              (isUploaded || isLockReport) && styles.inputDisabled,
            ]}
      ...
      >
        <View style={styles.sheetContainer}>
          ...
          <Text style={styles.sheetSubTitle}>
            Quản lý nhanh dữ liệu tồn kho trong báo cáo này
          </Text>
