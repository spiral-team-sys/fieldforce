...
const getDisplayScore = (value) => {
    ...
    if (Array.isArray(value)) return value.map(e => `${readField(e, ['name', 'criteriaName', 'categoryName'])}: ${readField(e, ['score', 'value', 'point'])}`).join(' | ')
...
const EmployeeItem = ({ item, index, isSelected, onPress }) => {
    ...
    const styles = StyleSheet.create({
        itemContainer: { width: '100%', marginBottom: 8, padding: 8, borderRadius: 8, backgroundColor: appcolor.white, borderWidth: 0.5, borderColor: appcolor.grayLight },
        titleName: { fontSize: 13, fontWeight: fontWeightBold, color: appcolor.dark },
        viewActionPress: { width: '100%' },
        employeeMainInfo: { marginTop: 4 },
        employeeMainInfoText: { fontSize: 11, color: appcolor.placeholderText, marginBottom: 2 },
        ...
        pcValue: { fontSize: 12, color: appcolor.dark, fontWeight: '500', marginBottom: 4 },
        selectedRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
        summaryCard: { marginTop: 6, padding: 8, backgroundColor: appcolor.surface, borderRadius: 8 },
        summaryText: { fontSize: 11, color: appcolor.dark, marginBottom: 2 },
    })
    const history = item.history || {}
    return (
        <View style={styles.itemContainer}>
            <TouchableOpacity style={styles.viewActionPress} onPress={() => onPress(item)}>
                <View style={styles.selectedRow}>
                    <Text style={styles.titleName}>{`${index + 1}. ${item.employeeName}`}</Text>
                    <Icon type='ionicon' name={isSelected ? 'checkmark-circle' : 'ellipse-outline'} size={20} color={isSelected ? appcolor.primary : appcolor.placeholderText} />
                </View>
                <View style={styles.employeeMainInfo}>
                    <Text style={styles.employeeMainInfoText}>{`Mã NV: ${item.employeeCode}`}</Text>
                </View>
            </TouchableOpacity>
            <View style={styles.pcInfoBox}>
                ...
                <Text style={styles.pcValue}>{`Khu vực: ${item.area} | Tỉnh: ${item.province}`}</Text>
