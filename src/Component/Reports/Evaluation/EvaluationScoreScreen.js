...
const EvaluationScoreScreen = ({ navigation }) => {
    ...
    const UploadData = async (dataUpload) => {
        ...
        const result = await REPORT.UploadDataRaw_Realtime(dataUpload, info, kpiinfo.id)
    ...
    const styles = StyleSheet.create({
        ...
        viewSummary: { marginTop: 4, borderRadius: 8, borderWidth: 1, borderColor: appcolor.surface, overflow: 'hidden' },
        tableHeader: { flexDirection: 'row', backgroundColor: appcolor.surface, paddingVertical: 4, paddingHorizontal: 6, marginBottom: 2 },
        tableRow: { flexDirection: 'row', paddingVertical: 4, paddingHorizontal: 6 },
        colGroup: { width: '50%', fontSize: 10, fontWeight: fontWeightBold, color: appcolor.dark },
        colName: { flex: 1, fontSize: 10, color: appcolor.dark },
        colPoint: { width: 30, fontSize: 10, fontWeight: fontWeightBold, color: appcolor.primary, textAlign: 'center' },
        colHeaderText: { fontSize: 10, fontWeight: fontWeightBold, color: appcolor.primary },
        viewTasks: { marginTop: 6, padding: 8, borderRadius: 8, backgroundColor: appcolor.surface, borderWidth: 0.5, borderColor: appcolor.grayLight },
        tasksLabel: { fontSize: 10, fontWeight: fontWeightBold, color: appcolor.primary, marginBottom: 2, textTransform: 'uppercase' },
    ...
    const renderItem = ({ item }) => {
        ...
        //
        const dataResult = groupDataByKey({ arr: JSON.parse(item.jsonDetail || '[]'), key: 'KPIGroup' }).arr
            ...
            <View>
                {item.isParent &&
                    <View style={styles.viewShopInfo}>
                        ...
                        <Text style={styles.subTitleName}>{`Tổng: ${sumEmployee} nhân viên có lịch đánh giá hôm nay`}</Text>
                        <Text style={styles.subTitleName}>{`CoVisit: ${item.coVisit}`}</Text>
                    </View>
                }
                <View style={styles.itemContainer}>
                    <TouchableOpacity style={styles.viewTitle} onPress={onPress} activeOpacity={0.6} disabled={item.isEvaluated}>
                        ...
                        <View style={styles.viewInfo}>
                            ...
                            <Text style={[styles.subTitleName, { color: item.isEvaluated ? appcolor.success : appcolor.greylight, textDecorationLine: 'underline' }]}>{`${item.statusName}`}</Text>
                        </View>
                        {!item.isEvaluated && <Icon name='chevron-forward' type='ionicon' size={12} color={appcolor.greylight} />}
                    </TouchableOpacity>
                    {item.isEvaluated &&
        ...
        return (
            <View>
                ...
                <View style={styles.tableRow}>
                    {item.isParent ? <Text style={styles.colGroup}>{item.KPIGroup}</Text> : <Text style={styles.colGroup} numberOfLines={2}></Text>}
                    <Text style={styles.colName}>{item.Note ?? '-'}</Text>
                    <Text style={styles.colName}>{item.KPIName == item.KPIGroup ? '-' : item.KPIName}</Text>
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            />
            <Modal visible={visibleModal} animationType="fade" statusBarTranslucent backdropColor={appcolor.light}>
                <SafeAreaProvider>
                    <EmployeeScoring
                        item={itemScore}
