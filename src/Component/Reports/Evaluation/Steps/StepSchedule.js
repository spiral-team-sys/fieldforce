...
const StepSchedule = ({
    ...
    const styles = StyleSheet.create({
        ...
        formActionRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
        selectDateButton: { paddingVertical: 8, paddingHorizontal: 12, borderRadius: 8, borderWidth: 0.5, borderColor: appcolor.grayLight, backgroundColor: appcolor.surface },
        selectDateText: { fontSize: 12, fontWeight: '600', color: appcolor.dark },
        inputCoVisit: { borderWidth: 0.5, borderColor: appcolor.grayLight, borderRadius: 8, marginTop: 8, paddingHorizontal: 10, color: appcolor.dark, fontSize: 12, height: 40, backgroundColor: appcolor.surface },
        pcInfoBox: { marginTop: 8, borderTopWidth: 0.5, borderTopColor: appcolor.grayLight, paddingTop: 8 },
        criteriaItem: { width: '100%', paddingVertical: 6, flexDirection: 'row', alignItems: 'center' },
        criteriaText: { marginLeft: 6, fontSize: 12, color: appcolor.dark, fontWeight: '500' },
        groupHeader: { backgroundColor: appcolor.primary, paddingVertical: 8, paddingHorizontal: 10, marginTop: 8, marginBottom: 4, borderRadius: 6 },
        groupHeaderText: { color: appcolor.light, fontSize: 12, fontWeight: fontWeightBold },
        itemRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, paddingHorizontal: 10, borderBottomWidth: 0.5, borderBottomColor: appcolor.grayLight },
        subWrap: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 10, paddingBottom: 6 },
        subChip: { flexDirection: 'row', alignItems: 'center', borderWidth: 0.5, borderColor: appcolor.grayLight, borderRadius: 16, paddingVertical: 6, paddingHorizontal: 10, marginRight: 6, marginTop: 6, backgroundColor: appcolor.surface, maxWidth: '100%' },
        subChipActive: { borderColor: appcolor.primary, backgroundColor: appcolor.primary + '20' },
        subChipText: { marginLeft: 6, fontSize: 12, color: appcolor.dark, flexShrink: 1 },
        subChipTextActive: { color: appcolor.blacklight, fontSize: 12, fontWeight: '500' },
        criteriaCategoryWrap: { flexDirection: 'row', flexWrap: 'wrap', marginTop: 4, marginBottom: 6, paddingLeft: 26 },
        ...
        calendarContainer: { marginTop: 8, borderTopWidth: 0.5, borderTopColor: appcolor.grayLight, paddingTop: 8 },
        actionSchedule: { marginTop: 8, borderRadius: 8, backgroundColor: appcolor.primary, alignItems: 'center', justifyContent: 'center', paddingVertical: 10 },
        actionScheduleDisable: { backgroundColor: appcolor.grayLight },
        actionScheduleText: { fontSize: 13, fontWeight: fontWeightBold, color: appcolor.light },
    })
    ...
    const buildGroupSelectionItem = (group) => {
        ...
        const subItems = (group.items || []).filter(item => String(item.kpiName || '').trim())
        return {
            ...mainItem,
    ...
    const onChooseGroup = (group) => {
        ...
        const subItems = groupItems.filter(item => String(item.kpiName || '').trim())
        const hasSubItems = subItems.length > 0
        if (!groupSelectionItem) return

        const groupId = getCriteriaIdentity(groupSelectionItem)
        const isGroupSelected = selectedCriteria.some(c => getCriteriaIdentity(c) === groupId)
    ...
    const renderItem = ({ item }) => {
        ...
        const subItems = items.filter(subItem => String(subItem.kpiName || '').trim())
        const hasSubItems = subItems.length > 0
        const isExpanded = hasSubItems && expandedGroups[group.groupId]
        const groupSelectionItem = buildGroupSelectionItem(group)
        const isGroupSelected = selectedCriteria.some(c => getCriteriaIdentity(c) === getCriteriaIdentity(groupSelectionItem))

        return (
            <View key={group.groupId}>
                <TouchableOpacity
                    style={[styles.groupHeader, { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }]}
                ...
                >
                    <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                        ...
                        />
                        <Text style={[styles.groupHeaderText, { flex: 1, marginStart: 8 }]}>{group.groupName}</Text>
                    </View>
                    {hasSubItems && (
                        <Icon
                            type='ionicon'
                ...
                {isExpanded && (
                    <View style={styles.subWrap}>
                        {subItems.map((subItem) => {
                            ...
                            const isSelected = selectedCriteria.some(c => getCriteriaIdentity(c) === getCriteriaIdentity(subSelectionItem))
                            return (
                                <TouchableOpacity
                                ...
                                >
                                    <Text style={[styles.subChipText, isSelected && styles.subChipTextActive]} numberOfLines={0}>
                                        {subItem.kpiName}
                                    </Text>
                                </TouchableOpacity>
                            )
    ...
    return (
        <View style={styles.sectionContainer}>
            ...
            <View style={styles.formActionRow}>
                <TouchableOpacity style={styles.selectDateButton} onPress={() => setShowCalendar(e => !e)}>
                    <Text style={styles.selectDateText}>{`Ngày đánh giá: ${moment(workDate).format('DD/MM/YYYY')}`}</Text>
                </TouchableOpacity>
                <Text style={styles.subTitleName}>{`Đã chọn: ${selectedPC.length} PC`}</Text>
            </View>
            {isShowCalendar && (
            ...
            <TouchableOpacity
                style={[styles.actionSchedule, selectedPC.length === 0 && styles.actionScheduleDisable]}
