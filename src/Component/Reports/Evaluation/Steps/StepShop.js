...
const StepShop = ({
  ...
  return (
    <>
      <SearchData placeholder="Tìm cửa hàng" onSearchData={onSearchShop} />
      <FilterList
        title={`Cửa hàng (${shopOptions.filter(o => o !== 'ALL').length})`}
        keyName="shop"
        options={shopOptions.filter(
          o =>
            o === 'ALL' || normalizeText(o).includes(normalizeText(searchShop)),
        )}
