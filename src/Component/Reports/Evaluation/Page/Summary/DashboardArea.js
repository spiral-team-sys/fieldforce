...
const DashboardArea = ({ title, data }) => {
    ...
    const chartMeta = useMemo(() => {
        if (!dataArea.length) {
            return { labels: [], fullNames: [], avgPoints: [], avgAreaPoints: [], chartData: {} }
        }
    ...
    const s = StyleSheet.create({
        ...
        card: { marginHorizontal: 12, borderRadius: 20, backgroundColor: appcolor.light, borderWidth: 1, borderColor: appcolor.surface, overflow: "hidden" },
        // header
        sectionHeader: { paddingHorizontal: 16, paddingTop: 16, paddingBottom: 12 },
        sectionTitle: { fontSize: 17, fontWeight: "700", color: "#0f172a" },
        legendRow: { flexDirection: "row", flexWrap: "wrap", alignItems: "center", paddingHorizontal: 16, paddingBottom: 8 },
        ...
        // ref table
        refTable: { marginHorizontal: 16, marginTop: 12, marginBottom: 16, borderRadius: 10, borderWidth: 1, borderColor: appcolor.border || "#e2e8f0", overflow: "hidden" },
        refHeader: { flexDirection: "row", backgroundColor: appcolor.primary, paddingVertical: 7, paddingHorizontal: 8 },
        refHeaderCell: { fontSize: 10, fontWeight: "700", color: "#ffffff", textAlign: "center" },
        refRow: { flexDirection: "row", paddingVertical: 7, paddingHorizontal: 8, borderTopWidth: 1, borderTopColor: appcolor.border || "#f1f5f9", alignItems: "center" },
        refRowAlt: { backgroundColor: appcolor.lightgray || "#f8fafc" },
        refCellLabel: { width: 36, fontSize: 11, fontWeight: "700", color: appcolor.primary, textAlign: "center" },
        refCellName: { flex: 1, fontSize: 11, color: appcolor.dark || "#374151", paddingHorizontal: 6, fontWeight: fontWeightBold },
        refCellVal: { width: 52, fontSize: 11, color: appcolor.dark || "#374151", textAlign: "center" },
    })
    ...
    const renderRefTable = () => (
        <View style={s.refTable}>
            <View style={s.refHeader}>
                <Text style={[s.refHeaderCell, { flex: 1, textAlign: "left", paddingHorizontal: 6 }]}>Tiêu chí</Text>
            ...
            {chartMeta.labels.map((key, i) => (
                <View key={key} style={[s.refRow, i % 2 === 1 && s.refRowAlt]}>
                    <Text style={s.refCellName} numberOfLines={2}>{chartMeta.fullNames[i]}</Text>
                    <Text style={s.refCellVal}>{chartMeta.avgPoints[i]?.toFixed(1) ?? "—"}</Text>
                    <Text style={s.refCellVal}>{chartMeta.avgAreaPoints[i]?.toFixed(1) ?? "—"}</Text>
                </View>
            ))}
        </View>
    )
