...
const DashboardCategory = ({ title, data }) => {
    ...
    const s = StyleSheet.create({
        ...
        card: { marginHorizontal: 12, marginTop: 12, marginBottom: 16, borderRadius: 20, backgroundColor: appcolor.light, borderWidth: 1, borderColor: appcolor.surface },
    ...
    const renderRefTable = () => (
        <View style={s.refTable}>
            <View style={s.refHeader}>
                <Text style={[s.refHeaderCell, { flex: 1, textAlign: "left", paddingHorizontal: 6 }]}>Ngành hàng</Text>
                <Text style={[s.refHeaderCell, { width: 52 }]}>{titleCode(chartMeta.title1)}</Text>
                <Text style={[s.refHeaderCell, { width: 52 }]}>{titleCode(chartMeta.title2)}</Text>
            </View>
            {chartMeta.kpiNames.map((name, i) => (
                <View key={i} style={[s.refRow, i % 2 === 1 && s.refRowAlt]}>
                    <Text style={s.refCellName} numberOfLines={2}>{name}</Text>
                    <Text style={s.refCellVal}>{chartMeta.avgPoint1[i]?.toFixed(1) ?? "—"}</Text>
                    <Text style={s.refCellVal}>{chartMeta.avgPoint2[i]?.toFixed(1) ?? "—"}</Text>
                </View>
            ))}
        </View>
    )
