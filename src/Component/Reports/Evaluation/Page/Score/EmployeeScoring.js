...
const getQuestionTitle = (question = {}) =>
  question.groupName || question.kpiName || question.kpi || '—';

const getQuestionSubItems = (question = {}) => {
  if (!Array.isArray(question?.subItems)) return [];
  return question.subItems.filter(sub =>
    String(sub?.kpiName || sub?.kpi || '').trim(),
  );
};

const getScoreTargetKey = ({ question, questionIndex, subItem, subIndex }) => {
  const baseKey = String(
    question?.id ?? question?.kpiId ?? question?.groupId ?? questionIndex,
  );
  if (subItem) {
...
const EmployeeScoring = ({ item, onClose, onUploadData }) => {
  ...
  const validData = dataUpload => {
    if (dataUpload.isNoted == 1) {
      ...
      if (missingNoteItems.length > 0) {
        alertNotify(
          `Vui lòng nhập ghi chú cho các mục sau:\n${missingNoteItems.join(
            '\n',
          )}`,
        );
        return false;
  ...
  // ─── Actions ──────────────────────────────────────────────────────────────
  useEffect(() => {
    const loading_upload = DeviceEventEmitter.addListener(
      'SCORING_DONE_UPLOAD',
      () => setUploading(false),
    );
  ...
  const currentTargets = scoringTargets.filter(
    target => target.questionIndex === currentIndex,
  );
  const currentMode = currentTargets.some(target => target.mode === 'subItem')
    ? 'subItems'
    : 'groupName';
  const activeTarget =
    currentTargets.find(target => target.key === activeTargetKey) ||
    currentTargets[0] ||
    null;
  ...
  useEffect(() => {
    ...
    const firstUnscoredTarget = currentTargets.find(
      target => scores[target.key] === undefined,
    );
  ...
  const totalScore =
    questions.length > 0 && scoreValues.length > 0
      ? round1(groupScoreAverages.reduce((a, b) => a + b, 0) / questions.length)
      : 0;
  const answeredCount = Object.keys(scores).length;
  const allQuestionsAnswered =
    scoringTargets.length > 0 && answeredCount === scoringTargets.length;
  const currentTargetsAnswered = currentTargets.every(
    target => scores[target.key] !== undefined,
  );
  // ─── Handlers ─────────────────────────────────────────────────────────────
  const onSelectScore = (targetKey, score) =>
    setScores(prev => ({ ...prev, [targetKey]: round1(score) }));
  const onStepUp = target => {
    const currentScore = scores[target.key] ?? 0;
    if (currentScore < target.maxScore)
      onSelectScore(
        target.key,
        Math.min(target.maxScore, round1(currentScore + SCORE_STEP)),
      );
  };
  const onStepDown = target => {
    const currentScore = scores[target.key] ?? 0;
    if (currentScore > 0)
      onSelectScore(target.key, Math.max(0, round1(currentScore - SCORE_STEP)));
  };
  const onNext = () => {
    if (currentIndex < total - 1) {
      if (item?.isNoted == 1) {
        const missing = currentTargets.filter(
          t => !notes[t.key] || notes[t.key].trim() === '',
        );
        if (missing.length > 0) {
          alertNotify(
            `Vui lòng nhập ghi chú cho:\n${missing
              .map(t => `• ${t.label}`)
              .join('\n')}`,
          );
          return;
  ...
  const onSubmit = () => {
    if (item?.isNoted == 1) {
      const missing = scoringTargets.filter(
        t => !notes[t.key] || notes[t.key].trim() === '',
      );
      if (missing.length > 0) {
        alertNotify(
          `Vui lòng nhập ghi chú cho các mục:\n${missing
            .map(t => `• ${t.label}`)
            .join('\n')}`,
        );
        return;
  ...
  const styles = StyleSheet.create({
    ...
    summaryNoteRow: {
      ...
      paddingVertical: 6,
      borderBottomWidth: 0.5,
      borderBottomColor: appcolor.grayLight + '80',
    },
    summaryNoteText: {
      fontSize: 11,
      fontStyle: 'italic',
  ...
  if (total === 0) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        >
          <Text style={{ color: appcolor.placeholderText, fontSize: 14 }}>
