...
const PlanEvaluation = () => {
    const { appcolor, shopinfo, kpiinfo, userinfo } = useSelector(state => state.GAppState)
    const [isLoading, setLoading] = useState(false)
    const [data, setData] = useState([])
    const [dataMain, setDataMain] = useState([])
    const [searchText, setSearchText] = useState('')
    const [filter, setFilter] = useState({ area: 'ALL', province: 'ALL', shop: 'ALL' })
    ...
    const roleTokens = useMemo(() => roleName.split(/[^a-z0-9]+/).filter(Boolean), [roleName])

    const isSUPRole = useMemo(() => roleTokens.includes('sup') || roleTokens.includes('supervisor'), [roleTokens])
    ...
    const isITPItem = (item) => {
        const value = [
            readField(item, ['categoryName', 'cateName', 'productCategoryName', 'typeName']),
    ...
    const isSUPManageEmployee = (item) => {
        ...
        ].filter(Boolean).map((e) => normalizeText(e))
        const managerData = [
        ...
        return managerData.some((manager) => {
            return identity.some((id) => manager === id || manager.includes(id) || id.includes(manager))
        })
    }

    const getHistorySummary = (item) => {
        const historyList = parseJsonData(readField(item, ['jsonHistory', 'historyJson', 'historyData']), []) || []
        const lastHistory = _.last(historyList) || {}
        const totalScore = readField(lastHistory, ['TotalScore', 'totalScore', 'SummaryScore']) || readField(item, ['totalScore'])
        const criteriaScore = parseJsonData(readField(lastHistory, ['criteriaScores', 'CriteriaScore', 'jsonCriteriaScore']), readField(lastHistory, ['criteriaScore', 'CriteriaScoreName']))
        const categoryScore = parseJsonData(readField(lastHistory, ['categoryScores', 'CategoryScore', 'jsonCategoryScore']), readField(lastHistory, ['categoryScore', 'CategoryScoreName']))
        const note = readField(lastHistory, ['ReviewNote', 'note', 'notes']) || readField(item, ['reviewNote'])
        const todo = readField(lastHistory, ['NeedToDo', 'todo', 'nextAction']) || readField(item, ['needToDo'])
        return {
            count: readField(item, ['evaluateCount', 'evaluationCount', 'countEvaluate']) || historyList.length || 0,
            date: readField(lastHistory, ['ReportDate', 'reportDate', 'createdDate']) || '--',
    ...
    const convertToPCData = (rawList = []) => {
        ...
        _.forEach(rawList, (row, index) => {
            const employeeCode = String(readField(row, ['employeeCode', 'pcCode', 'code']) || `PC_${index}`)
            ...
            if (!result[key]) {
                result[key] = {
                    ...
                    employeeId,
                    employeeCode,
                    employeeName: readField(row, ['employeeName', 'pcName']) || 'Chưa có tên',
                    phone: readField(row, ['phoneNumber', 'phone', 'mobile']) || '--',
                    workingDate: readField(row, ['workingDate', 'experience', 'tenure']) || '',
                    supName: readField(row, ['supName', 'supervisorName', 'managerName']) || '--',
                    categoryPermision: readField(row, ['categoryPermision', 'categoryPermission', 'categoryAccess']) || '--',
    ...
    const areaOptions = useMemo(() => getFilterOptions('area', dataMain), [dataMain])
    const provinceOptions = useMemo(() => {
        const byArea = filter.area === 'ALL' ? dataMain : _.filter(dataMain, (e) => e.area === filter.area)
        return getFilterOptions('province', byArea)
    }, [dataMain, filter.area])
    const shopOptions = useMemo(() => {
        let byFilter = dataMain
        if (filter.area !== 'ALL') byFilter = _.filter(byFilter, (e) => e.area === filter.area)
        if (filter.province !== 'ALL') byFilter = _.filter(byFilter, (e) => e.province === filter.province)
    ...
    const dataFiltered = useMemo(() => {
        ...
        return _.filter(dataMain, (e) => {
            ...
            return byArea && byProvince && byShop && searchValue.includes(valueSearch)
        })
    ...
    const onSelectFilter = (key, value) => {
        ...
        if (key === 'area') {
            ...
            if (next.province !== 'ALL' && value !== 'ALL') {
                const isProvinceInArea = _.some(dataMain, (e) => e.area === value && e.province === next.province)
    ...
    const toggleSelectedPC = (item) => {
        ...
        const next = isSelected ? selectedPC.filter((e) => e !== item.key) : [...selectedPC, item.key]
        setSelectedPC(next)
    }

    const getCriteriaIdentity = (criteria = {}) => {
        return String(criteria.id ?? criteria.Id ?? `${criteria.groupId || ''}_${criteria.kpi || ''}_${criteria.kpiName || ''}`)
    }

    const onChooseCriteria = (criteriaObject) => {
        if (!criteriaObject) return
        const nextId = getCriteriaIdentity(criteriaObject)
        const isSelected = selectedCriteria.some(c => getCriteriaIdentity(c) === nextId)
        const isGroupItem = criteriaObject?.selectionType === 'group'
        const willChoose = isGroupItem
            ? (typeof criteriaObject?.isChooseGroup === 'boolean' ? criteriaObject.isChooseGroup : !isSelected)
            : (typeof criteriaObject?.isChoose === 'boolean' ? criteriaObject.isChoose : !isSelected)
        ...
        const next = willChoose
            ? [...selectedCriteria.filter((e) => getCriteriaIdentity(e) !== nextId), criteriaObject]
    ...
    const onSubmitSchedule = async () => {
        ...
        const selectedGroups = selectedCriteria.filter(item => item?.selectionType === 'group' && item?.isChooseGroup === true)
        const selectedSubItems = selectedCriteria.filter(item => item?.selectionType === 'sub' && item?.isChoose === true)

        const groupMissingSubItem = selectedGroups.find(group =>
            group.hasSubItems === true &&
            !selectedSubItems.some(sub => sub.groupId === group.groupId)
        )
        if (groupMissingSubItem) {
            toastError('Thông báo', `Vui lòng chọn ít nhất 1 tiêu chí con cho: ${groupMissingSubItem.groupName}`)
            return
        }
        ...
        const selectedPCInfo = _.filter(dataMain, (e) => selectedPC.includes(e.key))
            .map(pc => ({
                key: pc.key,
        ...
        const jsonData = {
            ...
            province: filter.province !== 'ALL' ? filter.province : (refPC.province || ''),
            // Step 2
        ...
        const upload_result = await REPORT.UploadDataRaw_Realtime(itemUpload, shopinfo, kpiinfo.id)
        setLoading(false)
        if (upload_result?.statusId === 200) {
            ...
            toastError('Thông báo', upload_result?.messager || 'Lên lịch thất bại, vui lòng thử lại')
        }
    }
    ...
    const onChangeStep = (stepId) => {
        ...
        if (stepId >= 3 && data.length === 0) {
            toastError('Thông báo', 'Không có dữ liệu nhân viên theo khu vực và cửa hàng đã chọn')
            return
        }
        if (stepId === 4 && selectedPC.length === 0) {
            toastError('Thông báo', 'Vui lòng chọn ít nhất 1 nhân viên trước khi lên lịch')
            return
        }
        setCurrentStep(stepId)
        if (stepId === 1) { setSearchArea(''); setSearchShop('') }
    ...
    const renderListHeader = () => (
        <>
            ...
            />
            <StepHeader stepItems={stepItems} currentStep={currentStep} onChangeStep={onChangeStep} />
            {currentStep === 1 && (
                <StepArea
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <CustomListView
                ...
                extraData={{ currentStep, data, selectedPC, selectedCriteria, selectedCriteriaCate, filter, searchArea, searchShop, workDate, coVisitName, isShowCalendar }}
