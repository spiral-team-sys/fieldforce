...
const POPInstallScreen = ({ navigation, route }) => {
  ...
  const [floatAction, _setFloatAction] = useState({
    isOpen: false,
    type: null,
    title: null,
  });
  ...
  const mergeLocalPhotosIntoData = async sourceData => {
    try {
      ...
      for (let i = 0; i < (sourceData || []).length; i++) {
        ...
        const photosModel = await dataPhotoReport(
          shopinfo,
          kpiinfo.id,
          modelType,
        );
        const mergedPOPList = await Promise.all(
          (item.POPList || []).map(async p => {
            const popType = `${menu.Id}-${wareHouse.Id}-${item.ProductId}-${p.PosmId}`;
            const photosPOP = await dataPhotoReport(
              shopinfo,
              kpiinfo.id,
              popType,
            );
            return {
              ...p,
              ImageList: photosPOP && photosPOP.length > 0 ? photosPOP : [],
            };
          }),
        );
        updatedData.push({
  ...
  const UploadData = async () => {
    ...
    alertConfirm(
      'Gửi dữ liệu',
      'Sau khi gửi dữ liệu lên hệ thống sẽ không được chỉnh sửa nữa, Bạn có chắc không ?',
      async () => {
        await setLoading(true);
  ...
  const validData = async () => {
      ...
      for (let j = 0; j < POPList.length; j++) {
        ...
        if (itemPOP.POPStatus == undefined || itemPOP.POPStatus == null) {
          if (itemPOP.isVerifyInput == 1) {
            checkIsVerifyInput.push(
              ` - ${itemPOP.POPName} của sản phẩm ${itemProduct.ProductName}`,
            );
    ...
    if (checkIsVerifyInput.length > 0) {
      Alert.alert(
        `Chọn trạng thái lắp đặt đối với các POP bắt buộc (*) trước khi gửi`,
        `${checkIsVerifyInput.join('\n')}`,
      );
      return false;
    }
    if (checkDisplay.length > 0) {
      Alert.alert(
        `Chọn trạng thái lắp đặt POP cho sản phẩm có dữ liệu trưng bày > 0 trước khi gửi`,
        `${checkDisplay.join('\n')}`,
      );
      return false;
    }
    // Check Photo
    const dataInstall = _.filter(dataMain, e =>
      (e.POPList || []).some(x => x.InstallValue),
    );
  ...
  const handlerOptionCamera = (itemProduct, itemPOP, indexPOP, typeModel) => {
    const POPName =
      typeModel == 'ITEM_POP' ? itemPOP.POPName : itemProduct.ProductName;
    const title =
      typeModel == 'ITEM_POP' ? `cho ${POPName}` : `cho sản phẩm ${POPName}`;
    //
    OptionCameraPOP(
      'Hình ảnh',
      `Chụp ảnh ${title} hoặc xem ảnh bạn đã chụp trước đó`,
      () => {
  ...
  const handlerCamera = async (
    ...
    if (typeAction == 'CAPTURE') {
      await NativeCamera.cameraStart(photoinfo, res =>
        callBackPhoto(itemProduct, itemPOP, indexPOP, res, typeModel),
      );
    } else {
  ...
  const handlerShowImage = async (itemProduct, itemPOP, typeModel) => {
    if (typeModel == 'ITEM_POP') {
      if (itemPOP.ImageList == undefined || itemPOP.ImageList.length == 0) {
        ToastError(
          `Bạn chưa có ảnh sản phẩm ${itemPOP.POPName} nào`,
          'Thông báo',
          'top',
        );
        return;
      ...
      if (
        itemProduct.ImageList == undefined ||
        itemProduct.ImageList.length == 0
      ) {
        ToastError(
          `Bạn chưa có ảnh sản phẩm ${itemProduct.ProductName} nào`,
          'Thông báo',
          'top',
        );
        return;
    ...
    const ImageList =
      typeModel == 'ITEM_POP' ? itemPOP.ImageList : itemProduct.ImageList;
  ...
  const handlerStatus = async (itemPOP, itemStatus, tabIndex, index) => {
    const value =
      itemStatus.isChoose || itemPOP.POPStatus == itemStatus.ItemName ? 0 : 1;
    ...
    //
    itemPOP.InstallValue =
      itemStatus.isEditValue == 1 ? itemPOP.InstallValue : 0;
  ...
  const _searchData = (
    ...
    const dataSearch = _.filter(filterData, item => {
      return (
        ...
        removeVietnameseTones(item.ProductCode.toLowerCase()).includes(
          valueSearch,
        ) ||
        item.POPList.some(x =>
          removeVietnameseTones(x.POPName.toLowerCase()).includes(valueSearch),
        )
      );
    });
    if (isModelEdited) {
      const dataModelEdited = _.filter(dataSearch, item =>
        item.POPList.some(x => x.isChangeStatus == 1 || x.isChangeValue == 1),
      );
      return dataModelEdited;
    }
    if (isModelNotEdited) {
      const dataModelNotEdited = _.filter(dataSearch, item =>
        item.POPList.every(
          x => x.isChangeStatus !== 1 && x.isChangeValue !== 1,
        ),
      );
      return dataModelNotEdited;
    }
    if (isModelStatus) {
      const dataModelStatus = dataSearch
        .map(product => {
          const filteredPOPList = (product.POPList || []).filter(
            p => p.POPStatus === itemStatus,
          );
  ...
  const onChangeValue = async (itemPOP, text, type) => {
    ...
    if (
      itemPOP.QuantityMyHouse == null ||
      itemPOP.QuantityMyHouse == undefined ||
      ...
      if (value > itemPOP.QuantityMyHouse) {
        ToastError(
          'Số lượng không được lớn hơn số lượng còn lại trong kho',
          'Thông báo',
          'top',
        );
        return;
    ...
    if (totalUsed > itemPOP.QuantityMyHouse) {
      ToastError(
        'Tổng số lượng lắp đặt và thay thế không được vượt quá số lượng trong kho',
        'Thông báo',
        'top',
      );
      if (type == 'INSTALL_VALUE') {
  ...
  const onFilterInput = typeAction => {
    switch (typeAction) {
      case 'MODEL_EDITED':
        floatAction.type = 'MODEL_EDITED';
        setSelectedStatusName(null);
        const dataInstallSearch = _searchData(
          search.text,
          dataMain,
          true,
          false,
      ...
      case 'MODEL_NOT_EDITED':
        floatAction.type = 'MODEL_NOT_EDITED';
        setSelectedStatusName(null);
        const dataNotInstallSearch = _searchData(
          search.text,
          dataMain,
          false,
          true,
  ...
  const onBeforeShowFilter = item => {
    setTypeFilter(item);
    setDataFilter(
      dataMain.map(e => e?.POPList?.map(p => p?.StatusList))?.flat()[0] || [],
    );
  };
  ...
  const renderItem = (item, index, tabIndex) => {
    const onPressProduct = () =>
      handlerOptionCamera(item, null, index, 'MODEL_POP');
    ...
    const viewPOPList = item.POPList?.map((itemPOP, indexPOP) => {
      const onPressPOP = () =>
        handlerOptionCamera(item, itemPOP, indexPOP, 'ITEM_POP');
      const imageURI = itemPOP.Image
        ? { uri: itemPOP.Image }
        : require('../../../../Themes/Images/noimage.png');
      const isFocus =
        focus.isFocus && focus.index == `${item.ProductId}-${itemPOP.PosmId}`;
      return (
        <View
          style={[
            styles.itemPOPContainer,
            {
              borderColor:
          ...
          <View style={styles.contentPOP}>
            ...
            <View style={styles.viewPOPList}>
              {(itemPOP.StatusList || [])?.map((itemStatus, indexStatus) => {
                const isChooseStatus =
                  itemPOP.POPStatus == itemStatus.ItemName ||
                  itemStatus.isChoose == 1;
                return (
                  <TouchableOpacity
                    key={indexStatus}
                    style={[
                      styles.buttonStatus,
                      {
                        backgroundColor: isChooseStatus
                          ? appcolor.primary
                          : appcolor.surface,
                      },
                    ]}
                    onPress={() =>
                      handlerStatus(itemPOP, itemStatus, tabIndex, index)
                    }
                  >
                    <Text
                      style={[
                  ...
                  >
                    <Text
                      style={[
                        ...
                        {
                          color:
                            isFocus && focus.type == 'INSTALL_VALUE'
                              ? appcolor.primary
                              : appcolor.dark,
                        },
                      ]}
                    >{`Lắp mới: ${itemPOP.InstallValue || 0}`}</Text>
                  </TouchableOpacity>
                  <FormGroup
                    inputRefFull={el =>
                    ...
                    handleChangeForm={text =>
                      onChangeValue(itemPOP, text, 'INSTALL_VALUE')
                    }
                  ...
                  >
                    <Text
                      style={[
                        ...
                        {
                          color:
                            isFocus && focus.type == 'CHANGE_VALUE'
                              ? appcolor.primary
                              : appcolor.dark,
                        },
                      ]}
                    >{`Lắp thay thế: ${itemPOP.ChangeValue || 0}`}</Text>
                  </TouchableOpacity>
                  <FormGroup
                    inputRefFull={el =>
                    ...
                    containerStyle={styles.hidenInput}
                    handleChangeForm={text =>
                      onChangeValue(itemPOP, text, 'CHANGE_VALUE')
                    }
  ...
  const renderTab = (item, tabIndex) => {
    ...
    if (dataTab == null || dataTab.length == 0) return <View />;
    return (
  ...
  const renderItemFilter = ({ item, index }) => {
    ...
    const backgroundColor =
      checkInputType == item.ItemId ? appcolor.primary : appcolor.surface;
    const color =
      checkInputType == item.ItemId ? appcolor.light : appcolor.dark;
