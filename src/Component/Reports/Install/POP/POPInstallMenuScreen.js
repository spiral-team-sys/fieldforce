...
import { DeviceEventEmitter, RefreshControl, StyleSheet, Text, View } from 'react-native'
...
const POPInstallMenuScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        const _reload = DeviceEventEmitter.addListener('RELOAD_DATA_POP_INSTALL', LoadData)
        LoadData()
        return () => {
            _reload.remove()
        }
    ...
    const styles = StyleSheet.create({
        ...
        itemContent: {
            padding: 8, backgroundColor: appcolor.light, borderRadius: 8, borderWidth: 0.5, borderColor: appcolor.grayLight, margin: 4,
            elevation: 3, shadowOpacity: 0.3, shadowColor: appcolor.grayLight, shadowOffset: { width: 0, height: 2 }, shadowRadius: 4
        },
        titleMenu: { fontSize: 14, fontWeight: fontWeightBold, color: appcolor.primary, marginStart: 8 },
        viewInfo: { flex: 1 },
        viewIcon: { flexDirection: 'row', alignItems: 'center', paddingTop: 8, paddingBottom: 4 },
        subTitleMenu: { fontSize: 12, fontWeight: '500', color: appcolor.dark },
        loadingView: { flex: 1, justifyContent: 'center', alignItems: 'center' },
        iconForwardStyle: { position: 'absolute', end: 0, top: 0, bottom: 0, justifyContent: 'center' },
        emptyData: { flex: 1, justifyContent: 'center', alignItems: 'center' },
        titleEmptyData: { fontSize: 12, color: appcolor.dark, fontWeight: fontWeightBold, marginTop: 10 }
    })
            ...
            <View style={styles.itemContainer}>
                {item.isParent &&
                    <View style={styles.viewIcon}>
                        <Icon name='settings' type='ionicon' color={appcolor.primary} size={24} />
                        <Text style={styles.titleMenu}>{item.MenuName}</Text>
                    </View>
                }
                <TouchableOpacity style={styles.itemContent} key={index} onPress={onPress}>
                    <View style={styles.viewInfo}>
                        <Text style={styles.subTitleMenu}>Kho: {item.WareHouseName}</Text>
                        <Text style={styles.subTitleMenu}>Thời gian: Từ {item.FromDate} - {item.ToDate}</Text>
                        <Icon name='chevron-forward' type='ionicon' color={appcolor.greylight} size={16} containerStyle={styles.iconForwardStyle} />
                    </View>
                </TouchableOpacity>
            </View>
        )
        ...
        <View style={styles.mainContainer}>
            ...
            <View style={styles.contentContainer}>
                <FlashList
                    ...
                    ListEmptyComponent={<View style={styles.emptyData}>
