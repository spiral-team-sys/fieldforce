	...
	const renderItem = (item, index) => {
		return (
			<TouchableOpacity
				...
				key={`vsmm_${index}`} onPress={() => onItemPress(item)}>
				<View style={styles.itemMenuMain}>
					<View style={{ backgroundColor: appcolor.primary, padding: 16, borderRadius: 24, minWidth: deviceWidth / 5 }}>
						<Icon
		...
		<View style={styles.mainContainer}>
			{!isLoading &&
				<ScrollView
					...
					{dataMenu !== null && dataMenu.length > 0 &&
			...
			<View style={styles.mainIndicator} >
				<Animated.View
					style={[styles.scrollIndicator, {
						transform: [{ translateX: scrollX.interpolate({ inputRange: [0, deviceWidth - ((dataMenu.length * 10))], outputRange: [0, deviceWidth / dataMenu.length], extrapolate: 'clamp', }) }]
					}]} >
