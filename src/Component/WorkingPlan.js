...
class WorkingPlan extends PureComponent {
  ...
  async componentDidMount() {
    await GetListShift(mData => {
      this.setState({ dataShift: mData });
    });
  ...
  LoadData = async plandate => {
    ...
    await WORKINGPLAN_GetList(plandate, (message, responseJson, mEdit) => {
      if (message !== null && message.length > 0) alertNotify(message);
      this.setState({
        lstWorkingPlan: responseJson,
        lstRawSearch: responseJson,
        isEdit: mEdit,
      });
    });
  };
  SwitchChangePlan = rowData => {
    this.setState({
      lstWorkingPlan: this.state.lstWorkingPlan.map(i =>
        i.shopId === rowData.shopId
          ? { ...i, isWorking: i.isWorking === 1 ? 0 : 1 }
          : i,
      ),
  ...
  SHOP_filter = search => {
    this.setState({
      strSearch: search,
      lstWorkingPlan: this.state.lstRawSearch.filter(
        i =>
          i.shopName.toLowerCase().match(search.toLowerCase()) ||
          i.shopCode.toLowerCase().match(search.toLowerCase()) ||
          i.address.toLowerCase().match(search.toLowerCase()),
      ),
    });
  };
  ACTION_updatePlan = async () => {
    ...
    if (
      this.state.lstWorkingPlan === null ||
      this.state.lstWorkingPlan === undefined
    )
      return;
    Message(
      'Cập nhật',
      'Bạn có muốn thay đổi lịch làm việc mới nhất không ?',
      async () => {
        this.setProgressAction(true);
        await PLAN_uploadData(
          this.state.lstRawSearch,
          message => alertNotify(message),
          isProgress => this.setProgressAction(isProgress),
        );
        await this.LoadData(this.state.dateSelected);
      },
    );
  ...
  PLAN_ChangeShift = async items => {
    ...
    if (noteChange !== null && noteChange.length < 10) {
      alertWarning(
        'Lí do chuyển ca quá ngắn, vui lòng nhập lại (Tối thiểu 10 kí tự)',
      );
      return;
    }

    let mDataChange = this.state.lstWorkingPlan.map(i =>
      i.stt === items.stt
        ? { ...i, shiftChange: shiftChange, notes: noteChange }
        : i,
    );
    this.setState({ lstWorkingPlan: mDataChange, sendChangeShift: true });
    await SR_PLAN_ChangeShift(
      mDataChange,
      message => alertNotify(message),
      isProgress => this.setState({ sendChangeShift: isProgress }),
    );
  };
  PLAN_WorkingLate = async items => {
    ...
    if (noteLate !== null && noteLate.length < 10) {
      alertWarning(
        'Lí do đi trễ, về sớm quá ngắn, vui lòng nhập lại (Tối thiểu 10 kí tự)',
      );
      return;
    }
    let mDataChange = this.state.lstWorkingPlan.map(i =>
      i.stt === items.stt
        ? { ...i, timeLate: this.state.timeLate, noteLate: noteLate }
        : i,
    );
    ...
    await PLAN_WorkingLate(
      mDataChange,
      message => alertNotify(message),
      isProgress => this.setState({ sendWorkingLate: isProgress }),
    );
  };
      ...
      <Card>
        ...
        <View style={styles.styleItemRow}>
          <SpiralIcon
            ...
            color={appcolor.grey}
          />
          <Text style={{ flex: 1, fontWeight: 'normal', fontSize: 14 }}>
            {item.shopCode}
          </Text>
        </View>
        <View style={styles.styleItemRow}>
          <SpiralIcon
            ...
            color={appcolor.grey}
          />
          <Text style={{ flex: 1, fontWeight: 'normal', fontSize: 13 }}>
            {item.address}
          </Text>
        </View>
        <View style={styles.styleItemRow}>
          ...
          <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
            <View style={{ flexDirection: 'row' }}>
              {AppNameBuild !== psvApp && (
                <Button
                  type="solid"
                  title={
                    AppNameBuild !== nokiaApp ? 'Chuyển ca' : 'Xin nghỉ phép'
                  }
                  titleStyle={{ fontSize: 12 }}
                  buttonStyle={{
                    backgroundColor: item.colorChangeShift,
                    marginStart: 5,
                  }}
              ...
              {AppNameBuild !== nokiaApp && AppNameBuild !== psvApp && (
                <Button
                  ...
                  titleStyle={{ fontSize: 12 }}
                  buttonStyle={{
                    backgroundColor: item.colorWorkingLate,
                    marginStart: 5,
                  }}
            ...
            <View style={{ flex: 1, alignItems: 'flex-end' }}>
              <Switch
                ...
                trackColor={{
                  true: appcolor.switchEnable,
                  false: appcolor.switchDisible,
                }}
                thumbColor={appcolor.white}
                //Event
                disabled={isEdit}
                value={isWorking}
                onValueChange={() => this.SwitchChangePlan(item)}
              />
            </View>
          </View>
        }
      ...
      <View style={{ flex: 1 }}>
        ...
        <CalendarStrip
          ...
          onDateSelected={mDate => {
            this.LoadData(moment(mDate).format('YYYY-MM-DD'));
          }}
        />
        <SearchBar
          round
        ...
        />
        <ProgressCircleSnail
          Title="Đang cập nhật lịch làm việc"
        ...
        <Modalize
          ...
          onClose={() => this.setState({ bottomSheetData: {} })}
        >
          <View
            style={{
              flex: 1,
          ...
          >
            ...
            <Text style={{ fontSize: 13, fontWeight: '500' }}>
              {this.state.bottomSheetData.shopName} (
              {this.state.bottomSheetData.dateView})
            </Text>
            <View
              style={{
                flex: 2,
            ...
            >
              ...
              >
                <Text style={{ flex: 1 }}>
                  {this.state.bottomSheetData.shiftView}
                </Text>
                {/* <DropDownPicker
                                    zIndex={100}
              ...
              <Text style={{ flex: 1 }}>
                {AppNameBuild !== nokiaApp ? 'Lí do chuyển ca' : 'Nhập lý do'}
              </Text>
              <TextInput
                editable={this.state.bottomSheetData.supConfirm == null}
                value={this.state.bottomSheetData.notes}
                style={{
                  ...styles.inputNote,
                  flex: 1,
                  minHeight: 50,
                ...
                onChangeText={text => {
                  this.setState({
                    bottomSheetData: Object.assign(
                      {},
                      this.state.bottomSheetData,
                      { notes: text },
                    ),
              ...
              >
                {this.state.bottomSheetData.supNote}
              </Text>
              <Button
                loading={this.state.sendChangeShift}
                disabled={this.state.sendChangeShift}
                title={this.state.bottomSheetData.titleChangeShift}
                style={{ zIndex: 0 }}
                buttonStyle={{
                  backgroundColor: this.state.bottomSheetData.colorChangeShift,
                }}
                titleStyle={{ fontSize: 15, fontWeight: '700' }}
                onPress={() =>
                  this.PLAN_ChangeShift(this.state.bottomSheetData)
                }
              />
            </View>
          </View>
        ...
        >
          ...
          >
            ...
            >
              <View
                style={{ flex: 1, flexDirection: 'column', paddingBottom: 5 }}
              >
                <Text style={{ paddingBottom: 5 }}>
                  Thời gian đi trễ (Phút)
                </Text>
              ...
              <TextInput
                ...
                value={this.state.bottomSheetData.noteLate}
                style={{
                  ...styles.inputNote,
                  flex: 1,
                  minHeight: 50,
              ...
              >
                {this.state.bottomSheetData.supNoteLate}
              </Text>
              <Button
                loading={this.state.sendWorkingLate}
                disabled={this.state.sendWorkingLate}
                title={this.state.bottomSheetData.titleWorkingLate}
                titleStyle={{ fontSize: 15, fontWeight: '700' }}
                onPress={() =>
                  this.PLAN_WorkingLate(this.state.bottomSheetData)
                }
              />
            </View>
          </View>
