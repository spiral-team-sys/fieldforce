...
import { ScrollView, View, Text, SectionList, TextInput, Dimensions, Picker, Platform, RefreshControl, FlatList, SafeAreaView, TouchableOpacity, StyleSheet } from "react-native";
import { ButtonGroup, CheckBox, Card } from '@rneui/themed';
import { GetCTSSHistory, insertDisplayResult } from '../Controller/WorkController'
import * as Progress from 'react-native-progress';
import { ConvertDateFromInt, checkNetwork, alertToast, deviceWidth } from "../Core/Utility";
import { DEFAULT_COLOR, URL_GET_CTSS_HISTORY, URL_GET_REQUEST_ITEMS, URL_POST_SURVEY_UPLOAD } from '../Core/URLs';
import { UploadData, fetchGet, Message, ToastError, ToastSuccess, UUIDGenerator } from '../Core/Helper';
...
class SurveyCTSS extends PureComponent {
    ...
    DisplayItemMap(lstItemsProgram) {
        ...
        lstItemsProgram.map((item, i) => {
            ...
            let itemInput = { parent: item.questionName, answerId: item.id, answer: item.name, check: false, modelName: '', refCode: item.fieldSetting, fieldValue: item.fieldValue }
    ...
    fetchDataHistory = async () => {
        // alert(9)
        this.setShowProgress(true)
        var dataRes = await GetCTSSHistory(URL_GET_CTSS_HISTORY, parseInt(moment(this.state.dateSelected).format('YYYYMMDD')), this.props.workinfo.shopId);
        // alertPrint(dataRes)
    ...
    loadDataShow = async (groupName) => {
        let lstItemsProgram = this.state.AllData.filter(item => item.groupName === groupName);
    ...
    isFinishReport() {
        ...
        this.state.lstShow.map(itemA => {
            let lstCheckR = itemA.data.filter(itemD => itemD.check === true && itemD.refCode !== 'R');
            let lstCheckC = itemA.data.filter(itemD => itemD.check === true && itemD.refCode !== 'C');
            let lstCheckD = itemA.data.filter(itemD => itemD.modelName !== '' && itemD.refCode === 'D');
            let lstCheckT = itemA.data.filter(itemD => itemD.modelName !== '' && itemD.refCode === 'T');
            let lstCheckN = itemA.data.filter(itemD => itemD.modelName !== '' && itemD.refCode === 'N');
            let lstCheckL = itemA.data.filter(itemD => itemD.modelName !== '' && itemD.refCode === 'L');
    ...
    uploadAction = async () => {
        if (!this.isFinishReport()) { return }

        Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', () => this.UploadSurvey());
                    ...
                    details.push(
                        {
                            ...
                            "FieldText": itemD.modelName !== '' ? itemD.modelName : (itemD.check === true ? '1' : 'NULL'),
                        })
                });
            });
            var SurveyInfo = JSON.stringify({
                "ShopId": this.props.workinfo.shopId,
                "WorkDate": ConvertDateFromInt(this.props.workinfo.workDate, 'YYYYMMDD', 'YYYY-MM-DD'),
    ...
    isCheckSurvey = (e, item) => {
        this.setState({
            lstShow: this.state.lstShow.map(itemS =>
                itemS.title === item.parent ? { ...itemS, data: itemS.data.map(itemD => itemD.refCode === 'R' ? (itemD.answerId === item.answerId ? { ...itemD, check: true } : { ...itemD, check: false }) : (itemD.answerId === item.answerId ? { ...itemD, check: (itemD.check === false ? true : false) } : itemD)) } : itemS)
        })
    };
    enterModel = (item, textVal) => {
        this.setState({
            lstShow: this.state.lstShow.map(itemS =>
                itemS.title === item.parent ? { ...itemS, data: itemS.data.map(itemD => itemD.answerId === item.answerId ? { ...itemD, modelName: textVal } : itemD) } : itemS)
        })
    }
    ...
    rowTypeEnter = (item) => {
        ...
        switch (item.refCode) {
            ...
            case 'L':
                return <View style={{ width: '100%', height: 60, borderWidth: 0.8, borderColor: 'black', marginTop: 10 }}>
                    {
                        lstPick.length > 0 && <Picker
                            selectedValue={item.modelName}
                            style={{ ...styles.textbox, height: 50, width: '100%' }}
                            onValueChange={(itemValue, itemIndex) => this.enterModel(item, itemValue)}
                        >
                            {
    ...
    renderRow = ({ item }) => (
        <View>
            <View style={{
                ...
                justifyContent: 'space-between', paddingLeft: 35, paddingRight: 15
            }}>
    ...
    renderRowRes = ({ item }) => (
        <Card style={{ height: 160, width: '100%' }}>
            <Card.Title style={{ fontSize: 13, fontWeight: '700', textAlign: 'left' }}>{item.groupName}</Card.Title>
            <Card.Title style={{ fontSize: 11, fontWeight: '400', textAlign: 'left' }}>{item.groupId === 1 ? 'Model: ' + item.model : 'Model thay thế: ' + item.model}</Card.Title>
            <Card.Title style={{ fontSize: 11, fontWeight: '200', textAlign: 'left' }}>{item.createDate}</Card.Title>
            <Card.Title style={{ fontSize: 11, textAlign: 'left' }}>{item.detail}</Card.Title>
        </Card>
    )
    showDatepicker = () => {
    ...
    render() {
        ...
        return (
            <SafeAreaView style={{ height: '100%', width: '100%' }}>
                ...
                />
                {
                    this.state.Groups.length > 0 && <View style={{ backgroundColor: appcolor.light }}><ButtonGroup
                        onPress={this.updateIndex}
                        containerStyle={{ marginBottom: 8, flex: 0 }}
                        textStyle={{ fontSize: 11, color: appcolor.dark }}
                        selectedIndex={this.state.selectedIndex}
                        buttons={this.state.Groups} /></View>
                }
                <View style={{
                    backgroundColor: appcolor.light, width: '100%',
                    height: Dimensions.get('window').height - 60
                }}>

                    {
                        <View style={{ position: 'absolute', marginTop: 0, width: '100%', height: '85%', zIndex: 0 }}>
                            <View style={{
                                zIndex: 0, width: '100%', position: this.state.isHiddenNote ? 'relative' : 'absolute',
                                backgroundColor: appcolor.surface, height: '100%'
                            }}>
                                <SectionList
                                    sections={lstShowAll}
                    ...
                    {/* <Text>{JSON.stringify(this.state.LstResult)}</Text> */}
                    {
                        ...
                        <View>
                            <TouchableOpacity onPress={this.showDatepicker}>
                                <View style={{
                                    padding: 7, margin: 7, borderRadius: 10,
                                    ...
                                    <Text
                                        ...
                                        {
                                            `Chọn ngày xem khảo sát ${moment(this.state.dateSelected).format('DD-MM-YYYY')}`
                                        }
                            ...
                            <View>
                                {
                                    this.state.showDate &&
                                    <DatePicker
                                        maximumDate={new Date()} style={{ width: deviceWidth }}
                                        textColor={appcolor.dark}
                                        mode="date" date={this.state.dateSelected}
                                        onDateChange={(e, date) => {
                                ...
                                }
                                {
                                    ...
                                    <ScrollView
                                        contentContainerStyle={{ height: Dimensions.get('screen').height - 250 }}
