...
import { View, ScrollView, RefreshControl, LogBox, DeviceEventEmitter, AppState } from 'react-native';
...
function HomePSV(props) {
    ...
    const { location, startTracking, stopTracking, tracking } = useLocationTracker(100000);
    const { appcolor, userinfo, homemenu, shoplist, statusPermission } = useSelector(state => state.GAppState)
    ...
    goHyperlink = async (hyperLinks) => {
        ...
        if (hyperLinks === null) {
            ...
            await props.navigation.navigate('WebView', { link: webURL, titlePage: 'Trình duyệt' });
            ...
            await props.navigation.navigate('WebView', { link: hyperLinks, titlePage: 'Trình duyệt' });
        } else {
    ...
    SyncdataApp = async () => {
        ...
        await setState({ ...state, shops: _shops, menus: _menu, refreshing: false })
    }
    onFistLoad = async () => {
        startTracking()
            ...
            if (remoteMessage !== undefined && remoteMessage !== object && remoteMessage !== null) {
                if (remoteMessage.data.hyperLinks !== null) {
                    if (remoteMessage.data.hyperLinks !== "") {
                        ...
                        props.navigation.navigate('Notification', { messengerId: parseInt(remoteMessage.data.messengerId) });
                    }
                }
                else {
                    props.navigation.navigate('Notification', { messengerId: parseInt(remoteMessage.data.messengerId) });
        ...
        messaging().onNotificationOpenedApp(async remoteMessage => {
            if (state.notifyInfo?.data?.hyperLinks !== null) {
                if (state.notifyInfo?.data?.hyperLinks !== "") {
                    ...
                    props.navigation.navigate('Notification', { messengerId: parseInt(state.notifyInfo.data.messengerId) });
                }
            }
            else {
                props.navigation.navigate('Notification', { messengerId: parseInt(state.notifyInfo?.data?.messengerId) });
    ...
    gotoMessage = async () => {
        ...
        if (state.notifyInfo.data.hyperLinks !== null) {
            if (await state.notifyInfo.data.hyperLinks !== "") {
                ...
                await props.navigation.navigate('Notification', { messengerId: parseInt(state.notifyInfo.data.messengerId) });
            }
        }
        else {
            await props.navigation.navigate('Notification', { messengerId: parseInt(state.notifyInfo.data.messengerId) });
        ...
        state.welcome == 1 ?
            ...
            userinfo?.employeeId > 0 ?
                <View style={{ flex: 1, backgroundColor: appcolor.light }}>
                    ...
                    />
                    {state.refreshing == true && <LoadingView isLoading={state.refreshing} isHome={true} />}
                    <ScrollView
                        showsVerticalScrollIndicator={false}
                        style={{ flex: 0.6 }}
                        refreshControl={<RefreshControl refreshing={false} onRefresh={SyncdataApp} />} >
                        <SummaryHome navigation={props.navigation} isLoading={state.refreshing} />
                    </ScrollView>
                    {!state.refreshing && <MenuHomePSV navigation={props.navigation} menus={state.menus} />}
                </View>
                :
