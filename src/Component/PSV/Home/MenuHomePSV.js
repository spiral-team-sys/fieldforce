...
import { GetEmployeeInfo, Token, removeVietnameseTones } from "../../../Core/Helper"
...
export const MenuHomePSV = ({ navigation, menus }) => {
    ...
    const onItemPress = async (item) => {
        ...
        if (item.pageName.includes("trainee")) {
            ...
            navigation.navigate(item.pageName, { pageName: item.menuNameVN, urlPage: item.reportItem + app_access })
        } else {
        ...
        return (
            <TouchableOpacity onPress={() => onItemPress(item)}>
                <View key={`MN_${index}`} style={styles.itemMenuMain}>
                    <View style={{ backgroundColor: appcolor.info, borderRadius: 10, marginEnd: 8, width: 45, height: 45, justifyContent: 'center' }}>
                        <Icon name={item.iconName} type={item.iconType || "font-awesome-5"} size={20} color={appcolor.light} style={{ textAlign: 'center' }} />
                    </View>
    ...
    const styles = StyleSheet.create({
        ...
        titleMain: { width: '95%', padding: 8, fontSize: 21, fontWeight: '700', alignSelf: 'center', color: appcolor.dark },
        itemMenuMain: { width: '100%', padding: 8, backgroundColor: appcolor.surface, borderRadius: 15, marginBottom: 8, flexDirection: 'row', alignItems: 'center' },
        titleMenu: { width: '78%', color: appcolor.dark, fontSize: 15, fontWeight: '600' }
    })
    return (
