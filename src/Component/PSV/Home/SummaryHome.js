                ...
                <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'center' }}>
                    ...
                    {dataButtonMenu.match(typeButton.SSub) &&
                        <Button
                            ...
                            title='S-Sub'
                            type='solid'
                            buttonStyle={dashboardItem == typeButton.SSub ? styles.btnSelect : styles.btnNonSelect}
                            titleStyle={dashboardItem == typeButton.SSub ? styles.titleView : styles.titleNonView}
                        />
                    }
                    {dataButtonMenu.match(typeButton.TargetRow) &&
                        <Button
                            ...
                            title='Target'
                            type='solid'
                            buttonStyle={dashboardItem == typeButton.TargetRow ? styles.btnSelect : styles.btnNonSelect}
                            titleStyle={dashboardItem == typeButton.TargetRow ? styles.titleView : styles.titleNonView}
                        />
                    }
        ...
        <View style={styles.mainContainer}>
            ...
            <View style={styles.mainViewDashboard}>
                <View style={{ flex: 1, backgroundColor: appcolor.surface, margin: 8, borderRadius: 10 }}>
                    ...
                    {/* S-Sub */}
                    {dashboardItem == typeButton.SSub && !loading && dataSSub !== null &&
                        <SSubSummary appcolor={appcolor} dataSSub={dataSSub} />
                    }
                    {dashboardItem == typeButton.TargetRow && !loading && dataTargetRow !== null &&
                        <DashBoardTargetTF navigation={navigation} typeDashboard={typeButton.TargetRow} bgViewItem={appcolor.light} />
                    }
                </View>
            </View>
        </View>
...
const SellOutSummary = ({ appcolor, dataSellOut, navigation }) => {
    ...
    return (
        <TouchableOpacity style={{ zIndex: 10, width: '100%', height: '100%', backgroundColor: appcolor.light, }} onPress={() => navigation.navigate(dataSellOut.pageName)}>
            <View style={{ width: '100%', height: '100%', padding: 8 }}>
                <Text style={{
                    width: '100%', position: 'absolute', top: 12, textAlign: 'center',
                    fontSize: 15, fontWeight: '700', color: appcolor.dark, zIndex: 10, elevation: 10
                }}>{dataSellOut.cname}</Text>
...
const BarChartSummary = ({ appcolor, data, navigation, colorActual, colorTarget, colorConfirm }) => {
    ...
    return (
        <TouchableOpacity style={{ backgroundColor: appcolor.light, zIndex: 10, width: '100%', height: '100%' }} onPress={() => navigation.navigate(pageName)}>
    ...
    const renderItem = ({ item, index }) => {
        return (
            <TouchableOpacity key={`ITT_${index}`} onPress={() => navigation.navigate('attendanthistory')}>
                <View style={{ width: '100%', height: '100%', padding: 16, backgroundColor: appcolor.light }}>
        ...
        <View style={{ width: '100%', height: '100%', alignItems: 'center' }}>
            <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false}>
                {(dataAttendant || []).map((item, index) => (
                    <View key={`attendance_scroll_${index}`} style={{ width: deviceWidth }}>
...
const TargetSummary = ({ appcolor, dataTarget, navigation }) => {
    ...
    return (
        <TouchableOpacity style={{ zIndex: 10, width: '100%', height: '100%', backgroundColor: appcolor.light, }} >
...
const SSubSummary = ({ appcolor, dataSSub = [] }) => {
    ...
    const styles = StyleSheet.create({
        ...
        titleHeader: { width: deviceWidth / 5.5, textAlign: 'center', fontSize: 12, fontWeight: '500', padding: 8, color: appcolor.greylight, fontStyle: 'italic' },
        titleItem: { width: deviceWidth / 5.5, textAlign: 'center', fontSize: 14, fontWeight: '700', padding: 8 }
    })
    ...
    const HeaderView = () => {
        ...
        return (
            <View key={`header_ssub`} style={styles.mainHeader}>
                <Text style={{ ...styles.titleHeader, width: deviceWidth / 4.2, textAlign: 'left' }}>{itemHeader.label}</Text>
    ...
    const renderItem = (item, index) => {
        return (
            <View key={`isu_${index}`}>
                <View key={`header_ssub`} style={styles.mainHeader}>
                    <Text style={{ ...styles.titleItem, width: deviceWidth / 4.2, textAlign: 'left', fontSize: 13, color: appcolor.greylight }}>{item.rTime}</Text>
                    <Text style={{ ...styles.titleItem, color: appcolor.yellow }}>{item.dk}</Text>
                    <Text style={{ ...styles.titleItem, color: appcolor.info }}>{item.si}</Text>
                    <Text style={{ ...styles.titleItem, color: appcolor.red }}>{item.so}</Text>
                    <Text style={{ ...styles.titleItem, color: appcolor.success }}>{item.ck}</Text>
                </View>
            </View>
        )
    }
    return (
        <View style={{ width: '100%', height: '100%' }}>
            <Text style={{ width: '100%', margin: 10, textAlign: 'center', fontSize: 15, fontWeight: '700', color: appcolor.dark }}>{chartName}</Text>
            <View style={{ width: '100%', height: '100%', backgroundColor: appcolor.light, }}>
                <HeaderView />
                <ScrollView>
                    {dataSSub !== null && dataSSub.length > 0 && dataSSub.map((item, index) => {
