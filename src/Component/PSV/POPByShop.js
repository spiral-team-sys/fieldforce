...
export const POPByShop = ({ navigation, route }) => {
  ...
  const MapItemRes = async (lstOrder, i) => {
    ...
    lstOrder.map((it, index) => {
      ...
      let lstItemGroup = lstRes.filter(
        it =>
          it.groupName === itemStr &&
          it.installQuantity !== null &&
          it.installQuantity !== 'null',
      );
    ...
    let lstFilter = lstOrder.filter(it =>
      it.groupName.includes(lstTab[i].name),
    );
    let lstMap = [];
    lstFilter.map(it => {
      ...
      if (itemsHave.length > 0) {
        lstMap.push({
          ...it,
          installQuantity: itemsHave[0].installQuantity,
          reasonId: itemsHave[0].reasonId,
          reasonName: itemsHave[0].reasonName,
        });
      } else {
        lstMap.push(it);
      }
  ...
  const uploadPOPBySHop = async () => {
    ...
    let lstHave = lstRes.filter(
      it =>
        it.installQuantity !== undefined &&
        it.installQuantity !== 'null' &&
        it.installQuantity !== null,
    );
    lstHave.map(it => {
      if (it.reasonId === null) {
        MessageInfo(
          it.popName +
            ': Bạn đã nhập số lượng nhưng chưa chọn trạng thái. Vui lòng chọn trạng thái',
        );
        return;
      }
    });
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <RenderItemData
              item={item}
              details={arrDataShow}
              setDetails={setArrDataShow}
              reloadData={reloadData}
              route={route}
              bottomSheet={ref_bottomSheet}
              setSelectPOPId={setSelectPOPId}
            />
          )}
        />
      </ScrollView>
...
const RenderItemData = ({
  ...
  const [inputText, setInputText] = useState(
    item.installQuantity === 'null' ? null : item.installQuantity,
  );
  ...
  const editInputChange = async (text, item) => {
    ...
    if (text !== '') {
      if (parseInt(text) > quantityMyHouse) {
        MessageInfo(
          'số lượng lắp đặt không được lớn hơn số lượng tồn kho là: ' +
            quantityMyHouse,
        );
        return;
      }
  ...
  return (
    <View
      style={{ height: 120, padding: 10, width: '100%' }}
