...
import { View, Text, AppState, Dimensions, Platform, RefreshControl, ScrollView, TouchableOpacity } from 'react-native';
import { Icon, Divider, ButtonGroup } from '@rneui/themed';
import ActionSheet from "react-native-actions-sheet";
import Moment from 'moment';
import { fetchGet, GetEmployeeInfo, ManageMessenger, saveStore, getStore, ToastError, ToastSuccess } from '../../Core/Helper';
import { getLstMessengerNotSeen, getCategoryPro, getStoreList } from '../../Controller/WorkController';
import { AppNameBuild, _competitorId, URL_GET_SELLOUT_CHART_HMD, MENU_TYPE } from '../../Core/URLs';
            ...
            case 'DAILY':
                viewResult.push(
                    <View key={index + "-DAILY"} style={{ flex: 1, }}>
                        <ChartDaily ref={(ref) => this._ChartDaily = ref}
                            category={this.state.selectedCat} pageIndex={index} appcolor={this.props.appcolor} />
                    </View>
                )
                break;
            case 'WEEKLY':
                viewResult.push(
                    <View key={`WEEKLY${index}`} style={{ flex: 1, }}>
                        <ChartWeekly ref={(ref) => this._ChartWeekly = ref}
                            category={this.state.selectedCat} pageIndex={index} appcolor={this.props.appcolor} />
                    </View>
                );
                break;
            case 'MONTH':
                viewResult.push(
                    <View key={`MONTH${index}`} style={{ flex: 1, }}>
                        <ChartMonthly ref={(ref) => this._ChartMonth = ref}
                            category={this.state.selectedCat} pageIndex={index} appcolor={this.props.appcolor} />
                    </View>
                );
                break;
            default:
                viewResult.push(
                    <View key={`dafad${index}`} style={{ flex: 1 }}>
                        <ChartSellHMD ref={(ref) => this._ChartDaily = ref}
                            category={this.state.selectedCat} pageIndex={index} chartList={this.state.chartList}
                            showDetailHome={this.showDetailHome}
                            LoadData={this.loadChartSlide} loading={this.state.loading}
                            pageSelected={this.state.activeSlide} appcolor={this.props.appcolor} />
                    </View>
                );
                break;
        }
        return (
            <View style={{ backgroundColor: this.props.appcolor.white, height: Platform.OS === 'android' ? deviceHeight / 2.7 : deviceHeight / 3 }}>
                {
                    viewResult
                }
            </View>
                                ...
                                <ScrollView
                                    ...
                                    <View style={{ flex: 1, flexDirection: 'column', padding: 7 }}>
                                        <View style={{ height: 320 }}>
                                            <View style={{ width: '100%', backgroundColor: this.props.appcolor.light }}>
                                                <View style={{ width: '100%', backgroundColor: this.props.appcolor.primary, borderColor: 1, borderRadius: 12 }}>
                                                    <Text style={{ color: this.props.appcolor.light, flexGrow: 1, padding: 12, fontWeight: 'bold', fontSize: scaleSize(22) }}>{this.state.titlePage}</Text>
                                                    <View style={{ backgroundColor: this.props.appcolor.light, borderBottomRightRadius: 10, borderTopRightRadius: 10, width: '80%', height: 10 }} />
                                                    <View style={{ backgroundColor: this.props.appcolor.primary, width: '100%', height: 10 }} />
                                                </View>
                                                <View style={{ width: '100%', height: '75%', borderWidth: 1, borderColor: this.props.appcolor.greylight, borderBottomLeftRadius: 16, borderBottomRightRadius: 16 }}>
                                                    {this.DisplayChart(this.state.selectedIndex)}
                                                    <ButtonGroup
                                                        containerStyle={{ height: 32, borderWidth: 0, backgroundColor: this.props.appcolor.primary }}
                                                        textStyle={{ color: this.props.appcolor.light }}
                                                        selectedButtonStyle={{ backgroundColor: this.props.appcolor.light }}
                                                        selectedTextStyle={{ color: this.props.appcolor.primary, fontWeight: '700' }}
                                        ...
                                        <View style={{ height: '45%', backgroundColor: this.props.appcolor.light }}>
                                            <Text style={{
                                                color: this.props.appcolor.dark, flexGrow: 1,
                                                padding: 12, fontWeight: 'bold',
                                                fontSize: scaleSize(22)
                                            }}>Chức năng</Text>
                                            <View style={{ borderWidth: 1, borderColor: appcolor.surface, width: '100%' }} />
                                            <MainMenu menus={this.state.menus} navigation={this.props.navigation} />
                                        </View>
                                    </View>
                                </ScrollView>
                                <MessageForm ref={this.messageRef}
