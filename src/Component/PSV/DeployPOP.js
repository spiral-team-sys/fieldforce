...
import { View, Text, FlatList, KeyboardAvoidingView, TouchableOpacity, StyleSheet, Keyboard, TextInput, Platform } from "react-native"
...
export const DeployPOP = ({ navigation, route }) => {
    ...
    const [dataPOP, setDataPOP] = useState({ dataView: [], dataFilter: [], listPOP: [], listReasonPOP: [] })
    ...
    const loadData = async () => {
        ...
        await getPOPByShop(workinfo.shopId, async (mData) => {
            ...
            await setDataPOP({ dataView: listData[0].detailData, dataFilter: listData[0].detailData, listPOP: arr, listReasonPOP: listReasonPOP })
        })
    ...
    const groupPOP = async (data) => {
        ...
        data.map(it => {
            if (it.isParent) {
                const listItemGroup = data.filter(item => item.groupName === it.groupName)
                listGroup = [...listGroup, {
    ...
    const uploadAction = async () => {
        ...
        let listUpload = dataPOP.listPOP.filter(it => (it.quantity && it.quantity !== 'null' && it.quantity !== null) || (it.reasonId));

        const checkQuantity = listUpload.filter(it => it.quantity && it.quantity !== 'null' && it.quantity !== null && !it.reasonId)
        if (checkQuantity.length > 0) {
            ToastError('Bạn đã nhập số lượng nhưng chưa chọn lí do triển khai.  - Nhóm : ' + checkQuantity[0].groupName + '     - POP : ' + checkQuantity[0].popName, 'Thông báo', 'top');
            return;
        }

        const checkReason = listUpload.filter(it => (!it.quantity || it.quantity === 'null' || it.quantity === null) && it.reasonId)
        if (checkReason.length > 0) {
            ToastError('Bạn đã chọn lí do triển khai nhưng chưa nhập số lượng.  - Nhóm : ' + checkReason[0].groupName + '     - POP : ' + checkReason[0].popName, 'Thông báo', 'top');
            return;
        }
        ...
        Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', async () => {
            ...
            await installPOPByShop(JSON.stringify(listUpload), async (success) => {
                if (success.statusId) {
    ...
    const filterDeployData = async () => {
        ...
        let lstData = dataPOP.dataFilter.filter(it => it.quantity !== null && it.quantity >= 0)
        if (deployFilter) {
    ...
    const filterProduct = async (str) => {
        ...
        if (str !== null && str !== undefined && str.length > 0) {
            mDataFilter = dataPOP.dataFilter.filter(i => i.popName.toLowerCase().match(str.toLowerCase()))
        } else {
    ...
    const handlerItemSelect = async (item, index) => {
        ...
        let dataViewSearch = dataGroupPOP.filter(i => i.popName.toLowerCase().match(search.toLowerCase()))
        let dataViewFilter = dataGroupPOP.filter(it => it.quantity !== null && it.quantity > 0)
        await setDataPOP({ ...dataPOP, dataView: (isDone === true ? dataViewFilter : (search.length > 0 ? dataViewSearch : dataGroupPOP)), dataFilter: dataGroupPOP })
    ...
    const RenderItemGroup = ({ item, index }) => {
        ...
        const fontWeightTitle = itemSelect.groupId === item.groupId ? '800' : 'normal'
        const totalRow = item.detailData?.filter(i =>
            isDone ? (i.quantity !== null && i.quantity >= 0) : (i.popName.toLowerCase().match(search.toLowerCase()))).length || 0
        return (
            <TouchableOpacity
                ...
                <Text style={{ color: colorTitle, fontSize: 14, fontWeight: fontWeightTitle }}>{`${item.groupName} ${totalRow > 0 ? `(${totalRow})` : ''}`}</Text>
                {/* {
                    totalRow > 0 &&
                    < Badge
    ...
    const handleChaneQuantity = async (item, text) => {
        ...
        if (item.quantityMyHouse < display) {
            ToastError('Số lượng triển khai phải nhỏ hơn hoặc bằng số lượng kho đang có!!')
            return
        }
        const indexListPOP = dataPOP.listPOP.findIndex(it => it.popId === item.popId)
        const indexGroup = dataGroup.findIndex(it => it.groupId === itemSelect.groupId)
        const indexDetail = dataGroup[indexGroup].detailData.findIndex(it => it.popId === item.popId)
        ...
        await setDataPOP({ ...dataPOP, dataFilter: dataGroup[indexGroup].detailData })
    }
    const RenderItem = ({ item, index }) => {
        ...
        return (
            <View key={`I_P_P_${index}`} style={{ flex: 1, padding: 8, flexDirection: 'row', borderBottomWidth: 0.5, borderColor: appcolor.light }}>
                ...
                <View style={{ flex: 1, padding: 5, paddingLeft: 20, justifyContent: 'space-between' }}>
                    <View style={{ flex: 1 }}>
                        ...
                        {(item.quantityMyHouse == 0 || item.quantityMyHouse) ? <Text style={styles.textItem}>Tồn kho cá nhân : {item.quantityMyHouse}</Text> : null}
                    </View>
                    <View style={{ flex: 1, flexDirection: 'row' }}>
                        <TouchableOpacity
                            onPress={() => onShowReason()}
                            style={{ flex: 1, height: 30, backgroundColor: appcolor.light, borderWidth: 0.5, borderRadius: 5, borderColor: appcolor.greydark, marginRight: 5, justifyContent: 'center', alignItems: 'center' }}>
                            <Text style={{ fontWeight: '500', color: appcolor.dark, fontSize: 12, textAlign: 'center', }}>{item.reasonName || '-- Chọn --'}</Text>
                        </TouchableOpacity>
                        <NumberFormat
                            ...
                            renderText={value =>
                                <TextInput
                                    ...
                                    style={{
                                        ...
                                        width: '20%', textAlign: 'center', borderWidth: 0.5, borderRadius: 5, borderColor: appcolor.greydark,
                                    }}
    ...
    const styles = StyleSheet.create({
        ...
        titleItem: { width: '100%', color: appcolor.dark, fontSize: 14, fontWeight: '600', marginTop: 5 },
    ...
    const handleSelectReason = async (item) => {
        ...
        const itemReasonName = item.name === itemPOPSelect.reasonName ? null : item.name

        const index = dataPOP.listPOP.findIndex(it => it.popId === itemPOPSelect.popId)
        dataPOP.listPOP[index] = { ...itemPOPSelect, reasonId: itemReasonId, reasonName: itemReasonName }
        itemPOPSelect.reasonId = itemReasonId
        itemPOPSelect.reasonName = itemReasonName
        const listData = await groupPOP(dataPOP.listPOP)
        const listDataIndex = listData.findIndex(it => it.groupId === itemSelect.groupId)
        await setDataGroup(listData)
        const dataView = listData[listDataIndex]?.detailData.filter(it => isDone ? (it.quantity !== null && it.quantity >= 0) : (it.popName.toLowerCase().match(search.toLowerCase()))) || []
        await setDataPOP({ ...dataPOP, dataView: dataView, dataFilter: listData[listDataIndex].detailData })
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.surface }}>
            <HeaderCustom
                ...
                iconMiddle='poll-h'
                middleFunc={() => !isLockReport ? (!loading ? filterDeployData() : null) : ToastSuccess('Bạn đã hoàn thành chấm công nên không thể gửi dữ liệu báo cáo')}
            />
            <View style={{ flex: 1 }}>
                <View style={{ flexDirection: 'row', padding: 5, justifyContent: 'center', alignItems: "center", alignSelf: "center" }}>
                    <FormGroup
                        editable
                ...
                <KeyboardAvoidingView
                    ...
                    <FlatList
                        ...
                        ListFooterComponent={<View style={{ height: deviceHeight / 3, marginBottom: 28 }} />}
                    />
                </KeyboardAvoidingView>
                <ActionSheet
                    id={'ref_reasonPOP'}
                            ...
                            renderItem={({ item, index }) => {
                                return (
                                    <TouchableOpacity
                                        ...
                                        style={{
                                            padding: 5, flex: 1, borderRadius: 10, backgroundColor: itemPOPSelect?.reasonId === item.id ? appcolor.primary : appcolor.light, borderBottomColor: appcolor.grey,
                                            justifyContent: 'center', alignItems: 'center', borderBottomWidth: 1, margin: 5
                                        }}>
                                        <Text style={{ fontSize: 12, color: itemPOPSelect?.reasonId === item.id ? appcolor.white : appcolor.dark, padding: 5 }}>{item.name}</Text>
                                    </TouchableOpacity>
                                )
                            }}
                        />
