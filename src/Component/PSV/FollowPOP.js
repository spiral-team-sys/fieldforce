...
export const FollowPOP = ({ navigation, route }) => {
  ...
  const loadData = async () => {
    ...
    await setArrDataShow(
      lstOrder.filter(it => it.orderStatus.includes(lstSort[indexTab].name)),
    );
    setLoading(false);
  };
  useEffect(() => {
  ...
  const borderColor = item => {
    ...
    switch (item.status) {
      case 'PASS':
        color = item.orderStatus.includes('đang vận chuyển')
          ? appcolor.orange
          : appcolor.success;
        break;
      case 'REJECT':
  ...
  const renderItemData = ({ item, index }) => (
    <View
      key={'3ss' + index}
      style={{ height: 'auto', padding: 10, width: '100%' }}
      onPress={() => showDetailOrderPOP(item)}
    >
      <TouchableOpacity
        style={{
          borderRadius: 10,
  ...
  const handleChangeTab = (i, from) => {
    setIndexTab(i);
    setArrDataShow(
      arrDataShowF.filter(it => it.orderStatus.includes(arrTagShow[i].name)),
    );
  };
  return (
    <View style={{ flex: 1 }}>
      <HeaderCustom
        title={route.params.menuName}
