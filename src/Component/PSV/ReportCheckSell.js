...
import {
  clearDataTrackingSellOut,
  getDataCompetitor,
  getListCatProduct,
  getTrackingSellOutResult,
  updateItemTrackingSellOut,
  uploadTrackingSellOut,
...
export const ReportCheckSell = ({ navigation, route }) => {
  ...
  const loadData = async () => {
    ...
    await setData({
      dataTab: listTab,
      dataCompetitor: listData,
      dataCompetitorF: listData,
    });
    setTimeout(async () => {
  ...
  const uploadAction = async () => {
    ...
    let checkAmount = lstRes.filter(
      it =>
        it.quantity !== 'null' &&
        it.quantity !== null &&
        it.amount !== null &&
        it.amount !== 'null' &&
        (it.amount < 10000 || it.amount % 1000 > 0),
    );
    if (checkAmount.length > 0) {
      let competitor = data.dataCompetitorF.filter(
        it => it.id === checkAmount[0].id,
      );
      ToastError(
    ...
    let checkQuantity = lstRes.filter(
      it =>
        it.quantity === 0 &&
        it.quantity !== null &&
        it.amount !== 'null' &&
  ...
  const filterProduct = async text => {
    let dataSearch = [];
    if (isDone)
      dataSearch = data.dataCompetitorF.filter(
        it =>
          (it.quantity !== null && it.quantity >= 0) ||
          (it.amount !== null && it.amount >= 0),
      );
    else dataSearch = data.dataCompetitorF;

    if (text !== null && text.length > 0) {
      const mResult = await dataSearch.filter(it => {
        const nameProduct = it.competitorName
          ? it.competitorName.toUpperCase()
          : ''.toUpperCase();
  ...
  const filterDoneProduct = async () => {
    if (!isDone) {
      let lstRes = data.dataCompetitor.filter(
        it =>
          (it.quantity !== null && it.quantity >= 0) ||
          (it.amount !== null && it.amount >= 0),
      );
  ...
  const renderItem = ({ item, index }) => {
    ...
    const onEndChangeAmount = event => {
      let value =
        event.nativeEvent.text !== null && event.nativeEvent.text.length > 0
          ? event.nativeEvent.text.toString().replace(/,/g, '')
          : null;
      if (isNotInteger(value)) value = '';
    ...
    return (
      <View
        style={{
          ...
          borderBottomColor: appcolor.greydark,
        }}
      >
  ...
  const ViewItem = () => {
    return data.dataTab?.map((it, indexCate) => {
      let dataByCategoryId = data.dataCompetitor.filter(
        item => it.categoryId === item.categoryId,
      );
      ...
      >
        <View style={styles.sheetContainer}>
          ...
          <Text style={styles.sheetSubTitle}>
            Quản lý nhanh dữ liệu sell-out đã nhập
          </Text>
          <TouchableOpacity
