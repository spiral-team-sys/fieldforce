...
export const ViewListStoreKPI = ({ dataModal }) => {
  ...
  const renderItem = ({ item, index }) => {
    ...
    return (
      ...
      >
        <RenderDataKPI
          key={`item_${data.itemView[item]}}_`}
          group={group}
          dataItem={data.itemView}
          type={`GROUP_${index}`}
        />
      </View>
    );
...
const RenderDataKPI = ({ group, type, dataItem }) => {
      ...
      <View key={`${type}_${index}`} style={{ flex: 1, padding: 8 }}>
        ...
        >
          ...
          >
            ...
            >{`${
              dataItem[group.itemField[index]] > 0
                ? formatNumber(dataItem[group.itemField[index]], ',')
                : dataItem[group.itemField[index]]?.length > 0
                ? dataItem[group.itemField[index]]
                : '-'
            } `}</Text>
          </View>
        </View>
  ...
  return group.itemField?.length !== count ? (
    <View style={{ width: '100%' }}>
      <View
        style={{
          backgroundColor: group.color || appcolor.primary,
          borderRadius: 8,
          padding: 8,
          margin: 4,
        }}
