...
export const KPIIncentive = ({ navigation }) => {
  ...
  const [dataModal, setDataModal] = useState({
    titleModal: 'Tháng ' + (moment().month() + 1) + '-' + moment().year(),
    visible: false,
    dataSelect: [],
    dataFilter: [],
  });
  ...
  const loadData = async jsonCalendar => {
    ...
    const dataJson =
      jsonCalendar ||
      JSON.stringify({ year: moment().year(), month: moment().month() + 1 });
    await GetDataIncentive(dataJson, async listKPI => {
      const dataJson = jsonCalendar
  ...
  return (
    <View style={{ flex: 1, backgroundColor: appcolor.light }}>
      ...
      {!loading && (
        ...
        >
          {data.dataWorkingDay.length > 0 && (
            <RenderWorkingDay
              mData={data.dataWorkingDay}
              styles={styles}
              appcolor={appcolor}
            />
          )}
          <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            ...
            >{`Incentive ${dataModal.titleModal}`}</Text>
            {data.KPI1?.length > 0 && (
              <RenderKPI1
                mData={data.KPI1}
                styles={styles}
                appcolor={appcolor}
                onViewDetail={handlerViewDetail}
              />
            )}
            {data.KPI2?.length > 0 && (
              <RenderKPI2
            ...
            {data.KPI6?.length > 0 && (
              <RenderKPI6 dataKPI={data.KPI6} navigation={navigation} />
            )}
          </View>
        </ScrollView>
      )}
            ...
            >
              <View
                style={{
                  width: '40%',
                  flexDirection: 'row',
                  backgroundColor: item.color ? item.color : appcolor.light,
                  alignItems: 'center',
                  borderWidth: 0.5,
...
//         <View style={styles.mainKPI}>
//             <View style={{ flexDirection: 'row', alignItems: 'center' }}>
//                 <SpiralIcon type="font-awesome-6" color={appcolor.info} name="medal" size={18} solid />
