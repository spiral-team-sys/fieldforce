...
export const DashboardSellOut = ({ navigation, typeDashboard, viewHeight }) => {
    ...
    const styles = StyleSheet.create({
        ...
        titleDashboard: { flex: 1, fontSize: 16, fontWeight: '700', color: appcolor.blacklight },
    ...
    return isLoading ? null : <TouchableOpacity onPress={showDetailData} style={{ maxHeight: viewHeight - 40, height: viewHeight, padding: 8 }}>
        <View style={styles.mainContainer}>
            <View style={styles.headerContent}>
                <View style={{ flexDirection: 'row', width: '90%' }}>
                    <View style={{ backgroundColor: appcolor.surface, padding: 8, borderRadius: 12, marginRight: 8 }}>
                        <Icon name="chart-pie" type="font-awesome-5" size={18} color={appcolor.primary} style={{ padding: 5 }} />
                    </View>
                ...
                <View style={{ width: '50%', alignItems: 'center', justifyContent: 'center' }}>
                    <View style={{ width: '100%', justifyContent: 'center' }}>
                        <View style={{ flexDirection: 'row', alignItems: "center" }}>
                            ...
                            <Text style={{ fontSize: scaleSize(26), fontWeight: '800', color: appcolor.tomato, marginStart: 8 }}>{`${dataSellOut.targetValue || 0}`}</Text>
                        </View>
                        <Text style={{ fontSize: 15, fontWeight: '500', color: appcolor.dark, padding: 5 }}>{`Actual`}</Text>
                        <Text style={{ fontSize: scaleSize(16), fontWeight: '700', color: appcolor.info, marginStart: 8 }}>{`${dataSellOut.l1 || 'Số lượng'}: ${dataSellOut.v1 || 0}`}</Text>
                        <Text style={{ fontSize: scaleSize(16), fontWeight: '700', color: appcolor.info, marginStart: 8 }}>{`${dataSellOut.l2 || 'Thành tiền'}: ${dataSellOut.v2 || 0}`}</Text>
                    </View>
                </View>
