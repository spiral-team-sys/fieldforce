...
export const DashboardDisplayShare = ({ info, isReload = false, navigation }) => {
    ...
    const LoadData = async () => {
        await DashboardAPI.GetMasterFilter(shopinfo.shopId, info.pageName, async (mData, message) => {
            message && ToastError(message, 'MasterFilter', 'top')
            await setDataFilter(mData)
            //
            if (mData.length > 0) {
                const itemChooseTag = mData[0]
                await LoadDataDashboard({ group1: itemChooseTag.groupName1, group2: null, group3: null })
            }
        })
    }
    const LoadDataDashboard = async (filterObject = {}) => {
        await dispatch(setDashboardFilter({ [`${info.pageName}`]: filterObject }))
        await DashboardAPI.GetDashboardDisplayShare(shopinfo.shopId, filterObject, async (mData, message) => {
    ...
    // Handler 
    const onShowDetails = () => {
        navigation.navigate('displaydetails', { pageName: info.pageName, dataFilter: dataFilter })
    }
    //
    useEffect(() => {
        let isMounted = true
    ...
    const styles = StyleSheet.create({
        mainContainer: { width: '100%', overflow: 'hidden', minHeight: deviceHeight / 3, backgroundColor: appcolor.light, marginBottom: 12, borderRadius: 8, elevation: 3, shadowColor: appcolor.surface, shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.3, shadowRadius: 8 },
    ...
    return (
        <View style={styles.mainContainer}>
            <View style={styles.viewTitleChart}>
                <Icon color={appcolor.primary} type="font-awesome-5" name="chart-line" size={23} />
            ...
            />
            <TouchableOpacity style={{ alignSelf: 'flex-end' }} onPress={onShowDetails}>
                <Text style={styles.titleDetails}>{`Chi tiết trưng bày -->`}</Text>
            </TouchableOpacity>
            {indexGroup.dataDetails !== undefined && indexGroup.dataDetails !== null && indexGroup.dataDetails.length > 0 &&
