...
export const ScreenTableDetails = ({ data, groupName }) => {
    ...
    const LoadData = async () => {
        const _data = _.filter(data, (e) => groupName !== null && e.GroupView == groupName)
        await setMutate(e => !e)
    ...
    const styles = StyleSheet.create({
        ...
        itemHeaderView: { width: '20%', alignItems: 'center', backgroundColor: appcolor.primary, marginHorizontal: 0.5 },
        itemView: { width: '20%', alignItems: 'center', backgroundColor: appcolor.surface, marginHorizontal: 0.5 },
        titleGroup: { fontSize: 13, fontWeight: fontWeightBold, color: appcolor.light, padding: 8 },
        titleValue: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.dark, padding: 8 },
        viewTitlte: { width: '100%', flexDirection: 'row', alignItems: 'center' },
        titleTableView: { width: '70%', fontSize: 13, fontWeight: fontWeightBold, color: appcolor.dark, padding: 8, fontStyle: 'italic' },
    })
    const renderHeader = () => {
        ...
        return (
            <View style={styles.headerMain}>
                ...
                <View style={{ ...styles.itemHeaderView, width: '20%' }}>
                    <Text style={styles.titleGroup}>{itemHeader.TitleCW || 'Latest'}</Text>
                </View>
            </View>
        )
    }
    const renderItemTotal = () => {
        ...
        return (
            <View style={styles.itemMain}>
                <View style={{ ...styles.itemView, backgroundColor: appcolor.grayLight }}>
                    <Text style={styles.titleValue}>Total</Text>
                </View>
                <View style={{ ...styles.itemView, width: '40%', backgroundColor: appcolor.grayLight }}>
                    <Text style={styles.titleValue}>Total</Text>
                </View>
                <View style={{ ...styles.itemView, width: '20%', backgroundColor: appcolor.grayLight }}>
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View key={`stb_item${index}`} style={styles.itemMain}>
                <View style={{ ...styles.itemView, width: '20%', backgroundColor: backgroundColor }}>
                    <Text style={styles.titleValue}>{item.GroupName || '-'}</Text>
                </View>
                <View style={{ ...styles.itemView, width: '40%', backgroundColor: backgroundColor }}>
