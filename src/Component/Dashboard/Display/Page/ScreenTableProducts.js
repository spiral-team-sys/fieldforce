...
export const ScreenTableProducts = ({ navigation, data, groupName }) => {
  ...
  const LoadData = async (params = { keyValue: null, keyName: null }) => {
    const _data = _.filter(
      ...
      e =>
        groupName !== null &&
        e.GroupView == groupName &&
        (params.keyValue ? e.CompetitorId == params.keyValue : true),
    );
  ...
  const _searchData = async filterList => {
    ...
    if (search.growth !== null) {
      searchData = await _.filter(
        searchData,
        e =>
          e.growthT2 == search.growth ||
          e.growthT3 == search.growth ||
          e.growthT4 == search.growth,
      );
    }
    if (search.week !== null) {
      searchData = await _.filter(
  ...
  //
  useEffect(() => {
    const reload_key = DeviceEventEmitter.addListener(
      'reload_table_products',
      LoadData,
    );
    let isMounted = true;
    if (!isMounted) return;
  ...
  const renderHeader = () => {
    ...
    const opacityFilter = type => {
      return (search.week || null) == null ? 1 : search.week == type ? 1 : 0.5;
    };
    const onPress = type => {
      onTouchWeek(type);
    };
    return (
      <View style={styles.headerMain}>
        ...
        <View
          style={{ ...styles.itemHeaderView, opacity: opacityFilter('T1') }}
        >
          <Text style={styles.titleGroup}>{itemHeader.T1 || ''}</Text>
        </View>
        <TouchableOpacity onPress={() => onPress('T2')}>
          <View
            style={{ ...styles.itemHeaderView, opacity: opacityFilter('T2') }}
          >
        ...
        <TouchableOpacity onPress={() => onPress('T3')}>
          <View
            style={{ ...styles.itemHeaderView, opacity: opacityFilter('T3') }}
            onPress={() => onPress('T3')}
          >
        ...
        <TouchableOpacity onPress={() => onPress('T4')}>
          <View
            style={{ ...styles.itemHeaderView, opacity: opacityFilter('T4') }}
            onPress={() => onPress('T4')}
          >
  ...
  const renderItemTotal = () => {
    ...
    const _sumT1 = _.sumBy(dataMain, e => {
      return parseInt(e.T1) || 0;
    });
    const _sumT2 = _.sumBy(dataMain, e => {
      return parseInt(e.T2) || 0;
    });
    const _sumT3 = _.sumBy(dataMain, e => {
      return parseInt(e.T3) || 0;
    });
    const _sumT4 = _.sumBy(dataMain, e => {
      return parseInt(e.T4) || 0;
    });
    //
    const iconNameStatus = (value1, value2) => {
        ...
        <View style={{ ...styles.itemView, opacity: opacityFilter('T2') }}>
          ...
          {_sumT2 > 0 && (
            <SpiralIcon
              size={12}
              type="ionicon"
              name={iconNameStatus(_sumT2, _sumT1)}
              color={colorStatus(_sumT2, _sumT1)}
              containerStyle={styles.iconViewStatus}
            />
          )}
        </View>
        <View style={{ ...styles.itemView, opacity: opacityFilter('T3') }}>
          ...
          {_sumT3 > 0 && (
            <SpiralIcon
              size={12}
              type="ionicon"
              name={iconNameStatus(_sumT3, _sumT2)}
              color={colorStatus(_sumT3, _sumT2)}
              containerStyle={styles.iconViewStatus}
            />
          )}
        </View>
        <View style={{ ...styles.itemView, opacity: opacityFilter('T4') }}>
          ...
          {_sumT4 > 0 && (
            <SpiralIcon
              size={12}
              type="ionicon"
              name={iconNameStatus(_sumT4, _sumT3)}
              color={colorStatus(_sumT4, _sumT3)}
              containerStyle={styles.iconViewStatus}
            />
          )}
        </View>
      ...
      <View key={index} style={styles.itemMain}>
        ...
        >
          ...
          <SpiralIcon
            size={12}
            type="ionicon"
            name={item.iconStatusT2}
            color={appcolor[item.colorStatusT2]}
            containerStyle={styles.iconViewStatus}
          />
        </View>
        <View
        ...
        >
          ...
          <SpiralIcon
            size={12}
            type="ionicon"
            name={item.iconStatusT3}
            color={appcolor[item.colorStatusT3]}
            containerStyle={styles.iconViewStatus}
          />
        </View>
        <View
        ...
        >
          ...
          <SpiralIcon
            size={12}
            type="ionicon"
            name={item.iconStatusT4}
            color={appcolor[item.colorStatusT4]}
            containerStyle={styles.iconViewStatus}
          />
        </View>
      </View>
