...
const MultiGroupFilter = ({ isReloadFilter = false, pageName, data = [], containerStyle, handlerChangeData }) => {
    const { appcolor } = useSelector(state => state.GAppState);
    const { dashboardFilter } = useSelector(state => state.dashboard)
    const [dataMain, setDataMain] = useState([])
    const [dataView, setDataView] = useState([])
    const [filter, _setFilter] = useState({ group1: null, group2: null, group3: null });
    const [itemData, _setItemData] = useState({ data1: [], data2: [], data3: [] });
    ...
    const onHeaderPress = async (key) => {
        ...
        await setDataView(dataChange);
        dataChange.length > 0 && refList.current.scrollToIndex({ index: 0, animated: true })
        //
    ...
    const onDataFilterChange = async (level) => {
        ...
        for (let i = 1; i <= level; i++) {
            ...
            if (name !== null) {
                filteredData = filteredData.filter(item => item[`groupName${i}`] === name);
            }
        }
    ...
    const styles = StyleSheet.create({
        ...
        itemContainer: { minWidth: 80, borderWidth: 0.5, borderColor: appcolor.primary, borderRadius: 50, marginStart: 8, paddingHorizontal: 16 },
        headerContainer: { flexDirection: 'row', paddingBottom: 8 },
        titleName: { color: appcolor.dark, fontSize: 13, fontWeight: fontWeightBold, padding: 6, textAlign: 'center' },
        itemHeaderContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', minWidth: 80, borderWidth: 0.5, borderColor: appcolor.primary, backgroundColor: appcolor.primary, borderRadius: 50, paddingHorizontal: 4, marginStart: 8 },
        titleHeaderName: { color: appcolor.light, fontSize: 13, fontWeight: fontWeightBold, padding: 6, textAlign: 'center' },
    })
    const HeaderFilter = () => {
        let viewItem = []
        for (let i = 1; i <= maxGroupLevel; i++) {
            ...
            if (filter[nameKey]) {
                viewItem.push(
                    <TouchableOpacity key={nameKey} style={styles.itemHeaderContainer} onPress={() => onHeaderPress(i)}>
                        <Icon type="ionicon" name="close" size={18} color={appcolor.light} />
