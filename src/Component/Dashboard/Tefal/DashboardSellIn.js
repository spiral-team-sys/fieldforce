...
export const DashboardSellInTF = ({ navigation, typeDashboard, onTitleResolved }) => {
    ...
    const styles = StyleSheet.create({
        mainContainer: { maxHeight: ((deviceHeight / 2) * 0.7) - 60, padding: 5, paddingTop: 10 },
        actionSync: { width: 36, height: 36, borderRadius: 22, borderWidth: 0.3, borderColor: appcolor.dark, position: 'absolute', end: 0, top: 0, zIndex: 10, elevation: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: appcolor.surface }
    })

    return (
        <View style={styles.mainContainer}>
            <TouchableOpacity activeOpacity={0.7} disabled={isLoading} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} onPress={LoadData} style={styles.actionSync}>
                <IconAnimation isLoop={isLoading} sourceIcon={require('../../../Themes/lotties/sync_load.json')} />
            </TouchableOpacity>
            <TouchableOpacity activeOpacity={0.7} style={{ width: '100%' }} onPress={() => navigation.navigate(itemChart.pageName, { typeDashboard: typeDashboard })}>
