...
export const DashBoardTargetTF = ({
  ...
  const [configSheet, _setConfigSheet] = useState({
    itemSelect: {},
    dataItem: [],
    visible: false,
  });
  ...
  const LoadData = async () => {
    setLoading(true);
    await DataSummary(typeDashboard, mData => {
  ...
  const onViewDetail = () => {
    navigation.navigate(
      dataDashboard.dashboardMain?.[0]?.pageName || 'dashboardDetail',
    );
  };
  //
  useEffect(() => {
  ...
  const styles = StyleSheet.create({
    ...
    itemView: {
      ...
      alignItems: 'flex-end',
      justifyContent: 'center',
      backgroundColor: bgViewItem || appcolor.light,
      borderRadius: 5,
      overflow: 'hidden',
      paddingHorizontal: 5,
      borderWidth: 0.5,
  ...
  const UIByCate = () => {
    var uiDashboard = [];
    if (showEmptyState && dataDashboard.dashboardMain?.length === 0) {
      return (
        <View style={styles.emptyContainer}>
          <View style={styles.emptyIcon}>
            <SpiralIcon name="chart-bar" size={14} color="#2563EB" />
          </View>
          ...
          <View key={index.toString()} style={styles.fewCard}>
            <View style={styles.fewTitle}>
              <View
                style={[
                  styles.rowDot,
                  { backgroundColor: getCellColor((index % 3) + 1) },
                ]}
              />
            ...
            <View style={styles.fewCells}>
              {[1, 2, 3].map(column => {
                const cellContent = (
                  <View style={styles.fewCell}>
                    <View
                      style={{
                        ...styles.viewPercent,
                        width: item.isHidePercent == 1 ? 0 : widths[column - 1],
                        backgroundColor: getCellColor(column),
                      }}
                    />
                    <Text
                      numberOfLines={1}
                      style={[styles.fewLabel, { color: getCellColor(column) }]}
                    >
                      {labels[column - 1]}
                    </Text>
                    <Text
                      numberOfLines={1}
                      style={[styles.fewValue, { color: getCellColor(column) }]}
                    >
                ...
                return column == 1 ? (
                  ...
                  <View key={`few_${column}`} style={{ flex: 1 }}>
                    {cellContent}
                  </View>
                );
              })}
    ...
    dataDashboard.dashboardMain?.forEach((item, index) => {
      ...
      uiDashboard.push(
        <View
          ...
          style={[
            styles.itemRow,
            { backgroundColor: index % 2 == 0 ? appcolor.light : '#FAFBFC' },
          ]}
        >
            ...
            >
              ...
              >
                <View
                  style={{
                    ...styles.viewPercent,
                    width: item.isHidePercent == 1 ? 0 : widthDay,
                    backgroundColor: getCellColor(1),
                  }}
                />
                <Text
                  numberOfLines={1}
                  style={[styles.valueView, { color: getCellColor(1) }]}
                >
          ...
          <View style={styles.viewHeadValue}>
            ...
            >
              <View
                style={{
                  ...styles.viewPercent,
                  width: item.isHidePercent == 1 ? 0 : widthWeek,
                  backgroundColor: getCellColor(2),
                }}
              />
              <Text
                numberOfLines={1}
                style={[styles.valueView, { color: getCellColor(2) }]}
              >
                {getColumnValue(item, 2)}
              </Text>
            </View>
          </View>
          <View style={styles.viewHeadValue}>
            <View style={styles.itemView}>
              <View
                style={{
                  ...styles.viewPercent,
                  width: item.isHidePercent == 1 ? 0 : widthMonth,
                  backgroundColor: getCellColor(3),
                }}
              />
              <Text
                numberOfLines={1}
                style={[styles.valueView, { color: getCellColor(3) }]}
              >
                {getColumnValue(item, 3)}
              </Text>
            </View>
          </View>
        </View>,
...
const ViewDetailDashboard = ({
  ...
  const [data, setData] = useState({
    dataView: configSheet.dataItem,
    dataViewF: configSheet.dataItem,
  });
  ...
  const contains = (shop, query) => {
    ...
    let SdealerName =
      DealerName === null ? DealerName : DealerName?.toLowerCase();
    let SlistEmployee =
      listEmployee === null ? listEmployee : listEmployee?.toLowerCase();
    if (
      SshopName?.includes(query) ||
      SshopCode?.includes(query) ||
      Saddress?.includes(query) ||
      SlevelName?.includes(query) ||
      SareaVN?.includes(query) ||
      SdealerName?.includes(query) ||
      SlistEmployee?.includes(query)
    ) {
      return true;
    }
    return false;
  ...
  const handleSearch = text => {
    ...
    } else data.dataView = filteredData;
    setMutate(e => !e);
  };
  ...
  const renderItemFilter = ({ item, index }) => {
    ...
    return (
      ...
      >
        ...
        <Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>
          {item.Address}
        </Text>
        {item.LevelName !== null &&
          ...
          item.LevelName !== '' && (
            <Text
              style={{ fontWeight: '500', fontSize: 12, color: appcolor.dark }}
            >
              {item.LevelName}
            </Text>
          )}
        ...
        {item.DealerName !== null &&
          item.DealerName !== undefined &&
          item.DealerName !== '' && (
            <Text
              style={{ fontWeight: '500', fontSize: 12, color: appcolor.dark }}
            >
        ...
        {listEmployee.length > 0 && (
          ...
          >
            ...
            >
              {listEmployee?.map((it, idx) => {
                return (
                  `${it.EmployeeName} - ${it.EmployeeCode}` +
                  `${listEmployee.length - 1 == idx ? '' : '\n'}`
                );
              })}
            </Text>
          </View>
