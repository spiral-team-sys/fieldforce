...
const SummaryTargetTF = ({ navigation }) => {
  ...
  const DATE = new Date();
  var filter = {
    year: dateSelected?.year || DATE.getFullYear(),
    yearname: `Năm ${dateSelected?.year || DATE.getFullYear()}`,
    month: dateSelected?.month || DATE.getMonth() + 1,
    monthname: `Tháng ${dateSelected?.month || DATE.getMonth() + 1}`,
  };
          ...
          <View style={styles.infoContainer}>
            ...
            <View style={styles.rowBetween}>
              <Text style={styles.target}>
                Target:{' '}
                <Text style={styles.targetValue}>{item.targetUnit}</Text>
              </Text>
              <Text style={styles.actual}>
                Actual:{' '}
                <Text style={styles.actualValue}>{item.actualUnit}</Text>
              </Text>
            </View>
          </View>
        </View>
  ...
  return (
    <View
      style={{ backgroundColor: appcolor.light, height: '100%', width: '100%' }}
    >
      <HeaderCustom
        title={'Tổng số target'}
