...
import { View, Text, FlatList, TouchableOpacity, ScrollView, Image, StyleSheet, SafeAreaView, Platform } from "react-native"
...
export const DashboardRouteTF = ({ navigation, typeDashboard }) => {
    ...
    const LoadData = async () => {
        ...
        await DataSummary(typeDashboard, async (mData) => {
            const chartData = JSON.parse(mData[0]?.chartData || '[]')
            let _details = JSON.parse(chartData[0]?.moreData || '[]')
            _details = [..._details, { viewMonth: true }]
            await setData(chartData[0])
            await setDetails(_details)
            await setItemChart(mData[0])
        });
    ...
    const rowItem = ({ item, index }) => {
        return (
            item.viewMonth == undefined ?
                <TouchableOpacity onPress={() => showDetail(item)} key={index + "dasd6aj"} style={{ margin: 7 }}>
                    <View style={{ backgroundColor: appcolor.light, padding: 12, borderRadius: 12 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <Text style={{ fontWeight: '500', fontSize: scaleSize(18), color: appcolor.dark }}>{item.workDate}</Text>
                        </View>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <TouchableOpacity onPress={() => navigation.navigate("routingmonth")}
                    style={{ alignItems: 'center', justifyContent: 'center', marginEnd: 20 }}>
                    <View style={{
                        borderColor: appcolor.blacklight, borderWidth: 2, borderRadius: 100,
                        alignItems: 'center', justifyContent: 'center', marginEnd: 10,
                    }}>
                        <Icon name="arrow-right" size={20} containerStyle={{ alignSelf: 'center' }} color={appcolor.blacklight} />
                    </View>
    ...
    return (
        <View style={styles.mainContainer}>
            <View style={styles.headerContent}>
                <Icon name="chart-line" type="font-awesome-5" size={18} color={appcolor.blacklight} style={{ padding: 5 }} />
                <Text style={styles.titleDashboard}>{itemChart.chartName || 'Thống kê quãng đường'}</Text>
            ...
            <View style={{ flexDirection: 'row' }}>
                ...
                <View style={{ flexGrow: 1, alignItems: 'center' }}>
                    <Text style={{ fontSize: scaleSize(22), fontWeight: 'bold', color: appcolor.danger, padding: 2 }}>
                        {data?.tValue}</Text>
            ...
            <View style={{
                backgroundColor: appcolor.grey, opacity: 0.1, borderRadius: 700,
            ...
            <ActionSheet
                ...
                <SafeAreaView style={{ height: deviceHeight }}>
                    <View style={{ flexDirection: 'row', width: '100%' }}>
                        <Text style={{ width: '86%', textAlign: 'center', padding: 12, fontSize: scaleSize(18), color: appcolor.primary, fontWeight: 'bold' }}>
