...
export const DashboardRoutingTF = ({ navigation, data, handleRefreshData, loading }) => {
    ...
    const rowItem = ({ item, index }) => {
        return (
            ...
            <TouchableOpacity onPress={() => showDetail(item)} key={index + "dasd6aj"} style={{ marginRight: 7, height: 200 }}>
                <View style={{ backgroundColor: appcolor.surface, padding: 8, borderRadius: 12 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Text style={{ fontWeight: '500', fontSize: scaleSize(16), color: appcolor.dark }}>{item.workDate}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Text style={{ fontSize: scaleSize(11), color: appcolor.dark }}>Đã đi </Text>
                        <Text style={{ flexGrow: 1, textAlign: 'right', fontSize: scaleSize(16), color: appcolor.dark }}>{item.shopActual || "0"}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Text style={{ fontSize: scaleSize(11), color: appcolor.dark }}>Chỉ tiêu </Text>
                        <Text style={{ flexGrow: 1, textAlign: 'right', fontSize: scaleSize(16), color: appcolor.dark }}>{item.shopPlan || "0"}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Text style={{ fontSize: scaleSize(11), color: appcolor.dark }}>Số km đã đi </Text>
                        <Text style={{ flexGrow: 1, fontWeight: '700', textAlign: 'right', fontSize: scaleSize(16), color: appcolor.success }}>{item.distance}</Text>
                    </View>
                </View>
            </TouchableOpacity>
            //     :
            ...
            //     }}>
            //         <Icon name="arrow-right" size={20} containerStyle={{ alignSelf: 'center' }} color={appcolor.light} />
            //     </View>
    ...
    return (
        <View style={{ width: '100%', opacity: 0.9 }}>
            <TouchableOpacity onPress={loading ? null : handleRefreshData}
                style={{
                    borderRadius: 30, borderWidth: 0.3, borderColor: appcolor.dark,
                    padding: 5, position: 'absolute', end: 0, backgroundColor: appcolor.surface,
                    position: 'absolute', top: 0, right: 10, zIndex: 1000
                }}>
                <IconAnimation isLoop={false} sourceIcon={require('../../../Themes/lotties/sync_load.json')} />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => navigation.navigate("attendanthistory")}
                style={{ paddingTop: 30 }}
            >
                <View style={{ flexDirection: 'row' }}>
                    <View style={{ flexGrow: 1, alignItems: 'center' }}>
                        <Text style={{ fontSize: scaleSize(22), fontWeight: 'bold', color: appcolor.success, padding: 2 }}>
                            {data?.aValue || '0'}</Text>
                        <Text style={{ fontSize: scaleSize(12), color: appcolor.dark }}>{data?.aLable}</Text>
                    </View>
                    <View style={{ flexGrow: 1, alignItems: 'center' }}>
                        <Text style={{ fontSize: scaleSize(22), fontWeight: 'bold', color: appcolor.warning, padding: 2 }}>
                            {data?.tValue}</Text>
                        <Text style={{ fontSize: scaleSize(12), color: appcolor.dark }}>
                            {data?.tLable}</Text>
                    </View>
                    <View style={{ flexGrow: 1, alignItems: 'center' }}>
                        <Text style={{ fontSize: scaleSize(22), fontWeight: 'bold', color: appcolor.info, padding: 2 }}>
                            {data?.pValue || '0%'}</Text>
                        <Text style={{ fontSize: scaleSize(12), color: appcolor.dark }}>
                            {data?.pLable || '%'}</Text>
                    </View>
            ...
            <View style={{
                backgroundColor: appcolor.primary, opacity: 0.2, borderRadius: 700,
                position: 'absolute', width: 170, height: 170, left: '-9%', top: '-9%'
            }} />
            <View style={{
                backgroundColor: appcolor.primary, opacity: 0.5, borderRadius: 700,
                position: 'absolute', width: 170, height: 170, right: '-20%', bottom: '-30%'
            }} />
            <View style={{ flexGrow: 1 }}></View>
            <View style={{ marginVertical: 8, height: 100, flexDirection: 'row', justifyContent: 'space-between' }}>
                {
            ...
            <ActionSheet bottomOffset={50} id="shopList"
...
const Routing = ({ data, route }) => {
    ...
    var totalTime = routeList.length ? routeList?.map(o => o.Duration.value).reduce((a, c) => { return a + c }) : 0;
    ...
    return (
        <View style={{ flex: 1, alignItems: 'center', backgroundColor: appcolor.surface }}>
            <FlatList
