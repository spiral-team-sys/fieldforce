...
export const DashboardSummarySSub = ({ navigation, route }) => {
    ...
    var [monthFilter, setMonthFilter] = useState({ "year": DATE.getFullYear(), "yearname": `Năm ${DATE.getFullYear()}`, "month": DATE.getMonth() + 1, "monthname": `Tháng ${DATE.getMonth() + 1}` })
    const [search, setSearch] = useState('')

    const loadData = async () => {
        await setLoading(true)
        // const dataJson = jsonCalendar || JSON.stringify({ year: moment().year(), month: moment().month() + 1 })
        const dataJson = { year: monthFilter.year, month: monthFilter.month, shopId: shopinfo.shopId }
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View style={{ minHeight: 30, flexDirection: 'row', paddingTop: 5, paddingHorizontal: 5, width: deviceWidth, marginTop: index == 0 ? 10 : 0 }}>
                ...
                <View style={{ width: '15%', backgroundColor: index % 2 == 0 ? appcolor.homebackground : appcolor.light, justifyContent: 'center', alignItems: "center" }}>
                    <Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark, textAlign: 'center' }}>{item.fistStock == 0 ? 0 : (item.fistStock || '-')}</Text>
                </View>
                <View style={{ width: '15%', backgroundColor: index % 2 == 0 ? appcolor.homebackground : appcolor.light, justifyContent: 'center', alignItems: "center" }}>
                    <Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark, textAlign: 'center' }}>{item.sellOutByMonth == 0 ? 0 : (item.sellOutByMonth || '-')}</Text>
                </View>
                <View style={{ width: '15%', backgroundColor: index % 2 == 0 ? appcolor.homebackground : appcolor.light, justifyContent: 'center', alignItems: "center" }}>
                    <Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark, textAlign: 'center' }}>{item.sellInByMonth == 0 ? 0 : (item.sellInByMonth || '-')}</Text>
                </View>
                <View style={{ width: '15%', backgroundColor: index % 2 == 0 ? appcolor.homebackground : appcolor.light, justifyContent: 'center', alignItems: "center" }}>
                    <Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark, textAlign: 'center' }}>{(item.lastStock == 0 && item.fistStock !== null) ? 0 : (item.lastStock || '-')}</Text>
                </View>
            </View>
        )
    }
    const TitleDashboard = () => {
        return (
            <View style={{ minHeight: 30, marginTop: 10, paddingHorizontal: 5, flexDirection: 'row', width: deviceWidth, }}>
                <View style={{ width: '40%', paddingVertical: 5, backgroundColor: appcolor.homebackground, justifyContent: 'center', alignItems: "center" }}>
                    <Text style={{ fontWeight: '600', fontSize: 13, color: appcolor.dark, textAlign: 'center' }}>Sản phẩm</Text>
                </View>
                <View style={{ width: '15%', backgroundColor: appcolor.light, justifyContent: 'center', alignItems: "center" }}>
                    <Text style={{ fontWeight: '600', fontSize: 13, color: appcolor.dark, textAlign: 'center' }}>Đầu kì</Text>
                </View>
                <View style={{ width: '15%', backgroundColor: appcolor.homebackground, justifyContent: 'center', alignItems: "center" }}>
                    <Text style={{ fontWeight: '600', fontSize: 13, color: appcolor.dark, textAlign: 'center' }}>SellOut</Text>
                </View>
                <View style={{ width: '15%', backgroundColor: appcolor.light, justifyContent: 'center', alignItems: "center" }}>
                    <Text style={{ fontWeight: '600', fontSize: 13, color: appcolor.dark, textAlign: 'center' }}>SellIn</Text>
                </View>
                <View style={{ width: '15%', backgroundColor: appcolor.homebackground, justifyContent: 'center', alignItems: "center" }}>
                    <Text style={{ fontWeight: '600', fontSize: 13, color: appcolor.dark, textAlign: 'center' }}>Cuối kì</Text>
                </View>
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.surface }}>
            <HeaderCustom
                title={shopinfo?.shopName + ' (' + monthFilter.month + '/' + monthFilter.year + ')'}
            ...
            <ActionSheet id="sheetYearSSub" containerStyle={{ backgroundColor: appcolor.light }}>
