...
export const ScreenGroup = ({ navigation, route }) => {
    ...
    const styles = StyleSheet.create({
        ...
        titleHead: { fontSize: 14, color: appcolor.dark, fontWeight: fontWeightBold },
        titleContent: { fontSize: 12, color: appcolor.placeholderText, fontWeight: '500' },
        ...
        return (
            <View style={styles.headerMain}>
                ...
                <View style={{ ...styles.itemHeaderView, width: '30%' }}>
                    <Text style={styles.titleGroup}>{itemHeader.TitleValue || 'OOSDays'}</Text>
                </View>
            </View>
        )
    }
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View key={`ism_r-${index}`} style={styles.itemMain}>
                ...
                <View style={{ ...styles.itemView, width: '50%', backgroundColor: backgroundColor }}>
                    <Text style={styles.titleValue}>{item.ProductName || '-'}</Text>
                </View>
                <View style={{ ...styles.itemView, width: '30%', backgroundColor: backgroundColor }}>
