...
import { DeviceEventEmitter, RefreshControl, StyleSheet, View } from "react-native";
...
export const ScreenListProduct = ({ itemGroup }) => {
    ...
    const [search, _setItemSearch] = useState({ text: '', tagName: null, isSearch: false })
    ...
    const LoadData = async () => {
        ...
        await OOSAPI.GetListSummary('PRODUCT', itemGroup || {}, async (mData, message) => {
    ...
    const FilterData = async (filterObject) => {
        await setLoading(true)
        await OOSAPI.GetListSummary('PRODUCT', { ...filterObject, ...itemGroup }, async (mData, message) => {
            message && ToastError(message)
            const tabList = _.unionBy(mData, 'tabName')
            await setDataTab([])
            await setDataTab(tabList)
    ...
    const _searchData = (filterList) => {
        ...
        //   
        dataSearchMain = _.filter(dataSearchMain, (e) => (e.highlightDescription == search.tagName || search.tagName == null))
        //
        const valueSearch = removeVietnameseTones(search.text).toLowerCase()
        const searchData = _.filter(dataSearchMain, (e) => (
    ...
    const onSearchData = async (text) => {
        ...
        const _groupByTag = _.filter(listUpdate, (e) => (e.highlightDescription == search.tagName || search.tagName == null))
    ...
    const handlerSearchByGroup = async (item, keyValue, isMultiple) => {
        ...
        const _dataShow = _productByGroup !== null && _productByGroup.length > 0 ? _productByGroup : listChooseGroup
    ...
    useEffect(() => {
        let isMounted = true
        if (!isMounted) return
        const _filterData = DeviceEventEmitter.addListener('FILTER_DATA_OOS', FilterData)
        const _sortByTag = DeviceEventEmitter.addListener('SORT_TAG_OOS', handlerSortByTag)
        LoadData()
        return () => {
            isMounted = false
            _filterData.remove()
    ...
    const renderItem = ({ item, index }) => {
        return <ItemProduct item={item} index={index} onLayout={handleItemLayout} onPress={handlerShowStore} />
    }
    return (
            ...
            {dataTab !== null && dataTab.length > 0 &&
                <Tabs.Container
                    ...
                    {dataTab.map((item, index) => {
                        ...
                        const titleHead = `${item.tabName}${_dataProduct.length > 0 ? ` (${_dataProduct.length})` : ``}`
                        return (
                            <Tabs.Tab key={`tabisum_${index}`} label={titleHead} name={titleHead} >
