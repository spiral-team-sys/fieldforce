...
export const SummaryDataByShop = ({ navigation }) => {
    ...
    const LoadData = async () => {
        ...
        await DashboardAPI.GetSummaryDataByShop(shopinfo.shopId, async (mData, message) => {
    ...
    const styles = StyleSheet.create({
        ...
        groupTitle: { fontSize: 15, fontWeight: fontWeightBold, color: appcolor.dark }
    })
    const renderItem = ({ item, index }) => {
        ...
        const SummaryItem = () => {
            switch (item.ref_Name) {
                case "ATTENDANT":
                    return <AttendantSummary key={`ATTENDANT_${index}`} summaryData={dashboardData} />
                case "DISPLAY":
                    return <DisplaySummary key={`DISPLAY_${index}`} summaryData={dashboardData} />
                case "SELLOUT":
                    return <SellOutSummary key={`SELLOUT_${index}`} summaryData={dashboardData} />
                case "SELLIN":
                    return <SellInSummary key={`SELLIN_${index}`} summaryData={dashboardData} />
                case "INVENTORY":
                    return <InventorySummary key={`INVENTORY_${index}`} summaryData={dashboardData} />
            }
        }
        return (
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <View style={styles.contentMain}>
                <LoadingView isLoading={isLoading} title='Đang cập nhật dữ liệu thống kê' />
                <FlashList
