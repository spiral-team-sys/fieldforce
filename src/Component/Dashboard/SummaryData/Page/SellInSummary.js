...
export const SellInSummary = ({ summaryData }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    const [dataSellIn, setDataSellIn] = useState([])

    const LoadData = async () => {
        await setDataSellIn(summaryData)
    }
    ...
    const styles = StyleSheet.create({
        ...
        itemMain: { width: '100%' },
        contentMain: { width: '100%', backgroundColor: appcolor.light, borderRadius: 8, shadowColor: appcolor.dark, shadowOffset: { width: 3, height: 0 }, shadowOpacity: 0.5, elevation: 3, overflow: 'hidden' },
    ...
    return (
        <View style={styles.mainContainer}>
            {dataSellIn !== null && dataSellIn.length > 0 && dataSellIn.map((item, index) => { return renderItem(item, index) })}
        </View>
    )
}