...
export const InventorySummary = ({ summaryData }) => {
    ...
    const styles = StyleSheet.create({
        ...
        contentMain: { width: '100%', backgroundColor: appcolor.info, borderRadius: 8, shadowColor: appcolor.dark, shadowOffset: { width: 3, height: 0 }, shadowOpacity: 0.5, elevation: 3, overflow: 'hidden' },
        itemView: { width: '100%', padding: 8, alignItems: 'center' },
        titleView: { fontSize: 14, fontWeight: fontWeightBold, color: appcolor.light, fontStyle: 'italic' },
        subTitleView: { fontSize: 15, fontWeight: '500', color: appcolor.light },
        titlePriceView: { fontSize: 24, fontWeight: 'bold', color: appcolor.yellowdark },
        ...
        return (
            <View key={`is_${index}`} style={styles.itemMain}>
                ...
                <View style={styles.contentMain}>
                    {dataSummary !== null && dataSummary.length > 0 && dataSummary.map((i, idx) => {
                        return (
                            <View key={`isi_${idx}`} style={styles.itemView}>
                                ...
                                <Text style={styles.titleView}>{i.QLable}{i.AQuantity}</Text>
                            </View>
                        )
                    })}
                </View>
    ...
    return (
        <View style={styles.mainContainer}>
            {dataInventory !== null && dataInventory.length > 0 && dataInventory.map((item, index) => {
