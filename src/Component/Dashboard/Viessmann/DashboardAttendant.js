...
export const DashboardAttendant = ({ navigation, typeDashboard }) => {
	...
	const styles = StyleSheet.create({
		...
		viewPercent: { overflow: 'hidden', alignItems: 'center', minHeight: 38, position: 'absolute' },
		descriptionView: { flexDirection: 'row-reverse' }
	})
	const renderItem = (item, index) => {
		const timeLine = (
			<View style={{ width: '80%', minHeight: 38, borderRadius: 8, overflow: "hidden", flexDirection: 'row' }}>
				<View style={{ ...styles.viewPercent, backgroundColor: appcolor.success, width: '100%', opacity: 0.1 }} />
				<View style={{ ...styles.viewPercent, backgroundColor: appcolor.success, width: `${item.percent}%`, opacity: 0.8 }} />
				<Text style={{ width: '100%', alignSelf: 'center', textAlign: 'right', padding: 5, fontSize: 13, fontWeight: '600', color: appcolor.dark, paddingEnd: 8 }}>{`${item.actual} / ${item.target}`}</Text>
			</View>
		)
		return (
			<View key={`dds_${index}`}>
				<View style={styles.contentItem}>
					<Text style={{ width: '20%', fontSize: 13, fontWeight: '600', fontStyle: 'italic', color: appcolor.blacklight }}>{item.titleName}</Text>
					{timeLine}
				</View>
			</View>
		)
		...
		<TouchableOpacity onPress={showDetails} >
			<View style={styles.mainContainer}>
				...
				<View style={styles.descriptionView}>
					<Text style={{ fontSize: 12, fontWeight: '400', color: appcolor.dark, marginStart: 8, marginEnd: 3 }}>{itemMain.descriptionTarget}</Text>
					<Icon name="square" type="font-awesome-5" solid size={15} color={appcolor.success} style={{ opacity: 0.1 }} />
					<Text style={{ fontSize: 12, fontWeight: '400', color: appcolor.dark, marginStart: 8, marginEnd: 8, opacity: 0.8 }}>{itemMain.descriptionActual}</Text>
