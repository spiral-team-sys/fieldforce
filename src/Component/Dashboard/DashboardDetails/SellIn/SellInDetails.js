...
export const SellInDetails = ({ tagView, yearValue, monthValue }) => {
    ...
    const onSearchData = (text) => {
        ...
        const filterList = _.filter(dataMain, (e) => removeVietnameseTones(e.n1).toLowerCase().match(_searchValue))
        setData(filterList)
    }
    const onFocusSearch = () => {
    ...
    const styles = StyleSheet.create({
        ...
        titleNoData: { width: '100%', textAlign: 'center', color: appcolor.primary, fontSize: 14, fontWeight: fontWeightBold, padding: 8 },
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            />
            {!isLoading && dataMain == null && <Text style={styles.titleNoData}>{`Không có dữ liệu Tháng ${monthValue} - ${yearValue}`}</Text>}
