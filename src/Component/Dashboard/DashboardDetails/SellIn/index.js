...
export const DashboardSellInDetails = ({ navigation }) => {
    ...
    const [itemCalendar, setItemCalendar] = useState({ tagFilter: 'Dealer', year: moment().year(), month: moment().month() + 1 })
    const [itemCalendarLocal, setCalendarLocal] = useState({ tagFilter: 'Dealer', year: moment().year(), month: moment().month() + 1 })
    //
    const onBack = () => {
        navigation.goBack()
    }
    ...
    const handlerSearchCalendar = (info) => {
        setCalendarLocal({ ...itemCalendarLocal, year: info.year, month: info.month })
    }
    ...
    const styles = StyleSheet.create({
        ...
        titleHead: { paddingHorizontal: 8, fontSize: 14, color: appcolor.greylight, fontWeight: Platform.OS == 'ios' ? '600' : '700' }
    })
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            {/* // Search Calendar */}
            <ActionSheet id="search_calendar"
                onClose={onSearchAction}
                gestureEnabled
                containerStyle={{ width: '100%', height: deviceHeight / 2.5 }} >
                <FilterByTag
