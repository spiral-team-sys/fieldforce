...
export const GroupItemView = ({ dataMain, reload }) => {
    ...
    useEffect(() => {
        ...
        const _scrollToItem = DeviceEventEmitter.addListener('SCROLL_TO_ITEM', scrollToIndexMain)
        return () => {
            _load
    ...
    const styles = StyleSheet.create({
        ...
        itemMain: { width: '100%', padding: 8, borderRadius: 5, shadowOpacity: 0.5, elevation: 3, backgroundColor: appcolor.light, shadowOffset: { width: 3, height: 0 }, shadowColor: appcolor.darkt, marginBottom: 4, marginTop: 4 },
        titleGroupHead: { width: '100%', fontSize: 14, color: appcolor.dark, fontWeight: Platform.OS == 'ios' ? '600' : '700' },
        titleSubGroupHead: { width: '100%', fontSize: 12, color: appcolor.greylight, fontWeight: '500' },
            ...
            <View key={`${index}_ig1`} style={styles.viewItemContentMain} >
                <TouchableOpacity style={styles.itemMain} onPress={onPress}>
                    <View style={styles.viewHead}>
                        <Icon type="ionicon" name={item.iconG1} size={18} color={appcolor.blacklight} style={{ paddingEnd: 8 }} />
                        <View style={styles.viewSummary}>
                            <Text style={styles.titleGroupHead}>{item.n1}</Text>
                            {item.subn1 && <Text style={styles.titleSubGroupHead}>{item.subn1}</Text>}
                ...
                {item.isChoose == 1 &&
                    <View style={styles.subView}>
                        <SubGroupItemView key={`sublist_${item.g1}_${index}`} dataMain={subList} indexMain={index} />
                    </View>
                }
            </View>
        )
