...
export const SubGroupItemView = ({ dataMain, indexMain }) => {
  ...
  const handleSearch = text => {
    ...
    const listSearch = _.filter(dataMain, e =>
      removeVietnameseTones(e.n2).toLowerCase().match(valueSearch),
    );
    setData(listSearch);
  };
  const onFocusSearch = () => {
    ...
    return (
      <View key={`${index}_sig2`} style={styles.viewItemContentMain}>
        <TouchableOpacity
          ...
          onPress={onPress}
          disabled={item.waitRender == 1}
        >
          <View style={styles.viewHead}>
            <SpiralIcon
              type="ionicon"
              name={item.iconG2 || 'reorder-three'}
              size={18}
            ...
            <View
              style={{
                ...styles.viewSummary,
                width: item.waitRender == 1 ? '82%' : '86%',
              }}
            >
              <Text style={styles.titleGroupHead}>{item.n2}</Text>
              {item.subn2 && (
                <Text style={styles.titleSubGroupHead}>{item.subn2}</Text>
              )}
              {item.TotalSaleValue && (
            ...
            {item.waitRender == 1 ? (
              ...
              <SpiralIcon
                type="ionicon"
                name={item.isChoose == 1 ? 'chevron-up' : 'chevron-down'}
                size={16}
                color={appcolor.blacklight}
              />
            )}
