...
export const ListItemView = ({
  ...
  const renderItem = ({ item, index }) => {
    ...
    const hasConfirmStatus =
      item.IsConfirm !== undefined &&
      item.IsConfirm !== null &&
      item.IsConfirm !== '';
    const hasConfirmName =
      item.FullName !== undefined &&
      item.FullName !== null &&
      `${item.FullName}`.trim() !== '';
    const hasConfirmNote =
      item.ConfirmNote !== undefined &&
      item.ConfirmNote !== null &&
      `${item.ConfirmNote}`.trim() !== '';
    const hasConfirmInfo = hasConfirmStatus || hasConfirmName || hasConfirmNote;
    const isApproved =
      item.IsConfirm === true ||
      item.IsConfirm === 1 ||
      item.IsConfirm === '1' ||
      `${item.IsConfirm}`.toLowerCase() === 'true';
    const confirmColor = hasConfirmStatus
      ? isApproved
        ? appcolor.success
        : appcolor.danger
      : appcolor.primary;
        ...
        <View key={`${index}_iig3`} style={styles.itemMain}>
          {item.isParent ? (
            <View style={styles.viewHead}>
              <SpiralIcon
                type="ionicon"
                name={item.iconG3 || 'person-circle'}
                size={18}
                color={appcolor.primary}
                style={{ paddingEnd: 8 }}
              />
              <Text style={styles.titleGroupHead}>{item.n3}</Text>
              <Text style={styles.titlePrice}>{`${formatNumber(
                totalSale || 0,
                ',',
              )} ${item.UnitPrice}`}</Text>
            </View>
          ...
          <View style={styles.viewContent}>
            ...
            >{`Sản phẩm: ${item.ProductName}`}</Text>
            {item.OrderValue !== undefined ? (
              <Text
                style={styles.titleContent}
              >{`Số lượng: ${item.OrderValue}`}</Text>
            ) : null}
            {item.Price !== undefined ? (
              <Text
                style={styles.titleContent}
              >{`Giá: ${item.Price} ${item.UnitPrice}`}</Text>
            ) : null}
            {item.PriceNPP !== undefined ? (
              <Text
                style={styles.titleContent}
              >{`Giá NPP: ${item.PriceNPP} ${item.UnitPrice}`}</Text>
            ) : null}
            {item.PercentShareDetail !== undefined ? (
              <Text
                style={styles.titleContent}
              >{`Tỉ lệ: ${item.PercentShareDetail}%`}</Text>
            ) : null}
            {item.SalePrice !== undefined ? (
              <Text
                style={styles.titleContent}
              >{`Doanh số: ${item.SalePrice} ${item.UnitPrice}`}</Text>
            ) : null}
            {hasConfirmInfo ? (
