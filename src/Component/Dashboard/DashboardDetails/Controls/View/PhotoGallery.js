    ...
    return (
      <View key={`pg_item_${index}`} style={styles.itemMain}>
        ...
        {keyLayer2 && (
          ...
          >
            <Text
              style={{ color: appcolor.white, fontWeight: '900', fontSize: 16 }}
            >{`${moment(item.photoDate).format('DD/MM/YYYY')}`}</Text>
          </View>
        )}
        <TouchableOpacity style={styles.viewPhoto} onPress={onPress}>
          <View style={styles.viewPosition}>
            <Text style={styles.titleTimeView}>{`${
              item.photoNote ? item.photoNote + ' -' : ''
            } ${moment(item.photoTime).format('DD,MMM/YYYY')}`}</Text>
          </View>
          <Image
