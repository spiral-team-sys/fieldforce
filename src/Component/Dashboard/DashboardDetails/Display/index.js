...
export const DashboardDisplayDetails = ({ navigation, route }) => {
  ...
  const onSortData = async type => {
    ...
    //
    const itemFilter = {
      ...dashboardFilter[`${route?.params?.pageName}`],
      isViewProduct: value,
    };
    await dispatch(
      setDashboardFilter({ [`${route?.params?.pageName}`]: itemFilter }),
    );
    await DeviceEventEmitter.emit(
      'RELOAD_DASHBOARD_DETATLS_DISPLAY',
      itemFilter,
    );
  };
  //
    ...
    <View style={styles.mainContainer}>
      ...
      <View style={styles.contentMain}>
        {itemDetails.typeDisplay == 'DISPLAYSHARE' ? (
          <DisplayShareDetails
            key="DISPLAYSHARE_DETAILS"
            dataDetails={itemDetails.data}
          />
        ) : (
          <DisplayDetails
            key="DISPLAY_DETAILS"
            pageName={route?.params?.pageName}
            masterFilter={route?.params?.dataFilter}
          />
        )}
      </View>
