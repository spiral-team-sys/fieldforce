...
export const DisplayDetails = ({ pageName, masterFilter }) => {
  ...
  const LoadData = async (filter = null) => {
    ...
    const itemFilter = {
      ...
      jsonFilter: await JSON.stringify(
        filter || dashboardFilter[`${pageName}`],
      ),
    };
  ...
  useEffect(() => {
    let isMounted = true;
    if (!isMounted) return;
    const reload_dashboard = DeviceEventEmitter.addListener(
    ...
    <View style={styles.mainContainer}>
      ...
      >
        ...
        <View style={styles.listPhotoView}>
          {dataPhoto.length == 0 && (
            <Text style={styles.titleNoneImage}>Không có hình ảnh</Text>
          )}
