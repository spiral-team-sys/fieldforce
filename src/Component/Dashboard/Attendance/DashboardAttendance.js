...
const DashboardAttendance = ({ navigation }) => {
  ...
  const LoadData = async (pageNum, date) => {
    ...
    const result =
      jsonCalendar === null || hour !== 21
        ? await LGPLanbyWeekly(filter)
        : localData;
    if (result.statusId === 200) {
      let itemData = result?.data || [];
      if (itemData.length === 7) {
        await itemData.unshift({
          dayName: 'arrow-left',
          pageNum: -7,
        ...
        await itemData.push({
          dayName: 'arrow-right',
          pageNum: 7,
          date: itemData[7].date,
        });
      }
      await setData(itemData);
      if (itemData.length > 5)
        await refList.current.scrollToIndex({
          index: 4,
          animated: true,
          viewPosition: 0.5,
        });
    ...
    const renderDay = () => (
      <TouchableOpacity style={styles.itemDayMain} onPress={onDayPress}>
        <View style={{ ...styles.itemContentDay, backgroundColor: dayColor }}>
          ...
          <Text style={{ ...styles.titleContent, paddingTop: 8 }}>
            {item.target || '-'}
          </Text>
