                ...
                viewAll.push(
                    <View key={'item_' + index} style={{}}>
                        {item.isParent && item.RGroup !== null && item.RGroup?.length > 0 &&
                            <View style={{ flexDirection: 'row', alignItems: 'center', borderRadius: 5, marginTop: 8, marginBottom: 4 }}>
                                <View style={{ width: '20%' }}>
                                    <Text style={{ fontSize: 16, fontWeight: 'bold', color: appcolor.primary }}>{item.RGroup}</Text>
                                </View>
                                <View style={{ width: '30%', alignItems: 'center' }}>
                                    <Text style={{ fontSize: 13, textAlign: 'center', fontWeight: 'bold', color: appcolor.primary }}>{item.totalTargetByCate}</Text>
                                </View>
                                <View style={{ width: '30%', alignItems: 'center', }}>
                                    <Text style={{ fontSize: 13, textAlign: 'center', fontWeight: 'bold', color: appcolor.primary }}>{item.totalActualByCate}</Text>
                                </View>
                                <View style={{ width: '20%' }}>
                                    <Text style={{ fontSize: 13, textAlign: 'center', fontWeight: 'bold', color: appcolor.primary }}>{item.totalPercentByCate}</Text>
                                </View>
                            </View>
                        }
                        ...
                        <View key={index.toString()} style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <View style={{ width: '20%' }}>
                                <Text style={{ paddingLeft: 12, fontSize: 15, color: appcolor.dark, paddingTop: 5, paddingBottom: 5 }}>{item.RTime}</Text>
                            </View>
                            <View style={{ width: '30%', alignItems: 'center' }}>
                                <Text style={{ fontWeight: '700', fontSize: 14, color: appcolor.tomato, paddingTop: 5, paddingBottom: 5 }}>{item.Target || 0}</Text>
                            </View>
                            <View style={{ width: '30%', alignItems: 'center', }}>
                                <Text style={{ fontWeight: '700', fontSize: 14, color: appcolor.second, paddingTop: 5, paddingBottom: 5 }}>{item.Actual || 0}</Text>
                            </View>
                            <View style={{ width: '20%' }}>
