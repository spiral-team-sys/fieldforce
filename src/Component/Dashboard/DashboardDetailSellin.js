...
import { View, StyleSheet, FlatList, Text, Platform, TouchableOpacity, SafeAreaView } from "react-native";
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View key={index} style={styles.itemContainer}>
                {item.isParent &&
                    <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: appcolor.secondary, padding: 5, borderRadius: 5, marginBottom: 8 }}>
                        <Icon name='warehouse' color={appcolor.light} size={16} style={{ marginEnd: 8 }} />
                        <Text style={styles.titleHead}>Nhà phân phối: {item.DealerName}</Text>
                    </View>
                }
                ...
                <View style={{ flex: 1, flexDirection: 'row' }}>
                    ...
                    <View style={styles.itemStyle} key={index}>
                        ...
                        {item.Price && <Text style={styles.itemText}>Giá: {formatNumber(item.Price, ',')}</Text>}
                        {item.PriceNPP && <Text style={styles.itemText}>Giá NPP: {formatNumber(item.PriceNPP, ',')}</Text>}
                    </View>
                </View>
            </View>
        )
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <FlatList
                ...
                style={{ flex: 1, padding: 5, marginBottom: Platform.OS == 'ios' ? 8 : 0 }}
