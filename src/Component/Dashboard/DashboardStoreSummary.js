...
const DashboardStoreSummary = ({ info, isReload = false, navigation }) => {
    ...
    const LoadData = async () => {
        await DashboardAPI.GetDashboardStoreSummary(shopinfo.shopId, async (mData, message) => {
    ...
    const styles = StyleSheet.create({
        mainContainer: { flex: 1, backgroundColor: appcolor.light, marginBottom: 12, paddingBottom: 8, borderRadius: 8, overflow: 'hidden' },
        viewTitleChart: { flexDirection: 'row', alignItems: 'center', padding: 8, paddingBottom: 0 },
        itemContainer: { backgroundColor: appcolor.light, borderWidth: 1, borderColor: appcolor.grayLight, margin: 8, marginBottom: 0, borderRadius: 8, padding: 8, },
        titleChart: { width: '85%', marginEnd: 8, color: appcolor.dark, marginStart: 8, fontWeight: fontWeightBold, fontSize: 14 },
        ...
        titlePage: { width: '100%', fontSize: 13, fontWeight: fontWeightBold, color: appcolor.dark, marginTop: 8 },
        itemPlanContainer: { backgroundColor: appcolor.surface, margin: 8, marginBottom: 0, borderRadius: 8, padding: 8, flexDirection: 'row' },
        viewStoreInfo: { backgroundColor: appcolor.light, margin: 8, paddingBottom: 8, borderRadius: 8, elevation: 3, shadowColor: appcolor.dark, shadowOffset: { width: 3, height: 3 }, shadowOpacity: 0.5, shadowRadius: 8, overflow: 'hidden' },
        viewPlanByShop: { backgroundColor: appcolor.light, margin: 8, paddingBottom: 8, borderRadius: 8, elevation: 3, shadowColor: appcolor.dark, shadowOffset: { width: 3, height: 3 }, shadowOpacity: 0.5, shadowRadius: 8, overflow: 'hidden' },
    ...
    const headerSummary = () => {
        ...
        const typePCName = dataPCType.length > 0 ? _.join(_.map(dataPCType, i => `      ${i.CategoryName} - (${i.TotalQuantity})`), '\n') : null
        return (
            <View style={styles.itemContainer}>
                {item.address && <Text style={styles.titleName}>{`Địa chỉ: ${item.address}`}</Text>}
                {item.storeManager && <Text style={styles.titleName}>{`${item.storeManager}`}</Text>}
                {item.openYear && <Text style={styles.titleName}>{`${item.openYear}`}</Text>}
                {item.storeGrade && <Text style={styles.titleName}>{`${item.storeGrade}`}</Text>}
                {item.numberofFSM && <Text style={styles.titleName}>{`${item.numberofFSM}`}</Text>}
                {typePCName && <Text style={styles.titleName}>{`${item.titleTypePC}\n${typePCName}`}</Text>}
            </View>
        )
    }
            ...
            <View>
                {index == 0 &&
                    <View style={styles.viewTitleChart}>
                        <Icon color={appcolor.primary} type="font-awesome-5" name="users" size={18} />
                        <Text style={styles.titleChart}>{`Thống kê PG tại cửa hàng`}</Text>
                    </View>
                }
                <TouchableOpacity disabled={item.employeeId == 0} onPress={onPress} style={styles.itemPlanContainer}>
                    <View style={styles.viewTitleEmployee}>
                        <Text style={styles.titleName}>{item.fullName}</Text>
                        {item.experience && <Text style={styles.subTitleName}>{item.experience}</Text>}
                        {item.shiftType && <Text style={styles.subTitleName}>{`${item.shiftType} ${item.shiftName || ''}`}</Text>}
                    </View>
                    {item?.mobile &&
        ...
        return (
            <View style={{ ...styles.itemContainer, flexDirection: 'column' }}>
                <View style={styles.viewTitleDate}>
                    <Icon color={appcolor.dark} type="font-awesome-5" name="calendar-day" size={16} style={{ marginEnd: 5 }} />
                    <Text style={styles.titleName}>{item.DateView}</Text>
                </View>
                <Text style={styles.subTitleName}>{`        ${item.ShiftType} ${item.ShiftName}`}</Text>
            </View>
        )
    }
    return (
        <View style={styles.mainContainer}>
            <TouchableOpacity style={styles.viewTitleChart} onPress={handlerShowSummary}>
                <Icon color={appcolor.primary} type="font-awesome-5" name="store" size={20} />
                <Text style={styles.titleChart}>{info !== null ? info.chartName : ''}</Text>
                <Icon color={appcolor.greylight} type="font-awesome-5" name={isShowDetail ? "caret-up" : "caret-down"} size={18} />
            </TouchableOpacity>
            {isShowDetail &&
                <FlashList
            ...
            <ActionSheet id='plan-detail-sheet' gestureEnabled onBeforeShow={setDataSheet} >
