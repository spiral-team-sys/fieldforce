...
export const DashboardSellOutToshiba = ({ navigation, typeDashboard, viewHeight }) => {
    ...
    const styles = StyleSheet.create({
        ...
        titleDashboard: { width: '80%', fontSize: 14, fontWeight: '700', color: appcolor.blacklight },
    ...
    if (isLoading) {
        ...
        return (
            <TouchableOpacity onPress={showDetailData} style={{ maxHeight: viewHeight - 40, padding: 8 }} >
                    ...
                    <View style={{ width: '100%', flexDirection: "row" }}>
                        ...
                        <View style={{ width: '50%', alignItems: 'center', justifyContent: 'center' }}>
                            <View style={{ width: '100%', justifyContent: 'center' }}>
                                <View style={{ flexDirection: 'row', alignItems: "center" }}>
                                    <Text style={{ fontSize: 12, fontWeight: '500', color: appcolor.dark, padding: 5 }}>{`Target`}</Text>
                                    <Text style={{ fontSize: scaleSize(12), fontWeight: '800', color: appcolor.tomato, marginStart: 8 }}>{`${dataSellOut.targetValue || 0}`}</Text>
                                </View>
                                <Text style={{ fontSize: 12, fontWeight: '500', color: appcolor.dark, padding: 5 }}>{`Actual`}</Text>
                                <Text style={{ fontSize: scaleSize(12), fontWeight: '700', color: appcolor.info, marginStart: 8 }}>{`${dataSellOut.l1 || 'Số lượng'}: ${dataSellOut.v1 || 0}`}</Text>
                                <Text style={{ fontSize: scaleSize(12), fontWeight: '700', color: appcolor.info, marginStart: 8 }}>{`${dataSellOut.l2 || 'Thành tiền'}: ${dataSellOut.v2 || 0}`}</Text>
                            </View>
                        </View>
                        {/* Dashboard PieChart */}
                        <View style={{ width: '50%', alignItems: 'center' }}>
                            {item.data !== null && Object.keys(item.data).length > 0 &&
                                <PieChart
                                    style={{ width: viewHeight * 0.65, height: viewHeight * 0.65 }}
                                    logEnabled={true}
