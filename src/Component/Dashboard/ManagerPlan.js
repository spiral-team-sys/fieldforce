...
const ManagerPlan = ({ navigation, route }) => {
  ...
  const LoadData = async dateChoose => {
    await GetDataManagerPlan(
      dateChoose.firstDateWeek,
      dateChoose.lastDateWeek,
      async (mData, mEmployee) => {
        await setData({ dataPlan: mData, employeeList: mEmployee });
        await setDataModal({
          titleModal: 'Danh sách nhân viên',
          visible: false,
          dataSelect: mEmployee,
          dataFilter: mEmployee,
        });
      },
    );
  ...
  const handlerChooseItem = async item => {
    const employeeList = dataModal.dataFilter.map(i =>
      i.employeeId == item.employeeId ? { ...i, isSelect: 1 } : i,
    );
    await setDataModal({
      ...dataModal,
      dataFilter: employeeList,
      dataSelect: employeeList,
      visible: false,
    });
  };
  ...
  const handlerSearch = async text => {
    if (text) {
      ...
      await setDataModal({
        ...dataModal,
        dataSelect: dataFilter,
        dataFilter: dataFilter,
      });
    } else {
      await setDataModal({
        ...dataModal,
        dataSelect: data.employeeList,
        dataFilter: data.employeeList,
      });
    }
  };
  ...
  const renderWeekPlan = ({ item, index }) => {
    ...
    return (
      <View key={`IP_${index}`} style={styles.weekStyle}>
        <Text
          style={{ color: appcolor.dark, fontWeight: 'bold', fontSize: 15 }}
        >
          {item.DayView}
        </Text>
        {item.IsWorking > 1 && (
        ...
        {item.IsWorking == 1 && (
          <View style={styles.styleNonePlan}>
            <Text
              style={{ color: colorWorking, fontSize: 12, fontWeight: '800' }}
            >
              {item.TypeOff}
            </Text>
          </View>
      ...
      uiView.push(
        <View
          ...
          style={{
            ...
            flexDirection: 'row',
            borderTopWidth: 0.5,
            borderTopColor: appcolor.greylight,
          }}
        >
          <Text
            style={{
