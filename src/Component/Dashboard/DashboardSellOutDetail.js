...
import { View, StyleSheet, FlatList, Modal, Text, TouchableOpacity, Platform, RefreshControl, LayoutAnimation, UIManager } from "react-native"
import { GetDataDashboardDetail } from '../../Controller/DashboardController'
import { formatNumber, groupDataByKey, removeVietnameseTones, ToastError, ToastSuccess } from "../../Core/Helper";
...
export const DashboardSellOutDetail = ({ navigation, route }) => {
    ...
    const LoadDataDetail = async () => {
        setRefreshing(true)
        await GetDataDashboardDetail(filter.month, filter.year, shopinfo?.shopId || 0,
            async (mDataDetail) => {
            ...
            <View key={index.toString()} style={styles.dateSytle} >
                <TouchableOpacity onPress={onShowDetail} >
                    <View style={styles.viewTitleSellDate}>
                        <Icon name='calendar' type="font-awesome-5" size={24} color={appcolor.dark} style={{ paddingStart: 8 }} />
                        <Text style={styles.titleSellDate} >{item.sellDate}</Text>
                        <Icon
                            name={item.isShowView == 1 ? 'angle-up' : 'angle-down'}
                            size={18} type="font-awesome-5"
                            color={appcolor.dark} style={{ textAlign: 'right', paddingEnd: 8 }} />
                    </View>
                </TouchableOpacity>
                {/* Content */}
                {item.isShowView == 1 &&
    ...
    const handleShowImage = (item) => {
        ...
        if (listPhoto.length > 0) {
            ...
            ToastSuccess('Số bán này không có dữ liệu hình ảnh.', 'Thông Báo', 'bottom')
        }
    }
            ...
            <View style={styles.contentView} key={'item_' + index.toString()}>
                {item.isParent &&
                    <View style={styles.viewTitleShop}>
                        <Icon name='store' size={18} color={appcolor.dark} />
                        <Text style={styles.titleShop} >{item.ShopName}</Text>
                    </View>
                }
                <View style={styles.viewProduct}>
                    <View style={{ flex: 1, flexDirection: 'row', alignSelf: 'center', backgroundColor: appcolor.success, borderRadius: 8, marginBottom: 5 }}>

                        <View style={{ width: '10%', justifyContent: 'center', alignItems: 'center' }} >
                            <Text style={{ fontSize: 16, fontWeight: '600', textAlign: 'center', color: appcolor.white }}>{item.Quantity}</Text>
                        </View>
                        <View style={styles.itemStyle} key={index}>
                            <View style={{ width: '85%', padding: 5, borderRadius: 8 }}>
                                {item.ContactName?.length > 0 && <RenderItemText titleName='Khách hàng: ' itemValue={item.ContactName} appcolor={appcolor} />}
                                {item.Phone?.length > 0 && <RenderItemText titleName='SĐT: ' itemValue={item.Phone} appcolor={appcolor} />}
                                {item.ProductId > 0 && <RenderItemText titleName='Sản phẩm: ' itemValue={item.ProductName} appcolor={appcolor} />}
                                {(item.CompetitorName !== undefined && item.CompetitorName?.length > 0) && <RenderItemText titleName='Hãng: ' itemValue={item.CompetitorName} appcolor={appcolor} />}
                                <RenderItemText titleName='Ngành hàng: ' itemValue={item.CategoryVN} appcolor={appcolor} />
                                {item.ProductId > 0 && <RenderItemText titleName='Giá: ' itemValue={`${formatNumber(item.Price, ',')} VNĐ`} appcolor={appcolor} />}
                                {item.Incentive > 0 && <RenderItemText titleName='Incentive: ' itemValue={`${formatNumber(item.Incentive, ',')} VNĐ`} appcolor={appcolor} />}
                            </View>
                            {
                                item.listPhoto?.length > 0 &&
                                <TouchableOpacity
                                    onPress={() => handleShowImage(item)}
                                    style={{ width: '15%', borderTopRightRadius: 8, borderBottomRightRadius: 8, justifyContent: 'center', alignItems: 'center' }}>
                                    <Badge
                                        containerStyle={{ position: 'absolute', top: 10, end: 5, }}
                                        textStyle={{ color: appcolor.white, fontSize: 11, fontWeight: '500' }}
                                        badgeStyle={{ minWidth: 20, height: 20, backgroundColor: appcolor.danger, borderRadius: 50 }}
                                        value={item.listPhoto?.length || 0}
                                    />
                                    <Icon name={'images'} type={"font-awesome-5"} size={22} color={appcolor.dark} />
                                </TouchableOpacity>
                            }
                        </View>
                    </View>
    ...
    const handleSearch = (text) => {
        ...
        } else { setDataDetail(dataFilter); }
    };

    const styles = StyleSheet.create({
        mainContainer: { width: '100%', height: '100%', backgroundColor: appcolor.light, justifyContent: 'space-between', alignItems: 'center' },
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            >
                <MultipleShowImage key={'ShowItemImage'} listItem={listShowPhoto} closeShowImage={() => setShowImage(false)} indexItem={0} />
            </Modal>
        </View>
    )
}
const RenderItemText = ({ titleName, itemValue, appcolor }) => {
    ...
    return (
        <View style={{ width: '100%', height: 'auto', marginBottom: 3 }}>
            <Text style={{ width: '100%', fontSize: 15, color: colorItem }}>{titleName}{itemValue}</Text>
        </View>
    )
}