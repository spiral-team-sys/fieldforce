...
const LegendCustom = ({ legend = [], selectedValue, onSelectLine }) => {
  ...
  const renderItem = (item, index) => {
    ...
    const backgroundColor =
      item.keyTable === selectedValue ? item.config.colorHex : appcolor.light;
    const dotColor =
      item.keyTable !== selectedValue ? item.config.colorHex : appcolor.light;
    const color =
      item.keyTable === selectedValue ? appcolor.light : appcolor.dark;
    return (
      <TouchableOpacity
        key={index}
        style={{
  ...
  return <View style={styles.mainContainer}>{legend.map(renderItem)}</View>;
};
