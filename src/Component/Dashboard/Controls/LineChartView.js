...
export const LineChartView = ({ item }) => {
  ...
  const LoadData = async () => {
    ...
    const chartDataSets = item.map((dataSet, index) => {
      const isSelected =
        !filter.keyValue || filter.keyValue === dataSet.CompetitorId;
      const values = dataSet.dataByWeek.map(d => ({ y: d.value }));
      const lineColor = isSelected
        ? dataSet?.ColorDashboard || colorDashboardHex[index]
        : appcolor.surface;
      return {
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <LegendCustom
        legend={chartSetting?.data?.dataSets}
        selectedValue={filter.keyValue}
        onSelectLine={handlerSelectLine}
      />
    </View>
  );
};
