...
import { Text, View, TouchableOpacity, FlatList, Modal, SafeAreaView, ImageBackground } from "react-native";
...
export const DashboardIssue = ({ info }) => {
    ...
    if (Array.isArray(data) && data.length > 0) {
        ...
        viewAll.push(<Text key="s1oi" style={{ width: '100%', textAlign: 'center', fontSize: 15, color: appcolor.danger }}>Chưa có dữ liệu báo cáo vấn đề</Text>)
    }
    return (
        <TouchableOpacity onPress={() => setViewDetail(e => !e)}>
            ...
            <Modal visible={viewDetail}>
                <DetailData title={info.chartName} data={JSON.parse(info.detailData)} dataPhoto={dataPhoto} appcolor={appcolor} onClose={() => setViewDetail(false)} />
            </Modal>
        </TouchableOpacity >
    )
}
    ...
    const renderItemPhoto = ({ item, index }) => {
        return (
            <View style={{ flex: 1, padding: 5, paddingLeft: 10 }}>
                <View style={{ width: '100%', borderRadius: 5, backgroundColor: appcolor.surface, padding: 10, marginVertical: 5 }}>
                    <Text style={{ flex: 1, fontSize: 15, fontWeight: 'bold', fontStyle: 'italic', color: appcolor.dark }}>{item.photoNote}</Text>
                </View>
                <TouchableOpacity
                    ...
                    <ImageBackground
                        ...
                        source={{ uri: item.photoPath.indexOf('https://') === - 1 ? URLDEFAULT + item.photoPath : item.photoPath }}
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            ...
            </SafeAreaView>
            {
                ...
                >
                    <Tabs.Tab key={'Hình ảnh báo cáo'} label={'Hình ảnh báo cáo'} name={'Hình ảnh báo cáo'} >
                        <View key={'Hình ảnh báo cáo'} style={{ backgroundColor: appcolor.light, marginTop: 40, padding: 6, width: deviceWidth, }} >
                            <FlatList
                                showsVerticalScrollIndicator={false}
                                style={{ flex: 1, }}
                                keyExtractor={(_, index) => index.toString()}
