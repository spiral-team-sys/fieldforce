        ...
        <TouchableOpacity onPress={() => setViewDetail(e => !e)}>
            <View style={{ backgroundColor: appcolor.surface, padding: 8, borderRadius: 10, marginBottom: 8 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Text style={{ marginEnd: 8, color: appcolor.dark, marginStart: 8, fontWeight: '600', fontSize: 15 }}> {info !== null ? (info.chartName || 'Thống kê tổng hợp') : ''}</Text>
                </View>
            ...
            <Modal visible={viewDetail}>
                <DashboardDetailSynthesis title={info.chartName} dataSummary={JSON.parse(info.detailData)} dataHeader={JSON.parse(info.chartData)} onClose={() => setViewDetail(false)} />
            </Modal>
        </TouchableOpacity >
    )
}