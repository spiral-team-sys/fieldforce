...
export const RoutingReport = ({ navigation }) => {
  ...
  const rowItem = ({ item, index }) => {
    ...
    return (
      <View
        key={`mka${index}d`}
        style={{ width: '49%', marginRight: 7, marginBottom: 7 }}
      >
        <TouchableOpacity onPress={() => onShowbyDays(item)}>
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              >
                {Capitalize(moment(item.startWork).format('dddd, [Ngày] D'))}
              </Text>
            </View>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
      ...
      <View style={{ margin: 7 }}>
        ...
        >
          <Progress.Circle
            ...
            showsText={true}
            formatText={() => info?.PercentPlan || '0 %'}
            progress={info?.PercentValue || 0}
            size={130}
          />
          <View style={{ padding: 12, justifyContent: 'center' }}>
            <Text style={{ fontSize: 10, color: appcolor.dark }}>
              Đã đi
              <Text style={{ fontSize: 17, color: appcolor.dark }}>
                {' '}
                {data?.length}
              </Text>{' '}
              ngày trong tháng
            ...
            <Text style={{ fontSize: 10, color: appcolor.dark }}>
              ...
              <Text style={{ fontSize: 17, color: appcolor.dark }}>
                {' '}
                {data.length > 0
                  ? Number(info?.TotalDistance / data?.length).toFixed(2)
                  : 0}
              </Text>{' '}
              km/ngày
            </Text>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: appcolor.surface }}>
      <HeaderCustom
        ...
        title={`Routing ${filter.monthname} ${filter.yearname}`}
      />
      <LoadingDefault
        styles={{ zIndex: 100 }}
        isLoading={loading}
        ...
        <View style={{ height: deviceHeight - 100 }}>
          <View style={{ flexDirection: 'row', width: '100%' }}>
            ...
            <TouchableOpacity
              ...
              onPress={() => SheetManager.hide('sheetDaily')}
            >
          ...
          >
            <Tabs.Tab key={1} name="Lộ trình" label="Lộ trình">
              ...
              >
                <Routing
                  onClose={() => SheetManager.hide('sheetDaily')}
                  info={selected}
                  routedata={routeData}
                />
              </View>
...
const Routing = ({ info, routedata, onClose }) => {
  ...
  var totalTime = routedata.length
    ? routedata
    ...
    return (
      <View
        ...
        style={{
          ...
          marginBottom: index === routedata.length - 1 ? 90 : 0,
        }}
      >
        <View
          style={{
