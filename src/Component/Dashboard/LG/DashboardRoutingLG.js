...
export const DashboardRoutingLG = ({ navigation, data }) => {
    ...
    const rowItem = ({ item, index }) => {
        return (
            item.viewMonth == undefined ?
                <TouchableOpacity onPress={() => showDetail(item)} key={index + "dasd6aj"}
                    style={{
                        margin: 5, padding: 3, shadowColor: appcolor.white,
                        shadowOffset: { width: 0.7, height: 0.82 }, shadowRadius: 10,
                        elevation: 5, shadowOpacity: 0.8,

                    }}>
                    <View style={{ backgroundColor: appcolor.light, padding: 5, borderRadius: 8 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <Text style={{ fontWeight: '500', fontSize: scaleSize(12), fontWeight: '800', color: appcolor.dark }}>{item.workDate}</Text>
                        </View>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            ...
                            <Text style={{ flexGrow: 1, fontWeight: '700', textAlign: 'right', fontSize: scaleSize(16), color: appcolor.danger }}>{item.distance}</Text>
                        </View>
                    </View>
                </TouchableOpacity>
                :
    ...
    return (
        <View style={{ borderRadius: 12, height: '94%', opacity: 0.9 }}>
            {/* <Text style={{ padding: 7, fontSize: scaleSize(14), color: appcolor.white, fontWeight: 'bold' }}>{data?.titleChart}</Text> */}
            <TouchableOpacity onPress={() => navigation.navigate("attendanthistory")} style={{ padding: 12, flexDirection: 'row' }}>
                <View style={{ flexGrow: 1, alignItems: 'center' }}>
                    <Text style={{ fontSize: scaleSize(20), fontWeight: 'bold', color: appcolor.warning, padding: 2 }}>
                        {data?.aValue || '0'}</Text>
                    <Text style={{ fontSize: scaleSize(12), color: appcolor.white }}>{data?.aLable}</Text>
                </View>
                <View style={{ flexGrow: 1, alignItems: 'center' }}>
                    <Text style={{ fontSize: scaleSize(20), fontWeight: 'bold', color: appcolor.danger, padding: 2 }}>
                        {data?.tValue}</Text>
                    <Text style={{ fontSize: scaleSize(12), color: appcolor.white }}>
                        {data?.tLable}</Text>
                </View>
                <View style={{ flexGrow: 1, alignItems: 'center' }}>
                    <Text style={{ fontSize: scaleSize(20), fontWeight: 'bold', color: appcolor.info, padding: 2 }}>
                        {data?.pValue || '0%'}</Text>
                    <Text style={{ fontSize: scaleSize(12), color: appcolor.white }}>
                        {data?.pLable || '%'}</Text>
                </View>
            ...
            <View style={{
                ...
                position: 'absolute', width: 160, height: 160, left: '-9%', top: '-9%'
            }} />
            <View style={{ marginHorizontal: 8, flex: 1, justifyContent: 'center', alignItems: "center" }}>
