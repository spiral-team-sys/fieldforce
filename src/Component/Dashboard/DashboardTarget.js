...
export const DashboardTarget = ({ navigation, route }) => {
  ...
  const LoadDataDetail = async tabSelect => {
    ...
    const dataJson = {
      year: filter.year,
      month: filter.month,
      typeDashboard: tabSelect?.typeData || 'SELLOUTTARGETBYDEALER',
    };
  ...
  const contains = (item, query) => {
    ...
    return (
      removeVietnameseTones(Sc1)?.match(removeVietnameseTones(query)) ||
  ...
  const handleSearch = text => {
    ...
    const filteredData = _.filter(data.dataF, dataDealer => {
      return contains(dataDealer, formattedQuery);
    });
    const { arr } = groupDataByKey({
      arr: filteredData,
      key: 'DealerId',
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      {!refreshing && (
        <ListTab
          dataGroup={data.dataTab}
          LoadDataByTab={LoadDataByTab}
      ...
      {!refreshing && (
        <FlatList
          ...
          data={data.dataById}
          ListHeaderComponent={
            <DashboardHeaderStock dataMain={data.dataMain[0] || {}} />
          }
...
const SheetByItem = ({ beforeData }) => {
  ...
  const handleSearch = text => {
    ...
    const filteredData = _.filter(data.dataSheetF, shop => {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      >
        <Text style={{ fontWeight: '600', fontSize: 20, color: appcolor.dark }}>
          {data.itemSelect?.n2}
        </Text>
      </View>
        ...
        >
          {data?.itemSelect?.TotalStore && data?.itemSelect?.TotalStore > 0 && (
            ...
            >
              ...
              >
                {data?.itemSelect?.TotalStore}
              </Text>
            </View>
          )}
          {data?.itemSelect?.TotalEmployee &&
            data?.itemSelect?.TotalEmployee > 0 && (
              <View
                style={{
                  width: '46%',
                  justifyContent: 'center',
    ...
    <View style={styles.container}>
      ...
      <View style={styles.targetContainer}>
        {dataMain?.totalTarget && dataMain?.totalTarget > 0 && (
          <View
            style={[styles.targetCard, { backgroundColor: appcolor.surface }]}
          >
            <Text style={[styles.targetTitle, { color: appcolor.dark }]}>
              {dataMain?.titleTarget}
            </Text>
            <Text style={styles.targetValue}>
              {dataMain?.totalTarget}
              {dataMain?.unitTarget ? dataMain?.unitTarget : ''}
            </Text>
          </View>
        )}
        {dataMain?.totalActual && dataMain?.totalActual > 0 && (
          ...
          >
            ...
            <Text style={styles.targetValue}>
              {dataMain?.totalActual}
              {dataMain?.unitActual ? dataMain?.unitActual : ''}
            </Text>
          </View>
        )}
        {dataMain?.totalActual && dataMain?.totalPercent > 0 && (
          ...
          >
            ...
            <Text style={styles.targetValue}>
              {dataMain?.totalPercent}
              {dataMain?.unitPercent ? dataMain?.unitPercent : ''}
            </Text>
          </View>
        )}
      </View>
