...
const SurveyListScreen = ({ data = [], onCloseModal }) => {
  ...
  const stats = useMemo(() => {
    ...
    const thisWeek = normalizedData.filter(
      item => new Date(item.createdDate) >= startOfWeek,
    ).length;
      ...
      <View style={styles.card}>
        <View style={styles.row}>
          ...
          <View
            style={[
              styles.badge,
              { backgroundColor: appcolor.primary || '#2979FF' },
            ]}
          >
        ...
        {Object.keys(itemSelect).length > 0 && (
          <View style={{ width: deviceWidth }}>
            <Text style={styles.label}>
              Phân loại đề xuất : {itemSelect.id == 100 && itemSelect.nameVN}
            </Text>
            {itemSelect.id !== 100 && (
              <Text style={styles.content}>
                {itemSelect.id == 100
                  ? `${itemSelect.nameVN}`
                  : `${itemSelect.groupName} : ${itemSelect.nameVN}`}
              </Text>
            )}
            {item.noteClassify && (
              <Text style={styles.content}>
                Nội dung khác : {item.noteClassify}
              </Text>
            )}
          </View>
        )}
    ...
    <SafeAreaView edges={['bottom']} style={styles.container}>
      ...
      {!!onCloseModal && (
        <TouchableOpacity style={styles.closeButton} onPress={onCloseModal}>
          <SpiralIcon name="close" type="ionicon" size={22} color="#fff" />
        </TouchableOpacity>
      )}
    </SafeAreaView>
  );
