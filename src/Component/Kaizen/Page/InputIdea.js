...
import { MessageAction, MessageAction2, UUIDGenerator, groupDataByKey, removeStore } from '../../../Core/Helper';
import { PhotoInput } from './PhotoInput';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { clearPhotoReportZalo, deletePhotoByGuid, getDataPhotos } from '../../../Controller/PhotoController';
...
const InputIdea = ({ dataInput, reload }) => {
    ...
    const [itemInput, setItemInput] = useState({ ...template, guid: UUIDGenerator() });
    const [_Mutate, setMutate] = useState(false);
    const [listImages, setListImages] = useState([]);
    const [loading, setLoading] = useState(false);
    const [dataListSelect, setDataListSelect] = useState({ itemSelect: null, itemMain: {}, listSelect: [] });
    ...
    const LoadData = async () => {
        ...
        if (localStore != null) {
            MessageAction2('Đã có dữ liệu lưu tạm, bạn có muốn tiếp tục nhập không?', async () => {
                await setItemInput({ ...local });
                await getDataPhotos(userinfo.employeeId, TODAY, null, kpiinfo.id, local.guid, false, setListImages)
            }, async () => {
    ...
    const handlerSelectItem = (item, text) => {
        if (item.ref_Code == 'select') {
            ...
            const itemSelect = itemInput[item.ref_Name] ? JSON.parse(itemInput[item.ref_Name] || '{}') : null
            SheetManager.show('TypeIdeaSheet', { payload: { itemSelect: itemSelect, itemMain: item, listSelect: arr } })
        }
    };
    ...
    const InputList = () => {
        return (
            normalizedInputs?.map((item, index) => {
                switch (item.ref_Code) {
                    ...
                    case 'select':
                        ...
                        return <TouchableOpacity key={index} onPress={() => handlerSelectItem(item)} >
                            <FormGroup
                                key={`Select_${item.ref_Name}`}
                                rightFunc={() => handlerSelectItem(item)}
                                iconRight={"caret-down"} iconRightStyle={{ color: appcolor.primary }}
                                title={item.nameVN}
                                defaultValue={itemInput[item.ref_Name] ? `${itemSelect.id == 100 ? '' : `${itemSelect.groupName} : `}${itemSelect.nameVN}` : '--Chọn--'}
                                multiline={true}
                                containerStyle={{ padding: 4 }}
                            ...
                            />
                            {
                                ...
                                <FormGroup
                                    ...
                                    handleChangeForm={text => handlerEditInput(item, text, 'other')}
                                    onClearTextAndroid={() => handlerEditInput(item, null, 'other')}
                                    keyboardType={'default'}
                                    containerStyle={{ padding: 4 }}
                    ...
                    case 'image':
                        return <View key={index}>
                            <PhotoInput _guid={itemInput.guid} itemInput={itemInput} reload={reload} />
                        </View>
                }
            })
        )
        ...
        <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}
            ...
            <View style={styles.card}>
                <View style={styles.cardHeader}>
                    <Icon name="lightbulb" type="font-awesome-5" size={16}
                        color={appcolor.primary} style={styles.cardHeaderIcon} solid />
                    <Text style={[styles.cardTitle, { color: appcolor.dark }]}>Thông tin đề xuất</Text>
                    <View style={[styles.cardBadge, { backgroundColor: appcolor.primary }]}>
                        <Text style={styles.cardBadgeText}>{normalizedInputs.length} trường</Text>
                    </View>
                </View>
                {!loading && InputList()}
            </View>
            <View style={{ width: deviceWidth }}>
                <ActionSheet id={'TypeIdeaSheet'}
                ...
                >
                    <View style={{ padding: 8, marginBottom: 20, paddingTop: 20, backgroundColor: appcolor.surface, width: '100%', height: '100%' }}>
                                    ...
                                    return (
                                        <View key={`${it.code}_${idx}`} style={{ padding: 4 }}>
                                            ...
                                            <TouchableOpacity
                                                ...
                                                style={{ padding: 10, borderRadius: 10, backgroundColor: dataListSelect.itemSelect?.id == it.id ? appcolor.primary : appcolor.light, marginLeft: 8 }}
                                            >
                                                <Text style={{ fontSize: 12, color: dataListSelect.itemSelect?.id == it.id ? appcolor.white : appcolor.dark }}>{it.nameVN}</Text>
                                            </TouchableOpacity>
                                        </View>
                                    )
                                })
