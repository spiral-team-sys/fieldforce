...
const InAppView = ({ title, body, inAppId, isViewDetail, onClose }) => {
    ...
    const styles = StyleSheet.create({
        ...
        viewContent: { flex: 1, margin: 8, padding: 8, borderRadius: 12, borderWidth: 1, borderColor: appcolor.surface, overflow: 'hidden' },
        closeButton: { width: 150, alignItems: 'center', alignSelf: 'center', padding: 8, borderWidth: 1, borderColor: appcolor.primary, backgroundColor: appcolor.light, borderRadius: 8, marginBottom: 16 },
    });
            ...
            <View style={styles.viewContent}>
                <WebView
                    ...
                    <style>
                        ...
                        ${createdDate ? `<div style="text-align:end;color:red;width:100%;font-style:italic">Đã đăng ${moment(createdDate).calendar()}</div>` : ''}
...
const ToolKit = ({ config, appcolor, onPostLog, close, handlerChangeText }) => {
    ...
    return (
        <View style={{ padding: 3, backgroundColor: appcolor.light }}>
            {(config?.filter(it => it.name == 'FeedBack')).map((element) => {
                return (
                    <FormGroup
                        key={element.name}
                        editable
...
const ButtonAction = ({ element, close, onPostLog }) => {
    ...
    const styles = StyleSheet.create({
        buttonClose: { flexGrow: 0.3, alignSelf: 'center', borderRadius: 8, borderWidth: 1, borderColor: appcolor.primary, padding: 6, paddingHorizontal: 24, backgroundColor: appcolor.light },
        buttonRead: { flexGrow: 0.3, alignSelf: 'center', borderRadius: 8, borderWidth: 1, borderColor: appcolor.primary, padding: 6, paddingHorizontal: 24, backgroundColor: colorButton },
        buttonContainer: { flexGrow: 0.3, alignSelf: 'center', borderRadius: 8, overflow: 'hidden', marginVertical: 8, marginEnd: 3 },
        titleButtonClose: { fontSize: 11, fontWeight: fontWeightBold, color: element.name === 'SkipButton' ? appcolor.primary : appcolor.light },
    });
    const pressItem = () => {
