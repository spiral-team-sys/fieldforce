  ...
  const eventStateUrlChange = async event => {
    ...
    if (event.url !== null && event.url !== 'about:blank') {
      if (
        ...
        const newUrl =
          !event.url.includes('http://') && !event.url.includes('https://')
            ? `https://${event.url}`
            : event.url;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.light }}>
      <View style={{ padding: 3 }}>
        <Text style={{ color: appcolor.dark, fontWeight: '900' }}>
          {inAppMess?.title || 'InApp'}
        </Text>
      </View>
      <WebView
        style={{}}
...
const ButtonAction = ({ element, close, onPostLog }) => {
  ...
  return (
    ...
    >
      <Text
        style={{ textAlign: 'center', fontSize: 14, color: appcolor.white }}
      >
