...
import { GetEmployeeInfo, onShareLocalFile, StringTobase64 } from "../../Core/Helper";
...
const NotificationDetailScreen = ({ navigation, route }) => {
    ...
    const onGoPage = async () => {
        ...
        if (pageName?.includes('http')) {
            ...
            if (pageName.includes("spiral.com.vn")) {
                ...
                itemData.urlSite = pageName.includes('?') ? `${pageName}&appShare=${app_access}` : `${pageName}?appShare=${app_access}`;
                setMutate(e => !e)
            } else {
    ...
    const styles = StyleSheet.create({
        ...
        titleName: { fontSize: 16, fontWeight: fontWeightBold, padding: 8, paddingBottom: 0, color: appcolor.dark },
        imageStyle: { width: '100%', height: 200, borderRadius: 8, padding: 8 },
        bodyName: { fontSize: 13, backgroundColor: appcolor.light, color: appcolor.dark, padding: 8, borderWidth: 0.5, borderColor: appcolor.grayLight, borderRadius: 8, margin: 8 },
        titleInfo: { fontWeight: fontWeightBold, padding: 8, color: appcolor.info },
        dateName: { fontSize: 12, color: appcolor.greydark, textAlign: 'right', fontStyle: 'italic', paddingHorizontal: 16 },
        buttonGoView: { width: '30%', alignItems: 'center', margin: 8, marginHorizontal: 16, padding: 8, borderWidth: 0.5, borderColor: appcolor.primary, borderRadius: 20 },
        titleGo: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.primary }
    })
    const renderBodyContent = (content) => {
        let bodyView = [];
        content?.split(" ").forEach((element, index) => {
            if (element.includes("http") && (element?.includes(".jpg") || element?.includes(".jpeg") || element?.includes(".png"))) {
                bodyView.push(<Text style={{ color: appcolor.dark }} key={"t" + index.toString()}>-------------------------------</Text>)
                ...
                bodyView.push(
                    <Text key={index.toString()} onPress={() => onGoPage(element)} style={styles.titleInfo}>{'Chi tiết'}</Text>
                )
            } else {
                bodyView.push(
        ...
        <View style={styles.mainContainer}>
            ...
            <ScrollView contentContainerStyle={styles.contentContainer}>
                <ViewShot ref={viewShotRef} options={{ format: "jpg", quality: 0.8 }}>
                    ...
                    {(itemData.hyperLinks !== null && (itemData?.hyperLinks?.includes('.png') || itemData?.hyperLinks?.includes('.jpg') || itemData?.hyperLinks?.includes('.jpeg'))) &&
                        <Image
                            style={styles.imageStyle}
                            source={{ uri: itemData?.hyperLinks, }} />
                    }
                    <Text style={styles.bodyName}>{renderBodyContent(itemData?.body || "")}</Text>
                    <Text style={styles.dateName}>{moment(itemData.createdDate).format('dddd DD, MM/YYYY HH:mm:ss')}</Text>
                    {itemData.hyperLinks &&
