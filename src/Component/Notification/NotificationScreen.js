...
import { getDataNotify, makeReadAllNotify, readNotifyUpdate } from "../../Controller/NotificationController";
...
const NotificationScreen = ({ navigation }) => {
    ...
    const styles = StyleSheet.create({
        ...
        itemNotification: { flexDirection: 'row', alignItems: 'center', overflow: 'hidden', padding: 8, margin: 8, marginBottom: 0, borderWidth: 0.5, borderColor: appcolor.grayLight, borderRadius: 8, elevation: 3, shadowOpacity: 0.3, shadowOffset: { width: 0, height: 1 }, shadowColor: appcolor.grayLight, shadowRadius: 8 },
        ...
        readNow: { fontSize: 12, fontWeight: fontWeightBold, textDecorationLine: 'underline', color: appcolor.bluenavylight, textAlign: 'right' },
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity style={{ ...styles.itemNotification, backgroundColor: seened ? appcolor.light : appcolor.grayLight }} onPress={onPress}>
                <Avatar
                    title={`${index + 1}`}
                    rounded
                    size="small"
                    containerStyle={{ ...styles.viewNumber, backgroundColor: ColorRand(index) }}
                />
                <View style={styles.contentTitle}>
                    <Text style={styles.titleName}>{item.title}</Text>
                    <Text style={styles.bodyName} numberOfLines={3} ellipsizeMode='tail'>{item.body || `Nhấn vào để xem chi tiết`}</Text>
                    <Text style={styles.dateName}>{moment(item.createdDate).fromNow()}</Text>
                    {item.hyperLinks &&
                        <TouchableOpacity style={styles.viewReadNow} onPress={onPressDetail}>
