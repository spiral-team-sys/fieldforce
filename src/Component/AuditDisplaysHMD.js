...
import {
  insertDisplayResult,
  getItemsAuditDisplay,
  getGroupAudit,
  getAuditDisplayResult,
  getAuditDisplayYESResult,
  updateHavePack,
  updateNoteCommon,
  getPhotosAuditUploaded,
  getPhotosReport,
  getPhotosAuditPackBy,
  existAddAuDisRes,
  existNoChangeItem,
  existAuditResItem,
  deleteAuditDisplayResult,
  deleteNoChangeRes,
  checkResAuditDisplay,
  checkItemsAuditDisplay,
  deletePhotosAuditUploaded,
...
export default class AuditDisplaysHMD extends Component {
  ...
  DisplayItemMap(lstItemsProgram, resDisplay) {
    ...
    lstNochange.length > 0 &&
      this.setState({ Status: lstNochange[0].upload === 1 ? 1 : 0 });
    ...
    lstItemsProgram.map((item, i) => {
      ...
      if (Array.isArray(resDisplay)) {
        ...
        ItemsAdd = resDisplay.filter(
          itemRes =>
            itemRes.target === -1 &&
            itemRes.displayRef ===
              (this.state.groupName === ''
                ? this.state.lstGroup[0]
                : this.state.groupName),
        );
      ...
      if (!itemMap[item.refName]) {
        ...
        let itemInputt = {
          itemName: 'Thêm ' + item.refName,
          itemId: item.id,
          refName: item.refName,
          subCat: item.code,
          displayComment: '',
          upload: 0,
          fieldSetting: item.fieldSetting,
          kpi1: 0,
          kpi2: 0,
          kpi3: 0,
          kpi7: 0,
        ...
        if (ItemsAdd.length > 0) {
          ItemsAdd.map(itemA => {
            let itemInput = {
              ...
              rkpi5: itemA.kpi5,
              rkpi7: itemA.kpi7,
              kpi1Holder: item.kpi1Holder,
              kpi2Holder: item.kpi2Holder,
              target: itemA.target,
              displayNameVN: itemA.displayNameVN,
            };
      ...
      let itemInput = {
        ...
        kpi1Holder: item.kpi1Holder,
        kpi2Holder: item.kpi2Holder,
        displayNameVN: item.displayNameVN,
      };
  ...
  async componentDidMount() {
    ...
    try {
      let questionLst = JSON.parse(
        this.props.route.params.DisplayItem.fieldSetting,
      );
      let addAnswer = questionLst.map(itemq =>
        itemq.question !== '' ? { ...itemq, answer: null } : itemq,
      );
      questionLst.length > 0 &&
        (await this.setState({ questionShop: addAnswer }));

      await this.setState({
        isHavePack: null,
        noteCommon: '',
        HeightHeader: questionLst.length * 80,
      });
    } catch (error) {
      alertToast(error);
    }
  ...
  uploadAction = async () => {
    ...
    let itemsPack = await getItemsAuditDisplay(
      DisplayItem.id,
      '',
      '',
      workinfo.shopId,
    );
    let lstItemsProgram = await checkItemsAuditDisplay(
      DisplayItem.id,
      workinfo.workId,
    );
    let lstResItems = await checkResAuditDisplay(
      DisplayItem.id,
      workinfo.workId,
    );

    let resDisplayAD = await getAuditDisplayResult(
      workinfo.workId,
      DisplayItem.id,
    );
    let resPhotos = await getPhotosAuditUploaded(
      workinfo.reportId,
      workinfo.shopId,
      workinfo.workDate,
      '',
    ...
    for (let index = 0; index < lstQuestion.length; index++) {
      ...
      if (itemQ.answer === null) {
        errorQuestion =
          'Bạn chưa chọn câu trả lời cho câu hỏi: \n' + itemQ.name;
        break;
      }
    }
    ...
    if (this.state.isChangeMonth === true) {
      ...
      if (lstItemsProgram !== undefined) {
        if (lstItemsProgram.length > 0) {
          MessageInfo(
            lstItemsProgram[0].refName +
              ': Bạn chưa làm xong cho item: \n' +
      ...
      if (lstResItems !== undefined) {
        if (lstResItems.length > 0) {
          ...
          MessageInfo(
            lstResItems[0].displayRef +
              ': Bạn chưa làm xong cho item: \n' +
              lstResItems[0].kpi4Name +
              strErr,
          );
          return;
        }
      ...
      if (resDisplayAD.length > 0) {
        let itemHa = resDisplayAD.filter(
          item =>
            item.kpi3 === 1 &&
            item.kpi2 !== undefined &&
            item.displayRef === 'PRODUCT',
        );
        if (itemHa.length > 0) {
          let item = itemHa[0];
      ...
      this.state.lstRequestPhotos.map(async itemMast => {
        const lstitemPhoto = await getPhotosReport(
          this.props.route.params.workinfo.reportId,
          this.props.route.params.DisplayItem.id + '_' + itemMast.code,
        ...
        if (itemMast.numberValue > 0) {
          if (lstitemPhoto.length < itemMast.numberValue) {
            MessageInfo(
              'Vui lòng chụp đủ số lượng hình cho: ' +
                itemMast.name +
                ' (' +
                itemMast.numberValue +
                ' tấm )',
            );
            resPho = false;
          }
      ...
      if (lstQuestion.length > 0) {
        ...
        for (let index = 0; index < lstQuestion.length; index++) {
          ...
          if (itemQ.id === 1) {
            let resDisplay = await existAddAuDisRes(
              workinfo.workId,
              DisplayItem.id,
            );
            ...
            if (itemQ.answer === false) {
              if (resDisplay.length === 0) {
                errorStr =
                  'Bạn chưa thêm PRODUCT(POSM) thay thế. Vui lòng tick chọn có cho câu hỏi: \n' +
                  itemQ.name;
                break;
              }

              if (
                this.state.noteCommon === '' ||
                this.state.noteCommon === null
              ) {
                errorStr =
                  'Vui lòng nhập lý do khi cửa hàng không trưng bày theo gói.';
                break;
              } else {
              ...
              if (resDisplay.length > 0) {
                errorStr =
                  'Bạn đã thêm PRODUCT(POSM) thay thế, vui lòng tick chọn không cho câu hỏi: ' +
                  itemQ.name +
                  ' (Hoặc bạn xoá các mục đã thêm)';
              }
            }
          } else if (itemQ.id === 2 && AppNameBuild === nokiaApp) {
            let resItemRes = await getAuditDisplayYESResult(
              workinfo.workId,
              DisplayItem.id,
            );
            if (itemQ.answer === true) {
              for (let index = 0; index < resItemRes.length; index++) {
                ...
                let lstPhoto = await getPhotosAuditPackBy(
                  workinfo.reportId,
                  workinfo.shopId,
                  workinfo.workDate,
                  photoType,
                ...
                if (lstPhoto.length === 0) {
                  let itemHave = itemsPack.filter(
                    item => item.id === itemRS.itemId,
                  );

                  if (itemRS.kpi4Name !== null) {
                    errorStr =
                      itemRS.displayRef +
                      ': Vui lòng chụp hình cho (' +
                      itemRS.kpi4Name +
                      ')'; //itemHave[0].itemName
                    break;
                  } else {
      ...
      if (resPho === true) {
        Message(
          ...
          () => this.UploadData(resDisplayAD, resPhotos),
        );
      }
    } else {
      if (DisplayItem.visit === 0) {
        MessageInfo(
          'Tháng này bạn chưa gửi báo cáo cho gói trưng bày này, vui lòng tích chọn CÓ (Có thay đổi gì so với lần trước không?), sau đó làm báo cáo và chụp hình đầy đủ.',
        );
        return;
      }
      ...
      if (resDisplay.length > 0) {
        resDisplay.forEach((item, index) => {
          let dataItem = {
            ...
            displayRef:
              item.target === -1 ? item.displayRef : 'AuditDisplayItem',
            displayComment: JSON.stringify({
              kpi1: item.kpi1,
              kpi2: item.kpi2 === 'null' || item.kpi2 === '' ? null : item.kpi2,
              kpi3: item.kpi3,
              kpi4: item.kpi4,
              kpi7: item.kpi7,
            }),
      ...
      await fetch(URL_UPLOAD_AUDIT_DISPLAY, {
        ...
        .then(responseJson => {
          if (responseJson.status == 200) {
            ...
            let isUpdate = updateAuditDisplayResult(
              this.state.workinfo,
              this.props.route.params.DisplayItem.id,
            );
      ...
      if (this.state.groupName !== 'PHOTOS') {
        ...
        if (AppNameBuild === nokiaApp) {
          if (resAddDisplay.length > 0) {
            await this.setState({
              questionShop: this.state.questionShop.map(itemq =>
                itemq.id === 1 ? { ...itemq, answer: false } : itemq,
              ),
            });
            await this.setState({
              isHavePack: false,
              HeightHeader: this.state.questionShop.length * 80 + 70,
            }); //,noteCommon:''
          } else {
            await this.setState({
              questionShop: this.state.questionShop.map(itemq =>
                itemq.id === 1 ? { ...itemq, answer: true } : itemq,
              ),
            });
            await this.setState({
              isHavePack: true,
              HeightHeader: this.state.questionShop.length * 80,
            }); //,noteCommon:''
          }
          ...
          if (NoChangeItem.length > 0) {
            await this.setState({
              questionShop: this.state.questionShop.map(itemq =>
                itemq.id === 2 ? { ...itemq, answer: false } : itemq,
              ),
              isChangeMonth: false,
            });
          } else {
            resItem.length > 0
              ? await this.setState({
                  questionShop: this.state.questionShop.map(itemq =>
                    itemq.id === 2 ? { ...itemq, answer: true } : itemq,
                  ),
                  isChangeMonth: true,
                })
              : await this.setState({
                  questionShop: this.state.questionShop.map(itemq =>
                    itemq.id === 2 ? { ...itemq, answer: null } : itemq,
                  ),
                  isChangeMonth: null,
                });
          }
  ...
  async loadGroup() {
    ...
    let lstPhoto = await getRequestPhotos(
      this.props.route.params.DisplayItem.id,
    );
  ...
  loadDataShow = async (search, refName) => {
    let lstItemsProgram = await getItemsAuditDisplay(
      this.props.route.params.DisplayItem.id,
      search,
      refName,
      this.state.workinfo.shopId,
    );
    let resDisplay = await getAuditDisplayResult(
  ...
  chooseHaveDisplayPack = async (item, check) => {
    ...
    if (item.id === 1) {
      await this.setState({
        ...
        noteCommon: '',
        HeightHeader: !check
          ? questionShop.length * 80 + 70
          : questionShop.length * 80,
        questionShop: questionShop.map(itemq =>
          itemq.id === item.id ? { ...itemq, answer: check } : itemq,
        ),
      });
    } else if (item.id === 2) {
      await this.setState({
      ...
      if (check === false) {
        ...
        await deletePhotosAuditUploaded(
          workinfo.reportId,
          workinfo.shopId,
          workinfo.workDate,
          DisplayItem.id,
    ...
    return (
      <TouchableHighlight
        ...
        style={{
          ...
          backgroundColor: isTabActive ? 'lightgray' : 'white',
        }}
        underlayColor="#aaaaaa"
      >
        <Text
          style={{
            color: isTabActive ? 'red' : 'black',
            borderWidth: 1,
            borderColor: 'transparent',
            borderBottomColor: isTabActive ? 'red' : 'transparent',
          }}
        >
          {name}
        </Text>
  ...
  changeTabAction = async value => {
    if (this.state.lstShow.length > 0) {
      await this.setState({
        groupName: this.state.lstGroup[value.i],
        isPhotoView: false,
      });
  ...
  renderItem = ({ item }) => (
    ...
    >
      <Text
        ...
        numberOfLines={2}
        style={{ ...styles.labelSellout, width: '65%' }}
      >
        {item.name}
      </Text>
      <TouchableOpacity
        style={{ alignItems: 'center', padding: 2 }}
        onPress={() => this.chooseHaveDisplayPack(item, true)}
        disabled={this.state.Status === 0 ? false : true}
      >
        <RadioButton
          selected={
            item.answer === null ? null : item.answer === true ? true : false
          }
        />
      ...
      <TouchableOpacity
        style={{ alignItems: 'center', padding: 2 }}
        onPress={() => this.chooseHaveDisplayPack(item, false)}
        disabled={this.state.Status === 0 ? false : true}
      >
        <RadioButton
          selected={
            item.answer === null ? null : item.answer === false ? true : false
          }
        />
  ...
  render() {
    ...
    return (
      <ImageBackground style={{ height: '100%', width: '100%' }}>
        ...
        {this.state.questionShop.length > 0 &&
          this.state.isHavePack === false && (
            <TextInput
          ...
          )}
        {
          ...
          <View>
            ...
            {this.state.isPhotoView === false && (
              <AuditItemsHMDContent
                ProductItems={this.state.lstShow}
                workinfo={this.state.workinfo}
                DisplayItem={this.state.DisplayItem}
                loadData={this.refreshData}
                Status={this.state.Status}
                NoChange={this.state.isChangeMonth === false ? 1 : 0}
                RefName={
                  this.state.groupName === ''
                    ? this.state.lstGroup[0]
                    : this.state.groupName
                ...
                HeightHeader={this.state.HeightHeader}
                ChangeInMonth={this.state.isChangeMonth}
              />
            )}
            {this.state.isPhotoView === true && (
              <PhotoCustomHMD
                Photos={this.state.lstRequestPhotos}
                Workinfo={this.state.workinfo}
                DisplayId={this.state.DisplayItem.id}
                Props={this.props}
