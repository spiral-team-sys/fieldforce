...
const REPORT_TABS = [{ tabName: 'Nhập liệu' }, { tabName: 'Hình ảnh' }];
...
const DisplayCompetitorReport = ({ navigation }) => {
  ...
  const photoList = useMemo(
    () =>
      Array.isArray(reportItem?.ImageByList) ? reportItem.ImageByList : [],
    [reportItem?.ImageByList],
  );
  const photoDataReportId = kpiinfo.id || kpiinfo.kpiId;
  const photoReportIds = useMemo(
    () => [...new Set([photoDataReportId, kpiinfo.kpiId].filter(Boolean))],
    [kpiinfo.kpiId, photoDataReportId],
  );
  ...
  const validateRequiredPhotos = useCallback(async () => {
    ...
    if (requiredTotalImage > 0) {
      ...
      for (const reportId of photoReportIds) {
        const photos =
          (await getAllPhotos(reportId, workinfo.shopId, workinfo.workDate)) ||
          [];
      ...
      if (totalPhoto < requiredTotalImage) {
        toastError(
          'Thông báo',
          `Vui lòng chụp tối thiểu ${requiredTotalImage} hình ảnh trước khi gửi. (${totalPhoto}/${requiredTotalImage})`,
        );
        return false;
      }
    ...
    for (const photoItem of photoList) {
      ...
      if (photoCount < requiredCount) {
        missingPhotos.push(
          `${
            photoItem.nameVN || photoItem.name
    ...
    if (missingPhotos.length > 0) {
      toastError(
        'Thông báo',
        `Vui lòng chụp đủ hình ảnh trước khi gửi.\n${missingPhotos.join('\n')}`,
      );
      return false;
    }
  ...
  const uploadPhotosAndRefresh = useCallback(async () => {
    ...
    for (const reportId of photoReportIds) {
      ...
      if (
        result.statusId !== 200 &&
        result.messager !== 'Không có dữ liệu hình ảnh để gửi'
      ) {
        toastError(
          'Hình ảnh',
          result.messager || 'Gửi dữ liệu hình ảnh không thành công.',
        );
        return;
      }
      ...
      const refreshedPhotos =
        (await getAllPhotos(reportId, workinfo.shopId, workinfo.workDate)) ||
        [];
      const pendingFiles = refreshedPhotos.filter(
        photo => Number(photo.fileUpload) !== 1 && photo.photoPath,
      );
  ...
  const uploadAction = useCallback(async () => {
    ...
    const lstDataUpload = competitorRes.filter(
  ...
  const onChangeTextValue = useCallback(
    async (item, index, text) => {
      ...
      setLstShow(data =>
        data.map((it, idx) => (idx === index ? itemUpdate : it)),
      );
  ...
  const sumQuantityByCate = useCallback(
    competitorId => {
      let totalItem = 0;
      lstShow.forEach(i => {
