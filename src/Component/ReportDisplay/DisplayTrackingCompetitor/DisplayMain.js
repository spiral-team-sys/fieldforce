...
import { getAllListTrackResLG, getAllListTrackUpload, getListCategoryTracking, getListDataTracking } from '../../../Controller/TrackingDetailController';
...
import { getAllPhotosUpload, getPhotosReport, getPhotosReportByCate } from '../../../Controller/WorkController';
...
export const DisplayMain = ({ navigation, route }) => {
    ...
    const checkUploaded = async () => {
        ...
        const reportItem = JSON.parse(kpiinfo?.reportItem || '{}')
        let lstResDSUpload = await getAllListTrackUpload(workinfo, reportItem.DisplayCompetitor);
    ...
    const uploadData = async () => {
        ...
        if (!_valid)
            return
        MessageAction('Bạn có muốn tiếp tục gửi báo cáo không ? (Sau khi gửi dữ liệu sẽ không thể điều chỉnh)', uploadAction);
    }
    const validData = async () => {
        const { isConstraint, isTakeByCate, isTakeTemplate, imageByCate, DisplayCompetitor, ImageByList, isCheckAll, image, isOnlyItem } = reportItem
        //
        ...
        if (lstReuslt == null || lstReuslt.length == 0) {
            ToastError('Vui lòng nhập dữ liệu đầy đủ trước khi gửi báo cáo lên hệ thống')
            return false
        }
        if (isCheckAll == 1) {
            const dataTrackingList = await getListDataTracking(workinfo, DisplayCompetitor)
            if (dataTrackingList !== null && dataTrackingList.length > 0) {
                for (let index = 0; index < dataTrackingList.length; index++) {
                    ...
                    if (item.display == null) {
                        ToastError(`Vui lòng nhập dữ liệu ${item.competitorName} - ${isOnlyItem == 1 ? `Đối thủ` : (item.category || '')}`)
                        return false
                    }
                }
            }
        ...
        if (isConstraint !== undefined && isConstraint === 1) {
            let noteStr = ''
            const resPhotos = await getAllPhotosUpload(reportId, workinfo.shopId, workinfo.workDate);
            ...
            if (isTakeByCate == 1) {
                if (isTakeTemplate == 1) {
                    if (image !== undefined && image > 0) {
                        if (imageByCate == 1) {
                            for (let index = 0; index < lstTab.length; index++) {
                                ...
                                let lstPhoto = await getPhotosReportByCate(reportId, `_${itT.category}`, workinfo.shopId, workinfo.workDate);
                                if (lstPhoto.length < image) {
                                    ToastError(`Vui lòng chụp ${image} tấm hình cho ${itT.category}. (${lstPhoto.length}/${image})`);
                                    return false
                                }
                            }
                        }
                            ...
                            if (LstMenuPhotos[itT.category]?.length > 0) {
                                ...
                                for (let index = 0; index < lstTem.length; index++) {
                                    ...
                                    if (lstPhoto.length < itL.numberIMG) {
                                        noteStr += itT.category + ': Vui lòng chụp ' + itL.numberIMG + ' tấm hình cho ' + itL.nameVN + ', ';
                                    }
                                }
                                if (noteStr !== '') {
                                    ToastError(noteStr);
                    ...
                    if (imageByCate == 1) {
                        for (let index = 0; index < lstTab.length; index++) {
                            const itT = lstTab[index]
                            let lstPhoto = await getPhotosReport(reportId, itT.category, workinfo.shopId, workinfo.workDate);
                            if (lstPhoto.length < image) {
                                ToastError(`Vui lòng chụp ${image} tấm hình cho ${itT.category}. (${lstPhoto.length}/${image})`);
                                return false
                            }
                ...
                if (image !== undefined && image === 0) {
                    ...
                    for (let index = 0; index < LstMenuPhotos.length; index++) {
                        ...
                        let lstPhoto = await getPhotosReport(reportId, it.code, workinfo.shopId, workinfo.workDate);
    ...
    const uploadAction = async () => {
        await UploadController.DataTrackingCompetitor({ ...workinfo, reportId }, reportItem.DisplayCompetitor, async () => {
    ...
    const filterDoneData = async () => {
        ...
        setToolAction({ type: 'FILTER_DONE', enabled: nextDone, id: new Date().getTime() })
    ...
    const setClearAll = async () => {
        ...
        MessageAction('Bạn có chắc chắn muốn xóa hết dữ liệu đã nhập ?', async () => {
            await setDone(false)
    ...
    const renderScene = SceneMap({
        first: () => <TrackingCompetitor key={'trackingCompetitor'} navigation={navigation} route={route} combine={true} reload={reload} Status={Status} toolAction={toolAction} />,
        second: () => reportItem.isTakeByCate == 1 ? null : <PhotoItems key={'PhotoItem'} usedHeader={false} navigation={navigation} route={{ params: { Photos: reportItem.ImageByList || [], Status: Status } }} />,
    });
    ...
    return (
        <View style={{ flex: 1 }}>
            <HeaderCustom
                ...
                iconRight={!isLockReport ? (!Status ? 'cloud-upload-alt' : null) : 'user-lock'}
                rightFunc={() => !isLockReport ? (!Status ? uploadData() : null) : ToastSuccess('Bạn đã hoàn thành chấm công nên không thể gửi dữ liệu báo cáo')}
            />
            <TabForm
                initialPage={0}
                routes={reportItem.isTakeByCate == 1 ? [{ key: "first", title: "Nhập liệu" }] : (reportItem.tabList || routes)}
                positionTabBar={'top'}
                swipeEnabled={false}
            ...
            <ActionSheet
                ...
                gestureEnabled={true}
                containerStyle={{ backgroundColor: appcolor.light, padding: 10 }}
            >
                <View style={styles.sheetContainer}>
                    <Text style={styles.sheetTitle}>Công cụ</Text>
                    <Text style={styles.sheetSubTitle}>Quản lý nhanh dữ liệu trưng bày đối thủ</Text>
