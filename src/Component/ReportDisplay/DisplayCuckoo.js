...
import { StyleSheet, View, Text, TextInput, Platform, Keyboard, KeyboardAvoidingView, ScrollView, TouchableOpacity, } from "react-native"
...
export const DisplayCuckoo = ({ navigation }) => {
    ...
    const checkFormatNumber = async (e, i, type, item) => {
        ...
        let foundValue = colorInput.filter(obj => obj.index === i && obj.type === type && obj.categoryName === item.categoryName);
        ...
        if (mPrice % 1000 > 0 || mPrice == null) {
            ...
            if (foundValue == "")
                setColorInput(data => [...data, { type: type, index: i, categoryName: item.categoryName }]);
            ...
            setData(_listUpdate);
            if (mPrice == null) {
                ToastError("Sản phẩm thứ " + (i + 1) + " : " + item.productName + "\nMục : " + textAlert + "\nBạn chưa nhập giá trị!", "error", "top")
            } else if (mPrice >= 1000) {
                ToastError("Sản phẩm thứ " + (i + 1) + " : " + item.productName + "\nMục " + textAlert + "\nSố tiền không được lẻ, vui lòng nhập lại!", "error", "top")
            } else {
                ToastError("Sản phẩm thứ " + (i + 1) + " : " + item.productName + "\nMục " + textAlert + "\nNhập sai định dạng, vui lòng nhập lại!", "error", "top")
            }
        }
        else {
            let foundValue = colorInput.filter(obj => (obj.index !== i || obj.type !== type));
            setColorInput(foundValue)
        }
    }
    const colorInputError = (index, type, item) => {
        let color = null;
        const dataColor = colorInput.find(obj => (obj.index === index && obj.type === type && obj.categoryName === item.categoryName))
        if (upload) {
            color = appcolor.lightgray
        } else {
            if (dataColor == undefined || (dataColor.type != type && dataColor.index != index && dataColor.categoryName != item.categoryName)) {
                color = appcolor.light
            } else { color = appcolor.warning }
        }
        return color;
    }
    ...
    const onValidated = async (resultValid) => {
        ...
        let isDone = true;
        let uiTask = []
        await uiTask.push(<Text key={"dasda"} style={{ padding: 12, color: appcolor.dark, fontSize: scaleSize(18) }}>
            {
                upload ? "Báo cáo đã gửi lên hệ thống" : "Bạn chưa hoàn thành các mục màu đỏ bên dưới"
            }
        </Text>)
            ...
            uiTask.push(
                <View key={index + "92KK"}>
                    <View style={{ padding: 7, flexDirection: 'row' }} key={v.category}>
                        <Text style={{
                            flexGrow: 1,
                            textDecorationLine: v.countInput < v.totalRow ? 'line-through' : 'none',
                            padding: 3, color: v.countInput < v.totalRow ? appcolor.danger : appcolor.dark
                        }}>{v.categoryName}</Text>
                        <Text style={{ padding: 3, color: appcolor.dark }}>{v.countInput}/{v.totalRow}</Text>
                    </View>
        ...
        await uiTask.push(<Text key={"aaa"} style={{ padding: 12, color: appcolor.dark, fontSize: scaleSize(18) }}>Dữ liệu hình ảnh</Text>)
        await reportItem.ImageByList?.forEach(async (item, index) => {
            ...
            if (isDone === true && photoSize < item.numberIMG)//set false neu trang thai dang done
            {
                isDone = false
            }
            await uiTask.push(
                <View key={index + "-29dkl"}>
                    <View style={{ padding: 7, flexDirection: 'row' }}>
                        <Text style={{
                            flexGrow: 1,
                            textDecorationLine: item.numberIMG > photoSize ? 'line-through' : 'none',
    ...
    const onSummitReport = async () => {
        ...
        await onValidated(async (res) => {
            if (await res === false || upload) {//chua nhap xong du lieu
                await _sheet.current.show()
            } else {//hoan thanh
                if (await checkNetwork()) {
                    await DisplayContext.displayUpload({ ...workinfo, reportId: kpiinfo.kpiId }, (result) => {
        ...
        return (
            <View key={"dss" + index} style={{ padding: 5, borderRadius: 8, marginBottom: 7, backgroundColor: appcolor.light }}>
                <View style={{ padding: 3, flex: 1, flexDirection: 'row' }}>
                    ...
                    <Text style={{ marginStart: 3, fontSize: 14, fontWeight: 'bold', color: appcolor.dark }}>{item.productName}</Text>
                </View>
                <View style={{ padding: 3, flex: 1, flexDirection: 'row' }}>
                    <Text style={{ color: appcolor.dark, opacity: 0.8, fontSize: 12, fontStyle: 'italic' }}>{item.subCategory} {item.segment} {item.subCategories}</Text>
                </View>
                <View style={{ width: '100%', alignItems: 'flex-end', }}>
                    <View >
                        <View style={{ flexDirection: 'row', alignItems: 'center', width: '100%', justifyContent: 'space-between' }}>
                            <View style={{ flexGrow: 1, flexDirection: 'row', alignItems: 'center', }}>
                                <Text style={{ textAlignVertical: 'bottom', color: appcolor.dark, }}>POP </Text>
                                <View style={{ flexDirection: 'row' }}>
                                    <TouchableOpacity disabled={upload ? true : false} onPress={() => onCheckChanged(item, 1)} style={{ padding: 7, marginEnd: 7 }}>
                                        <Text style={{ color: appcolor.dark }}>Đủ</Text>
                                        <Icon size={28} type='font-awesme-5'
                                            name={item.popValue === 1 ? "check-circle" : "circle"}
                                            color={item.popValue === 1 ? appcolor.primary : appcolor.dark}
                                        />
                                    </TouchableOpacity>
                                    <TouchableOpacity disabled={upload ? true : false} onPress={() => onCheckChanged(item, 0)} style={{ padding: 7 }}>
                                        <Text style={{ color: appcolor.dark }}>Thiếu</Text>
                                        <Icon type='font-awesme-5' size={28} name={item.popValue === 0 ? "check-circle" : "circle"}
                                            color={item.popValue === 0 ? appcolor.danger : appcolor.dark} />
                                    </TouchableOpacity>
                                </View>
                            </View>
                            {
                                !upload &&
                                <TouchableOpacity onPress={() => removeAdd(item)} style={{ padding: 7, display: item.addMore ? 'flex' : 'none', flex: 1 }}>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%', }}>
                            <View style={{ flexDirection: 'column', justifyContent: 'center', flex: 1, alignItems: 'center' }}>
                                <TextInput keyboardType="number-pad"
                                    ...
                                    onEndEditing={event => checkFormatNumber(event, index, PRICE_VALUE, item)}
                                    autoCapitalize="none"
                                    ref={(e) => refInput[index] = e}
                                    selectTextOnFocus
                                    placeholder="Giá"
                                    ...
                                    defaultValue={
                                        item.priceValue === null || isNaN(item.priceValue) ? '' :
                                            item.priceValue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
                                    ...
                                    style={{
                                        backgroundColor: //upload ? appcolor.lightgray : appcolor.light,
                                            colorInputError(index, PRICE_VALUE, item), width: '90%',
                                        padding: 7,
                                        borderColor: appcolor.dark, borderWidth: 0.51,
                                        fontSize: scaleSize(14), color: appcolor.dark, textAlign: 'right'
                                    }} />
                                <Text style={{ textAlign: 'center', color: appcolor.dark, fontSize: 13, marginTop: 5 }} >Niêm yết</Text>
                            </View>

                            <View style={{ flexDirection: 'column', justifyContent: 'center', flex: 1, alignItems: 'center' }}>
                                <TextInput keyboardType="number-pad"
                                    onChangeText={(e) => handerNumberChange(item, e, NET_VALUE)}
                                    onEndEditing={event => checkFormatNumber(event, index, NET_VALUE, item)}
                                    autoCapitalize="none"
                                    ref={(e) => refInput[index] = e}
                                    selectTextOnFocus
                                    placeholder="Giá"
                                    ...
                                    defaultValue={
                                        item.netValue === null || isNaN(item.netValue) ? '' :
                                            item.netValue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
                                        //.toLocaleString("en-US")
                                    }
                                    blurOnSubmit={false}
                                    autoCorrect={false}
                                    ...
                                    style={{
                                        backgroundColor: //upload ? appcolor.lightgray : appcolor.light,
                                            colorInputError(index, NET_VALUE, item), width: '90%', padding: 7,
                                        borderColor: appcolor.dark, borderWidth: 0.51,
                                        fontSize: scaleSize(14), color: appcolor.dark, textAlign: 'right'
                                    }} />
                                <Text style={{ textAlign: 'center', color: appcolor.dark, fontSize: 13, marginTop: 5 }} >Thực bán</Text>
                            </View>
                            <View style={{ flexDirection: 'column', justifyContent: 'center', flex: 1, alignItems: 'center' }}>
                                <TextInput keyboardType="number-pad"
                                    onChangeText={(e) => handerNumberChange(item, e, FSM_VALUE)}
                                    onEndEditing={event => checkFormatNumber(event, index, FSM_VALUE, item)}
                                    autoCapitalize="none"
                                    ref={(e) => refInput[index] = e}
                                    selectTextOnFocus
                                    ...
                                    defaultValue={
                                        item.fsmValue === null || isNaN(item.fsmValue) ? '' :
                                            item.fsmValue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
                                        //.toLocaleString("en-US")
                                    }
                                    blurOnSubmit={false}
                                    autoCorrect={false}
                                    ...
                                    style={{
                                        backgroundColor: //upload ? appcolor.lightgray : appcolor.light,
                                            colorInputError(index, FSM_VALUE, item), width: '90%', padding: 7,
                                        borderColor: appcolor.dark, borderWidth: 0.51,
                                        fontSize: scaleSize(14), color: appcolor.dark, textAlign: 'right'
                                    }} />
                                <Text style={{ textAlign: 'center', color: appcolor.dark, fontSize: 13, marginTop: 5 }} >FSM Incentive</Text>
                            </View>
                        </View>
                    </View>
                    {
    ...
    const rowProduct = (item, index) => {
        return (
            <TouchableOpacity onPress={() => onSelected(item, index)} key={"hig" + index} >
                <View key={"rs" + index} style={{ alignItems: 'center', padding: 7, backgroundColor: item.addMore ? appcolor.primary : appcolor.light, flexDirection: 'row', marginEnd: 7 }}>
                    ...
                    {
                        item.addMore ?
                            <Icon size={30} name="playlist-add-check" color={appcolor.light} /> : null
                    }
                </View>
                <View style={{ borderWidth: 1, borderColor: appcolor.surface, width: '100%' }} />
            </TouchableOpacity>
        ...
        <KeyboardAvoidingView
            ...
            <View style={{ flex: 1, backgroundColor: appcolor.light }}>
                <HeaderCustom leftFunc={() => navigation.goBack()}
                    rightFunc={onSummitReport} iconRight={upload === true ? "poll" : "cloud-upload-alt"}
                    iconMiddle="search" middleFunc={upload === true ? null : () => _sheet.current.show()}
                    title={kpiinfo.name} />
                ...
                <ActionSheet
                    ...
                    ref={_sheet} onOpen={() => onLoadProduct()}>
                    {
                        taskDone.length > 0 ?
                            <View style={{ height: '95%' }}>
                                ...
                                <TouchableOpacity style={{ width: '100%', position: 'absolute', bottom: 0, padding: 7, alignItems: 'center' }}
                                    onPress={() => {
                                        setDone([])
                                        _sheet.current.hide()
                                    }}>
                                    <Text style={{ color: appcolor.primary, fontSize: scaleSize(18) }}>Đã hiểu</Text>
                                </TouchableOpacity>
                            </View>
                            :
                            <Fragment>
                                <FormGroup useClearAndroid={false} editable={true}
                                ...
                                <TouchableOpacity onPress={() => onAddMore()}>
                                    <View style={{
                                        ...
                                        backgroundColor: appcolor.primary, padding: 12,
                                    }}>
