...
import { InputDisplayStockCasper } from "./InputDisplayStockCasper";
import moment from "moment";

export const DisplayAndStockCasper = ({ navigation, route }) => {
    ...
    const uploadAction = async () => {
        ...
        let checkPrice = resDisplay.filter(it => (it.quanity !== 'null' && it.quanity !== null) &&
            (it.price !== null && it.price !== 'null' && (it.price < 10000 || it.price % 1000 > 0)
                // || it.fsmValue !== null && it.fsmValue !== 'null' && (it.fsmValue < 10000 || it.fsmValue % 1000 > 0)
            ));
    ...
    const ViewItemInput = () => {
        return (
            <InputDisplayStockCasper Status={Status} navigation={navigation} reloadView={reloadView} />
        );
    };
    const ViewItemPhoto = () => {
        return (
