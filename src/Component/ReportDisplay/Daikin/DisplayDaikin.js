...
import { actualPhoto, checkPhotoByProduct, clearAllDataDisplay, getDisplayProduct, getListTagPOP, isPhotoNoData, updateItemDisplay, uploadDisplayPOP } from "../../../Controller/DisplayController";
import { checkLockReport } from "../../../Controller/ShopController";
import { getDisplayResult, getNoteDisplayReport, updateMockupDisplay, updateNoteDisplayReport } from "../../../Controller/WorkController";
...
import { getCompetitorByProduct, getlistTabByCategory } from "../../../Controller/StockOutController";
...
export const DisplayDaikin = ({ navigation }) => {
    ...
    const uploadAction = async () => {
            ...
            for (let index = 0; index < dataDisplay.length; index++) {
                ...
                if (isPrice && item.quanity !== null && item.quanity > 0) {
                    if (item.price == 'null' || item.price == null) {
                        MessageInfo(`Giá sản phẩm ${item.productName} ngành hàng ${item.categoryName} hãng ${item.division} chưa nhập`, 'Giá sản phẩm', 'top')
                        return
                    }
                }
                //
                if (item.price !== 'null' && item.price !== null && (item.price < 10000 || item.price % 1000 > 0)) {
                    MessageInfo(`Giá sản phẩm ${item.productName} ngành hàng ${item.categoryName} hãng ${item.division} sai định dạng`, 'Giá sản phẩm', 'top')
                    return
                }
                if ((item.quanity == 'null' || item.quanity == null) && (item.price !== 'null' && item.price !== null && item.price > 0)) {
                    MessageInfo(`Chưa nhập số lượng ${item.productName} ngành hàng ${item.categoryName} hãng ${item.division}`, 'Số lượng', 'top')
                    return
                }
            }
            ...
            if (lstNoData.length > 0) {
                for (let index = 0; index < lstNoData.length; index++) {
                    ...
                    if (item.countPhoto > 0) {
                        ToastError(`Chưa nhập dữ liệu ngành hàng ${item.categoryName} hãng ${item.division} (Đã chụp hình nhưng chưa nhập dữ liệu)`, 'Dữ liệu', 'top')
                        return
                    }
                }
            }
        ...
        Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', async () => {
            await UploadController.DataDisplay(dataDisplay, { ...workinfo, reportId: kpiinfo.id }, async () => {
                await LoadData()
            })
        })
    }
    ...
    const handlerNoteAction = async (item) => {
        const dataNote = await getNoteDisplayReport(workinfo.workId, null, item.division)
    ...
    const handlerShowItemInput = (show) => {
        ...
        if (show) {
            const dataInput = _.filter(dataMain, (i) => {
                return (i.quanity !== null && i.quanity >= 0) || (i.price !== null && i.price >= 0)
            })
            setData(dataInput)
        } else {
            setData(dataMain)
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View key={`ood_oo_${index}`} style={styles.mainItem}>
                ...
                <View style={{ flex: 1, flexDirection: 'row', backgroundColor: appcolor.surface, borderRadius: 5 }}>
                                    ...
                                    <Tabs.Tab key={`iid-i_${i}`} label={labelItem} name={labelItem} >
                                        <View style={{ backgroundColor: appcolor.light, marginTop: 40, padding: 6, width: deviceWidth }}>
                                            <FlatList
                                                ...
                                                ListFooterComponent={<Text style={{ width: '100%', height: deviceHeight / 2.5, textAlign: 'center', color: appcolor.dark, padding: 8 }}>{arr.length > 10 ? 'Đã xem hết' : ''}</Text>}
                                            />
                                        </View>
                                    </Tabs.Tab>
                                    :
...
const HeaderItemView = ({ itemHeader, competitorName, handlerNote }) => {
    ...
    const onChangeValue = async (text) => {
        let mockup = text !== null && text.length > 0 ? text.toString().replace(/,/g, '') : null
    ...
    return (
        <View style={{ width: deviceWidth - 16, margin: 3, borderRadius: 5, alignSelf: 'center' }}>
            <FormGroup
                keyboardType="numeric"
                containerStyle={{ padding: 3, backgroundColor: appcolor.placeholderBody }}
