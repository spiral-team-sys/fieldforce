...
import {
  getDataDisplaySurvey,
  getDisplaySurveyResult,
  getTabDisplaySurvey,
  uploadDisplaySurvey,
...
export const DisplaySurveyReport = ({ navigation, route }) => {
  ...
  const uploadAction = async () => {
    ...
    const checkDone = data.listData.filter(
      it =>
        (it.posmValue === null ||
          it.posmValue === 'null' ||
          it.posmValue === '') &&
    ...
    if (checkDone.length > 0) {
      ToastError(
        'Bạn phải hoàn thành các khảo sát có chấm đỏ trước khi gửi!!',
        'Thông báo',
        'top',
      );
      return;
