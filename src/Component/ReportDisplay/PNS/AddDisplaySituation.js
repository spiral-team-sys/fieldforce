...
const AddDisplaySituation = ({ loadData, onClose, ItemSaved, loadHistory, reload }) => {
    ...
    const [photo, setPhoto] = useState({ arrPhoto: [], isReport: [], isProduct: [] })
    ...
    const loadPhoto = async () => {
        ...
        const isReport = listPhoto.filter(it => it.photoType == `DISPLAY_REPORT_${guiId}`) || []
        const isProduct = listPhoto.filter(it => it.photoType == `DISPLAY_PRODUCT_${guiId}`) || []
        await setPhoto({ arrPhoto: listPhoto, isReport: isReport, isProduct: isProduct })
    }
    ...
    const takePhoto = async (item, tag) => {
        const reloadpage = async (result) => {
            ...
            const itemPhoto = photo.arrPhoto.find(it => it.guid === item.guiId && it.photoType == tag) || {}
            await Object.keys(itemPhoto).length > 0 && itemPhotoByType.length > 1 ? deleteItemPhotoDuplicate(itemPhoto) : null
            await loadPhoto()
        }
        let photoinfo = {};
        photoinfo = {
        ...
        <View style={{ flex: 1 }}>
            ...
            <View style={{ flex: 1, backgroundColor: appcolor.surface, padding: 5 }}>
                <FormGroup rightFunc={() => { }}
                    ...
                    inputStyle={{ height: 50, maxHeight: 120 }} multiline
                    title={'Thông tin / Vấn đề trưng bày'}
                    placeholder={'Nhập thông tin'}
                    defaultValue={itemShow.issueDisplay}
                    handleChangeForm={text => setItemShow({ ...itemShow, issueDisplay: text })}
                    onClearTextAndroid={() => setItemShow({ ...itemShow, issueDisplay: '' })}
                ...
                <FormGroup rightFunc={() => { }}
                    ...
                    handleChangeForm={text => setItemShow({ ...itemShow, displayItemName: text })}
                    onClearTextAndroid={() => setItemShow({ ...itemShow, displayItemName: '' })}
                    placeholderTextColor={appcolor.greydark}
                    editable
                />
                <FormGroup rightFunc={() => { }}
                ...
                />
                <View style={{ flex: 1 }}>
                    <View style={{ width: '100%', height: photo.arrPhoto.length > 0 ? 150 : 0, flexDirection: 'row', justifyContent: 'space-between', justifyContent: "center" }}>
                        <View style={{ width: '50%' }} >
                            {
                                photo.isReport?.filter(it => it.photoPath).map((it, index) => (
                                    <TouchableOpacity key={`report_${it.id || it.photoPath || index}`} style={{ flex: 1, margin: 5, justifyContent: 'center', alignItems: 'center', borderColor: appcolor.dark, borderWidth: 0.5, borderRadius: 10 }} onPress={() => handlerOpenModal(it.photoPath)}>
                                        <Image style={{ height: 130, margin: 5, width: 100, borderRadius: 10 }}
                            ...
                            }
                            {
                                ...
                                <View style={{ flex: 1, margin: 5, justifyContent: 'center', alignItems: 'center', borderColor: appcolor.dark, borderWidth: 0.5, borderRadius: 10 }} >
                                    <Icon name="mobile-alt" color={appcolor.grey} size={60} />
                                </View>
                            }
                        </View>
                        <View style={{ width: '50%', }}>
                            {
                                photo.isProduct?.filter(it => it.photoPath).map((it, index) => (
                                    <TouchableOpacity key={`product_${it.id || it.photoPath || index}`} style={{ flex: 1, margin: 5, justifyContent: 'center', alignItems: 'center', borderColor: appcolor.dark, borderWidth: 0.5, borderRadius: 10 }} onPress={() => handlerOpenModal(it.photoPath)}>
                                        <Image style={{ height: 100, margin: 5, width: 130, borderRadius: 10 }}
                                            source={{ uri: it.photoPath }} />
                                    </TouchableOpacity>
                                ))
                            }
                            {
                                ...
                                <View style={{ flex: 1, margin: 5, justifyContent: 'center', alignItems: 'center', borderColor: appcolor.dark, borderWidth: 0.5, borderRadius: 10 }} >
                                    <Icon name="mobile-alt" style={{ transform: [{ rotate: '90deg' }] }} color={appcolor.grey} size={60} />
                                </View>
                            }
                        </View>

                    </View>
                    <View style={{ width: '100%', padding: 5, flexDirection: 'row', justifyContent: 'space-between', justifyContent: "center", alignItems: "center" }}>
                        <TouchableOpacity style={{ flex: 1, padding: 5, borderRadius: 5, margin: 5, borderWidth: 0.5, borderColor: appcolor.primary, flexDirection: 'row', justifyContent: 'center', alignItems: "center" }} onPress={() => takePhoto(itemShow, `DISPLAY_REPORT_${itemShow.guiId}`)}>
                            <Icon name="mobile-alt" color={appcolor.primary} size={20} />
                            <Text style={{ paddingLeft: 5, color: appcolor.dark }}>Biên bản</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={{ flex: 1, padding: 5, borderRadius: 5, margin: 5, borderWidth: 0.5, borderColor: appcolor.primary, flexDirection: 'row', justifyContent: 'center', alignItems: "center" }} onPress={() => takePhoto(itemShow, `DISPLAY_PRODUCT_${itemShow.guiId}`)}>
                            <View style={{ width: 20, alignItems: "center" }}>
                                <Icon name="mobile-alt" type="font-awesome-5" style={{ transform: [{ rotate: '90deg' }] }} color={appcolor.primary} size={20} />
                            </View>
                            <Text style={{ paddingLeft: 5, color: appcolor.dark }} >Sản phẩm</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </View>
            <Modal visible={visibleModal} style={{ flex: 1 }}>
                <View style={{ width: '100%', height: '100%', backgroundColor: appcolor.light }}>
                    ...
                    <TouchableOpacity onPress={handlerCloseModal}
                        ...
                        <Icon name='times' color={appcolor.dark} size={30} />
                    </TouchableOpacity>
                </View>
            </Modal>
        </View>
