...
export const DisplayPricePNS = ({ navigation }) => {
  ...
  const [tagItemView, setTagItemView] = useState({
    popId: 0,
    popColor: appcolor.primary,
    displayId: 0,
  ...
  const loadPhotoCount = useCallback(async () => {
    const listPhoto = await getAllPhotos(
      reportId,
      workinfo.shopId,
      workinfo.workDate,
    );
  ...
  const LoadData = async (nextActiveTabIndex = 0) => {
    ...
    try {
      ...
      setSettings({
        ...
        isUploaded: workinfo.workDate === day ? status : true,
      });
      ...
      setPhotoByCategory(listPhoto || []);
      setActiveTabIndex(
        Math.min(nextActiveTabIndex, Math.max(tabCompetitor.length - 1, 0)),
      );
      setTagItemView({
  ...
  const contains = item => {
    if (
      ...
      item.displayArea != null ||
      item.mockupValue != null ||
      item.tagPOPId != null ||
      ...
      for (let index = 0; index < lstCheckPhoto.length; index++) {
        ...
        for (let j = 0; j < configItem.length; j++) {
          ...
          if (actual < jItem.numberIMG) {
            toastError(
              'Hình ảnh',
              `Chưa chụp đầy đủ hình ${jItem.name} ngành hàng ${item.categoryName} (${actual}/${jItem.numberIMG} tấm)`,
            );
            return;
          }
        }
      ...
      for (let index = 0; index < dataDisplay.length; index++) {
        ...
        if (isPrice && item.quanity !== null && item.quanity > 0) {
          if (item.price == 'null' || item.price == null) {
            toastError(
              'Giá sản phẩm',
              `Giá sản phẩm ${item.productName} ngành hàng ${item.categoryName} chưa nhập`,
            );
            return;
          }
        }
        ...
        if (item.productComment != null && item.productComment != '') {
          ...
          if (item.productComment?.length < 5) {
            toastError(
              'Ghi chú',
              `Ghi chú ít nhất 5 kí tự, sản phẩm ${item.productName} ngành hàng ${item.categoryName}`,
              'top',
            );
            return;
          }
      ...
      if (lstNoData.length > 0) {
        for (let index = 0; index < lstNoData.length; index++) {
          ...
          if (item.countPhoto > 0) {
            toastError(
              'Dữ liệu',
              `Chưa nhập dữ liệu ngành hàng ${item.categoryName} (Đã chụp hình nhưng chưa nhập dữ liệu)`,
              'top',
            );
            return;
          }
  ...
  const handlerCameraAction = useCallback(
    async item => {
      navigation.navigate('photogroup', {
        Status: settings.isUploaded,
        keyPhoto: item.categoryName,
        hideIcon: true,
      });
    },
    [navigation, settings.isUploaded],
  );
  const handlerNoteAction = useCallback(
    async item => {
    ...
    if (show) {
      const dataInput = _.filter(dataMain, i => {
        return (
          ...
          (i.productComment != null && i.productComment !== '')
        );
      });
      setData(dataInput);
    } else {
  ...
  const dataByTab = useMemo(() => {
    return dataTab.map(it => {
      ...
      const totalPhoto = (
        photoByCategory?.filter(photo =>
          photo.photoType?.includes(it.categoryName),
        ) || []
      ).length;
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        tagButtonPOP: {
          padding: 8,
          backgroundColor: tagItemView.popColor,
          borderRadius: 10,
          flexDirection: 'row',
        },
        tagButtonDisplay: {
          padding: 8,
          backgroundColor: tagItemView.displayColor,
          borderRadius: 10,
          flexDirection: 'row',
        },
        tagTitle: {
  ...
  const renderTab = useCallback(
    (itemTab, index) => {
      const tabData = dataByTab[index] || { items: [], totalPhoto: 0 };
      return (
        <View style={styles.tabBody}>
          <CustomListView
            extraData={tabData.items}
            data={tabData.items}
      ...
      {!loading && isPOP && (
        <View style={styles.tagMain}>
          <TouchableOpacity onPress={tagPOPAction} style={styles.tagButtonPOP}>
            <SpiralIcon name="tag" size={18} color={appcolor.light} />
          </TouchableOpacity>
          <Text style={styles.tagTitle}>POP</Text>
          <TouchableOpacity
            onPress={tagDisplayAction}
            style={styles.tagButtonDisplay}
          >
            <SpiralIcon name="tag" size={18} color={appcolor.light} />
          </TouchableOpacity>
...
const InputNoteProduct = ({ item, index }) => {
  ...
  const changeNote = text => {
    const noteProduct =
      text !== null && text !== '' && text.length > 0 ? text : null;
...
const InputPrice = ({ item, index }) => {
  ...
  return (
    <View
      key={`ppd_${index}`}
      style={{ flex: 1.45, alignSelf: 'center', marginStart: 4 }}
    >
      <TextInput
...
const ToolsAction = ({
  ...
  const renderButton = ({
    ...
    return (
      <TouchableOpacity
        activeOpacity={0.8}
        style={[styles.sheetAction, danger && styles.sheetActionDanger]}
        onPress={actionPress}
      >
        <View style={[styles.sheetIconView, danger && styles.sheetIconDanger]}>
          <SpiralIcon
            name={iconName}
            size={20}
            color={iconColor}
        ...
        <Text
          style={[
            ...
            danger && styles.sheetActionTextDanger,
          ]}
        >
          {title}
        </Text>
        <SpiralIcon
          ...
          color={
            danger
              ? appcolor.danger
              : appcolor.greydark || appcolor.placeholderText
          }
        />
      </TouchableOpacity>
    );
  ...
  return (
    <View style={styles.sheetContainer}>
      ...
      <Text style={styles.sheetSubTitle}>
        Quản lý nhanh dữ liệu trưng bày trong báo cáo này
      </Text>
      {renderButton({
        title: 'Xem dữ liệu đã nhập',
        iconName: isShowInput ? 'check-circle' : 'keyboard',
        iconColor: isShowInput ? appcolor.success : appcolor.dark,
        active: isShowInput,
        actionPress: onShow,
      })}
      {!isLock &&
  ...
  const RenderButton = ({ iconName, titleName, onAtion }) => {
    ...
    return (
      ...
      >
        ...
        >
          <SpiralIcon name={iconName} size={21} color={appcolor.yellow} solid />
          <Text
            style={{
              fontSize: 13,
              fontWeight: '400',
