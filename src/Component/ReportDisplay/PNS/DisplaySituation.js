...
import { DataDisplaySituation, deleteItemDisplaySituation, deleteItemPhotoDuplicate, getDataDisplaySituation, getPhotoByType, getPhotoDisplaySituation } from "../../../Controller/DisplayController";
...
export const DisplaySituation = ({ navigation }) => {
    const { appcolor, workinfo, shopinfo, kpiinfo } = useSelector(state => state.GAppState)
    ...
    const uploadAction = () => {
        Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?',
            async () => {
                ...
                await DataDisplaySituation(data.listData, work, async () => {
                    await loadData();
                }, async () => {
                })
            })
    ...
    const takePhoto = async (item, tag) => {
        const reloadpage = async (result) => {
            ...
            const itemPhoto = data.listPhoto.find(it => it.guid === item.guiId && it.photoType === tag) || {}
    ...
    const renderItem = ({ item, index }) => {
        ...
        const reportPhotos = arrPhoto.filter(it => it.photoPath && it.photoType === `DISPLAY_REPORT_${item.guiId}`) || []
        const productPhotos = arrPhoto.filter(it => it.photoPath && it.photoType === `DISPLAY_PRODUCT_${item.guiId}`) || []
        return (
            ...
            >
                <TouchableOpacity style={styles.itemContainer} onPress={() => Status !== 1 && handlerOnSelectItem(item)}>
                    <Text style={styles.itemTitle}>{index + 1}. Thông tin/Vấn đề trưng bày : {item.issueDisplay} </Text>
                    <Text style={styles.itemTitle}>Gói Trưng Bày : {item.displayItemName} </Text>
                        ...
                        <View style={styles.photoColumn} >
                            {
                                reportPhotos.map((it, photoIndex) => (
                                    <TouchableOpacity key={`report_${it.id || it.photoPath || photoIndex}`} style={styles.photoWrapper} onPress={() => handlerOpenModal(it.photoPath)}>
                            ...
                            }
                            {
                                ...
                                <View style={styles.photoWrapper} >
                                    <Icon name="mobile-alt" type="font-awesome-5" color={appcolor.grey} size={60} />
                                </View>
                            }
                        </View>
                        <View style={styles.photoColumn}>
                            {
                                productPhotos.map((it, photoIndex) => (
                                    <TouchableOpacity key={`product_${it.id || it.photoPath || photoIndex}`} style={styles.photoWrapper} onPress={() => handlerOpenModal(it.photoPath)}>
                            ...
                            }
                            {
                                ...
                                <View style={styles.photoWrapper} >
                                    <Icon name="mobile-alt" type="font-awesome-5" style={styles.productPlaceholderIcon} color={appcolor.grey} size={60} />
                                </View>
                            }
                        </View>
                    </View>
                    {
                        ...
                        <View style={styles.actionRow}>
                            <TouchableOpacity style={styles.actionButton} onPress={() => takePhoto(item, `DISPLAY_REPORT_${item.guiId}`)}>
                                <Icon name="mobile-alt" type="font-awesome-5" color={appcolor.primary} size={20} />
                                <Text style={styles.actionText}>Biên bản</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={styles.actionButton} onPress={() => takePhoto(item, `DISPLAY_PRODUCT_${item.guiId}`)}>
                                <Icon name="mobile-alt" type="font-awesome-5" style={styles.productActionIcon} color={appcolor.primary} size={20} />
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={[styles.container, { display: showAdd }]}>
                <AddDisplaySituation key={'addItem' + reload} loadData={loadData} onClose={handlerCloseModal} ItemSaved={itemSave} loadHistory={loadHistory} closeModal={(value) => setVisibleModal(value)} reload={reload} />
            </View>
            <Modal visible={visibleModal} style={styles.container}>
                <View style={styles.modalContainer}>
                    ...
                    <TouchableOpacity onPress={handlerCloseModal}
                        ...
                        <Icon name='close' color={appcolor.dark} size={30} />
                    </TouchableOpacity>
                </View>
