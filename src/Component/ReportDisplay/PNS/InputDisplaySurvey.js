...
export const InputDisplaySurvey = ({
  ...
  const ViewItem = () => {
    return data.listTab.map(it => {
      let listDataByGroup = [];
      listDataByGroup = data.listData.filter(
        item => item.groupId === it.groupId && item.kpi1 !== -1,
      );
      const totalRow = listDataByGroup.length;
      return (
        <Tabs.Tab
          key={it.groupName + `(${totalRow})`}
          label={it.groupName + `(${totalRow})`}
          name={it.groupName + `(${totalRow})`}
        >
      ...
      uiTask.push(
        <View key={`TEXT` + item.kpi1} style={Styles.viewColumn}>
          ...
          <TextInput
            ...
            selectTextOnFocus={item.upload !== 1 ? true : false}
            multiline={true}
            autoCorrect={false}
          ...
          >
            {listCheck.map(it => {
              return (
                ...
                >
                  <SpiralIcon
                    name={item.posmValue == it.name ? 'check-circle' : 'circle'}
                    size={20}
                    style={{
                      color:
                        item.posmValue == it.name
                          ? appcolor.success
                          : appcolor.greydark,
                    }}
                  />
...
const RenderItem = ({ item, index, data, appcolor, listCheck, Styles }) => {
  ...
  const onOpenDropDown = () => {
    const listDropDown = data.listData.filter(
      it => it.groupId == item.groupId && it.itemName == 'PRODUCT',
    );
    listDropDown.map(it =>
      item.posmValue?.includes(it.productName)
        ? (it.isCheck = true)
        : (it.isCheck = false),
    );
  ...
  const onSelectItemSheet = itemSelect => {
    if (
      itemSelect.productName !== 'Đầy Đủ' &&
      !item.posmValue?.includes('Đầy Đủ')
    ) {
      if (!item.posmValue?.includes(itemSelect.productName)) {
        item.posmValue !== '' && item.posmValue !== 'null' && item.posmValue
          ? (item.posmValue += `, ${itemSelect.productName}`)
          : (item.posmValue = `${itemSelect.productName}`);
        itemSelect.isCheck = true;
      } else {
        if (item.posmValue?.includes(`, ${itemSelect.productName}`)) {
          item.posmValue = item.posmValue.replace(
      ...
      if (!item.posmValue?.includes(itemSelect.productName)) {
        item.posmValue = `${itemSelect.productName}`;
        listDropDown.map(it =>
          it.id === itemSelect.id ? (it.isCheck = true) : (it.isCheck = false),
        );
      } else {
        item.posmValue = item.posmValue.replace(
          `${itemSelect.productName}`,
      ...
      >
        <View style={Styles.viewSheet}>
          <Calendar
            firstDay={1}
            current={moment(
              item.kpi1 === 5 && item.posmValue
                ? item.posmValue.toString()
                : new Date(),
            ).format('yyyy-MM-DD')}
            monthFormat={'MM - yyyy'}
            hideExtraDays={true}
            ...
            {listDropDown.map((it, id) => {
              return (
                ...
                >
                  <Text
                    style={{
                      color: it.isCheck ? appcolor.white : appcolor.dark,
                      padding: 5,
                    }}
                  >
                    {it.productName}
