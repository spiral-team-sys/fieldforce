...
import { getDisplayProduct, getlistTabCompetitor } from '../../../Controller/DisplayController'
...
export const DisplayAndStock = ({ navigation, route }) => {
    ...
    const uploadAction = async () => {
        ...
        if (lstReport.checkAllProduct == 1) {
            if (lstReport.checkStock == 1) {
                const dataByStock = resDisplay.filter(it => it.quanity !== 'null' && it.quanity !== null && it.quantityStock !== 'null' && it.quantityStock !== null)
                if (dataByStock.length !== listProduct.length) {
                    ToastError('Vui lòng nhập số lượng trưng bày, tồn kho tất cả sản phẩm trước khi gửi!');
                    return;
                }
            } else {
                if (resDisplay.length !== listProduct.length) {
                    ToastError('Vui lòng nhập số lượng trưng bày tất cả sản phẩm trước khi gửi!');
                    return
                }
            }
        }
        ...
        let checkFsmValue = resDisplay.filter(it => (it.price === 'null' || it.price === null) &&
            (it.fsmValue !== 'null' && it.fsmValue !== null && it.fsmValue > 0));
        if (checkFsmValue.length > 0) {
            let product = listProduct.filter(it => it.productId === checkFsmValue[0].productId)
            ToastError('Bạn đã nhập Tiền thưởng nhưng chưa nhập Giá . Hãng: ' + product[0].division + '  - Loại : ' + product[0].categoryName + '  - sản phẩm: ' + product[0].productName, 'Thông báo', 'top');
            return;
        }
        ...
        let checkQuantity = resDisplay.filter(it => (it.quanity === 'null' || it.quanity === null) &&
            ((it.quantityStock !== 'null' && it.quantityStock !== null && it.price >= 0) ||
                (it.quantitySuggest !== 'null' && it.quantitySuggest !== null && it.quantitySuggest >= 0)));
    ...
    const ViewItemInput = () => {
        return (
            <InputDisplayStock Status={Status}
                navigation={navigation} listInput={listInput} reloadView={reloadView} />
        );
    };
