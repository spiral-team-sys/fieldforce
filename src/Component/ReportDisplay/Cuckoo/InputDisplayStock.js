...
export const InputDisplayStock = ({ navigation, route, Status, listInput, reloadView }) => {
    ...
    const filterEmptyProduct = () => {
        if (!isEmpty) {
            let lstRes = data.dataShow.filter(it => (it.quanity == null &&
                it.price == null &&
                    ...
                    <Tabs.Tab key={it.categoryName + `(${totalRow})`} label={it.categoryName + `(${totalRow})`} name={it.categoryName + `(${totalRow})`}>
                        <View style={Styles.viewTabStyle}>
                            ...
                            <FlatList
                                ...
                                renderItem={({ item, index }) => <RenderItem item={item} index={index} data={data} Styles={Styles} listInput={listInput} appcolor={appcolor} workinfo={workinfo} />}
                            />
                        </View>
                    </Tabs.Tab>
                )
    ...
    const Styles = StyleSheet.create({
        ...
        itemStyle: { height: 150, backgroundColor: appcolor.light, padding: 5, margin: 5, elevation: 2, borderRadius: 10 },
                    ...
                    <View style={Styles.actionSheetStyle}>
                        <View style={{ width: '100%' }}>
                            ...
                            <TouchableOpacity onPress={filterEmptyProduct}>
                                <View style={{
                                    backgroundColor: isEmpty ? appcolor.light : appcolor.surface,
                                    ...
                                    <Icon name={'keyboard'} size={18} color={appcolor.success} />
                                    <Text style={{ width: '100%', fontSize: 14, fontWeight: '400', color: appcolor.dark, padding: 8 }}>Xem dữ liệu chưa nhập</Text>
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity
                                onPress={setClearAll}>
...
const RenderItem = ({ item, index, data, Styles, workinfo, listInput, appcolor }) => {
...
const RenderItemInput = ({ itemInput, indexInput, item, Styles, listInput, appcolor, data, workinfo }) => {
    ...
    const endInput = async (e) => {
        ...
        let intValue = value === '' ? null : parseInt(value);
        let isError = 0
        if (intValue < 1000 && (itemInput.displayType === 'price' || itemInput.displayType === 'fsmValue')) {
            item[itemInput.displayType] = null
            intValue = null;
            isError = 1;
            setInput(null)
