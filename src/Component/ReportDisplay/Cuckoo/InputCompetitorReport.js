...
import { addItemDisplayCompetitor, clearAllDataCompetitor, deleteItemDisplayCompetitor, getListCategoryDisplay, getListCompetitorProduct, getListSubCategoryDisplay, getTabCompetitorReport, updateItemCompetitor } from "../../../Controller/DisplayController";
import { groupDataByKey, Message, ToastError, ToastSuccess } from "../../../Core/Helper";
import { deviceHeight, minWidthTab, removeAccents } from "../../../Core/Utility";
...
export const InputCompetitorReport = ({ navigation, route, Status, reloadView }) => {
                ...
                return (
                    <Tabs.Tab key={it.division + `(${totalRow})`} label={it.division + `(${totalRow})`} name={it.division + `(${totalRow})`}>
                        ...
                        <Tabs.FlatList
                            ...
                            ListHeaderComponent={Status !== 1 ? <AddProduct appcolor={appcolor} loadDataInput={loadDataInput} itemCompetitor={it} Styles={Styles} listInput={listInput} workinfo={workinfo} data={data} /> : <View />}
                            ListFooterComponent={<Text style={Styles.footerStyle} >Đã xem hết</Text>}
                            renderItem={({ item, index }) => <RenderItem item={item} index={index} data={data} deleteItem={deleteItem} listInput={listInput} Styles={Styles} appcolor={appcolor} ref_addProduct={ref_addProduct} workinfo={workinfo} />}
                        />
                        {/* </View> */}
                    </Tabs.Tab>
                )
    ...
    const filterDoneProduct = async () => {
        if (!isDone) {
            let lstRes = data.dataShow.filter(it => (it.quantity !== null && it.quantity >= 0) ||
                (it.priceValue !== null && it.priceValue >= 0) ||
                (it.netValue !== null && it.netValue >= 0) ||
                (it.fsmValue !== null && it.fsmValue >= 0))
            data.dataShow = lstRes;
    ...
    const Styles = StyleSheet.create({
        ...
        titleStyle: { color: appcolor.dark, fontSize: 16, padding: 4, fontWeight: '600' },
        viewTabStyle: { backgroundColor: appcolor.surface, marginTop: 10, padding: 5, display: !showProgress ? 'flex' : 'none' },
        ...
        itemStyle: { height: 140, backgroundColor: appcolor.light, padding: 5, margin: 5, elevation: 2, borderRadius: 10 },
        textItemStyle: { color: appcolor.dark, fontSize: 12, fontStyle: 'italic' },
        titleGroupCategory: { color: appcolor.white, fontSize: 18, paddingLeft: 16, fontWeight: "600", },
                ...
                <View style={Styles.actionSheetStyle}>
                    <View style={{ width: '100%' }}>
                        ...
                        <TouchableOpacity
                            ...
                            <Text style={[Styles.textSheetStyle, { color: appcolor.dark }]} >Sản phẩm đã thêm</Text>
                            <Icon name={!isFilterAdd ? 'checkmark-circle-outline' : 'check-circle'} type={!isFilterAdd ? 'ionicon' : ''} size={23} color={!isFilterAdd ? appcolor.dark : appcolor.success} />
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={[Styles.buttonSheetStyle, { borderColor: appcolor.dark }]}
...
const AddProduct = ({ appcolor, Styles, workinfo, loadDataInput, itemCompetitor, data, listInput }) => {
    ...
    const onSelectCategory = (itemCategory) => {
        setProductItem({
            ...productItem,
            categoryId: productItem.categoryId === itemCategory.categoryId ? 0 : itemCategory.categoryId,
            categoryName: productItem.categoryName === itemCategory.categoryName ? '' : itemCategory.categoryName,
            subCatId: 0,
            subCategory: "",
        })
    }
    ...
    const renderCategory = ({ item, index }) => {
        return (
            <TouchableOpacity style={{ marginEnd: 5, padding: 8, minWidth: 50, flexDirection: 'row', justifyContent: "center", backgroundColor: productItem.categoryId === item.categoryId ? appcolor.primary : appcolor.surface, borderColor: appcolor.greydark, borderWidth: 0.2, borderRadius: 50, }}
                onPress={() => onSelectCategory(item)}
            >
                <Text style={{ color: productItem.categoryId === item.categoryId ? appcolor.white : appcolor.dark }}>{item.categoryName}</Text>
            </TouchableOpacity>
        )
    }
    ...
    const saveProduct = async () => {
        ...
        let value = removeAccents(productItem.modelName.replace(/ /g, '').toUpperCase())
        let itemsUpload = [{
        ...
        Message('Chú ý', 'Bạn có chắc chắn muốn lưu sản phẩm?', async () => uploadProduct(itemsUpload));
    }
    ...
    const viewCategory = () => {
        const dataSubCat = data.dataSubCategory.filter(it => it.categoryId === productItem.categoryId)
        return (
            <View>
                <View style={{ flexDirection: 'row', paddingBottom: 10 }}>
                    <Text style={{ width: '30%', color: appcolor.dark, justifyContent: "center", padding: 8 }} >Ngành hàng :</Text>
                    <View style={{ width: '70%' }}>
                        <FlatList
                            horizontal
                ...
                </View>
                {
                    ...
                    <ViewSubCategory dataSubCat={dataSubCat} productItem={productItem} appcolor={appcolor} />
                }
            </View>
        )
    }
    ...
    return (
        <View style={{ width: '100%', backgroundColor: appcolor.light, borderRadius: 10, borderColor: appcolor.greydark, borderWidth: 0.2 }}>
            <View style={{ paddingTop: 10, margin: 10 }}>
                <TextInput
                    editable={true} selectTextOnFocus={true}
                    autoCorrect={false}
                    onChangeText={onChangeText}
                    style={{ padding: 10, color: appcolor.dark, height: 40, textAlign: 'left', borderWidth: 0.4, borderRadius: 10, borderColor: appcolor.dark, backgroundColor: appcolor.light }}
            ...
            <View style={{ marginTop: 10 }} >
                <FlatList
                    ...
                    getItemLayout={getItemLayout}
                    removeClippedSubviews={true}
                    renderItem={({ item, index }) => <RenderItemInput itemInput={item} indexInput={index} item={productItem} listInput={listInput} Styles={Styles} appcolor={appcolor} data={data} workinfo={workinfo} />}
                    style={{ padding: 5 }}
            ...
            <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center' }}>
                <TouchableOpacity
                    ...
                    onPress={saveProduct}
                >
                    <Text style={[Styles.textSheetStyle, { color: appcolor.white }]} >Thêm</Text>
                </TouchableOpacity>
            </View>
        </View>
    )
...
const ViewSubCategory = ({ dataSubCat, productItem, appcolor }) => {
    ...
    const onSelectSubCategory = (itemCategory) => {
        productItem.subCatId = (productItem.subCatId === itemCategory.subCatId ? 0 : itemCategory.subCatId)
        productItem.subCategory = (productItem.subCategory === itemCategory.subCategory ? '' : itemCategory.subCategory)
        setMutate(e => !e)
    }
    const renderSubCategory = ({ item, index }) => {
        return (
            <TouchableOpacity style={{ marginEnd: 5, padding: 8, minWidth: 50, flexDirection: 'row', justifyContent: "center", backgroundColor: productItem.subCatId === item.subCatId ? appcolor.primary : appcolor.surface, borderColor: appcolor.greydark, borderWidth: 0.2, borderRadius: 50, }}
                onPress={() => onSelectSubCategory(item)}
            >
                <Text style={{ color: productItem.subCatId === item.subCatId ? appcolor.white : appcolor.dark }}>{item.subCategory}</Text>
            </TouchableOpacity>
        )
    }
    return (
        <View style={{ flexDirection: 'row', paddingBottom: 10 }}>
            <Text style={{ width: '40%', color: appcolor.dark, justifyContent: "center", padding: 8 }} >Ngành hàng nhỏ :</Text>
            <View style={{ width: '60%' }}>
                <FlatList
                    horizontal
                    showsHorizontalScrollIndicator={false}
        ...
        <View>
            ...
            <View style={Styles.itemStyle}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={Styles.titleStyle}>{`${index + 1}. ` + item.modelName}</Text>
                    {
                        item.isAddProduct == 1 && item.upload !== 1 &&

                        <Icon name="minus-circle" type="font-awesome-5" size={25} onPress={onClickButton} color={appcolor.danger} style={{ marginEnd: 10 }} />
                    }
                </View>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }} >
                    {listInput.map((it, id) => {
                        return (
                            <RenderItemInput key={it.id + itemDisplay.displayCompetitorId} itemInput={it} indexInput={id} item={itemDisplay} listInput={listInput} Styles={Styles} appcolor={appcolor} data={data} workinfo={workinfo} />
                        )
                    })}
                    {/* <FlatList
                    data={listInput}
...
const RenderItemInput = ({ itemInput, indexInput, item, listInput, Styles, appcolor, data, workinfo }) => {
    ...
    return (
        <View style={[Styles.itemInputStyle, { width: ((indexInput > 2 && indexInput < 5) || (listInput.length % 2 === 0)) ? '49.3%' : '32.6%' }]}>
            ...
            <NumberFormat
                ...
                renderText={value =>
                    <TextInput
                        textAlign={'center'}
                        value={value}
                        style={[Styles.textInputStyle, { backgroundColor: (itemInput.displayType === 'fsmValue' && item.isFsmValueError === 1) || (itemInput.displayType === 'netValue' && item.isNetValue === 1) || (itemInput.displayType === 'priceValue' && item.isPriceError === 1) ? appcolor.warning : appcolor.light }]}
