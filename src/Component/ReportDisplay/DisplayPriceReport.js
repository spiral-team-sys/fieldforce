...
import { getDisplayResult, getCompetitorProductBy, getAllPhotosUpload, getPhotosReport } from '../../Controller/WorkController'
import { clearAllDataDisplay, getAllDisplayProduct, getDataHistoryDisplayByShop, getlistTabCompetitor } from '../../Controller/DisplayController';
import { checkNetwork, deviceWidth, deviceHeight } from "../../Core/Utility";
import { groupDataByKey, Message, MessageAction, ToastError } from '../../Core/Helper';
...
export const DisplayPriceReport = ({ navigation }) => {
    ...
    const [data, setData] = useState({ arrTabShow: [], dataTabByCompe: [], arrDataShow: [], arrDataShowF: [], lstCompetitor: [] })
    const [select, setSelect] = useState({ competitorSelect: _competitorName, competitorIdSelect: _competitorId })
    ...
    const styles = useMemo(() => {
        return StyleSheet.create({
            ...
            searchInput: { backgroundColor: appcolor.grayLight, marginHorizontal: 8, marginTop: 8, marginBottom: 6, alignSelf: 'center' },
            searchText: { fontSize: scaleSize(12), color: appcolor.dark },
            competitorSection: { width: '100%', height: 52, justifyContent: 'center', backgroundColor: appcolor.light, paddingBottom: 4 },
            competitorList: { width: '100%', height: 44 },
            competitorButton: { minWidth: deviceWidth / 5, paddingVertical: 7, paddingHorizontal: 10, backgroundColor: appcolor.surface, alignItems: 'center', borderRadius: 20, marginHorizontal: 4, marginVertical: 4 },
            ...
            sheetTitle: { color: appcolor.dark, fontSize: scaleSize(17), fontWeight: fontWeightBold, paddingTop: 4 },
            sheetSubTitle: { color: appcolor.greydark || appcolor.placeholderText, fontSize: scaleSize(11), paddingTop: 4, paddingBottom: 12 },
            ...
            sheetActionText: { color: appcolor.dark, flex: 1, paddingHorizontal: 12, fontSize: scaleSize(13), fontWeight: fontWeightBold },
            sheetActionDangerText: { color: appcolor.danger },
            loadingOverlay: { width: '100%', position: 'absolute', alignItems: 'center', alignSelf: 'center', marginTop: deviceHeight / 2 }
        })
    }, [appcolor])
    const loadDataShow = async () => {
        await setProgress(true)
        let lstRes = await getDisplayResult(workinfo);
        let isUpload = ((lstReport?.isMultiSendData == 1) ? 0 : (lstRes.length > 0 ? lstRes[0].upload : 0))
        ...
        if (lstReport.isProductByShop == 1) {
            ...
            await REPORT.GetDataReportByShop_RealTime(params, async (mData) => {
                const detailProduct = mData?.length > 0 ? (mData[0]?.details || null) : null
                if (detailProduct) {
        ...
        let competitors = await getCompetitorProductBy(_competitorId, lstReport?.isByType || 0);//list competitor
        ...
        setTimeout(async () => { await setProgress(false) }, 500)
    }
    ...
    const filterProduct = async (text) => {
        if (text.length > 0) {
            let filterList = data.arrDataShowF.filter(i => (i.productName.toLowerCase().match(text.toLowerCase()) || i.productCode.toLowerCase().match(text.toLowerCase())))
    ...
    const uploadAction = async () => {
                ...
                for (let index = 0; index < data.arrTabShow.length; index++) {
                    ...
                    for (let idx = 0; idx < lstTem?.length || 0; idx++) {
                        ...
                        if (lstPhoto.length < it.numberIMG) {
                            ToastError(itT.name || itT.categoryName + ': Vui lòng chụp ' + it.numberIMG + ' tấm hình cho ' + it.nameVN + ', ');
                            return
                        }
                    }
                }
        ...
        if (resDisplay.length == 0) {
            ...
            for (let index = 0; index < resDisplay.length; index++) {
                ...
                if (item.price !== null && item.price !== 'null' && (item.price < ((lstReport?.minPrice && lstReport?.minPrice !== '') ? lstReport?.minPrice : 1000)
                    || item.price % ((lstReport?.minPrice && lstReport?.minPrice !== '') ? lstReport?.minPrice : 1000) > 0)) {
                    ToastError(`Giá sản phẩm ${item.productName} - ${item.categoryName} - ${item.division} sai định dạng`, 'Giá sản phẩm', 'top')
                    return
                }
                if ((item.price === 'null' || item.price === null) && (item.fsmValue !== 'null' && item.fsmValue !== null && item.fsmValue > 0)) {
                    ToastError(`Chưa nhập giá ${item.productName} - ${item.categoryName} - ${item.division}`, 'Giá', 'top')
                    return
                }
                if ((item.quanity === 'null' || item.quanity === null) && (item.price !== 'null' && item.price !== null && item.price > 0)) {
                    ToastError(`Chưa nhập số lượng ${item.productName} - ${item.categoryName} - ${item.division}`, 'Số lượng', 'top')
                    return
                }
                if ((item.quanity === 'null' || item.quanity === null) && (item.quantityStock !== 'null' && item.quantityStock !== null && item.price >= 0)) {
                    ToastError(`Chưa nhập số lượng ${item.productName} - ${item.categoryName} - ${item.division}`, 'Số lượng', 'top')
                    return
                }
                if (lstReport?.isConstraintPrice == 1 || lstReport?.isConstraintPrice == 2) {
                    if ((item.quanity !== 'null' && item.quanity !== null && item.quanity > 0) && (item.price === 'null' || item.price === null || item.price === '')) {
                        ToastError(`Đã nhập số lượng, chưa nhập giá. ${item.productName} - ${item.categoryName} - ${item.division}`, 'Giá', 'top')
                        return;
                    }
                }
                if (lstReport?.isUseStock == 2) {
                    if ((item.quanity !== 'null' && item.quanity !== null && item.quanity > 0) && (item.quantityStock === 'null' || item.quantityStock === null || item.quantityStock === '')) {
                        ToastError(`Đã nhập số lượng, chưa nhập tồn kho. ${item.productName} - ${item.categoryName} - ${item.division}`, 'Giá', 'top')
                        return;
                    }
                }
            }
        ...
        if (Object.keys(lstReport || {}).length > 0) {
            try {
                ...
                if (lstReport?.numCheckProduct !== undefined || lstReport?.isCheckByProduct == 1) {
                ...
                if (numCheckProduct) {
                    ...
                    let satisfyConditions = _.filter(data.arrDataShowF, item => {
                        const checkHistory = dataHistory.filter(it => it.productId == item.productId && it.hDisplay >= numCheckProduct)
                        if (item.quanity >= (numCheckProduct || 1) && (checkHistory || [])?.length == 0)
                            return true
                        else return false
                    });
                    if (satisfyConditions.length > 0) {
                        Message('Chú ý', `Sản phẩm trưng bày mới nhập có số lượng >= ${numCheckProduct}, bạn có chắc chắn muốn gửi không?`, () => UploadData(data.arrDataShowF, resPhotos));
                    } else {
                        Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', () => UploadData(data.arrDataShowF, resPhotos));
                    }
    ...
    const clearDataByCate = async (currentItem) => {
        data.arrDataShowF.map(it => {
            if (it.categoryId == currentItem.categoryId && it.divisionId == currentItem.divisionId) {
                it.quanity = null
                it.price = null
                it.displayComment = null
                it.productComment = null
    ...
    const UploadData = async (resDisplay, resPhotos) => {
        ...
        resDisplay.map(it => resDisplayFilter.push({
    ...
    const scrollOnPress = useCallback(async (item, index) => {
        await setProgress(true)
        data.dataTabByCompe = data.arrTabShow?.filter(it => it.divisionId === item.id)
    ...
    const RenderItemCompetitor = useCallback(({ item, index }) => {
        const onPress = () => {
            scrollOnPress(item, index)
        }
        const colorTitle = select.competitorIdSelect === item.id ? appcolor.primary : appcolor.dark
        const fontWeightTitle = select.competitorIdSelect === item.id ? '800' : 'normal'
        return (
            <TouchableOpacity
                key={`DD_${index}`} onPress={onPress}
                style={styles.competitorButton}>
                <Text numberOfLines={1} style={[styles.competitorTitle, { color: colorTitle, fontWeight: fontWeightTitle }]}>{item.name}</Text>
            </TouchableOpacity>
        )
    }, [appcolor, scrollOnPress, select.competitorIdSelect, styles.competitorButton, styles.competitorTitle])
    ...
    const handlerClearByCategory = () => {
        if (Status !== 1) {
            ...
            MessageAction(`Bạn có muốn xoá dữ liệu ngành hàng ${currentItem.categoryName} hãng ${currentItem.division || _competitorName} đã nhập không ?`, async () => {
    ...
    const currentCategoryName = currentTab.objTab?.index !== undefined
        ...
        <View style={styles.mainContainer}>
            ...
            <View style={[appStyles.contentContainer, styles.contentContainer]}>
                ...
                }
                {
                    ...
                    <InputDisplayReport
                        navigation={navigation} appcolor={appcolor} workinfo={workinfo}
                        kpiinfo={kpiinfo} data={data} select={select} Status={Status}
                        isClear={isClear} showProgress={showProgress} reload={reload}
            ...
            <ActionSheet
                ...
                containerStyle={{ padding: 10, backgroundColor: appcolor.light }}
                closeOnPressBack={true}
                gestureEnabled={true}
                indicatorColor={appcolor.primary} >
                <View style={styles.sheetContainer}>
                    <Text style={styles.sheetTitle}>Công cụ</Text>
                    <Text style={styles.sheetSubTitle}>Quản lý nhanh dữ liệu trưng bày & giá trong báo cáo này</Text>
                    ...
                    <TouchableOpacity style={[styles.sheetAction, styles.sheetActionDanger]} onPress={handlerClearByCategory}>
                        <View style={[styles.sheetIconView, styles.sheetIconDanger]}>
                            <Icon name={'trash'} type={'ionicon'} size={22} color={appcolor.danger} />
                        </View>
                        <Text numberOfLines={2} style={[styles.sheetActionText, styles.sheetActionDangerText]}>{`Xóa dữ liệu ${currentCategoryName} đã nhập`}</Text>
                    ...
                    <TouchableOpacity style={[styles.sheetAction, styles.sheetActionDanger]} onPress={setClearAll}>
                        ...
                        <Text style={[styles.sheetActionText, styles.sheetActionDangerText]}>Xóa dữ liệu đã nhập</Text>
                        <Icon name='chevron-forward' type='ionicon' size={18} color={appcolor.danger} />
                    </TouchableOpacity>
                </View>
