...
import { ScrollView, Image, View, Text, ImageBackground, SectionList, TextInput, Dimensions, Switch } from "react-native";
import { Icon, Button } from '@rneui/themed';
import PageHeader from '../../Content/PageHeader';
import { appcolor } from '../../Themes/AppColor';
import { KeyboardAwareScrollView } from 'react-native-keyboard-aware-scroll-view'
import { getItemsProgramDisplay, getDisplayProgramResult, insertDisplayResult, updateDisplayResult } from '../../Controller/WorkController'
import { uploadAuditDisplay, getPhotosUploaded, updateAuditDisplayResult, getCategoryAudit, getSubCategoriesAudit } from '../../Controller/WorkController'
...
export default class DisplayProgram extends Component {
    ...
    DisplayItemMap(lstItemsProgram, resDisplay) {
        ...
        lstItemsProgram.map((item, i) => {
            ...
            if (subItems.indexOf(item.code) < 0) {
                ...
                itemMap[item.refName].push({ SubCatId: item.refId, SubCatName: item.code, category: item.refName })
            }

            let itemInput = { itemName: item.itemName, itemId: item.id, refName: item.refName, upload: 0 }
            ...
            if (isnew == 1 || lstItemsProgram.length - 1 === i) {
                ...
                refName != '' && MapArr.push({ title: { name: refName, comment: itemsHave.length > 0 ? itemsHave[0].displayComment : '' }, data: items })
            }
        })
    ...
    UploadData = async (resDisplay, resPhotos) => {
        let res = uploadAuditDisplay(this.props.route.params.DisplayItem.id, resDisplay, resPhotos, this.state.workinfo);
        if (res) {
            let isUpdate = await updateAuditDisplayResult(this.state.workinfo, this.props.route.params.DisplayItem.id);
    ...
    changeValue = (text, item) => {
        this.setState({
            lstShow: this.state.lstShow.map(itemShow => ((itemShow.title.name === item.refName) ? {
                ...itemShow,
                data: itemShow.data.map(itemS => (itemS.id === item.itemId) ? { ...itemS, quanity: (text != '' ? parseInt(text) : '') } : itemS)
            } : itemShow))
        });
    ...
    takePhoto = (itemDisplay) => {
        let item = {
            ...
            "photoType": '' + this.props.route.params.DisplayItem.name + '_' + itemDisplay.category + '_' + itemDisplay.name,
    ...
    renderTypeRowSwitch(item) {
        switch (this.state.DisplayItem.id) {
            ...
            case 27:
            case 28:
                return <View style={{ width: '100%', padding: 5, justifyContent: 'space-between', flexDirection: 'row' }}>
            ...
            case 19:
                return <TextInput
                    ...
                    defaultValue={(typeof item.quanity === 'undefined') ? '' : item.quanity + ''}
    ...
    renderRow = ({ item }) => (
        <View>
            {
                !("SubCatId" in item) ?
                    <View style={{ height: 70, width: '100%', flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 35, paddingRight: 15 }}>
                            ...
                            />
                            {
                                this.state.Status === 0 ?
                                    <View style={{ flexDirection: 'row' }}><Icon
                                        ...
                                        size={25}
                                        activeOpacity={0.7}
                                        onPress={(e) => { this.showNoteForm(item) }}
                                    />
                                        <Icon
                                            type='font-awesome'
                                            style='baseline'
    ...
    render() {
        ...
        return (
            <ImageBackground style={{ height: '100%', width: '100%' }} source={require('./../Themes/Images/back_3.png')}>
                <PageHeader
                ...
                <View style={{ backgroundColor: 'white', width: '100%', height: Dimensions.get('window').height - 60 }}>
                    ...
                    <View style={{ position: 'absolute', marginTop: 75, width: '100%', height: '85%', zIndex: 0 }}>
                        {/* view note */}
                        <View style={{ zIndex: 1, display: this.state.isHiddenNote ? 'none' : 'flex', flex: 0, justifyContent: 'center', alignItems: 'center', width: '100%', height: Dimensions.get('window').height }}>
                            <View
                            ...
                            />
                            <View style={{ zIndex: 3, position: this.state.isHiddenNote ? 'absolute' : 'relative', width: '80%', height: 'auto', opacity: 1.0, backgroundColor: 'white', borderRadius: 15, flexDirection: 'column' }}>
