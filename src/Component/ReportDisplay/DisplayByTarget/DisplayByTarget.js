...
export const DisplayByTarget = ({ navigation }) => {
    const { appcolor, kpiinfo, workinfo } = useSelector(
    ...
    const ViewItemInput = () => {
        return (
            <InputDisplay reload={reload} upload={upload} workinfo={workinfo} />
        );
    };
    const ViewItemPhoto = () => {
        return (
    ...
    return (
        ...
        >
            <CheckDone navigation={navigation} appcolor={appcolor} _sheet={_sheet} upload={upload} kpiinfo={kpiinfo} workinfo={workinfo} reportItem={reportItem} loadData={loadData} />
...
const CheckDone = ({ navigation, appcolor, _sheet, upload, kpiinfo, workinfo, reportItem, loadData }) => {
    ...
    const onValidated = async (resultValid) => {
        ...
        await res.forEach((v, index) => {
            isDone === true && v.countInput < v.totalRow
                ? (isDone = false)
                : null;
            uiTask.push(
                <View key={index + "92KK"}>
    ...
    const contains = (item, query) => {
        ...
        let SCode =
            productCode === null ? productCode : productCode.toLowerCase();
        let SName =
            productName === null ? productName : productName.toLowerCase();
        if (
    ...
    return (
        <View >
            <HeaderCustom
                ...
                iconMiddle="search"
                middleFunc={
                    upload === true ? null : () => _sheet.current.show()
                }
                title={kpiinfo.name}
            />
            <ActionSheet
