...
const DisplayAndStock = ({ loading, dataDisplay }) => {
  ...
  const renderItem = ({ item, index }) => {
    return (
      <View
        key={`ID_${index}`}
        style={{ padding: 7, backgroundColor: appcolor.danger }}
      >
        ...
        <View key={'inputDisplay'} style={styles.inputView}>
          <RenderInputNumber
            ...
            appcolor={appcolor}
            itemValue={item.quanity}
            isUploaded={item.upload}
            handlerChangeText={value => {
              handlerChangeText(item, value, TYPE.quanity);
            }}
          />
          <RenderInputNumber
            viewStyle={{ ...styles.itemInputStyle }}
            ...
            placeholder="Tồn kho"
            appcolor={appcolor}
            itemValue={item.quantityStock}
            isUploaded={item.upload}
            handlerChangeText={value => {
              handlerChangeText(item, value, TYPE.quantityStock);
            }}
          />
          <RenderInputNumber
            viewStyle={{ ...styles.itemInputStyle }}
            ...
            placeholder="Đề xuất"
            appcolor={appcolor}
            itemValue={item.quantitySuggest}
            isUploaded={item.upload}
            handlerChangeText={value => {
              handlerChangeText(item, value, TYPE.quantitySuggest);
            }}
          />
          <RenderInputNumber
            viewStyle={{ ...styles.itemInputStyle }}
            ...
            isUploaded={item.upload}
            handlerChangeText={value => {
              handlerChangeText(item, value, TYPE.price);
            }}
          />
        </View>
      </View>
    ...
    <View style={styles.mainContainer}>
      ...
      <Tabs.Container
        ...
        renderTabBar={props => (
          <MaterialTabBar
            ...
            tabStyle={{
              minWidth: minWidthTab(dataDisplay.tabData),
              height: 36,
            }}
          />
        )}
      >
        {dataDisplay.tabData.map((item, index) => {
          var dataResult = dataDisplay.dataMain.filter(
            i => i.categoryId == item.tabId,
          );
          return (
            <Tabs.Tab
              key={`${index} (${dataResult.length})1`}
