...
import { getDisplayProduct, getListDisplayByModel, uploadDataDisplayByModel } from '../../../Controller/DisplayController'
...
export const DisplayByModel = ({ navigation, route }) => {
    ...
    const uploadAction = async () => {
        ...
        let checkPrice = resDisplay.filter(it => (it.quanity !== 'null' && it.quanity !== null) &&
            (it.price !== null && it.price !== 'null' && (it.price < 10000 || it.price % 1000 > 0)));
        // console.log(checkPrice, 'check price');
        if (checkPrice.length > 0) {
            let product = listProduct.filter(it => it.productId === checkPrice[0].productId)
        ...
        let items = resDisplay.filter(it => (it.quanity === 'null' || it.quanity === null) &&
            ((it.price !== 'null' && it.price !== null && it.price > 0)));
        // console.log(items, 'check item');
        if (items.length > 0) {
            let product = listProduct.filter(it => it.productId === items[0].productId)
        ...
        let checkPOSM1 = resDisplay.filter(it => (it.quanity === 'null' || it.quanity === null) &&
            ((it.displayValue !== 'null' && it.displayValue !== null && it.displayValue > 0)));
        // console.log(items, 'check item');
        if (checkPOSM1.length > 0) {
            let product = listProduct.filter(it => it.productId === checkPOSM1[0].productId)
            ToastError('Bạn chưa nhập số lượng trưng bày. Hãng: ' + product[0].division + '  - Loại : ' + product[0].categoryName + '  - sản phẩm: ' + product[0].productName, 'Thông báo', 'top');
            return;
        }

        let checkPOSM2 = resDisplay.filter(it => (it.displayValue === 'null' || it.displayValue === null || it.displayValue === 0) &&
            ((it.posmList !== 'null' && it.posmList !== null && it.posmList.length > 0)));
        // console.log(items, 'check item');
        if (checkPOSM2.length > 0) {
            let product = listProduct.filter(it => it.productId === checkPOSM2[0].productId)
            ToastError('Bạn chưa nhập số lượng POSM. Hãng: ' + product[0].division + '  - Loại : ' + product[0].categoryName + '  - sản phẩm: ' + product[0].productName, 'Thông báo', 'top');
            return;
        }

        let checkPOSMList = resDisplay.filter(it => (it.posmList === 'null' || it.posmList === null || it.posmList.length === 0) &&
            ((it.displayValue !== 'null' && it.displayValue !== null && it.displayValue > 0)));
        const outGuid = await POSMContext.PosmInGuid(checkPOSMList[0].productId)
        // console.log(items, 'check item');
        if (checkPOSMList.length > 0 && outGuid.length > 0) {
            let product = listProduct.filter(it => it.productId === checkPOSMList[0].productId)
            ToastError('Bạn chưa chọn loại nhãn POSM. Hãng: ' + product[0].division + '  - Loại : ' + product[0].categoryName + '  - sản phẩm: ' + product[0].productName, 'Thông báo', 'top');
            return;
        }

        let checkQuantity = resDisplay.filter(it => (it.quanity === 'null' || it.quanity === null) &&
            ((it.quantityStock !== 'null' && it.quantityStock !== null && it.price >= 0)));
        // console.log(items, 'check item');
        if (checkQuantity.length > 0) {
            let product = listProduct.filter(it => it.productId === checkQuantity[0].productId)
            ToastError('Bạn chưa nhập số lượng trưng bày. Hãng: ' + product[0].division + '  - Loại : ' + product[0].categoryName + '  - sản phẩm: ' + product[0].productName, 'Thông báo', 'top');
            return;
        }
    ...
    const UploadData = async (resDisplay) => {
        ...
        await uploadDataDisplayByModel(resDisplay, work, async () => {
            await loadData();
        }, async () => {
        })
    }
    ...
    const ViewItemInput = () => {
        return (
            <InputDisplayByModel Status={Status} navigation={navigation} listInput={listInput} reloadView={reloadView} />
        );
    };
    const ViewItemPhoto = () => {
        return (
