...
import { clearAllDataDisplay, clearAllDataPOSM, getCompetitorByModel, getListDisplayByModel, getlistTabCompetitor, updatePOPDisplayBymodel } from '../../../Controller/DisplayController'
...
export const InputDisplayByModel = ({ navigation, route, Status, listInput, reloadView }) => {
    const { appcolor, kpiinfo, workinfo, userinfo } = useSelector(state => state.GAppState)
    const [data, setData] = useState({ dataTab: [], dataShow: [], dataShowF: [], dataCompetitor: [], dataTabByCompe: [] })
    ...
    const loadData = async () => {
        ...
        const listTabByCompetitor = listTab.filter(it => it.divisionId === _competitorId)
        ...
        await setData({ dataTab: listTab, dataTabByCompe: listTabByCompetitor, dataShow: arr, dataShowF: arr, dataCompetitor: listCompetitor })
    ...
    const Styles = StyleSheet.create({
        ...
        inputNoteStyle: { flex: 1, padding: 10, color: appcolor.dark, height: 60, textAlign: 'left', borderWidth: 0.4, borderRadius: 10, borderColor: appcolor.dark, backgroundColor: appcolor.light },
        itemStyle: { backgroundColor: appcolor.light, padding: 5, margin: 5, elevation: 2, borderRadius: 10 },
    ...
    const setCompeSelect = async (item) => {
        await setSelect({ competitorSelect: item?.name, competitorIdSelect: item?.id })
    ...
    const scrollOnPress = async (item, index) => {
        ...
        setData({ ...data, dataTabByCompe: data.dataTab?.filter(it => it.divisionId === item.id) })
        setCompeSelect(item)
    ...
    const RenderItemCompetitor = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity
                ...
                style={{ minWidth: widthItem, padding: 8, backgroundColor: appcolor.surface, alignItems: 'center', borderRadius: 20, margin: 5 }}>
                <Text style={{ color: colorTitle, fontSize: 15, fontWeight: fontWeightTitle }}>{item.name}</Text>
            </TouchableOpacity>
        )
    }
    ...
    return (
        <View style={Styles.container}>
            ...
            </View>
            {
                ...
                <View style={{ justifyContent: 'center', alignSelf: 'center', width: '100%', paddingBottom: 8, backgroundColor: appcolor.light }}>
                    <View style={{ flexDirection: 'column', width: '100%', alignSelf: 'center', alignItems: 'center', justifyContent: 'center', backgroundColor: appcolor.light }}>
                        <FlatList
                            horizontal
                            ref={competitorRef}
                            style={{ width: '100%' }}
            ...
            <View style={Styles.container}>
                <KeyboardAvoidingView
                    ...
                    {
                        dataTab.length === data.dataTabByCompe.length && dataTab.length > 0 ?
                            <ViewItemTab data={data} Status={Status} dataTab={dataTab} appcolor={appcolor} Styles={Styles} listInput={listInput} workinfo={workinfo} tabRef={tabRef} select={select} />
                            :
                            <View style={{ flex: 1 }}><Text style={{ fontWeight: '600', fontSize: 16, textAlign: 'center', padding: 10 }}>Không có sản phẩm</Text></View>
                    }
                ...
                <View style={Styles.actionSheetStyle}>
                    <View style={{ width: '100%' }}>
                        ...
                        <TouchableOpacity
                            ...
                            <Text style={[Styles.textSheetStyle, { color: appcolor.dark }]} >Sản phẩm đã nhập</Text>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={setClearAll}>
                            <Text style={[Styles.textSheetStyle, { color: appcolor.danger }]} >Xóa dữ liệu đã nhập</Text>
...
const POSMUI = ({ props }) => {
    ...
    return (
        <View style={{ width: '100%', paddingLeft: 7 }}>
            <Text style={{ padding: 3, color: appcolor.dark, fontSize: scaleSize(12) }}>Các loại posm dán trên sản phẩm</Text>
            <FlatList
                horizontal
                showsHorizontalScrollIndicator={false}
                data={posmData}
                renderItem={({ item, index }) => {
                    return (
                        <View style={{
                            borderRadius: 40, marginEnd: 7,
                            backgroundColor: item.posmValue > 0 ? appcolor.surface : appcolor.warning
                        }} key={`${itemProduct.productId}ka02${index}`}>
                            <TouchableOpacity
                                onPress={() => onShowPosm(itemProduct)} style={{ padding: 12 }}>
                                <View style={{ flexDirection: 'row' }}>

                                    <Badge status="error" value={item.posmTarget} />

                                    <Text style={{
                                        marginStart: 3, fontSize: scaleSize(14),
...
const ViewItemTab = ({ data, dataTab, appcolor, Styles, listInput, workinfo, tabRef, select, Status }) => {
    ...
    const ViewItem = () => {
        return (
            dataTab.map(it => {
                let listDataByCate = []
                listDataByCate = data.dataShow.filter(item => item.categoryName === it.categoryName && item.divisionId === select.competitorIdSelect && item.categoryId === it.categoryId)
                    ...
                    <Tabs.Tab key={it.categoryName + `(${totalRow})`} label={it.categoryName + `(${totalRow})`} name={it.categoryName + `(${totalRow})`}>
                        <View style={Styles.viewTabStyle}>
                            <FlatList
                                ...
                                renderItem={({ item, index }) => <RenderItem item={item} index={index} data={data} Status={Status} Styles={Styles} listInput={listInput} appcolor={appcolor} workinfo={workinfo} />}
                            />
...
const RenderItem = ({ item, index, data, Styles, workinfo, listInput, appcolor, Status }) => {
    const [itemDisplay, _] = useState(item)
    const [posmByP, setPosmByP] = useState([]);
    const jsonPosm = JSON.parse(item?.posmList && item?.posmList !== 'null' ? item?.posmList : (Status !== 1 ? (`[{"posmId": 0,"posmName": "Thêm","productId":${item.productId}}]`) : `[]`))
    ...
    const onShowPosm = async (it) => {
        if (it.posmList === null || it.posmList === 'null') {// Ngoài guidline
    ...
    const handerChangePosm = async (item, e) => {
        ...
        const indexShowF = data.dataShowF.findIndex(it => it.productId === itemDisplay.productId && it.workId === itemDisplay.workId)
        const indexShow = data.dataShow.findIndex(it => it.productId === itemDisplay.productId && it.workId === itemDisplay.workId)
    ...
    return (
        ...
        <View>
            ...
            <View style={[Styles.itemStyle, { height: jsonPosm.length !== 0 ? 170 : 100 }]}>
                ...
                <View style={{}}>
                    <FlatList
                        ...
                        getItemLayout={getItemLayout}
                        removeClippedSubviews={true}
                        renderItem={({ item, index }) => <RenderItemInput itemInput={item} indexInput={index} item={itemDisplay} Styles={Styles} listInput={listInput} appcolor={appcolor} data={data} workinfo={workinfo} />}
                        style={{ padding: 5 }}
                        numColumns={4}
                        initialNumToRender={5}
                        ListFooterComponent={jsonPosm.length !== 0 ? <POSMUI key={item.productCode} props={{ "posmData": jsonPosm, "onShowPosm": onShowPosm, "itemProduct": item }} /> : <View />}
                        keyExtractor={keyExtractor}
                    />
                </View>
            </View>
            <ActionSheet ref={_sheetPosm}
                    ...
                    <FlatList
                        ...
                        renderItem={
                            ({ item, index }) => {
                                return (
                                    <Fragment>
                                        <View index={`${index}nnma`} style={{ padding: 7, flexDirection: 'row', alignItems: "center" }}>
                                            <Text style={{ width: '58%', color: appcolor.dark }}>{item.posmName}</Text>
                                            <NumPad
                                                containerStyle={{ width: '40%', }}
                                                index={index} value={item?.posmValue}
                                                item={item} editable={false} upload={itemDisplay.upload !== 1 ? false : true}
                ...
                <TouchableOpacity onPress={() => _sheetPosm.current.hide()} style={{ padding: 12, alignItems: 'center', marginBottom: 30 }}>
                    <Text> Đóng </Text>
                </TouchableOpacity>
            </ActionSheet>
        </View>
...
const RenderItemInput = ({ itemInput, indexInput, item, Styles, listInput, appcolor, data, workinfo }) => {
    const [input, setInput] = useState(
        itemInput.displayType == 'priceValue' ? (item.price || '') :
            itemInput.displayType == 'quantityStock' ? (item.quantityStock === 0 ? 0 : (item.quantityStock || '')) :
                (item.displayValue === 0 ? 0 : (item.displayValue || '')));
    ...
    const changeValue = async (text) => {
        ...
        } else if ((itemInput.displayType === 'quantityStock' || itemInput.displayType === 'displayValue') && intValue === 0) {
    ...
    const indexItem = listInput.length === 4 ? 3 : ((listInput.length > 4 || listInput.length === 3) ? 2 : 1)
    return (
        <View style={[Styles.itemInputStyle, { width: ((indexInput > 2 && indexInput < 5) || (listInput.length % 2 !== 0) || (itemInput.displayType === 'priceValue')) ? '33%' : '22%' }]}>
            ...
            <NumberFormat
                ...
                renderText={value =>
                    <TextInput
                        textAlign={'center'}
                        value={value}
                        style={[Styles.textInputStyle, { backgroundColor: (itemInput.displayType === 'fsmValue' && item.fsmValueError === 1) || (itemInput.displayType === 'priceValue' && item.priceError === 1) ? appcolor.warning : appcolor.light }]}
                        keyboardType='numeric'
                        placeholder={(itemInput.displayType === 'priceValue' ? 'Giá' : 'Số lượng')}
