...
import { Platform, View, Text, ImageBackground, SectionList, TextInput, Dimensions, Switch, KeyboardAvoidingView } from "react-native";
import { Icon, Button } from '@rneui/themed';
import PageHeader from '../../Content/PageHeader';
import { getItemsProgramDisplay, getDisplayProgramResult, insertDisplayHPResult, updateDisplayHPResult } from '../../Controller/WorkController'
import { getPhotosUploaded, updateAuditDisplayResult, getCategoryAudit, getSubCategoriesAudit } from '../../Controller/WorkController'
...
export default class DisplayProgram extends Component {
    ...
    DisplayItemMap(lstItemsProgram, resDisplay) {
        ...
        lstItemsProgram.map((item, i) => {
            ...
            if (subItems.indexOf(item.code) < 0) {
                let itemsHaveComment = resDisplay.filter(itemRes => itemRes.displaySubCat === item.code)
                subItems.push(item.code);
                itemMap[item.refName].push({ SubCatId: item.refId, SubCatName: item.code, category: item.refName, displayComment: itemsHaveComment.length > 0 ? itemsHaveComment[0].displayComment : '' })
            }

            let itemInput = { itemName: item.itemName, itemId: item.id, refName: item.refName, subCat: item.code, displayComment: '', upload: 0 }
    ...
    loadCategory = async () => {
        let Lstcategory = [];
        let categoryTem = await getCategoryAudit(this.props.route.params.DisplayItem.id);
    ...
    uploadAction = async () => {
        ...
        let resDisplay = await getDisplayProgramResult(this.state.workinfo.workId, this.props.route.params.DisplayItem.id);
        let resPhotos = await getPhotosUploaded(this.props.route.params.workinfo.reportId, this.state.workinfo.shopId, this.state.workinfo.workDate, '' + this.props.route.params.DisplayItem.name);
    ...
    UploadData = async (resDisplay, resPhotos) => {
        ...
        try {
            ...
            let UploadJson = { DisplayId: this.props.route.params.DisplayItem.id, ShopId: this.state.workinfo.shopId, WorkDate: Moment(new Date).format('YYYY-MM-DD'), Details: JSON.stringify(items), Photos: JSON.stringify(itemsPhoto) }
            ...
            await fetch(URL_UPLOAD_AUDIT_DISPLAY, {
                ...
                .then(async responseJson => {
                    if (responseJson.status == 200) {
                        ...
                        updateAuditDisplayResult(this.state.workinfo, this.props.route.params.DisplayItem.id);
                        this.refreshData();
    ...
    loadDataShow = async () => {
        let lstItemsProgram = await getItemsProgramDisplay(this.props.route.params.DisplayItem.id, this.state.categorySelect, this.state.subCategorySelect);
    ...
    render() {
        ...
        return (
            <View style={{ height: '100%', width: '100%' }}>
                ...
                <View style={{ backgroundColor: 'white', width: '100%', height: Dimensions.get('window').height - 60 }}>
                    {
                        this.state.isHiddenNote && <View style={{ flexDirection: 'row', padding: 8, zIndex: 1 }}>
                            <View style={{ marginTop: 15 }}>
                                <Text style={{ fontSize: 13 }}>{''}{this.state.LblCategory}{' '}</Text>
                            </View>
                            <View style={{ alignItems: "flex-start", width: '25%' }}>
                                <View style={{ alignContent: 'center', backgroundColor: '#FFFFFF50' }}>
                                    {
                                        (this.state.categorySelect !== null && this.state.Categories.length > 0) &&
                                        this.ComboboxCustom(this.state.LblCategory, this.state.Categories, 0)
                                    }
                                </View>
                            </View>
                            <View style={{ marginTop: 15 }}>
                                <Text style={{ fontSize: 13 }}>{'  '}{
                                    (this.state.isShowSubCat === true && this.state.subCategorySelect !== null && this.state.SubCategories.length > 0) &&
                            ...
                            <View style={{ alignItems: "flex-start", width: '25%' }}>
                                <View style={{ alignContent: 'center', backgroundColor: '#FFFFFF50' }}>
                                    {
                                        (this.state.isShowSubCat === true && this.state.subCategorySelect !== null && this.state.SubCategories.length > 0) &&
                                        this.ComboboxCustom(this.state.LblSubCategory, this.state.SubCategories, 0)
                                    }
                                </View>
                            </View>
                        </View>
                    }
                    <View style={{ position: 'absolute', marginTop: this.state.isHiddenNote ? 75 : 0, width: '100%', height: this.state.isHiddenNote ? '85%' : '100%', zIndex: 0 }}>
                        {/* view note */}
                        <View style={{ zIndex: 1, display: this.state.isHiddenNote ? 'none' : 'flex', flex: 0, width: '100%', height: Dimensions.get('window').height }}>
                            <View
                            ...
                            <View style={{ zIndex: 3, position: this.state.isHiddenNote ? 'absolute' : 'relative', width: '80%', height: 'auto', opacity: 1.0, backgroundColor: 'white', borderRadius: 15, flexDirection: 'column', marginTop: 70, marginLeft: Dimensions.get("window").width / 2 - (40 * Dimensions.get("window").width) / 100 }}>
                                ...
                                <View style={{ marginTop: 15, flexDirection: 'row', justifyContent: 'space-between', height: 50, width: '100%', padding: 5 }}>
                                    <Button
                                        buttonStyle={{ width: '90%', backgroundColor: 'red', borderRadius: 15 }}
                                        title='Huỷ'
                                        onPress={(e) => this.cancelNote()}
                                    />
                                    <Button
                                        buttonStyle={{ width: '90%', backgroundColor: 'green', borderRadius: 15 }}
                                        title='Lưu'
                        ...
                        <View style={{ zIndex: 0, width: '100%', position: this.state.isHiddenNote ? 'relative' : 'absolute', backgroundColor: 'white', height: '100%' }}>
                            <KeyboardAvoidingView
                                ...
                                <SectionList
                                    ...
                                    renderItem={({ item, index }) => (<RenderRow item={item} index={index} Status={this.state.Status} refreshView={() => this.refreshView()} workinfo={this.state.workinfo} DisplayItem={this.state.DisplayItem} Props={this.props} UpdateKeyNote={this.showNoteForm} />)}
...
const RenderRow = ({ item, index, Status, refreshView, workinfo, DisplayItem, Props, UpdateKeyNote }) => {
    return (
        <View>
            {
                !("SubCatId" in item) ?
                    <View style={{ height: 70, width: '100%', flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 5 }}>
                        ...
                        <View style={{ width: '50%', alignSelf: 'center' }}>
                            {
                                RenderTypeRowSwitch(item, Status, refreshView, workinfo, DisplayItem)
                            }
                        </View>
                    </View> :
...
const RenderTypeRowSwitch = (item, Status, refreshView, workinfo, DisplayItem) => {

    if (DisplayItem.fieldSetting !== null) {
        switch (DisplayItem.fieldSetting) {
            case "R":
                return <View style={{ width: '100%', justifyContent: 'space-between', flexDirection: 'row' }}>
            ...
            case "T":
                const [todoText, setTodoText] = useState((typeof item.quanity === 'undefined') ? '' : item.quanity + '');
                ...
                return <View>
                    <TextInput
                        ...
                        style={{ fontSize: 12, color: 'black', minHeight: 37, textAlign: 'center', borderWidth: 0.6, borderColor: 'gray', width: '98%' }}
