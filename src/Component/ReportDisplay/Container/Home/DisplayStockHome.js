...
import { Message, MessageAction, ToastError, UUIDGenerator, debounce, formatNumber, groupDataByKey, removeVietnameseTones } from "../../../../Core/Helper";
import _ from 'lodash'
import { Text } from '@rneui/themed';
import { TODAY, deviceHeight, deviceWidth, minWidthTab } from "../../../../Core/Utility";
...
export const DisplayStockHome = ({ navigation }) => {
	...
	const LoadData = async (group) => {
		...
		await REPORT.GetDataReportByShop(dataFilter, async (mData, message) => {
			...
			const dataGroup = _.filter(mData, (e) => { return e.ItemGroup == _itemgroup })
			if (message !== null)
				ToastError(message)
	...
	const checkInput = async () => {
		...
		for (let index = 0; index < competitorData.length; index++) {
			...
			const dataValidate = _.filter(dataMain, (e) => {
				return e.ItemGroup == item.ItemGroup && ((e.Display !== null && e?.Display?.length > 0) || (e.QuanityStock !== null && e?.QuanityStock?.length > 0))
			})
			if (dataValidate == null || dataValidate.length == 0) {
				ToastError(`Hoàn thành báo cáo ${item.ItemGroup} trước khi gửi dữ liệu lên hệ thống`, 'Dữ liệu báo cáo', 'top')
				return
			}
		}
		// Check Photo      
			...
			for (let index = 0; index < dataMain.length; index++) {
				...
				for (let j = 0; j < configPhoto.length; j++) {
					...
					if (actual < jItem.numberIMG) {
						ToastError(`Chưa chụp đầy đủ hình ${item.GroupName} của ${item.ItemGroup} (${actual}/${jItem.numberIMG} tấm)`, 'Hình ảnh', 'top')
						return
					}
				}
			}
	...
	const onSearchProduct = debounce((text) => {
		const filterProducts = _.filter(dataMain, (e) => {
			return e.ItemGroup == itemGroup && (
				removeVietnameseTones(e.ProductName.toLowerCase()).match(removeVietnameseTones(text?.toLowerCase())) ||
				removeVietnameseTones(e.ProductCode.toLowerCase()).match(removeVietnameseTones(text?.toLowerCase()))
			)
		})
	...
	const onChangeItem = debounce((item) => {
		...
		const filter = _.filter(dataMain, (e) => {
			return e.ItemGroup == item.ItemGroup &&
				(isShowInput ? ((e.Display !== null && e.Display >= 0) || (e.QuanityStock !== null && e.QuanityStock >= 0)) : e.ItemGroup == item.ItemGroup)
		})
	...
	const handlerClearAll = () => {
		MessageAction('Bạn có muốn xoá tất cả dữ liệu đã nhập không ?', () => {
			const dataUpdate = dataMain.map((item, _index) => ({ ...item, Display: null, QuanityStock: null }))
	...
	const handlerClearByCategory = (itemCategory) => {
		MessageAction(`Bạn có muốn xoá dữ liệu ngành hàng ${itemCategory.CategoryName} đã nhập không ?`, () => {
			const dataUpdate = dataMain.map((item, _index) =>
				(item.CategoryName == itemCategory.CategoryName) ? { ...item, Display: null, QuanityStock: null } : item
			)
			saveJsonData(shopinfo.shopId, kpiinfo.id, TODAY, dataUpdate)
			setShowInput(false)
			SheetManager.hide('actionDisplay')
	...
	const handlerShowItemInput = (show) => {
		...
		if (show) {
			const dataInput = _.filter(dataMain, (i) => {
				return i.ItemGroup == itemGroup && ((i.Display !== null && i.Display >= 0) || (i.QuanityStock !== null && i.QuanityStock >= 0))
			})
			setData(dataInput)
		} else {
			const dataInput = _.filter(dataMain, (i) => { return i.ItemGroup == itemGroup })
			setData(dataInput)
		}
	}
	// View Action Sheet
	...
	const styles = StyleSheet.create({
		...
		itemMain: { width: deviceWidth, flexDirection: 'row', borderBottomColor: appcolor.grayLight, borderBottomWidth: 0.5 },
		titleView: { fontSize: 14, fontWeight: '500', color: appcolor.dark },
		subTitleView: { fontSize: 12, fontWeight: '400', color: appcolor.greylight },
		contentInput: { position: 'absolute', end: 0, top: 8, bottom: 0, flexDirection: 'row', alignItems: 'center' },
		...
		titleHeader: { padding: 8, fontSize: 15, fontWeight: '700', color: appcolor.info, fontStyle: 'italic', marginTop: 8 },
		searchView: { margin: 8, padding: 5, borderRadius: 30 },
		inputStyle: { fontSize: 13, color: appcolor.dark },
		warningData: { width: '100%', textAlign: 'center', padding: 8, color: appcolor.red, fontSize: 13, fontWeight: '500' }
	})
	const renderItem = (item, index, dataSize) => {
		...
		return (
			<View key={`aall_${index}`} >
				{item.isParent && <Text style={styles.titleHeader}>{`${item.SubGroupName}`}</Text>}
				<View style={styles.itemMain}>
					<View style={{ width: '60%', padding: 8 }}>
						<Text style={styles.titleView}>{`${index + 1}/${dataSize}. ${item.ProductName}`}</Text>
						<Text style={styles.subTitleView}>{`Mã SP: ${item.ProductCode}`}</Text>
					</View>
	...
	const renderContentProducts = () => {
		return (
			tabData !== null && tabData.length > 0 && tabData.map((item, index) => {
				const lstProducts = _.filter(data, (e) => { return e.GroupName == item.GroupName })
				return (
					<Tabs.Tab key={`category_${index}`} label={item.GroupName} name={item.GroupName} >
						<FlatList
							key='productlist'
							keyExtractor={(_item, index) => index.toString()}
							data={lstProducts}
							renderItem={({ item, index }) => renderItem(item, index, lstProducts.length)}
							showsVerticalScrollIndicator={false}
							style={{ paddingTop: 48 }}
							ListFooterComponent={<View style={{ paddingBottom: deviceHeight / 1.8 }} />}
							ListHeaderComponent={
								<HeaderItemView
					...
					<Tabs.Container
						...
						renderTabBar={props => (
							<MaterialTabBar
								...
								style={{ backgroundColor: appcolor.light, borderWidth: 0, marginStart: 8, marginEnd: 8 }}
							/>
						)}
						containerStyle={{ backgroundColor: appcolor.light }}
						headerContainerStyle={{ backgroundColor: appcolor.light, shadowColor: appcolor.transparent, paddingBottom: 8 }}>
