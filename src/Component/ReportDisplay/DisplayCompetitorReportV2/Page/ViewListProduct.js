...
export const ViewListProduct = ({ navigation, route, Status }) => {
  ...
  const renderItem = ({ item, index }) => {
    ...
    const keyLayer3 =
      item[`${item.divisionId}${item.categoryId}${item.subCatId}`];
    return (
      // <ViewItemProduct
      //     item={item}
      //     index={index}
      ...
      <View
        key={`it_${item.divisionId}_${index}_${item.modelName}`}
      ...
      >
        {keyLayer2 && (
          ...
          >
            <Text
              style={{ color: appcolor.white, fontSize: 16, fontWeight: '700' }}
            >
              {item.categoryName}
            </Text>
          </View>
        ...
        >
          <View
            style={{
              flexDirection: 'row',
              flexWrap: 'wrap',
              width: deviceWidth - (12 * 2 + 50),
            }}
          >
            {listInput.map((it, idx) => {
              return (
                <ViewItemInput
                  key={`${item.displayCompetitorId}_${index}_${it.id}`}
                  itemInput={it}
                  indexInput={idx}
                  dataItem={item}
                  indexItem={index}
          ...
          <TouchableOpacity
            ...
            key={`${item.displayCompetitorId}_${index}`}
            style={{
              position: 'absolute',
              right: -4,
              height: 50,
  ...
  const filterDoneProduct = async isDone => {
    if (!isDone) {
      let lstRes = data.dataShowF.filter(
        it =>
                    ...
                    {!loading && (
                      <FlashList
                        ...
                        keyExtractor={(it, _index) =>
                          it.displayCompetitorId.toString()
                        }
                        data={_dataProduct}
