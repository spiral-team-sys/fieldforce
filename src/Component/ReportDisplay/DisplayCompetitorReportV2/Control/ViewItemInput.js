...
export const ViewItemInput = forwardRef((props, ref) => {
  ...
  const styles = StyleSheet.create({
    ...
    inputStyle: {
      ...
      fontWeight: fontWeightBold,
      color:
        dataItem[itemInput.displayType] == 0 ||
        (dataItem[itemInput.displayType] || -1) >= 0
          ? appcolor.dark
          : appcolor.placeholderText,
    },
    itemMain: {
      width: (deviceWidth - (12 * 2 + 50)) / 2,
      flexDirection: 'row',
      padding: 2,
      paddingLeft: 8,
    },
  ...
  return (
    <View
      key={`${dataItem.displayCompetitorId}_${indexInput}_${itemInput.id}`}
    ...
    >
      ...
      >
        <Text style={styles.inputStyle}>
          {(dataItem[itemInput.displayType] == 0 ||
            dataItem[itemInput.displayType] ||
            -1) > 0
            ? `${
                dataItem[itemInput.displayType] == 0
                  ? 0
                  : toCurrency(dataItem[itemInput.displayType])
              }`
