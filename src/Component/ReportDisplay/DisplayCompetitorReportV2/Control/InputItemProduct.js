...
export const InputItemProduct = forwardRef((props, ref) => {
    ...
    const handlerSelectNum = (number) => {
        try {
            ...
            if (itemEdit !== null && itemName !== undefined && itemName !== null) {
                let currentStr = itemEdit[itemName] ? JSON.stringify(itemEdit[itemName]) : null
                let newStr = null
                if (number == '_') {
                    ...
                    isError == 0 && (itemEdit[itemName] = (newStr == 0 ? 0 : parseInt(newStr)))
                }

                if (itemEdit?.displayCompetitorId && isError == 0) {
                    const indexF = data.dataShowF.findIndex(it => it.displayCompetitorId === itemEdit.displayCompetitorId)
                    const index = data.dataShow.findIndex(it => it.displayCompetitorId === itemEdit.displayCompetitorId)
    ...
    const RenderItemInput = ({ itemInput, indexInput, }) => {
        ...
        const value = (itemName != null && itemEdit !== null) ? (itemEdit[itemName] == 0 ? 0 : itemEdit[itemName] || null) : null
        return (
                    ...
                    <View style={{
                    ...
                    }}>
                        {
                            value !== null ?
                                ...
                                :
                                <Text style={{ color: inputSelect.itemSelect?.id == itemInput.id ? appcolor.white : appcolor.greydark, textAlign: "center", fontWeight: '600' }}>
            ...
            <ActionSheet
                ...
                <View style={{ width: deviceWidth, height: deviceWidth + 150, flexDirection: 'column', justifyContent: 'space-between' }}>
                    <View style={{ flexWrap: 'wrap', flexDirection: 'row', marginTop: 20, height: 150, justifyContent: 'center', alignItems: 'center' }}>
                        {
                            (Object.keys(inputItem?.item || {}).length > 0 && listInput?.length > 0) && listInput?.map((it, idx) => {
                                return (
                                    <RenderItemInput key={`${inputItem?.item?.displayCompetitorId}_${index}_${it.id}`} itemInput={it} indexInput={idx} />
                                )
                            })
                        }
                    </View>
                    <KeyboardInput onSelectNum={handlerSelectNum} disableKeyboard={inputSelect.itemSelect?.id == undefined && inputSelect.itemSelect?.id == null} />
                </View>
            </ActionSheet >
        </View>
    )
