...
export const AddNewListInput = forwardRef((props, ref) => {
  ...
  const handlerSelectNum = number => {
    ...
    if (itemName !== undefined && itemName !== null) {
      let currentStr = productItem[itemName]
        ? JSON.stringify(productItem[itemName])
        : null;
      let newStr = null;
      if (number == '_') {
    ...
    return (
      ...
      >
        ...
        >
          <NumberFormat
            value={
              productItem[itemInput.displayType] === 0
                ? 0
                : productItem[itemInput.displayType] || ''
            }
            displayType="text"
  ...
  return (
    <View style={{ width: '100%', height: '100%' }}>
      ...
      <KeyboardInput
        onSelectNum={number => handlerSelectNum(number)}
        disableKeyboard={
