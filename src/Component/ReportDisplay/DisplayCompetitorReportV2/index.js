...
import {
  getCompetitorReportResult,
  getCompetitorResult,
  getListCompetitorProductV2,
  insertListProduct,
  uploadDataCompetitorReport,
...
export const DisplayCompetitorReportV2 = ({ navigation, route }) => {
  ...
  const onValidated = async listUpload => {
    ...
    for (let indexC = 0; indexC < listUpload?.length; indexC++) {
      ...
      for (let indexL = 0; indexL < listInput?.length || 0; indexL++) {
        ...
        if (
          (itemC[itemL.displayType] == null ||
            itemC[itemL.displayType] == undefined ||
            (itemC[itemL.displayType] == 0 && itemL.isZero !== 1)) &&
          itemL.isRequired == 1
        ) {
          ToastError(
            `Chưa nhập ${itemL.name} sản phẩm ${itemC.modelName} - ${itemC.categoryName} - ${itemC.division}`,
            'Thông báo',
            'top',
          );
          return false;
        ...
        if (
          itemC[itemL.displayType] !== null &&
          itemC[itemL.displayType] !== undefined &&
          (itemC[itemL.displayType] !== 0 ||
            (itemC[itemL.displayType] == 0 && itemL.isZero !== 1)) &&
          ((itemC[itemL.displayType] <
          ...
          ToastError(
            `Sai định dạng ${itemL.name} sản phẩm ${itemC.modelName} - ${itemC.categoryName} - ${itemC.division}`,
            'sản phẩm',
            'top',
          );
          return false;
        ...
        if (
          (itemC.quantity === 'null' ||
            itemC.quantity === null ||
            itemC.quantity === 0) &&
          itemC.isAddProduct == 1
        ) {
          ToastError(
            'Số lượng sản đã nhập thêm phải lớn hơn 0. Hãng: ' +
              itemC.division +
              '  - Loại : ' +
              itemC.categoryName +
              '  - sản phẩm: ' +
  ...
  const uploadAction = async () => {
    ...
    let checkPrice = resDisplay.filter(
      it =>
        ...
        ((it.priceValue !== null &&
          it.priceValue !== 'null' &&
          (it.priceValue < 1000 || it.priceValue % 1000 > 0)) ||
          (it.fsmValue !== null &&
            it.fsmValue !== 'null' &&
            (it.fsmValue < 1000 || it.fsmValue % 1000 > 0)) ||
          (it.netValue !== null &&
            it.netValue !== 'null' &&
            (it.netValue < 1000 || it.netValue % 1000 > 0))),
    );
    // console.log(checkPrice, 'check price');
    if (checkPrice.length > 0) {
      let product = listProduct.filter(
        it => it.displayCompetitorId === checkPrice[0].displayCompetitorId,
      );
      ToastError(
    ...
    let checkFsmValue = resDisplay.filter(
      it =>
        (it.netValue === 'null' || it.netValue === null) &&
        it.fsmValue !== 'null' &&
        it.fsmValue !== null &&
        it.fsmValue > 0,
    );
    if (checkFsmValue.length > 0) {
      let product = listProduct.filter(
        it => it.displayCompetitorId === checkFsmValue[0].displayCompetitorId,
      );
      ToastError(
    ...
    let checkPriceValue = resDisplay.filter(
      it =>
        (it.netValue === 'null' || it.netValue === null) &&
        it.priceValue !== 'null' &&
        it.priceValue !== null &&
    ...
    if (checkPriceValue.length > 0) {
      let product = listProduct.filter(
        it => it.displayCompetitorId === checkPriceValue[0].displayCompetitorId,
      );
      ToastError(
        'Bạn đã nhập Niêm yết nhưng chưa nhập Giá . Hãng: ' +
          product[0].division +
          '  - Loại : ' +
          product[0].categoryName +
          '  - sản phẩm: ' +
    ...
    if (items.length > 0) {
      let product = listProduct.filter(
        it => it.displayCompetitorId === items[0].displayCompetitorId,
      );
      ToastError(
    ...
    let checkItemAdd = resDisplay.filter(
      it =>
        (it.quantity === 'null' || it.quantity === null || it.quantity === 0) &&
        it.isAddProduct === 1,
    );
    if (checkItemAdd.length > 0) {
      let product = listProduct.filter(
        it => it.displayCompetitorId === checkItemAdd[0].displayCompetitorId,
      );
      ToastError(
        'Số lượng sản phẩm đã thêm phải lớn hơn 0. Hãng: ' +
          product[0].division +
          '  - Loại : ' +
          product[0].categoryName +
          '  - sản phẩm: ' +
    ...
    let itemsUpload = resDisplay.filter(
      it => it.quantity !== 'null' && it.quantity !== null,
    );
  ...
  const UploadData = async resDisplay => {
    ...
    await uploadDataCompetitorReport(
      resDisplay,
      work,
      async () => {
        await loadData();
