...
export const DisplayCompetitorTF = ({ navigation, route }) => {
  ...
  const uploadAction = async () => {
    ...
    if (listReport) {
      try {
        if (listReport?.isConstraint !== undefined) {
          isConstraint = listReport?.isConstraint;
        }
        if (listReport?.image !== undefined) {
          numLimitPhoto = listReport?.image;
        }
      } catch (error) {
        console.log(error);
      }
    ...
    if (isConstraint !== undefined && isConstraint === 1) {
      if (numLimitPhoto !== undefined && numLimitPhoto === 0) {
        const LstMenuPhotos = listReport?.ImageByList || [];
    ...
    let checkPrice = resDisplay.filter(
      it =>
        ...
        ((it.priceValue !== null &&
          it.priceValue !== 'null' &&
          (it.priceValue < 10000 || it.priceValue % 1000 > 0)) ||
          (it.fsmValue !== null &&
            it.fsmValue !== 'null' &&
  ...
  const ViewItemInput = () => {
    return <InputReport Status={Status} navigation={navigation} />;
  };
  const ViewItemPhoto = () => {
    return (
      <PhotoItems
