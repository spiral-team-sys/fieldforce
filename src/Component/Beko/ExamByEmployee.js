...
const ExamByEmployee = ({ navigation, route }) => {
  ...
  const checkDataQuestion = async () => {
    ...
    const notSuccess = [];
    listQuest.forEach(q => {
      const _check = dataExam.filter(
        it => it.questionId == q.questionId && it.answer == true,
      );
      if (_check.length == 0) notSuccess.push(q);
    });
    ...
    const success = dataExam.filter(
      a => a.correct == true && (a.answer == null || a.answer == false),
    );
    if (success.length > 0) {
      ...
      if (dataExam[0].examCount == 1) {
        ...
        // Chi cần làm ko tính đúng sai
        await QueryStringSql(
          `UPDATE ${taskList.tableName} SET taskDone=1,taskAlter='Đã hoàn thành' WHERE reportId=${menuItem.id}`,
        );
        await ToastSuccess('Đã gửi bài kiểm tra');
        await navigation.pop();
      }
    } else {
      let dataUpload = await dataExam.map(i => ({
        ExamId: i.examId,
        ...
        >
          <CheckBox
            textStyle={{
              fontWeight: 'normal',
              color:
                formMode == FORMMODE.FAILD && item.correct == true
                  ? appcolor.white
                  : appcolor.dark,
              fontSize: 12,
              padding: 5,
