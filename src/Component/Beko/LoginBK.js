...
import { View, Text, Platform, TouchableOpacity, TextInput, SafeAreaView, ActivityIndicator, } from "react-native";
...
import { MessageInfo, defaultSetting, colorList, ToastError } from "../../Core/Helper";
...
const LoginBK = ({ onLoginCallBack }) => {
    ...
    const renderDot = () => {
        const listdot = []
        for (a = 0; a < 24; a++) {
            listdot.push(<View style={{
                transform: [{ rotateZ: "190deg" }],
                marginTop: -a + 10,
                borderRadius: 20, borderColor: appcolor.white,
                borderWidth: 8, width: 16 + a, height: 26 + a,
    ...
    return (
        <SafeAreaView style={{ width: '100%', height: '100%', backgroundColor: appcolor.info }} >
            {requestPass ?
                <ForgotPassword
                ...
                />
                :
                <View style={{ width: '100%', height: '100%', backgroundColor: appcolor.primary }}>
                    {/* <View style={{ flexDirection: 'row' }}>
                        {renderDot()}
                    </View> */}
                    <View style={{ justifyContent: 'center', backgroundColor: appcolor.white, paddingTop: deviceWidth / 8 }} >
                        <Image
                            style={{ minHeight: 120 }}
                            source={require('../../Themes/Images/logo_spiral.png')}
                            resizeMode='contain' />
                        <View style={{ height: 50, backgroundColor: appcolor.primary, borderTopLeftRadius: 120, }} />
                    </View>
                    <Text style={{ fontSize: 28, fontWeight: '500', color: appcolor.white, paddingLeft: 12, marginTop: -20 }}>Welcome back,</Text>
                    <View style={{ opacity: 0.8, padding: 12 }}>
                        <View style={{
                            borderRadius: 30, borderColor: appcolor.primary, marginBottom: 20,
                            borderWidth: 0.8, backgroundColor: appcolor.light, opacity: 0.9
                        }}>
                            <TextInput
                                style={{ fontSize: 13, padding: 10, color: appcolor.dark }} autoCorrect={false}
                                returnKeyType='next'
                        ...
                        <View style={{
                            ...
                            />
                            <TouchableOpacity onPress={() => setShowPass(e => !e)}
                                style={{ zIndex: 121, position: 'absolute', right: 20, top: 16 }}>
                                <Icon
                    ...
                    <View style={{ alignItems: 'flex-end' }}>
                        <TouchableOpacity style={{ paddingLeft: 30, paddingRight: 30, opacity: 0.8, borderRadius: 30, padding: 12, backgroundColor: appcolor.info, }} onPress={handlerLogin}>
                            <Text style={{ textAlign: 'center', color: appcolor.white }}>Sign In</Text>
                        </TouchableOpacity>
                    </View>
                    {showProgress && <ActivityIndicator />}
                    <TouchableOpacity style={{ alignSelf: 'center' }} onPress={handlerFogotPassword} >
                        <Text style={{
                            marginTop: 20,
                            padding: 10, fontSize: 12, color: appcolor.white,
                            fontStyle: 'italic', textDecorationLine: 'underline'
                        }}>Bạn quên mật khẩu?</Text>
                    </TouchableOpacity>
                    <View style={{ flexGrow: 1 }} />
                    <View style={{ width: '100%', bottom: 8 }}>
                        <Text style={{ padding: 3, textAlign: 'center', color: appcolor.light, fontSize: 11 }}>{'Version: ' + versionApp}</Text>
                        <Text style={{ padding: 3, textAlign: 'center', fontWeight: '900', fontSize: 11, color: appcolor.light }}>Spiral Co.,Ltd</Text>
                        <Text style={{ padding: 3, fontSize: 10, textAlign: 'center', color: appcolor.light }}>27B Nguyễn Đình Chiểu, Phường Sài Gòn, Thành Phố Hồ Chí Minh</Text>
                    </View>
                </View>
            }
        </SafeAreaView >
