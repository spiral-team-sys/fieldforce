...
export const InstoreShareBK = ({ navigation, route }) => {
  ...
  const MapItemRes = async (lstTrack, i) => {
    ...
    lstFilter.map(it => {
      ...
      if (lstRes !== undefined) {
        let itemsHave = lstRes.filter(
          ir =>
            ir.categoryId === it.categoryId &&
            ir.competitorId === it.competitorId &&
            ir.subCatId === it.subCatId &&
            ir.itemId === it.id,
        );
        if (itemsHave.length > 0) {
          lstMap.push({
            ...it,
            display: itemsHave[0].display,
            upload: isUpload,
          });
        } else {
    ...
    <View style={{ flex: 1, backgroundColor: appcolor.homebackground }}>
      <HeaderCustom
        ...
        rightFunc={() =>
          uploaded !== 1
            ? uploadTrackingDisplay()
            : MessageInfo('Dữ liệu đã được gửi.')
        }
      />
      {
        arrTagShow.length > 0 && (
          ...
          >
            {arrTagShow.map((it, index) => (
              <Tabs.Tab
                key={Capitalize(it.name)}
                label={Capitalize(it.name)}
...
const RenderItemData = ({
  ...
  const editInputChange = async item => {
    await insertTrackingBK(
      route.params.workinfo,
      item,
      inputText !== '' ? parseInt(inputText) : null,
    );
    ...
    >
      {'compId' in item ? (
        ...
        >
          ...
          >
            {item.competitorName + ' ' + item.subCategory}
          </Text>
          <TextInput
            style={{
              fontSize: 13,
