...
import { LayoutAnimation, RefreshControl, Text, UIManager, View } from "react-native";
import { useSelector } from "react-redux";
import { REPORT } from "../../API/ReportAPI";
import { MessageAction, MessageAction2, ToastError, ToastSuccess, groupDataByKey, removeVietnameseTones } from "../../Core/Helper";
...
import { alertConfirm, alertWarning, checkNetwork, minWidthTab } from "../../Core/Utility";
...
export const PGReportHome = ({ navigation, route }) => {
    ...
    const [data, setData] = useState({ dataMain: [], dataMainF: [], dataTab: [] })
    const [configReport, setConfigReport] = useState({})
    const [formSetting, setFormSetting] = useState({ isLocked: false, configReport: {}, typeAction: 'NEW' })
    const [isEditable, setEditable] = useState(false)
    const [_mutate, setMutate] = useState(false)
    const [open, setOpen] = useState(false);
    const [currentTab, setCurrentTab] = useState({ currentIndex: 0, currentItem: {}, currentTabName: '' })
    ...
    const loadData = async () => {
        ...
        await REPORT.GetDataReportByShop_RealTime(params, async (mData) => {
            ...
            await setFormSetting({ isLocked: isUploaded == 1, configReport: formConfig, typeAction: typeAction })
    ...
    const handleReloadData = async () => {
        if (isEditable) {
            MessageAction('Bạn chưa gửi dữ liệu lên, Bạn có muốn tiếp tục tải lại báo cáo không ? (Sau khi tải lại dữ liệu bạn đã chỉnh sửa sẽ mất đi)', async () => {
                await setOpen(false)
    ...
    const onBack = () => {
        if (isEditable) {
            MessageAction2('Chưa gửi dữ liệu đã chỉnh sửa lên hệ thống, bạn có muốn gửi dữ liệu lên trước khi thoát báo cáo?', async () => {
                uploadData('BACK')
            }, () => {
                navigation.goBack()
            });
    ...
    const uploadData = async (type) => {
        ...
        if (_valid) {
            alertConfirm('Gửi dự liệu lên hệ thống', `Bạn có muốn gửi dữ liệu đã nhập như bên dưới không ?`, async () => {
                const dataUpload = data.dataMainF.map(it => ({ ...it, typeAction: formSetting.typeAction }))
                const result = await REPORT.UploadDataRaw_Realtime([...dataUpload], shopinfo, kpiinfo.id)
    ...
    const validData = async () => {
        const listUpload = _.filter(data.dataMainF, (e) => e.inputValue || e.inputValue == 0)
        ...
        const { isConstraint, image, ImageByList, isUseTakePhoto } = formSetting.configReport
        // Check Photo
        if (isConstraint !== undefined && isConstraint === 1 && isUseTakePhoto == 1) {
            if (image && image > 0) {
                const resPhotos = await getAllPhotosUpload(kpiinfo.kpiId, shopinfo.shopId, shopinfo.auditDate);
                if (resPhotos.length < image) {
                    ToastError(`Vui lòng chụp ${image} tấm hình cho báo cáo.(${resPhotos.length}/${image})`);
                    return false
                }
            } else {
                ...
                for (let index = 0; index < LstMenuPhotos.length; index++) {
                    ...
                    let lstPhoto = await getPhotosReport(kpiinfo.kpiId, it.code, shopinfo.shopId, shopinfo.auditDate);
                    if (lstPhoto.length < it.numberIMG) {
                        ToastError(`Vui lòng chụp ${it.numberIMG} tấm hình cho ${it.nameVN}.(${lstPhoto.length}/${it.numberIMG})`);
                        return false
                    }
                }
            }
    ...
    const onTabChange = async (itemTab) => {
        try {
            ...
            const _itemNameTab = data.dataTab[itemTab.index][formSetting.configReport?.group1 || 'g1']
            //
            setCurrentTab({
                ...currentTab,
    ...
    const contains = (item, query) => {
        ...
        return removeVietnameseTones(Sg1)?.match(removeVietnameseTones(query)) ||
    ...
    const handleSearch = (text) => {
        ...
        const filteredData = _.filter(data.dataMainF, dataF => { return contains(dataF, formattedQuery) })
    ...
    const renderItem = ({ item, index }) => {
        return (
            <View style={{ width: '100%' }}>
                {item.isParent && formSetting?.configReport?.isViewTab !== 1 && item[formSetting.configReport?.group1 || 'g1'] && (
                    <View style={{ padding: 8, borderRadius: 8, backgroundColor: appcolor.primary, marginTop: 12 }}>
                        <Text style={{ color: appcolor.white, fontWeight: '800', fontSize: 16 }}>
                            {item[formSetting.configReport?.group1 || 'g1']}
                        </Text>
                    </View>
                )}
                <View style={{ padding: 8, backgroundColor: appcolor.surface, alignItems: "center", flexDirection: 'row', justifyContent: 'space-between', marginTop: 4, borderRadius: 8 }}>
                    <Text style={{ color: appcolor.dark, fontWeight: '800', fontSize: 16 }}>
                        {item[formSetting.configReport?.name1 || 'itemName']}
                    </Text>
                    <View style={{ width: '35%' }}>
                        <NumPad_V2
                            inputStyle={{
                                fontSize: scaleSize(12), color: appcolor.dark, fontWeight: '500',
                                backgroundColor: appcolor.light,
                                width: '100%', textAlign: 'center', borderRadius: 5, padding: 8
                            }}
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            ...
            <LoadingView isLoading={isLoading} title='Đang cập nhật dữ liệu' styles={{ position: 'absolute', top: deviceHeight / 2.5, width: '100%' }} />
            {
                ...
                <View style={{ flex: 1 }}>
                    ...
                    {formSetting?.configReport?.isViewTab == 1 && data.dataTab !== null && data?.dataTab?.length > 0 &&
                        <Tabs.Container
                        ...
                        >
                            {data.dataTab.map((item, index) => {
                                const _dataIssues = _.filter(data.dataMain, (e) => e[formSetting?.configReport?.group1 || 'g1'] == item[formSetting?.configReport?.group1 || 'g1'])
                                const titleHead = `${item[formSetting.configReport?.group1 || 'g1']}${_dataIssues.length > 0 ? ` (${_dataIssues.length})` : ''}`
                                return (
                    ...
                    } */}
                    {
                        <SpeedDial
                            style={{
                                elevation: 3, shadowColor: appcolor.black,
                                shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.6, shadowRadius: 4,
                            }}
                            isOpen={open}
