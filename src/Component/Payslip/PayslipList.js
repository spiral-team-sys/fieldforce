...
export const PayslipList = ({ navigation }) => {
  ...
  const SalaryItem = ({ item, index }) => {
    return (
      <TouchableOpacity
        onPress={() => navigation.navigate('paydetail', { item: item })}
        key={`${index}091`}
      ...
      >
        ...
        <View style={styles.textContainer}>
          ...
          <NumericFormat
            ...
            renderText={value => (
              <Text style={styles.details}>{`Tổng thu nhập: ${value}`}</Text>
            )}
          />
          <NumericFormat
            ...
            thousandSeparator={true}
            renderText={value => (
              <Text style={styles.details}>{`Trừ Bảo hiểm: ${value}`}</Text>
            )}
          />
          <NumericFormat
            value={item.incomeTaxCost || 0}
            displayType={'text'}
            thousandSeparator={true}
            renderText={value => (
              <Text style={styles.details}>{`Trừ thuế TNCN: ${value}`}</Text>
            )}
          />
          <NumericFormat
            ...
            renderText={value => (
              <Text
                style={[styles.header, { fontSize: 14 }]}
              >{`Lương thực nhận: ${value}`}</Text>
            )}
          />
          <Text
            style={[
    ...
    <View style={{ flex: 1, backgroundColor: appcolor.primary }}>
      ...
      <View style={{ height: deviceHeight, backgroundColor: appcolor.surface }}>
        {salaryData?.length > 0 ? (
          ...
          <TouchableOpacity
            onPress={() => navigation.navigate('o-payslip')}
            style={{
              flexDirection: 'row',
              width: '100%',
              alignItems: 'center',
