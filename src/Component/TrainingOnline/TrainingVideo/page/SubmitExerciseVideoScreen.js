...
import { View, Alert, ActivityIndicator, StyleSheet, ScrollView, DeviceEventEmitter } from 'react-native';
import { pick, types } from '@react-native-documents/picker';
import Video from 'react-native-video';
import { HeaderCustom } from '../../../../Content/HeaderCustom';
import { useSelector } from 'react-redux';
import { Button, Text, Card, Icon } from '@rneui/base';
import { MessageInfo, ToastError, ToastSuccess, UUIDGenerator } from '../../../../Core/Helper';
// import moment from 'moment';
import { TODAY, alertConfirm, alertWarning, getFilePathFromContentUri, type } from '../../../../Core/Utility';
...
const SubmitExerciseVideoScreen = ({ route, navigation }) => {
    ...
    const LoadData = async () => {
        ...
        if (dataParse.videoUri) {
            const newUlr = (dataParse.videoUri.includes('uploaded') ? URLDEFAULT + dataParse.videoUri : (dataParse.videoUri || null))
            oldData = {
    ...
    const pickVideo = async () => {
        ...
        try {
            ...
            const fileUri = await getFilePathFromContentUri(newUriCompress.compressedUri, `${_guid}.${fileExtension}`);
            if (!fileUri) {
    ...
    const styles = StyleSheet.create({
        ...
        video: { width: '100%', height: 200, backgroundColor: appcolor.dark, borderRadius: 10, marginTop: 15, },
        pickButton: { backgroundColor: appcolor.primary, paddingVertical: 12, borderRadius: 8, },
    });
    ...
    const handleVideoLoad = async (meta) => {
        ...
        if (realDurationSec < minSec || realDurationSec > maxSec) {
            Alert.alert(
                'Video không hợp lệ',
                `Video dài ${parseInt(Math.round(realDurationSec) / 60)} phút ${Math.round(realDurationSec) % 60} giây, yêu cầu từ ${minMinutes} đến ${maxMinutes} phút.`
            );
    ...
    const validData = async () => {
        ...
        if ((dataVideo.size / (1024 * 1024)).toFixed(2) > (exercise?.limitSize || 40)) {
    ...
    const uploadData = async () => {
        ...
        if (_valid) {
            ...
            const filePath = dataVideo.videoUri.startsWith('file://') ? dataVideo.videoUri : `file://${dataVideo.videoUri}`
            ...
            alertConfirm('Gửi dự liệu lên hệ thống', `Bạn có muốn gửi dữ liệu đã nhập như bên dưới không ?`, async () => {
                ...
                try {
                    await UploadController.UploadVideo(filePath, formData, dataVideo,
                        async (result) => {
                            ...
                            await ToastError(`${error.statusId}: ${error.messager}`, 'Cảnh báo', 'top')
                            await setProgress(false)
                        })
                } catch (e) {
                    await setProgress(false)
    ...
    const handleSelectVideo = async () => {
        await navigation.navigate("videoplay", { 'urlVideo': dataVideo.videoUri, "item": dataVideo, "guid": null, lockTracking: true })
    }
                ...
                <ScrollView contentContainerStyle={styles.content}>
                    <Card containerStyle={styles.card}>
                        ...
                        <Button
                            title="Chọn video"
                            onPress={pickVideo}
                            icon={<Icon name="upload" type="feather" color="white" style={{ marginRight: 8 }} />}
                            buttonStyle={styles.pickButton}
                            containerStyle={{ marginTop: 20 }}
                        />
                        {dataVideo.name && <Text style={styles.fileInfo}>🎞 <Text style={styles.fileInfo}>{dataVideo.name} {(dataVideo.size && dataVideo.size > 0) && `- ${(dataVideo.size / (1024 * 1024)).toFixed(2)}MB`}</Text></Text>}
                        {videoLoading && <ActivityIndicator style={{ marginTop: 10 }} color={appcolor.primary} />}
                        {dataVideo.videoUri && (
                            <>
                                <TouchableOpacity
                                ...
                                {!videoLoading && dataVideo.duration && (
                                    <Text style={styles.duration}>
                                        ⏱ Thời lượng: {Math.round(dataVideo.duration)} giây (~{(dataVideo.duration / 60).toFixed(0)} phút)
                                    </Text>
                                )}
                            </>
                        )}
