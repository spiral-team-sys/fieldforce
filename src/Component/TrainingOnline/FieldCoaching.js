...
const FieldCoaching = ({ navigation, route }) => {
  ...
  const takePictureAudit = item => {
    ...
    navigation.navigate('Camera', {
      ...itemPhoto,
      callBackReport: handlerCallBack,
    });
  };
  ...
  const renderItemSelect = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity onPress={selectItem}>
        ...
        >
          <Text style={{ padding: 8, fontSize: 15, color: appcolor.dark }}>
            {index + 1}. {item.name}
          </Text>
        </View>
      </TouchableOpacity>
  ...
  return (
    <TouchableOpacity onPress={eventPress}>
      <View style={styles.selectStyle}>
        <Text style={{ fontSize: 15, color: appcolor.dark, fontWeight: '500' }}>
