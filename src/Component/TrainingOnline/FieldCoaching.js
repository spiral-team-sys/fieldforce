...
const FieldCoaching = ({ navigation, route }) => {
    ...
    const chosesPhoto = async (item) => {
        ...
        await NativeCamera.imageGalleryLaunch(photoinfo, handlerCallBack);
    }
        ...
        return (
            <View style={{ width: '100%' }} key={index.toString()} >
                ...
                <View style={{ width: '100%', flexDirection: 'row', backgroundColor: appcolor.yellowdark, borderRadius: 5, padding: 8, marginBottom: 3, alignItems: 'center' }}>
                    <TouchableOpacity style={{ width: 45, height: 30, borderRadius: 10, backgroundColor: appcolor.surface, justifyContent: "center", alignItems: 'center', marginRight: 5 }} onPress={onPressItem} >
                        <Icon name='camera' type="ionicon" size={15} color={appcolor.dark} />
                    </TouchableOpacity>
                    {
                        configData.isAlbum == 1 &&
                        <TouchableOpacity style={{ width: 45, height: 30, borderRadius: 10, backgroundColor: appcolor.surface, justifyContent: "center", alignItems: 'center', marginRight: 5 }} onPress={() => onPressItem('Chose')} >
    ...
    const styles = StyleSheet.create({
        ...
        itemAudit: { width: '75%', paddingStart: 8, paddingEnd: 8, fontSize: 15, color: appcolor.black, textAlignVertical: 'center' }
    })
    useEffect(() => { LoadData() }, [])
    return (
