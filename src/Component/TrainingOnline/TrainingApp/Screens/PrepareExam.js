...
const PrepareExam = ({ navigation, route }) => {
  ...
  const LoadData = async () => {
    ...
    const { isDoing, totalTimeLocal, timer, lastUpdateTime } =
      JSON.parse(isDoingExam) || {};
    await TrainingAPI.PrepareBeforeExam(
      { scheduleSysCode: data.scheduleSysCode },
      erpToken,
      async res => {
        if (res.sysCode == 1) {
  ...
  const onDoExam = () => {
    ...
    if (!dataExam.isAllowExam && from.isAfter(now)) {
      ToastError(
        'Thông báo',
        `Chưa tới giờ làm, bài thi bắt đầu lúc ${from.format('HH:mm:ss')}`,
      );
      return;
    } else if (!dataExam.isAllowExam && to.isBefore(now)) {
      ToastError(
        'Thông báo',
        `Đã hết giờ làm, bài thi kết thúc lúc ${to.format('HH:mm:ss')}`,
      );
      return;
    } else {
      navigation.reset({
        ...
        routes: [
          {
            name: 'trainingExam',
            params: { data: data, times: dataExam.times },
          },
        ],
      });
    }
    ...
    return (
      <View style={styles.viewContainerRules} key={index}>
        <Text
          style={[
            styles.dot,
            { color: item.value ? appcolor.danger : appcolor.dark },
          ]}
        >
          •{' '}
        </Text>
    ...
    <View style={styles.mainContainer}>
        ...
        <View style={styles.iconContainer}>
          <View style={styles.containerIcon}>
            ...
            <View style={styles.containerTextRules}>
              <Text
                style={[
                  styles.textRules,
                  { fontWeight: fontWeightBold, color: appcolor.primary },
                ]}
              >
                {dataExam.totalQuestion}
              </Text>
          ...
          <View style={styles.containerIcon}>
            ...
            <View style={styles.containerTextRules}>
              ...
              >{`${dataExam.times}/${dataExam.numOfTest} lần`}</Text>
              <Text
                style={styles.textRules}
              >{`bạn có ${dataExam.numOfTest} lần để hoàn thành bài kiểm tra`}</Text>
            </View>
          </View>
        </View>
      )}
      ...
      {dataExam.listRule?.length > 0 && (
        <View style={styles.containerRules}>
          <Text style={styles.titleRules}>
            {'Vui lòng đọc kỹ quy định sau: '}
          </Text>
          <View
            style={{
              height: 1,
      ...
      {data.errorMessage !== '' && data.errorMessage !== undefined && (
        <Text style={styles.textErrorMessage}>{data.errorMessage}</Text>
      )}
      <TouchableOpacity
        activeOpacity={0.8}
        style={[
          styles.buttonDoExam,
          {
            backgroundColor: dataExam.isAllowExam
              ? appcolor.primary
              : appcolor.greydark,
          },
        ]}
