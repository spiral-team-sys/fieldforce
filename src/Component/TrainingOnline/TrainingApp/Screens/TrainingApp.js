...
const TrainingApp = ({ navigation, route }) => {
  ...
  const LoadMoreData = async () => {
    ...
    await TrainingAPI.SearchLesson(
      searchLessonInfo,
      tokenAutoLogin,
      async res => {
  ...
  const onFilterHeader = async item => {
    ...
    const nextStatus =
      statusHeader === item.code ? '0' : item.code === 'NotPass' ? '1,2' : '3';
  ...
  const styles = StyleSheet.create({
    ...
    button: {
      ...
      borderRadius: 8,
      borderWidth: 1,
      borderColor: appcolor.primary + '50',
      alignItems: 'center',
      justifyContent: 'center',
    },
    textBottom: {
    ...
    return (
      <View key={index} style={{ flexDirection: 'row' }}>
        <View style={styles.lessonCard}>
          ...
          >
            <Image
              source={{ uri: item.lessonThumbnailLink }}
              style={{ width: 50, height: 50, borderRadius: 8 }}
              resizeMode="center"
            />
          </View>
          <View style={styles.lessonContent}>
            <Text style={styles.lessonTitle}>{item.scheduleName}</Text>
            {item.lastTestDate.lD > 0 && (
              <Text style={[styles.dateText, { fontWeight: fontWeightBold }]}>
                Ngày làm bài:{' '}
                {moment(item.lastTestDate.sD, 'DD/MM/YYYY HH:mm:ss:SSS').format(
                  'DD/MM/YYYY',
                )}
              </Text>
            )}
            <View style={styles.dateContainer}>
              <Text style={styles.dateText}>
                {moment(item.fromDate.sD, 'DD/MM/YYYY HH:mm:ss:SSS').format(
                  'DD/MM/YYYY',
                )}{' '}
                -{' '}
                {moment(item.fromDate.sD, 'DD/MM/YYYY HH:mm:ss:SSS').format(
            ...
            <View style={styles.buttonContainer}>
              {item.scheduleType == 0 ||
                (item.scheduleType == 1 && (
                  <TouchableOpacity style={styles.button} onPress={onLearn}>
                    <Text style={styles.buttonText}>{'Học bài'}</Text>
                  </TouchableOpacity>
                ))}
              {item.scheduleType == 3 && (
                <TouchableOpacity style={styles.button} onPress={onPrepare}>
            ...
            >
              <Text
                style={[
                  ...
                  { color: item.status === 0 ? appcolor.dark : appcolor.light },
                ]}
              >
                {item.percent || 0}%
              </Text>
            ...
            >
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <SpiralIcon
                  ...
                  style={{ marginRight: 4 }}
                />
                <Text style={[styles.textBottom, { color: appcolor.danger }]}>
                  {moment(item.toDate.sD, 'DD/MM/YYYY HH:mm:ss:SSS').fromNow()}
                </Text>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <SpiralIcon
        ...
        {dataSum.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              >
                {item.code === 'NotPass' ? 'Chưa hoàn thành' : 'Hoàn thành'}
              </Text>
  ...
  return (
    <View style={styles.mainContainer}>
      <HeaderCustom
        title={kpiinfo.menuNameVN || 'Đào tạo'}
        leftFunc={goBack}
        rightFunc={onFilter}
        iconRight={'filter'}
      />
      ...
      <LessonSheet
        onUpdateData={handlerFilter}
        onActiveOptions={onActiveOptions}
