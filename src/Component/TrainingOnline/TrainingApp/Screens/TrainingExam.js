...
import { View, StyleSheet, FlatList, BackHandler, DeviceEventEmitter, Text } from 'react-native'
...
import { deleteDataRawTraining, getDataByReport, saveJsonData } from '../../../../Controller/ReportController'
...
const TrainingExam = ({ navigation, route }) => {
    ...
    const LoadData = async () => {
        try {
            ...
            if (getDataExam?.data?.length > 0) {
                ...
                const { timer, isDoing, totalTimeLocal, lastUpdateTime } = JSON.parse(isDoingExam) || '{}'
                const now = Date.now()
                ...
                if (isDoing && newTimeLeft > 0) {
                    ...
                    MessageAcept('Thông báo', 'Bạn đã hết thời gian làm bài, hãy nộp bài', async () => {
                ...
                if (!isConnected) {
                    ToastError('Lỗi kết nối', 'Kết nối dữ liệu Wifi/4G không ổn định, Vui lòng kiểm tra lại')
                    await setLoading(false)
                    await setDataExam([])
                    return
                }
                await TrainingAPI.GetDataExam({ scheduleSysCode: data?.scheduleSysCode, times: times + 1 }, erpToken, (result) => {
                    if (result.sysCode == 1) {
                        setMessage(result.sysName)
                    } else {
                        const dataUpdate = _.map(result.listQuestion, (item) => {
                            return {
                                ...item, lessonName: data.lessonName
                            }
                        })
                        setMessage('')
    ...
    const handlerAnswer = useCallback((item, index, itemParent) => {
        setDataExam(prev => {
        ...
        TrainingAPI.SaveAnswer({ sysCode: itemParent.sysCode, answerKey: item.code, answerContent: item.answerContent }, tokenAutoLogin)
    }, [kpiinfo.id])
    ...
    const onSubmit = useCallback(async () => {
        ...
        if (dataExam.filter(item => item.answerKey == '')?.length > 0) {
            MessageAcept('Thông báo', 'Bạn có câu hỏi chưa làm, hãy xem lại các câu hỏi chưa làm', async () => {
            ...
            Message('Chú ý', 'Bạn có chắc chắn muốn nộp bài không ?', async () => {
                await setLoading(true)
                const getDataExam = await getDataByReport(0, kpiinfo.id || 3) || {}
                await TrainingAPI.SubmitExam({ listQuestion: getDataExam.data }, tokenAutoLogin, async (result) => {
                    if (result.sysCode == 1) {
    ...
    const renderItemNoAnswer = useCallback(({ item, index }) => {
        const mainIndex = _.findIndex(dataExam, (question) => question.questionSysCode === item.questionSysCode)
        return (
            <TouchableOpacity style={[styles.containerNoAnswer, { backgroundColor: item.isFlag ? appcolor.primary : appcolor.light, borderColor: item.isNotDone ? appcolor.danger : appcolor.surface }]} onPress={() => onGotoQuestion(mainIndex)} key={index}>
                <View style={styles.noAnswer}>
                    <Text style={[styles.textNoAnswer, { color: item.isFlag ? appcolor.light : appcolor.dark }]}>{mainIndex + 1}</Text>
                    <Text style={[styles.textAnswer, { color: item.isFlag ? appcolor.light : appcolor.primary }]}>{item.codeDisplay}</Text>
                </View>
            </TouchableOpacity>
        )
    }, [dataExam, appcolor])
    ...
    const styles = StyleSheet.create({
        ...
        textQuestion: { fontSize: 13, color: appcolor.dark, fontWeight: fontWeightBold, marginBottom: 8, },
        textAnswer: { fontSize: 11, color: appcolor.dark, width: '80%' },
        containerAnswer: { flexDirection: 'row', alignItems: 'center', borderRadius: 12, borderWidth: 1, borderColor: appcolor.surface, padding: 12, marginBottom: 8 },
        iconAnswer: { marginRight: 8 },
        containerQuestion: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%' },
        containerNoAnswer: { marginTop: 4, width: 50, height: 50, marginRight: 4, borderRadius: 12, borderWidth: 1, borderColor: appcolor.surface, justifyContent: 'center', alignItems: 'center' },
        noAnswer: { alignItems: 'center', justifyContent: 'space-between' },
        textNoAnswer: { fontSize: 12, color: appcolor.dark, fontWeight: fontWeightBold, textAlign: 'center' },
        message: { fontSize: 12, color: appcolor.dark, fontWeight: fontWeightBold, textAlign: 'center' },
        textAnswer: { fontSize: 11, color: appcolor.dark, width: '80%', textAlign: 'center', fontWeight: fontWeightBold },
    ...
    return (
        <View style={styles.mainContainer}>
            <HeaderCustom title={data?.lessonName || dataExam[0]?.lessonName || 'Đào tạo'} rightFunc={isDoing && dataExam.length > 0 ? onSubmit : undefined} iconRight={'cloud-upload-alt'} />
            <TimerComponent
                timeLeft={timeLeft}
                totalTime={totalTime}
            />
            <TouchableOpacity style={styles.iconListQuestion} onPress={onToggleListQuestion}>
                <Icon name={isShowListQuestion ? 'caret-up' : 'caret-down'} type='font-awesome-5' size={20} color={appcolor.dark} />
            </TouchableOpacity>
            {message !== '' && message !== undefined && <Text style={styles.message}>{`${message}`}</Text>}
            {dataExam.length == 0 && <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                <Text style={{ fontSize: 12, fontWeight: fontWeightBold, color: appcolor.dark, textAlign: 'center' }}>{'Không thể lấy dữ liệu bài thi, nhấn về trang chủ để quay lại'}</Text>
                <TouchableOpacity onPress={onGoback}>
                    <Text style={{ fontSize: 12, fontWeight: fontWeightBold, color: appcolor.primary, textAlign: 'center', marginTop: 8 }}>{'Về trang chủ'}</Text>
                </TouchableOpacity>
            </View>}
            {isShowListQuestion &&
                <View>
                    <FlatList
                        ...
                        numColumns={5}
                        contentContainerStyle={{ justifyContent: 'center', alignItems: 'center', paddingBottom: 8 }}
                    />
                </View>
            }
            <View style={styles.containerQuestion}>
                <FlashList
                    ...
                    ListFooterComponent={<View style={{ paddingBottom: deviceHeight / 1.2 }} />}
