...
import { FlatList, StyleSheet, View, Text, TouchableOpacity, Platform, UIManager, LayoutAnimation } from "react-native"
...
const TrainingList = ({ navigation, route }) => {
    ...
    const [dataMain, setDataMain] = useState({ dataTraining: [], dataTrainingF: [], dataTrainingM: [], dataGroupTraining: [] })
    ...
    const LoadData = async () => {
        ...
        if (shopinfo.shopId !== 0) {
            await getListTraining(async (mData) => {
                ...
                const dataFilterByTab = _.filter(arr, (it) => it.positionName == currentTab.positionName)
                await setDataTab({ indexTab: 0, itemTab: currentTab })
                await setDataMain({ dataTraining: dataFilterByTab, dataTrainingF: dataFilterByTab, dataTrainingM: arr, dataGroupTraining: dataGroupTraining })
            })
        } else {
    ...
    const styles = StyleSheet.create({
        ...
        itemMain: { width: '100%', padding: 8, backgroundColor: appcolor.surface, borderRadius: 10, marginBottom: 8, flexDirection: 'row', alignItems: 'center' },
        titleMenu: { width: '100%', color: appcolor.dark, fontSize: 16, fontWeight: '700' },
        descriptionView: { width: '100%', color: appcolor.dark, fontSize: 13, fontStyle: "italic" }
    })

    const filterItem = async (text) => {
        if (text !== null && text.length > 0) {
            const mResult = await dataMain.dataTrainingF.filter((it) => {
                const nameTraining = it.title ? it.title.toUpperCase() : ''.toUpperCase();
    ...
    const onPressTab = (item, index) => {
        ...
        const dataFilterByTab = _.filter(dataMain.dataTrainingM, (it) => it.positionName == item.positionName)
    ...
    const renderItemGroup = ({ item, index }) => {
        ...
        const styleDefault = { minWidth: 80, padding: 8, margin: 8, marginEnd: 1, borderRadius: 20, borderWidth: 0.5, borderColor: appcolor.greylight, marginEnd: (index + 1) == dataMain.dataGroupTraining?.length ? 8 : 1 }
        const styleChoose = { minWidth: 80, padding: 8, margin: 8, marginEnd: 1, borderRadius: 20, borderWidth: 0.5, borderColor: appcolor.primary, backgroundColor: appcolor.primary, marginEnd: (index + 1) == dataMain.dataGroupTraining?.length ? 8 : 1 }
        return (
            <TouchableOpacity key={`igi_${index}`} style={isChoose ? styleChoose : styleDefault} onPress={() => onPressTab(item, index)}>
                <Text style={{ width: '100%', fontSize: 14, fontWeight: Platform.OS == 'ios' ? '600' : '700', color: appcolor.dark, textAlign: 'center', color: isChoose ? appcolor.light : appcolor.dark }}>{item.positionName || 'NONE'}</Text>
            </TouchableOpacity>
        )
    }
    useEffect(() => {
