...
export const FieldCoachingForEmployee = ({ navigation }) => {
	...
	const onFilterEmployee = (text) => {
		...
		const dataSearch = _.filter(dataMain, (e) => {
			return removeVietnameseTones(e.EmployeeName || '').toLowerCase().match(keyword)
		})
	...
	const styles = StyleSheet.create({
		...
		childTitleMain: { flex: 1, fontSize: 13, fontWeight: '400', color: appcolor.dark },
		contentAction: { width: '100%', height: '100%' },
		sheetContainer: { height: deviceHeight * 0.92, width: deviceWidth, backgroundColor: appcolor.light },
	})
	// View
