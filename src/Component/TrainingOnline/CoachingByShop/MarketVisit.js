...
import { FlatList, Keyboard, LayoutAnimation, Platform, StyleSheet, UIManager } from "react-native";
import { View } from "react-native";
import { useSelector } from "react-redux";
import { HeaderCustom } from "../../../Content/HeaderCustom";
import { Message, MessageInfo, UUIDGenerator, deleteReportEdit, groupDataByKey, updateReportEdit } from "../../../Core/Helper";
...
import { getAllPhotosUploaded, getPhotosReport } from "../../../Controller/WorkController";
...
export const MarketVisit = ({ navigation }) => {
		...
		for (let index = 0; index < dataMarket.length; index++) {
				...
				if (item.Id == 1) {
					...
					for (let j = 0; j < dataFilter.length; j++) {
						...
						if (child.ItemValue !== 0 && (child.ItemValue || null) == null) {
							toastError('Thông báo', `Chưa nhập dữ liệu ${item.ItemName} - ${child.ItemName}`)
							return false
						}
					}
				} else {
			...
			if (item.ImageItem == 1 && listReport.isCheckImage == 1) {
				...
				if (listPhoto?.length == 0) {
					MessageInfo(`Chưa chụp hình ${listReport.checkByCode !== 1 ? '"của ' + item.ItemName + '" ' : ''}báo cáo (${listPhoto?.length}/${listReport?.limitPhoto || 1})`)
					return false
				} else if (listPhoto.length < (listReport?.limitPhoto || 1)) {
					MessageInfo(`Chưa chụp đủ hình ${listReport.checkByCode !== 1 ? '"của ' + item.ItemName + '" ' : ''}của báo cáo (${listPhoto?.length}/${listReport?.limitPhoto || 1})`)
					return false
				}
			}
		}
	...
	const handlerChangeTextInList = (dataMain, indexItem, indexMain, text) => {
		...
		if (configStore.checkReportEdit == 1) {
			updateReportEdit({ shopId: shopinfo.shopId, reportId: kpiinfo.kpiId, menuNameVN: kpiinfo.menuNameVN })
		}
	}
	...
	const handlerChooseItem = (item, indexMain) => {
		...
		const dataDependent = _.filter(dataMarket, it => it.ItemDependent == (dataMarket[indexMain]?.RefId || 0))
		if (dataDependent.length > 0) {
			for (let i = 0; i < dataDependent.length; i++) {
				const itemDependent = dataDependent[i]
				const indexByItem = _.findIndex(dataMarket, { RefId: itemDependent.RefId });
	...
	const handlerViewPhoto = async (item) => {
		const photos = await getPhotosReport(kpiinfo.id, `${item.Code}`, shopinfo.shopId, TODAY);
		setPictureShow({ visible: true, photos, index: 0 })
	}
	const callBackPhoto = async () => {
		const listPhoto = await getAllPhotosUploaded(kpiinfo.kpiId, shopinfo.shopId, TODAY);
	...
	const styles = StyleSheet.create({
		...
		titleHeader: { fontSize: 14, fontWeight: '700', color: appcolor.dark, padding: 8 },
		...
		if (!isUploaded) {
			Message('Chú ý', 'Bạn có chắc chắn muốn tải lại dữ liệu?',
				async () => {
					...
					if (configStore.checkReportEdit == 1) {
						deleteReportEdit({ shopId: shopinfo?.shopId, reportId: kpiinfo?.kpiId })
					}
					LoadData()
					setMutate(e => !e)
			...
			toastError('Thông báo', 'Dữ liệu đã được gửi lên hệ thống bạn không thể tải lại!')
			SheetManager.hide('ref_bottomSheet')
		}
	}
	...
	const itemCheck = (item) => {
		const itemMain = _.filter(dataMarket, itemM => itemM.RefId == item.ItemDependent)[0] || null
		const listMainCheck = itemMain ? JSON.parse(itemMain?.FilterList || '[]') : []
		const itemCheck = _.filter(listMainCheck, itemM => itemM.itemName == itemMain.ItemValue)[0] || null
	...
	const renderItem = ({ item, index }) => {
		...
		const lockTitle = (item.ItemDependent && lockItem ? '(KHÔNG CẦN TRẢ LỜI)' : '')
			...
			<View key={`typ_${index}`} style={{ padding: 4 }}>
				{item.isParent && item.GroupName !== undefined &&
					<View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: appcolor.primary, padding: 8, borderRadius: 5, marginVertical: 8 }}>
						<Text style={{ flex: 1, fontSize: 14, fontWeight: 'bold', color: appcolor.white }}> {item.GroupName}</Text>
					</View>
				}
				<Text style={styles.titleHeader}>{item.ItemName} {<Text style={[styles.titleHeader, { color: appcolor.danger }]}> {isRequired}</Text>} {lockTitle}</Text>
				{item.NumberValue == 2 &&
					<InputInList
