...
export const FieldCoachingDetails = ({ itemCoaching, dataGroup, dataDetails = [], onSaveData, onCloseView }) => {
	...
	const LoadData = async () => {
		const details = parseList(dataDetails).length > 0 ? parseList(dataDetails) : parseList(itemCoaching?.DataSurvey)
		const groups = parseList(dataGroup)
		const groupNameById = {}
		const groupIds = groups.map(item => {
			const groupId = getValueByKeys(item, ['ParentId', 'parentId', 'Id', 'id', 'GroupId', 'groupId', 'GroupID'])
			if (groupId !== null) {
				groupNameById[groupId] = item.GroupName || item.groupName || item.ItemName || item.name
			}
			return groupId
		...
		if (groupIds.length > 0) {
			const dataFilter = details.filter(item => {
				const itemParentId = getValueByKeys(item, ['ParentId', 'parentId', 'GroupId', 'groupId', 'GroupID'])
		...
		listShow = listShow.map(item => {
			...
			return {
				...item,
				ParentId: item.ParentId ?? item.parentId ?? item.GroupId ?? item.groupId ?? item.GroupID,
				GroupName: item.GroupName || item.groupName || groupNameById[itemParentId] || ''
			}
		})
	...
	const onChangeValue = (value, indexItem) => {
		...
		const listUpdate = coachingList.map((it, index) => index === indexItem ? { ...it, ItemValue: inputValue } : it)
		...
		return (
			<View key={`kkp_P_${index}`} style={styles.itemMain}>
				...
				<View style={{ width: '100%', flexDirection: 'row' }}>
					<Text style={styles.titleView}>{`${index + 1}. ${item.ItemName || item.itemName || ''}`}</Text>
					<FormGroup
		...
		<View style={styles.mainContainer}>
			...
			<ScrollView
				...
				{(!coachingList || coachingList.length === 0) &&
					<Text style={{ padding: 16, color: appcolor.greydark, textAlign: 'center' }}>Không có tiêu chí đánh giá</Text>
				}
