...
export const PhotoControlView = ({ isUploaded, itemHeader, index, handlerCamera, handlerAlbums, dataPhoto }) => {
	const { appcolor, kpiinfo, shopinfo } = useSelector(state => state.GAppState)

	const countPhoto = () => {
		const listPhoto = _.filter(dataPhoto, (it) => it.photoType == `${itemHeader.Code}`)
	...
	const RenderButton = ({ iconName, titleName, onAtion }) => {
		...
		return (
			<TouchableOpacity
				style={{ width: deviceWidth / 5, backgroundColor: appcolor.surface, marginEnd: 3, marginStart: 3, borderRadius: 5 }}
	...
	return (
		<View style={{ width: '100%', flexDirection: 'row', borderRadius: 5, padding: 5, paddingStart: 10 }}>
