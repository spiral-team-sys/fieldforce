...
const ItemProductsSellIn = ({ navigation, route }) => {
  ...
  const renderItemSelect = ({ item, index }) => {
    ...
    const isInputLockedByPrice =
      isPriceVisible && isLockByPrice && !hasOriginalPrice;
    ...
    const onChangePriceItem = value => {
      ...
      const indexItem = dataProduct?.listProduct.findIndex(
        it => it.id === item.id,
      );
    ...
    const handlerValidatePrice = () => {
      ...
      if (priceValue < minPrice || priceValue > maxPrice) {
        alertWarning(
          `Giá nhập phải nằm trong khoảng ${formatNumber(
            minPrice,
            ',',
          )}đ đến ${formatNumber(maxPrice, ',')}đ`,
        );
    ...
    const onNumChanged = async (e, index) => {
      let numValue = null;
      if (e !== null && parseInt(e) < 1000)
        numValue = e.toString().replace(/,/g, '.');
      else numValue = e.toString().replace(/,/g, '');
      //
      var sanitizedInput =
        /^\d*\.?\d*$/.test(numValue) && numValue !== ''
          ? parseFloat(numValue)
          : null;
      const indexItem = dataProduct?.listProduct.findIndex(
        it => it.id === item.id,
    ...
    return listReport?.isMultiProduct === 1 &&
      dataModalBS.typeSelect === TYPE_PRODUCTS ? (
      <View style={{ paddingHorizontal: 4 }}>
        ...
        >
          ...
          {isInputLockedByPrice && (
            <View
              style={{ width: '100%', paddingHorizontal: 8, paddingBottom: 8 }}
            >
              <Text
                style={{
                  fontSize: 12,
  ...
  const handlerFilterItem = async (dataBS, value) => {
    ...
    const _filterList = _.filter(dataMain, e =>
      removeVietnameseTones(e.name).toLowerCase().includes(_valueSearch),
    );
  ...
  const handlerGetCalendarMinDate = () => {
    ...
    if (listReport.isLockPastOneMonth == 1)
      return moment().subtract(1, 'month').format('YYYY-MM-01');
    return null;
  };
  ...
  const handlerDatePress = async date => {
    ...
    if (
      listReport.isLockMonth == 1 &&
      dateSelect.isBefore(today.clone().startOf('month'), 'day')
    ) {
    ...
    if (
      listReport.isLockPastOneMonth == 1 &&
      dateSelect.isBefore(
    ...
    switch (typeView) {
      case TYPE_STORELIST:
        itemParams = {
          ...
          ListCategory:
            listReport.productByStore == 1 ? item.listCategory : null,
        };
        break;
      case TYPE_DEALER:
        itemParams = {
  ...
  return (
    ...
    >
      ...
      {dataModalBS.typeSelect !== TYPE_CALENDAR && (
        <View style={{ height: deviceHeight }}>
          <CustomListView
            ...
            ListHeader={
              listReport.isMultiProduct == 1 ? renderHeaderComponent() : null
            }
      ...
      {/* <View style={{ position: 'absolute', top: 50, left: 10 }}>
				...
				>
					<SpiralIcon name="close" size={28} color={appcolor.dark} />
				</TouchableOpacity>
			</View> */}
      {showApply && selectedIds.length > 0 && selectedCount > 0 && (
        ...
        >
          ...
          >
            <Text
              style={{ color: appcolor.light, fontSize: 16, fontWeight: '600' }}
            >
              {'Áp dụng'}{' '}
            </Text>
            <Avatar
