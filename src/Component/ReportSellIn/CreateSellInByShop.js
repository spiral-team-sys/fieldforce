...
const CreateSellInByShop = ({ navigation, route }) => {
  ...
  const [dataModalBS, setDataModalBS] = useState({
    data: [],
    typeSelect: '',
    dataAll: [],
  });
  ...
  const LoadDataCreate = async () => {
    ...
    if (
      itemSave.ShopName == 'Danh sách cửa hàng' &&
      listReport.shopName !== undefined
    ) {
    ...
    if (
      itemSave.DealerName == 'Nhà phân phối' &&
      listReport.dealerName !== undefined
    ) {
  ...
  const handlerChangeItem = async typeView => {
    ...
    switch (typeView) {
      ...
      case TYPE_PRODUCTS:
        ...
        const categoryByStore = JSON.parse(
          route?.params?.itemSave?.ListCategory || '[]',
        );
  ...
  const handlerGetAmountValue = (quantity, price, originalPrice) => {
    ...
    const priceValue =
      handlerGetNumberValue(price) || handlerGetNumberValue(originalPrice);
  ...
  const handlerGetTotalAmountValue = (listProduct = []) => {
    return (listProduct || []).reduce((total, item) => {
      return (
        total +
        handlerGetAmountValue(item.quantityValue, item.priceValue, item.price)
      );
    }, 0);
  };
  ...
  const handlerValidatePhoto = () => {
    ...
    if (numIMG > 0 && (photoData.listPhoto || []).length < numIMG) {
      ToastError(
        `Bạn cần chụp tối thiểu ${numIMG} hình ảnh`,
        'Thông báo',
        'top',
      );
      return false;
  ...
  const handlerSaveItem = async () => {
    ...
    const itemQuantity =
      Quantity.length > 0 ? ConvertToInt(Quantity.replace(/,/g, '')) : '';
    const itemPrice =
      Price.length > 0 ? ConvertToInt(Price.replace(/,/g, '')) : '';
  ...
  const handlerSaveMultiItem = async () => {
    ...
    if (items.length > 0) {
      if (listReport.isUsePercent == 1) {
        ...
        ToastError(
          'Bạn đã nhập giá nhưng chưa nhập số lượng. sản phẩm: ' +
            items[0].name,
          'Thông báo',
          'top',
        );
    ...
    handlerConfirmAlertSave(async () => {
      ...
      dataProduct.listProduct =
        productByStore.length > 0
          ? [...productByStore]
          : [...dataProduct.listProductM];
  ...
  useEffect(() => {
    switch (dataModalBS.typeSelect) {
      case TYPE_CALENDAR:
        ...
        break;
      case TYPE_STORELIST:
        const productByStore = JSON.parse(
          route?.params?.itemSave?.ListProduct || '[]',
        );
  ...
  const RenderProductSelect = ({ item, index }) => {
    const amountValue = handlerGetAmountValue(
      item.quantityValue,
      item.priceValue,
      item.price,
    );
        ...
        >
          ...
          {itemSave.ShopId > 0 && (
            <RenderSelectItem
              ...
              selectValue={
                itemSave.DealerId !== 0
                  ? `Nhà phân phối : ${itemSave.DealerName}`
                  : itemSave.DealerName
              }
              onPress={handlerChangeItem}
            />
                ...
                <View style={styles.listProductHeader}>
                  ...
                  {listReport.isUsePercent !== 1 && (
                    <Text style={styles.totalAmountValue}>
                      {toCurrency(
                        handlerGetTotalAmountValue(dataProduct.listSelect),
                      ) || '0'}
                    </Text>
                  )}
                </View>
