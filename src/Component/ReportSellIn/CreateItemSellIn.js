...
const CreateItemSellIn = ({ dataWork, resultEvent, itemSellIn }) => {
  ...
  const getProductBySelection = currentItem => {
    return mainData.dataProduct.filter(
      i =>
        (currentItem.CompetitorId == 0 ||
          i.competitorId == currentItem.CompetitorId) &&
  ...
  const handlerFilterItem = async (dataBS, value) => {
    ...
    if (value !== null && value !== undefined && value.length > 0) {
      dataFilter = dataModalMain.filter(i =>
  ...
  const onSelectItem = (itemChoose, typeView) => {
    ...
    switch (typeView) {
      ...
      case TYPE_CATEGORY:
        ...
        if (
          itemUpdate.CompetitorId !== 0 &&
          itemUpdate.CompetitorId !== _competitorId
        ) {
        ...
        break;
      case TYPE_PRODUCTS:
        const categoryNameSelect = mainData.dataCategory.find(
          it => it.id === itemChoose.categoryId,
        ).name;
  ...
  const handlerSaveItem = async () => {
    ...
    if (itemSave.isUploaded == 1) {
      ToastError(
        'Bạn đã gửi hóa đơn này lên hệ thống, \nKhông thể lại chỉnh sửa!',
      );
      return;
    }
    ...
    let dataSellIn = {
      ...
      productId:
        itemSave.CompetitorId === _competitorId ? itemSave.ProductId : 0,
    ...
    if (itemSave.id) {
      ...
      itemSellIn = {
        ...
        productId:
          itemSave.CompetitorId === _competitorId ? itemSave.ProductId : 0,
        productName:
          itemSave.CompetitorId === _competitorId ? itemSave.ProductName : '',
        competitorId: itemSave.CompetitorId,
  ...
  return (
    <View style={{ flex: 1 }}>
      <HeaderCustom
        title="Khai báo số bán"
        rightFunc={handlerSaveItem}
        iconRight="save"
        ...
        >
          ...
          >
            ...
            <RenderSelectItem
              ...
              styles={styles}
              selectValue={itemSave.DealerName}
            ...
            {AppNameBuild === psvApp && (
              <RenderInput
                ...
                typeView={TYPE_PRICE}
                styles={styles}
                inputValue={itemSave.Price}
                handlerTextChange={handlerTextChange}
              />
            )}
            <RenderInput
              appcolor={appcolor}
              typeView={TYPE_QUANTITY}
              styles={styles}
              inputValue={itemSave.Quantity}
              handlerTextChange={handlerTextChange}
            />
            <RenderInput
              appcolor={appcolor}
    ...
    <View style={{ width: '98%', alignSelf: 'center', margin: 3 }}>
      {typeView === TYPE_ORDER_NO ? (
        ...
        <FormGroup
          ...
          value={text || ''}
          placeholder={
            typeView === TYPE_PRICE_NPP || typeView === TYPE_PRICE
              ? 'Nhập giá'
              : 'Nhập số lượng'
          }
