...
const SellInByEmployee = ({ navigation, route }) => {
  ...
  const handlerChooseStore = async (item, index) => {
    ...
    let dataChange = dataStore.map(i =>
      i.id !== item.id ? { ...i, isChoose: 0 } : i,
    );
    await setChooseShop({
      ...itemShopChoose,
      ShopName: item.name,
      ShopId: item.isChoose == 1 ? item.id : 0,
    });
    ...
    if (dateString !== null && dateString !== undefined) {
      ...
      if (!dataCalendar.isStartDay) {
        ...
        if (range > 0) {
          ...
          itemShopChoose.FromDate = moment(dataCalendar.startDate).format(
            'YYYYMMDD',
          );
  ...
  const handlerFilterShop = async value => {
    ...
    if (value !== null && value !== undefined && value.length > 0) {
      dataFilter = dataStoreFilter.filter(i =>
  ...
  const renderItemShop = ({ item, index }) => {
    const isChooseStyle =
      item.isChoose == 1 ? appcolor.yellowdark : appcolor.grayLight;
    const onPressItem = () => {
    ...
    return (
      <TouchableOpacity onPress={onPressItem}>
        <View
          style={{ ...styles.itemShopStyle, backgroundColor: isChooseStyle }}
          key={index}
        >
          <Text style={{ width: '100%', fontSize: 16, color: appcolor.dark }}>
            {index + 1 + '. ' + item.name}
          </Text>
        </View>
      </TouchableOpacity>
  ...
  const renderItem = ({ item, index }) => {
    ...
    if (item.isParent) {
      data
        .filter(it => it.workDate == item.workDate)
        .forEach(it => {
      ...
      <View key={'daa200-' + index}>
        {item.isParent && (
          <RenderItemText
            ...
            type="Header"
            titleName=""
            itemValue={moment(item.date).format('dddd DD/MMM/YY')}
          />
        )}
        ...
        >
          <View style={{ width: '100%', alignSelf: 'center' }}>
            <View style={styles.itemStyle} key={index}>
              {/* Status Order */}
              {item.statusOrder !== null && item.statusOrder !== undefined && (
                <RenderItemText
                  appcolor={appcolor}
                  type="Status"
                  titleName="Trạng thái: "
                  itemValue={item.statusOrder}
                  colorByRow={item.colorStatus}
                />
              )}
              {item.confirmBy != null && (
                <RenderItemText
              ...
              <RenderItemText
                appcolor={appcolor}
                type="Info"
                titleName="Mã đơn hàng: "
                itemValue={item.orderNo}
              />
              <RenderItemText
                appcolor={appcolor}
                type="Info"
                titleName="NPP: "
                itemValue={item.dealerName}
              />
              <RenderItemText
                appcolor={appcolor}
                type="ShopName"
                titleName="Cửa hàng: "
                itemValue={item.shopName}
              />
              <RenderItemText
                appcolor={appcolor}
                type="Info"
                titleName="Sản phẩm: "
                itemValue={item.categoryName + ' ' + item.productName}
              />
              {item.priceCount !== null &&
                item.priceCount !== undefined &&
                item.priceCount !== '' && (
                  <RenderItemText
                    appcolor={appcolor}
                    type="PriceCount"
                    itemValue={item.priceCount}
                  />
                )}
              {item.price !== null &&
              ...
              {item.priceNPP !== null &&
                item.priceNPP !== undefined &&
                item.priceNPP !== 0 && (
                  <RenderItemText
                    appcolor={appcolor}
                    type="PriceNPP"
                    titleName="Giá NPP: "
                    itemValue={item.priceNPP}
                  />
                )}
              {item.isConfirm == 1 && (
                <View
        ...
        >
          ...
          >
            ...
            >
              <TouchableOpacity
                style={{}}
                onPress={() => SheetManager.hide('filtersell')}
              >
                <View
                  style={{
                    width: 40,
  ...
  return (
    <View style={{ width: '100%', height: 'auto' }}>
      <View style={{ width: '90%', padding: 3 }}>
        ...
        {type == 'Info' && (
          <Text
            style={{ width: '100%', fontSize: scaleSize(12), color: colorItem }}
          >
            {titleName}
            {itemValue}
          </Text>
