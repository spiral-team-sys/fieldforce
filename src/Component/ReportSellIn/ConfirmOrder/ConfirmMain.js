...
export const ConfirmMain = ({ navigation, route }) => {
  ...
  const LoadData = async () => {
    ...
    const filter = {
      fromDate: route?.params?.filterItem.FromDate,
      toDate: route?.params?.filterItem.ToDate,
    };
      ...
      <View style={styles.contentView}>
        {dataConfirm !== null && dataConfirm.length > 0 && (
          ...
          >
            {tabList.length > 0 &&
              tabList.map((it, i) => {
                let dataItem = _.filter(dataConfirm, {
                ...
                return (
                  <Tabs.Tab key={`iod_${i}`} label={title} name={title}>
                    <View
                      style={{ marginTop: 62, padding: 5, width: deviceWidth }}
                    >
                      <FlatList
                        key={'lstconfirmorder'}
                        extraData={arr}
