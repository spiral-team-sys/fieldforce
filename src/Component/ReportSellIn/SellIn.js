...
const SellIn = ({ navigation }) => {
  ...
  const uploadData = async () => {
    ...
    if (dataUpload.length == 0) {
      alertWarning(
        'Không có dữ liệu SellIn, Vui lòng thêm dữ liệu số bán trước khi gửi dữ liệu',
      );
      return;
    }
    alertConfirm(
          ...
          >
            <View style={styles.itemStyle} key={index}>
              ...
              <RenderItemText
                ...
                itemValue={`${
                  item.competitorName
                    ? item.competitorName + (item.categoryName ? ' / ' : '')
                    : ''
                }${
                  item.categoryName
              ...
              {item.priceValue > 0 && (
                <RenderItemText
                  type="Price"
                  titleName="Giá sản phẩm : "
                  itemValue={`${item.priceValue}`}
                  appcolor={appcolor}
                />
              )}
            </View>
  ...
  return !isCreateView ? (
    <View style={styles.mainContainer}>
      <HeaderCustom
        title={
          visibleOption === true ? `Đã chọn (${isDelete})` : kpiinfo.menuName
        }
        leftFunc={() =>
          visibleOption === true ? onPressDel(null) : navigation.goBack()
        }
