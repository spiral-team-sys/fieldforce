...
import { View, StyleSheet, Keyboard, KeyboardAvoidingView, Platform, ScrollView, TouchableOpacity, Modal, SafeAreaView, Text, FlatList } from 'react-native';
...
const SellInModel = ({ actionClose, dataListInput }) => {
    ...
    const [dataModal, setDataModal] = useState({ dataSelect: [], dataFilter: [], visible: false, titleModal: null, type: null })
    const [itemSellIn, setItemSellIn] = useState({
        shopId: workinfo.shopId, workDate: workinfo.workDate, orderNo: `HD${workinfo.workDate}`, quantityValue: null, priceValue: null,
        dealerId: 0, dealerName: '', productId: 0, productName: '', competitorId: 0, competitorName: '', categoryId: 0, categoryName: '',
        subCategoryId: 0, subCategoryName: '', segmentId: 0, segmentName: '', subSegmentId: 0, subSegmentName: '', note: ''
    })
    ...
    const ListItemValue = (item) => {
        switch (item.code) {
            ...
            case TYPE_SELLIN.CATEGORY:
                const lstCategory = dataListInput.categoryList.filter(i =>
                    (itemSellIn.competitorId == 0 || i.type == itemSellIn.competitorId)
                )
                return lstCategory
            case TYPE_SELLIN.SUB_CATEGORY:
                const lstSubCategory = dataListInput.subCategoryList.filter(i =>
                    (itemSellIn.competitorId == 0 || i.type == itemSellIn.competitorId) &&
                    (itemSellIn.categoryId == 0 || i.categoryId == itemSellIn.categoryId)
                )
                return lstSubCategory
            case TYPE_SELLIN.SEGMENT:
                const lstSegmentList = dataListInput.segmentList.filter(i =>
                    (itemSellIn.competitorId == 0 || i.type == itemSellIn.competitorId) &&
                    (itemSellIn.categoryId == 0 || i.categoryId == itemSellIn.categoryId) &&
                    (itemSellIn.subCategoryId == 0 || i.subCatId == itemSellIn.subCategoryId)
                )
                return lstSegmentList
            case TYPE_SELLIN.SUB_SEGMENT:
                const lstSubSegmentList = dataListInput.subSegmentList.filter(i =>
                    (itemSellIn.competitorId == 0 || i.type == itemSellIn.competitorId) &&
                    (itemSellIn.categoryId == 0 || i.categoryId == itemSellIn.categoryId) &&
                    (itemSellIn.subCategoryId == 0 || i.subCatId == itemSellIn.subCategoryId) &&
            ...
            case TYPE_SELLIN.PRODUCTS:
                const lstProducts = dataListInput.productsList.filter(i =>
                    (itemSellIn.competitorId == 0 || i.competitorId == itemSellIn.competitorId) &&
                    (itemSellIn.categoryId == 0 || i.categoryId == itemSellIn.categoryId) &&
                    (itemSellIn.subCategoryId == 0 || i.subCatId == itemSellIn.subCategoryId) &&
                    (itemSellIn.segmentId == 0 || i.segmentId == itemSellIn.segmentId) &&
                    (itemSellIn.subSegmentId == 0 || i.subSegmentId == itemSellIn.subSegmentId)
                )
                return lstProducts;
        }
    }
    ...
    const handlerChooseItem = async (item, index, value, type) => {
        switch (type) {
            case TYPE_SELLIN.DEALER:
                await setItemSellIn({ ...itemSellIn, dealerId: item.id, dealerName: value })
                break
            case TYPE_SELLIN.COMPETITOR:
                await setItemSellIn({ ...itemSellIn, competitorId: item.id, competitorName: value })
                break
            case TYPE_SELLIN.CATEGORY:
                await setItemSellIn({ ...itemSellIn, categoryId: item.id, categoryName: value })
                break
            case TYPE_SELLIN.SUB_CATEGORY:
                await setItemSellIn({ ...itemSellIn, subCategoryId: item.id, subCategoryName: value })
                break
            case TYPE_SELLIN.SEGMENT:
                await setItemSellIn({ ...itemSellIn, segmentId: item.id, segmentName: value })
                break
            case TYPE_SELLIN.SUB_SEGMENT:
                await setItemSellIn({ ...itemSellIn, subSegmentId: item.id, subSegmentName: value })
                break
            case TYPE_SELLIN.PRODUCTS:
                await setItemSellIn({ ...itemSellIn, productId: item.id, productName: value })
                break
        }
        ...
        return (
            dataConfig?.map(i => {
                ...
                switch (i.ref_Code) {
                    case 'selected':
                        return (
                            <View key={i.code} style={{ backgroundColor: appcolor.lightgray, padding: 1 }}>
                                <TouchableOpacity style={{ width: '100%' }} onPress={() => showModalSelect(i)} >
                                    <FormGroup
                                        title={i.name}
                            ...
                            <NumberFormat
                                ...
                                renderText={value =>
                                    <FormGroup
                                        ...
                                        onSubmitEditing={() => handlerSubmitEditing(i, dataConfig[i + 1] !== undefined ? dataConfig[i + 1].ref_Id : 0)}
                    ...
                    UIItemView.push(
                        <View key={index} style={{ flex: 1 }}>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 8 }}>
                                <Text style={{ fontSize: 21, padding: 8, fontWeight: '700', color: appcolor.dark, textAlign: 'center' }}>{item.name}</Text>
                                <TouchableOpacity style={{ padding: 10 }} onPress={() => setDataModal({ ...dataModal, visible: false })}>
                                    <Icon name="times" size={25} color={appcolor.dark} />
                                </TouchableOpacity>
                            </View>
                            <FormGroup
                                containerStyle={styles.filterStyle}
                                placeholder={"Tìm kiếm ..."} editable handleChangeForm={handlerSearchModal} multiline iconName='search' />
                            <FlatList
                                style={{ flex: 1, padding: 8 }}
                                keyExtractor={(_, index) => index.toString()}
                                data={ListItemValue(item)}
                                renderItem={({ item, index }) => renderItemModal(item, index, TYPE_SELLIN.DEALER)}
                    ...
                    UIItemView.push(
                        <View key={index} style={{ flex: 1 }}>
                            ...
                            <FlatList
                                ...
                                keyExtractor={(_, index) => index.toString()}
                                data={ListItemValue(item)}
                                renderItem={({ item, index }) => renderItemModal(item, index, TYPE_SELLIN.COMPETITOR)}
                                removeClippedSubviews={true}
                            />
                        </View>
                    )
                    ...
                    UIItemView.push(
                        <View key={index} style={{ flex: 1 }}>
                            ...
                            <FlatList
                                ...
                                keyExtractor={(_, index) => index.toString()}
                                data={ListItemValue(item)}
                                renderItem={({ item, index }) => renderItemModal(item, index, TYPE_SELLIN.CATEGORY)}
                                removeClippedSubviews={true}
                            />
                        </View>
                    )
                    ...
                    UIItemView.push(
                        <View key={index} style={{ flex: 1 }}>
                            ...
                            <FlatList
                                ...
                                keyExtractor={(_, index) => index.toString()}
                                data={ListItemValue(item)}
                                renderItem={({ item, index }) => renderItemModal(item, index, TYPE_SELLIN.SUB_CATEGORY)}
                                removeClippedSubviews={true}
                            />
                        </View>
                    )
                    ...
                    UIItemView.push(
                        <View key={index} style={{ flex: 1 }}>
                            ...
                            <FlatList
                                ...
                                keyExtractor={(_, index) => index.toString()}
                                data={ListItemValue(item)}
                                renderItem={({ item, index }) => renderItemModal(item, index, TYPE_SELLIN.SEGMENT)}
                                removeClippedSubviews={true}
                            />
                        </View>
                    )
                    ...
                    UIItemView.push(
                        <View key={index} style={{ flex: 1 }}>
                            ...
                            <FlatList
                                ...
                                keyExtractor={(_, index) => index.toString()}
                                data={ListItemValue(item)}
                                renderItem={({ item, index }) => renderItemModal(item, index, TYPE_SELLIN.SUB_SEGMENT)}
                                removeClippedSubviews={true}
                            />
                        </View>
                    )
                    ...
                    UIItemView.push(
                        <View key={index} style={{ flex: 1 }}>
                            ...
                            <FlatList
                                ...
                                keyExtractor={(_, index) => index.toString()}
                                data={ListItemValue(item)}
                                renderItem={({ item, index }) => renderItemModal(item, index, TYPE_SELLIN.PRODUCTS)}
                                removeClippedSubviews={true}
                            />
                        </View>
                    )
