...
const SellInShopDetailScreen = ({ navigation, route }) => {
    ...
    const { shopInfo, statusInfo, listReport, dataFilter: dataFilterRoute } = route.params
    ...
    useEffect(() => {
        LoadData();
        const reloadData = DeviceEventEmitter.addListener('RELOAD_DATA_SELLIN', LoadData);
    ...
    const LoadData = async () => {
        ...
        try {
            ...
            await REPORT.GetDataReportByShop_RealTime(dataFilter, async (mData, message) => {
                ...
                const jsonData = itemData?.jsonData ? JSON.parse(itemData.jsonData || '[]') : (Array.isArray(mData) ? mData : []);
    ...
    const handlerGetShopOrders = (jsonData) => {
        ...
        if (jsonData[0]?.dataShop !== undefined) {
            const statusData = jsonData.find(item => item.statusId == statusInfo.statusId) || {};
            const dataShop = statusData.dataShop || _.flatMap(jsonData, item => item.dataShop || []);
            const shopData = dataShop.find(item => item.shopId == shopInfo.shopId) || {};
            return handlerBuildOrderList(shopData);
        }
        if (jsonData[0]?.dataOrders !== undefined || jsonData[0]?.dataDetail !== undefined) {
            const shopData = jsonData.find(item => item.shopId == shopInfo.shopId) || {};
            return handlerBuildOrderList(shopData);
        }
    ...
    const handlerGetOrderDate = (item) => {
        ...
        return firstItem.date ? moment(firstItem.date).format('DD/MM/YYYY') : firstItem.workDate;
    };
    ...
    const handlerGetDeleteItems = () => {
        return _.flatMap(data.filter(item => item.isChoose === 1), item => item.dataDetail || []);
    };
    ...
    const handlerGetConfirmInfo = (item) => {
        return _.find(item.dataDetail || [], detail => detail.confirmBy != null) || {};
    };
    ...
    const renderProductDetail = (detail, idx) => (
        <View key={`${detail.orderId || idx}_detail`} style={styles.productDetail}>
            {!!detail.confirmTime && <RenderItemText appcolor={appcolor} type="Info" titleName="Thời gian xác nhận: " itemValue={moment(detail.confirmTime).format('HH:mm - DD/MM/YYYY')} />}
            {!!detail.confirmNote && <RenderItemText appcolor={appcolor} type="Info" titleName="Ghi chú xác nhận: " itemValue={detail.confirmNote} />}
            <RenderItemText appcolor={appcolor} type="Info" titleName="Mã sản phẩm: " itemValue={detail.productId} />
            <RenderItemText appcolor={appcolor} type="Info" titleName="Sản phẩm: " itemValue={`${detail.categoryName || ''} ${detail.productName || ''}`} />
            <RenderItemText appcolor={appcolor} type="Info" titleName="Số lượng: " itemValue={detail.quantityValue || 0} />
            {(detail.price !== null && detail.price !== undefined && detail.price !== 0) && <RenderItemText appcolor={appcolor} type="Price" titleName="Giá: " itemValue={detail.price} />}
            {(detail.revenue !== null && detail.revenue !== undefined) && <RenderItemText appcolor={appcolor} type="Price" titleName="Doanh thu: " itemValue={detail.revenue} />}
        </View>
    );
    ...
    const onDelete = async () => {
        ...
        alertConfirm('Xóa dữ liệu', `Bạn có chắc chắn muốn xoá hoá đơn đã chọn không ?`, async () => {
    ...
    const styles = StyleSheet.create({
        ...
        shopHeader: { margin: 16, padding: 16, borderRadius: 16, backgroundColor: appcolor.surface },
        shopName: { fontSize: 20, lineHeight: 26, fontWeight: '600', letterSpacing: -0.3, color: appcolor.dark },
        shopMeta: { fontSize: 14, lineHeight: 18, fontWeight: '400', color: appcolor.placeholderText, marginTop: 4 },
        shopRevenue: { fontSize: 14, lineHeight: 18, fontWeight: '700', color: appcolor.primary, marginTop: 4 },
        shopAddress: { fontSize: 12, lineHeight: 16, fontWeight: '400', color: appcolor.greydark, marginTop: 8 },
        statusText: { fontSize: 12, lineHeight: 16, fontWeight: '600', color: appcolor.primary, marginTop: 8 },
        itemStyle: { position: 'relative', backgroundColor: appcolor.surface, marginHorizontal: 8, marginBottom: 8, padding: 12, paddingRight: 56, borderRadius: 16 },
        textButtonAction: { color: appcolor.light, fontSize: 12, lineHeight: 16, fontWeight: '600', borderRadius: 6, borderWidth: 0.5, padding: 8 },
        buttonAction: { minHeight: 44, justifyContent: 'center', marginRight: 8 },
        groupPhotoContainer: { width: '100%', marginTop: 12, paddingTop: 12, gap: 8, borderTopWidth: 0.5, borderTopColor: appcolor.grayLight },
        productDetail: { paddingVertical: 8, borderTopWidth: 0.5, borderTopColor: appcolor.grayLight },
        orderTitle: { fontSize: 16, lineHeight: 22, fontWeight: '600', color: appcolor.dark },
    ...
    const renderItem = ({ item, index }) => {
        ...
        const onShowPhoto = (photoIndex) => setPhotoViewer({ visible: true, photos, index: photoIndex });
        ...
        return (
            <TouchableOpacity
                activeOpacity={0.7}
                onLongPress={handlerCanDeleteOrder(item) ? () => onLongPressDel(item) : null}
                onPress={() => handlerToggleOrder(item, index)}
            >
                <View style={styles.itemStyle}>
                    {!!confirmInfo.statusOrder && <RenderItemText appcolor={appcolor} type="Status" titleName="Trạng thái: " itemValue={confirmInfo.statusOrder} colorByRow={confirmInfo.colorStatus} />}
                    {confirmInfo.confirmBy != null && <RenderItemText appcolor={appcolor} type="Status" titleName="Xác nhận bởi " itemValue={confirmInfo.confirmByName || confirmInfo.confirmBy} colorByRow={confirmInfo.colorStatus} />}
                    <Text style={styles.orderTitle}>{item.orderNo || 'Hóa đơn'}</Text>
                    <RenderItemText appcolor={appcolor} type="Info" titleName="Ngày: " itemValue={handlerGetOrderDate(item)} />
                    <RenderItemText appcolor={appcolor} type="Info" titleName="Tổng số lượng: " itemValue={handlerGetOrderQuantity(item)} />
                    <RenderItemText appcolor={appcolor} type="Price" titleName="Tổng doanh thu: " itemValue={item.totalOrderRevenue || _.sumBy(item.dataDetail || [], it => Number(it.revenue || 0))} />
                    {isExpand && (
                    ...
                    {_.some(item.dataDetail || [], detail => detail.isConfirm == 1) && (
                        <View style={{ borderTopWidth: 0.3, borderTopColor: appcolor.greydark, marginTop: 8 }}>
                            <ScrollView style={{ width: '100%', padding: 8 }} horizontal showsHorizontalScrollIndicator={false}>
                                {dataConfirmValue.map((it, idx) => (
                                    <TouchableOpacity key={`${idx}_confirm`} onPress={() => handlerUpdateOrderDetail(item, it.numberValue, it.name)} style={styles.buttonAction}>
                    ...
                    {item.isChoose != 1 && (
                        <Badge
                            ...
                            badgeStyle={{ backgroundColor: confirmInfo.confirmBy != null ? appcolor.success : appcolor.primary, height: 40, width: 40, borderRadius: 20 }}
    ...
    return (
        <View style={styles.mainContainer}>
            {isDelete > 0 ? (
                <HeaderCustom title={titleDelete} rightFunc={onDelete} iconRight="trash" />
            ) : (
                <HeaderCustom
            ...
            <View style={styles.shopHeader}>
                ...
                {shopInfo.totalRevenue !== null && shopInfo.totalRevenue !== undefined && (
                    <NumericFormat
                        value={shopInfo.totalRevenue}
                        displayType="text"
                        thousandSeparator
                        renderText={formattedValue => (
                            <Text style={styles.shopRevenue}>{`Doanh thu: ${formattedValue} đ`}</Text>
                        )}
                    />
                )}
                {!!shopInfo.dealerName && <Text style={styles.shopMeta}>{`NPP: ${shopInfo.dealerName}`}</Text>}
                {!!shopInfo.addressName && <Text style={styles.shopAddress}>{shopInfo.addressName}</Text>}
                {!!statusInfo.statusName && <Text style={styles.statusText}>{statusInfo.statusName}</Text>}
            </View>
            <CustomListView
                data={data}
                ...
                ListEmpty={!loading ? <Text style={styles.shopMeta}>Không có dữ liệu</Text> : null}
            />
            <ViewPictures
...
const RenderItemText = ({ type, titleName, itemValue, appcolor, colorByRow = 'dark' }) => {
    const colorItem = appcolor.dark;
    return (
        <View style={{ width: '100%', height: 'auto' }}>
            <View style={{ width: '90%', padding: 3 }}>
                {type == 'Status' && <Text style={{ fontSize: 13, lineHeight: 18, fontWeight: '700', color: appcolor[colorByRow || 'dark'], fontStyle: 'italic' }}>{titleName}{itemValue}</Text>}
                {type == 'Info' && <Text style={{ width: '100%', fontSize: scaleSize(12), lineHeight: 18, color: colorItem }}>{titleName}{itemValue}</Text>}
                {type == 'Price' && (
                    <NumericFormat
                        value={itemValue}
                        displayType="text"
                        thousandSeparator
                        renderText={formattedValue => (
                            <Text style={{ fontWeight: '700', color: appcolor.dark, fontSize: scaleSize(13), lineHeight: 18 }}>
