...
import { DeviceEventEmitter, Platform, SafeAreaView, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
const SellInShopScreen = ({ navigation, route }) => {
    ...
    const LoadData = async (dataChoose = itemShopChoose) => {
        ...
        const dataStoreChoose = lstStore.map(item => ({ ...item, isChoose: item.id == dataChoose.ShopId ? 1 : 0 }));
    ...
    const handlerBuildStatusList = (jsonData) => {
        ...
        return [{ statusId: 'ALL', statusName: 'Tất cả', statusTabName: 'Tất cả', dataShop, totalShop: dataShop.length, totalOrder: _.sumBy(dataShop, item => item.totalOrder || 0), totalRevenue: _.sumBy(dataShop, item => item.totalRevenue || 0) }];
    };

    const handlerBuildShopList = (jsonData) => {
        if (!Array.isArray(jsonData)) return [];
        const validData = jsonData.filter(item => item && (item.shopId || item.ShopId));
        if (validData.length === 0) return [];
        if (validData[0]?.dataOrders !== undefined) {
            return validData.map(item => {
                const dataOrders = item.dataOrders || [];
                const dataDetail = _.flatMap(dataOrders, order => order.dataDetail || []);
                return {
                    ...item,
                    dataOrders,
                    dataDetail,
                    totalOrder: dataOrders.length,
                    totalQuantity: _.sumBy(dataDetail, it => Number(it.quantityValue || 0)),
                    totalRevenue: _.sumBy(dataOrders, order => Number(order.totalOrderRevenue || 0)) || _.sumBy(dataDetail, it => Number(it.revenue || 0))
                };
            });
        }
        if (validData[0]?.dataDetail !== undefined) {
            return validData.map(item => ({
                ...
                dataOrders: item.dataOrders || [{ orderNo: item.orderNo, totalOrderRevenue: _.sumBy(item.dataDetail || [], it => Number(it.revenue || 0)), dataDetail: item.dataDetail || [] }],
                totalOrder: item.dataOrders?.length || 1,
                totalQuantity: _.sumBy(item.dataDetail || [], it => Number(it.quantityValue || 0)),
                totalRevenue: _.sumBy(item.dataDetail || [], it => Number(it.revenue || 0))
            }));
        }
        ...
        return Object.keys(groupList).map(shopId => {
            ...
            return {
                ...
                totalOrder: dataOrders.length,
                totalQuantity: _.sumBy(groupList[shopId], it => Number(it.quantityValue || 0)),
    ...
    const handlerChooseStore = (item) => {
        ...
        const dataChange = dataStore.map(it => ({ ...it, isChoose: it.id === item.id ? isChoose : 0 }));
        setChooseShop({ ...itemShopChoose, ShopName: isChoose == 1 ? item.name : 'Tìm kiếm', ShopId: isChoose == 1 ? item.id : 0 });
    ...
    const handlerSelectCalendar = async (date) => {
        ...
        if (dateString !== null && dateString !== undefined) {
            ...
            if (!dataCalendar.isStartDay) {
                setDataCalendar({
                    ...dataCalendar,
                    markedDates: { [dateString]: { startingDay: true, color: appcolor.warning, textColor: appcolor.dark } },
                    markingType: 'period',
                    isStartDay: true,
                    isEndDay: false,
                ...
                if (range > 0) {
                    for (let i = 1; i <= range; i++) {
                        const tempDate = moment(startDate.add(1, 'day')).format('YYYY-MM-DD');
                        markedDates[tempDate] = i < range
                            ? { color: appcolor.warningLight, textColor: appcolor.dark }
                            : { endingDay: true, color: appcolor.warning, textColor: appcolor.dark };
                    }
                    setDataCalendar({
                        ...dataCalendar,
                        markedDates,
    ...
    const handlerFilterShopInput = (value) => {
        ...
        const dataFilter = valueSearch
            ? dataStoreFilter.filter(item => removeVietnameseTones(item.name || '').toLowerCase().includes(valueSearch))
            : dataStoreFilter;
    ...
    const onOpenDetail = (item, statusInfo) => {
        navigation.navigate('sellinshopdetail', { shopInfo: item, statusInfo, listReport, dataFilter: itemShopChoose });
    };
    ...
    const styles = StyleSheet.create({
        ...
        card: { minHeight: 96, padding: 16, marginBottom: 12, borderRadius: 16, backgroundColor: appcolor.surface, borderWidth: 0.5, borderColor: appcolor.grayLight },
        cardTop: { flexDirection: 'row', alignItems: 'center' },
        iconBox: { width: 44, height: 44, borderRadius: 16, backgroundColor: appcolor.primary, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
        shopInfo: { flex: 1 },
        shopName: { fontSize: 16, lineHeight: 22, fontWeight: '600', color: appcolor.dark },
        shopCode: { fontSize: 12, lineHeight: 16, fontWeight: '500', color: appcolor.placeholderText, marginTop: 2 },
        shopMeta: { fontSize: 12, lineHeight: 16, fontWeight: '500', color: appcolor.greydark, marginTop: 4 },
        shopAddress: { fontSize: 12, lineHeight: 16, fontWeight: '400', color: appcolor.placeholderText, marginTop: 4 },
        statRow: { flexDirection: 'row', gap: 8, marginTop: 12 },
        statChip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 9999, backgroundColor: appcolor.light },
        statText: { fontSize: 12, lineHeight: 16, fontWeight: '500', color: appcolor.dark },
        emptyText: { fontSize: 14, lineHeight: 18, fontWeight: '400', color: appcolor.placeholderText, textAlign: 'center', marginTop: 48 },
        filterSheet: { width: '100%', height: deviceHeight, padding: 12, backgroundColor: appcolor.light },
        filterHeader: { width: '100%', flexDirection: 'row', padding: 12, alignItems: 'center' },
        filterTitle: { flex: 1, fontSize: 16, lineHeight: 22, fontWeight: '600', color: appcolor.dark },
        divider: { borderWidth: 1, borderColor: appcolor.surface, width: '100%' },
        itemShopStyle: { width: '100%', borderRadius: 12, padding: 12, marginBottom: 8, backgroundColor: appcolor.surface },
        itemShopName: { width: '100%', fontSize: 16, lineHeight: 22, fontWeight: '400', color: appcolor.dark },
    ...
    const renderItemShop = ({ item }) => {
        ...
        return (
            <TouchableOpacity activeOpacity={0.7} onPress={onPressItem}>
                <View style={{ ...styles.itemShopStyle, backgroundColor: item.isChoose == 1 ? appcolor.yellowdark : appcolor.surface }}>
    ...
    const renderShopItem = (statusInfo) => ({ item }) => (
        <TouchableOpacity activeOpacity={0.7} style={styles.card} onPress={() => onOpenDetail(item, statusInfo)}>
            <View style={styles.cardTop}>
                ...
                <View style={styles.shopInfo}>
                    ...
                    {!!item.shopCode && <Text style={styles.shopCode}>{item.shopCode}</Text>}
                    {!!item.dealerName && <Text style={styles.shopMeta} numberOfLines={1}>{`NPP: ${item.dealerName}`}</Text>}
                    {!!item.addressName && <Text style={styles.shopAddress} numberOfLines={2}>{item.addressName}</Text>}
                </View>
                <Icon type="ionicon" name="chevron-forward" size={20} color={appcolor.placeholderText} />
            </View>
            <View style={styles.statRow}>
                <View style={styles.statChip}>
                    <Text style={styles.statText}>{`${item.totalOrder || 0} đơn hàng`}</Text>
                </View>
                <View style={styles.statChip}>
                    <Text style={styles.statText}>{`${item.totalQuantity || 0} sản phẩm`}</Text>
                </View>
                <View style={styles.statChip}>
                    <Text style={styles.statText}>{`${(item.totalRevenue || 0).toLocaleString()} đ`}</Text>
                </View>
            </View>
        </TouchableOpacity>
    );
    ...
    const renderTab = (item) => (
        <CustomListView
            ...
            onRefresh={LoadData}
            isRefresh={loading}
            ListEmpty={!loading ? <Text style={styles.emptyText}>Không có dữ liệu cửa hàng</Text> : null}
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            {handlerHasDataShop() ? (
                <CustomTab
                    data={data}
                    dataMain={_.flatMap(data, item => item.dataShop?.map(shop => ({ ...shop, statusTabName: item.statusTabName })) || [])}
                ...
                <SafeAreaView style={styles.filterSheet}>
                    <View style={styles.filterHeader}>
                        ...
                        <TouchableOpacity activeOpacity={0.7} onPress={downloadDataByShop}>
                            <Icon type="font-awesome-5" name="search" size={20} color={appcolor.primary} />
                        </TouchableOpacity>
                    </View>
