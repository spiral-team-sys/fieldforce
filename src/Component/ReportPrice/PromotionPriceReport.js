...
import {
  clearAllPromotionPrice,
  clearPromotionPriceByCat,
  getListPromotion,
  getPromotionResult,
  updateItemPricePromotion,
  uploadPromotionPrice,
...
const parsePriceText = text => {
  const value =
    text !== null && text !== undefined && text.toString().length > 0
...
const getPromotionInputRows = (rows = []) => {
  return rows.filter(
    it =>
      hasPriceValue(it.retailPrice) ||
      hasPriceValue(it.discountPrice) ||
...
export const PromotionPriceReport = ({ navigation, route }) => {
  ...
  const loadData = async () => {
    ...
    const competitors = await getCompetitorProductBy(
      _competitorId,
      lstReport?.isByType || 0,
    ); //list competitor
    const dataTab = await getlistTabCompetitor(
      _competitorId,
      lstReport?.isByType || 0,
    );
    const lstTabByCompe = dataTab?.filter(
    ...
    setData({
      ...
      dataShow: dataShow,
      dataShowF: dataShow,
      dataTabByCompe: lstTabByCompe,
      dataCompetitor: competitors,
    });
  ...
  const uploadAction = async () => {
    ...
    let checkPrice = resPrice.filter(
      it =>
        it.retailPrice !== 'null' &&
        it.retailPrice !== null &&
        it.discountPrice !== null &&
        it.discountPrice !== 'null' &&
        (it.discountPrice < 10000 ||
          it.discountPrice % 1000 > 0 ||
          it.retailPrice < 10000 ||
          it.retailPrice % 1000 > 0),
    );
    if (checkPrice.length > 0) {
      let product = data.dataShowF.filter(
    ...
    let checkPricePromotion = resPrice.filter(
      it =>
        it.retailPrice !== 'null' &&
        it.retailPrice !== null &&
        it.retailPrice > 0 &&
        it.promotionType > 0 &&
        it.promotionType !== 2 &&
        (it.discountPrice === 'null' || it.discountPrice === null),
    );
    if (checkPricePromotion.length > 0) {
      let product = data.dataShowF.filter(
        it => it.productId === checkPricePromotion[0].productId,
      );
      toastError(
        'Thông báo',
    ...
    let items = resPrice.filter(
      it =>
        (it.retailPrice === 'null' || it.retailPrice === null) &&
        it.discountPrice !== 'null' &&
        it.discountPrice !== null &&
        it.discountPrice > 0,
    );
    ...
    let checkDiscountPrice = resPrice.filter(
      it =>
        it.retailPrice !== 'null' &&
        it.retailPrice !== null &&
        it.discountPrice !== null &&
        it.discountPrice !== 'null' &&
        it.retailPrice < it.discountPrice,
    );
    if (checkDiscountPrice.length > 0) {
      let product = data.dataShowF.filter(
        it => it.productId === checkDiscountPrice[0].productId,
      );
      toastError(
        'Thông báo',
    ...
    let checkPromotionType = resPrice.filter(
      it =>
        ((it.retailPrice !== 'null' && it.retailPrice !== null) ||
          (it.discountPrice !== null && it.discountPrice !== 'null')) &&
        it.promotionType === 0,
    ...
    if (checkPromotionType.length > 0) {
      let product = data.dataShowF.filter(
        it => it.productId === checkPromotionType[0].productId,
      );
      toastError(
        'Thông báo',
    ...
    let checkIsSelectType = resPrice.filter(
      it =>
        (it.retailPrice === 'null' || it.retailPrice === null) &&
        (it.discountPrice === null || it.discountPrice === 'null') &&
        it.promotionType > 0,
    ...
    if (checkIsSelectType.length > 0) {
      let product = data.dataShowF.filter(
        it => it.productId === checkIsSelectType[0].productId,
      );
      toastError(
        'Thông báo',
    ...
    let checkLenghtGiftName = resPrice.filter(
      it =>
        it.promotionType == 2 &&
        it.giftName?.length > 0 &&
        it.giftName?.length < 5,
    );
    if (checkLenghtGiftName.length > 0) {
      let product = data.dataShowF.filter(
        it => it.productId === checkLenghtGiftName[0].productId,
      );
      toastError(
        'Thông báo',
    ...
    let checkPromotionGift = resPrice.filter(
      it =>
        it.promotionType == 2 &&
        (it.giftName == null ||
          it.giftName === '' ||
          (it.giftName?.length > 0 && it.giftName?.length < 5)),
    );
    if (checkPromotionGift.length > 0) {
      let product = data.dataShowF.filter(
        it => it.productId === checkPromotionGift[0].productId,
      );
      toastError(
        'Thông báo',
    ...
    let itemsUpload = resPrice.filter(
      it => it.retailPrice !== 'null' && it.retailPrice !== null,
    );
...
const ViewContent = ({ data, Status }) => {
  ...
  const setClearAll = async () => {
    if (Status !== 1) {
      ...
      toastError(
        'Cảnh báo',
        'Dữ liệu đã được gửi lên hệ thống bạn không thể xóa!',
      );
      SheetManager.hide('actionStock');
    }
  };
  const handlerClearByCategory = async itemCategory => {
    if (Status != 1) {
      MessageAction(
        `Bạn có muốn xoá dữ liệu ngành hàng ${itemCategory.categoryName} thuộc hãng ${itemCategory.division} đã nhập không ?`,
        async () => {
  ...
  const clearDataByCate = itemCategory => {
    data.dataShow.map(it => {
      if (
        ...
        it.divisionId == itemCategory.divisionId
      ) {
        it.retailPrice = null;
        it.discountPrice = null;
        it.promotionType = 0;
  ...
  const ViewItem = () => {
    return dataTab.map((it, indexCate) => {
      ...
      dataByCategoryId = data.dataShow?.filter(
        i =>
          i.categoryName === it.categoryName && i.divisionId == it.divisionId,
      );
      const totalRow = dataByCategoryId?.length || 0;
      return (
        <Tabs.Tab
          key={`TV_IIT_${indexCate}`}
...
const ViewListItem = ({ dataByCategoryId, indexCate, data, isClearByCate }) => {
    ...
    ({ item, index }) => {
      return (
        <RenderItemProduct
          ...
          data={data}
          appcolor={appcolor}
          listPromotiponType={listPromotiponType}
          isClearByCate={isClearByCate}
        />
      );
    },
  ...
  return (
    <Tabs.FlatList
      ...
      ListFooterComponent={
        <Text style={{ height: deviceHeight / 2, textAlign: 'center' }}>
          Đã xem hết
        </Text>
      }
...
const RenderItemProductComponent = ({
  item,
  index,
  data,
  listPromotiponType,
  ...
  const [inputDiscountPrice, setInputDiscount] = useState(
    item.discountPrice || '',
  );
  const [promotionType, setPromotionType] = useState(item.promotionType || 0);
  const [inputGiftName, setInputGiftName] = useState(
    item.giftName != 'null' ? item.giftName : '',
  );
  ...
  useEffect(() => {
    if (
      ...
      isClearByCate.isClear - countClear.current > 0
    ) {
  ...
  const syncPriceToData = useCallback(
    (priceType, intValue, isError) => {
      ...
      if (index >= 0) {
        ...
        data.dataShow[index][
          priceType === 'retailPrice' ? 'retailError' : 'discountError'
        ] = isError;
      }
      if (indexF >= 0) {
        data.dataShowF[indexF][priceType] = intValue;
        data.dataShowF[indexF][
          priceType === 'retailPrice' ? 'retailError' : 'discountError'
        ] = isError;
      }
    },
  ...
  const syncPriceErrorsToData = useCallback(
    (nextRetailError, nextDiscountError) => {
  ...
  const changeValuePromotion = useCallback(
    (text, priceType) => {
      ...
      const nextRetailPrice =
        priceType === 'retailPrice' ? intValue : item.retailPrice;
      const nextDiscountPrice =
        priceType === 'discountPrice' ? intValue : item.discountPrice;
      const nextRetailError = getPriceError(
        nextRetailPrice,
        nextRetailPrice,
        nextDiscountPrice,
      );
      const nextDiscountError = getPriceError(
        nextDiscountPrice,
        nextRetailPrice,
        nextDiscountPrice,
      );
      const isError =
        priceType === 'retailPrice' ? nextRetailError : nextDiscountError;
  ...
  const endInputPromotion = useCallback(
    async (e, priceType) => {
      ...
      if (intValue !== null && intValue < 10000) {
        ...
        if (priceType === 'discountPrice') {
          toastError(
            'Cảnh báo',
            'Nhập giá Khuyến mãi không được lớn hơn giá niêm yết!',
            'top',
          );
        } else {
          toastError(
      ...
      const nextDiscountPrice =
        priceType === 'discountPrice' ? intValue : item.discountPrice;
      const nextRetailError =
        priceType === 'retailPrice'
          ? isError
          : getPriceError(nextRetailPrice, nextRetailPrice, nextDiscountPrice);
      const nextDiscountError =
  ...
  const onChangeGiftName = useCallback(
    text => {
      item.giftName = text || '';
      const index = data.dataShow.findIndex(it => it.Id === item.Id);
      const indexF = data.dataShowF.findIndex(it => it.Id === item.Id);
  ...
  const onEndChangeGiftName = useCallback(
    async e => {
      let value =
  ...
  const discountPriceText = useMemo(
    () => formatPriceText(inputDiscountPrice),
    [inputDiscountPrice],
  );
  return (
    <View key={'item__ii_' + index} style={styles.card}>
      ...
      <View style={styles.controlRow}>
        ...
        <TextInput
          ...
          style={[
            ...
            {
              backgroundColor:
                retailError === 1 ? appcolor.warning : appcolor.light,
            },
          ]}
        ...
        <TextInput
          ...
          style={[
            ...
            {
              backgroundColor:
                discountError === 1 ? appcolor.warning : appcolor.light,
            },
          ]}
          keyboardType="numeric"
      ...
      {promotionType === 2 && (
        <FormGroup
          ...
          rightFunc={() =>
            Platform.OS === 'ios' ? onChangeGiftName('') : null
          }
        ...
        <View style={styles.sheetContent}>
          <View style={styles.sheetHeader}>
            ...
            <Text style={styles.sheetSubTitle}>
              Chạm lại loại đang chọn để bỏ chọn
            </Text>
          </View>
    ...
    return (
      <TouchableOpacity onPress={actionPress}>
        <View style={[styles.actionRow, isShowInput && styles.actionRowActive]}>
          ...
          <Text
            numberOfLines={2}
            style={[styles.titleAction, isDanger && styles.dangerText]}
          >
            {title}
          </Text>
          {isShowInput && (
