...
const PriceComperitor = ({ navigation }) => {
  ...
  const getListByCompetitor = async () => {
    const lstData = await getListPriceCompetitor(
      workinfo,
      listConfig?.isAllCompetitor == 1,
    );
    return lstData;
  ...
  const UploadData = async () => {
    ...
    await dataAll.dataCompetitor.forEach(async it => {
      ...
      const dataMainById = dataAll.dataProduct?.filter(
  ...
  const checkFormatNumber = async (event, index, type, item) => {
    ...
    if (mPrice % 1000 > 0 || mPrice == null) {
      ...
      if (mPrice == null) {
        ToastError(
          `${index + 1}. ${item.productName}: Chưa nhập ${textAlert}`,
          'Chưa nhập liệu',
          'top',
        );
      } else if (mPrice >= 1000) {
        ToastError(
          `${index + 1}. ${
            item.productName
          }: ${textAlert} không được nhập số lẻ`,
          'Lỗi nhập số lẻ',
          'top',
        );
      } else {
        ToastError(
          `${index + 1}. ${item.productName}: ${textAlert} nhập sai định dạng`,
          'Sai định dạng',
          'top',
        );
      }
  ...
  const filterProduct = debounce(async str => {
    ...
    if (str !== null && str !== undefined && str.length > 0)
      mDataFilter = dataAll.mDataProduct.filter(i =>
        i.productName.toLowerCase().match(str.toLowerCase()),
      );
    else mDataFilter = dataAll.mDataProduct;
  ...
  const styles = StyleSheet.create({
    ...
    inputNumber: {
      ...
      borderBottomColor: appcolor.placeholderText,
      padding: 5,
      borderRadius: 6,
      textAlign: 'center',
    },
  ...
  const renderItem = ({ item, index, totalRow }) => {
    ...
    return (
      <View key={'k' + index} style={{ width: '100%', borderRadius: 5 }}>
        {item.isParent && (
          <Text style={styles.titleGroupCategory}>{item?.categoryName}</Text>
        )}
        <View style={{ flex: 1 }}>
          {keyLayer2 && item.subCatId !== 0 && item.subCatId !== null && (
            ...
            >
              <SpiralIcon
                name="tags"
                style={{ padding: 8, color: appcolor.info }}
              />
              <Text
                style={styles.titleSubCategory}
            ...
            <View style={styles.viewInputPrice}>
              {listConfig.priceValue == 1 && (
                <RenderInputNumber
                  ...
                  editable={
                    statusReport.isOldDay ? false : !statusReport.isUpload
                  }
  ...
  const renderTabView = () => {
    ...
    return dataAll.dataCompetitor?.map((itemTab, indexTab) => {
      ...
      dataByCategoryId = dataAll.dataProduct?.filter(
        i => i.competitorId === itemTab.competitorId,
      );
      // }
      ...
      return (
        <Tabs.Tab
          key={itemTab.competitorId}
          label={`${itemTab.competitorName} (${totalRow}) `}
          name={`${itemTab.competitorName} (${totalRow}) `}
        >
          <View
            key={indexTab.toString()}
            style={{
              backgroundColor: appcolor.light,
              marginTop: 35,
              width: deviceWidth,
  ...
  const handleSendReport = async () => {
    ...
    const listFilter = lstUpload.filter(
      it =>
        it.netValue !== null && it.netValue !== undefined && it.netValue !== '',
    );
    if (listFilter.length == 0) {
      alertWarning(
        'Bạn chưa nhập giá của đối thủ. Vui lòng nhập giá của đối thủ trước khi gửi báo cáo.',
      );
      return;
    }
  ...
  return (
    ...
    >
      <HeaderCustom
        ...
        rightFunc={
          statusReport.isOldDay
            ? null
            : !statusReport.isUpload
            ? UploadData
            : null
        }
        iconRight={
        ...
        <Tabs.Container
          renderTabBar={props => (
            <MaterialTabBar
              ...
              tabStyle={{
                backgroundColor: appcolor.light,
                minWidth: minWidthTab(dataAll.dataCompetitor),
                height: 38,
              }}
              ...
              styles.inputNumber,
              {
                backgroundColor:
                  ...
                  (dataColor.type !== type &&
                    ...
                    dataColor.categoryName !== itemInput.categoryName &&
