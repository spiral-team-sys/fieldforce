...
export const ViewPriceCompetitorDealer = memo(({ navigation, route, data, dataByDealer }) => {
    ...
    const Styles = StyleSheet.create({
        ...
        searchStyle: { borderColor: appcolor.grayLight, borderWidth: 0.5, backgroundColor: appcolor.light, padding: 3, width: '85%', marginBottom: 0 },
        ...
        progressStyle: { alignItems: 'center', alignSelf: "center", marginTop: deviceHeight / 3 },
        ...
        viewTabStyle: { backgroundColor: appcolor.surface, marginTop: 42, padding: 5, width: deviceWidth, height: deviceHeight * 0.8, display: !showProgress ? 'flex' : 'none' },
        ...
        itemStyle: {
            height: dataView.listInput?.length < 4 ?
                (dataView.listConfig?.isNoteBySKU == 1 ? (dataView.listInput?.length > 1 ? 190 : 150) : 110)
                : (dataView.listConfig?.isNoteBySKU == 1 ? (dataView.listInput?.length > 1 ? 230 : 190) : 150), backgroundColor: appcolor.light, justifyContent: 'center', padding: 5, margin: 5, elevation: 2, borderRadius: 10
        },
    ...
    const getItemLayout = (data, index) => ({
        length: dataView.listConfig?.isNoteBySKU == 1 ? (dataView.listInput.length > 1 ? 210 : 180) : 150,
        offset: (dataView.listConfig?.isNoteBySKU == 1 ? (dataView.listInput.length > 1 ? 210 : 180) : 150) * index,
        index
    })
    const ViewItem = () => {
        return (
            dataView.listCompetitor?.map(it => {
                ...
                return (
                    <Tabs.Tab key={it.CompetitorName + `(${totalRow})`} label={it.CompetitorName + `(${totalRow})`} name={it.CompetitorName + `(${totalRow})`}>
    ...
    const contains = (item, query) => {
        const { ProductCode, ProductName, Category, Segment, SubCateName, SubCategory } = item;
        ...
        return removeVietnameseTones(SproductCode)?.match(queryRemove) ||
                    ...
                    <Tabs.Container
                        ...
                        renderTabBar={(props) => (
                            <MaterialTabBar
                                ...
                                scrollEnabled={true}
                                tabStyle={{
                                    borderRadius: 8, backgroundColor: appcolor.light, minWidth: minWidthTab(dataView.listCompetitor),
            ...
            <ActionSheet
                ...
                <View style={Styles.actionSheetStyle}>
                    <View style={{ width: '100%' }}>
                        ...
                        <TouchableOpacity onPress={filterDoneProduct} style={{ marginTop: 8 }}>
                            <View style={{
                        ...
                        <TouchableOpacity onPress={filterSelect} style={{ marginTop: 8 }}>
                            <View style={{
                                backgroundColor: isSelect ? appcolor.light : appcolor.surface,
                                ...
                                <Icon name={'keyboard'} size={18} color={appcolor.success} />
                                <Text style={{ width: '100%', fontSize: 14, fontWeight: '400', color: appcolor.dark, padding: 8 }}>Xem dữ liệu đã chọn</Text>
                            </View>
                        </TouchableOpacity>
                        {
...
const RenderItem = ({ item, index, data, dataView, listDataByCompe, Styles, workinfo, listInput, appcolor, Status, lstReport, listMaster }) => {
    ...
    const handleSelectCheckBox = async () => {
        ...
        const index = dataView.listCompetitor.findIndex(it => it.CompetitorId == item.Type)
        const indexF = dataView.listCompetitorF.findIndex(it => it.CompetitorId === item.Type)
        ...
        <View key={'itemProduct_' + item.productId + index} style={{ width: '100%' }}>
            {item.isParent &&
                <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Text style={{ color: DEFAULT_COLOR, fontSize: 16, fontWeight: '700' }}>{item.SubCategory}</Text>
                </View>
            }
            ...
            <View style={Styles.itemStyle}>
                ...
                <View>
                    <FlatList
                        ...
                        getItemLayout={getItemLayout}
                        removeClippedSubviews={true}
                        renderItem={({ item, index }) => <RenderItemInput itemInput={item} indexInput={index} item={itemReport} Styles={Styles} listInput={listInput} appcolor={appcolor} data={data} workinfo={workinfo} Status={Status} lstReport={lstReport} dataView={dataView} listDataByCompe={listDataByCompe} />}
                        style={{ padding: 5 }}
                ...
                {
                    dataView.listNote.length > 0 && <RenderItemNote listNote={dataView.listNote} appcolor={appcolor} item={itemReport} listMaster={listMaster} data={data} workinfo={workinfo} Status={Status} listDataByCompe={listDataByCompe} dataView={dataView} />
                }
            </View>
        </View>
    )
}
const RenderItemNote = ({ listNote, appcolor, item, listMaster, data, workinfo, Status, listDataByCompe, dataView }) => {
                ...
                listNote?.map((it, idx) => {
                    return it.typeValue == 'selectText' ?
                        <View key={`${item.productId}_${it.noteType}`}>
                            <TouchableOpacity key={it.noteType}
                                onPress={() => dataView.isUploaded !== 1 ? showListSelect(it) : null} style={{
                    ...
                    <ScrollView style={{ marginHorizontal: 10 }}>
                        {
                            showList.dataShow?.map((itSelect, idxSelect) => {
                                return (
                                    <TouchableOpacity key={itSelect.id} onPress={() => selectByList(itSelect)} style={{
...
const RenderItemInput = ({ itemInput, indexInput, item, Styles, listInput, appcolor, data, workinfo, Status, lstReport, dataView, listDataByCompe }) => {
    const [input, setInput] = useState(itemInput.isZero == 1 ? (item[itemInput.displayType] == 0 ? 0 : item[itemInput.displayType]) : item[itemInput.displayType])
    ...
    const endInput = async (e) => {
        ...
        let dataProduct = [...listDataByCompe]
        const indexP = dataProduct.findIndex(it => it.ProductId === item.ProductId)
        if (intValue < ((itemInput.min && itemInput.min !== '') ? itemInput.min : 1000)) {
            intValue = null;
            isError = 1;
            ToastError(`Nhập ${itemInput.name} không được nhỏ hơn ${itemInput.min || 1000}!`, "Lỗi", "top");
        } else if (intValue % ((itemInput.min && itemInput.min !== '') ? itemInput.min : 1000) > 0) {
            intValue = null;
            isError = 1;
            ToastError("Nhập số tiền không được lẻ!", "lỗi", "top");
        } else if (intValue > ((itemInput.max && itemInput.max !== '') ? itemInput.max : 1000000000)) {
            intValue = null;
            isError = 1;
            ToastError(`Nhập ${itemInput.name} không được lớn hơn ${itemInput.max || 1000000000}!`, "Lỗi", "top");
        } else {
            isError = 0
        }
        item[itemInput.displayType] = intValue
    ...
    return (
        <View style={[Styles.itemInputStyle, { width: ((indexInput > 2 && indexInput < 5) || (listInput.length % 2 === 0)) ? '49.5%' : '33%' }]}>
            ...
            <NumberFormat
                ...
                renderText={value =>
                    <TextInput
                        textAlign={'center'}
                        value={value}
                        style={[Styles.textInputStyle, { backgroundColor: isError === 1 ? appcolor.warning : appcolor.light }]}
