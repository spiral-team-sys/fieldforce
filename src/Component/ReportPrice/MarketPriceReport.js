...
const MarketPriceReport = ({ navigation, route }) => {
  ...
  const LoadData = async () => {
    ...
    const lstTabByCompe = lstTabList?.filter(
    ...
    await setDataCompetitor(
      lstCompetitor.map(i =>
        i.competitorId === _competitorId ? { ...i, isSelect: 1 } : i,
      ),
    );
    await setIsOldDay(
      ConvertToInt(moment(new Date()).format('YYYYMMDD').toString()) !==
        workinfo.workDate
        ? true
        : false,
    );
        ...
        >
          ...
          >
            <Text
              style={{
                color:
                  countCompetitor === 0 ? appcolor.success : appcolor.danger,
              }}
            >
              {it.competitorName}
            </Text>
          ...
          >
            ...
            >
              <Text
                style={{
                  color:
                    countPriceFalse === 0 ? appcolor.success : appcolor.danger,
                }}
              >
                Niêm yết
              </Text>
            ...
            >
              <Text
                style={{
                  color:
                    countNetFalse === 0 ? appcolor.success : appcolor.danger,
                }}
              >
                Thực bán
              </Text>
            ...
            >
              <Text
                style={{
                  color:
                    countFsmFalse === 0 ? appcolor.success : appcolor.danger,
                }}
              >
                FSM Incentive
              </Text>
  ...
  const filterProduct = async str => {
    ...
    if (str !== null && str !== undefined && str.length > 0) {
      mDataFilter = mDataMain.filter(i =>
  ...
  const viewItemInput = async () => {
    if (!isCheckViewInput) {
      let ROW_NUMBER = 0;
      let dataChange = await mDataMain.filter(
        i =>
          (i.priceValue != null && i.priceValue >= 1000) ||
          (i.netValue != null && i.netValue >= 1000) ||
          (i.fsmValue != null && i.fsmValue >= 1000),
      );
  ...
  const filterItemError = async () => {
    if (!isCheckViewError) {
      ...
      let dataChange = await mDataMain.filter(
        i =>
          i.priceValue == null ||
          (i.priceValue !== 0 && i.priceValue < 1000) ||
          i.priceValue % 1000 > 0 ||
          i.netValue == null ||
          (i.netValue !== 0 && i.netValue < 1000) ||
          i.netValue % 1000 > 0 ||
          i.fsmValue == null ||
          (i.fsmValue !== 0 && i.fsmValue < 1000) ||
          i.fsmValue % 1000 > 0,
      );
      dataChange.forEach(i => {
        i.indexMain = ROW_NUMBER;
  ...
  const competitorSelected = async (item, index) => {
    ...
    let dataFilter = dataCompetitor.map(i =>
      i.competitorId === item.competitorId
        ? { ...i, isSelect: selected }
        : { ...i, isSelect: 0 },
    );
    ...
    const lstTabByCompe = dataTabView?.filter(
      it => it.competitorId === item.competitorId,
    );
  ...
  const renderItemCompetitor = ({ item, index }) => {
    return (
      ...
      >
        <TouchableOpacity
          onPress={() => {
            competitorSelected(item, index);
          }}
          style={{
            width: deviceWidth * 0.5 * 0.8 - 20,
            marginHorizontal: 10,
            padding: 10,
            height: 40,
            justifyContent: 'center',
            ...
            padding: 8,
            backgroundColor:
              item.isSelect == 1 ? appcolor.primary : appcolor.grayLight,
            borderRadius: 50,
          }}
        >
          <Text
        ...
        {/* <TouchableOpacity style={{ width: '90%', padding: 5 }} onPress={() => competitorSelected(item)} >
                ...
                <SpiralIcon style={{ padding: 13, textAlignVertical: 'center' }} name={item.isSelect == 1 ? 'check' : ''} size={15} color={appcolor.black} /> */}
      </View>
    );
  };
  const openSheet = () => {
          ...
          <View style={{ width: '100%' }}>
            ...
            >
              ...
              <SpiralIcon
                ...
                type={!isCheckViewInput ? 'ionicon' : ''}
                size={23}
                color={!isCheckViewInput ? appcolor.dark : appcolor.success}
              />
            </TouchableOpacity>
            ...
            >
              ...
              >
                ...
                <SpiralIcon
                  ...
                  type={!isCheckViewError ? 'ionicon' : ''}
                  size={23}
                  color={!isCheckViewError ? appcolor.dark : appcolor.success}
                />
              </TouchableOpacity>
            </View>
          </View>
...
const ViewItem = ({
  ...
  const checkFormatNumber = async (event, index, type, item) => {
    ...
    let foundValue = colorInput.filter(
      obj =>
        ...
        obj.competitorId === item.competitorId,
    );
  ...
  const renderTabView = () => {
    ...
    return dataTab.map(tab => {
      dataByCategoryId = mData.filter(i => i.categoryId === tab.categoryId);
      const totalRow = dataByCategoryId.length;
      return (
        <Tabs.Tab
          key={tab.categoryName + `${totalRow !== 0 ? `(${totalRow})` : ''}`}
          label={tab.categoryName + `${totalRow !== 0 ? `(${totalRow})` : ''}`}
          name={tab.categoryName + `${totalRow !== 0 ? `(${totalRow})` : ''}`}
        >
          <View
            style={{
              backgroundColor: appcolor.light,
...
const RenderInputNumber = ({
  ...
  const dataColor = colorInput.find(
    obj =>
      obj.index === itemIndex &&
      obj.type === type &&
      obj.categoryId === itemInput.categoryId &&
