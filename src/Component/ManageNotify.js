...
const ManageNotify = ({ navigation }) => {
  ...
  const [inAppShow, setInAppShow] = useState({
    showInApp: false,
    messengerId: 0,
  });
  ...
  const LoadData = async () => {
    ...
    const dataFilter = mData.filter(
      it =>
        moment(it.createdDate || '').format('YYYY-MM-DD') >=
          moment().subtract(3, 'M').format('YYYY-MM-DD') || it.seen != 1,
    );
        ...
        <TouchableOpacity onPress={ViewDetailNofity}>
          ...
          >
            ...
            <View style={{ width: '90%' }}>
              ...
              <Text
                style={{ fontWeight: readStatus, color: appcolor.dark }}
                numberOfLines={2}
                ellipsizeMode="tail"
              >
                {item.body}
              ...
              {item.hyperLinks !== null && item.hyperLinks?.length > 0 && (
                ...
                >
                  <TouchableOpacity
                    style={styles.viewHiddenLink}
                    onPress={showDetailNow}
                  >
                    <View
