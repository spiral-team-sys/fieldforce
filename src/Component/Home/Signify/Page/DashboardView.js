...
const PROGRAM_SUMMARY_SUBTITLE = 'Tổng quan nhanh từ dashboard, nhấn để mở chi tiết.'
...
export const DashboardView = ({ navigation, isLoadMain }) => {
    ...
    const onResolveDashboardTitle = useCallback((pageName, title) => {
        ...
        setDashboardTitles((prev) => (prev?.[pageName] === title ? prev : { ...prev, [pageName]: title }))
    }, [])
        ...
        if (!programMetrics) {
            return (
                <View style={styles.emptyBox}>
                    <Text style={styles.emptyText}>Chưa có dữ liệu thống kê chương trình</Text>
                </View>
            )
        }
    ...
    const styles = StyleSheet.create({
        ...
        emptyBox: { width: slideWidth, height: slideHeight, borderRadius: 24, alignItems: 'center', justifyContent: 'center', backgroundColor: appcolor.surface, paddingHorizontal: 18, borderWidth: 1, borderColor: appcolor.grayLight },
    ...
    const renderChartSlide = (pageName, content) => {
        const resolvedTitle = dashboardTitles?.[pageName] || getDashboardTitle(pageName)
        const config = pageName === 'ROUTING'
            ? {
                title: resolvedTitle,
            ...
            : pageName === 'SELLIN'
                ? {
                    ...
                    colors: [appcolor.success || '#2E7D32', appcolor.warning || '#F39C12'],
                    bodyStyle: styles.slideBody,
