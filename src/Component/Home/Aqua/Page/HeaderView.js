...
const HeaderView = ({ navigation }) => {
    ...
    const styles = StyleSheet.create({
        ...
        contentMain: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 8 },
        contentTitle: { alignContent: 'space-around' },
        contentButton: { flexDirection: 'row', justifyContent: 'center' },
        titleWelcome: { color: appcolor.light, fontWeight: fontWeightBold, fontSize: 16 },
        titleEmployeeCode: { fontSize: 12, fontWeight: '500', fontStyle: 'italic', color: appcolor.grayLight },
        viewTop: { borderTopRightRadius: deviceWidth / 3, opacity: 0.3, backgroundColor: appcolor.light, width: '50%', position: 'absolute', top: -60, bottom: -10, start: 0 },
        ...
        titleDashboardName: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.light, textAlign: 'center', padding: 8, paddingBottom: 16 },
    });
        ...
        <View style={styles.mainContainer}>
            ...
            <View style={styles.contentMain}>
                <View style={styles.contentTitle}>
                    <Text style={styles.titleWelcome}>{`Xin chào, ${userinfo.fisrtName || userinfo.employeeName}`}</Text>
                    <Text style={styles.titleEmployeeCode}>{`Mã ${userinfo?.employeeCode}`}</Text>
                </View>
                <View style={styles.contentButton}>
                    <TouchableOpacity style={styles.buttonAction} onPress={handlerNotifyPress}>
                        <Icon reverse size={20} color={appcolor.primary} type="ionicon" name='notifications' />
                        {countNotification > 0 &&
                    ...
                    <TouchableOpacity style={styles.buttonAction} onPress={handlerMenuPress}>
                        <Icon reverse size={20} color={appcolor.primary} type="ionicon" name='menu' />
                    </TouchableOpacity>
                </View>
            </View>
