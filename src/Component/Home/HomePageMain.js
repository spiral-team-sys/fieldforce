...
import { View, BackHandler, DeviceEventEmitter, StyleSheet } from "react-native";
import { useDispatch, useSelector } from "react-redux";
import { AUTH_SESSION_EXPIRED_EVENT } from "../../Core/Helper";
import { AppNameBuild, aquaApp, cuckooApp, dsmHvnApp, honorApp, lgApp, officeApp, psvApp, signifyApp } from "../../Core/URLs";
...
const HomePageMain = ({ navigation, route }) => {
    ...
    const validDataEmployeeInfo = React.useCallback(async () => {
        ...
        await Employee.validData_EmployeeInfo(async (statusId, message) => {
            const _valueRequired = statusId === 500 && message !== null && message.length > 0;
            const currentRequired = employeeInfo?.requiredData;
            if (currentRequired !== _valueRequired) {
                dispatch(SET_EmployeeInfo({ ...employeeInfo, requiredData: _valueRequired }));
            }
            if (message !== null && message.length > 0) {
                const options = statusId === 500
    ...
    // Effect
    useEffect(() => {
        const back_handler = BackHandler.addEventListener("hardwareBackPress", onBackHandler);
        const valid_employee = DeviceEventEmitter.addListener("REVALID_EMPLOYEEINFO", validDataEmployeeInfo);
        const auth_expired = DeviceEventEmitter.addListener(AUTH_SESSION_EXPIRED_EVENT, () => {
