...
const dashboardConfig = [
  {
    type: DASHBOARD_TYPE.attendance,
    title: 'Chấm công',
    subtitle: 'Tiến độ ngày',
    icon: 'clock',
  },
  {
    type: DASHBOARD_TYPE.sellOut,
    title: 'Sell Out',
    subtitle: 'Doanh số ra',
    icon: 'shopping-bag',
  },
  {
    type: DASHBOARD_TYPE.sellIn,
    title: 'Sell In',
    subtitle: 'Nhập hàng',
    icon: 'package',
  },
  {
    type: DASHBOARD_TYPE.kpi5,
    title: 'KPI5',
    subtitle: 'Hiệu quả',
    icon: 'activity',
  },
  {
    type: DASHBOARD_TYPE.target,
    title: 'Target',
    subtitle: 'Chỉ tiêu',
    icon: 'target',
  },
  {
    type: DASHBOARD_TYPE.routing,
    title: 'Routing',
    subtitle: 'Lộ trình',
    icon: 'map',
  },
  {
    type: DASHBOARD_TYPE.sSub,
    title: 'S-Sub',
    subtitle: 'Tổng hợp',
    icon: 'layers',
  },
  {
    type: DASHBOARD_TYPE.targetRow,
    title: 'Target Row',
    subtitle: 'Tiến độ',
    icon: 'list',
  },
...
const DashboardView = ({ navigation, isReloadData }) => {
  ...
  const slideWidth = useMemo(
    () => width - SLIDE_HORIZONTAL_PADDING * 2,
    [width],
  );
  ...
  const slides = useMemo(() => {
    return dashboardConfig
      .filter(
        item =>
          item.type === DASHBOARD_TYPE.attendance ||
          menuAccess.includes(item.type),
      )
      .flatMap(item => {
        const attendance = dashboardData[DASHBOARD_TYPE.attendance] || [];
        if (item.type !== DASHBOARD_TYPE.attendance || attendance.length < 2)
          return [item];
        ...
        case DASHBOARD_TYPE.kpi5:
          return (
            <BarChartCard
              ...
              data={data}
              navigation={navigation}
              accentColor={appcolor.highlightDate}
            />
          );
