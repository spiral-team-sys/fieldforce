...
export const SummaryTF = ({ navigation, refreshing }) => {
    ...
    const styles = StyleSheet.create({
        ...
        cardView: { width: '100%', backgroundColor: appcolor.surface, borderRadius: 8, marginBottom: 10, overflow: 'hidden' },
        titleMain: { padding: 8, fontSize: 18, fontWeight: '700', alignSelf: 'center', color: appcolor.dark }
    })
    return (
        <View style={styles.contentMain}>
            <ViewDashboard key={'ViewDashboard'} dataMenu={dataMenu} navigation={navigation} />
        </View>
    )
}
                        ...
                        >
                            ...
                            {/* Attendant */}
                            {
                                ...
                                <View key={'Attendant_' + index} style={{ flex: 1 }}>
                                    <DashboardAttendantTF navigation={navigation} typeDashboard={type.Attendant} />
                                </View>
                            }
                            {/* Sell-In Daily */}
                            {
                                ...
                                <View key={'SellIn_' + index} style={{ width: '100%' }}>
                                    <DashboardSellInTF navigation={navigation} typeDashboard={type.SellIn} />
                                </View>
                            }
                            {/* Sell-Out Daily */}
                            {
                                ...
                                <View key={'SellOut_' + index}>
                                    <DashboardSellOutTF navigation={navigation} typeDashboard={type.SellOut} />
                                </View>
                            }
                            {JSON.parse(item) == type.Routing &&
                                <View key={'Routing_' + index} style={{ width: '100%' }}>
                                    <DashboardRoutingMain navigation={navigation} typeDashboard={type.Routing} />
                                </View>
                            }
                            {
                                JSON.parse(item) !== type.SellOut && JSON.parse(item) !== type.SellIn && JSON.parse(item) !== type.Attendant && JSON.parse(item) !== type.Routing &&
                                <View key={'Dashboard_' + index} style={{ width: '100%', height: deviceHeight / 3.5 }}>
                                    <DashBoardTargetTF navigation={navigation} typeDashboard={JSON.parse(item)} />
                                </View>
                            }
