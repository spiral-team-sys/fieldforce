...
import { View, StyleSheet, ScrollView, RefreshControl, AppState, LogBox, DeviceEventEmitter } from "react-native";
import { APPDOWNLOAD, downloadAll } from '../../../Controller/DownloadDataController';
import { getStoreList } from '../../../Controller/WorkController';
import { GetToken, MessageAcept, MessageAction, ToastSuccess } from '../../../Core/Helper';
    ...
    const checkProfile = async () => {
        ...
        if (resulCheck.status == 500) {
            if (resulCheck.isConstraint == 1) {
                MessageAcept('Thông báo', resulCheck.message + 'Đi đến trang thông tin cá nhân', async () => {
                    navigation.navigate('Profile');
                })
            } else if (resulCheck.isConstraint == 0) {
                MessageAction(resulCheck.message + '\nĐi đến trang thông tin cá nhân', async () => {
                    navigation.navigate('Profile');
                })
            }
    ...
    return (
        welcome === 1 ?
            ...
            userinfo.employeeId > 0 ?
                <View style={{ ...styles.mainContainer, backgroundColor: appcolor.primary }}>

                    <View style={{ flex: 5 / 10 }}>
                        <View style={{ flex: 3 / 10 }}>
                            {/* Header */}
                            <HeaderTF navigation={navigation} onShowMenu={showMenuHome} />
                        </View>
                        <View style={{ flex: 7 / 10, marginBottom: 40, }}>
                            ...
                            {!refreshing && <ScrollView
                                ...
                                <SummaryTF navigation={navigation} refreshing={refreshing} />
                            </ScrollView>
                            }
                        </View>
                    </View>
                    <View style={{ flex: 5 / 10, backgroundColor: appcolor.surface, borderTopLeftRadius: 20, borderTopRightRadius: 20 }}>
                        <MenuHomeTF navigation={navigation} isLoading={refreshing} />
                    </View>
                </View >
                :
                <LoginTF onLoginCallBack={downloadData} />
    )
}
export default HomeTF;
