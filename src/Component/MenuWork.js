...
import { DEFAULT_COLOR, URL_DOWNLOAD_MENULIST, URL_FIELD_COACHING } from '../Core/URLs';
...
export default class MenuWork extends PureComponent {
  ...
  ItemSelect = async (item) => {
    if (item.Id !== 0) {
      ...
      this.props.navigation.navigate('WebView', { link: url, titlePage: item.title });
    }
  }
  renderItem = ({ item }) => (
    <ListItem style={{ height: 180, width: '50%', backgroundColor: '#fff' }} onPress={() => this.ItemSelect(item)}>
      <ListItem.Content style={{ height: '100%', alignItems: 'center', backgroundColor: '#fff', padding: 15, borderRadius: 10 }}>
        <Avatar size="large" showEditButton rounded icon={{ name: 'reader-outline', type: 'ionicon', size: 60, color: 'gray' }} />
        <ListItem.Title style={{ fontSize: 12, color: 'black' }}>{item.title}</ListItem.Title>
        <ListItem.Subtitle style={{ fontSize: 12, color: 'gray' }}>{item.subTitle}</ListItem.Subtitle>
      </ListItem.Content>
    </ListItem>
  )
  render() {
    ...
    return (
      ...
      // />
      <View style={{ flex: 1, justifyContent: 'flex-start', backgroundColor: DEFAULT_COLOR }}>
        <StatusBar hidden={true}></StatusBar>
        <WavyHeader customStyles={{ position: 'absolute', width: deviceWidth }} flip={true}></WavyHeader>
        <View style={{ height: 100, backgroundColor: 'transparent' }}>
          <View style={{ width: '100%', height: '100%', position: 'absolute', alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ color: 'gray', fontWeight: '500', fontSize: 17 }}>{this.props.route.params.titlePage}</Text>
          </View>
        ...
        <Card containerStyle={{ top: 52, borderRadius: 20, height: '73%', backgroundColor: '#fff' }}>
          <FlatList
