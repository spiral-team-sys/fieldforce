...
const WeekView = ({ data = [], onChooseDay, isRefreshData = false }) => {
  ...
  const styles = StyleSheet.create({
    ...
    titlePrevious: {
      fontSize: 13,
      fontWeight: '500',
      color: currentWeekIndex === 0 ? appcolor.grayLight : appcolor.dark,
      paddingHorizontal: 8,
    },
    titleNext: {
      fontSize: 13,
      fontWeight: '500',
      color:
        currentWeekIndex === data.length - 1
          ? appcolor.grayLight
          : appcolor.dark,
      paddingHorizontal: 8,
    },
          ...
          <View key={`weekday_${index}`} style={styles.mainHeader}>
            <Text
              style={[
                ...
                {
                  color:
                    item.dayOfWeek === 0 ? appcolor.danger : appcolor.greylight,
                },
              ]}
            >
              {item.weekDayName}
    ...
    <View style={styles.headerDays}>
      {weekdays.map((weekday, index) => {
        ...
        const isChoose =
          ...
          dataByDay.DayValue === itemDay.DayValue;
          ...
          >
            <Text
              style={[
                ...
                {
                  color:
                    weekday.dayOfWeek === 0 ? appcolor.danger : appcolor.dark,
            ...
            <View
              style={{
                ...styles.contentSummaryValue,
                backgroundColor:
                  itemDay.TotalValue > 0
                    ? appcolor.primary
                    : appcolor.greylight,
              }}
            >
              <Text style={styles.titleTotalValue}>
                {itemDay.TotalValue > 99 ? '99+' : itemDay.TotalValue || 0}
              </Text>
            </View>
          </TouchableOpacity>
        );
