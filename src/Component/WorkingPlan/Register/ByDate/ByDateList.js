...
const ByDateList = ({ dataMain = [] }) => {
    ...
    //
    useEffect(() => {
        const updateList = DeviceEventEmitter.addListener('PLAN_UPDATE_LIST', handlerUpdateList)
        const scrollToIndex = DeviceEventEmitter.addListener('SCROLL_TO_INDEX', handlerScrollToIndex)
        LoadData()
        return () => {
            updateList.remove()
    ...
    const renderItem = ({ item, index }) => { return <ByDateItem item={item} index={index} /> }
    return (
