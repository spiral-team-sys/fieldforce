...
const OTRegisterForm = ({ dataOTSummary = [], masterOT = [], shopinfo, initialRegisteredTime = null, onClose, onRegistered }) => {
    ...
    const updateSummaryAfterRegister = useCallback((overTime) => {
        setLocalSummary((summary) => {
            ...
            return summary.map((item, index) => {
                ...
                return {
                    ...item,
                    totalOT: Number.isNaN(currentTotal) ? overTime : currentTotal + overTime
                };
            });
        });
    }, []);
    ...
    const validateForm = useCallback(() => {
        ...
        if (confirmMonth != undefined) {
            ...
            if (sumOT >= confirmMonth.limitOT) {
                toastError('Tăng ca', `Giờ tăng ca của bạn đã ${confirmMonth.limitOT} giờ hoặc hơn`);
                return false;
            }
        }
    ...
    const confirmOT = useCallback(async () => {
        ...
        setSubmitting(true);
        await uploadTimeOT(shopinfo.shopId, shopinfo.auditDate, selectedTime, noteText, async (responseJson) => {
            let result = false;
            if (responseJson.status === 200) {
                result = await UpdateTimeOT(selectedTime, noteText, shopinfo.shopId, shopinfo.auditDate, '0');
            }
            ...
            if (result) {
                ...
                onRegistered && onRegistered({ timeOT: selectedTime, note: noteText });
            } else {
                toastError('Lỗi', 'Lỗi khi gửi dữ liệu!');
            }
        });
        setSubmitting(false);
    }, [note, onRegistered, registeredTime, selectedTime, shopinfo, submitting, updateSummaryAfterRegister, validateForm]);
