...
const OTStatusCard = ({
  ...
  const styles = useMemo(
    () => createStyles(appcolor, isRegistered),
    [appcolor, isRegistered],
  );
      ...
      <View style={styles.statusBox}>
        ...
        <View style={styles.textWrap}>
          <Text style={styles.title}>
            {isRegistered
              ? `Đã đăng ký hôm nay: ${registeredTime}h`
              : 'Chưa đăng ký tăng ca hôm nay'}
          </Text>
          <Text style={styles.subTitle}>
            {isRegistered
              ? 'Thông tin đã được cập nhật trên máy và gửi lên hệ thống.'
              : `Tổng tháng hiện tại: ${totalMonth}`}
          </Text>
        </View>
      </View>
    </View>
...
const createStyles = (appcolor, isRegistered) =>
  StyleSheet.create({
    section: {
      marginBottom: 12,
    },
