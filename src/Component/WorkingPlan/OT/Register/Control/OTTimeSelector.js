...
const OTTimeSelector = ({ appcolor, data, disabled, onSelect, selectedItem, selectedTime }) => {
    const styles = useMemo(() => createStyles(appcolor), [appcolor]);

    const renderItem = useCallback(({ item }) => {
        const isSelected = selectedItem.listCode == item.listCode && selectedItem.code == item.code;
    ...
    const listEmpty = useMemo(() => (
        <View style={styles.emptyBox}>
            <Icon name='clock' type='font-awesome-5' size={16} color={appcolor.greylight} />
