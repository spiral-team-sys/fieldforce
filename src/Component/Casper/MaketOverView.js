...
export const MaketOverView = ({ navigation, route }) => {
  ...
  const onSeleted = async item => {
    const details = await rootdata
      .filter(d => d.type === item.type && d.provinceCode > 0)
      .sort((a, b) => {
  ...
  const rowSum = ({ item, index }) => {
    return (
      ...
      >
        <Text style={{ color: appcolor.greylight, fontSize: 10, margin: 3 }}>
          {item.type}
        </Text>
        <View style={{ flexDirection: 'row', width: '100%' }}>
          ...
          >
            {typeSelect === item.type && (
              <SpiralIcon color={appcolor.danger} name="arrow-drop-down" />
            )}
          </Text>
          <Text
            style={{
  ...
  const rowDetails = ({ item, index }) => {
    return (
      <View
        style={{ backgroundColor: appcolor.light, padding: 7 }}
        key={`dd${index}29`}
      >
        {item.category === null && (
          <View>
        ...
        <View style={{ flexDirection: 'row', padding: 7, width: '100%' }}>
          <Text style={{ fontSize: 12, width: '30%', color: appcolor.dark }}>
            {item.category}
          </Text>
          <Text
            style={{
              fontWeight: item.category === null ? 'bold' : 'normal',
              fontSize: 12,
              width: '23%',
              color: appcolor.dark,
            }}
