...
export const Casper_BussinessTripDetails = ({
  bussinessInfo,
  appcolor,
  nextView,
  bottomSheet,
  ...
  const saveResult = async () => {
    ...
    //     }
    // }
    UploadBussinessTrip(bussinessShow, () => {
      nextView(VIEW_HIS);
    });
  };
  useEffect(() => {
  ...
  const CalculatorBy = () => {
    ...
    if (bussinessShow?.fromDate && bussinessShow?.toDate) {
      let fromDateInt = 0;
      let toDateInt = 0;
      fromDateInt = !isNaN(bussinessShow?.fromDate)
        ? bussinessShow?.fromDate
        : parseInt(bussinessShow?.fromDate.replace(/-/gm, ''));
      toDateInt = !isNaN(bussinessShow?.toDate)
        ? bussinessShow?.toDate
        : parseInt(bussinessShow?.toDate.replace(/-/gm, ''));
      if (toDateInt >= fromDateInt && toDateInt !== 0 && fromDateInt !== 0) {
        ...
        let supportHotel =
          bussinessShow?.typeKmInfo?.isRequired === 1
            ? numberNight *
                (bussinessShow?.typePeopleInfo?.code == '1A'
                  ? 430000
                  : 270000) || 0
            : 0;
        let supportRestaurant = numberDate * 150000 || 0;
        // if (bussinessShow?.typeKm) {

        if (bussinessShow.provinceFromVN == bussinessShow.provinceToVN) {
          supportMove = 150000 * numberDate;
          if (bussinessShow?.typeKm == '50km ~ 100km') supportHotel = 0;
        } else {
          supportMove = bussinessShow?.supportKM || 0;
        }
        supportTotal =
          bussinessShow?.typeKmInfo?.isRequired === 1
            ? supportHotel + supportRestaurant + supportMove
            : supportRestaurant + supportMove || 0;
        // } else
    ...
    <View style={{ flex: 1, backgroundColor: appcolor.surface }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        ...
        <FormGroup
          ...
          title={'Ngày yêu cầu'}
          value={
            (Moment(bussinessShow?.workDate, 'YYYYMMDD').format('ddd DD MM') ||
              Moment(new Date()).format('ddd DD MM')) + ''
          }
          useClearAndroid={false}
        />
      </View>
      ...
      <View
        style={{ borderWidth: 1, borderColor: appcolor.surface, width: '100%' }}
      />
      {PeopleInRoom !== null && PeopleInRoom.length > 0 && (
        <FormGroup
          ...
          placeholder={'0'}
          rightFunc={
            modeRes === MODE_EDIT ? () => _bottomSheet.current.show() : null
          }
          value={(bussinessShow?.typePeople || '') + ''}
          useClearAndroid={false}
        />
      )}
      <FormGroup
      ...
      <NumberFormat
        key={'s1'}
        disabled={true}
        value={bussinessShow?.supportHotel || bussinessShow?.supportNight}
        displayType={'text'}
        thousandSeparator={true}
        renderText={value => (
          <FormGroup
      ...
      <NumberFormat
        ...
        renderText={value => (
          <FormGroup
            title={
              bussinessShow.provinceFromVN == bussinessShow.provinceToVN
                ? 'Trợ cấp di chuyển (Nội tỉnh)'
                : 'Trợ cấp di chuyển dự kiến (Ngoại tỉnh)'
            }
            placeholder={'0'}
            value={value}
            inputStyle={{ textAlign: 'right' }}
      ...
      >
        {bussinessInfo?.isLock === false && (
          <Button
            containerStyle={{ height: 50, alignItems: 'flex-end', right: 15 }}
            buttonStyle={{
      ...
      >
        <View key={'date'} style={{ height: '80%' }}>
          <FlatList data={MoneyMove} renderItem={renderItemKM}></FlatList>
