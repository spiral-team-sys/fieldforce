...
export const Casper_BussinessTripDetails = ({ bussinessInfo, appcolor, nextView, bottomSheet, presentView, MoneyMove, remaining, PeopleInRoom }) => {
    ...
    const CalculatorBy = () => {
        ...
        if (bussinessShow?.fromDate && bussinessShow?.toDate) {
            ...
            if (toDateInt >= fromDateInt && toDateInt !== 0 && fromDateInt !== 0) {
                ...
                let supportHotel = bussinessShow?.typeKmInfo?.isRequired === 1 ?
                    (numberNight * (bussinessShow?.typePeopleInfo?.code == '1A' ? 430000 : 270000)) || 0 : 0
                ...
                if (bussinessShow.provinceFromVN == bussinessShow.provinceToVN) {
                    ...
                    if (bussinessShow?.typeKm == '50km ~ 100km')
                        supportHotel = 0;
                } else {
                    supportMove = bussinessShow?.supportKM || 0
                }
                supportTotal = (bussinessShow?.typeKmInfo?.isRequired === 1) ? (supportHotel + supportRestaurant + supportMove) : (supportRestaurant + supportMove) || 0;
                // } else
        ...
        <View style={{ flex: 1, backgroundColor: appcolor.surface }}>
                    ...
                    <FormGroup
                        title={
                            bussinessShow.provinceFromVN == bussinessShow.provinceToVN ?
                                'Trợ cấp di chuyển (Nội tỉnh)' : 'Trợ cấp di chuyển dự kiến (Ngoại tỉnh)'
                        }
                        placeholder={'0'}
                        value={value} inputStyle={{ textAlign: 'right' }}
                        useClearAndroid={false}
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', width: '50%', alignSelf: 'flex-end' }}>
                {bussinessInfo?.isLock === false &&
                    <Button containerStyle={{ height: 50, alignItems: 'flex-end', right: 15 }}
