...
export const Casper_BussinessTripInput = ({ appcolor, nextView, myAddress, searchMap, bussinessInput, setBussinessInput, Provinces, MoneyMove, PeopleInRoom }) => {
    ...
    useEffect(() => {
        CalculatorBy();
        // return () => false;
    }, [bussinessInput?.fromDate, bussinessInput?.toDate, bussinessInput?.typeKm])
    // calcu
    const CalculatorBy = async () => {
        ...
        if (bussinessInput?.fromDate && bussinessInput?.toDate) {
            ...
            if (toDateInt >= fromDateInt) {
                ...
                let supportHotel = numberNight * (bussinessInput?.typePeopleInfo?.code == '1A' ? 430000 : 270000)
                ...
                if (bussinessInput?.typeKm) {
                    if (bussinessInput?.provinceFromVN == bussinessInput?.provinceToVN) {
                        ...
                        if (bussinessInput?.typeKm == '50km ~ 100km')
                            supportHotel = 0;
                    } else {
                        supportMove = bussinessInput?.supportKM || 0
                    }
                    supportTotal = (bussinessInput?.typeKmInfo?.isRequired === 1) ? (supportHotel + supportRestaurant + supportMove) : (supportRestaurant + supportMove);
                }
                await setBussinessInput({ ...bussinessInput, numberDate: numberDate, numberNight: numberNight, supportHotel: supportHotel, supportRestaurant: supportRestaurant, supportMove: supportMove, supportTotal: supportTotal })
            }
        }
    }
    useEffect(() => {
        (bussinessInput?.numberDate !== undefined) && loadWorkingPlan(parseInt(bussinessInput?.fromDate.replace(/-/gm, '')), parseInt(bussinessInput?.toDate.replace(/-/gm, '')));
        return () => false
    ...
    const pressFromAddress = (type) => {
        if (type === 'from') {
            if (bussinessInput?.fromAddress && bussinessInput?.fromAddress != '' && bussinessInput?.fromAddress.length > 6) {
                const textSeach = `${bussinessInput?.fromAddress}`
                AttendantController.LocationFromAddress(textSeach, (res) => {
                    if (res.statusId === 500) {
                        MessageInfo("Địa chỉ bạn nhập chúng tôi không tìm thấy vui lòng kiểm tra lại hoặc tìm 1 địa chỉ khác")
                    } else {
                        setBussinessInput({ ...bussinessInput, fromAddress: res.address, locationFrom: res?.location?.lat + ',' + res?.location?.lng })
                    }
                }, err => {
                    alert(err)
                });
            } else {
                MessageInfo('Vui lòng nhập địa chỉ ít nhất 6 ký tự (địa chỉ bao gồm số nhà, tên đường, quận huyện, thành phố)');
            }
        }
        else if (type === 'to') {
            if (bussinessInput?.toAddress && bussinessInput?.toAddress != '' && bussinessInput?.toAddress.length > 10) {
                ...
                AttendantController.LocationFromAddress(textSeach, (res) => {
                    ...
                    if (res.statusId === 500) {
                        ...
                        setBussinessInput({ ...bussinessInput, toAddress: res.address, locationTo: `${res?.location?.lat},${res?.location?.lng}` })
                    }
                }, err => {
                    alert(err)
                });
            } else {
                MessageInfo('Vui lòng nhập địa chỉ ít nhất 10 ký tự (địa chỉ bao gồm số nhà, tên đường, quận huyện, thành phố )');
            }
        }
    }
    ...
    const renderItemWorkPlan = ({ item, index }) => {
        return (
            'auditDateKM' in item ?
                <View style={{ width: '100%', backgroundColor: appcolor.surface, marginBottom: 7 }}>
                    <Text style={{ color: appcolor.dark, fontSize: scaleSize(16), marginLeft: 30, padding: 12 }}>{Moment(item.auditDateKM.toString()).format('dddd, DD MM YY')}</Text>
                    <FormGroup
                        keyboardType={'numeric'}
                        value={(item?.numberKm || '') + ''}
                        editable={true} inputStyle={{ textAlign: 'right' }}
                        placeholder={'Nhập số km trong ngày'} useClearAndroid={false}
                ...
                <Fragment>
                    <View style={{ flexDirection: 'row', justifyContent: 'center', maxHeight: 70, width: '100%' }}>
                        <Text style={{ width: '25%', textAlign: 'center', color: appcolor.dark, fontSize: 10, padding: 10 }}>
                            Cửa hàng
                        </Text>
                        <Text style={{ width: '25%', textAlign: 'center', color: appcolor.dark, fontSize: 10, padding: 10 }}>{item.shopName}</Text>
                        <Text style={{ width: '50%', textAlign: 'center', color: appcolor.dark, fontSize: 10, padding: 10 }}>{item.address}</Text>
                    </View>
    ...
    const continueView = () => {
        if (!bussinessInput?.fromAddress || bussinessInput?.fromAddress === '' || bussinessInput?.fromAddress.length < 10) {
            MessageInfo('Vui lòng nhập địa chỉ đi ít nhất 10 ký tự (địa chỉ gồm số nhà, tên đường, quận huyện, thành phố)');
            return
        }
        if (!bussinessInput?.toAddress || bussinessInput?.toAddress === '' || bussinessInput?.toAddress.length < 10) {
            MessageInfo('Vui lòng nhập địa chỉ đến ít nhất 10 ký tự (địa chỉ gồm số nhà, tên đường, quận huyện, thành phố)');
            return
        }
        if (!bussinessInput?.locationFrom || bussinessInput?.locationFrom === '') {
            MessageInfo('Chưa có toạ độ nơi đi, Vui lòng nhập nơi đi rồi bấm tìm kiếm, hoặc bấm vào map để chọn toạ độ nơi đi.)');
            return
        }
        if (!bussinessInput?.locationTo || bussinessInput?.locationTo === '') {
            MessageInfo('Chưa có toạ độ từ, Vui lòng nhập nơi đến rồi bấm tìm kiếm, hoặc vào map để chọn toạ độ nơi đi.)');
            return
        }
        if (!bussinessInput?.provinceFromVN || bussinessInput?.provinceFromVN === '') {
        ...
        if ((bussinessInput?.typeKm == 'Từ 100km' || bussinessInput?.provinceFromVN !== bussinessInput?.provinceToVN) && (!bussinessInput?.typePeople || bussinessInput?.typePeople === '')) {
            MessageInfo('Chưa chọn số lượng người/phòng');
            return
        }
        if (bussinessInput?.workingplan && bussinessInput?.workingplan.length === 0) {
            MessageInfo('Chưa có lịch làm việc cho khoảng thời gian này.');
            return
        }

        let itemsNoneEnter = bussinessInput?.workingplan.filter(it => ('auditDateKM' in it && (it.numberKm === undefined || it.numberKm === null)))
        if (itemsNoneEnter.length > 0) {
            MessageInfo('Bạn chưa nhập km cho ngày: ' + itemsNoneEnter[0].auditDateKM)
            return
        }
        CalculatorBy()
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            ...
            <FormGroup
                ...
                title={'Từ nơi công tác'} placeholder='Tỉnh đi'
                rightFunc={() => {
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
                <FormGroup
                    containerStyle={{ paddingBottom: 5, paddingTop: 5, maxHeight: 90, width: '100%' }}
                    // inputStyle={{ height: 70 }}
                    multiline={true}
                    rightFunc={() => pressFromAddress('from')}
                    iconRight="search" iconRightStyle={{ color: appcolor.primary }}
                    editable={true}
                    title={'Nơi đi'}
                ...
                <View style={{ flexDirection: 'column' }}>
                    {bussinessInput?.locationFrom &&
                        <Icon name='location' type='ionicon' onPress={() => searchMap('from')} color={appcolor.green} />}
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
                <FormGroup
                    containerStyle={{ maxHeight: 90, borderColor: appcolor.darklight, width: '100%' }}
                    // inputStyle={{ height: 70 }}
                    multiline={true} inputStyle={{ fontSize: 12 }}
                    ...
                    handleChangeForm={(text) => setBussinessInput({ ...bussinessInput, toAddress: text })}
                />
                <View style={{ flexDirection: 'column' }}>
                    {bussinessInput?.locationTo &&
                        <Icon name='location' type='ionicon' onPress={() => searchMap('to')} color={appcolor.success} />}
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <FormGroup
                    ...
                    containerStyle={{ width: '49%' }}
                    title={'Từ ngày'}
                    value={bussinessInput?.fromDate ? Moment(bussinessInput?.fromDate).format('YYYY-MM-DD') : Moment().startOf('month').format('YYYY-MM-DD')}
                    useClearAndroid={false}
                />
                <FormGroup
                    ...
                    containerStyle={{ width: '49%' }}
                    title={'Đến ngày'}
                    value={Moment(bussinessInput?.toDate || new Date()).format('YYYY-MM-DD')}
                    useClearAndroid={false}
                />
            </View>
            ...
            <View style={{ backgroundColor: appcolor.surface, padding: 10, borderTopLeftRadius: 5, borderTopRightRadius: 5 }}>
                <Text style={{ fontWeight: '600', fontSize: 15, color: appcolor.dark }}>{'Kế hoạch công tác'}</Text>
            </View>
            <FlatList
            ...
            {bussinessInput?.provinceFromVN !== bussinessInput?.provinceToVN &&
                <FormGroup
                    ...
                    useClearAndroid={false}
                    handleChangeForm={text => setBussinessInput({ ...bussinessInput, supportKM: parseInt(text) })}
                />
            }
            ...
            <FormGroup
                ...
                useClearAndroid={false}
            />
            {PeopleInRoom !== null && PeopleInRoom.length > 0 && (bussinessInput?.typeKm == 'Từ 100km' || bussinessInput?.provinceFromVN !== bussinessInput?.provinceToVN) &&
                <FormGroup
            ...
            <Button containerStyle={{ width: '100%', height: 50, backgroundColor: appcolor.tranparents, alignItems: 'flex-end', right: 15, padding: 5 }}
            ...
            <ActionSheet
                ...
                <View key={'date'} style={{ marginBottom: 50 }}>
                    {typeDate === TYPE_KM &&
                        <ScrollView
                            ...
                            showsHorizontalScrollIndicator={false}>
                            {
                                MoneyMove.map((v, i) => {
                                    return (
                                        <TouchableOpacity key={i + "lllaa"} onPress={() => {
                                            setBussinessInput({ ...bussinessInput, typeKm: v.name, typeKmInfo: v });
                                            _bottomSheet.current.hide();
                                        }}>
                                            <View style={{ backgroundColor: appcolor.light, borderRadius: 20, marginEnd: 12 }}>
                                                <Text style={{ color: appcolor.dark, fontSize: 12, padding: 10 }}>{v.name}</Text>
                                            </View>
                                        </TouchableOpacity>
                                    )
                                })
                    ...
                    {(typeDate === TYPE_PROVINCE_FROM || typeDate === TYPE_PROVINCE_TO) &&
                        <ScrollView horizontal>
                            {Provinces.map((item, i) => {
                                return (
                                    <TouchableOpacity key={"dasda" + i}
                                        style={{ marginEnd: 12 }}
                                        onPress={() => {
                                            typeDate === TYPE_PROVINCE_FROM ? setBussinessInput({ ...bussinessInput, provinceFromVN: item.provinceName, provinceFromCode: item.provinceCode }) :
                                                setBussinessInput({ ...bussinessInput, provinceToVN: item.provinceName, provinceToCode: item.provinceCode });
                                            _bottomSheet.current.hide();
                                        }}>
                                        <View style={{ padding: 12, borderRadius: 20, backgroundColor: appcolor.light }}>
                                            <Text style={{ color: appcolor.dark }}>{item.provinceName}</Text>
                                        </View>
                                    </TouchableOpacity>
                                )
                            })
                        ...
                        <ScrollView
                            ...
                            showsHorizontalScrollIndicator={false}>
                            {
                                PeopleInRoom.map((v, i) => {
                                    return (
                                        <TouchableOpacity key={i + "llla12"} onPress={() => {
                                            setBussinessInput({ ...bussinessInput, typePeople: v.name, typePeopleInfo: v });
                                            _bottomSheet.current.hide();
                                        }}>
                                            <View style={{ backgroundColor: appcolor.light, borderRadius: 20, marginEnd: 12 }}>
                                                <Text style={{ color: appcolor.dark, fontSize: 12, padding: 10 }}>{v.name}</Text>
