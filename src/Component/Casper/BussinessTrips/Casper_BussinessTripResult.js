...
export const Casper_BussinessTripResult = ({
  bussinessInfo,
  appcolor,
  nextView,
  bottomSheet,
  ...
  const saveResult = async () => {
    ...
    const busMotnh = Moment(bussinessShow?.fromDate, 'YYYYMMDD').format(
      'YYYYMM',
    );
    if (busMotnh < today) {
    ...
    let lstWorkingSchedule = await GetListWorkingSchedule(
      Moment(bussinessShow?.fromDate).format('YYYYMMDD'),
      Moment(bussinessShow?.toDate).format('YYYYMMDD'),
    );
  ...
  const CalculatorBy = () => {
    ...
    if (bussinessShow?.fromDate && bussinessShow?.toDate) {
      ...
      if (toDateInt >= fromDateInt && toDateInt !== 0 && fromDateInt !== 0) {
        ...
        let supportHotel =
          numberNight *
          (bussinessShow?.typePeopleInfo?.code == '1A' ? 430000 : 270000);
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <NumberFormat
        key={'s4'}
        // disabled={true}
        value={(bussinessShow?.supportKM || '0') + ''}
        displayType={'text'}
        thousandSeparator={true}
        renderText={value => (
          <FormGroup
