...
export const Casper_BussinessTripHistory = ({
  bussinessInfo,
  appcolor,
  nextView,
  resetBussinessInput,
  ...
  const [bussinessShow, setBussinessShow] = useState({
    ...bussinessInfo,
    fromDateF: bussinessShow?.fromDateF
      ? Moment(bussinessShow?.fromDateF).format('YYYYMMDD')
      : Moment().startOf('month').format('YYYYMMDD'),
    toDateF: Moment(bussinessShow?.toDateF || new Date()).format('YYYYMMDD'),
  });
  ...
  const loadData = async () => {
    ...
    let lstWorkingSchedule = await GetListWorkingSchedule(
      Moment(bussinessShow?.fromDateF).format('YYYYMMDD'),
      Moment(bussinessShow?.toDateF).format('YYYYMMDD'),
    );
    ...
    if (Array.isArray(lstWorkingSchedule)) {
      await lstWorkingSchedule.map((it, index) => {
        if (!lstId.includes(it.status)) {
          ...
          const _data = lstWorkingSchedule.filter(
            item => item.status === it.status,
          );
          it.status !== undefined &&
            lstTab.push({
              id: index,
  ...
  const handleDateSelect = async date => {
    ...
    if (typeDate === TYPE_DATE_FROM) {
      ...
      let fromDateCV = parseInt(
        bussinessShow?.fromDateF.replace(/[^\w\s]/gi, ''),
      );
  ...
  const renderItem = ({ item, index }) => {
    return (
      <TouchableOpacity
        key={`index_rd_${index}`}
        onPress={() => nextView(VIEW_RESULT, { ...item, isLock: true })}
      >
        <View
          style={{
            flexDirection: 'row',
        ...
        >
          ...
          >
            {Moment(item.fromDate?.toString()).format('DD-MMM') +
              ' -> ' +
              Moment(item.toDate, 'YYYYMMDD').format('DD-MMM YY')}
          </Text>
          <Text
            style={{
              width: '35%',
          ...
          >
            {item.provinceFromVN + ' -> ' + item.provinceToVN}
          </Text>
          <NumberFormat
            key={'s1'}
            disabled={true}
  ...
  const ViewItem = ({ data, index }) => {
    return (
      <View style={{ backgroundColor: appcolor.surface }} key={`${index}09h`}>
        <View
          style={{
            flexDirection: 'row',
            justifyContent: 'center',
    ...
    <View style={{ width: '100%', height: '100%' }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        <FormGroup
          ...
          title={'Từ ngày'}
          value={
            bussinessShow?.fromDateF
              ? Moment(bussinessShow?.fromDateF).format('YYYY-MM-DD')
        ...
        <FormGroup
          ...
          title={'Đến ngày'}
          value={Moment(bussinessShow?.toDateF || new Date()).format(
            'YYYY-MM-DD',
          )}
          useClearAndroid={false}
        />
          ...
          <Tabs.Container
            ...
            renderTabBar={props => (
              <MaterialTabBar
                ...
                scrollEnabled={true}
                style={{ backgroundColor: appcolor.light, width: deviceWidth }}
          ...
          >
            {LstTab.map((value, i) => {
              return (
                <Tabs.Tab
                  key={`tab_rb_${i}`}
                  name={`(${value.details.length}) ${Capitalize(value.name)}`}
                >
                  <View
                    style={{
                  ...
                  >
                    <ViewItem data={value.details} index={i} key={value.key} />
                  </View>
                </Tabs.Tab>
              );
            })}
      ...
      >
        <Button
          containerStyle={{ backgroundColor: appcolor.info }}
          buttonStyle={{ backgroundColor: appcolor.success }}
          icon={<SpiralIcon name="add" size={22} color={appcolor.white} />}
          onPress={async () => {
