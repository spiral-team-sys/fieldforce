...
export const Casper_BussinessTripHistory = ({ bussinessInfo, appcolor, nextView, resetBussinessInput, remaining, setRemaining }) => {
    ...
    const renderItem = ({ item, index }) => {
        return (
            <TouchableOpacity key={`index_rd_${index}`} onPress={() => nextView(VIEW_RESULT, { ...item, isLock: true })}>
    ...
    return (
        <View style={{ width: '100%', height: '100%' }}>
            ...
            <View style={{ width: '100%', alignItems: 'center' }}>
                {LstTab.length > 0 &&
                    <Tabs.Container
                        ...
                        {LstTab.map((value, i) => {
                            return (
                                <Tabs.Tab key={`tab_rb_${i}`} name={`(${value.details.length}) ${Capitalize(value.name)}`}>
            ...
            <LoadingView isLoading={showProgress} title=' ' styles={{ position: 'absolute', alignSelf: 'center', marginTop: deviceHeight / 3 }} />
        </View >
    )
}