...
export const DashboardWorking = ({ typeDashboard, isLoadMain }) => {
	...
	const styles = StyleSheet.create({
		...
		titleView: { fontSize: 14, fontWeight: '700', fontStyle: 'italic', padding: 8, paddingStart: 0 },
		valueView: { fontSize: 13, fontWeight: '700' },
		percentView: { fontSize: 12, fontWeight: '700', position: 'absolute', end: 5, top: -16 },
		lineProgress: { width: '100%', height: 8, backgroundColor: appcolor.grey, borderRadius: 8, overflow: 'hidden' },
		...
		return (
			<View key={`iomm_${index}`}>
				...
				<View style={{ width: '100%', flexDirection: 'row' }}>
					<Text style={{ ...styles.percentView, color: COLOR.ColorRand(index) }}>{`${item.actualValue}`} / {`${item.percentValue}%`}</Text>
				</View>
				<View style={styles.lineProgress}>
					<View style={{ ...styles.valueLineProgressView, width: `${item.percentValue}%`, backgroundColor: COLOR.ColorRand(index) }} />
				</View>
			</View>
		)
	}
