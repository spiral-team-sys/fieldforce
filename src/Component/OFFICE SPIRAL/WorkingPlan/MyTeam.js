        ...
        <View style={[styles.card, { backgroundColor: appcolor.surface }]}>
            ...
            >
                ...
                <View style={styles.info}>
                    <Text style={[styles.name, { color: appcolor.dark }]}>{item.employeeName || ''}</Text>
                    <Text style={[styles.group, { color: appcolor.dark }]}>{item.groupType || '0'}</Text>
                </View>
                <Icon type='ionicon' name={expanded ? 'chevron-up' : 'chevron-down'} size={16}
                    color={appcolor.greylight} />
            </TouchableOpacity>

            {expanded && (
                <CalendarView
                    dataPlanMonth={item.plan}
                    Control={false}
                />
            )}
        </View>
