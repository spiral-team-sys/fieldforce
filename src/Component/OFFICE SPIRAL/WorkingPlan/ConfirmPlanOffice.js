...
export const ConfirmPlanOffice = ({ navigation }) => {
  ...
  const LoadDataConfirm = async (_year, _month) => {
    ...
    //
    const result = await WorkingPlanAPI.GetPlanOfficeConfirm(
      yearValue,
      monthValue,
    );
  ...
  const onSendConfirm = async () => {
    ...
    if (dataConfirmByEmployee !== null && dataConfirmByEmployee.length > 0) {
      for (let index = 0; index < dataConfirmByEmployee.length; index++) {
        ...
        if (item.isValidData) {
          setErrorValueUpload(
            `Vui lòng cập nhật đầy đủ dữ liệu bắt buộc bên dưới trước khi gửi lên hệ thống`,
          );
          return;
        }
      }
      ...
      alertConfirm(
        'Gửi dữ liệu',
        'Bạn có chắc chắn với các xác nhận yêu cầu thay đổi bên dưới không ?',
        async () => {
          await WorkingPlanAPI.ConfirmPlanOffice(
            listEmployees,
            dataConfirmByEmployee,
            async result => {
  ...
  const handlerSearch = text => {
    ...
    const _searchlist = _.filter(dataMain, e => {
      return (
        removeVietnameseTones(e.dateView)
          .toLowerCase()
          .match(_valueSearch.toLowerCase()) ||
        removeVietnameseTones(e.dataEmployee)
          .toLowerCase()
          .match(_valueSearch.toLowerCase())
      );
    });
      ...
      <View key={`qpw_p${index}`}>
        ...
        <View style={styles.summaryMain}>
          <LinearGradient
            ...
            colors={[appcolor.surface, appcolor.primary + '15']}
  ...
  const renderPageView = () => {
    ...
    return (
      dataConfirmByEmployee !== null &&
      dataConfirmByEmployee.length > 0 &&
  ...
  return (
    <View style={styles.mainContainer}>
      <HeaderCustom
        ...
        rightFunc={() => {
          console.log('Xác nhận tất cả yêu cầu');
        }}
      />
      <SearchData placeholder="Tìm kiếm ..." onSearchData={handlerSearch} />
      ...
      <View style={styles.contentMain}>
        <FlatList
          ...
          refreshControl={
            <RefreshControl refreshing={false} onRefresh={LoadDataConfirm} />
          }
        />
      </View>
      <ActionSheet
      ...
      >
        <SafeAreaView
          style={{ width: '100%', height: deviceHeight / 2, margin: 8 }}
        >
          <Text style={styles.titleSheet}>{itemConfirm.dateView}</Text>
          <Text
            style={styles.titleEmployeeSheet}
          >{`${itemConfirm.EmployeeCode} - ${itemConfirm.EmployeeName}`}</Text>
          {errorValueUpload && (
            <Text style={styles.errorUploadTitle}>* {errorValueUpload}</Text>
          )}
          ...
          <TouchableOpacity
            ...
            onPress={handlerBackConfirm}
          >
            <View style={styles.backRegisterView}>
              <SpiralIcon name="close" size={21} color={appcolor.light} />
            </View>
          </TouchableOpacity>
