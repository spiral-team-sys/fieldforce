...
export const PlanOffice = ({ navigation }) => {
	...
	const [filter, setFilter] = useState({ month: moment().month() + 1, year: moment().year() })
	//
	const LoadData = async (_year, _month) => {
		await setLoading(true)
		const result = await WorkingPlanAPI.PlanOfficeByMonth((_year || filter.year), (_month || filter.month), indexTab)
					...
					renderItem={() => {
						return (
							<View style={styles.detailView}>
								<PlanDetailByDay dataDetails={dataDetailByDay}
									itemDay={itemDayMain} onCallBackData={GetDetailByDay} />
							</View>
						)
					}}
				/>
	...
	return (
		<View style={styles.mainContainer}>
			...
			<View style={styles.contentMain}>
				<CustomTab
					data={[{ tabId: 1, tabName: 'Lịch làm việc' }, { tabId: 2, tabName: 'Thành viên' }]}
					keyTabName="tabName"
