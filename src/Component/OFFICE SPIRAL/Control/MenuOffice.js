...
export const MenuOffice = ({ navigation, isLoading }) => {
	...
	const renderItem = ({ item, index }) => {
		return (
			<TouchableOpacity key={`menuoffc_${index}`} style={{ width: deviceWidth / 4.2 }} onPress={() => onItemPress(item)}>
	...
	const styles = StyleSheet.create({
		...
		titleMenu: { color: appcolor.blacklight, fontSize: 11, fontWeight: '500', minHeight: 24, textAlign: 'center' },
