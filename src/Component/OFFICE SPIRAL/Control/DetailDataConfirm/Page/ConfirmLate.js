...
export const ConfirmLate = ({ dataDetail, itemMain }) => {
	...
	const handlerConfirm = (value) => {
		const _value = itemData.WPLateConfirm == value ? 3 : value
		itemData.WPLateConfirm = _value
		const dataUpdate = dataDetail.map((item, _index) => ({ ...item, WPLateConfirm: _value }))
	...
	const onChangeNote = (text) => {
		if (itemData.WPLateConfirm == -1 && (text == null || (text?.length || '') == 0)) {
			itemMain.isValidData = true
			error.isError = true
		...
		itemData.WPLateConfirmNote = text
		const dataUpdate = dataDetail.map((item, _index) => ({ ...item, WPLateConfirmNote: text }))
		itemMain.dataDetail = JSON.stringify(dataUpdate)
		setMutate(e => !e)
	}
	...
	return (
		<View style={styles.mainContainer}>
			...
			<View style={styles.contentSummary}>
				...
				{/* // Status Confirm */}
				{itemData.isConfirm !== 1 && itemData.WPLateConfirmNote !== null && itemData.WPLateConfirmNote !== undefined &&
			...
			<ConfirmData
				typeConfirm='WPLateConfirm'
				typeNote='WPLateConfirmNote'
