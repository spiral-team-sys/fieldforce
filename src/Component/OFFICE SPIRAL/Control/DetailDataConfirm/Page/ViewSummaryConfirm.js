...
export const ViewSummaryConfirm = ({ itemMain, dataSummary, actionSelected }) => {
		...
		dataMain.map((item, index) => {
			...
			return (
				<TouchableOpacity key={`item_emp_${index}`} onPress={onSelected}>
					<Text style={styles.subTitleView}>{`${item.EmployeeCode} - ${item.EmployeeName}`}</Text>
...
const SummaryByEmployee = ({ data, styles }) => {
	...
	return (
		<ScrollView contentContainerStyle={{ width: '100%', justifyContent: 'center' }}
			horizontal showsHorizontalScrollIndicator={false} scrollEnabled={false} >
			{data !== null && data.length > 0 &&
				data.map((item, index) => {
					return (
						<View key={`item_emp_${index}`} style={styles.itemMain}>
							<Icon name={item.IconStatus} size={21} color={appcolor[item.ColorStatus]} />
