...
export const ConfirmOFF = ({ dataDetail, itemMain }) => {
	...
	const handlerConfirm = (value) => {
		...
		const dataUpdate = dataDetail.map((item, _index) => ({ ...item, WPConfirm: _value }))
	...
	const onChangeNote = (text) => {
		if (itemData.WPConfirm == -1 && (text == null || (text?.length || '') == 0)) {
			itemMain.isValidData = true
			error.isError = true
		...
		const dataUpdate = dataDetail.map((item, _index) => ({ ...item, WPConfirmNote: text }))
		itemMain.dataDetail = JSON.stringify(dataUpdate)
		setMutate(e => !e)
	}
	...
	const styles = StyleSheet.create({
		...
		contentSummary: { width: '80%', minHeight: 80, marginStart: 8, justifyContent: 'center', backgroundColor: appcolor.light },
	...
	return (
		<View style={styles.mainContainer}>
			...
			<View style={styles.contentSummary}>
				...
				{/* // Status Confirm */}
				{itemData.isConfirm !== 1 && itemData.WPConfirmNote && <Text style={styles.descriptionSummary}>{`QL ghi chú: ${itemData.WPConfirmNote}`}</Text>}
				{/* // Note */}
