...
export const RegisterWorkEarlier = ({ dataDetail, itemMain, actionBack }) => {
  ...
  const LoadData = () => {
    ...
    if ((itemDetail?.EarlierTime || null) !== null) {
      const hour = parseInt(
        moment(new Date(itemDetail.EarlierTime)).format('HH'),
      );
      const minute = parseInt(
        moment(new Date(itemDetail.EarlierTime)).format('mm'),
      );
      registerSave.earlierTitle = `${
        hour > 0 ? `${hour}H ` : ''
      }${minute} Phút`;
  ...
  const handlerCancel = () => {
    alertConfirm(
      'Huỷ yêu cầu',
      'Bạn có muốn huỷ yêu cầu xin về sớm không?',
      async () => {
        const itemSave = {
          earlierNote: null,
          earlierTime: null,
  ...
  const validData = () => {
    if (sliderValue > 0) {
      if (
        registerSave?.earlierNote == null ||
        registerSave.earlierNote?.length == 0
      ) {
