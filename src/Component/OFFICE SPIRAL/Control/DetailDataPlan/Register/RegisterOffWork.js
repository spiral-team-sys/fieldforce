...
export const RegisterOffWork = ({ dataDetail, itemMain, actionBack }) => {
  ...
  const LoadData = () => {
    ...
    if ((itemDetail?.WPChangeShift || null) !== null) {
      const shiftChange =
        _.filter(itemDetail.DataShift, e => {
          return e.ShiftCode == itemDetail.WPChangeShift;
        })[0] || {};
      ...
      if ((itemDetail?.ChangeTimeFrom || null) !== null) {
        const hour = parseInt(
          moment(new Date(itemDetail.ChangeTimeFrom)).format('HH'),
        );
        const minute = parseInt(
          moment(new Date(itemDetail.ChangeTimeFrom)).format('mm'),
        );
        registerSave.sliderValueFrom = minute + hour * 60;
      }
      if ((itemDetail?.ChangeTimeTo || null) !== null) {
        const hour = parseInt(
          moment(new Date(itemDetail.ChangeTimeTo)).format('HH'),
        );
        const minute = parseInt(
          moment(new Date(itemDetail.ChangeTimeTo)).format('mm'),
        );
  ...
  const validData = async () => {
    if (
      registerSave.shiftCode == null ||
      registerSave.shiftCode == '' ||
      registerSave.shiftCode.length == 0
    ) {
    ...
    if (
      ...
      (registerSave.sliderValueFrom == 0 || registerSave.sliderValueTo == 0)
    ) {
    ...
    <View style={styles.mainContainer}>
      ...
      <View style={styles.contentView}>
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={{ marginTop: -8 }}>{renderShiftView()}</View>
          {registerSave.isTimeWorking == 1 && (
            <View style={styles.mainTimer}>
              <Text
                style={styles.titleTimeHeader}
              >{`Thời gian: Từ ${registerSave.timeFrom} - Đến ${registerSave.timeTo}`}</Text>
