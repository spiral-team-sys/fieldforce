...
export const RegisterWorkLate = ({ dataDetail, itemMain, actionBack }) => {
	...
	const LoadData = () => {
		...
		if ((itemDetail?.LateTime || null) !== null) {
			...
			const minute = parseInt(moment(new Date(itemDetail.LateTime)).format('mm'))
	...
	const handlerSendRegister = async () => {
		...
		if (checkData) {
			...
			await WorkingPlanAPI.PlanOfficeRegister('LATE', itemSave, async (result) => {
	...
	const handlerCancel = () => {
		alertConfirm('Huỷ yêu cầu', 'Bạn có muốn huỷ yêu cầu xin đi trễ không?', async () => {
			const itemSave = {
				lateNote: null,
				lateTime: null,
				planId: itemMain.planId
			}
			await WorkingPlanAPI.PlanOfficeRegister('CANCEL_LATE', itemSave, async (result) => {
				ToastSuccess(result.messeger, 'Thông báo', 'top')
				if (result.status == 200)
					actionBack()
			})
	...
	const validData = () => {
		if (sliderValue > 0) {
			if (registerSave?.lateNote == null || registerSave.lateNote?.length == 0) {
	...
	const onChangeTime = (value) => {
		...
		registerSave.lateTitle = `${change.hour() > 0 ? `${change.hour()}H ` : ''}${change.minute()} Phút`
		if (value > 0)
			setErrorValue(null)
		setMutate(e => !e)
	}
			...
			<View style={styles.contentView}>
				<View style={{ width: '100%', flexDirection: 'row', alignItems: 'center' }}>
					<View style={styles.buttonChangeTime}>
						<Text style={{ ...styles.titleNameButton, color: appcolor.primary }}>{registerSave.lateTitle}</Text>
					</View>
				...
				<View style={styles.mainButtonAction}>
					...
					{(itemData?.WPLateConfirm || 0) == 0 && (itemData?.isLockSend || 0) == 0 &&
