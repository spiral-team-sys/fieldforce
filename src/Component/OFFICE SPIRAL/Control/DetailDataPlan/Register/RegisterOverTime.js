...
export const RegisterOverTime = ({ dataDetail, itemMain, actionBack }) => {
	...
	const LoadData = () => {
		...
		if ((itemDetail?.OTCode || null) !== null) {
			...
			if ((itemDetail?.TimeFrom || null) !== null) {
				const hour = parseInt(moment(new Date(itemDetail.TimeFrom)).format('HH'))
				const minute = parseInt(moment(new Date(itemDetail.TimeFrom)).format('mm'))
			...
			if ((itemDetail?.TimeTo || null) !== null) {
				...
				const minute = parseInt(moment(new Date(itemDetail.TimeTo)).format('mm'))
	...
	const validData = async () => {
		if (registerSave.typeOT == null || registerSave.typeOT == '' || registerSave.typeOT.length == 0) {
	...
	const handlerCancel = () => {
		alertConfirm('Huỷ yêu cầu', 'Bạn có muốn huỷ yêu cầu xin nghỉ không?', async () => {
			...
			await WorkingPlanAPI.PlanOfficeRegister('CANCEL_OT', itemSave, async (result) => {
				ToastSuccess(result.messeger, 'Thông báo', 'top')
				if (result.status == 200)
					actionBack()
			})
			...
			<View style={styles.contentView}>
				<ScrollView showsVerticalScrollIndicator={false}>
					...
					<View style={styles.mainTimer}>
						<Text style={styles.titleTimeHeader}>{`Thời gian: Từ ${registerSave.timeOTFrom} - Đến ${registerSave.timeOTTo}`}</Text>
						<View style={{ marginEnd: 8, marginTop: 16 }}>
							...
							{registerSave !== null && Object.keys(registerSave).length > 0 &&
								<Slider
					...
					<View style={styles.mainButtonAction}>
						...
						{(itemData?.OTConfirm || 0) == 0 && (itemData?.isLockSend || 0) == 0 &&
