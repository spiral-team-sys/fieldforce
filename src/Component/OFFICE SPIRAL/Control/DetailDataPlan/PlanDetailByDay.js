...
export const PlanDetailByDay = ({ dataDetails, itemDay, onCallBackData }) => {
	...
	const LoadDataDetail = () => {
		const _attendants = _.filter(dataDetails, (e) => { return e.typePage == 'ATTENDANT' })
		setDataAttendant(_attendants)
		const _register = _.filter(dataDetails, (e) => { return e.typePage !== 'ATTENDANT' })
	...
	const renderPageAttendant = () => {
		...
		return (
			dataAttendant !== null && dataAttendant.length > 0 &&
	...
	const renderPageView = () => {
		...
		return (
			dataRegister !== null && dataRegister.length > 0 &&
			dataRegister.map((item, index) => {
				const onRegister = () => {
					if (item?.isChange == 1)
						handlerRegister(item)
				}
				return (
					<TouchableOpacity key={`pid_${index}`} style={{ width: deviceWidth }} onPress={onRegister}>
						{itemView(item)}
					</TouchableOpacity>
				)
			})
	...
	const renderRegisterMain = (item) => {
		const itemView = () => {
			...
			switch (item.typePage) {
				case UtilityOffice.typeOT:
					return <RegisterOverTime dataDetail={dataDetail} itemMain={item} actionBack={onCallBack} />
				case UtilityOffice.typeOFF:
					return <RegisterOffWork dataDetail={dataDetail} itemMain={item} actionBack={onCallBack} />
				case UtilityOffice.typeLATE:
					return <RegisterWorkLate dataDetail={dataDetail} itemMain={item} actionBack={onCallBack} />
				case UtilityOffice.typeEARLIER:
					return <RegisterWorkEarlier dataDetail={dataDetail} itemMain={item} actionBack={onCallBack} />
				default:
					return null
			}
		}
	...
	const styles = StyleSheet.create({
		...
		titleSheet: { width: '80%', fontSize: 18, fontWeight: '700', color: appcolor.blacklight },
		contentRegister: { width: deviceWidth - 16, padding: 8 },
		backMain: { width: '15%', padding: 8, alignItems: 'flex-end', marginEnd: 8 },
		...
		<View style={styles.mainContainer}>
			<View style={{ flexDirection: 'row', alignItems: 'center' }}>
				...
				{itemDay.isRegisterByDay == 1 &&
					<TouchableOpacity
						style={{ width: 80, backgroundColor: appcolor.yellowdark, alignItems: 'center', padding: 6, borderRadius: 5, position: 'absolute', end: 0 }}
						onPress={handlerShowRegisterWork}>
						<Text style={{ fontSize: 13, fontWeight: '400', color: appcolor.blacklight }} >{`Đăng kí`}</Text>
					</TouchableOpacity>
				}
			</View>
			<View style={styles.contentMain}>
				{renderPageAttendant()}
				<ActionSheet id="officework"
					gestureEnabled={false}
					closeOnTouchBackdrop={!isRegister}
					closable={!isRegister} >
					<SafeAreaView style={{ width: '100%', height: deviceHeight / 1.5, margin: 8 }}>
						<View style={{ flexDirection: 'row', alignItems: 'center' }}>
							<Text style={styles.titleSheet}>{itemMain.dateView}</Text>
							<TouchableOpacity style={styles.backMain} onPress={handlerBackRegister}>
								<View style={styles.backRegisterView}>
									<Icon name={isRegister ? "arrow-back" : "close"} size={21} color={appcolor.light} />
								</View>
							</TouchableOpacity>
						</View>
						<View style={styles.contentRegister}>
							{isRegister ?
								renderRegisterMain(itemRegister)
								:
								renderPageView()
							}
						</View>
