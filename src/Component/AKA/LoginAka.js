...
import {
    AppNameBuild, APPNAME, signifyApp,
    AppStoreURL
}
    from './../../Core/URLs';
...
class LoginAka extends PureComponent {
    ...
    async SignIn() {
        ...
        await onLogin(requestInfo, async (res) => {
            await saveTokenUserToGapp(res, this.props.GAppController).then(success => {
    ...
    stylesGradient = StyleSheet.create({
        ...
        mainContainer: { width: '100%', height: '100%', backgroundColor: this.props.appcolor.light, padding: 24 },
        ...
        titleLogin: { width: '100%', padding: 16, fontSize: scaleSize(28), fontWeight: '700', color: this.props.appcolor.dark },
        titleForgot: { fontSize: scaleSize(14), color: this.props.appcolor.dark, fontWeight: '600', textAlign: 'center', position: 'absolute', right: 16 },
        ...
        switch (AppNameBuild) {
                            ...
                            this.state.requestPass ?
                                ...
                                <View style={{ justifyContent: 'space-between', alignItems: 'stretch', flex: 1, backgroundColor: appcolor.light }}>
                                    ...
                                    resizeMode='contain' /> */}
                                        ...
                                        <View style={{
                                            paddingLeft: 12, paddingRight: 12, margin: 12, backgroundColor: appcolor.light,
                                            borderTopColor: appcolor.primary, borderTopWidth: 11, borderBottomColor: appcolor.primary,
                                            borderBottomWidth: 14, borderRadius: 42, borderLeftWidth: 0.5, borderRightWidth: 0.1
                                        }}>
                                            <Text style={{
                                                marginLeft: 12, width: '100%', fontWeight: "500", fontSize: scaleSize(23),
                                                color: appcolor.primary, padding: 7
                                            ...
                                            <View>
                                                <Input
                                                    style={{ fontSize: 15, color: appcolor.dark, margin: 0, borderWidth: 0 }}
                                                    ref={ref => { this._refPass = ref }}
                                                    returnKeyType='done'
                                                    onSubmitEditing={() => this.state.accept ? this.onLoginPress() : null}
                                                    placeholder='Mật khẩu'
                                                    placeholderTextColor='#c2c2c2'
                                                    leftIcon={<Icon name='lock-closed-outline' type='ionicon'
                                                        color={appcolor.primary} size={23} style={{ padding: 0 }} />}
                                                    rightIcon={
                                                        <Icon
                                            ...
                                            <TouchableOpacity onPress={() => this.props.navigation.navigate("WebView", { urlPage: `${AppStoreURL}/privacyvn.html` })} style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                <CheckBox onPress={() => this.setState({ accept: !this.state.accept })} checked={this.state.accept} style={{ marginRight: 1 }} />
                                                <Text style={{ color: appcolor.dark, fontSize: 10, marginRight: 80 }}>Bằng việc đăng nhập bạn đã đồng ý với <Text style={{ color: appcolor.danger, fontSize: 10, fontStyle: 'italic', textDecorationLine: 'underline' }}>
                                                    Điều khoản, điều kiện cùng chính sách bảo mật chia sẽ thông tin của ứng dụng</Text>
                                                </Text>
                                            </TouchableOpacity>
                                            <View style={{ flexDirection: 'row' }}>
                                                <Button buttonStyle={{
                                                    ...
                                                    title="Lấy lại mật khẩu" onPress={() => this.setState({ requestPass: true })} />
                                                <Button disabled={!this.state.accept}
                                        ...
                                        <View style={{ position: 'absolute', zIndex: -1, justifyContent: 'center', bottom: 40, width: '100%', }}>
            ...
            default:
                return <View style={{ flex: 1, backgroundColor: appcolor.light }} >
                    <Text style={{ color: appcolor.danger, fontSize: 20, textAlign: 'center' }}>No App Config</Text>
                </View>
        }
    }
}
