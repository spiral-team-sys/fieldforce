...
import { View, Text, AppState, RefreshControl, ScrollView, DeviceEventEmitter, Modal } from 'react-native';
import { Divider } from '@rneui/themed';
import Moment from 'moment';
import { GetEmployeeInfo, ManageMessenger, saveStore, getStore, ToastSuccess, GetToken } from '../../Core/Helper';
...
class HomeAka extends PureComponent {
    ...
    async componentDidMount() {
        ...
        this.updatealldata = DeviceEventEmitter.addListener('updatealldata', () => { this.SyncdataApp() })
    }
    ...
    messageInApp = async (remoteMessage) => {
        console.log(remoteMessage, 'getInitialNotificationgetInitialNotificationgetInitialNotification');
        if (remoteMessage !== null) {
            ...
            <Fragment>
                {
                    this.state.welcome === 1 ?
                        ...
                        userinfo.employeeId !== undefined ?
                                        ...
                                        <View style={{ height: 240, paddingEnd: 0 }}>
                                            ...
                                            {chartdata.length > 0 &&
                                                <ScrollView
                                                    ...
                                                    nestedScrollEnabled
                                                    pagingEnabled
                                                    contentContainerStyle={{ width: chartdata.length * deviceWidth }}
                                                    horizontal
                                                >
                                                    {
                                                        chartdata.map((it => {
                                                            if (it.pageName == 'ROUTING') {
                                                                return <View key={`db_Routing`} style={{ width: deviceWidth, backgroundColor: appcolor.surface, borderRadius: 10, marginBottom: 10, overflow: 'hidden', padding: 4 }}>
                                                                    <DashboardRouting navigation={this.props.navigation} data={(JSON.parse(it.chartData)[0]) || {}} />
                                                                </View>
                                                            } else if (it.pageName == 'SELLIN') {
                                                                return <View key={`db_Sellin`} style={{ width: deviceWidth, backgroundColor: appcolor.surface, borderRadius: 35, marginBottom: 10, overflow: 'hidden', padding: 4 }}>
                                                                    <DashboardSellInTF navigation={this.props.navigation} typeDashboard={it.pageName} />
                                                                </View>
                                                            } else if (it.pageName !== 'SELLIN' && it.pageName !== 'ROUTING') {
                                                                return <View key={'Dashboard_' + it.pageName} style={{ width: deviceWidth, backgroundColor: appcolor.light, borderRadius: 35, marginBottom: 10, overflow: 'hidden', padding: 10, margin: 4 }}>
                                                                    <DashBoardTargetTF navigation={this.props.navigation} typeDashboard={it.pageName} />
                                                                </View>
                                                            } else return null
                                                        }))
                                                    }
                            ...
                            : <LoginAka {...this.props} onLoginCallBack={() => this.SyncdataApp()} />
                }
            </Fragment>
        );
    }
