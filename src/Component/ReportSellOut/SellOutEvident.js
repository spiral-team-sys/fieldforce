...
import { fetchSellOutEvident, uploadEvident, uploadPhoto } from '../../Controller/SellOutController';
...
class SellOutEvident extends PureComponent {
    constructor(props) {
        ...
        this.state = {
            ...
            markedDates: { [moment(new Date()).format('YYYY-MM-DD').toString()]: { selected: true, marked: true, selectedColor: DEFAULT_COLOR } },
            isStartDay: false,
            isEndDay: false,
            startDate: '',
            dataItem: {}
        }
        this.markedDatesDefault = { [this.state.toDay]: { selected: true, marked: true, selectedColor: DEFAULT_COLOR } }
    ...
    takePhoto = (item) => {
        let photoType = item.productId !== undefined ? 'EVIDENT_' + item.productName : 'EVIDENT_FULL';
        let photoInfo = {
            "reportId": 0,
            "shopId": item.shopId,
    ...
    handlerSelectDate = (dateString) => {
        if (dateString !== null && dateString !== undefined) {
            if (this.state.startDate === dateString || dateString < this.state.startDate) {
                this.setState({
        ...
        if (typeFilter == 'APPLY') {
            if (this.state.dateFilter.length > 0) {
                this.state.lstDataContent.filter(item => {
                    let items = this.state.dateFilter.filter(mFilter => item.sellDate.indexOf(mFilter) > -1)
                    if (items.length > 0) {
                        sortList.push(item)
                    }
                })
    ...
    handlerInputKeyFilter = (strKey) => {
        let lstFilter = this.state.lstDataContent.filter((i) => (
        ...
        this.setState({
            lstData: strKey !== null && strKey !== undefined ? lstFilter : this.state.lstDataContent
        })
    }
    ...
    handlerUpload = async (item) => {
        //console.log(item)
        await uploadEvident(item, async (message) => {
            alertNotify(message.toString())
        }, (countImage) => {
            let count = item.photoCount + countImage;
            let countFull = item.photoCountFull + countImage;
            item.productId !== undefined && this.setState({ lstData: this.state.lstData.map(i => (i.stt === item.stt ? { ...i, photoCount: count } : i)) })
            item.productId === undefined && this.setState({ lstData: this.state.lstData.map(i => (i.stt === item.stt ? { ...i, photoCountFull: countFull } : i)) })
        })
    ...
    renderButtonAction = (title, icon, typeAction) => {
        return (
            <Button type='clear'
                icon={<Icon type='material-community' name={icon} size={20} color={DEFAULT_COLOR} />}
    ...
    renderItem = ({ item }) => {
        ...
        return (
            <View style={{ flex: 1, flexDirection: 'column' }}>
                <View style={{ ...styles.rowTitleSellOutCheck, display: item.titleItem.length > 0 ? 'flex' : 'none' }}>
                    <Text style={{ flex: 3, fontWeight: '500', fontSize: 15 }}>{item.titleItem}</Text>
                    <Button type='outline'
                        icon={<Icon type='material-community' name='camera' size={20} color='black' />}
                        style={{ flex: 1 }}
                        titleStyle={{ fontSize: 13, color: item.colorConfirmTL, marginEnd: 3 }}
                        buttonStyle={{ borderColor: 'black', marginEnd: 3 }}
                        onPress={() => this.showBottomButton(itemShop)}
                    />
                    <Button type='outline'
                        icon={<Icon type='material-community' name='upload' size={20} color='black' />}
                        style={{ flex: 1 }}
                        title={item.photoCountFull + ' Hình'}
                        titleStyle={{ fontSize: 13, color: item.colorConfirmTL, padding: 8 }}
                        buttonStyle={{ borderColor: 'black' }}
                        onPress={() => this.handlerUpload(itemShop)}
                    />
                </View>
                <View style={{ marginBottom: 8, borderColor: '#e2e2e2', borderWidth: 0.5, borderRadius: 5 }}>
                    <View style={{ padding: 8, flex: 1, flexDirection: 'column' }}>
                        <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                            <Badge
                                status='primary'
                                badgeStyle={{ height: 32, width: 32, borderRadius: 20, marginEnd: 8 }}
                                value={item.quantity} />
                            <Text style={{ flex: 1, fontSize: 15, fontWeight: '500' }}>{item.productName}</Text>
                            <Button type='outline'
                                icon={<Icon type='material-community' name='camera' size={20} color={DEFAULT_COLOR} />}
                                style={{ flex: 1 }}
                            ...
                            />
                            <Button type='outline'
                                icon={<Icon type='material-community' name='upload' size={20} color={DEFAULT_COLOR} />}
                                style={{ flex: 1 }}
                        ...
                        {item.adminNote !== null && item.adminNote !== undefined &&
                            <Text style={{ paddingBottom: 3, fontSize: 15, fontWeight: '500', color: 'red' }}>Reject: Sai hoa don</Text>
                        }
                    </View>
                </View>
            </View>
    ...
    render() {
        return (
            <View style={{ flex: 1, backgroundColor: 'white' }}>
                ...
                <View style={{ flex: 1, flexDirection: 'column' }}>
                    <SearchBar
                        containerStyle={{ borderTopColor: 'white', backgroundColor: appcolor.transparent, borderBottomColor: appcolor.transparent }}
                ...
                <Modalize ref={modalRef} modalHeight={450}>
                    <View View style={{ flex: 1, margin: 8 }}>
                        <View style={{ flex: 1, marginBottom: 8, flexDirection: 'row-reverse' }}>
                            <Button
                                type='solid' title='Áp dụng' titleStyle={{ fontSize: 12 }} buttonStyle={{ marginStart: 5 }}
                                onPress={() => this.handlerApplyFilter('APPLY')} />
                            <Button
                                type='outline' title='Làm mới' titleStyle={{ fontSize: 12 }}
                ...
                <Modalize ref={actionRef} modalHeight={230} keyboardAvoidingOffset={50} >
                    <View style={{ flex: 1, flexDirection: 'column', margin: 16 }}>
                        <Text style={{ ...styles.title, textAlign: 'center', marginBottom: 8 }}>Upload hình ảnh</Text>
                        {this.state.dataItem.productId !== undefined && <Text style={{ fontSize: 15, fontWeight: '500', textAlign: 'center', marginBottom: 8 }}>{this.state.dataItem.productName}</Text>}
                        {this.renderButtonAction('Chụp hình', 'camera', this.aCamera)}
                        {this.renderButtonAction('Chọn hình ảnh từ thư viện', 'upload', this.aUploadFile)}
                        {this.renderButtonAction('Xem hình ảnh', 'image-album', this.aViewPhoto)}
                    </View>
                </Modalize>
            </View>
        )
