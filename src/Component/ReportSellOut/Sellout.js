...
import { View, TouchableOpacity, Text, Keyboard, SafeAreaView } from "react-native";
...
import { getResSellOut, SellOutGetList, SELLOUTContext } from "../../Controller/SellOutController";
import { checkLockReport, getStoreListSO } from "../../Controller/ShopController";
import { SelloutResRow } from "../../Content/SelloutResRow";
import { getRequestSellout } from "../../Controller/MasterController";
import { getCategorySO, getSegmentSO, getSubSegmentSO, getProductSO, getNOSELLSO, getSubCategorySO, getCompetitorSO } from "../../Controller/ProductController";
...
const Sellout = (props) => {
    ...
    const [auditDate, setAuditDate] = useState(moment(new Date()).format('YYYYMMDD'));
    const [infoSave, setInfoSave] = useState({ tickInfoSave: false, dataInfoSave: {} });
    ...
    const canEditSellout = Number(workinfo?.workDate) === currentDate && !lockReport;
    ...
    const selloutLoad = async () => {
        ...
        setSellouts(previousSellouts => {
            if ((!sellout || sellout.length === 0) && total > 0 && previousSellouts.length > 0) {
    ...
    const Upload = async () => {
        ...
        await UploadController.DataSellout({ ...workinfo, reportId: config.isUseSellOut == 1 ? 5 : kpiinfo.id }, async () => {
    ...
    // #endregion
    const canUseNosell = IdNosell !== 0 && typeId !== 170 && canEditSellout && !idHaveNosell && !isHiddentNosell;
                    ...
                    {sellouts?.length > 0 &&
                        <View style={{ height: '100%', width: '100%' }}>
                            <CustomListView
                                ...
                                data={sellouts}
                                extraData={[sellouts, totalSell, idHaveNosell, isHiddentNosell, listVersion]}
                                renderItem={({ item, index }) => (
                                    <SelloutResRow key={"9992" + index} item={item} index={index}
                                        selloutLoad={selloutLoad} ShowDetail={onShowDetail}
                                        canEdit={canEditSellout}
                                        onBlockedAction={() => toastError('Bạn đang báo cáo dữ liệu ngày cũ!')}
                                        Props={props} workinfo={workinfo} appcolor={appcolor} />
                                )}
                                ListFooterComponent={<View style={{ height: deviceHeight / 3, width: '100%' }} />}
                                keyExtractor={(_, index) => index.toString()}
                            />
                        </View>
                    }
                    {canAddSellout &&
                        <TouchableOpacity style={{ flexDirection: 'row', position: 'absolute', zIndex: 10, right: 12, bottom: 32 }}
                            onPress={onAddSellOut}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', borderRadius: 30, backgroundColor: appcolor.primary }}>
                                <Text style={{ paddingStart: 16, color: appcolor.white, textAlignVertical: 'center', fontSize: 14, fontWeight: fontWeightBold }}>Tổng số lượng {totalSell}</Text>
                                <Icon raised size={16} name='add-circle-outline' type='ionicon' />
                            </View>
                        </TouchableOpacity>
                    }
                    {
                        ...
                        <TouchableOpacity style={{
                            position: 'absolute', zIndex: 11, bottom: 40, left: 8, maxHeight: 50,
                        }}
                            ...
                            <View style={{
                                ...
                                <Icon size={20} name='circle' type='font-awesome-5' color={appcolor.greylight} containerStyle={{ padding: 10 }} />
                                <Text style={{
                                    paddingEnd: 12, color: appcolor.dark, textAlignVertical: 'center', fontSize: scaleSize(20)
                                }}>No Sell</Text>
                            </View>
                        </TouchableOpacity>
                    }
                </View>}
            {display && <View style={{ flex: 1, backgroundColor: appcolor.light }}>
                <SelloutModel
