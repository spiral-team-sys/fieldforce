...
export const ApproachReport = ({ navigation }) => {
    ...
    for (let index = 0; index < data.length; index++) {
      ...
      if (item.isRequired == 1) {
        if (
          ...
          ToastError(
            `Ban phải nhập đầy đủ thông tin bắt buộc: ${item.name}`,
            'Thông báo',
            'top',
          );
          return false;
        ...
        if (item.ref_Id > 0 && item.numberValue == 0) {
          ToastError(
            `Ban phải nhập thông tin : ${item.name} lớn hơn 0`,
            'Thông báo',
            'top',
          );
          return false;
  ...
  const ResetDataAll = async () => {
    ...
    result.map(it => {
      const itemFilter = _.filter(data, e => {
        return e.id == it.id;
      });
    ...
    return (
      ...
      >
        <View style={{ marginBottom: 7 }}>
          <Text style={{ padding: 7, color: appcolor.dark }}>
            {item.name}
            {item?.isRequired == 1 && (
              <Text style={{ color: appcolor.danger }}>(*)</Text>
            )}
          </Text>
          <UIControl
            key={item.id}
