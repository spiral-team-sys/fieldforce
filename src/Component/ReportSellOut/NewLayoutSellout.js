...
class Sellout extends PureComponent {
  ...
  async loaddata() {
    ...
    if (this.state.WFHome !== true) {
      ...
      if (nosellProduct.length != 0) {
        this.setState({
          IdNosell: nosellProduct[0].productId,
          idHaveNosell: false,
          isHiddentNosell: false,
        });
        ...
        if (sellout.length > 0) {
          ...
          let haveNosell = sellout.filter(
            item => item.productId === nosellProduct[0].productId,
          );
      ...
      sellout.map(
        itemR =>
          itemR.guiId !== undefined &&
          itemR.guiId !== null &&
          this.mapItem(itemR, workinfo),
      );
  ...
  mapItem = async (item, workinfo) => {
    ...
    await this.setState({
      Sellouts: this.state.Sellouts.map(itemS =>
        itemS.guiId === item.guiId ? { ...itemS, numPhoto: lst.length } : itemS,
      ),
    });
  };
  async componentDidMount() {
    this.props.route?.params.wfh !== undefined &&
      (await this.setState({ WFHome: this.props.route?.params.wfh }));
    await this.setState({ workinfo: this.props.workinfo });
    await this.loaddata();
  }
  async AddSellOut() {
    const workinfo = this.state.workinfo;
    let workDate = this.state.WFHome
      ? parseInt(this.state.auditDate)
      : workinfo.workDate;
  ...
  async Upload(res) {
    ...
    let workDate = (await this.state.WFHome)
      ? parseInt(this.state.auditDate)
      : workinfo.workDate;
  ...
  async uploadSellout() {
    ...
    await Store().then(async db => {
      const { res, err } =
        this.state.WFHome === false
          ? await SellOutUpload(db, workinfo)
          : await SellOutSSUpload(db, this.state.auditDate);
          ...
          <View style={{ width: '100%', height: 100 }}>
            ...
            {this.state.IdNosell !== 0 && this.state.typeId !== 170 && (
              <Button
                ...
                disabled={
                  this.state.idHaveNosell === false
                    ? this.state.isHiddentNosell === false
                      ? false
                      : true
                    : true
                }
