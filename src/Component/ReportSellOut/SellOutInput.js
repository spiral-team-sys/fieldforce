...
import { SafeAreaView, StatusBar, TouchableOpacity, View, Text, FlatList } from 'react-native'
                    ...
                    Layout.push(
                        <View style={{ padding: 7, backgroundColor: appcolor.light }}>
                            <View style={{ padding: 12 }}>
                                <Text style={{ fontWeight: '900', marginBottom: 7, fontSize: scaleSize(16), color: appcolor.dark }}>{item.title}</Text>
                                <NumPad placeholderText={item.title} value="1" handerNumberChange={(e) => onSeletedItem(item, e)} />
                            </View>
                            <FlatList
                                data={item.list}
    ...
    return (
        <Fragment>
            <SafeAreaView style={{ backgroundColor: appcolor.light, }}>
                <View style={{ flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', marginEnd: 7, marginTop: StatusBar.currentHeight }}>
                    <Text style={{ textAlignVertical: 'center', fontSize: scaleSize(20), padding: 7 }}>Nhập doanh số bán hàng</Text>
                    <TouchableOpacity>
                        <Icon size={28} color={appcolor.dark} name='close' />
                    </TouchableOpacity>
                </View>
