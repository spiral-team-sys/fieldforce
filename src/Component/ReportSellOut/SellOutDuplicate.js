...
import { View, Text, KeyboardAvoidingView, Platform, FlatList, TextInput, StyleSheet } from 'react-native';
import { fetchSellOutDuplicate, confirmDuplicate, updatePhoneNumber } from '../../Controller/SellOutController';
...
class SellOutDuplicate extends PureComponent {
    ...
    handlerGetData = async () => {
        ...
        let mDataTab = [{ tabId: 1, tabName: 'Kiểm tra trùng SP' }, { tabId: 0, tabName: 'Cập nhật SĐT' }]
        ...
        this.setState({ lstData: mDataSellOut, lstDataContent: mDataSellOut, lstTabView: mDataTab })
    }
    handlerActionDuplicate = async (item) => {
        const labelAlert = item.duplicatePS == 0 ? 'Bạn có muốn xoá sản phẩm này không ?' : 'Sản phẩm đang chờ xác nhận, Bạn có muốn huỷ không ?';
        if (item.isPhoneInput === 1) {
            Message('Thông báo', labelAlert, async () => {
                let lstDataDelete = this.state.lstData.map(i =>
                (i.itemId == item.itemId ? {
                    ...i, duplicatePS: i.duplicatePS == 1 ? 0 : 1, titleConfirmResult: i.duplicatePS == 0 ? 'Đang chờ xác nhận' : ''
                } : i))
                this.setState({
            ...
            if (item.phone == null || item.phone == undefined || item.phone.length < 1) {
            ...
            if (item.phone != null && item.phone.length > 9 && !this.regexPhone.test(item.phone)) {
                alertWarning("Số điện thoại không đúng định dạng, vui lòng thử lại")
                return;
            }
    ...
    handlerUpdatePhone = (item, str) => {
        let mUpdatePhone = this.state.lstData.map(i => (i.itemId === item.itemId ? { ...i, phone: str } : i))
        this.setState({
    ...
    renderTabView = () => {
        let dataGroup = [];
        return (
            this.state.lstTabView.map(item => {
                ...
                return (
                    <Tabs.Tab key={item.tabName} label={item.tabName} name={item.tabName} >
                        <View style={{ backgroundColor: appcolor.light, marginTop: 40, padding: 6, width: deviceWidth, height: '100%' }}>
                            ...
                            <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS == "ios" ? "padding" : "height"} keyboardVerticalOffset={200} >
                                <FlatList
                                    style={{ padding: 8 }}
    ...
    renderItem = ({ item }) => {
        ...
        return (
            <View style={{ flex: 1, flexDirection: 'column' }}>
                <Text style={{ fontWeight: '500', fontSize: 15, marginBottom: 8, padding: 8, display: item.contactName.length > 0 ? 'flex' : 'none', backgroundColor: '#ffd600' }}>{item.contactName}</Text>
                    ...
                    <View style={{ padding: 8, flex: 1, flexDirection: 'column' }}>
                        <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                            ...
                            <Button type='outline'
                                icon={
                                    item.isPhoneInput == 1 ?
                                        <Icon type='material-community' name={mIconDuplicate} size={20} color={item.colorConfirmTL} /> :
                                        <Icon type='material-community' name='update' size={20} color={DEFAULT_COLOR} />
                                }
                                style={{ flex: 1 }}
                                ...
                                onPress={() => { this.handlerActionDuplicate(item) }}
                            />
                        </View>
                        ...
                        <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                            ...
                            <TextInput
                                ...
                                maxLength={11}
                                value={item.phone}
                                style={{ ...styles.inputNumber, marginStart: 0, width: 150, textAlign: 'left' }}
                    ...
                    <Tabs.Container
                        renderTabBar={props => (
                            <MaterialTabBar
                                {...props}
                                scrollEnabled={true}
                                tabStyle={{ minWidth: minWidthTab(this.state.lstTabView), height: 32 }}
