            ...
            <View key={'Dashboard' + index} style={{ width: deviceWidth }}>
              <View style={{ padding: 8, alignItems: 'center' }}>
                ...
                {(JSON.parse(item) == type.SellOut ||
                  JSON.parse(item) == type.SellOutByHiend) && (
                  <View
                    key={'SellOut_' + JSON.parse(item) + '_' + index}
                    style={styles.itemView}
                  >
                    <DashboardSellOutToshiba
                      navigation={navigation}
                ...
                {JSON.parse(item) == type.SellOutByCate && (
                  <View key={'SellOutByCate_' + index} style={styles.itemView}>
                    {/* <SellOutSummaryByCate
                                            ...
                                            navigation={navigation} /> */}
                    <DashboardSellOutByCateToshiba
                      navigation={navigation}
                      typeDashboard={type.SellOutByCate}
                      viewHeight={viewHeight}
                    />
                  </View>
                )}
                {JSON.parse(item) == type.SummaryByStore && (
                  <View
                    key={'Dashboard_' + index + '_' + JSON.parse(item)}
                    style={styles.itemView}
                  >
                    <DashBoardSellStore
                      navigation={navigation}
                      typeDashboard={type.SummaryByStore}
                      viewHeight={viewHeight}
                    />
                  </View>
                )}
