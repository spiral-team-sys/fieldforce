...
import { View, Text, FlatList, Platform, TouchableOpacity, RefreshControl, Linking, Image, ActivityIndicator, ScrollView } from "react-native";
...
const ShopOneMoreToshiba = ({ shops, navigation }) => {
    ...
    const [dataItemFinish, setDataItemFinish] = useState({ dataAtt: [], dataItem: {} })
    ...
    const onFinishPress = async (item) => {
        let dataByShop = await AttendantController.GetAttendant(item, item.auditDate)
    ...
    renderItem = ({ item, index }) => {
        ...
        const sourceImage = item.imageUrl !== null && item.imageUrl.indexOf('file://') === - 1 && !item.imageUrl.includes('http') ? URLDEFAULT + item.imageUrl : item.imageUrl
        return (
            index < 3 ?
                        ...
                        <View style={{ borderRadius: 20 }}>
                            {sourceImage !== null ?
                                <Image
                                    ...
                                    style={{ borderRadius: 10, height: deviceWidth * 0.55, resizeMode: 'cover' }}
                                    resizeMethod='resize'
                                /> : <Image
                            ...
                            <View style={{
                                width: '100%', height: deviceWidth * 0.55, borderRadius: 20,
                                position: 'absolute', backgroundColor: 'rgba(97, 96, 101, 0.3)', zIndex: 10
                            }} />
                            <View style={{
                                width: '70%', position: 'absolute', top: 60, left: 10, backgroundColor: appcolor.transparent, zIndex: 10000
                            }}>
                                <Text style={{ fontSize: scaleSize(26), fontWeight: '700', color: appcolor.white }}>{item.title}</Text>
                                <Text style={{ fontSize: scaleSize(14), fontWeight: '500', color: appcolor.placeholderBody, opacity: 0.8 }}>{'Mã CH : ' + item.shopCode}</Text>
                            </View>
                            {
                                ...
                                <TouchableOpacity
                                    ...
                                    style={{
                                        borderRadius: 50,
                                        width: '40%', position: 'absolute', top: 10, right: 10, backgroundColor: 'rgba(97,96,101,0.6)', zIndex: 10000, flexDirection: 'row', justifyContent: "flex-end", alignItems: 'center',

                                    }}>
                                    <Text style={{ fontSize: scaleSize(14), fontWeight: '500', color: appcolor.placeholderBody, textAlign: "center" }}>Hoàn thành</Text>
                                    <LottieView
                            ...
                            <View style={{
                                position: 'absolute', bottom: 10, right: 10, backgroundColor: appcolor.transparent, width: '90%', zIndex: 10000
                            }}>
                                <Text numberOfLines={2}
                                    style={{ paddingLeft: 7, color: appcolor.white, fontSize: scaleSize(12), textAlign: 'right' }}>
                                    {item.address || ""}</Text>
                                <View style={{ flexGrow: 1 }}></View>
                                <Text style={{ paddingLeft: 7, textAlign: 'right', color: appcolor.white, fontSize: scaleSize(12) }}>
    ...
    const configData = (dataPhoto) => {
        ...
        for (let i = 0; i < dataPhoto.length - 1; i++) {
            ...
            if ((i + 1) % 2 == 1 && (dataPhoto[i + 1].photoFullTime != null)) {
                dataAtt.push({ ...dataPhoto[i], titleImage: dataPhoto[i].photoType % 2 == 1 ? 'Check IN' : 'Check OUT' },
                    { ...dataPhoto[i + 1], titleImage: dataPhoto[i].photoType % 2 == 1 ? 'Check IN' : 'Check OUT' })
                const duration = moment.duration(moment(dataPhoto[i + 1].photoFullTime).diff(moment(dataPhoto[i].photoFullTime)));
                DataGroup.push({ photoId: dataPhoto[i].id, totalTime: moment.utc(duration.asMilliseconds()).format("HH:mm:ss"), dataAtt: dataAtt });
            }
        }
        return DataGroup
    }
        ...
        const shop = (
            <View key={indexCheck.toString()} style={{ padding: 5 }}>
                <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center' }}>
                    <Text style={{ color: appcolor.primary, textAlign: 'center' }}>Tổng</Text>
                    <Text style={{ color: appcolor.primary, textAlign: 'center', fontWeight: 'bold', }}>{itemCheck.totalTime}</Text>
                </View>
                    ...
                    renderItem={({ item, index }) => {
                        return (
                            <View style={{ height: '100%', padding: 4 }}>
                                <Image
                                    style={{ width: deviceWidth / 2 - 20, height: deviceWidth / 2 - 20, borderRadius: 25, backgroundColor: appcolor.surface }}
                                    source={{ uri: URLDEFAULT + item.photoPath }}
                                    // onPress={() => handlerShowImage(listPhoto, index)}
                                    PlaceholderContent={<ActivityIndicator />}
                                />
                                <Text style={{ color: appcolor.onBackground, textAlign: 'center', paddingTop: 10 }}>{item.photoFullTime}</Text>
                                <Text style={{ color: appcolor.primary, textAlign: 'center', }}>{item.titleImage}</Text>
                            </View>
                        )
                    }} />
            </View>
    ...
    return (
        <View style={{ width: '100%', backgroundColor: appcolor.light, paddingHorizontal: 8 }}>
            <FlatList
                scrollEnabled={false}
            ...
            <ActionSheet
                ...
                containerStyle={{ backgroundColor: appcolor.light, alignSelf: 'center', padding: 8, height: deviceHeight / 1.4 }}
            >
