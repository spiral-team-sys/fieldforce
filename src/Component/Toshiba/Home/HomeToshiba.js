...
import { View, StyleSheet, AppState, LogBox, DeviceEventEmitter, Modal, SafeAreaView } from "react-native";
...
import { LoginToshiba } from './LoginToshiba';
import { MenuHomeToshiba } from './MenuHomeToshiba';
import { HeaderToshiba } from './HeaderToshiba';
import { SummaryToshiba } from '../SummaryData/SummaryToshiba';
...
const HomeToshiba = ({ navigation }) => {
    ...
    const downloadData = async () => {
        await console.log("downloadData")
    ...
    const styles = StyleSheet.create({
        mainContainer: { width: '100%', height: '100%', backgroundColor: appcolor.light, paddingTop: 20 },
        ...
        welcome === 1 ?
            ...
            userinfo.employeeId > 0 ?
                <SafeAreaView style={styles.mainContainer}>
                    <View style={{ flex: 4.2 / 10 }}>
                        <View style={{ flex: 3 / 10, backgroundColor: appcolor.transparent }}>
                            <HeaderToshiba navigation={navigation} onShowMenu={showMenuHome} />
                        </View>
                        {refreshing && <LoadingView isLoading={refreshing} title=' ' />}
                        <View style={{ flex: 7 / 10, marginTop: 8, }} onLayout={onLayout}>
                            {!refreshing &&
                                <SummaryToshiba navigation={navigation} refreshing={refreshing}
                                    viewHeight={viewHeight} downloadData={downloadData} />
                            }
                            {refreshing && <LoadingView isLoading={refreshing} title=' ' />}
                        </View>
                    </View>
                    <View style={{ flex: 5.8 / 10, backgroundColor: appcolor.transparent, }}>
                        {/* {
                            !refreshing &&
                            <ScrollView
                        ...
                        <MenuHomeToshiba navigation={navigation} isLoading={refreshing} downloadData={downloadData} />
                        {/* </ScrollView>
                        } */}
                        {refreshing && <LoadingView isLoading={refreshing} title=' ' />}
                    </View>
                ...
                :
                <LoginToshiba onLoginCallBack={downloadData} />
    )
}
export default HomeToshiba;
