...
export const MenuHomeToshiba = ({ navigation, isLoading, downloadData }) => {
    ...
    const LoadData = async () => {
        ...
        await loadShop()
        await setData({ dataMenu: _menu, isShowToday: isHaveToday, currentTab: isShowToday ? data.currentTab : 2 })
    ...
    const renderItem = ({ item, index }) => {
        return (
            <TouchableOpacity
                key={"hk3d2-" + index}
                style={{ justifyContent: 'flex-end', height: 70, minWidth: deviceWidth * 0.4, paddingRight: userinfo.dropCategory !== 1 ? 8 : 0 }}
                onPress={() => onItemPress(item)}
                activeOpacity={0.8}
            >
                <View style={{ height: 66, flexDirection: 'row', borderRadius: 50, backgroundColor: appcolor.surface, alignItems: 'center', padding: 4 }}>
                    <View style={{ backgroundColor: appcolor.primary, borderRadius: 50, height: 52, width: 52, justifyContent: 'center', alignItems: 'center' }}>
                        <Icon containerStyle={{ alignItems: 'flex-start' }} name={item.iconName} color={appcolor.white} type={item.iconType || "font-awesome-5"} size={24} />
                    </View>
                    <View style={{ paddingHorizontal: 10, justifyContent: 'center' }}>
                        <Text style={{ color: appcolor.primary, fontWeight: '700' }}>{item.menuNameVN}</Text>
                        <Text style={{ color: appcolor.greylight, opacity: 0.5, fontWeight: '600', fontSize: scaleSize(10) }}>{item.menuName}</Text>
                    </View>
                </View>
            </TouchableOpacity>
        )
    ...
    const styles = StyleSheet.create({
        mainContainer: { flex: 1, backgroundColor: appcolor.transparent, borderTopLeftRadius: 20, borderTopRightRadius: 20 },
        ...
        <View style={styles.mainContainer}>
            {userinfo.dropCategory !== 1 ?
                <View style={{ width: deviceWidth, paddingLeft: 10 }}>
                    <Text style={{ fontSize: 16, fontWeight: fontWeightBold, color: appcolor.placeholderText, padding: 4 }}>Chức năng</Text>
                    {
                        !isLoading &&
                        <FlatList data={data.dataMenu}
                ...
                <View style={{ flex: 1, padding: 10 }}>
                    <Text style={{ fontSize: 20, fontWeight: fontWeightBold, color: appcolor.placeholderText, padding: 4 }}>Chức năng</Text>
                    {
                        !isLoading &&
                        <FlatList data={data.dataMenu}
            ...
            {userinfo.dropCategory !== 1 && shops.length > 0 &&
                <View style={{ width: deviceWidth, padding: 4, flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={{ fontSize: 20, fontWeight: '500', color: appcolor.primary, padding: 8 }}>Cửa hàng </Text>
                    <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', alignContent: 'center', justifyContent: 'center', padding: 4 }} onPress={() => navigation.navigate("ShopList")}>
                        <Icon name='arrow-right' color={appcolor.primary} size={28} style={{ alignItems: 'center' }} />
                        <Text style={{ color: appcolor.primary, fontWeight: '500', fontSize: 12 }}>Tất cả ({shops?.length})</Text>
                    </TouchableOpacity>
                </View>
            }
            {userinfo.dropCategory !== 1 && shops.length > 0 &&
                <View style={{ flex: 1 }}>
                    {
                        ...
                        >
                            <ShopOneMoreToshiba shops={shops} navigation={navigation} loading={loading} />
                        </ScrollView>
                    }
                </View>
            }
