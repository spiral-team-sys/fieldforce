...
export const SurveyResultReport = ({ navigation }) => {
  ...
  const config = useMemo(
    () => parseConfig(kpiinfo?.reportItem),
    [kpiinfo?.reportItem],
  );
  const tabRef = useRef();
  ...
  const uploadData = useCallback(async () => {
    ...
    if (config.isCheckItem == 1) {
      ...
      for (let index = 0; index < data.length; index++) {
        ...
        const itemCheck = _.sumBy(
          _.filter(data, e => {
            return e.trackingId == item.trackingId && e.textValue != null;
          }),
          'textValue',
        );
  ...
  const handleChooseItem = useCallback(
    (itemMain, itemAnswer) => {
      const value =
        itemMain.textValue == itemAnswer.ItemName ? '' : itemAnswer.ItemName;
      setData(prevData =>
        prevData.map(item =>
          item.id === itemMain.id ? { ...item, textValue: value } : item,
        ),
      );
      updateItemSurvey({
...
const RenderItem = memo(({ item, index, styles, appcolor, onChooseItem }) => {
  return (
    <View key={`asd00_${index}`} style={styles.itemContainer}>
      {item.refName.length > 0 && (
        <Text style={styles.titleHeader}>{`${item.refName}`}</Text>
      )}
      <View style={styles.optionContainer}>
        {(item.dataAnswer || []).length > 0 &&
          item.dataAnswer.map((e, i) => {
            const onPressItem = () => {
              onChooseItem(item, e);
            };
            const colorSelected =
              item.textValue == e.ItemName
                ? appcolor.yellowdark
                : appcolor.light;
            return (
              <TouchableOpacity
                key={`${i}_kka`}
                onPress={item.upload == 1 ? null : onPressItem}
              >
                <View
                  style={[styles.itemView, { backgroundColor: colorSelected }]}
                >
