...
export const extractProvinceFromAddress = (address) => {
    ...
    return addressParts.length > 0 ? addressParts[addressParts.length - 1] : 'N/A'
}
...
export const getValueByItem = (dataReport, keyValue, defaultValue = 'Value') => {
    const item = dataReport.find(d => d.KeyValue === keyValue)
...
export const formatReportPayload = (item) => {
    ...
    const purchasedProduct = getValueByKey(dataReport, 'purchasedProduct') || 'N/A'
    ...
    const hashtag = `${getValueByKey(dataReport, 'hashtag') || '#Homevisit'}`.replace(/,/g, ' ').replace(/\s+/g, ' ').trim() || '#Homevisit'
    const houseType = getValueByKey(dataReport, 'houseType') || 'N/A'
    const customerNote = getValueByKey(dataReport, 'customerNote') || 'N/A'
    const customerPurchaseJourney = getValueByKey(dataReport, 'customerPurchaseJourney') || ''
    const currentHomeAppliancesStatus = getValueByKey(dataReport, 'currentHomeAppliancesStatus') || ''

    const province = getValueByItem(dataReport, 'address', 'Province') || 'N/A'
    const region = getValueByItem(dataReport, 'address', 'Area') || 'N/A'
    const reportDate = item.createdDate ? moment(item.createdDate).format('YYYY-MM-DD') : moment().format('YYYY-MM-DD')
...
export const formatReportTemplate = (item) => {
    ...
    const journeyText = payload.purchaseJourney.length > 0 ? payload.purchaseJourney.join('\n') : 'N/A'
    const appliancesText = payload.homeAppliancesStatus.length > 0 ? payload.homeAppliancesStatus.join('\n') : 'N/A'
...
export const formatReportTemplateJsonString = (item) => JSON.stringify(formatReportTemplate(item))
