    ...
    const validData = () => {
        ...
        for (let i = 0; i < dataMain.length; i++) {
            ...
            if (itemType === 'listinput') {
                ...
                const missingFields = listValues.filter(inputItem => !`${inputItem?.Value || ''}`.trim())

                if (item?.IsRequired == 1 && (listValues.length === 0 || missingFields.length > 0)) {
                    const missingNames = missingFields.map(inputItem => inputItem?.ItemName).filter(Boolean)
                    missingRequired.push(
                        missingNames.length > 0
                            ? `${item?.ItemName || item.GroupName || 'Câu hỏi'}: ${missingNames.join(', ')}`
    ...
    const handleSaveData = async (itemUpdated) => {
        ...
        const applyDistanceValue = (source = []) => {
            ...
            return source.map((detail) => {
                ...
                const keyValue = `${detail?.KeyValue ?? detail?.keyValue ?? ''}`.toLowerCase()
    ...
    const renderItem = ({ item }) => {
        return (
            <View style={styles.itemContainer}>
                <Text style={styles.titleName}>{item.ItemName} {item.IsRequired == 1 && <Text style={styles.requireText}>*</Text>}</Text>
