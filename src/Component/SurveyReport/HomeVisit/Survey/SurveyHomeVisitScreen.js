...
import { deleteDataRaw, removeRawReport, saveJsonData } from "../../../../Controller/ReportController";
import { alertConfirm, alertNotify, onValidPhoneNumber, TODAY } from "../../../../Core/Utility";
import _ from 'lodash';
import { LoadingView } from "../../../../Control/ItemLoading";
import AsyncStorage from "@react-native-async-storage/async-storage";

const SurveyHomeVisitScreen = ({ navigation, route }) => {
    ...
    const itemMain = route.params || {}
    //
    const LoadData = async () => {
        await setLoading(true)
        const params = { ...itemMain, reportId: kpiinfo.id }
    ...
    const UploadData = async () => {
        ...
        alertConfirm('Gửi dữ liệu', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', async () => {
            ...
            if (result.statusId == 200) {
                await removeRawReport(itemMain.shopId || 0, kpiinfo.id)
    ...
    const validData = () => {
        ...
        for (let i = 0; i < dataMain.length; i++) {
            ...
            if (itemType === 'phone' && value) {
                ...
                if (phoneError) {
                    invalidPhones.push(`${item?.ItemName || item.GroupName || 'Số điện thoại'}: ${value}`)
                }
            }
        }
        ...
        if (missingRequired.length > 0 || invalidPhones.length > 0) {
            ...
            if (missingRequired.length > 0) {
                messageParts.push(`Vui lòng điền đầy đủ thông tin cho:\n${missingRequired.join('\n')}`)
            }
            if (invalidPhones.length > 0) {
                messageParts.push(`Số điện thoại không hợp lệ:\n${invalidPhones.join('\n')}`)
            }
    ...
    const handleSaveData = async () => {
        await saveJsonData(itemMain.shopId || 0, kpiinfo.id, TODAY, dataMain)
    }
    ...
    const onRenewData = () => {
        Message('Chú ý', 'Bạn có chắc chắn muốn tải lại dữ liệu?',
            async () => {
                await deleteDataRaw(itemMain.shopId || 0, kpiinfo.id)
                await setData([])
                await setDataMain([])
                await LoadData()
            })
    ...
    const styles = StyleSheet.create({
        ...
        itemContainer: { flex: 1, padding: 8, paddingHorizontal: 14, backgroundColor: appcolor.light, borderRadius: 8 },
    ...
    const renderTab = (item) => {
        ...
        return (
            <CustomListView
                ...
                bottomView={{ paddingBottom: deviceHeight / 2.3 }}
            />
        )
    }
    const renderItem = ({ item }) => {
        return (
            <View style={styles.itemContainer}>
                {(item?.ItemName || null) !== null && <Text style={styles.titleName}>{item.ItemName} {item.IsRequired == 1 && <Text style={styles.requireText}>*</Text>}</Text>}
                {item.DescriptionName && <Text style={styles.subTitleName}>{item.DescriptionName}</Text>}
                <SurveyItemDetails
                    itemMain={item}
                    paramShop={itemMain}
...
export default SurveyHomeVisitScreen;