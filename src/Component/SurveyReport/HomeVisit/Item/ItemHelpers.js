...
export const getItemInputType = itemType =>
  `${itemType || 'text'}`.toLowerCase();
    ...
    .map((photo, index) => {
      if (typeof photo === 'string') {
        return {
          id: `${index}_${photo}`,
          photoPath: photo,
          fileName: photo.split('/').pop() || `photo_${index + 1}.jpg`,
        };
      }
...
export const mergeListInputItems = item => {
  ...
  return baseItems.map(configItem => {
    const savedItem = savedItems.find(
      saved =>
        Number(saved.ItemId) === Number(configItem.ItemId) ||
        saved.ItemName === configItem.ItemName,
    );
