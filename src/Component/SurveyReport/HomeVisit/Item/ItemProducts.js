...
const ItemProducts = ({ item, onUpdateItem }) => {
    ...
    const getSavedSelectedItems = (products = []) => {
        const savedIds = parseJsonArray(item?.ProductIds).map(id => Number(id)).filter(Number.isFinite)
        if (savedIds.length > 0) {
            return products.filter(product => savedIds.includes(Number(product?.ProductId)))
        }

        const savedNames = parseTextValues(item?.Value)
        if (savedNames.length === 0) return []
        return products.filter(product => savedNames.includes(product?.ProductName))
    }
    ...
    const styles = StyleSheet.create({
        ...
        // List rows
        groupHeader: { paddingHorizontal: 12, paddingVertical: 5, backgroundColor: appcolor.primary + '20', flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap' },
        groupHeaderText: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.dark },
        segmentText: { fontSize: 11, color: appcolor.placeholderText, marginLeft: 6 },
        productRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 9, borderBottomWidth: 1, borderBottomColor: appcolor.surface },
        productRowSelected: { backgroundColor: appcolor.surface },
        productInfo: { flex: 1 },
        productCode: { fontSize: 11, color: appcolor.placeholderText, fontWeight: '500' },
    ...
    const handleConfirm = () => {
        item.Value = selectedItems.map(product => product?.ProductName).filter(Boolean).join(', ')
        item.ProductIds = JSON.stringify(selectedItems.map(product => product?.ProductId).filter(id => id !== undefined && id !== null))
    ...
    const getGroupedData = (categoryName) => {
        ...
        products.forEach(p => {
            ...
            if (!seen.has(key)) {
                seen.add(key)
                result.push({ _isHeader: true, SubCategory: p.SubCategory || '', Segment: p.Segment || '', _key: key })
            }
    ...
    const savedItems = useMemo(() => getSavedSelectedItems(listItems), [item?.ProductIds, item?.Value, listItems])
        ...
        if (row._isHeader) {
            return (
                <View style={styles.groupHeader}>
                    {row.SubCategory ? <Text style={styles.groupHeaderText}>{row.SubCategory}</Text> : null}
                    {row.Segment ? <Text style={styles.segmentText}>• {row.Segment}</Text> : null}
                </View>
            )
        }
        const selected = selectedItems.some(s => s.ProductId === row.ProductId)
        return (
            <TouchableOpacity style={[styles.productRow, selected && styles.productRowSelected]} onPress={() => toggleSelect(row)}>
