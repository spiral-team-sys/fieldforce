...
const ItemDateTime = ({ item, onUpdateItem }) => {
  ...
  const getDisplayValue = () => {
    const value = `${item?.Value || ''}`;
    if (!value)
      return item?.DescriptionName || `Chọn ${item?.ItemName || 'thời gian'}`;
  ...
  const styles = StyleSheet.create({
    ...
    inputContainer: {
      ...
      borderRadius: 8,
      borderColor: item?.Value ? appcolor.grayLight : appcolor.red,
      borderWidth: 0.5,
      marginTop: 8,
    },
    titleValue: {
      fontSize: 12,
      color: item?.Value ? appcolor.dark : appcolor.placeholderText,
    },
  });
