...
const ItemEmployees = ({ item, onUpdateItem }) => {
  ...
  const getSavedSelectedItems = (employees = []) => {
    const savedIds = parseJsonArray(item?.EmployeeIds)
      .map(id => Number(id))
      .filter(Number.isFinite);
    if (savedIds.length > 0) {
      return employees.filter(emp =>
        savedIds.includes(Number(emp?.EmployeeId)),
      );
    }
  ...
  const handleConfirm = () => {
    item.Value = selectedItems
      ...
      .filter(Boolean)
      .join(', ');
    item.EmployeeIds = JSON.stringify(
      selectedItems
  ...
  const savedItems = useMemo(
    ...
    [item?.EmployeeIds, item?.Value, listItems],
  );
          ...
          <SafeAreaView style={styles.modalBackdrop}>
            <View style={styles.modalContainer}>
              <View style={styles.modalHeader}>
                ...
                >{`Chọn người thực hiện (${selectedItems.length})`}</Text>
                <TouchableOpacity
