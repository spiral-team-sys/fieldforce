...
const ItemPhoto = ({ item, paramShop, onUpdateItem }) => {
    ...
    const handleCaptureResult = async (result) => {
        ...
        const newPhotos = (Array.isArray(result?.data) ? result.data : [result?.data])
            .map((photo, i) => {
                ...
                return {
                    ...
                    photoPath,
                    fileName: photo?.fileName || photoPath.split('/').pop() || `photo_${Date.now()}_${i}.jpg`,
                }
            })
            .filter(Boolean)
        ...
        const merged = [...photos]
        newPhotos.forEach(p => {
            if (!merged.some(exist => exist.photoPath === p.photoPath)) merged.push(p)
        })
        setPhotos(merged)
    ...
    const styles = StyleSheet.create({
        ...
        // Add button
        viewActionAdd: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 8 },
        addButton: { width: THUMB_SIZE, height: THUMB_SIZE, borderRadius: 10, borderWidth: 1.5, borderColor: appcolor.primary, borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center', backgroundColor: appcolor.primary + '0D' },
        addLabel: { fontSize: 10, color: appcolor.primary, marginTop: 3, fontWeight: '600' },
        // Thumbnail
        thumbWrap: { width: THUMB_SIZE, height: THUMB_SIZE, borderRadius: 10, overflow: 'hidden', backgroundColor: appcolor.surface },
        thumbImage: { width: '100%', height: '100%' },
        deleteButton: { position: 'absolute', top: 4, right: 4, width: 20, height: 20, borderRadius: 10, backgroundColor: appcolor.black + 'CC', alignItems: 'center', justifyContent: 'center' },
        // Counter
        counterRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 6, gap: 4 },
    ...
    const renderAddButton = () => (
        <View style={styles.viewActionAdd}>
            <TouchableOpacity style={styles.addButton} onPress={capturePhoto} activeOpacity={0.7}>
                <Icon name='camera-alt' type='material-icons' size={26} color={appcolor.primary} />
                <Text style={styles.addLabel}>Chụp ảnh</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.addButton} onPress={captureLibrary} activeOpacity={0.7}>
                <Icon name='photo-library' type='material-icons' size={26} color={appcolor.primary} />
    ...
    const renderThumb = ({ item: photo, index }) => (
        <TouchableOpacity style={styles.thumbWrap} onPress={() => setViewPhoto({ visible: true, index })} activeOpacity={0.85}>
            <Image source={{ uri: checkLinkType(photo.photoPath) }} style={styles.thumbImage} resizeMode='cover' />
            <TouchableOpacity style={styles.deleteButton} onPress={() => removePhoto(photo.id)} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
                <Icon name='close' type='material-icons' size={13} color={appcolor.white} />
            </TouchableOpacity>
        </TouchableOpacity>
    )
        ...
        <View style={styles.mainContainer}>
            {photos.length > 0 && (
                <View style={styles.counterRow}>
                    <Icon name='photo-library' type='material-icons' size={14} color={appcolor.placeholderText} />
