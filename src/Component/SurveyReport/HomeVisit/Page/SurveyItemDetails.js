...
const SurveyItemDetails = ({ itemMain, paramShop, onSaveData }) => {
  ...
  const renderItem = () => {
    ...
    switch (itemType) {
      ...
      case 'listinput':
        return (
          <ItemInputList item={itemMain} onUpdateItem={handleUpdateItem} />
        );
      case 'employee':
        return (
          <ItemEmployees item={itemMain} onUpdateItem={handleUpdateItem} />
        );
      case 'photo':
        return (
          <ItemPhoto
  ...
  return <View style={styles.mainContainer}>{renderItem()}</View>;
};
