...
export const VerifyDataSellout = ({ navigation }) => {
  ...
  const updateItemInfo = async (item, modelUpdate) => {
    ...
    const dataBySaleId = await _.filter(dataUpload, e => {
      return e.saleId == item.SaleId;
    });
    const itemSave = {
      ...dataBySaleId[0],
      saleId: item.SaleId,
    ...
    if (dataBySaleId.length == 0) {
      ...
      const _update = dataUpload.map(it =>
        it.saleId == item.SaleId ? itemSave : it,
      );
  ...
  const UploadData = async () => {
    await handlerCheckData(async (success = false) => {
      if (success) {
        alertConfirm(
          'Gửi báo cáo',
          'Sau khi gửi dữ liệu sẽ không chỉnh sửa được. Bạn có chắc chắn không?',
          async () => {
  ...
  const handleCheckDataAqua = actionResult => {
    if (dataUpload.length > 0) {
      for (let index = 0; index < dataUpload.length; index++) {
        ...
        const itemEvident =
          _.filter(JSON.parse(dataVerify[0].dataDetail), e => {
            return e.SaleId == item.saleId;
          })[0] || {};
        if (item.itemPhoto !== '[]') {
          if (
            (check[keyVAT]?.length || 0) > 0 &&
            (check[keyBILL]?.length || 0) > 0 &&
            (check[keyCRM]?.length || 0) > 0
          ) {
            continue;
          } else {
            if (
              itemEvident.isImageVAT == 1 &&
              (check[keyVAT]?.length || 0) == 0
            ) {
              ToastError(
                `Vui lòng cập nhật đầy đủ dữ liệu hình ảnh VAT trước khi gửi báo cáo (SP: ${itemEvident.ProductName})`,
              );
              actionResult(false);
              return;
            } else if (
              itemEvident.isImageBill == 1 &&
              (check[keyBILL]?.length || 0) == 0
            ) {
              ToastError(
                `Vui lòng cập nhật đầy đủ dữ liệu hình ảnh "Biên nhận" trước khi gửi báo cáo (SP: ${itemEvident.ProductName})`,
              );
              actionResult(false);
              return;
            } else if (
              itemEvident.isImageCRM == 1 &&
              (check[keyCRM]?.length || 0) == 0
            ) {
              ToastError(
                `Vui lòng cập nhật đầy đủ dữ liệu hình ảnh "CRM" trước khi gửi báo cáo (SP: ${itemEvident.ProductName})`,
              );
              actionResult(false);
              return;
            } else {
              if (itemEvident.billCodeFilter == 1) {
                if ((item.billCodeFilter || '').length == 0) {
                  ToastError(
                    `Vui lòng cập nhật đầy đủ dữ liệu mã tra cứu trước khi gửi báo cáo (SP: ${itemEvident.ProductName})`,
                  );
                  actionResult(false);
                  return;
                }
              ...
              if (itemEvident.isStatusVerify == 1) {
                if ((item.statusVerify || '').length == 0) {
                  ToastError(
                    `Vui lòng cập nhật "Trạng thái đơn hàng" trước khi gửi báo cáo (SP: ${itemEvident.ProductName})`,
                  );
                  actionResult(false);
                  return;
                }
              ...
              if (itemEvident.infoVerify == 1) {
                ...
                for (let index = 0; index < dataCheck.length; index++) {
                  ...
                  if (
                    (_update[item.Ref_Code] || '').length == 0 &&
                    item.IsRequired == 1
                  ) {
                    ToastError(
                      `Vui lòng cập nhật thông tin "${item.ItemName}" trước khi gửi báo cáo (SP: ${itemEvident.ProductName})`,
                    );
                    actionResult(false);
                    return;
                  }
      ...
      for (let index = 0; index < dataUpload.length; index++) {
        ...
        if (item.itemPhoto !== '[]' || _update.isCheckInfo == 1) {
          if (
            ...
            if (
              (itemEvident.isImageVAT == 1 || _update.isImageVAT == 1) &&
              (check[keyVAT]?.length || 0) == 0
            ) {
              ToastError(
                `Vui lòng cập nhật đầy đủ dữ liệu hình ảnh VAT trước khi gửi báo cáo (SP: ${
                  itemEvident.ProductName || _update.ProductName
                })`,
              );
              actionResult(false);
              return;
              ...
              if (
                _update.isBillFromLink == 1 &&
                _update.isCheckBillFromLink == 1
              ) {
                if ((item.billFromLink || '').length == 0) {
                  ToastError(
                    `Vui lòng cập nhật "Đường dẫn hoá đơn" trước khi gửi báo cáo (SP: ${_update.ProductName})`,
                  );
                  actionResult(false);
                  return;
                }
  ...
  const handlerChangeValue = async (type, item, text) => {
    ...
    if (type == 'BILLCODE') {
      ...
      //
      const _update = dataUpload.map(it =>
        it.saleId == item.SaleId ? itemUpdate : it,
      );
      await setDataUpload(_update);
    }
    if (type == 'LINK') {
  ...
  const handlerSearch = text => {
    const filterList = dataMain.filter((_item, index) => {
      return index == changeIndex;
    });
    const itemFilter = JSON.parse(filterList[0]?.dataDetail || '[]').filter(
      i => {
        return (
          i.ProductName.toLowerCase().match(text.toLowerCase()) ||
          i.ProductCode.toLowerCase().match(text.toLowerCase()) ||
          (i.statusEvident !== undefined &&
            i.statusEvident?.toLowerCase().match(text.toLowerCase()))
        );
      },
    );
    const searchResult = dataVerify.map((item, index) =>
      index === changeIndex
        ? { ...item, dataDetail: JSON.stringify(itemFilter) }
        : item,
    );
  ...
  const contains = item => {
    ...
    for (let indexM = 0; indexM < dataUpdate.length || 0; indexM++) {
      ...
      if (itemM.IsRequired == 1) countRequire = countRequire + 1;
      if ((item[itemM.Ref_Code] || '').length > 0 && itemM.IsRequired == 1) {
        countHave = countHave + 1;
      }
    }

    if (countRequire == countHave && countRequire !== 0) return true;
    else return false;
  };
  const renderItem = ({ item, index }) => {
    const isPhotoVAT =
      _.filter(dataPhotos, e => {
        return e.photoType == `VAT_${item.SaleId}`;
      }).length > 0;
    const isPhotoBILL =
      _.filter(dataPhotos, e => {
        return e.photoType == `BILL_${item.SaleId}`;
      }).length > 0;
    const isPhotoCRM =
      _.filter(dataPhotos, e => {
        return e.photoType == `CRM_${item.SaleId}`;
      }).length > 0;
    const isPhotoFSM =
      _.filter(dataPhotos, e => {
        return e.photoType == `FSM_${item.SaleId}`;
      }).length > 0;
    const isCancel =
      _.filter(dataUpload, e => {
        return e.saleId == item.SaleId;
    ...
    const itemUpload = _.filter(dataUpload, e => {
      return e.saleId == item.SaleId;
    });
    const isInfoUpdate = contains(
      JSON.parse(itemUpload[0]?.itemUpdate || '{}'),
    );
    ...
    return (
      <View key={`iiti_${index}`} style={styles.itemContainer}>
        ...
        {item.billCodeFilter == 1 && (
          <View style={styles.contentContainer}>
            <ItemInput
              ...
              isRequire={true}
              placeholder={
                'Nhập mã tra cứu biên nhận (bắt buộc nhập tối thiểu 1 kí tự)'
              }
        ...
        {item.statusEvident !== null && item.statusEvident?.length > 0 && (
          <Text
            style={{
              ...styles.itemView,
          ...
          >{`Trạng thái: ${item.statusEvident}`}</Text>
        )}
        <View style={styles.viewHeader}>
          <Text
            style={styles.itemView}
          >{`${item.SaleId}. Cửa hàng: ${item.ShopName}`}</Text>
          ...
          <Text
            style={styles.itemView}
          >{`Khách hàng: ${item.CustomerName}`}</Text>
  ...
  const styles = StyleSheet.create({
    ...
    tabViewTop: {
      ...
      minWidth: minWidthTab(dataVerify),
      height: 32,
    },
    itemContainer: {
      flex: 1,
            ...
            {groupStatus.map((item, index) => {
              ...
              return (
                <TouchableOpacity
                  key={`ggi_${index}`}
                  style={
                    isSelected ? styles.tabGroupSelected : styles.tabGroupView
                  }
