...
export const HomeTodoList = ({ navigation }) => {
    ...
    const [itemTask, setItemTask] = useState({ statusId: 1, statusName: 'Hôm nay', taskName: null, isColor: TaskAPI.colorStatus.OPEN })
    ...
    const uploadTaskAction = async () => {
        ...
        if (result.status == 200)
            ToastSuccess(result.messeger, '', 'top')
        else
            ToastError(result.messeger, '', 'top')
    }
    const updateTaskStatus = async (item, index) => {
        const indexMain = todoList.findIndex((it, idx) => it.taskName == item.taskName)
    ...
    const handlerChooseItem = async (item, type) => {
        if (type == 'UPDATE_STATUS') {
            ...
            await setItemTask({ ...itemTask, statusId: item.id, statusName: item.name })
        }
        item.id !== 3 && await LoadData()
    }
    const handlerViewReulst = async () => {
        const lstCompleted = await dataStatus.filter(i => !viewResult ? i.id == 3 : i.id !== 3)
    ...
    const styles = StyleSheet.create({
        ...
        itemText: { width: '80%', fontSize: 14, fontWeight: '500', color: appcolor.dark },
        contentBottom: { width: '100%', paddingBottom: 16, alignItems: 'center', alignSelf: 'center', backgroundColor: appcolor.light },
        itemMain: { padding: 3, shadowRadius: 10, color: appcolor.light, shadowOpacity: 0.5, elevation: 3 }
    })
    const renderTaskItem = ({ item, index }) => {
        const itemPress = () => { updateTaskStatus(item, index) }
        const itemClear = () => { removeTask(item) }
        return (
            <View key={`pp_3${index}`} style={{ width: '100%', padding: 8, paddingTop: 0, paddingBottom: 0 }} >
                <View style={{ flexDirection: "row", alignItems: 'center', padding: 8 }}>
                    <TouchableOpacity onPress={itemPress}>
                        <View style={{ borderWidth: 1, borderColor: appcolor.greydark, borderRadius: 5, padding: 3, marginEnd: 8 }}>
                            <View style={{ width: 16, height: 16, backgroundColor: item.isColor, borderRadius: 1 }} />
                        </View>
                    </TouchableOpacity>
                    <Text style={styles.itemText}>{item.taskName}</Text>
                    {item.statusId == 1 &&
                        <TouchableOpacity style={{ position: 'absolute', end: 0, paddingRight: 18, padding: 10 }} onPress={itemClear}>
                            <Icon name="trash" type="font-awesome-5" size={16} color={appcolor.red} />
                        </TouchableOpacity>
                    }
                </View>
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View key={`pp_3${index}`} style={{ width: '100%' }} >
                <View style={{ flexDirection: 'row', alignItems: 'center', margin: 8, backgroundColor: item.isColor, borderRadius: 8 }}>
                    <Text style={{ fontSize: 14, fontWeight: '600', padding: 8, color: appcolor.light }}>{item.name}</Text>
                    <View style={{ ...styles.itemMain, backgroundColor: item.isColor, borderRadius: 5 }}>
                        <Text style={{ textAlign: 'center', fontSize: 14, fontWeight: '600', color: appcolor.light }}>{totalItem}</Text>
                    </View>
                    {item.id == 1 &&
                        <TouchableOpacity
                            style={{ position: 'absolute', end: 8 }}
                            onPress={createNewTask} >
                            <Icon name={'add'} size={23} solid={true} color={appcolor.light} />
                        </TouchableOpacity>
                    }
                </View>
                <FlatList
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <FlatList
                ...
                showsVerticalScrollIndicator={false}
            />
            <ActionSheet id={'createTask'} containerStyle={{ backgroundColor: appcolor.light }} >
                <View style={{ width: '100%', padding: 5 }}>
                    <Text numberOfLines={2} style={{ textAlign: 'center', fontSize: scaleSize(18), fontWeight: '700', padding: 5, color: appcolor.dark }}>Thêm mới</Text>
            ...
            <ActionSheet id={'updateStatus'}>
                <View style={{ width: '100%', height: deviceHeight / 5, padding: 5, alignItems: 'center' }}>
...
const ItemInput = ({ onActionRight, typeFilter, itemValue, placeholder, onChangeText, keyboardType = 'default', iconName }) => {
    ...
    const styles = StyleSheet.create({
        ...
        inputView: { width: widthItem, backgroundColor: appcolor.light, borderRadius: 5, marginBottom: 0 }
    })
    const onPress = async () => {
        ...
        <View style={styles.mainItem}>
            <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center' }}>
                ...
                {onActionRight !== undefined &&
                    <TouchableOpacity
                        style={{ padding: 8, marginStart: 8, width: '18%', alignItems: 'center', backgroundColor: appcolor.yellowdark, borderRadius: 5 }}
                        onPress={onPress}>
                        <Text style={{ fontSize: 14, fontWeight: '500', color: appcolor.dark }}>Thêm</Text>
                    </TouchableOpacity>
                }
            </View>
        </View>
