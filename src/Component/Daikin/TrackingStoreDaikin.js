...
import { deviceHeight, deviceWidth, minWidthTab, removeAccents } from "../../Core/Utility";
...
export const TrackingStoreDaikin = ({ navigation, route }) => {
    ...
    const loadData = async () => {
        ...
        await GetListTrackingStore(shopinfo.shopId, kpiinfo.id, async (dataTracking) => {
    ...
    const onChangePhone = (item, text) => {
        if (text !== null) {
            ...
            (!itemShop[item.ref_Name] || textValue == '' || textValue?.length < 12) && (itemShop[item.ref_Name] = textValue)
        } else {
            itemShop[item.ref_Name] = null
        }
    }
...
const RenderItemInput = ({ item, index, itemShop, showListSelect, onNumberChanged, onTextChanged, onSelectAddress, reloadView, onChangePhone }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    ...
    switch (item.ref_Code) {
        case 'number':
            return item.decimalValue == 0 ? <NumberFormat
                key={`n${index}`}
        ...
        case 'selected':
            const itemName = JSON.parse(item.filterList || '[]').find(it => it.Id == itemShop[item.ref_Name])?.Name
            return <View style={{ minHeight: 10, minWidth: 10, borderRadius: 10, backgroundColor: appcolor.surface }}>
                <TouchableOpacity
                    ...
                    <Text style={{ width: '90%', fontSize: 14, fontWeight: '400', color: appcolor.dark, padding: 5 }}>{itemName}</Text>
                    <Icon type="font-awesome-5" color={appcolor.dark} containerStyle={{ padding: 10 }} style={{ color: appcolor.dark }} name={"caret-down"} size={18} />
                </TouchableOpacity>
            </View >
        case 'address':
            return <View key={item.ref_Name + item.code} style={{ minHeight: 10, minWidth: 10 }}>
...
const ViewShopInfo = ({ dataByGroup, itemShop, onTextChanged, onNumberChanged, onChangePhone, onSelectAddress, onCloseSheet }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    ...
    const showListSelect = async (item) => {
        ...
        listSelect.map(it => it.Id == itemShop[item.ref_Name] ? (it.isSelect = true) : (it.isSelect = false))
