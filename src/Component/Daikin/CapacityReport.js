...
import { GetItemCapacity, UpdateItemCapacity, uploadItemCapacity } from "../../Controller/CapacityController"
import { LoadingView } from "../../Control/ItemLoading";
import moment from "moment";

const TYPE = { PROVINCE: 'Province', STORE: 'Store', CAPACITY: 'CapacityValue', NOTE: 'Note' }
export const CapacityReport = ({ navigation, route }) => {
    ...
    const LoadData = async () => {
        ...
        await GetItemCapacity(workinfo, async (item) => {
            ...
            await setItemCapacity({ ...itemCapacity, capacityValue: item.capacityValue, noteValue: item.note, isUploaded: workinfo.workDate === day ? item.isUploaded : 1 })
        })
    ...
    const uploadData = async () => {
        ...
        alertConfirm('Gửi báo cáo', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', async () => {
            await setLoading(true)
            await uploadItemCapacity(itemCapacity, { ...workinfo, reportId: kpiinfo.id }, async () => {
                await LoadData()
            })
        })
    }
    const handlerSelectItem = async (item, type) => {
        switch (type) {
            case TYPE.PROVINCE:
                await setItemCapacity({ ...itemCapacity, provinceCode: item.provinceCode, provinceName: item.provinceName })
                const filterStore = dataStore.main.filter(i => i.provinceCode == item.provinceCode)
                await setDataStore({ ...dataStore, view: filterStore })
                break
            case TYPE.STORE:
                await setItemCapacity({ ...itemCapacity, shopId: item.shopId, shopName: item.shopName })
                break
        }
    }
    const handlerItemChangeText = async (text, typeItem) => {
        if (typeItem == TYPE.CAPACITY) {
            ...
            await UpdateItemCapacity({ ...itemCapacity, capacityValue: parseInt(text) })
        }
...
const ItemInput = ({ titleName, iconName, isRequire, onActionRight, typeFilter, itemValue, placeholder, onChangeText, keyboardType = 'default', isEditable = true }) => {
