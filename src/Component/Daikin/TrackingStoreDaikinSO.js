...
export const TrackingStoreDaikinSO = ({ navigation, route }) => {
  ...
  const onChangePhone = (item, text) => {
    if (text !== null) {
      ...
      (!itemShop[item.ref_Code] || itemValue == '' || itemValue?.length < 11) &&
        (itemShop[item.ref_Code] = itemValue);
    } else {
      itemShop[item.ref_Code] = null;
    }
  };
      ...
      >
        ...
        <View style={{ margin: 5, justifyContent: 'space-between', flex: 1 }}>
          <View>
            ...
            <Text
              style={{ fontWeight: '300', fontSize: 12, color: appcolor.dark }}
            >
              {item.shopCode}
            </Text>
          </View>
  ...
  const filterProduct = async text => {
    if (text) {
      const newDataShow = listShopM.filter(it => {
        const nameProduct = it.shopName
          ? it.shopName.toUpperCase()
          : ''.toUpperCase();
  ...
  if (parseInt(item.numberValue) == 1) {
      ...
      <NumberFormat
        ...
        renderText={value => (
          <FormGroup
            ...
            containerStyle={{
              backgroundColor:
                item.ref_Id == 1 ? appcolor.surface : appcolor.light,
              borderColor: appcolor.switchDisible,
              marginBottom: 0,
              padding: 3,
            }}
    ...
    return (
      <FormGroup
        ...
        placeholder={'Số lượng ' + item.itemName}
        containerStyle={{
          backgroundColor: item.ref_Id == 1 ? appcolor.surface : appcolor.light,
          borderColor: appcolor.switchDisible,
          marginBottom: 0,
          padding: 3,
        }}
    ...
    const itemName = JSON.parse(item.filterList || '[]').find(
      it => it.Id == itemShop[item.ref_Code],
    )?.Name;
    return (
      <View
        style={{
          ...
          {listSelect.map(it => {
            return (
              <CheckBox
                ...
                onPress={() =>
                  item.ref_Id !== 1 ? handleSelectCheck(item, it) : null
                }
