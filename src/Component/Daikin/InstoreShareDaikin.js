...
export const InstoreShareDaikin = ({ navigation, route }) => {
  ...
  const handlerUploadData = async () => {
    ...
    let lstReuslt = await getAllListTrackResLG(
      workinfo,
      reportItem.InstoreShare,
    );
    ...
    MessageAction(
      ...
      async () => {
        await UploadController.DataInstoreShare(
  ...
  const RenderItemData = ({ item, index }) => {
    const keyLayer2 =
      item[
        AppNameBuild === bekoApp
          ? `${item.competitorId}${item.subCatId}`
          : `${item.subCatId}${item.competitorId}`
      ];
    ...
    const colorTotal =
      item.refName == 'TOTAL' ? appcolor.primary : appcolor.dark;
    return (
                  ...
                  >
                    <FlatList
                      ...
                      ListFooterComponent={
                        ...
                        >
                          {dataByCategoryId.length > 10 ? 'Đã xem hết' : ''}
                        </Text>
                      }
                    />
                  </View>
