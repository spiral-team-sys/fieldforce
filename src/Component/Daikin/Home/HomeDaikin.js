...
function HomeDaikin(props) {
    const { shoplist, homemenu, statusPermission, userinfo, appcolor } = useSelector(state => state.GAppState)
    ...
    useEffect(() => {
        ...
        messaging().onMessage(async remoteMessage => {
            ...
            await setState({ ...state, notifyInfo: remoteMessage, isShowNotify: true })
        });
        return () => { _fisrtLoad }
    }, [userinfo])
        ...
        state.welcome == 1 ?
            ...
            userinfo?.employeeId > 0 ?
                <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.light }}>
                    ...
                    {!state.refreshing && <MenuHomeDaikin
                        onDowload={SyncdataApp}
                        ViewHeader={<>
                            <SummaryHome
                                navigation={props.navigation}
                                isLoading={state.refreshing} />
                            <Text style={{ paddingStart: 5, fontSize: 18, fontWeight: '700', color: appcolor.primary }}>Chức năng</Text>
                        </>
                        }
                        navigation={props.navigation} menus={state.menus} />}
