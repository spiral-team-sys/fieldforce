...
import { View, Text, TextInput, SafeAreaView, VirtualizedList, TouchableOpacity, FlatList } from "react-native";
import { CheckBox, Divider, Icon } from '@rneui/themed';
import { getCategory, GetCometitorTracking } from '../Controller/WorkController'
...
class TrackingCompetitorReport extends PureComponent {
    ...
    async loadData() {
        ...
        if (kpiinfo.id !== undefined) {
            ...
            if (localStore == undefined || localStore == null)//chua co trong store
            {
    ...
    onTextChange = (key, e, item) => {
        ...
        var index = addList.findIndex(f => f.categoryId === item.categoryId && f.id === item.id && f.isAdd == 1)
        var edit = { ...item }
        if (key === "model")
            edit[key] = e
        else {
    ...
    onAddModel = (item) => {
        ...
        const index = editsList.findIndex(e => e.model == item.model && e.isAdd === 0)
        if (index > -1) {
            ToastError("Mã sản phẩm này bạn đã thêm vào trước đó, vui lòng kiểm tra lại");
            return
        }
        // Reset row input
        const indexDf = editsList.findIndex(e => e.model == item.model && e.isAdd === 1)
    ...
    onPostData = async () => {
        ...
        const data = await CompetitorResult.filter(c => c.model !== undefined && c.isAdd === 0 && c.quantity > 0)
        Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?',
            () => UploadController.uploadServer({ ...workinfo, reportId: kpiinfo.id }, data, (result) => {
    ...
    onRemoveModel = (item) => {
        Message('Thông báo', 'Bạn có chắc muốn xoá mục này đi không',
            () => {
                ...
                const index = delList.findIndex(a => a.model == item.model && a.isAdd == 0)
        ...
        return (
            item.isAdd === 1 ?
                <View  //key={`${item.categoryId}192${index}2khjad`}
                    ...
                    {
                        item.isAdd === 1 &&
                        <Text style={{ padding: 7, fontWeight: '700', fontSize: 10, marginLeft: 10, color: appcolor.dark }}>{item?.name}</Text>
                    }
                    {
                        item.categoryId === 300 &&
                        <View style={{ alignItems: 'center', flexDirection: 'row', backgroundColor: appcolor.light }}>
                            <CheckBox key="gaming"
                                title="gaming"
                                onPress={() => this.onTextChange("gaming", 1, item)}
                                containerStyle={{ backgroundColor: appcolor.light, flexGrow: 0.5 }}
                                checked={item.gaming == undefined ? null : item.gaming === 1 ? true : false}>
                            </CheckBox>
                            <CheckBox key="non-gaming" title="non-gaming"
                                onPress={() => this.onTextChange("gaming", 0, item)}
                                containerStyle={{ backgroundColor: appcolor.light, flexGrow: 0.5 }}
                                checked={item.gaming == undefined ? null : item.gaming === 1 ? false : true}>
                            </CheckBox>
                        </View>
                    }
                    <View style={{ flexDirection: 'row', flex: 1, backgroundColor: appcolor.light, padding: 3 }}>
                        <TextInput
                            ...
                            style={{
                                fontSize: 12, width: '60%', padding: 3,
                                color: appcolor.dark, borderWidth: 0.2, borderColor: appcolor.grey
                        ...
                        <TextInput
                            ...
                            style={{
                                ...
                                borderWidth: 0.2, borderColor: appcolor.grey, width: 90,
                            }}
                            placeholder='số lượng' placeholderTextColor={appcolor.grey}
                        />
                        <TouchableOpacity onPress={() => this.onAddModel(item)}
                            style={{ alignSelf: 'flex-end', }}>
                            <Icon name={item.isAdd === 1 ? 'add' : 'close'} size={14} raised
                ...
                : <View  //key={`${item.categoryId}koq${index}`} 
                    ...
                    <View style={{ flexDirection: 'row', flex: 1, backgroundColor: item.isAdd == 1 ? appcolor.light : appcolor.light, padding: 3 }}>
                        <Text style={{
                            padding: 7, width: '60%', color: appcolor.dark, fontSize: 12,
                            textAlignVertical: 'center', fontWeight: 'bold'
                        }}>
                            {item.model} {item.gaming == undefined ? "" : item.gaming == 1 ? "gaming" : "non-gaming"}
                        </Text>
                        <Text style={{
                            padding: 7, width: 90, textAlign: 'center', fontWeight: 'bold',
                            color: appcolor.dark, fontSize: 12, textAlignVertical: 'center',
                        }}>{item.quantity}</Text>
                        {
                            this.state.uploaded === 0 &&
                            <TouchableOpacity onPress={() => this.onRemoveModel(item)}
                                style={{ alignSelf: 'flex-end', }}>
                                <Icon name={item.isAdd === 1 ? 'add' : 'close'} size={14} raised
                                    color={item.isAdd === 1 ? appcolor.primary : appcolor.danger} />
                            </TouchableOpacity>
    ...
    showSummary = () => {
        ...
        var _sumlist = []
        Categories?.map(c => {
            var _temp = CompetitorResult.filter(r => r.isAdd === 0 && r.categoryId == c.id && r.quantity > 0)
            totalq = 0
            _temp?.forEach(s => {
                totalq += s.quantity
    ...
    rowSummary = ({ item, index }) => {
        ...
        return (
            <View>
                <View style={{ flexDirection: 'row', backgroundColor: appcolor.light, padding: 7 }}>
                    <Text style={{ flexGrow: 1, color: appcolor.dark }}>{item.category}</Text>
                    <Text style={{ textAlign: 'center', width: 100, color: appcolor.dark }}>{item.quantity || 0}</Text>
                </View>
            </View>)
    }
    render() {
        const { Categories, CompetitorResult, showProgress, summdata, total, uploaded } = this.state;
        ...
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.surface }}>
                    ...
                    <Tabs.Container
                        renderTabBar={props => (
                            <MaterialTabBar
                                ...
                                tabStyle={{ minWidth: minWidthTab(Categories || []), height: 42 }}
                                scrollEnabled={true}
                        ...
                        containerStyle={{ backgroundColor: appcolor.surface, flex: 1 }}>
                        {
                            Categories?.map((cateinfo, idx) => {
                                const dataByCate = CompetitorResult.filter(a => a?.categoryId === cateinfo.id).sort((a, b) => a.id - b.id)
                                return (
                                    <Tabs.Tab key={`mens${idx}`} label={cateinfo.name}
                                        name={cateinfo.name}>
                                        <View style={{
                                            flex: 1, backgroundColor: appcolor.surface,
                                            marginTop: 40, padding: 6, width: deviceWidth
                                        }}>
                                            <VirtualizedList
                                                data={dataByCate}
                                                key={`34k${cateinfo.id}19`}
                                                showsVerticalScrollIndicator={false}
                                                keyExtractor={(_, index) => { `${cateinfo.id}sf${index}2als` }}
                ...
                <ActionSheet containerStyle={{ backgroundColor: appcolor.light }}
                    id='sheetSummary'>
                    <Text style={{ fontSize: 16, color: appcolor.primary, padding: 7, textAlign: 'center' }}>Kết quả</Text>
                    <View style={{ backgroundColor: appcolor.surface, padding: 7 }}>
                        ...
                        />
                        {
                            ...
                            <TouchableOpacity onPress={this.onPostData}
                                style={{ width: '100%', padding: 7, backgroundColor: appcolor.primary, marginBottom: 20, marginTop: 10 }}>
                                <Text style={{ color: appcolor.white, fontSize: 12, textAlign: 'center' }}>
...
export default connect(mapStateToProps, mapDispatchToProps)(TrackingCompetitorReport);
