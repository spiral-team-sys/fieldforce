...
export default class AuditDisplayItems extends Component {
  ...
  async componentDidMount() {
    ...
    await this.setState({
      questionShop: this.props.route.params.DisplayItem.fieldSetting,
    });
  ...
  uploadAction = async () => {
    ...
    if (this.state.questionShop !== '' && this.state.questionShop !== null) {
      if (this.state.isHavePack === null) {
        MessageInfo(
          'Bạn chưa chọn câu trả lời ( ' + this.state.questionShop + ' ?)',
        );
        return;
      } else {
        if (this.state.isHavePack === false) {
          if (this.state.noteCommon === '') {
            MessageInfo(
              'Vui lòng nhập lý do khi bạn chọn cửa hàng không trưng bày theo gói.',
            );
            return;
          } else {
  ...
  UploadData = async (resDisplay, resPhotos) => {
    ...
    try {
      ...
      let UploadJson = {
        ...
        DisplayResult:
          this.state.questionShop === '1'
            ? this.state.isHavePack === true
              ? 'YES'
              : 'NO'
            : 'YES',
        ...
        .then(async responseJson => {
          ...
          if (responseJson.status == 200) {
            ...
            updateHavePack(
              this.state.workinfo,
              this.props.route.params.DisplayItem.id,
              this.state.isHavePack === true ? 'YES' : 'NO',
  ...
  loadDataShow = async (search, refName) => {
    let lstItemsProgram = await getItemsAuditDisplayMitsu(
      this.props.route.params.DisplayItem.id,
      search,
      refName,
    );
      ...
      <ImageBackground style={{ height: '100%', width: '100%' }}>
        ...
        {this.state.questionShop !== '' && this.state.questionShop !== null && (
          ...
          >
            ...
            >
              {this.state.questionShop}
            </Text>
            <TouchableOpacity
              style={{ alignItems: 'center', padding: 2 }}
              onPress={() => this.chooseHaveDisplayPack(true)}
              disabled={this.state.Status === 0 ? false : true}
            >
              <RadioButton
                selected={this.state.isHavePack === true ? true : false}
              />
              <Text style={{ marginTop: 8, paddingLeft: 1 }}>Có</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={{ alignItems: 'center', padding: 2 }}
              onPress={() => this.chooseHaveDisplayPack(false)}
              disabled={this.state.Status === 0 ? false : true}
            >
              <RadioButton
                selected={this.state.isHavePack === false ? true : false}
              />
