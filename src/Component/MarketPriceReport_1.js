...
const MarketPriceReport = ({ navigation, route }) => {
  ...
  const getListByCompetitor = async competitorId => {
    const lstData = await getListMarketPrice(
      route.params.workinfo,
      competitorId,
    );
    let subCategoryId = 0;
  ...
  const UploadData = async () => {
    ...
    if (checkData !== null && checkData.length > 0) {
      ...
      if (lstUpload.length > 0) {
        ...
        alertWarning(
          'Vui lòng hoàn thành đầy đủ dữ liệu báo cáo trước khi gửi lên hệ thống',
        );
      }
    }
  };
  ...
  const viewItemInput = async () => {
    if (!isCheckViewInput) {
      ...
      let dataChange = await mDataMain.filter(
        i => i.priceValue > 0 || i.netValue > 0 || i.fsmValue > 0,
      );
    ...
    return (
      ...
      >
        <TouchableOpacity
          ...
          onPress={() => competitorSelected(item)}
        >
          <Text
            h4
            style={{
        ...
        <SpiralIcon
          type="font-awesome-6"
          style={{ padding: 13, textAlignVertical: 'center' }}
          name={item.isSelect == 1 ? 'check' : ''}
          size={15}
          color={appcolor.black}
        />
      </View>
  ...
  return (
    ...
    >
      <PageHeader
        Title={route?.params?.itemMenu?.name}
      ...
      >
        <SearchBar
          containerStyle={{
            width: '90%',
            borderTopColor: appcolor.transparent,
      ...
      <Modal animationType="slide" visible={filterVisible}>
        <View style={styles.styleModal}>
          <View style={styles.modalHeader}>
            <Text
              h4
              style={{ color: DEFAULT_COLOR, fontWeight: 'bold', fontSize: 20 }}
            >
              {' '}
              Tìm kiếm sản phẩm{' '}
            </Text>
