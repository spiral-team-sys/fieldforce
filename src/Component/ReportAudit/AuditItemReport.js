...
import { formatNumber, groupDataByKey, isNotInteger, Message, MessageAction, ToastError, ToastSuccess } from "../../Core/Helper";
...
export const AuditItemReport = ({ navigation }) => {
	...
	const KeyStore = `${shopinfo.shopId || 0}AUDITITEMREPORT${shopinfo.auditDate}`
	...
	const uploadAction = async () => {
		...
		let itemsUpload = data.dataTaskList.filter(it => it.posmValue != null && it.posmValue != undefined);
		if (itemsUpload.length === 0) {
			Message('Chú ý', 'Bạn chưa điền thông tin, có muốn tiếp tục?', () => UploadData(data.dataTaskList));
			return;
		}
		Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', () => UploadData(data.dataTaskList));
	}
	const UploadData = async (itemsUpload) => {
		...
		await AuditAPI.UploadTaskList(itemsUpload, work, async () => {
	...
	const onSelectCheck = async (item, it, idx) => {
		...
		await arrValue.map(itm => {
			itm.ItemId === it.ItemId ? (itm.isCheck = itm.isCheck ? false : true) : (itm.isCheck = false)
		})
		...
		switch (item.kpI1) {
			case 1:
				return <View style={{ minHeight: 10, minWidth: 10 }}>
					<FormGroup
						containerStyle={{ backgroundColor: item.upload === 1 ? appcolor.surface : appcolor.light, alignSelf: 'center', marginBottom: 5 }}
						inputStyle={{ fontSize: 13, color: appcolor.dark }}
						placeholder={item.unit !== null ? item.unit : 'Nhập ' + item.productNameVN}
						editable={item.upload === 1 ? false : true}
						title={item.productNameVN}
						iconName='comment-alt'
						value={item.posmValue} handleChangeForm={(text) => onChangeText(item, text)}
					/>
				</View>
			case 2:
				return <View style={{ minHeight: 10, minWidth: 10 }}>
					<FormGroup
						...
						containerStyle={{ backgroundColor: item.upload === 1 ? appcolor.surface : appcolor.light, marginBottom: 5 }}
			...
			case 3:
				return <View style={{ minHeight: 10, minWidth: 10 }}>
					<View style={{ flex: 1, borderRadius: 10, borderWidth: 1, borderColor: appcolor.surface, padding: 5, marginBottom: 5 }}>
						<Text style={{ fontSize: 14, fontWeight: '600', color: appcolor.dark }}>{item.productNameVN}</Text>
								...
								JSON.parse(item.listItem || '[]').map(it => {
									return (
										<CheckBox
											...
											key={it.ItemId} size={20}
											title={it.ItemNameVN}
											checkedColor={appcolor.success}
											checkedIcon='check-square-o'
											uncheckedIcon='square-o'
											onPress={item.upload !== 1 ? () => onSelectCheck(item, it) : null}
											checked={it.isCheck}
										/>
									)
								})
			...
			<View style={{ flex: 1, padding: 5 }}>
				<KeyboardAvoidingView
					...
					<FlatList
						...
						ListFooterComponent={<View style={{ height: deviceHeight / 2, width: '100%' }} />}
					/>
				</KeyboardAvoidingView>
			</View>
		</View>
