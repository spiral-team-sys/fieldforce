...
export const RegionChoose = ({ itemMain, keyValue, isNewAddress = false }) => {
  ...
  const { appcolor, employeeInfo, regionData } = useSelector(
  ...
  const onSearchData = text => {
    ...
    const listUpdate = _.filter(dataMain, e =>
      removeVietnameseTones(e.name).toLowerCase().match(valueSearch),
    );
    setData(listUpdate);
  };
  const onFocusSearch = () => {
  ...
  const handlerOpenDetails = async item => {
    ...
    if (_isOpen) {
      ...
      _.map(data, e => {
        item.level1_id == e.level1_id &&
          lstChoose.push({ ...e, isOpen: _isOpen });
      });
    } else lstChoose = dataMain;
      ...
      <View key={`it_group_${index}`} style={styles.mainView}>
        <TouchableOpacity style={styles.itemViewMain} onPress={onPress}>
          ...
          <View style={{ marginStart: item.isOpen ? 8 : 0 }}>
            ...
            >{`${districtList.length} Quận/Huyện/Thị Xã`}</Text>
          </View>
        </TouchableOpacity>
        {item.isOpen && (
          <GroupListDistrict
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <ActionSheet
        id={`regionchoose_${itemMain.Ref_Id}`}
        drawUnderStatusBar
