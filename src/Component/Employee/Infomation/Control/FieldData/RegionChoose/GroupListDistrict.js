...
export const GroupListDistrict = ({ type, keyValue, keyInfo, dataDetails }) => {
    ...
    const styles = StyleSheet.create({
        ...
        itemViewMain: { flexDirection: 'row', alignItems: 'center', padding: 5, marginHorizontal: 8, backgroundColor: appcolor.light, marginBottom: 8, elevation: 3, shadowColor: appcolor.dark, shadowOffset: { width: 1, height: 1 }, shadowOpacity: 0.3, borderRadius: 8 },
        titleHead: { fontSize: 14, fontWeight: fontWeightBold, color: appcolor.dark, padding: 8 },
    })
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View key={`it_${type}_${index}`}>
                ...
                {item.isOpen &&
                    <GroupListTown key={`group_itt_${item.level2_id}`} type='TownCode' keyValue='name' keyInfo='addressTown' dataDetails={townList} />
                }
            </View>
        )
    }
