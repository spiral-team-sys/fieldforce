...
export const PhoneNumber = ({ itemMain, keyValue, isEdit }) => {
    ...
    const onChangeValue = (async (text, index) => {
        if (text !== null && text.length > 0)
            (index + 1) < dataPhone.length ? inputRef.current[index + 1].focus() : inputRef.current[index].blur();
        else
            inputRef.current[index - 1].focus();
        dataPhone[index] = text;
        const newEmployeeInfo = { ...employeeInfo, [keyValue]: dataPhone.join('').toString() }
    ...
    const styles = StyleSheet.create({
        ...
        contentMain: { width: '100%', flexDirection: 'row', marginTop: 5, justifyContent: 'center' },
        itemMain: { width: deviceWidth / 13.5, alignItems: 'center', marginEnd: 5 },
        containerStyle: { width: '100%', justifyContent: 'center', marginHorizontal: 4, padding: 0, backgroundColor: appcolor.light, borderWidth: 0.5, borderColor: appcolor.greylight },
        inputStyle: { fontSize: 15, fontWeight: '500', color: appcolor.dark, textAlign: 'center' },
        ...
        return (
            <View key={`it_phnm_${index}`} style={styles.itemMain}>
                <FormGroup
                    ...
                    containerStyle={{ ...styles.containerStyle, backgroundColor: colorBackgroud }}
        ...
        <View style={styles.mainContainer}>
            {isEdit ?
                <View style={styles.contentMain}>
                    {dataPhone !== null && dataPhone.length > 0 && dataPhone.map((item, index) => {
