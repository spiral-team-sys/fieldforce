...
export const PictureChoose = ({ itemMain, keyValue }) => {
    ...
    const LoadData = async (resultData) => {
        ...
        if (_photoList !== null && _photoList.length > 0) {
            for (let index = 0; index < _photoList.length; index++) {
                ...
                const fileName = info.photoPath.substring(info.photoPath.lastIndexOf('/') + 1, info.photoPath.length || 0);
                const itemUpload = {
                    ...info,
    ...
    const onRemoveAction = async (item) => {
        ...
        await LoadData(async (mDataResult) => {
            ...
            await dispatch(SET_EmployeeInfo(employeeInfo))
            //
            if (mDataResult == null || mDataResult.length == 0)
                setRemoveImage(false)
        })
    }
    //  
    ...
    const styles = StyleSheet.create({
        ...
        contentValue: { width: '100%', flexDirection: 'row', alignItems: 'center', borderRadius: 8, padding: 3, backgroundColor: appcolor.light, borderWidth: 0.5, borderColor: appcolor.grayLight },
        itemMain: { minWidth: deviceWidth / 2.5, height: deviceHeight / 7.5, margin: 8, marginEnd: 1, borderRadius: 5, borderWidth: 1.5, borderColor: appcolor.grayLight, overflow: 'hidden' },
