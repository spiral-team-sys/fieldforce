...
import { ColorRand, ToastError, removeVietnameseTones, requestPerrmission } from "../../../../../Core/Helper";
...
export const ContactInfo = ({ itemMain, keyValue }) => {
    ...
    const handlerChooseContacts = async (contacts) => {
        const _name = contacts.displayName || contacts.familyName || contacts.givenName || 'No Name'
        let _phone = []
    ...
    const searchContacts = (text) => {
        ...
        const searchList = _.filter(dataMain, (e) => removeVietnameseTones(e.displayName).toLowerCase().match(searchValue) ||
    ...
    const handlerShowContacts = () => {
        try {
            if (Platform.OS == 'android') {
                requestPerrmission({ android: PERMISSIONS.ANDROID.READ_CONTACTS, ios: PERMISSIONS.IOS.CONTACTS }, async (status) => {
    ...
    const onShowAction = () => {
        try {
            Contacts.getAll().then(async data => {
                ...
                const _sortData = Platform.OS == 'android' ? _.sortBy(data, 'displayName') : _.sortBy(data, 'familyName')
    ...
    const styles = StyleSheet.create({
        ...
        itemMain: { width: '100%', padding: 8, flexDirection: 'row', alignItems: 'center' },
        searchContainer: { margin: 8, marginTop: 12, padding: 3, paddingHorizontal: 8, borderRadius: 20, backgroundColor: appcolor.light, borderWidth: 0.5, borderColor: appcolor.greylight },
        searchInput: { fontSize: 13, color: appcolor.dark, fontWeight: '500' },
        avatarView: { width: '20%', alignSelf: 'center', alignItems: 'center' },
        infoView: { width: '80%' },
        circleView: { alignItems: 'center', justifyContent: 'center', width: 50, height: 50, borderRadius: 50, backgroundColor: appcolor.light, elevation: 3, shadowColor: appcolor.dark, shadowOffset: { width: 3, height: 3 }, shadowOpacity: 0.5, borderWidth: 0.5 },
        titleShortName: { fontSize: 21, fontWeight: '700', color: appcolor.primary },
        actionBottom: { alignItems: 'center', position: 'absolute', bottom: deviceHeight / 12, justifyContent: 'center', alignSelf: 'center', zIndex: 1000 },
        titleName: { fontSize: 15, fontWeight: fontWeightBold, color: appcolor.dark },
    ...
    const renderItem = ({ item, index }) => {
        ...
        const titleName = item.displayName || item.familyName || item.givenName || 'No Name'
        return (
            <View key={`cti_item_${index}`} style={styles.itemMain} >
                <View style={styles.avatarView}>
                    <View style={{ ...styles.circleView, borderColor: colorItem }}>
                        <Text style={{ ...styles.titleShortName, color: colorItem }}>{titleName.substring(0, 1)}</Text>
                    </View>
                </View>
                <TouchableOpacity style={styles.infoView} onPress={onPress}>
                    ...
                    {item.phoneNumbers !== null && item.phoneNumbers.length > 0 &&
                        item.phoneNumbers.map((it, idx) => {
                            return <Text key={`${it.number}_${idx}`} style={styles.contentName}>{it.number}</Text>
                        })
                    }
                </TouchableOpacity>
            </View>
