...
export const DateInput = ({ keyValue }) => {
    ...
    const updateEmployeeDate = async (dateValue) => {
        await dispatch(SET_EmployeeInfo({
            ...employeeInfo,
            [keyValue]: dateValue
        }))
    }
    ...
    const onChangeValue = async (text, type) => {
        ...
        //
        const isCompleted = (nextValueDate.DD || '').length === 2 && (nextValueDate.MM || '').length === 2 && (nextValueDate.YYYY || '').length === 4
        if (!isCompleted) {
    ...
    const styles = StyleSheet.create({
        ...
        inputContainer: { width: deviceWidth / 5, padding: Platform.OS === 'ios' ? 3 : 0, borderWidth: 0.5, borderColor: appcolor.grayLight, marginEnd: 8 },
        inputStyle: { fontSize: 14, fontWeight: '400', color: appcolor.dark, textAlign: 'center' },
        contentMain: { flexDirection: 'row' },
        characterView: { padding: 8, paddingStart: 0, color: appcolor.blacklight },
        errorText: { marginTop: 2, fontSize: 11, color: appcolor.red, fontStyle: 'italic' }
    })
    return (
        <View style={styles.mainContainer}>
            ...
            {dateError.length > 0 && <Text style={styles.errorText}>{dateError}</Text>}
        </View>
    )
}
