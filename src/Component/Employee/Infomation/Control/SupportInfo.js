...
import { Alert, Image, Linking, Modal, Platform, StyleSheet, TouchableOpacity, View } from "react-native";
...
export const SupportInfo = ({ navigation }) => {
    ...
    const handlerActionInfo = (type, item) => {
        switch (type) {
            case "ZALO":
                ...
                break
            case "CALL":
                const optionPhone = [{ text: 'Đóng' }, { text: 'Gọi ngay', onPress: () => { onCallAction(item.PhoneNumber) } }]
                optionConfirm(item.FullName, item.PhoneNumber, optionPhone)
                break
            case "EMAIL":
                const optionEmail = [{ text: 'Đóng' }, { text: 'Gửi ngay', onPress: () => { onEmailAction(item.Email) } }]
    ...
    const styles = StyleSheet.create({
        ...
        itemMain: { alignItems: 'center', margin: 5, borderRadius: 8, backgroundColor: appcolor.surface, borderWidth: 0.5, borderColor: appcolor.grayLight, overflow: 'hidden' },
        viewPhotoText: { borderWidth: 1, borderColor: appcolor.grayLight, padding: 3, backgroundColor: appcolor.light },
        titlePhotoText: { fontSize: 24, color: appcolor.primary, fontWeight: '600' },
        titleName: { fontSize: 14, padding: 7, fontWeight: fontWeightBold, color: appcolor.dark },
        descriptionName: { fontSize: 13, fontStyle: 'italic', fontWeight: '500', color: appcolor.dark },
        contentAction: { flexDirection: 'row', end: 0, top: 0, bottom: 0, paddingVertical: 5 },
    })
        ...
        return (
            <View key={`si_item_${index}`} style={styles.itemMain}>
                ...
                <View style={styles.contentAction}>
                    <TouchableOpacity onPress={() => onPress("ZALO")} style={{ marginLeft: 5, shadowColor: appcolor.dark, shadowOffset: { width: 3, height: 0 }, elevation: 3, shadowOpacity: 0.5 }}>
                        <Image
