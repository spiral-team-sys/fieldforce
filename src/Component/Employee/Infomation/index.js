...
import { ActivityIndicator, DeviceEventEmitter, Platform, RefreshControl, ScrollView, StatusBar, StyleSheet, View } from "react-native";
...
import { SET_EmployeeInfo, SET_MasterData, SET_RegionData, SetUserInfo } from "../../../Redux/action";
import _ from 'lodash'
import CustomListView from "../../../Control/Custom/CustomListView";

export const EmployeeInfo = ({ navigation }) => {
    const { appcolor, userinfo, employeeInfo } = useSelector(state => state.GAppState)
    ...
    const LoadData = async () => {
        ...
        await EmployeeAPI.getDataEmployeeInfo(async (mEmployee, mMasterData, mConfigPage, messager) => {
            messager && ToastError(messager)
            await asyncUserNewInfo(mEmployee)
            await setConfigPage(JSON.parse(mConfigPage.configResign || '{}')[0] || {})
            await setConfigMaternity(JSON.parse(mConfigPage.configMaternity || '{}')[0] || {})
            await dispatch(SET_EmployeeInfo(mEmployee))
            await dispatch(SET_MasterData(JSON.parse(mMasterData.masterList || '[]')))
            await dispatch(SET_RegionData(JSON.parse(JSON.stringify((mConfigPage.isUseRegionLocal == 1 ? dvhc2025.data : dvhcvn.data) || []))))
        })
    ...
    const onGetInformation = async (data) => {
        ...
        //
        const options = [{ text: 'Bỏ qua' },
        {
            text: 'Cập nhật', onPress: async () => {
                await EmployeeAPI.saveDataEmployeeInfo(info, async (statusId, message) => {
                    statusId == 200 ? ToastSuccess(message, 'Thông báo', 'top') : ToastError(message, 'Lỗi dữ liệu', 'top')
                })
                await LoadData()
            }
        }]
    ...
    //
    useEffect(() => {
        const reload_employee = DeviceEventEmitter.addListener('RELOAD_EMPLOYEE', LoadData)
        let isMounted = true
        if (!isMounted)
            return
        LoadData()
    ...
    const styles = StyleSheet.create({
        mainContainer: { width: '100%', height: '100%', backgroundColor: appcolor.primary, paddingTop: insets().top },
        headMain: { width: '100%', height: '15%' },
        viewHeadMain: { width: '100%', height: '100%' },
        contentMain: { width: '100%', height: '100%', marginTop: 32 },
        buttonCloseView: { position: 'absolute', top: 0, start: 8, zIndex: 1000, elevation: 1000 },
        gradientView: { width: '100%', height: '100%', zIndex: 0 },
        viewLoading: { position: 'absolute', top: 0, end: 0, start: 0, bottom: 0 },
        overflowView: { width: '100%', height: '100%', position: 'absolute', backgroundColor: appcolor.dark, opacity: 0.8, justifyContent: 'center', zIndex: 20, elevation: 20 },
        fabLayer: { position: 'absolute', top: 0, end: 0, bottom: 0, start: 0, zIndex: 100, elevation: 100 }
    })

    return (
        <View style={styles.mainContainer}>
            <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 3 }} colors={[appcolor.primary, appcolor.light]} style={styles.gradientView}>
                <View style={styles.buttonCloseView}>
                    <ButtonAction iconName='arrow-back' typeAction='CLOSE' onPress={handlerGoBack} />
                </View>
                {isLoading && <ActivityIndicator color={appcolor.light} style={styles.viewLoading} />}
                {/* // Content Info */}
                {!isLoading &&
                    <CustomListView
                        data={['']}
                        renderItem={() => <ScreenEmployeeInfo key={`itemEmployeeInfo`} navigation={navigation} />}
                        onRefresh={LoadData}
                        bottomView={{ paddingBottom: 32 }}
                    />
                }
                {menu.isOpen ? <View pointerEvents="none" style={styles.overflowView} /> : <View />}
