...
export const ItemDetails = ({ navigation, itemMain }) => {
  ...
  const itemEdits = item => {
    switch (item.Ref_Name) {
      ...
      case 'phonenumber':
        return (
          <PhoneNumber itemMain={item} keyValue={item.Ref_Code} isEdit={true} />
        );
      case 'date':
      ...
      case 'region':
        return (
          <RegionChoose itemMain={item} keyValue={item.Ref_Code} isNewAddress />
        );
      case 'checkbox':
  ...
  const itemView = item => {
    const dataPhoto =
      item.Ref_Name == 'picture'
        ? JSON.parse(employeeInfo[item.Ref_Code] || '[]')
        : [];
  ...
  const renderItem = ({ item, index }) => {
    return (
      <View
        key={`dimmi_${itemMain.GroupId}_${index}`}
        style={styles.itemViewMain}
      >
        {item.ItemName !== null && item.ItemName.length > 0 && (
        ...
        {item.DescriptionName && itemMain.isUpdate && (
          <Text style={styles.subTitleView}>{item.DescriptionName}</Text>
        )}
        {!itemMain.isUpdate || item.IsLock == 1
          ? itemView(item)
          : itemEdits(item)}
      </View>
    );
