...
import { ActivityIndicator, DeviceEventEmitter, LayoutAnimation, Platform, SafeAreaView, StyleSheet, UIManager, View } from "react-native";
...
import { TODAY, alertWarning, deviceHeight, optionConfirm } from "../../../../Core/Utility";
...
export const ScreenEmployeeInfo = ({ navigation }) => {
    const { masterData, employeeInfo, appcolor, userinfo } = useSelector(state => state.GAppState)
    ...
    const syncDataEmployee = async () => {
        ...
        await EmployeeAPI.saveDataEmployeeInfo(employeeInfo, async (statusId, message) => {
            if (statusId == 200) {
    ...
    const onUpdateInfo = async (item) => {
        ...
        //
        const listUpdate = _.map(data, (e) => { return e.GroupId == item.GroupId ? { ...e, isUpdate: _update } : { ...e, isUpdate: false } })
    ...
    const isValidDateValue = (value) => {
        ...
        return moment(valueString, 'DD/MM/YYYY', true).isValid()
    ...
    const validData = (dataDetailList) => {
        ...
        if (dataRequired !== null && dataRequired.length > 0) {
            ...
            for (let index = 0; index < dataRequired.length; index++) {
                ...
                if (_value == null || _value.length < (item.MaxLength || 1) || _value == '[]')
                    strError += `- ${item.ItemName}\n`
            }
            if (strError !== null && strError.length > 0) {
                alertWarning(`Vui lòng nhập đầy đủ thông tin bắt buộc (Có gắn sao đỏ):\n${strError}`)
                return false
            }
        }
        const dataDateInvalid = _.filter(dataDetailList, (e) => e.Ref_Name === 'date' && !isValidDateValue(employeeInfo[e.Ref_Code]))
        if (dataDateInvalid !== null && dataDateInvalid.length > 0) {
            const strError = _.map(dataDateInvalid, (item) => `- ${item.ItemName}`).join('\n')
            alertWarning(`Ngày tháng năm không hợp lệ, vui lòng nhập đúng định dạng DD/MM/YYYY:\n${strError}`)
            return false
        }
        return true
    }
    const onOpenInfo = (item) => {
        const listUpdate = _.map(data, (e) => {
            const itemUpdate = e.GroupId == item.GroupId ?
    ...
    const handlerChangeAvatar = () => {
        const options = [{ text: 'Hủy' }, { text: 'Máy ảnh', onPress: handlerCameraAction }, { text: 'Thư viện', onPress: handlerGalleryAction }]
        optionConfirm('Thay đổi hình ảnh', 'Bạn có thể tự chụp hình chân dung bằng Máy ảnh hoặc Chọn ảnh từ thư viện ảnh', options)
    }
    ...
    useEffect(() => {
        ...
        //
        LoadData()
        const _syncData = DeviceEventEmitter.addListener('SYNC_DATA_EMPLOYEE', syncDataEmployee)
        return () => {
            isMounted = false
            _syncData.remove()
        }
    ...
    const styles = StyleSheet.create({
        ...
        titleHeadName: { width: '100%', fontSize: 18, fontWeight: '700', color: appcolor.light, textAlign: 'center' },
        titleContentName: { fontSize: 13, fontWeight: '500', color: appcolor.surface, textAlign: 'center' },
        viewPhoto: { backgroundColor: appcolor.light, padding: 3 },
        viewPhotoText: { borderWidth: 3, borderColor: appcolor.grayLight, padding: 3, backgroundColor: appcolor.light },
        titlePhotoText: { fontSize: 68, color: appcolor.primary, fontWeight: '600' },
        avatarView: { alignSelf: 'center', borderRadius: 100 },
        infoBasicView: { width: '100%', alignSelf: 'center', padding: 8 },
        actionAvatar: { flexDirection: 'row', position: 'absolute', bottom: 0, end: 0, backgroundColor: appcolor.light, borderRadius: 50 },
        contentMain: { width: '100%', height: '100%', padding: 12 },
        itemMain: { width: '100%' },
        parentView: { width: '100%', flexDirection: 'row', alignItems: 'center', marginVertical: 8, padding: 8, marginBottom: 0 },
        contentInfoMain: { width: '100%', minHeight: deviceHeight / 2.3, borderRadius: 16, backgroundColor: appcolor.light, elevation: 3, shadowColor: appcolor.dark, shadowOffset: { width: 3, height: 0 }, marginBottom: 8 },
        itemHeadName: { fontSize: 15, fontWeight: fontWeightBold, color: appcolor.surface, marginStart: 8 },
        itemEditName: { fontSize: 15, fontWeight: '500', color: appcolor.surface, textDecorationLine: 'underline', padding: 8, textAlign: 'right', fontStyle: 'italic' },
    ...
    const renderItem = ({ item, index }) => {
        ...
        const iconView = item.isOpen == 0 ? "chevron-down-outline" : "chevron-up-outline"
        return (
                ...
                {item.isParent &&
                    <TouchableOpacity style={styles.parentView} onPress={onPressOpen}>
                        {item.isOpen == 1 && isLoading ?
                            ...
                            :
                            <Icon type="ionicon" name="information-circle" size={18} color={appcolor.surface} />
                        }
                        ...
                        {item.isEditData == 1 && item.isOpen == 1 ?
                            <View style={styles.endActionView}>
                                ...
                                <TouchableOpacity onPress={onPress} >
                                    <Text style={styles.itemEditName}>{item.isUpdate ? 'Lưu dữ liệu' : 'Chỉnh sửa'}</Text>
                                </TouchableOpacity>
                            </View>
                            :
                            <View style={styles.endActionView}>
                                <Icon type="ionicon" name={iconView} size={18} color={appcolor.light} />
                            </View>
                        }
                    </TouchableOpacity>
                }
            ...
            <View style={styles.avatarView}>
                {info.photo ?
                    <Avatar
                        ...
                        size='xlarge'
                        source={{
                            uri: (info.avatarPath || null) !== null ? info.avatarPath : (info.photo.includes('file://') ? info.photo : `${URLDEFAULT}${info.photo}`)
                        }}
                ...
                <View style={styles.actionAvatar}>
                    <ButtonAction typeAction='UPDATEPHOTO' iconName='camera' sizeView={38} iconSize={18} onPress={handlerChangeAvatar} />
                </View>
            </View>
