...
export const EmployeeSOTrend = ({ data }) => {
    ...
    const processData = () => {
        ...
        var _barData = [], _lineData = [], _colData = [];
        list?.forEach((value) => {
        ...
        updateChart.data = {
            barData: {
                dataSets: [{
                    ...
                    config: {
                        colors: [processColor(appcolor.primary), processColor(appcolor.grey)],
    ...
    return (
        <Fragment>
            <Text style={{ marginLeft: 12, padding: 7, fontSize: scaleSize(18), color: appcolor.dark, fontWeight: '600' }}>{data.groupNameVN}</Text>
            <View style={{ height: 260, backgroundColor: appcolor.white, borderRadius: 12, margin: 7, }}>
                <CombinedChart
