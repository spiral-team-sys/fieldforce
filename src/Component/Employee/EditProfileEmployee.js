...
const EditProfileEmployee = ({ navigation, route }) => {
  ...
  const [dataCalendar, setDataCalendar] = useState({
    markedDatesDefault: {
      [moment(new Date()).format('YYYY-MM-DD').toString()]: {
        color: '#ffa500',
        startingDay: workingStatusId === 3 ? false : true,
    ...
    markedDates: {
      [moment(new Date()).format('YYYY-MM-DD').toString()]: {
        color: '#ffa500',
        startingDay: employee.toDate?.toString() ? true : false,
        textColor: appcolor.dark,
      },
    },
    markingType: 'period',
  ...
  const loadDataCalendar = () => {
    if (employee.toDate) {
      ...
      handlerSelectCalendar(
        moment(employee.toDate?.toString()).format('YYYY-MM-DD'),
      );
    }
  };
  ...
  const uploadAction = async () => {
    ...
    let fromdateConvert =
      parseInt(
        moment(dataCalendar.startDate || new Date()).format('YYYYMMDD'),
      ) || '';
    let todateConvert =
      parseInt(moment(dataCalendar.endDate).format('YYYYMMDD')) || '';
    ...
    if (workingStatusId !== 3) {
      ...
      workingStatusId === 3 &&
      fromdateConvert < parseInt(moment(new Date()).format('YYYYMMDD')) &&
      AppNameBuild !== lgApp
    ) {
    ...
    if (workingStatusId === 3 && lstReport?.isShowResign === 1) {
      if (
        ...
        lstReport?.isCheckResignImage === 1 &&
        employee.isUncheckImage !== 1
      ) {
    ...
    if (
      ...
      (employeeProfile.notes || '').length === 0
    ) {
      MessageAction(
        'Bạn chưa nhập ghi chú về việc vi phạm thời gian báo trước theo hợp đồng lao động của nhân viên. Bạn có chắc chắn muốn tiếp tục cho nhân viên nghỉ việc không ?',
        async () => {
  ...
  const UploadData = async () => {
    Message('Chú ý', 'Bạn chắc chắn muốn lưu thay đổi?', async () => {
      ...
      let ImgName =
        ...
        employeeProfile.photo.substring(
          employeeProfile.photo.lastIndexOf('/') + 1,
          employeeProfile.photo?.length,
        );
      let fileName =
      ...
      const itemUpload = [
        {
          ...
          photo: employeeProfile.photo?.includes('file://')
            ? fileName
            : employeeProfile.photo,
          ...
          reasonId:
            workingStatusId === 3
              ? lstReport?.isUseDetailReason === 1
                ? employeeProfile.detailReasonId || null
                : employeeProfile.reasonId || null
              : null,
        },
      ];
      if (workingStatusId === 3 && lstReport?.isShowResign === 1) {
        ...
        if (
          ...
          if (employeeProfile.isSendMailById == 1) {
            ...
            if (result.statusId === 500 && result.messager.includes('mail')) {
              MessageInfo(
                'Thông tin nghỉ việc đã được cập nhật nhưng có lỗi xảy ra khi gửi mail thông báo cho các phòng ban liên quan. Vui lòng liên hệ quản lí để biết thêm chi tiết.',
              );
            } else {
              MessageInfo(
                'Thông tin nghỉ việc đã được cập nhật và đã gửi mail thông báo cho các phòng ban liên quan.',
              );
            }
          }
          setLoadData(true);
        ...
        await setProgress(true);
        const result = await Employee.uploadProfile(
  ...
  const handleConfirmDevice = async (needCheck, devices) => {
    ...
    MessageAction(
      'Ban có chắc chắn muốn cập nhật thay đổi này không?',
      async () => {
  ...
  const onChangeWorkingStatus = item => {
    ...
    setEmployeeProfile(data => ({
      ...data,
      workingStatusName: item.WorkingStatusName,
    }));
  ...
  const onChangeReason = item => {
    setEmployeeProfile(data => ({
      ...data,
      reasonId:
        item.ReasonName !== employeeProfile.reasonName ? item.ReasonId : null,
      reasonName:
        item.ReasonName !== employeeProfile.reasonName ? item.ReasonName : '',
    }));
  ...
  const onChangeDetailReason = item => {
    setEmployeeProfile(data => ({
      ...data,
      detailReasonId:
        item.DetailReasonName !== employeeProfile.detailReasonName
          ? item.DetailReasonId
          : null,
      detailReasonName:
        item.DetailReasonName !== employeeProfile.detailReasonName
          ? item.DetailReasonName
          : '',
    }));
  ...
  const onChangeStatus = value => {
    if (!value) {
      if (workingStatusId === 2 || workingStatusId === 1) {
        MessageInfo(
          'Vui lòng cập nhật trạng thái làm việc trước khi khoá tài khoản!',
        );
        return;
      }
    } else if (value) {
      if (workingStatusId !== 2 && workingStatusId !== 1) {
        MessageInfo(
          'Vui lòng cập nhật trạng thái làm việc trước khi mở tài khoản!',
        );
        return;
      }
    }
  ...
  const RenderItemPhoto = ({ item, index, listPhotoItem }) => {
    ...
    return item.photoPath === null ? (
      <TouchableOpacity
        key={index}
        style={styles.itemPhoto}
        onLongPress={() =>
          listPhotoItem.length > 1 ? onLongPressImage() : null
        }
        onPress={() =>
          isShowDelete ? null : SheetManager.show('ref_takePhoto')
        }
      >
        <SpiralIcon
          color={appcolor.primary}
      ...
      >
        <Image
          ...
          style={styles.itemInputPhotoImage}
        />
        {isShowDelete && (
  ...
  const styles = StyleSheet.create({
    ...
    profileAvatarBox: {
      ...
      justifyContent: 'center',
      alignItems: 'center',
      height: deviceHeight / 4 + 40,
      backgroundColor: appcolor.light,
    },
    avatarOverlay: { borderRadius: 20 },
    profileKeyboardAvoiding: {
      flex: 1,
    ...
    takePhotoContainerText: {
      padding: 8,
      flexDirection: 'row',
      justifyContent: 'center',
      alignItems: 'center',
    },
    takePhotoContainerTextTitle: {
      color: appcolor.dark,
      fontSize: 17,
      fontWeight: '600',
    },
  ...
  if (showProgress)
    return (
      <LoadingDefault isLoading={showProgress} styles={styles.loadingView} />
    );
  return (
      ...
      <View style={styles.profileContainer}>
        ...
        >
          ...
          {isShowDelete && (
            <TouchableOpacity
              onPress={() =>
                numDelete > 0 ? deleteItemImage() : cancelDelete()
              }
              style={styles.deleteItemImage}
            >
              <Text style={styles.deleteItemImageText}>
                {numDelete > 0 ? 'Xoá(' + numDelete + ')' : 'Huỷ'}
              </Text>
            </TouchableOpacity>
          )}
        </KeyboardAvoidingView>
          ...
          <View style={styles.containerWorkButton}>
            {workingStatusId == 3 && (
              <TouchableOpacity
                ...
                onPress={() =>
                  handleNextStep('ref_reasonSheet', 'ref_workStatusSheet')
                }
              >
                <Text style={styles.textWork}>Tiếp</Text>
              </TouchableOpacity>
            )}
          </View>
          {employeeProfile.workingStatusName !== 'Nhân viên chính thức' && (
            <Calendar
              ...
              style={{ height: 370 }}
              current={moment(
                dataCalendar.startDate?.toString() || new Date(),
              ).format('yyyy-MM-DD')}
              monthFormat={'MM - yyyy'}
              hideExtraDays={true}
            ...
            return (
              <TouchableOpacity
                ...
                style={{
                  ...
                  borderRadius: 10,
                  backgroundColor:
                    item.name == employeeProfile.gender
                      ? appcolor.primary
                      : appcolor.surface,
                  borderBottomColor: appcolor.grey,
                  borderBottomWidth: 1,
          ...
          <View style={{ flex: 1 }}>
            ...
            >
              <TouchableOpacity
                ...
                onPress={() =>
                  handlePrevStep('ref_reasonSheet', 'ref_workStatusSheet')
                }
              >
                <Text
                  style={{
                  ...
                  >
                    ...
                    <TouchableOpacity
                      ...
                      style={{
                        ...
                        borderRadius: 10,
                        backgroundColor:
                          it.ReasonName == employeeProfile.reasonName
                            ? appcolor.primary
                            : appcolor.light,
                        marginBottom: 4,
                        marginLeft: 8,
    ...
    return (
      <View style={styles.itemInputRow}>
        ...
        {field === 'date' && (
          ...
          >
            <Text
              style={{ textAlign: 'center', padding: 5, color: appcolor.dark }}
            >
