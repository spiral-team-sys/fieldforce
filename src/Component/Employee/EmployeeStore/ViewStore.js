...
export const ViewStore = ({ navigation, route }) => {
    ...
    const [data, setData] = useState({ dataStore: [], dataStoreF: [], dataEmployee: {} });
    ...
    const loadData = async () => {
        ...
        if (storeList == null) {
            ...
            dataStore.forEach((it) => {
                const indexStorage = storeList.findIndex((its) => its.shopId == it.shopId);
    ...
    const handleSort = (type) => {
        ...
        switch (type) {
            case 'MTD':
                ...
                if (newStatus == 1) {
                    list = data.dataStore?.sort((a, b) => (a.TotalMTD || 0) - (b.TotalMTD || 0));
                    listF = data.dataStoreF?.sort((a, b) => (a.TotalMTD || 0) - (b.TotalMTD || 0));
                } else {
                    list = data.dataStore?.sort((a, b) => (b.TotalMTD || 0) - (a.TotalMTD || 0));
                    listF = data.dataStoreF?.sort((a, b) => (b.TotalMTD || 0) - (a.TotalMTD || 0));
                }
    ...
    const saveToStorage = async () => {
        ...
        await AsyncStorage.setItem(KEYSTORE_REPORT, JSON.stringify(data.dataStoreF));
    };
