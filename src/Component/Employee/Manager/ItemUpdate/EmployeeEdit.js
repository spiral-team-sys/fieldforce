...
import { TODAY, alertConfirm, alertError, alertNotify, alertWarning, deviceHeight } from '../../../../Core/Utility';
...
export const EmployeeEdit = ({ itemEdit, onClose }) => {
    ...
    const [configReport, _setConfigReport] = useState(JSON.parse(kpiinfo?.reportItem))
    //
    ...
    const updateProfile = async () => {
        if (checkInput()) {
            alertConfirm('Cập nhật dữ liệu', 'Bạn có muốn cập nhật các thay đổi không ?', handlerUpload)
        }
    }
    const handlerUpload = async () => {
        const fileNameAvartar = employee.photo !== null ? employee.photo.substring(employee.photo.lastIndexOf('/') + 1, employee.photo.length) : null
        const fromDateValue = employee.fromDate !== null ? moment(employee.fromDate).format('YYYYMMDD') : null
        const toDateValue = employee.toDate !== null ? moment(employee.toDate).format('YYYYMMDD') : null
        const itemUpload = [{
            ...
            photo: fileNameAvartar !== null ? `/uploaded/${TODAY}/${fileNameAvartar}` : null,
        ...
        if (employee.workingStatusId == 3) {
            const photoResign = await Employee.getPhotoResignByEmployee(employee.employeeCode)
            let photoUpload = []
            if (photoResign !== null && photoResign.length > 0) {
                photoResign?.forEach(item => {
                    if (item.photoPath != null) {
                        let ImgName = item.photoPath.substring(item.photoPath.lastIndexOf('/') + 1, item.photoPath.length);
            ...
            const notifyContent = `Quản lí ${userinfo.employeeName} đã cho nhân viên ${employee.lastName || ''} ${employee.fisrtName || ''} nghỉ việc vào ngày ${employee.fromDate} lúc ${moment(new Date()).format('HH:mm')}.`
            const itemConfirm = {
                "confirm": 1,
                "employeeId": employee.employeeId,
                "fromDate": fromDateValue,
            ...
            // Send Data
            const result = await Employee.sendEmployeeResigns(JSON.stringify(itemConfirm), dataPhoto, 'CONFIRM', (userinfo.groupType || 'SUP'), employee.employeeId)
        ...
        // Update profile info
        const updateProfile = await Employee.uploadProfile(JSON.stringify(itemUpload), dataPhoto)
        if (updateProfile) {
    ...
    const checkInput = () => {
        if (configReport?.isShowResign == 1)
            if (employee.workingStatusId == 3 || employee.workingStatusName == 'Nghỉ việc') {
                if ((employee.photoResign == null || employee.photoResign.length == 0) && configReport?.isCheckResignImage == 1) {
                    alertWarning('Vui lòng bổ sung hình ảnh "Đơn xin nghỉ việc" của nhân viên trước khi cập nhật thông tin')
                    return false
                }
                if ((employee?.reasonId || 0) == 0) {
                    alertWarning('Vui lòng chọn "Nội dung nghỉ việc" của nhân viên trước khi cập nhật thông tin')
                    return false
                }
                // Khác
                if (employee?.reasonId == 100 && (employee.noteResign == null || employee.noteResign.length == 0 || employee.noteResign.length <= 10)) {
                    alertWarning('Vui lòng nhập "Lí do" nghỉ việc chi tiết trước khi cập nhật thông tin')
                    return false
                }
            }

        if (employee.workingStatusName !== 'Nghỉ việc' && employee.workingStatusName !== 'Nhân viên chính thức') {
            if (employee.fromDate == null || employee.toDate == null) {
                alertWarning(`Vui lòng chọn thời gian Từ ngày - Đến ngày "${employee.workingStatusName}" trước khi cập nhật thông tin`)
                return false
            }
        }
        return true
    ...
    const handlerViewImage = async () => {
        if (employee.photo == null || employee.photo == undefined)
            return
        const path = employee.photo && (employee.photo.includes('file://') ? employee.photo : (URLDEFAULT + employee.photo))
    ...
    const actionGalaryProfile = async () => {
        let options = { mediaType: 'photo', maxWidth: 800, maxHeight: 1024, quality: 0.4, includeBase64: true }
    ...
    const _handlerLockUser = () => {
        const valueContent = employee.status == 1 ? 'Bạn có muốn khoá tài khoản này không ?' : 'Bạn có muốn mở tài khoản này không ?'
    ...
    const styles = StyleSheet.create({
        ...
        mainItem: { margin: 8, borderWidth: 0.8, borderRadius: 8, overflow: 'hidden' },
        titleEmployee: { fontSize: 16, fontWeight: '700', color: appcolor.light },
        subTitleEmployee: { fontSize: 13, fontWeight: '500', color: appcolor.light, fontStyle: 'italic' },
        headerView: { width: '100%', flexDirection: 'row', zIndex: 1 },
        inputView: { width: '100%', height: deviceHeight, padding: 8, marginTop: 8 },
        iconType: { padding: 8 },
        actionAvatar: {
            flexDirection: 'row', position: 'absolute', bottom: 0, end: 0, backgroundColor: appcolor.light,
            borderRadius: 30, borderWidth: 0.5, borderColor: appcolor.grayLight
        },
        buttonUpdate: { padding: 8, margin: 8, borderWidth: 0.5, borderColor: appcolor.primary, borderRadius: 20, width: deviceWidth / 2.5 }
    })
    return (
        <View style={styles.mainContainer}>
            <View style={{ width: '100%', height: deviceHeight / 6, zIndex: -10, backgroundColor: employee.backgroundColor, position: 'absolute', top: -100 }} />
            {/* Content Header */}
            <View style={styles.headerView}>
                <View style={{ alignSelf: 'center', width: '100%', backgroundColor: employee.backgroundColor || appcolor.primary, padding: 8 }}>
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        <Icon
                            ...
                            style={{ backgroundColor: appcolor.light, padding: 2, borderRadius: 20 }}
                        />
                    </View>
                    <Text style={styles.subTitleEmployee}>{`Code: ${employee.employeeCode}`}</Text>
                    <Text style={styles.subTitleEmployee}>{`User: ${employee.username}`}</Text>
                </View>
                <View style={{ position: 'absolute', end: 16, top: -8 }}>
                    {employee.photo !== null ?
                        <Avatar
                            rounded size='xlarge'
                            source={{ uri: employee.photo && (employee.photo.includes('file://') ? employee.photo : (URLDEFAULT + employee.photo)) }}
                        ...
                        <Avatar
                            ...
                            containerStyle={{ borderWidth: 3, borderColor: appcolor.placeholderBody, padding: 3, backgroundColor: appcolor.light }}
                            onPress={handlerViewImage}
                        />
                    }
            ...
            <TouchableOpacity onPress={updateProfile} style={styles.buttonUpdate}>
                <Text style={{ width: '100%', textAlign: 'center', color: appcolor.primary, fontWeight: '700', fontSize: 13 }}>Cập nhật thông tin</Text>
            </TouchableOpacity>
            {/* Content Input */}
            <ScrollView
                showsVerticalScrollIndicator={false}
                style={{ width: '100%', height: deviceHeight, padding: 5, paddingTop: 8 }}
                nestedScrollEnabled  >
                <KeyboardAwareScrollView
                    ...
                    />
                    {/* Resign Employee */}
                    {configReport?.isShowResign == 1 && employee.workingStatusName == 'Nghỉ việc' &&
                        <ResignView
                            key='resign'
                    ...
                    {employee.workingStatusName !== 'Nghỉ việc' && employee.workingStatusName !== 'Nhân viên chính thức' &&
                        <FromDateToDate
            ...
            {/* Photo View */}
            <ActionSheet id='imageview'
                containerStyle={{ backgroundColor: appcolor.dark }}
                gestureEnabled
