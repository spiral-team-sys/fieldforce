...
import { DeviceEventEmitter, FlatList, Linking, Platform, RefreshControl, StyleSheet, TouchableOpacity, View } from "react-native";
...
export const HomeManager = ({ navigation }) => {
	...
	const onCallEmployee = (phone) => {
		let phoneNumber = Platform.OS == "ios" ? `telprompt:${phone}` : `tel:${phone}`
	...
	const onSearchEmployee = async (text) => {
		...
		const filterlist = _.filter(dataMain, (e) => {
			return removeVietnameseTones(e.employeeName).toLowerCase().match(value.toLowerCase()) ||
				removeVietnameseTones(e?.employeeCode || '').toLowerCase().match(value.toLowerCase()) ||
				removeVietnameseTones(e?.address || '').toLowerCase().match(value.toLowerCase()) ||
				removeVietnameseTones(e?.mobile || '').toLowerCase().match(value.toLowerCase()) ||
				removeVietnameseTones(e?.gender || '').toLowerCase().match(value.toLowerCase())
		})
	...
	useEffect(() => {
		...
		const _updateprofile = DeviceEventEmitter.addListener('UPDATE_LIST_PROFILE', () => { LoadData() })
		return () => {
			_load
	...
	const styles = StyleSheet.create({
		...
		subTitleEmployee: { fontSize: 14, fontWeight: '500', color: appcolor.light, fontStyle: 'italic' },
		contentUser: { padding: 8 },
		titleContent: { width: '75%', fontWeight: '500', fontSize: 14, marginTop: 3, color: appcolor.blacklight },
		contentAction: { position: 'absolute', bottom: 8, end: 10, flexDirection: 'row', alignItems: 'center' },
			...
			<TouchableOpacity key={`ide_${index}`} onPress={handlerDetails}>
				<View style={{ ...styles.mainItem, borderColor: colorItem }}>
					...
					<View style={styles.contentHeader}>
						<View style={{ alignSelf: 'center', width: '100%', backgroundColor: colorItem, padding: 8 }}>
							...
							<Text style={styles.subTitleEmployee}>{`Code: ${item.employeeCode} / User: ${item.username} `}</Text>
						</View>
					...
					<View style={styles.contentUser}>
						{item.listShopName !== null && <Text style={styles.titleContent}>{`${item.listShopName}`}</Text>}
						<Text style={styles.titleContent}>{`Địa chỉ: ${item.address || 'Không có địa chỉ'}`}</Text>
						<Text style={styles.titleContent}>{`SĐT: ${item.mobile}`}</Text>
						<Text style={styles.titleContent}>{`Email: ${item.email}`}</Text>
						<Text style={styles.titleContent}>{`Giới tính: ${item.gender}`}</Text>
						<Text style={styles.titleContent}>{`Ngày vào làm: ${item.workingDate}`}</Text>
						<Text style={styles.titleContent}>{`Phép còn lại: ${item.holiday}`}</Text>
						<Text style={styles.titleContent}>{`Trạng thái làm việc: ${item.workingStatusName}`}</Text>
					</View>
					{/* Action User */}
					<View style={styles.contentAction}>
						<TouchableOpacity onPress={handlerCall}>
							<Icon
								...
								size={18}
								color={appcolor.info}
								style={{ padding: 8, paddingHorizontal: 24, borderRadius: 12, borderWidth: 0.5, borderColor: appcolor.info }} />
						</TouchableOpacity>
					</View>
				</View>
			</TouchableOpacity>
	...
	const contentEmployee = () => {
		return (
			tabList !== null && tabList.length > 0 && tabList.map((item, index) => {
				const dataType = _.filter(dataEmployee, (e) => { return e.groupType == item.groupType })
	...
	return (
		<View style={styles.mainContainer}>
			<HeaderCustom
				...
				rightFunc={JSON.parse(kpiinfo?.reportItem || '{}')?.isShowConfirm == 1 ? handlerConfirmResign : null}
			...
			<View style={styles.contentMain}>
				...
				/>
				<LoadingView isLoading={isLoading} title='Đang cập nhật danh sách nhân viên' styles={{ marginTop: 8 }} />
					...
					<Tabs.Container
						...
						renderTabBar={props => (
							<MaterialTabBar
								...
								style={{ backgroundColor: appcolor.light, borderWidth: 0, paddingStart: 8, paddingEnd: 8 }}
							/>
						)}
