...
import { deleteItemPhotoDuplicate, getPhotoByType } from "../../../../Controller/DisplayController"
...
export const ResignView = ({ title, employee, dataReason = [], onDateChange, handlerChoose, onChangeNoteResign, onPhotoResign }) => {
	...
	const onSeachReason = (text) => {
		const filterList = _.filter(dataMainReason, (e) => { return e.itemName.toLowerCase().match(text.toLowerCase()) })
	...
	const styles = StyleSheet.create({
		...
		subtitle: { color: appcolor.info, fontSize: 13, padding: 5, fontWeight: "700", fontStyle: 'italic' },
		contentView: { width: '65%', borderWidth: 0.5, borderColor: appcolor.grayLight, borderRadius: 8, padding: 8, marginEnd: 8 },
		contentFile: { width: '35%', borderWidth: 0.5, borderColor: appcolor.grayLight, borderRadius: 8, padding: 8 },
		itemContent: { borderWidth: 0.5, borderColor: appcolor.grayLight, borderRadius: 5, padding: 8, marginBottom: 5 },
		itemValue: { fontSize: 13, fontWeight: '500' },
		contentPhoto: { width: deviceWidth / 8, margin: 5, alignSelf: 'center' },
		itemPhoto: { width: deviceWidth / 8, height: deviceWidth / 5.5, padding: 8, borderRadius: 10, borderWidth: 0.5, borderColor: appcolor.primary, justifyContent: 'center' },
		titleHeader: { color: appcolor.info, fontSize: 18, padding: 8, fontWeight: '600', textAlign: 'center' },
		seachView: { padding: 3, margin: 8, backgroundColor: appcolor.placeholderBody },
		groupTitle: { fontSize: 16, fontWeight: '700', color: appcolor.info, padding: 5, marginTop: 8 },
		reasonTitle: { fontSize: 14, fontWeight: '400', color: appcolor.dark, padding: 8 }
	})
	const renderItem = ({ item, index }) => {
		const handlerPhotoPlus = () => {
			alertConfirm('Đơn xin nghỉ', 'Chụp hình bằng máy ảnh hoặc chọn hình ảnh từ thư viện', () => {
		...
		const countItemDelete = () => {
			const value = (item?.isDelete || 0) == 1 ? (countDelete - 1) : (countDelete + 1)
					...
					<View style={styles.itemPhoto}>
						{countDelete == 0 ?
							<TouchableOpacity onPress={handlerPhotoPlus}>
								<Icon name='camera' color={appcolor.primary} size={32} />
							</TouchableOpacity>
							:
							<TouchableOpacity onPress={onDeleteImage} style={{ alignItems: 'center' }}>
								<Icon name='trash' type='font-awesome-5' color={appcolor.primary} size={28} />
								<Text style={{ fontSize: 13, fontWeight: '700', color: appcolor.light, position: 'absolute', bottom: 0, top: 8 }}>{countDelete}</Text>
							</TouchableOpacity>
						}
					</View>
					:
					/* Content Photo */
					<View style={{ ...styles.itemPhoto, borderColor: appcolor.grayLight, padding: 0, overflow: 'hidden' }}>
						<TouchableOpacity onPress={countDelete > 0 ? countItemDelete : handlerShowImage} onLongPress={countItemDelete}>
							{(item?.isDelete || 0) == 0 ?
								<Image source={{ uri: item.photoPath }} style={{ width: '100%', height: '100%' }} />
								:
								<Icon name='check-circle' type='font-awesome-5' color={appcolor.primary} size={24} />
							}
						</TouchableOpacity>
					</View>
				}
	...
	const renderItemSelected = (item, index) => {
		...
		const colorItem = employee.reasonName == item.itemName ? appcolor.primary : appcolor.dark
		const fontWeightItem = employee.reasonName == item.itemName ? '700' : '400'
		return (
			<View key={`stm_${index}`} style={{ width: '100%', borderBottomColor: appcolor.grayLight, borderBottomWidth: 0.5 }} >
				{item.isParent && <Text style={styles.groupTitle}>{item.GroupName}</Text>}
				<TouchableOpacity onPress={onItemChoose}>
					<Text style={{ ...styles.reasonTitle, color: colorItem, fontWeight: fontWeightItem }}>{item.itemName}</Text>
				</TouchableOpacity>
			</View>
		)
	}
		...
		<View style={styles.mainItem}>
			...
			<View style={{ flexDirection: 'row', justifyContent: 'center' }}>
				<View style={styles.contentView}>
					...
					<TouchableOpacity style={styles.itemContent} onPress={handlerTimeWork}>
						<Text style={styles.itemValue}>{employee.fromDate || 'Ngày nghỉ việc'}</Text>
					</TouchableOpacity>
					<Text style={styles.subtitle}>{`Nội dung nghỉ việc`}</Text>
					<TouchableOpacity style={styles.itemContent} onPress={handlerReason}>
						<Text style={styles.itemValue}>{employee.reasonName || 'Nội dung nghỉ việc'}</Text>
					</TouchableOpacity>
					...
					<View style={{ padding: 0 }}>
						<FormGroup
							...
							containerStyle={{ borderWidth: 0.5, borderColor: appcolor.grayLight, margin: 0, padding: 2, paddingTop: 3, color: appcolor.dark, borderRadius: 5 }}
			...
			{/* // Calendar View */}
			<ActionSheet id='calendarview' gestureEnabled>
			...
			{/* // Image Review */}
			<ActionSheet id='imagereview'
				drawUnderStatusBar={Platform.OS == 'ios'}
				containerStyle={{ backgroundColor: '#000' }} >
				<SafeAreaView style={{ width: '100%', height: deviceHeight }}>
					<MultipleShowImage key={'imagereview'} listItem={dataPhoto} closeShowImage={() => SheetManager.hide('imagereview')} indexItem={indexImage} />
				</SafeAreaView>
			</ActionSheet>
		</View>
	)
