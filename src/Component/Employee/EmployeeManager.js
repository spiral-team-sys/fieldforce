...
import { ColorRand, ToastError, removeVietnameseTones } from '../../Core/Helper';
...
export const EmployeeManager = ({ navigation, route }) => {
	...
	const handleSearch = (text) => {
		...
		const filteredData = _.filter(list, (employee) => contains(employee, formattedQuery));
	...
	const styles = StyleSheet.create({
		container: { flex: 1, backgroundColor: appcolor.light, marginHorizontal: 12, marginVertical: 8, borderRadius: 16, borderWidth: 1, borderColor: appcolor.greylight, overflow: 'hidden' },
		touchableContainer: { flexDirection: 'row', width: '100%', height: '100%', justifyContent: 'center' },
		leftContainer: { width: '35%', justifyContent: 'center', alignItems: 'center', padding: 16, borderTopLeftRadius: 16, borderBottomLeftRadius: 16 },
		employeeCode: { color: appcolor.white, fontSize: scaleSize(12), textAlign: 'center', padding: 8, marginBottom: 12, fontWeight: '600', backgroundColor: appcolor.greylight, borderRadius: 8 },
		positionText: { color: appcolor.white, fontSize: scaleSize(11), textAlign: 'center', padding: 6, marginTop: 12, fontWeight: '500', backgroundColor: appcolor.greylight, borderRadius: 6 },
		rightContainer: { flexGrow: 1, width: '65%', padding: 16 },
		fullName: { color: appcolor.dark, textAlign: 'left', padding: 4, fontSize: scaleSize(16), fontWeight: '700', marginBottom: 8 },
		infoText: { color: appcolor.dark, padding: 2, fontSize: scaleSize(11), marginBottom: 2, opacity: 0.8 },
		actionContainer: { position: 'absolute', bottom: 12, right: 12, borderRadius: 20, justifyContent: 'center', alignItems: 'center', backgroundColor: appcolor.light, padding: 4, shadowColor: appcolor.dark, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 20, elevation: 12 },
		actionButton: { borderRadius: 20, backgroundColor: appcolor.primary, width: 36, height: 36, marginBottom: 6, justifyContent: 'center', alignItems: 'center' },
		mainContainer: { backgroundColor: appcolor.light, height: '100%', width: '100%' },
		...
		searchContainer: { margin: 12, backgroundColor: appcolor.light, borderRadius: 12, shadowColor: appcolor.dark, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 12, elevation: 20 },
		flatListStyle: { alignContent: 'center', height: deviceHeight / 1.2 },
		storeSelectContainer: { backgroundColor: appcolor.light, marginTop: 20, width: deviceWidth },
		confirmResignContainer: { backgroundColor: appcolor.light, flex: 1 },
		modalStyle: { backgroundColor: 'red', height: deviceHeight, width: deviceWidth },
		statusIconContainer: { position: 'absolute', right: 12, top: 12, backgroundColor: appcolor.light, borderRadius: 12, padding: 4 },
		headerContainer: { backgroundColor: appcolor.light, borderRadius: 16, margin: 12, padding: 16, shadowColor: appcolor.dark, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
	...
	const renderItem = ({ item, index }) => {
		...
		const onEditItem = () => {
			navigation.navigate('employeedetails', { employee: item, index: index + 1 });
		};
		const onSelectStore = () => {
			...
			<View key={index + '_222kj'}>
				<View style={[styles.container, { borderColor: ColorRand(index) }]}>
					<TouchableOpacity
						...
						activeOpacity={0.8}
					>
						<View style={[styles.leftContainer, { backgroundColor: ColorRand(index) }]}>
							...
							<Avatar
								...
								containerStyle={[styles.avatarContainer, { backgroundColor: ColorRand(index + 1) }]}
							/>
						...
						<View style={styles.rightContainer}>
							...
							{item.listShopName !== null && (
								<Text style={[styles.infoText, { fontWeight: '600', color: appcolor.primary }]}>
									{item.listShopName}
								</Text>
							)}
							...
							<Text style={styles.infoText}>Ngày vào làm: {item.workingDate}</Text>
							<Text style={styles.infoText}>{item.holiday || '0'} ngày phép</Text>
							<Text style={styles.infoText}>Tài khoản: {item.username}</Text>
							<Text style={[styles.infoText, { width: '80%' }]}>Trạng thái: {item.workingStatusName}</Text>
							<Text style={[styles.infoText, { width: '80%' }]}>Trạng thái: {item.userStatus}</Text>
							<Text style={[styles.infoText, { width: '80%' }]}>Hợp đồng: {item.typeOfContract}</Text>
							{item.city !== null && item.city !== undefined && <Text style={[styles.infoText, { width: '80%' }]}>Thành phố: {item.city}</Text>}
						</View>
	...
	return (
		<View style={styles.mainContainer}>
			<View style={styles.contentContainer}>
				...
				/>
				{!isMaps && <LoadingView isLoading={loading} styles={styles.loadingView} title="Đang cập nhật dữ liệu" />}
				<CustomTab
					data={dataTab}
					keyTabName='name'
