...
const WorkShopInfo = () => {
  ...
  const imagePath = useMemo(
    () => checkLinkType(urlImage || shopinfo?.imageUrl),
    [urlImage, shopinfo?.imageUrl],
  );
    ...
    await LocationEnabled(async enabled => {
      if (enabled === true) {
        ...
        NativeCamera.cameraStart(
          { ...photoinfo, latitude: latitudePo, longitude: longitudePo },
          { ...shopinfo, callBackOverView },
        );
      } else {
        CheckLocation(() => {
  ...
  useEffect(() => {
    ...
    const takePhotoListener = DeviceEventEmitter.addListener(
      'TAKE_PHOTO_OVERVIEW',
      async () => {
        await takeOverview();
      },
  ...
  const styles = StyleSheet.create({
    ...
    mapContainer: { width: '100%', height: 240 },
    captureButton: {
      position: 'absolute',
      right: 12,
      bottom: 12,
      ...
      >
        <View style={styles.toggleWrap}>
          <TouchableOpacity
            ...
            style={[
              styles.toggleButton,
              { backgroundColor: !isMap ? appcolor.primary : 'transparent' },
            ]}
          >
            <SpiralIcon
              size={17}
          ...
          <CustomListView
            ...
            renderItem={({ item, index }) => (
              <View
                style={[
                  styles.rowLine,
                  index === shopRows.length - 1 && styles.rowLast,
                ]}
              >
