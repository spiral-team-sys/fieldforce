...
import { ActivityIndicator, DeviceEventEmitter, Keyboard, KeyboardAvoidingView, Platform, ScrollView, StyleSheet, TouchableOpacity, View } from "react-native";
import { HeaderCustom } from "../../../Content/HeaderCustom";
import FormGroup from "../../../Content/FormGroup";
import { SearchData } from "../../../Control/SearchData/SearchData";
import { SHOPAPI } from "../../../API/ShopAPI";
import Geolocation from '@react-native-community/geolocation'
import { CheckLocation, distanceBetween2Points, formatNumber, removeVietnameseTones, UUIDGenerator } from "../../../Core/Helper";
...
const ShopManageScreen = ({ navigation }) => {
    const { appcolor, kpiinfo, searchData } = useSelector(state => state.GAppState)
    ...
    const [search, _setItemSearch] = useState({ text: '', groupByTag: null, isSearch: false })
    const [itemFilter, setItemFilter] = useState({ search: null, newest: false, headcount: null, byTag: null })
    ...
    const [explainForm, setExplainForm] = useState({ shop: null, guid: '', reason: '', photos: [] })
    const [isSubmittingExplain, setSubmittingExplain] = useState(false)
    const [_mutate, setMutate] = useState(false)
    const [mainReportId, SetMainReportId] = useState(null)
    const reportItem = parseReportItem(kpiinfo?.reportItem || kpiinfo?.ReportItem)
    const isAddStoreTool = reportItem?.isAddStoreTool == 1
    const getExplainStorageKey = (shop) => `SHOP_EXPLAIN_${mainReportId || kpiinfo.id}_${shop?.shopId || 0}`
    ...
    const handlerSearchByGroup = async (item, keyValue, isMultiple) => {
        ...
        const _dataShow = _shopByGroup !== null && _shopByGroup.length > 0 ? _shopByGroup : listChooseGroup
    ...
    const saveExplainCache = async (nextForm = explainForm) => {
        ...
        await AsyncStorage.setItem(getExplainStorageKey(nextForm.shop), JSON.stringify({
            cacheDate: TODAY,
            guid: nextForm.guid,
    ...
    const onSubmitExplainStore = async () => {
        ...
        if (reason.length < 5) {
            toastError('Thông báo', 'Vui lòng nhập lý do giải trình tối thiểu 5 ký tự')
            return
        }
        ...
        alertConfirm('Gửi giải trình', 'Bạn có chắc chắn muốn gửi giải trình cửa hàng này không?', async () => {
            ...
            if (result.statusId == 200) {
                toastSuccess('Thông báo', result.messager || 'Gửi giải trình thành công')
                await AsyncStorage.removeItem(getExplainStorageKey(itemShop))
                setExplainForm({ shop: null, guid: '', reason: '', photos: [] })
                await SheetManager.hide('sheetShopExplain')
                await LoadData()
            } else {
                toastError('Thông báo', result.messager || 'Gửi giải trình không thành công')
            }
        })
    }
    ...
    const _searchData = async (filterList, itemFilter = {}) => {
        let mainDataSearch = await _.orderBy(filterList, ['sortLevel'], [itemFilter.newest ? 'desc' : 'asc']);
        //
        const valueSearch = removeVietnameseTones(search.text).toLowerCase()
        mainDataSearch = await _.filter(mainDataSearch, (e) => (
            ...
            removeVietnameseTones(e.dealerName || 'none').toLowerCase().match(valueSearch) ||
            removeVietnameseTones(e.levelName || 'none').toLowerCase().match(valueSearch) ||
            removeVietnameseTones(e.groupView || 'none').toLowerCase().match(valueSearch)
        ))

        if (itemFilter.headcount !== null) {
            mainDataSearch = await _.filter(mainDataSearch, (e) => e.headCountType == itemFilter.headcount)
        }
        mainDataSearch = await _.filter(mainDataSearch, (e) => (
            (searchData.dealerName == null || e.dealerName?.includes(searchData.dealerName)) &&
            (searchData.areaName == null || e.areaName?.includes(searchData.areaName)) &&
            (searchData.provinceName == null || e.province?.includes(searchData.provinceName)) &&
            (searchData.districtName == null || e.district?.includes(searchData.districtName))
        ))
        if (itemFilter.byTag)
            mainDataSearch = await _.filter(mainDataSearch, (e) => (e.groupView == itemFilter.byTag))
    ...
    useEffect(() => {
        ...
        const reloadListShop = DeviceEventEmitter.addListener('reload_shop_manager', LoadData);
        LoadData()
    ...
    const styles = StyleSheet.create({
        ...
        loadingView: { position: 'absolute', zIndex: 10, top: 0, end: 0, bottom: 0, start: 0, alignItems: 'center', justifyContent: 'center' },
        itemContainer: { backgroundColor: appcolor.light, borderWidth: 1, borderColor: appcolor.grayLight, borderRadius: 8, marginHorizontal: 8, marginVertical: 4, overflow: 'hidden', shadowColor: appcolor.dark, shadowRadius: 8, shadowOffset: { width: 1, height: 3 }, shadowOpacity: 0.5, elevation: 3 },
        styleOverview: { height: 280 },
        infoView: { backgroundColor: appcolor.light, padding: 8, paddingBottom: 0, position: 'absolute', start: 0, end: 0, bottom: 0 },
        titleName: { fontSize: 14, fontWeight: fontWeightBold, color: appcolor.dark },
        titleSubName: { fontSize: 12, fontWeight: '500', color: appcolor.greylight },
        titleTarget: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.greylight },
        actionView: { width: '100%' },
        actionSheetView: { width: '100%', minHeight: deviceHeight / 5 },
        functionSheetView: { width: '100%', padding: 12, paddingBottom: 24, backgroundColor: appcolor.light },
        functionTitle: { color: appcolor.dark, fontSize: 16, fontWeight: fontWeightBold, paddingHorizontal: 4, paddingBottom: 8 },
        functionGroup: { marginTop: 8 },
        functionGroupTitle: { color: appcolor.greylight, fontSize: 12, fontWeight: fontWeightBold, paddingHorizontal: 4, paddingVertical: 4, textTransform: 'uppercase' },
        functionItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 10, borderRadius: 8, backgroundColor: appcolor.surface, marginTop: 8 },
        functionIcon: { width: 32, alignItems: 'center' },
        functionText: { flex: 1, color: appcolor.dark, fontSize: 14, fontWeight: '600', marginStart: 8 },
        explainSheetView: { width: '100%', maxHeight: deviceHeight * 0.88, backgroundColor: appcolor.light, borderTopStartRadius: 16, borderTopEndRadius: 16 },
        explainContent: { padding: 14, paddingBottom: 28, gap: 12 },
        explainTitle: { color: appcolor.dark, fontSize: 18, fontWeight: fontWeightBold, textAlign: 'center' },
        explainSubTitle: { color: appcolor.greylight, fontSize: 12, fontWeight: '500', textAlign: 'center', lineHeight: 17 },
        explainShopBox: { backgroundColor: appcolor.surface, borderRadius: 8, padding: 10, gap: 4 },
        explainShopName: { color: appcolor.dark, fontSize: 14, fontWeight: fontWeightBold },
        explainShopAddress: { color: appcolor.greylight, fontSize: 12, fontWeight: '500' },
        explainLabel: { color: appcolor.dark, fontSize: 13, fontWeight: fontWeightBold, marginBottom: 6 },
        explainInput: { backgroundColor: appcolor.surface, borderRadius: 8, borderWidth: 1, borderColor: appcolor.grayLight, minHeight: 96, paddingVertical: 4 },
        explainInputText: { color: appcolor.dark, fontSize: 13, minHeight: 84, textAlignVertical: 'top' },
        explainButton: { minHeight: 46, borderRadius: 8, backgroundColor: appcolor.primary, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 },
        explainButtonDisabled: { opacity: 0.65 },
        explainButtonText: { color: appcolor.light, fontSize: 14, fontWeight: fontWeightBold },
        explainCancelButton: { minHeight: 42, borderRadius: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: appcolor.surface },
        explainCancelText: { color: appcolor.dark, fontSize: 13, fontWeight: '600' },
        ...
        lineView: { width: '100%', height: 1, backgroundColor: appcolor.surface, marginVertical: 6 }
    })
        ...
        return (
            <KeyboardAvoidingView
                ...
                <ScrollView
                    ...
                    <Text style={styles.explainSubTitle}>Nhập lý do, chụp hình xác nhận và gửi realtime lên hệ thống.</Text>
                    <View style={styles.explainShopBox}>
                        <Text style={styles.explainShopName}>{selectedShop.shopName || ''}</Text>
                        <Text style={styles.explainShopAddress}>{selectedShop.address || ''}</Text>
                    </View>
                    <View>
                        <Text style={styles.explainLabel}>Lý do giải trình <Text style={{ color: appcolor.red }}>*</Text></Text>
                        <FormGroup
                    ...
                    <TouchableOpacity
                        ...
                        style={[styles.explainButton, isSubmittingExplain && styles.explainButtonDisabled]}
                        onPress={onSubmitExplainStore}>
                        {isSubmittingExplain && <ActivityIndicator size='small' color={appcolor.light} />}
                        <Text style={styles.explainButtonText}>{isSubmittingExplain ? 'Đang gửi...' : 'Gửi lên hệ thống'}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        disabled={isSubmittingExplain}
    ...
    const renderItem = ({ item }) => {
        ...
        const imageOverview = isOverviewImage ? { uri: `${URLDEFAULT}${item.imageUrl}` } : require('../../../Themes/Images/store.png')
        const distance = item.latitude == 0 ? 'Không xác định' : `${formatNumber(distanceBetween2Points(item.latitude, item.longitude, myLocation.latitude, myLocation.longitude).toFixed(0), ',')} km`
        //
        const dataTarget = JSON.parse(item.dataTarget || '[]')
        const strTarget = dataTarget.length > 0 ? _.join(_.map(dataTarget, i => `${i.type} - ${formatNumber(i.amount, ',')}`), ' | ') : null
        return (
            <View style={styles.itemContainer}>
                ...
                <View style={styles.infoView}>
                    ...
                    {/* TypeInfo */}
                    {item.levelName && <Text style={styles.titleSubName}>{item.levelName}</Text>}
                    {item.shopFormat && <Text style={styles.titleSubName}>{item.shopFormat}</Text>}
                    {item.dealerName && <Text style={styles.titleSubName}>{`Loại hình: ${item.dealerName}`}</Text>}
                    {item.supType && <Text style={styles.titleSubName}>{item.supType}</Text>}
                    {item.headCountType && <Text style={styles.titleSubName}>{item.headCountType}</Text>}
                    {/* Target */}
                    {strTarget && <Text style={styles.titleTarget}>{`Target: ${strTarget}`}</Text>}
                    {/* // */}
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <ActionSheet id='sheetShopManageFunctions'>
                <View style={styles.functionSheetView}>
                    ...
                    <View style={styles.functionGroup}>
                        ...
                        <TouchableOpacity style={styles.functionItem} onPress={onOpenMapFunction}>
                            <View style={styles.functionIcon}>
                                <Icon name='map' type='font-awesome-5' size={18} color={appcolor.primary} />
                            </View>
                            <Text style={styles.functionText}>Bản đồ cửa hàng</Text>
                            <Icon name='chevron-right' type='font-awesome-5' size={14} color={appcolor.greylight} />
                        </TouchableOpacity>
                    </View>
                    <View style={styles.functionGroup}>
                        <Text style={styles.functionGroupTitle}>Cửa hàng</Text>
                        {isAddStoreTool &&
                            <TouchableOpacity style={styles.functionItem} onPress={onCreateStoreRequest}>
                                <View style={styles.functionIcon}>
                                    <Icon name='plus-circle' type='font-awesome-5' size={18} color={appcolor.primary} />
                                </View>
                        ...
                        {isAddStoreTool &&
                            <TouchableOpacity style={styles.functionItem} onPress={onOpenStoreRequestHistory}>
                                <View style={styles.functionIcon}>
                                    <Icon name='history' type='font-awesome-5' size={18} color={appcolor.primary} />
                                </View>
