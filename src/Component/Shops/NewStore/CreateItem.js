...
import { AppState, DeviceEventEmitter, Platform, ScrollView, StyleSheet, TouchableOpacity, View } from "react-native";
import { useSelector } from "react-redux";
import { HeaderCustom } from "../../../Content/HeaderCustom";
import { CreateItemStore, GetConfigNewStore, getMasterNewStore } from "../../../Controller/ShopController";
...
import { alertConfirm, deviceHeight, deviceWidth, isValid } from "../../../Core/Utility";
import { CheckLocation, GetUrl, LocationEnabled, ToastError, ToastSuccess, UUIDGenerator, isPhone } from "../../../Core/Helper";
...
const CreateItem = ({ navigation }) => {
	...
	const [storeItem, setStoreItem] = useState({
		shopId: 0,
		shopCode: `${JSON.parse(kpiinfo.reportItem).codeCreate || 'SM.'}${Math.floor((Math.random() * 999999) + 100)}`,
		codeDealer: null,
		shopName: null,
		address: null,
		warehouseName: null,
	...
	const getLocationLast = async () => {
		await Geolocation.getCurrentPosition(async info => {
			...
			await setStoreItem({ ...storeItem, latitude: info.coords.latitude, longitude: info.coords.longitude, idStore: guidValue });
		}, (error) => {
			alertConfirm('Cài đặt vị trí', "Vui lòng kiểm tra cài đặt vị trí của thiết bị và thử lại sau", async () => {
				if (Platform.OS == 'ios') {
	...
	const handlerCreate = async () => {
		await checkInput(async (result, message) => {
			if (!result) {
				...
				alertConfirm('Thông báo', 'Bạn có muốn tạo cửa hàng này không ?', async () => {
					...
					await CreateItemStore({ ...storeItem, typeAction: 'INSERT' }, contentMassage, async (data) => {
						if (data.status == 200) {
	...
	const handlerSelectItem = (item, type) => {
		switch (type) {
			case 'warehouse':
				...
				break
			case 'dealerName':
				setStoreItem({ ...storeItem, dealerId: item.dealerId, dealerName: item.itemName })
				break
			case 'shopAddress':
				setStoreItem({
					...storeItem,
			...
			case 'potentialOutput':
				...
				break
			case 'level':
				setStoreItem({ ...storeItem, levelCode: item.itemCode, level: item.itemName })
				break
			case 'shopFormat':
				setStoreItem({ ...storeItem, shopFormatId: item.itemId, shopFormat: item.itemName })
				break
		}
	}
				...
				await LocationEnabled(async (enabled) => {
					if (enabled === true) {
						NativeCamera.cameraStart(photoinfo, {
							...storeItem, callBackOverView: handlerSetPhoto
						});
					}
				});
			});
	...
	const styles = StyleSheet.create({
		...
		mainItem: { flexGrow: 1, flexDirection: 'row', padding: 8, marginBottom: 0 },
		titleHeader: { width: '100%', fontSize: 13, fontWeight: '700', color: appcolor.blacklight, marginStart: 8 },
		overflowView: { width: deviceWidth, height: deviceHeight, position: 'absolute', zIndex: 1000, backgroundColor: appcolor.dark, opacity: 0.8, justifyContent: 'center' }
	})
	useEffect(() => {
		configData()
	}, [])
		...
		<View style={styles.mainContainer}>
			...
			<ScrollView
				...
				<View style={styles.mainItem}>
					<Icon name={'camera'} type="font-awesome-5" size={15} color={appcolor.blacklight} />
					<Text style={styles.titleHeader}>{`Hình tổng quan cửa hàng`}
						<Text style={{ fontSize: 14, color: appcolor.red }}>*</Text>
					</Text>
				</View>
				<View style={{ margin: 8, backgroundColor: appcolor.surface, borderRadius: 10 }}>
					<TouchableOpacity onPress={takeOverview} style={{ position: 'absolute', end: 0, top: 0, zIndex: 10, padding: 16 }}>
						<Icon name="camera" type="font-awesome-5" color={appcolor.blacklight} size={28} />
					</TouchableOpacity>
					<ViewImage />
					{/* <TouchableOpacity onPress={() => storeItem.imageOverview !== null ? SheetManager.show('photoview') : takeOverview()}>
						<Image source={{ uri: GetUrl(storeItem.imageOverview) }} style={{ width: '100%', minHeight: 230, borderRadius: 12 }} />
...
const ItemInput = forwardRef((props, _ref) => {
	const { titleName, iconName, isRequire, onActionRight, typeFilter, itemValue, placeholder, onChangeText, keyboardType = 'default', editable = true, isViewInput = true, mobileLength = false } = props
		...
		<View style={styles.mainItem}>
			...
			<View style={{ width: '100%', flexDirection: 'row', alignItems: 'center' }}>
				...
				{onActionRight !== undefined &&
					<TouchableOpacity
						style={{ width: '10%', padding: 10, marginStart: 10, backgroundColor: appcolor.info, borderRadius: 50 }}
						onPress={onPress}>
...
const SliderView = ({ titleName, iconName, isRequire, typeFilter, itemValue, placeholder, onChangeText, maxLength }) => {
	const { appcolor } = useSelector(state => state.GAppState)
	const styles = StyleSheet.create({
		mainItem: { flexGrow: 1, padding: 8, marginBottom: 1 },
		titleHeader: { width: '100%', fontSize: 13, fontWeight: '600', color: appcolor.blacklight, marginStart: 8 },
		placeholderHeader: { width: '100%', fontSize: 13, fontWeight: '300', color: appcolor.placeholderText, marginStart: 8, marginBottom: 8, fontStyle: 'italic' },
		inputView: { width: '30%', backgroundColor: appcolor.surface, borderRadius: 5, marginBottom: 0, marginEnd: 16 },
		viewItem: { fontSize: 16, fontWeight: '600', color: appcolor.dark, margin: 10 }
	})
	const handlerChangeValue = (text) => {
		itemValue = text
		onChangeText !== undefined && onChangeText(text, typeFilter)
...
const MultipleSelect = ({ titleName, iconName, typeItem, isRequire = false, dataView, onItemChange, containerStyle }) => {
	const { appcolor } = useSelector(state => state.GAppState)
	...
	const renderItem = (item, index) => {
		...
		const styleView = item.isChoose == 1 ? { ...styles.itemContent, borderWidth: 1, borderColor: appcolor.primary } : styles.itemContent
		const styleTitle = item.isChoose == 1 ? { ...styles.itemName, fontWeight: '700', color: appcolor.primary } : styles.itemName
		return (
			<TouchableOpacity key={`${typeItem}_${index}`} style={styleView} onPress={onPress}>
	...
	const styles = StyleSheet.create({
		...
		itemMain: { borderRadius: 5, borderWidth: 0.3, borderColor: appcolor.placeholderText, marginEnd: 8, marginTop: 5, justifyContent: 'center', alignSelf: 'center' },
		itemMainChoose: { borderRadius: 5, backgroundColor: appcolor.primary, marginEnd: 8, marginTop: 5, justifyContent: 'center', alignSelf: 'center' }
	})
	return (
		<View style={[styles.mainContainer, containerStyle]}>
			...
			<View style={{ flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', minHeight: 50 }}>
				{dataView.map((item, i) => { return renderItem(item, i) })}
			</View>
		</View>
	)
}
