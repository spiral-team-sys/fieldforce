...
const UpdateStore = ({ navigation, route }) => {
  ...
  const handlerUpdate = async () => {
    await checkInput(async (result, message) => {
      if (!result) {
        ...
        alertConfirm(
          'Thông báo',
          'Bạn có muốn cập nhật cửa hàng này không ?',
          async () => {
            await setLoading(true);
            await CreateItemStore(
              { ...storeItem, typeAction: 'UPDATE' },
              '',
              async data => {
                console.log(
                  data,
                  'lưu thông tin thành cônglưu thông tin thành công',
                );
                if (data.status == 200) {
                  const listStore = await GetShopLists();
                  const currentStore = _.filter(
                    listStore,
                    it => it.shopId == shopinfo.shopId,
                  );
  ...
  const handlerItemChangeText = (text, typeItem, itemConfig = {}) => {
    const inputText =
      isTitleCaseTextField(itemConfig) && text !== null && text !== undefined
    ...
    switch (typeItem) {
      ...
      case 'phone':
        if (inputText !== null && inputText !== '') {
          ...
          (!storeItem.phone || textValue == '' || textValue?.length < 12) &&
            setStoreItem({ ...storeItem, phone: textValue });
        } else {
    ...
    case 'itemInput':
      return (
        <ItemInput
          ...
          editable={
            item.ref_Code == 'shopName'
              ? (storeItem?.shopName?.length || 0) == 0
              : true
          }
          ...
          onChangeText={(text, typeItem) =>
            handlerItemChangeText(text, typeItem, item)
          }
        />
      );
    case 'numberInput':
