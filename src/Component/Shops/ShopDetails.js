...
import { View, Text, ScrollView, TouchableOpacity, Image, DeviceEventEmitter, StyleSheet } from "react-native";
...
const ShopDetails = ({ navigation }) => {
        ...
        if (lstOverview.length > 0) {
            ...
            if (url !== '') {
                ...
                if (isNullPhotoShop === true) {
                    await UpdateLocationStoreList(lstOverview[0].latitude, lstOverview[0].longitude, shopinfo.shopId);
                    dispatch(SetShopInfo({ ...shopinfo, latitude: lstOverview[0].latitude, longitude: lstOverview[0].longitude }));
                }
            }
        } else {
        ...
        await LocationEnabled(async (enabled) => {
            if (enabled === true) {
                if ((shopinfo.latitude == 0 || shopinfo.longitude == 0) && (!photoinfo.latitude || !photoinfo.longitude)) {
                    ...
                    NativeCamera.cameraStart(photoinfo, { ...shopinfo, callBackOverView });
                }
            } else {
                CheckLocation(() => { setIsPermissLocation(true) });
                await delay(1000);
    ...
    useEffect(() => {
        ...
        const goShopMapListener = DeviceEventEmitter.addListener('GO_SHOP_MAP', async () => {
            setIsMap(true);
        });
    ...
    const styles = StyleSheet.create({
        container: { height: '100%', width: '100%', backgroundColor: appcolor.light },
        headerContainer: { padding: 5, flexDirection: 'row', justifyContent: 'space-between' },
        scrollView: { flex: 1 },
        buttonContainer: { flexDirection: 'row', backgroundColor: appcolor.light },
        mapButton: { borderRadius: 50, padding: 5, justifyContent: 'center', alignItems: 'center', flexDirection: 'row', height: 35, marginEnd: 5 },
        mapButtonText: { alignSelf: "center", fontWeight: '500', paddingLeft: 10, paddingRight: 10 },
        imageButton: { justifyContent: 'center', alignItems: 'center', flexDirection: 'row', borderRadius: 50 },
        imageButtonText: { alignSelf: "center", fontWeight: '700', paddingLeft: 10, paddingRight: 10 },
        updateButton: { justifyContent: 'center', alignItems: 'center', backgroundColor: appcolor.primary, flexDirection: 'row', borderRadius: 50, marginStart: 5 },
        updateButtonText: { alignSelf: "center", fontWeight: '500', color: appcolor.white, paddingLeft: 10, paddingRight: 10 },
        imageContainer: { justifyContent: 'flex-end', margin: 7 },
        imageWrapper: { width: '100%', minHeight: 230, backgroundColor: appcolor.surface, borderRadius: 12 },
        image: { width: '100%', minHeight: 230, borderRadius: 12 },
        cameraButton: { position: 'absolute', top: 10, right: 10, justifyContent: 'center', alignItems: 'center', backgroundColor: appcolor.blacklight, borderRadius: 5, padding: 5, opacity: 0.8 },
        cameraButtonText: { alignSelf: "center", fontWeight: '700', color: appcolor.light },
        mapContainer: { width: '100%', minHeight: 230 },
        formContainer: { padding: 5, paddingBottom: 150, flexDirection: 'column', justifyContent: 'flex-start' },
        actionSheetContainer: { width: '100%', height: '100%', backgroundColor: appcolor.light },
