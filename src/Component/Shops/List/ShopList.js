...
const ShopList = ({ navigation, data }) => {
  ...
  const onSearchData = text => {
    ...
    const listUpdate = (data || []).filter(
      item =>
        removeVietnameseTones(item?.shopName || '')
          .toLowerCase()
          .match(valueSearch) ||
        removeVietnameseTones(item?.shopCode || '')
          .toLowerCase()
          .match(valueSearch) ||
        removeVietnameseTones(item?.address || '')
          .toLowerCase()
          .match(valueSearch),
    );
  ...
  const renderItem = ({ item, index }) => {
    ...
    const distanceStore =
      item.latitude > 0 && location?.latitude > 0
        ? distanceBetween2Points(
            location.latitude,
            location.longitude,
            item.latitude,
    ...
    return (
      <TouchableOpacity key={index} style={styles.itemMain} onPress={onPress}>
          ...
          <Image
            source={
              !item.imageUrl
                ...
                : { uri: checkLinkType(item.imageUrl) }
            }
        ...
        <View style={styles.infoView}>
          ...
          >{`ShopCode: ${item.shopCode}`}</Text>
          <Text style={styles.subTitleName}>{`Địa chỉ: ${item.address}`}</Text>
          <Text style={styles.subTitleDistance}>{`Khoảng cách ~ ${formatNumber(
            parseInt(distanceStore),
            ',',
          )} km`}</Text>
