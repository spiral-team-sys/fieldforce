...
import { View, TouchableOpacity, Text, Image, RefreshControl, StyleSheet, Platform } from "react-native";
...
import { ToastSuccess, UUIDGenerator, distanceBetween2Points, openSettingPermission, removeVietnameseTones, requestPerrmission } from '../../Core/Helper';
...
export const ShopManager = ({ navigation, route }) => {
    ...
    const [itemFilter, setItemFilter] = useState({ search: null, newest: false, headcount: null })
    ...
    const permissionLocation = async (status) => {
        switch (status) {
            case RESULTS.BLOCKED:
                await openSettingPermission.settingLocation(`Vui lòng truy cập vào ứng dụng trong phần "Cài đặt" để mở quyền sử dụng vị trí`)
                break;
            default:
                await requestPerrmission({ android: PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION, ios: PERMISSIONS.IOS.LOCATION_WHEN_IN_USE }, async (status) => {
                    if (status) {
    ...
    const requestMylocation = async () => {
        await check(Platform.OS === 'ios' ? PERMISSIONS.IOS.LOCATION_WHEN_IN_USE : PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION).then(async (result) => {
    ...
    const _searchData = (filterList, itemFilter) => {
        let mainDataSearch = _.orderBy(filterList, ['sortLevel'], [itemFilter.newest ? 'desc' : 'asc']);
        //
        const valueSearch = removeVietnameseTones(search.text).toLowerCase()
        mainDataSearch = _.filter(mainDataSearch, (e) => (
        ...
        if (itemFilter.headcount !== null) {
            mainDataSearch = _.filter(mainDataSearch, (e) => e.headCountType == itemFilter.headcount)
        }
        mainDataSearch = _.filter(mainDataSearch, (e) => (
    ...
    const styles = StyleSheet.create({
        titleView: { color: appcolor.dark, padding: 4, fontSize: scaleSize(15), fontWeight: '500' },
        contentView: { color: appcolor.blacklight, padding: 2, fontSize: scaleSize(12) },
        searchContainer: { width: '82%', marginTop: 8, padding: 3, paddingHorizontal: 8, borderRadius: 20, backgroundColor: appcolor.light, borderWidth: 0.5, borderColor: appcolor.primary },
        searchContainerInput: { width: '82%', marginTop: 8, padding: 3, paddingHorizontal: 8, borderRadius: 20, backgroundColor: appcolor.primary, borderWidth: 0.5 },
        searchInputStyle: { fontSize: 13, color: appcolor.light, fontWeight: '500' },
        searchStyle: { fontSize: 13, color: appcolor.primary },
        viewButtonAction: {
            width: 42, height: 42,
            justifyContent: 'center', alignItems: 'center', alignSelf: 'center', alignContent: 'center', borderRadius: 50, marginStart: 8, backgroundColor: appcolor.light,
            shadowColor: appcolor.dark, shadowOffset: { width: 3, height: 3 }, elevation: 3, shadowOpacity: 0.3,
            borderWidth: 0.5, borderColor: appcolor.grayLight
        },
        viewButtonFilter: { backgroundColor: appcolor.surface, borderWidth: 0, borderColor: appcolor.primary, width: '100%', flexDirection: 'row', alignItems: 'center', padding: 5, marginTop: 8, borderRadius: 5 },
        textButtonFilter: { width: '100%', fontSize: 14, fontWeight: '400', color: appcolor.dark, padding: 8 },
        viewFilterData: { width: '100%', height: deviceHeight / 2 },
        titleAction: { fontSize: 14, fontWeight: fontWeightBold, color: appcolor.dark, padding: 8 },
        headAction: { width: '100%', flexDirection: 'row', alignItems: 'center', paddingStart: 14 },
        loadingView: { width: '100%', height: '100%', justifyContent: 'center', position: 'absolute', zIndex: 3 },
        actionSheetView: { width: '100%', minHeight: deviceHeight / 5 }
    })
    const renderItem = ({ item, index }) => {
        const _distant = item.latitude > 0 && myLocation?.latitude > 0 ?
            distanceBetween2Points(myLocation.latitude, myLocation.longitude, item.latitude, item.longitude).toLocaleString("vi-VN", "#,##0.##") + " km" : 'Chưa xác định'
        return (
            <View key={`qwe_${index}`} style={{ backgroundColor: appcolor.light, margin: 5, marginTop: 0 }}>
                <View style={{ borderRadius: 8, borderWidth: 0.5, padding: 3, borderColor: appcolor.greylight }}>
                    <View>
                        <Image
                            source={item.imageUrl ? { uri: (URLDEFAULT + item.imageUrl) } : require('../../Themes/Images/noimage.png')}
                            containerStyle={{ width: '90%' }}
                            style={{ borderRadius: 8, height: 250, width: '100%', resizeMode: 'cover' }} />
                        <View style={{ position: 'absolute', width: '100%', bottom: 0, backgroundColor: appcolor.surface, opacity: 0.8 }}>
                            <Text style={styles.titleView}>Cửa hàng {item.shopName}</Text>
                            <Text style={styles.contentView}>Địa chỉ {item.address}</Text>
                            <Text style={styles.contentView}>Khoảng cách bạn đến cửa hàng {_distant}</Text>
                            {item.levelName !== null && item.levelName !== undefined && item.levelName !== '' && <Text style={styles.contentView}>{item.levelName}</Text>}
                            {item.shopFormat !== null && item.shopFormat !== undefined && item.shopFormat !== '' && <Text style={styles.contentView}>{item.shopFormat}</Text>}
                            {item.dealerName && <Text style={styles.contentView}>{`Loại hình: ${item.dealerName}`}</Text>}
                            {item.supType && <Text style={styles.contentView}>{item.supType}</Text>}
                            {item.headCountType && <Text style={styles.contentView}>{item.headCountType}</Text>}
                        </View>
                    </View>
                </View>
                <View style={{ width: '100%', flexDirection: 'row-reverse', marginTop: 5 }}>
                    <ButtonAction key={`qwe_${index}_${item.shopId}`} itemStore={item} reportItem={reportItem} handlerPressButton={handlerPressButton} TYPE={TYPE} />
                </View>
            </View>
        )
    }
    if (loading) return <LoadingView isLoading={loading} title='Đang cập nhật dữ liệu' styles={styles.loadingView} />
    if (!isMap) return (
        ...
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            ...
            <View style={{ width: '100%', height: '88%' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    />
                    <TouchableOpacity style={styles.viewButtonAction} onPress={() => { SheetManager.show('sheetSortStore') }} >
                        <Icon type='ionicon' name='funnel' size={20} color={appcolor.primary} />
                    </TouchableOpacity>
                </View>
                <CustomListView
                    data={shops}
