...
export const ButtonAction = ({ itemStore, reportItem, handlerPressButton, TYPE }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    const [dataConfig, setDataConfig] = useState(JSON.parse(itemStore?.dataConfig || '[]'))
    ...
    return (
        <View style={{ width: '100%', flexDirection: 'row-reverse', flexWrap: 'wrap' }}>
            {
                dataConfig?.map((item, index) => {
                    return (
                        <RenderItemButton key={`${itemStore.shopId}_${index}_${item.id}`} item={item} />
                    )
                })
            }
        </View>
