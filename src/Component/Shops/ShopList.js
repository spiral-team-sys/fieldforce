...
import { View, Text, Platform, TouchableOpacity, RefreshControl, Linking, DeviceEventEmitter } from "react-native";
...
import { Message, MessageInfo, MessageSetting, OnTime, ToastSuccess, ToastError, distanceBetween2Points, UUIDGenerator } from "../../Core/Helper";
...
class ShopList extends PureComponent {
    ...
    LoadData = async (search) => {
        ...
        await this.setState({ startInfo: _start.length > 0 ? _start[0] : {}, stopInfo: _stop.length > 0 ? _stop[0] : {}, });
        await this.setState({ shops: lst, shopsF: search ? this.state.shopsF : lst });
    }
                ...
                switch (result) {
                    ...
                    case RESULTS.BLOCKED:
                        MessageSetting('Chú ý', Platform.OS === 'ios' ? 'Vui lòng cho phép chức năng vị trí trong phần cài đặt của thiết bị, Privacy -> Location Services-> Location Services(ON)' : 'Vui lòng cho phép chức năng vị trí trong phần cài đặt của thiết bị', () => {
                            Platform.OS === 'ios' ? Linking.openURL("App-Prefs:root=Privacy&path=LOCATION") : openSettings().catch(() => ToastError('cannot open settings'));;
                        })
                        break;
                }
            })
    ...
    async getLocationUserFast() {
        await Geolocation.getCurrentPosition(async info => {
            await this.setState({ myLocation: info.coords, isTimeoutLocation: false });
        ...
        await OnTime(async () => {
            ...
            if (shopCode == 'Z' && (stopConfig?.isNoteStop || 0) == 1) {
                ...
                if (checkAttendant !== true) {
                    ...
                    Message('Thông báo', shopCode === '1' ? 'Bạn có muốn bắt đầu ngày làm việc không?' : 'Bạn có muốn kết thúc ngày làm việc không?', () => {
    ...
    renderItem = ({ item, index }) => {
        ...
        const sourceImage = item.imageUrl !== null && ((item.imageUrl.indexOf('file://') === - 1 && item.imageUrl.indexOf('https://') === -1) ? URLDEFAULT + item.imageUrl : item.imageUrl)
        return (
            item.shopCode === '1' ? this.StartUI()
                :
                <View key={"shoddp-" + index.toString()}
                    style={{
                        backgroundColor: appcolor.light, marginLeft: 7,
                        marginRight: 7, marginBottom: 7, borderRadius: 12,
                        padding: 7,
                    }}>
                    <TouchableOpacity onPress={() => this.onItemPress(item)}>
                        <View style={{ flexDirection: 'row' }}>
                            ...
                            <View style={{ width: '70%' }}>
                                <Text style={{ paddingLeft: 7, fontSize: 14, color: appcolor.dark }}>{item.title}</Text>
    ...
    StartUI = () => {
        ...
        myLocation.latitude !== undefined && ATTENDANT_API.GetAddressbyGeo(`${myLocation.latitude},${myLocation.longitude}`).then(
            address => {
        ...
        const appcolor = this.props.appcolor
        return (
            <View style={{ flex: 1, backgroundColor: appcolor.surface, justifyContent: 'space-between' }}>
                <TouchableOpacity style={{ flex: 1 }} onPress={() => this.startAction('1')}>
                    <View style={{ flex: 2 / 10, justifyContent: 'center', alignItems: 'center', padding: 7 }}>
                        <Text style={{
                            fontSize: scaleSize(18), textAlign: 'center',
                            color: appcolor.dark
                        }}>Nhấn "Start" để bắt đầu thực hiện công việc</Text>
                    </View>
                    <View style={{ flex: 4 / 10, justifyContent: 'center' }}>
                        <LottieView autoPlay style={{ height: '100%' }} source={require('../../Themes/lotties/call.json')} />
                        <Text style={{
                            color: appcolor.white, position: 'absolute', fontWeight: 'bold',
                            textAlign: 'center', width: '100%', fontSize: scaleSize(24)
                        }} >Start</Text>
                    </View>
                    <View style={{ flex: 4 / 10 }}>
                        <Text style={{ fontSize: scaleSize(16), fontWeight: 'bold', color: appcolor.dark, textAlign: 'center' }}>Hôm nay {moment().format('LLLL')}</Text>
                        <View style={{ padding: 6, paddingHorizontal: 20 }}>
                            <Text style={{ fontSize: scaleSize(18), fontWeight: 'bold', color: appcolor.dark }}>Thông tin</Text>
                            <Text style={{ color: appcolor.dark }}>Ví trí hiện tại: {this.state.addressCurrent}</Text>
                            <Text style={{ color: appcolor.dark }}>Tọa độ lat:{myLocation.latitude}, long: {myLocation.longitude}</Text>
                            <Text></Text>
                        </View>
                    </View>
                </TouchableOpacity>
        ...
        this.state.isDownload !== true &&
            await Geolocation.getCurrentPosition(async info => {
                ...
                if (this.state.stopInfo.timeIn !== undefined) {
                    ...
                    myLocation.latitude !== undefined && await ATTENDANT_API.GetAddressbyGeo(`${myLocation.latitude},${myLocation.longitude}`).then(
                        address => {
                            this.setState({ addressCurrent: address })
                        }
                    )
    ...
    render() {
        ...
        return (
            startInfo.timeIn !== undefined && startInfo.timeIn === 0 ?
                this.StartUI()
                :
                <View style={{ height: '100%', width: '100%', backgroundColor: appcolor.surface }}>
                    {this.props.header === 'none' ? null : <HeaderCustom
                    ...
                    {!this.state.mapmode ?
                        <View style={{
                            ...
                            height: this.props.header === 'none'
                                && this.state.shops?.length < 8 ? '100%' : this.props.header === 'none' ? '96%' : '86%'
                        }}>
                            <View style={{ display: this.state.shopsF?.length > 7 ? 'flex' : "none" }}>
                                <FormGroup
                                    containerStyle={{ marginLeft: 7, marginRight: 7, backgroundColor: appcolor.homebackground }}
                                    appcolor={appcolor}
                                    placeholder={"Tìm kiếm cửa hàng"} editable
                                    handleChangeForm={(text) => this.updateSearch(text)} multiline iconName='search' />
                                <View style={{ borderWidth: 1, borderColor: appcolor.surface, width: '100%', height: 0.2, marginBottom: 7 }} />
                            </View>
                            <View style={{ flex: 1, backgroundColor: appcolor.surface, marginTop: 10 }}>
                                {
                                    (userinfo.employeeId !== undefined || this.state.shops?.length > 0) &&
                                    <FlashList
                                        keyExtractor={(_, index) => index.toString()}
                                        showsVerticalScrollIndicator={false}
                                        data={userinfo.employeeId === undefined ? [] : this.state.shops}
                                        refreshControl={
                                            <RefreshControl
                                        ...
                                        ListHeaderComponent={(
                                            (userinfo.employeeId === undefined || this.state.shops?.length == 0) ?
                                                <View style={{ padding: 10, height: 100, justifyContent: 'center', alignItems: 'center' }}>
                                                    <Text style={{ fontWeight: '500', fontSize: 14, color: appcolor.dark }}>Chưa có danh sách cửa hàng</Text>
                                                </View>
                                                :
                                                <View></View>
                                        )
                        ...
                        <MapApp navigation={this.props.navigation} slist={this.state?.shops || []} />
                    }
                    {
                        (!this.state.mapmode && (startInfo.timeIn !== undefined || stopInfo.timeIn !== undefined)) &&
                        <TouchableOpacity onPress={this.onShowStop}
                            style={{ opacity: 1, position: 'absolute', zIndex: 202, left: 0, bottom: 0, }}>
                            <View style={{
                                justifyContent: 'center',
                                height: 110, width: 110
                            }}>
                                <LottieView style={{ height: '100%' }} autoPlay source={require('../../Themes/lotties/stopwork.json')} />
                                <Icon name="stop" color={appcolor.white} size={36} />
                            </View>
                        </TouchableOpacity>
                    }
                        ...
                        <View style={{ height: '96%' }}>
                            <View style={{ padding: 12, backgroundColor: appcolor.light }}>
                                ...
                                <View style={{ padding: 12 }}>
                                    <Text style={{ fontSize: scaleSize(18), fontWeight: 'bold', color: appcolor.dark }}>Thông tin điểm bắt đầu</Text>
                                    <Text style={{ color: appcolor.dark }}>Thời gian {moment(startInfo?.timeIn, "YYYYMMDDHHmmss").format('HH:mm dddd DD MMMM')} </Text>
                                    <Text style={{ color: appcolor.dark }}>Ví trí: {startInfo?.address || 'Không xác định'}</Text>
                                </View>
                            </View>
                            {
                                stopInfo.timeIn === 0 ?
                                    <Fragment>
                                        <View style={{ width: '100%', padding: 7, display: stopInfo.timeIn === undefined ? 'none' : 'flex' }}>
                                            <Text style={{
                                                fontSize: scaleSize(18), textAlign: 'center', color: appcolor.dark
                                            }}>Nhấn "End" để kết thúc ngày làm việc</Text>
                                        </View>
                                        <View style={{ padding: 12, backgroundColor: appcolor.light, display: stopInfo.timeIn === undefined ? 'none' : 'flex' }}>
                                            <TouchableOpacity onPress={() => this.startAction('Z')}>
                                                <View style={{ justifyContent: 'center', height: '70%' }}>
                                                    ...
                                                    <Text style={{
                                                        color: appcolor.white, position: 'absolute', fontWeight: 'bold',
                                                        textAlign: 'center', width: '100%', fontSize: scaleSize(34)
                                                    }} >End</Text>
                                                </View>
                                            </TouchableOpacity>
                                            <View style={{ marginTop: -80 }}>
                                                <View style={{ padding: 12 }}>
                                                    <Text style={{ fontSize: scaleSize(18), fontWeight: 'bold', color: appcolor.dark }}>Thông tin</Text>
                                                    <Text style={{ color: appcolor.dark }}>Ví trí hiện tại: {this.state.addressCurrent}</Text>
                                                    <Text style={{ color: appcolor.dark }}>Tọa độ lat:{this.state.myLocation.latitude}, long: {this.state.myLocation.longitude}</Text>
                                                </View>
                                            </View>
                                        </View>
                                    </Fragment>
                                    ...
                                    <View style={{ padding: 12, backgroundColor: appcolor.light }}>
                                        <View style={{ padding: 12 }}>
                                            <Text style={{ fontSize: scaleSize(18), fontWeight: 'bold', color: appcolor.dark }}>Thông tin điểm kết thúc ngày làm việc</Text>
                                            <Text style={{ color: appcolor.dark }}>Thời gian {moment(stopInfo?.timeIn, "YYYYMMDDHHmmss").format('HH:mm dddd DD MMMM')} </Text>
                                            <Text style={{ color: appcolor.dark }}>Ví trí: {stopInfo?.address || 'Không xác định'}</Text>
                                        </View>
                                    </View>
                            }
                            <View style={{ width: '100%', padding: 12, alignItems: 'center', position: 'absolute', bottom: 0 }}>
                                <TouchableOpacity onPress={() => SheetManager.hide('sheetStartStop')}>
                                    <View style={{ borderWidth: 1, borderColor: appcolor.surface, marginBottom: 7, width: '100%' }} />
                    ...
                    <ActionSheet
                        ...
                        <View style={{ width: '100%', height: deviceHeight * 0.45, alignContent: 'center' }}>
                            <View key='noteview' style={{ width: deviceWidth, height: deviceHeight / 3, padding: 12, marginBottom: 10 }}>
                                <Text style={{ marginLeft: 12, padding: 7, fontSize: scaleSize(18), fontWeight: 'bold', color: appcolor.primary }}>{'Nhập lí do chưa check In/Out Shop'}</Text>
                                <FormGroup
                                    title={"Nhập lý do"}
                                    handleChangeForm={(text) => this.setState({ stopNote: text })}
                                    defaultValue={this.state.stopNote} editable
                                    inputStyle={{ minHeight: 120 }}
                                    numberOfLines={3} multiline={true}
                                    onClearTextAndroid={() => this.setState({ stopNote: '' })}
                                    placeholder='Nhập ghi chú ở đây'
                                />
                                <View style={{ flexDirection: 'row', padding: 7, backgroundColor: appcolor.light, alignSelf: 'center', bottom: 10 }}>
                                    <Button
                                        type="outline"
                                        title='Huỷ'
                                        buttonStyle={{ borderColor: appcolor.primary, backgroundColor: appcolor.light }}
                                        titleStyle={{ color: appcolor.primary, fontSize: 15, fontWeight: '500' }}
                                        containerStyle={{ borderColor: appcolor.transparent, width: '48%', marginRight: 7 }}
                                        onPress={this.actionPress} />
                                    <Button
                                        title='Xác nhận'
                                        titleStyle={{ fontSize: 15, fontWeight: '500' }}
                                        buttonStyle={{ borderColor: appcolor.primary, backgroundColor: appcolor.primary }}
