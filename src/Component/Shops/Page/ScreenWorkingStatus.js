...
export const ScreenWorkingStatus = ({
  locationInfo,
  workingInfo,
  navigation,
}) => {
  ...
  const getLocation = async () => {
    if (workingInfo.isStartAction && workingInfo.isEndAction) return;
    //
  ...
  const onUploadWorkingStatus = async shopCode => {
    ...
    } else await toastError('Error', 'Lỗi chưa thực hiện được');
  ...
  const validData = async () => {
    ...
    if (address == null || address.length == 0) {
      MessageInfo(
        'Chưa lấy được địa chỉ hiện tại của bạn, Vui lòng kiểm tra và thử lại sau',
      );
      return false;
    }
    if (
      ...
      MessageInfo(
        'Chưa lấy được vị trí hiện tại của bạn, Vui lòng kiểm tra lại vị trí và thử lại sau vài giây',
      );
      return false;
    }
    ...
    if (
      !workingInfo.isStartAction &&
      startConfig.isNoteLateStart == 1 &&
      startConfig.minuteLate > 0 &&
      isNoteLate
    ) {
      if (workingInfo.note == null || workingInfo.note?.length == 0) {
        MessageInfo(
          'Bạn đi trễ giờ làm việc, Vui lòng nhập lí do "Đi trễ giờ làm việc"',
        );
        return false;
      }
    }
    ...
    if (workingInfo.isStartAction && !workingInfo.isEndAction) {
      ...
      if (!checkAttendant) {
        if (
          (endConfig.isNoteStop || 0) == 1 &&
          (workingInfo.note == null || workingInfo.note.length == 0)
        ) {
      ...
      if (endConfig.isNoteEarlyStop == 1) {
        ...
        if (endConfig.timeEnd && isNoteEarly) {
          ...
          if (
            workingInfo.noteEarly == null ||
            workingInfo.noteEarly?.length == 0
          ) {
            MessageInfo(
              `Bạn kết thúc thời gian làm việc trước ${moment(validTime).format(
                'HH:mm',
              )}, Vui lòng nhập lí do "Kết thúc ngày làm việc sớm" để hoàn thành công việc`,
            );
            isResult = false;
            return;
          }
          ...
          await ATTENDANT_API.validTimeAttendant(
            ...
            async (itemTime, message) => {
              ...
              if (itemTime.isCheckTimeOut == 1) {
                const defaultTime = itemTime.timeCheckOut
                  ? moment(itemTime.timeCheckOut)
                  : moment();
                ...
                if (minuteBetween < 0) {
                  if (
                    ...
                    MessageInfo(
                      `Bạn kết thúc thời gian làm việc trước ${moment(
                        validTime,
                      ).format(
                        'HH:mm',
                      )}, Vui lòng nhập lí do "Kết thúc ngày làm việc sớm" để hoàn thành công việc`,
  ...
  const handlerPressWork = async () => {
    ...
    if (workingInfo.isStartAction && !workingInfo.isEndAction) {
      ...
      const shopNotFinish = Array.isArray(checkShopNotFinish)
        ? checkShopNotFinish
        : [];
      const listText = shopNotFinish.map(it => `${it.shopName}`).join(', ');
      if (workingInfo.isDoneShop && shopNotFinish.length > 0) {
        MessageInfo(
          `Bạn còn ${shopNotFinish.length} shop chưa chấm công: ${listText}. Vui lòng hoàn thành đầy đủ. Nếu không đi vui lòng tắt shop trong mục lịch làm việc.`,
        );
        return;
      } else {
  ...
  const handlerWorkingStatus = async shopCode => {
    ...
    await OnTime(async () => {
      ...
      Message('Thông báo', titleUpload, async () => {
        if (
          (shopCode === '1' && workingInfo.isTakePicture) ||
          (shopCode === 'Z' && workingInfo.isTakePicture)
        ) {
          let itemPhotoInfo = {
  ...
  const onViewAttendantImage = index => {
    const list = [];
    if (startImageUrl)
      list.push({ photoPath: startImageUrl, photoDesc: 'Check In' });
    if (endImageUrl)
      list.push({ photoPath: endImageUrl, photoDesc: 'Check Out' });
      ...
      <View style={styles.actionMain}>
        <View style={styles.viewTitleMain}>
          <Text style={styles.titleView}>
            Nhấn "Bắt đầu" để bắt đầu thực hiện công việc
          </Text>
        </View>
        <TouchableOpacity
          style={styles.viewActionWorking}
          onPress={handlerPressWork}
        >
          <LottieView
            autoPlay
            style={{ height: '80%' }}
          ...
          {loading ? (
            <ActivityIndicator
              size="small"
              color={appcolor.light}
              style={styles.titleContentView}
            />
          ) : (
            <Text style={styles.titleContentView}>Bắt đầu</Text>
          )}
        </TouchableOpacity>
        <View style={styles.viewInfo}>
          <View style={{ padding: 6, paddingHorizontal: 20 }}>
            ...
            <Text
              style={styles.titleInfoContent}
            >{`Ví trí hiện tại: ${address}`}</Text>
            <Text
              style={styles.titleInfoContent}
            >{`Tọa độ: Latitude ${locationInfo.latitude}, Longitude: ${locationInfo.longitude}`}</Text>
          </View>
          {startConfig.isNoteLateStart == 1 &&
            startConfig.minuteLate > 0 &&
            !workingInfo.isStartAction &&
    ...
    return (
      <View style={{ width: '100%', marginTop: 8 }}>
        <View style={styles.actionResultMain}>
          <Text style={{ ...styles.titleInfoHead, color: appcolor.primary }}>
            Thông tin điểm bắt đầu
          </Text>
          <Text style={styles.titleInfoContent}>{`Địa điểm: ${
            workingInfo.start?.address || 'Không xác định'
          }`}</Text>
          <Text style={styles.titleInfoContent}>{`Thời gian ${moment(
            workingInfo.start?.timeIn,
            'YYYYMMDDHHmmss',
          ).format('HH:mm - dddd, DD MMMM')}`}</Text>
        </View>
      </View>
    );
  };
  const EndUI = () => {
    ...
    return (
      <View style={styles.actionMain}>
        <View style={styles.viewTitleMain}>
          <Text style={styles.titleView}>
            Nhấn "Kết thúc" để kết thúc ngày làm việc
          </Text>
        </View>
          ...
          >
            <View style={{ flex: 1, alignItems: 'center' }}>
              <TouchableOpacity
                ...
                onPress={() =>
                  workingInfo.start?.imageUrl && onViewAttendantImage(0)
                }
              >
                <View
                  style={{
                ...
                >
                  {workingInfo.start?.imageUrl ? (
                    <TouchableOpacity onPress={() => onViewAttendantImage(0)}>
                      <Image
                        source={{ uri: workingInfo.start.imageUrl }}
              ...
              {workingInfo.start?.timeIn ? (
                ...
                >
                  {moment(workingInfo.start.timeIn, 'YYYYMMDDHHmmss').format(
                    'HH:mm',
                  )}
                </Text>
              ) : null}
        ...
        >
          ...
          {loading ? (
            ...
            <Text style={styles.titleContentView}>Kết thúc</Text>
          )}
        </TouchableOpacity>
        <View style={styles.viewInfo}>
  ...
  const EndResult = () => {
    return (
      <View style={styles.actionResultMain}>
        <Text style={{ ...styles.titleInfoHead, color: appcolor.primary }}>
          Thông tin điểm kết thúc
        </Text>
        <Text style={styles.titleInfoContent}>{`Địa điểm: ${
          workingInfo.end?.address || 'Không xác định'
        }`}</Text>
        <Text style={styles.titleInfoContent}>{`Thời gian ${moment(
          workingInfo.end?.timeIn,
          'YYYYMMDDHHmmss',
        ).format('HH:mm - dddd, DD MMMM')}`}</Text>
      </View>
    );
    ...
    <>
      ...
      >
        <SafeAreaView style={styles.mainContainer}>
          <Text
            style={{ ...styles.titleInfoHead, textAlign: 'center' }}
          >{`Hôm nay ${moment().format('LLLL')}`}</Text>
            ...
            >
              {startImageUrl && (
                <TouchableOpacity
                  ...
                  onPress={() => onViewAttendantImage(0)}
                >
                  <Image
              ...
              {endImageUrl && (
                <TouchableOpacity
                  style={{ flex: 1, flexDirection: 'column' }}
                  onPress={() => onViewAttendantImage(startImageUrl ? 1 : 0)}
                >
                  <Image
          ...
          {workingInfo.isStartAction &&
            (!workingInfo.isEndAction ? EndUI() : EndResult())}
          <TouchableOpacity
            style={styles.actionBackView}
      ...
      {isShowImage.visible && isShowImage.data.length > 0 && (
        ...
        >
          <MultipleShowImage
            ...
            closeShowImage={() =>
              setIsShowImage(e => ({ ...e, visible: false }))
            }
            isUseTool={false}
            useDeleteTool={false}
            useEditImage={false}
