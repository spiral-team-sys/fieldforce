...
import { ActivityIndicator, DeviceEventEmitter, RefreshControl, StyleSheet, TouchableOpacity, View } from "react-native";
...
import { removeVietnameseTones, ToastError, ToastSuccess } from "../../../Core/Helper";
...
const POPFollowScreen = ({ navigation, route }) => {
    ...
    const UploadData = (itemReject) => {
        ...
        // Upload
        const titleUpload = itemReject ? `Bạn có muốn hủy đơn hàng này không?` : itemOrder.Status == 'DELIVERY' ? `Kiểm tra số lượng sản phẩm và xác nhận nhận hàng` : `Bạn có muốn cập nhật đơn hàng này không?`
        alertConfirm(`${itemUpload.OrderNo}`, titleUpload, async () => {
            const params = {
                typeUpdate: typeUpdate,
                itemUpload: JSON.stringify(itemUpload)
            }
            await POPAPI.UpdatePOP(params, (message) => {
                ToastSuccess(message, 'Thông báo', 'top')
                LoadData()
                DeviceEventEmitter.emit('RELOAD_MENU_POP')
                SheetManager.hide('follow-detail-sheet')
            }, (error) => {
                setMessageError(error || 'Dữ liệu không thể cập nhật, Vui lòng kiểm tra lại')
            })
        }, () => { }, 'Đồng ý', 'Hủy')
    }
        ...
        if (item.Status == `DELIVERY`) {
            ...
            for (let index = 0; index < dataContent.length; index++) {
                ...
                if (!isValid(e.QuantityDamaged)) {
                    setMessageError(`${e.POPName} - Số lượng hư hỏng không được để trống`)
                    return false
                }
                if (!isValid(e.QuantityPickup)) {
                    setMessageError(`${e.POPName} - Số lượng nhận được không được để trống`)
                    return false
                }
                if (e.QuantityDamaged > e.QuantityPickup) {
                    setMessageError(`${e.POPName} - Số lượng hư hỏng không được lớn hơn số lượng nhận được`)
                    return false
                }
            }
        }
    ...
    const handlerUpdateData = (params) => {
        ...
        setData(prev => POPController.updateContentOrder(prev, orderNo, newContent));
        setDataMain(prev => POPController.updateContentOrder(prev, orderNo, newContent));
    }
    // Action
    ...
    const _searchData = (filterList) => {
        ...
        return filterList.map(group => {
            ...
            const filteredDetails = detailArr.filter(e =>
                removeVietnameseTones(e.OrderNo || "").toLowerCase().includes(valueSearch)
            )
            return {
                ...group,
    ...
    //
    useEffect(() => {
        const update_order = DeviceEventEmitter.addListener('UPDATE_ORDER_POP', handlerUpdateData)
        LoadData()
        return () => { update_order.remove() }
    }, [])
    //
    const styles = StyleSheet.create({
        ...
        subTitleName: { color: appcolor.greylight, fontSize: 12, fontWeight: '500' },
        ...
        buttonUpdate: { backgroundColor: appcolor.primary, padding: 8, paddingHorizontal: 24, borderRadius: 8, marginStart: 8 },
        titleUpdate: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.light },
        buttonClose: { backgroundColor: appcolor.light, borderWidth: 1, borderColor: appcolor.primary, padding: 8, paddingHorizontal: 24, borderRadius: 8, marginStart: 8 },
        titleClose: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.primary },
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <TouchableOpacity style={styles.itemContainer} onPress={onPressDetail}>
                ...
                {item.BillCode && <Text style={styles.subTitleName}>{`${item.BillCode}`}</Text>}
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <ActionSheet id="follow-detail-sheet" containerStyle={styles.sheetContainer} onBeforeShow={setItemOrder}>
                {/* Header */}
                <SafeAreaView edges={['top']} style={styles.viewHead}>
                    ...
                    <View style={styles.actionButton}>
                        ...
                        {(itemOrder.Status == 'SEND' || itemOrder.Status == 'DELIVERY') &&
                            <TouchableOpacity style={styles.buttonUpdate} onPress={() => { UploadData() }} disabled={isLoading}>
                                <Text style={{ ...styles.titleUpdate, color: isLoading ? appcolor.primary : appcolor.white }}>{itemOrder.Status == 'DELIVERY' ? `Nhận hàng` : `Cập nhật`}</Text>
