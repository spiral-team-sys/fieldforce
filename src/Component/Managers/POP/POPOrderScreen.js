...
import { debounce, removeVietnameseTones, ToastError } from "../../../Core/Helper";
...
const POPOrderScreen = ({ navigation, route }) => {
    ...
    useEffect(() => {
        LoadData()
        const update_data = DeviceEventEmitter.addListener('UPDATE_DATA_ORDER', hanlderUpdateData)
        const reload_data = DeviceEventEmitter.addListener('RELOAD_DATA_ITEM_ORDER', LoadData)
        return () => {
            update_data.remove()
            reload_data.remove()
        }
