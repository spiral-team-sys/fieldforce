...
import { ActivityIndicator, DeviceEventEmitter, StyleSheet, TouchableOpacity, View } from "react-native";
...
import { alertError, alertWarning, optionConfirm } from "../../../../Core/Utility";
;
...
const OrderDetails = ({ }) => {
    ...
    const orderRef = useRef({ address: '', note: '', dataOrder: [], warehouseId: 0 })
    //
    const LoadData = (data = []) => {
        if (data.length == 0) {
    ...
    const validData = () => {
        if (orderRef.current.address == null || orderRef.current.address.trim().length < 10) {
            alertWarning(`Vui lòng nhập địa chỉ nhận hàng rõ ràng để nhận đơn hàng này`)
            return false
        }
        return true
    }
    ...
    const handlerSearchAddress = async () => {
        if (orderRef.current.address || userinfo.address)
            await ATTENDANT_API.LocationFromAddress(orderRef.current.address || userinfo.address, async (location) => {
    ...
    const onDeleteOrder = () => {
        ...
        optionConfirm('Thông báo', 'Bạn có muốn xóa tất cả sản phẩm đơn hàng này không?', options)
    ...
    const styles = StyleSheet.create({
        ...
        buttonSend: { backgroundColor: appcolor.primary, padding: 8, paddingHorizontal: 24, borderRadius: 8, marginStart: 8 },
        titleOrder: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.light },
        buttonBack: { backgroundColor: appcolor.light, borderWidth: 1, borderColor: appcolor.primary, padding: 8, paddingHorizontal: 24, borderRadius: 8, marginStart: 8 },
        titleBack: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.primary },
        itemContainer: { flex: 1, flexDirection: 'row', padding: 8, borderRadius: 8, borderWidth: 0.5, borderColor: appcolor.grayLight, backgroundColor: appcolor.light, margin: 4, elevation: 3, shadowOpacity: 0.3, shadowColor: appcolor.grayLight, shadowOffset: { width: 0, height: 2 }, shadowRadius: 4 },
        viewImage: { width: '40%', overflow: 'hidden', borderRadius: 8, borderWidth: 0.5, borderColor: appcolor.surface },
        ...
        titleNumberItem: { color: appcolor.greylight, fontSize: 12, fontWeight: '500' },
        viewSummaryOrder: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
        titleSummary: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.dark, textDecorationLine: 'underline' },
        buttonDelete: { padding: 8, borderRadius: 8, marginStart: 8 },
        titleDelete: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.primary, textDecorationLine: 'underline' },
        loadingView: { position: 'absolute', alignItems: 'center', top: 0, end: 0, start: 0, bottom: 0 }
    })
    const renderItem = ({ item, index }) => {
        const imageURI = item.Image ? { uri: item.Image } : require('../../../../Themes/Images/noimage.png')
        const totalValue = item.UserInput ? item.Quantity - item.UserInput : item.Quantity
        const totalMyValue = item.UserInput ? item.QuantityMyHouse + item.UserInput : item.QuantityMyHouse
        return (
            <View style={styles.itemContainer}>
                ...
                <View style={styles.viewInfo}>
                    <Text style={styles.titleName}>{`${index + 1}. ${item.POPName}`}</Text>
                    <Text style={styles.titleNumberItem}>{`Tồn kho tổng: ${totalValue}`}</Text>
                    <Text style={styles.titleNumberItem}>{`Tồn kho cá nhân: ${totalMyValue}`}</Text>
                    <PlusMinusEdit
                        isDelete
                        itemEdit={item}
                        keyValue='UserInput'
    ...
    return (
        <ActionSheet id="order-details-sheet" containerStyle={styles.mainContainer} onBeforeShow={LoadData}>
            {/* Header */}
            <View style={styles.viewHead}>
                ...
                <View style={styles.actionButton}>
                    ...
                    <TouchableOpacity style={styles.buttonSend} onPress={UploadDataOrder} disabled={isLoading}>
                        <Text style={{ ...styles.titleOrder, color: isLoading ? appcolor.primary : appcolor.white }}>Đặt hàng</Text>
                        {isLoading && <ActivityIndicator size='small' color={appcolor.white} style={styles.loadingView} />}
                    </TouchableOpacity>
                </View>
            </View>
            {/* Content */}
            ...
            <View style={styles.viewItemOrder}>
                <View style={styles.viewSummaryOrder}>
                    <Text style={styles.titleSummary}>{`Tổng ${dataView.length} sản phẩm`}</Text>
