...
import { debounce, removeVietnameseTones, ToastError, ToastSuccess } from "../../../Core/Helper";
...
const POPItemScreen = ({ navigation, route }) => {
    ...
    const _searchData = (filterList) => {
        ...
        return filterList.map(group => {
            ...
            const filteredDetails = detailArr.filter(e =>
                removeVietnameseTones(e.POPName || "").toLowerCase().includes(valueSearch)
            )
            return {
                ...group,
    ...
    const onEditNumber = (text) => {
        ...
        if (value !== null && itemEdit.Quantity < parseInt(value)) {
            ...
            ToastError("Số lượng hư hỏng không được lớn hơn số lượng trong kho đang có", "Thông báo", 'top')
        } else {
    ...
    const styles = StyleSheet.create({
        ...
        itemContainer: { flex: 1, padding: 8, borderRadius: 8, borderWidth: 0.5, borderColor: appcolor.grayLight, backgroundColor: appcolor.light, margin: 4, elevation: 3, shadowOpacity: 0.3, shadowColor: appcolor.grayLight, shadowOffset: { width: 0, height: 2 }, shadowRadius: 4 },
        contentTab: { flex: 1, width: deviceWidth },
        itemTab: { flex: 1, backgroundColor: appcolor.light, marginHorizontal: 4, marginTop: 4 },
        imageView: { width: '100%', height: 80 },
        viewImage: { overflow: 'hidden', borderRadius: 8 },
        viewInfo: { paddingVertical: 8 },
        loadingView: { backgroundColor: appcolor.light, position: 'absolute', top: 0, bottom: 0, start: 0, end: 0, justifyContent: 'center', zIndex: 1 },
        titleName: { fontSize: 13, fontWeight: fontWeightBold, color: appcolor.dark, minHeight: 38 },
        titleNumberItem: { color: appcolor.greylight, fontSize: 12, fontWeight: '500' },
        titleNumberEdit: { color: appcolor.greylight, fontSize: 12, fontWeight: fontWeightBold, textDecorationLine: 'underline' },
        viewQuantity: { position: 'absolute', top: 4, end: 4, backgroundColor: appcolor.redgray, width: 28, height: 28, justifyContent: 'center', alignItems: 'center', borderRadius: 28, borderWidth: 1, borderColor: appcolor.surface },
        titleQuantity: { fontSize: 11, fontWeight: fontWeightBold, color: appcolor.white },
        sheetContainer: { height: 230, padding: 8, margin: 4, elevation: 3, shadowOpacity: 0.3 },
        ...
        return (
            <View key={index} style={styles.itemTab}>
                <FlashList
                    ...
                    ListFooterComponent={<View style={{ paddingBottom: deviceHeight / (dataDetails.length > 6 ? 1.8 : 5) }} />}
                />
            </View>
        )
    }
    const renderItem = ({ item, index }) => {
        ...
        const isWarehouse = ((itemMenu.isWarehouse || 0) == 0) && itemMenu.isWarningItem == 0
        const onPress = () => handlerShowDetail(item)
        return (
            <TouchableOpacity style={styles.itemContainer} onPress={onPress} disabled={!isWarehouse}>
                ...
                <View style={styles.viewInfo}>
                    ...
                    {isWarehouse && <Text style={styles.titleNumberItem}>{`Hư hỏng khi vận chuyển: ${item.DamagedShipping || 0}`}</Text>}
                    {isWarehouse && <Text style={styles.titleNumberItem}>{`Hư hỏng khi nhập kho: ${item.DamagedInWarehouse || 0}`}</Text>}
                </View>
            </TouchableOpacity>
        )
    }
    ...
    return (
        <View style={styles.mainContainer}>
            <HeaderCustom
                ...
                leftFunc={onBack}
                rightFunc={(((itemMenu.isWarehouse || 0) == 0) && itemMenu.isWarningItem == 0) && UploadData}
            />
            ...
            <ActionSheet id="product-view-sheet" keyboardHandlerEnabled onBeforeShow={setItemEdit}>
                <View style={styles.sheetContainer}>
                    <View style={styles.viewImage}>
                        <Image
                            source={itemEdit.Image ? { uri: itemEdit.Image } : require('../../../Themes/Images/noimage.png')}
                    ...
                    <TouchableOpacity style={styles.viewInfo} onPress={handlerOpenEdit}>
                        <Text style={{ ...styles.titleName, minHeight: 0 }}>{`${itemEdit.POPName}`}</Text>
                        {(itemMenu.isWarehouse || 0) == 0 && <Text style={styles.titleNumberItem}>{`Số lượng trong kho: ${itemEdit.Quantity}`}</Text>}
                        {(itemMenu.isWarehouse || 0) == 0 && <Text style={styles.titleNumberItem}>{`Hư hỏng khi vận chuyển: ${itemEdit.DamagedShipping}`}</Text>}
                        {(itemMenu.isWarehouse || 0) == 0 && <Text style={styles.titleNumberEdit}>{`Số lượng hư hỏng khi nhập kho: ${itemEdit.DamagedInWarehouse || '0'}`}</Text>}
                        <FormGroup
