...
const POPPromotionScreen = ({ navigation, route }) => {
    ...
    const _searchData = (filterList) => {
        ...
        return _.filter(filterList, (e) => (
            removeVietnameseTones(e.shopName || "").toLowerCase().includes(valueSearch) ||
            removeVietnameseTones(e.shopCode || "").toLowerCase().includes(valueSearch)
        ))
    }
    ...
    const styles = StyleSheet.create({
        ...
        itemContent: { flex: 1, padding: 8, borderRadius: 8, borderWidth: 0.5, borderColor: appcolor.grayLight, backgroundColor: appcolor.light, margin: 4, elevation: 3, shadowOpacity: 0.3, shadowColor: appcolor.grayLight, shadowOffset: { width: 0, height: 2 }, shadowRadius: 4 },
        contentTab: { flex: 1, width: deviceWidth },
        itemTab: { flex: 1, backgroundColor: appcolor.light, marginHorizontal: 4, marginTop: 4 },
        titleGroup: { fontSize: 14, fontWeight: '800', color: appcolor.primary, paddingHorizontal: 8, backgroundColor: appcolor.primary + 10, padding: 8, textAlign: 'center' },
        titleName: { fontSize: 13, fontWeight: fontWeightBold, color: appcolor.dark },
        subTitleName: { fontSize: 12, fontWeight: '500', color: appcolor.greylight },
        subTitleCount: { fontSize: 11, fontWeight: fontWeightBold, color: appcolor.greylight, fontStyle: 'italic', textAlign: 'right', marginTop: 4 },
        viewTitleShop: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 },
        viewDetails: { padding: 8, borderRadius: 8, borderWidth: 0.5, borderColor: appcolor.grayLight, backgroundColor: appcolor.light, margin: 4, elevation: 3, shadowOpacity: 0.3, shadowColor: appcolor.grayLight, shadowOffset: { width: 0, height: 2 }, shadowRadius: 4 },
        titleShop: { fontSize: 13, fontWeight: fontWeightBold, color: appcolor.primary, textAlign: 'center' },
        titleShopCode: { fontSize: 12, fontWeight: '500', color: appcolor.primary, textAlign: 'center', marginBottom: 4 },
    })
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View style={styles.itemContainer}>
                {item.isParent && <Text style={styles.titleGroup}>{item.dealerName}</Text>}
                <TouchableOpacity style={styles.itemContent} onPress={onPress}>
                    <View style={styles.viewTitleShop}>
                        <Icon type="ionicon" name="business" size={18} color={appcolor.dark} />
                    ...
                    <Text style={styles.subTitleCount}>{`Tổng ${countItem} sản phẩm`}</Text>
                </TouchableOpacity>
            </View>
        )
    }

    if (isLoading) return <LoadingView isLoading={isLoading} styles={styles.loadingView} />
    return (
        <View style={styles.mainContainer}>
            ...
            <ActionSheet id="item-promotion-sheet" onBeforeShow={setItemDetail}>
