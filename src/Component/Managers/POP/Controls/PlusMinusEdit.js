...
const PlusMinusEdit = ({ title = null, itemEdit, keyValue, onChange, isDelete = false, isEditable = true }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    const [_mutate, setMutate] = useState(false)
    const inputRef = useRef()
    const isValue = itemEdit[keyValue] !== undefined && itemEdit[keyValue] !== null
    //
    ...
    const styles = StyleSheet.create({
        ...
        titleHead: { color: appcolor.greylight, fontSize: 12, fontWeight: '500', marginTop: 4 },
        headContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 8 },
        contentContainer: {
            width: '75%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', alignSelf: 'center', borderWidth: 0.5, borderColor: appcolor.grayLight, borderRadius: 8, paddingHorizontal: 8, backgroundColor: appcolor.light,
        ...
        titleName: { fontSize: 13, fontWeight: isValue ? fontWeightBold : '500', color: isValue ? appcolor.dark : appcolor.grey },
        titleNumberItem: { color: appcolor.greylight, fontSize: 12, fontWeight: '500' },
    })
    if (!isEditable)
        return <Text style={styles.titleNumberItem}>{`${title}: ${itemEdit[keyValue] || 0}`}</Text>
            ...
            <View style={styles.headContainer}>
                <View style={styles.contentContainer}>
                    ...
                    <TouchableOpacity style={styles.viewInput} onPress={handlerOpenEdit}>
                        <Text style={styles.titleName}>{isValue ? itemEdit[keyValue] : `Số lượng`}</Text>
                    </TouchableOpacity>
                    <Icon
                        type="ionicon"
                        name="add"
