...
export const isRemotePath = (path = '') => path.startsWith('http://') || path.startsWith('https://');
...
export const getReportName = (item, reportNameMap = {}) => {
    if (item?.reportId === 1) {
        const inOut = item?.photoType?.toString() === '0' ? 'Vào ca' : item?.photoType?.toString() === '1' ? 'Ra ca' : `Loại ${item?.photoType}`;
...
export const getPhotoTime = (item) => {
    if (item?.photoFullTime) {
        const full = moment(item.photoFullTime, ['YYYY-MM-DD HH:mm:ss', moment.ISO_8601], true);
        if (full.isValid()) return full.format('HH:mm:ss');
    }

    if (item?.photoDate && item?.photoTime) {
        const fromFields = moment(`${item.photoDate}${item.photoTime}`, 'YYYYMMDDx', true);
