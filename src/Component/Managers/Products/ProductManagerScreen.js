...
const ProductManagerScreen = ({ navigation }) => {
  ...
  const LoadData = async () => {
    ...
    listCompetitor = listCompetitor.map(item => {
      ...
      item.groupData = removeDuplicate(item.listProduct, 'categoryId').map(
        g => {
          const products = item.listProduct.filter(
            p => p.categoryId === g.categoryId,
          );
          return {
            id: g.categoryId,
            name: g.category || 'N/A',
  ...
  const filteredSections = useMemo(() => {
    ...
    return sections.reduce((acc, section) => {
      const chips = section.groupData.filter(c =>
      ...
      if (
        ...
        acc.push({
          ...section,
          groupData: chips.length ? chips : section.groupData,
        });
      return acc;
    }, []);
  }, [sections, search]);
  ...
  const renderChips = (section, sectionIndex) =>
    section.groupData.map((chip, idx) => {
      ...
      return (
        ...
        >
          <Avatar
            containerStyle={{
              backgroundColor: colorList[sectionIndex % colorList.length],
            }}
