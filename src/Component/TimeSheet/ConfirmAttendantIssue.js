...
import { groupDataByKey, MessageAction, MessageInfo, ToastError, ToastSuccess } from "../../Core/Helper";
...
export const ConfirmAttendantIssue = ({ navigation, route }) => {
    ...
    const [data, setData] = useState({ dataSubmit: [], dataConfirm: [], dataReject: [], dataLock: [] })
    const tabRef = useRef()
    ...
    const loadAllData = async (fromDate, toDate) => {
        ...
        let groupData = { dataSubmit: [], dataConfirm: [], dataReject: [], dataLock: [] }
        let jsonData = { jsonDataSubmit: '', jsonDataConfirm: '', jsonDataReject: '', jsonDataLock: '' }
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity key={`fa${index}2lla`}
                onPress={onPress}
    ...
    const handlerSubmitAll = async (type) => {
        try {
            ...
            const indexTabUpdate = itemTimeSheet.find(it => it.code == (type == 3 ? (indexTab.code + 1) : (indexTab.code + 2)));
            MessageAction(
                'Bạn chắc chắn muốn ' + (type == 3 ? 'xác nhận' : 'từ chối') + ` tất cả (${data.dataSubmit.length}) đề xuất của nhân viên?`,
                async () => {
                    setLoading(true);
                    try {
                        let ok = 0, fail = 0;
                        for (const it of data.dataSubmit) {
                            const isSkipReason = Number(it.isCountExplain) === 1 || Number(it.isCountExplain) === 2;
                            const notify = `Người quản lí ${userinfo.groupType || ''} đã ${type == 3 ? 'đồng ý' : 'từ chối'} đề xuất chấm công ngày ${it.workDate}${type === 3 || isSkipReason ? '' : ' với lí do : '}`;
                            ...
                            if (res?.statusId === 200) ok++; else fail++;
                        }
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.bluelight }}>
            ...
            />
            <View style={{ height: '100%', backgroundColor: appcolor.surface, width: deviceWidth, }}>
                ...
                <View style={{ flex: 1, }}>
                    ...
                    }
                    {
                        ...
                        <View style={{ flex: 1 }}>
                            {config.isConfirmAll && data.dataSubmit.length > 0 && <View style={{ alignItems: 'center', justifyContent: 'center', paddingBottom: 8 }}>
                                <TouchableOpacity onPress={() => handlerSubmitAll(3)} style={{ padding: 8, borderRadius: 10, backgroundColor: appcolor.primary, marginRight: 10 }}>
                                    <Text style={{ color: appcolor.light, fontWeight: '500', fontSize: 12 }}>Xác nhận tất cả</Text>
                                </TouchableOpacity>
                            </View>}
                            <FlatList
                                key={'listTimeSheet'}
                                data={indexTab.code == 2 ? data.dataSubmit : (indexTab.code == 3 ? data.dataConfirm : (indexTab.code == 4 ? data.dataReject : data.dataLock))}
                                keyExtractor={(_, index) => index.toString()}
                                style={{ padding: 5, flex: 1, borderTopLeftRadius: 40, borderTopRightRadius: 40, }}
                                renderItem={({ item, index }) => <RenderItemTimeSheet item={item} index={index} appcolor={appcolor} indexTab={indexTab} itemDate={itemDate} loadAllDataNew={loadAllDataNew} data={data} />}
                                ListFooterComponent={<View style={{ height: deviceHeight / 2, width: deviceWidth }} />}
                                showsHorizontalScrollIndicator={false}
                                refreshing={loading}
                                onRefresh={() => //loadAllData(itemDate.fromDate, itemDate.toDate)
                    ...
                    }
                    {
                        ...
                        <FlatList
                            ...
                            keyExtractor={(_, index) => index.toString()}
                            style={{ padding: 5, flex: 1, borderTopLeftRadius: 40, borderTopRightRadius: 40, }}
                            renderItem={({ item, index }) => <ViewTimeSheet item={item} index={index} appcolor={appcolor} indexTab={indexTab} itemDate={itemDate} loadAllDataNew={loadAllDataNew} data={data} />}
                            ListFooterComponent={<View style={{ height: deviceHeight / 2, width: deviceWidth }} />}
                            showsHorizontalScrollIndicator={false}
                            refreshing={loading}
                            onRefresh={() => //loadAllData(itemDate.fromDate, itemDate.toDate)
            ...
            >
                <View style={{ width: deviceWidth, minHeight: '50%', paddingBottom: 30 }} >
...
const ViewTimeSheet = ({ item, index, appcolor, indexTab, loadAllDataNew, itemDate, data }) => {
        ...
        <View key={`${indexTab.code}`}>
            ...
            </TouchableOpacity>
            {
                ...
                <FlatList
                    ...
                    keyExtractor={(_, index) => index.toString()}
                    style={{ padding: 5, flex: 1, borderTopLeftRadius: 40, borderTopRightRadius: 40, }}
                    renderItem={({ item, index }) => <RenderItemTimeSheet key={`${indexTab.code}_TimeSheet_${item.workDate}`} item={item} index={index} appcolor={appcolor} indexTab={indexTab} itemDate={itemDate} loadAllDataNew={loadAllDataNew} data={data} />}
...
const RenderItemTimeSheet = ({ item, index, indexTab, loadAllDataNew, itemDate, data }) => {
    const { appcolor, kpiinfo, userinfo } = useSelector(state => state.GAppState)
    ...
    const renderItemShop = (item, index) => {
        ...
        return (
            <View key={`${item.workDate}_${index}`} style={{ width: '100%', padding: 8, backgroundColor: appcolor.light, marginVertical: 5, borderRadius: 10 }}>
                ...
                {(item.ShopId > 0 || configCheckTimeStore.isEditShopEmpty == 1) && configCheckTimeStore.isCheckTimeStore == 1 &&
                    <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                        <View style={{ flex: 1 }}>
                            <Text style={{ textAlign: 'center', fontSize: 12, color: appcolor.dark }}>CheckIn</Text>
                            <FormGroup value={item.TimeCI_Update || ''}
                                ...
                                editable={false} placeholder={'Nhập CI'}
                        ...
                        <View style={{ flex: 1 }}>
                            <Text style={{ textAlign: 'center', fontSize: 12, color: appcolor.dark }}>CheckOut</Text>
                            <FormGroup value={item.TimeCO_Update || ''}
                                inputStyle={{ padding: 0, margin: 0, height: 28, textAlign: 'center' }}
                                containerStyle={{ margin: 4, marginRight: 5, borderWidth: 1, marginBottom: 2 }}
                                editable={false} placeholder={'Nhập CO'}
                                useClearAndroid={false}
                                keyboardType="number-pad"
                                onEndEditing={() => handlerEndEditTime(item)}
                                handleChangeForm={(value) => handlerEditTime(value, item, 'CO')} multiline
                            />
                        </View>
                        <View style={{ flex: 1 }}>
                            <Text style={{ textAlign: 'center', fontSize: 12, color: appcolor.dark }}>Tổng thời gian</Text>
                            <FormGroup value={item.TotalTime_Update || ''}
                                inputStyle={{ padding: 0, margin: 0, height: 28, textAlign: 'center' }}
                                containerStyle={{ margin: 4, marginRight: 5, borderWidth: 1, marginBottom: 2 }}
                ...
                {
                    listEvidence?.length > 0 && configCheckTimeStore.isCheckTimeStore == 1 && <PhotoInput _guid={item.guid} itemInput={item} listPhoto={indexTab != 1 ? listEvidence : null} indexTab={indexTab} />
                }
            </View>
        )
    }
    ...
    const handleSubmit = async (type) => {
        if (type !== 3) {
            if (note === '') {
                ToastError('Nhập ghi chú khi ' + (type == 3 ? 'đồng ý' : 'từ chối') + ', ngày ' + item.workDate + '!!!')
                return
            } else if (note.replace(/ /g, "").length < 5) {
                ToastError('Nhập ghi chú ít nhất 5 kí tự, ngày ' + item.workDate + '!!!')
                return
            }
        }
        let notify = "Người quản lí đã " + (type == 3 ? 'đồng ý' : 'từ chối') + " đề xuất chấm công ngày " + item.workDate + (type == 3 || isSkipReason ? '' : " với lí do : " + note);
        ...
        let indexTabUpdate = itemTimeSheet.find(it => it.code == (type == 3 ? (indexTab.code + 1) : (indexTab.code + 2)))
        MessageAction('Bạn chắc chắn muốn ' + (type == 3 ? 'đồng ý' : 'từ chối') + ' đề xuất của nhân viên : ' + item.employeeName + ' ngày : ' + item.workDate + '?', async () => {
            ...
            <View style={{ flex: 1, padding: 5, backgroundColor: appcolor.light, margin: 5, borderRadius: 10 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 8 }}>
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        <Icon size={17} color={appcolor.dark} name={"stopwatch"} type="font-awesome-5" />
                    </View>
                </View>
                <View
                ...
                >
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                        <View style={{ flexDirection: 'row' }}>
                            ...
                            <Text style={{ color: appcolor.dark, paddingLeft: 10 }}>Cửa hàng đi trong ngày </Text>
                        </View>
                        <Text style={{ color: appcolor.dark }}>{item.totalShopActual} </Text>
                    </View>
                </TouchableOpacity>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: appcolor.surface, padding: 5, margin: 5, borderRadius: 8, height: 35, paddingLeft: 12 }}>
                    <Text style={{ color: appcolor.dark, fontWeight: '500', fontSize: 14 }}>{item.shiftCode || `${indexTab.code == 1 ? 'Chọn ca' : ''}`}</Text>
                </View>
                <View >
                    {
                        !isSkipReason && item.subNote && <FormGroup value={`Lí do : ${item.subNote || ''}`} containerStyle={{ marginLeft: 5, padding: 5, marginBottom: 5, marginRight: 5, borderWidth: 0.2 }} editable={false} multiline />
                    }
                    {
                        item.submitNote != undefined && item.submitNote != null && item.submitNote != '' &&
                        <FormGroup value={`${'Ghi chú : '}${item.submitNote || ''}`} containerStyle={{ marginLeft: 5, marginRight: 5, borderWidth: 0.2, marginBottom: 5, padding: 5 }} editable={false} multiline />
                    }
                    {
                        item.subNoteLevel1 != undefined && item.subNoteLevel1 != null && item.subNoteLevel1 != '' &&
                        <FormGroup value={`${item.titleNoteLevel1 ? item.titleNoteLevel1 : 'Quản lí ghi chú : '}${item.subNoteLevel1 || ''}`} containerStyle={{ marginLeft: 5, marginRight: 5, borderWidth: 0.2, marginBottom: 5, padding: 5 }} editable={false} multiline />
                    }
                    {
                        ((indexTab.code != 5 && indexTab.code != 2 && item.confirmNote != '' && item.confirmNote) || indexTab.code == 2) &&
                        <FormGroup
                            value={indexTab.code == 2 ? note : `${item.titleNoteLevel2 ? item.titleNoteLevel2 : 'Quản lí ghi chú : '}${item.confirmNote || ''}`}
                            containerStyle={{ marginLeft: 5, marginRight: 5, borderWidth: 0.8, marginBottom: 0, padding: 5, color: appcolor.dark }}
                            editable={indexTab.code == 2 ? true : false}
                            placeholder={'Nhập ghi chú'} placeholderColor={appcolor.placeholderText}
                            handleChangeForm={(text) => setNote(text)} />
                    }
                </View>
                <ViewPhotoEvidence key={`photoById_${item.tabId}_${item.attendantId}`} listPhotoItem={listPhotoItem} showItemImage={showItemImage} itemMain={item} />
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        {
                            ...
                            <TouchableOpacity
                                ...
                                style={{ minWidth: 60, justifyContent: 'center', alignItems: 'center', padding: 8, borderRadius: 10, backgroundColor: appcolor.grayLight }}
                            >
                                <Text style={{ fontWeight: '500', fontSize: 12, color: appcolor.dark }} >xác nhận</Text>
                            </TouchableOpacity>
                        }
                        {
                            indexTab.code == 2 &&
                            <TouchableOpacity
                                ...
                                style={{ minWidth: 60, justifyContent: 'center', alignItems: 'center', padding: 8, borderRadius: 10, backgroundColor: appcolor.grayLight, marginLeft: 5 }}
                            >
                                <Text style={{ fontWeight: '500', fontSize: 12, color: appcolor.dark }}>Từ chối</Text>
                            </TouchableOpacity>
                        }
                    </View>
                </View>
                ...
                >
                    <View style={{ padding: 5, paddingBottom: 30, width: deviceWidth, maxHeight: deviceHeight / 1.5, minHeight: deviceHeight / 2.5 }} >
                        <ScrollView>
                            {
                                shopDetail.length > 0 &&
                                shopDetail.map((it, index) => {
                                    return (
                                        renderItemShop(it, index)
                                    )
                                })
                            }
                        </ScrollView>
...
const ViewPhotoEvidence = ({ listPhotoItem, showItemImage, itemMain }) => {
    ...
    const RenderItemPhoto = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity key={itemMain.workDate + index}
            ...
            >
                <CacheImage resizeMode={'cover'} containerStyle={{ borderRadius: 12, }}
                    source={{ uri: item.photoPath !== null ? (item.photoPath.indexOf('file://') > -1 || item.photoPath.indexOf('https://') > -1 ? item.photoPath : URLDEFAULT + item.photoPath) : null }}
                />
            </TouchableOpacity>
        )
    }
    ...
    return (
        <View key={'Photo_' + itemMain.workDate} style={{ flexDirection: 'column', width: '100%' }}>
