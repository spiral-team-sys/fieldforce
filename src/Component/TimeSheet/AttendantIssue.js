...
const itemTimeSheet = [
  {
    ...
    name: 'Default',
    nameVN: 'Đề xuất',
    nameData: 'dataDefault',
    nameJson: 'jsonDataDefault',
  },
  {
    code: 2,
    id: 2,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: appcolor.onBackground }}>
      <HeaderCustom
        ...
        title={menuitem?.menuNameVN || 'Đề xuất chấm công'}
      />
          ...
          {!loading && indexTab.code < 3 && (
            <FlatList
              ...
              renderItem={({ item, index }) => (
                <RenderItemTimeSheet
                  key={indexTab.code + item.workDate}
                  item={item}
                  index={index}
                  appcolor={appcolor}
                  indexTab={indexTab}
    ...
    <View>
      ...
      {isShowTimeSheet && (
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <RenderItemTimeSheet
              key={`itemAttendant_${item.tabId}_${item.attendantId}_${item.workDate}`}
              item={item}
              index={index}
              appcolor={appcolor}
              indexTab={indexTab}
...
const RenderItemTimeSheet = ({
  ...
  const listReason = Array.isArray(item.listReason)
    ? item.listReason
    : JSON.parse(item.listReason ?? '[]');
  const shopDetail = Array.isArray(item.shopDetail)
    ? item.shopDetail
    : JSON.parse(item.shopDetail ?? '[]');
  const listShift = Array.isArray(item.shiftDetail)
    ? item.shiftDetail
    : JSON.parse(item.shiftDetail ?? '[]');
  //
  ...
  const [shopPermission, setShopPermission] = useState(
    Array.isArray(item.shopPermission)
      ? item.shopPermission
      : JSON.parse(item.shopPermission ?? '[]'),
  );
  ...
  const handleAddStore = itemStore => {
    ...
    const countShop = shopDetail.filter(
      it => it.ShopId > 0 && it.isShopAdded == 1,
    ).length;
    const checkShift = listShift.find(it => it.ShiftCode === currentShift);
    if (checkShift?.NumberValue && countShop >= checkShift.NumberValue) {
      MessageInfo(
        `Bạn chỉ được thêm tối đa ${checkShift.NumberValue} cửa hàng trong ca ${checkShift.ShiftName}`,
      );
      return;
    }
    const newShop = {
      ...
      >
        <Text
          style={{
            fontSize: 12,
            color:
              selectReason.Code == item.Code ? appcolor.black : appcolor.dark,
          }}
        >
          {item.Reason}
        </Text>
  ...
  const checkHalfShiftOver4Hours = (selectedShift, type) => {
    ...
    if (type != 'update') {
      const [inHourTotal, inMinuteTotal] = item.totalActual
        .split(':')
        .map(Number);
      ...
      for (const shop of shopFilter) {
        const timeIn =
          type == 'update' ? shop.TimeCI_Update || shop?.TimeCI : shop?.TimeCI;
        const timeOut =
          type == 'update' ? shop.TimeCO_Update || shop?.TimeCO : shop?.TimeCO;
      ...
      >
        <Text
          style={{
            fontSize: 12,
            color:
              currentShift == item.ShiftCode ? appcolor.black : appcolor.dark,
          }}
        >
          {item.ShiftName || 'null'}
        </Text>
  ...
  const handlerEndEditTime = item => {
    ...
    if (item.TimeCI_Update != null && item.TimeCO_Update != null) {
      if (!isValidRange(item.TimeCI_Update, item.TimeCO_Update)) {
        MessageInfo(
          `Thời gian "Check OUT" phải lớn hơn "Check IN", vui lòng nhập lại`,
        );
        return;
      }
  ...
  const renderItemShop = (item, index, itemMain) => {
    ...
    item.guid == null || item.guid == '' || item.guid != guidByShop
      ? (item.guid = guidByShop)
      : null;
    ...
    return (
      <View
        key={`${item.ShopId}_${item.EmployeeId}_${itemMain.workDate}_${index}`}
        style={{
          width: '100%',
          padding: 8,
      ...
      >
        ...
        {item.isShopAdded == 1 && (
          <TouchableOpacity
            onPress={() => {
              const indexShop = shopDetail.findIndex(
                it => it.guid === item.guid,
              );
              if (indexShop > -1) {
  ...
  const handleSubmit = async type => {
    const checkTime = checkHalfShiftOver4Hours(
      listShift.find(it => it.ShiftCode === currentShift),
    );
    if (
      !isSkipReason &&
      (Object.keys(selectReason).length == 0 || selectReason === undefined) &&
      type == 2 &&
      shiftPlan !== 'OFF' &&
      isHideReasonButton == false
    ) {
      toastError(
        'Cảnh báo',
        'Chọn lí do trước khi gửi, ngày ' + item.workDate + '!!!',
      );
      return;
      ...
      if (note.replace(/ /g, '') == '') {
        toastError(
          'Cảnh báo',
          'Nhập ghi chú trước khi gửi, ngày ' + item.workDate + '!!!',
        );
        return;
      } else if (note.replace(/ /g, '').length < 10) {
        toastError(
          'Cảnh báo',
          'Nhập ghi chú ít nhất 10 kí tự, ngày ' + item.workDate + '!!!',
        );
        return;
      }
    }
    ...
    if (
      ...
      let dataShopDetail = Array.isArray(item.shopDetail)
        ? item.shopDetail
        : JSON.parse(item.shopDetail || '[]');
      for (let i = 0; i < dataShopDetail.length; i++) {
        let itemShop = dataShopDetail[i];
        const listEvident = Array.isArray(itemShop.Evidence)
          ? itemShop.Evidence
          : JSON.parse(itemShop.Evidence || '[]');
        if (
          (itemShop.ShopId > 0 ||
            (configCheckTimeStore.isEditShopEmpty && !itemShop.ShopId)) &&
          listEvident?.length == 0
        ) {
      ...
      if (hasShopWithoutEvidence) {
        toastError(
          'Cảnh báo',
          `Ngày ${item.workDate} có cửa hàng chưa có ảnh bằng chứng, vui lòng kiểm tra lại!!!`,
        );
        return;
      }
      ...
      const checkShift = listShift.find(
        it => it.ShiftCode === currentShift && it.isShopAdded == 1,
      );
      if (checkShift?.NumberValue && countShop != checkShift.NumberValue) {
        toastError(
          'Cảnh báo',
          `Vui lòng chọn đủ ${checkShift.NumberValue} cửa hàng theo ca đã chọn, ngày ${item.workDate}!!!`,
        );
        return;
      }
    }
    ...
    if (checkTime.isHalfShift && !checkTime.isValid) {
      const checkTimeUpdate = checkHalfShiftOver4Hours(
        listShift.find(it => it.ShiftCode === currentShift),
        'update',
      );
    ...
    if (configCheckTimeStore.isCheckTimeStore == 1 && !isHideReasonButton) {
      ...
      for (let i = 0; i < dataShopDetail.length; i++) {
        ...
        const listEvident = Array.isArray(itemShop.Evidence)
          ...
          : JSON.parse(itemShop.Evidence || '[]');
        if (
          (itemShop.TimeCI_Update && !isValidTime24(itemShop.TimeCI_Update)) ||
          (itemShop.TimeCO_Update && !isValidTime24(itemShop.TimeCO_Update))
        ) {
          toastError(
            'Cảnh báo',
        ...
        if (itemShop.TimeCI_Update && itemShop.TimeCO_Update) {
          if (!isValidRange(itemShop.TimeCI_Update, itemShop.TimeCO_Update)) {
            toastError(
              'Cảnh báo',
              `Ngày ${item.workDate} cửa hàng ${itemShop.ShopCode} phải có thời gian "Check OUT" lớn hơn "Check IN", vui lòng nhập lại!!!`,
            );
            return;
          }
        }
        if (
          ...
          toastError(
            'Cảnh báo',
            `Ngày ${item.workDate} cửa hàng ${itemShop.ShopCode} phải có đầy đủ thời gian "Check IN" và "Check OUT", vui lòng nhập lại!!!`,
          );
          return;
        }
    ...
    let indexTabUpdate = itemTimeSheet.find(
      it => it.code == (type == 2 ? indexTab.code + 1 : indexTab.code - 1),
    );
    (photoUpload.length || isHaveEvident) &&
  ...
  const showItemImage = async indexImage => {
    await setDataPhoto({
      listPhoto: listPhotoItem.listPhoto,
  ...
  return (
    <View key={indexTab.code + item.workDate}>
      ...
      >
          ...
          >
            <Text
              style={{
                fontWeight: '700',
                fontSize: 16,
                color: item.statusColor || appcolor.dark,
              }}
            >
              {item.statusTitle}
            </Text>
        ...
        <TouchableOpacity
          onPress={() => (indexTab.code == 1 ? openSheet('SHOW_SHIFT') : null)}
          opacity={indexTab.code == 1 ? 0.5 : 1}
          style={{
            flex: 1,
            padding: 10,
            margin: 5,
            borderWidth: item.tabId == 1 ? 0.2 : 0,
            justifyContent: 'center',
            borderColor: appcolor.dark,
            backgroundColor:
              item.tabId == 1 ? appcolor.light : appcolor.surface,
            borderRadius: 8,
            height: 35,
          }}
        >
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            {item.tabId == 1 && (
              <SpiralIcon
                ...
                color={appcolor.dark}
                style={{ height: 20, width: 20 }}
                name={isShowShift ? 'chevron-down' : 'chevron-right'}
                type="font-awesome-5"
              />
            )}
            <Text
            ...
            >
              {currentShift || `${indexTab.code == 1 ? 'Chọn ca' : ''}`}
            </Text>
          </View>
        </TouchableOpacity>
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <SpiralIcon
                ...
                color={appcolor.dark}
                style={{ height: 20, width: 20 }}
                name={isShowReason ? 'chevron-down' : 'chevron-right'}
                type="font-awesome-5"
              />
              <Text
                style={{
              ...
              >
                {selectReason.Reason || 'Chọn lí do'}
              </Text>
            </View>
          </TouchableOpacity>
        )}
        {indexTab.code == 1 &&
          ...
          Number(item.isCountExplain) == 1 && (
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                {/* <SpiralIcon size={18} color={appcolor.dark} style={{ height: 20, width: 20 }} name={isShowReason ? "chevron-down" : "chevron-right"} type="font-awesome-5" /> */}
                <Text
                  style={{
                    color: appcolor.dark,
                    fontWeight: '500',
                ...
                >
                  Số lượt còn lại: {item.remainingExplanation}
                </Text>
              </View>
            </View>
          )}
        <View>
          ...
          {(indexTab.code == 1 || (noteValue != '' && indexTab.code != 2)) && (
            <FormGroup
              ...
              editable={indexTab.code == 1 ? true : false}
              placeholder={'Nhập ghi chú'}
              handleChangeForm={setNote}
              multiline
            />
          )}
          {indexTab.code > 2 && indexTab.code !== 5 && (
            <FormGroup
              value={'Quản lí ghi chú : ' + (item.confirmNote || ' ')}
              containerStyle={{
                marginLeft: 5,
                marginRight: 5,
                borderWidth: 0.2,
            ...
            <ScrollView>
              {listShift.length > 0 &&
                listShift.map((it, index) => {
                  return renderItemShift(
                    it,
                    index,
                    `${item.tabId}_${item.attendantId}_${it.ShiftCode}`,
                  );
                })}
            </ScrollView>
          </View>
          ...
          {showShopList && (
            ...
            >
              ...
              <FlatList
                ...
                keyExtractor={(_, index) => index.toString()}
                renderItem={({ item }) => {
                  const checkExist = shopDetail.find(
                    it => it.ShopId == item.ShopId,
                  );
              ...
              <ScrollView>
                {shopDetail.length > 0 &&
                  shopDetail.map((it, index) => {
                    return renderItemShop(it, index, item);
                  })}
                <View style={{ height: 100 }} />
              </ScrollView>
              {configCheckTimeStore.isCheckTimeStore == 1 &&
                item.tabId == 1 &&
                shopPermission.length > 0 &&
                shiftPlan !== 'OFF' && (
                  <TouchableOpacity
                    style={{
                  ...
                  >
                    ...
                    >
                      <SpiralIcon size={30} name="add" color={appcolor.white} />
                      <Text
                        style={{
                          paddingHorizontal: 4,
...
const ViewPhotoEvidence = ({
  ...
  const RenderItemPhoto = ({ item, index, listPhotoItem }) => {
    ...
    const imageUrl =
      item.photoPath !== null
        ...
        : null;
    return item.photoPath === null && index == -1 ? (
      <TouchableOpacity
        key={index}
        style={{
          width: deviceWidth / 5,
  ...
  const reloadpage = async () => {
    const itemPhotoByGuiId = await getPhotoByTypeDesc(
      'AttendantIssue',
      `${itemAttendant.tabId}_${itemAttendant.attendantId}`,
    );
  ...
  const deleteItemImage = async itemDelete => {
    listPhotoItem.listPhoto = listPhotoItem.listPhoto.filter(
      it => it.id != itemDelete.id,
    );
  ...
  return (
    <View style={{ flexDirection: 'column', width: '100%' }}>
      ...
      <View style={{ flex: 1, flexDirection: 'row' }}>
        {listPhotoItem?.itemTakePicture !== undefined &&
          itemAttendant.tabId === 1 && (
            <RenderItemPhoto
              item={listPhotoItem.itemTakePicture}
              index={-1}
              listPhotoItem={listPhotoItem}
            />
          )}
