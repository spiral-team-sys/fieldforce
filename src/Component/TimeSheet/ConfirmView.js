...
export const ConfirmView = ({ data, onLock }) => {
  ...
  const CardTemplate = ({ item, index }) => {
    return (
      <View
        key={`${index}02ma`}
        style={{
          backgroundColor: data.color !== '' ? data.color : appcolor.light,
          borderRadius: 12,
          margin: 3,
          padding: 7,
        }}
      >
        <Image
          resizeMode="contain"
          style={{ width: widthImage, height: deviceWidth * 0.4 }}
          source={{ uri: `${URLDEFAULT}${item?.photoPath}` }}
        />
        <Text
          style={{ color: appcolor.dark, fontSize: 12, textAlign: 'center' }}
        >
  ...
  const photoView = ({ item, index }) => {
    const photoPath = item.photoPath?.includes('http')
      ? item.photoPath
    ...
    <View style={{ backgroundColor: appcolor.surface, margin: 7 }}>
      <ScrollView>
        <View style={{ flexDirection: 'row', backgroundColor: appcolor.light }}>
          <Text
            style={{ flexGrow: 1, color: appcolor.dark, fontWeight: '600' }}
          >
            {data.employeeCode} {data.employeeName}
          </Text>
          <Text numberOfLines={1} style={{}}>
            {moment(data.workDate, 'YYYY-MM-DD').format('dddd, DD MMMM yy')}
          </Text>
        </View>
        <ScrollView
        ...
        >
          {detail.map((v, i) => {
            return <CardTemplate key={`${i}nkal`} item={v} index={i} />;
          })}
        </ScrollView>
        <View style={{ paddingTop: 0 }}>
          <View>
            <Text style={{ color: appcolor.dark, fontWeight: '600' }}>
              Thông tin chấm công
            </Text>
          </View>
          <View
            style={{ padding: 12, margin: 3, backgroundColor: appcolor.light }}
          >
            <View style={{ flexDirection: 'row' }}>
              <Text style={{ flexGrow: 1, color: appcolor.dark, fontSize: 10 }}>
                Ca làm việc
              </Text>
              <Text
                style={{
          ...
          >
            <Text
              numberOfLines={1}
              style={{ color: appcolor.dark, padding: 3 }}
            >
            ...
            <Text
              style={{ fontWeight: '700', color: appcolor.dark, padding: 3 }}
            >
          ...
          {userinfo.groupType === 'SUP' &&
            (data.confirmedBy == null ||
              ...
              data.confirmedBy === userinfo.employeeId) && (
              <View
                style={{
              ...
              >
                ...
                >
                  <SpiralIcon size={20} name="lock" raised />
                  <Text style={{ color: appcolor.white, padding: 12 }}>
                    Xác nhận
                  </Text>
                </TouchableOpacity>
              </View>
