...
export const ReportTimeSheet = ({ navigation, route }) => {
  ...
  const onLoad = async months => {
    ...
    const reportDate = `${filter.year}-${
      monthChoose > 9 ? monthChoose : '0' + monthChoose
    }-01`;
  ...
  const rowItem = ({ item, index }) => {
    return (
      <View
        style={{
          backgroundColor:
            index % 2 === 0 ? appcolor.light : appcolor.graylight,
          padding: 7,
        }}
        key={`${index}l9a`}
      >
        <View style={{ flexDirection: 'row' }}>
          ...
          <Text style={{ color: appcolor.dark, marginEnd: 20 }}>
            {item.colValue}
          </Text>
        </View>
      </View>
