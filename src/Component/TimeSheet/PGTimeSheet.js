...
import { alertError, checkNetwork, deviceHeight, deviceWidth } from "../../Core/Utility"
...
export const PGTimeSheet = ({ navigation }) => {
    ...
    const onLoad = async (isSearch) => {
        ...
        const localData = isSearch !== undefined ? null : await AsyncStorage.getItem(TIMESHEET.DATA_SHEET)
        if (localData === undefined || localData === null) {
            if (await checkNetwork()) {
                setLoading(true)
                const jsonFilter = {
                    "fromDate": `${filter.year}${filter.month > 9 ? filter.month : "0" + filter.month}01`,
                    "supId": null,
                    "employeeId": null
                }
                const result = await TimeSheetAPI.GetPGTimeSheet(JSON.stringify(jsonFilter))
                if (result.statusId === 200) {
                    ...
                    setDataTS(_table)
                    await AsyncStorage.setItem(TIMESHEET.DATA_SHEET, JSON.stringify(result.data?.table))
                } else {
        ...
        if (masterLocal === null) {
            ...
            if (masterlist.statusId === 200) {
                await AsyncStorage.setItem(TIMESHEET.MASTERDATA, JSON.stringify(masterlist.data || '{}'))
            }
        }
    }
    ...
    const rowItem = ({ item, index }) => {
        ...
        return (
            <View key={`${index}4q9bye`} style={{ backgroundColor: appcolor.surface }}>
                <View style={{ color: appcolor.dark, flexDirection: 'row' }}>
                    {userinfo.groupType === 'SUP' && <Text style={{ flexGrow: 1, marginLeft: 12, padding: 7, color: appcolor.dark, fontWeight: '700' }}>
                        {item.employeeCode} {item.employeeName}
                    </Text>
                    }
                    {
                        userinfo.groupType === 'SUP' &&
                        <TouchableOpacity onPress={() => onEmployeePress(item, eData)} style={{ padding: 7 }}>
                            <Text style={{ color: appcolor.dark, fontSize: 12, fontStyle: 'italic', textAlign: 'right' }}>Xem tất cả</Text>
                        </TouchableOpacity>
                    }
                </View>
                ...
                <ScrollView showsHorizontalScrollIndicator={false} horizontal style={{
                    backgroundColor: appcolor.light, borderRadius: 5, marginLeft: 10, marginEnd: 10
                    , flexDirection: 'row'
                }}>
                    {
                        userinfo.groupType === 'SUP' ?
                            eData?.map((ditem, i) => {
                                if (ditem.week !== displayWeek.value)
                                    return
                                return (
                                    <View key={`${i}$hiaj${ditem.employeeId}`}
                                        style={{
                                            width: deviceWidth / 7, padding: 3, alignItems: 'center',
                                            borderRightColor: appcolor.surface,
                                            borderRightWidth: 1, opacity: ditem.shift === '' ? 0.6 : 1
                                            , backgroundColor: ditem.shift === '' ? appcolor.grey : ditem.color !== '' ? ditem.color : appcolor.light
                                        }}>
                                        <TouchableOpacity onPress={() => {
                                            onDayPress(ditem)
                                        }}>
                                            <Text style={{ color: appcolor.dark, fontSize: 10, fontWeight: 'bold', textAlign: 'center' }}>
                                                {moment(ditem.workDate, 'YYYY-MM-DD').format('dd/DD')}
                                            </Text>
                                            {
                                                ditem.shift !== '' ?
                                                    <View style={{}}>
                                                        <Badge textStyle={{ color: appcolor.light }} value={ditem.shift}
                                                            badgeStyle={{ backgroundColor: appcolor.dark, borderWidth: 0 }} />
                                                        <Text style={{ color: appcolor.dark, textAlign: 'center', fontSize: 10, fontWeight: '600' }}>{ditem.totalTime}</Text>
                                                        <Text style={{ color: appcolor.dark, textAlign: 'center', fontSize: 10, fontWeight: '600' }}>{ditem?.ot > 0 ? ditem?.ot : ''}</Text>
                                                    </View>
                            ...
                            : eData?.length > 0 &&
                            <MonthPGView info={item}
                                gridColl={7} onPressDay={onDayPress}
                                eData={eData} />
                    }
                </ScrollView >
            </View >
    ...
    const onConfirm = async (info) => {
        ...
        const resultCheck = await TimeSheetAPI.PostCheckTimeSheet(JSON.stringify(confirm))
        if (resultCheck.statusId === 500) {
            ...
            if (result.statusId === 200) {
                ...
                if (info?.photoTimeSheet?.length > 0)
                    UploadController.PostFile()
                const _empData = dataTS.filter(a => a.employeeId === rowInfo.employeeId)[0]
                if (_empData === undefined) {
                    setLoading(false)
                    return
                }
                let detailEdit = JSON.parse(_empData.detail)
                const dayIndex = detailEdit.findIndex(a => a.workDate === rowInfo.workDate)
                detailEdit[dayIndex] = rowInfo
                _empData.detail = JSON.stringify(detailEdit)
                const empIndex = dataTS.findIndex(e => e.employeeId === rowInfo.employeeId)
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.surface }}>
            <HeaderCustom
                ...
                iconMiddle="searchengin" rightFunc={() => onGuid()}
                iconRight="info"
            ...
            <ActionSheet
                id="sheetsearch"
                containerStyle={{ backgroundColor: appcolor.light, maxHeight: deviceHeight * 0.75 }}
                headerAlwaysVisible
                gestureEnabled
            >
                <ScrollView contentContainerStyle={styles.searchSheetContent}>
                    ...
                    <View style={styles.weekWrap}>
                        {
                            listWeek.map((w, index) => {
                                return (
                                    <View key={`wea${index}`} style={[styles.weekItem, {
                                        backgroundColor: w.selected === 1 ? appcolor.primary : appcolor.surface,
                                    }]}>
                                        <TouchableOpacity onPress={() => onDisplayWeek(w)}>
                                            <Text style={[styles.weekText, { color: w.selected === 1 ? appcolor.white : appcolor.dark }]}>{w.name}</Text>
                                        </TouchableOpacity>
                                    </View>
                                )
                            })
                    ...
                    <View style={[styles.applyButtonWrap, { backgroundColor: appcolor.surface }]}>
                        <TouchableOpacity onPress={() => onLoad(true)} style={styles.applyButton}>
                            <Icon name="search" color={appcolor.primary} />
                            <Text style={{ color: appcolor.primary, fontWeight: '600', textAlign: 'center', padding: 7 }}>Áp dụng</Text>
                        </TouchableOpacity>
                    </View>
                </ScrollView>
            </ActionSheet>
            <ActionSheet containerStyle={{ backgroundColor: appcolor.surface, }} headerAlwaysVisible gestureEnabled id="sheetdetail">
                <ScrollView>
                    <MonthPGView info={eSelected}
                        gridColl={7} onPressDay={onDayPress}
                        eData={detailEmployee} />
                </ScrollView>
            </ActionSheet>
            <ActionSheet containerStyle={{ backgroundColor: appcolor.surface, }} headerAlwaysVisible id="sheetconfirm">
                <TouchableOpacity onPress={() => SheetManager.hide("sheetconfirm")} style={{ padding: 3, position: 'absolute', top: 10, right: 10, zIndex: 12 }}>
                    <Icon color={appcolor.primary} name="close" />
                </TouchableOpacity>
                <LoadingView styles={{ marginLeft: '35%', marginTop: 100, zIndex: 20, position: 'absolute' }} isLoading={loading} />
                <ConfirmView onLock={(info) => onConfirm(info)} shiftCol={3} data={dayData} />
            </ActionSheet>
