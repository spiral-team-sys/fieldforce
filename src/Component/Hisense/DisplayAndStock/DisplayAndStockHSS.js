...
import { getDataHistoryDisplayByShop, getDisplayProduct, getlistTabCompetitor } from '../../../Controller/DisplayController'
import { getAllPhotosUpload, getDisplayResult, getPhotosReport, getPhotosReportByCate } from "../../../Controller/WorkController";
...
export const DisplayAndStockHSS = ({ navigation, route }) => {
    ...
    const listInput = lstReport?.listInput?.length ? lstReport?.listInput : [
    ...
    const containsEmpty = (item, query) => {
        if ((lstReport?.listInput?.length || 0) == 0)
            return false
        for (let indexL = 0; indexL < lstReport?.listInput?.length || 0; indexL++) {
            const itemL = lstReport?.listInput[indexL]
            if (item[itemL.displayType] == null || item[itemL.displayType] == undefined || item[itemL.displayType] == 0)
                return true
        }
        return false;
    };
    ...
    const onValidatedPhoto = async (listTab, resDisplay, displayByQuantity) => {
        ...
        if (lstReport.isNeedShelve == 1) {
            ...
            for (let index = 0; index < listTab.length; index++) {
                ...
                const listByTab = dataByCate.filter(it => it.categoryId == itT.categoryId && it.quantityShelves !== null && it.quantityShelves !== 'null' && it.quantityShelves !== "")
                if (listByTab.length == 0) {
                    noteStr += `Bạn chưa nhập quầy kệ ngành hàng : ${itT.categoryName} !!`
                    ToastError(noteStr);
                    return false
                }
                const listCheckPhoto = dataByCate.filter(it => it.categoryId == itT.categoryId && it.quantityShelves !== null && it.quantityShelves !== 'null' && it.quantityShelves !== "" && it.quantityShelves !== 0)
                if (listCheckPhoto.length > 0) {
                    if (tempReport[itT.displayRef]?.length > 0) {
                        ...
                        for (let indexT = 0; indexT < itemTemp.length; indexT++) {
                            ...
                            if (lstPhoto.length < itLT.numberIMG && itLT.nameVN.includes('Quầy kệ')) {
                                noteStr += itT.categoryName + ': Vui lòng chụp tối thiểu ' + itLT.numberIMG + ' tấm hình cho ' + itLT.nameVN + '!!';
                                ToastError(noteStr);
                                return false;
                            }
                        }
        ...
        if (lstReport?.imageByCate == 1 && lstReport?.isCheckByProduct == 1) {
            ...
            let satisfyConditions = filter(displayByQuantity, item => {
                ...
                if (item.quanity >= (numCheckProduct || 3) && (checkHistory || [])?.length == 0)
                    return true
                else return false
            });

            if (lstReport?.isTakeTemplate == 1 && lstReport?.imageByCate == 1 && satisfyConditions?.length > 0) {
                for (let index = 0; index < listTab.length; index++) {
                    const itT = listTab[index]
                    ...
                    if (listByCateT?.length > 0) {
                        let lstPhoto = await getPhotosReportByCate(kpiinfo.kpiId, '_' + itT.displayRef, workinfo.shopId, workinfo.workDate);
                        if (lstPhoto.length < (lstReport.numPhotoByProduct || 1)) {
                            noteStr += `Vui lòng chụp tối đa ${lstReport.numPhotoByProduct || 1} tấm hình cho ${itT.categoryName}. (${lstPhoto.length}/${lstReport.numPhotoByProduct || 1})`
                            ToastError(noteStr);
                            return false
                        }
                    }
                ...
                for (let index = 0; index < listTab.length; index++) {
                    ...
                    if (listByCate?.length > 0) {
                        let lstPhoto = await getPhotosReport(kpiinfo.kpiId, itT.displayRef, workinfo.shopId, workinfo.workDate);
                        if (lstPhoto.length < (lstReport.numPhotoByProduct || 1)) {
                            noteStr += `Vui lòng chụp tối đa ${lstReport.numPhotoByProduct || 1} tấm hình cho ${itT.categoryName}. (${lstPhoto.length}/${lstReport.numPhotoByProduct || 1})`
                            ToastError(noteStr);
                            return false
                ...
                if (lstReport?.isTakeTemplate == 1) {
                    if (numLimitPhoto !== undefined && numLimitPhoto > 0) {
                        if (lstReport?.imageByCate == 1) {
                            for (let index = 0; index < (lstReport?.isCheckByDisplay == 1 ? displayByQuantity.length : listTab.length); index++) {
                                const itT = (lstReport?.isCheckByDisplay == 1 ? displayByQuantity[index] : listTab[index])
                                let lstPhoto = await getPhotosReportByCate(kpiinfo.kpiId, '_' + itT.displayRef, workinfo.shopId, workinfo.workDate);
                            ...
                            if (LstMenuPhotos[itT.displayRef]?.length > 0) {
                                ...
                                for (let index = 0; index < lstTem.length; index++) {
                                    ...
                                    if (lstPhoto.length < itL.numberIMG) {
                                        noteStr += itT.categoryName + ': Vui lòng chụp ' + itL.numberIMG + ' tấm hình cho ' + itL.nameVN + ', ';
                                        ToastError(noteStr);
                                        return false;
                                    }
                                }
    ...
    const onValidatedData = (listTab, listProduct, resDisplay, displayByQuantity) => {
        // let noteStr = '';
        if (displayByQuantity.length == 0 && lstReport?.isDisplayBlank !== 1) {
            ...
            for (let index = 0; index < resDisplay.length; index++) {
                ...
                for (let indexL = 0; indexL < lstReport?.listInput?.length || 0; indexL++) {
                    const itemL = lstReport?.listInput[indexL]
                    if ((item[itemL.displayType] == null || item[itemL.displayType] == undefined || (item[itemL.displayType] == 0 && itemL.isZero !== 1)) && itemL.isRequired == 1) {
                        ToastError(`Chưa nhập ${itemL.name} sản phẩm ${item.productName} - ${item.categoryName}`, 'Thông báo', 'top')
                        return false
                    }
                    if (item[itemL.displayType] !== null && item[itemL.displayType] !== undefined && (item[itemL.displayType] !== 0 || (item[itemL.displayType] == 0 && itemL.isZero !== 1)) &&
                        (item[itemL.displayType] < (((itemL.min || itemL.min == 0) && itemL.min !== '') ? itemL.min : 1000)
                            || item[itemL.displayType] > ((itemL.max && itemL.max !== '') ? itemL.max : 1000000000)
                            || item[itemL.displayType] % ((itemL.min && itemL.min !== '') ? (itemL.min || 1) : 1000 > 0)
                        )
                    ) {
                        ToastError(`Sai định dạng ${itemL.name} sản phẩm ${item.productName} - ${item.categoryName}`, 'sản phẩm', 'top')
                        return false
                    }
                }
                if (lstReport.isCheckNote == 1 && lstReport.isNoteBySKU == 1) {
                    for (let iNote = 0; iNote < lstReport?.listNote || 0; iNote++) {
                        ...
                        if ((item[itemNote.noteType] == null || item[itemNote.noteType] == '') && item.quanity !== null && item.quanity >= 0 && itemNote.isRequired == 1) {
                            ToastError(`Chưa nhập ${itemNote.name} ${item.productName} ngành hàng ${item.categoryName}`, 'Ghi chú', 'top')
                            return false
                        }
                    }
                }
            ...
            if (lstReport.isNoteBySKU == 1) {
                ...
                for (let indexP = 0; indexP < resDisplayEmpty.length; indexP++) {
                    ...
                    for (let iNote = 0; iNote < listNote.length || 0; iNote++) {
                        ...
                        for (let indexL = 0; indexL < lstReport?.listInput?.length || 0; indexL++) {
                            const itemL = lstReport?.listInput[indexL]
                            if (itemNote.subCode == itemL.id && (itemP[itemNote.noteType] == null || itemP[itemNote.noteType] == '') &&
                                (itemP[itemL.displayType] == null || itemP[itemL.displayType] == undefined || itemP[itemL.displayType] == 0)) {
                                ToastError(`Chưa nhập ${itemNote.name} ${itemP.productName} ngành hàng ${itemP.categoryName}`, 'Ghi chú', 'top')
                                return false
                            }
                        }
                    }
    ...
    const uploadAction = async () => {
        ...
        const listProduct = await getDisplayProduct(workinfo, _competitorId, lstReport, (shopinfo?.dealerId || 0))
        ...
        let displayByQuantity = resDisplay.filter(it => it.quanity !== null && it.quanity !== 'null')
        let resPhotos = await getAllPhotosUpload(kpiinfo.kpiId, workinfo.shopId, workinfo.workDate);
        let noteStr = '';
        const checkValidatedPhoto = await onValidatedPhoto(listTab, resDisplay, displayByQuantity)
        if (checkValidatedPhoto) {
            const checkValidatedData = await onValidatedData(listTab, listProduct, resDisplay, displayByQuantity)
                    ...
                    try {
                        ...
                        if (numCheckProduct) {
                            ...
                            if (satisfyConditions.length > 0) {
                                Message('Chú ý', `Sản phẩm trưng bày mới nhập có số lượng >= ${numCheckProduct}, bạn có chắc chắn muốn gửi không?`, () => UploadData(listProduct));
                            } else {
                                Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', () => UploadData(listProduct));
                            }
    ...
    const ViewItemInput = () => {
        return (
            <InputDisplayStockHSS Status={settings.isUploaded ? 1 : 0} navigation={navigation}
                listInput={listInput} reloadView={reloadView} lstReport={lstReport} />
        );
    };
    const ViewItemPhoto = () => {
        return (
            ...
            />
            {
                lstReport?.isTakeByCate == 1 ?
                    settings.isLoading == false && (
                        <InputDisplayStockHSS Status={settings.isUploaded ? 1 : 0} navigation={navigation} listInput={listInput} reloadView={reloadView} lstReport={lstReport} />
                    )
                    :
                    (routes.length > 0 && settings.isLoading == false) && (
                        <TabForm
