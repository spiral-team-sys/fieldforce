...
import { FlatList, Image, LayoutAnimation, Modal, Platform, RefreshControl, SafeAreaView, Text, TouchableOpacity, UIManager, View } from "react-native";
import { useSelector } from "react-redux";
import FormGroup from "../../../Content/FormGroup";
import { HeaderCustom } from "../../../Content/HeaderCustom";
import { itemUploaded } from "../../../Controller/ReportController";
import { groupDataByKey, MessageAction, MessageInfo, ToastSuccess, UUIDGenerator } from "../../../Core/Helper";
...
export const IssueReportHome = ({ navigation, route }) => {
    ...
    const [data, setData] = useState({ dataTab: [], dataMain: [], dataShow: [], listIssueType: [] })
    ...
    const loadData = async () => {
        ...
        await GetDataIssueReport(dataFilter, async (mData, mesager) => {
            ...
            await setData({ dataTab: listTab, dataMain: dataIssueSort, dataShow: dataIssueSort })
        ...
        return (
            <View style={{}}>
                <TouchableOpacity
                    ...
                    <View style={{ width: '75%', justifyContent: 'center', paddingHorizontal: 10, color: appcolor.dark }}>
                        <Text>Vấn đề của bạn là gì?</Text>
                    </View>
                    <View style={{ flexDirection: 'row', width: '10%', justifyContent: "center", alignItems: 'center' }}>
                        <TouchableOpacity
                            style={{ backgroundColor: appcolor.surface, borderRadius: 40, justifyContent: 'center', alignItems: 'center', width: 30, height: 30 }}
                        >
            ...
            <View >
                {
                    ...
                    <View style={{ backgroundColor: appcolor.grayLight }}>
                        <View style={{ padding: 8, borderTopEndRadius: 10, borderTopStartRadius: 10, flexDirection: 'row', backgroundColor: appcolor.primary }}>
                            <Icon color={appcolor.white} name='store-alt' type='font-awesome-5' size={18} />
                            <Text style={{ color: appcolor.white, fontWeight: '700', fontSize: 16, paddingHorizontal: 10 }}>{`${item.shopNameVN}`}</Text>
                        </View>
                    </View>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 6, alignItems: "center" }}>
                    <View style={{ paddingLeft: 5 }}>
                        <Text style={{ color: appcolor.primary, fontWeight: '600', fontSize: 16 }}>{moment(item.reportDate?.toString() || new Date()).format("YYYY-MM-DD")}</Text>
                    </View>
                    {
                        item.isEditItem == 1 &&
                        <TouchableOpacity
                ...
                <View style={{ height: 1, width: deviceWidth - 40, marginHorizontal: 20, backgroundColor: appcolor.grayLight }} />
                <View style={{ height: 30, width: deviceWidth, flexDirection: 'row' }}>
                    <TouchableOpacity
                        onPress={() => handleSelectDecription(item, index)}
                        style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ fontWeight: '600', fontSize: 14, color: appcolor.dark }}>Thông tin theo dõi</Text>
                    </TouchableOpacity>
    ...
    const ViewItemIssue = () => {
        const listByTab = data.dataShow?.filter(it => it.issueStatus == currentTab.id)
        return (
            <View style={{}}>
        ...
        return (
            index < 7 ?
                <TouchableOpacity
                    ...
                    style={{ width: deviceWidth / 3, height: deviceWidth / 3, alignItems: index == 1 || index == 4 ? 'center' : 'baseline', marginBottom: 2 }}>
                    <Image source={{ uri: item.photoPath.includes('uploaded') ? (URLDEFAULT + item.photoPath) : (item.photoPath || '') }} style={{ width: '99%', marginRight: index == 0 || index == 3 ? 6 : 0, marginLeft: index == 2 || index == 5 ? 2 : 0, height: '99%', backgroundColor: appcolor.surface }} />
                    {
                        (index == 5 && listImage.length - 5 > 1) &&
                        <View style={{ position: 'absolute', top: 0, bottom: 0, right: 0, left: 0, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.5)' }}>
                            <Text style={{ fontSize: 20, color: appcolor.white, fontWeight: '500' }}>{listImage.length - 5}</Text>
                        </View>
                    }
                </TouchableOpacity>
                : null
...
const ViewCreateIssue = ({ guiIdItem, handleSelectQuestion, lstReport, data, itemSelect }) => {
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.light }} >
            <View style={{ flexDirection: 'row', padding: 5, borderBottomWidth: 1.5, borderBottomColor: appcolor.grayLight }}>
                ...
                <View style={{ flexDirection: 'row', padding: 10 }}>
                    ...
                    </TouchableOpacity>
                    {
                        ...
                        <TouchableOpacity
                            ...
                            style={{ borderRadius: 5, borderWidth: 0.6, borderColor: appcolor.dark, padding: 5, flexDirection: 'row', justifyContent: 'flex-end', alignItems: "center" }}
                        >
                            <Text style={{ fontSize: 13, color: appcolor.dark, fontWeight: '300', paddingHorizontal: 10 }}>{'Loại vấn đề'}</Text>
                            <Icon name="angle-down" type="font-awesome-5" size={15} color={appcolor.primary} />
                        </TouchableOpacity>
                    }
            ...
            <ActionSheet
                id='testSheet'
            >
                <View style={{ backgroundColor: 'red', height: deviceWidth, width: deviceWidth }}></View>
            </ActionSheet>
            <ActionSheet
                id={'imageSheetModal'}
                containerStyle={{ height: deviceHeight, width: deviceWidth, backgroundColor: appcolor.light }}
