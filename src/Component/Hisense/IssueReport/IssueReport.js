...
export const IssueReport = ({ navigation, route }) => {
  ...
  const checkHashTag = dataItem => {
    if (
      dataItem.issueType != undefined &&
      itemSelect.itemIssue?.issueType?.length > 0
    ) {
    ...
    return (
      <View style={{}}>
        ...
        >
          ...
          >
            <Text
              style={{ fontWeight: '500', fontSize: 14, color: appcolor.dark }}
            >
      ...
      await IssueReportUpload(
        ...
        async result => {
          if (result.statusId === 200) {
            ...
            MessageInfo(
              `Xảy ra lỗi khi thay đổi trạng thái vấn đề!` + result.messager,
              'Lỗi',
              'top',
            );
          }
  ...
  const renderItemIssue = ({ item, index }) => {
    const hashtagArr =
      item.issueType != undefined ? item.issueType.split(',') : [];
        ...
        >
          <View
            style={{
              ...
              flexDirection: 'row',
              alignItems: 'center',
              width: item.isEditItem == 1 ? '90%' : '100%',
            }}
          >
            <Text
              style={{
  ...
  return (
    <View style={{ flex: 1, backgroundColor: appcolor.light }}>
      ...
      >
        <SafeAreaView
          style={{ height: deviceHeight, backgroundColor: 'rgba(0,0,0,0.2)' }}
        >
          <TouchableOpacity
      ...
      >
        {listImage.length == 1 ? (
          ...
          >
            <Image
              source={{
                uri: listImage[0].photoPath?.includes('uploaded')
                  ? URLDEFAULT + listImage[0]?.photoPath
                  : listImage[0].photoPath || '',
              }}
              style={{
                width: '100%',
                height: '100%',
...
const ViewCreateIssue = ({
  ...
  const handleCreateIssue = () => {
    ...
    const dataUpload = {
      ...
      guiid: guiIdItem,
      issueType:
        itemHashtag.textHashtag !== '' ? itemHashtag.textHashtag : null,
      issueStatus: 1,
  ...
  const onSelectItemSheet = itemSelect => {
    if (!itemHashtag.textHashtag?.includes(itemSelect.nameVN)) {
      ...
      if (itemHashtag.textHashtag?.includes(`, ${itemSelect.nameVN}`)) {
        itemHashtag.textHashtag = itemHashtag.textHashtag.replace(
          `, ${itemSelect.nameVN}`,
          '',
        );
