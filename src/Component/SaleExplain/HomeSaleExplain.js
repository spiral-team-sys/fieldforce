...
import { View, Text, StyleSheet, TouchableOpacity, SectionList, ScrollView, ActivityIndicator, Platform, } from "react-native";
import { HeaderCustom } from "../../Content/HeaderCustom";
import { MessageInfo, UUIDGenerator } from "../../Core/Helper";
import { REPORT } from "../../API/ReportAPI";
import { useSelector } from "react-redux";
import ActionSheet, { SheetManager } from "react-native-actions-sheet";
import { TODAY, alertConfirm, checkNetwork, deviceWidth } from "../../Core/Utility";
...
const groupByWeek = (list = []) => {
    ...
    return Object.keys(grouped)
        ...
        .map((title) => ({
            title,
            data: grouped[title].sort((a, b) => new Date(b.salesDate) - new Date(a.salesDate)),
        }));
};
...
const HomeSaleExplain = ({ navigation, route }) => {
    ...
    const initialFilter = routeFilter
        ? {
            ...defaultFilter,
            ...routeFilter,
            monthname: routeFilter.monthname || `Tháng ${routeFilter.month || defaultFilter.month}`,
            yearname: routeFilter.yearname || `Năm ${routeFilter.year || defaultFilter.year}`,
    ...
    const loadData = async (reload, selectedFilter = filter) => {
        try {
            setLoading(true);
            const cacheData = reload || employee?.employeeId > 0
                ? selectedFilter
            ...
            const params = { reportId: kpiinfo.id, month, year, userId: employee?.employeeId || null, typeReport: 'GET_DATA' }
            ...
            await REPORT.GetDataReportByShop_RealTime(params, async (mData) => {
                ...
                const parsedDataTab = normalizeDataTab(JSON.parse(dataResult.dataTab || '[]'));
    ...
    const summary = useMemo(() => getStatusSummary(data, dataTab), [data, dataTab]);
        ...
        alertConfirm(
            ...
            async () => {
                try {
                    ...
                    const result = await REPORT.UploadDataRaw_Realtime(payload, shop, kpiinfo.id);
                    ...
                    if (result.statusId == 200) {
                        ...
                        setData(newData);
                        onCloseDetail();
                        MessageInfo(result.messager || "Gửi giải trình thành công, đang chờ leader duyệt.");
                    } else {
    ...
    const onLeaderConfirm = async (payload) => {
        alertConfirm('Gửi thông tin', `Bạn có chắc chắn muốn ${payload.confirm == -1 ? 'Từ chối' : 'đồng ý'} giải trình này không ?`, async () => {
            try {
                setSubmitting(true);
                ...
                if (result.statusId == 200) {
                    ...
                    MessageInfo(
                        result.messager ||
                        `Đã ${payload.confirm == -1 ? 'từ chối' : 'đồng ý'} giải trình.`
                    );
                } else {
                    MessageInfo(
                        result.messager ||
                        `Xử lý thất bại, không thể ${payload.confirm == -1 ? 'từ chối' : 'đồng ý'} giải trình.`
                    );
                }
            } catch (error) {
                console.log("onLeaderConfirm error", error);
                MessageInfo(
                    `Có lỗi khi xử lý giải trình, không thể ${payload.confirm == -1 ? 'từ chối' : 'đồng ý'} giải trình.`
                );
            } finally {
                setSubmitting(false);
            }
            ...
            <View style={{ flex: 1, backgroundColor: appcolor.light }}>
                ...
                {loading ? (
                    ...
                    <SectionList
                        ...
                        renderItem={({ item }) => (
                            <VerifyCard item={item} onPress={() => onOpenDetail(item)} colorStatus={colorStatus} lableStatus={lableStatus} dataTab={dataTab} styles={styles} />
                        )}
                        ListEmptyComponent={
                ...
                {
                    !!selectedItem?.guid != null && <VerifyDetailModal
...
const SummaryTabs = ({ summary, selectedStatus, onSelect, dataTab, styles }) => {
    return (
...
const createStyles = (appcolor) => StyleSheet.create({
    ...
    headerWrap: { paddingHorizontal: 16, paddingTop: 14, paddingBottom: 8, backgroundColor: appcolor.light, },
    headerTitle: { fontSize: 22, fontWeight: "700", marginBottom: 12, color: appcolor.dark, },
    row: { flexDirection: "row", alignItems: "center", marginBottom: 8, },
    filterChip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, backgroundColor: appcolor.surface, marginRight: 8, },
    tabsWrap: { height: 50, paddingHorizontal: 12, paddingVertical: 12, backgroundColor: appcolor.light, marginBottom: 8, },
    tabItem: { paddingHorizontal: 14, paddingVertical: 10, backgroundColor: appcolor.surface, borderRadius: 18, marginRight: 8, },
    ...
    card: {
        backgroundColor: appcolor.light, marginHorizontal: 16, marginBottom: 12, borderRadius: 16,
        padding: 14, shadowColor: appcolor.dark, shadowOpacity: 0.05, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: 2,
    },
    cardTop: { flexDirection: "row", justifyContent: "space-between", marginBottom: 10, alignItems: "center", },
    cardDate: { fontSize: 14, fontWeight: "700", color: appcolor.dark, },
    cardModel: { fontSize: 16, fontWeight: "700", color: appcolor.dark, marginBottom: 6, },
    cardSub: { fontSize: 13, color: appcolor.greylight, marginBottom: 3, },
    cardReason: { marginTop: 6, fontSize: 13, color: appcolor.highlightDate, fontWeight: "600", },
    ...
    modalContainer: { flex: 1, width: '100%', backgroundColor: appcolor.surface, position: "relative" },
    modalHeader: {
        paddingTop: Platform.OS === "ios" ? 54 : 18, paddingHorizontal: 16, paddingBottom: 14,
        backgroundColor: appcolor.light, flexDirection: "row", justifyContent: "space-between", alignItems: "center", flexShrink: 0,
    },
    ...
    infoBlock: { backgroundColor: appcolor.light, marginTop: 12, marginHorizontal: 16, borderRadius: 16, padding: 14, },
    ...
    infoValue: {
        fontSize: 14, color: appcolor.dark, fontWeight: "600",
    },
    infoValueRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", },
    label: { fontSize: 14, color: appcolor.dark, fontWeight: "700", marginBottom: 8, marginTop: 8, },
    input: { backgroundColor: appcolor.surface, borderWidth: 1, borderColor: appcolor.danger, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 12, fontSize: 14, color: "#111827", },
    textArea: { minHeight: 110, },
    disabledInput: { backgroundColor: appcolor.surface, color: appcolor.greylight, },
    photoRow: { flexDirection: "row", alignItems: "center", },
    photoItem: { width: 100, height: 100, borderRadius: 14, overflow: "hidden", marginRight: 10, position: "relative", backgroundColor: appcolor.surface, },
    photoImage: { width: "100%", height: "100%", },
    removePhotoBtn: { position: "absolute", top: 6, right: 6, width: 24, height: 24, borderRadius: 12, backgroundColor: "rgba(0,0,0,0.7)", justifyContent: "center", alignItems: "center", },
    removePhotoText: { color: appcolor.light, fontWeight: "700", },
    addPhotoBox: { width: 100, height: 100, borderRadius: 14, borderWidth: 1.5, borderStyle: "dashed", borderColor: appcolor.greylight, justifyContent: "center", alignItems: "center", },
    ...
    primaryButton: { backgroundColor: appcolor.info, borderRadius: 14, minHeight: 48, paddingVertical: 10, },
    ...
    employeeInfoWrap: { backgroundColor: appcolor.light, margin: 10, padding: 8, borderRadius: 16, },
