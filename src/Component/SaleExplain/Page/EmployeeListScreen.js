...
const EmployeeListScreen = ({ navigation }) => {
  ...
  const _searchData = filterList => {
    ...
    const searchData = _.filter(filterList, item => {
      ...
      const code = removeVietnameseTones(
        (item.employeeCode || '').toLowerCase(),
      );
      const shop = removeVietnameseTones((item.shopName || '').toLowerCase());

      return (
        name.includes(valueSearch) ||
        code.includes(valueSearch) ||
        shop.includes(valueSearch)
      );
    });
    return searchData;
  };
  ...
  const showMonth = async () => {
    ...
    if (
      itemCache &&
      (itemCache.month != filter.month || itemCache.year != filter.year)
    ) {
      await AsyncStorage.setItem(
        KeyStore,
        JSON.stringify(buildFilterCache(filter)),
      );
    }
