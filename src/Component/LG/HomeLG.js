...
import { View, AppState, SafeAreaView, ScrollView, RefreshControl, LogBox, DeviceEventEmitter, Modal, } from 'react-native';
import { defaultSetting, ManageMessenger, GetEmployeeInfo, deviceSize, ToastSuccess, GetToken } from '../../Core/Helper';
...
import { GetMenu, RemoveUser, SetRequestReportAction } from '../../Controller/UserController';
...
const HomeTab = forwardRef((props, ref) => {
  ...
  const countItem = async () => {
    let countItem = count
    userinfo.employeeId > 0 && 'pm'.includes(userinfo.groupType.toLowerCase()) && countItem++
    userinfo.employeeId > 0 && 'sup,leader'.includes(userinfo.groupType.toLowerCase()) && countItem++
    ('asm,pg,mer,leader'.includes(userinfo.groupType.toLowerCase())) && countItem++
    ...
    <Tabs.Container
      ...
      renderTabBar={props => (
        <MaterialTabBar
          ...
          scrollEnabled={true}
          style={{ backgroundColor: appcolor.light, borderTopLeftRadius: 20, borderTopRightRadius: 20 }}
        />
      )}
      containerStyle={{ backgroundColor: appcolor.surface }}>
      {
        userinfo.employeeId > 0 && 'pm'.includes(userinfo.groupType.toLowerCase()) &&
      ...
      {
        userinfo.employeeId > 0 && ('sup,leader').includes(userinfo.groupType.toLowerCase()) &&
      ...
      }
      {
        ...
        <Tabs.Tab label="Hôm nay" name="Hôm nay">
          <View style={{ backgroundColor: appcolor.light, marginTop: 40, width: deviceWidth }}>
            <ShopList shops={shops} header="none" navigation={navigation} loading={loading} />
          </View>
        </Tabs.Tab>
      }
      {
...
class HomeLG extends PureComponent {
  ...
  goHyperlink = async (hyperLinks) => {
    ...
    if (hyperLinks == undefined || hyperLinks === null) {
      ...
      await this.props.navigation.navigate('WebView', { link: webURL, titlePage: 'Trình duyệt' });
      ...
      await this.props.navigation.navigate('WebView', { link: hyperLinks, titlePage: 'Trình duyệt' });
    } else if (hyperLinks === "")
  ...
  SyncdataApp = async (serverSync) => {
    ...
    if (serverSync === true || _hour < 18) {//đồng bồ trên hệ thống truoc 18h || or tự đồng bộ
      ...
      this.state.welcome === 1 ?
        ...
        userinfo.employeeId > 0 ?
          <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.light }}>
            <LGHeader appcolor={appcolor}
              isHome={true} navigation={navigation}
              title={userinfo.employeeName}
              countNotify={this.state.badgeNotify}
              iconLeft='bars' iconRight='bell'
            ...
            <ScrollView
              ...
              />}
            >
              <View style={{ height: 80 + deviceSize.dwidth / 3, borderTopLeftRadius: 30, borderTopRightRadius: 30, zIndex: 3 }}>
                <LGCenterTab navigation={navigation} />
              </View>
            </ScrollView>
            <View style={{ backgroundColor: appcolor.primary, height: 50, marginTop: -20, zIndex: -1 }} />
            <View style={{
              flexGrow: 10000, marginTop: -30,
              borderTopRightRadius: 40,
              borderTopLeftRadius: 30, backgroundColor: appcolor.light,
            }}>
              <HomeTab
                appcolor={appcolor}
                userinfo={userinfo}
