...
import { TouchableOpacity, View, Text, FlatList, ScrollView, StyleSheet, ViewBase } from "react-native"
import { DataSummary, LGPLanbyWeekly } from "../../Controller/DashboardController"
...
export const LGCenterTab = ({ navigation }) => {
    ...
    const onLoad = async (pageNum, date) => {
        ...
        const result = (jsonStr === null || _hour !== 21) ? await LGPLanbyWeekly(sFilter) : localData
        if (result.statusId === 200) {
            let _temp = result?.data;
            if (_temp.length === 7) {
                await _temp.unshift({ "dayName": "arrow-left", "pageNum": -7, "date": _temp[0].date });
                await _temp.push({ "dayName": "arrow-right", "pageNum": 7, "date": _temp[7].date });
            }
            await setData(_temp)
            if (_temp.length > 5)
                await _flatRef.current.scrollToIndex({ index: 4, animated: true, viewPosition: 0.5 });
    ...
    const LoadDashboard = async (typeGroup) => {
        await setLoading(true)
        typeGroup == typeButton.Attendant && onLoad(0, moment().format('YYYY-MM-DD'));
    ...
    const dayTemple = ({ item, index }) => {
        ...
        return (
            item.pageNum === 7 || item.pageNum === -7 ?
                <TouchableOpacity onPress={() => onLoad(item.pageNum, item.date)} style={{ marginRight: 7 }}>
                    <View style={{
                        borderRadius: 50, alignContent: 'center', alignSelf: 'center',
                        width: 50, padding: 12, justifyContent: 'center', height: 50, backgroundColor: appcolor.light
                    }}>
                        <Icon color={appcolor.primary} type="font-awesome-5" size={20} name={item.dayName} />
                    </View>
                </TouchableOpacity> :
                <TouchableOpacity onPress={() => navigation.navigate("attendanthistory", { planDate: moment(item.date).format('YYYY-MM-DD') })} key={index + "_29"}
                    style={{
                        marginRight: 7, elevation: 5, borderRadius: 12,
                    }}
                >
                    < View style={{
                        shadowOpacity: 0.8, width: deviceWidth / 7,
                        height: dataMenu.length > 0 ? '94%' : '97%',
                        shadowOffset: { width: 0.4, height: 4 }, paddingTop: 7,
                        borderRadius: 12, backgroundColor: dayColor, elevation: 5,
                        justifyContent: 'flex-start', alignItems: 'center'
                    }}>
                        <View style={{ alignItems: 'center', backgroundColor: appcolor.white, borderRadius: 20, minWidth: 33, padding: 2, }}>
                            <Text style={{ textAlign: 'center', color: appcolor.black, fontSize: scaleSize(9) }}>{item.dayName}</Text>
                            <View style={{ borderTopWidth: 1, borderColor: appcolor.grayLight, width: 10, }} />
                            <Text style={{ textAlign: 'center', fontSize: scaleSize(10), color: appcolor.black, }}>{moment(item.date).format("DD")}</Text>

                        </View>
                        <Text style={{ fontSize: scaleSize(11) }}>{item.target || '-'}</Text>
                        <Text style={{ fontSize: scaleSize(9), color: appcolor.white }}>{item.l1}</Text>
                        <Text style={{ fontSize: scaleSize(11) }}>{item.i}</Text>
                        <Text style={{ fontSize: scaleSize(9), color: appcolor.white }}>{item.l2}</Text>
    ...
    const RenderButton = ({ }) => {
        ...
        return (
            <ScrollView
                pagingEnabled style={{ position: 'absolute', bottom: -15 }}
                horizontal showsHorizontalScrollIndicator={false} >
                <View style={{ flexDirection: 'row', justifyContent: 'center', bottom: 0, zIndex: 10 }}>
                    <Button
                        key={'attendant'}
    ...
    return (
        <View style={{ height: '100%' }}>
            <LoadingView styles={{ zIndex: 100, position: 'absolute', top: '10%', left: '40%' }} isLoading={loading} />
            <View style={{ backgroundColor: appcolor.primary, borderTopLeftRadius: 30, borderTopRightRadius: 30 }}>
                <Fragment>
                    {
                        dashboardItem !== typeButton.Routing && <FlatList data={data}
                            ref={_flatRef}
