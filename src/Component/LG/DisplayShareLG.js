...
export const DisplayShareLG = ({ navigation, route }) => {
  ...
  const editInputChange = async (item, index, e) => {
    ...
    await insertTotalTrackingLG(
      workinfo,
      item,
      numberInput !== '' && numberInput !== null && numberInput !== undefined
        ? parseInt(numberInput)
        : null,
    );
    arrDataShow[indexD].display =
      numberInput !== '' && numberInput !== null && numberInput !== undefined
        ? parseInt(numberInput)
        : null;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: appcolor.light }}>
      ...
      {arrTagShow.length > 0 && !loading && (
        ...
        >
          {arrTagShow.map((it, i) => {
            let dataByCategoryId = arrDataShow.filter(
              data => data.categoryId == it.categoryId,
            );
            ...
            return (
              <Tabs.Tab
                key={i}
                label={
                  it.category + `${totalRow !== 0 ? ` (${totalRow})` : ''}`
                }
