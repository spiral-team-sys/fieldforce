...
import { groupDataByKey, Message, MessageAction, MessageInfo, ToastError, ToastSuccess } from "../../Core/Helper"
import { checkNetwork, deviceHeight, deviceWidth } from "../../Core/Utility"
import { ItemDecorForm } from "./ItemDecorForm"
const { useState, useEffect } = require("react")
const { View, VirtualizedList, Text, StyleSheet, TouchableOpacity, TextInput, FlatList, Modal, Image } = require("react-native")
const { REPORT } = require("../../API/ReportAPI")
const tabs = [{ "key": "listDC", "name": 'Decor' }, { "key": "listQK", "name": 'Quầy kệ' }];
...
const DecorForm = ({ navigation }) => {
    ...
    const onLoad = async () => {
        ...
        if (localStore === null) {
            ...
            if (result.statusId === 200) {
                ...
                const dataAll = { listQK: _qkList.sort((a, b) => a.cate - b.cate && a.groupId - b.groupId), listDC: arr }
    ...
    const RowDecor = ({ item, index, lastitem }) => {
        ...
        return (<View style={{ marginLeft: 7, marginRight: 7 }}>
            {
                (index == 0 || item?.groupId !== lastitem?.groupId) &&
                <TouchableOpacity style={{ flexDirection: 'row' }}>
                    <Text style={{ flexGrow: 1, padding: 7, color: appcolor.dark, fontWeight: '400', fontSize: 12 }}>{item.groupName}</Text>
                </TouchableOpacity>
            }
            <View style={{ backgroundColor: appcolor.light, padding: 7, borderRadiusTopLef: 12, borderTopRightRadius: 12 }}>
                {
                    (item.isParent) &&
                    <View>
                        <Text style={{ fontWeight: '300', color: appcolor.primary, fontSize: 12, padding: 10 }}>{item.cate}</Text>
                        <View style={{ marginBottom: 7, borderWidth: 1, borderColor: appcolor.surface, width: '100%' }} />
                    </View>
                }
                <ItemDecorForm key={'itemDecor_' + index} item={item} index={index} template={template} data={data} KeyStore={KeyStore} />
            </View>
        </View >)
    }
    const RowShelf = ({ item, index, lastitem }) => {
        const template = JSON.parse(item.template)
        return (<View style={{ marginRight: 7, marginLeft: 7 }}>
            {
                (index == 0 || item.cate !== lastitem?.cate) &&
                <TouchableOpacity style={{ flexDirection: 'row', }}>
                    <Text style={{ flexGrow: 1, fontWeight: '400', padding: 7, color: appcolor.dark }}>{item.cate}</Text>
                </TouchableOpacity>
            }
            <View style={{ backgroundColor: appcolor.light, padding: 7, borderRadiusTopLef: 12, borderTopRightRadius: 12 }}>
                {
                    (index === 0 || item?.groupId !== lastitem?.groupId || (item.cate === lastitem.cate && item?.groupId !== lastitem?.groupId))
                    &&
                    <View>
                        <Text style={{ fontWeight: '700', color: appcolor.primary, fontSize: 12 }}>{item.groupName}</Text>
                        <View style={{ marginBottom: 7, borderWidth: 1, borderColor: appcolor.surface, width: '100%' }} />
                    </View>
                }
                <ItemDecorForm key={'itemQK_' + index} item={item} index={index} template={template} data={data} KeyStore={KeyStore} />
            </View>
        </View>)
    }
    const styles = StyleSheet.create({
        container: { flex: 1, },
        tabsContainer: { flexDirection: 'row', justifyContent: 'center', borderWidth: 1, borderColor: appcolor.grey },
    ...
    const onSummit = async () => {
        ...
        if (countData == 0) {
            ToastError("Bạn chưa nhập bất kì kích thước quầy kệ hay decor nào!!", "Thông báo", 'Top')
            return
        }
        ...
        Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?',
            async () => {
                await UploadController.uploadServer({ ...workinfo, reportId: kpiinfo.id }, jsonUpload, async (result) => {
    ...
    return (
        <View style={{ flex: 1 }}>
            <HeaderCustom title={kpiinfo?.menuName || 'Báo cáo decor'}
                leftFunc={() => navigation.goBack()}
                iconRight={!data.isLockReport ? 'cloud-upload-alt' : 'user-lock'}
                rightFunc={!data.isLockReport ? () => onSummit() : () => {
                ...
                <View style={styles.tabsContainer}>{
                    tabs.map((tab, index) => (
                        <TouchableOpacity
                            ...
                            <Text style={styles.tabText}>{tab.name} ({data[tab.key]?.length || 0})</Text>
                        </TouchableOpacity>
                    ))
                }</View>
                <PagerView
                    ...
                    <View style={styles.page}>
                        <VirtualizedList
                            ...
                            renderItem={({ item, index }) => <RowDecor key={'itemDC_' + index}
                    ...
                    <View style={styles.page}>
                        <VirtualizedList
                            ...
                            getItem={(_data, index) => _data[index]}
                            getItemCount={(_data) => _data?.length}
                            renderItem={({ item, index }) => <RowShelf key={'itemQC_' + index}
            ...
            <ViewSheet appcolor={appcolor} handleClearAll={handleClearAll} handleReloadDataStock={handleReloadDataStock} isLoading={isLoading} />
        </View >
    )
}
const ViewSheet = ({ appcolor, handleClearAll, handleReloadDataStock, isLoading }) => {
