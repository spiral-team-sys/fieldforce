...
export const StatisticalDisplay = ({ navigation }) => {
    ...
    const [itemFilter, setItemFilter] = useState({ groupName: null, search: null, newest: false, headcount: null })
    ...
    const LoadData = async () => {
        try {
            setItemFilter({ groupName: null, search: null, newest: false, headcount: null })
    ...
    const _searchData = (filterList, itemFilter) => {
        let mainDataSearch = _.orderBy(filterList, ['lastUpdate'], [itemFilter.newest ? 'desc' : 'asc']);
        //
        ...
        if (itemFilter.groupName !== null) {
            mainDataSearch = _.filter(mainDataSearch, (e) => e.groupName == itemFilter.groupName)
        }
    ...
    const styles = StyleSheet.create({
        ...
        itemMain: { backgroundColor: appcolor.backgroundContent, padding: 8, borderRadius: 8, marginBottom: 8 },
        loadingView: { width: '100%', height: '100%', justifyContent: 'center', position: 'absolute', zIndex: 3 },
        bottomView: { paddingBottom: 0 },
        titleBottomList: { fontSize: 10, fontWeight: '500', color: appcolor.dark, textAlign: 'center', padding: 8 },
        dateStatusView: { minWidth: 100, borderRadius: 16, paddingVertical: 5, paddingHorizontal: 8, end: 0, top: 0, bottom: 0, alignItems: 'center' },
        titleDateUpdate: { fontSize: 11, fontWeight: '500', color: appcolor.light },
        actionSheetView: { width: '100%', minHeight: 100, marginBottom: 16 },
        actionBottom: { width: '100%', flexDirection: 'row-reverse', alignItems: 'center', marginTop: 5 },
        contentRequire: { minWidth: 100, borderRadius: 16, paddingVertical: 5, paddingHorizontal: 8, marginEnd: 8, alignItems: 'center', backgroundColor: appcolor.blacklight },
        titleRequired: { fontSize: 11, fontWeight: '500', color: appcolor.light, fontStyle: 'italic' },
    })
    if (isLoading) return <LoadingView isLoading={isLoading} title='Đang cập nhật dữ liệu' styles={styles.loadingView} />
    //
    const renderItem = ({ item, index }) => {
        const onPress = () => handlerShowProfile(item)
        const onNotify = () => handlerDetailReport(item)
        return (
            <TouchableOpacity key={`sd-it-${index}`} style={styles.itemMain} onPress={onPress}>
                ...
                <View style={styles.actionBottom}>
                    <View style={{ ...styles.dateStatusView, backgroundColor: appcolor[item.colorGroup] }}>
                        <Text style={styles.titleDateUpdate}>{moment(item.lastUpdate).fromNow()}</Text>
                    </View>
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <View style={styles.contentMain}>
                ...
                />
                <GroupStatus data={dataMain} onChange={handlerChangeGroup} itemFilter={itemFilter} />
                <FlashList
                    ref={listRef}
            ...
            {/* // */}
            <ActionSheet id="statistical-setting">
                <View style={styles.actionSheetView}>
                    <ActionFilter key='action-filter-statistical' itemFilter={itemFilter} handlerFilterData={handlerFilterData} />
                </View>
            </ActionSheet>
            <ActionSheet id="statistical-required-report" onBeforeShow={setItemData}>
                <View style={styles.actionSheetView}>
                    <NotifyDisplayReport key='notify-display-report' itemData={itemData} />
                </View>
            </ActionSheet>
        </View>
    )
