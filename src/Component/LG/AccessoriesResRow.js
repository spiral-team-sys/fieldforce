...
import {
  ...
  updateItemAceess,
  updateNoteAceess,
} from '../../Controller/AccessoriesController';
...
const AccessoriesResRow = ({
  ...
  const renderItemVendor = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          backgroundColor:
            itemData.selectVal == item.nameVN
              ? appcolor.primary
              : appcolor.surface,
          padding: 10,
    ...
    return (
      ...
      >
        <Text
          style={{
            color:
              itemData.yearVal === item.name ? appcolor.white : appcolor.dark,
          }}
        >
          {item.name}
        </Text>
  ...
  const RenderCalendarBS = ({ currentDB, onSelect }) => {
    const [dateShow, setDateShow] = useState(
      currentDB ? new Date(currentDB) : new Date(),
  ...
  const handleChangeNote = debounce(async text => {
    if (text != null && text.length > 0 && text.length < 5) {
      ToastError(
        'Ghi chú ngắn, Vui lòng nhập ghi chú ít nhất 5 ký tự.',
        'Thông báo',
        'top',
      );
      return;
        ...
        <View style={{ flex: 1 }}>
          {keyLayer2 && (
            ...
            >
              <SpiralIcon
                name="tags"
                style={{ padding: 8, color: appcolor.primary }}
              />
              <Text
                style={{
              ...
              >{`${itemData.categoryName} - ${itemData.categoryType}`}</Text>
            </View>
          )}
          {keyLayer2 && (
            <FormGroup
  ...
  //                     <SpiralIcon name='tags' style={{ padding: 8, color: appcolor.primary }} />
