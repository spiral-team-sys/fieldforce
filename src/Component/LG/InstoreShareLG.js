...
export const InstoreShareLG = ({ navigation, route, Status }) => {
  ...
  const loadData = async () => {
    setLoading(true);
    const lstData = await getListDataTracking(
      workinfo,
  ...
  const ViewItem = () => {
    return arrTagShow.map((it, i) => {
      const dataByCategoryId = arrDataShow.filter(
        i => i.categoryId === it.categoryId,
      );
      const totalRow = dataByCategoryId.length;
      return (
        <Tabs.Tab
          key={it.categoryId}
          label={it.category + `${totalRow !== 0 ? ` (${totalRow})` : ''}`}
...
const RenderItemData = ({
  ...
  const editChangeQuanity = async e => {
    ...
    await insertTotalTrackingLG(
      workinfo,
      item,
      numberInput !== '' && numberInput !== undefined && numberInput !== null
        ? parseInt(numberInput)
        : null,
    );

    let indexD = detailsTem.findIndex(
      it =>
        it.categoryId === item.categoryId &&
        it.competitorId === item.competitorId,
    );
  ...
  const editChangePrice = async e => {
    ...
    let lstCommon = detailsTem.filter(
      it =>
        it.competitorId === item.competitorId &&
        it.categoryId === item.categoryId,
    );
    ...
    lstCommon.map(it => {
      if (it.refName.includes('TOTAL') == false) {
        ...
        if (parseInt(numberInput) < itemQuanity) {
          ...
          numberInput = null;
          ToastError(
            'Tổng số lượng không thể nhỏ hơn số lượng thành phần',
            'Lỗi',
            'top',
          );
        }
  ...
  const fontWeightTotal =
    item.refName.includes('TOTAL') == true ? '800' : '500';
  const colorTotal =
    item.refName.includes('TOTAL') == true ? appcolor.primary : appcolor.dark;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {item.segment ? item.segment : item.refName}
        </Text>
        <View
          style={{
            flexDirection: 'row',
        ...
        >
          {item.numberValue == 1 && (
            <TextInput
              ...
              onEndEditing={editChangeQuanity}
            >
              {inputText}
            </TextInput>
          )}
