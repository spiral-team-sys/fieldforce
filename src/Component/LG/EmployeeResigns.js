...
import { View, Text, TouchableOpacity, ScrollView, Image, RefreshControl, LayoutAnimation, Platform, UIManager, StyleSheet } from "react-native"
...
import { Message, MessageInfo, UUIDGenerator, groupDataByKey } from "../../Core/Helper"
...
export const EmployeeResigns = ({ navigation, route }) => {
    ...
    const [data, setData] = useState({ dataResigns: {}, dataReason: [], dataPhoto: [], dataPhotoSystem: [], dataFile: {} })
    ...
    const checkContract = (dateCheck, dateString, config = configResign) => {
        ...
        if (contractNoticeDays > 0 && dateCheck && dateString) {
            const countDayResign = moment(dateString, 'YYYY-MM-DD').diff(dateCheck, 'days');
            ...
            if (countDayResign < contractNoticeDays) {
                ...
                const dateLate = moment(dateLimit, 'YYYY-MM-DD').diff(dateString, 'days');
                const costViolate = config?.salaryDefault ? ((countDayResign > contractNoticeDays ? contractNoticeDays : dateLate) * (config?.salaryDefault / 30)) : 0
                let contentAlert = `*Cảnh báo vi phạm thời gian báo trước: \n- Số ngày nhân viên cần báo trước: ${contractNoticeDays} ngày\n- Số ngày nhân viên báo trước: ${countDayResign} ngày\n- Số ngày vi phạm: ${dateLate > contractNoticeDays ? contractNoticeDays : dateLate} ngày\n- Ngày nghỉ sớm nhất đúng quy định: ${dateLimit}\n${costViolate > 0 ? `- Mức phạt vi phạm: ${costViolate.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}` : ''}${config?.noteResignByEmployee ? `\n- ${config.noteResignByEmployee}` : ''}`
                setAlertResignDate(contentAlert)
            } else {
                setAlertResignDate(`${config?.noteResignByEmployee ? `\n- ${config.noteResignByEmployee}` : ''}`)
            }
        }
    }
    ...
    const loadData = async (config = configResign) => {
        ...
        if (dataResigns.statusId === 200) {
            ...
            if (dataResigns.data?.length > 0 && contentData?.isDelete !== 1) {
                ...
                JSON.parse(contentData?.photo || '[]').map(it => listPhoto.push({ photoPath: it.photo }))

                const itemReason = arr.find(it => it.id == contentData?.reasonId)
                setData(prev => ({ ...prev, dataPhotoSystem: listPhoto, dataResigns: contentData, dataReason: arr, dataPhoto: listPhoto, dataFile: fileResign }))

                await setOrtherReason(contentData?.notes)
                await setSelectReason(itemReason)
                await setDateSelect(contentData?.fromDate)
                checkContract(moment(contentData.createdDate).format('YYYY-MM-DD'), moment(contentData.fromDate, 'YYYYMMDD').format('YYYY-MM-DD'), config)

                if (config?.resignRegisterView !== null && config?.resignRegisterView !== undefined) {
                    const resignRegisterView = JSON.parse(config?.resignRegisterView || '[]')
                    await handleLoadImageSize(resignRegisterView)
                    await onViewNoteResign(config?.resignConfirmNote, resignRegisterView, contentData || {})
                }
            } else {
    ...
    const LoadDataConfig = async () => {
        ...
        const ConfigResigns = JSON.parse(dataConfigResigns?.configResign || '[]')?.[0] || {}
    ...
    const loadResignRegisterView = async (ConfigResigns) => {
        if (ConfigResigns?.resignRegisterView !== null && ConfigResigns?.resignRegisterView !== undefined) {
            const resignRegisterView = JSON.parse(ConfigResigns?.resignRegisterView || '[]')
            await handleLoadImageSize(resignRegisterView)
            await onViewNoteResign(ConfigResigns?.resignConfirmNote, resignRegisterView, data.dataResigns)
        }
    }
    useEffect(() => {
        const init = async () => {
    ...
    const uploadFile = async () => {
        ...
        let photoinfo = {};
        let options = {
            mediaType: 'photo', maxWidth: 1336, maxHeight: 1336, quality: 1, includeBase64: true
        };
    ...
    const uploadPdf = async () => {
        ...
        try {
            ...
            await UploadController.uploadFilePDF([{ ...response }], new Date().getFullYear(), (new Date().getMonth() + 1), async (result) => {
                let dataFile = {}
    ...
    const uploadAction = async () => {
        ...
        if (count === 0) {
            ...
            const countDayResign = moment(resignsDate, 'YYYY-MM-DD').diff(moment(currentDate, 'YYYY-MM-DD'), 'days');
            const minDateResign = configResign?.minDateResign || 0
            if (countDayResign <= minDateResign && minDateResign !== 0) {
                toastError(`Ngày nghỉ phải sau ${minDateResign} ngày so với ngày hiện tại!`, 'Thông báo', 'top')
                return
            }
            ...
            const notifyContent = 'Nhân viên ' + (userinfo.employeeName || '') + ' đã gửi yêu cầu xin nghỉ việc vào ' + moment(new Date()).format('YYYY-MM-DD HH:mm') + '.'
            ...
            Message('Chú ý', `Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?`,
                async () => {
                    ...
                    const result = await Employee.sendEmployeeResigns(JSON.stringify(dataUpload), dataPhoto, notifyNote, group, null)
                    if (result.statusId === 200 || (result.statusId === 500 && result.messager.includes('mail'))) {
                        const nextDataResigns = normalizeDataResigns(result.data?.[0] || dataUpload)
    ...
    const deleteResign = async () => {
        Message('Chú ý', `Bạn có chắc chắn muốn huỷ yêu cầu?`,
            async () => {
                const notifyContent = 'Nhân viên ' + (userinfo.employeeName || '') + ' đã huỷ yêu cầu xin nghỉ việc vào ' + moment(new Date()).format('YYYY-MM-DD HH:mm') + '.'
                const dataUpload = {
                    ...(data.dataResigns || {}),
                    isDelete: 1,
                    notifyContent: notifyContent
                ...
                const group = userinfo.groupType
                const result = await Employee.sendEmployeeResigns(JSON.stringify(dataUpload), null, notifyNote, group, null)
                if (result.statusId === 200 || (result.statusId === 500 && result.messager.includes('mail'))) {
                    setData(prev => ({ ...prev, dataResigns: {}, dataPhoto: [], dataFile: {} }))
                    setSelectReason({})
                    setOrtherReason('')
                    setDateSelect(moment(new Date()).format('YYYYMMDD'))
                    setDataCalendar({
                            ...
                            <Text key={index}>
                                {parts.map((part, i) =>
                                    phoneRegex.test(part) ? (
                                        <Text key={i} style={{ fontWeight: 'bold', color: appcolor.danger }}>{part}</Text>
                                    ) : (
                                        <Text key={i} style={{ fontSize: 14, color: appcolor.dark }}>{part}</Text>
                                    )
                                )}
                                {'\n'}
                            </Text>
            ...
            <View style={{ height: deviceHeight * 0.6 }}>
                ...
                >
                    <View style={{ width: '100%', height: '100%', justifyContent: 'center', alignItems: 'center' }}>
                        {parsedView.length > 0 && <View style={{ width: '100%' }}>
                            {parsedView.map((item, index) => (
                                <TouchableOpacity style={{ alignItems: 'center', justifyContent: 'center', width: '90%' }} key={index} onPress={() => handleViewImage(item)}>
                                    <Image
                                        source={item.URI ? { uri: URLDEFAULT + item.Url.replace(/\\/g, '/') } : require('../../Themes/Images/noimage.png')}
                                        resizeMode="stretch"
                        ...
                        {configResign?.resignConfirmNote && <View style={{ width: '80%', padding: 10 }}>
                ...
                {parsedView.length > 0 &&
                    <View style={{ width: '100%' }}>
                        {parsedView.map((item, index) => {
                            ...
                            return (
                                <TouchableOpacity key={'ImageNote_' + index} style={{ alignItems: 'center', justifyContent: 'center', width: '100%' }} onPress={() => handleViewImage(item)}>
                                    <Image
                                        source={{ uri: URLDEFAULT + item.Url.replace(/\\/g, '/') }}
                                        resizeMode="contain"
                                        style={{ width: '100%', height: refImageSheet.current[index] || 200 }}
                                    />
                                </TouchableOpacity>
                            )
                        })}
    ...
    const styles = StyleSheet.create({
        ...
        headerContainer: { padding: 10, width: deviceWidth, flexDirection: 'row', justifyContent: 'space-between' },
        headerLeft: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center' },
        ...
        headerLink: { fontWeight: '600', textDecorationLine: 'underline', fontSize: 16, paddingLeft: 10 },
        contentContainer: { flexDirection: 'column', padding: 10 },
        buttonContainer: { flexDirection: "row", padding: 10, width: '100%', justifyContent: 'space-between', marginTop: 10, marginBottom: 20 },
        buttonLeft: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center' },
        photoButton: { height: 40, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', borderRadius: 25, borderWidth: 0.6, borderColor: appcolor.primary },
        photoButtonActive: { backgroundColor: appcolor.primary },
        photoButtonInactive: { backgroundColor: appcolor.light },
        photoIconContainer: { height: 35, width: 35, marginLeft: 2, borderRadius: 50, justifyContent: 'center', alignItems: 'center' },
        ...
        actionButton: { height: 35, width: 35, marginLeft: 2, borderRadius: 50, alignItems: 'center', backgroundColor: appcolor.primary, justifyContent: 'center' },
        actionIcon: { color: appcolor.white },
        imageContainer: { margin: 7 },
        imageButton: { borderWidth: 0.5, borderColor: appcolor.dark, borderRadius: 12 },
        image: { width: '100%', minHeight: 230, borderRadius: 12 },
        fileContainer: { flexDirection: "row", width: '100%', marginRight: 5, backgroundColor: appcolor.surface, padding: 4, borderRadius: 8 },
        fileIconContainer: { width: 50, height: 65, backgroundColor: appcolor.light, borderRadius: 5, justifyContent: 'center', alignItems: 'center' },
        ...
        fileInfo: { fontWeight: '300', fontSize: 12, color: appcolor.dark, fontStyle: 'italic' },
        noticeButton: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', backgroundColor: appcolor.primary, margin: 12, borderRadius: 8, borderWidth: 0.6, padding: 8 },
        noticeText: { fontWeight: '600', fontSize: 16, paddingLeft: 10, color: appcolor.white }
    })
    ...
    return (
        <View style={styles.container}>
            <HeaderCustom
                ...
                iconRight={!isResigned ? 'cloud-upload-alt' : (data.dataResigns?.confirm === 3 ? 'trash' : null)}
                rightFunc={() => !isResigned ? uploadAction() : (data.dataResigns?.confirm === 3 ? deleteResign() : null)}
            />
                ...
                <View style={{ flex: 1 }}>
                    {
                        ...
                        <View style={styles.headerContainer}>
                            <View style={styles.headerLeft}>
                                <Icon color={data.dataResigns?.confirm == 1 ? appcolor.success : (data.dataResigns?.confirm == 0 ? appcolor.danger : appcolor.warning)} name={getConfirmIcon(data.dataResigns?.confirm)} type='font-awesome' size={20} style={{ padding: 5 }} />
                                <Text style={[styles.headerText, { color: data.dataResigns?.confirm == 1 ? appcolor.success : (data.dataResigns?.confirm == 0 ? appcolor.danger : appcolor.warning) }]}>{data.dataResigns?.confirmContent}</Text>
                            </View>
                            {
                                ...
                                <TouchableOpacity
                                    ...
                                    <Text style={[styles.headerLink, { color: data.dataResigns?.confirm == 1 ? appcolor.success : (data.dataResigns?.confirm == 0 ? appcolor.danger : appcolor.warning) }]}>Gửi lại</Text>
                                </TouchableOpacity>
                            }
                        </View>
                    }
                    ...
                    >
                        <View style={styles.contentContainer}>
                            {
                                ...
                                <TouchableOpacity onPress={() => !isResigned ? selectItem('REASON') : null}>
                                    <FormGroup
                                        key={'selectReason'}
                                        rightFunc={() => !isResigned ? selectItem('REASON') : null}
                                        iconRight={!isResigned ? "caret-down" : null} iconRightStyle={{ color: appcolor.primary }}
                            ...
                            />
                            {
                                configResign?.contractNoticeDays > 0 && alertResignDate?.length > 0 &&
                                <Text style={{ fontSize: 14, color: appcolor.danger, marginVertical: 8, marginHorizontal: 4 }}>Cảnh báo: {alertResignDate}</Text>
                            }
                            <TouchableOpacity onPress={() => !isResigned ? selectItem('CALENDAR') : null}>
                                <FormGroup
                                    key={'selectCalendar'}
                                    rightFunc={() => !isResigned ? selectItem('CALENDAR') : null}
                                    iconRight={!isResigned ? "caret-down" : null} iconRightStyle={{ color: appcolor.primary }}
                            ...
                            {
                                (isResigned && data.dataResigns?.confirm !== 3 && data.dataResigns?.confirmNote) &&
                                <FormGroup
                                    key={'SupConformNote'}
                                    rightFunc={() => !isResigned ? selectItem('CALENDAR') : null}
                                    multiline={true}
                        ...
                        <View style={styles.buttonContainer}>
                            <View style={styles.buttonLeft}>
                                <TouchableOpacity
                                    ...
                                    style={[styles.photoButton, { backgroundColor: modeEvident == 1 ? appcolor.primary : appcolor.light }]}
                                >
                                        ...
                                        return (
                                            ...
                                            >
                                                <TouchableOpacity
                                                    ...
                                                    onPress={() => { setImageIndex(index), selectItem('IMAGE_ZOOM') }}
                                                >
                                                    <Image
                                                        source={it.photoPath ? { uri: (isResigned && it.photoPath.includes('uploaded')) ? (URLDEFAULT + it.photoPath) : (it.photoPath || '') } : require('../../Themes/Images/noimage.png')}
                                                        style={styles.image}
                                                    />
                                                </TouchableOpacity>
                                            </GmailStyleSwipeableRow>
                        ...
                        }
                        {
                            ...
                            <View>
                                <GmailStyleSwipeableRow key={'delete_file'} enableRight={isResigned} deleteItem={() => deleteFile()}>
                                    <TouchableOpacity key={'ItemDoc_Resign'} style={styles.fileContainer}
                                    ...
                                    >
                                        ...
                                        <View style={styles.fileContent}>
                                            <View style={styles.fileTextContainer}>
                                                <Text numberOfLines={2} style={styles.fileName}>{data.dataFile?.name}</Text>
                                                <Text style={styles.fileInfo}>{(data.dataFile?.size >= 1000000 ? ((data.dataFile?.size / 1000000).toFixed(2) + 'MB, ') : ((data.dataFile.size / 1000).toFixed(2)) + 'KB, ') + 'PDF tài liệu'}</Text>
                                            </View>
                                        </View>
                                    </TouchableOpacity>
                                </GmailStyleSwipeableRow>
                    ...
                    </ScrollView>
                    {
                        ...
                        <TouchableOpacity
                            onPress={() => handleSelectFile(configResign?.resignLetterSample || '')}
                            style={styles.noticeButton}>
                            <Text style={styles.noticeText}>Mẫu đơn nghỉ việc</Text>
                        </TouchableOpacity>
                    }
                    {
                        isResigned && configResign?.resignConfirmNote && !configResign?.resignRegisterView &&
                        <TouchableOpacity
            ...
            <View>
                ...
                >
                    <View style={{ height: deviceHeight / 2, marginTop: 10, marginBottom: 20, backgroundColor: appcolor.light, justifyContent: 'center', width: '100%', padding: 5 }} >
                        <Calendar
                            firstDay={1}
                            current={moment(new Date()).format("yyyy-MM-DD")}
                            minDate={configResign?.selectPastDate == 1 ? null : moment(new Date()).format("yyyy-MM-DD")}
                            monthFormat={'MM - yyyy'}
                            hideExtraDays={true}
                            theme={{
                ...
                <ActionSheet id='imageSheet'>
                    <View style={{ height: deviceHeight, width: deviceWidth }}>
                        <MultipleShowImage key={'ShowItemImage'} listItem={data.dataPhoto} closeShowImage={() => closeSheet('IMAGE_ZOOM')} indexItem={imageIndex} />
                    </View>
                </ActionSheet>
                {isVisible &&
                    <ModalNotify
