...
const AccessoriesModal = ({ closedModal, loadDataAccessories, data }) => {
  ...
  const RenderItemType = ({ }) => {
    ...
    (itemSave.length === 0 ||
      itemSave.length !== itemType.length ||
      itemSave[0].shopProfileId !== itemType[0].shopProfileId) &&
      createItem();
    return (
      <View style={{ padding: 10, width: '100%' }}>
        ...
        {itemType[0].decimalValue === 1 &&
          itemType[0].shopProfileId !== 101 && <ViewInput />}
  ...
  const createItem = async () => {
    const itemType =
      (await data.dataItemType.filter(
        it => it.shopProfileId === categoryTypeSelect.shopProfileId,
      )) || [];
    let itemR = [];
  ...
  const handleChangeType = async item => {
    const itemType =
      data.dataItemType.filter(it => it.shopProfileId === item.shopProfileId) ||
      [];
    if (itemType[0].textValue === 1 && itemType[0].shopProfileId !== 101) {
      await setMode('textVal');
    } else if (
      itemType[0].numberValue === 1 &&
      ...
      itemType[0].dateValue === 1 &&
      itemType[0].shopProfileId !== 101
    ) {
      await setMode('DATE');
    } else if (
      itemType[0].yearValue === 1 &&
      itemType[0].shopProfileId !== 101
    ) {
      await setMode('YEAR');
    } else if (
      itemType[0].selectValue === 1 &&
      itemType[0].shopProfileId !== 101
    ) {
  ...
  const renderListType = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          backgroundColor:
            categoryTypeSelect.shopProfileId === item.shopProfileId
              ? appcolor.primary
              : appcolor.surface,
          marginHorizontal: 5,
          borderRadius: 50,
  ...
  const handleSelectCategory = async item => {
    const dataTypeF = await data?.dataType.filter(
  ...
  const renderItemCategory = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          backgroundColor:
            categorySelect?.categoryId === item.categoryId
              ? appcolor.primary
              : appcolor.surface,
          marginHorizontal: 5,
          borderRadius: 50,
  ...
  const renderItemVendor = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          backgroundColor:
            itemVendor.selectVal == item.name
              ? appcolor.primary
              : appcolor.surface,
          padding: 10,
          justifyContent: 'center',
    ...
    return (
      ...
      >
        <Text
          style={{
            color:
              itemYear.yearVal === item.name ? appcolor.white : appcolor.dark,
          }}
        >
          {item.name}
        </Text>
  ...
  const onChangeDate = async selectedDate => {
    ...
    itemSave.map(it =>
      it.dateValue === 1 ? (it.dateVal = currentDate) : null,
    );
    await setMutate(e => !e);
  };
  const RenderCalendarBS = ({ currentDB, onSelect }) => {
    ...
    if (Array.isArray(currentDB)) {
      currentDB.forEach(it =>
        it.dateValue === 1 ? (dateCurrent = it.dateVal) : null,
      );
    }
    const [dateShow, setDateShow] = useState(
      dateCurrent ? new Date(dateCurrent) : new Date(),
    ); //currentBS.selectedDay
  ...
  const HeaderSheet = () => {
    ...
    return (
      ...
      >
        ...
        >
          {categoryTypeSelect.categoryType}
        </Text>
        <TouchableOpacity
          onPress={onPress}
          style={{ padding: 8, justifyContent: 'center', alignItems: 'center' }}
        >
          <SpiralIcon name="times" color={appcolor.primary} size={24} />
        </TouchableOpacity>
      </View>
    );
  };
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        {mode === 'DATE' && (
          ...
          >
            <HeaderSheet />
            <RenderCalendarBS currentDB={itemSave} onSelect={onChangeDate} />
          </View>
        )}
        {mode === 'SHOPINFO_DATE' && (
          <View
