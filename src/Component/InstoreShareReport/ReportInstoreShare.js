...
import { SafeAreaView, ScrollView, StatusBar, TouchableOpacity, UIManager, View } from "react-native";
...
export const ReportInstoreShare = ({ dataCategory, dataCompetitor, closeModal }) => {
    const { appcolor, userinfo, kpiinfo, shopinfo } = useSelector(state => state.GAppState);
    ...
    const handlerSaveItem = async () => {
        ...
        if (reportConfig.limitDate > 0) {
            ...
            if (currenMonth == 1) {
                if (reportConfig.limitDate >= currentDate && filterMonth.month !== currenMonth && filterMonth.month !== 12 && filterMonth.month < currenMonth) {
                    MessageInfo("Bạn không được báo cáo quá xa so với tháng hiện tại!!!!")
                    return
                }
            } else {
                if ((reportConfig.limitDate >= currentDate && filterMonth.month < (currenMonth - 1) && filterMonth.year == currentYear)) {
                    MessageInfo("Bạn không được báo cáo quá xa so với tháng hiện tại!!!!")
                    return
                }
            }
            if (currentYear > filterMonth.year && filterMonth.month !== 12) {
                MessageInfo("Bạn không được báo cáo quá xa so với tháng hiện tại!!!!")
                return
            } else if (reportConfig.limitDate < currentDate && (filterMonth.month !== currenMonth || currentYear !== filterMonth.year)) {
                MessageInfo(`Qua ngày ${reportConfig.limitDate} bạn không được báo cáo tháng khác !!`)
                return
            }
        }
        ...
        const cateFilterValue = cateFilter.filter(it => it.quantityByCate == undefined || it.quantityByCate == null)
    ...
    const ViewMonth = () => {
        return (
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: "center", padding: 4 }}>
                <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: '600' }}>Chọn tháng : </Text>
                <View style={{ flexDirection: 'row', justifyContent: "space-between", width: '50%', alignItems: "center" }}>
                    <TouchableOpacity
                        onPress={() => handleSelectCalendar()}
                        style={{ flexDirection: "row", borderRadius: 20, width: '60%', padding: 4, borderWidth: 0.6, borderColor: appcolor.grey, }}>
                        <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: '400', textAlign: 'center', flex: 1 }} >{`${filterMonth.month}-${filterMonth.year}`}</Text>
                        <Icon size={12} name='calendar-day' type='font-awesome-5' style={{ flex: 1, paddingHorizontal: 2 }} />
                    </TouchableOpacity>
                    <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: '400' }}>MM-YYYY</Text>
                </View>
            </View>
        )
    }
    const ViewCompetitor = () => {
        return (
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: "center", padding: 4 }}>
                <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: '600' }}>Hãng : </Text>
                <TouchableOpacity
                    onPress={() => handleSelectCompetitor()}
                    style={{ flexDirection: "row", width: '50%', borderRadius: 20, borderWidth: 0.6, borderColor: appcolor.grey, padding: 4, alignItems: 'center' }}>
                    <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: '400', textAlign: 'center', flex: 1 }} >{compeSelect.competitorName || 'Chọn hãng'}</Text>
                    <Icon size={12} name='chevron-down' type='font-awesome-5' style={{ flex: 1, paddingHorizontal: 2 }} />
                </TouchableOpacity>
            </View>
        )
    }
    const ViewCategory = () => {
        return (
            <View style={{ padding: 4 }}>
                <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: '600' }}>Ngành hàng : </Text>
                            ...
                            dataAll.dataCate.map((item, index) => {
                                return (
                                    <TouchableOpacity
                                        ...
                                        style={{
                                            padding: 4, borderRadius: 20, borderWidth: 0.6, borderColor: item.isCheck == true ? appcolor.primary : appcolor.grey, minWidth: 80, justifyContent: "center", alignItems: "center",
                                            backgroundColor: appcolor.light, margin: 5
                                        }}
                                    >
                                        <Text style={{ fontSize: 12, color: item.isCheck == true ? appcolor.primary : appcolor.dark, fontWeight: '500' }}>{item.categoryName}</Text>
                                    </TouchableOpacity>
                                )
    ...
    return (
        <View style={{ backgroundColor: appcolor.light, flex: 1 }}>
            <SafeAreaView style={{ width: '100%', flexDirection: 'row', backgroundColor: appcolor.primary, padding: 5, alignItems: 'center', justifyContent: 'space-between', }}>
                <TouchableOpacity onPress={closeModal} style={{ padding: 10, paddingRight: 15, width: '15%', }}>
                    <Icon name={'chevron-left'} size={scaleSize(28)} solid={true} color={appcolor.white} />
                </TouchableOpacity>
                <Text style={{ width: '70%', textAlign: 'center', fontSize: scaleSize(18), fontWeight: '700', padding: 5, color: appcolor.white, }}>{'Tạo số bán'}</Text>
                <TouchableOpacity onPress={handlerSaveItem} style={{ padding: 10, width: '15%', }}>
                    <Icon name={'save'} size={scaleSize(28)} solid={true} color={appcolor.white} />
                </TouchableOpacity>
            </SafeAreaView>
            <View style={{ flex: 1, padding: 8 }}>
                <KeyboardAvoidingView
                    ...
                    <ScrollView
                        ...
                        <ViewCompetitor />
                        <ViewCategory />
                        <View style={{ justifyContent: 'space-between', padding: 4, paddingTop: 8 }}>
                            {
                                dataAll.dataCate.map((item, index) => {
                                    return (
                                        item.isCheck == true ?
                                            <RenderItemInput key={'itemInput_' + index} item={item} index={index} />
                                            : null
                                    )
                                })
                            }
                        ...
                        <View style={{ padding: 4 }}>
                            <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: '600', paddingBottom: 10 }}>Ghi chú</Text>
                            <FormGroup
            ...
            <ActionSheet id={'SheetMonthCreate'}  >
                <View style={{ height: 200 }} >
                    <YearMonthSelected option={filterMonth} onYearMonth={(search) => onFilterChange(search)} numMonth={4} />
                </View>
            </ActionSheet>
            <ActionSheet id={'SheetCompetitor'}  >
                <View style={{ height: 400 }} >
                    <Text style={{ fontSize: 16, color: appcolor.dark, fontWeight: '600', padding: 12 }}>Chọn hãng </Text>
                    <View style={{ flexDirection: 'row', flexWrap: "wrap", }}>
                        {
                            dataAll.dataCompe.map((item, index) => {
                                return (
                                    <TouchableOpacity
                                        ...
                                        style={{
                                            padding: 4, borderRadius: 20, borderWidth: 0.6, borderColor: item.competitorId == compeSelect.competitorId ? appcolor.primary : appcolor.grey, minWidth: 80, height: 35, justifyContent: "center", alignItems: "center",
                                            backgroundColor: appcolor.light, margin: 5
                                        }}
                                    >
                                        <Text style={{ fontSize: 12, color: item.competitorId == compeSelect.competitorId ? appcolor.primary : appcolor.dark, fontWeight: '500' }}>{item.competitorName}</Text>
                                    </TouchableOpacity>
                                )
                            })
                        }
...
const RenderItemInput = ({ item, index }) => {
    ...
    return (
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: "center" }}>
            <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: '600', width: '50%' }}>{item.categoryName}</Text>
            <NumberFormat
                ...
                renderText={value =>
                    <TextInput
                        textAlign={'center'}
                        value={value}
                        style={{ fontSize: 12, minWidth: 80, color: appcolor.dark, fontWeight: '500', textAlign: 'center', borderWidth: 0.5, borderRadius: 5, borderColor: appcolor.greydark, padding: 6, marginBottom: 2, backgroundColor: appcolor.light }}
