...
import { FlatList, Modal, TouchableOpacity, UIManager, View } from "react-native";
...
import { getInstoreShareByShop, sendInstoreShareByShop } from "../../Controller/SellOutController";
...
export const HomeInstoreShare = ({ navigation, route }) => {
    ...
    const [data, setData] = useState({ dataCategory: '', dataCompetitor: '', dataByMonth: [] })
    ...
    const loadData = async () => {
        ...
        const dataByShop = await getInstoreShareByShop({ "shopId": shopinfo.shopId, "year": filterMonth.year, "month": filterMonth.month })
        console.log(dataByShop, 'filterMonth.monthfilterMonth.month');
        if (dataByShop !== null) {
            const listByMonth = JSON.parse(dataByShop?.data[0]?.listByMonth || '[]')
            setData({ dataCategory: dataByShop?.data[0]?.listCategory || '[]', dataCompetitor: dataByShop?.data[0]?.listCompetitor || '[]', dataByMonth: listByMonth })
        }
            ...
            <LoadingView isLoading={isLoading} title={'Đang cập nhật dữ liệu'} />
            {
                ...
                <View style={{ flex: 1 }}>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item, index }) => <ItemHomeResRow item={item} index={index} handleDelete={handleDeleteItem} />}
                    ...
                    <TouchableOpacity style={{
                        flexDirection: 'row',
                        position: 'absolute', zIndex: 10, right: 12, bottom: 30
                    }}
                        // disabled={this.state.idHaveNosell === false ? false : true}
                        onPress={() => setVisible(true)}>
                        <View style={{
                            flexDirection: 'row', alignItems: 'center', //, opacity: idHaveNosell === false ? 1 : 0.3,
                            borderRadius: 30, backgroundColor: appcolor.primary
                        }}>
                            {/* s */}
                            <Icon raised size={20} name='add-circle-outline' type='ionicon' />
                        </View>
                    </TouchableOpacity>
                </View>
            }
            <Modal visible={isVisible} animationType="fade">
                <ReportInstoreShare closeModal={handleCloseModal} dataCategory={JSON.parse(data.dataCategory || '[]')} dataCompetitor={JSON.parse(data.dataCompetitor || '[]')} />
            </Modal>
            <ActionSheet id={'SheetInstoreShare'}  >
