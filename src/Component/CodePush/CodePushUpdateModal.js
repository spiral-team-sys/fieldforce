        ...
        {isLoading ? (
          <View style={styles.viewItemContent}>
            ...
            {isDownloadingUpdate && downloadProgress && (
              <Text
                style={[styles.subTitleName, { fontWeight: fontWeightBold }]}
              >
                {`${percent}%`}
              </Text>
            )}
          ...
          <View style={styles.viewItemContent}>
            ...
            >{`Thông báo cập nhật ứng dụng`}</Text>
            <Text style={styles.subTitleName}>{`${
              updateInfo?.label || `Nội dung cập nhật`
            }\n${updateInfo?.description}`}</Text>
            <Text
              style={[styles.subTitleName, { fontWeight: fontWeightBold }]}
            >{`Phiên bản: ${updateInfo?.version}`}</Text>
            {updateInfo?.isMandatory && (
              <View style={styles.viewWarning}>
                <Text
                  style={styles.subTitleName}
                >{`⚠️ Cập nhật bắt buộc`}</Text>
              </View>
            )}
          </View>
        )}
