...
class StockoutEPS extends Component {
  ...
  insertHistory = async () => {
    if (this.state.isClear === false) {
      let displayHistory = await getStockHistory(
        this.props.route.params.workinfo,
      );
      let lstProduct = await getAllProduct();
      let resDisplay = await getStockUpload(
  ...
  UploadData = async () => {
    ...
    let resStock = await getStockUpload(
      this.props.route.params.workinfo.workId,
    );

    if (AppNameBuild === 'lg') {
      this.setShowProgress(true);
      UploadController.DataStock(
        resStock,
        workinfo,
        () => {
          this.setShowProgress(false);
  ...
  setClearAll = async () => {
    ...
    let res = await deleteAllStockResult(
      this.props.route.params.workinfo.workId,
    );
    this.loadDataShow();
  };
      ...
      >
        ...
        >
            ...
            <View style={{ flexDirection: 'row', paddingTop: 15 }}>
              <View
                style={{
                  ...
                  flexDirection: 'column',
                  width: AppNameBuild !== nokiaApp ? '33%' : '50%',
                }}
              >
                <Text
                  style={{
          ...
          <View
            style={{
              position: 'absolute',
              marginTop: this.state.isHiddenNote ? 180 : 0,
              width: '100%',
                ...
                <SectionList
                  ...
                  renderItem={({ item, index }) => (
                    <RenderRow
                      ...
                      NoteCommon={this.state.noteCommon}
                      appcolor={appcolor}
                    />
                  )}
      ...
      {!('SubCatId' in item) && !('CategoryId' in item) ? (
        ...
        >
          <View style={{ width: '40%', alignSelf: 'center', left: 15 }}>
            <Text style={{ fontSize: 11, color: appcolor.dark }}>
              {item.itemName}
            </Text>
            <Text style={{ fontSize: 11, color: appcolor.dark }}>
              {item.productCode}
