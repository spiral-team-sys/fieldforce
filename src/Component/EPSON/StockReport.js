...
export const StockReport = ({ navigation }) => {
  ...
  const { appcolor, workinfo, kpiinfo, userinfo, shopinfo } = useSelector(
  ...
  const filterDoneData = async () => {
    ...
    let lstData = arrDataShow.filter(
      it => it.quanity !== null && it.quanity >= 0,
    );
    if (done) {
  ...
  const ViewItem = () => {
    ...
    return arrTagShow.map(it => {
      dataByCategoryId = arrDataShow?.filter(
        i => i.categoryName === it.categoryName,
      );
