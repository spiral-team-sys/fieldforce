...
import { getDisplayUpload, getCategoryProduct, getItemsProduct, getItemsProductLG, getDisplayResult, insertDisplay, insertPrice, getAllPhotosUpload, getDisplayHistory, deleteAllDisplayResult, uploadDisplayData, getCompetitorProductBy, getCategoryProductLG, getSubCatProductLG } from '../../Controller/WorkController'
...
class DisplayEPS extends PureComponent {
    ...
    UploadData = async () => {
        ...
        if (AppNameBuild === 'lg') {
            ...
            UploadController.DataDisplay(resDisplay, workinfo, () => {
                this.setShowProgress(false);
                this.refreshData();
            }, () => {
                this.setShowProgress(false);
            ...
            let resPhotos = await getAllPhotosUpload(this.props.route.params.reportId, this.props.route.params.workinfo.shopId, this.props.route.params.workinfo.workDate);
            ...
            this.setShowProgress(true);
            uploadDisplayData(resDisplay, resPhotos, workinfo, (message) => {
                this.setShowProgress(false);
                this.refreshData();
                resPhotos.length > 0 ? this.UploadFilePhoto(resPhotos) : MessageInfo(message);
            }, () => {
                this.setShowProgress(false);
            })
        }
    ...
    loadDataShow = async () => {
        this.loadCategory()
        this.loadSubCat();
        let displayHistory = await getDisplayHistory(this.props.route.params.workinfo);
    ...
    setClearAll = async () => {
        ...
        let res = await deleteAllDisplayResult(this.props.route.params.workinfo.workId);
        this.loadDataShow();
    }
    render() {
            ...
            <ImageBackground style={{ height: '100%', width: '100%', backgroundColor: appcolor.light }}>
                ...
                <View style={{ backgroundColor: appcolor.light, width: '100%', height: Dimensions.get('window').height - HEADER_SIZE }}>
                    <View style={{ flexDirection: 'column', padding: 8, zIndex: 1, width: '100%' }}>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', right: 10 }}>
                            <CheckBox
                                right
                                containerStyle={{ width: '40%', backgroundColor: appcolor.light, borderColor: appcolor.light, height: 30, padding: 2 }}
                                title='Đã nhập'
                            ...
                            <Button
                                ...
                                buttonStyle={{ backgroundColor: 'clear', borderWidth: 1, borderColor: 'gray' }}
                        ...
                        <View style={{ flexDirection: 'row', paddingTop: 15 }}>
                            <View style={{ alignItems: "flex-start", flexDirection: 'column', width: '33%' }}>
                                ...
                                <View style={{ alignContent: 'center', backgroundColor: appcolor.darklight }}>
                                    {
                    ...
                    <View style={{ position: 'absolute', marginTop: this.state.isHiddenNote ? 130 : 0, width: '100%', height: this.state.isHiddenNote ? '85%' : '100%', zIndex: 0 }}>
                        ...
                        <View style={{ zIndex: 0, width: '100%', position: this.state.isHiddenNote ? 'relative' : 'absolute', backgroundColor: 'white', height: '87%' }}>
                            <KeyboardAvoidingView
                                ...
                                <SectionList
                                    ...
                                    renderItem={({ item, index }) => (<RenderRow item={item} index={index} Status={this.state.Status} refreshView={() => this.refreshView()} workinfo={this.props.route.params.workinfo} DisplayItem={this.state.DisplayItem} Props={this.props} UpdateKeyNote={this.showNoteForm} appcolor={appcolor} />)}
                ...
                >
                    <View style={{ zIndex: 1, flex: 0, width: '100%', height: Dimensions.get('window').height }}>
                        ...
                        <View style={{ zIndex: 3, position: this.state.isHiddenNote ? 'absolute' : 'relative', width: '80%', height: 'auto', opacity: 1.0, backgroundColor: appcolor.light, borderRadius: 15, flexDirection: 'column', marginTop: 70, marginLeft: Dimensions.get("window").width / 2 - (40 * Dimensions.get("window").width) / 100 }}>
...
const RenderRow = ({ item, index, Status, refreshView, workinfo, DisplayItem, Props, UpdateKeyNote, appcolor }) => {
    return (
        <View>
            {
                (!("SubCatId" in item) && !("CategoryId" in item)) ?
                    <View style={{ height: 70, width: '100%', flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 5, backgroundColor: appcolor.darklight }}>
                        <View style={{ width: '40%', alignSelf: 'center', left: 15 }}>
                            <Text style={{ fontSize: 13, color: appcolor.dark }}>{item.itemName}</Text>
                            <Text style={{ fontSize: 11, color: appcolor.grey }}>{item.productCode}</Text>
                        </View>
