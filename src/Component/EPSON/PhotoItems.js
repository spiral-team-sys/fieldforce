import { FlatList, TouchableOpacity, Text, View, ScrollView, Image, Modal, PermissionsAndroid, Platform, SafeAreaView } from "react-native"
...
export const PhotoItems = ({ navigation, route, usedHeader = true }) => {
    const [locked, setLocked] = useState(route.params.Status === 1 ? true : false);
    const keyPhoto = route.params.keyPhoto

    const { kpiinfo, workinfo, appcolor, shopinfo, userinfo } = useSelector(state => state.GAppState);
    ...
    const [itemImage, setItemImage] = useState({ uri: '', heightViewShot: 0, widthViewShot: 0 })
    ...
    const loadPhotoItems = async (_template, lockedStatus) => {
        ...
        for (const item of (_template || [])) {
            let photoType = keyPhoto !== undefined && keyPhoto !== '' ? item.code + '_' + keyPhoto : item.code
            let lstPhoto = await getPhotosReport(reportId, photoType, workinfo.shopId, workinfo.workDate) || [];
    ...
    const MapData = async () => {
        ...
        const _template = keyPhoto !== undefined && keyPhoto !== '' && !isTakeInOther ? await photoByReport[keyPhoto] : await photoByReport || []
    ...
    const reloadCanvas = async (result) => {
        Image.getSize(result?.data[0]?.uri, (width, height) => {
            setItemImage({ uri: result?.data[0]?.uri, heightViewShot: height * (deviceWidth / width), widthViewShot: deviceWidth })
        }, () => { })
    }
    const takePhoto = async (item) => {
        ...
        await NativeCamera.cameraStart(photoinfo, callBackPhoto, itemReport?.isDrawInfo == 1 ? optionCustom : null);
    }
    ...
    const rowPhoto = (item, index, parent) => {
        return (
            <View key={index + "_pdd"} style={{
                backgroundColor: appcolor.surface,
                width: deviceWidth / 3, height: deviceWidth / 3, alignItems: 'center',
                justifyContent: 'center', marginRight: 12,
            }}>
                <View>
                    {item.photoPath === null ?
                        <TouchableOpacity disabled={deleted ? true : false} onPress={() => takePhoto(item)}>
                            <Icon
                        ...
                        <View style={{ marginEnd: 7, backgroundColor: appcolor.surface }}>
                            <TouchableOpacity onPress={() => onShowImage(parent, index)}
                                onLongPress={(locked || (item.dataUpload == 1 && item.fileUpload == 1)) ? null : () => setDeteted(true)}>
                                <Image style={{ width: (deviceWidth / 3), height: (deviceWidth / 3), }}
                                    source={{ uri: item.photoPath }} />
                                <Icon
                                    containerStyle={{ position: 'absolute', left: 10, top: 10, display: deleted ? 'none' : 'flex' }}
                            ...
                            <TouchableOpacity onPress={() => onSelected(item, parent)}
                                style={{ display: deleted === true ? 'flex' : 'none', position: 'absolute', width: '100%', height: '100%' }}>
                                <View style={{
                                    width: '100%', height: '100%', backgroundColor: appcolor.grey, opacity: 0.7,
                                    justifyContent: 'center', alignItems: 'center'
                                }}>
                                    {
                                        item.selected === 1 ?
                                            <Icon name={"check-circle"} color={appcolor.danger} size={64} /> :
                                            <View style={{
                                                display: deleted === true ? 'flex' : 'none',
                                                borderWidth: 3, borderColor: appcolor.dark, height: 50,
                                                width: 50, borderRadius: 90
                                            }} />
                                    }
                                </View>
                            </TouchableOpacity>
        ...
        return (
            <View style={{ padding: 7 }}>
                <View style={{ flexDirection: 'row' }}>
                    <Text style={{ fontWeight: 'bold', color: appcolor.dark, fontSize: scaleSize(20), }}>{item.nameVN}</Text>
                    <Badge value={item?.numPhoto} />
                </View>
                {item.numberIMG > 0 &&
                    < Text style={{
                        color: item.numPhoto < item.numberIMG ? appcolor.danger : appcolor.dark,
                        textDecorationLine: item.numPhoto < item.numberIMG ? 'underline' : 'none',
                        fontSize: scaleSize(12), fontStyle: 'italic'
                    }}>
                        {(item.subName ? item.subName : '') + (item.numberIMG ? `(Chụp tối thiểu ${item.numberIMG} tấm)` : '')}
                    </Text>
                }
                <View style={{ backgroundColor: appcolor.light, padding: 7, borderRadius: 12, marginTop: 7 }}>
    ...
    const onUpload = async () => {
        ...
        const imageGroup = route.params?.dataImageList || itemReport?.ImageByList || []
        const uiTask = [<Text key={"aaa"} style={{ padding: 12, color: appcolor.dark, fontSize: scaleSize(18) }}>Bạn chưa chụp đủ hình ảnh các hạng mục</Text>]
        const photoChecks = await Promise.all((imageGroup || []).map(async (item, index) => {
            let photoType = `${item.code}`
            let lstPhoto = await getPhotosReport(reportId, photoType, workinfo.shopId, workinfo.workDate) || [];
            const photoSize = lstPhoto.length || 0
            return {
                isEnough: photoSize >= item.numberIMG,
                content: <View key={index + "-29dkl"}>
                    <View style={{ padding: 7, flexDirection: 'row' }}>
                        <Text style={{
                            flexGrow: 1,
                            textDecorationLine: item.numberIMG > photoSize ? 'underline' : 'none',
                            padding: 3, color: item.numberIMG > photoSize ? appcolor.danger : appcolor.dark
                        }}>{item.nameVN}</Text>
                        <Text style={{ padding: 3, color: appcolor.dark }}>{photoSize}/{item.numberIMG}</Text>
                    </View>
        ...
        if (isDone === false) {
            ...
            if (result.statusId === 200) {
                await QueryStringSql(`UPDATE ${taskList.tableName} SET taskDone=1,taskAlter='Đã hoàn thành' WHERE shopId=${workinfo.shopId} AND reportId=${reportId}`)
                setLocked(true);
        ...
        <View style={{ height: '100%', backgroundColor: appcolor.surface }}>
            {
                route.params.hideIcon ?
                    ...
                    <HeaderCustom
                        title={dataKPIData?.menuNameVN || kpiinfo.menuNameVN}
                        rightFunc={(!locked && (parseInt(moment(new Date()).format('YYYYMMDD'))) == workinfo.workDate) ? () => onUpload() : null}
            ...
            }
            {
                ...
                <ViewShot
                    key={'imageSave'}
                    ref={canvasRef}
                    style={{ position: 'absolute', top: -10000, width: itemImage.widthViewShot === 0 ? '100%' : itemImage.widthViewShot, height: itemImage.heightViewShot === 0 ? '80%' : itemImage.heightViewShot }}
                    options={{ format: "jpg", quality: 0.8 }}>
                    <Image source={{ uri: itemImage.uri }} style={{ width: itemImage.widthViewShot, height: itemImage.heightViewShot }} />
                    <Svg height={itemImage.heightViewShot} width={itemImage.widthViewShot} style={{ position: 'absolute', top: 0, left: 0 }}>
                        <SvgText x="10" y="12" fill="red" fontSize="7px" fontFamily="Arial" fontWeight="bold">{shopinfo.shopName} - {shopinfo.shopCode}</SvgText>
                        <SvgText x="10" y="20" fill="red" fontSize="7px" fontFamily="Arial" fontWeight="bold">{shopinfo.address}</SvgText>
                        <SvgText x={10} y={itemImage.heightViewShot - 10} fill="red" fontSize="7px" fontFamily="Arial" fontWeight="bold">{dataKPIData?.menuNameVN || kpiinfo.menuNameVN} || {moment(new Date()).format("YYYY-MM-DD HH:mm:ss")}</SvgText>
                    </Svg>
                </ViewShot>
            }
            <FlatList
            ...
            <TouchableOpacity onPress={() =>
                removeList.length === 0 ? setDeteted(false) : _confirmSheet.current.show()
            } style={{ display: deleted > 0 ? 'flex' : "none" }}>
                <View style={{ backgroundColor: appcolor.dark, flexDirection: 'row', alignItems: 'center', padding: 12, justifyContent: 'center' }}>
                    <Icon name={removeList.length === 0 ? "rotate-left" : "restore-from-trash"} size={32}
                        color={appcolor.danger} />
                    <Text style={{ textAlignVertical: 'center', color: appcolor.light, fontSize: 18 }}>
                        {
                            removeList.length === 0 ? "Hủy bỏ" :
                                "(" + removeList.length + ")Xóa"
                        }
                    </Text>
                </View>
            </TouchableOpacity>
            <BottomConfirm onConfirm={onDeleteFile} ref={_confirmSheet}
            ...
            <ActionSheet id="sheetalter" containerStyle={{ backgroundColor: appcolor.light }}
                ref={_errorPage}>
