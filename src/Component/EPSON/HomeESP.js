...
import { View, Text, AppState, Dimensions, Platform, LogBox } from 'react-native';
...
import { fetchGet, GetEmployeeInfo, MessageInfo, ManageMessenger, saveStore, getStore } from '../../Core/Helper';
import { getIdMaxMessenger, getLstMessengerNotSeen, getCategoryPro, getStoreList } from '../../Controller/WorkController';
import { AppNameBuild, DEFAULT_COLOR, URL_DOWNLOAD_MESSENGER, _competitorId, URL_GET_SELLOUT_CHART_HMD, artApp, MENU_TYPE, URL_UPLOAD_ATTENDANT } from '../Core/URLs';
...
class HomeEPS extends PureComponent {
    ...
    goHyperlink = (hyperLinks) => {
        let lstNextPage = ['WorkingPlanPG', 'WorkingPlanSR', 'WorkingPlan', 'confirmplansr', 'confirmplanpg']
        if (lstNextPage.includes(hyperLinks)) {
            this.props.navigation.navigate(hyperLinks);
        }
        else {
            this.props.navigation.navigate('WebView', { link: hyperLinks, titlePage: 'browser' });
        }
    }
    SyncData = async () => {
    ...
    async componentDidMount() {
        ...
        (Object.keys(this.props.appcolor).length === 0) && this.props.GAppController.SetTheme({ type: "MODE", mode: true })
        messaging()
                ...
                if (remoteMessage !== undefined && remoteMessage !== object && remoteMessage !== null) {
                    if (remoteMessage.data.hyperLinks !== null) {
                        if (remoteMessage.data.hyperLinks !== "") {
                            ...
                            this.props.navigation.navigate('Notification', { messengerId: parseInt(remoteMessage.data.messengerId) });
                        }
                    }
                    else {
                        this.props.navigation.navigate('Notification', { messengerId: parseInt(remoteMessage.data.messengerId) });
    ...
    loadChartSlide = async () => {
        if (AppNameBuild === 'hmd' || AppNameBuild === 'mevn' || AppNameBuild === 'lg') {
            ...
            if (dataJson !== undefined) {
                ...
                try {
                    ...
                    this.loadTitleChart()
                } catch (error) { console.log('Load data chart false: ', error) }
            }
        }
        else {
            this.setState({ chartList: [{ pageName: 'DAILY', id: 1 }, { pageName: 'WEEKLY', id: 2 }, { pageName: 'MONTH', id: 3 }] })
            this.loadTitleChart()
        }
    }
            ...
            await Store().then(async (db) => {
                try {
                    lst.forEach(async element => {
                        await InsertItems(db, 'messenger', [{ id: element.id, title: element.title, body: element.body, typeReport: element.typeReport, createdDate: element.createdDate, seen: 0, hyperLinks: element.hyperLinks }]);
                    });
                } catch (errorStr) { console.log('EEE ' + errorStr) }
            })
            await delay(500);
            this.setBadge();
        }
    ...
    DisplayChart(index) {
        if (index == 0) {
            return (<ChartDaily ref={(ref) => this._ChartDaily = ref} category={this.state.selectedCat} appcolor={this.props.appcolor} />);
        } else if (index == 1) {
            return (<ChartWeekly ref={(ref) => this._ChartWeekly = ref} category={this.state.selectedCat} appcolor={this.props.appcolor} />);
        }
        else
            return (<ChartMonthly ref={(ref) => this._ChartMonth = ref} category={this.state.selectedCat} appcolor={this.props.appcolor} />);
    }
            ...
            case 'DAILY':
                viewResult.push(
                    <View key={index} style={{ flex: 1, }}>
                        <ChartDaily ref={(ref) => this._ChartDaily = ref} category={this.state.selectedCat} pageIndex={index} appcolor={this.props.appcolor} />
                    </View>
                )
                break;
            case 'WEEKLY':
                viewResult.push(
                    <View key={index} style={{ flex: 1, }}>
                        <ChartWeekly ref={(ref) => this._ChartWeekly = ref} category={this.state.selectedCat} pageIndex={index} appcolor={this.props.appcolor} />
                    </View>
                );
                break;
            case 'MONTH':
                viewResult.push(
                    <View key={index} style={{ flex: 1, }}>
                        <ChartMonthly ref={(ref) => this._ChartMonth = ref} category={this.state.selectedCat} pageIndex={index} appcolor={this.props.appcolor} />
                    </View>
                );
                break;
            default:
                viewResult.push(
                    <View key={index} style={{ flex: 1 }}>
                        <ChartSellHMD ref={(ref) => this._ChartDaily = ref} category={this.state.selectedCat} pageIndex={index} chartList={this.state.chartList} showDetailHome={this.showDetailHome} LoadData={this.loadChartSlide} loading={this.state.loading} pageSelected={this.state.activeSlide} appcolor={this.props.appcolor} />
                    </View>
                );
                break;
        }
        return (
            <View style={{ width: '100%', height: Platform.OS === 'android' ? Dimensions.get('window').height / 2.7 : Dimensions.get('window').height / 3 }}>
                {
                    viewResult
                }
            </View>
    ...
    get paginationDot() {
        ...
        return (
            <Pagination containerStyle={{ margin: 0, maxHeight: 20, padding: 0, marginTop: 0, position: 'absolute', bottom: -10, width: '100%' }}
                dotsLength={this.state.chartList.length > 5 ? 5 : this.state.chartList.length}
    ...
    loadTitleChart = () => {
        ...
        if (AppNameBuild !== 'hmd' && AppNameBuild !== 'mevn' && AppNameBuild !== 'lg') {
            if (index === 1) {
                title += "hằng tuần"
                this._ChartWeekly && this._ChartWeekly.ChartSetup(this.state.selectedCat)
            }
            else if (index === 2) {
                title += "hằng tháng"
            ...
            title = (this.state.chartList.length > 0 ? this.state.chartListFull[0].chartName + ' ' + this.state.chartList[index].pageName : '')
        }
            ...
            <View style={{ width: '100%', height: '100%', backgroundColor: this.props.appcolor.light }}>
                ...
                <View style={{ height: '100%', flexDirection: 'column' }}>
                    ...
                    <View style={{ height: '40%', padding: 7, marginBottom: 10 }}>
                        ...
                        <View style={{
                            width: '100%', backgroundColor: this.props.appcolor.light
                        }}>
                            <View style={{ width: '100%', backgroundColor: this.props.appcolor.primary, borderColor: 1, borderRadius: 12 }}>
                                <Text style={{ padding: 10, color: this.props.appcolor.light, fontSize: 17 }}>{this.state.titlePage}</Text>
                                <View style={{ borderColor: this.props.appcolor.light, borderBottomRightRadius: 10, borderTopRightRadius: 10, width: '80%', height: 10, borderWidth: 1 }} />
                                <View style={{ borderColor: this.props.appcolor.primary, width: '100%', height: 10, borderWidth: 1 }} />
                            </View>
                            <Icon
                                name='refresh-circle-outline'
                                color={this.props.appcolor.light} type='ionicon'
                                size={35}
                                containerStyle={{ height: '14%', position: 'absolute', right: 20, top: 5, justifyContent: 'center' }}
                                onPress={() => {
                                    this.loadChart()
                                }}
                            />
                            ...
                            <View style={{ height: Platform.OS === 'ios' ? (iphonex ? '85%' : '80%') : '80%', borderWidth: 0.5, borderColor: this.props.appcolor.greylight, borderBottomLeftRadius: 10, borderBottomRightRadius: 10, backgroundColor: this.props.appcolor.light, padding: 0 }}>
                                <Carousel
                                    initialNumToRender={this.state.chartList.length}
                                    ref={(c) => { this._carousel = c; }}
                                    data={this.state.chartList}
                                    slideStyle={{ backgroundColor: this.props.appcolor.light, right: 15, height: '95%' }}
                                    layout='default'
                    ...
                    <View style={{ height: '40%', padding: 7, top: 0 }}>
                        <View style={{
                            width: '100%', borderColor: 2, borderRadius: 12, backgroundColor: this.props.appcolor.primary,
                            // shadowOpacity: 0.7, shadowColor: DEFAULT_COLOR, shadowRadius: 10
                        }}>
                            <Text style={{ padding: 10, color: this.props.appcolor.white, fontSize: 17 }}>{'Chức năng'}</Text>
                            <View style={{ borderColor: this.props.appcolor.light, borderBottomRightRadius: 10, borderTopRightRadius: 10, width: '80%', height: 10, }} />
                            <View style={{ borderColor: this.props.appcolor.primary, width: '100%', height: 10, borderWidth: 1 }} />

                            <Icon
                                name={this.state.displayMenu === 0 ? 'list-outline' : 'grid-outline'}
                                color='white' type='ionicon'
                                size={30}
                                containerStyle={{ height: '14%', position: 'absolute', right: 20, top: 10, justifyContent: 'center' }}
                                onPress={() => {
                                    this.changeDisplayMenu()
                                }}
                            />
                            <View style={{ height: '86%', borderWidth: 0.5, borderColor: this.props.appcolor.greylight, borderBottomLeftRadius: 10, borderBottomRightRadius: 10, backgroundColor: this.props.appcolor.light, padding: 5 }}>
                                <MainMenu ref={ref => this.menuRef1 = ref} {...this.props} updateApp={this.state.isNewApp} loadCate={() => this.loadCategory(_competitorId)} menuDisplay={this.state.displayMenu} employee={this.props.userinfo} appcolor={this.props.appcolor} />
                            </View>
                        </View>
                    </View>

                </View>
                <MessageForm ref={ref => this.messageRef = ref} animation={'slideY'} appcolor={this.props.appcolor}
                    position={'top'} type={this.props.appcolor.primary} messageHeight={100} callBack={() => this.gotoMessage()}>
                    {this.state.isShowNotify && this.messageRef?.showMessage(this.state.notifyInfo?.notification?.title, this.state.notifyInfo?.notification?.body, 7000)}
                </MessageForm>
                <ActionSheet
