...
export const InputPOP = ({ navigation }) => {
  ...
  const handerNumberChange = (item, e) => {
    let text =
      e !== null && e.length > 0 ? e.toString().replace(/,/g, '') : 'null';
  ...
  const showResult = async () => {
    ...
    await reportItem.ImageByList?.forEach(async (item, index) => {
      ...
      await _result.push({
        title: `${item.nameVN}`,
        value: `(${photoSize / item.numberIMG})`,
        done: item.numberIMG > photoSize ? false : true,
      });
    });
  ...
  const onSummitReport = async () => {
    if (countInput === 0) {
      ...
      //
      await POSMContext.UploadReport(
                      ...
                      >
                        <Text
                          style={{
                            flexGrow: 1,
                            textDecorationLine:
                              item.done === false ? 'line-through' : 'none',
                            padding: 3,
                            color:
                              item.done === false
                                ? appcolor.danger
