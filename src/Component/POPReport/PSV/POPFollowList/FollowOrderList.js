...
export const FollowOrderList = ({ navigation, route }) => {
  ...
  const handleShowDetailItem = async item => {
    ...
    await navigation.navigate('orderfollowdetail', {
      dataOrderDetail: item,
      itemTab: itemTab,
    });
  };
  const handlerClearOrder = item => {
    Message(
      'Chú ý',
      `Bạn có chắc chắn muốn huỷ ${item.orderNo} không?`,
      async () => {
  ...
  return (
    <View style={styles.mainContainer}>
      <View style={{ height: deviceHeight / 10 }}>
        <HeaderCustom
          title={
            route.params?.popMenu.wareHouseName ||
            route.params?.popMenu.menuName
          }
