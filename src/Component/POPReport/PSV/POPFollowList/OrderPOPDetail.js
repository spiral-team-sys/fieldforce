...
export const OrderPOPDetail = ({ navigation, route }) => {
  ...
  const uploadAction = async type => {
    ...
    if (dataOrderDetail.status === 'DELIVERY') {
      for (let index = 0; index < dataUpload.length; index++) {
        ...
        if (!isValid(item.QuantityDamaged)) {
          ToastError(
            `${item.POPName} - Số lượng hư hỏng không được để trống`,
            'Thông báo',
            'top',
          );
          return;
        ...
        if (!isValid(item.QuantityPickup)) {
          ToastError(
            `${item.POPName} - Số lượng nhận được không được để trống`,
            'Thông báo',
            'top',
          );
          return;
        ...
        if (item.QuantityDamaged > item.QuantityPickup) {
          ToastError(
            `${item.POPName} - Số lượng hư hỏng không được lớn hơn số lượng nhận được`,
            'Thông báo',
            'top',
          );
          return;
    ...
    Message(
      ...
      async () => {
        ...
        const jsonUpload = {
          ...dataOrderDetail,
          content: JSON.stringify(dataUpload),
        };
        await updatePOP(
          type,
          JSON.stringify(jsonUpload),
          async success => {
            if (success.statusId) {
  ...
  const handerNumberChange = (item, value, field) => {
    ...
    if (field === 'QuantityDamaged' && value > item.QuantityPickup) {
      ToastError(
        'Số lượng hư hỏng không được lớn hơn số lượng nhận được!!',
        'Thông báo',
        'top',
      );
      setReloadNum(reloadNum + 1);
  ...
  const deleteItemOrder = item => {
    data[1].orderDetail = data[1].orderDetail.filter(
      it => it.POPId !== item.POPId || it.popId !== item.popId,
    );
    setMutate(e => !e);
  };
  const renderOrderDetailItem = ({ item, index }) => {
    return (
      ...
      >
        ...
        >
          <Text style={{ padding: 5, fontWeight: '700', color: appcolor.dark }}>
                ...
                >
                  ...
                  <View style={{ flex: 2 / 5 }}>
                    <NumPad_V2
                      ...
                      placeholderText={'SL'}
                      item={item}
                      showIcon={dataOrderDetail.status === 'SEND'}
                      editable={dataOrderDetail.status === 'SEND'}
                      reloadNum={reloadNum}
                      handerNumberChange={(item, value) =>
                        handerNumberChange(item, value, 'UserInput')
                      }
                    />
                  </View>
                </View>
                ...
                >
                  ...
                  <View style={{ flex: 2 / 5 }}>
                    <NumPad_V2
                      ...
                      reloadNum={reloadNum}
                      handerNumberChange={(item, value) =>
                        handerNumberChange(item, value, 'QuantityEdit')
                      }
                    />
                  </View>
                </View>
                ...
                >
                  ...
                  <View style={{ flex: 2 / 5 }}>
                    <NumPad_V2
                      ...
                      editable={dataOrderDetail.status === 'DELIVERY'}
                      reloadNum={reloadNum}
                      handerNumberChange={(item, value) =>
                        handerNumberChange(item, value, 'QuantityDamaged')
                      }
                    />
                  </View>
                </View>
                ...
                >
                  ...
                  <View style={{ flex: 2 / 5 }}>
                    <NumPad_V2
                      ...
                      reloadNum={reloadNum}
                      handerNumberChange={(item, value) =>
                        handerNumberChange(item, value, 'QuantityPickup')
                      }
                    />
                  </View>
                </View>
          ...
          >
            ...
            >
              <FlatList
                ...
                renderItem={
                  index == 0 ? renderOrderStatusItem : renderOrderDetailItem
                }
              />
            </View>
          </Tabs.Tab>
  ...
  return (
    ...
    >
      ...
      <LoadingView
        ...
        styles={{ backgroundColor: appcolor.light }}
      />
      <KeyboardAvoidingView
