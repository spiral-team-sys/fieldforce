...
export const POPMenu = ({ navigation, route }) => {
    ...
    const ColorRand = (index) => {
        const colorList = ['#FF5F1F', '#32CD32', '#FFBA00', '#4169E1', '#822659', '#228B22', '#40B5AD', '#6495ED', '#FF3131']
    ...
    const handlerItemPress = (item) => {
        ...
        if (item.pageName !== null && item.pageName.length > 0)
            ...
            ToastSuccess("Chức năng hiện tại đang trong quá trình hoàn thiện, Vui lòng bỏ qua mục này !")
    }

    const styles = StyleSheet.create({
        mainContainer: { flex: 1, backgroundColor: appcolor.surface },
        itemView: { flex: 1, flexWrap: 'wrap', margin: 5, padding: 8, backgroundColor: appcolor.light, borderRadius: 10 },
        titleMenu: { width: '100%', fontSize: 14, fontWeight: '500', color: appcolor.dark, textAlign: 'center', marginTop: 8 },
        titleDetailMenu: { width: '100%', fontSize: 18, fontWeight: '700', color: appcolor.dark, textAlign: 'center', marginTop: 8 }
    })
    const renderItem = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity key={`I_D_${index}`} style={{ flex: 1 }} onPress={onItemPress}>
                <View style={styles.itemView}>
                    <View style={{ width: '100%', alignItems: 'center' }}>
                        <View style={{ width: 70, height: 70, backgroundColor: appcolor.surface, padding: 16, borderRadius: 100, justifyContent: 'center' }}>
                            <Icon
                                name={item.imageItem} type='font-awesome-5' size={25}
                            ...
                            />
                            {
                                ...
                                <Badge
                                    ...
                                    badgeStyle={{ backgroundColor: appcolor.alert, width: 30, height: 30, borderRadius: 50 }}
                                    value={item.numberNotify > 99 ? '99+' : item.numberNotify} />
                            }
                        </View>
                        <View style={{ width: '100%' }}>
                            {item.detailMenu !== null && item.detailMenu.length > 0 && <Text style={styles.titleDetailMenu}>{item.detailMenu}</Text>}
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <FlatList
                ...
                refreshControl={<RefreshControl refreshing={false} onRefresh={LoadMenu} />}
            />
        </View>
    )
}