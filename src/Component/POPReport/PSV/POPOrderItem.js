...
import { DeviceEventEmitter, FlatList, Keyboard, Modal, SafeAreaView, StyleSheet, TouchableOpacity, View } from "react-native"
import { useSelector } from "react-redux"
import { HeaderCustom } from "../../../Content/HeaderCustom"
import { LoadingView } from "../../../Control/ItemLoading"
import { POPTotalWarehouse, UploadPOPOrder } from "../../../Controller/POPController"
import { checkNetwork, deviceWidth } from "../../../Core/Utility"
import FormGroup from "../../../Content/FormGroup"
import { Badge, Text, Image } from '@rneui/themed'
import AnimatedLottieView from "lottie-react-native"
import { useRef } from "react"
import { Message, MessageInfo, ToastError, ToastSuccess, UUIDGenerator } from "../../../Core/Helper"
...
export const POPOrderItem = ({ navigation, route }) => {
    ...
    const uploadAction = async () => {
        const listUpload = listOrder.filter(it => it.UserInput >= 0 && it.UserInput !== null)
        ...
        Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', async () => {
            ...
            await UploadPOPOrder(JSON.stringify(itemOrder), async (success) => {
                if (success.statusId) {
    ...
    const filterOrderData = async () => {
        ...
        let lstData = dataProduct.dataFilter.filter(it => it.UserInput !== null && it.UserInput > 0)
        if (orderFilter) {
            await setDataProduct({ ...dataProduct, dataView: lstData })
        } else {
            await setDataProduct({ ...dataProduct, dataView: dataProduct.dataFilter })
            await setSearch('')
        }
    ...
    const handlerItemSelect = async (item, index) => {
        ...
        let dataViewFilter = dataProducts.filter(it => it.UserInput !== null && it.UserInput > 0)
        await setDataProduct({ dataView: (isOrder === true ? dataViewFilter : (search.length > 0 ? dataViewSearch : dataProducts)), dataFilter: dataProducts })
    ...
    const RenderItemGroup = ({ item, index }) => {
        ...
        const totalRow = JSON.parse(item.detailData || [])?.filter(i =>
            isOrder ? (i.UserInput !== null && i.UserInput > 0) : (i.POPName.toLowerCase().match(search.toLowerCase()))).length || 0
        return (
            <TouchableOpacity
    ...
    const handerNumberChange = (item, value) => {
        if (value > item.Quantity) {
            MessageInfo("Số lượng đặt hàng không được lớn hơn số lượng kho tổng!!", "Thông báo", "top");
            setReloadNum(reloadNum + 1)
            return;
        }
        let orderItem = { ...item, UserInput: value ? value : null, QuantityEdit: value ? value : null }
        ...
        const indexProduct = dataProduct.dataView.findIndex(it => it.POPId === item.POPId)
        const indexProductF = dataProduct.dataFilter.findIndex(it => it.POPId === item.POPId)
            ...
            <View key={`I_P_P_${index}`} style={{ flex: 1, padding: 8, flexDirection: 'row', borderBottomWidth: 0.5, borderColor: appcolor.light }}>
                ...
                <View style={{ flex: 1, padding: 5, paddingLeft: 20, justifyContent: 'space-between' }}>
                    <View style={{ flex: 1 }}>
                        ...
                        {item.Quantity ? <Text style={{ ...styles.textItem, color: appcolor.red }}>Tồn kho tổng : {item.Quantity}</Text> : null}
                        {(item.QuantityMyHouse == 0 || item.QuantityMyHouse) ? <Text style={styles.textItem}>Tồn kho cá nhân : {item.QuantityMyHouse}</Text> : null}
                    </View>
                </View>
                <NumPad_V2
                    containerStyle={{ width: '30%', bottom: 8, end: 8, position: 'absolute' }}
                    index={index}
                    value={item.UserInput}
                    item={item}
                    editable={true} reloadNum={reloadNum}
                    placeholderText={"SL"}
    ...
    const handlerClearOrder = (item) => {
        Message('Chú ý', `Bạn có chắc chắn muốn xóa sản phẩm ${item.POPName} đã đặt này không?`,
            async () => {
    ...
    const handlerAddress = async () => {
        await AttendantController.LocationFromAddress(employeeAddress, async (dataLocation) => {
            setEmpoyeeAddres(dataLocation.address)
        }, () => alertError('Không tìm thấy địa chỉ, vui lòng kiểm tra và thử lại'))
    }
    const handleCloseModal = () => {
        const listOrderFilter = listOrder.filter(it => it.UserInput >= 0 && it.UserInput !== null)
        setListOrder(listOrderFilter)
        setVisible(false)
    }
    return (
                        ...
                        {listOrder.length > 0 && listOrder.map((item, index) => {
                            return (
                                <GmailStyleSwipeableRow
                                    ...
                                    <View style={{ flex: 1, padding: 8, flexDirection: 'row', borderBottomWidth: 0.6, borderColor: appcolor.grayLight, backgroundColor: appcolor.light }}>
                                        <View style={{ backgroundColor: appcolor.surface, width: 80, height: 80, borderRadius: 15, padding: 8 }}>
                                            {item.Image !== null && item.Image?.length > 0 ?
                                                <Image source={{ uri: item.Image }} style={{ width: '100%', height: '100%' }} /> :
                                                <AnimatedLottieView autoPlay={false} source={require('../../../Themes/lotties/no_image.json')} />
                                            }
                                            <Badge
                                                ...
                                                textStyle={{ color: appcolor.light, fontSize: 11, fontWeight: '500' }}
                                                badgeStyle={{ minWidth: 30, height: 25, backgroundColor: appcolor.blacklight, borderRadius: 50 }}
                                                value={item.Quantity}
                                            />
                                        </View>
                                        <View style={{ flex: 1, padding: 5, paddingLeft: 18 }}>
                                            <View style={{ flex: 1 }}>
                                                <Text style={styles.titleItem}>{`${index + 1}. ${item.POPName}`}</Text>
                                                <Text style={{ ...styles.textItem, color: appcolor.greylight }}>Tồn kho cá nhân : {item.QuantityMyHouse || 0}</Text>
                                            </View>
                                            <NumPad_V2
