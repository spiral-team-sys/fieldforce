...
export const POPWarehouse = ({ navigation, route }) => {
  ...
  const handlerUpload = async () => {
    await updatePOP(
      'UPDATEWARE',
      JSON.stringify(dataProduct.dataView),
      result => {
  ...
  const showActionInput = (value, item) => {
    if (item.Quantity > 0) {
      setItemEdit(item);
      value ? actionRef.current?.show() : actionRef.current?.hide();
    }
  };
  ...
  const handlerItemSelect = async (item, index) => {
    ...
    await setDataProduct({
      dataView: search.length > 0 ? dataViewSearch : dataProducts,
  ...
  const handlerDamaged = value => {
    ...
    let mDataChange = dataProduct.dataFilter.map(i =>
      i.POPName == itemEdit.POPName
        ? { ...i, DamagedInWarehouse: valueChange }
        : i,
    );
        ...
        <View key={`I_P_P_${index}`} style={{ width: '100%', padding: 8 }}>
          ...
          {isWarehouse == 0 && (
            <Text style={styles.numberItem}>
              Hư hỏng khi vận chuyển: {item.DamagedShipping}
            </Text>
          )}
          {isWarehouse == 0 && (
            <Text style={styles.numberItem}>
              Hư hỏng khi nhập kho: {item.DamagedInWarehouse}
            </Text>
          )}
        </View>
      </TouchableOpacity>
  ...
  return (
    <View style={styles.mainContainer}>
      <HeaderCustom
        ...
        iconRight={
          route.params?.popMenu.isWarehouse == 1 ? null : 'cloud-upload-alt'
        }
      />
      <FormGroup
        editable
      ...
      >
        <View style={{ width: '100%', height: deviceHeight / 3.5 }}>
          <Text style={{ ...styles.titleItem, fontSize: 16, padding: 8 }}>
            {itemEdit?.POPName}
          </Text>
          <Text
            style={{
