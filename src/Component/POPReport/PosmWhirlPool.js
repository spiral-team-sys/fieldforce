...
const ViewHeaderPOSM = ({
  ...
  const onSendReport = async () => {
    const noteProduct = listData.filter(
      it =>
        (it.posmNote == '' || it.posmNote == null || it.posmNote == 'null') &&
        it.defaultValue > 0 &&
        it.defaultValue !== null &&
        it.defaultValue !== (it.displayValue || 0),
    );
    ...
    if (noteProduct.length > 0 && reportItem.isDisplayTarget == 1) {
      ...
      errorView.push(
        <View
          key={'titleItem'}
          style={{ padding: 5, flexDirection: 'row', width: deviceWidth * 0.8 }}
        >
          <View style={{ width: '60%' }}>
            <Text
              style={{ fontWeight: '500', fontSize: 13, color: appcolor.dark }}
            >
              Sản phẩm
            </Text>
          </View>
          ...
          <View key={element.productId} style={{ width: deviceWidth * 0.8 }}>
            {element.isParent && (
              ...
              >
                ...
                >
                  Ngành hàng: {element.categoryName}
                </Text>
              </View>
            )}
      ...
      await setMessager(
        <View style={{ height: deviceHeight * 0.4 }}>
          <ScrollView style={{ flex: 1 }}>{errorView}</ScrollView>
          <View
            key={'noteView'}
            style={{
          ...
          >
            ...
            >
              Cần ghi chú các sản phẩm không đúng chỉ tiêu trưng bày
            </Text>
          </View>
        </View>,
      );
  ...
  const rowError = ({ item, index }) => {
    const errorCode =
      (item.countInput === 0 && item.code === 0) ||
      (item.countInput > 0 && item.code === 1);
    return (
      <Fragment key={`nkajda${index}`}>
        <View style={{ flexDirection: 'row', padding: 12 }}>
          <Text
            style={{
              textDecorationLine: errorCode ? 'line-through' : 'none',
              color: errorCode ? appcolor.danger : appcolor.dark,
              flexGrow: 1,
              padding: 7,
  ...
  return (
    <View>
      <HeaderCustom
        ...
        iconMiddle="search"
        middleFunc={
          Status === true ? null : () => SheetManager.show('_sheetAddMore')
        }
        title={kpiinfo.name}
      />
      {isVisible && (
      ...
      >
        {taskDone === true && (
          <View>
            ...
            <ScrollView
              style={{
                height: reportItem?.isUseNoteProduct !== 1 ? '80%' : '90%',
              }}
            >
            ...
            <View style={Styles.taskDoneBottomStyle}>
              ...
              />
              <TouchableOpacity
                style={{ padding: 7, flexGrow: 1, alignItems: 'center' }}
                onPress={() => {
                  setDone(false);
                  SheetManager.hide('_sheet');
                }}
