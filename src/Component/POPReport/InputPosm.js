...
import { View, Text, FlatList, ScrollView, TouchableOpacity, StyleSheet, UIManager, Platform, LayoutAnimation, } from "react-native"
...
export const InputPosm = memo(({ upload, reload }) => {
    ...
    const [dataPOSM, setDataPOSM] = useState({ dataPOSM: [], dataPOSMF: [], groupData: [] });
    ...
    const filterDoneProduct = async () => {
        if (!isDone) {
            let lstRes = dataPOSM.dataPOSM.filter(it => (it.displayValue !== null && it.displayValue > 0))
    ...
    const filterProduct = async (text) => {
        let dataSearch = []
        if (isDone)
            dataSearch = dataPOSM.dataPOSMF.filter(it =>
                (it.displayValue !== null && it.displayValue >= 0))
        else
            dataSearch = dataPOSM.dataPOSMF
    ...
    const setClearAll = async (itemCategory) => {
        if (!upload) {
            Message('Chú ý', `Bạn có chắc chắn muốn xóa hết dữ liệu ${itemCategory ? itemCategory.categoryName : ''} đã nhập ?`,
                async () => {
                    await POSMContext.PosmClearData(workinfo, itemCategory?.categoryId || null);
                    await setDone(false);
    ...
    const ButtonClearCate = () => {
        return (
            <TouchableOpacity
                ...
                onPress={() => setClearAll(dataPOSM.groupData[tabRef.current.getCurrentIndex()])}>
                <Text style={{ color: appcolor.danger, width: '80%', textAlign: 'center' }} >Xóa dữ liệu {dataPOSM.groupData[tabRef.current.getCurrentIndex()].categoryName} Đã nhập</Text>
        ...
        return (
            dataPOSM.groupData.map(
                (g, index) => {
                    ...
                    return (
                        <Tabs.Tab key={g.categoryName + `(${totalRow})`} label={g.categoryName + `(${totalRow})`} name={g.categoryName + `(${totalRow})`}>
                            <View style={Styles.viewTabStyle}>
                                <ItemPOSM item={g} index={index} upload={upload} dataPOSM={dataPOSM} dataByTab={dataByTab} Styles={Styles} reload={reloadView} reloadPOSM={reloadPOSM} />
                            </View>
                        </Tabs.Tab>
                    )
                }
    ...
    const Styles = StyleSheet.create({
        ...
        viewTabStyle: { flex: 1, backgroundColor: appcolor.surface, marginTop: 40, width: deviceWidth, display: !showProgress ? 'flex' : 'none' },
        flatListTab: { padding: 7, marginBottom: 7, backgroundColor: appcolor.surface },
        scrollSheet: { padding: 7, marginBottom: 20, flexGrow: 1 },
        closeSheet: { padding: 12, alignItems: 'center', marginBottom: 30 },
        itemListSheet: { padding: 7, flexDirection: 'row', justifyContent: 'space-between' },
        textSheet: { width: '58%', color: appcolor.dark },
        inputNumSheet: { width: '40%', },
        viewPOSM: { padding: 7, width: '100%' },
        textPOSMStyle: { padding: 3, color: appcolor.dark, fontSize: scaleSize(12) },
        textButtonPOSM: { marginStart: 3, fontSize: scaleSize(12), fontWeight: 'bold', color: appcolor.dark },
        viewItemRow: { padding: 5, borderRadius: 8, marginBottom: 7, backgroundColor: appcolor.light },
        viewTitleRow: { padding: 3, flex: 1, flexDirection: 'row' },
        modelNameStyle: { marginStart: 3, fontSize: 14, fontWeight: 'bold', color: appcolor.dark },
        subCatNameStyle: { color: appcolor.dark, opacity: 0.8, fontSize: 12, fontStyle: 'italic' },
        viewInputRow: { width: '100%', alignItems: 'flex-end', },
        viewNumPad: { flexDirection: 'column', minWidth: '35%', justifyContent: 'center', alignItems: 'center' },
        viewMidleRow: { flexDirection: 'row', alignItems: 'center', width: '100%', justifyContent: 'space-between' },
        viewFooterRow: { width: '100%', justifyContent: 'center', marginTop: 20, height: 50, backgroundColor: appcolor.surface, },
        actionSheetStyle: { padding: 8, width: '100%', height: '40%' },
        buttonSheetStyle: { width: '100%', flexDirection: 'row', alignItems: 'center', padding: 8, justifyContent: 'space-between', borderRadius: 20, borderWidth: 0.5, marginTop: 5 },
        textSheetStyle: { width: '80%', textAlign: 'center' },
        headerItem: { flex: 1, padding: 8, marginTop: 5, marginBottom: 3, borderRadius: 10, flexDirection: 'row', alignItems: 'center', backgroundColor: appcolor.primary, },
        headerText: { color: appcolor.white, fontSize: 14, paddingLeft: 8, fontWeight: '600' }
    })
    ...
    return (
        <View style={Styles.container}>
            ...
            {
                dataPOSM.groupData.length > 0 && dataPOSM.dataPOSM.length > 0 && !showProgress &&
                <Tabs.Container
                    ref={tabRef}
                    key={"TapCategory"}
                    ...
                    renderTabBar={(props) => (
                        <MaterialTabBar
                            ...
                            tabStyle={{ minWidth: minWidthTab(dataPOSM.groupData), height: 42 }}
                        />
                    )}
            ...
            <ActionSheet
                ...
                <View style={Styles.actionSheetStyle}>
                    <View style={{ width: '100%', height: 100 }}>
                        ...
                        <TouchableOpacity
                            style={[Styles.buttonSheetStyle, { borderColor: appcolor.dark, marginTop: 12 }]}
                            onPress={filterDoneProduct}>
                            <Text style={[Styles.textSheetStyle, { color: appcolor.dark }]} >Sản phẩm đã kiểm tra</Text>
...
const ItemPOSM = memo(({ item, indexList, upload, dataPOSM, dataByTab, Styles, reload, reloadPOSM }) => {
    ...
    return (
        <View style={Styles.container}>
            <FlatList data={dataItem} key={"ListItem_" + indexList}
                style={Styles.flatListTab}
                scrollToOverflowEnabled={true}
                initialNumToRender={7}
                keyExtractor={(_, index) => index.toString()}
                ListFooterComponent={<View style={{ height: deviceHeight / 3 }} />}
                renderItem={({ item, index }) => <RenderRow key={indexList + "_item_" + index} indexList={indexList} item={item} index={index} Styles={Styles} removeAdd={removeAdd} upload={upload} />} />
        </View >
    )
})
const RenderRow = memo(({ item, index, indexList, Styles, upload, removeAdd, }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    let jsonPosm = JSON.parse(item?.posmList && item?.posmList !== 'null' ? item?.posmList : `[{"posmId": 0,"posmName": "Thêm","productId":${item.productId}}]`)
    ...
    const showSheetPosm = async () => {
        let posmGuid = []
        if (item.posmList === null || item.posmList === 'null') {// Ngoài guidline
    ...
    const handerNumberChange = (item, e, type) => {
        let intValue = (type == 'displayValue' ? (e === null || e === '') ? null : parseInt(e) : (e == null || e == 'null' ? '' : e));
        item[type] = intValue
    ...
    return (
        <View key={indexList + '_View_' + index} style={Styles.container}>
            ...
            <View key={"dss" + index} style={Styles.viewItemRow}>
                {
                    !upload &&
                    <TouchableOpacity onPress={() => removeAdd(item)} style={{ width: 50, padding: 5, display: item.addMore ? 'flex' : 'none', flexDirection: 'row' }}>
                        <Icon name="minus-circle" type="font-awesome-5" color={appcolor.danger} />
                    </TouchableOpacity>
                }
                <View style={[Styles.viewTitleRow, { justifyContent: 'space-between', justifyContent: 'center', padding: 5 }]}>
                ...
                <View style={Styles.viewTitleRow}>
                    <FormGroup
                        ...
                        containerStyle={{ padding: 3, marginTop: 5, backgroundColor: appcolor.light, flex: 1 }}
                        inputStyle={{ fontSize: 13 }}
                        multiline editable={!upload}
                        value={item.posmNote || ''}
                ...
                <View style={Styles.viewInputRow}>
                    <View style={Styles.viewMidleRow}>
                        <View style={{ flexGrow: 1, flexDirection: 'row', alignItems: 'center', }}>
                            {
                                item.defaultValue > 0 &&
                                <Text style={Styles.textPOSMStyle}>Chỉ tiêu : {item.defaultValue}</Text>
                            }
                        </View>
                        <View style={Styles.viewNumPad}>

                            <NumPad_V2
                                inputStyle={{ backgroundColor: appcolor.surface, width: 60, padding: 6 }}
                                ...
                                editable={true}
                                // reloadNum={reloadNum}
                                handerNumberChange={(it, e) => handerNumberChange(it, e, 'displayValue')}
                            />
                        </View>
                    </View>
                    {
                        jsonPosm.length > 0 &&
                        <POSMUI key={item.productCode} props={{ "posmData": jsonPosm, "onShowPosm": showSheetPosm, "item": item, "Styles": Styles }} />
                    }
                </View>
            </View>
            ...
            >
                ...
                <TouchableOpacity onPress={() => _sheetPosm.current.hide()} style={Styles.closeSheet}>
                    <Text> Đóng </Text>
                </TouchableOpacity>
            </ActionSheet>
        </View>
            ...
            <ScrollView showsHorizontalScrollIndicator={false}
                horizontal={true}>
                {
                    posmData?.map((v, index) => {
                        return (
                            <View style={{
                                borderRadius: 40, marginEnd: 5,
                                backgroundColor: v.posmValue > 0 ? appcolor.surface : appcolor.warning
                            }} key={`${item.productId}ka02${index}`}>
                                <TouchableOpacity disabled={item.displayValue > 0 ? false : true}
                                    onPress={() => onShowPosm(item)} style={{ padding: 8 }}>
