...
export const AddressByShop = ({
  ...
  const renderItemTag = ({ item, index }) => {
    ...
    if (isUseMainStore && item.isMainStore !== 1)
      return <View key={`ma - ${index} `} />;
    return (
      <View key={`ma - ${index} `}>
        <TouchableOpacity
          onPress={onPress}
  ...
  const renderItemDistrict = ({ item, index }) => {
    ...
    const dataByDistrict = _.filter(
      dataByProvince,
      it =>
        it.districtCode == item.districtCode &&
        ((isUseMainStore && it.IsMainStore == 1) || !isUseMainStore),
    );
    return (
  ...
  const handleSelectStore = async itemD => {
    ...
    if (isNullLocation == 1 && Object.keys(dataItem.dataSelect)?.length > 0) {
      itemLocation.locationPoint = `${dataItem.dataSelect.geometry.location.lat || 0
    ...
    switch (typeItem) {
      case TYPE.PROVINCE_FROM:
        ...
        itemLocation.addressPoint =
          ...
          (itemTrips.districtFrom || itemTrips.provinceFrom
            ? `${itemTrips.districtFrom},${itemTrips.provinceFrom}`
            : null);
      ...
      case TYPE.PROVINCE_TO:
        ...
        itemLocation.addressPoint =
          itemTrips.addressTo ||
          (itemTrips.districtTo || itemTrips.provinceTo
            ? `${itemTrips.districtTo},${itemTrips.provinceTo}`
            : null);
      ...
      default:
        ...
        itemLocation.addressPoint =
          itemTrips.addressPoint ||
          (itemTrips.district || itemTrips.province
            ? `${itemTrips.district},${itemTrips.province}`
            : null);
  ...
  return (
    <View key={`${typeItem}_ViewItem`} style={{ width: '100%' }}>
      ...
      <View style={{ width: '100%', backgroundColor: appcolor.transparent }}>
        <TouchableOpacity
          key={`${typeItem}_button`}
          onPress={() => handleSelectProviceData()}
          style={{
            width: isUseMainStore ? '96%' : '80%',
            padding: 8,
            justifyContent: 'center',
            flexDirection: 'row',
            marginVertical: 4,
            ...
            <View style={{ width: '100%' }}>
              ...
              {listDistrict?.length > 0 && (
                <FlatList
                  ...
                  keyExtractor={(_item, index) =>
                    _item.provinceCode + _item.districtCode
                  }
                  data={listDistrict}
        ...
        {itemLocation.addressPoint && (
          ...
          >
            ...
            >
              Cửa hàng: {itemLocation.shopName}
            </Text>
            <Text
              style={{ fontSize: 11, fontWeight: '500', color: appcolor.dark }}
            >
              Địa chỉ: {itemLocation.addressPoint}
            </Text>
            <Text
              style={{ fontSize: 11, fontWeight: '500', color: appcolor.dark }}
            >
              Vị trí: {itemLocation.locationPoint}
            </Text>
          </View>
        )}
      </View>
