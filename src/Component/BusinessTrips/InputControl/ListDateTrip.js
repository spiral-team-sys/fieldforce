...
export const ListDateTrip = ({ defaultDate, itemTrips, typeItem, handleSelectDate, theme, reloadDate }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    const [dataCalendar, setDataCalendar] = useState({
        "markingType": 'period',
        "markedDates": { [defaultDate ? defaultDate : moment().format('YYYY-MM-DD').toString()]: { selected: true, marked: true, selectedColor: appcolor.surface } },
        "isStartDay": false,
        "isEndDay": false,
        "startDate": '',
        "endDate": ''
    ...
    const loadData = () => {
        ...
        if (!Array.isArray(itemTrips[typeItem] || [])) {
            listDate = itemTrips[typeItem]?.split(',').map(d => parseInt(d.trim(), 10)).filter(d => !isNaN(d))
        }
    ...
    const checkDateFromTo = (start, end, listDate) => {
        ...
        listDate.forEach(day => {
            const date = moment(`${start.format('YYYY-MM')}-${String(day).padStart(2, '0')}`, 'YYYY-MM-DD');
        ...
        while (current.isSameOrBefore(end)) {
            ...
            markedDates[dateStr] = {
                ...
                'customContainerStyle': isHighLight ? { borderRadius: 50, elevation: 3, backgroundColor: appcolor.yellow } : null,
                'startingDay': isStart,
                'endingDay': isEnd
            }
            current.add(1, 'day')
    ...
    const onSelectDay = (date) => {
        ...
        markedDates[dateString] = {
            'color': appcolor.yellowdark,
            'textColor': appcolor.dark,
            'customContainerStyle': (!markedDates[dateString].customContainerStyle ? { borderRadius: 50, elevation: 3, backgroundColor: appcolor.yellow } : null),
        ...
        <View style={{ width: '100%' }}>
            ...
            />
            {
                ...
                <Calendar
                    ...
                    firstDay={1}
                    minDate={moment(itemTrips.fromDate, 'YYYYMMDD').format('YYYY-MM-DD') || moment().startOf('month').format('YYYY-MM-DD') || null}
                    maxDate={moment(itemTrips.toDate, 'YYYYMMDD').format('YYYY-MM-DD') || moment().endOf('month').format('YYYY-MM-DD') || null}
                    current={moment(itemTrips.fromDate, 'YYYYMMDD').format('YYYY-MM-DD') || moment().format("yyyy-MM-DD")}
