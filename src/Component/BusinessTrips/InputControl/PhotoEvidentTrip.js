...
export const PhotoEvidentTrip = ({
  ...
  const onAddImage = (photo, dataNewPhoto) => {
    let ImgName = dataNewPhoto.photoPath.substring(
      dataNewPhoto.photoPath.lastIndexOf('/') + 1,
      dataNewPhoto.photoPath.length,
    );
    let fileName =
      '/uploaded/' + (dataNewPhoto.photoDate || TODAY) + '/' + ImgName;
    handlerAddImage(
      {
        ...photo,
        id: dataNewPhoto.id,
        photoPath: dataNewPhoto.photoPath,
        photo: fileName,
      },
      'add',
    );
  ...
  const actionCallBackResult = async (photoinfo, result) => {
    if (result.statusId == 200) {
      const dataPhotoLocal = await getDataPhotos(
        userinfo.employeeId,
        TODAY,
        photoType || 'workingschedule',
        kpiinfo.id,
