...
export const CheckBoxLists = ({
  ...
  const handlerAddImage = (photo, type) => {
    ...
    if (type === 'add') {
      ...
      itemTrips.photoByType = JSON.stringify(
  ...
  const renderItem = ({ item, index }) => {
    ...
    const listPhotoByCode =
      listPhotoByType.filter(
        i => i.photoType === 'Workingschedule_' + item.fieldCost,
      ) || [];
    return (
    ...
    <View style={{ flexGrow: 1, padding: 8, marginBottom: 1, paddingLeft: 12 }}>
      <View style={{ flexDirection: 'row', paddingVertical: 4 }}>
        <SpiralIcon
          ...
          color={appcolor.dark}
        />
        <Text style={{ fontSize: 14, fontWeight: '600', color: appcolor.dark }}>
          Chọn loại chi phí
        </Text>
      </View>
