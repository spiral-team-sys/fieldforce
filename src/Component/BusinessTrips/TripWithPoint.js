...
export const TripWithPoint = ({
  ...
  const loadData = () => {
    console.log(
      itemTrips.provinceList,
      'itemTrips.provinceListitemTrips.provinceList',
    );
  ...
  const handleSelectItem = (it, idx) => {
    if (
      itemTrips.locationStart != null &&
      itemTrips.locationStart != '' &&
      (typeVehicle?.code !== undefined || quotaData.isVehicleEmpty == 1)
    ) {
    ...
    switch (type) {
      case 'LIST_VEHICAL':
        if (quotaData.typeVehicleChose == 1) {
          ...
          console.log(
            itemTrips.typeVehicle,
            'dataSelectVehcledataSelectVehcle',
          );
  ...
  const handleSelectCheckBox = async (itemCheck, indexCheck, value) => {
    ...
    listTypeCost.forEach(it => {
      it.id == itemCheck.id ? (it.isChoose = value ? 1 : 0) : (it.isChoose = 0);
    });
    // xoas ảnh khi uncheck
    const photoByType = JSON.parse(itemTrips?.photoByType || '[]') || [];
    const listPhotoByType =
      photoByType.filter(
        i => i.photoType === 'Workingschedule_' + itemCheck.fieldCost,
      ) || [];
  ...
  return (
    <View style={{ flex: 1 }}>
      {listTypeCost.length > 0 && (
        <CheckBoxLists
          dataCheck={listTypeCost}
          handleSelectCheckBox={handleSelectCheckBox}
          itemTrips={itemTrips}
          guid={dateFilter.guid}
        />
      )}
      <ItemInput
        isRequire
              ...
              >
                ...
                >
                  ...
                  >
                    Điểm {it.id + 1}{' '}
                    {it.arrivedDay
                      ? `- ${moment(it.arrivedDay, 'YYYYMMDD').format(
                        'DD/MM/YYYY',
                      )}`
                      : ''}{' '}
                    {it.addressPoint == null || it.addressPoint == '' ? (
                  ...
                  {it.distance !== null && it.distance > 0 && (
                    ...
                    >
                      {aquaApp == AppNameBuild ? (
                        ...
                        >
                          Khoảng cách ~
                          {Math.round(it.distance / 1000) +
                            (it.numberStore || 0) * (quotaData.kmDefault || 5)}
                          km
                        </Text>
                      ) : (
                        <Text
                  ...
                  <View style={{ width: '100%', borderRadius: 4 }}>
                    ...
                    {AppNameBuild == aquaApp && (
                      ...
                      >
                        ...
                        >
                          Số lượng cửa hàng : {it.numberStore || 0}
                        </Text>
                      </View>
                    )}
                    {AppNameBuild !== aquaApp && (
                      ...
                      >
                        ...
                        >
                          Chi phí :{' '}
                          {
                            ...
                            >
                              Khách sạn :{' '}
                              {formatNumber(it.nightRestValue, ',') || 0} |{' '}
                            </Text>
                          }
                          {
                            <Text
