...
export const TripWithStages = ({ itemTrips, ItemInput }) => {
  ...
  const onPress = (typeFilter, itemValue) => {
    if (
      ...
      numStages != (itemTrips.provinceList?.length || 0)
    ) {
      if (itemValue > 1) {
        if (itemValue > 5) {
          Message(
...
const ViewCreateStage = ({ closeModal, modalConfig, handleSaveChange }) => {
  ...
  const AddMarker = dataState => {
    ...
    let _point = {};
    if (
      dataState?.locationStart != null &&
      dataState?.locationStart != undefined
    ) {
      const [latitudeStart, longitudeStart] =
        dataState.locationStart?.split(',');
      const floatLatitudeStart = parseFloat(latitudeStart);
      const floatLongtitudeStart = parseFloat(longitudeStart);

      if (
        floatLatitudeStart !== null &&
        isDecimal(floatLatitudeStart) &&
        isDecimal(floatLongtitudeStart)
      ) {
        polygon.push({
          latitude: floatLatitudeStart,
          longitude: floatLongtitudeStart,
        });
        _point = {
          latitude: floatLatitudeStart,
          longitude: floatLongtitudeStart,
    ...
    if (
      dataState?.locationEnd !== null &&
      dataState?.locationEnd != undefined
    ) {
      ...
      if (
        floatLatitudeEnd !== null &&
        isDecimal(floatLatitudeEnd) &&
        isDecimal(floatLongtitudeEnd)
      ) {
        polygon.push({
          latitude: floatLatitudeEnd,
          longitude: floatLongtitudeEnd,
        });
      ...
      <View style={{ flex: 1 }}>
        ...
        >
          <ScrollView>
            ...
            <View
              style={{ width: '100%', padding: 8, height: deviceHeight * 0.5 }}
            >
              <MapView
          ...
          {itemStage.startPosition !== undefined &&
            itemStage.startPosition !== null &&
            itemStage.startPosition !== '' && (
              <TouchableOpacity
                onPress={() => handleSaveChange(itemStage)}
                style={{
                  position: 'absolute',
                  bottom: 0,
                  left: 0,
