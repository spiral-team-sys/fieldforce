...
export const UploadDocument = ({
  ...
  const provincePlan = `${itemBussiness.provinceFrom}${!itemBussiness.districtFrom ? '' : `(${itemBussiness.districtFrom})`
    } - ${itemBussiness.provinceTo}${!itemBussiness.districtTo ? '' : `(${itemBussiness.districtTo})`
    }`;
  ...
  const loadData = async () => {
    ...
    await setDataDetail({
      dataTab: dataTab,
      hotelCosts: dataHotelCost[0],
      moveCosts: dataMoveCosts[0],
      supportKMCosts: dataSupportKMCosts[0],
      foodCosts: dataFoodCost[0],
      otherCosts: dataOtherCosts[0],
      workCosts: dataWorkCosts[0],
      dataUpload: {},
    });
  };
  ...
  const onCancelItem = () => {
    ...
    MessageAction('Bạn chắc chắn muốn bỏ quyết toán?', () =>
      uploadAction(dataUpload, 'cancel', 'Bỏ quyết toán'),
    );
  };
  const uploadAction = async (dataUpload, sendType, messageText) => {
    const result = await SendInvoice(
      JSON.stringify({ ...dataUpload, sendType: sendType }),
    );
    if (result.statusId === 200) {
  ...
  const onPressStage = async () => {
    ...
    for (let index = 0; index < listProvinceWork.length; index++) {
      ...
      pointUI.push(
        <View
          key={`ViewPoint_${element.workDate}_${element.provinceTo}_${element.addressPoint}`}
          style={{
            borderRadius: 8,
            backgroundColor: appcolor.surface,
            padding: 4,
        ...
        >
          ...
          >
            ...
            {element.distance !== null && element.distance > 0 && (
              ...
              >
                ...
                >
                  Khoảng cách ~{element.distanceText}
                </Text>
              </View>
            )}
          </View>
          <View style={{ width: '100%', borderRadius: 4 }}>
              ...
              >
                ...
                >
                  Địa chỉ :
                  {
                    ...
                    >
                      {element.addressPoint || element.shopVisit}
                    </Text>
                  }
                </Text>
              </View>
            ...
            {((element.amountHotel != null && element.amountHotel >= 0) ||
              (element.amountAllowance != null &&
                element.amountAllowance >= 0) ||
              (element.amountTransport != null &&
                element.amountTransport >= 0)) && (
                <View
                  style={{
                    width: '100%',
                    flexDirection: 'row',
                ...
                >
                  ...
                  >
                    Chi phí :{' '}
                    {
                      ...
                      >
                        Khách sạn : {formatNumber(element.amountHotel, ',') || 0}{' '}
                        |{' '}
                      </Text>
                    }
                    {
                      ...
                      >
                        Phụ cấp : {element.amountAllowance || 0} |{' '}
                      </Text>
                    }
                    {
                      <Text
                      ...
                      >
                        Di chuyển : {element.amountTransport || 0}
                      </Text>
                    }
                  </Text>
                </View>
    ...
    <View style={styles.mainContainer}>
      <View style={{ minHeight: 150, paddingHorizontal: 8 }}>
        ...
        >
          <TouchableOpacity
            style={{ height: 50 }}
            onPress={() => closeAction(dataDetail.dataUpload)}
          >
            <SpiralIcon reverse name="arrow-back" size={18} />
          </TouchableOpacity>
          <Text
            style={{ fontWeight: '800', fontSize: 20, color: appcolor.primary }}
          >
            Chi tiết chứng từ
          </Text>
          <TouchableOpacity
            style={{ height: 50, minWidth: 30 }}
            onPress={() =>
        ...
        {isShowType && (
          <View style={{}}>
            {((itemBussiness.isCanCancel == 1 &&
              itemBussiness.isCancelInView !== 1) ||
              (itemBussiness.isCanNext == 1 &&
                itemBussiness.isCanNextInView !== 1) ||
              itemBussiness.isCanSubmit == 1) && (
                <FormGroup
                  containerStyle={{
                    padding: 3,
                    marginTop: 5,
            ...
            >
              {itemBussiness.isCanNext == 1 &&
                itemBussiness.isCanNextInView !== 1 &&
                itemBussiness.isCanNextInView !== 2 && (
                  <ActionPress
        ...
        >
          {AppNameBuild == lgApp && listProvinceWork.length > 0 && (
            <View
              style={{
                width: '100%',
                flexDirection: 'row',
          ...
          <Text
            style={{ fontWeight: '600', fontSize: 11, color: appcolor.dark }}
          >
            Tên :{' '}
            {itemBussiness?.employeeCode + '-' + itemBussiness?.employeeName}
          </Text>
          <Text
            style={{ fontWeight: '300', fontSize: 11, color: appcolor.dark }}
          >
            Đợt công tác :{' '}
            {moment(itemBussiness?.fromDate).format('YYYY/MM/DD') +
              ' - ' +
              moment(itemBussiness?.toDate).format('YYYY/MM/DD')}
          </Text>
              ...
              {itemBussiness.isUsePoint == 1 ? (
                <View style={{ flexDirection: 'row' }}>
                  ...
                  <ScrollView
                    ...
                    nestedScrollEnabled
                  >
                    {itemBussiness.provinceFrom !== null &&
                      itemBussiness.provinceFrom !== undefined && (
                        <Text
                          key={`ProvinceStart}`}
                          style={{
                            fontSize: 11,
                        ...
                        >
                          {provinceByAddress(itemBussiness.provinceFrom)
                            .province || ''}{' '}
                          -
                        </Text>
                      )}
                    ...
                    {listProvinceWork.map((item, index) => {
                      return (
                        ...
                        >
                          {`${index > 0 ? ' -' : ''} ${item.provinceName} (${AppNameBuild == aquaApp
                              ? item.numberStore
                              : item.numberDay
                            })`}
                        </Text>
                    ...
                    >
                      {listProvinceWork.map((item, index) => {
                        return (
                          ...
                          >
                            {`${index > 0 ? ' -' : ''} ${item.provinceName}${!item.district ? '' : `(${item.district})`
                              } (${item.numberDay})`}
                          </Text>
                        );
                      })}
          ...
          >
            Số ngày đi :{' '}
            {`${itemBussiness?.dayWork} ngày - ${itemBussiness?.nightWork} đêm`}
          </Text>
          <Text
            style={{
              fontWeight: '600',
              fontSize: 12,
              color: itemBussiness.colorHightlight || appcolor.dark,
            }}
          >
            Trạng thái : {itemBussiness?.statusName || ''}
          </Text>
          {itemBussiness?.adminNote?.length > 0 && (
            <Text
              style={{ fontWeight: '600', fontSize: 12, color: appcolor.dark }}
            >
              Ghi chú Quản trị viên : {itemBussiness?.adminNote || ''}
            </Text>
          )}
          {itemBussiness?.adminNote?.length > 0 && (
            <Text
              style={{ fontWeight: '600', fontSize: 12, color: appcolor.dark }}
            >
              Thời gian ghi chú : {itemBussiness?.adminTime || ''}
            </Text>
          )}
        </View>
        {(itemBussiness.statusId === 3 || itemBussiness.statusId === 4) &&
          ...
          {dataDetail.dataTab.length > 0 && (
              ...
              renderTabBar={props => (
                <MaterialTabBar
                  ...
                  tabStyle={{
                    ...
                    minWidth: minWidthTab(dataDetail.dataTab),
                    height: 38,
                    marginEnd: 8,
                  }}
                  scrollEnabled={true}
            ...
            >
              {dataDetail.dataTab?.map((item, index) => {
                return dataDetail[item.ref_Name] ||
                  dataDetail[item.ref_Name]?.length > 0 ? (
                  <Tabs.Tab
                    key={`bill_${index}`}
                    label={`${item.nameVN}`}
                    name={`${item.nameVN}`}
                  >
                    <View
                      style={{ flex: 1, paddingTop: 38, width: deviceWidth }}
                    >
                      <ViewItemBill
                        ...
                        itemBussiness={itemBussiness}
                        filterInvoice={filterInvoice}
                        ItemView={ItemView}
                        isShowType={isShowType}
                        showTypeUpdate={showTypeUpdate}
...
const ViewListReturn = ({ dataDetail }) => {
  ...
  const loadData = () => {
    const listFilter = dataDetail.dataTab.filter(
      it =>
        dataDetail[it.ref_Name]?.statusId == 3 ||
        dataDetail[it.ref_Name]?.statusId == 4,
    );
    ...
    <View style={{ paddingTop: 2 }}>
      <View
        style={{
          ...
          borderWidth: 1,
          flexDirection: 'row',
          borderColor: isShowList ? appcolor.primary : appcolor.light,
          minHeight: 40,
        }}
      >
        {isShowList && (
...
const ViewNextCost = ({ itemTab, indexTab, itemNextCosts, dataDetail }) => {
    ...
    switch (type) {
      case 'COSTS':
        ...
        if (intValue > dataDetail[itemTab.ref_Name].costs) {
          ...
          const nextPeriod =
            dataDetail[itemTab.ref_Name].costs - (intValue || 0);
      ...
      case 'NEXTCOST':
        ...
        if (intValueCosts > dataDetail[itemTab.ref_Name].costs) {
          ...
          const inventoryCost =
            dataDetail[itemTab.ref_Name].costs - intValueCosts || 0;
  ...
  return (
    <View style={{ flex: 1, width: '100%', padding: 6 }}>
      ...
      >
        Chuyển chi phí {itemTab.nameVN} sang kỳ sau
      </Text>
      <Text
        style={{
          fontSize: 15,
...
const ActionPress = ({ type, title, onPress, colorAction, itemAction }) => {
  ...
  return (
    ...
    >
      ...
      <Text style={{ fontSize: 13, fontWeight: '600', color: appcolor.white }}>
        {title}
      </Text>
    </TouchableOpacity>
  );
...
const ViewItemBill = ({
  ...
  const countFile = fileUpload => {
    ...
    if (fileUpload) {
      fileUpload?.map(it => {
        it.type === 1
          ? (fileByLink = fileByLink + 1)
          : it.type === 2
            ? (fileByImage = fileByImage + 1)
    ...
    setQuantityFile({
      fileByDoc: fileByDoc,
      fileByLink: fileByLink,
      fileByImage: fileByImage,
    });
  };
        ...
        if (!response.didCancel) {
          ...
          if (assets !== undefined) {
            await assets?.forEach(async res => {
              ...
              await handleLoadPhoto(
                newImageUrl?.uri || res.uri,
                newImageUrl?.size || res.fileSize,
              );
            });
          }
        }
  ...
  const handleLoadPhoto = async (photoPath, fileSize) => {
    try {
      let arrBill = [...JSON.parse(dataDetail[itemTab.ref_Name].billFile)];
      let ImgName = photoPath.substring(
  ...
  const handleDelete = type => {
    LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
    const listDelete = JSON.parse(
      dataDetail[itemTab.ref_Name]?.billFile || '[]',
    ).filter(it => it.isSelect === true);
    ...
    if (listDelete.length > 0) {
      MessageAction('Bạn chắc chắn muốn xoá tệp tin chứng từ?', async () => {
        isShowItemDoc
          ...
          : isShowItemImage
            ? ref_ScrollImage.current.scrollTo({ x: 0, y: 0, animated: true })
            : ref_ScrollLink.current.scrollTo({ x: 0, y: 0, animated: true });
        dataDetail[itemTab.ref_Name].billFile = JSON.stringify(dataAfter);

        if (isShowItemDoc) {
          dataAfter.filter(it => it.type === 0).length === 0 &&
            setShowDelete(false);
        }
        if (isShowItemImage) {
          dataAfter.filter(it => it.type === 2).length === 0 &&
            setShowDelete(false);
        } else {
          dataAfter.filter(it => it.type === 1).length === 0 &&
            setShowDelete(false);
        }
      ...
      await checkLinkInvoice(
        ...
        async result => {
          if (result.statusId === 200) {
            ...
            dataDetail[itemTab.ref_Name].billFile = await JSON.stringify(
              newBillFiles,
            );
            ...
            await MessageInfo(
              'Đường dẫn được gửi sai hoặc không hỗ trợ tải xuống. Bạn vui lòng tải về máy và gửi chứng từ bằng tệp tin. Xin cảm ơn!',
            );
          }
        },
      );
  ...
  const RenderItemDocument = ({ item, index }) => {
    return (
      <TouchableOpacity
        ...
        onLongPress={() =>
          itemBussiness.isCanEdit == 1 ? handleLongPress(item) : null
        }
          ...
          <View style={{ paddingLeft: 8, flex: 1, justifyContent: 'center' }}>
            ...
            >
              {(item.size >= 1000000
                ...
                : (item.size / 1000).toFixed(2) + 'KB, ') + 'PDF tài liệu'}
            </Text>
          </View>
        </View>
      </TouchableOpacity>
  ...
  const RenderItemImageByBill = ({ item, index }) => {
    ...
    const urlImage =
      uri.indexOf('file://') > -1 || uri.indexOf('https://') > -1
        ? uri
        : URLDEFAULT + uri;
    return (
      <TouchableOpacity
        key={'ItemPhoto_' + index}
        style={{
          width: deviceWidth / 5,
  ...
  const handleViewImage = item => {
    ...
    const urlItem =
      uriItem.indexOf('file://') > -1 || uriItem.indexOf('https://') > -1
        ? uriItem
        : URLDEFAULT + uriItem;
    billFiles.map(it => {
  ...
  const sendInvoiceByType = () => {
    ...
    if (dataDetail[itemTab.ref_Name]?.isRequestNote == 1) {
      if (
        dataDetail[itemTab.ref_Name]?.note?.length == 0 ||
        dataDetail[itemTab.ref_Name]?.note == undefined ||
        dataDetail[itemTab.ref_Name]?.note == null
      ) {
    ...
    MessageAction('Bạn chắc chắn gửi thay đổi?', async () => {
      actionSend(sendBillFile);
    });
  };
  const actionSend = async dataBillFile => {
    const result = await SendInvoice(
      JSON.stringify({
        ...dataDetail[itemTab.ref_Name],
        billFile: JSON.stringify(dataBillFile),
        sendType: 'save',
      }),
    );
    if (result.statusId === 200) {
  ...
  const sendNextPeriodByTab = async () => {
    ...
    if (
      itemNextCosts.note.length == 0 ||
      itemNextCosts.note == undefined ||
      itemNextCosts.note == null
    ) {
    ...
    MessageAction(
      `Bạn chắc chắn muốn chuyển ${itemTab.nameVN} sang kỳ sau?`,
      async () => {
        const result = await SaveNextBill(JSON.stringify(dataUpload));
        if (result.statusId === 200) {
          if (result.data[0].status === 200) {
  ...
  const isShowCurrentAction =
    dataDetail[itemTab.ref_Name]?.statusId !== 7 &&
    ((itemBussiness.isCanEdit == 1 &&
      itemBussiness.statusId !== 3 &&
      itemBussiness.statusId !== 4) ||
      (itemBussiness.isCanEdit == 1 &&
        (itemBussiness.statusId == 3 || itemBussiness.statusId == 4) &&
        (dataDetail[itemTab.ref_Name]?.statusId == 3 ||
          dataDetail[itemTab.ref_Name]?.statusId == 4)));
  const currentActionPaddingBottom = 16;
  const renderCurrentAction = () => (
    <View
      key={'ButtonCurrent_' + indexTab}
      style={{
        paddingHorizontal: 5,
        paddingTop: 6,
        paddingBottom: 8,
    ...
    >
      <View>
        <View style={{ flexDirection: 'row' }}>
          ...
          <View
            style={{
              width: itemBussiness.isCanNext == 1 ? '50%' : '100%',
              padding: 5,
            }}
          >
            <TouchableOpacity
              onPress={() => sendInvoiceByType()}
              style={{
                minHeight: 44,
                padding: 8,
                borderRadius: 10,
    ...
    >
      {isShowNextCost ? (
        <View style={{ flex: 1, paddingBottom: 72 }}>
          <ViewNextCost
            itemNextCosts={itemNextCosts}
            key={'ViewNextCost_' + indexTab}
            itemTab={itemTab}
            indexTab={indexTab}
            itemBussiness={itemBussiness}
            dataDetail={dataDetail}
        ...
        <View
          key={'keyTab_' + indexTab}
          style={{ flex: 1, flexDirection: 'column' }}
        >
          <ScrollView
            key={'scrollTab_' + indexTab}
            style={{ flex: 1 }}
            scrollEventThrottle={16}
            nestedScrollEnabled
            onScroll={() => (isShowType ? showTypeUpdate() : null)}
            contentContainerStyle={{
              paddingBottom: currentActionPaddingBottom,
            }}
          >
              ...
              <View style={{ padding: 10, flexDirection: 'row' }}>
                ...
                <Text
                  style={{
                    ...
                    fontWeight: '500',
                    color:
                      dataDetail[itemTab.ref_Name]?.colorHightlight ||
                      appcolor.dark,
                  }}
                >
                  {' '}
                  {dataDetail[itemTab.ref_Name]?.statusName || ''}
                </Text>
              </View>
            )}
            <View
            ...
            >
              <View
                style={{ flexDirection: 'row', paddingLeft: 10, paddingTop: 8 }}
              >
                <TouchableOpacity
                  key={'Doccumnt_' + indexTab}
                  style={{
                    paddingHorizontal: 8,
                    height: 30,
                    borderRadius: 10,
                    backgroundColor: isShowItemDoc
                      ? appcolor.primary
                      : appcolor.surface,
                    justifyContent: 'center',
                    alignItems: 'center',
                ...
                >
                  <SpiralIcon
                    name="attach-file"
                    size={15}
                    color={isShowItemDoc ? appcolor.white : appcolor.dark}
                  />
                  <Text
                    style={{
                      fontWeight: '300',
            ...
            {isShowItemDoc && billFiles.length > 0 && (
              ...
              >
                <ScrollView
                  key={'listScrollDoc_' + indexTab}
                  ref={ref_ScrollDoc}
                  horizontal={true}
                  showsHorizontalScrollIndicator={false}
                >
                  {billFiles.map((it, idx) => {
                    return it.type !== 1 && it.type !== 2 ? (
                      <RenderItemDocument
                        key={indexTab + 'File_' + idx}
                        item={it}
                        index={idx}
                      />
                    ) : null;
            ...
            {isShowItemImage && billFiles.length > 0 && (
              <View key={'listByImage_' + indexTab} style={{}}>
                ...
                >
                  {billFiles.map((it, idx) => {
                    return it.type == 2 ? (
                      <RenderItemImageByBill
                        key={indexTab + 'image_' + idx}
                        item={it}
                        index={idx}
                      />
                    ) : null;
            ...
            <View
              key={'CostsTab_' + indexTab}
              style={{ minHeight: 10, minWidth: 10 }}
            >
              <NumericFormat
                key={`n${indexTab}`}
                value={
                  dataDetail[itemTab.ref_Name]?.costs
                    ? dataDetail[itemTab.ref_Name]?.costs
                    : ''
                }
                displayType={'text'}
                ...
                renderText={value => (
                  <FormGroup
                    ...
                    inputStyle={{ textAlign: 'left' }}
                    keyboardType="numeric"
                    handleChangeForm={onNumberChanged}
                    returnKeyLabel={Platform.OS === 'ios' ? 'tiếp' : 'tiếp'}
              ...
              {itemBussiness.isUseVAT == 1 && (
                <View style={{ minHeight: 10, minWidth: 10 }}>
                  <NumericFormat
                    key={`nVAT${indexTab}`}
                    value={
                      dataDetail[itemTab.ref_Name]?.costVAT
                        ? dataDetail[itemTab.ref_Name]?.costVAT
                        : ''
                    }
                    displayType={'text'}
                    ...
                    renderText={value => (
                      <FormGroup
                        ...
                        inputStyle={{ textAlign: 'left' }}
                        keyboardType="numeric"
                        handleChangeForm={onNumberVATChanged}
                        returnKeyLabel={Platform.OS === 'ios' ? 'tiếp' : 'tiếp'}
            ...
            />
            {dataDetail[itemTab.ref_Name]?.confirmNote !== '' &&
              dataDetail[itemTab.ref_Name]?.confirmNote?.length > 0 && (
                <FormGroup
                  containerStyle={{
                    padding: 3,
                    marginTop: 5,
              ...
              <View key={'ViewConfirm_' + indexTab} style={{}}>
                ...
                >
                  {confirmFile.map((it, idx) => {
                    return (
                      <RenderItemPhoto
                        key={'ImageConfirm_' + indexTab + '_' + idx}
                        item={it}
                        index={idx}
                        listPhotoItem={confirmFile}
...
const RenderItemPhoto = ({ item, index, listPhotoItem, handleSelectImage }) => {
  ...
  const [urlImage, setUrlImage] = useState(
    item.link.indexOf('file://') > -1 || item.link.indexOf('https://') > -1
      ? item.link
      : URLDEFAULT + item.link,
  );
