...
export const InvoiceResultDefault = ({
  styles,
  dataBill,
  filterInvoice,
  handleReloadInvoice,
  ...
  useEffect(() => {
    if (isLoadDone) {
      refInvoice.current !== null
        ? setTimeout(
          () =>
            refInvoice.current?.setIndex(
              currentItem?.currentIndexHome || 0,
            ) || null,
          300,
        )
        : null;
      ...
      {dataStatus?.length > 0 && dataBill?.dataDetail?.length > 0 && (
          ...
          renderTabBar={props => (
            <MaterialTabBar
              ...
              tabStyle={{
                ...
                minWidth: minWidthTab(dataStatus),
                height: 35,
                marginEnd: 8,
              }}
              scrollEnabled={true}
        ...
        >
          {dataStatus.map((item, index) => {
            const dataInvoice = _.filter(dataBill.dataDetail, {
              statusId: item.Id,
            });
            return dataInvoice.length > 0 ? (
              ...
              >
                <View style={{ flex: 1, paddingTop: 45, width: deviceWidth }}>
                  <ScrollView
                    onScroll={() =>
                      filterInvoice.loadYearMonth ? handleOnScroll() : null
                    }
...
const ViewInvoiceDefault = ({ dataBill, styles }) => {
  ...
  const confirmCosts = `Chi phí được quyết toán kỳ này ${dataBill?.dataDefault?.confirmCosts > 0
      ? formatNumber(dataBill?.dataDefault?.confirmCosts, ',')
      : 0
    } VNĐ`;
  const prevCosts = `Chi phí đã chuyển từ kỳ trước sang ${dataBill?.dataDefault?.prevCosts > 0
      ? formatNumber(dataBill?.dataDefault?.prevCosts, ',')
      : 0
    } VNĐ`;
  const waitCosts = `Chi phí chờ duyệt ${dataBill?.dataDefault?.waitCosts > 0
      ? formatNumber(dataBill?.dataDefault?.waitCosts, ',')
      : 0
    } VNĐ`;
  const nextCosts = `Chi phí chuyển kỳ sau ${dataBill?.dataDefault?.nextCosts > 0
      ? formatNumber(dataBill?.dataDefault?.nextCosts, ',')
      : 0
    } VNĐ`;
  const cancelCosts = `Chi phí bỏ quyết toán ${dataBill?.dataDefault?.cancelCosts > 0
      ? formatNumber(dataBill?.dataDefault?.cancelCosts, ',')
      : 0
    } VNĐ`;
  ...
  return (
    ...
    >
      <View style={{ width: '100%' }}>
        ...
        >
          {userinfo.employeeCode}-{userinfo.employeeName}
        </Text>
        {/* <ItemView styleView={styles.contentTopView} value={buhCosts} iconName='road' /> */}
        <ItemViewDefault
          styleView={styles.contentTopView}
          value={confirmCosts}
          iconName="hotel"
        />
        {showItem && (
