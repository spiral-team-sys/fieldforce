...
export const TripWithPointLG = ({ itemTrips, ItemInput, quotaData, dateFilter, config, dataProvince }) => {
                    ...
                    <Text style={{ fontWeight: '600', fontSize: 12, color: appcolor.dark, padding: 4, width: '80%' }}>Thông tin chặng đường</Text>
                    {
                        dataShow.map((it, idx) => {
                            ...
                            if (!it.shopVisit && !it.provinceTo && !it.workDate) {
                                return (
                                    <View key={`ViewPoint_${it.id}_${it.workDate}_${it.provinceTo}_${it.shopVisit}`}
                                        style={{ borderRadius: 8, backgroundColor: appcolor.surface, padding: 4, marginBottom: 4 }}>
                                        <Text style={{ width: '100%', fontWeight: '600', fontSize: 12, color: appcolor.tomato, paddingHorizontal: 4 }}>Thông tin Điểm đi</Text>
                                    </View>
                                )
                            }
                            return (
                                <View key={`ViewPoint_${it.id}_${it.workDate}_${it.provinceTo}_${it.shopVisit}`}
                                    ...
                                    <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between', alignItems: "center" }}>
                                        {it.workDate && <Text style={{ width: '100%', fontWeight: '600', fontSize: 12, color: appcolor.tomato, paddingHorizontal: 4 }}>Ngày đi : {it.workDate}</Text>}
                                    </View>
                                    <View style={{ width: '100%', borderRadius: 4 }}>
                                        {
                                            (it.shopVisit !== null) &&
                                            <View style={{ width: '100%', flexDirection: "row", paddingHorizontal: 4 }}>
                                                <Text style={{ fontWeight: '600', fontSize: 12, color: appcolor.tomato }}>Địa chỉ : {<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>{it.shopVisit}</Text>}</Text>
                                            </View>
                                        }
                                        {
                                            (it.provinceTo !== null) &&
                                            <View style={{ width: '100%', flexDirection: "row", paddingHorizontal: 4 }}>
                                                <Text style={{ fontWeight: '600', fontSize: 12, color: appcolor.dark }}>Tỉnh đến : {<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>{it.provinceTo}</Text>}</Text>
                                            </View>
                                        }
                                        {
                                            ((it.note !== undefined && it.note !== null && it.note !== '' && it.note.length > 0) ||
                                                (it.remark !== undefined && it.remark !== null && it.remark !== '' && it.remark.length > 0)
                                            ) &&
                                            <View style={{ width: '100%', flexDirection: "row", paddingHorizontal: 4, }}>
                                                <Text style={{ fontWeight: '600', fontSize: 12, color: appcolor.dark }}>Ghi chú : {<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>{it.note || it.remark}</Text>}</Text>
                                            </View>
                                        }
                                        {
                                            ((it.amountHotel != null && it.amountHotel >= 0) ||
                                                (it.amountAllowance != null && it.amountAllowance >= 0) ||
                                                (it.amountTransport != null && it.amountTransport >= 0)) &&
                                            <View style={{ width: '100%', flexDirection: "row", paddingHorizontal: 4, paddingBottom: 2 }}>
                                                <Text style={{ fontWeight: '600', fontSize: 12, color: appcolor.dark }}>Chi phí : {<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>Khách sạn : {formatNumber(it.amountHotel, ',') || 0} | </Text>}{<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>Phụ cấp : {it.amountAllowance || 0} | </Text>}{<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>Di chuyển : {it.amountTransport || 0}</Text>}</Text>
                                            </View>
                                        }
                                    </View>
                                </View>
