...
export const AquaCreateTripsSR = ({
  onCloseCreate,
  onNextCreate,
  dateFilter,
  limitCost,
      ...
      if (isSuccess) {
        ...
        await dispatch(
          SetBusinesTrips({
            ...
            typeAdvance: itemTrips.typeAdvance,
            typeVehicle: itemTrips.typeVehicle,
          }),
        );
  ...
  const checkInputData = actionDone => {
    ...
    if (isValid(itemTrips.provinceList) && itemTrips.provinceList?.length > 0) {
      ...
      if (strValid !== '') strValid += 'của Thông tin chặng đường, ';
    } else {
      strValid += 'Chưa nhập số điểm đến của chuyến, ';
    }

    const betweenDate = moment(itemTrips.toDate, 'YYYYMMDD').diff(
      moment(itemTrips.fromDate, 'YYYYMMDD'),
      'days',
    );
    const day = betweenDate + 1;
    const night =
      itemTrips.isDayNightEqual == 1 || tripConfig.isDayNightEqual == 1
        ? betweenDate + 1
        : betweenDate;
  ...
  const handlerWorkingDate = async (fromValue, toValue) => {
    ...
    const day = betweenDate + 1;
    const night =
      tripConfig.isDayNightEqual == 1 || itemTrips.isDayNightEqual == 1
        ? betweenDate + 1
        : betweenDate;
  ...
  const getStoreByDate = async (fromDate, toDate) => {
    await WorkingPlanAPI.getStoreByDate(
