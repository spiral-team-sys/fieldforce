...
import { TripWithStages } from "../TripWithStages"
...
export const AquaCreateTripsSR = ({ onCloseCreate, onNextCreate, dateFilter, limitCost, quotaData, tripConfig }) => {
    ...
    const LoadData = async () => {
        ...
        if (quotaData.isUsePlanSystem == 1 && dateFilter.mode == MODE.UPDATE) {
            const fromDate = tripResult.fromDate || dateFilter.itemDate.fromDate
            const toDate = tripResult.toDate || dateFilter.itemDate.toDate
            await getStoreByDate(fromDate, toDate)
        }
    ...
    const actionNext = async () => {
        ...
        await checkInputData(async (isSuccess, message) => {
            if (isSuccess) {
                ...
                //     itemTrips: { ...tripResultItem, provinceList: itemTrips.provinceList, movingSteps: itemTrips.movingSteps, typeAdvance: itemTrips.typeAdvance, typeVehicle: itemTrips.typeVehicle }
                // })
                await dispatch(SetBusinesTrips({ ...tripResultItem, provinceList: itemTrips.provinceList, movingSteps: itemTrips.movingSteps, typeAdvance: itemTrips.typeAdvance, typeVehicle: itemTrips.typeVehicle }))
    ...
    const checkInputData = (actionDone) => {
        ...
        const betweenDate = moment(itemTrips.toDate, 'YYYYMMDD').diff(moment(itemTrips.fromDate, 'YYYYMMDD'), 'days')
        const day = betweenDate + 1
        const night = ((itemTrips.isDayNightEqual == 1 || tripConfig.isDayNightEqual == 1) ? betweenDate + 1 : betweenDate)
        if (day < itemTrips.dayValue) {
            strValid += `Tổng ngày ăn lớn hơn số ngày đi công tác, `
        }
        if (night < itemTrips.nightValue) {
    ...
    const getStoreByDate = async (fromDate, toDate) => {
        await WorkingPlanAPI.getStoreByDate({ "fromDate": fromDate, "toDate": toDate }, async (mData) => {
            dateFilter.dataPlan = mData
        })
    }
            ...
            <ScrollView
                ...
                <KeyboardAwareScrollView
                    ...
                    <ItemInput
                        ...
                        key={`${TYPE.TYPE_LUNCH}`}
                        titleName='Ngày ăn'
                        placeholder='Số ngày bạn ăn uống trong chuyến đi công tác'
                        iconName='comment'
