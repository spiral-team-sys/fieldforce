...
export const GSVCreateTripsSR = ({
  ...
  const actionNext = async () => {
    ...
    // tripConfig : {"isDuplicateProvince": 1, "maxDay": 7, "minDay": 1,"isUseReplaceTrip":1}
    // ${itemTrips.dateFilterFrom} - ${itemTrips.dateFilterTo}

    // "Chi phí di chuyển (Tàu xe, máy bay,..)"
  ...
  const checkInputData = actionDone => {
    ...
    if (dateFilter.mode == MODE.CREATE || dateFilter.mode == MODE.UPDATE) {
      ...
      if (
        itemTrips.provinceCodeFrom !== null &&
        itemTrips.provinceCodeTo !== null &&
        itemTrips.provinceCodeFrom == itemTrips.provinceCodeTo &&
        tripConfig.isDuplicateProvince == 1
      ) {
