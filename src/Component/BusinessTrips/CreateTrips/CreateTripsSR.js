...
import { DataAddressByGeo, GetAddressByGeo, GetProvinceByShop } from "../../../Controller/BussinessTripController"
import { ACTION } from "../../../Core/ReduxController"
import { alertError, alertWarning, deviceHeight, isValid, } from "../../../Core/Utility"
import { HeaderBusiness } from "../HeaderBusiness"
import { MODE, TYPE } from "../UtilityBusiness"
import _ from "lodash"
import { KeyboardAwareScrollView } from "react-native-keyboard-aware-scroll-view"
import { AutoCompleteAddress } from "../AutoCompleteAddress"
import { ViewExistsTrip } from "../ReplaceExistingTrip/ViewExistsTrip"
import { ScrollView } from "react-native-actions-sheet"
import dvhc2025 from '../../../Themes/filedata/dvhc2025.json'
import { SetBusinesTrips } from "../../../Redux/action";

export const CreateTripsSR = ({ onCloseCreate, onNextCreate, dateFilter, tripConfig, dataTrips, onReSelectCopyDate }) => {
    ...
    const isCopyEnabled = ((tripConfig?.isCopy ?? reportConfig?.isCopy ?? 0) == 1)
    const hasCopyDateRanges = Array.isArray(tripResult.tripDateRanges) && tripResult.tripDateRanges.length > 0

    //
    const LoadData = async () => {
        await setLoading(true)
        if (tripConfig.isUseDataLocal == 1) {
            await setDataProvince(dvhc2025.data)
            await setDataProvinceMain(dvhc2025.data)
        } else {
    ...
    //
    const settingAddressFrom = async (mData) => {
        ...
        //     const _dataProvince = await mData.filter(i => i.provinceName.toLowerCase().match(province.toLowerCase()))[0] || {}
        //     if (Object.keys(_dataProvince).length > 0) {
        //         await setDataProvince(mData.sort((a, b) => { return b.provinceName == province }))
        //     }
        //     if (location !== null)
        //         await setItemTrips({
        //             ...itemTrips,
    ...
    const checkInputData = (actionDone) => {
        ...
        if (!isValid(itemTrips.provinceTo))
            strValid += 'Tỉnh đến, '
        if (tripConfig.isSelectDistrict == 1) {
            if (!isValid(itemTrips.districtCodeFrom))
                strValid += 'Phường đi, '
            if (!isValid(itemTrips.districtCodeTo))
                strValid += 'Phường đến, '
        }
    ...
    const onMultipleChoose = (item, typeItem) => {
        if (typeItem == TYPE.PROVINCE_FROM)
            setItemTrips({ ...itemTrips, provinceFrom: (item.itemName || item.name), provinceCodeFrom: (item.provinceCode || item.level1_id), districtCodeFrom: null, districtFrom: null })
        if (typeItem == TYPE.DISTRICT_FROM)
            setItemTrips({ ...itemTrips, districtFrom: (item.itemName || item.name), districtCodeFrom: (item.districtCode || item.level2_id) })
        if (typeItem == TYPE.PROVINCE_TO)
            setItemTrips({ ...itemTrips, provinceTo: (item.itemName || item.name), provinceCodeTo: (item.provinceCode || item.level1_id), districtCodeTo: null, districtTo: null })
        if (typeItem == TYPE.DISTRICT_TO)
            setItemTrips({ ...itemTrips, districtTo: (item.itemName || item.name), districtCodeTo: (item.districtCode || item.level2_id) })
    ...
    const styles = StyleSheet.create({
        ...
        copyDateWrapper: { width: '100%', backgroundColor: appcolor.surface, padding: 10, borderRadius: 10, marginTop: 8 },
        copyDateHeader: { width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 },
        copyDateTitle: { fontSize: 13, fontWeight: '700', color: appcolor.dark, flexShrink: 1, paddingEnd: 8 },
        copyDateItem: { fontSize: 12, color: appcolor.dark },
        copyDateDesc: { fontSize: 11, color: appcolor.greylight, marginTop: 6, textAlign: 'center' },
        copyDateActionButton: { alignSelf: 'flex-end' }
    })
    return (
            ...
            <ScrollView
                ...
                <KeyboardAwareScrollView
                    ...
                    <View key={'dateBusiness'} style={styles.viewDate}>
                        {(isCopyEnabled || onReSelectCopyDate !== undefined) && hasCopyDateRanges ? (
                            <View style={styles.copyDateWrapper}>
                                <View style={styles.copyDateHeader}>
                                    <Text style={styles.copyDateTitle}>{`Bạn đang tạo ${tripResult.tripDateRanges.length} lịch công tác`}</Text>
                                    {onReSelectCopyDate !== undefined &&
                                        <TouchableOpacity style={styles.copyDateActionButton} onPress={onReSelectCopyDate}>
                                            <Icon reverse name='calendar-alt' type='font-awesome-5' size={14} color={appcolor.info} />
                                        </TouchableOpacity>
                                    }
                                </View>
                                {tripResult.tripDateRanges.map((r, idx) => (
                                    <Text key={`tr_${idx}`} style={styles.copyDateItem}>{`${idx + 1}) ${r.dateFilterFrom} → ${r.dateFilterTo} ${r.dayAndNight}`}</Text>
                                ))}
                                <Text style={styles.copyDateDesc}>Khung ngày đã được chọn ở bước Copy. Các thông tin công tác bên dưới áp dụng chung cho tất cả các lịch.</Text>
                            </View>
                        ) : (
                            <>
                                <FormGroup
                    ...
                    <MutipleItemSelected
                        ...
                        defaultValueDistrict={itemTrips.districtFrom}
                        dataItems={dataProvince}
                        onItemChoose={onMultipleChoose}
                        isSelectDistrict={tripConfig.isSelectDistrict || 0}
                        provinceCode={itemTrips.provinceCodeFrom}
                    />

                    {/* <ItemInput
                        key={`${TYPE.PROVINCE_FROM}_Input`}
                        isRequire
                    ...
                    <MutipleItemSelected
                        ...
                        defaultValueDistrict={itemTrips.districtTo}
                        dataItems={dataProvince}
                        onItemChoose={onMultipleChoose}
                        isSelectDistrict={tripConfig.isSelectDistrict || 0}
                        provinceCode={itemTrips.provinceCodeTo}
                    />
                    {/* <ItemInput
                        key={`${TYPE.PROVINCE_TO}_Input`}
                        isRequire
                    ...
                    />
                    {
                        (tripConfig.isUseReplaceTrip == 1 && (dataTrips[0]?.idTrip || 0) > 0) &&
                        // itemTrips.tripReplace  Object.keys(itemReplace)?.length > 0 &&
                        <ViewExistsTrip
                            itemTrips={itemTrips}
                            dataTrips={dataTrips}
                            itemReplace={(dataTrips?.filter(it => it.idTrip == tripResult.tripReplace)?.length > 0 ?
                                dataTrips?.filter(it => it.idTrip == tripResult.tripReplace)[0] : {}) || {}}
                            onTripSelect={handlerTripSelect}
                        />
                    }
                    <ItemInput
                        isRequire
