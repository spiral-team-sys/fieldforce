...
export const HafeleCreateTrips = ({ onCloseCreate, onNextCreate, dateFilter, tripConfig, dataTrips, quotaData }) => {
    ...
    const checkInputData = (actionDone) => {
        ...
        if (dateFilter.mode == MODE.CREATE || dateFilter.mode == MODE.UPDATE) {
            ...
            if (tripConfig?.maxNumberNight !== null && tripConfig?.maxNumberNight !== undefined && tripConfig?.maxNumberNight > 0 && countNightDay > tripConfig?.maxNumberNight) {
                alertWarning(`Số đêm ở lại không được lớn hơn số đêm giới hạn: ${countNightDay} / ${tripConfig?.maxNumberNight}`)
                return
            }
        }
