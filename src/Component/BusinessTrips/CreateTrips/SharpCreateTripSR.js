...
export const SharpCreateTripsSR = ({
  onCloseCreate,
  onNextCreate,
  dateFilter,
  limitCost,
  ...
  const checkInputData = actionDone => {
    ...
    //     const listStage = itemTrips.provinceList
    //     let countNightDay = 0
    //     let countEatDay = 0
    //     for (let index = 0; index < listStage.length; index++) {
    //         const element = listStage[index];
    //         if (element.startPosition == null || element.startPosition == '') {
    //             strValid += `Chưa nhập điểm bắt đầu chặng ${element.id + 1}, `
    //         }
    //         if (element.endPosition == null || element.endPosition == '') {
    //             strValid += `Chưa nhập điểm kết thúc chặng ${element.id + 1}, `
    //         }
    //         if ((element.numberDay == "" || element.numberDay == null) && element.numberDay !== 0) {
    //             strValid += `Chưa nhập lưu trú chặng ${element.id + 1}, `
    //         }
    //         if ((element.eatDay == "" || element.eatDay == null) && element.eatDay !== 0) {
    //             strValid += `Chưa nhập ngày ăn chặng ${element.id + 1}, `
    //         }
    ...
    if (isValid(itemTrips.provinceList) && itemTrips.provinceList?.length > 0) {
      ...
      if (itemTrips.typeVehicle == 'DRIVING') {
      ...
      if (
        itemTrips.typeAdvance instanceof Object == false ||
        (itemTrips.typeAdvance instanceof Object == true &&
          Object.keys(itemTrips.typeAdvance).length == 0)
      ) {
        strValid += `Chưa chọn loại tạm ứng, `;
      }
      ...
      >
        ...
        >
          ...
          {/* <MutipleItemSelected
                        ...
                        titleName='Tỉnh đến'
                        placeholder='Tỉnh check in xa nhất của chuyế\n công tác'
