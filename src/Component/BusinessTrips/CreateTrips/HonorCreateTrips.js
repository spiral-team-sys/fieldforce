...
export const HonorCreateTrips = ({
  ...
  const checkInputData = actionDone => {
    ...
    if (
      ((dateFilter.isEvidentDateChange &&
        (itemTrips.dayValue > itemTrips.daysOld ||
          itemTrips.nightValue > itemTrips.nightsOld)) ||
        itemTrips.isHaveExceed == 1) &&
      itemTrips.isActual
    ) {
      const evidenceParse = JSON.parse(itemTrips.evidence || '[]');
      if (evidenceParse.length == 0) {
        strValid +=
          'Vui lòng thêm hình ảnh làm bằng chứng cho việc thay đổi ngày đi công tác, ';
      }
    }

    if (!isValid(itemTrips.locationStart)) strValid += 'Vị trí điểm đi, ';
    if (!isValid(itemTrips.locationEnd)) strValid += 'Vị trí điểm đến, ';
    ...
    if (isValid(itemTrips.provinceList) && itemTrips.provinceList?.length > 0) {
      ...
      const listTypeCost = _.filter(
        itemTrips.listTypeCost || [],
        it => it.isChoose == 1,
      );
      let countNightDay = 0;
      let countEatDay = 0;
      for (let index = 0; index < listPoints.length; index++) {
        ...
        if (
          (element.numberDay == '' || element.numberDay == null) &&
          element.numberDay !== 0
        ) {
          strValid += `Chưa nhập số đêm lưu trú điểm ${element.id + 1}, `;
        }
        if (
          element.arrivedDay == null ||
          element.arrivedDay == '' ||
          element.arrivedDay == undefined
        ) {
          strValid += `Chưa nhập ngày đến điểm thứ ${element.id + 1}, `;
        }
        if (
          (element.eatDay == '' || element.eatDay == null) &&
          element.eatDay !== 0
        ) {
        ...
        if (
          element.numberDay !== '' &&
          element.numberDay !== null &&
          listTypeCost.length == 0
        ) {
          if (
            (element.nightRestValue == 0 ||
              element.nightRestValue == null ||
              element.nightRestValue == undefined) &&
            element.numberDay > 0
          ) {
      ...
      if (countNightDay > itemTrips.nightValue) {
        ...
        (listTypeCost?.length || 0) == 0 &&
        countNightDay !== itemTrips.nightValue
      ) {
      ...
      if (countEatDay > itemTrips.dayValue) {
        ...
        (listTypeCost?.length || 0) == 0 &&
        countEatDay != itemTrips.dayValue
      ) {
      ...
      if (itemTrips.typeVehicle == 'DRIVING' || quotaData.isUseGoogleKm == 1) {
        if (quotaData.isFirstPointLastPoint == 1) {
          const itemLast =
            listPoints?.length > 0 ? listPoints[listPoints?.length - 1] : null;
          if (itemLast.distance == null || itemLast.distance == 0) {
            strValid += `Chưa bấm tính quảng đường di chuyển, `;
          }
        } else {
          const checkDistance = listPoints.filter(
            it => it.distance == null || it.distance == 0,
          );
      ...
      if (strValid !== '') strValid += 'trong mục Thông tin chặng đường, ';
    } else {
    ...
    const listTypeCost = _.filter(
      itemTrips.listTypeCost || [],
      it => it.isChoose == 1 && it.isNeedEvident == 1,
    );
    if (listTypeCost.length > 0) {
      ...
      for (let index = 0; index < listTypeCost.length; index++) {
        ...
        const checkPhoto = photoByType.filter(
          i => i.photoType == 'Workingschedule_' + element.fieldCost,
        );
  ...
  const handlerWorkingDate = async (fromValue, toValue) => {
    ...
    if (
      ...
      alertWarning(
        `Ngày được chọn phải nằm trong tháng ${moment(
          itemTrips.fromDate?.toString(),
          'YYYYMMDD',
        ).format('MM/YYYY')}`,
      );
  ...
  const handlerAddImage = (photo, type) => {
    ...
    if (type === 'add') {
      ...
      itemTrips.evidence = JSON.stringify(
        dataPhotoByType.filter(i => i.photoPath !== photo.photoPath),
      );
    } else if (type === 'new') {
        ...
        >
          <View key={'dateBusiness'} style={styles.viewDate}>
            ...
            {itemTrips.loadCalendar && (
              <CalendarSelected
                ...
                lockOutMonth={true}
                fromDate={moment(itemTrips.dateFilterFrom, 'DD/MM/YYYY').format(
                  'YYYY-MM-DD',
                )}
          ...
          </View>
          {
            ...
            dateFilter.isEvidentDateChange && dateFilter.isUpdate && (
              <PhotoEvidentTrip
                _guid={dateFilter.guid}
                itemInput={itemTrips}
                listPhoto={JSON.parse(itemTrips.evidence || '[]')}
                reload={reload}
                photoType={'Workingschedule_Exceed'}
          ...
          <ItemInput
            ...
            iconName="road"
            itemValue={
              itemTrips.kmValueText || (itemTrips.kmValue / 1000).toFixed(2)
            }
          />
          <TripWithPoint
            config={config}
