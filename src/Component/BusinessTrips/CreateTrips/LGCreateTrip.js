...
import { GetBusinesTrips, GetProvinceByShop } from "../../../Controller/BussinessTripController"
...
import { MODE, TYPE, checkDateExists, provinceByAddress } from "../UtilityBusiness"
...
export const LGCreateTrips = ({ onCloseCreate, onNextCreate, dateFilter, quotaData }) => {
    ...
    const checkCondition = async () => {
        if (dateFilter.mode == MODE.CREATE && !dateFilter.isUpdate) {
            ...
            if (moment(itemTrips.fromDate, 'YYYYMMDD').format('YYYYMM') < moment().format('YYYYMM')) {
        ...
        await checkInputData(async (isSuccess, message) => {
            if (isSuccess) {
                ...
                await dispatch(SetBusinesTrips({ ...tripResultItem, provinceList: itemTrips.provinceList, movingSteps: itemTrips.movingSteps }))
    ...
    const checkDate = () => {
        const result = checkDateExists(itemTrips.fromDate, itemTrips.toDate, dataByMonth)
        return result
    }
        ...
        if (itemTrips.typeKM != 2) {
            if (isValid(itemTrips.provinceList) && itemTrips.provinceList?.length > 0) {
                ...
                for (let index = 0; index < listPoints.length; index++) {
                    ...
                    if (element.workDate == null || element.workDate == '' || element.workDate == undefined) {
                        strValid += `Chưa nhập ngày đến điểm thứ ${element.id + 1}, `
                    }
                    if (itemTrips.typeKM == 1 && (element.provinceTo == null || element.provinceTo == '' || element.provinceTo == undefined)) {
                        strValid += `Chưa nhập tỉnh đến điểm thứ ${element.id + 1}, `
                    }
                    if (itemTrips.typeKM == 3 && (element.shopVisit == null || element.shopVisit == '' || element.shopVisit == undefined)) {
    ...
    const handlerItemChangeText = async (text, typeItem) => {
        let valueInput = 0
        if (typeItem == TYPE.PROVINCE_FROM)
            await setItemTrips({ ...itemTrips, addressFrom: text, provinceFrom: text, locationStart: null })
        if (typeItem == TYPE.PROVINCE_TO)
            await setItemTrips({ ...itemTrips, addressTo: text, provinceTo: text, locationEnd: null })
    ...
    const styles = StyleSheet.create({
        ...
        viewDate: { width: '100%', borderRadius: 5, alignItems: 'center', padding: 4 }
    })
    return (
            ...
            <ScrollView
                ...
                <KeyboardAwareScrollView
                    ...
                    <View key={'dateBusiness'} style={styles.viewDate}>
                        ...
                        />
                        {itemTrips.loadCalendar && <CalendarSelected onChangeData={handlerWorkingDate} month={dateFilter.month} lockOutMonth={config?.isLockOutMonth == 0 ? false : true} />}
                    </View>
                    {
                        itemTrips.typeKM == 2 && <View key={'listDateBusiness'} style={styles.viewDate}>
                            <ListDateTrip itemTrips={itemTrips} itemValue={itemTrips.listDate} typeItem={'listDate'} reloadDate={reloadDate} />
                        </View>
                    }
                    ...
                    {
                        (itemTrips.isViewByList == 1 || itemTrips.provinceList.length > 0) &&
                        <TripWithPointLG
                            config={config}
                            quotaData={quotaData}
                            itemTrips={itemTrips}
...
const ItemInput = ({ titleName, iconName, isRequire, onActionRight, iconRightName, typeFilter, itemValue, placeholder, onChangeText, keyboardType = 'default', editable = true }) => {
