...
export const HSSCreateTripsSR = ({ onCloseCreate, onNextCreate, dateFilter, limitCost }) => {
    ...
    const checkInputData = (actionDone) => {
        ...
        if (!isValid(itemTrips.provinceList) || itemTrips.provinceList?.length == 0) {
            ...
            for (let index = 0; index < lstProvince.length; index++) {
                ...
                if (element.eatDay == "" && element.eatDay !== 0) {
                    strValid += `Chưa nhập ngày ăn ${element.itemName}, `
                }
