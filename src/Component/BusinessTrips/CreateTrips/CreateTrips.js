...
export const CreateTrips = ({ onCloseCreate, onNextCreate, dateFilter, tripConfig }) => {
    ...
    const onMultipleChoose = (item, typeItem) => {
        if (typeItem == TYPE.PROVINCE_FROM)
            setItemTrips({ ...itemTrips, provinceFrom: (item.itemName || item.name), provinceCodeFrom: (item.provinceCode || item.level1_id) })
        ...
        if (typeItem == TYPE.PROVINCE_TO)
            setItemTrips({ ...itemTrips, provinceTo: (item.itemName || item.name), provinceCodeTo: (item.provinceCode || item.level1_id) })
            ...
            <ScrollView
                ...
                <KeyboardAwareScrollView
                    ...
                    <View key={'dateBusiness'} style={styles.viewDate}>
                        ...
                        />
                        {/* {itemTrips.loadCalendar && <CalendarSelected onChangeData={handlerWorkingDate} lockOutMonth={dateFilter.isLockOutMonth} />} */}
                    </View>
                    <MutipleItemSelected
