...
export const TripResult = ({
  ...
  const handlerSave = async () => {
    ...
    alertConfirm(
      'Xác nhận',
      'Bạn có đồng ý với chi phí công tác và thời gian của chuyến đi như trên, nếu có vui lòng xác nhận để gửi yêu cầu',
      async () => {
        setLoading(true);
        ...
        try {
          ...
          const baseUpload = {
            ...
            typeVehicle:
              typeof tripResult.typeVehicle == 'object'
                ? JSON.stringify(tripResult.typeVehicle) || null
                : tripResult.typeVehicle,
          ...
          if (isUpdate) {
            ...
            if (AppNameBuild == psvApp)
              if (
                tripResult.fromDate.toString().substring(0, 6) ==
          ...
          const ranges = Array.isArray(tripResult.tripDateRanges)
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      {!loading && (
        <ItemTripResults tripResult={tripResult} quotaData={quotaData} />
      )}
    </View>
  );
};
