...
export const ItemLocationDefault = ({
  dataProvince,
  itemTrips,
  ItemInput,
  quotaData,
    ...
    >
      ...
      >
        ...
        {itemTrips.locationStart && (
          ...
          >
            <Text
              style={{ fontWeight: 'bold', fontSize: 18, color: appcolor.blue }}
            >
              🚀 Điểm đi
            </Text>
            {itemTrips.addressFrom && (
              <Text style={{ fontSize: 14, color: '#555', marginTop: 4 }}>
                {itemTrips.addressFrom}
              </Text>
            )}
            <Text style={{ fontSize: 14, color: appcolor.dark, marginTop: 4 }}>
              {itemTrips.locationStart}
            </Text>
          </View>
        )}
        ...
        {itemTrips.kmValue && (
          ...
          >
            ...
            >
              Khoảng cách : {(itemTrips.kmValue / 1000).toFixed(0)}km
            </Text>
...
const ViewMapLocation = ({ closeModal, dataProvince, itemTrips }) => {
  ...
  const [modeOverlay, setModeOverlay] = useState({
    visible: false,
    contentOverlay: '',
  });
  const [modalConfig, setModalConfig] = useState({
  ...
  const loadData = () => {
    ...
    const indexStart = dataDistrict.findIndex(
      it =>
        (it.latitude == locationStart.latitude &&
          it.longitude == locationStart.longitude) ||
        it.address == itemTrips.addressFrom,
    );
    const indexEnd = dataDistrict.findIndex(
      it =>
        (it.latitude == locationEnd.latitude &&
          it.longitude == locationEnd.longitude) ||
        it.address == itemTrips.addressTo,
    );

    const provinceSelect = dataProvince.filter(
      it =>
        it.provinceCode ==
        dataDistrict[typePoint == 'START_POINT' ? indexStart : indexEnd]
          ?.provinceCode,
    );
    const dataDistrictByProvince = JSON.parse(
      provinceSelect[0]?.dataByProvince || '[]',
    );

    itemSelect.provinceSelect = provinceSelect[0] || {};
    itemSelect.districtSelect =
      dataDistrict[typePoint == 'START_POINT' ? indexStart : indexEnd] || {};

    locationData.itemStart =
      {
        ...dataDistrict[indexStart],
        latitude: dataDistrict[indexStart]?.latitude || locationStart.latitude,
        longitude:
          dataDistrict[indexStart]?.longitude || locationStart.longitude,
      } || {};
    locationData.itemEnd =
      {
        ...dataDistrict[indexEnd],
        latitude: dataDistrict[indexEnd]?.latitude || locationEnd.latitude,
        longitude: dataDistrict[indexEnd]?.longitude || locationEnd.longitude,
      } || {};
  ...
  const getMyLocation = async () => {
    await CheckLocation(() => {
      Geolocation.getCurrentPosition(position => {
        ...
        if (
          Object.keys(locationData.itemStart)?.length > 0 &&
          locationData.itemStart.latitude &&
          locationData.itemStart.longitude
        ) {
          moveLocation(locationData.itemStart, 'NEW');
        } else if (
          Object.keys(locationData.itemEnd).length > 0 &&
          locationData.itemEnd.latitude &&
          locationData.itemEnd.longitude
        ) {
  ...
  const handlerSelectTag = async (item, index, key) => {
    ...
    if (!item.latitude || !item.longitude) {
      isNullLocation = 1;
      await onGetAddress(
        itemProvince.address ||
        `${itemProvince.district},${itemProvince.province}`,
      );
    }
    ...
    if (dataByProvince.length == 1) {
      ...
      let indexDistrict = 0;
      if (item.latitude && item.longitude) {
        indexDistrictP = dataByProvince.findIndex(
          it => it.latitude == item.latitude && it.longitude == item.longitude,
        );
      } else {
        indexDistrictP = 0;
      }
      indexDistrict = dataDistrict.findIndex(
        it =>
          it.districtCode == dataByProvince[indexDistrictP]?.districtCode &&
          it.longitude == dataByProvince[indexDistrictP]?.longitude,
      );
    ...
    if (
      ...
      item.longitude &&
      isNullLocation == 1 &&
      Object.keys(dataItem.dataSelect)?.length > 0
    ) {
    ...
    if (
      ...
      (itemByIndex.latitude && itemByIndex.longitude)
    ) {
  ...
  const handlerSelectDistrict = (item, index, key) => {
    ...
    const dataByProvince = JSON.parse(
      itemSelect.provinceSelect?.dataByProvince || '[]',
    );
    const dataByDistrict = dataByProvince.filter(
      it => it.districtCode == item.districtCode,
    );
    if (dataByDistrict.length > 1) {
      ...
      if (itemSelect.districtSelect.address !== item.address) {
        ...
        if (typePoint == 'START_POINT') {
          if (
            locationData.itemStart.latitude !== item.latitude ||
            locationData.itemStart.longitude !== item.longitude
          ) {
          ...
          if (
            locationData.itemEnd.latitude !== item.latitude ||
            locationData.itemEnd.longitude !== item.longitude
          ) {
      ...
      <View key={`ma - ${index} `}>
        <TouchableOpacity
          ...
          style={{
            ...
            elevation: 3,
            backgroundColor:
              itemSelect.provinceSelect?.provinceCode == item.provinceCode
                ? appcolor.primary
                : appcolor.light,
          }}
        >
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          ...
          elevation: 3,
          backgroundColor:
            itemSelect.districtSelect?.districtCode == item.districtCode
              ? appcolor.primary
              : appcolor.light,
        }}
      >
  ...
  const handleSelectButton = type => {
    LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
    if (type == 'START_POINT') {
      const indexProvince = dataProvince.findIndex(
        it => it.provinceCode == locationData.itemStart?.provinceCode,
      );
      const dataByProvince = JSON.parse(
        dataProvince[indexProvince]?.dataByProvince || '[]',
      );
      const dataDistrict = _.unionBy(dataByProvince, 'districtCode');
      itemSelect.districtSelect =
        Object.keys(locationData.itemStart).length > 0
          ? locationData.itemStart
          : {};
      itemSelect.provinceSelect =
        Object.keys(locationData.itemStart).length > 0
          ? dataProvince[indexProvince]
          : {};
      Object.keys(locationData.itemStart).length > 0 &&
        locationData.itemStart.latitude &&
        locationData.itemStart.longitude
        ? moveLocation(locationData.itemStart)
        : getMyLocation();
      setListDistrict(
        Object.keys(locationData.itemStart).length > 0 &&
          locationData.itemStart.latitude &&
      ...
      const indexProvinceEnd = dataProvince.findIndex(
        it => it.provinceCode == locationData.itemEnd?.provinceCode,
      );
      const dataByProvince = JSON.parse(
        dataProvince[indexProvinceEnd]?.dataByProvince || '[]',
      );
      const dataDistrict = _.unionBy(dataByProvince, 'districtCode');
      itemSelect.districtSelect =
        Object.keys(locationData.itemEnd).length > 0
      ...
      itemSelect.provinceSelect =
        Object.keys(locationData.itemEnd).length > 0
          ? dataProvince[indexProvinceEnd]
          : {};
      Object.keys(locationData.itemEnd).length > 0 &&
        locationData.itemEnd.latitude &&
        locationData.itemEnd.longitude
        ? moveLocation(locationData.itemEnd)
        : getMyLocation();
      setListDistrict(
        Object.keys(locationData.itemEnd).length > 0 &&
          locationData.itemEnd.latitude &&
  ...
  return (
    <SafeAreaView style={{ flexGrow: 1, backgroundColor: appcolor.light }}>
      <View style={{ flex: 1 }}>
        <TouchableOpacity
          style={{ position: 'absolute', top: 8, start: 8, zIndex: 5 }}
          onPress={() => closeModal()}
        >
        ...
        >
          <View style={{}}>
            ...
            >
              <TouchableOpacity
                onPress={() => handleSelectButton('START_POINT')}
                style={{
                  width: '46%',
                  borderRadius: 8,
                  backgroundColor:
              ...
              <TouchableOpacity
                onPress={() => handleSelectButton('END_POINT')}
                style={{
                  width: '46%',
                  borderRadius: 8,
                  backgroundColor:
              ...
              >
                <Text
                  style={{
                    ...
                    fontWeight: '500',
                    color:
                      typePoint == 'END_POINT' ? appcolor.white : appcolor.dark,
                  }}
                >
                  {'Điểm đến'}
                </Text>
            ...
            {(typePoint == 'START_POINT' &&
              ...
              <View style={{ width: '100%', paddingLeft: 12 }}>
                ...
                >
                  Thông tin :{' '}
                  {typePoint == 'START_POINT'
                    ? locationData.itemStart?.shopName
                    : locationData.itemEnd?.shopName}
                </Text>
                <Text
                  style={{
                    fontWeight: '600',
                ...
                >
                  Địa chỉ :{' '}
                  {typePoint == 'START_POINT'
                    ? locationData.itemStart?.address
                    : locationData.itemEnd?.address}
                </Text>
                <Text
                  style={{
                    fontWeight: '600',
                ...
                >
                  Vị trí :{' '}
                  {typePoint == 'START_POINT'
                    ? `${locationData.itemStart?.latitude || 0},${locationData.itemStart?.longitude || 0
                    } `
                    : `${locationData.itemEnd?.latitude},${locationData.itemEnd?.longitude} `}
                </Text>
              </View>
            ) : (
              <View
          ...
          >
            <Text
              style={{ fontWeight: '800', fontSize: 13, color: appcolor.white }}
            >
...
const ListShop = ({
  ...
  const contains = (shop, query) => {
    ...
    let Saddress = removeVietnameseTones(
      address === null ? address : address.toLowerCase(),
    );
    let SshopName = removeVietnameseTones(
      shopName === null ? shopName : shopName.toLowerCase(),
    );
    //
  ...
  const handleSearch = text => {
    ...
    const filteredData = filter(dataDistrict.dataMainF, shop => {
  ...
  const checkSelect = item => {
    if (
      (item.latitude || 0) !== 0 &&
      (item.longitude || 0) !== 0 &&
      ((itemSelect.districtSelect.latitude == item.latitude &&
        itemSelect.districtSelect.longitude == item.longitude) ||
        itemSelect.districtSelect.address == item.address)
    ) {
      return true;
    } else if (
      itemSelect.districtSelect.address == item.address &&
      itemSelect.districtSelect.latitude !== 0 &&
      itemSelect.districtSelect.longitude !== 0 &&
      !item.latitude &&
      !item.longitude
    ) {
      return true;
    ...
    >
      ...
      >
        ...
        <ScrollView style={{ flex: 1, padding: 12 }}>
          {dataDistrict.dataMain.length > 0 &&
            dataDistrict.dataMain.map((it, idx) => {
                ...
                >
                  <Text
                    style={{
                      fontSize: 16,
                      fontWeight: '600',
                      color: isSelect ? appcolor.white : appcolor.primary,
                    }}
                  >
                  ...
                  <Text
                    style={{
                      fontSize: 13,
                      fontWeight: '500',
                      color: isSelect ? appcolor.white : appcolor.dark,
                    }}
                  >
                    Địa chỉ: {it.address}
                  </Text>
                  ...
                  >
                    Vị trí:{' '}
                    {it.latitude ||
                      (isSelect ? itemSelect.districtSelect.latitude || 0 : 0)}
                    ,{' '}
                    {it.longitude ||
                      (isSelect ? itemSelect.districtSelect.longitude || 0 : 0)}
                  </Text>
                </TouchableOpacity>
              );
            })}
