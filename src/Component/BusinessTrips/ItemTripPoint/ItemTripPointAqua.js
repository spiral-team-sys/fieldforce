...
export const ItemTripPointAqua = ({ item, index, itemTrips, itemPoint, typeVehicle, config, quotaData, dateFilter }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    const [_, setMutate] = useState(false)
    const [dataByDate, setDataByDate] = useState({ dataPlan: dateFilter.dataPlan || [], isShowPlan: false })
    ...
    const handlePressCopy = (itemStore) => {
        ...
        item.locationPoint = (itemStore.latitude != null && itemStore.latitude != 0) ? itemStore.latitude + ',' + itemStore.longitude : null
        item.distance = 0
        item.distanceText = ''
        dataByDate.isShowPlan = false
        setReload(e => !e)
    ...
    return (
        <View style={{ borderRadius: 8, borderWidth: 0.6, borderColor: appcolor.primary, padding: 4, margin: 4 }}>
            ...
            <TouchableOpacity onPress={() => handleSelectPlanData()} style={{
                ...
                <Text style={{ fontWeight: '700', textAlign: 'center', fontSize: 14, color: appcolor.white }}>{dataByDate.isShowPlan ? 'Đóng' : 'Mở danh sách shop (' + (dataByDate.dataPlan.length || 0) + ')'}</Text>
            </TouchableOpacity>
            {
                quotaData.isUsePlanSystem == 1 && dataByDate.isShowPlan == true && dataByDate.dataPlan?.length > 0 &&
                <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center' }}>
                    {dataByDate.dataPlan.map((it, idx) => {
                        return (
                            <TouchableOpacity key={`listTrip_${item.arrivedDay}_${idx}`} onPress={() => handlePressCopy(it)} style={{
                                flexDirection: 'row', marginVertical: 4, marginHorizontal: 8, borderRadius: 8,
                                shadowOpacity: 0.5, elevation: 3, backgroundColor: appcolor.surface,
                                shadowColor: appcolor.dark, shadowOffset: { width: 3, height: 0 }
                            }}>
                                <View style={{ padding: 8, width: '100%' }}>
                                    <Text style={{ fontWeight: '600', fontSize: 14, color: appcolor.dark }}>{it.shopName}</Text>
                                    <Text style={{ fontWeight: '500', fontSize: 12, color: appcolor.placeholderText }}>{it.address}</Text>
                                </View>
                            </TouchableOpacity>
                        )
                    })}
            ...
            <View style={{ width: '100%' }}>
                <AutoCompleteAddress
                    ...
                    borderInputColor={item.locationPoint != null && item.locationPoint?.length > 0 ? appcolor.success : null}
                ...
                />
                {
                    ...
                    <View style={{ width: '100%', paddingHorizontal: 8 }}>
                        <Text style={{ color: appcolor.success, textAlign: 'left', fontWeight: '700', fontSize: 12, paddingBottom: 4 }}>Khoảng cách từ điểm {index == 0 ? ('Bắt đầu => ' + (itemPoint.length == 1 ? 'Kết thúc' : 'điểm 1')) : `${index} => điểm ${index + 1}`} : {item.distanceText} </Text>
                        <Text style={{ color: appcolor.primary, textAlign: 'left', fontWeight: '700', fontSize: 12 }}>Tổng khoảng cách tạm tính : {Math.round(item.distance / 1000) + ((item.numberStore || 0) * (quotaData.kmDefault || 5))} km </Text>
                    </View>
                }
            </View>
            <ItemInput
