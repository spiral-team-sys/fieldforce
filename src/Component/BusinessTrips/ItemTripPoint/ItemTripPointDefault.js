...
export const ItemTripPointDefault = ({
  item,
  index,
  itemTrips,
  itemPoint,
  ...
  const listTypeCost =
    _.find(itemTrips.listTypeCost || [], it => it.isChoose == 1) || {};
  const arrivedDate = parseTripDate(item.arrivedDay);
  const arrivedDateValue = arrivedDate?.isValid()
    ? arrivedDate.format('YYYY/MM/DD')
    : null;
  const calendarDefaultDate = (
    arrivedDate?.isValid() ? arrivedDate : parseTripDate(itemTrips.fromDate)
  )?.format('YYYY-MM-DD');
  ...
  const handlerWorkingDate = async fromValue => {
    ...
    if (
      arrivedDayInt.isBetween(itemTrips.fromDate, itemTrips.toDate, null, '[]')
    ) {
      LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
  ...
  return (
    ...
    >
      ...
      {quotaData.isUseProviceList == 1 && (
        <TouchableOpacity
          onPress={() => handleSelectProviceData()}
          style={{
            width: '80%',
            padding: 8,
            justifyContent: 'center',
        ...
        >
          ...
          >
            {listProvinceData.isShowProvince ? 'Đóng' : 'Mở danh sách'}
          </Text>
        </TouchableOpacity>
      )}
      {quotaData.isUseProviceList == 1 &&
        listProvinceData.isShowProvince == true &&
        listProvinceData.dataMain?.length > 0 && (
          <View style={{ width: '100%' }}>
            <FlashList
              ...
              keyExtractor={(_item, index) =>
                _item.provinceCode + index.toString()
              }
              data={dataProvince}
            ...
            {listDistrict?.length > 0 && (
              <FlashList
                ...
                keyExtractor={(_item, index) =>
                  _item.provinceCode + index.toString() + _item.districtCode
                }
                data={listDistrict}
      ...
      {item.addressPoint && quotaData.isUseProviceList == 1 && (
        ...
        >
          <Text
            style={{ fontSize: 13, fontWeight: '600', color: appcolor.primary }}
          >
            Cửa hàng: {item.shopName}
          </Text>
          <Text
            style={{ fontSize: 11, fontWeight: '500', color: appcolor.dark }}
          >
            Địa chỉ: {item.addressPoint}
          </Text>
          <Text
            style={{ fontSize: 11, fontWeight: '500', color: appcolor.dark }}
          >
            Vị trí: {item.locationPoint}
          </Text>
        </View>
      )}
      ...
      >
        ...
        {((typeVehicle?.code == 'DRIVING' &&
          ...
          <ItemInput
            ...
            onChangeText={handlerItemChangeText}
            editable={
              typeVehicle?.code == 'DRIVING' || quotaData.lockEditMoveCost == 1
                ? false
                : true
            }
          />
