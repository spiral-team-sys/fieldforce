...
export const ItemTripPointBosch = ({
  item,
  index,
  itemTrips,
  itemPoint,
  ...
  const handlerWorkingDate = async fromValue => {
    ...
    if (
      ...
      alertWarning(
        `Ngày đến phải nằm trong khoảng ngày đi và ngày về của chuyến công tác (${moment(
          itemTrips.fromDate,
        ).format('YYYY/MM/DD')}-${moment(itemTrips.toDate).format(
          'YYYY/MM/DD',
        )})`,
      );
    }
  ...
  const handlerItemChangeText = async (text, typeItem) => {
    if (typeItem == TYPE.TYPE_NOTE) {
      ...
      if (typeItem == TYPE.TYPE_KM_DISTANCE) {
        ...
        if (
          (item.numberDay || item.numberDay == 0) &&
          (item.eatDay || item.eatDay == 0)
        ) {
  ...
  const countFoodCost = () => {
    ...
    if (itemTrips.typeAdvance?.code == 'INSIDEPROVINCE') {
      if (
        item.distance >= quotaData.kmFoodLimit1 &&
        item.distance < quotaData.kmFoodLimit2
      ) {
  ...
  const countHotelCost = () => {
    ...
    if (itemTrips.typeAdvance?.code == 'INSIDEPROVINCE') {
      ...
      if (
        countProvince >= quotaData.limitPointSupHotel ||
        item.distance > item.kmHotelLimit
      ) {
        hotelCost = item.numberDay * quotaData.costHotelSup;
      }
    }
    ...
    >
      ...
      >
        <FormGroup
          ...
          value={
            item.arrivedDay
              ? moment(item.arrivedDay).format('YYYY/MM/DD')
              : null
          }
