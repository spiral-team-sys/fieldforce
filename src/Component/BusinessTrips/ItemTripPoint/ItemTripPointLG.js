...
export const ItemTripPointLG = ({ item, index, itemTrips, itemPoint, typeVehicle, config, quotaData, dataProvince }) => {
    ...
    const onEditEatValue = (value) => {
        ...
        item.maxCostVehicle = ((quotaData.moveSup && quotaData.moveSup > 0) ? quotaData.moveSup * intValue : 0)
        item.vehicalValue = ((quotaData.moveSup && quotaData.moveSup > 0) ? quotaData.moveSup * intValue : 0)
        ...
        if (arrivedDayInt.isBetween(fromDate, toDate, null, '[]')) {
            if (quotaData.isCheckDoubleDate == 1) {
                ...
                const exists = itemPoint.some(item =>
                    moment(item.workDate, 'YYYY-MM-DD').isSame(moment(fromValue, 'YYYY-MM-DD'))
                );
                if (exists) {
    ...
    const handlerItemChangeText = async (text, typeItem) => {
        ...
        if (typeItem == TYPE.TYPE_NOTE || typeItem == TYPE.PROVINCE_TO || typeItem == TYPE.TYPE_STOREVISIT || typeItem == TYPE.TYPE_REMARK) {
            item[typeItem] = text
            setMutate(e => !e)
        } else {
            ...
            <View key={'dateBusiness'} style={{ width: '100%', borderRadius: 5, paddingBottom: 8, paddingHorizontal: 8, alignItems: 'center' }}>
                ...
                <View style={{ paddingHorizontal: 8 }}>
                    <FormGroup
                        ...
                        value={item.workDate ? moment(item.workDate).format('YYYY/MM/DD') : null}
                        rightFunc={handleSelectDate}
                    />
                </View>

                {item.loadCalendar && <CalendarSelected month={10} onChangeData={handlerWorkingDate} isBetween={false} lockOutMonth={config?.isLockOutMonth == 0 ? false : true} />}
                <ItemInput
                    isRequire
                ...
                <>
                    ...
                    <View key={`ViewPointDay`} style={{ width: '100%', flexDirection: 'row', alignItems: 'center', alignSelf: 'center', justifyContent: 'center', padding: 5 }}>
                        <FormGroup
                            ...
                            containerStyle={{ width: '33%', borderRadius: 8, borderWidth: 1, marginBottom: 0, padding: 0, backgroundColor: 'transparent' }}
                        ...
                        <FormGroup
                            ...
                            keyboardType="numeric"
                            titleStyle={{ textAlign: 'center' }}
                            containerStyle={{ width: '33%', borderRadius: 8, borderWidth: 1, marginBottom: 0, padding: 0, marginStart: 8, backgroundColor: 'transparent' }}
                            inputStyle={{ textAlign: 'center', fontSize: 12, backgroundColor: 'transparent' }}
                            editable={false}
