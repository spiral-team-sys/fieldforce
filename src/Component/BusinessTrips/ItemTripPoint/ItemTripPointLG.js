...
export const ItemTripPointLG = ({
  ...
  const onEditValue = value => {
    ...
    item.maxCostHotel =
      quotaData.hotelSup && quotaData.hotelSup > 0
        ? quotaData.hotelSup * intValue
        : 0;
    // if (intValue == 0) {
  ...
  const onEditEatValue = value => {
    ...
    item.maxCostFood =
      quotaData.foodSup && quotaData.foodSup > 0
        ? quotaData.foodSup * intValue
        : 0;
    item.foodCostPoint =
      quotaData.foodSup && quotaData.foodSup > 0
        ? quotaData.foodSup * intValue
    ...
    item.maxCostVehicle =
      quotaData.moveSup && quotaData.moveSup > 0
        ? quotaData.moveSup * intValue
        : 0;
    item.vehicalValue =
      quotaData.moveSup && quotaData.moveSup > 0
        ? quotaData.moveSup * intValue
  ...
  const handlerWorkingDate = async fromValue => {
    ...
    const fromDate = moment(itemTrips.fromDate, 'YYYYMMDD');
    const toDate = moment(itemTrips.toDate, 'YYYYMMDD');
    if (arrivedDayInt.isBetween(fromDate, toDate, null, '[]')) {
      if (quotaData.isCheckDoubleDate == 1) {
        // itemPoint
        const exists = itemPoint.some(item =>
          moment(item.workDate, 'YYYY-MM-DD').isSame(
            moment(fromValue, 'YYYY-MM-DD'),
          ),
        );
        if (exists) {
          alertWarning(`Ngày bạn chọn đã tồn tại`);
        } else {
          LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
          item.workDate = fromValue;
          item.loadCalendar = item.loadCalendar == true ? false : true;
          setMutate(e => !e);
        }
      } else {
      ...
      alertWarning(
        `Ngày đến phải nằm trong khoảng ngày đi và ngày về của chuyến công tác\n${fromDate.format(
          'YYYY/MM/DD',
  ...
  const handlerItemChangeText = async (text, typeItem) => {
    if (
      typeItem == TYPE.TYPE_NOTE ||
      typeItem == TYPE.PROVINCE_TO ||
      typeItem == TYPE.TYPE_STOREVISIT ||
      typeItem == TYPE.TYPE_REMARK
    ) {
      item[typeItem] = text;
      setMutate(e => !e);
    } else {
  ...
  const handleSelectProviceData = () => {
    LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
    listProvinceData.isShowProvince = listProvinceData.isShowProvince
      ? false
      : true;
    setMutate(e => !e);
  };
  ...
  const handlerSelectDistrict = async (itemD, indexD, key) => {
    ...
    const dataByDistrict = dataByProvince.filter(
      it => it.districtCode == itemD.districtCode,
    );
  ...
  const handleSelectStore = async itemD => {
    ...
    if (!itemD.latitude || !itemD.longitude) {
      isNullLocation = 1;
      await onGetAddress(
        itemD.address || `${itemD.district},${itemD.province}`,
      );
    }
    if (isNullLocation == 1 && Object.keys(dataItem.dataSelect)?.length > 0) {
      ...
      item.addressPoint =
        itemD.address || `${itemD.district},${itemD.province}`;
      item.locationPoint = `${dataItem.dataSelect.geometry.location.lat || 0},${
        dataItem.dataSelect.geometry.location.lng || 0
      }`;
      item.distance = 0;
      item.distanceText = '';
    } else {
  ...
  return (
    ...
    >
      {quotaData.isFirstPointLastPoint == 1 &&
        itemPoint.length - 1 == index && (
          <View
            style={{
              width: '80%',
              height: 40,
      ...
      >
        {itemTrips.typeKM == 3 && (
          <ItemInput
            isRequire
            titleName="Địa chỉ cửa hàng đến"
            placeholder="Nhập địa chỉ cửa hàng đến"
            iconName="store"
            itemValue={item.shopVisit}
            key={`${TYPE.TYPE_STOREVISIT}`}
            typeFilter={TYPE.TYPE_STOREVISIT}
        ...
        {itemTrips.typeKM == 1 && (
          <ItemInput
            ...
            titleName="Tỉnh đến"
            placeholder="Nhập tỉnh đến"
            iconName="map-marker-alt"
            typeFilter={TYPE.PROVINCE_TO}
            itemValue={item.provinceTo}
            onChangeText={handlerItemChangeText}
          />
        )}
        <View style={{ paddingHorizontal: 8 }}>
          <FormGroup
            ...
            value={
              item.workDate ? moment(item.workDate).format('YYYY/MM/DD') : null
            }
        ...
        <ItemInput
          isRequire
          key={`${TYPE.TYPE_REMARK}`}
          titleName="Ghi chú"
          placeholder="Nội dung ghi chú"
          iconName="comment"
          typeFilter={TYPE.TYPE_REMARK}
          itemValue={item.remark}
          onChangeText={handlerItemChangeText}
        />
      </View>
        ...
        <>
          ...
          >
            <FormGroup
              ...
              editable={false}
              defaultValue={item.quantityHotel?.toString() || null}
            ...
            <FormGroup
              ...
              editable={false}
              defaultValue={item.quantityAllowance?.toString() || null}
              placeholder={`Số lượng`}
              useClearAndroid={false}
            />
            <FormGroup
              ...
              editable={false}
              defaultValue={item.quantityGasoline?.toString() || null}
              placeholder={`Số lượng`}
              useClearAndroid={false}
            />
          </View>
          ...
          >
            ...
            <ItemInput
              ...
              iconName="hotel"
              itemValue={valueItem(item.amountHotel)}
              onChangeText={handlerItemChangeText}
            />
            <ItemInput
              typeFilter={TYPE.TYPE_ALLOWANCE}
              keyboardType="numeric"
              isRequire
              editable={false}
              titleName={`Chi phí phụ cấp`}
              placeholder={`Tổng chi phí phụ cấp`}
              iconName="utensils"
              itemValue={valueItem(item.amountAllowance)}
            />
            <ItemInput
              typeFilter={TYPE.TYPE_GASOLINE}
              keyboardType="numeric"
              isRequire
              editable={false}
              titleName="Chi phí xăng xe trong ngày"
              placeholder={`Tổng chi phí xăng xe trong ngày`}
              iconName="car"
              itemValue={valueItem(item.amountGasoline)}
            />
            <ItemInput
              typeFilter={TYPE.TYPE_TICKET}
              keyboardType="numeric"
              isRequire
              editable={false}
              titleName="Chi phí vé trong ngày"
              placeholder={`Tổng chi phí vé trong ngày`}
              iconName="car"
              itemValue={valueItem(item.amountTicket)}
            />
            <ItemInput
              typeFilter={TYPE.TYPE_VEHICALHIRE}
              keyboardType="numeric"
              isRequire
              editable={false}
              titleName="Chi phí thuê xe trong ngày"
              placeholder={`Tổng chi phí thuê xe trong ngày`}
              iconName="car"
              itemValue={valueItem(item.amountVehicleHire)}
            />
          </View>
        </>
      )}
...
const ListShop = ({
  ...
  const loadData = () => {
    dataDistrict.dataMain =
      dataByDistrict.length > 0 ? [...dataByDistrict] : [];
    dataDistrict.dataMainF =
      dataByDistrict.length > 0 ? [...dataByDistrict] : [];
    setMutate(e => !e);
  };
  useEffect(() => {
  ...
  const handleSearch = text => {
    ...
    const filteredData = _.filter(dataDistrict.dataMainF, shop => {
