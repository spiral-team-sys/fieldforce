...
import { LayoutAnimation, Platform, SafeAreaView, ScrollView, StyleSheet, TouchableOpacity, UIManager, View } from "react-native"
    ...
    const LoadData = async (fromDate, toDate) => {
        ...
        await GetBusinesTrips(
            ...
            async (mData) => {
                ...
                await setLockCreate(mData[0]?.isCreateInMonth == 1 ? false : (moment(fromValue).format('YYYYMM') <= moment().add('days', config?.dayCheck || 0).format('YYYYMM') ? true : false))
            }
        )
                ...
                {(filter.mode == MODE.HISTORY || filter.mode == MODE.CONFIRM) &&
                    <SafeAreaView style={styles.sheetView}>
                        ...
                        {modeHeader !== 'invoice' && !lockCreate &&
                            <TouchableOpacity style={{ position: 'absolute', bottom: deviceHeight / 10, zIndex: 1000, end: 16 }} onPress={handlerAction}>
