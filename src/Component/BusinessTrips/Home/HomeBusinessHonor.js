...
import { CheckLocation, UUIDGenerator } from '../../../Core/Helper';
import {
  alertConfirm,
  alertWarning,
  deviceHeight,
...
import { deleteItemPhotoByType } from '../../../Controller/DisplayController';
...
export const HomeBusinessHonor = ({ navigation }) => {
  ...
  const [limitCostSupport, setLimitCostSupport] = useState([]);
  const [limitCostSupportTotal, setLimitCostSupportTotal] = useState([]);
  ...
  const LoadData = async (fromDate, toDate) => {
    ...
    await GetBusinesTrips(
      ...
      async mData => {
        ...
        await setLimitCostSupport(mData[0]?.limitCostSupport);
        await setLimitCostSupportTotal(mData[0]?.limitCostSupportTotal);
        await setTotalDayMonth(mData[0]?.totalDayMonth || 0);
        await setLockCreate(
          mData[0]?.isCreateInMonth == 1
            ? false
            : moment(fromValue).format('YYYYMM') <=
              moment()
                .add('days', config?.dayCheck || 0)
                .format('YYYYMM')
              ? true
              : false,
        );
  ...
  const getMyLocation = async () => {
    await CheckLocation(() => {
      LOCATION_INFO.getCurrentLocation(async position => {
        setMarker({
  ...
  const handlerRegisterTrips = async () => {
    ...
    await setFilter({
      ...
      quotaData: quotaData,
      limitCostSupport: limitCostSupport,
      limitCostSupportTotal: limitCostSupportTotal,
  ...
  const handlerEditTrips = async item => {
    ...
    const itemEdit = {
      ...
      dayValue: day,
      nightValue: night,
      daysOld: item.days || day,
      nightsOld: item.nights || night,
      ...
      supportWork: item.supportWork,
      totalNumberDay:
        _.sumBy(JSON.parse(item.provinceList || '[]'), 'numberDay') || 0,
      ...
      supportWorkMaint: item.supportWorkMaint,
      totalCostsTrip: item.totalCostsTrip,
      countUpdate: item.countUpdate,
      shopStartName: item.shopStartName,
      shopEndName: item.shopEndName,
      guid: item.guid || UUIDGenerator(),
      evidence: item.evidence || null,
      listTypeCost: JSON.parse(item.listTypeCost || '[]'),
      photoByType: item.photoByType || null,
      isHaveExceed: item.isHaveExceed || 0,
      isActual: item.isActual || 0,
    };
    ...
    await setFilter({
      ...
      isUpdate: true,
      isPlus: false,
      tripConfig: item.tripConfig || tripConfig,
      quotaData: item.quotaData || quotaData,
      limitCostSupport: limitCostSupport,
      limitCostSupportTotal: limitCostSupportTotal,
      totalDayMonth: totalDayMonth,
      isEvidentDateChange: item.isEvidentDateChange == 1,
      guid: itemEdit.guid,
    });
  };
  ...
  const deletePhoto = async () => {
    await deleteItemPhotoByType('Workingschedule_');
  };
  useEffect(() => {
    getMyLocation();
    SetDataByMode();
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <MapView
        ...
        initialRegion={{
          latitude: marker.latitude || 10.792573,
          longitude: marker.longitude || 106.702454,
          latitudeDelta: 0.01,
          longitudeDelta: 0.01,
        }}
      >
