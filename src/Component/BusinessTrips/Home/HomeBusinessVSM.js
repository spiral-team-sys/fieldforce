...
export const HomeBusinessVSM = ({ navigation }) => {
  ...
  const [limitCostMain, setLimitCostMain] = useState({});
  ...
  const LoadData = async (fromDate, toDate) => {
    ...
    await GetBusinesTrips(
      ...
      async mData => {
        ...
        await setLimitCost(mData[0]?.limitCost);
        await setLimitCostMain(mData[0]?.limitCostMain);
      },
    );
  ...
  const handlerEditTrips = async item => {
    ...
    await setFilter({
      ...
      isUpdate: true,
      isPlus: false,
      limitCost: item.limitCost,
      limitCostMain: item.limitCostMain,
    });
  };
  ...
  useEffect(() => {
    const _mylocation = getMyLocation();
    const _databymode = SetDataByMode();
    return () => {
      _mylocation;
      _databymode;
    };
                        ...
                        >
                          ...
                          >
                            <FlatList
                              ...
                              ListFooterComponent={
                                <View
                                  style={{ paddingBottom: deviceHeight / 3 }}
                                />
                              }
                            />
                          </View>
