...
export const HomeBusinessLG = ({ navigation }) => {
  ...
  const [limitCostSupport, setLimitCostSupport] = useState(0);
  ...
  const handlerEditTrips = async item => {
    ...
    const itemEdit = {
      ...
      isViewByList: item.isViewByList || 0,
      typeKM: item.typeKM || 1,
  ...
  const handlerChangeHeader = async mode => {
    console.log(mode, 'modemodemodemode');
    if (loading) return;
    setCurrentItem({});
    setModeHeader(mode);
    switch (mode) {
