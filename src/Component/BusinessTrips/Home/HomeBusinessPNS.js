...
import { FlatList, LayoutAnimation, Platform, SafeAreaView, ScrollView, StyleSheet, TouchableOpacity, UIManager, View } from "react-native"
...
export const HomeBusinessPNS = ({ navigation }) => {
    ...
    const pendingCopyTripTemplateRef = useRef(null)
    const [copyTripDatePending, setCopyTripDatePending] = useState(false)
    const [copyTripRanges, setCopyTripRanges] = useState([])
    const [copyCalendarKey, setCopyCalendarKey] = useState(0)

    const config = JSON.parse(kpiinfo?.reportItem) || {}
    const getTripsByDateRange = async (fromYYYYMMDD, toYYYYMMDD) => {
        return await new Promise((resolve) => {
            GetBusinesTrips(fromYYYYMMDD, toYYYYMMDD, async (mData) => resolve(Array.isArray(mData) ? mData : []))
        })
    }
    ...
    const LoadDataConfirm = async (fromDate, toDate, month) => {
    ...
    const handlerUploadDocument = (currentItem, item, currentIndexHome) => {
        setCurrentItem({
            ...currentItem, isCanEdit: item.isCanEdit, isCanNext: item.isCanNext, isCanCancel: item.isCanCancel,
            currentIndexHome: currentIndexHome, isCancelInView: item.isCancelInView, isCanNextInView: item.isCanNextInView
        })
    ...
    const cancelCopyTripDateStep = useCallback(() => {
        pendingCopyTripTemplateRef.current = null
        setCopyTripRanges([])
        setCopyTripDatePending(false)
        setCopyCalendarKey((k) => k + 1)
    }, [])
    const removeCopyRangeAt = (index) => {
        setCopyTripRanges((prev) => {
            const next = [...prev]
            next.splice(index, 1)
            return next
        })
    }
    const addCopyRange = (fromStr, toStr) => {
        const fromDate = moment(fromStr).format('YYYYMMDD')
        const toDate = moment(toStr).format('YYYYMMDD')
        const betweenDate = moment(toStr).diff(moment(fromStr), 'days')
        const calculatorDate = `(${betweenDate + 1} Ngày ${betweenDate} Đêm)`
        const nextRange = {
            fromDate,
            toDate,
            dayValue: betweenDate + 1,
            nightValue: betweenDate,
            dateFilterFrom: moment(fromStr).format('DD/MM/YYYY'),
            dateFilterTo: moment(toStr).format('DD/MM/YYYY'),
            dayAndNight: calculatorDate,
        }
        setCopyTripRanges((prev) => [...prev, nextRange])
    }
    const confirmCopyRangesAndOpenForm = async () => {
        const base = pendingCopyTripTemplateRef.current
        if (!base)
            return
        if (!copyTripRanges || copyTripRanges.length === 0) {
            alertWarning('Vui lòng chọn ít nhất 1 khung ngày')
            return
        }
        const first = copyTripRanges[0]
        const tripTemplate = {
            ...base,
            ...first,
            loadCalendar: false,
            tripDateRanges: copyTripRanges,
        }
        // await dispatch({ type: ACTION.SET_BUSINESS_TRIPS, itemTrips: tripTemplate })
        await dispatch(SetBusinesTrips(tripTemplate))
        await setFilter({
            ...filter,
            moneyLimit: moneyLimit,
            mode: MODE.CREATE,
            dayAndNight: first.dayAndNight,
            dayValue: first.dayValue,
            nightValue: first.nightValue,
            itemDate: itemTrips,
            isUpdate: false,
            isPlus: false,
            isLockOutMonth: false,
    ...
    const reopenCopyDateSelector = () => {
        if (!pendingCopyTripTemplateRef.current) {
            alertWarning('Không có dữ liệu sao chép để chọn lại lịch')
            return
        }
        setCopyCalendarKey((k) => k + 1)
        setCopyTripDatePending(true)
    }
    const handlerCopyTripItem = async (template) => {
        if (!config.isCopy)
            return
        if (loading)
            return
        ...
        try {
            if (!template || Object.keys(template).length === 0) {
                alertWarning('Không tìm thấy lịch đăng ký để sao chép')
                return
            }
            const provinceList = (() => {
                try { return JSON.parse(template.provinceList || '[]') || [] } catch (e) { return [] }
            })()
            const tripTemplateBase = {
                idTrip: 0,
                loadCalendar: false,
                moneyLimit: moneyLimit,
                provinceFrom: template.provinceFromVN || null,
                provinceTo: template.provinceToVN || null,
                provinceList: provinceList,
                provinceCodeFrom: template.provinceFromCode || null,
                provinceCodeTo: template.provinceToCode || null,
                addressFrom: template.addressStart || null,
                addressTo: template.addressEnd || null,
                locationStart: template.locationStart || null,
                locationEnd: template.locationEnd || null,
                note: template.note || null,
                supportKM: template.supportKM || 0,
                supportVehical: template.supportCar || 0,
                supportVehicalOther: template.supportVehicalOther || 0,
                supportNight: template.supportNight || 0,
                supportLunch: template.supportLunch || 0,
                supportDinner: template.supportDinner || 0,
                supportOther: template.supportOther || 0,
                totalNumberDay: _.sumBy(provinceList, 'numberDay') || 0,
                typePeople: template.typePeople || null,
                typeKM: template.typeKM || null,
                daysMove: template.days || null,
                kmValue: template.kilometer || null,
                tripReplace: template.tripReplace || null,
                districtFrom: template.districtFrom || null,
                districtTo: template.districtTo || null,
                districtCodeFrom: template.districtCodeFrom || null,
                districtCodeTo: template.districtCodeTo || null,
            }
            pendingCopyTripTemplateRef.current = tripTemplateBase
            setCopyTripRanges([])
            setCopyCalendarKey((k) => k + 1)
            setCopyTripDatePending(true)
        } finally {
    ...
    const handlerNextCreate = async (typeArrow) => {
        ...
        if (mode !== undefined)
            await setFilter({ ...filter, mode: mode, modeDefault: MODE.CREATE, loadCalendar: false })
    }
    ...
    const renderItem = ({ item, index }) => {
        return (
            <ItemResultView
                ...
                handlerCopyTrip={(config.isCopy && modeHeader == 'create' && !isConfirmTrip) ? handlerCopyTripItem : undefined}
            />
        )
    }
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <ActionSheet id='sheetAction'
                ...
                onClose={cancelCopyTripDateStep}
                ...
                {copyTripDatePending &&
                    <SafeAreaView style={styles.sheetView}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', padding: 12, borderBottomWidth: 1, borderBottomColor: appcolor.surface }}>
                            <TouchableOpacity onPress={cancelCopyTripDateStep} style={{ padding: 8 }}>
                                <Icon name="arrow-back" type="material" color={appcolor.dark} size={24} />
                            </TouchableOpacity>
                            <Text style={{ flex: 1, textAlign: 'center', fontSize: 16, fontWeight: '600', color: appcolor.dark, marginEnd: 32 }}>Chọn khung ngày (sao chép)</Text>
                        </View>
                        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }} keyboardShouldPersistTaps="handled">
                            <Text style={{ fontSize: 13, color: appcolor.greylight, marginBottom: 10, textAlign: 'center' }}>Chọn nhiều khung ngày (từ - đến). Mỗi khung ngày sẽ tạo thành 1 lịch công tác riêng, dữ liệu công tác giữ nguyên theo lịch đã copy.</Text>
                            <CalendarSelected
                                key={`copyTripCal_${copyCalendarKey}`}
                                lockOutMonth={true}
                                defaultDate={filter.fromDateCaculator.clone().format('YYYY-MM-DD')}
                                minDate={filter.fromDateCaculator.clone().subtract(1, 'month').startOf('month').format('YYYY-MM-DD')}
                                maxDate={filter.fromDateCaculator.clone().add(1, 'month').endOf('month').format('YYYY-MM-DD')}
                                month={filter.fromDateCaculator.clone().add(1, 'month').format('MM')}
                                onChangeData={(fromStr, toStr) => {
                                    if (fromStr && toStr)
                                        addCopyRange(fromStr, toStr)
                                }}
                            />
                            ...
                            <View style={{ marginTop: 12, padding: 10, borderRadius: 10, backgroundColor: appcolor.surface }}>
                                <Text style={{ fontSize: 13, fontWeight: '700', color: appcolor.dark, marginBottom: 8 }}>Danh sách khung ngày đã chọn</Text>
                                {copyTripRanges.length === 0 ? (
                                    <Text style={{ fontSize: 12, color: appcolor.greylight, textAlign: 'center' }}>Chưa có khung ngày nào</Text>
                                ) : (
                                    copyTripRanges.map((r, idx) => (
                                        <View key={`r_${idx}`} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: idx === (copyTripRanges.length - 1) ? 0 : 0.5, borderBottomColor: appcolor.lightgrey }}>
                                            <Text style={{ fontSize: 13, color: appcolor.dark }}>{`${idx + 1}) ${r.dateFilterFrom} → ${r.dateFilterTo} ${r.dayAndNight}`}</Text>
                                            <TouchableOpacity onPress={() => removeCopyRangeAt(idx)} style={{ padding: 6 }}>
                                                <Icon name="trash" type="font-awesome-5" size={16} color={appcolor.danger} />
                                            </TouchableOpacity>
                                        </View>
                                    ))
                                )}
                            </View>
                            <TouchableOpacity onPress={confirmCopyRangesAndOpenForm} style={{ marginTop: 12, padding: 12, backgroundColor: appcolor.primary, borderRadius: 10, alignItems: 'center' }}>
                                <Text style={{ color: appcolor.light, fontWeight: '700', fontSize: 15 }}>Tiếp tục</Text>
                            </TouchableOpacity>
                        </ScrollView>
                    </SafeAreaView>
                }
                ...
                {(filter.mode == MODE.HISTORY || filter.mode == MODE.CONFIRM) &&
                    ...
                    <SafeAreaView style={styles.sheetView}>
                        {modeHeader !== 'invoice' &&
                            <View style={{ position: 'absolute', bottom: deviceHeight / 10, zIndex: 10, end: 16, alignItems: 'center' }}>
                                <TouchableOpacity onPress={handlerAction}>
                                    <View style={{ width: 52, height: 52, justifyContent: 'center', backgroundColor: appcolor.dark, borderRadius: 60, opacity: 0.85 }}>
                ...
                {(filter.mode == MODE.CREATE || filter.mode == MODE.UPDATE) && !copyTripDatePending &&
                    <SafeAreaView style={styles.sheetView}>
                        <CreateNewTrip
                            dataTrips={dataTrips}
                            dateFilter={filter}
                            ...
                            onReSelectCopyDate={(config.isCopy && copyTripRanges.length > 0) ? reopenCopyDateSelector : undefined} />
                    </SafeAreaView>
                }
