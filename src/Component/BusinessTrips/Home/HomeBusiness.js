...
import { GetBusinesTrips, GetConfirmTrips, UploadBusiness } from "../../../Controller/BussinessTripController";
...
export const HomeBusiness = ({ navigation }) => {
    ...
    const handlerShowTrip = () => {
        ...
        setFilter({ ...filter, mode: filter.mode == MODE.CONFIRM ? MODE.HISTORY : filter.mode })
    }
    ...
    const handlerCreateTrips = async (mode) => {
        ...
        // }
        if (isConfirmTrip) {
            if (moment(filter.fromDateCaculator).format('MM') !== moment().format('MM')) {
                alertWarning(`Vui lòng thêm chuyến đi trong tháng ${moment().format('MM-YYYY')}`)
                return
            }
        }
        const betweenDate = moment(filter.toDateCaculator).diff(moment(filter.fromDateCaculator), 'days')
    ...
    const styles = StyleSheet.create({
        ...
        actionHeader: { width: '100%', alignItems: 'center', padding: 8, justifyContent: 'center' },
        ...
        historyViewItem: { width: '100%', height: deviceHeight / (moneyLimit > 0 ? 1.4 : 1.3), alignSelf: 'center' },
                    ...
                    <SafeAreaView style={styles.sheetView}>
                        <View style={styles.actionHeader}>
                            ...
                            <View style={{ width: deviceWidth, flexDirection: 'row' }}>
                                {!lockCreate && !isConfirmTrip &&
                                    <TouchableOpacity style={styles.buttonAction} onPress={handlerRegisterTrips} >
                                        <Text style={{ ...styles.titleCreate, minWidth: 100 }}>{'Đăng kí'}</Text>
                                    </TouchableOpacity>
                                }
                                {!lockCreateOld && isConfirmTrip &&
                                    <TouchableOpacity style={styles.buttonActionRegister} onPress={handlerCreateTrips} >
                                        <Text style={{ ...styles.titleCreate, minWidth: 100 }}>{'Thêm chuyến đi'}</Text>
                                    </TouchableOpacity>
                                }
                                {!lockResultTrips &&
                                    <TouchableOpacity
                                        style={{
                                            ...styles.buttonAction, marginStart: 8, backgroundColor: isConfirmTrip ? appcolor.info : appcolor.light,
                                            borderWidth: 1, borderColor: appcolor.info
                                        }}
                                        onPress={handlerShowConfirm}>
                                        <Text style={{ ...styles.titleCreate, color: isConfirmTrip ? appcolor.light : appcolor.dark }}>Kết quả công tác</Text>
                                    </TouchableOpacity>
                                }
                            </View>
                        </View>
                        <View style={styles.actionHistory}>
                            <View style={styles.viewDate}>
                                ...
                                />
                                {filter.loadCalendar && <CalendarSelected onChangeData={isConfirmTrip ? LoadDataConfirm : LoadData} lockOutMonth={true} />}
                            </View>
                        </View>
