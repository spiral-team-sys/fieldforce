...
export const HomeBusinessAqua = ({ navigation }) => {
  ...
  const handlerDeleteTrip = item => {
    ...
    alertConfirm(
      'Chú ý',
      `Bạn có muốn xoá đăng kí công tác${textNotify} Từ ${moment(
  ...
  const handlerRegisterTrips = async () => {
    ...
    const calculatorDate =
      tripConfigParse.isDayNightEqual == 1
        ? `(${betweenDate + 1} Ngày ${betweenDate + 1} Đêm)`
        : `(${betweenDate + 1} Ngày ${betweenDate} Đêm)`;

    const quotaParse = JSON.parse(quotaData);
    if (quotaParse.isUsePlanSystem == 1) {
      await WorkingPlanAPI.getStoreByDate(itemTrips, async mData => {
        filter.dataPlan = mData;
      });
    }
    ...
    await setFilter({
      ...
      nightValue:
        tripConfigParse.isDayNightEqual == 1 ? betweenDate + 1 : betweenDate,
      itemDate: itemTrips,
      isUpdate: false,
      isPlus: false,
      isLockOutMonth: false,
  ...
  const handlerBackCreate = typeArrow => {
    const mode =
      filter.modeDefault == MODE.UPDATE &&
        actionBackHeader(typeArrow) == MODE.CREATE
        ? filter.modeDefault
        : actionBackHeader(typeArrow);
    setFilter({
      ...filter,
      mode: mode,
  ...
  const handlerEditTrips = async item => {
    const betweenDate = moment(item.toDate, 'YYYYMMDD').diff(
      moment(item.fromDate, 'YYYYMMDD'),
      'days',
    );
    ...
    const itemEdit = {
      ...
      toDate: item.toDate,
      dayValue: item.days || day,
      nightValue: item.nights || night,
      ...
      typeAdvance: JSON.parse(item.typeAdvance || '{}') || {},
  ...
  useEffect(() => {
    let isMounted = true; // Đánh dấu component đang được mounted
    if (!isMounted) {
      return;
    }
    getMyLocation();
    SetDataByMode();
    return () => {
      isMounted = false; // Đánh dấu component đã bị unmounted
    };
  }, []);
          ...
          <SafeAreaView style={styles.sheetView}>
            ...
            {!loading && tabStatus !== null && tabStatus.length > 0 && (
              <View style={styles.historyViewItem}>
                ...
                {modeHeader !== 'invoice' && tabStatus?.length > 0 && (
                  <Tabs.Container
                    pagerProps={{
                      scrollEnabled: false,
                    }}
