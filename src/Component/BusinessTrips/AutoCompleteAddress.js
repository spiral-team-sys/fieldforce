...
import {
  AttendantAPI,
  AttendantController,
} from '../../Controller/AttendantController';
...
export const AutoCompleteAddress = ({
        ...
        await AttendantController.DataLocationFromAddress(
          ...
          async dataLocation => {
            if (dataLocation !== null && dataLocation.length > 0)
              ...
              alertNotify(
                'Không tìm thấy địa chỉ, vui lòng kiểm tra lại thông tin và tìm kiếm lại',
              );
          },
        );
      }, 500);
    else await setDataShow([]);
  };
  ...
  const styles = StyleSheet.create({
    ...
    inputView: {
      ...
      borderColor:
        borderInputColor != null ? borderInputColor : appcolor.surface,
      borderWidth: borderInputColor != null ? 0.8 : 0,
      borderRadius: 5,
      marginBottom: 0,
    },
  });
  ...
  return (
    <View style={styles.mainItem}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        >
          <SpiralIcon name="search" size={25} color={appcolor.primary} />
        </TouchableOpacity>
      </View>
      <View style={{ marginTop: 8 }}>
        {isLoading && <ActivityIndicator />}
        <ScrollView key={'idlistdropdow'} showsVerticalScrollIndicator={false}>
