...
export const ItemTripResults = ({ tripResult, quotaData }) => {
	const ItemView = () => {
		switch (AppNameBuild) {
			case psvApp:
				return (tripResult.provinceList !== undefined && tripResult.provinceList?.length > 0) ?
			...
			case honorApp:
				return <ViewResultHonor tripResult={tripResult} quotaData={quotaData} />
			case hafeleApp:
				return <ViewResultHafele tripResult={tripResult} quotaData={quotaData} />
			case lgApp:
