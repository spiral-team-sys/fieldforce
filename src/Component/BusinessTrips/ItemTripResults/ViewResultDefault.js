...
export const ViewResultDefault = ({ tripResult }) => {
	...
	const styles = StyleSheet.create({
		...
		contentOnly: { backgroundColor: appcolor.surface, padding: 8, margin: 8, borderRadius: 10 },
		...
		rangeCard: { backgroundColor: appcolor.surface, padding: 12, marginHorizontal: 8, marginTop: 8, borderRadius: 12, borderWidth: 0.5, borderColor: appcolor.lightgrey, flexDirection: 'row', alignItems: 'center' },
			...
			<View style={styles.contentView}>
				...
				<View style={{ width: deviceWidth / 1.4 }}>
					...
					<View style={styles.costView}>
						...
						<CostItemView title='Ăn trưa' name={tripResult.supportLunch} iconName='utensils' />
	...
	return (
		<View>
			{tripRanges.length > 0 && tripRanges.map((r, idx) => (
				<View key={`range_${idx}`} style={styles.rangeCard}>
					<Text style={{ position: 'absolute', right: 12, top: 10, fontSize: 13, fontWeight: '800', color: appcolor.tomato }}>
						{`${r.dayValue}N ${r.nightValue}Đ`}
					</Text>
					<View style={{ width: 120, alignItems: 'flex-start', marginEnd: 10 }}>
						<Text style={styles.titleDateLine}>{moment(r.dateFilterFrom, 'DD/MM/YYYY').format('DD/MM/YYYY')}</Text>
						<Text style={{ ...styles.titleDateLine, paddingTop: 4 }}>{moment(r.dateFilterTo, 'DD/MM/YYYY').format('DD/MM/YYYY')}</Text>
					</View>
					...
					<View style={{ flex: 1, justifyContent: 'center' }}>
						<Text style={{ fontSize: 15, fontWeight: '800', color: appcolor.info }}>{`Ngày ${idx + 1}`}</Text>
						<Text style={{ fontSize: 13, color: appcolor.dark, marginTop: 2 }}>{r.dayAndNight}</Text>
					</View>
				</View>
			))}

			<View style={tripRanges.length > 0 ? styles.contentOnly : styles.contentView}>
				<View style={{ width: tripRanges.length > 0 ? '100%' : deviceWidth / 1.4 }}>
					<Text style={styles.titleDateLine}>{`${tripResult.provinceFrom}`}</Text>
					<Text style={styles.titleBodyLine}>{`${tripResult.addressFrom}`}</Text>
					<View style={styles.costView}>
						<Text style={{ ...styles.itemCost, fontSize: 14, fontWeight: '600' }}>
