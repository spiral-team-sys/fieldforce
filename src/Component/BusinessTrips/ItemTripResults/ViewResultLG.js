...
export const ViewResultLG = ({ tripResult }) => {
  ...
  const addressTo =
    tripResult.typeKM == 1
      ? listProvinceWork[listProvinceWork.length - 1]?.addressPoint ||
      listProvinceWork[listProvinceWork.length - 1]?.provinceTo ||
      listProvinceWork[listProvinceWork.length - 1]?.shopVisit ||
      ''
      : tripResult.addressTo;
    ...
    <View style={{ width: deviceWidth, padding: 8 }}>
      ...
      <View style={styles.contentView}>
        ...
        <View style={{ width: deviceWidth / 1.4 }}>
          <Text style={styles.titleDateLine}>{`${tripResult.addressFrom || addressFrom
            }`}</Text>
          <View style={styles.costView}>
            <Text
              style={{ ...styles.itemCost, fontSize: 14, fontWeight: '600' }}
            >
              {`Tổng chi phí: ${tripResult.totalSupport !== 0
