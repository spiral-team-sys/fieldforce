				...
				<View style={{ width: deviceWidth / 1.4 }}>
					...
					<View style={styles.costView}>
						...
						{tripResult.kmValue > 0 ?
							...
							<CostItemView title={`Ngày di chuyển`} name={`${tripResult.daysMove} / ${formatNumber(tripResult.daysMove * 250000, ',')} VNĐ`} iconName='road' isUnit={false} isFormat={false} />
						}
						{/* <CostItemView  title='Nghỉ qua đêm' name={tripResult.supportNight} iconName='hotel' /> */}
						<CostItemView title='Di chuyển khác' name={tripResult.supportVehicalOther} iconName='road' />
						<CostItemView title='Lưu trú' name={tripResult.supportStay} iconName='hotel' />
					...
					<View>
						<Text style={styles.titleDateLine}>{`${tripResult.addressTo}`}</Text>
					</View>
				</View>
			</View>
		</View>
