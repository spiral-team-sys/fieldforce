...
export const ViewResultBosch = ({ tripResult }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    const [showStage, setShowStage] = useState()
    const provinceList = Array.isArray(tripResult?.provinceList) ? tripResult.provinceList : []
    ...
    const addressTo = provinceList[provinceList.length - 1]?.addressPoint || ''
    const onPressStage = () => {
            ...
            {provinceList.length > 0 &&
                <TouchableOpacity onPress={() => onPressStage()}>
                    <Text key={`NumStage`}
                        ...
                        {`Tổng số điểm đến : ${provinceList.length}`}
                    </Text>
                </TouchableOpacity>
            }
            {showStage === true && provinceList.length > 0 &&
                <View style={{ margin: 8, borderRadius: 8, borderWidth: 0.6, borderColor: appcolor.primary, padding: 4 }}>

                    {
                        provinceList.map((it) => {
                            const hasAddressPoint = it.addressPoint !== null && it.addressPoint !== undefined && it.addressPoint !== ''
                            ...
                            return (
                                <View key={'ViewPoint_' + it.id} style={{ borderRadius: 8, backgroundColor: appcolor.surface, padding: 4, marginBottom: 4 }}>
                                    <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between', alignItems: "center" }}>
                                        <Text style={{ width: hasAddressPoint ? '50%' : '100%', fontWeight: '600', fontSize: 12, color: hasAddressPoint ? appcolor.dark : appcolor.tomato, paddingHorizontal: 4 }}>Điểm {it.id + 1}{hasAddressPoint ? '' : <Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.tomato }}>{': chưa có thông tin'}</Text>}</Text>
                                        {
            ...
            <View style={styles.contentView}>
                ...
                <View style={{ width: deviceWidth / 1.4 }}>
                    ...
                    <View style={styles.costView}>
                        ...
                        <CostItemView title='Di chuyển' name={tripResult.totalKM} iconName='road' />
                        <CostItemView title='Di chuyển khác' name={tripResult.supportVehicalOther} iconName='car' />
