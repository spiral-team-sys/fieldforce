...
export const ViewExistsTrip = ({
  ...
  const handleShowListTrip = async () => {
    ...
    if (dataView.isShowTrip == false) {
      if (
        dataView.dataTrips &&
        (dataView.dataTrips?.length == 0 ||
          moment(itemTrips.fromDate, 'YYYYMMDD').month() + 1 !==
          moment(dataTrips[0]?.fromDate, 'YYYYMMDD').month() + 1)
      ) {
        const listShow = dataTrips?.filter(
          it =>
            it.isCanReplace == 1 &&
            moment(it.fromDate, 'YYYYMMDD').month() + 1 ==
            moment(itemTrips.fromDate, 'YYYYMMDD').month() + 1 &&
            (itemTrips.idTrip == null || it.idTrip !== itemTrips.idTrip),
        );
  ...
  return (
    ...
    >
      ...
      >
        Thay thế chuyến đã có
      </Text>
      <TouchableOpacity
        key={'ButtonShowTrip'}
        onPress={() =>
          itemTrips.confirmReplace === 1 || itemTrips.idTrip
            ? null
            : handleShowListTrip()
        }
        style={{
          ...
          >
            <View style={{ flexDirection: 'column' }}>
              {JSON.parse(dataReplace?.provinceList || '[]')?.length > 0 && (
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  ...
                  <ScrollView
                    ...
                    nestedScrollEnabled
                  >
                    {JSON.parse(dataReplace?.provinceList || '[]').map(
                      (itemP, indexP) => {
                        return (
                          <Text
                            key={`iib_${indexP}`}
              ...
              {dataReplace?.fromDate && (
                ...
                >
                  {`Từ ${moment(dataReplace?.fromDate?.toString()).format(
                    'DD/MM/YY',
                  )} - Đến ${moment(dataReplace.toDate.toString()).format(
                    'DD/MM/YY',
                  )}`}
                </Text>
              )}
      ...
      {dataView.isShowTrip == true &&
        dataView.dataTrips?.map((it, idx) => {
          return (
            <TouchableOpacity
              key={`CreateChangeTrip_${idx}`}
              onPress={() => handlePressTrip(it)}
              style={{
                flexDirection: 'row',
                marginVertical: 4,
                marginHorizontal: 8,
              ...
              {JSON.parse(it?.provinceList || '[]')?.length > 0 && (
                <View style={{ padding: 8, width: '100%' }}>
                  <ScrollView
                    ...
                    nestedScrollEnabled
                  >
                    {JSON.parse(it?.provinceList || '[]').map(
                      (itemP, indexP) => {
                        return (
                          <Text
                            key={`iib_${indexP}`}
                  ...
                  >
                    {`Từ ${moment(it.fromDate?.toString()).format(
                      'DD/MM/YY',
                    )} - Đến ${moment(it.toDate?.toString()).format(
                      'DD/MM/YY',
                    )}`}
                  </Text>
                </View>
