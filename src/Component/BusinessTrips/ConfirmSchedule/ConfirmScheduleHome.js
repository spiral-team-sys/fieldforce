...
import { GetDataConfirmBusiness, UploadConfirmBusiness } from "../../../Controller/BussinessTripController";
...
export const ConfirmScheduleHome = ({ navigation }) => {
	...
	const LoadData = async (from, to) => {
		const fromValue = from !== undefined ? moment(from).format('YYYYMMDD') : filter.valueFromDate
		const toValue = to !== undefined ? moment(to).format('YYYYMMDD') : filter.valueToDate
		//
		await setLoading(true)
		await GetDataConfirmBusiness(fromValue, toValue, async (mData, mEmployee) => {
		...
		await setFilter({
			...filter,
			loadCalendar: false,
			fromDate: from !== undefined ? moment(from).format('DD/MM/YYYY') : filter.fromDate,
			toDate: to !== undefined ? moment(to).format('DD/MM/YYYY') : filter.toDate,
			valueFromDate: fromValue,
			valueToDate: toValue,
			fromDateCaculator: from !== undefined ? moment(from) : filter.fromDateCaculator,
			toDateCaculator: to !== undefined ? moment(to) : filter.toDateCaculator
		})
	}
	const handlerUpload = async () => {
		const dataUpload = _.filter(dataConfirm, (e) => { return e.pressConfirm == true })
		if (dataUpload !== null && dataUpload.length > 0) {
			for (let index = 0; index < dataUpload.length; index++) {
				...
				for (let p = 0; p < dataPlan.length; p++) {
					...
					if (plan.ConfirmPlan == 0 && (plan.ConfirmNote == undefined || plan.ConfirmNote == null || plan.ConfirmNote.length < 5)) {
						ToastError(`Vui lòng nhập lí do từ chối chuyến đi của nhân viên ${item.employeeName} (Tối thiểu 5 kí tự)`, 'Thông báo', 'top')
						return
					}
				}
				// Actual
				...
				for (let a = 0; a < dataActual.length; a++) {
					...
					if (actual.ConfirmPlan == 0 && (actual.ConfirmNote == undefined || actual.ConfirmNote == null || actual.ConfirmNote.length < 5)) {
						ToastError(`Vui lòng nhập lí do từ chối chuyến đi của nhân viên ${item.EmployeeName || item.employeeName} (Tối thiểu 5 kí tự)`, 'Thông báo', 'top')
						return
					}
				}
			}
			// Return
			alertConfirm("Xác nhận", "Bạn có đồng ý với các xác nhận như dưới, nếu có vui lòng xác nhận để gửi yêu cầu", async () => {
	...
	const handlerViewByEmployee = (employeeId) => {
		if (employeeId == 0) {
			...
			const dataByEmployee = _.filter(dataConfirmMain, (e) => { return e.employeeId == employeeId })
	...
	const styles = StyleSheet.create({
		...
		viewDate: { width: '100%', borderRadius: 5, alignItems: 'center', padding: 8 },
		contentView: { width: '100%', },
		titleHeader: { fontSize: 14, color: appcolor.info, fontWeight: '700', fontStyle: 'italic', padding: 8 }
	})
	const renderItem = ({ item, index }) => {
		...
		return (
			<View key={`gghh_${index}`}>
				{item.isParent && <Text style={styles.titleHeader}>{`${index + 1}. ${item.employeeCode} - ${item.employeeName}`}</Text>}
	...
	return (
		<View style={styles.mainContainer}>
			<HeaderCustom
				...
				iconMiddle='search'
				middleFunc={() => { SheetManager.show('employees') }}
