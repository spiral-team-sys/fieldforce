...
export const ActualTrip = ({ type, indexMain, data, dataPlan, isCheckData = false, handlerConfirm, handlerNote }) => {
	...
	const styles = StyleSheet.create({
		mainContainer: { width: (isCheckData == 1 ? (dataPlan?.length > 0 ? (deviceWidth / 2) : deviceWidth) : (deviceWidth / 2)), backgroundColor: appcolor.light }
	})
	const ViewItem = () => {
		switch (AppNameBuild) {
			case lgApp:
