...
export const ItemDetailViewLG = ({
  ...
  const supportKM = `Di chuyển ${item.SupportKM > 0 ? formatNumber(item.SupportKM, ',') : 0
    } VNĐ`;
  const supportVehicalOther = `Di chuyển khác ${item.SupportVehicalOther > 0
  ...
  const supportOther = `Khác ${item.SupportOther > 0 ? formatNumber(item.SupportOther || 0, ',') : 0
    } VNĐ`;
  const supportCar = `Thuê xe ${item.SupportCar > 0 ? formatNumber(item.SupportCar || 0, ',') : 0
    } VNĐ`;
  const supportWork = `Phụ cấp ${item.SupportWork > 0 ? formatNumber(item.SupportWork || 0, ',') : 0
    } VNĐ`;
  ...
  return (
    ...
    >
      <View style={styles.itemTrips}>
        <ItemView
          styleView={styles.titleContentView}
          value={`${title} - ${item.StatusConfirm} ${item.ConfirmNote || ''}`}
          iconName="stream"
        />
        ...
        {item.ListDate !== null && item.ListDate > 0 && (
          <ItemView
            styleView={styles.contentView}
            value={listDate}
            iconName="road"
        ...
        {item.SupportCar !== null && item.SupportCar > 0 && (
          <ItemView
            styleView={styles.contentView}
            value={supportCar}
            iconName="car"
        ...
        >
          <View style={{ flexDirection: 'row', end: 0 }}>
            ...
            <ActionPress
              ...
              type={ACTION_UPLOAD.APPROVED}
              colorAction={
                item.ConfirmPlan == 0 ? appcolor.red : appcolor.placeholderText
              }
              itemAction={item}
...
const ActionPress = ({ type, title, onPress, colorAction, itemAction }) => {
  ...
  return (
    <TouchableOpacity
      ...
      onPress={actionItem}
    >
      <Text style={{ fontSize: 12, fontWeight: '600', color: appcolor.light }}>
        {title}
      </Text>
    </TouchableOpacity>
  );
