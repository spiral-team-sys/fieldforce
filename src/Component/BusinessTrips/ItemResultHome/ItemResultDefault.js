...
export const ItemResultDefault = ({ item, index, handlerDeleteTrip, handlerConfirmTrip, handlerEditTrips, handlerUploadDocument, handlerReConfirmTrip, handlerCopyTrip, styles }) => {
	const { appcolor } = useSelector(state => state.GAppState)
	const [isVisible, setVisible] = useState(false)
	const [messager, setMessager] = useState('')
	const listProvinceWork = JSON.parse(item?.provinceList || '[]')
	const provincePlan = `${item.provinceFromVN}${!item.districtFromVN ? '' : `(${item.districtFromVN})`} - ${item.provinceToVN}${!item.districtTo ? '' : `(${item.districtTo})`}`
	...
	const supportLunchAndDinner = `Ăn uống ${item.supportLunch > 0 ? formatNumber(item.supportLunch || 0, ',') : 0} VNĐ`
	...
	const onPressDetailReplace = async () => {
		...
		for (let index = 0; index < tripReplaceDetail.length; index++) {
			...
			pointUI.push(
				<View key={'ViewPoint_' + element.workingScheduleId} style={{ borderRadius: 8, backgroundColor: appcolor.surface, padding: 4, marginBottom: 4 }}>
						...
						<View style={{ flexDirection: 'column' }}>
							...
							</View>
							{
								...
								<Text key={`itemTripDate`} style={{ fontWeight: '600', fontSize: 14, color: appcolor.dark, paddingBottom: 4 }}>
									{`Từ ${moment(element?.fromDate?.toString()).format('DD/MM/YY')} - Đến ${moment(element.toDate.toString()).format('DD/MM/YY')}`}
								</Text>
							}
						</View>
						{
			...
			<View style={styles.itemTrips}>
						...
						{listProvinceWork.map((itemP, indexP) => {
							return (
								<Text key={`iib_${indexP}`} style={styles.titleView}>
									{`${indexP > 0 ? ' -' : ''} ${itemP.provinceName}${!itemP.district ? '' : `(${itemP.district})`} (${itemP.numberDay})`}
								</Text>
							)
						})}
					</ScrollView>
					:
					<View style={{ width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', paddingHorizontal: 5 }}>
						<Icon type="font-awesome-5" name='plane-departure' size={14} color={appcolor.dark} style={{ width: 30, paddingTop: 5 }} />
						...
						{handlerCopyTrip &&
							<TouchableOpacity
								onPress={onCopyTrip}
								style={{ flexDirection: 'row', alignItems: 'center', alignSelf: 'center', backgroundColor: appcolor.info, borderRadius: 16, paddingVertical: 6, paddingHorizontal: 10, marginStart: 8 }}>
								<Icon name="copy" type="font-awesome-5" color={appcolor.light} size={11} />
								<Text style={{ color: appcolor.light, fontSize: 12, fontWeight: '700', marginStart: 6 }}>Copy</Text>
							</TouchableOpacity>
						}
					</View>
				...
				</Text>
				{
					...
					<TouchableOpacity
						...
						style={{ position: 'absolute', top: 8, right: 8, backgroundColor: appcolor.primary, borderRadius: 50, padding: 8, zIndex: 10000 }} >
						<Icon type="font-awesome-5" name={'exchange-alt'} size={14} color={appcolor.white} style={{ width: 30, padding: 5 }} />
					</TouchableOpacity>
				}
			</View>
			{item.isNotePlan == 1 &&
