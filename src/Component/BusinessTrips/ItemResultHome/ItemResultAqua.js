...
export const ItemResultAqua = ({ item, index, handlerDeleteTrip, handlerConfirmTrip, handlerEditTrips, handlerUploadDocument, handlerReConfirmTrip, styles }) => {
    ...
    const onPressStage = async () => {
        let pointUI = []
        const quotaData = JSON.parse(item.quotaData || '{}')
                    ...
                    <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between', alignItems: "center" }}>
                        <Text style={{ width: (element.addressPoint == null || element.addressPoint == '') ? '100%' : '50%', fontWeight: '600', fontSize: 12, color: (element.addressPoint == null || element.addressPoint == '') ? appcolor.tomato : appcolor.dark, paddingHorizontal: 4 }}>Điểm {element.id + 1}</Text>
                        {
                            ...
                            <View style={{ justifyContent: "center", padding: 4, backgroundColor: appcolor.light, borderRadius: 4 }}>
                                ...
                                <Text style={{ fontWeight: '600', fontSize: 12, color: appcolor.primary }}>Khoảng cách ~{Math.round(element.distance / 1000) + ((element.numberStore || 0) * (quotaData.kmDefault || 5))}km</Text>
                            </View>
                        }
                    </View>
                    {
                        ...
                        <View style={{ width: '100%', borderRadius: 4 }}>
                            ...
                            <View style={{ width: '100%', flexDirection: "row", paddingHorizontal: 4 }}>
                                <Text style={{ fontWeight: '700', fontSize: 12, color: appcolor.dark }}>Số lượng cửa hàng : {element.numberStore || 0}</Text>
                            </View>
                            {
                                (element.note !== undefined && element.note !== null && element.note !== '' && element.note.length > 0) &&
                                <View style={{ width: '100%', flexDirection: "row", paddingHorizontal: 4, }}>
                                    <Text style={{ fontWeight: '600', fontSize: 12, color: appcolor.dark }}>Ghi chú :{<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>{element.note}</Text>}</Text>
                                </View>
                            }
                        </View>
                    }
                    ...
                    <View >
                        ...
                        >
                            ...
                            {listProvinceWork.map((item, index) => {
                                return (
                                    <Text key={`iib_${index}`} style={styles.titleView}>
                                        {`${index > 0 ? ' -' : ''} ${item.provinceName} (${item.numberStore})`}
                                    </Text>
                                )
                            })}
                        </ScrollView>
