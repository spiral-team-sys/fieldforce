...
export const ItemResultHafele = ({
  ...
  const supportVehicalOther = `Chi phí di chuyển ${item.supportVehicalOther > 0
      ? formatNumber(item.supportVehicalOther, ',')
      : 0
    } VNĐ`;
  const supportKM = `Chi phí di chuyển theo KM ${item.supportKM > 0 ? formatNumber(item.supportKM, ',') : 0
    } VNĐ`;
  const supportLunch = `Chi phí ăn uống ${item.supportLunch > 0 ? formatNumber(item.supportLunch || 0, ',') : 0
    } VNĐ`;
  ...
  const supportWork = `Chi phí tiếp khách ${item.supportWork > 0 ? formatNumber(item.supportWork || 0, ',') : 0
    } VNĐ`;
  const supportCar = `Chi phí di chuyển ${item.supportCar > 0 ? formatNumber(item.supportCar || 0, ',') : 0
    } VNĐ`;
  ...
  const onPressStage = async () => {
    ...
    for (let index = 0; index < listProvinceWork.length; index++) {
      ...
      pointUI.push(
              ...
              >
                ...
                >
                  Chi phí :{' '}
                  {
                    ...
                    >
                      Khách sạn :{' '}
                      {formatNumber(element.nightRestValue, ',') || 0} |{' '}
                    </Text>
                  }
                  {
                    <Text
                    ...
                    >
                      Ăn uống : {element.foodCostPoint || 0} |{' '}
                    </Text>
                  }
                  {
                    <Text
                    ...
                    >
                      Di chuyển : {element.vehicalValue || 0}
                    </Text>
                  }
                </Text>
              </View>
        ...
        // <View key={'ViewStages_' + element.id} style={{}}>
        //     <View style={{ flexDirection: 'row' }}>
        //         <Text style={{ fontWeight: '600', fontSize: 12, color: appcolor.dark, padding: 4, width: '80%' }}>Chặng thứ : {element.id + 1}</Text>
        //         {
        //             element.distance > 0 &&
        //             <View style={{ width: '20%', justifyContent: "center", padding: 4, backgroundColor: appcolor.light, borderRadius: 4 }}>{element.distance}</View>
        //         }
        //     </View>
        //     <TouchableOpacity
        //         // onPress={() => handleSelectItem(element, index)}
        //         key={'itemStages_' + element.id} style={{ padding: 4, borderRadius: 4, margin: 2, backgroundColor: appcolor.surface, flexDirection: 'row' }}>

        //         <View style={{ width: '100%', borderRadius: 4 }}>
        //             <View style={{ width: '100%', flexDirection: "row", paddingBottom: 4 }}>
        //                 <Text style={{ fontWeight: '600', fontSize: 12, color: element.startPosition !== null ? appcolor.dark : appcolor.tomato }}>{element.startPosition !== null ? 'Điểm bắt đầu : ' : 'Chưa có điểm bắt đầu'}{<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>{element.startPosition}</Text>}</Text>
        //             </View>
        //             <View style={{ width: '100%', flexDirection: "row", paddingBottom: 4 }}>
        //                 <Text style={{ fontWeight: '600', fontSize: 12, color: element.endPosition !== null ? appcolor.dark : appcolor.tomato }}>{element.endPosition !== null ? 'Điểm kết thúc : ' : 'Chưa có điểm kết thúc'}{<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>{element.endPosition}</Text>}</Text>
        //             </View>
        //             <View style={{ width: '100%', flexDirection: "row" }}>
        //                 <Text style={{ fontWeight: '600', fontSize: 12, color: appcolor.dark }}>Thông tin lưu trú : {<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>Ngày ăn : {element.eatDay || 0} | </Text>}{<Text style={{ fontWeight: '400', fontSize: 12, color: appcolor.dark }}>Lưu trú : {element.numberDay || 0}</Text>}</Text>
        //             </View>
        //         </View>
        //     </TouchableOpacity>
        // </View>
    ...
    >
      <View style={styles.itemTrips}>
        ...
        <ItemView
          ...
          iconName="hotel"
        />
        {item.supportWork !== null && item.supportWork > 0 && (
          <ItemView
            styleView={styles.contentView}
            value={supportWork}
            iconName="money-bill"
