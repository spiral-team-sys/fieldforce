...
export const ItemResultHafele = ({ item, index, handlerDeleteTrip, handlerConfirmTrip, handlerEditTrips, handlerUploadDocument, handlerReConfirmTrip, styles }) => {
    ...
    const supportVehicalOther = `Chi phí di chuyển ${item.supportVehicalOther > 0 ? formatNumber(item.supportVehicalOther, ',') : 0} VNĐ`
    const supportKM = `Chi phí di chuyển theo KM ${item.supportKM > 0 ? formatNumber(item.supportKM, ',') : 0} VNĐ`
    const supportLunch = `Chi phí ăn uống ${item.supportLunch > 0 ? formatNumber(item.supportLunch || 0, ',') : 0} VNĐ`
    const supportNight = `Chi phí khách sạn ${item.supportNight > 0 ? formatNumber(item.supportNight, ',') : 0} VNĐ`
    const supportOther = `Chi phí khác ${item.supportOther > 0 ? formatNumber(item.supportOther || 0, ',') : 0} VNĐ`
    const supportWork = `Chi phí tiếp khách ${item.supportWork > 0 ? formatNumber(item.supportWork || 0, ',') : 0} VNĐ`
