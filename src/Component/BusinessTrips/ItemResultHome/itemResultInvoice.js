...
export const ItemResultInvoice = ({
  item,
  index,
  ItemView,
  filterInvoice,
  ...
  const supportFood = `Chi phí ăn uống ${
    item.foodCosts > 0 ? formatNumber(item.foodCosts, ',') : 0
  } VNĐ`;
  const supportHotel = `Chi phí khách sạn ${
    item.hotelCosts > 0 ? formatNumber(item.hotelCosts, ',') : 0
  } VNĐ`;
  const supportKM =
    AppNameBuild == hpiApp
      ? `Chi phí di chuyển máy bay ${
          item.supportKMCosts > 0 ? formatNumber(item.supportKMCosts, ',') : 0
        } VNĐ`
      : `Chi phí di chuyển ${AppNameBuild == lgApp ? '' : 'Km '}${
          item.supportKMCosts > 0 ? formatNumber(item.supportKMCosts, ',') : 0
        } VNĐ`;
  const supportKMCosts =
    AppNameBuild == hisenApp ||
    AppNameBuild == toshibaApp ||
    AppNameBuild == sharpApp ||
    AppNameBuild == hpiApp
      ? `Chi phí di chuyển ${
          item.moveCosts > 0 ? formatNumber(item.moveCosts, ',') : 0
        } VNĐ`
      : AppNameBuild == tefalApp
      ? `Chi phí thuê xe ${
          item.moveCosts > 0 ? formatNumber(item.moveCosts, ',') : 0
        } VNĐ`
  ...
  const supportOther = `Chi phí khác ${
    item.otherCosts > 0 ? formatNumber(item.otherCosts || 0, ',') : 0
  } VNĐ`;
  const supportWork = `${
    AppNameBuild == lgApp ? `Chi phí phụ cấp ` : 'Chi phí tiếp khách '
  }${item.workCosts > 0 ? formatNumber(item.workCosts || 0, ',') : 0} VNĐ`;
  const provincePlan = `${item.provinceFrom}${
    !item.districtFrom ? '' : `(${item.districtFrom})`
  } - ${item.provinceTo}${!item.districtTo ? '' : `(${item.districtTo})`}`;
  const listProvinceWork = JSON.parse(item.provinceList) || [];
  const totalSupport = `Tổng chi phí ${
    item.totalCosts > 0 ? formatNumber(item.totalCosts || 0, ',') : 0
  } VNĐ`;
  ...
  const onSubmitItem = async () => {
    ...
    const dataDetail = await GetDataTripBillsDetail(item.detailId);
    const jsonDetail = dataDetail[0];
    const fileFoodCosts = JSON.parse(
      JSON.parse(jsonDetail.foodCosts || '[]')[0].billFile || '[]',
    ).length;
    const fileHotelCosts = JSON.parse(
      JSON.parse(jsonDetail.hotelCosts || '[]')[0].billFile || '[]',
    ).length;
    const fileMoveCosts = JSON.parse(
      JSON.parse(jsonDetail.moveCosts || '[]')[0].billFile || '[]',
    ).length;
    const fileOtherCosts = JSON.parse(
      JSON.parse(jsonDetail.otherCosts || '[]')[0].billFile || '[]',
    ).length;
    const fileSupportKMCosts = JSON.parse(
      JSON.parse(jsonDetail.supportKMCosts || '[]')[0].billFile || '[]',
    ).length;
    if (
      fileFoodCosts == 0 &&
      fileHotelCosts == 0 &&
      fileMoveCosts == 0 &&
      fileOtherCosts == 0 &&
      fileSupportKMCosts == 0
    ) {
      MessageAction(
        'Chuyến đi chưa được đính kèm hóa đơn bạn có muốn tiếp tục gửi?',
        () => uploadAction(dataUpload, 'submit', 'Gửi xác nhận'),
      );
      return;
    }
    MessageAction(
      'Bạn có chắc chắn muốn gửi xác nhận là đã gửi hết chứng từ?',
      () => uploadAction(dataUpload, 'submit', 'Gửi xác nhận'),
    );
  };
  ...
  const onCancelItem = () => {
    ...
    MessageAction('Bạn chắc chắn muốn bỏ quyết toán?', () =>
      uploadAction(dataUpload, 'cancel', 'Bỏ quyết toánn'),
    );
  };
              ...
              >
                ...
                >
                  Thông tin lưu trú :{' '}
                  {
                    ...
                    >
                      Đêm lưu trú : {element.numberDay || 0} |{' '}
                    </Text>
                  }
                  {
                    <Text
      ...
      <View style={styles.itemTrips}>
        <View>
          <ItemView
            ...
            value={`Từ ${item.fromDate} Đến ${item.toDate}`}
          ...
          {item.isUsePoint == 1 && listProvinceWork.length > 0 ? (
            <View>
              ...
              >
                <TouchableOpacity
                  onPress={() =>
                    AppNameBuild == lgApp ? onPressStage() : onPressPoint()
                  }
                  style={{
                    padding: 4,
                    backgroundColor: appcolor.light,
              ...
              <ScrollView
                ...
                nestedScrollEnabled
              >
                {item.provinceFrom !== null &&
                  item.provinceFrom !== undefined && (
          ...
          <ItemView
            ...
            iconName="hotel"
          />
          {item.workCosts !== 0 && item.workCosts && (
            <ItemView
              styleView={styles.contentView}
              value={supportWork}
              iconName="road"
          ...
          <ItemView
            ...
            iconName="money-bill"
          />
          {item.confirmNote?.length > 0 && (
            <ItemView
              styleView={{ ...styles.contentView, width: '90%' }}
              value={confirmNote}
...
const CountFile = memo(({ item }) => {
  ...
  const loadDataDetail = async () => {
    ...
    const fileWorkCosts = JSON.parse(
      JSON.parse(jsonDetail.workCosts || '[]')[0]?.billFile || '[]',
    ).length;
    setCountFile(
      fileFoodCosts +
        fileHotelCosts +
