...
export const ItemResultHPI = ({ item, index, handlerDeleteTrip, handlerConfirmTrip, handlerEditTrips, handlerUploadDocument, handlerReConfirmTrip, styles }) => {
    ...
    const supportVehicalOther = `Chi phí di chuyển máy bay ${item.supportVehicalOther > 0 ? formatNumber(item.supportVehicalOther, ',') : 0} VNĐ`
    const supportNight = `Chi phí khách sạn ${item.supportNight > 0 ? formatNumber(item.supportNight, ',') : 0} VNĐ`
    const supportLunch = `Ăn có uống ${item.supportLunch > 0 ? formatNumber(item.supportLunch || 0, ',') : 0} VNĐ`
    const supportDinner = `Ăn không chứng từ ${item.supportDinner > 0 ? formatNumber(item.supportDinner || 0, ',') : 0} VNĐ`
