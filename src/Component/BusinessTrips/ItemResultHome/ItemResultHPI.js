...
export const ItemResultHPI = ({
  ...
  const supportVehicalOther = `Chi phí di chuyển máy bay ${item.supportVehicalOther > 0
  ...
  const supportLunch = `Ăn có uống ${item.supportLunch > 0 ? formatNumber(item.supportLunch || 0, ',') : 0
    } VNĐ`;
  const supportDinner = `Ăn không chứng từ ${item.supportDinner > 0 ? formatNumber(item.supportDinner || 0, ',') : 0
    } VNĐ`;
