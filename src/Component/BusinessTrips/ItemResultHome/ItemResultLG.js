...
export const ItemResultLG = ({
  ...
  const supportWork = `Phụ cấp công tác ${item.supportWork > 0 ? formatNumber(item.supportWork || 0, ',') : 0
    } VNĐ`;
    ...
    >
      <View style={styles.itemTrips}>
        ...
        <ItemView
          styleView={[
            { ...styles.contentView },
            { color: appcolor.dark, fontWeight: fontWeightBold },
          ]}
          value={datePlan}
        ...
        {item.confirmBy !== null && (
          <ItemView
            styleView={[
              styles.contentView,
              { color: item.colorConfirm || appcolor.greylight },
            ]}
            iconColor={item.colorConfirm}
            value={item.confirmBy}
            iconName="check"
          />
        )}
...
const ItemView = ({ value, iconName, styleView, iconColor = null }) => {
  const appcolor = useSelector(state => state.GAppState);
  return (
