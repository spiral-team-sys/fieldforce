...
export const ItemResultLG = ({ item, index, handlerDeleteTrip, handlerConfirmTrip, handlerEditTrips, handlerUploadDocument, handlerReConfirmTrip, styles }) => {
    ...
    const supportWork = `Phụ cấp công tác ${item.supportWork > 0 ? formatNumber(item.supportWork || 0, ',') : 0} VNĐ`
    ...
    return (
        <View key={`int_${index}`} style={{ width: '100%', paddingStart: 5, paddingEnd: 5, alignSelf: 'center' }}>
            <View style={styles.itemTrips}>
                ...
                <ItemView styleView={[{ ...styles.contentView }, { color: appcolor.dark, fontWeight: fontWeightBold }]} value={datePlan} iconName='calendar-alt' />
                ...
                {item.supportWork !== null && item.supportWork > 0 && <ItemView styleView={styles.contentView} value={supportWork} iconName='utensils' />}
                ...
                {item.confirmBy !== null && <ItemView styleView={[styles.contentView, { color: item.colorConfirm || appcolor.greylight }]} iconColor={item.colorConfirm} value={item.confirmBy} iconName='check' />}
...
const ItemView = ({ value, iconName, styleView, iconColor = null }) => {
    ...
    return (
        <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center' }}>
            <Icon type="font-awesome-5" name={iconName} size={14} color={iconColor || appcolor.dark} style={{ width: 30, padding: 5 }} />
