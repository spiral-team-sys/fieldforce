...
export const CostHafele = ({ onBack, onNext, quotaData }) => {
  ...
  const countByProvince = () => {
    ...
    for (
      ...
      countNightValue =
        countNightValue + item.numberDay * (quotaData.hotelSup || 250000);
    }
    setItemCost({
      ...itemCost,
      // typeKM
  ...
  const countSupportKm = valueInput => {
    ...
    const itemCostSupport = _.filter(
      listCostKM,
      it => valueInput < it.maxKm && valueInput >= it.minKm,
    );
    itemCost.supportKM =
      itemCost.nightRestValue > 0
        ? itemCostSupport[0]?.costOvernight || 0
        : itemCostSupport[0]?.costNonOvernight || 0;
  };
