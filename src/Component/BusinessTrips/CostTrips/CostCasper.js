...
export const CostCasper = ({ onBack, onNext }) => {
    ...
    const handlerItemChangeText = async (text, typeItem) => {
        ...
        const isInProvince = tripResult.provinceCodeFrom == tripResult.provinceCodeTo

        let itemUpdate = {}
        switch (typeItem) {
            case TYPE.TYPE_KM:
                itemUpdate = { ...itemCost, supportKM: valueInput, vehicalOtherValue: isInProvince ? 0 : itemCost.vehicalOtherValue }
                break
            case TYPE.TYPE_VEHICAL_OTHER:
                itemUpdate = { ...itemCost, vehicalOtherValue: valueInput, supportKM: isInProvince ? 0 : itemCost.supportKM }
                break
        ...
        <View style={styles.mainContainer}>
            ...
            <ScrollView
                ...
                <KeyboardAwareScrollView
                    ...
                    {((tripResult.provinceCodeFrom == tripResult.provinceCodeTo && itemCost.typeKM == 'Từ 100km') || (tripResult.provinceCodeFrom !== tripResult.provinceCodeTo)) &&
                        <ItemInput
                            checkMaxValue
                            ...
                            placeholder={`Tổng tiền khách sạn - Hạn mức: ${valueItem(itemCost.priceForPeople)} VNĐ/Đêm`}
                            iconName='hotel'
...
const ItemInput = ({
    titleName, iconName, isRequire, onActionRight, typeFilter, itemValue, placeholder, onChangeText, keyboardType = 'default',
    masterData = [], masterChoose, masterValue = null, checkMaxValue = false, maxValue = 0
}) => {
    ...
    const handlerChangeValue = (text) => {
        ...
        if (checkMaxValue) {
            if (textInput > 0 && maxValue > 0 && textInput > maxValue) {
                setIsError({ value: true, title: `Bạn nhập quá hạn mức cho phép: ${formatNumber(maxValue, ',')} VNĐ` })
                return
            }
        }
    ...
    return (
        <View style={styles.mainItem}>
            ...
            <Text style={{ ...styles.placeholderHeader, color: isError.value ? appcolor.red : appcolor.placeholderText }}>{`${isError.value ? isError.title : placeholder} `}</Text>
            {masterData.length > 0 &&
                <MutipleItemSelected
...
const PlanBusiness = ({ dataPlan, iconName, titleName, isRequire, isLoading, changeItemPlan }) => {
    const { appcolor } = useSelector(state => state.GAppState)
    const styles = StyleSheet.create({
        mainContainer: { padding: 8, marginBottom: 1, zIndex: 10 },
        titleHeader: { width: '100%', fontSize: 13, fontWeight: '600', color: appcolor.blacklight, marginStart: 8 },
        titleDate: { color: appcolor.highlightDate, fontSize: 14, fontWeight: '600', padding: 8, width: '75%' },
    ...
    const renderItem = (item, index) => {
        ...
        return (
            <View key={`sl_item_${index}`} style={{ width: '100%' }}>
                ...
                {
                    lstShop.length > 0 && lstShop.map((it, idx) => {
                        return (
                            <View key={`idx_${idx}`} style={{ backgroundColor: appcolor.surface, marginBottom: 3, borderRadius: 5, padding: 5, width: '100%' }}>
                                <Text style={styles.titleShop}>{`Cửa hàng: ${it.shopName}`}</Text>
                            </View>)
                    })
                }
