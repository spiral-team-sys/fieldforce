...
export const CostAqua = ({
  ...
  const countByProvince = () => {
    ...
    for (
      ...
      totalKm =
        totalKm +
        item.distance +
        (item.numberStore || 0) * (quotaData.kmDefault || 5) * 1000;
    }
    for (
      let index = 0;
      index < (tripResult.typeVehicle?.length || 0);
      index++
    ) {
    ...
    if (totalKm > quotaData.kmLimit2) {
      ...
      countVehicalValue =
        ((totalKm > tripResult.dayValue
          ? tripResult.dayValue
          : numKmValueActual) || 0) * quotaData.costKmLimit;
      // Tiền khách sạn tính theo đêm đi công các
      countNightValue =
        ((numKmValueActual > tripResult.nightValue
          ? tripResult.nightValue
          : numKmValueActual) || 0) * quotaData.hotelSup;
    ...
    setItemCost({
      ...itemCost,
      nightRestValue:
        itemCost.nightRestValue > 0 && itemCost.nightRestValue > countNightValue
          ? itemCost.nightRestValue
          : countNightValue,
      lunchValue:
        itemCost.lunchValue > 0 && itemCost.lunchValue > countFoodValue
          ? itemCost.lunchValue
          : countFoodValue,
      vehicalValue:
        itemCost.vehicalValue > 0 && itemCost.vehicalValue > countVehicalValue
          ? itemCost.vehicalValue
          : countVehicalValue,
      kmValue: totalKm,
  ...
  const checkInputData = actionDone => {
    ...
    if (
      itemCost.maxLunchValue > 0 &&
      itemCost.lunchValue > itemCost.maxLunchValue &&
      quotaData.isHigherAllowed != 1
    ) {
      strValid += `Chi phí ăn uống phải nhỏ hơn hoặc bằng định mức hỗ trợ ${itemCost.maxLunchValue},`;
      checkCost = true;
    }
    if (
      itemCost.maxNightValue > 0 &&
      itemCost.nightRestValue > itemCost.maxNightValue &&
      quotaData.isHigherAllowed != 1
    ) {
    ...
    if (
      itemCost.maxVehicalCost > 0 &&
      itemCost.vehicalValue > itemCost.maxVehicalCost &&
      quotaData.isHigherAllowed != 1
    ) {
    ...
    if (
      itemCost.maxVehicalOther > 0 &&
      itemCost.supportVehicalOther > itemCost.maxVehicalOther &&
      quotaData.isHigherAllowed != 1
    ) {
