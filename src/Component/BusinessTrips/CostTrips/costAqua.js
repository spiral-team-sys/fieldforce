...
export const CostAqua = ({ onBack, onNext, limitCost, listProvinceCentral, quotaData }) => {
    ...
    const [itemCost, setItemCost] = useState({
        ...
        isCentralCity: false,
        dayValue: tripResult.dayValue,
        nightValue: tripResult.nightValue
    })
    ...
    const countByProvince = () => {
        ...
        let totalKm = 0
        let numKmValueActual = 0
        let maxCostVehicle = 0

        for (let index = 0; index < (tripResult.provinceList?.length || 0); index++) {
            const item = tripResult.provinceList[index]
            totalKm = totalKm + item.distance + (((item.numberStore || 0) * (quotaData.kmDefault || 5)) * 1000)
        }
        for (let index = 0; index < (tripResult.typeVehicle?.length || 0); index++) {
            const itemVehicle = tripResult.typeVehicle[index]
            maxCostVehicle = maxCostVehicle + itemVehicle.maxCostSupport || 0
        }

        if (totalKm > quotaData.kmLimit2) {
            numKmValueActual = Math.floor((totalKm / quotaData.kmLimit2) || 1)
            // Tiền công tác tính theo ngày đi công các
            countVehicalValue = ((totalKm > tripResult.dayValue ? tripResult.dayValue : numKmValueActual) || 0) * quotaData.costKmLimit
            // Tiền khách sạn tính theo đêm đi công các
            countNightValue = ((numKmValueActual > tripResult.nightValue ? tripResult.nightValue : numKmValueActual) || 0) * quotaData.hotelSup
            // Tiền ăn uống tính theo đêm đi công các
            countFoodValue = ((tripResult.dayValue || 0) * quotaData.foodSup || 100000)

        } else {
            numKmValueActual = Math.floor((totalKm - quotaData.kmLimit1) / 1000 || 0)
            countVehicalValue = numKmValueActual * (quotaData.kmSup || 1200)
            countFoodValue = (quotaData.foodSup || 100000)
        }

        setItemCost({
            ...itemCost,
            nightRestValue: itemCost.nightRestValue > 0 && itemCost.nightRestValue > countNightValue ? itemCost.nightRestValue : countNightValue,
            lunchValue: itemCost.lunchValue > 0 && itemCost.lunchValue > countFoodValue ? itemCost.lunchValue : countFoodValue,
            vehicalValue: itemCost.vehicalValue > 0 && itemCost.vehicalValue > countVehicalValue ? itemCost.vehicalValue : countVehicalValue,
            kmValue: totalKm,
            maxNightValue: countNightValue,
            maxLunchValue: countFoodValue,
            maxVehicalCost: countVehicalValue,
            maxVehicalOther: Math.round(maxCostVehicle) || 0
        })
    }
    ...
    const handlerItemChangeText = async (text, typeItem) => {
        ...
        switch (typeItem) {
            ...
            case TYPE.TYPE_VEHICAL_OTHER:
                ...
                tripResult.supportVehicalOther = valueInput
                break
    ...
    const checkInputData = (actionDone) => {
        ...
        if (itemCost.maxLunchValue > 0 && itemCost.lunchValue > itemCost.maxLunchValue && quotaData.isHigherAllowed != 1) {
            strValid += `Chi phí ăn uống phải nhỏ hơn hoặc bằng định mức hỗ trợ ${itemCost.maxLunchValue},`
            checkCost = true
        }
        if (itemCost.maxNightValue > 0 && itemCost.nightRestValue > itemCost.maxNightValue && quotaData.isHigherAllowed != 1) {
            strValid += `Chi phí khách sạn phải nhỏ hơn hoặc bằng định mức hỗ trợ ${itemCost.maxNightValue},`
            checkCost = true
        }
        if (itemCost.maxVehicalCost > 0 && itemCost.vehicalValue > itemCost.maxVehicalCost && quotaData.isHigherAllowed != 1) {
            strValid += `Chi phí di chuyển phải nhỏ hơn hoặc bằng định mức hỗ trợ ${itemCost.maxVehicalCost}`
            checkCost = true
        }
        if (itemCost.maxVehicalOther > 0 && itemCost.supportVehicalOther > itemCost.maxVehicalOther && quotaData.isHigherAllowed != 1) {
            strValid += `Chi phí di chuyển phải nhỏ hơn hoặc bằng định mức hỗ trợ ${itemCost.maxVehicalOther}`
            checkCost = true
        }
            ...
            <ScrollView
                ...
                <KeyboardAwareScrollView
                    ...
                    <ItemInput
                        ...
                        editable={quotaData.isEditHotelCost == 1 || false}
                    />
                    <ItemInput
                        typeFilter={TYPE.TYPE_LUNCH}
                        keyboardType="numeric"
                        isRequire
                        editable={quotaData.isEditLunchCost == 1 || false}
                    ...
                    <ItemInput
                        ...
                        onChangeText={handlerItemChangeText}
                        editable={quotaData.isEditMoveCost == 1 || false}
                    />
                    <ItemInput
