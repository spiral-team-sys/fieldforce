...
export const CostBosch = ({ onBack, onNext, quotaData }) => {
    ...
    const countByProvince = () => {
        ...
        if (quotaData.typeCountCost == 2) {
            ...
            // tiền khách sạn
            countNightValue = (((tripResult.provinceList?.length || 0) >= quotaData.limitPointSupHotel)
                || (sumKmValue >= (quotaData.supHotelByKm || 100))) ? countNightDay * quotaData.costHotelSup : 0

            // tiền ăn
            if (sumKmValue >= quotaData.kmFoodLimit1 && sumKmValue < quotaData.kmFoodLimit2) {
