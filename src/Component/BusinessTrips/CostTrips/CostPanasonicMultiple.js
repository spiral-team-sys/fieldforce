...
export const CostPanasonicMultiple = ({ onBack, onNext }) => {
    ...
    const [itemCost, setItemCost] = useState({
        ...
        supportKM: tripResult.supportKM || (tripResult.dayValue * 250000),
        vehicalValue: tripResult.supportVehical || 0,
        nightRestValue: tripResult.supportNight || 0,
        lunchValue: tripResult.supportLunch || tripResult.dayValue * 120000,
        dinnerValue: tripResult.supportDinner || tripResult.dayValue * 120000,
    ...
    const checkInputData = (actionDone) => {
        ...
        if (!isValid(itemCost.nightRestValue))
            strValid += 'Chi phí nghỉ qua đêm, '
        if (!isValid(itemCost.lunchValue))
            strValid += 'Chi phí ăn trưa, '
        if (!isValid(itemCost.dinnerValue))
            strValid += 'Chi phí ăn tối '
    ...
    const tripRanges = Array.isArray(tripResult.tripDateRanges) ? tripResult.tripDateRanges : []
                ...
                <KeyboardAwareScrollView
                    ...
                    {tripRanges.length > 0 &&
                        <View style={{ paddingHorizontal: 8, paddingTop: 8, paddingBottom: 4 }}>
                            <Text style={{ fontSize: 13, fontWeight: '700', color: appcolor.dark, marginBottom: 8 }}>Các lịch công tác đã chọn</Text>
                            {tripRanges.map((r, idx) => (
                                <View key={`range_${idx}`} style={{ backgroundColor: appcolor.surface, borderRadius: 10, padding: 10, marginBottom: 8, borderWidth: 0.5, borderColor: appcolor.lightgrey }}>
                                    <Text style={{ fontSize: 13, fontWeight: '700', color: appcolor.info, marginBottom: 4 }}>{`Lịch ${idx + 1}`}</Text>
                                    <Text style={{ fontSize: 13, color: appcolor.dark }}>{`${r.dateFilterFrom} → ${r.dateFilterTo}`}</Text>
                                    <Text style={{ fontSize: 12, color: appcolor.greylight }}>{r.dayAndNight}</Text>
                                </View>
                            ))}
                            <Text style={{ fontSize: 11, color: appcolor.greylight, textAlign: 'center', marginBottom: 8 }}>Chi phí bên dưới sẽ áp dụng cho tất cả các lịch.</Text>
                        </View>
                    }
                    {/* <ItemInput
                            ...
                            placeholder={`Tổng tiền di chuyển (${tripResult.dayValue} ngày - Đơn giá: 250,000 VNĐ)`}
                            iconName='road'
                            itemValue={valueItem(itemCost.supportKM)}
                            onChangeText={handlerItemChangeText}
                        /> */}
                    <ItemInput
                        typeFilter={TYPE.TYPE_DAYS_MOVE}
                        keyboardType="numeric"
                        isRequire
                        titleName={`Số ngày di chuyển`}
                        placeholder={`Số ngày di chuyển`}
                        iconName='road'
                        itemValue={valueItem(itemCost.daysMove)}
                        onChangeText={handlerItemChangeText}
                    />
                    <ItemInput
                    ...
                    />
                    <ItemInput
                        typeFilter={TYPE.TYPE_NUMBER_DAY}
                        keyboardType="numeric"
                        isRequire
                        titleName='Chi phí lưu trú'
                        placeholder={`Tổng lưu trú (${itemCost.totalNumberDay} đêm - Đơn giá: 400,000 VNĐ)`}
                        iconName='hotel'
                        itemValue={valueItem(itemCost.stayValue)}
                        onChangeText={handlerItemChangeText}
                    />
                    <ItemInput
                        ...
                        placeholder={`Tổng tiền ăn trưa (${tripResult.dayValue} ngày - Đơn giá: 120,000 VNĐ)`}
                        iconName='utensils'
                    ...
                    <ItemInput
                        ...
                        placeholder={`Tổng tiền ăn tối (${tripResult.dayValue} ngày - Đơn giá: 120,000 VNĐ)`}
                        iconName='utensils'
