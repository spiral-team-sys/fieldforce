...
export const CostGSV = ({ onBack, onNext, quotaData }) => {
  ...
  const handlerTripResult = async () => {
    await checkInputData(async (isSuccess, message) => {
      if (isSuccess) {
        ...
        if (
          costResult.supportLunch > 0 &&
          costResult.supportLunch < (quotaData.minFoodCost || 100000)
        ) {
          alertWarning(
            `Chi phí ăn uống phải lớn hơn ${quotaData.minFoodCost || 100000}`,
          );
          return;
        } else if (
          costResult.supportLunch > (quotaData.maxFoodCost || 100000000)
        ) {
          alertWarning(
            `Chi phí ăn uống phải nhỏ hơn ${quotaData.maxFoodCost || 100000000
            }`,
          );
          return;
        } else if (
          costResult.supportNight > 0 &&
          costResult.supportNight < (quotaData.minNightCost || 100000)
        ) {
          alertWarning(
            `Chi phí khách sạn phải lớn hơn ${quotaData.minFoodCost || 100000}`,
          );
          return;
        } else if (
          costResult.supportNight > (quotaData.maxNightCost || 100000000)
        ) {
          alertWarning(
            `Chi phí khách sạn phải nhỏ hơn ${quotaData.maxNightCost || 100000000
            }`,
          );
          return;
        } else if (
          costResult.supportKM > 0 &&
          costResult.supportKM < (quotaData.minMoveCost || 100000)
        ) {
          alertWarning(
            `Chi phí di chuyển phải lớn hơn ${quotaData.minMoveCost || 100000}`,
          );
          return;
        } else if (
          costResult.supportKM > (quotaData.maxMoveCost || 100000000)
        ) {
          alertWarning(
            `Chi phí di chuyển phải nhỏ hơn ${quotaData.maxMoveCost || 100000000
            }`,
          );
          return;
        } else if (
          costResult.supportVehical > 0 &&
          costResult.supportVehical < (quotaData.minCarCost || 100000)
        ) {
          alertWarning(
            `Chi phí thuê xe phải lớn hơn ${quotaData.minCarCost || 100000}`,
          );
          return;
        } else if (
          costResult.supportVehical > (quotaData.maxCarCost || 100000000)
        ) {
          alertWarning(
            `Chi phí thuê xe phải nhỏ hơn ${quotaData.maxCarCost || 100000000}`,
          );
          return;
        }
  ...
  const checkInputData = actionDone => {
    ...
    if (!isValid(itemCost.vehicalValue)) strValid += 'Chi phí thuê xe, ';
    actionDone(
      strValid.length == 0,
