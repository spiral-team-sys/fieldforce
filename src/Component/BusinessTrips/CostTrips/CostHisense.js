...
export const CostHisense = ({ onBack, onNext, limitCost, listProvinceCentral, quotaData }) => {
    ...
    const countByProvince = () => {
        ...
        for (let index = 0; index < (tripResult.provinceList?.length || 0); index++) {
            ...
            const checkCentral = listProvinceCentral.filter(it => it.provinceCode == item.provinceCode)
            if (checkCentral?.length > 0) {
                countNightValue = countNightValue + (item.numberDay * (quotaData.centralNightSup || 500000))
                countFoodValue = countFoodValue + (item.eatDay * (quotaData.centralFoodSup || 100000))
            } else {
                countNightValue = countNightValue + (item.numberDay * (quotaData.otherNightSup || 350000))
                countFoodValue = countFoodValue + (item.eatDay * (quotaData.otherFoodSup || 75000))
            }
        }
        setItemCost({
            ...itemCost,
            nightRestValue: tripResult.supportNight == undefined ? countNightValue : tripResult.supportNight,
            lunchValue: tripResult.supportLunch == undefined ? countFoodValue : tripResult.supportLunch,
