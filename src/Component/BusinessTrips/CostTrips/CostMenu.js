...
export const CostMenu = ({ onBack, onNext, dateFilter }) => {
  ...
  const listProvinceCentral =
    JSON.parse(dateFilter?.listProvinceCentral || '[]') || [];
  const quotaData = JSON.parse(dateFilter?.quotaData || '{}') || {};
  const limitCostSupport =
    JSON.parse(dateFilter?.limitCostSupport || '[]') || '[]';
  const typeArrow = {
    titleHeader: 'Chi phí',
    typeBack: 'arrow-back',
  ...
  const menuItem = () => {
    switch (AppNameBuild) {
      ...
      case casperApp:
        return (
          <CostCasper typeArrow={typeArrow} onBack={onBack} onNext={onNext} />
        );
      case viessmannApp:
  ...
  return <View style={styles.mainContainer}>{menuItem()}</View>;
};
