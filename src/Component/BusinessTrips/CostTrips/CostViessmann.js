...
export const CostViessmann = ({ onBack, onNext, limitCost }) => {
  ...
  const [itemCost, setItemCost] = useState({
    ...
    supportKM:
      limitCost?.isSupportKM > 0 && tripResult.supportKM == undefined
        ? 400000
        : tripResult.supportKM || 0,
    vehicalValue:
      limitCost?.isSupportCar > 0 && tripResult.supportVehical == undefined
        ? (limitCost?.isSupportCar || tripResult.dayValue) * 250000
        : tripResult.supportVehical || 0,
    nightRestValue:
      limitCost?.isSupportNight > 0 && tripResult.supportNight == undefined
        ? (limitCost?.isSupportNight || tripResult.nightValue) * 300000
        : tripResult.supportNight || 0,
  ...
  const handlerTripResult = async () => {
    await checkInputData(async (isSuccess, message) => {
      if (isSuccess) {
        ...
        //
        const itemTripResult = await _.merge(
          tripResult,
          costResult,
          limitCostResult,
        );
      ...
      >
        ...
        >
          ...
          <ItemInput
            ...
            placeholder={
              limitCost?.isSupportNight > 0
                ? `Tổng tiền khách sạn / nhà nghỉ (Đơn giá: 300,000 VNĐ/Đêm)\nHạn mức còn lại ${limitCost?.isSupportNight} Đêm`
                : `Đã hết hạn mức đăng kí`
            }
            iconName="hotel"
          ...
          <ItemInput
            ...
            placeholder={
              limitCost?.isSupportCar > 0
                ? `Chi phí thuê xe (Đơn giá: 250,000 VNĐ/Ngày)\nHạn mức còn lại ${limitCost?.isSupportCar} Ngày`
                : `Đã hết hạn mức đăng kí`
            }
            iconName="car"
