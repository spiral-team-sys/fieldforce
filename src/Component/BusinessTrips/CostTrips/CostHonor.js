...
export const CostHonor = ({
  onBack,
  onNext,
  quotaData,
  limitCostSupport,
  limitCostSupportTotal,
  dateFilter,
}) => {
  ...
  const listTypeCost =
    _.find(tripResult.listTypeCost, it => it.isChoose == 1) || {};
  ...
  const countByProvince = () => {
    ...
    const listChoose = _.filter(
      tripResult.listTypeCost,
      it =>
        it.isChoose == 1 &&
        (it.code == 'onlyWorkCosts' || it.code == 'onlyOtherCosts'),
    );
    if (listChoose.length == 0) {
      if (
        tripResult.provinceList != null &&
        ...
        for (
          ...
          countVehicalValueOther =
            countVehicalValueOther + (item.vehicalValue || 0);
        ...
        countNightValue =
          tripResult.nightValue * (quotaData.hotelSup || 360000);
      ...
      const result = distanceCheck.find(
        item => kmValue >= item.minKm && kmValue <= item.maxKm,
      );
  ...
  const handlerTripResult = async () => {
    await checkInputData(async (isSuccess, message, alertNotify) => {
      if (isSuccess) {
        ...
        let alertDate = '';
        if (
          (tripConfig.maxDayConstrant || 0) > 0 &&
          (tripConfig.maxDayInMonth || 0) > 0 &&
          listTypeCost.code !== 'onlyWorkCosts'
        ) {
          const totalDayMonth =
            (dateFilter.totalDayMonth || 0) -
            (dateFilter.isUpdate ? tripResult.daysOld || 0 : 0);
          const dayValue = tripResult.dayValue || 0;

          if (
            tripConfig.maxDayConstrant == 2 &&
            totalDayMonth + dayValue > tripConfig.maxDayInMonth
          ) {
            alertDate = `Tổng số ngày lớn hơn ${tripConfig.maxDayInMonth} (${totalDayMonth + dayValue
              }/${tripConfig.maxDayInMonth})\n`;
          ...
          if (
            tripConfig.maxDayConstrant == 1 &&
            totalDayMonth + dayValue > tripConfig.maxDayInMonth
          ) {
            alertWarning(
              `Tổng số ngày đăng kí công tác phải nhỏ hơn hoặc bằng ${tripConfig.maxDayInMonth
              } ngày (${totalDayMonth + dayValue}/${tripConfig.maxDayInMonth})`,
            );
            return;
          }
        }
  ...
  const checkInputData = actionDone => {
    ...
    if (tripConfig.maxUpdate > 0 && dateFilter.isUpdate) {
      if (tripConfig.maxUpdate == itemCost.countUpdate) {
        actionDone(
          false,
          tripConfig.notifyAlert ||
          `Giới hạn số lần cập nhật (${itemCost.countUpdate}/${tripConfig.maxUpdate})`,
        );
        return;
      } else if (
        tripConfig.maxUpdate > itemCost.countUpdate &&
        dateFilter.isEvidentDateChange
      ) {
        ...
        if (totalCostValue > itemCost.totalCostsTrip) {
          ...
          if (evidenceParse.length == 0) {
            actionDone(
              false,
              `Vui lòng thêm hình ảnh làm bằng chứng cho chi phí lớn hơn chi phí đăng kí trước đó (${totalCostValue}/${itemCost.totalCostsTrip})`,
            );
            return;
          } else {
    ...
    let strValid = '';
    if (!isValid(itemCost.vehicalOtherValue)) strValid += 'Chi phí di chuyển, ';
    ...
    const limitCostMonth =
      _.filter(
        limitCostSupport,
        itemlimit => itemlimit.Month == tripMonth,
      )[0] || null;
    const limitCostTotalMonth =
      _.filter(
        limitCostSupportTotal,
        itemlimit => itemlimit.Month == tripMonth,
      )[0] || null;
    ...
    if (limitCostMonth) {
      const limitByWork = dateFilter.isUpdate
        ? limitCostMonth.CostLimit + (tripResult.supportWorkMaint || 0)
        : limitCostMonth.CostLimit;
      const limitByWorkMain =
        limitByWork > limitCostMonth.MaxCostSupportWork
          ? limitCostMonth.MaxCostSupportWork
          : limitByWork;
      if (
        itemCost.supportWorkValue > 0 &&
        itemCost.supportWorkValue > limitByWorkMain
      ) {
        if (dateFilter.isEvidentDateChange) {
          if (evidenceParse.length == 0) {
            strValid += `Vui lòng thêm hình ảnh làm bằng chứng cho chi phí tiếp khách đang lớn hơn định mức hỗ trợ (${formatNumber(
              itemCost.supportWorkValue,
              ',',
            )}/${formatNumber(limitByWorkMain, ',') || 0}),\n`;
          } else {
            strAlertWorkValue += `Tổng chi phí tiếp khách đang lớn hơn định mức hỗ trợ (${formatNumber(
              itemCost.supportWorkValue,
              ',',
            )}/${formatNumber(limitByWorkMain, ',') || 0}),\n`;
          }
        ...
        if (!dateFilter.isEvidentDateChange) {
          strValid += `Chi phí tiếp khách phải nhỏ hơn hoặc bằng định mức hỗ trợ (${formatNumber(
            itemCost.supportWorkValue,
            ',',
          )}/${formatNumber(limitByWorkMain, ',') || 0}),\n`;
        }
    ...
    if (limitCostTotalMonth && limitCostMonth) {
      const limitByTotal = dateFilter.isUpdate
        ? limitCostTotalMonth.CostLimit +
        (tripResult.totalCostsTrip - tripResult.supportWorkMaint || 0)
        : limitCostTotalMonth.CostLimit;
      const limitByTotalMain =
        limitByTotal > limitCostMonth.MaxCostSupport
          ? limitCostMonth.MaxCostSupport
          : limitByTotal;
      ...
      if (totalCostValue - costResult.supportWork > limitByTotalMain) {
        if (dateFilter.isEvidentDateChange) {
          if (evidenceParse.length == 0) {
            strValid += `Vui lòng thêm hình ảnh làm bằng chứng cho chi phí công tác đang lớn hơn định mức hỗ trợ (${formatNumber(
              totalCostValue - costResult.supportWork,
              ',',
            )}/${formatNumber(limitByTotalMain, ',') || 0}),\n`;
          } else {
            strAlertWorkValue += `Tổng chi phí công tác đang lớn hơn định mức hỗ trợ (${formatNumber(
              totalCostValue - costResult.supportWork,
              ',',
            )}/${formatNumber(limitByTotalMain, ',') || 0}),\n`;
          ...
          strValid += `Tổng chi phí công tác phải nhỏ hơn hoặc bằng định mức hỗ trợ (${formatNumber(
            totalCostValue - costResult.supportWork,
            ',',
          )}/${formatNumber(limitByTotalMain, ',') || 0}),\n`;
        }
      ...
      >
        ...
        >
          ...
          <ItemInput
            ...
            iconName="money-bill"
            editable={
              Object.keys(listTypeCost).length > 0
                ? listTypeCost.code == 'onlyWorkCosts'
                  ? true
                  : false
                : true
          ...
          <ItemInput
            ...
            editable={
              Object.keys(listTypeCost).length > 0
                ? listTypeCost.code == 'onlyOtherCosts'
                  ? true
                  : false
                : true
            }
