...
export const CostHPI = ({ onBack, onNext, quotaData }) => {
  ...
  const checkInputData = actionDone => {
    ...
    if (itemCost.nightRestValue > 0 && itemCost.nightRestValue < priceMin)
      strValid += `Vui lòng nhập chi phí khách sạn phải lớn hơn ${formatNumber(
        priceMin,
        ',',
      )} VNĐ\n`;

    if (itemCost.lunchValue > 0 && itemCost.lunchValue < priceMin)
      strValid += `Vui lòng nhập chi phí ăn có chứng từ phải lớn hơn ${formatNumber(
        priceMin,
        ',',
      )} VNĐ\n`;

    if (itemCost.dinnerValue > 0 && itemCost.dinnerValue < priceMin)
      strValid += `Vui lòng nhập chi phí ăn không chứng từ phải lớn hơn ${formatNumber(
        priceMin,
        ',',
      )} VNĐ\n`;

    if (itemCost.vehicalValue > 0 && itemCost.vehicalValue < priceMin)
      strValid += `Vui lòng nhập chi phí di chuyển phải lớn hơn ${formatNumber(
        priceMin,
        ',',
      )} VNĐ\n`;

    if (itemCost.otherValue > 0 && itemCost.otherValue < priceMin)
      strValid += `Vui lòng nhập chi phí khác phải lớn hơn ${formatNumber(
        priceMin,
        ',',
      )} VNĐ\n`;
    actionDone(
