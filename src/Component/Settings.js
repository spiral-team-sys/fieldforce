...
export const Settings = ({ navigation }) => {
  ...
  const [cleanDataPhoto, setCleanDataPhoto] = useState({
    isOpenClean: false,
    cleanFrom: 1,
    isWeek: false,
  });
    ...
    try {
      ...
      switch (key) {
        ...
        case 'exportPhotoReport':
          ...
          break;
        case 'cleanDataPhoto':
          await setCleanDataPhoto({
            ...cleanDataPhoto,
            isOpenClean: value,
            cleanFrom: 1,
          });
          if (value) SheetManager.show('actioncalendar');
          break;
      }
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <Text style={styles.headerTitle}>
        Sử dụng đăng nhập bằng Khuôn mặt / Vân tay
      </Text>
      <ControlView
        itemKey="faceId"
        styles={styles}
        title={
          !faceId ? 'Xác thực bằng Khuôn mặt / Vân tay' : 'Đã bật xác thực'
        }
      ...
      <ControlView
        ...
        title={
          !exportAttendant
            ? 'Lưu hình ảnh chấm công'
            : 'Đã bật lưu hình ảnh chấm công'
        }
        iconName="id-badge"
        iconType="font-awesome-5"
      ...
      <ControlView
        ...
        title={
          !exportPhotoReport
            ? 'Lưu hình ảnh báo cáo'
            : 'Đã bật lưu hình ảnh báo cáo'
        }
        iconName="images"
        iconType="font-awesome-5"
      ...
      <ControlView
        ...
        styles={styles}
        title={`${cleanDataPhoto?.isOpenClean
            ? 'Đã bật xoá dữ liệu hình ảnh'
            : 'Xoá dữ liệu hình ảnh'
          } ${cleanDataPhoto?.isOpenClean
            ? `(${cleanDataPhoto?.cleanFrom} ${cleanDataPhoto?.isWeek ? 'Tuần' : 'Tháng'
            })`
            : ''
          }`}
        iconName="trash-alt"
      ...
      >
        <Text style={styles.calendarTitle}>
          Thời gian giữ hình ảnh trên thiết bị
        </Text>
        <View
          style={{
            borderWidth: 1,
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <View
              ...
              style={{
                ...
                marginEnd: 0,
                borderRadius: 20,
                backgroundColor: !cleanDataPhoto?.isWeek
                  ? appcolor.primary
                  : appcolor.surface,
              }}
            >
              <TouchableOpacity
                onPress={() =>
                  handlerChangeTimeDelete('cleanDataPhoto', 'isWeek', false)
                }
              >
                <Text
                  style={{
                    ...styles.itemTitle,
                    color: !cleanDataPhoto?.isWeek
                      ? appcolor.white
                      : appcolor.dark,
                  }}
                >
            ...
            >
              <TouchableOpacity
                onPress={() =>
                  handlerChangeTimeDelete('cleanDataPhoto', 'isWeek', true)
                }
              >
                <Text
                  style={{
