...
import { Text, View, ScrollView, Image, Modal, Dimensions, TouchableOpacity, StyleSheet } from "react-native";
    ...
    const CreateBody = (content) => {
        ...
        arrString.forEach((element, index) => {
            if (element.includes("http") && (element?.includes(".jpg") || element?.includes(".jpeg") || element?.includes(".png"))) {
                bodyView.push(<Text style={{ color: appcolor.dark }} key={"t" + index.toString()}>
                    -------------------------------</Text>)
                const imageUrl = element.replace(/(\r\n|\n|\r)/gm, "")
                bodyView.push(
                    <Image key={index.toString()}
                        style={{ width: Dimensions.get("window").width - 20, borderRadius: 10, height: 240 }}
                        source={{
                            uri: imageUrl,
                        }} />
                )
                ...
                bodyView.push(
                    <Text key={index.toString()} onPress={() => gotoPage(element)}
                        style={{ fontWeight: '600', padding: 2, color: appcolor.info }}>{'Chi tiết>>'}</Text>
                )
            }
            else {
                bodyView.push(
        ...
        <View style={styles.content}>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 15 }}
                ...
                <ViewShot style={{ padding: 12 }} ref={refShot} options={{ format: "jpg", quality: 0.8 }}>
                    ...
                    <Text style={{ backgroundColor: appcolor.light, color: appcolor.dark }}>{CreateBody(data?.body || "")}</Text>
                    <Text style={{ color: appcolor.dark, width: '100%', padding: 5, textAlign: 'right', }}>{moment(data.createdDate).calendar()}</Text>
                    {data.hyperLinks !== null && data.hyperLinks !== "" && data.hyperLinks !== "null" &&
                        <View style={{ alignItems: 'flex-end', marginBottom: 10, display: ((!data?.hyperLinks?.includes('.png') && !data?.hyperLinks.includes('.jpg') && !data.hyperLinks.includes('.jpeg'))) ? "flex" : "none" }}>
                            <TouchableOpacity style={{ padding: 7, flexDirection: 'row', borderWidth: 0.5, borderColor: appcolor.primary }} onPress={() => gotoPage(data.hyperLinks)}>
                                <Icon color={appcolor.primary}
                                    name="hand-o-right" type="font-awesome" size={20} />
