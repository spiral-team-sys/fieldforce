...
import { View, ScrollView, Modal, TouchableOpacity, SafeAreaView } from "react-native";
import { ListItem, Text } from '@rneui/themed';
import { getLstMessengerPayslip, updateMessenger } from "../Controller/WorkController";
    ...
    return (
        <View style={{ flex: 1 }}>
            <View style={{ height: '100%', width: '100%', backgroundColor: appcolor.light }}>
                <HeaderCustom title='Quản lý tin nhắn lương' leftFunc={() => navigation.goBack()} />
                <ScrollView>
                    {lstNotify.length > 0 && lstNotify.map((item, i) => (
                        <ListItem
                            key={item.id}
                        ...
                        >
                            <View style={{
                                ...
                                borderRadius: 20, backgroundColor: ColorRand(i)
                            }}>
                            ...
                            <ListItem.Content>
                                ...
                                }} numberOfLines={3} lineBreakMode='middle'>
            ...
            <Modal animationType="slide" visible={isSec > 0}>
                <View style={{ backgroundColor: appcolor.light, height: '100%' }}>
                    ...
                    <View style={{ padding: 12 }}>
                        <Text style={{ textAlign: 'center', color: appcolor.dark, fontWeight: 'bold' }}>Xác thực thông tin</Text>
                        <View style={{ paddingHorizontal: 12 }}>
                            <Text style={{ color: appcolor.dark, textAlign: 'center' }}>Nhằm nâng cao tính bảo mật về lương bạn cần xác thực 2 bước</Text>
                        </View>
                        <TouchableOpacity onPress={() => LocalSignIn.onAuthenticateID((e) => { e === 1 && setIsSec(-1); })}>
                    ...
                    <View style={{ padding: 12 }}>
                        <Text style={{ color: appcolor.danger, textAlign: 'center' }}>
                            {`Ấn vào biểu tượng ${isSec === 1 ? 'khuôn mặt' : "vân tay"} => Sử dụng ${isSec === 1 ? 'FaceID' : "vân tay"} để mở khoá`}
                        </Text>
                    </View>
                </View>
            </Modal>
