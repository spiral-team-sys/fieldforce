...
export const BussinessTrip = ({ navigation }) => {
  ...
  const searchLocation = async () => {
    ...
    AttendantController.LocationFromAddress(
      ...
      error => MessageInfo('Không tìm thấy địa chỉ cần tìm'),
    );
  };
      ...
      <TouchableOpacity
        onPress={() => {
          if (mode === VIEW_INPUT && modeAddress === 'from') {
            ...
            setBussinessInput({
              ...bussinessInput,
              locationFrom: item.geometry.lat + ',' + item.geometry.lng,
              fromAddress: item.formatted_address,
            });
            ...
            setBussinessInput({
              ...bussinessInput,
              locationTo: item.geometry.lat + ',' + item.geometry.lng,
              toAddress: item.formatted_address,
            });
            _bottomSheetGoogle.current.hide();
          }
        }}
    ...
    <View style={{ flex: 1 }}>
      ...
      </MapView>
      {
        ...
        >
          <TouchableOpacity onPress={() => navigation.goBack()}>
            <SpiralIcon name="arrow-back" reverse />
          </TouchableOpacity>
          {/* <FormGroup
                        inputRef={_searchRef}
      ...
      >
        <View style={{ backgroundColor: appcolor.light }}>
          {mode !== VIEW_HIS && (
            <TouchableOpacity
              onPress={() => {
                (mode === VIEW_RESULT || mode === VIEW_INPUT) &&
                  setMode(VIEW_HIS);
              }}
              style={{
                flexDirection: 'row',
            ...
            >
              ...
              />
              <Text
                style={{ color: appcolor.dark, flexGrow: 0.7, paddingLeft: 12 }}
              >
          ...
          {AppNameBuild === mitsuApp ? (
            <ScrollView
              nestedScrollEnabled
              onMomentumScrollEnd={() => {
                _actionSheetRef.current?.handleChildScrollEnd();
              }}
            ...
            >
              ...
              {mode === VIEW_INPUT && (
                <BussinessTripInput
                  ...
                  myAddress={myAddress}
                  searchMap={searchMap}
                  bussinessInput={bussinessInput}
                  setBussinessInput={setBussinessInput}
                  Provinces={Provinces}
                  MoneyMove={MoneyMove}
                />
              )}
              {mode === VIEW_HIS && (
                <BussinessTripHistory
                  ...
                  appcolor={appcolor}
                  nextView={nextView}
                  resetBussinessInput={resetBussinessInput}
                  MoneyMove={MoneyMove}
                  remaining={remaining}
      ...
      >
        <View key={'date'} style={{ height: '80%' }}>
          {LstAddress.length > 0 && (
            <FlatList data={LstAddress} renderItem={renderItemAddress} />
          )}
        </View>
      </ActionSheet>
