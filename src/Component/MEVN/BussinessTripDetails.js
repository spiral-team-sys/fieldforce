...
export const BussinessTripDetails = ({
  bussinessInfo,
  appcolor,
  nextView,
  bottomSheet,
  ...
  useEffect(() => {
    if (bussinessInfo.view === MODE_VIEW) setBussinessShow(bussinessInfo);
    else CalculatorBy();
    return () => false;
  ...
  const CalculatorBy = () => {
    ...
    if (bussinessShow?.fromDate && bussinessShow?.toDate) {
      ...
      if (toDateInt >= fromDateInt && toDateInt !== 0 && fromDateInt !== 0) {
        ...
        } else supportTotal = supportRestaurant;
        setBussinessShow({
          ...bussinessShow,
    ...
    <View style={{ flex: 1, backgroundColor: appcolor.surface }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        ...
        <NumberFormat
          ...
          disabled={true}
          value={
            bussinessShow?.supportTotal || bussinessInfo?.totalSupport || '0'
          }
          displayType={'text'}
          thousandSeparator={true}
          renderText={value => (
      ...
      <NumberFormat
        ...
        disabled={true}
        value={
          bussinessShow?.supportRestaurant || bussinessShow?.supportLunch || '0'
        }
        displayType={'text'}
        thousandSeparator={true}
        renderText={value => (
