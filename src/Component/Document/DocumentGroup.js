...
export const DocumentGroup = ({ navigation }) => {
    ...
    const onSelected = (item) => {
        if (item.ref_Name == 'Video') {
            navigation.navigate("video", { titlePage: item.groupName, documentData: JSON.parse(item.detailDocument), formType: 'DOCUMENT' })
        } else {
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity onPress={onSelectItem} style={styles.itemGroup} key={index} >
                <Icon color={appcolor.primary} size={36} name='folder-open' solid />
                <Text style={{ padding: 3, fontSize: 12, marginTop: 5, color: appcolor.dark, textAlign: 'center' }}>{item.groupName}</Text>
            </TouchableOpacity>
        )
    }
    return (
