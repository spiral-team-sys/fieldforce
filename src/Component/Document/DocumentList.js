...
import { View, FlatList, Text, ScrollView, Modal, ActivityIndicator, StyleSheet, } from "react-native"
import { deviceWidth } from '../../Themes/AppsStyle'
import { ListItem, Button, Divider, Icon } from '@rneui/themed'
import { downloadFile, openFileViewer, checkFileExist, alertNotify, alertError } from '../../Core/Utility'
...
export const DocumentList = ({ navigation, route }) => {
            ...
            for (let i = 0, lenData = _data.length; i < lenData; ++i) {
                ...
                if (Array.isArray(filePath)) {
                    for (let j = 0, lenFilePath = filePath.length; j < lenFilePath; ++j) {
                        filePath[j].downloadUrl = filePath[j].downloadUrl || filePath[j].Url
    ...
    const itemFile = (jsonFile, parentIndex, blockDownload) => {
        ...
        if (Array.isArray(files)) {
            files.forEach((item, childIndex) => {
                vFiles.push(
                    <View key={childIndex} style={{ marginRight: 5, alignContent: 'center' }}>
                        <Button onPress={() => onViewDoc(item, parentIndex, childIndex, blockDownload)}
                            titleStyle={{ fontSize: 12, color: appcolor.tomato }}
                            buttonStyle={{ borderRadius: 5, borderColor: appcolor.tomato, }}
                            icon={<Icon name="eye" size={14} type="font-awesome" color={appcolor.tomato} />}
                            title={`Xem ${item.FileType}`} type="outline" />
    ...
    const setViewFileData = (urlPage, item, parentIndex, childIndex, blockDownload) => {
    ...
    const onViewDoc = (item, parentIndex, childIndex, blockDownload) => {
        ...
        } else if (!item.Url.includes("http") && MediaSupport.includes(item.FileType)) {
    ...
    const renderRow = ({ item, index }) => {
        return (
            <ListItem key={index} containerStyle={{ backgroundColor: appcolor.light, borderRadius: 8 }}>
                <ListItem.Content>
                    {index !== 0 && <Divider style={{ backgroundColor: appcolor.light, width: deviceWidth + 100, height: 0.7, marginBottom: 5 }} />}
                    <ListItem.Title style={{ color: appcolor.dark }}>{`${index + 1}/ ${item.DocumentName}`}</ListItem.Title>
                    <ListItem.Subtitle style={{ color: appcolor.dark, fontStyle: 'italic', }}>{item.Description}</ListItem.Subtitle>
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            ...
            <Modal visible={viewFile !== null ? true : false} animationType="slide" statusBarTranslucent>
                <SafeAreaProvider>
                    <WebViewScreen
                        ...
                        onDownloadFile={() => onDownloadFile(viewFile?.item, viewFile?.parentIndex, viewFile?.childIndex)}
