...
export const AlbumPhoto = ({ navigation, route }) => {
  ...
  const loadData = async () => {
    let lstPhotos = await getPhotosReportByPhototype(
      detailInfo.photoType,
      detailInfo.shopId,
      detailInfo.photoDate,
    );
  ...
  const showImage = isShow => {
    isShow
      ? SheetManager.show('imageSheetAlbum')
      : SheetManager.hide('imageSheetAlbum');
  };
  ...
  return (
    <SafeAreaView
      style={[styles.container, { backgroundColor: appcolor.surface }]}
    >
      <HeaderCustom
        leftFunc={handlerGoback}
        title={detailInfo.titlePage || 'Quản lý hình ảnh'}
      />

      <TouchableOpacity
        style={styles.listWrapper}
        onLongPress={() => showActionDelete(false)}
      >
        <FlatList
      ...
      <View style={styles.actionSheetWrapper}>
        ...
        >
          <View
            style={[styles.sheetContent, { backgroundColor: appcolor.light }]}
          >
            <MultipleShowImage
              key={'ShowItemImage'}
              listItem={lstShow}
