...
export const PhotoByList = ({ navigation, route }) => {
  ...
  const loadPhoto = async () => {
    const res = await GetPhotosByReportId(
      shopinfo.shopId,
      workinfo.workDate,
      isTakeInOther == true ? dataKPIData.id : kpiinfo.id,
    );
  ...
  const SendReport = async () => {
    ...
    if (data.dataConfig?.isConstraint == 1) {
      ...
      if (
        ...
        numLimitPhoto > 0 &&
        res?.length < numLimitPhoto
      ) {
        ToastError(
          `Vui lòng chụp ${numLimitPhoto} tấm hình cho báo cáo.(${res?.length}/${numLimitPhoto})`,
        );
        return false;
      }
      ...
      for (let index = 0; index < data.dataView?.length; index++) {
        ...
        const listByItemM = _.filter(res, it =>
          it.photoType?.includes(`${itemM.code}`),
        );
        if (itemM.isRequired == 1) {
          if (
            itemM.numberValue !== undefined &&
            itemM.numberValue > 0 &&
            listByItemM?.length < itemM.numberValue
          ) {
            ToastError(
              `Vui lòng chụp ${itemM.numberValue} tấm hình cho ${itemM.name}.(${listByItemM?.length}/${itemM.numberValue})`,
            );
            return false;
          }
          ...
          if (filterList?.length > 0) {
            for (let idxP = 0; idxP < filterList.length; idxP++) {
              ...
              const listByTemplate = _.filter(
                res,
                it => it.photoType == photoType,
              );
              if (listByTemplate?.length < itL.numberIMG) {
                ToastError(
                  `${itemM.name} : Vui lòng chụp ${itL.numberIMG} tấm hình cho ${itL.nameVN}`,
                );
                return false;
              }
            }
  ...
  const onUploadData = async () => {
    ...
    if (result.statusId === 200) {
      await QueryStringSql(
        `UPDATE ${
          taskList.tableName
        } SET taskDone=1,taskAlter='Đã hoàn thành' WHERE shopId=${
          workinfo.shopId
        } AND reportId=${isTakeInOther == true ? dataKPIData.id : kpiinfo.id}`,
      );
  ...
  const handlerSelectItem = async (item, listByItem) => {
    ...
    if (!listTakePhoto || listTakePhoto?.length == 0) {
      ...
      if (!listTakePhoto || listTakePhoto?.length == 0) {
        listTakePhoto = getTemplateByCode(
          data.dataConfig?.ImageByList,
          item.code,
        );
      }
    }
    const itemStatus = listByItem[0]?.dataUpload || 0;
    await navigation.navigate('photogroup', {
      Status: itemStatus,
      dataImageList: listTakePhoto || [],
      keyPhoto: item.code,
      kpiData: isTakeInOther == true ? dataKPIData : kpiinfo,
      isTakeInOther: true,
      hideIcon: true,
    });
  };
  ...
  const renderItem = ({ item, index }) => {
    const listByItem =
      _.filter(data.dataPhoto, e => e.photoType?.includes(item.code)) || [];
    const itemStatus = listByItem[0]?.dataUpload || 0;
    return (
      <TouchableOpacity
      ...
      >
        <View style={styles.itemContent}>
          ...
          {item.descriptionName && (
            <Text style={styles.itemDescription}>{item.descriptionName}</Text>
          )}
        </View>
        <Badge
          badgeStyle={[
            ...
            {
              backgroundColor:
                itemStatus == 1 ? appcolor.success : appcolor.primary,
            },
          ]}
          textStyle={styles.badgeText}
          value={listByItem.length}
        />
      </TouchableOpacity>
    );
  };
  ...
  return (
    <View style={[defaultStyles.contentContainer, styles.mainContainer]}>
      <HeaderCustom
        ...
        title={
          (isTakeInOther == true
            ? dataKPIData?.menuNameVN
            : kpiinfo?.menuNameVN) || 'Chụp hình'
        }
      />
