...
export const PhotoSystemReport = ({ navigation, route }) => {
  ...
  const [newConfigFilter, setNewConfigFilter] = useState({
    year: DATE.getFullYear(),
    yearname: `Năm ${DATE.getFullYear()}`,
    month: DATE.getMonth() + 1,
    monthname: `Tháng ${DATE.getMonth() + 1}`,
  ...
  const loadData = async () => {
    ...
    const dataFilter = {
      shopId: 0,
      reportId: kpiinfo.id,
      month: configFilter.month,
      year: configFilter.year,
    };
  ...
  const handleGoBack = async () => {
    ...
    try {
      ...
      const imageFiles = files.filter(
        file => file.isFile() && /\.(jpg|jpeg|png|gif)$/.test(file.name),
      );
