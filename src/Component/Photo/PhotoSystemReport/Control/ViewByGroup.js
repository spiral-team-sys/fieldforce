        ...
        <View key={'ViewSaveImage'} style={{ width: deviceWidth * 0.8 }}>
          ...
          >
            ...
            >
              ...
              >
                Lưu {dataSelect.dataSelectItem.length || 0} hình?
              </Text>
            </View>
            <View
              style={{
  ...
  const filterPhoto = async text => {
    ...
    if (text) {
      dataGroup.dataAll.map(it => {
        const itemName = `${it[filter.keyFilter]}_${it.shopCode}`
          .toString()
          .toUpperCase();
    ...
    <View style={styles.mainContainer}>
      {isLongPress && (
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          ...
          >
            <Text
              style={{
                fontWeight: Platform.OS == 'iso' ? '600' : '700',
                fontSize: 16,
                color: appcolor.dark,
              }}
            >
              {selectData?.dataSelectItem?.length == 0
                ? 'Chọn hình'
                : `Đã chọn ${selectData?.dataSelectItem?.length || 0} hình`}
            </Text>
          </View>
          <View
            style={{
      ...
      {!isLoading && dataGroup.dataGroup.length > 0 && (
        <FlashList
          ...
          ListFooterComponent={
            <View style={{ paddingBottom: deviceHeight / 8 }} />
          }
        />
      )}
      {isLongPress && (
...
const RenderItem = ({
  ...
  const dataPhotoFilter = dataGroup.dataAll.filter(
    it =>
      it[filter.keyFilter] == item[filter.keyFilter] &&
      !it.isHide &&
      !!getPhotoUri(it.photoPath),
  );
  ...
  const handleSelectPhoto = async (type, itemSelect) => {
    if (type === 'photo') {
      const indexItem = dataPhotoFilter.findIndex(
        it => it.photoPath === itemSelect.photoPath,
      );
      await setDataModal({
        mode: type,
        imageIndex: indexItem,
        visibleModal: true,
        listByGroup: dataPhotoFilter,
      });
    } else if (type === 'select') {
  ...
  const handleSelectItem = itemSelect => {
    ...
    if (indexUrl !== -1) {
      ...
      arrItemSelect = dataSelect.dataSelectItem.filter(
        it => it.photoPath !== itemSelect.photoPath,
      );
  ...
  return (
    ...
    >
      <TouchableOpacity
        key={`ShopName_${item.shopId}_${index}`}
        style={{
          marginTop: 8,
          borderRadius: 8,
          flex: 1,
      ...
      >
        ...
        >
          ...
          >
            {item[filter.keyFilter || 'shopName']}_{`${item.shopCode}`}{' '}
            {`(${dataPhotoFilter.length})`}
          </Text>
        </View>
      </TouchableOpacity>
      {isShowPhotobyGroup && (
              ...
              dataPhotoFilter.map((itemPhoto, indexPhoto) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      handleSelectPhoto(
                        isLongPress ? 'select' : 'photo',
                        itemPhoto,
                      )
                    }
                    onLongPress={() =>
                      !isLongPress ? handleLongSelect(itemPhoto) : null
                    }
                    style={{ margin: 1 }}
                  >
                    <View
                    ...
                    {isLongPress && (
                      <SpiralIcon
                        type="feather"
                        name={itemPhoto.isCheck ? 'check-circle' : 'circle'}
                        size={20}
                        containerStyle={{
                          position: 'absolute',
                          bottom: 5,
                        ...
                        color={
                          itemPhoto.isCheck ? appcolor.success : appcolor.white
                        }
                      />
                    )}
                    <View
      ...
      <Modal
        key={`${item.shopId}_${item.photoID}`}
        visible={dataModal.visibleModal}
        style={{ flex: 1 }}
      >
        {dataModal.mode == 'photo' && (
          <MultipleShowImage
            ...
            indexItem={dataModal.imageIndex}
            isUseTool={true}
            useEditImage={false}
            useDeleteTool={false}
            isShowText={true}
            sortFeild={filter.keyFilter}
          />
        )}
      </Modal>
    </View>
