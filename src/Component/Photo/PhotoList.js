...
import { Text, View, FlatList, TouchableOpacity, ImageBackground, Modal, Image, LayoutAnimation, UIManager, Platform, SafeAreaView, PermissionsAndroid } from 'react-native';
import { deviceWidth } from '../../Themes/AppsStyle';
import { HeaderCustom } from '../../Content/HeaderCustom';
import { deletePhotoByPath, getAllPhotosLocal, InsertPhotosItem } from '../../Controller/PhotoController';
import { groupDataByKey, Message, onShareLocalFile, ToastError, ToastSuccess, UUIDGenerator } from '../../Core/Helper';
...
export const PhotoList = ({ navigation, route }) => {
    const [data, setData] = useState({ groupPhoto: [], groupPhotoF: [], listPhoto: [], listByGroup: [] })
    ...
    const [itemSortFeild, setItemSortFeild] = useState({ sortFeild: 'shopId', titleFeild: 'shopName' })
    ...
    const loadData = async () => {
        ...
        const result = await groupByShop(arr, itemSortFeild.sortFeild, itemSortFeild.titleFeild)
        // res?.sort((a, b) => a.id < b.id);
        await setData({ groupPhoto: result, groupPhotoF: result, listPhoto: res, listByGroup: [] })
            ...
            it.dataGroup.map(item => {
                if (!isSelectAll) {
                    ...
                    if (item.photoPath.includes('uploaded')) {
                        const name = item.photoPath.substring(item.photoPath.lastIndexOf('/') + 1, item.photoPath?.length);
    ...
    const onSelectAllItem = async (item) => {
        ...
        if (item.isCheckAll === true) {
            listSelectF = listSelect.filter(it => it[itemSortFeild.sortFeild] !== item.groupId);
            data.listPhoto.map(it => { it[itemSortFeild.sortFeild] === item.groupId && (it.isCheck = false) })
            item.dataGroup.map(it => {
                const name = it.photoPath.substring(it.photoPath.lastIndexOf('/') + 1, it.photoPath?.length);
                const indexUrl = urlShare.findIndex(itm => itm === (it.photoPath.includes('uploaded') ? (path + name) : it.photoPath));
            ...
            listSelectF = listSelect.filter(it => it[itemSortFeild.sortFeild] !== item.groupId);
            data.listPhoto.map(it => { it[itemSortFeild.sortFeild] === item.groupId && (it.isCheck = true) })
            item.dataGroup.map(it => {
                const name = it.photoPath.substring(it.photoPath.lastIndexOf('/') + 1, it.photoPath?.length);
                const indexUrl = urlShare.findIndex(itm => itm == (it.photoPath.includes('uploaded') ? (path + name) : it.photoPath));
                if (indexUrl < 0) {
                    urlShare.push((it.photoPath.includes('uploaded') ? (path + it.photoPath) : it.photoPath))
                }
                listSelectF.push(it)
                it.isCheck = true
            })
    ...
    const handleSelectPhoto = async (type, item) => {
        if (type === 'edit') {
            ...
            const sortBy = itemSortFeild.sortFeild === 'shopId' ? 'shopId' : 'photoDate'
            const dataPhoto = data.groupPhoto.find(it => it.groupId === item[sortBy])?.dataGroup || []
            const indexItem = dataPhoto.findIndex(it => it.photoPath === item.photoPath)
            ...
            data.groupPhoto.map(it => {
                if (it.groupId === item[itemSortFeild.sortFeild] && it.isCheckAll === true) {
                    it.isCheckAll = false
                }
            })
            ...
            const extension = (Platform.OS === 'android') ? 'file://' : ''
            const path = `${extension}${RNFS.CachesDirectoryPath}/Camera/`;
            const pathFile = item.photoPath.includes('uploaded') ? `${path}${name}` : item.photoPath;

            const indexUrl = urlShare.findIndex(it => it === pathFile)
            if (indexUrl !== -1) {
                arrUrl = urlShare.filter((it, idx) => idx !== indexUrl)
                arrItemSelect = listSelect.filter(it => it.photoPath !== item.photoPath)
    ...
    const handleLongSelect = async (item) => {
        LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
        item.isCheck = true
        data.listPhoto.map(it => { it.photoPath === item.photoPath && (it.isCheck = item.isCheck) })
        const name = item.photoPath.substring(item.photoPath.lastIndexOf('/') + 1, item.photoPath?.length);
        const extension = (Platform.OS === 'android') ? 'file://' : ''
        const path = `${extension}${RNFS.CachesDirectoryPath}/Camera/`;
        const pathFile = item.photoPath.includes('uploaded') ? `${path}${name}` : item.photoPath;
            ...
            if (Platform.OS === "android") {
                ...
                if (Platform.Version < 33) {
                    ...
                    listSelect.map(async it => {
                        ...
                        if (it.photoPath.indexOf('https://') > -1 || it.photoPath.includes('uploaded')) {
                            const name = it.photoPath.substring(it.photoPath.lastIndexOf('/') + 1, it.photoPath?.length);
                            const extension = (Platform.OS === 'android') ? 'file://' : ''
                            const path = `${extension}${RNFS.CachesDirectoryPath}/Camera/`;
                            photoPath = `${path}${name}`
    ...
    const sortData = async () => {
        ...
        const sortBy = itemSortFeild.sortFeild === 'shopId' ? 'photoDate' : 'shopId'
        const title = itemSortFeild.titleFeild === 'shopName' ? 'photoDate' : 'shopName'
        ...
        await setData({ groupPhoto: result, groupPhotoF: result, listPhoto: data.listPhoto })
    }
            ...
            <View style={{ flex: 1 }}>
                {
                    ...
                    <View style={{ padding: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Text style={{ padding: 10, fontWeight: '600', fontSize: 14, color: appcolor.dark }}>{item.title}</Text>
                        {
                            isLongPress &&
                            <TouchableOpacity
                                onPress={() => onSelectAllItem(item)}
                                // onLongPress={() => !isLongPress ? handleLongSelect(item) : null}
                                style={{ padding: 5, borderRadius: 20, backgroundColor: appcolor.surface, justifyContent: 'center', alignItems: 'center', height: 30, paddingHorizontal: 10, borderWidth: 0.3, borderColor: item.isCheckAll ? appcolor.info : appcolor.dark }}>
                                <Text style={{ fontWeight: '300', fontSize: 12, color: item.isCheckAll ? appcolor.info : appcolor.dark }}>{item.isCheckAll ? 'Bỏ chọn' : 'chọn tất cả'}</Text>
                            </TouchableOpacity>
                        }
                    </View>
                }
    ...
    const renderItemGroup = ({ item, index }) => {
        return (
            <TouchableOpacity
                onPress={() => handleSelectPhoto(isLongPress ? 'select' : 'photo', item)}
                onLongPress={() => !isLongPress ? handleLongSelect(item) : null}
                style={{ margin: 1 }}>
                <View style={{ width: (deviceWidth - 16) / 4, borderRadius: 10, height: 130, backgroundColor: appcolor.surface }}>
                    {
                ...
                {
                    isLongPress &&
                    <Icon type='feather' name={item.isCheck ? 'check-circle' : 'circle'} size={20} containerStyle={{ position: 'absolute', bottom: 5, right: 5, width: 20, height: 20 }} color={item.isCheck ? appcolor.success : appcolor.white} />
                }
            </TouchableOpacity>
        )
    }
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: "center", alignItems: 'center' }}>
                            <TouchableOpacity
                                ...
                                style={{ width: 50, height: 50, justifyContent: 'center', alignItems: 'center', marginLeft: 10, borderRadius: 10 }}
                            >
                                <Icon type='feather' name='x' size={30} color={appcolor.dark} />
                            </TouchableOpacity>
                            <Text style={{ color: appcolor.dark, fontWeight: '600', fontSize: 16 }} >Đã chọn : {listSelect?.length}</Text>
                        </View>
                        <TouchableOpacity
                            onPress={() => handleSelectTask()}
                            style={{ width: 50, height: 50, justifyContent: 'center', alignItems: 'center', marginLeft: 10, borderRadius: 10 }}
                        >
                            <Icon type='font-awesome-5' name='tasks' size={25} color={isSelectAll ? appcolor.primary : appcolor.dark} />
                        </TouchableOpacity>
                    </View>
                }
                {
                    data.groupPhoto?.length > 0 ?
                        ...
                        <View style={{ paddingTop: 30 }}>
                            <Text style={{ fontWeight: '600', fontSize: 20, textAlign: 'center', color: appcolor.dark }}>Chưa có dữ liệu hình ảnh</Text>
                        </View>
                }
            ...
            {
                isLongPress &&
                <View style={{ position: 'absolute', bottom: 0, height: 70, borderTopStartRadius: 10, borderTopEndRadius: 10, width: '100%', backgroundColor: appcolor.surface, justifyContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
                    <View style={{ padding: 5, flex: 1 }}>
                        <TouchableOpacity
                            onPress={() => handleSelectSave()}
                            style={{ width: '100%', height: 50, borderRadius: 10, alignItems: 'center' }}
                        >
                    ...
                    <View style={{ padding: 5, flex: 1 }}>
                        ...
                        >
                            ...
                            <Text style={{ fontWeight: '500', fontSize: 12, color: appcolor.dark }}>Sửa</Text>
                        </TouchableOpacity>
                    </View>
                    <View style={{ padding: 5, flex: 1 }}>
                        <TouchableOpacity
                    ...
                    <View style={{ padding: 5, flex: 1 }}>
                        ...
                        >
                            ...
                            <Text style={{ fontWeight: '500', fontSize: 12, color: appcolor.dark }}>Xoá</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            }
            <ActionSheet
                id={'ref_MultiDelete'}
            >
                <View style={{ height: 160, width: deviceWidth, backgroundColor: appcolor.light, borderTopEndRadius: 20, borderTopStartRadius: 20 }}>
                    <View style={{ height: 100, width: deviceWidth, justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ fontWeight: '700', fontSize: 18, color: appcolor.dark, padding: 5 }}>Xoá</Text>
                        <Text style={{ fontWeight: '300', fontSize: 14, color: appcolor.dark }}>Xoá {listSelect?.length} mục đã chọn?</Text>
                    </View>
                    <View style={{ flexDirection: 'row', height: 60, justifyContent: 'space-between' }}>
                        ...
                        >
                            <Text style={{ fontWeight: '400', fontSize: 16, color: appcolor.light }}>Xoá</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </ActionSheet>
            ...
            >
                <View style={{ height: 160, width: deviceWidth, backgroundColor: appcolor.light, borderTopEndRadius: 20, borderTopStartRadius: 20 }}>
                    <View style={{ height: 100, width: deviceWidth, justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ fontWeight: '700', fontSize: 18, color: appcolor.dark, padding: 5 }}>Lưu hình</Text>
                        <Text style={{ fontWeight: '300', fontSize: 14, color: appcolor.dark }}>Lưu {listSelect.length} hình?</Text>
                    </View>
                    <View style={{ flexDirection: 'row', height: 60, justifyContent: 'space-between' }}>
                        <TouchableOpacity
            ...
            <Modal visible={visibleModal} style={{ flex: 1 }}>
                {mode == 'photo' && <MultipleShowImage key={'ShowItemImage'} listItem={data.listByGroup} closeShowImage={(value) => handlerCloseModal(value)} indexItem={imageIndex} isUseTool={true} isShowText={true} sortFeild={itemSortFeild.sortFeild} />}
                {mode == 'edit' && <ModalEditImage itemPhoto={itemPhoto} data={data} onClose={handlerCloseModal} appcolor={appcolor} itemSortFeild={itemSortFeild} />}
            </Modal>
        </View>
    )
}

const ModalEditImage = gestureHandlerRootHOC(({ appcolor, data, onClose, itemPhoto, showReview = false, itemSortFeild }) => {
    ...
    const SavePhoto = async () => {
        Message('Chú ý', 'Sau khi lưu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', async () => {
            ...
            const indexGroup = data.groupPhoto.findIndex(it => it.groupId === itemPhoto[itemSortFeild.sortFeild])
            data.groupPhoto[indexGroup].dataGroup = [...data.groupPhoto[indexGroup].dataGroup, itemGroup]
            const arrListPhoto = [...data.listPhoto, itemGroup]
            data.listPhoto = arrListPhoto.sort((a, b) => a[itemSortFeild] - b[itemSortFeild])
            await onClose()
                ...
                isShowReview ?
                    <View style={{ width: '100%', height: '100%' }}>
                        <HeaderCustom
                            ...
                            rightFunc={() => SavePhoto()}
                        />
                        <View style={{ flex: 1, justifyContent: 'center', backgroundColor: appcolor.light }}>
                            <ViewShot style={{ width: viewSize.widthViewShot === 0 ? '100%' : viewSize.widthViewShot, height: viewSize.heightViewShot === 0 ? '80%' : viewSize.heightViewShot }} ref={viewShot} options={{ format: "jpg", quality: 0.8, result: 'base64' }}>
                                <ImageZoom ImagePath={pathReview} />
                                <View style={{ position: 'absolute', width: '100%', height: '100%', right: 0, top: 0, bottom: 0, left: 0, justifyContent: "space-between" }}>
                                    <View style={{ paddingLeft: 10, paddingTop: 5 }}>
                                        <Text style={{ color: 'red', width: '100%', fontWeight: '600', textAlign: 'left', fontSize: 10 }}>
                                            {`${itemPhoto.address}`}
                                        </Text>
                                        <Text style={{ color: 'red', fontWeight: '600', textAlign: 'left', fontSize: 10 }}>
                                    ...
                                    </View>
                                    {
                                        ...
                                        } resizeMode={"contain"} style={{ height: 300, opacity: 0.5, width: '100%' }}>
                                        </Image>
                                    }
                    ...
                    :
                    <SafeAreaView style={{ width: '100%', height: '100%', backgroundColor: appcolor.black }}>
                        {
