...
class AlbumPhoto extends PureComponent {
  ...
  loadData = async () => {
    ...
    if (this.state.detailInfo.photoType === 'KTV') {
      let lstPhotos = await getPhotosReportByPhototype(
        this.state.detailInfo.photoType,
        this.state.detailInfo.shopId,
        this.state.detailInfo.photoDate,
      );
      let lstUpload = lstPhotos?.filter(item => item.fileUpload === 0);
      this.setState({
        lstShow: lstPhotos?.map((ele, i) => {
          return this.state.lstShow?.length === 0
            ? { ...ele, isDelete: false }
            : { ...ele, isDelete: this.state.isAnimatingIcon };
        }),
        isHiddenButton: lstUpload?.length > 0 ? false : true,
      });
    } else {
      if (
        this.state.detailInfo.reportId == 4 ||
        this.state.detailInfo.reportId == 18 ||
        this.state.detailInfo.reportId == 6 ||
        this.state.detailInfo.reportId == 0
      ) {
        let lstPhotos = await getPhotosReportByGuiId(
          this.state.detailInfo.reportId,
          this.state.detailInfo.guiId,
          this.state.detailInfo.shopId,
          this.state.detailInfo.photoDate,
        );
        let lstUpload = lstPhotos?.filter(item => item.fileUpload === 0);
        ...
        let lstPhotos = await getPhotosByGuiId(
          this.state.detailInfo.guiId,
          this.state.detailInfo.shopId,
        );
        let lstUpload = lstPhotos?.filter(item => item.fileUpload === 0);
        ...
        let lstPhotos = await getPhotosPOP(
          this.state.detailInfo.photoDate,
          this.state.detailInfo.photoType,
        );
        let lstUpload = lstPhotos?.filter(item => item.fileUpload === 0);
        ...
        let lstPhotos = await getPhotosReport(
          this.state.detailInfo.reportId,
          this.state.detailInfo.photoType,
          this.state.detailInfo.shopId,
          this.state.detailInfo.photoDate,
  ...
  uploadPhotos = async () => {
    let lstUpload = await this.state.lstShow?.filter(
      itemP => itemP.fileUpload === 0,
    );
  ...
  showImage = isShow => {
    isShow
      ? this.ref_imageSheet.current.show()
      : this.ref_imageSheet.current.hide();
  };

  onShowImage = item => {
    const index = this.state.lstShow.findIndex(
            ...
            <SpiralIcon
              ...
              color={
                item.isDelete && item?.dataUpload !== 1
                  ? this.props.appcolor.danger
                  : item?.fileUpload == 1 && item?.dataUpload == 1
                  ? this.props.appcolor.success
                  : item?.fileUpload == 1 && item?.dataUpload == 0
                  ? this.props.appcolor.warning
                  : item?.fileUpload == 0 && item?.dataUpload == 1
                  ? this.props.appcolor.tomato
                  : 'transparent'
              }
              onPress={() =>
                item.dataUpload !== 1 &&
      ...
      <View style={{ flex: 1, backgroundColor: this.props.appcolor.surface }}>
        <HeaderCustom
          leftFunc={() => {
            ...
            this.state.reloadView &&
              typeof this.props.route.params.reloadView === 'function' &&
        ...
        <TouchableOpacity
          style={{ flex: 1 }}
          onLongPress={() => this.showActionDelete(false)}
        >
          <FlatList
            style={{ padding: 10 }}
        ...
        <ActionSheet ref={this.ref_imageSheet}>
          ...
          >
            <MultipleShowImage
              key={'ShowItemImage'}
              listItem={this.state.lstShow}
              closeShowImage={() => this.showImage(false)}
              indexItem={this.state.imageIndex}
            />
