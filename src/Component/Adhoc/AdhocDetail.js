...
import {
  SubmitAdhocSurvey,
  SubmitImageAdhocSurvey,
} from '../../Controller/AdhocController';
...
export const AdhocDetail = ({ navigation, route }) => {
  ...
  const loadData = async () => {
    try {
      ...
      for (let i = 0, lenData = formData.length; i < lenData; ++i) {
        const isDone =
          formData[i].max !== null || formData[i].min !== null
            ? false
            : !formData[i].required;
        ...
        if (
          isSubmitted &&
          (anwserType === ID_IMAGE || anwserType === ID_CAMERA)
        ) {
          ...
          try {
            listAnsweredImage = JSON.parse(
              anwserItem?.[0]?.anwserValue || '[]',
            );
          } catch (e) {
            listAnsweredImage = [];
          }
  ...
  const onSubmitAnswer = () => {
    ...
    alertConfirm(
      'Thông Báo',
      'Bạn có chắc chắn muốn gửi báo cáo?',
      async () => {
        setIsUploading(true);
        try {
          const dataSubmit = [];
          ...
          for (
            let i = 0, lenMainData = mainData?.length;
            i < lenMainData;
            ++i
          ) {
            const { anwserItem, questionId } = mainData[i];
            if (
              paginator[questionId] === undefined &&
              (anwserItem?.[0]?.anwserType === ID_MULTICHOICE ||
                anwserItem?.[0]?.anwserType === ID_CHECKBOX)
            ) {
              for (
                let i = 0, lenAnswerItem = anwserItem.length;
                i < lenAnswerItem;
                i++
              ) {
  ...
  const onPrevQuestion = () => {
    try {
      ...
      const prevId =
        itemParent?.prevStepId ??
        idQuesContainer?.[itemParent?.questionId]?.prevQuesId;
  ...
  const onPressCheckbox = (itemChild, indexChild, isSingleSelect) => {
    try {
      ...
      if (isSingleSelect) {
        itemChild.anwserValue = required
          ? 'true'
          : isSelected
          ? 'false'
          : 'true';
        ...
        // * Check Other
        const indexOther =
          prevAnswerIndex === undefined ? indexChild : prevAnswerIndex;
  ...
  const onPressTest = itemChild => {
    // * Test Question Type = 9 || 10
    const { itemParent } = currentBS;
  ...
  const onChangeInput = (value, itemChild, isNumber) => {
    try {
      ...
      let isDone =
        min !== null && min !== undefined && isNumber && +value < min
          ? false // * Min
          : max !== null && max !== undefined && isNumber && +value > max
          ? false // * Max
          : required === false
          ? true
          : value !== '';
  ...
  const onDayPress = date => {
    try {
      ...
      if (
        min !== null &&
        min !== undefined &&
        moment(date).isBefore(moment(min))
      ) {
        itemChild.anwserValue = min;
      ...
      if (
        max !== null &&
        max !== undefined &&
        moment(date).isAfter(moment(max))
      ) {
        itemChild.anwserValue = max;
  ...
  const handleDisplayModal = (itemChild, type) => {
    ...
    switch (type) {
      case 'DATE':
        ...
        currentBS.selectedDate = itemChild.anwserValue
          ? itemChild.anwserValue
          : new Date();
        break;
      case 'IMAGE_SELECT':
  ...
  const imageGalleryLaunch = () => {
    try {
      ...
      NativeCamera.imageGalleryLaunch(
        photoinfo,
        result => {
          const asset = result?.data?.[0];
          if (asset?.uri) onAfterTakeImage(asset);
  ...
  const checkInputOtherCheckbox = () => {
    try {
      ...
      if (
        anwserItem?.[indexOtherCheckbox]?.anwserValue === 'true' &&
  ...
  const onAfterTakeImage = res => {
    try {
      ...
      let pathToImage = `/uploaded/${+moment().format(
        'YYYYMMDD',
      )}/${safeFileName}`;

      let anwserValueArr = [];
      try {
        anwserValueArr = JSON.parse(itemChild?.anwserValue || '[]');
      } catch (e) {
        anwserValueArr = [];
      }
  ...
  const onRemoveImage = indexImage => {
    try {
      ...
      if (imageGroup?.[questionId] === undefined) imageGroup[questionId] = [];
      imageGroup[questionId] = imageGroup[questionId].filter(
        (_, idx) => idx !== indexImage,
      );
  ...
  const onSelectAddress = (dropdownItem, indexAnswer) => {
    try {
      ...
      const isDone = required
        ? !!anwserItem[0]?.anwserValue &&
          !!anwserItem[1]?.anwserValue &&
      ...
      switch (indexAnswer) {
        case 1:
          ...
          dataModalAddress.dataDistrictPrototype = JSON.parse(
            JSON.stringify(level2s),
          );
          anwserItem[2].anwserValue = '';
          anwserItem[3].anwserValue = '';
          onSwipe(indexAnswer);
        ...
        case 2:
          ...
          dataModalAddress.dataTownPrototype = JSON.parse(
            JSON.stringify(level3s),
          );
          anwserItem[3].anwserValue = '';
          onSwipe(indexAnswer);
          break;
  ...
  const onSequelAction = ({ itemParent, itemChild, prevStep, isDone }) => {
    try {
      ...
      const isFinishImediately =
        nextStep === 9999 && isDone
          ? true
          : nextStep === -1 && isDone
          ? itemParent.indexRow === currentBS.lenData
          : false;
  ...
  const onIndexModalAddressChanged = index => {
    switch (index) {
      case 0:
        dataModalAddress.dataProvince = JSON.parse(
          JSON.stringify(dataModalAddress.dataProvincePrototype),
        );
        dataModalAddress.searchProvince = '';
        break;
      case 1:
        dataModalAddress.dataDistrict = JSON.parse(
          JSON.stringify(dataModalAddress.dataDistrictPrototype),
        );
        dataModalAddress.searchDistrict = '';
        break;
      case 2:
        dataModalAddress.dataTown = JSON.parse(
          JSON.stringify(dataModalAddress.dataTownPrototype),
        );
  ...
  const styles = StyleSheet.create({
    ...
    modalHeader: {
      ...
      borderWidth: 2,
      borderBottomColor: appcolor.light,
      borderBottomWidth: 0,
    },
    modalFooter: {
      padding: 10,
  ...
  const isFinaleDone =
    Object.keys(currentBS?.isFinaleDone || {}).length === currentBS.lenData;
  ...
  const visibleCompleteButton =
    (isFinishImediately || isFinaleDone) &&
    !isSubmitted &&
    (isLastItem || isFinishImediately);
  const primaryColor = appcolor.primary;
  const secondaryColor = appcolor.secondary;
  let processStatus = isFinishImediately
    ? '100%'
    : `${
        ((((currentBS.itemParent?.indexRow || 1) - 1) /
          (currentBS.lenData - 1)) *
          100) |
        0
      }%`;
  ...
  return (
    ...
    >
      <HeaderCustom
        ...
        iconRight={
          overlayLayer.visibleSubmitBtn === true && visibleCompleteButton
            ? 'cloud-upload-alt'
            : null
        }
      ...
      <View
        style={{
          width: '100%',
          height: '100%',
          backgroundColor: overlayLayer.overlay ? '#222' : appcolor.bluelight,
        }}
      >
        ...
        {overlayLayer.overlay && (
          <View
            style={{ position: 'absolute', top: '50%', width: deviceWidth }}
          >
            <View
              style={{
            ...
            >
              ...
              >
                Bạn đã hoàn thành bảng khảo sát, bấm vào biểu tượng bên phải để
                gửi báo cáo
              </Text>
            </View>
          </View>
        ...
        <View
          style={{
            opacity: overlayLayer.overlay ? 0 : 1,
            height: deviceHeight,
            ...
            >
              ...
              >
                <ScrollView>
                  <View style={{ height: '100%' }}>
                    ...
                    >
                      ...
                      >
                        <Text
                          style={{ color: appcolor.dark, fontWeight: '700' }}
                        >
                          {processStatus}
                        </Text>
                      </View>
                    ...
                    <View>
                      ...
                      <ScrollView style={{ padding: 10, marginTop: 5 }}>
                        <FlatList
                          key="id"
                          data={data.anwserItem || []}
                          scrollEnabled={true}
                          nestedScrollEnabled={true}
                          keyExtractor={(_, indexChild) =>
                            indexChild.toString()
                          }
                          style={{
                            paddingLeft: 10,
                          ...
                          renderItem={itemKid => {
                            ...
                            switch (itemChild.anwserType) {
                              ...
                              case ID_IMAGE:
                                return (
                                  <AnswerImage
                                    ...
                                    questionId={currentBS.questionId}
                                    isSubmitted={isSubmitted}
                                  />
                                );
                              case ID_CAMERA:
                              ...
                              case ID_GRIDSELECTION:
                                return (
                                  <AnswerGridSelection
                                    itemChild={itemChild}
                                    indexChild={indexChild}
                                    appcolor={appcolor}
                                    onPressTest={onPressTest}
                                    isSubmitted={isSubmitted}
                                  />
                                );
                              case ID_GRIDMULTISELECTION:
                                return (
                                  <AnswerGridMultiSelection
                                    itemChild={itemChild}
                                    indexChild={indexChild}
                                    appcolor={appcolor}
                                    onPressTest={onPressTest}
                              ...
                              case ID_ADDRESS:
                                return (
                                  <AnswerAddress
                                    ...
                                    indexChild={indexChild}
                                    appcolor={appcolor}
                                    onChangeInputAddress={onChangeInputAddress}
                                    isSubmitted={isSubmitted}
                ...
                <View
                  style={{
                    height:
                      !isFirstItem || visibleNextButton || visibleCompleteButton
                        ? 50
                        : 0,
                  }}
                >
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    {!isFirstItem && (
                      <TouchableOpacity
                        ...
                        style={{
                          width:
                            visibleNextButton || visibleCompleteButton
                              ? '50%'
                              : '100%',
                          height: 50,
                          justifyContent: 'center',
                    ...
                    {visibleNextButton && (
                      <TouchableOpacity
                        ...
                        style={{
                          width: !isFirstItem ? '50%' : '100%',
                          height: 50,
                          justifyContent: 'center',
                        }}
                        disabled={isLastItem || !isDone}
                      >
                        <Text
                          style={{
                            color: appcolor.dark,
                    ...
                    {visibleCompleteButton && (
                      <TouchableOpacity
                        onPress={onShowSubmitButton}
                        style={{
                          width: isFirstItem ? '100%' : '50%',
                          height: 50,
                          justifyContent: 'center',
                        }}
                      >
      ...
      <Modal
        ...
        animationType="slide"
        transparent={
          currentBS.type === 'IMAGE_SELECT' ||
          currentBS.type === 'CAMERA_SELECT'
        }
      >
        <SafeAreaView
          style={{
        ...
        >
          <Swiper
            ...
            onIndexChanged={onIndexModalAddressChanged}
          >
            <View
              key={1}
            ...
            >
              ...
              >
                <TouchableOpacity
                  onPress={() => onOpenModalAddress(false)}
                  style={{ padding: 15 }}
                >
                  <SpiralIcon
                    type="font-awesome-6"
              ...
              <FormGroup
                ...
                handleChangeForm={e =>
                  onFilterModalAddress(e, 'searchProvince', 'dataProvince')
                }
              />
              <FlatList
                ...
                renderItem={({ item, index }) => {
                  ...
                  const answerAddressItem =
                    currentBS?.itemParent?.anwserItem?.[indexAnswer] || {};
                  const isSelected =
                  ...
                  return (
                    <View key={index} style={{ margin: 5 }}>
                      <TouchableOpacity
                        disabled={false}
                        onPress={() => onSelectAddress(item, indexAnswer)}
                        style={{
            ...
            >
              ...
              <FormGroup
                ...
                placeholder="Tìm kiếm..."
                handleChangeForm={e =>
                  onFilterModalAddress(e, 'searchDistrict', 'dataDistrict')
                }
                stateName="search"
              />
              <FlatList
            ...
            >
              ...
              <FormGroup
                ...
                placeholder="Tìm kiếm..."
                handleChangeForm={e =>
                  onFilterModalAddress(e, 'searchTown', 'dataTown')
                }
                stateName="search"
              />
              <FlatList
...
const AnswerInput = ({ itemChild, appcolor, onChangeInput, isSubmitted }) => {
  ...
  return (
    <View
      style={{ borderWidth: 1, borderColor: appcolor.dark, borderRadius: 5 }}
    >
      <TextInput
        editable={!isSubmitted}
        value={itemChild.anwserValue}
    ...
    <View style={{ marginBottom: 5, marginTop: 5 }}>
      ...
      >
        <ListItem.CheckBox
          disabled={isSubmitted}
          onPress={handlePress}
          checked={itemChild.anwserValue === 'true'}
          checkedColor={appcolor.dark}
          uncheckedColor={appcolor.dark}
        />
        <Text style={{ color: appcolor.dark, marginLeft: 5, fontSize: 16 }}>
          {itemChild.anwserName}
        </Text>
      </TouchableOpacity>
      {data.showInputOther && itemChild.id === ID_OTHERCHECKBOX && (
        <TextInput
          value={itemChild.ortherValue}
          placeholder="Other..."
  ...
  return (
    ...
    >
      <SpiralIcon
        ...
        color={appcolor.dark}
      />
      <Text style={{ color: appcolor.dark, fontSize: 16, marginLeft: 10 }}>
        {itemChild.anwserValue !== '' ? itemChild.anwserValue : 'Chọn ngày'}
      </Text>
    </TouchableOpacity>
  );
};
      ...
      <View style={{ width: '100%' }}>
        ...
        >
          <SpiralIcon
            ...
            color={appcolor.dark}
          />
          <Text style={{ color: appcolor.dark, fontSize: 18, marginLeft: 10 }}>
            Thêm hình ảnh
          </Text>
        </TouchableOpacity>
        <ScrollView>
          <FlatList
            data={imageGroup[questionId] || []}
            key="images"
            numColumns={2}
            scrollEnabled={false}
              ...
              return (
                <View key={index} style={{ margin: 5 }}>
                  ...
                  <RNImage
                    source={{ uri: item.url }}
                    style={[
                      { flex: 1, width: 150, height: 150, resizeMode: 'cover' },
                    ]}
                  />
                </View>
              );
  ...
  return (
    <View style={{ backgroundColor: appcolor.light }}>
      {itemChild?.id === 1 ? (
        ...
        <TouchableOpacity
          disabled={isSubmitted}
          onPress={() => onOpenModalAddress(true, 0, itemChild)}
          style={{
            marginBottom: 10,
            flexDirection: 'row',
            alignItems: 'center',
        ...
        >
          <SpiralIcon
            ...
            color={appcolor.dark}
          />
          <Text style={{ fontSize: 16, marginLeft: 10, color: appcolor.dark }}>
            {!!itemChild.anwserValue ? itemChild.anwserValue : PROVINCE}
          </Text>
        </TouchableOpacity>
      ) : itemChild?.id === 3 ? (
        <TouchableOpacity
          disabled={isSubmitted}
          onPress={() => onOpenModalAddress(true, 1, itemChild)}
          style={{
            marginBottom: 10,
            flexDirection: 'row',
            alignItems: 'center',
        ...
        >
          ...
          />
          <Text style={{ fontSize: 16, marginLeft: 10, color: appcolor.dark }}>
            {!!itemChild.anwserValue ? itemChild.anwserValue : DISTRICT}
          </Text>
        </TouchableOpacity>
      ) : (
        itemChild?.id === 4 && (
          <TouchableOpacity
            disabled={isSubmitted}
            onPress={() => onOpenModalAddress(true, 2, itemChild)}
            style={{
              marginBottom: 10,
              flexDirection: 'row',
              alignItems: 'center',
          ...
          >
            ...
            />
            <Text
              style={{ fontSize: 16, marginLeft: 10, color: appcolor.dark }}
            >
              {!!itemChild.anwserValue ? itemChild.anwserValue : TOWN}
            </Text>
          </TouchableOpacity>
        )
      )}
  ...
  try {
    return (
      ...
      >
        ...
        >
          <Text
            style={{ color: appcolor.dark, fontSize: 20, fontWeight: '700' }}
          >
            Chọn ngày
          </Text>
        ...
        <View style={{ alignItems: 'center', justifyContent: 'center' }}>
          <DateChoose onChooseDate={onDayPress} />
      ...
      <View style={styles.modalContent}>
        <View style={{ width: '100%', paddingLeft: 5 }}>
          <TouchableOpacity style={{ padding: 15 }} onPress={cameraLaunch}>
            <Text style={{ color: appcolor.dark, fontSize: 16 }}>
              Chụp hình...
            </Text>
          </TouchableOpacity>
...
const RenderPaginations = ({
  ...
  try {
    ...
    paginators.forEach(([key, value], idx) => {
      ...
      const isOutOfBound =
        currentBS.semanticIndex >= 3 &&
        currentBS.semanticIndex < paginators.length - 1;
      if (idx < 3 || (paginators.length > 3 && idx === paginators.length - 1)) {
        cell.push(
          <TouchableOpacity
            disabled={false}
            onPress={() => onJumpToQuestion(key, value)}
            key={idx}
            style={{
              backgroundColor: isFocusing ? appcolor.tomato : appcolor.light,
              width: 30,
              height: 30,
              justifyContent: 'center',
              alignItems: 'center',
          ...
          >
            <Text
              style={{ color: isFocusing ? appcolor.light : appcolor.dark }}
            >
              {value + 1}
            </Text>
          </TouchableOpacity>,
      ...
      if (idx === 3 && idx < paginators.length - 1) {
        cell.push(
          <View
            key={idx}
            style={{
              backgroundColor: isOutOfBound ? appcolor.tomato : appcolor.light,
              width: 30,
              height: 30,
              justifyContent: 'center',
              alignItems: 'center',
