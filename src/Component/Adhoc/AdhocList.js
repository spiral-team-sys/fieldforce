...
import { StyleSheet, View, Text, FlatList, RefreshControl, TouchableOpacity, Modal, } from "react-native"
import { deviceWidth, fontWeightBold } from '../../Themes/AppsStyle'
import { GetSpiralForm } from '../../Controller/AdhocController';
import { ColorRand, StringTobase64, GetEmployeeInfo, removeVietnameseTones, } from "../../Core/Helper";
...
const AdhocList = ({ navigation, route }) => {
    ...
    const renderItem = useCallback(({ item, index }) => {
        return (
            <ListItem key={index} bottomDivider containerStyle={{ backgroundColor: appcolor.light }}>
                <TouchableOpacity onPress={() => handleDisplayFormDetail(item)} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', }}>
                    <Avatar size="medium" backgroundColor={ColorRand(index)} rounded title={(index + 1).toString()} />
                    <ListItem.Content style={{ marginLeft: 16 }}>
                        <ListItem.Title style={{ color: appcolor.dark, fontSize: 14, fontWeight: fontWeightBold }}>{item.title}</ListItem.Title>
                        <ListItem.Subtitle style={{ fontSize: 12, color: appcolor.greylight, fontWeight: '500' }}>{item.subTitle}</ListItem.Subtitle>
                        <Text style={{ alignSelf: 'flex-end', textAlign: 'right', color: appcolor.greylight, fontSize: 11, fontStyle: 'italic' }}>{moment(item.createDate).calendar()}</Text>
                    </ListItem.Content>
                </TouchableOpacity>
            </ListItem>
        )
    ...
    const styles = StyleSheet.create({
        mainContainer: {
            backgroundColor: appcolor.light, height: '100%', width: deviceWidth, shadowOpacity: 0.7, shadowRadius: 10,
            marginTop: 0, overflow: 'hidden',
            borderWidth: 0, marginLeft: 0, marginRight: 0,
        },
    })
    return (
        <View style={{ flex: 1, }}>
