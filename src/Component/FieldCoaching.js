...
const FieldCoaching = ({ navigation, route }) => {
    ...
    const LoadData = async () => {
        ...
        const workInfo = await GetWorkInfoBy(route?.params.workinfo.shopId, route?.params.workinfo.workDate)
    ...
    const handlerFilterProduct = async (text) => {
        ...
        const listData = keyword.length > 0
            ? dataMain.filter(i => `${i.name || ''}`.toUpperCase().includes(keyword))
            : dataMain
        await setDataModalBS({ data: [{ Id: 0, code: '', name: 'Tất cả' }, ...listData] })
    }
            ...
            <View style={{ width: '100%' }} key={index.toString()} >
                <View style={{ width: '100%', flexDirection: 'row', backgroundColor: appcolor.yellowdark, borderRadius: 5, padding: 8, marginBottom: 3, alignItems: 'center' }}>
                    <TouchableOpacity style={styles.actionButton} onPress={onPressItem}>
                        <Icon name='camera' size={17} color={appcolor.dark} style={{ textAlign: 'center' }} />
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.actionButton} onPress={onViewPhoto}>
                        <Icon name='images' size={17} color={appcolor.dark} style={{ textAlign: 'center' }} />
                        <Badge
    ...
    const renderItemSelect = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity onPress={selectItem}>
                <View style={{ borderBottomWidth: 0.5, borderBottomColor: appcolor.grey }}>
                    <Text style={{ padding: 8, fontSize: 15 }} >{(index + 1)}. {item.name}</Text>
                </View>
            </TouchableOpacity>
        )
    }
    const styles = StyleSheet.create({
        ...
        selectStyle: { flexDirection: 'row', width: '95%', height: 'auto', alignSelf: 'center', alignItems: "center", backgroundColor: appcolor.grayLight, padding: 8, borderRadius: 8 },
        ...
        inputStyle: { width: '15%', height: 'auto', alignSelf: 'center', padding: 8, margin: 3, borderRadius: 8, borderWidth: 0.5, borderColor: 'black' },
        actionButton: { width: 45, height: 32, borderRadius: 8, backgroundColor: appcolor.surface, justifyContent: 'center', alignItems: 'center', marginRight: 6 },
        itemAudit: { flex: 1, paddingStart: 8, paddingEnd: 8, fontSize: 15, color: appcolor.dark, textAlignVertical: 'center' }
    })
    useEffect(() => { LoadData() }, [])
    return (
                ...
                <Modal visible={isModalVisible} >
                    <View style={styles.modalStyle}>
                        ...
                        <View style={{ width: '100%', height: '8%', alignContent: 'center' }}>
                            <Text style={{ position: 'absolute', top: 20, start: 0, fontSize: 16, fontWeight: '700' }} >
                                Tìm kiếm hình ảnh
                            </Text>
                            <Icon
                                style={{ position: 'absolute', top: 16, end: 0 }}
                                solid name='window-close' size={28} color='black' onPress={() => setModalVisible(false)} />
                        </View>
                        {/* Content */}
    ...
    return (
        <TouchableOpacity onPress={eventPress}>
            <View style={styles.selectStyle}>
                <Text style={{ fontSize: 15, color: appcolor.black, fontWeight: '500' }} >{selectValue !== undefined ? selectValue : 'Tìm kiếm ...'}</Text>
                <Icon
