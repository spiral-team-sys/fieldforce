...
import { Platform, View, Text, TextInput, Dimensions, FlatList, TouchableOpacity, Keyboard, KeyboardAvoidingView } from "react-native";
...
import { clearAllDataStock, clearStockByCategory, getAllCategoryByProduct, getCompetitorByProduct, getlistTabByCategory, getStockProduct, updateStockItem, updateStockNote } from '../../Controller/StockOutController'
...
export const DaikinInventoryReport = ({ navigation }) => {
    ...
    const [data, setData] = useState({ arrDataShow: [], arrDataShowF: [], dataCompetitor: [], arrTagShow: [] })
    ...
    const loadDataShow = async () => {
        ...
        await setData({ arrDataShow: lstItemsProgram, arrDataShowF: lstItemsProgram, arrTagShow: lstTab, dataCompetitor: lstCompetitor })
    ...
    const filterDoneData = async () => {
        let done = !isDone
        let lstData = arrDataShow.filter(it => (it.quanity !== null && it.quanity >= 0) || it.haveBusiness !== null)
        if (done) {
    ...
    const uploadAction = async () => {
        ...
        let itemsUpload = resStock.filter(it => (it.quanity !== 'null' && it.quanity !== null) || it.haveBusiness !== null);
            ...
            for (let i = 0, lenList = itemsUpload.length; i < lenList; i++) {
                ...
                if (item.division == _competitorName) {
                    if (item.haveBusiness == 1 && item.quanity == null) {
                        ToastError(`Sản phẩm ${item.division} - ${item.productName} có kinh doanh nhưng chưa nhập số lượng`);
                        return
                    }
                }
            }
    ...
    const ViewItem = () => {
        ...
        return (
            arrTagShow.map(it => {
                dataByCategoryId = data.arrDataShow?.filter(i => i.categoryName === it.categoryName) || []
    ...
    const handleEndChangeNote = async () => {
        ...
        if (result) {
            note.noteStock?.length !== 0 && ToastSuccess('Đã lưu ghi chú.', 'Thông báo', 'top');
        } else {
    ...
    const ViewItemStock = () => {
        return (
            ...
            <InputStockItem dataTabByCompe={dataTabByCompe} data={data} arrDataShow={arrDataShow} arrDataShowF={arrDataShowF}
                arrTagShow={arrTagShow} showProgress={showProgress} Status={Status} loadDataShow={loadDataShow} reload={reload} reloadView={reloadView}
            />
            // }
            // </KeyboardAvoidingView>
        )
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            ...
            <View style={{ backgroundColor: appcolor.light, width: '100%', height: Dimensions.get('window').height - HEADER_SIZE }}>
                <FormGroup
                    containerStyle={{ backgroundColor: appcolor.surface, margin: 8, marginBottom: 0, alignSelf: 'center' }}
...
const InputStockItem = ({ data, dataTabByCompe, arrDataShowF, arrDataShow, arrTagShow, showProgress, Status, loadDataShow, reload, reloadView }) => {
    ...
    const [isClearByCate, setClearByCate] = useState({ isClearByCate: 0, isClearAll: 0, isClear: 0 })
    ...
    const clearDataByCate = (itemCategory) => {
        data.arrDataShow.map(it => {
            if (it.categoryId == itemCategory.categoryId && it.divisionId == itemCategory.competitorId) {
                it.quanity = null;
    ...
    const filterDoneData = async () => {
        let done = !isDone
        let lstData = data.arrDataShow.filter(it => (it.quanity !== null && it.quanity >= 0) || it.haveBusiness !== null)
        if (done) {
        ...
        return (
            dataTab.map(it => {
                ...
                return (
                    <Tabs.Tab key={it.categoryName} label={label} name={label} >
                        <View style={{ backgroundColor: appcolor.light, marginTop: 40, padding: 6, width: deviceWidth, display: !showProgress ? 'flex' : 'none' }}>
                            <ViewListItemStock totalRow={totalRow} dataByCategoryId={dataByCategoryId} it={it} isClearByCate={isClearByCate} />
                            <KeyboardSpacer topSpacing={Platform.OS === 'android' ? 40 : null} />
                        </View>
                    </Tabs.Tab>
                )
...
const ViewListItemStock = ({ totalRow, dataByCategoryId, it, isClearByCate }) => {
    const { appcolor, workinfo } = useSelector(state => state.GAppState)
    ...
    return (
        !loading ?
            <FlatList
                ...
                windowSize={10}
                renderItem={
                    ({ item, index }) => <RenderItemData item={item} totalRow={totalRow} index={index} isClearByCate={isClearByCate} appcolor={appcolor} workinfo={workinfo} />
                }
            />
            : null
    )
...
const RenderItemData = ({ item, isClearByCate, appcolor, workinfo, totalRow, index }) => {
    ...
    if (((!item.quanity && inputDisplay !== null && inputDisplay >= 0) || (!item.haveBusiness && haveBusiness !== null && haveBusiness >= 0))
        && isClearByCate.isClear !== 0 && (isClearByCate.isClear - countClear.countClear > 0) && isClearByCate.isClearAll === 1) {
        countClear.countClear = isClearByCate.isClear
        setInputDisplay(null)
        setHaveBusiness(null)
    }
    if (((!item.quanity && inputDisplay !== null && inputDisplay >= 0) || (!item.haveBusiness && haveBusiness !== null && haveBusiness >= 0))
        && isClearByCate.isClearByCate !== 0 && (isClearByCate.isClearByCate - countClear.countClear > 0) && isClearByCate.categoryId === item.categoryId && isClearByCate.competitorId === item.divisionId) {
    ...
    const onCheckedItem = async (action) => {
        isClearByCate.isClearAll === 1 ? (isClearByCate.isClear - countClear.countClear > 0 ? (countClear.countClear = isClearByCate.isClear) : null)
            : (isClearByCate.isClearByCate - countClear.countClear > 0 ? (countClear.countClear = isClearByCate.isClearByCate) : null)
    ...
    return (
        <View key={`stock_${item.productCode}`} style={{ width: '100%', alignItems: 'center' }}>
            <View style={{ padding: 8, width: '100%', alignItems: 'center', backgroundColor: appcolor.surface, marginBottom: 5, borderRadius: 8 }}>
