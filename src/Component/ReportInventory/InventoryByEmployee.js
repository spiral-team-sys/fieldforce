...
const InventoryByEmployee = React.forwardRef((props, ref) => {
  ...
  const [dateSelect, setDateSelect] = useState(
    ConvertToInt(moment(new Date()).format('YYYYMMDD')),
  );
  const insets = useSafeAreaInsets();
  //..
  const [itemShopChoose, setChooseShop] = useState({
    ShopName: 'Tìm kiếm cửa hàng',
    ShopId: 0,
    WorkDate: ConvertToInt(moment(new Date()).format('YYYYMMDD')),
  });
  ...
  const getDataStock = async dataChoose => {
    ...
    await GetListInventory(dataChoose, async dataResult => {
      ...
      await setDataTab(
        dataResult.length > 0 ? JSON.parse(dataResult[0].tabData) : [],
      );
    });
    setLoading(false);
  };
  ...
  const handlerFilterProduct = async value => {
    ...
    if (value && value !== null && value !== undefined && value.length > 0) {
      dataFilter = dataMain.filter(i =>
        i.productName.toLowerCase().match(value.toLowerCase()),
      );
    } else {
      dataFilter = dataMain;
    }
      ...
      <View key={'8900' + index} style={{ flex: 1 }}>
        {item.isParent && (
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <SpiralIcon name="tags" style={{ color: DEFAULT_COLOR }} />
  ...
  const renderItemShop = ({ item, index }) => {
    const isChooseStyle =
      item.isChoose == 1 ? appcolor.primary : appcolor.light;
    const onPressItem = () => {
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <TouchableOpacity onPress={() => _sheet.current.show()}>
        ...
        >
          <Text
            style={{ flexGrow: 1, color: appcolor.white, fontWeight: '600' }}
          >
            {itemShopChoose.ShopName}
          </Text>
          <SpiralIcon name="filter" color={appcolor.white} size={21} solid />
        </View>
      </TouchableOpacity>
      <View
        ...
        <Tabs.Container
          ...
          renderTabBar={props => (
            <MaterialTabBar
              ...
              tabStyle={{
                minWidth: minWidthTab(dataTab ? dataTab : 1),
                height: 42,
              }}
            />
          )}
        ...
        >
          ...
          <TouchableOpacity onPress={downloadDataByShop}>
            <View style={{ backgroundColor: appcolor.light, padding: 12 }}>
              <Text style={{ textAlign: 'center', color: appcolor.primary }}>
                Áp dụng
              </Text>
            </View>
          </TouchableOpacity>
