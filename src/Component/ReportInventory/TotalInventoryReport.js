...
import { ActivityIndicator, FlatList, Keyboard, KeyboardAvoidingView, Platform, ScrollView, Text, TextInput, TouchableOpacity, View } from "react-native";
...
import { getAllCategoryByProduct, getAllProductStock, getCompetitorByProduct, GetListWarehouse, uploadTotalInventory } from "../../Controller/StockOutController";
import { isNotInteger, Message, MessageAction, ToastError, ToastSuccess } from "../../Core/Helper";
...
export const TotalInventoryReport = ({ navigation, route }) => {
    ...
    const [data, setData] = useState({ dataInventory: [], dataMain: [], dataProduct: [], dataProductF: [], dataProductMain: [], dataCompetitor: [], dataCategory: [], isUpload: false })
    const [viewSheet, setViewSheet] = useState({ uiProvice: [], uiDistrict: [], uiWarehouse: [] })
    const [dataSelect, setDataSelect] = useState({ itemProvices: {}, itemDistrict: {}, itemWarehouse: {} })
    ...
    const loadData = async () => {
        ...
        await GetListWarehouse(async (dataResult) => {
            ...
            const dataAll = { dataInventory: dataResult, dataMain: dataResult, dataProduct: listProducts, dataProductF: listProducts, dataCompetitor: listCompetitor, dataCategory: listTab }
    ...
    const onSelect = async (item, type, dataAll) => {
        switch (type) {
            case WAREHOUSE:
                if (dataSelect.itemWarehouse?.warehouseId !== item.warehouseId || Object.keys(dataSelect.itemWarehouse).length == 0) {
                    await setLoading(true)
                    const listProducts = await getAllProductStock()
                    const isUpload = (JSON.parse(item.listStock)?.length > 0 && JSON.parse(item.listStock)[0].IsUpload == 1) ? true : false;
                    let DataStock = [...listProducts]
                    if (JSON.parse(item.listStock)?.length > 0) {
                        JSON.parse(item.listStock)?.map(it => {
                            const indexProduct = listProducts.findIndex(itm => itm.productId == it.ProductId)
                            indexProduct !== -1 && (DataStock[indexProduct].HaveBusiness = it.HaveBusiness)
                            indexProduct !== -1 && (DataStock[indexProduct].Stock = it.Stock)
                        })
                    }
                    dataSelect.itemWarehouse = item
                    SheetManager.hide('ref_warehouseSheet')
                    let dataView = { ...data, dataProduct: DataStock, dataProductF: DataStock, dataProductMain: listProducts, isUpload: isUpload }
    ...
    const uploadAction = async () => {
        Keyboard.dismiss()
        let dataDone = data.dataProductF.filter(it => (it.Stock !== null && it.HaveBusiness !== undefined) || (it.HaveBusiness !== null && it.HaveBusiness !== undefined))
        let dataUpload = []
        ...
        for (let index = 0; index < dataUpload.length; index++) {
            ...
            if ((item.Stock == undefined || item.Stock == null) && item.HaveBusiness !== null && item.HaveBusiness !== undefined && item.HaveBusiness == 1 && item.type == _competitorId) {
                const itemError = data.dataProductF.find(it => item.ProductId == it.productId && item.type == it.type)
                ToastError(`Sản phẩm ${itemError.division} - ${itemError.productName} có kinh doanh nhưng chưa nhập số lượng`);
                return
            }
        }
        Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', () => UploadData(dataUpload));
    }
    const UploadData = async (dataUpload) => {
        ...
        await uploadTotalInventory(dataUpload, async (result) => {
            if (result.status == 200 || result.statusId == 200) {
                ...
                await setDataSelect({ itemProvices: {}, itemDistrict: {}, itemWarehouse: {} })
                await setData({ dataInventory: [], dataMain: [], dataProduct: [], dataProductF: [], dataProductMain: [], dataCompetitor: [], dataCategory: [], isUpload: false })
    ...
    const filterWarehouse = async (text) => {
        if (text) {
            const newDataShow = data.dataMain.filter(it => {
                const nameWarehouse = it.wareHouseName ? it.wareHouseName.toUpperCase() : ''.toUpperCase()
                const textSearch = text.toUpperCase()
                if (nameWarehouse.indexOf(textSearch) === -1) {
                    const addressWarehouse = it.address ? it.address.toUpperCase() : ''.toUpperCase()
                        ...
                        data.dataInventory.map((it, idx) => {
                            return (
                                <TouchableOpacity
                                    ...
                                    style={{ borderRadius: 5, borderWidth: 0.5, borderColor: dataSelect.itemWarehouse.wareHouseCode == it.wareHouseCode ? appcolor.tomato : appcolor.dark, padding: 5, margin: 3, backgroundColor: appcolor.light }} key={'district_' + idx}>
                                    <Text style={{ fontSize: scaleSize(14), color: dataSelect.itemWarehouse.wareHouseCode == it.wareHouseCode ? appcolor.tomato : appcolor.dark, }}>{idx + 1}. {it.wareHouseName}</Text>
                                    <Text style={{ fontSize: scaleSize(12), color: dataSelect.itemWarehouse.wareHouseCode == it.wareHouseCode ? appcolor.tomato : appcolor.dark, fontStyle: 'italic' }}>{it.address}</Text>
                                </TouchableOpacity>
                            )
                        })
                    }
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            <HeaderCustom
                ...
                rightFunc={(Object.keys(dataSelect.itemWarehouse).length > 0 && data.isUpload === false) ? () => uploadAction() : null}
                iconMiddle='poll-h'
                middleFunc={openSheet}
            />
            <View style={{ flex: 1 }}>
                <TouchableOpacity onPress={() => handleShowSheet()}
                    style={{ backgroundColor: appcolor.surface, padding: 10, flexDirection: 'row', margin: 5, borderRadius: 5, paddingHorizontal: 10, borderWidth: 0.5, borderColor: appcolor.dark, alignItems: "center" }}
                >
                    <Icon name={'warehouse'} size={20} solid={true} type='font-awesome-5' color={appcolor.dark} />
                    <View style={{ paddingLeft: 15 }}>
                        <Text style={{ fontWeight: '600', fontSize: 14, color: appcolor.dark }} >{Object.keys(dataSelect?.itemWarehouse)?.length > 0 ? dataSelect?.itemWarehouse?.wareHouseName : 'Kho'}</Text>
                    </View>
                    {loadingSheet && <ActivityIndicator style={{ position: 'absolute', end: 8 }} />}
                </TouchableOpacity>
                {loading && <LoadingView title={'Đang tải dữ liệu...'} isLoading={loading} styles={{ marginTop: 8 }} />}
                {
                    ...
                    <ViewContent data={data} reload={reload} loading={loading} dataSelect={dataSelect} />
                }
            </View>
            <ActionSheet id={'ref_warehouseSheet'} containerStyle={{ backgroundColor: appcolor.surface }}  >
                <View style={{ height: deviceHeight / 1.5, padding: 5 }}>
                    <View style={{ width: '100%', padding: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        {/* <TouchableOpacity onPress={() => mode !== PROVINCE && handleSelectGoBack(mode)} style={{ padding: 10 }}  >
                            {mode !== PROVINCE && <Icon name={'arrow-left'} size={23} solid={true} type='font-awesome-5' color={appcolor.dark} />}
                        </TouchableOpacity> */}
                        <Text style={{ padding: 5, fontWeight: '600', fontSize: 18, color: appcolor.dark, textAlign: 'center' }}>
                            {'Danh sách kho'}
                        </Text>
                        <TouchableOpacity onPress={() => SheetManager.hide('ref_warehouseSheet')} style={{ padding: 10 }}  >
                            <Icon name={'times'} size={23} type='font-awesome-5' solid={true} color={appcolor.dark} />
                        </TouchableOpacity>
                    </View>
                    <FormGroup
                        containerStyle={{ backgroundColor: appcolor.light, marginBottom: 10, width: '95%', alignSelf: 'center' }}
                        inputStyle={{ fontSize: 13, color: appcolor.dark }}
                        placeholder='Tìm kiếm kho' editable
                        // onEndEditing={() => setDone(false)}
                        onClearTextAndroid={filterWarehouse}
                        iconName='search'
                        value={searchWarehouse} handleChangeForm={filterWarehouse}
                    />
                    <ViewSheetItem />
                </View>
            </ActionSheet>
...
const ViewContent = ({ data, reload, loading, dataSelect }) => {
    ...
    const loadCurrentTab = async () => {
        const dataTab = data.dataCategory.filter(it => it.competitorId == _competitorId)
        await setDataTabByCompe(dataTab)
    }
        ...
        return (
            <KeyboardAvoidingView
                ...
                keyboardVerticalOffset={Platform.OS === "ios" ? 64 : 0}>
                {
                    ...
                    <InputStockItem data={data} tabRef={tabRef} loading={loading} dataTabByCompe={dataTabByCompe} />
                }
            </KeyboardAvoidingView>
        )
    }
    const setClearAll = async () => {
        if (!data.isUpload) {
            MessageAction(`Bạn có chắc chắn muốn xóa hết dữ liệu đã nhập ?`,
                async () => {
    ...
    const handlerClearByCategory = async itemCategory => {
        if (!data.isUpload) {
            MessageAction(`Bạn có muốn xoá dữ liệu ngành hàng ${itemCategory.categoryName} thuộc hãng ${itemCategory.competitorName} đã nhập không ?`, async () => {
                data.dataProduct.map(it => {
                    if (it.categoryId == itemCategory.categoryId && it.type == itemCategory.competitorId) {
                        it.Stock = null;
                        it.HaveBusiness = null
                    }
                })
    ...
    const filterDoneData = async () => {
        let done = !isDone
        let lstData = data.dataProduct.filter(it => (it.Stock !== null && it.Stock !== undefined && it.Stock >= 0) || (it.HaveBusiness !== null && it.HaveBusiness !== undefined))
        if (done) {
            ...
            <ActionSheet
                ...
                <View style={{ padding: 8, width: '100%' }}>
                    <ToolsAction
                        ...
                        isLock={(Object.keys(dataSelect.itemWarehouse).length > 0 && data.isUpload === false)}
                    />
                </View>
            </ActionSheet>
        </View>
    ...
    const ViewItem = () => {
        ...
        return (
            dataTab.map((it, indexCate) => {
                dataByCategoryId = data.dataProduct?.filter(i => i.categoryName === it.categoryName && i.type === it.competitorId) || []
                ...
                return (
                    dataByCategoryId.length > 0 ?
                        <Tabs.Tab key={it.categoryName} label={label} name={label} >
                            ...
                            <View key={it.categoryName + indexCate} style={{ backgroundColor: appcolor.light, marginTop: 40, padding: 6, width: deviceWidth, display: 'flex' }}>
                                <FlatList
                                    ...
                                    windowSize={10}
                                    renderItem={
                                        ({ item, index }) => <RenderItemData item={item} totalRow={totalRow} index={index} appcolor={appcolor} isUpload={data.isUpload} />
                                    }
                                />
        ...
        <View style={{ flex: 1 }}>
            {
                ...
                <Tabs.Container
                    ...
                    containerStyle={{ backgroundColor: appcolor.surface, height: deviceHeight }}>
                    {ViewItem()}
                </Tabs.Container>
            }
        </View>
...
const RenderItemData = ({ item, isClear, appcolor, totalRow, index, isUpload }) => {
    ...
    if ((!inputDisplay && item.Stock)) {
        ...
        setInputDisplay(item.Stock)
    } else if (!item.Stock && isClear !== 0 && inputDisplay == 0 && isClear - countClear > 0) {
        setInputDisplay(null);
    }
    const onCheckedItem = async (action) => {
        let isChecked = null;
        if (action === 'yes')
            isChecked = item.HaveBusiness == 1 ? null : 1;
        else
            isChecked = item.HaveBusiness == 0 ? null : 0;
        //
    ...
    return (
        <View key={`stock_${item.productCode}`} style={{ width: '100%', }}>
            <View style={{ padding: 5, width: '100%', alignItems: 'center', backgroundColor: appcolor.surface, marginBottom: 3, borderRadius: 8 }}>
                    ...
                    {(item.type == _competitorId && haveBusiness === 1) &&
                        <NumberFormat
                            ...
                            renderText={value =>
                                <TextInput
                                    style={{
                                        padding: 8, fontSize: 12, color: appcolor.dark, backgroundColor: appcolor.light, fontWeight: '500',
                                        width: '30%', textAlign: 'center', borderRadius: 5, borderWidth: 0.3, borderColor: appcolor.greylight
                                    }}
                                    value={value}
                ...
                <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'center', backgroundColor: appcolor.light, borderRadius: 5 }}>
                    <CheckBox
                        checkedColor={appcolor.success}
                        disabled={isUpload}
                        containerStyle={{ padding: 0, margin: 3, backgroundColor: appcolor.light, borderWidth: 0 }}
                        // title={item.type == _competitorId ? 'Có kinh doanh' : 'Còn hàng'}
                        title={'Có kinh doanh'}
                        textStyle={{ padding: 0, margin: 0, fontSize: 13, color: appcolor.greylight, fontWeight: '500' }}
