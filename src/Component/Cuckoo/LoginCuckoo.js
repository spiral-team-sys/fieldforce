...
class LoginCuckoo extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            showProgress: false,
    ...
    async SignIn() {
        ...
        await onLogin(requestInfo, (res) => {
            saveTokenUserToGapp(res, this.props.GAppController, JSON.stringify(requestInfo)).then(() => {
                    ...
                    this.state.requestPass ?
                        ...
                        <View style={{ justifyContent: 'space-between', alignItems: 'stretch', flex: 1, backgroundColor: appcolor.primary }}>
                            <View style={{ height: '100%', width: '100%' }}>
                                <View style={{ height: '27%', paddingTop: 40 }}>
                                    <Text style={{ marginLeft: 12, width: '100%', fontWeight: "bold", fontSize: scaleSize(32), color: appcolor.white }}>Đăng nhập {APPNAME.toLocaleLowerCase()}</Text>
                                    <View style={{ padding: 17 }}>
                                        <TouchableOpacity onPress={() => this.setState({ showModel: true })}>
                                            <Text style={{ color: appcolor.white, fontSize: scaleSize(12) }}>Bằng việc đăng nhập bạn đã đồng ý với
                                                <Text style={{ color: appcolor.white, fontWeight: '500', textDecorationLine: 'underline', fontStyle: 'italic', fontSize: scaleSize(14) }}> Điều khoản & điều kiện </Text> cùng
                                                <Text style={{ color: appcolor.white, fontWeight: '500', textDecorationLine: 'underline', fontStyle: 'italic', fontSize: scaleSize(14) }}> chính sách bảo mật chia sẻ thông tin của Spiral</Text>
                                            </Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                                <View style={{ flexGrow: 1, paddingLeft: 22, paddingRight: 22, borderTopRightRadius: 40, borderTopLeftRadius: 40, backgroundColor: appcolor.light }}>
                                    ...
                                    <View style={{ backgroundColor: appcolor.transparent }}>
                                        <TouchableOpacity style={{ padding: 12, alignItems: 'flex-end' }} onPress={() => this.setState({ requestPass: true })} >
                                            <Text style={{ color: appcolor.dark, fontStyle: 'italic', textDecorationLine: 'underline' }}>Lấy lại mật khẩu ?</Text>
                                        </TouchableOpacity>
                                    </View>
                                    ...
                                    <View style={{ alignSelf: 'center', }}>
                                        {
                                            this.state.isSecLocal > 0 && <TouchableOpacity
                                                style={{ flexDirection: 'row', alignItems: 'center', padding: 0, backgroundColor: appcolor.primary, borderRadius: 30 }}
                                ...
                                <Modal animationType="slide" visible={this.state.showModel || false}>
                                    <WebViewUI onClose={() => this.setState({ showModel: false })}
                                        ...
                                        urlPage={"https://cuckoo.spiral.com.vn/privacy.html?"} />
                                </Modal>
                            </View>
                            {
                                this.state.showProgress === true &&
                                <Progress.CircleSnail color={appcolor.primary} thickness={5} size={85}
                                    indeterminate={true} style={{
                                        position: 'absolute', zIndex: 7, alignSelf: "center",
...
export default connect(mapStateToProps, mapDispathToProps)(LoginCuckoo)
