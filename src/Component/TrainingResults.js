...
const TrainingResults = ({ navigation, route }) => {
    ...
    const loadData = async () => {
        ...
        await setLinkTraining(route?.params?.shopinfo.linkTraining + route?.params?.workinfo.guiid)
        RNQRGenerator.generate({
            value: route?.params?.shopinfo.linkTraining + route?.params?.workinfo.guiid,
            height: 300,
            width: 300,
        }).then(async response => {
            const { uri } = response;
    ...
    const styles = StyleSheet.create({
        ...
        bgQRStyle: {
            width: '95%', height: '96%', borderRadius: 10, borderWidth: 0.5, borderColor: appcolor.greylight, backgroundColor: appcolor.white, alignSelf: 'center',
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <Tabs.Container
                ...
                <Tabs.Tab key={'Training Results'} label={'Training Results'} name={'Training Results'} >
                    <View style={{ backgroundColor: appcolor.light, marginTop: 40, padding: 6, width: deviceWidth }}>
...
const RenderViewQRCode = ({ QRValue, LinkTraining, style, copyLink }) => {
    return (
        <View style={style.bgQRStyle}>
            <Image style={{ width: 300, height: 300, margin: 42 }} source={{ uri: QRValue }} />
            <View style={{ width: '100%', flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                <Text style={{ width: '78%' }} >{LinkTraining}</Text>
                <View style={{ marginStart: 8, borderWidth: 0.5, borderRadius: 50, padding: 12 }}>
                    <Icon solid name='copy' size={18} onPress={copyLink} />
                </View>
            </View>
        </View>
    )
        ...
        <View style={style.bgQRStyle}>
            <ScrollView
                ...
                <WebView
                    ...
                    onMessage={(event) => { }}
                    source={{ uri: TRAINING_RESULT + EmployeeCode + '-' + today + '/' + Guiid }
                    } />
            </ScrollView>
        </View>
    )
