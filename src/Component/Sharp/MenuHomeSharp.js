...
export const MenuHomeSharp = ({ navigation, isLoading, downloadData }) => {
    ...
    const renderItem = ({ item, index }) => {
        return (
            <TouchableOpacity key={"hk3d2-" + index} style={{
                width: deviceWidth * 0.44, height: deviceWidth * 0.30, marginRight: 12, borderRadius: 20,
                backgroundColor: appcolor.surface, marginBottom: 10
            }} onPress={() => onItemPress(item)}>
                <View style={{ width: '100%', height: '100%', padding: 12 }}>
                    <Icon containerStyle={{ alignItems: 'flex-start' }} name={item.iconName} color={appcolor.primary}
                        type={item.iconType || "font-awesome-5"} size={38} />
                    <View style={{ flexGrow: 1 }} />
                    <View style={{ width: '100%', bottom: 10, alignItems: 'flex-end' }}>
                        <Text style={{ color: appcolor.dark, fontWeight: '700' }}>{item.menuNameVN}</Text>
                        <Text style={{ color: appcolor.dark, opacity: 0.3, fontWeight: '600', fontSize: scaleSize(9), }}>{item.menuName}</Text>
                    </View>
                </View>
            </TouchableOpacity>
        )
        ...
        <View style={styles.mainContainer}>
            {isLoading && <LoadingView isLoading={isLoading} title=' ' />}
            {
                ...
                <View style={{ width: deviceWidth, paddingLeft: 10 }}>
                    <Text style={{ fontSize: 20, fontWeight: '500', color: appcolor.primary, padding: 4 }}>Chức năng</Text>
    ...
    return (
        <View style={{
            ...
            {data.map((_, index) => {
                ...
                return data.length / 2 > index ? <Animated.View key={index} style={{
                    height: 8,
                    borderRadius: 4,
