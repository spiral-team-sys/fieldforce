...
import { View, FlatList, TouchableOpacity, RefreshControl, ScrollView, Platform, SafeAreaView } from "react-native"
...
export const ProductList = ({ navigation, route }) => {
    ...
    const renderGroup = ({ item, index }) => {
        ...
        return (
            <View key={`${index}09ll`} style={{ flex: 1, marginLeft: 3, marginBottom: 3, alignItems: 'center', padding: 12 }}>
                <TouchableOpacity onPress={() => onShowItem(item?.products)}>
                    ...
                    />
                    <Text style={{ textAlign: 'center', color: appcolor.dark, fontSize: 14, fontWeight: '500', paddingTop: 5 }}>{item.categoryName || 'N/A'}</Text>
                </TouchableOpacity>
            </View>
        )
        ...
        return (
            <View
                ...
                style={{
                    ...
                    flex: 1, flexDirection: 'row', marginBottom: 3, paddingBottom: 5, alignItems: 'center',
                    borderBottomWidth: 0.5, borderBottomColor: appcolor.grayLight
                }}>
                <Text style={{ flex: 1.5, fontSize: 14, fontWeight: '400', color: appcolor.dark, textAlign: 'center' }}>{`${index + 1}. `}</Text>
                <View style={{ flex: 6, backgroundColor: appcolor.light, flexDirection: 'column' }}>
                    <Text style={{ fontSize: 15, fontWeight: '600', color: appcolor.dark }}>{`${item.productName}`}</Text>
                    <Text style={{ fontSize: 13, fontWeight: '400', color: appcolor.greydark }}>{`${item.productCode}`}</Text>
                    <Text style={{ fontSize: 13, fontWeight: '400', color: appcolor.greydark }}>{`${item.subCategory || ''} ${item.segment || ''}`}</Text>
                </View>
                <Text style={{ flex: 2.5, fontSize: 14, fontWeight: '500', color: appcolor.dark, textAlign: 'center' }}>
                    {`${formatNumber(item.price, ',') || 0}`}</Text>
            </View>
        )
    }
    ...
    const renderItemCompetitor = ({ item, index }) => {
        ...
        return (
            <View key={`${index}amal`}>
                <View style={{ padding: 8, flexDirection: 'row', backgroundColor: appcolor.light }}>
                    <Text style={{ fontWeight: '600', color: appcolor.dark, fontSize: 16, marginHorizontal: 5 }} >
                        {(item.name || 'ProductName').toUpperCase()}
                    </Text>
                    <Text style={{ fontSize: 10, color: appcolor.dark }}>{item?.listProduct?.length || 0}</Text>
                </View>
                <View style={{ backgroundColor: appcolor.light, marginStart: 8, marginEnd: 8, borderRadius: 10, marginTop: 7 }} >
                    <FlatList
                        numColumns={3}
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.surface }}>
            ...
            />
            <LoadingView title={'Đang cập nhật dữ liệu sản phẩm'} isLoading={refreshing} styles={{ marginTop: 8 }} />
            <FlatList
                key={'headerproduct'}
                keyExtractor={(_, index) => index.toString()}
                data={data}
            ...
            />
            <ActionSheet id="sheetProduct" drawUnderStatusBar={Platform.OS == 'ios'} >
                <SafeAreaView style={{ width: deviceWidth, height: deviceHeight, }}>
                    <FormGroup
                        containerStyle={{ backgroundColor: appcolor.light, margin: 7, alignSelf: 'center' }}
                    ...
                    />
                    <TouchableOpacity onPress={() => SheetManager.hide("sheetProduct")}
                        style={{ position: 'absolute', bottom: deviceHeight / 12, alignSelf: 'center' }}>
