...
import { View, ScrollView, Text, TextInput, FlatList, Dimensions, StyleSheet } from "react-native";
import PageHeader from '../Content/PageHeader';
import { KeyboardAwareScrollView } from 'react-native-keyboard-aware-scroll-view'
import { getStatusCompetitorResult, getUploadDisplayCompetitorResult, updateStatusCompetitorResult, uploadDisplayCompetitor, insertCompetitorResult } from '../Controller/WorkController'
...
class CompetitorReport extends PureComponent {
    ...
    async CompetitorAutoMap(lstDivision, CompetitorRes) {
        ...
        lstDivision.forEach(item => {
            if (!CompeMap[item.name]) {
                CompeMap[item.name] = [];
                let itemRes = (!Array.isArray(CompetitorRes)) ? [] : CompetitorRes.filter((itemRe) => itemRe.divisionId === item.id);
                let itemPush = {
                    ...item,
                ...
                if (itemRes.length > 0) {
                    itemRes.map(itemR => {
                        itemPush = {
                            ...itemPush, quantity: itemR.quantity == 'undefined' ? '' : itemR.quantity,
                            upload: 0
                        };
                    });
                }
    ...
    async loadData() {
        ...
        this.setState({ workinfo: workinfo, Status: status, Division: division, CompetitorResult: CompetitorRes, lstShow: await this.CompetitorAutoMap(division, CompetitorRes) });
    ...
    async uploadAction() {
        let competitorRes = await getUploadDisplayCompetitorResult(this.state.workinfo);
        if (competitorRes.length == 0) {
            alert('Bạn chưa làm báo cáo.')
            return
        }

        Message('Chú ý', 'Sau khi gửi dữ liệu sẽ không thể điều chỉnh, Bạn có chắc chắn không ?', () => this.UploadAction(competitorRes));
    }
        ...
        return (
            <View style={{ height: '100%', width: '100%' }}>
                <PageHeader
                    ...
                    rightclick={(this.state.Status === 1) ? () => { alert('Báo cáo này đã hoàn thành.') } : () => this.uploadAction()}
                >
                </PageHeader>
                <ScrollView style={{ height: '68%', zIndex: 0, backgroundColor: 'white' }}>
                    <KeyboardAwareScrollView
                        ...
                        <View>
                            <FlatList
                                ...
                                data={lstShowAll}
                                renderItem={({ item }) => (<RenderRow item={item} reload={() => this.refreshView()} WordInfo={WordInfo} Status={this.state.Status}></RenderRow>)}
                                numColumns={1}
                            />
                        </View>
                    </KeyboardAwareScrollView>
                ...
                {
                    this.state.showProgress === true && <Progress.Circle thickness={1} size={65} indeterminate={true} style={{ position: 'absolute', alignSelf: "center", marginTop: Dimensions.get('window').height / 2 }} />
                }
            </View>
        );
    }
...
const RenderRow = ({ item, WordInfo, reload, Status }) => {
    const [quantity, setQuantity] = useState((item.quantity !== 'undefined' && item.quantity !== null && item.quantity !== undefined) ? item.quantity + '' : '');
    return (
        <View style={{ height: 70, width: '100%', padding: 5, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 15, paddingRight: 15, alignItems: 'center' }}>
            ...
            <View style={{ width: '40%', alignSelf: 'center' }}>
                <TextInput
                    ...
                    style={{ fontSize: 15, color: 'black', minHeight: 37, textAlign: 'center', borderWidth: 0.6, borderColor: 'gray', width: '98%', backgroundColor: Status === 0 ? 'white' : 'lightgray' }}
