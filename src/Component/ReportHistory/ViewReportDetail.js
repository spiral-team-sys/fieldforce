...
class ViewReportDetail extends PureComponent {
    constructor(props) {
        ...
        this.state = {
            ...
            markingType: 'custom',
            markedDates: { [moment(new Date()).format('YYYY-MM-DD').toString()]: { selected: true, marked: true, selectedColor: 'blue' } },
            isStartDay: false,
            isEndDay: false,
            startDate: '',
            dateFilter: [],
        ...
        this.onPage = 1;
        this.markedDatesDefault = { [this.state.toDay]: { selected: true, marked: true, selectedColor: 'blue' } }
    ...
    selectItemReport = (rowData) => {
        ...
        if (KPIName === mOOS)
            this.setState({ showOOS: !this.state.showOOS })
    ...
    handlerViewChange = async ({ viewableItems }) => {
        viewableItems.forEach(item => {
            if (!this.state.fetchDataServer) {
                ...
                if (index === getData) {
                    this.setState({ fetchDataServer: true }, () => this.DATA_FetchFromServer())
                }
            }
        })
    }
        ...
        try {
            ...
            fetch(URL_GET_HISTORY_REPORT, {
                ...
                .then((responseJson) => {
                    ...
                    this.setState({ dataRenderAll: dataRender, lstHistoryReport: dataRender, isLoadingData: false, fetchDataServer: false })
    ...
    handlerSortData = (responseJson) => {
        if (responseJson.length > this.state.lstHistoryReport.length) {
            this.onPage = this.onPage + 1;
            this.setState({ dataRenderAll: responseJson, lstHistoryReport: responseJson })
        ...
        return (
            <View>
                {this.state.fetchDataServer ?
                    <ActivityIndicator style={{ padding: 8 }} /> : null
                }
            </View>
        );
    }
    ...
    REPORT_renderItem = ({ item }) => {
        return (
            <Card containerStyle={{ borderRadius: 8, flexDirection: 'column', borderWidth: 0, padding: 8, margin: 8 }}>
                <View style={styles.styleRow}>
                    <Avatar
                        rounded size='small'
                        size={28}
                        icon={{ name: 'home', type: 'material-community', color: 'white' }}
                        overlayContainerStyle={{ flex: 1, backgroundColor: appcolor.greydark }}
                    />
                    <View style={{ flex: 1 }}>
                        <Text style={{ marginStart: 8, fontSize: 13, fontWeight: '700' }}>{item.employeeName}</Text>
                        <Text style={{ marginStart: 8, fontSize: 10 }}>{item.shopName}</Text>
                        <Text style={{ marginStart: 8, fontSize: 10 }}>{item.dateView}</Text>
                    </View>
                    <Button
                        type='clear'
                        title='Báo cáo'
                        iconRight
                        icon={<Icon name='arrow-right-circle' type='material-community' color={appcolor.yellow} />}
                        titleStyle={{ marginEnd: 3, fontSize: 12, color: appcolor.greydark }}
                ...
                <View style={{ justifyContent: 'center' }}>
                    <FBCollage
                        ...
                        imageOnPress={() => this.props.navigation.navigate('ViewDetailPhoto', { dataPhoto: JSON.parse(item.dataPhoto), shopName: item.shopName })}
                    />
                </View>
            </Card>
        );
    ...
    MODAL_DETAIL_renderItem = ({ item }) => {
        return (
            <View>
                <View style={{ flexDirection: 'row', backgroundColor: DEFAULT_COLOR, padding: 8, marginBottom: 5, alignItems: 'center' }}>
                    <Text style={{ flex: 1, margin: 0, color: 'white', fontSize: 13, fontWeight: '700' }}>{item.KPIName}</Text>
                    <Icon style={{ flex: 1 }} name='plus' type='material-community' color='white' size={20} onPress={() => this.selectItemReport(item)} />
                </View>
                {item.Name == mSellOut && this.state.showSellOut ? <ReportSellOut dataReport={item.ReportSellOut} /> : null}
                {item.Name == mDisplay && this.state.showDisplay ? <ReportDisplay dataReport={item.ReportDisplay} /> : null}
                {item.Name == mOOS && this.state.showOOS ? <ReportOOS dataReport={item.ReportOOS} /> : null}
                {item.Name == mInventory && this.state.showInventory ? <ReportInventory dataReport={item.ReportInventory} /> : null}
            </View>
        );
    }
    ...
    render() {
        return (
            <View style={{ flex: 1 }}>
                ...
                <ProgressCircleSnail Title="Đang cập nhật dữ liệu" isShowing={this.state.isLoadingData} />
                ...
                <Modalize ref={DETAIL_modalizeRef} modalHeight={500} onOverlayPress={this.handler_REPORT_UNSHOW}>
