...
import { View, Text, Platform, TouchableOpacity, TextInput, Keyboard, SafeAreaView } from "react-native";
...
const LoginHFL = ({ onLoginCallBack }) => {
    ...
    return (
        <SafeAreaView style={{ width: '100%', height: '100%', backgroundColor: appcolor.light }} >
            <LottieView style={{ width: '100%', height: '50%' }} autoPlay source={require('../../Themes/lotties/sign-hfl.json')} />
            <ActionSheet overlayColor={appcolor.trasparent} closeOnTouchBackdrop={false} id="login">
                <Text style={{ padding: 7, textAlign: 'center', color: appcolor.dark, fontSize: 11 }}>{'Version: ' + versionApp}</Text>
                {requestPass ?
                    <ForgotPassword
                    ...
                    <View style={{ padding: 12, backgroundColor: appcolor.light }}>
                        ...
                        <View style={{ backgroundColor: appcolor.light, flexGrow: 1 }}>
                            <Text style={{ fontWeight: '500', color: appcolor.dark, marginBottom: 12 }}>Xin chào,
                                <Text style={{ fontSize: 13, color: appcolor.primary }}> Đăng nhập để làm việc</Text>
                            </Text>

                            <View style={{}}>
                                <View style={{
                                    flexDirection: 'row', marginBottom: 10, borderColor: Ufocus === 1 ? appcolor.primary : appcolor.dark,
                                    borderRadius: 30, borderWidth: Ufocus === 1 ? 0.7 : 0.1
                                }}>
                                    <Icon containerStyle={{ top: 0, bottom: 0 }} name='person' reverse color={appcolor.primary} size={17} />
                                    <TextInput
                                        style={{
                                            flexGrow: 1, fontSize: 12, color: appcolor.primary, padding: 8
                                        }} autoCorrect={false}
                                        returnKeyType='next'
                                        onBlur={() => setUFocus(0)}
                                        onFocus={() => setUFocus(1)}
                                ...
                                <View style={{
                                    flexDirection: 'row', marginBottom: 10, borderColor: Pfocus === 1 ? appcolor.primary : appcolor.dark,
                                    borderRadius: 30, borderWidth: Pfocus === 1 ? 0.7 : 0.1
                                }}>
                                    <Icon containerStyle={{ top: 0, bottom: 0 }} name='lock' reverse color={appcolor.primary} size={17} />
                                    <TextInput
                                        style={{
                                            flexGrow: 1, color: appcolor.primary, fontSize: 12, padding: 8
                                        }} autoCorrect={false}
                                        ...
                                        placeholderTextColor={appcolor.grayLight}
                                    />
                                    <TouchableOpacity onPress={(e) => setShowPass(e => !e)} style={{
                                        position: 'absolute',
                                        right: 20, top: 18, bottom: 0
                                    }} >
                                        <Icon color={appcolor.primary}
                                            type="font-awesome-5" name={isShowPass ? 'eye' : 'eye-slash'} size={18} />
                                    </TouchableOpacity>
                                </View>
                            </View>
                            <View>
                                <TouchableOpacity onPress={handlerFogotPassword} >
                                    <Text style={{
                                        width: '100%', padding: 16, fontSize: 12, fontWeight: "600", color: appcolor.dark, textAlign: 'right',
                                        fontStyle: 'italic', textDecorationLine: 'underline'
                                    }}>Quên mật khẩu?</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    style={{ padding: 12, borderRadius: 20, alignSelf: 'center', backgroundColor: appcolor.primary }}
                                    onPress={handlerLogin}>
                                    <Text style={{ color: appcolor.white, paddingLeft: 12, paddingRight: 12 }}>Đăng nhập</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
