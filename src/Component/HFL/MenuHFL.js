...
export const MenuHFL = ({ navigation, menus }) => {
    ...
    const menuItem = ({ item, index }) => {
        return (
            <TouchableOpacity key={"hk3d2-" + index} style={{
                width: deviceWidth * 0.45, height: deviceWidth * 0.3, marginRight: 12, borderRadius: 20,
                backgroundColor: index == 0 ? appcolor.primary : appcolor.light, marginBottom: 10
    ...
    return (
        <View style={{ backgroundColor: appcolor.surface, padding: 12, paddingTop: 2 }}>
            <FlatList data={dataMenu}
                numColumns={2}
                renderItem={menuItem} />
