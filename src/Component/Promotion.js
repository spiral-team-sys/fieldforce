...
const Promotion = ({ navigation }) => {
    ...
    const mapItem = useCallback(async (item) => {
        ...
        setSellouts(prev => prev.map(s => s.guiId === item.guiId ? { ...s, numPhoto: lst.length } : s));
    ...
    const promotionLoad = useCallback(() => {
        Store().then(async () => {
            ...
            setSellouts(lst);
            lst.forEach(item => { if (item.guiId != null) mapItem(item); });
        });
    }, [workinfo, mapItem]);
                ...
                <View style={{ flex: 1 }}>
                    ...
                    {showProgress && (
                        <Progress.Circle
                            ...
                            size={90}
                            indeterminate
                            style={{ position: 'absolute', alignSelf: 'center', marginTop: Dimensions.get('window').height / 2 }}
                        />
                    )}
                    <Icon
                        iconStyle={{ color: DEFAULT_COLOR }}
                        onPress={handleAddNew}
                        containerStyle={{ position: 'absolute', zIndex: 10, right: 20, bottom: 40, maxHeight: 40 }}
                        size={40}
