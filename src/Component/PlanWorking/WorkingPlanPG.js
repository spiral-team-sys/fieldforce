...
import { Image, ListItem, Divider, Icon } from '@rneui/themed';
...
import { groupDataByKey, debounce, ToastError, UUIDGenerator } from '../../Core/Helper'
...
const WorkingPlanPG = ({ navigation, route }) => {
        ...
        try {
            if (data.length > 0) { // * Save
                for (let i = 0, lenData = data.length; i < lenData; i++) {
                    const { shiftType, workDate, shopName, notes, shiftGroup, isNoteCopy } = data[i]
                    console.log(isNoteCopy, shiftGroup);
                    ...
                    if (isNoteCopy == 1 && shiftGroup == 'OFF' && (notes || '').length < 5) {
    ...
    const renderItem = ({ item, index }) => {
        const { workDate, shopName, address, shiftTypeName, shiftChange, shiftChangeName, checkIn1, checkOut1, checkIn2, checkOut2, checkIn3, checkOut3, totalTime, isParent, } = item
        ...
        return (
            <View key={index} style={{ width: '100%', alignSelf: 'center', marginBottom: isParent ? 8 : 0 }}>
                ...
                <View style={{ backgroundColor: appcolor.light, borderRadius: 10, padding: 10, margin: 8 }}>
                    ...
                    {item.confirmPlan !== 1 && item.isNoteCopy == 1 &&
                        ...
                        <View style={styles.expandItem}>
                            <View style={{ width: '95%' }}>
                                ...
                                <RenderTextNote defaultValue={item.notes} index={index} item={item} editable={isShiftEditable} onChangeNote={onChangeNote} appcolor={appcolor}
                                    keyEdit={"notes"} keyForgot={"forgotNoteShift"} />
                                {isShiftEditable &&
                                    <TouchableOpacity onPress={() => handleSubmit(SUBMIT_SHIFT, item)}
                                        style={{
                                            height: 40, marginTop: 10, backgroundColor: appcolor.primary, borderRadius: 10,
                                            justifyContent: 'center', alignItems: "center",
                                        }}>
                                        <Text style={{ textAlign: 'center', fontSize: 16, color: appcolor.light }}>Xin đổi ca</Text>
                                    </TouchableOpacity>
                                }
                            </View>
                        </View>
                        ...
                        <View style={styles.expandItem}>
                            <View style={{ width: '95%' }}>
                                ...
                                <View style={{ flexDirection: 'row', width: '100%', position: 'relative' }}>
                                    <View style={{ width: '100%', marginBottom: 10, }}>
                                        <RenderTextMinute index={index} item={item} keyForgot={"forgotTimeLate"} value={item.timeLate?.toString()} styles={styles} appcolor={appcolor}
                                            onChangeMinute={onChangeMinute} keyBusy={"timeLate"} iconName="alarm-off" isBusyEditable={isLateEditable} />
                                    </View>
                                    {isLateEditable &&
                                ...
                                <View style={{ flexDirection: 'row', width: '100%', position: 'relative' }}>
                                    <View style={{ width: '100%', marginBottom: 10 }}>
                                        <RenderTextMinute index={index} item={item} keyForgot={"forgotTimeEarlier"} value={item.timeEarlier?.toString()} styles={styles} appcolor={appcolor}
                                            onChangeMinute={onChangeMinute} keyBusy={"timeEarlier"} iconName="alarm-on" isBusyEditable={isEarlierEditable} />
                                    </View>
                                    ...
                                    keyEdit={"noteEarlier"} keyForgot={"forgotNoteEarlier"} />
                                {
                                    // isShiftEditable &&
                                    isEarlierEditable &&
                                    <TouchableOpacity onPress={() => handleSubmit(SUBMIT_EARLIER, item)} style={{ height: 40, marginTop: 10, backgroundColor: appcolor.primary, borderRadius: 10, justifyContent: 'center', alignItems: "center", }}>
                    ...
                    {disableCopyButton && item.photoAttendant > 4 &&
                        <View style={{ width: '100%', flexDirection: 'row', alignSelf: 'center' }}>
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.homebackground }}>
            <HeaderCustom
                title={route?.params?.titlePage || "Lịch làm việc PG"}
            ...
            {currentWeek.label !== undefined && (
                <View style={{ flexDirection: 'row', alignItems: 'center', margin: 3, }}>
                    <TouchableOpacity onPress={() => handleDisplayBS(WEEK)} style={{
            ...
            <LoadingView title="Đang cập nhật dữ liệu" isLoading={refreshing} />
            ...
            <Modal animationType="slide" visible={visibleBS}>
                <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS == "ios" ? "padding" : 'height'}>
                    <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.surface, padding: 10, overflow: 'hidden' }}>
            ...
            <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center', borderRadius: 5, backgroundColor: appcolor.light }}>
                {imageURL ?
                    <TouchableOpacity style={{ width: '90%', height: 120, borderRadius: 5 }} onPress={showPhoto}>
                        <Image source={{ uri: imageURL?.includes('https://') ? imageURL : URLDEFAULT + imageURL }} style={{ width: '100%', height: 120, borderRadius: 5 }} />
                    </TouchableOpacity>
                    :
