...
const PlanSR = ({ navigation, route }) => {
    ...
    const handlerNotes = async (index, text, item, typeInput) => {
        switch (typeInput) {
            ...
            case 'SHOP':
                ...
                data.dataMain[index].notes = text
                break;
            case 'LATE':
                data.mPlanList[index].noteLate = text
                data.dataMain[index].noteLate = text
                break;
            case 'EARLIER':
                data.mPlanList[index].noteEarlier = text
                data.dataMain[index].noteEarlier = text
                break;
        }
    }
    const handlerShopAction = async (item, index, value) => {
        ...
        data.dataMain[index].status = statusChange
        data.dataMain[index].lastStatus = lastStatus
        data.dataMain[index].confirmShop = 3
        assignData(index)
        setMutate(e => !e)
    }
    ...
    const chooseItemModal = async (item, index, value, typeModal) => {
        ...
        if (typeModal == 'SHIFT') {
            ...
            let listIndex = JSON.parse(data.dataMain[0].listIndex)
    ...
    const styles = StyleSheet.create({
        ...
        itemOFFView: { flex: 1, marginBottom: 8, backgroundColor: appcolor.grayLight, padding: 8, borderRadius: 8 },
        itemShopView: { flex: 1, marginBottom: 8, backgroundColor: appcolor.light, padding: 8, borderRadius: 8, borderWidth: 0.5, borderColor: appcolor.greydark },
        styleSwitchView: { ...(Platform.OS === 'ios' && { transform: [{ scaleX: .8 }, { scaleY: .8 }] }) },
        inputView: { flex: 1, color: appcolor.dark, padding: 8, marginTop: 8, borderRadius: 8, borderWidth: 0.5, fontSize: 14, borderColor: appcolor.greydark, textAlignVertical: 'center' },
        titleShiftType: { flex: 1, fontSize: 15, fontWeight: '700', fontStyle: "italic" },
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <ReactNativeCalendarStrip
                ...
                minDate={'2023-01-01'}
                calendarHeaderStyle={{ color: appcolor.white, marginBottom: 16 }}
                calendarColor={DEFAULT_COLOR}
                dateNumberStyle={{ color: appcolor.white }}
                dateNameStyle={{ color: appcolor.white }}
                customDatesStyles={configDate.customDatesStyles}
                highlightDateContainerStyle={{ backgroundColor: appcolor.white }}
                highlightDateNumberStyle={{ color: appcolor.dark }}
                highlightDateNameStyle={{ color: appcolor.dark }}
                disabledDateNameStyle={{ color: appcolor.grey }}
                disabledDateNumberStyle={{ color: appcolor.grey }}
...
const ViewItemOFF = ({ item, index, styles, visibleView, actionNotes, actionChooseItem, appcolor }) => {
    ...
    return (
        <View key={index} style={[styles.itemOFFView, { display: visibleView }]}>
            {/* <Text style={styles.titleShiftType}>{'Ca hiện tại: ' + item.shiftName}</Text> */}
...
const ViewItemStore = ({ item, index, styles, onValueChange, actionNotes, appcolor }) => {
    ...
    const colorStatusChange = item.confirmShop == 1 ? appcolor.green : item.confirmShop == -1 ? appcolor.danger : appcolor.rejection
    ...
    return (
        <View key={index} style={styles.itemShopView}>
            ...
            <Text style={{ display: visibleSupNoteConfirm, width: '100%', fontSize: 14, fontWeight: '700', color: appcolor.danger }}>Quản lý ghi chú: {item.confirmPlanNote}</Text>
            <View style={{ flex: 8, flexDirection: 'row', alignItems: 'center' }}>
                ...
                {item.isLocked == 0 &&
                    <Switch
                        ...
                        trackColor={{ true: appcolor.green, false: appcolor.greylight }}
...
const renderItemModal = (item, index, dataModal, actionChooseItem, actionClearItem, appcolor) => {
    ...
    return (
        <View key={index} style={{ flexDirection: 'row', backgroundColor: appcolor.grayLight, marginBottom: 8, borderRadius: 8 }}>
...
const ModalItem = ({ dataModal, appcolor, styles, actionSearch, actionResultModal, actionChooseItem, actionClearItem }) => {
    const titleModal = dataModal.type == 'SHIFT' ? 'Xin phép' : (dataModal.type == 'NOTE' ? 'Lí do chuyển ca' : 'Danh sách cửa hàng')
    return (
        <Modal animationType="slide" visible={dataModal.visibleModal}>
            <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.light }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingStart: 12, paddingEnd: 12 }}>
...
const RenderTextInput = ({ index, typeInput, item, value, styles, handlenotes, isLock, appcolor }) => {
    ...
    return (
        <TextInput
            ...
            placeholderTextColor={appcolor.greylight} placeholder="Ghi chú..."
