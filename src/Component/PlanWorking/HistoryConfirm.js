...
const HistoryConfirm = ({ navigation, route }) => {
  ...
  const LoadData = async (typeConfirm, ChildEmployee, Month, Year) => {
    ...
    await GetHistoryConfirm(
      ...
      Month,
      Year,
      async (mData, mEmployee, monthList) => {
  ...
  const handleSelectedChange = async item => {
    ...
    await setDataModal({
      ...dataModal,
      list: dataModal.listFilter.map(i =>
  ...
  const renderItem = ({ item, index }) => {
    ...
    const ItemEarlier = () => (
      <View style={{ flex: 1 }}>
        {item.TimeEarlier !== undefined && (
          <Text style={styles.textItem}>{item.TimeEarlier}</Text>
        )}
        {item.NoteEarlier !== undefined && (
          <Text style={styles.textItem}>{item.NoteEarlier}</Text>
        )}
        {item.ConfirmNoteEarlier !== undefined && (
          <Text style={[styles.textItem, { color: appcolor.danger }]}>
          ...
          <ListItem.Content>
            <View style={{ flex: 1, flexDirection: 'row' }}>
              ...
              <Text
                style={[
                  styles.textItem,
                  { marginStart: 5, fontWeight: '700', flex: 6 },
                ]}
              >
                {item.ShopName}
              </Text>
              {item[`${item.EmployeeId}${item.AuditDate}`] && (
                ...
                >
                  {moment(item.AuditDate, 'YYYYMMDD').format('D-MMM-YY')}
                </Text>
              )}
            </View>
            ...
            {(menuSelected.type == TYPE_SHIFT ||
              menuSelected.type == TYPE_OFF) && <ItemShift />}
      ...
      >
        <SafeAreaView style={{ backgroundColor: appcolor.surface }}>
          ...
          >
            ...
            <TouchableOpacity
              style={{ padding: 8 }}
              onPress={() => SheetManager.hide('sheetEmp')}
            >
...
const RenderHeader = ({ styles, appcolor, onSelectItem, menuSelected }) => {
  const colorShiftSelect =
    menuSelected == TYPE_SHIFT ? appcolor.primary : appcolor.dark;
  const colorOffSelect =
    menuSelected == TYPE_OFF ? appcolor.primary : appcolor.dark;
  const colorLateSelect =
    menuSelected == TYPE_LATE ? appcolor.primary : appcolor.dark;
  const colorEarlierSelect =
    menuSelected == TYPE_EARLIER ? appcolor.primary : appcolor.dark;
  return (
    <View style={[styles.headerView]}>
      <TouchableOpacity
        onPress={() => onSelectItem(TYPE_SHIFT)}
        style={[styles.headerItem, { borderColor: colorShiftSelect }]}
      >
        <SpiralIcon
          name="exchange-alt"
          type="font-awesome-5"
          size={21}
          color={colorShiftSelect}
          style={styles.iconHeader}
        />
        <Text
          style={[
            styles.titleHeader,
            { color: colorShiftSelect, textAlign: 'center' },
          ]}
        >
          Chuyển ca
        </Text>
      ...
      <TouchableOpacity
        onPress={() => onSelectItem(TYPE_OFF)}
        style={[styles.headerItem, { borderColor: colorOffSelect }]}
      >
        <SpiralIcon
          name="window-close"
          type="font-awesome-5"
          size={21}
          color={colorOffSelect}
          style={styles.iconHeader}
        />
        <Text
          style={[
            styles.titleHeader,
            { color: colorOffSelect, textAlign: 'center' },
          ]}
        >
          Nghỉ Phép
        </Text>
      ...
      <TouchableOpacity
        onPress={() => onSelectItem(TYPE_LATE)}
        style={[styles.headerItem, { borderColor: colorLateSelect }]}
      >
        <SpiralIcon
          name="clock"
          type="font-awesome-5"
          size={21}
          color={colorLateSelect}
          style={styles.iconHeader}
        />
        <Text
          style={[
            styles.titleHeader,
            { color: colorLateSelect, textAlign: 'center' },
          ]}
        >
          Đi trễ
        </Text>
      ...
      <TouchableOpacity
        onPress={() => onSelectItem(TYPE_EARLIER)}
        style={[styles.headerItem, { borderColor: colorEarlierSelect }]}
      >
        <SpiralIcon
          name="business-time"
          type="font-awesome-5"
          size={21}
          color={colorEarlierSelect}
          style={styles.iconHeader}
        />
        <Text
          style={[
            styles.titleHeader,
            { color: colorEarlierSelect, textAlign: 'center' },
          ]}
        >
          Về sớm
        </Text>
...
const renderItemModal = (
  ...
  return (
    <ListItem
      ...
      containerStyle={{
        backgroundColor:
          item.isSelect === 1 ? appcolor.primary : appcolor.light,
      }}
    >
      <ListItem.Content>
        <View
        ...
        >
          <View
            style={{
              ...
              marginEnd: 8,
              backgroundColor:
                item.isData > 0 ? appcolor.yellow : appcolor.dark,
              padding: 5,
              borderRadius: 50,
              alignItems: 'center',
            }}
          >
            <Text
              style={{ fontWeight: '700', fontSize: 15, color: appcolor.light }}
            >
              {item.isData}
            </Text>
          </View>
