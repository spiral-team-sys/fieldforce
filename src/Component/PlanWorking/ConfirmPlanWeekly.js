...
export const ConfirmPlanWeekly = ({ navigation, route }) => {
    ...
    const [filterData, setFilterData] = useState({ fromDate: moment().startOf('month').format('YYYY-MM-DD'), toDate: moment().endOf('month').format('YYYY-MM-DD') })
    ...
    const LoadData = async (fromDate, toDate) => {
        ...
        const result = await WorkingPlanAPI.GetConfirmPlanWeekly(fromValue, toValue)
        if (result.statusId === 200) {
            const dataConfirmFilter = await result.data.table.filter(i => i.weekByYear == result.data.table1[0].weekNum)
            ...
            for (let j = 0, lenList = planList.length; j < lenList; j++) {
                ...
                if (item.ConfirmPlan == -1) {
                    if (item.SupNote == null || item.SupNote.length == 0) {
                        ToastError(`Chưa nhập lí do từ chối ngày ${item.ShortDate}-${item.DateView}`, item.EmployeeName, 'top')
                        return
                    }
                }
    ...
    const onChangeData = async (index) => {
        ...
        const dataFilter = await dataConfirmMain.filter(i => i.weekByYear == weekNum)
    ...
    const onRejectDay = async (item, indexMain) => {
        ...
        const dataReject = await dataUpdate.map(i => i.Date === item.Date && i.ShopId === item.ShopId ? { ...i, ConfirmPlan: i.ConfirmPlan !== -1 ? -1 : 1 } : i)
        //
    ...
    const onConfirmAll = async (item, valueConfirm, indexMain) => {
        const dataUpdate = JSON.parse(dataConfirm[indexMain].planList || [])
        const confirmAll = await dataUpdate.map(i => i.WeekByYear == item.weekByYear && i.EmployeeId == item.employeeId ? { ...i, ConfirmPlan: valueConfirm } : i)
        //
    ...
    const styles = StyleSheet.create({
        ...
        itemContainer: { flex: 1, borderRadius: 12, backgroundColor: appcolor.light },
        titleItem: { flex: 9, fontSize: 15, fontWeight: '600', color: appcolor.dark, padding: 8, margin: 3 },
        bodyItem: { flex: 1 },
        dayView: { flex: 8, padding: 5, fontSize: 15, fontWeight: '600', color: appcolor.info },
        headerView: { backgroundColor: appcolor.light, flexDirection: 'row', width: deviceWidth, justifyContent: 'center' },
        dateDetailView: { fontSize: 14, fontWeight: '300', fontStyle: 'italic' },
        itemConfirmContainer: { padding: 8, marginBottom: 5, backgroundColor: appcolor.light, borderRadius: 8 },
        titleItemView: { fontSize: 14, fontWeight: '500', color: appcolor.dark },
        itemEmployeeContent: { backgroundColor: appcolor.light, borderRadius: 5, padding: 8, margin: 5, borderWidth: 0.5, borderColor: appcolor.grayLight },
        itemEmployeeName: { fontSize: 14, fontWeight: '300', color: appcolor.dark, textAlign: 'center', marginStart: 8, marginEnd: 8 },
        paginationView: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: -8, marginBottom: 8 },
        paginationDot: { height: 5, borderRadius: 3, marginHorizontal: 3 },
        paginationDotActive: { width: 10, backgroundColor: appcolor.dark },
        paginationDotInactive: { width: 6, backgroundColor: appcolor.dark, opacity: 0.4 }
    })
    const paginationDot = () => {
        return (
    ...
    const renderItemEmployee = ({ item, index }) => {
        ...
        const onConfirm = () => { onConfirmAll(item, 1, index) }
        const onReject = () => { onConfirmAll(item, -1, index) }
        const planbyWeek = JSON.parse(item.planList) || []
        return (
            <View key={`it_emp_${index}`}>
                <TouchableOpacity onPress={onShowView}>
                    <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginBottom: 8, backgroundColor: appcolor.light, padding: 5, borderRadius: 5 }}>
                        <Text style={styles.dayView}>{`${index + 1}. ${item.employeeName}`}</Text>
                        {item.isShowView == 1 &&
                            <View style={{ flexDirection: "row" }}>
                                <Icon name="check-square" type="font-awesome-5" size={28} color={appcolor.success} solid style={{ padding: 3 }} onPress={onConfirm} />
                                <Icon name="window-close" type="font-awesome-5" size={28} color={appcolor.danger} style={{ padding: 3 }} onPress={onReject} />
                            </View>
                        }
                    </View>
                </TouchableOpacity>
                {item.isShowView == 1 &&
                    <CustomListView
                        key={'planconfirmtime'}
                        data={planbyWeek}
                        renderItem={({ item: itemPlan, index: indexPlan }) => renderItemPlan(itemPlan, indexPlan, index)}
    ...
    const renderItemPlan = (item, index, mainIndex) => {
        const onPressDay = () => { onRejectDay(item, mainIndex) }
        const onChangeText = (text) => { handlerChangeNote(index, text, mainIndex, item) }
        const styleItemReject =
            item.ConfirmPlan == -1 ? { borderEndWidth: 10, borderEndColor: appcolor.danger } :
                item.ConfirmPlan == 1 ? { borderEndWidth: 10, borderEndColor: appcolor.success } : {}
        return (
                ...
                <View style={{ flexDirection: 'row' }}>
                    ...
                    <TouchableOpacity style={{ flex: 8 }} onPress={onPressDay}>
                        <View style={[styles.itemConfirmContainer, styleItemReject]}>
                            ...
                            <Text style={{ ...styles.titleItemView, fontWeight: '300' }}>{item.Address}</Text>
                            <Text style={{ ...styles.titleItemView, fontWeight: '300' }}>Ca hiện tại: {item.ShiftType}</Text>
                            {item.ShiftTypeUpdate !== null && <Text style={{ ...styles.titleItemView, fontWeight: '400', color: appcolor.tomato }}>Ca thay đổi: {item.ShiftTypeUpdate}</Text>}
                            {item.Notes !== undefined && <Text style={{ ...styles.titleItemView, fontWeight: '300' }}>Lí do thay đổi: {item.Notes}</Text>}
                            {item.ConfirmPlan == -1 &&
                                <FormGroup
                                    containerStyle={{ marginTop: 5, backgroundColor: appcolor.surface }}
                                    title='Lí do từ chối'
                                    editable
                                    value={item.SupNote}
        ...
        return (
            <View key={`it_wp_${index}`} style={styles.headerView}>
                <View style={{ flex: 8, alignSelf: 'center', padding: 16 }}>
                    <Text style={{ fontSize: 23, color: appcolor.dark, fontWeight: '700' }}>{item.weekByYear}</Text>
                    <Text style={styles.dateDetailView}>{`${item.fromDate} / ${item.toDate}`}</Text>
                </View>
                <View style={{ flex: 2, backgroundColor: appcolor.yellowdark, alignItems: 'center', padding: 16 }}>
                    <Text style={{ fontSize: 23, color: appcolor.dark, fontWeight: '600', fontStyle: 'italic' }}>{countRequest}</Text>
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            />
            <View style={{ width: deviceWidth, alignSelf: 'center', alignContent: 'center', justifyContent: 'center', alignItems: 'center' }}>
                <CustomListView
                    ref={weekListRef}
                    horizontal
                    pagingEnabled
