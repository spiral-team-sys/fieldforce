...
const PhotoItem = memo(({ item, index, showPhoto, appcolor }) => {
  ...
  const photoInnerStyle = useMemo(
    () => [staticStyles.photoInner, { backgroundColor: appcolor.lightgray }],
    [appcolor.lightgray],
  );
  const placeholderStyle = useMemo(
    () => [staticStyles.photoImage, { backgroundColor: appcolor.surface }],
    [appcolor.surface],
  );
  const photoTypeStyle = useMemo(
    () => [staticStyles.photoType, { color: appcolor.primary }],
    [appcolor.primary],
  );
  const photoTimeStyle = useMemo(
    () => [staticStyles.photoTime, { color: appcolor.dark }],
    [appcolor.dark],
  );
  const photoDistanceStyle = useMemo(
    () => [staticStyles.photoDistance, { color: appcolor.dark }],
    [appcolor.dark],
  );
