...
const AttendantHistoryScreen = ({ navigation, route }) => {
    ...
    const LoadData = async (mDate) => {
        ...
        const AttendantDate = moment(
            mDate !== undefined ? mDate : (configDate.dateChoose !== undefined ? configDate.dateChoose : moment())
        ).format('YYYYMMDD')
    ...
    const groupBySort = useCallback((data, type) => {
        const fieldMap = {
            'EMPLOYEE': { field: 'employeeId', fieldName: 'employeeName', subName: 'employeeCode' },
            'SHOP': { field: 'shopId', fieldName: 'shopName', subName: 'shopCode', address: 'address' },
            'POSITION': { field: 'typeId', fieldName: 'typeName' },
        }
        const { field, fieldName, subName, address } = fieldMap[type] || fieldMap['EMPLOYEE']
        const groupMap = new Map()
        for (const item of data) {
            const key = item[field]
            if (!groupMap.has(key)) {
                groupMap.set(key, { name: item[fieldName], subName: item[subName], address: item[address] || null, items: [], count: 0 })
            }
            const group = groupMap.get(key)
            group.items.push(item)
            if (item.photoList && item.photoList !== '[]' && item.photoList !== 'null') {
                group.count++
            }
        }
        const groupResult = Array.from(groupMap.values())
            .map(g => ({ [fieldName]: g.name, [subName]: g.subName, address: g.address, dataByType: g.items, numAttendant: g.count }))
    ...
    const ConfigCalendar = (dateSelect) => {
        ...
        setConfigDate({ dateChoose: dateSelect == undefined ? moment().format('YYYY-MM-DD') : dateSelect, customDatesStyles: customDatesStyles })
    }
    const showModalView = () => {
        const titleMap = {
        ...
        setDataModal(prev => ({ ...prev, titleModal: titleMap[groupType] || '', visible: true }))
    }
    const searchModal = (text) => {
        if (!text) {
            ...
            const filterShift = dataModal.dataFilter.filter(item => (
                removeVietnameseTones(item?.employeeName?.toLowerCase()).includes(searchText) ||
                removeVietnameseTones(item?.employeeCode?.toLowerCase()).includes(searchText) ||
                removeVietnameseTones(item?.shopName?.toLowerCase()).includes(searchText) ||
                removeVietnameseTones(item?.shopCode?.toLowerCase()).includes(searchText) ||
                removeVietnameseTones(item?.address?.toLowerCase()).includes(searchText) ||
                removeVietnameseTones(item?.typeName?.toLowerCase()).includes(searchText)
            ))
    ...
    const chooseItemModal = (item) => {
        ...
        const lstFilter = rawDataRef.current.filter(i => i[itemField] === item[itemField])
    ...
    const filterDataBy = useCallback((type) => {
        ...
        InteractionManager.runAfterInteractions(() => {
            try {
                ...
                allGroupedDataRef.current = listSort
                setGroupType(type)
                setDataAtendant(prev => ({ ...prev, mData: listSort.slice(0, PAGE_SIZE), mDataMain: rawDataRef.current }))
                setDataModal(prev => ({
                    ...prev,
    ...
    const styles = useMemo(() => StyleSheet.create({
        ...
        loadingView: { position: 'absolute', alignItems: 'center', justifyContent: 'center', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(255,255,255,0.5)' },
        filterStyle: { marginStart: 8, marginEnd: 8 },
    }), [appcolor])

    const renderListFooter = useMemo(() => {
        if (dataAttendant.mData.length >= allGroupedDataRef.current.length) return undefined
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            <ViewPictures
                ...
                onSwipeDown={() => setDataModalPhoto({ ...dataModalPhoto, visiblePhoto: false })}
            />
        </View>
    )
}
