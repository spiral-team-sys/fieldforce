...
import FilterSheet from "./Items/FilterSheet"
...
const AttendantResult = ({ navigation, route }) => {
    ...
    const groupBySort = useCallback((data, type) => {
        const fieldMap = {
            ...
            'SHOP': { field: 'shopId', fieldName: 'shopName', subName: 'shopCode' },
            'POSITION': { field: 'typeId', fieldName: 'typeName' },
        }
        const { field, fieldName, subName } = fieldMap[type] || fieldMap['EMPLOYEE']

        // O(n) Map-based grouping — thay thế nested forEach O(n²)
        const groupMap = new Map()
        for (const item of data) {
            const key = item[field]
            if (!groupMap.has(key)) {
                groupMap.set(key, { name: item[fieldName], subName: item[subName], items: [], count: 0 })
            }
        ...
        return Array.from(groupMap.values())
            .map(g => ({ [fieldName]: g.name, [subName]: g.subName, dataByType: g.items, numAttendant: g.count }))
    ...
    const searchModal = (text) => {
        if (!text) {
            ...
            const filterShift = dataModal.dataFilter.filter(item => removeVietnameseTones(item[searchField].toLowerCase()).includes(searchText))
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            />
            <ActionSheet ref={filterSheet} animated={false}>
                <FilterSheet
                    appcolor={appcolor}
