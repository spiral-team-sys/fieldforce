...
import { TODAY, alertNotify, countDuplicate, deviceHeight, deviceWidth, removeDuplicate } from "../../Core/Utility"
...
import { MessageAction, ToastError, ToastSuccess, getDaysInMonth, removeVietnameseTones } from "../../Core/Helper"
...
export const MonthlyPlan = ({ navigation }) => {
    ...
    const [confirmMess, setConfirmMess] = useState('Bạn xác nhận thông tin đăng kí như trên');
    const [dailyshopSelect, setdailyShopSelect] = useState([])
    const [listShopSelect, setListShopSelect] = useState({ dailyshopSelect: [], shopData: [] })
    ...
    const [lockShift, setLockShift] = useState({ isLockShiftOFF: 0, isLockShiftON: 0 })
    ...
    const onLoad = async (m, y) => {
        ...
        if (result.statusId === 200) {
            ...
            var _off = await result.data?.table2?.filter(a => a.shiftGroup === 'OFF') || []
            var _on = await result.data?.table2?.filter(a => a.shiftGroup === 'ON') || []
    ...
    const onDayPress = (item, planData, shiftCode, shiftType) => {
        ...
        if (shiftType !== null && shiftType === SHIFT_MODE.ON) {
            ...
            _shiftON = _shiftON.sort((a, b) => b.shiftCode.localeCompare(a.shiftCode))
        ...
        else if (shiftType !== null) {//OFF MODE
            ...
            // dailyshopSelect already reset at start of onDayPress
            const resetList = shiflistRoot.map(a => a.selected === 1 ? { ...a, selected: 0 } : a)
    ...
    const dayItem = ({ item, index }) => {
        ...
        const planShiftOn = _.filter(planData, (it) => it.ShiftType === SHIFT_MODE.ON)
        const isCheckMinimal = (item.isCheckMinimal > 0 ? (item.isCheckMinimal == 2 ? true : (planShiftOn.length > 0 ? true : false)) : false)
        const statusCheck = planShiftOn.length < (item.minimalStore || 0) ? false : true
        const colorWarning = (!statusCheck && isCheckMinimal) ? item.colorWarning : null
        const colorStatus = item.isSendSystem == 3 ? appcolor.blue : (planstatus === 3 ? appcolor.yellow : planstatus === 1 ? appcolor.success : appcolor.transparent)

        return (
            <View style={{ borderLeftWidth: 0.2, borderBottomWidth: 0.2, minHeight: _minHeight, flexGrow: 1, width: _widthItem, opacity: item.enableDay === 1 ? 1 : 0.4 }}
                key={`s292${index}`}>
                <TouchableOpacity style={{ minHeight: _minHeight }} onPress={() => onDayPress(item, planData, shiftCode, shiftType)}>
                    <Text style={{ marginTop: 3, fontSize: scaleSize(12), textAlign: 'center', color: appcolor.dark }}>{moment(item.date).format('D/M')}</Text>
                    {
                        shiftType !== null && shiftType === SHIFT_MODE.ON ?
                            <View style={{ justifyContent: 'center', flexDirection: 'row', backgroundColor: colorStatus, borderRadius: 20, padding: 3 }}>
                                <Text style={{ textAlign: 'center', color: appcolor.white }}>
                                    {shiftCode}
                                </Text>
                                <Text style={{ fontSize: 10, textAlign: 'center', color: appcolor.white }}>
                                    {planData.length}
                                </Text>
                            </View> : shiftType !== null && shiftType === SHIFT_MODE.OFF ? //OFF VIEW
                                <View style={{ backgroundColor: colorStatus, borderRadius: 20, padding: 3 }}>
                                    <Text style={{ textAlign: 'center', color: appcolor.white }}>
                                        {shiftCode}
                                    </Text>
                                </View>
                    ...
                    {
                        isCheckMinimal &&
                        <View style={{ position: 'absolute', width: '100%', bottom: 0, padding: 3, backgroundColor: colorWarning || colorStatus }}>
                            <Text style={{ textAlign: 'center', fontSize: 14, color: appcolor.black }}>{`${planShiftOn.length}/${item.minimalStore}`}</Text>
                        </View>
                    }
                    <Text numberOfLines={2}
                        style={{ paddingTop: 3, color: appcolor.danger, fontSize: scaleSize(10) }}>{note}</Text>
                    {/* 
                    <View style={{
    ...
    const contains = (item, query) => {
        ...
        return removeVietnameseTones(Saddress)?.match(removeVietnameseTones(query)) ||
    ...
    const handleSearch = async (text, province) => {
        ...
        if (province !== undefined && province.length > 0) {
            searchStore = await shoplist.filter(a => province.toString().includes(a.province))
        }
    ...
    const sortShopDataBySelected = (selectedShopIds = []) => {
        const selectedOrder = new Map((selectedShopIds || []).map((shopId, index) => [shopId, index]))
    ...
    const saveShopOrderChanges = async () => {
        ...
        const isOnPlan = planData.length > 0 && planData[0].ShiftType === SHIFT_MODE.ON
        ...
        const selectedOrder = new Map(listShopSelect.dailyshopSelect.map((shopId, index) => [shopId, index]))
        const originalPosition = new Map(planData.map((shop, index) => [shop.ShopId, index]))
        const orderedPlanData = [...planData].sort((a, b) => {
            const aPosition = selectedOrder.has(a.ShopId) ? selectedOrder.get(a.ShopId) : Number.MAX_SAFE_INTEGER
            const bPosition = selectedOrder.has(b.ShopId) ? selectedOrder.get(b.ShopId) : Number.MAX_SAFE_INTEGER
            return aPosition === bPosition
                ? originalPosition.get(a.ShopId) - originalPosition.get(b.ShopId)
                : aPosition - bPosition
        })

        if (orderedPlanData.some((shop, index) => shop.ShopId !== planData[index].ShopId)) {
    ...
    const onAcceptDays = () => {
        ...
        if (mode === SHIFT_MODE.ON) {
            if (shiftDefaulOn === null || shiftDefaulOn === '') {
                ToastError("Bạn chưa chọn ca đi làm hoặc lý do nghỉ", "Chọn trạng thái", "top")
                return
            }
            const maxCall = config.maxCall || 0
            if (maxCall > 0 && listShopSelect.dailyshopSelect !== null && listShopSelect.dailyshopSelect.length < maxCall) {
                ToastError(`Vui lòng đăng kí đủ ${maxCall} call trong ngày`, "Tổng call đăng kí", "top")
                return
            }
            //
            let _shopSelect = []
            listShopSelect.dailyshopSelect?.forEach(shopId => {
                _shopSelect.push({ "ShopId": shopId, "ShiftType": SHIFT_MODE.ON, "ShiftCode": shiftDefaulOn, "Status": 3, "Note": null })
            });

            if (_shopSelect.length == 0 && isNoneShop) {
                _shopSelect.push({
                    "ShopId": userinfo.employeeId, "ShiftType": SHIFT_MODE.ON, "ShiftCode": shiftDefaulOn, "Status": 3, "Note": null
                })
            }
            ...
            const indexUpdate = _calendar.findIndex(i => i.dayInt === dayEdit)
            let editItem = _calendar[indexUpdate];
            editItem.planData = (shiftDefaulOff !== null ? JSON.stringify([{ "ShopId": userinfo.employeeId, "ShiftType": SHIFT_MODE.OFF, "ShiftCode": shiftDefaulOff, "Status": 3, "Note": note }]) : null)
            _calendar[indexUpdate] = editItem;
            setCalendar(_calendar);
            Summary(_calendar)
            AsyncStorage.setItem(KeyData, JSON.stringify(_calendar))
        ...
        while (nextDayIndex < _calendar.length) {
            ...
            if (candidate && candidate.enableDay === 1 && moment(candidate.date).day() !== 0) break
            nextDayIndex++
        }
        if (currentIndex !== -1 && nextDayIndex < _calendar.length) {
            ...
            if (nextDay) {
                ...
                const nextShiftCode = nextPlanData.length > 0 ? nextPlanData[0].ShiftCode : null
                const nextShiftType = nextPlanData.length > 0 ? nextPlanData[0].ShiftType : null
    ...
    const Summary = (calendar) => {
        let offDay = 0, totalShop = 0, onDay = 0, noPlan = 0, phepTon = 0
        ...
        setSummary({ "onday": onDay, "offday": offDay, "totalshop": totalShop, "noplan": noPlan, "phepTon": phepTon })
    }
    const duplicateWeek = (result) => {
        const weekList = removeDuplicate(calendar, "weekly")
        let _result = true;
        weekList.forEach(w => {
            const shopweek = calendar.filter(sh => sh.weekly === w.weekly && sh.planData !== null)
            let _temp = [];
            shopweek.forEach(shop => {
                const _stemp = JSON.parse(shop.planData).filter(a => a.ShiftType !== SHIFT_MODE.OFF)
    ...
    const Valiaded = async (result) => {
        const dayFromRegister = config?.registerBy == 'month' ? getDaysInMonth(year, month) : 7
        //

        const daysRemaining = calendar[0].daysRemaining || dayFromRegister
        const daysRegister = (summary.offday + summary.onday) - (calendar[0].dayNone || 0)

        const dataFrequency = listShopSelect.shopData?.filter(i => i.highlightFrequency == 1)
        if (daysRegister < daysRemaining) {
            result(500, `Bạn phải đăng kí ít nhất ${daysRemaining} ngày (${daysRegister}/${daysRemaining})`)
            return
            ...
            for (let index = 0; index < dataFrequency.length; index++) {
                ...
                if (element.totalMonth == 1 && element.actualFrequency < element.monthlyFrequency) {
                    result(500, `Bạn chưa đăng kí đủ tần suất ghé thăm cửa hàng ${element.shopName}`)
                    return
                }
            }
        }
        for (let index = 0; index < calendar.length; index++) {
            ...
            const planShiftOn = _.filter(planData, (it) => it.ShiftType === SHIFT_MODE.ON)
            if (element.isConstraintPlan === 1 && (element.isCheckMinimal == 2 || (planShiftOn.length > 0 && element.isCheckMinimal == 1))) {
                const minimalStore = element.minimalStore || 0
                if (minimalStore > 0 && planShiftOn.length < minimalStore) {
                    result(500, `Ngày ${moment(element.date).format('DD/MM')} bạn đăng kí số shop tối thiểu là ${minimalStore}`)
                    return
                }
            }
        }
        ...
        duplicateWeek(value => {
            if (value !== undefined) {
                setConfirmMess("Lịch làm việc bạn đăng kí có cửa hàng đến trên 1 lần trong tuần,Bạn muốn tiếp tục đăng kí?")
                value === false ? result(400, '') :
    ...
    const onConfirm = async () => {
        ...
        const result = await WorkingPlanAPI.uploadPlanbyMonth(dataUpload, notifyMessager)
        if (result.statusId === 200) {
            const KeyData = `MONTHLYPLAN${month}${year}`
    ...
    const deleteDataAsync = () => {
        MessageAction('Bạn có chắc chắn muốn xoá dữ liệu lưu tạm?',
            async () => {
                const KeyData = `MONTHLYPLAN${month}${year}`
                await AsyncStorage.removeItem(KeyData);
                await onLoad(month, year)
    ...
    const ClearAllDataPlan = () => {
        MessageAction('Bạn có chắc chắn muốn xoá tất cả kế hoạch đã đăng ký trong tháng?',
            async () => {
                const dataMain = [{
                    typeSend: 'DELETE_ALL',
                    month: month,
                ...
                const result = await REPORT.UploadDataRaw_Realtime(dataMain, shop, kpiinfo.id)
                if (result.statusId == 200) {
                    ToastSuccess(`Đã xoá dữ liệu tháng ${month}/${year} trên hệ thống`, 'Thông báo', 'top')
                    const KeyData = `MONTHLYPLAN${month}${year}`
                    await AsyncStorage.removeItem(KeyData);
                    await onLoad(month, year)
                }
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            <HeaderCustom iconRight="cloud-upload-alt"
                iconMiddle={'eraser'}
                middleFunc={() => deleteDataAsync()}
                rightFunc={
                    lock === false ? () => onSummit() : null
                } leftFunc={() => navigation.goBack()} title={kpiinfo?.menuNameVN || 'Đăng ký lịch tháng'} />
            <View style={{ flexDirection: 'row', padding: 7, backgroundColor: appcolor.light }}>
                <View style={{ flexGrow: 1, alignItems: 'center' }}>
                    <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: 'bold' }}>Ngày đi làm</Text>
                    <Text style={{ fontSize: 20, color: appcolor.success }}>{summary?.onday || 0}</Text>
                </View>
                <View style={{ flexGrow: 1, alignItems: 'center' }}>
                    <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: 'bold' }}>Tổng Call ĐK</Text>
                    <Text style={{ fontSize: 20, color: appcolor.info }}>{summary?.totalshop || 0}</Text>
                </View>
                <View style={{ flexGrow: 1, alignItems: 'center' }}>
                    <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: 'bold' }}>Chưa ĐK</Text>
                    <Text style={{ fontSize: 20, color: appcolor.dark }}>{summary?.noplan || 0}</Text>
                </View>
                <View style={{ flexGrow: 1, alignItems: 'center' }}>
                    <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: 'bold' }}>Ngày nghỉ</Text>
                    <Text style={{ fontSize: 20, color: appcolor.danger }}>{summary?.offday}</Text>
                </View>
                <View style={{ flexGrow: 1, alignItems: 'center' }}>
                    <Text style={{ fontSize: 12, color: appcolor.dark, fontWeight: 'bold' }}>Phép tồn</Text>
                    <Text style={{ fontSize: 20, color: appcolor.info }}>{summary?.phepTon}</Text>
                </View>
            </View>
            <View style={{ width: '100%', height: '80%', backgroundColor: appcolor.light }}>
                <LoadingView isLoading={loading} styles={{ left: 0, right: 0, top: deviceHeight / 3, position: 'absolute' }} />
                <View style={{ flexDirection: 'row', marginTop: 0.5, alignItems: 'center', backgroundColor: appcolor.primary }}>
                    <TouchableOpacity style={{ padding: 7 }} onPress={() => onCalendar(-1)} >
                        <Icon size={scaleSize(20)} color={appcolor.white} name="arrow-left" />
                    </TouchableOpacity>
                    <TouchableOpacity style={{ flexGrow: 1, alignItems: 'center' }}>
                        <Text style={{ color: appcolor.white, fontWeight: 'bold', fontSize: scaleSize(20) }}>
                            {moment(`${year}-${month > 9 ? month : '0' + month}-01`).format("MMMM, YYYY")}
                        </Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => onCalendar(1)} style={{ padding: 7 }}>
                        <Icon size={scaleSize(20)} color={appcolor.white} name="arrow-right" />
                    </TouchableOpacity>
                </View>
                <View style={{ backgroundColor: appcolor.light }}>
                    <FlatList
                        ...
                        renderItem={dayItem}
                        ListFooterComponent={<View style={{ height: deviceHeight / 3.5 }} />}
                    />
                </View>
                {
                    config.isUseClearAll == 1 && ((config.isLockCurrentMonth !== 1 && month >= _currentMonth && year >= _currentYear) || (config.isLockCurrentMonth == 1 && month > _currentMonth && year >= _currentYear)) &&
                    <View style={{ position: 'absolute', bottom: 0, width: '100%', alignItems: 'center' }}>
                        <TouchableOpacity style={{ backgroundColor: appcolor.danger, flexDirection: 'row', width: '90%', justifyContent: 'center', alignItems: 'center', padding: 8, borderRadius: 30 }}
                            onPress={ClearAllDataPlan}>
                            <Icon size={20} type="font-awesome-5" color={appcolor.white} name="trash" />
                            <Text style={{ fontWeight: '600', fontSize: 14, color: appcolor.white, paddingHorizontal: 12 }}>Xoá lịch đã đăng kí</Text>
                        </TouchableOpacity>
                    </View>
                }
            </View>
            <ActionSheet id="sheetPlan" ref={_sheetAction} onClose={saveShopOrderChanges}
                keyboardHandlerEnabled={false}
                containerStyle={{ backgroundColor: appcolor.surface }} >
                <View style={{ height: _sheetheight }}>
                    <Text style={{ textAlign: 'center', padding: 7, fontSize: scaleSize(14), fontWeight: 'bold', color: appcolor.primary }}>
                        Bạn đang xem lịch {moment(dayEdit, 'YYYYMMDD').format('dddd DD MMM, YYYY')}
                    </Text>
                    <View style={{ flexDirection: 'row', marginBottom: 7 }}>
                        {
                            lockShift.isLockShiftON != 1 &&
                            <View style={{
                                padding: 7, borderBottomLeftRadius: 22, flexGrow: 1,
                                ...
                                <TouchableOpacity onPress={() => {
                                    ...
                                    <Text style={{ color: mode === 'ON' ? appcolor.success : appcolor.dark, textAlign: 'center' }}>Đi làm</Text>
                                </TouchableOpacity>
                            </View>
                        }
...

                        {
                            ...
                            <View style={{
                                flexGrow: 1, borderBottomRightRadius: 22,
                                padding: 7, backgroundColor: appcolor.light
                            }}>
                                <TouchableOpacity onPress={() => {
                                    setMode('OFF')
                                    // setShift(null)
                                    ...
                                    <Text style={{ color: mode === 'OFF' ? appcolor.danger : appcolor.dark, textAlign: 'center' }}>Nghỉ(Off)</Text>
                                </TouchableOpacity>
                            </View>
                        }
                    ...
                    {
                        mode == SHIFT_MODE.ON ?
                            <ONPlan props={{ navigation, shiftOn, lock, lockDay, tag, tagSelect, shopData: listShopSelect.shopData, dailyshopSelect: listShopSelect.dailyshopSelect, onSelectTag, handleSearch, onShopPress, onMoveShopOrder, onSelectON }} />
                            :
                            <OFFPlan props={{ shiftlist, note, onSelectOFF, setNote }} />
                    }
                    <View style={{
                        flexDirection: 'row', backgroundColor: appcolor.light,
                        justifyContent: 'center', borderBottomRightRadius: 12, borderBottomLeftRadius: 12
                    }}>
                        {lockDay === false &&
                            <TouchableOpacity
                                ...
                                disabled={(mode === SHIFT_MODE.ON && listShopSelect.dailyshopSelect?.length === 0 && !isNoneShop) ? true : false}
                                onPress={() => onAcceptDays()}
                                style={{ margin: 7, alignItems: 'center', }}>
                                <Text style={{ color: appcolor.primary, padding: 8, paddingBottom: 16 }}>
                                    {mode === SHIFT_MODE.ON && `(${listShopSelect.dailyshopSelect.length})`}
                                    Xác nhận {(mode === SHIFT_MODE.ON ? shiftDefaulOn : shiftDefaulOff) !== null && `(${(mode === SHIFT_MODE.ON ? shiftDefaulOn : shiftDefaulOff)})`}
                                </Text>
                            </TouchableOpacity>
                        }
                    </View>
...
const ONPlan = ({ props }) => {
    ...
    const { navigation, shiftOn, lock, lockDay, tag, tagSelect, shopData, dailyshopSelect, onSelectTag, handleSearch, onShopPress, onMoveShopOrder, onSelectON } = props
    const [dataShopF, setDataShopF] = useState([...shopData])
    const loadDataShop = async () => {
        const dataF = shopData.filter(it => dailyshopSelect?.includes(it.shopId) || !lock)
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ padding: 7, minHeight: 50 }}>
                {
                    shiftOn?.map((item, index) => {
                        return (<TouchableOpacity onPress={() => onSelectON(item, index)} key={`${index}alk2`}>
                            <View style={{ flexDirection: 'row', backgroundColor: item.selected === 1 ? appcolor.info : appcolor.surface, padding: 7, marginRight: 7, borderRadius: 22 }}>
                                <Text style={{ color: item.selected === 1 ? appcolor.white : appcolor.dark, fontSize: 16, fontWeight: '500' }}>{item.shiftCode}</Text>
                                <Text style={{ color: item.selected === 1 ? appcolor.white : appcolor.dark, fontSize: 10 }}>
                                    {item.shiftName.length < 20 ? `${item.shiftName}` : `${item.shiftName.substring(0, 16)}...`}
                                </Text>
                            </View>
                        </TouchableOpacity>)
                    })
                }</ScrollView>
            <FormGroup editable={true}
                useClearAndroid={false}
                handleChangeForm={(e) => handleSearch(e, tagSelect)}
                placeholder={`Tìm kiếm cửa hàng...`} />
            <Text style={{ padding: 3, color: appcolor.dark, fontSize: scaleSize(12), marginLeft: 22 }}>Lọc theo tỉnh</Text>
            <View style={{ padding: 3 }}>
                        ...
                        ({ item, index }) => {
                            return (
                                <TouchableOpacity onPress={() => onSelectTag(item)}>
                                    <View style={{
                                        height: 40, marginEnd: 7, padding: 12, borderRadius: 22,
                                        backgroundColor: tagSelect?.includes(item.province) ? appcolor.primary : appcolor.surface
                                    }}
                                        key={`${index}-298`}>
                                        <Text style={{ color: tagSelect?.includes(item.province) ? appcolor.white : appcolor.dark }}>{item.province}</Text>
                                    </View>
                ...
                />
                <View style={{ marginTop: 7, borderWidth: 1, borderColor: appcolor.surface, width: '100%' }} />
                    ...
                    <FlashList
                        ...
                        renderItem={
                            useCallback(({ item, index }) => {
                                ...
                                return (
                                    <TouchableOpacity style={{ display: isSelected ? 'flex' : lock ? 'none' : 'flex' }} onPress={onItemPress}>
                                        <View style={{
                                            padding: 12,
                                            backgroundColor: isSelected ? appcolor.success : appcolor.light
                                        }}
                                            key={`${index}-2sd8`}>
                                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                <Text style={{ flex: 1, fontSize: scaleSize(12), color: isSelected ? appcolor.white : appcolor.dark }}>
                                                    {isSelected ? selectedOrder + 1 : index + 1})  {item.shopCode} - {item.shopName} - {item.shopId}
                                                </Text>
                                                {isSelected && !lock && !lockDay && item.sortAble == 1 &&
                                                    <View style={{ flexDirection: 'row', marginLeft: 8 }}>
                                                        <TouchableOpacity
                                                            ...
                                                            style={{
                                                                ...
                                                                backgroundColor: appcolor.white, alignItems: 'center', justifyContent: 'center',
                                                                opacity: selectedOrder === 0 ? 0.4 : 1
                                                            }}>
                                                            <Icon type="font-awesome-5" size={18} color={appcolor.success} name="chevron-up" />
                                                        </TouchableOpacity>
                                                        <TouchableOpacity
                                                            disabled={selectedOrder === dailyshopSelect.length - 1}
                                                            onPress={() => onMoveShopOrder(item.shopId, 1)}
                                                            style={{
                                                                width: 34, height: 34, borderRadius: 17,
                                                                backgroundColor: appcolor.white, alignItems: 'center', justifyContent: 'center',
                                                                opacity: selectedOrder === dailyshopSelect.length - 1 ? 0.4 : 1
                                                            }}>
                                                            <Icon type="font-awesome-5" size={18} color={appcolor.success} name="chevron-down" />
                                                        </TouchableOpacity>
                                                    </View>
                                                }
                                            </View>
                                            <Text style={{ fontSize: scaleSize(10), color: isSelected ? appcolor.white : appcolor.dark }}>{item.shopAddress} {item.province}</Text>
                                            <View style={{ flexDirection: 'row', padding: 3 }}>
                                                <Text style={{ color: isSelected ? appcolor.white : (item.total > 4 ? appcolor.danger : appcolor.dark), fontSize: scaleSize(12), flexGrow: 1 }}>
                                                    {item.dayPlan}
                                                </Text>
                                                <Text style={{ fontWeight: 'bold', color: isSelected ? appcolor.white : (item.total > 4 ? appcolor.danger : appcolor.dark), fontSize: 12 }}>Tổng: {item.total || 0}</Text>
                                            </View>
                                            {item.highlightFrequency == 1 &&
                                                <View style={{ padding: 3 }}>
                                                    <Text style={{
                                                        ...
                                                        color: isSelected
                                                            ? appcolor.white
                                                            : (item.actualFrequency < item.monthlyFrequency ? appcolor.tomato : appcolor.dark)
                ...
                {/* <FlatList
                    ...
                    /> */}
                {/* </ScrollView> */}
                <TouchableOpacity onPress={gotoMap} style={{ left: 20, position: 'absolute', bottom: 20 }}>
                    <Icon name="map" color={appcolor.primary} reverse containerStyle={{ opacity: 0.8 }} />
                </TouchableOpacity>
            </View>
        </View>
    )
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}
                ...
                {shiftlist !== null && shiftlist.length > 0 && shiftlist.map((item, index) => {
                    return (
                        <TouchableOpacity key={`${index}-2slld8`} onPress={() => onSelectOFF(item, index)}>
                            <View style={{ padding: 12, backgroundColor: item.selected === 1 ? appcolor.danger : appcolor.light, flexDirection: 'row', alignItems: 'center' }}>
                                <Badge value={item.shiftCode} containerStyle={{ marginRight: 10 }} />
                                <View style={{ flexGrow: 1 }}>
                                    <Text style={{ fontSize: 12, color: appcolor.dark }}>{item.shiftNameVN}</Text>
                                    <Text style={{ fontSize: 10, color: appcolor.dark }}>{item.shiftName}</Text>
                                </View>
                            </View>
                        </TouchableOpacity>
                    )
...
const WeekItem = ({ }) => {
    ...
    return (
        <View style={{ flexDirection: 'row', borderBottomWidth: 0.2 }}>
            <Text style={{ textAlign: 'center', flexGrow: 1, fontWeight: 'bold', padding: 5, color: appcolor.dark }}>T2</Text>
            <Text style={{ textAlign: 'center', flexGrow: 1, fontWeight: 'bold', padding: 5, color: appcolor.dark }}>T3</Text>
            <Text style={{ textAlign: 'center', flexGrow: 1, fontWeight: 'bold', padding: 5, color: appcolor.dark }}>T4</Text>
            <Text style={{ textAlign: 'center', flexGrow: 1, fontWeight: 'bold', padding: 5, color: appcolor.dark }}>T5</Text>
            <Text style={{ textAlign: 'center', flexGrow: 1, fontWeight: 'bold', padding: 5, color: appcolor.dark }}>T6</Text>
            <Text style={{ textAlign: 'center', flexGrow: 1, fontWeight: 'bold', padding: 5, color: appcolor.dark }}>T7</Text>
            <Text style={{ textAlign: 'center', flexGrow: 1, fontWeight: 'bold', padding: 5, color: appcolor.danger }}>CN</Text>
        </View>
    )
}
