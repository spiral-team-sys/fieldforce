...
const WorkingPlanSR = ({ navigation, route }) => {
    ...
    const RenderPressable = ({ index, item, styles, appcolor, typeBS, listSelect, value, titleStyle = null }) => {
        ...
        return (
            <View>
                <TouchableOpacity disabled={item.isLockShift == 1 ? true : false} style={[styles.inputShop, { flex: 1, flexDirection: 'row', alignItems: 'center' }]} onPress={handlePress}>
        ...
        return (
            <View key={index} >
                    ...
                    <View style={{ flex: 1 }}>
                        {item.checkWP > 0 &&
                            <View style={styles.viewWorking}>
                                ...
                                {item.supNote !== null && item.supNote.length > 0 && <Text style={[styles.titleView, { color: colorStatusShift }]} >{item.supNote}</Text>}
                                ...
                                <RenderTextInput index={index} typeInput="SHIFT" item={item} isLock={item.isLockShift} value={item.notes} handlenotes={handlenotes} styles={[styles.inputShop, { marginTop: 5 }]} />
                            </View>
                        }
                    </View>
                }
                ...
                {item.shopId !== item.employeeId &&
                    <ListItem.Content style={[highlightShop, { borderLeftColor: item.colorPlan }]}>
                        <View style={{ flex: 1 }} >
                            ...
                            <RenderTextInput index={index} typeInput="SHOP" item={item} isLock={item.isLockStatus} value={item.notes} handlenotes={handlenotes} styles={[styles.inputShop, { marginTop: 5 }]} />
    ...
    const styles = StyleSheet.create({
        ...
        viewWorking: { flex: 1, borderRadius: 10, backgroundColor: appcolor.homebackground, padding: 8, margin: 8 },
                    ...
                    <KeyboardAvoidingView
                        ...
                        <View style={{ padding: 5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: paddingTopIPX }}>
                            ...
                            <TouchableOpacity style={{ padding: 10 }} onPress={() => setVisibleBS(false)}>
                                <Icon name="times" size={25} color={appcolor.onSurfaceSecond} />
                            </TouchableOpacity>
                        </View>
