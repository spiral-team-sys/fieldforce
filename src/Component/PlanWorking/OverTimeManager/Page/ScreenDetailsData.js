...
export const ScreenDetailsData = ({ data }) => {
  ...
  const onUpdateItem = async itemUpdate => {
    ...
    MessageAction(
      `Bạn có muốn "Gửi yêu cầu cập nhật đề xuất" đăng kí tăng ca không ?`,
      async () => {
  ...
  const _searchData = filterList => {
    ...
    if (dataChooseTag.length == 0) dataChooseTag = filterList;
    //
    const searchData = _.filter(
      ...
      e =>
        ...
        removeVietnameseTones(e.WorkingDayView)
          .toLowerCase()
          .match(valueSearch),
    );
    return searchData;
    ...
    return (
      <View key={index} style={styles.itemContainer}>
        ...
        <View style={styles.itemContent}>
          <View style={styles.contentHead}>
            ...
            <TouchableOpacity
              ...
              style={{
                ...
                backgroundColor: appcolor[item.ColorStatusConfirm || 'light'],
              }}
              onPress={onRemove}
            >
              <Text style={styles.titleStatus}>{item.StatusConfirm}</Text>
              {item.isDelete == 1 && (
                <SpiralIcon type="ionicon" name="trash" size={18} />
              )}
            </TouchableOpacity>
          </View>
          <Text
            style={styles.titleContent}
          >{`- Nhân viên: ${item.EmployeeName} (${item.EmployeeCode})`}</Text>
          <Text
            style={styles.titleContent}
          >{`- Tăng ca: ${item.ShiftName} (${item.TimeFrom} ~ ${item.TimeTo})`}</Text>
          <Text
            style={styles.titleContent}
          >{`- Tổng thời gian: ${item.TotalTimeView}`}</Text>
          ...
          />
          <Text style={styles.titleTimeContent}>{`${moment(
            item.CreatedDate,
          ).fromNow()}`}</Text>
        </View>
      </View>
