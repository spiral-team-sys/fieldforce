...
export const ShiftList = ({ data = [], info, callIndex }) => {
    ...
    const onItemChoose = (item) => {
        ...
        if (dateValue.timeFrom) {
            dateValue.timeTo = moment(dateValue.timeFrom).add('hour', dateValue.shiftValue)
        }
        //
    ...
    const handlerSearchInfo = (text) => {
        ...
        const lstFilter = _.filter(dataMain, (e) => {
            return removeVietnameseTones(e.ShiftName).toLowerCase().match(valueSearch) ||
    ...
    const onChangeDate = (workingDay) => {
        ...
        if ((dateValue.timeFrom || null) !== null && (dateValue.timeTo || null) !== null && (dateValue.shiftCode || null) !== null) {
            callIndex(3)
        }
        const objectUpdate = { item: dateValue, type: 'SHIFT' }
        DeviceEventEmitter.emit('UPDATE_REQUEST_INFO', objectUpdate)
    ...
    const onChangeTime = (keyValue, time) => {
        try {
            ...
            if ((dateValue.timeFrom || null) !== null && (dateValue.timeTo || null) !== null && (dateValue.shiftCode || null) !== null) {
                if (moment(dateValue.timeFrom).isSameOrAfter(moment(dateValue.timeTo))) {
                    dateValue.timeTo = moment(dateValue.timeTo).add(1, 'days')
                }
                const totalTime = moment(dateValue.timeFrom).add('hours', dateValue.shiftValue)
                if (moment(dateValue.timeTo).isAfter(totalTime)) {
                    ToastError(`Ca ${dateValue.shiftName} có tổng thời gian là ${dateValue.shiftValue}h, Vui lòng chọn lại`, 'Thông báo', 'top')
                    dateValue.timeTo = null
                } else {
                    const diff = moment.duration(dateValue.timeTo.diff(dateValue.timeFrom));
    ...
    const handlerChangeData = (typeId) => {
        const dataShift = _.filter(data, (e) => (e.PositionId == typeId || e.PositionId == 0))
        setDataShift(dataShift)
    }
    //
    useEffect(() => {
        const _setData = DeviceEventEmitter.addListener('SET_DATA_SHIFT', handlerChangeData)
        LoadData()
        return () => {
            _setData.remove()
        }
    ...
    const styles = StyleSheet.create({
        ...
        contentItem: { flex: 1 },
        itemContainer: { width: '100%', flexDirection: 'row', alignItems: 'center', padding: 8, paddingHorizontal: 5, borderBottomWidth: 0.5, borderBottomColor: appcolor.grayLight },
        titleMain: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.blacklight, padding: 5, paddingHorizontal: 0 },
        ...
        itemShiftContainer: { minWidth: 80, marginEnd: 8, borderWidth: 0.5, borderColor: appcolor.grayLight, borderRadius: 5 },
        titleItem: { fontSize: 13, fontWeight: '700', color: appcolor.primary, padding: 8, textAlign: 'center' },
        titleShiftItem: { fontSize: 13, fontWeight: '700', color: appcolor.dark, padding: 8 },
        otViewDate: { width: '30%', borderWidth: 0.5, borderRadius: 5, borderColor: appcolor.primary, marginEnd: 8 },
        otViewTime: { width: '68%', borderWidth: 0.5, borderRadius: 5, borderColor: appcolor.primary },
        groupView: { width: '100%', flexWrap: 'wrap', flexDirection: 'row', paddingBottom: 8 },
            ...
            <View style={styles.itemContainer}>
                ...
                <View style={{ width: '70%' }}>
                    <TouchableOpacity onPress={onPress}>
                        ...
                        <Text style={styles.subTitleMain}>{`Mã ca: ${item.ShiftCode}`}</Text>
                        {item.From && <Text style={styles.subTitleMain}>{`Thời gian: ${item.From} - ${item.To}`}</Text>}
                    </TouchableOpacity>
                </View>
            </View>
        )
        ...
        <View style={styles.mainContainer}>
            ...
            <View style={styles.contentItem}>
                <View style={styles.viewDateValue}>
                    <DateChoose onChooseDate={onChangeDate} allowPast={config.allowPast} allowFuture={config.allowFuture} />
                    <TimeChoose
