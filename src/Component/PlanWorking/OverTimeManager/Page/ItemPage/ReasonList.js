...
export const ReasonList = ({ data, info }) => {
    ...
    const LoadData = async () => {
        await getDataPhotos(info.shopId, TODAY, 'OT_EVIDENT', kpiinfo.id, info.guid, false, setDataPhoto)
    ...
    const handlerSearchInfo = (text) => {
        ...
        const lstFilter = _.filter(dataMain, (e) => {
            return removeVietnameseTones(e.ReasonName).toLowerCase().match(valueSearch)
        })
    ...
    const handlerTakePicture = async () => {
        ...
        const options = [
            { text: 'Hủy' },
            { text: 'Chụp hình', onPress: async () => await NativeCamera.cameraStart(photoinfo, LoadData) },
            { text: 'Thư viện', onPress: async () => await NativeCamera.imageGalleryLaunch(photoinfo, LoadData) }
        ]
    ...
    const styles = StyleSheet.create({
        ...
        itemMainContainer: { width: '100%', alignItems: 'center', padding: 5, paddingHorizontal: 5, borderBottomWidth: 0.5, borderBottomColor: appcolor.grayLight },
        inputContainer: { backgroundColor: appcolor.background, borderRadius: 5, paddingHorizontal: 5, paddingVertical: 5 },
        inputStyle: { fontSize: 12, color: appcolor.homebackground },
        inputReasonStyle: { fontSize: 12, color: appcolor.dark },
        contentItem: { flex: 1 },
        itemContainer: { width: '100%', flexDirection: 'row', alignItems: 'center', padding: 8, paddingHorizontal: 5 },
        ...
        viewCamera: { flex: 1, alignItems: 'center', justifyContent: 'center', borderWidth: 0.5, borderColor: appcolor.primary, marginHorizontal: 4, padding: 8, borderRadius: 8 },
        viewAlbum: { width: 100, alignItems: 'center', borderWidth: 0.5, borderColor: appcolor.primary, marginHorizontal: 4, padding: 8, borderRadius: 8 },
        titleCamera: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.primary },
        viewCircle: { width: 18, height: 18, backgroundColor: appcolor.redgray, borderRadius: 24, justifyContent: 'center', alignItems: 'center', position: 'absolute', top: 8, end: 8 },
        ...
        return (
            <View style={styles.itemMainContainer}>
                <View style={styles.itemContainer}>
                    <View style={{ padding: 8, paddingStart: 0, justifyContent: 'center' }}>
                        <Icon
                            type="ionicon"
                    ...
                    <View style={{ width: '90%' }}>
                        <TouchableOpacity key={`spi_${index}`} onPress={onPress}>
                            <Text style={styles.titleMain}>{`${index + 1}. ${item.ReasonName}`}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
                {item.ReasonId == 100 &&
        ...
        <View style={styles.mainContainer}>
            ...
            <View style={styles.contentItem}>
                <View style={styles.contentImage}>
                    <TouchableOpacity style={styles.viewCamera} onPress={handlerTakePicture}>
                        <Text style={styles.titleCamera}>{`Hình ảnh chứng minh`}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.viewAlbum} onPress={handlerViewPicture}>
