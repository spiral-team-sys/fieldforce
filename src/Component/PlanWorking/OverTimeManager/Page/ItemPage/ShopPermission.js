...
export const ShopPermission = ({ data, info, callIndex }) => {
    ...
    const handlerSearchInfo = (text) => {
        ...
        const lstFilter = _.filter(dataMain, (e) => {
            return removeVietnameseTones(e.ShopCode).toLowerCase().match(valueSearch) ||
            ...
            <View style={styles.itemContainer}>
                ...
                <View style={{ width: '90%' }}>
                    <TouchableOpacity onPress={onPress}>
                        <Text style={styles.titleMain}>{`${index + 1}. ${item.ShopName}`}</Text>
                        <Text style={styles.subTitleMain}>{`Mã cửa hàng: ${item.ShopCode}`}</Text>
                        <Text style={styles.subTitleMain}>{`Địa chỉ: ${item.Address}`}</Text>
                    </TouchableOpacity>
                </View>
            </View>
        )
