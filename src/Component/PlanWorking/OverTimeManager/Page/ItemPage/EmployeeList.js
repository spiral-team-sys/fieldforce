...
export const EmployeeList = ({ data, info, callIndex }) => {
    ...
    const handlerSearchInfo = (text) => {
        ...
        const lstFilter = _.filter(dataMain, (e) => {
            return removeVietnameseTones(e.EmployeeName).toLowerCase().match(valueSearch) ||
            ...
            <View style={styles.itemContainer}>
                ...
                <View style={{ width: '90%' }}>
                    <TouchableOpacity key={`spi_${index}`} onPress={onPress}>
                        <Text style={styles.titleMain}>{`${index + 1}. ${item.EmployeeName}`}</Text>
                        <Text style={styles.subTitleMain}>{`Mã nhân viên: ${item.EmployeeCode}`}</Text>
                    </TouchableOpacity>
                </View>
            </View>
        )
