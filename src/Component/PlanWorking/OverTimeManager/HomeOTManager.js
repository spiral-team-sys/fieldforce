...
export const HomeOTManager = ({ navigation }) => {
      ...
      if (result.statusId == 200) {
        ...
        if (mCheckList !== null && mCheckList.length > 0) {
          ...
          const _contentCheck = `Cửa hàng ${_itemCheck.shopName}\nNhân viên ${
            _itemCheck.employeeName
          }\nĐã có lịch ngày ${moment(_itemCheck.workingDay).format(
            'DD/MM/YYYY',
          )} (${_itemCheck.shiftName} ${_itemCheck.timeFrom} - ${
            _itemCheck.timeTo
          })`;
          MessageInfo(_contentCheck);
        } else {
          const dataMessage = await getDataMessage();
          const dataPhoto = await getDataPhotos(
            requestInfo.shopId,
            TODAY,
            'OT_EVIDENT',
            kpiinfo.id,
          ...
          MessageAction(
            `Bạn có muốn "Gửi yêu cầu đề xuất" đăng kí tăng ca không ?`,
            async () => {
  ...
  const getTimeOnWorkingDay = (value, workingDay) => {
    ...
    const time = moment(
      value,
      ['HH:mm', 'YYYY-MM-DD HH:mm:ss', moment.ISO_8601],
      true,
    );
  ...
  const validData = () => {
    ...
    if (
      requestInfo.workingDay == null ||
      requestInfo.timeFrom == null ||
      requestInfo.timeTo == null
    ) {
    ...
    if (maximumMinutes > 0 && durationMinutes > maximumMinutes) {
      ToastError(
        `Ca ${requestInfo.shiftName} tối đa ${requestInfo.shiftValue}h, vui lòng chọn lại thời gian`,
        'Dữ liệu',
        'top',
      );
      return false;
    ...
    for (const item of dataManager.dataDetails) {
      ...
      const isOverlap =
        requestFrom.isBefore(itemTo) && requestTo.isAfter(itemFrom);
      if (isOverlap) {
        ToastError(
          `Đã có lịch ngày ${moment(requestInfo.workingDay).format(
            'DD/MM/YYYY',
          )} (${item.ShiftName} ${item.TimeFrom} - ${item.TimeTo})`,
          'Dữ liệu',
          'top',
        );
        return false;
    ...
    if (requestInfo.reasonId == 0) {
      ...
      requestInfo.reasonId == 100 &&
      (requestInfo.reasonOther || '').length == 0
    ) {
  ...
  const getDataMessage = () => {
    ...
    return [
      { title: _title, content: _content, parentList: dataMain.parentList },
    ];
  };
  const onBack = () => {
    navigation.goBack();
  ...
  useEffect(() => {
    ...
    const _requestinfo = DeviceEventEmitter.addListener(
      'UPDATE_REQUEST_INFO',
      handlerUpdateInfo,
    );
    LoadData();
