...
const DateChoose = ({ onChooseDate, allowPast = false, allowFuture = false }) => {
    ...
    const handlerChooseDate = (date) => {
        ...
        if (!allowFuture && workingDay > today) {
            ToastError(`Không được phép chọn ngày quá tương lai`, 'Chọn ngày lại', 'top')
            setOpen(false)
            return
        }
        setDateValue(date)
    ...
    const styles = StyleSheet.create({
        ...
        titleView: { fontSize: 13, lineHeight: 19, fontWeight: "700", color: appcolor.primary },
        ...
        <View style={styles.containerStyle}>
            <TouchableOpacity activeOpacity={0.8} onPress={handlerChange} style={styles.mainContainer}>
                <View style={styles.iconContainer}>
                    <Icon name="calendar" type="feather" size={15} color={appcolor.primary} />
                </View>
                <View style={styles.content}>
                    <Text style={styles.label}>Ngày</Text>
                    <Text style={styles.titleView}>{moment(dateValue).format("DD/MM/YYYY")}</Text>
                </View>
            </TouchableOpacity>
            <DatePicker
                modal
