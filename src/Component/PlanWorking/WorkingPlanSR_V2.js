...
import { View, StyleSheet, Switch, TouchableOpacity, SafeAreaView, RefreshControl, Platform, Modal, BackHandler } from "react-native";
import { HeaderCustom } from "../../Content/HeaderCustom";
import { SR_PLAN_GETLIST, PLANSR_ACTION } from "../../Controller/PlanController";
import { alertError, alertWarning, alertNotify, alertConfirm, TODAY, optionConfirm } from "../../Core/Utility";
import { LoadingView } from "../../Control/ItemLoading/index";
import { groupDataByKey, ToastSuccess, UUIDGenerator } from "../../Core/Helper";
...
const WorkingPlanSR_V2 = ({ navigation, route }) => {
    ...
    const [isDownload, setDownloading] = useState(false)
    ...
    const [guidByDate, setGuidByDate] = useState(null)
    ...
    const LoadDataPlan = async (dateSelect) => {
        ...
        const dataHeader = {
            ...
            "Week": moment(dateSelect).isoWeek() + 1,
        ...
        await SR_PLAN_GETLIST(dataHeader, async (mDataPlan, mDataWeek, mDataIndex) => {
            ...
            let guid = mDataPlan?.[0]?.guid || UUIDGenerator()
            await setGuidByDate(guid)
            ...
            await getDataPhotos(0, TODAY, 'PLAN_EVIDENT', kpiinfo.id, guid || 'none', false, setDataPhoto)
            await getDataPhotos(0, TODAY, 'PLAN_VIDEO_EVIDENT', kpiinfo.id, guid || 'none', false, setDataVideo)
        }, (error) => {
    ...
    const handlerSave = async () => {
        if (isSubmitting || isDownload) return
        ...
        if (listModified.length > 0) {
            ...
            data.dataMain.forEach((i, index) => {
                ...
                if (i.confirmShop == 3) {
                    if (i.lastStatus == 'ADD' || i.lastStatus == 'DEL') {
                        ...
                        if (i.checkNoteOnClose == 1 && i.lastStatus == 'DEL') {
                            if (dataNote.length == 0) {
                                if (i.notes == null || i.notes.length == 0) {
                                    checkData = "Vui lòng nhập lí do thay đổi cửa hàng " + i.shopName;
                                } else if (i.notes.length < 5) {
            ...
            alertConfirm("Thông báo", "Bạn có muốn thay đổi lịch làm việc như bên dưới không ?", async () => {
                setSubmitting(true)
                try {
                    const photoUpload = await getDataPhotos(0, TODAY, 'PLAN_EVIDENT', kpiinfo.id, guidByDate || 'none', true)
                    const videoUpload = await getDataPhotos(0, TODAY, 'PLAN_VIDEO_EVIDENT', kpiinfo.id, guidByDate || 'none', true)
                    const dataUpload = {
                        dataPlan: JSON.stringify(dataSave),
                        contentMessage: contentMessage,
                        jsonPhoto: JSON.stringify(_.concat(photoUpload, videoUpload)),
                    }
                    const result = await PLANSR_ACTION(dataUpload, itemOFF.parentList)
                    if (result.statusId === 200) {
                        await handlerUpdateStoreData()
                        //
                        await setUpdate(true)
                        await LoadDataPlan(configDate.dateChoose)
                        await alertNotify(result.messager)
                        // Upload Photo
                        await QueryStringSql(`UPDATE ${photos.tableName} SET dataUpload=1 WHERE shopId=0 AND reportId=${kpiinfo.id} AND photoDate=${TODAY}`)
                        await onReloadPhoto()
                        await onReloadVideo()
                    ...
                    alertError('Gửi dữ liệu lỗi, vui lòng thử lại sau (' + error + ')')
                } finally {
                    setSubmitting(false)
                }
            })
    ...
    const handlerUnPlan = async () => {
        ...
        if (data.dataMain.length > 0) {
            ...
            contentMessage = "Nhân viên " + itemOFF.fullName + " yêu cầu gỡ phép ngày " + itemOFF.dateView
            dataSave.push(itemSaveOFF);
            alertConfirm("Thông báo", "Bạn có muốn gỡ phép ngày không ?", async () => {
                setSubmitting(true)
                try {
            ...
            <View key={index} style={{ flex: 1 }}>
                {item.isParent ?
                    <ViewItemOFF
                        ...
                        actionUnPlanOFF={handlerUnPlan}
                        handlerTakePicture={handlerTakePicture}
                        handlerTakeVideo={handlerTakeVideo}
                        handlerViewPicture={handlerViewPicture}
                        handlerViewVideo={handlerViewVideo}
                    />
                    :
                    <ViewItemStore
                        appcolor={appcolor}
    ...
    const handleGoBack = () => {
        if (!isDownload && !isSubmitting)
            navigation.goBack()
    }
    const handlerTakePicture = async () => {
        let photoinfo = {
            "photoType": 'PLAN_EVIDENT',
            "dataUpload": 0,
            "fileUpload": 0,
            "shopId": 0,
            "reportId": kpiinfo.id,
        ...
        const options = [
            { text: 'Hủy' },
            { text: 'Chụp hình', onPress: async () => await NativeCamera.cameraStart(photoinfo, onReloadPhoto) },
            { text: 'Thư viện', onPress: async () => await NativeCamera.imageGalleryLaunch(photoinfo, onReloadPhoto) }
        ]
        optionConfirm('Hình ảnh chứng minh', 'Chọn hình ảnh từ thư viện hoặc bạn tự chụp bằng máy ảnh', options)
    }
    const handlerTakeVideo = async () => {
        let videoInfo = {
            "photoType": 'PLAN_VIDEO_EVIDENT',
            "dataUpload": 0,
            "fileUpload": 0,
            "shopId": 0,
            "tag": "VIDEO",
        ...
        const options = [
            { text: 'Hủy' },
            { text: 'Quay', onPress: async () => await NativeCamera.videoStart(videoInfo, onReloadVideo) },
            { text: 'Thư viện', onPress: async () => await NativeCamera.videoGalleryLaunch(videoInfo, onReloadVideo) }
        ]
        optionConfirm('Video chứng minh', 'Chọn video từ thư viện hoặc bạn tự quay một đoạn bằng máy ảnh', options)
    }
    const onReloadPhoto = async () => {
        await getDataPhotos(0, TODAY, 'PLAN_EVIDENT', kpiinfo.id, guidByDate || 'none', false, setDataPhoto)
    }
    const onReloadVideo = async () => {
        await getDataPhotos(0, TODAY, 'PLAN_VIDEO_EVIDENT', kpiinfo.id, guidByDate || 'none', false, setDataVideo)
    }
    ...
    const handlerViewVideo = () => {
        if (dataVideo.length > 0)
            SheetManager.show('media-video-show', { payload: dataVideo })
    }

    const styles = StyleSheet.create({
        mainContainer: { flex: 1, backgroundColor: appcolor.light },
        itemOFFView: { flex: 1, marginBottom: 8, backgroundColor: appcolor.surface, padding: 8, paddingTop: 0, borderRadius: 8, borderWidth: 1, borderColor: appcolor.grayLight, elevation: 3, shadowOffset: { width: 0, height: 3 }, shadowColor: appcolor.dark, shadowOpacity: 0.5, shadowRadius: 8 },
        ...
        titleShiftType: { flex: 1, fontSize: 13, fontWeight: '700', fontStyle: "italic", color: appcolor.dark },
        chooseItem: { flex: 1, padding: 10, borderRadius: 8, borderRadius: 8, borderWidth: 0.5, borderColor: appcolor.grey },
        statusText: { width: '100%', fontSize: 12, fontWeight: '700', color: appcolor.dark, padding: 8, paddingBottom: 5 },
        filterStyle: { marginStart: 8, marginEnd: 8 },
        overflowView: { ...StyleSheet.absoluteFillObject, zIndex: 2000, elevation: 2000, backgroundColor: appcolor.dark, opacity: 0.8, justifyContent: 'center' }
    })
    useEffect(() => {
        const isActionLocked = isSubmitting || isDownload
        const removeBeforeRemove = navigation.addListener('beforeRemove', (event) => {
            if (!isActionLocked) return
            event.preventDefault()
        })
        const backSubscription = BackHandler.addEventListener('hardwareBackPress', () => isActionLocked)

        return () => {
            removeBeforeRemove()
            backSubscription.remove()
        }
    }, [isSubmitting, isDownload, navigation])
    useEffect(() => {
        LoadDataPlan()
    }, [])
    return (
        <View style={styles.mainContainer}>
            <HeaderCustom
                title={route?.params?.menuitem.menuNameVN || "Lịch làm việc"}
                leftFunc={handleGoBack}
                iconRight='cloud-upload-alt'
                rightFunc={handlerSave}
                disabled={isSubmitting || isDownload}
            />
            <ReactNativeCalendarStrip
                style={{ flexDirection: 'row', alignItems: 'center', paddingBottom: 16 }}
                minDate={'2023-01-01'}
                calendarHeaderStyle={{ color: 'white', marginBottom: 16, fontSize: 14 }}
                calendarColor={DEFAULT_COLOR}
                dateNumberStyle={{ color: 'white', fontSize: 12 }}
                dateNameStyle={{ color: 'white', fontSize: 12, paddingTop: 5 }}
                customDatesStyles={configDate.customDatesStyles}
                highlightDateContainerStyle={{ backgroundColor: 'white', width: 38, height: 38, paddingTop: 5, borderRadius: 8 }}
                highlightDateNumberStyle={{ color: 'black', fontSize: 12 }}
            ...
            <SearchData
                ...
                iconName='search'
                containerStyle={{ margin: 8, padding: 3 }}
                inputStyle={{ fontSize: 12 }}
                onSearchData={searchShops}
            />
            <LoadingView title={'Đang cập nhật dữ liệu'} isLoading={refreshing} styles={{ zIndex: 200, marginTop: 8 }} />
            {(isDownload || isSubmitting) &&
                <View style={styles.overflowView}>
                    <LoadingView
                        titleStyle={{ color: appcolor.light }}
                        title={isSubmitting ? 'Đang gửi dữ liệu lên hệ thống' : 'Đang cập nhật danh sách cửa hàng làm việc'}
                        isLoading={isDownload || isSubmitting} />
                </View>
            }
            <FlashList
                keyExtractor={(_, index) => index.toString()}
                data={data.mPlanList}
                extraData={[data.mPlanList]}
...
// Render Child View 
const ViewItemOFF = ({
    item, index, styles, visibleView, actionNotes, actionChooseItem, appcolor, actionUnPlanOFF, dataPhoto, dataVideo,
    handlerTakePicture, handlerViewPicture, handlerTakeVideo, handlerViewVideo
}) => {
    ...
    const styleItem = StyleSheet.create({
        contentImage: { width: '100%', flexDirection: 'row', marginTop: 8 },
        viewCamera: { flex: 1, alignItems: 'center', justifyContent: 'center', borderWidth: 0.5, borderColor: appcolor.primary, borderRadius: 8, marginEnd: 8 },
        viewAlbum: { width: 60, alignItems: 'center', borderWidth: 0.5, borderColor: appcolor.primary, padding: 8, borderRadius: 8 },
        titleCamera: { fontSize: 12, fontWeight: fontWeightBold, color: appcolor.primary },
        viewCircle: { width: 18, height: 18, backgroundColor: appcolor.redgray, borderRadius: 24, justifyContent: 'center', alignItems: 'center', position: 'absolute', top: 3, end: 5 },
        titleCount: { fontSize: 11, fontWeight: fontWeightBold, color: appcolor.light }
    })

    return (
        <View key={index} style={[styles.itemOFFView, { display: visibleView }]}>
            {(item.phepTon || null) !== null && <Text style={[styles.statusText, { color: appcolor.tomato, fontSize: 13, fontStyle: 'italic' }]} >{item.phepTon}</Text>}
            <Text style={[styles.statusText, { color: colorStatusShift }]} >{item.shiftStatus}</Text>
            {item.supNote !== null && item.supNote.length > 0 &&
                <Text style={[styles.statusText, { color: colorStatusShift }]} >{item.supNote}</Text>
            }
            ...
            {item.isUnPlan == 1 &&
                <TouchableOpacity style={{ width: 100, padding: 8, backgroundColor: appcolor.red, borderRadius: 5, position: 'absolute', top: 8, end: 8 }} onPress={actionUnPlanOFF}>
                    <Text style={{ textAlign: 'center', color: appcolor.light }}>Gỡ phép</Text>
                </TouchableOpacity>
            }
            {/* {item.ref_Code == 'OFF' && */}
            <View style={styleItem.contentImage}>
                <TouchableOpacity style={styleItem.viewCamera} onPress={handlerTakePicture}>
                    <Text style={styleItem.titleCamera}>{`Hình ảnh`}</Text>
                </TouchableOpacity>
                <TouchableOpacity style={{ ...styleItem.viewAlbum, marginEnd: 8 }} onPress={handlerViewPicture}>
                    <Icon type="ionicon" name="images" size={18} color={appcolor.primary} />
                    <View style={styleItem.viewCircle}>
                        <Text style={styleItem.titleCount}>{dataPhoto.length}</Text>
                    </View>
                </TouchableOpacity>
                <TouchableOpacity style={styleItem.viewCamera} onPress={handlerTakeVideo}>
                    <Text style={styleItem.titleCamera}>{`Video`}</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styleItem.viewAlbum} onPress={handlerViewVideo}>
                    <Icon type="ionicon" name="videocam" size={18} color={appcolor.primary} />
                    <View style={styleItem.viewCircle}>
                        <Text style={styleItem.titleCount}>{dataVideo.length}</Text>
                    </View>
                </TouchableOpacity>
            </View>
            {/* } */}
...
const ViewItemStore = ({ item, index, styles, onValueChange, actionNotes, appcolor }) => {
    ...
    const visibleMonthlyFrequency = (item.monthlyFrequency?.length > 0 && item.lastDate?.length > 0) ? 'flex' : 'none'
    return (
        <View key={index} style={styles.itemShopView}>
            <Text style={{ display: visibleStatusChange, width: '100%', fontSize: 12, fontWeight: '700', color: colorStatusChange }} >{item.statusChangeShop}</Text>
            <Text style={{ display: visibleSupNoteConfirm, width: '100%', fontSize: 12, fontWeight: '700', color: appcolor.danger }}>Quản lý ghi chú: {item.confirmPlanNote}</Text>
            <View style={{ flex: 8, flexDirection: 'row', alignItems: 'center' }}>
                <Icon name='store' size={21} color={appcolor.dark} />
                <Text style={{ width: '80%', paddingStart: 3, paddingEnd: 8, fontSize: 13, fontWeight: '700', color: visibleMonthlyFrequency === 'flex' ? (item.colorHighlight || appcolor.dark) : (item.titleStoreColor || appcolor.dark) }} >Cửa hàng: {item.shopName}</Text>
                {item.isLocked == 0 &&
                    <Switch
            ...
            <View style={{ flex: 2, justifyContent: 'center' }} >
                <Text style={{ width: '80%', fontSize: 12, color: appcolor.greylight, fontWeight: '500' }}>{item.shopCode} </Text>
                <Text style={{ width: '80%', fontSize: 12, color: appcolor.greylight, fontWeight: '500' }}>{item.address} </Text>
                {item.actualFrequency !== null && <Text style={{ display: visibleMonthlyFrequency, width: '80%', fontSize: 12, color: appcolor.dark }} >{item.actualFrequency}</Text>}
                <Text style={{ display: visibleMonthlyFrequency, width: '80%', fontSize: 12, color: appcolor.dark }} >{item.monthlyFrequency}</Text>
                <Text style={{ display: visibleMonthlyFrequency, width: '80%', fontSize: 12, color: appcolor.dark }} >{item.lastDate}</Text>
                <RenderTextInput
                    styles={styles} appcolor={appcolor}
                    index={index} typeInput="SHOP" item={item}
                    isLock={item.isLockStatus} value={item.notes} handlenotes={actionNotes} />
...
const renderItemModal = (item, index, dataModal, actionChooseItem, actionClearItem, appcolor) => {
    ...
    return (
        <View key={`${dataModal.type || 'modal'}_${item.Ref_Code || item.ShiftCode || item.Id || item.ShopId || index}`}>
            {item.isParent && <Text style={{ width: '100%', fontSize: 13, fontWeight: fontWeightBold, color: appcolor.danger, padding: 8 }} >{item.Ref_Code}</Text>}
            <View style={{ flexDirection: 'row', backgroundColor: item.isSelect == 1 ? appcolor.primary : appcolor.surface, marginBottom: 8, borderRadius: 8 }}>
                <TouchableOpacity style={{ flex: 8, padding: 12 }} onPress={chooseItem}>
                    <Text style={{ width: '80%', fontSize: 13, fontWeight: fontWeightBold, color: item.isSelect == 1 ? appcolor.white : appcolor.dark }} >{value}</Text>
                    <Text style={{ width: '80%', fontSize: 11, fontWeight: '500', color: item.isSelect == 1 ? appcolor.grayLight : appcolor.greylight }} >{item.ShiftCode}</Text>
                </TouchableOpacity>
                {item.isSelect == 1 &&
                    <TouchableOpacity style={{ padding: 12, alignItems: 'center' }} onPress={clearItem} >
                        <Icon name="backspace" size={18} color={appcolor.white} />
                    </TouchableOpacity>
                }
            </View>
        </View>
...
const ModalItem = ({ dataModal, styles, actionSearch, actionResultModal, actionChooseItem, actionClearItem }) => {
    ...
    const titleModal = dataModal.type == 'SHIFT' ? 'Đổi ca làm việc' : (dataModal.type == 'NOTE' ? 'Lý do thay đổi' : 'Danh sách cửa hàng')
    const { arr } = groupDataByKey({
            ...
            <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.light, }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 8 }}>
                    ...
                    <TouchableOpacity style={{ padding: 10 }} onPress={actionResultModal}>
                        <Icon name="close" size={25} color={appcolor.dark} />
                    </TouchableOpacity>
                </View>
                <SearchData
...
// Control
const RenderTextInput = ({ index, typeInput, item, value, handlenotes, isLock, appcolor }) => {
    const handleChange = (inputText) => {
        handlenotes(index, inputText, item, typeInput)
    }
    const styles = StyleSheet.create({
        containerInput: { flex: 1, color: appcolor.dark, paddingVertical: 3, paddingHorizontal: 0, marginTop: 8, marginBottom: 0, borderRadius: 8, borderWidth: 0.5, borderColor: appcolor.grey, fontSize: 12, textAlignVertical: 'center', backgroundColor: 'transparent' },
        inputView: { fontSize: 12, color: appcolor.dark }
    })
    return (
        <View>
            <FormGroup
...
const RenderChooseItem = ({ index, item, styles, appcolor, typeBS, listSelect, value, titleStyle = null, handlerChooseItem }) => {
    ...
    return (
        <TouchableOpacity key={index} disabled={item.isLockShift == 1 ? true : false} style={styles.chooseItem} onPress={handlerChoose}>
            <Text style={{ fontSize: 12, fontWeight: '400', color: appcolor.dark }} >{value}</Text>
        </TouchableOpacity>
    )
}
export default (WorkingPlanSR_V2);
