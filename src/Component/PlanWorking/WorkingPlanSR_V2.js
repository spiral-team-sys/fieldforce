import React, { useState, useEffect, useRef } from 'react';
import {
  View,
  StyleSheet,
  Switch,
  TouchableOpacity,
  SafeAreaView,
  RefreshControl,
  Platform,
  Modal,
  BackHandler,
  KeyboardAvoidingView,
} from 'react-native';
import { HeaderCustom } from '../../Content/HeaderCustom';
import {
  SR_PLAN_GETLIST,
  PLANSR_ACTION,
} from '../../Controller/PlanController';
import {
  alertError,
  alertWarning,
  alertNotify,
  alertConfirm,
  TODAY,
  optionConfirm,
} from '../../Core/Utility';
import { LoadingView } from '../../Control/ItemLoading/index';
import { groupDataByKey, ToastSuccess, UUIDGenerator } from '../../Core/Helper';
import FormGroup from '../../Content/FormGroup';
import ReactNativeCalendarStrip from 'react-native-calendar-strip';
import { DEFAULT_COLOR } from '../../Core/URLs';
import moment from 'moment';
import { useSelector } from 'react-redux';
import { Icon, Text } from '@rneui/themed';
import {
  APPDOWNLOAD,
  downloadAll,
} from '../../Controller/DownloadDataController';
import { FlashList } from '@shopify/flash-list';
import { SearchData } from '../../Control/SearchData/SearchData';
import { deviceHeight, fontWeightBold } from '../../Themes/AppsStyle';
import { getDataPhotos } from '../../Controller/PhotoController';
import NativeCamera from '../../Control/NativeCamera';
import { MultipleShowImage } from '../../Control/MultipleShowImage';
import { REPORT } from '../../API/ReportAPI';
import { QueryStringSql } from '../../Core/SqliteDbContext';
import { photos } from '../../Core/TableLocal';
import MultipleShowVideo from '../../Control/Video/MultipleShowVideo';
import { SheetManager } from 'react-native-actions-sheet';
import _ from 'lodash';

const WorkingPlanSR_V2 = ({ navigation, route }) => {
  const { appcolor, kpiinfo } = useSelector(state => state.GAppState);
  const [refreshing, setRefreshing] = useState(false);
  const [isDownload, setDownloading] = useState(false);
  const [data, setData] = useState({ mPlanList: [], dataMain: [] });
  const [dataModal, setDataModal] = useState({
    type: '',
    dataSelect: [],
    dataFilter: [],
    visibleModal: false,
  });
  const [configDate, setConfigDate] = useState({
    dateChoose: moment().format('YYYY-MM-DD'),
    markedDates: [],
    customDatesStyles: [],
  });
  const [dataModified, setDataModified] = useState({});
  const [isUpdate, setUpdate] = useState(false);
  const [mListIndex, setListIndex] = useState([]);
  const [dataPhoto, setDataPhoto] = useState([]);
  const [dataVideo, setDataVideo] = useState([]);
  const [images, _setImages] = useState({ data: [], isShow: false, index: 0 });
  const [_mutate, setMutate] = useState(false);
  const [guidByDate, setGuidByDate] = useState(null);
  const [isSubmitting, setSubmitting] = useState(false);
  const [isListAtTop, setListAtTop] = useState(true);
  const listRef = useRef(null);

  // Config Data
  const LoadDataPlan = async dateSelect => {
    await setRefreshing(true);
    await setDataModified({});
    setListAtTop(true);
    const dataHeader = {
      PlanDate: parseInt(moment(dateSelect).format('YYYYMMDD')),
      Week: moment(dateSelect).isoWeek() + 1,
      Year: moment(dateSelect).year(),
    };
    await SR_PLAN_GETLIST(
      dataHeader,
      async (mDataPlan, mDataWeek, mDataIndex) => {
        const { arr } = groupDataByKey({
          arr: mDataPlan,
          key: 'auditDate',
        });
        let guid = mDataPlan?.[0]?.guid || UUIDGenerator();
        await setGuidByDate(guid);
        await setData({ mPlanList: arr, dataMain: arr });
        await ConfigCalendar(dateSelect, mDataWeek);
        await setListIndex(
          mDataIndex.length > 0
            ? JSON.parse(mDataIndex[0].mListIndex || '[]') || []
            : [],
        );
        await getDataPhotos(
          0,
          TODAY,
          'PLAN_EVIDENT',
          kpiinfo.id,
          guid || 'none',
          false,
          setDataPhoto,
        );
        await getDataPhotos(
          0,
          TODAY,
          'PLAN_VIDEO_EVIDENT',
          kpiinfo.id,
          guid || 'none',
          false,
          setDataVideo,
        );
      },
      error => {
        alertError('Dữ liệu lỗi, vui lòng thử lại sau (' + error + ')');
      },
    );
    await setRefreshing(false);
  };
  const ConfigCalendar = async (dateSelect, mDataWeek) => {
    let markedDates = [],
      customDatesStyles = [];
    mDataWeek.forEach(e => {
      let i = 0;
      let date = moment(e.planDate).format('YYYY-MM-DD');
      let dots = [];
      dots.push({ color: e.colorPlan });
      markedDates.push({ date, dots });
      i++;
    });
    const ok = '#ff6600';
    customDatesStyles.push({
      startDate: new Date(),
      dateNameStyle: { color: ok },
      dateNumberStyle: { color: ok },
      highlightDateNameStyle: { color: ok },
      highlightDateNumberStyle: { color: ok },
    });
    await setConfigDate({
      dateChoose:
        dateSelect == undefined ? moment().format('YYYY-MM-DD') : dateSelect,
      markedDates: markedDates,
      customDatesStyles: customDatesStyles,
    });
  };
  const assignData = async index => {
    dataModified[index] = true;
  };
  const handlerSave = async () => {
    if (isSubmitting || isDownload) return;
    const listModified = Object.keys(dataModified);
    if (listModified.length > 0) {
      let countShopAdd = 0;
      let contentMessage = '';
      let checkData = null;
      let dataSave = [];

      let itemOFF = data.dataMain[0];
      // Check Off
      let dataShiftOFF = JSON.parse(itemOFF.shiftList).filter(
        c => c.Name == itemOFF.shiftChange,
      );
      let dataNote = JSON.parse(itemOFF.refList || '[]').filter(
        s => s.title == itemOFF.reasonName,
      );

      if (dataShiftOFF.length > 0 && dataNote.length == 0) {
        checkData = 'Vui lòng nhập lí do xin nghỉ phép';
      } else if (
        dataShiftOFF.length == 0 &&
        dataNote.length > 0 &&
        dataNote[0].Ref_Code == 'OFF'
      ) {
        checkData = 'Vui lòng chọn loại nghỉ phép';
      } else {
        if (dataNote.length > 0 && dataNote[0].Id == 100) {
          if (itemOFF.notes == null || itemOFF.notes.length == 0) {
            checkData = 'Vui lòng nhập lí do khác';
          } else if (itemOFF.notes.length < 5) {
            checkData = 'Lí do quá ngắn, vui lòng nhập lại';
          }
        }
      }
      let itemSaveOFF = {
        PlanId: itemOFF.planId,
        ShopId: itemOFF.shopId,
        PlanDate: itemOFF.auditDate,
        Status: itemOFF.status ? 1 : 0,
        LastStatus: itemOFF.lastStatus,
        ShiftType: itemOFF.shiftType,
        ShiftChange: dataShiftOFF.length > 0 ? dataShiftOFF[0].ShiftCode : null,
        ReasonId: dataNote.length > 0 ? dataNote[0].Id : null,
        Notes:
          dataNote.length > 0
            ? dataNote[0].Id == 100
              ? itemOFF.notes
              : dataNote[0].title
            : null,
        SupConfirm: itemOFF.supConfirm,
        Guid: guidByDate,
      };
      dataSave.push(itemSaveOFF);
      // Data Status Add Shop
      data.dataMain.forEach((i, index) => {
        if (index == 0) return;
        // Check Add-Del Shop
        if (i.confirmShop == 3) {
          if (i.lastStatus == 'ADD' || i.lastStatus == 'DEL') {
            if (i.auditDate == moment(new Date()).format('YYYY-MM-DD')) {
              countShopAdd++;
            }
            if (i.checkNote == 1) {
              // if (moment(new Date()).isoWeek() == moment(configDate.dateChoose).isoWeek()) {
              if (dataNote.length == 0) {
                if (i.notes == null || i.notes.length == 0) {
                  checkData =
                    'Vui lòng nhập lí do thay đổi cửa hàng ' + i.shopName;
                } else if (i.notes.length < 5) {
                  checkData =
                    'Lí do thay đổi cửa hàng ' + i.shopName + ' quá ngắn';
                }
              }
              // }
            }
            if (i.checkNoteOnClose == 1 && i.lastStatus == 'DEL') {
              if (dataNote.length == 0) {
                if (i.notes == null || i.notes.length == 0) {
                  checkData =
                    'Vui lòng nhập lí do thay đổi cửa hàng ' + i.shopName;
                } else if (i.notes.length < 5) {
                  checkData =
                    'Lí do thay đổi cửa hàng ' + i.shopName + ' quá ngắn';
                }
              }
            }
            let itemSave = {
              PlanId: i.planId,
              ShopId: i.shopId,
              PlanDate: i.auditDate,
              Status: i.status ? 1 : 0,
              LastStatus: i.lastStatus,
              ShiftType: i.shiftType,
              ShiftChange: null,
              ReasonId: null,
              Notes: i.notes,
              ConfirmPlan: i.confirmShop,
            };
            dataSave.push(itemSave);
          }
        }
        contentMessage =
          'Nhân viên ' +
          i.fullName +
          ' yêu cầu thay đổi lịch làm việc ngày ' +
          i.dateView;
      });
      if (checkData !== null) {
        alertWarning(checkData);
        return;
      }

      alertConfirm(
        'Thông báo',
        'Bạn có muốn thay đổi lịch làm việc như bên dưới không ?',
        async () => {
          setSubmitting(true);
          try {
            const photoUpload = await getDataPhotos(
              0,
              TODAY,
              'PLAN_EVIDENT',
              kpiinfo.id,
              guidByDate || 'none',
              true,
            );
            const videoUpload = await getDataPhotos(
              0,
              TODAY,
              'PLAN_VIDEO_EVIDENT',
              kpiinfo.id,
              guidByDate || 'none',
              true,
            );
            const dataUpload = {
              dataPlan: JSON.stringify(dataSave),
              contentMessage: contentMessage,
              jsonPhoto: JSON.stringify(_.concat(photoUpload, videoUpload)),
            };
            const result = await PLANSR_ACTION(dataUpload, itemOFF.parentList);
            if (result.statusId === 200) {
              await handlerUpdateStoreData();
              //
              await setUpdate(true);
              await LoadDataPlan(configDate.dateChoose);
              await alertNotify(result.messager);
              // Upload Photo
              await QueryStringSql(
                `UPDATE ${photos.tableName} SET dataUpload=1 WHERE shopId=0 AND reportId=${kpiinfo.id} AND photoDate=${TODAY}`,
              );
              await onReloadPhoto();
              await onReloadVideo();
              await REPORT.UploadFilePhoto();
            } else {
              alertError(result.messager);
            }
          } catch (error) {
            alertError('Gửi dữ liệu lỗi, vui lòng thử lại sau (' + error + ')');
          } finally {
            setSubmitting(false);
          }
        },
      );
    } else {
      alertWarning(
        'Bạn chưa có yêu cầu thay đổi lịch làm việc nào, vui lòng thay đổi trước khi gửi dữ liệu lên hệ thống',
      );
    }
  };
  // Handler Action
  const handlerNotes = async (index, text, item, typeInput) => {
    switch (typeInput) {
      case 'SHIFT':
      case 'SHOP':
        data.mPlanList[index].notes = text;
        data.dataMain[item.mainIndex].notes = text;
        break;
      case 'LATE':
        data.mPlanList[index].noteLate = text;
        data.dataMain[item.mainIndex].noteLate = text;
        break;
      case 'EARLIER':
        data.mPlanList[index].noteEarlier = text;
        data.dataMain[item.mainIndex].noteEarlier = text;
        break;
    }
    setMutate(e => !e);
  };
  const handlerShopAction = async (item, index, value) => {
    let valueChange = value ? 1 : 0;
    let statusChange = item.status;
    let lastStatus = null;
    if (valueChange === item.statusMain) {
      statusChange = item.statusMain;
      lastStatus = null;
    } else {
      statusChange = valueChange;
      lastStatus = valueChange == 0 ? 'DEL' : 'ADD';
    }
    // Update dong gia
    data.mPlanList[0].lastStatus = lastStatus;
    //
    data.mPlanList[index].status = statusChange;
    data.mPlanList[index].lastStatus = lastStatus;
    data.mPlanList[index].confirmShop = 3;
    data.dataMain[item.mainIndex].status = statusChange;
    data.dataMain[item.mainIndex].lastStatus = lastStatus;
    data.dataMain[item.mainIndex].confirmShop = 3;
    assignData(index);
    setMutate(e => !e);
  };
  const searchShops = async text => {
    if (text && text !== undefined) {
      let filterShops = data.dataMain.filter(
        i =>
          i.mainIndex === 0 ||
          i.shopName?.toLowerCase().match(text.toLowerCase()) ||
          i.address?.toLowerCase().match(text.toLowerCase()) ||
          i.shopCode.toLowerCase().match(text.toLowerCase()),
      );
      await setData({ ...data, mPlanList: filterShops });
    } else {
      await setData({ ...data, mPlanList: data.dataMain });
    }
  };
  const handlerShowViewChoose = async (index, item, listSelect, typeBS) => {
    switch (typeBS) {
      case 'NOTE':
        let changeItem =
          item.ref_Code == null
            ? listSelect.filter(i => i.Ref_Code == 'ON')
            : listSelect.filter(i => i.Ref_Code == item.ref_Code);
        const mModalNote = {
          type: typeBS,
          dataSelect: changeItem.map(i =>
            i.title == item.reasonName ? { ...i, isSelect: 1 } : i,
          ),
          dataFilter: changeItem,
          visibleModal: true,
        };
        setDataModal(mModalNote);
        break;
      case 'SHIFT':
        const mModalShift = {
          type: typeBS,
          dataSelect: listSelect.map(i =>
            i.Name == item.shiftChange ? { ...i, isSelect: 1 } : i,
          ),
          dataFilter: listSelect,
          visibleModal: true,
        };
        setDataModal(mModalShift);
        break;
      default:
        const mModal = {
          type: typeBS,
          dataSelect: listSelect,
          dataFilter: listSelect,
          visibleModal: true,
        };
        setDataModal(mModal);
        break;
    }
  };
  const handlerUnPlan = async () => {
    if (isSubmitting || isDownload) return;
    if (data.dataMain.length > 0) {
      let dataSave = [];
      let contentMessage = '';
      let itemOFF = data.dataMain[0];
      let itemSaveOFF = {
        PlanId: itemOFF.planId,
        ShopId: itemOFF.shopId,
        PlanDate: itemOFF.auditDate,
        Status: itemOFF.status ? 1 : 0,
        LastStatus: 'UNPLAN',
        ShiftType: itemOFF.shiftType,
        SupConfirm: itemOFF.supConfirm,
      };
      contentMessage =
        'Nhân viên ' +
        itemOFF.fullName +
        ' yêu cầu gỡ phép ngày ' +
        itemOFF.dateView;
      dataSave.push(itemSaveOFF);
      alertConfirm(
        'Thông báo',
        'Bạn có muốn gỡ phép ngày không ?',
        async () => {
          setSubmitting(true);
          try {
            const dataUpload = {
              dataPlan: JSON.stringify(dataSave),
              contentMessage: contentMessage,
            };
            const result = await PLANSR_ACTION(dataUpload, itemOFF.parentList);
            if (result.statusId === 200) {
              alertNotify(result.messager);
              await setUpdate(true);
              await LoadDataPlan(configDate.dateChoose);
              await downloadAll(async e => {
                await ToastSuccess(e, 'Thông báo', 'top');
              });
              await APPDOWNLOAD.downloadMenu();
            } else {
              alertError(result.messager);
            }
          } catch (error) {
            alertError('Gửi dữ liệu lỗi, vui lòng thử lại sau (' + error + ')');
          } finally {
            setSubmitting(false);
          }
        },
      );
    }
  };
  const searchModal = async text => {
    if (text.length > 0) {
      const filterShift =
        dataModal.type == 'SHIFT'
          ? dataModal.dataFilter.filter(
              i =>
                i.Name.toLowerCase().match(text.toLowerCase()) ||
                i.ShiftCode.toLowerCase().match(text.toLowerCase()),
            )
          : dataModal.type == 'NOTE'
          ? dataModal.dataFilter.filter(
              i =>
                (i.title !== undefined &&
                  i.title.toLowerCase().match(text.toLowerCase())) ||
                i.Ref_Code.toLowerCase().match(text.toLowerCase()),
            )
          : dataModal.dataFilter.filter(
              i =>
                i.ShopName !== undefined &&
                i.ShopName.toLowerCase().match(text.toLowerCase()),
            );
      await setDataModal({ ...dataModal, dataSelect: filterShift });
    } else {
      await setDataModal({ ...dataModal, dataSelect: dataModal.dataFilter });
    }
  };
  const chooseItemModal = async (item, index, value, typeModal) => {
    let isShiftOff = item.ShiftGroup == 'OFF' ? 1 : 0;
    // Action changep
    if (typeModal == 'SHIFT') {
      data.mPlanList[0].shiftChange = value;
      data.mPlanList[0].reasonName = 'Chọn lí do thay đổi';
      data.mPlanList[0].ref_Code = item.ShiftGroup;
      data.mPlanList[0].supConfirm = 3;

      let listIndex =
        mListIndex.length > 0
          ? mListIndex
          : JSON.parse(data.dataMain[0].listIndex);
      for (let i = 0, lenList = listIndex.length; i < lenList; i++) {
        let mainIndex = listIndex[i].MainIndex;
        data.mPlanList[mainIndex].isLocked = isShiftOff;
      }
      await handlerChangeDataModal(item);
    }
    if (typeModal == 'NOTE') {
      data.mPlanList[0].reasonName = value;
      setDataModal({ ...dataModal, visibleModal: false });
    }
    assignData(0);
  };
  const clearItemModal = async (item, index, typeModal) => {
    if (typeModal == 'SHIFT') {
      data.mPlanList[0].shiftChange = 'Chọn loại nghỉ phép';
      data.mPlanList[0].reasonName = 'Chọn lí do thay đổi';
      data.mPlanList[0].ref_Code = null;
      data.mPlanList[0].isLocked = 0;
      data.mPlanList[0].notes = null;
      data.mPlanList[0].supConfirm = 0;
      //
      let listIndex =
        mListIndex.length > 0
          ? mListIndex
          : JSON.parse(data.dataMain[0].listIndex);
      for (let i = 0, lenList = listIndex.length; i < lenList; i++) {
        let mainIndex = listIndex[i].MainIndex;
        data.mPlanList[mainIndex].isLocked = 0;
      }
    }
    if (typeModal == 'NOTE') {
      data.mPlanList[0].reasonName = 'Chọn lí do thay đổi';
      data.mPlanList[0].notes = null;
      if (
        item.shiftChange == undefined ||
        item.shiftChange == 'Chọn loại nghỉ phép'
      ) {
        data.mPlanList[0].ref_Code = null;
        data.mPlanList[0].isLocked = 0;
      } else {
        data.mPlanList[0].ref_Code = 'OFF';
        data.mPlanList[0].isLocked = 1;
      }
    }
    setDataModal({ ...dataModal, visibleModal: false });
    assignData(0);
  };
  const handlerChangeDataModal = async item => {
    let changeItem = JSON.parse(data.mPlanList[0].refList).filter(
      i => i.Ref_Code == item.ShiftGroup,
    );
    let dataModal = changeItem.map(i =>
      i.title == item.notes ? { ...i, isSelect: 1 } : i,
    );
    await setDataModal({
      ...dataModal,
      type: 'NOTE',
      dataSelect: dataModal,
      dataFilter: dataModal,
    });
  };
  const handlerUpdateStoreData = async () => {
    await setDownloading(true);
    await downloadAll(() => {});
    await setDownloading(false);
  };
  // Render View
  const renderItem = ({ item, index, target }) => {
    const visibleOffView = item.checkWP > 0 ? 'flex' : 'none';
    const isStickyHeader = target === 'StickyHeader';
    return (
      <View key={index} style={{ flex: 1 }}>
        {item.isParent ? (
          <ViewItemOFF
            appcolor={appcolor}
            dataPhoto={dataPhoto}
            dataVideo={dataVideo}
            item={item}
            index={index}
            styles={styles}
            visibleView={visibleOffView}
            isCollapsed={isStickyHeader && !isListAtTop}
            actionNotes={handlerNotes}
            actionChooseItem={handlerShowViewChoose}
            actionUnPlanOFF={handlerUnPlan}
            handlerTakePicture={handlerTakePicture}
            handlerTakeVideo={handlerTakeVideo}
            handlerViewPicture={handlerViewPicture}
            handlerViewVideo={handlerViewVideo}
            onExpand={() =>
              listRef.current?.scrollToOffset({ offset: 0, animated: true })
            }
          />
        ) : (
          <ViewItemStore
            appcolor={appcolor}
            item={item}
            index={index}
            styles={styles}
            onValueChange={handlerShopAction}
            actionNotes={handlerNotes}
          />
        )}
      </View>
    );
  };
  const handleGoBack = () => {
    if (!isDownload && !isSubmitting) navigation.goBack();
  };
  const handleListScroll = ({ nativeEvent }) => {
    const offsetY = nativeEvent.contentOffset.y;
    const isAtTop = offsetY <= 5;

    setListAtTop(current => (current === isAtTop ? current : isAtTop));
  };
  const handlerTakePicture = async () => {
    let photoinfo = {
      photoType: 'PLAN_EVIDENT',
      dataUpload: 0,
      fileUpload: 0,
      shopId: 0,
      reportId: kpiinfo.id,
      photoPath: null,
      guid: guidByDate,
      photoDate: moment().format('YYYYMMDD'),
      photoTime: new Date().getTime(),
      photoFullTime: moment().format('YYYY-MM-DD HH:mm:ss'),
    };
    const options = [
      { text: 'Hủy' },
      {
        text: 'Chụp hình',
        onPress: async () =>
          await NativeCamera.cameraStart(photoinfo, onReloadPhoto),
      },
      {
        text: 'Thư viện',
        onPress: async () =>
          await NativeCamera.imageGalleryLaunch(photoinfo, onReloadPhoto),
      },
    ];
    optionConfirm(
      'Hình ảnh chứng minh',
      'Chọn hình ảnh từ thư viện hoặc bạn tự chụp bằng máy ảnh',
      options,
    );
  };
  const handlerTakeVideo = async () => {
    let videoInfo = {
      photoType: 'PLAN_VIDEO_EVIDENT',
      dataUpload: 0,
      fileUpload: 0,
      shopId: 0,
      tag: 'VIDEO',
      reportId: kpiinfo.id,
      photoPath: null,
      guid: guidByDate,
      photoDate: moment().format('YYYYMMDD'),
      photoTime: new Date().getTime(),
      photoFullTime: moment().format('YYYY-MM-DD HH:mm:ss'),
    };
    const options = [
      { text: 'Hủy' },
      {
        text: 'Quay',
        onPress: async () =>
          await NativeCamera.videoStart(videoInfo, onReloadVideo),
      },
      {
        text: 'Thư viện',
        onPress: async () =>
          await NativeCamera.videoGalleryLaunch(videoInfo, onReloadVideo),
      },
    ];
    optionConfirm(
      'Video chứng minh',
      'Chọn video từ thư viện hoặc bạn tự quay một đoạn bằng máy ảnh',
      options,
    );
  };
  const onReloadPhoto = async () => {
    await getDataPhotos(
      0,
      TODAY,
      'PLAN_EVIDENT',
      kpiinfo.id,
      guidByDate || 'none',
      false,
      setDataPhoto,
    );
  };
  const onReloadVideo = async () => {
    await getDataPhotos(
      0,
      TODAY,
      'PLAN_VIDEO_EVIDENT',
      kpiinfo.id,
      guidByDate || 'none',
      false,
      setDataVideo,
    );
  };
  const handlerViewPicture = () => {
    if (dataPhoto.length > 0) {
      images.data = dataPhoto;
      images.isShow = true;
      images.index = 0;
      setMutate(e => !e);
    }
  };
  const onCloseShowImage = () => {
    images.data = [];
    images.isShow = false;
    images.index = 0;
    setMutate(e => !e);
  };
  const handlerViewVideo = () => {
    if (dataVideo.length > 0)
      SheetManager.show('media-video-show', { payload: dataVideo });
  };

  const styles = StyleSheet.create({
    mainContainer: { flex: 1, backgroundColor: appcolor.light },
    itemOFFView: {
      flex: 1,
      marginBottom: 8,
      backgroundColor: appcolor.surface,
      padding: 8,
      paddingTop: 0,
      borderRadius: 8,
      borderWidth: 1,
      borderColor: appcolor.grayLight,
      elevation: 3,
      shadowOffset: { width: 0, height: 3 },
      shadowColor: appcolor.dark,
      shadowOpacity: 0.5,
      shadowRadius: 8,
    },
    itemOFFCompactView: {
      minHeight: 52,
      paddingTop: 8,
      justifyContent: 'center',
    },
    compactHeaderContent: { flexDirection: 'row', alignItems: 'center' },
    compactHeaderText: { flex: 1, paddingEnd: 8 },
    compactHeaderTitle: {
      fontSize: 12,
      fontWeight: fontWeightBold,
      color: appcolor.dark,
    },
    compactHeaderSubtitle: {
      marginTop: 3,
      fontSize: 11,
      color: appcolor.greylight,
    },
    compactHeaderIcon: {
      width: 38,
      height: 38,
      marginEnd: 10,
      borderRadius: 19,
      alignItems: 'center',
      justifyContent: 'center',
      backgroundColor: appcolor.primary,
    },
    itemShopView: {
      flex: 1,
      marginBottom: 8,
      backgroundColor: appcolor.light,
      padding: 8,
      borderRadius: 8,
      borderWidth: 0.5,
      borderColor: appcolor.grayLight,
    },
    styleSwitchView: {
      ...(Platform.OS === 'ios' && {
        transform: [{ scaleX: 0.8 }, { scaleY: 0.8 }],
      }),
    },
    titleShiftType: {
      flex: 1,
      fontSize: 13,
      fontWeight: '700',
      fontStyle: 'italic',
      color: appcolor.dark,
    },
    chooseItem: {
      flex: 1,
      padding: 10,
      borderRadius: 8,
      borderRadius: 8,
      borderWidth: 0.5,
      borderColor: appcolor.grey,
    },
    statusText: {
      width: '100%',
      fontSize: 12,
      fontWeight: '700',
      color: appcolor.dark,
      padding: 8,
      paddingBottom: 5,
    },
    filterStyle: { marginStart: 8, marginEnd: 8 },
    overflowView: {
      ...StyleSheet.absoluteFillObject,
      zIndex: 2000,
      elevation: 2000,
      backgroundColor: appcolor.dark,
      opacity: 0.8,
      justifyContent: 'center',
    },
  });
  useEffect(() => {
    const isActionLocked = isSubmitting || isDownload;
    const removeBeforeRemove = navigation.addListener('beforeRemove', event => {
      if (!isActionLocked) return;
      event.preventDefault();
    });
    const backSubscription = BackHandler.addEventListener(
      'hardwareBackPress',
      () => isActionLocked,
    );

    return () => {
      removeBeforeRemove();
      backSubscription.remove();
    };
  }, [isSubmitting, isDownload, navigation]);
  useEffect(() => {
    LoadDataPlan();
  }, []);
  return (
    <KeyboardAvoidingView
      style={styles.mainContainer}
      behavior={Platform.OS === 'ios' ? 'padding' : undefined}
    >
      <HeaderCustom
        title={route?.params?.menuitem.menuNameVN || 'Lịch làm việc'}
        leftFunc={handleGoBack}
        iconRight="cloud-upload-alt"
        rightFunc={handlerSave}
        disabled={isSubmitting || isDownload}
      />
      <ReactNativeCalendarStrip
        style={{
          flexDirection: 'row',
          alignItems: 'center',
          paddingBottom: 16,
        }}
        minDate={'2023-01-01'}
        calendarHeaderStyle={{ color: 'white', marginBottom: 16, fontSize: 14 }}
        calendarColor={DEFAULT_COLOR}
        dateNumberStyle={{ color: 'white', fontSize: 12 }}
        dateNameStyle={{ color: 'white', fontSize: 12, paddingTop: 5 }}
        customDatesStyles={configDate.customDatesStyles}
        highlightDateContainerStyle={{
          backgroundColor: 'white',
          width: 38,
          height: 38,
          paddingTop: 5,
          borderRadius: 8,
        }}
        highlightDateNumberStyle={{ color: 'black', fontSize: 12 }}
        highlightDateNameStyle={{ color: 'black', fontSize: 12 }}
        disabledDateNameStyle={{ color: 'grey', fontSize: 12 }}
        disabledDateNumberStyle={{ color: 'grey', fontSize: 12 }}
        iconLeft={require('../../Themes/Images/chevron-left.png')}
        iconRight={require('../../Themes/Images/chevron-right.png')}
        iconContainer={{ flex: 0.1 }}
        markedDates={configDate.markedDates}
        selectedDate={configDate.dateChoose}
        //Event
        scrollToOnSetSelectedDate={false}
        onDateSelected={date => {
          LoadDataPlan(date);
        }}
      />
      <SearchData
        placeholder="Tìm kiếm cửa hàng"
        iconName="search"
        containerStyle={{ margin: 8, padding: 3 }}
        inputStyle={{ fontSize: 12 }}
        onSearchData={searchShops}
      />
      <LoadingView
        title={'Đang cập nhật dữ liệu'}
        isLoading={refreshing}
        styles={{ zIndex: 200, marginTop: 8 }}
      />
      {(isDownload || isSubmitting) && (
        <View style={styles.overflowView}>
          <LoadingView
            titleStyle={{ color: appcolor.light }}
            title={
              isSubmitting
                ? 'Đang gửi dữ liệu lên hệ thống'
                : 'Đang cập nhật danh sách cửa hàng làm việc'
            }
            isLoading={isDownload || isSubmitting}
          />
        </View>
      )}
      <FlashList
        ref={listRef}
        keyExtractor={(_, index) => index.toString()}
        data={data.mPlanList}
        extraData={[data.mPlanList, isListAtTop]}
        estimatedItemSize={100}
        stickyHeaderIndices={[0]}
        removeClippedSubviews={false}
        showsVerticalScrollIndicator={false}
        ListFooterComponent={<View style={{ height: deviceHeight / 2 }} />}
        renderItem={renderItem}
        contentContainerStyle={{ padding: 8, paddingTop: 0 }}
        keyboardShouldPersistTaps="handled"
        onScroll={handleListScroll}
        scrollEventThrottle={16}
        refreshControl={
          <RefreshControl refreshing={false} onRefresh={LoadDataPlan} />
        }
      />
      <ModalItem
        styles={styles}
        dataModal={dataModal}
        actionResultModal={() =>
          setDataModal({ ...dataModal, visibleModal: false })
        }
        actionSearch={searchModal}
        actionChooseItem={chooseItemModal}
        actionClearItem={clearItemModal}
      />
      <Modal visible={images.isShow}>
        <MultipleShowImage
          key="showimageplan"
          listItem={images.data || []}
          indexItem={images.index}
          closeShowImage={onCloseShowImage}
        />
      </Modal>
      <MultipleShowVideo />
    </KeyboardAvoidingView>
  );
};
// Render Child View
const ViewItemOFF = ({
  item,
  index,
  styles,
  visibleView,
  actionNotes,
  actionChooseItem,
  appcolor,
  actionUnPlanOFF,
  dataPhoto,
  dataVideo,
  handlerTakePicture,
  handlerViewPicture,
  handlerTakeVideo,
  handlerViewVideo,
  isCollapsed,
  onExpand,
}) => {
  const shiftList = JSON.parse(item.shiftList);
  const reasonList = JSON.parse(item.refList);
  const colorStatusShift =
    item.colorStatusShift == '#' ? appcolor.dark : item.colorStatusShift;

  const styleItem = StyleSheet.create({
    contentImage: { width: '100%', flexDirection: 'row', marginTop: 8 },
    viewCamera: {
      flex: 1,
      alignItems: 'center',
      justifyContent: 'center',
      borderWidth: 0.5,
      borderColor: appcolor.primary,
      borderRadius: 8,
      marginEnd: 8,
    },
    viewAlbum: {
      width: 60,
      alignItems: 'center',
      borderWidth: 0.5,
      borderColor: appcolor.primary,
      padding: 8,
      borderRadius: 8,
    },
    titleCamera: {
      fontSize: 12,
      fontWeight: fontWeightBold,
      color: appcolor.primary,
    },
    viewCircle: {
      width: 18,
      height: 18,
      backgroundColor: appcolor.redgray,
      borderRadius: 24,
      justifyContent: 'center',
      alignItems: 'center',
      position: 'absolute',
      top: 3,
      end: 5,
    },
    titleCount: {
      fontSize: 11,
      fontWeight: fontWeightBold,
      color: appcolor.light,
    },
  });

  if (isCollapsed) {
    return (
      <TouchableOpacity
        activeOpacity={0.8}
        accessibilityRole="button"
        accessibilityLabel="Mở thông tin đổi ca làm việc"
        style={[
          styles.itemOFFView,
          styles.itemOFFCompactView,
          { display: visibleView },
        ]}
        onPress={onExpand}
      >
        <View style={styles.compactHeaderContent}>
          <View style={styles.compactHeaderIcon}>
            <SpiralIcon name="swap-horiz" size={22} color={appcolor.light} />
          </View>
          <View style={styles.compactHeaderText}>
            <Text
              style={[styles.compactHeaderTitle, { color: appcolor.primary }]}
              numberOfLines={1}
            >
              Đổi ca làm việc
            </Text>
            <Text style={styles.compactHeaderSubtitle} numberOfLines={1}>
              {[item.shiftStatus, item.shiftChange].filter(Boolean).join(' • ')}
            </Text>
          </View>
          <SpiralIcon
            name="keyboard-arrow-down"
            size={22}
            color={appcolor.primary}
          />
        </View>
      </TouchableOpacity>
    );
  }

  return (
    <View key={index} style={[styles.itemOFFView, { display: visibleView }]}>
      {(item.phepTon || null) !== null && (
        <Text
          style={[
            styles.statusText,
            { color: appcolor.tomato, fontSize: 13, fontStyle: 'italic' },
          ]}
        >
          {item.phepTon}
        </Text>
      )}
      <Text style={[styles.statusText, { color: colorStatusShift }]}>
        {item.shiftStatus}
      </Text>
      {item.supNote !== null && item.supNote.length > 0 && (
        <Text style={[styles.statusText, { color: colorStatusShift }]}>
          {item.supNote}
        </Text>
      )}
      <RenderChooseItem
        item={item}
        styles={styles}
        appcolor={appcolor}
        typeBS="SHIFT"
        listSelect={shiftList}
        value={item.shiftChange}
        handlerChooseItem={actionChooseItem}
      />
      <Text style={styles.statusText}>Lý do thay đổi</Text>
      <RenderChooseItem
        item={item}
        styles={styles}
        appcolor={appcolor}
        typeBS="NOTE"
        listSelect={reasonList}
        value={item.reasonName}
        handlerChooseItem={actionChooseItem}
      />
      <RenderTextInput
        index={index}
        typeInput="SHIFT"
        item={item}
        isLock={item.isLockShift}
        value={item.notes}
        handlenotes={actionNotes}
        styles={styles}
        appcolor={appcolor}
      />
      {item.isUnPlan == 1 && (
        <TouchableOpacity
          style={{
            width: 100,
            padding: 8,
            backgroundColor: appcolor.red,
            borderRadius: 5,
            position: 'absolute',
            top: 8,
            end: 8,
          }}
          onPress={actionUnPlanOFF}
        >
          <Text style={{ textAlign: 'center', color: appcolor.light }}>
            Gỡ phép
          </Text>
        </TouchableOpacity>
      )}
      {/* {item.ref_Code == 'OFF' && */}
      <View style={styleItem.contentImage}>
        <TouchableOpacity
          style={styleItem.viewCamera}
          onPress={handlerTakePicture}
        >
          <Text style={styleItem.titleCamera}>{`Hình ảnh`}</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={{ ...styleItem.viewAlbum, marginEnd: 8 }}
          onPress={handlerViewPicture}
        >
          <SpiralIcon
            type="ionicon"
            name="images"
            size={18}
            color={appcolor.primary}
          />
          <View style={styleItem.viewCircle}>
            <Text style={styleItem.titleCount}>{dataPhoto.length}</Text>
          </View>
        </TouchableOpacity>
        <TouchableOpacity
          style={styleItem.viewCamera}
          onPress={handlerTakeVideo}
        >
          <Text style={styleItem.titleCamera}>{`Video`}</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={styleItem.viewAlbum}
          onPress={handlerViewVideo}
        >
          <SpiralIcon
            type="ionicon"
            name="videocam"
            size={18}
            color={appcolor.primary}
          />
          <View style={styleItem.viewCircle}>
            <Text style={styleItem.titleCount}>{dataVideo.length}</Text>
          </View>
        </TouchableOpacity>
      </View>
      {/* } */}
    </View>
  );
};
const ViewItemStore = ({
  item,
  index,
  styles,
  onValueChange,
  actionNotes,
  appcolor,
}) => {
  const visibleStatusChange =
    item.statusChangeShop.length > 0 ? 'flex' : 'none';
  const visibleSupNoteConfirm =
    item.confirmPlanNote !== null && item.confirmPlanNote.length > 0
      ? 'flex'
      : 'none';
  const colorStatusChange =
    item.confirmShop == 1
      ? appcolor.success
      : item.confirmShop == -1
      ? appcolor.danger
      : appcolor.rejection;
  const visibleMonthlyFrequency =
    item.monthlyFrequency?.length > 0 && item.lastDate?.length > 0
      ? 'flex'
      : 'none';
  return (
    <View key={index} style={styles.itemShopView}>
      <Text
        style={{
          display: visibleStatusChange,
          width: '100%',
          fontSize: 12,
          fontWeight: '700',
          color: colorStatusChange,
        }}
      >
        {item.statusChangeShop}
      </Text>
      <Text
        style={{
          display: visibleSupNoteConfirm,
          width: '100%',
          fontSize: 12,
          fontWeight: '700',
          color: appcolor.danger,
        }}
      >
        Quản lý ghi chú: {item.confirmPlanNote}
      </Text>
      <View style={{ flex: 8, flexDirection: 'row', alignItems: 'center' }}>
        <SpiralIcon name="store" size={21} color={appcolor.dark} />
        <Text
          style={{
            width: '80%',
            paddingStart: 3,
            paddingEnd: 8,
            fontSize: 13,
            fontWeight: '700',
            color:
              visibleMonthlyFrequency === 'flex'
                ? item.colorHighlight || appcolor.dark
                : item.titleStoreColor || appcolor.dark,
          }}
        >
          Cửa hàng: {item.shopName}
        </Text>
        {item.isLocked == 0 && (
          <Switch
            disabled={item.isLockStatus == 1}
            style={styles.styleSwitchView}
            ios_backgroundColor={appcolor.greydark}
            trackColor={{ true: appcolor.success, false: appcolor.greylight }}
            thumbColor={appcolor.dark}
            //Event
            value={item.status == 1}
            onValueChange={value => onValueChange(item, index, value)}
          />
        )}
      </View>
      <View style={{ flex: 2, justifyContent: 'center' }}>
        <Text
          style={{
            width: '80%',
            fontSize: 12,
            color: appcolor.greylight,
            fontWeight: '500',
          }}
        >
          {item.shopCode}{' '}
        </Text>
        <Text
          style={{
            width: '80%',
            fontSize: 12,
            color: appcolor.greylight,
            fontWeight: '500',
          }}
        >
          {item.address}{' '}
        </Text>
        {item.actualFrequency !== null && (
          <Text
            style={{
              display: visibleMonthlyFrequency,
              width: '80%',
              fontSize: 12,
              color: appcolor.dark,
            }}
          >
            {item.actualFrequency}
          </Text>
        )}
        <Text
          style={{
            display: visibleMonthlyFrequency,
            width: '80%',
            fontSize: 12,
            color: appcolor.dark,
          }}
        >
          {item.monthlyFrequency}
        </Text>
        <Text
          style={{
            display: visibleMonthlyFrequency,
            width: '80%',
            fontSize: 12,
            color: appcolor.dark,
          }}
        >
          {item.lastDate}
        </Text>
        <RenderTextInput
          styles={styles}
          appcolor={appcolor}
          index={index}
          typeInput="SHOP"
          item={item}
          isLock={item.isLockStatus}
          value={item.notes}
          handlenotes={actionNotes}
        />
      </View>
    </View>
  );
};
const renderItemModal = (
  item,
  index,
  dataModal,
  actionChooseItem,
  actionClearItem,
  appcolor,
) => {
  const value =
    dataModal.type == 'SHIFT'
      ? item.Name
      : dataModal.type == 'NOTE'
      ? item.title
      : item.ShopName;
  const chooseItem = () => {
    actionChooseItem(item, index, value, dataModal.type);
  };
  const clearItem = () => {
    actionClearItem(item, index, dataModal.type);
  };
  return (
    <View
      key={`${dataModal.type || 'modal'}_${
        item.Ref_Code || item.ShiftCode || item.Id || item.ShopId || index
      }`}
    >
      {item.isParent && (
        <Text
          style={{
            width: '100%',
            fontSize: 13,
            fontWeight: fontWeightBold,
            color: appcolor.danger,
            padding: 8,
          }}
        >
          {item.Ref_Code}
        </Text>
      )}
      <View
        style={{
          flexDirection: 'row',
          backgroundColor:
            item.isSelect == 1 ? appcolor.primary : appcolor.surface,
          marginBottom: 8,
          borderRadius: 8,
        }}
      >
        <TouchableOpacity style={{ flex: 8, padding: 12 }} onPress={chooseItem}>
          <Text
            style={{
              width: '80%',
              fontSize: 13,
              fontWeight: fontWeightBold,
              color: item.isSelect == 1 ? appcolor.white : appcolor.dark,
            }}
          >
            {value}
          </Text>
          <Text
            style={{
              width: '80%',
              fontSize: 11,
              fontWeight: '500',
              color:
                item.isSelect == 1 ? appcolor.grayLight : appcolor.greylight,
            }}
          >
            {item.ShiftCode}
          </Text>
        </TouchableOpacity>
        {item.isSelect == 1 && (
          <TouchableOpacity
            style={{ padding: 12, alignItems: 'center' }}
            onPress={clearItem}
          >
            <SpiralIcon name="backspace" size={18} color={appcolor.white} />
          </TouchableOpacity>
        )}
      </View>
    </View>
  );
};
const ModalItem = ({
  dataModal,
  styles,
  actionSearch,
  actionResultModal,
  actionChooseItem,
  actionClearItem,
}) => {
  const appcolor = useSelector(state => state.GAppState.appcolor);
  const titleModal =
    dataModal.type == 'SHIFT'
      ? 'Đổi ca làm việc'
      : dataModal.type == 'NOTE'
      ? 'Lý do thay đổi'
      : 'Danh sách cửa hàng';
  const { arr } = groupDataByKey({
    arr: dataModal.dataSelect,
    key: 'Ref_Code',
  });
  return (
    <Modal animationType="slide" visible={dataModal.visibleModal}>
      <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.light }}>
        <View
          style={{
            flexDirection: 'row',
            justifyContent: 'space-between',
            alignItems: 'center',
            padding: 8,
          }}
        >
          <Text
            style={{
              fontSize: 21,
              padding: 8,
              fontWeight: '700',
              color: appcolor.dark,
              textAlign: 'center',
            }}
          >
            {titleModal}
          </Text>
          <TouchableOpacity style={{ padding: 10 }} onPress={actionResultModal}>
            <SpiralIcon name="close" size={25} color={appcolor.dark} />
          </TouchableOpacity>
        </View>
        <SearchData
          placeholder="Tìm kiếm dữ liệu"
          containerStyle={styles.filterStyle}
          onSearchData={actionSearch}
        />
        <View style={{ flex: 1, padding: 8 }}>
          <FlashList
            keyExtractor={(_, index) => index.toString()}
            data={arr}
            extraData={[arr]}
            estimatedItemSize={100}
            renderItem={({ item, index }) =>
              renderItemModal(
                item,
                index,
                dataModal,
                actionChooseItem,
                actionClearItem,
                appcolor,
              )
            }
            removeClippedSubviews={true}
            ListFooterComponent={<View style={{ paddingBottom: 32 }} />}
            showsVerticalScrollIndicator={false}
          />
        </View>
      </SafeAreaView>
    </Modal>
  );
};
// Control
const RenderTextInput = ({
  index,
  typeInput,
  item,
  value,
  handlenotes,
  isLock,
  appcolor,
  onFocus,
  onEndEditing,
}) => {
  const handleChange = inputText => {
    handlenotes(index, inputText, item, typeInput);
  };
  const styles = StyleSheet.create({
    containerInput: {
      flex: 1,
      color: appcolor.dark,
      paddingVertical: 3,
      paddingHorizontal: 0,
      marginTop: 8,
      marginBottom: 0,
      borderRadius: 8,
      borderWidth: 0.5,
      borderColor: appcolor.grey,
      fontSize: 12,
      textAlignVertical: 'center',
      backgroundColor: 'transparent',
    },
    inputView: { fontSize: 12, color: appcolor.dark },
  });
  return (
    <View>
      <FormGroup
        editable={isLock == 1 ? false : true}
        defaultValue={value}
        multiline
        useClearAndroid={false}
        inputStyle={styles.inputView}
        containerStyle={styles.containerInput}
        placeholder="Nhập ghi chú"
        handleChangeForm={handleChange}
        onFocus={onFocus}
        onEndEditing={onEndEditing}
      />
    </View>
  );
};
const RenderChooseItem = ({
  index,
  item,
  styles,
  appcolor,
  typeBS,
  listSelect,
  value,
  titleStyle = null,
  handlerChooseItem,
}) => {
  const handlerChoose = () => {
    handlerChooseItem(index, item, listSelect, typeBS);
  };
  return (
    <TouchableOpacity
      key={index}
      disabled={item.isLockShift == 1 ? true : false}
      style={styles.chooseItem}
      onPress={handlerChoose}
    >
      <Text style={{ fontSize: 12, fontWeight: '400', color: appcolor.dark }}>
        {value}
      </Text>
    </TouchableOpacity>
  );
};
export default WorkingPlanSR_V2;
