...
import { View, Text, TextInput, StyleSheet, ScrollView, RefreshControl, Platform, FlatList, KeyboardAvoidingView, Dimensions, LayoutAnimation, UIManager, TouchableOpacity, Modal, SafeAreaView, ImageBackground } from 'react-native';
import { Image, ListItem, Divider, } from '@rneui/themed';
import { REGISTERPLAN_GetByWeek, GET_ListWeek, PLAN_ChangeShift, PLAN_WorkingLate, PLAN_UploadImage, REGISTERPLAN_SavePlan, REGISTERPLAN_GetCopyPlan, V2_REGISTERPLAN_GetCopyPlan, V2_REGISTERPLAN_GetByWeek } from '../../../Controller/PlanController';
...
const WorkingPlanHMDPG = ({ navigation, route }) => {
    ...
    const loadListCopyPlan = async (currentWeek = null) => {
        ...
        try {
            ...
            currentWeek = !currentWeek ? listWeek[0] ?
                listWeek.filter(e => e.value === moment().isoWeek())[0] : { value: moment().isoWeek() } : currentWeek
        ...
        try {
            switch (type) {
                case SUBMIT_SHIFT:
                    ...
                    break;
                case SUBMIT_LATE:
                    if ((item.timeLate && (!item.noteLate || item.noteLate?.length < 5)) || (!item.timeLate && item.noteLate)) {
                        alertWarning("Thời gian hoặc ghi chú không được để trống, ghi chú tối thiểu 5 ký tự")
                        return
                    }
                    break;
                case SUBMIT_EARLIER:
                    if ((item.timeEarlier && (!item.noteEarlier || item.noteEarlier?.length < 5)) || (!item.timeEarlier && item.noteEarlier)) {
                        alertWarning("Thời gian hoặc ghi chú không được để trống, ghi chú tối thiểu 5 ký tự")
                        return
                    }
                    break;
            ...
            alertConfirm("Thông báo", "Bạn có chắc chắn muốn gửi đề xuất thay đổi?", () => {
                switch (type) {
                    case SUBMIT_SHIFT:
                        PLAN_ChangeShift(
                    ...
                    case SUBMIT_IMAGE:
                        const { index } = currentBS
                        PLAN_UploadImage(item, imageData[index] || [], (e) => alertNotify(e.msg))
                        break;
                }
            })
        } catch (e) { }
    ...
    const handleDisplayBS = (type, index = -1, isPastOrPresentWeek = true) => {
        ...
        switch (type) {
            ...
            case SHIFT:
                ...
                break;
            case IMAGE:
                list = data[index].photoMore !== undefined ? JSON.parse(data[index].photoMore) : []
    ...
    const onChangeMinute = async ({ value, isPress = true, index, isPlus, keyBusy }) => {
        const mainIndex = data[index].stt
        ...
        if (isPress) {
            ...
            const nextValue = isPlus && prevValue <= 90 ?
                ceilNum === 0 && prevValue < 90 ? 5 : ceilNum :
                prevValue > 0 ? floorNum === 0 ? -5 : -floorNum : 0
    ...
    const cameraLaunch = () => {
        try {
            let options = { mediaType: 'photo', cameraType: 'front', includeBase64: true, maxHeight: 1360, maxWidth: 1360 };
    ...
    const imageGalleryLaunch = () => {
        try {
            let options = { mediaType: 'photo', includeBase64: true, maxHeight: 1360, maxWidth: 1360 };
    ...
    const renderItem = ({ item, index }) => {
        const { workDate, shopName, address, shiftTypeName, shiftChange, shiftChangeName, checkIn1, checkOut1, checkIn2, checkOut2, totalTime, isParent, } = item
        ...
        return (
            <View key={index} style={{ width: '100%', alignSelf: 'center', marginBottom: isParent ? 8 : 0 }}>
                {isParent && (
                    <View style={{ backgroundColor: appcolor.yellowdark, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padidng: 15, height: 40, }}>
                        <Icon name="calendar-check" size={20} color={appcolor.black} />
                        <Text style={{ color: appcolor.black, fontSize: 18, fontWeight: '700', marginLeft: 10, }}>{workDate}</Text>
                    </View>
                )}
                <View style={{ backgroundColor: appcolor.light, borderRadius: 10, padding: 10, margin: 8 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', paddingTop: 8, paddingBottom: 8, paddingEnd: 8 }}>
                            <Icon name="store" size={17} style={{ width: 30, textAlign: 'center', }} color={appcolor.dark} />
                            <TouchableOpacity onPress={() => (item.confirmPlan != 1 && AppNameBuild == 'hmd') && handleDisplayBS(SHOPL, index, false)} style={{ borderWidth: (item.confirmPlan != 1 && AppNameBuild == 'hmd') ? 1 : 0, borderRadius: 8, borderColor: 'lightgray', padding: 5 }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', paddingTop: 10, paddingBottom: 10, }}>
                        <Icon name="map-marked-alt" size={17} style={{ width: 30, textAlign: 'center', }} color={appcolor.dark} />
                        <Text style={{ color: appcolor.dark, fontSize: 15 }}>{`Địa chỉ: ${address}`}</Text>
                    </View>

                    <TouchableOpacity onPress={() => { item.confirmPlan == 1 ? null : handleDisplayBS(SHIFT, index, false) }} style={{ flexDirection: 'row', alignItems: 'center', paddingTop: 8, paddingBottom: 8, paddingEnd: 8 }}>
                        <Icon name="business-time" size={17} style={{ width: 30, textAlign: 'center', }} color={appcolor.dark} />
                        <Text style={{ color: appcolor.dark, fontSize: 15, }}>{`Ca hiện tại: ${shiftTypeName || ""}`}</Text>
                    </TouchableOpacity>

                    {(item.confirmPlan == 1 && shiftChange !== null && shiftChange !== "") && (
                        <View style={{ flexDirection: 'row', alignItems: 'center', paddingTop: 8, paddingBottom: 8, paddingEnd: 8 }}>
                            <Icon name="sync" size={17} style={{ width: 30, textAlign: 'center', }} color={appcolor.dark} />
                    ...
                    }
                    {
                        item.confirmPlan == 1 &&
                        <View>
                            <View style={{ borderColor: appcolor.grayLight, height: 0.5, width: '100%', marginBottom: 8, borderWidth: 1 }} />
                            <View style={{ flexDirection: "row" }}>
                                <View style={[styles.itemShopContainer, { width: '49.5%', justifyContent: 'center' }]}>
                                    <RenderExpandBtn setExpanded={setExpanded} expanded={expanded} isShift={true} shiftStatus={item.shiftStatus ? item.shiftStatus : "Đổi ca"} colorStatus={colorStatus}
                                        appcolor={appcolor} showState={shiftExpand} hideState={busyExpand} handleExpanded={handleExpanded} confirmShift={confirmShift} />
                                </View>
                                <View style={{ height: 40, width: "1%", backgroundColor: 'rgb(50,51,52)', borderRadius: 20, }} />
                                <View style={[styles.itemShopContainer, { width: '49.5%', justifyContent: 'center' }]}>
                                    <RenderExpandBtn appcolor={appcolor} setExpanded={setExpanded} expanded={expanded} lateStatus={item.lateStatus ? item.lateStatus : "Đi trễ"} earlierStatus={item.earlierStatus ? item.earlierStatus : "Về sớm"}
                                        showState={busyExpand} hideState={shiftExpand} handleExpanded={handleExpanded} confirmLate={confirmLate} confirmEarlier={confirmEarlier} colorStatus={colorStatus} />
                                </View>
                            </View>
                        </View>
                    }
                        ...
                        expanded[shiftExpand] === true && (
                            <View style={styles.expandItem}>
                                <View style={{ width: '95%' }}>
                                    ...
                                    <View style={styles.iconContainer}>
                                        <Icon name="sync" size={17} style={styles.iconStyle} color={appcolor.dark} />
                                    ...
                                    {isShiftEditable &&
                                        <TouchableOpacity onPress={() => handleSubmit(SUBMIT_SHIFT, item)} style={{ height: 40, marginTop: 10, backgroundColor: appcolor.secondary, borderRadius: 10, justifyContent: 'center', alignItems: "center", }}>
                                            <Text style={{ textAlign: 'center', fontSize: 16, color: appcolor.dark, }}>Xin đổi ca</Text>
                                        </TouchableOpacity>
                                    }
                                </View>
                            </View>
                            ...
                            <View style={styles.expandItem}>
                                <View style={{ width: '95%' }}>
                                    ...
                                    <View style={{ flexDirection: 'row', width: '100%', position: 'relative' }}>
                                        ...
                                        {isLateEditable && <RenderChangeMinute keyBusy="timeLate" index={index} onChangeMinute={onChangeMinute} appcolor={appcolor} />}
                                    </View>
                                    <RenderTextNote appcolor={appcolor} defaultValue={item.noteLate} index={index} item={item} editable={isLateEditable} onChangeNote={onChangeNote} appcolor={appcolor}
                                        keyEdit={"noteLate"} keyForgot={"forgotNoteLate"} />
                                    {
                                        // isShiftEditable &&
                                        isLateEditable &&
                                        <TouchableOpacity onPress={() => handleSubmit(SUBMIT_LATE, item)} style={{ height: 40, marginTop: 10, backgroundColor: appcolor.secondary, borderRadius: 10, justifyContent: 'center', alignItems: "center", }}>
                                            <Text style={{ textAlign: 'center', fontSize: 16, color: appcolor.dark }}>Xin đi trễ</Text>
                                        </TouchableOpacity>
                                    }
                                    <View style={styles.iconContainer}>
                                        <Icon name="clock" size={17} style={styles.iconStyle} />
                                    ...
                                    <View style={{ flexDirection: 'row', width: '100%', position: 'relative' }}>
                                        ...
                                        {isEarlierEditable && <RenderChangeMinute appcolor={appcolor} keyBusy="timeEarlier" index={index} onChangeMinute={onChangeMinute} appcolor={appcolor} />}
                                    </View>
                                    <RenderTextNote appcolor={appcolor} defaultValue={item.noteEarlier} index={index} item={item} editable={isEarlierEditable} onChangeNote={onChangeNote} appcolor={appcolor}
                    ...
                    }
                    {
                        disableCopyButton && (
                            <View style={{ padding: 10, }}>
                                <Text style={{ textAlign: 'center', fontSize: 17, }}>{totalTime}</Text>
                            </View>
                        )
                    }
                    {
                        disableCopyButton && (
                            <View style={{ width: '100%', flexDirection: 'row', alignSelf: 'center' }}>
                                <RenderViewPhotoAttendant appcolor={appcolor} setShowImage={setShowIMV} setUrl={setUrlIMV} imageURL={checkIn1} timeValue={item.timeIn1} />
                                <RenderViewPhotoAttendant appcolor={appcolor} setShowImage={setShowIMV} setUrl={setUrlIMV} imageURL={checkOut1} timeValue={item.timeOut1} />
                            </View>
                        )
                    }
                    {
                        disableCopyButton && item.photoAttendant > 2 && (
                            <View style={{ width: '100%', flexDirection: 'row', alignSelf: 'center' }}>
                                <RenderViewPhotoAttendant appcolor={appcolor} setShowImage={setShowIMV} setUrl={setUrlIMV} imageURL={checkIn2} timeValue={item.timeIn2} />
                                <RenderViewPhotoAttendant appcolor={appcolor} setShowImage={setShowIMV} setUrl={setUrlIMV} imageURL={checkOut2} timeValue={item.timeOut2} />
                            </View>
                        )
                    }
                </View >
    ...
    const styles = StyleSheet.create({
        mainContainer: { height: '100%', shadowOpacity: 0.7, shadowColor: appcolor.light, shadowRadius: 10, borderTopLeftRadius: 30, backgroundColor: appcolor.light, marginTop: 0, borderWidth: 0, borderTopRightRadius: 30, marginLeft: 0, marginRight: 0, width: deviceWidth, },
        shopContainer: { padding: 10, backgroundColor: appcolor.dark, marginTop: 0 },
    ...
    const colorHeaderWeek = AppNameBuild == psvApp ? appcolor.yellowdark : appcolor.secondary
    const colorTextHeader = AppNameBuild == psvApp ? appcolor.dark : appcolor.light
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.homebackground }}>
            <HeaderCustom
                title={route?.params?.titlePage !== undefined && route?.params?.titlePage.length > 0 ? route?.params?.titlePage : "Lịch làm việc PG"}
                leftFunc={() => navigation.goBack()}
                iconRight={textCopyButton == "Copy" && !disableCopyButton ? 'copy' : (!disableCopyButton || checkSaveWeekNow ? confirmPlanS === 1 ? null : 'cloud-upload-alt' : null)}
            ...
            />
            {
                ...
                <View style={{ width: '100%', height: '100%', borderRadius: 5 }}>
                    <TouchableOpacity onPress={() => setShowIMV(false)} style={{ position: 'absolute', top: 20, right: 15, zIndex: 2 }}>
                        <Icon name='times' size={35} color={appcolor.dark} ></Icon>
                    </TouchableOpacity>
            ...
            {currentWeek.label !== undefined && (
                <View style={{ flexDirection: 'row', alignItems: 'center', height: 45 }}>
                    <TouchableOpacity onPress={() => handleDisplayBS(WEEK)} style={{ width: '100%', height: '100%', backgroundColor: colorHeaderWeek, justifyContent: 'center', alignItems: 'center' }}>
                        <View style={{ flexDirection: 'row', width: '90%', justifyContent: 'center', alignItems: 'center' }}>
                            <Icon name="calendar-week" size={18} color={colorTextHeader} />
                            <Text style={{ fontSize: 15, fontWeight: '700', color: colorTextHeader, marginLeft: 5, }}>{currentWeek.label || "-"}</Text>
                        </View>
                    </TouchableOpacity>
                </View>
            )
            ...
            <Modal animationType="slide" visible={visibleBS}>
                <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS == "ios" ? "padding" : 'height'}>
                    <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.light, padding: 10, overflow: 'hidden' }}>
                        <View style={{ padding: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Text style={{ fontSize: 20, color: appcolor.dark, fontWeight: '700' }}>
                                {currentBS.type === WEEK ? "Chọn Tuần" : currentBS.type === SHIFT ? "Chọn Ca Làm Việc" : currentBS.type === SHOPL ? "Chọn cửa hàng" : "Upload Hình Ảnh"}
                            </Text>
                        ...
                        {currentBS.type === IMAGE ?
                            <RenderImageModal styles={styles} appcolor={appcolor} currentBS={currentBS} data={data} handleExpanded={handleExpanded} handleRemoveImage={handleRemoveImage} handleSubmit={handleSubmit} cameraLaunch={cameraLaunch} imageGalleryLaunch={imageGalleryLaunch} expanded={expanded} />
                            :
                            currentBS.type === SHOPL ?
                                <RenderShopModal currentBS={currentBS} onSelectItemBS={onSelectItemBS} onClearItemSelectedBS={onClearItemSelectedBS} appcolor={appcolor} />
                                :
...
const RenderShopModal = ({ appcolor, onSelectItemBS, currentBS, onClearItemSelectedBS }) => {
                ...
                return (
                    <ListItem key={index} onPress={() => onSelectItemBS(item)}
                        ...
                        <ListItem.Content style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', }}>
                            ...
                            <ListItem.Title style={{ color: appcolor.dark, fontSize: 16, padding: 1, }}>{item.ShopName}</ListItem.Title>
                        </ListItem.Content>
                        {(isExist && currentBS.type !== WEEK && currentBS.isPastOrPresentWeek) && (
                            <TouchableOpacity onPress={() => onClearItemSelectedBS(currentBS.index)}>
                                <Icon name="times" size={18} color={appcolor.dark} />
                            </TouchableOpacity>
                        )}
                    </ListItem>)
            }} />
...
const RenderImageModal = ({ currentBS, data, handleRemoveImage, handleSubmit, cameraLaunch, imageGalleryLaunch, styles, appcolor }) => {
    const takePhoto = () => {
    ...
    return (
        <View style={{ backgroundColor: appcolor.light, justifyContent: 'space-between', height: '100%', }}>
            <View style={{ width: '100%', height: '89%' }}>
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) => {
                        return (
                            <View key={index} style={{ alignItems: 'center', backgroundColor: appcolor.background, marginTop: 8 }}>
                                ...
                                <TouchableOpacity onPress={() => item.dataUpload == 0 ? handleRemoveImage(item, index) : null} style={{ position: 'absolute', top: 5, right: 16, }}>
                                    <Icon name={item.dataUpload == 0 ? "times" : "check"} size={23} color={item.dataUpload == 0 ? appcolor.danger : appcolor.green} />
                                </TouchableOpacity>
                            </View>)
                    }} />
            </View>
...
const RenderExpandBtn = ({ appcolor, shiftStatus, lateStatus, earlierStatus, expanded, handleExpanded, showState, hideState, isShift, confirmShift, confirmLate, confirmEarlier, colorStatus }) => {
    const handlePress = () => {
    ...
    return (
        <TouchableOpacity onPress={handlePress} style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
            <Icon name={expanded[showState] === true ? "chevron-down" : "chevron-right"} size={15} style={{ color: appcolor.dark, minWidth: 18 }} />
            {isShift ?
                <View>
                    <Text style={{ textAlign: 'center', color: colorStatus(confirmShift), fontSize: 16, }}>{shiftStatus}</Text>
                </View>
                :
                <View>
                    <View style={{ flexDirection: 'row', alignItems: 'center', }}>
                        <Icon name="hourglass-start" size={12} style={{ color: colorStatus(confirmLate), marginRight: 5 }} />
                        <Text style={{ color: colorStatus(confirmLate), fontSize: 16, }}>{lateStatus}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Icon name="hourglass-end" size={12} style={{ color: colorStatus(confirmEarlier), marginRight: 5 }} />
...
const RenderButtonDisplayBS = ({ appcolor, styles, index, item, keyForgot, type, handleDisplayBS, value, isEditable }) => {
    const handlePress = () => {
...
const RenderTextNote = ({ appcolor, index, item, defaultValue, keyEdit, keyForgot, onChangeNote, editable, }) => {
    ...
    return (
        <View style={{ backgroundColor: appcolor.light, borderColor: checkForgot ? appcolor.warning : appcolor.dark, borderWidth: 0.3, padding: 6, borderRadius: 10, }}>
            <View style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                <Icon style={{ padding: 8, color: checkForgot ? appcolor.warning : appcolor.dark }} name={"sticky-note"} size={18} />
                <TextInput
                    ...
                    placeholder="Ghi chú... (tối thiểu 5 ký tự)" placeholderTextColor={checkForgot ? appcolor.warning : appcolor.greydark} />
            </View>
        </View>
    )
}
...
const RenderTextMinute = ({ index, value, item, keyForgot, onChangeMinute, appcolor, keyBusy, iconName, isBusyEditable, }) => {
    const handleChange = (e) => {
        ...
        <View style={{ backgroundColor: appcolor.light, borderWidth: 0.3, borderColor: checkForgot ? appcolor.warning : appcolor.dark, padding: 6, borderRadius: 10, }}>
            <View style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                ...
                <TextInput
                    ...
                    style={{ color: checkForgot ? appcolor.warning : appcolor.dark, fontSize: 15, flex: 1, padding: 0, paddingLeft: 10 }}
                    placeholder="Phút..." keyboardType="numeric"
                    placeholderTextColor={checkForgot ? appcolor.warning : appcolor.greydark} />
            </View>
        </View>
    )
}
const RenderViewPhotoAttendant = ({ appcolor, setShowImage, setUrl, imageURL, timeValue }) => {
    const showPhoto = () => {
        setShowImage(true);
    ...
    return (
        <View style={{ width: '50%', alignSelf: 'center', alignItems: 'center' }}>
            <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center', borderRadius: 5, backgroundColor: appcolor.light }}>
                {imageURL ?
                    <TouchableOpacity style={{ width: '90%', height: 120, borderRadius: 5 }} onPress={showPhoto}>
                        <Image source={{ uri: URLDEFAULT + imageURL }} style={{ width: '100%', height: 120, borderRadius: 5 }} />
                    </TouchableOpacity>
                    :
