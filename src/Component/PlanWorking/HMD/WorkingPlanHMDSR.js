...
import {
    View, StyleSheet, TextInput, Platform, RefreshControl, Modal,
    TouchableOpacity, KeyboardAvoidingView, ScrollView, Switch, FlatList
} from "react-native";
...
import { alertWarning, alertNotify, alertConfirm, alertError, TODAY } from '../../../Core/Utility';
...
const WorkingPlanHMDSR = ({ navigation, route }) => {
    const [searchText, setSearchText] = useState('');
    const [dateSelected, setDateSelect] = useState(moment(new Date()).format('YYYY-MM-DD'))
    ...
    const LoadDataPlan = async (mDate) => {
        ...
        await SR_PLAN_GETLIST(dataHeader, async (mDataPlan, mDataWeek) => {
    ...
    const onChangeMinute = ({ item, index, value, isPress = true, isPlus, keyBusy }) => {
        let valueTime
        if (!isPress) {
            valueTime = parseInt(value)
        } else {
            valueTime = keyBusy == 'timeLate' ? (isPlus ? item.timeLate + 5 : item.timeLate - 5) : (isPlus ? item.timeEarlier + 5 : item.timeEarlier - 5)
        }
        if (valueTime > 90)
            valueTime = 90
    ...
    const searchShiftAction = (text) => {
        if (text.length > 0) {
            const filterShift = typeBS == 'SHIFT' ?
                FilterDataModalBS.filter((i) => i.Name.toLowerCase().match(text.toLowerCase()) || i.Ref_Code.toLowerCase().match(text.toLowerCase())) :
                typeBS == 'NOTE' ?
                    FilterDataModalBS.filter((i) => i.title !== undefined && i.title.toLowerCase().match(text.toLowerCase()) || i.Ref_Code.toLowerCase().match(text.toLowerCase())) :
                    FilterDataModalBS.filter((i) => i.ShopName !== undefined && i.ShopName.toLowerCase().match(text.toLowerCase()))
    ...
    const searchShops = (text) => {
        ...
        if (text && text !== undefined) {
            let filterShops = mainData.filter(i => i.mainIndex === 0 ||
    ...
    const handlerChangeDataBS = (item) => {
        let changeItem = JSON.parse(dataPlan[0].refList).filter(i => i.Ref_Code == item.Ref_Code);
    ...
    const handlerSave = async () => {
        ...
        alertConfirm("Thông báo", "Bạn có muốn thay đổi lịch làm việc như bên dưới không ?", async () => {
            ...
            let itemOFF = mainData[0];
            // Check Off 
            let dataShiftOFF = JSON.parse(itemOFF.shiftList).filter(c => c.Name == itemOFF.shiftChange);
            let dataNote = JSON.parse(itemOFF.refList !== null ? itemOFF.refList : '[]').filter(s => s.title == itemOFF.reasonName);
            ...
            let itemSaveOFF = {
                ...
                'ConfirmPlan': 3,
                'ShiftType': dataShiftOFF.length > 0 ? dataShiftOFF[0].Code : null,//itemOFF.shiftType,
            ...
            mainData.forEach((i, index) => {
                ...
                if (i.confirmShop == 3) {
                    if (i.lastStatus == 'ADD' || i.lastStatus == 'DEL') {
                        ...
                        if (moment(new Date()).isoWeek() == moment(dateSelected).isoWeek()) {
            ...
            const data = { dataPlan: JSON.stringify(dataSave), contentMessage: contentMessage }
    ...
    const RenderPressable = ({ index, item, styles, appcolor, typeBS, listSelect, value, titleStyle = null }) => {
        const handlePress = () => {
            switch (typeBS) {
                case 'NOTE':
                    let changeItem = item.ref_Code == null ? listSelect.filter(i => i.Ref_Code == 'ON') : listSelect.filter(i => i.Ref_Code == item.ref_Code);
                    setDataModalBS(changeItem.map(i => (i.title == item.reasonName ? { ...i, isSelect: 1 } : i)))
                    setVisibleBS(true)
                    setFilterDataModalBS(changeItem)
                    break;
                case 'SHIFT':
                    setDataModalBS(listSelect.map(i => (i.Name == item.shiftChange ? { ...i, isSelect: 1 } : i)))
        ...
        return (
            <View>
                <TouchableOpacity disabled={item.isLockShift == 1 ? true : false} style={[styles.inputShop, { flex: 1, flexDirection: 'row', alignItems: 'center', borderColor: appcolor.dark, borderWidth: 0.5 }]} onPress={handlePress}>
                    <Text style={{ ...titleStyle, color: appcolor.dark }} accessibilityHint="test" >{value}</Text>
                </TouchableOpacity>
            </View>
        )
    }
    const RenderTextInput = ({ index, typeInput, item, value, styles, handlenotes, isLock }) => {
        ...
        return (
            <TextInput
                ...
                placeholderTextColor={appcolor.gray} placeholder="Ghi chú..."
    ...
    const renderItem = ({ item, index }) => {
        ...
        const highlightShop = item.lastStatus !== null ? styles.shopAddContainer : styles.shopContainer
        const colorStatusShift = item.colorStatusShift == '#' ? appcolor.dark : item.colorStatusShift
        const colorConfirmShop = item.confirmShop == 1 ? 'green' : item.confirmShop == -1 ? appcolor.danger : appcolor.rejection
        return (
            <View key={index} >
                {item.isParent &&
                    <View style={{ flex: 1 }}>
                        {item.checkWP > 0 &&
                            <View style={styles.viewWorking}>
                                {item.shiftName !== null && <Text style={[styles.titleView, { color: appcolor.bluenavylight }]}>{'Ca hiện tại: ' + item.shiftName}</Text>}
                                <Text style={[styles.titleView, { color: colorStatusShift }]} >{item.shiftStatus}</Text>
                                {item.supNote !== null && <Text style={[styles.titleView, { color: colorStatusShift }]} >{item.supNote}</Text>}
                                <RenderPressable item={item} styles={styles} appcolor={appcolor} typeBS='SHIFT' listSelect={JSON.parse(item.shiftList)} value={item.shiftChange} />
                                <Text style={styles.titleView} >Lí do thay đổi</Text>
                                <RenderPressable index={index} item={item} styles={styles} appcolor={appcolor} typeBS='NOTE' listSelect={JSON.parse(item.refList)} value={item.reasonName} />
                                <RenderTextInput index={index} typeInput="SHIFT" item={item} isLock={item.isLockShift} value={item.notes} handlenotes={handlenotes} styles={[styles.inputShop, { marginTop: 5, borderColor: appcolor.dark, borderWidth: 0.5 }]} />
                            </View>
                        }
                    </View>
                }
                {item.statusChangeShop !== undefined && item.statusChangeShop.length > 0 && <Text style={[styles.titleView, { color: colorConfirmShop, fontWeight: '500' }]}> {item.statusChangeShop}</Text>}
                {item.shopId !== item.employeeId &&
                    <ListItem.Content style={[highlightShop, { borderLeftColor: item.colorPlan, backgroundColor: appcolor.light }]}>
                        <View style={{ flex: 1 }} >
                            <View style={[styles.itemShopContainer, { padding: 5 }]}>
                                <View style={styles.iconContainer}>
                                    <Icon name="store" size={16} style={styles.iconHeaderStyle} />
                                </View>
                                <Text style={[styles.titleView, { width: '75%' }]} >{item.shopName}</Text>
                                <View style={{ flex: 1, alignItems: 'flex-end' }}>
                                    {item.isLocked == 0 &&
                                        <Switch
                                            ...
                                            //Event 
                                            value={item.status == 1} onValueChange={(value) => toggleSwitch(item, index, value)} />
                                    }
                                </View>
                            </View>
                            <Text style={{ flex: 1, paddingTop: 3, paddingStart: 5, fontSize: 13, color: appcolor.dark }}>{item.shopCode}</Text>
                            <Text style={{ flex: 1, paddingTop: 3, paddingStart: 5, fontSize: 13, color: appcolor.dark }}>{item.address}</Text>
                            <RenderTextInput index={index} typeInput="SHOP" item={item} isLock={item.isLockStatus} value={item.notes} handlenotes={handlenotes} styles={[styles.inputShop, { marginTop: 5, borderColor: appcolor.dark, borderWidth: 0.5 }]} />
                            {item.confirmPlanNote !== null && item.confirmPlanNote.length > 0 && <Text style={{ flex: 1, padding: 8, fontSize: 13, color: appcolor.danger }}>{item.confirmPlanNote}</Text>}
                        </View>
                    </ListItem.Content>
                }
            </View>
    ...
    const renderItemModal = (item, index, typeBS) => {
        const value = typeBS == 'SHIFT' ? item.Name : (typeBS == 'NOTE' ? item.title : item.ShopName)
        const mPress = () => {
            handleSelectedChange(item, index, value, typeBS)
        }
        return (
            <ListItem bottomDivider onPress={mPress} containerStyle={{ backgroundColor: appcolor.background }} >
                <ListItem.Content>
                    <ListItem.Title style={{ color: appcolor.dark, fontSize: 14 }}>{value}</ListItem.Title>
                </ListItem.Content>
                {item.isSelect == 1 &&
                    <TouchableOpacity onPress={() => clearItemSelect(item, index, typeBS)}>
    ...
    const styles = StyleSheet.create({
        mainContainer: { flex: 1, backgroundColor: appcolor.white },
        shopContainer: { margin: 5, padding: 5, borderWidth: 1, borderRadius: 8, borderColor: appcolor.greylight },
        shopAddContainer: { margin: 5, padding: 5, borderWidth: 1.5, borderRadius: 8, borderColor: appcolor.yellow, backgroundColor: appcolor.greylight },
        itemShopContainer: { flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', padding: 0, width: '100%', },
        itemShopText: { color: appcolor.dark, fontSize: 13, width: '90%', padding: 3 },
        ...
        inputShop: { flex: 1, borderColor: appcolor.darklight, borderWidth: 0.5, borderRadius: 3, padding: 8 },
        ...
        titleView: { color: appcolor.dark, fontWeight: '700', fontSize: 15, padding: 3 },
        titleContainer: { flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', width: '100%', padding: 8 },
        viewWorking: { flex: 1, borderRadius: 10, backgroundColor: appcolor.light, padding: 8, margin: 8, borderColor: appcolor.dark, borderWidth: 0.5 },
    ...
    return (
        <KeyboardAvoidingView
            style={{ flex: 1 }}
            behavior={Platform.OS == "ios" ? "padding" : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 40 : 50}>
            <View style={{ flex: 1 }}>
                <HeaderCustom
                    title={route?.params?.titlePage !== undefined && route?.params?.titlePage.length > 0 ? route?.params?.titlePage : "Lịch làm việc SR"}
                ...
                <View style={styles.mainContainer}>
                    ...
                    <FormGroup
                        ...
                        placeholder={"Tìm kiếm cửa hàng"} value={searchText} editable
                        handleChangeForm={searchShops}
                    />
                    <LoadingView title={titleLoading} isLoading={refreshing} />
                    <ScrollView style={{ flex: 1, paddingBottom: paddingTopIPX }}
                ...
                <Modal animationType="slide" visible={visibleBS}>
                    <KeyboardAvoidingView
                        ...
                        <View style={{ padding: 5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: paddingTopIPX }}>
                            <Text h4 style={{ padding: 8, fontWeight: '500', color: appcolor.dark, textAlign: 'center' }}>
                                {typeBS == 'SHIFT' ? 'Xin phép' : (typeBS == 'NOTE' ? 'Lí do chuyển ca' : 'Danh sách cửa hàng')}
                            </Text>
                            <TouchableOpacity style={{ padding: 10 }} onPress={() => setVisibleBS(false)}>
                                <Icon name="times" size={25} color={appcolor.onSurfaceSecond} onPress={() => setVisibleBS(false)} />
                            </TouchableOpacity>
                        </View>

                        <FormGroup
                            containerStyle={styles.filterStyle}
                            placeholder={"Tìm kiếm ..."} defaultValue={searchText} editable handleChangeForm={searchShiftAction} multiline iconName='search' />
                        <FlatList
                            data={mDataModalBS}
                            extraData={mDataModalBS}
                            listKey="mDataModalBS"
                            keyExtractor={(item, index) => index.toString()}
                            renderItem={({ item, index }) => renderItemModal(item, index, typeBS)} />
                    </KeyboardAvoidingView>
                </Modal>
            </View>
        </KeyboardAvoidingView>
...
export default WorkingPlanHMDSR;