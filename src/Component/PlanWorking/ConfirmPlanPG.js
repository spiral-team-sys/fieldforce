...
const ConfirmPlanPG = ({ navigation, route }) => {
        ...
        async () => {
          ...
          if (listModified.length > 0) {
            ...
            for (let i = 0, lenList = listModified.length; i < lenList; i++) {
              ...
              if (
                (confirmLate === REJECT_VALUE(typeId) && !supNoteLate) ||
                (supNoteLate && supNoteLate.length < 5)
              ) {
                errorStr =
                  'Vui lòng nhập ghi chú lí do không xác nhận đi trễ cửa hàng ' +
                  shopName +
                  ' - ' +
                  employeeName +
                  ' (Tối thiểu 5 kí tự)';
              ...
              if (
                (confirmEarlier === REJECT_VALUE(typeId) && !supNoteEarlier) ||
                (supNoteEarlier && supNoteEarlier.length < 5)
              ) {
                errorStr =
                  'Vui lòng nhập ghi chú lí do không xác nhận về sớm cửa hàng ' +
                  shopName +
                  ' - ' +
                  employeeName +
                  ' (Tối thiểu 5 kí tự)';
              ...
              if (confirm !== 0) dataPlan.push(item);
            ...
            alertWarning(
              'Bạn chưa thực hiện phê duyệt/từ chối yêu cầu nào, Vui lòng phê duyệt/từ chối các yêu cầu bên dưới trước khi gửi dữ liệu',
            );
          }
        },
      );
  ...
  const ViewImageEvident = ({ listPhotoItem, itemIndex }) => {
    return (
      <View
        style={{
          flexDirection: 'row',
  ...
  const renderItem = ({ item, index }) => {
    ...
    const imageEvident = JSON.parse(item.photoEvident || '[]');
    return (
        ...
        {item.isParent && (
          ...
          >
            <SpiralIcon
              ...
              name="user"
              size={18}
              style={{ color: 'black', paddingEnd: 8 }}
            />
            <ListItem.Title style={{ color: 'black', fontSize: 16 }}>
          ...
          {item[`${item.employeeId}${item.auditDate}`] && (
            <View>
              ...
              >
                ...
                {imageEvident.length > 0 && (
                  <ViewImageEvident
                    listPhotoItem={imageEvident}
                    itemIndex={index}
                  />
                )}
                {item.confirmStatus !== null && (
  ...
  return (
    <View style={{ flex: 1 }}>
      <HeaderCustom
        title={
          route?.params?.menuitem.menuNameVN || 'Xác nhận lịch làm việc PG'
        }
      ...
      >
        <MultipleShowImage
          ...
          closeShowImage={() => setVisibleImage(false)}
