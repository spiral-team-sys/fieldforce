...
import { alertWarning, alertNotify, alertConfirm, deviceWidth } from '../../Core/Utility';
...
const ConfirmPlanPG = ({ navigation, route }) => {
                ...
                if (listModified.length > 0) {
                    ...
                    for (let i = 0, lenList = listModified.length; i < lenList; i++) {
                        ...
                        if ((confirmLate === REJECT_VALUE(typeId) && !supNoteLate) || (supNoteLate && supNoteLate.length < 5)) {
                            errorStr = "Vui lòng nhập ghi chú lí do không xác nhận đi trễ cửa hàng " + shopName + " - " + employeeName + " (Tối thiểu 5 kí tự)"
                        }
                        if ((confirmEarlier === REJECT_VALUE(typeId) && !supNoteEarlier) || (supNoteEarlier && supNoteEarlier.length < 5)) {
                            errorStr = "Vui lòng nhập ghi chú lí do không xác nhận về sớm cửa hàng " + shopName + " - " + employeeName + " (Tối thiểu 5 kí tự)"
                        }
                        let item = {
                            "PlanId": planId,
    ...
    const RenderItemPhoto = ({ item, index, listPhotoItem }) => {
        ...
        return (
            <View key={index} style={{ width: deviceWidth / 4, height: deviceWidth / 4, backgroundColor: appcolor.surface, margin: 5, borderRadius: 12 }}>
                <TouchableOpacity key={index} onPress={onSelectImage} >
                    <Image source={{ uri: URLDEFAULT + item.photoPath }} style={{ width: '100%', height: '100%', borderRadius: 12 }} />
                </TouchableOpacity>
            </View>
        )
    }
            ...
            <View key={index} style={{ flex: 1 }}>
                {item.isParent &&
                    <View style={[styles.titleContainer, { backgroundColor: appcolor.yellowdark }]}>
                        <Icon solid name="user" size={18} style={{ color: 'black', paddingEnd: 8 }} />
                        <ListItem.Title style={{ color: 'black', fontSize: 16 }}>Nhân viên: {item.employeeName}</ListItem.Title>
                    </View>
                }
                <View key={index}>
                        ...
                        <View>
                            ...
                            <ListItem.Content style={[styles.shopContainer, { marginStart: 8, marginEnd: 8 }]}>
                                <View style={[styles.itemShopContainer, { paddingStart: 2, paddingBottom: 5 }]}>
                                    <Icon name="home" size={18} style={[styles.iconStyle]} color={appcolor.dark} />
                                ...
                                {imageEvident.length > 0 && <ViewImageEvident listPhotoItem={imageEvident} itemIndex={index} />}
    ...
    return (
        <View style={{ flex: 1 }}>
            <HeaderCustom
                title={route?.params?.menuitem.menuNameVN || 'Xác nhận lịch làm việc PG'}
            ...
            <KeyboardAvoidingView style={styles.keyboardAvoidingView} behavior={Platform.OS == "ios" ? "padding" : 'height'}>
                <View style={styles.mainContainer}>
                    <CalendarStrip
                        scrollable={true}
                        scrollerPaging={true}
                        style={{ flexDirection: 'row', height: 80, paddingTop: 8, paddingBottom: 8, fontSize: 12 }}
                        minDate={'2023-01-01'}
                        calendarHeaderStyle={{ color: 'white', marginBottom: 16, fontSize: 12 }}
                        calendarColor={DEFAULT_COLOR}
                        dateNumberStyle={{ color: 'white', fontSize: 12 }}
                        dateNameStyle={{ color: 'white', fontSize: 12 }}
                        customDatesStyles={customDatesStyles}
                        highlightDateContainerStyle={{ backgroundColor: 'white', fontSize: 12 }}
            ...
            >
                <MultipleShowImage key={'ShowItemImage'} listItem={dataPhoto.listPhoto} closeShowImage={() => setVisibleImage(false)} indexItem={dataPhoto.indexImage} />
            </Modal>
        </View>
    )
}
