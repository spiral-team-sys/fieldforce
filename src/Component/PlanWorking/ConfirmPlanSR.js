...
const ConfirmPlanSR = ({ navigation, route }) => {
    ...
    const [dataModal, setDataModal] = useState({ titleModal: '', visible: false, dataSelect: [], dataFilter: [] })
    ...
    const LoadDataPlan = async (weekChange) => {
        ...
        try {
            ...
            await GetDataConfirmBySR(dataBody, async (mDataPlan, mDataWeek, dataEmp) => {
                ...
                await setDataModal({ ...dataModal, dataSelect: dataEmp, dataFilter: dataEmp })
    ...
    const loadPlanByDate = async (mDate) => {
        ...
        setDataModal({ ...dataModal, dataSelect: dataModal.dataFilter.map(i => ({ ...i, isSelect: 0 })), visible: false })
    }
    ...
    const dataMarkedDates = (mDataWeek) => {
        ...
        customDatesStyles.push({
            startDate: new Date(),
            dateNameStyle: { color: appcolor.highlightDate, fontSize: 10 },
            dateNumberStyle: { color: appcolor.highlightDate, fontSize: 14 },
            highlightDateNameStyle: { color: appcolor.highlightDate, fontSize: 10 },
            highlightDateNumberStyle: { color: appcolor.highlightDate, fontSize: 14 },
        });
    ...
    const handlerConfirm = (item, typeConfirm, isConfirm, index) => {
        ...
        switch (typeConfirm) {
            ...
            case 'LATE':
                ...
                setColorConfirm = prevConfirmLate === isConfirm ? appcolor.darklight : isConfirm == CONFIRM_VALUE(item.typeId) ? 'green' : appcolor.darklight
                setColorReject = prevConfirmLate === isConfirm ? appcolor.darklight : isConfirm == REJECT_VALUE(item.typeId) ? 'red' : appcolor.darklight
            ...
            case 'EARLIER':
                ...
                setColorConfirm = prevConfirmEarlier === isConfirm ? appcolor.darklight : isConfirm == CONFIRM_VALUE(item.typeId) ? 'green' : appcolor.darklight
                setColorReject = prevConfirmEarlier === isConfirm ? appcolor.darklight : isConfirm == REJECT_VALUE(item.typeId) ? 'red' : appcolor.darklight
    ...
    const handlerConfirmShop = async (item, isConfirm, index, indexMain) => {
        ...
        let colorConfirm = item.confirm === isConfirm ? appcolor.darklight : isConfirm == CONFIRM_VALUE(item.typeId) ? 'green' : appcolor.darklight
        let colorReject = item.confirm === isConfirm ? appcolor.darklight : isConfirm == REJECT_VALUE(item.typeId) ? 'red' : appcolor.darklight
    ...
    const RenderViewConfirm = ({ item, typeConfirm, index }) => {
        ...
        return (
            <View key={`ascc_${index}`} style={styles.bordertab}>
    ...
    const renderItemShopList = (item, index, indexMain) => {
        const colorConfirmShop = item.confirm == CONFIRM_VALUE(item.typeId) ? 'green' : item.confirm == REJECT_VALUE(item.typeId) ? appcolor.danger : appcolor.rejection
        return (
            <View key={`asds_${index}`} style={{ flex: 1 }}>
                ...
                {item.shopId !== item.employeeId &&
                    <ListItem.Content style={[styles.shopContainer, { marginStart: 8, marginEnd: 8 }]}>
                            ...
                            <View style={{ flex: 1, alignItems: 'flex-end' }}>
                                {item.TypeList == 0 ?
                                    <Switch
                                        ...
                                        ios_backgroundColor={appcolor.grey}
                        ...
                        <RenderTextInput item={item} index={index}
                            handleChangeNote_SHOP={handleChangeNote_SHOP} typeConfirm="SHOP" styles={styles} appcolor={appcolor} indexMain={indexMain} />
                    </ListItem.Content>
                }
            </View>
        )
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View key={`iids_${index}`} style={{ flex: 1 }}>
                {item.isParent &&
                    <View style={[styles.titleContainer, { backgroundColor: appcolor.primary }]}>
                        <Icon solid name="user" size={18} style={{ color: appcolor.white, paddingEnd: 8 }} />
                        <ListItem.Title style={{ color: appcolor.white, fontSize: 16 }}>Nhân viên: {item.employeeName}</ListItem.Title>
                    </View>
                }
                <View key={index}>
                        ...
                        <View>
                            ...
                            {item.shopList !== '[]' &&
                                <TouchableOpacity onPress={() => handleExpanded(item, index)} style={styles.shopListView}>
                                    ...
                                    <Icon name="chevron-down" size={18} style={{ color: appcolor.bluenavylight, marginRight: 8, position: 'absolute', end: 0 }}
                                        solid={true} />
                                </TouchableOpacity>
                            }
                        </View>
                    ...
                    {item.expandShops == 1 &&
                        <View>
                            <FlatList
                                style={{ flex: 1, backgroundColor: appcolor.surface, margin: 8, padding: 8, borderRadius: 10 }}
    ...
    const styles = StyleSheet.create({
        ...
        shopContainer: { padding: 10, borderRadius: 10, backgroundColor: appcolor.light, marginBottom: 5 },
        ...
        iconStyle: { color: appcolor.grey, paddingEnd: 5 },
        ...
        filterStyle: { width: '95%', fontSize: 13, alignSelf: 'center', borderWidth: 0.5, borderRadius: 8, borderColor: appcolor.grey, padding: 8 },
    ...
    const searchModal = async (text) => {
        if (text.length > 0) {
            const filterShift = dataModal.dataFilter.filter((i) => i.employeeName.toLowerCase().match(text.toLowerCase()))
    ...
    const chooseItemModal = async (item) => {
        const lstFilter = mainData.filter(i => (i.employeeCode == item.employeeCode))
        const { arr } = groupDataByKey({
            arr: lstFilter,
            key: "employeeId",
            keyLayer2: "auditDate",
        ...
        setData(JSON.parse(JSON.stringify(arr)))
        setDataModal({ ...dataModal, dataSelect: dataModal.dataFilter.map(i => i.employeeCode == item.employeeCode ? { ...i, isSelect: 1 } : { ...i, isSelect: 0 }), visible: false })
    }
    ...
    const showModalView = async () => {
        await setDataModal({ ...dataModal, titleModal: 'Danh sách nhân viên', visible: true })
    }
    ...
    return (
        <View style={{ flex: 1 }}>
            <HeaderCustom
                title={kpiinfo.menuNameVN || 'Xác nhận LLV'}
            ...
            <View style={styles.mainContainer}>
                <CalendarStrip
                    key={`calendar_${dateSelected}`}
                    style={{ flexDirection: 'row', height: 100, alignItems: 'center' }}
                    shouldAllowFontScaling={false}
                    minDate={'2023-01-01'}
                    calendarHeaderStyle={{ color: appcolor.light, marginBottom: 16, fontSize: 16 }}
                    calendarColor={DEFAULT_COLOR}
                    dateNumberStyle={{ color: appcolor.light, fontSize: 14 }}
                    dateNameStyle={{ color: appcolor.light, fontSize: 12 }}
                    customDatesStyles={customDatesStyles}
                    highlightDateContainerStyle={{ backgroundColor: appcolor.light, borderRadius: 8 }}
                    highlightDateNumberStyle={{ color: appcolor.dark, fontSize: 14 }}
                    highlightDateNameStyle={{ color: appcolor.dark, fontSize: 12 }}
                    disabledDateNameStyle={{ color: appcolor.lightgray, fontSize: 12 }}
                    disabledDateNumberStyle={{ color: appcolor.lightgray, fontSize: 14 }}
                ...
                />
                <LoadingView title={'Đang tải dữ liệu...'} isLoading={refreshing} styles={{ marginTop: 8, zIndex: 20 }} />
                    ...
                    <CustomListView
                        ...
                        ListEmpty={
                            <View style={{ width: "100%", justifyContent: 'center', alignItems: 'center', padding: 20 }}>
                                <Text style={{ color: appcolor.danger, fontSize: 18 }}>Không có yêu cầu phê duyệt</Text>
                            </View>
                        }
                    />
            ...
            <Modal animationType="slide" visible={visibleBS}>
                <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.background, height: '100%', padding: 10, overflow: 'hidden' }}>
                    ...
                    <RenderContentModal currentBS={currentBS} onSelectItemBS={onSelectItemBS} appcolor={appcolor} onClearItemSelectedBS={onClearItemSelectedBS} />
                </SafeAreaView>
            </Modal>
            <ModalItem
    ...
    return (
        <FlatList
            ...
            renderItem={({ item, index }) => {
                ...
                return (
                    <ListItem key={index} onPress={() => onSelectItemBS(item)} containerStyle={{ backgroundColor: isExists ? appcolor.greylight : appcolor.background }} >
                        <ListItem.Content style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', }}>
                            ...
                            <ListItem.Title style={{ color: appcolor.onBackground, fontSize: 16, padding: 1, }}>{item[key]}</ListItem.Title>
                        </ListItem.Content>
                        {isExists && (
                            <TouchableOpacity onPress={() => onClearItemSelectedBS(currentBS.index)}>
                                <Icon name="backspace" size={18} color={appcolor.onBackground} />
                            </TouchableOpacity>
                        )}
                    </ListItem>)
            }} />
...
const RenderTextInput = ({ item, index, handleChangeNote, handleChangeNote_SHOP, typeConfirm, styles, appcolor, indexMain }) => {
    ...
    return (
        <TextInput editable={isLock == 0 ? true : false} defaultValue={value} multiline={true} style={styles.inputShop}
            placeholderTextColor={appcolor.placeholderTextColor} placeholder={titleNote} onChangeText={handleChange} />
    )
}
export default ConfirmPlanSR;
