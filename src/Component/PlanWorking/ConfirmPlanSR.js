...
const ConfirmPlanSR = ({ navigation, route }) => {
  ...
  const LoadDataPlan = async weekChange => {
    ...
    try {
      let week =
        weekChange !== undefined ? weekChange : moment(new Date()).isoWeek();
      const dataBody = { Week: week, Year: moment(new Date()).year() };

      await GetDataConfirmBySR(
        dataBody,
        async (mDataPlan, mDataWeek, dataEmp) => {
  ...
  const loadPlanByDate = async mDate => {
    let dateChange =
      mDate == undefined
        ? mDate
        : new Date(mDate).getTime() === new Date(dateSelected).getTime()
        ? undefined
        : mDate;
    ...
    setDataModal({
      ...dataModal,
      dataSelect: dataModal.dataFilter.map(i => ({ ...i, isSelect: 0 })),
      visible: false,
    });
  };
        ...
        async () => {
          ...
          if (listModified.length > 0) {
            ...
            for (let i = 0, lenList = listModified.length; i < lenList; i++) {
              ...
              if (
                (confirm === REJECT_VALUE(typeId) && !supNote) ||
                (supNote && supNote.length < 5)
              ) {
                errorStr =
                  'Vui lòng nhập lí do không xác nhận chuyển ca cửa hàng ' +
                  shopName +
                  ' - ' +
                  employeeName +
                  ' (Tối thiểu 5 kí tự)';
              ...
              JSON.parse(shopList).forEach(s => {
                if (
                  s.TypeList == 1 &&
                  s.confirm == REJECT_VALUE(typeId) &&
                  s.confirmPlanNote.length < 5
                ) {
                  errorStr =
                    'Vui lòng nhập lí do không đồng ý cửa hàng ' +
                    s.shopName +
                    ' - ' +
                    employeeName +
                    ' (Tối thiểu 5 kí tự)';
                ...
                if (
                  ...
                  errorStr =
                    'Vui lòng nhập lí do thêm cửa hàng cửa hàng ' +
                    s.shopName +
                    ' - ' +
                    employeeName +
                    ' (Tối thiểu 5 kí tự)';
                ...
                let shopItem = {
                  ...
                  Confirm: s.confirm,
                  SupNoteShop:
                    s.confirmPlanNote == undefined ? '' : s.confirmPlanNote,
                };
  ...
  const onFilterContentBS = value => {
    ...
    if (value) {
      const filterList = listFilter.filter(e => {
        const labelFilter = e[keyFilter]
          ? e[keyFilter].toUpperCase()
          : ''.toUpperCase();
  ...
  const onSelectItemBS = async item => {
    ...
    if (type === 'EMPLOYEE') {
      let filterEmployee = await mainData.filter(
        e => e.employeeName == item.employeeName,
      );
  ...
  const isShiftBeforeCurrentTime = (dateCheck, from, to) => {
    ...
    to = moment(to, 'HH:mm');
    if (
      isSameDate(dateCheck, moment()) &&
      (from - moment() <= 0 || to - moment() <= 0)
    ) {
      return true;
    }
    return false;
  ...
  const handleDisplayBS = (type, index = -1) => {
    ...
    switch (type) {
      ...
      case 'SHIFT':
        ...
        const listShift =
          JSON.parse(JSON.stringify(dataBS.listShiftPrototype || [])) || [];
        for (let i = 0, lenArr = listShift.length; i < lenArr; i++) {
          let isValidShift = true;
          for (const value of Object.values(
            shiftRegistered[
              `${data[index].employeeId}${data[index].auditDate}`
            ],
          )) {
            if (
              listShift[i].Code === value.code ||
              (listShift[i].From === value.from &&
                listShift[i].To === value.to &&
                listShift[i].From !== undefined &&
                listShift[i].To !== undefined)
            ) {
            ...
            if (
              ...
              data[index].from !== value.from &&
              data[index].to !== value.to
            ) {
              listShift[i].isShiftCoincident = true;
              isValidShift = false;
            }
            if (
              isShiftBeforeCurrentTime(
                data[index].auditDate,
                listShift[i].From,
                listShift[i].To,
              )
            ) {
      ...
      case 'REASON':
        ...
        list = dataBS.listReason.filter(
          e => e.Ref_Code === data[index].ref_Code,
        );
        break;
    }
      ...
      case 'SHIFT':
        ...
        setColorConfirm =
          prevConfirm === isConfirm
            ? appcolor.darklight
            : isConfirm == CONFIRM_VALUE(item.typeId)
            ? 'green'
            : appcolor.darklight;
        setColorReject =
          prevConfirm === isConfirm
            ? appcolor.darklight
            : isConfirm == REJECT_VALUE(item.typeId)
            ? 'red'
            : appcolor.darklight;
  ...
  const RenderViewConfirm = ({ item, typeConfirm, index }) => {
    ...
    const statusConfirm =
      typeConfirm == 'SHIFT'
        ? item.confirmStatus
        : typeConfirm == 'LATE'
        ? item.confirmLateStatus
        : item.confirmEarlierStatus;
    const valueChange =
      typeConfirm == 'SHIFT'
        ? 'Ca chuyển: ' + item.lastChangedName || ''
        : typeConfirm == 'LATE'
        ? item.timeLate
        : item.timeEarlier;
    const noteChange =
      typeConfirm == 'SHIFT'
        ? 'Lý do :' + item.changeNote || ''
        : typeConfirm == 'LATE'
        ? item.noteLate
        : item.noteEarlier;
    const colorConfirm =
      typeConfirm == 'SHIFT'
        ? item.colorConfirmShift
        : typeConfirm == 'LATE'
        ? item.colorConfirmLate
        : item.colorConfirmEarlier;
    const colorReject =
      typeConfirm == 'SHIFT'
        ? item.colorRejectShift
        : typeConfirm == 'LATE'
        ? item.colorRejectLate
        : item.colorRejectEarlier;
    const statusColor =
      typeConfirm == 'SHIFT'
        ? item.statusConfirmShift
        : typeConfirm == 'LATE'
        ? item.statusConfirmLate
        : item.statusConfirmEarlier;
    const isLock =
      typeConfirm == 'SHIFT'
        ? item.isLockConfirm
        : typeConfirm == 'LATE'
        ? item.isLockConfirmLate
        : item.isLockConfirmEarlier;

    const onConfirm = () => {
      isLock == 1
        ? null
        : handlerConfirm(item, typeConfirm, CONFIRM_VALUE(item.typeId), index);
    };
    const onReject = () => {
      isLock == 1
        ? null
        : handlerConfirm(item, typeConfirm, REJECT_VALUE(item.typeId), index);
    };
      ...
      <View key={`ascc_${index}`} style={styles.bordertab}>
        <View style={styles.titleConfirm}>
          <SpiralIcon
            ...
            name={icon}
            size={15}
            style={[styles.iconStyle, { color: statusColor }]}
          />
          <Text style={[styles.tabHeader, { color: statusColor }]}>
            {statusConfirm}
          </Text>
          {isLock !== 1 && (
  ...
  const renderItemShopList = (item, index, indexMain) => {
    const colorConfirmShop =
      item.confirm == CONFIRM_VALUE(item.typeId)
        ? 'green'
        : item.confirm == REJECT_VALUE(item.typeId)
        ? appcolor.danger
        : appcolor.rejection;
    return (
      <View key={`asds_${index}`} style={{ flex: 1 }}>
        {item.statusChangeShop !== undefined &&
          item.statusChangeShop.length > 0 && (
            <Text
              style={[
                styles.titleView,
                { marginStart: 8, color: colorConfirmShop, fontWeight: '500' },
              ]}
            >
              {' '}
        ...
        {item.shopId !== item.employeeId && (
          <ListItem.Content
            style={[styles.shopContainer, { marginStart: 8, marginEnd: 8 }]}
          >
            <View
              style={[
                styles.itemShopContainer,
                { paddingStart: 2, paddingBottom: 5 },
              ]}
            >
              <SpiralIcon
                type="font-awesome-6"
              ...
              <ListItem.Title
                style={[
                  styles.itemShopText,
                  { fontWeight: '700', fontSize: 15, width: '75%' },
                ]}
              >
                {item.shopName}
              </ListItem.Title>
                ...
                {item.TypeList == 0 ? (
                  <Switch
                    ...
                    trackColor={{
                      ...
                      false: appcolor.lightgray,
                    }}
                    thumbColor={appcolor.dark}
                    //Event
                    value={item.status == 1}
                    onValueChange={() => toggleSwitch(index, indexMain)}
                  />
                ) : (
                  <View style={{ flex: 1, flexDirection: 'row-reverse' }}>
                    <SpiralIcon
                      ...
                      name="window-close"
                      size={28}
                      color={item.colorRejectShop}
                      style={{ padding: 3 }}
                      onPress={() =>
                        handlerConfirmShop(
                          item,
                          REJECT_VALUE(item.typeId),
                          index,
                          indexMain,
                        )
                      }
                    ...
                    <SpiralIcon
                      ...
                      name="check-square"
                      size={28}
                      color={item.colorConfirmShop}
                      solid
                      style={{ padding: 3 }}
                      onPress={() =>
                        handlerConfirmShop(
                          item,
                          CONFIRM_VALUE(item.typeId),
                          index,
                          indexMain,
                        )
                      }
            ...
            <Text style={[styles.itemShopText, { fontSize: 13 }]}>
              {item.address}
            </Text>
            <Text
              style={[styles.itemShopText, { fontSize: 13, fontWeight: '700' }]}
            >
              {item.notes}
            </Text>
            <RenderTextInput
              item={item}
              index={index}
              handleChangeNote_SHOP={handleChangeNote_SHOP}
              typeConfirm="SHOP"
              styles={styles}
              appcolor={appcolor}
              indexMain={indexMain}
        ...
        {item.isParent && (
          ...
          >
            <SpiralIcon
              ...
              name="user"
              size={18}
              style={{ color: appcolor.white, paddingEnd: 8 }}
            />
            <ListItem.Title style={{ color: appcolor.white, fontSize: 16 }}>
            ...
            <View>
              <View style={[styles.titleContainer, { marginStart: 8 }]}>
                <SpiralIcon
                  ...
                  name="calendar-day"
                  size={18}
                  style={{ color: appcolor.dark, paddingEnd: 8 }}
                />
                <ListItem.Title style={{ color: appcolor.dark, fontSize: 16 }}>
                  {item.dayName}
                </ListItem.Title>
              </View>
              <ListItem.Content
              ...
              >
                ...
                {item.shiftName !== null && (
                  ...
                  >
                    ...
                    />
                    <ListItem.Subtitle
                      style={[styles.itemShopText, { fontSize: 13 }]}
                    >
                      {item.shiftName}
                    </ListItem.Subtitle>
                  </View>
                ...
                {item.confirmEarlierStatus !== null && (
                  <RenderViewConfirm
                    item={item}
                    index={index}
                    dataIndex={index}
              ...
              {item.shopList !== '[]' && (
                <TouchableOpacity
                  onPress={() => handleExpanded(item, index)}
            ...
            <View>
              <FlatList
                ...
                renderItem={({ item, index }) =>
                  renderItemShopList(item, index, indexMain)
                }
              />
            </View>
          )}
  ...
  const styles = StyleSheet.create({
    ...
    bordertab: {
      width: '100%',
      borderTopColor: appcolor.darkslategray,
      borderTopWidth: 1,
      flexDirection: 'column',
    },
    shopListView: {
  ...
  const searchModal = async text => {
    if (text.length > 0) {
      const filterShift = dataModal.dataFilter.filter(i =>
        i.employeeName.toLowerCase().match(text.toLowerCase()),
      );
  ...
  const chooseItemModal = async item => {
    ...
    setDataModal({
      ...
      dataSelect: dataModal.dataFilter.map(i =>
        i.employeeCode == item.employeeCode
          ? { ...i, isSelect: 1 }
          : { ...i, isSelect: 0 },
      ),
      visible: false,
  ...
  const showModalView = async () => {
    await setDataModal({
      ...dataModal,
      titleModal: 'Danh sách nhân viên',
      visible: true,
    });
  };
        ...
        >
          <CustomListView
            ...
            ListEmpty={
              ...
              >
                <Text style={{ color: appcolor.danger, fontSize: 18 }}>
