...
import { View, Text, TextInput, StyleSheet, RefreshControl, Platform, FlatList, KeyboardAvoidingView, LayoutAnimation, UIManager, TouchableOpacity, Modal, SafeAreaView, ImageBackground, } from 'react-native';
import { Image, ListItem, Divider, Icon as ICO } from '@rneui/themed';
import { GET_ListWeek, PLAN_ChangeShift, PLAN_WorkingLate, REGISTERPLAN_SavePlan, V2_REGISTERPLAN_GetCopyPlan, V2_REGISTERPLAN_GetByWeek } from '../../../Controller/PlanController';
...
const LG_WorkingPlanPC = ({ navigation, route }) => {
    ...
    const loadListCopyPlan = async (currentWeek = null) => {
        ...
        try {
            ...
            currentWeek = !currentWeek ? listWeek[0] ? listWeek.filter(e => e.value === moment().isoWeek())[0] : { value: moment().isoWeek() } : currentWeek
        ...
        try {
            switch (type) {
                case SUBMIT_SHIFT:
                    ...
                    const countDayChange = moment(item.workDate, 'YYYY-MM-DD').diff(moment(currentDate, 'YYYY-MM-DD'), 'days');
                    const minChangeDate = listConfig?.minChangeDate || 0

                    if (countDayChange <= minChangeDate && minChangeDate !== 0) {
                        alertWarning(`Ngày đổi ca phải sau ${minChangeDate} ngày so với ngày hiện tại!`, 'Thông báo', 'top')
                        return
                    }
            ...
            alertConfirm("Thông báo", `Bạn có chắc chắn muốn gửi ${type == SUBMIT_IMAGE ? 'hình ảnh' : 'đề xuất thay đổi'}?`, async () => {
                switch (type) {
                    case SUBMIT_SHIFT:
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View key={index} style={{ width: '100%', alignSelf: 'center', marginBottom: isParent ? 8 : 0 }}>
                {isParent && (
                    <View style={{ flexDirection: 'row', alignItems: 'center', padidng: 7, marginLeft: 15 }}>
                        <Icon name="calendar-check" color={appcolor.dark} size={20} />
                        <Text style={{ color: appcolor.dark, fontSize: 18, fontWeight: '700', marginLeft: 10, }}>{moment(workDate).format("dddd, DD MMMM YY")}</Text>
                    </View>
                )}
                <View style={{ backgroundColor: appcolor.light, borderRadius: 10, padding: 10, margin: 8 }}>
                    <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', }}>
                        <View style={{ width: '90%', flexDirection: 'row', alignItems: 'center', paddingTop: 8, paddingBottom: 8, paddingEnd: 8 }}>
                        ...
                        {item.isEvident == 1 &&
                            <TouchableOpacity
                                ...
                                <Icon solid name="file-image" size={20} color={appcolor.light} />
                            </TouchableOpacity>
                        }
                    </View>
                    ...
                    {item.confirmPlan == 1 &&
                        <View>
                            <View style={{ height: 0.5, width: '100%', marginBottom: 8, borderWidth: 1, borderColor: appcolor.greylight }} />
                            <View style={{ flexDirection: "row" }}>
                                <View style={[styles.itemShopContainer, { width: '49.5%', justifyContent: 'center' }]}>
                                    <RenderExpandBtn
                                        ...
                                        showState={shiftExpand} hideState={busyExpand} handleExpanded={handleExpanded} confirmShift={confirmShift} />
                                </View>
                    ...
                    {disableCopyButton && <Text style={{ textAlign: 'center', fontSize: 17, padding: 8, color: appcolor.dark }}>{totalTime}</Text>}
                    {disableCopyButton &&
                        <View style={{ width: '100%', flexDirection: 'row', alignSelf: 'center' }}>
                            <RenderViewPhotoAttendant setShowImage={setShowIMV} setUrl={setUrlIMV} imageURL={checkIn1} timeValue={item.timeIn1} appcolor={appcolor} />
                            <RenderViewPhotoAttendant setShowImage={setShowIMV} setUrl={setUrlIMV} imageURL={checkOut1} timeValue={item.timeOut1} appcolor={appcolor} />
                        </View>
                    }
                    {disableCopyButton && item.photoAttendant > 2 &&
                        <View style={{ width: '100%', flexDirection: 'row', alignSelf: 'center' }}>
                            <RenderViewPhotoAttendant setShowImage={setShowIMV} setUrl={setUrlIMV} imageURL={checkIn2} timeValue={item.timeIn2} appcolor={appcolor} />
                            <RenderViewPhotoAttendant setShowImage={setShowIMV} setUrl={setUrlIMV} imageURL={checkOut2} timeValue={item.timeOut2} appcolor={appcolor} />
                        </View>
                    }
                </View >
            </View >
    ...
    const styles = StyleSheet.create({
        ...
        styleViewSelectImage: { flexDirection: 'row', width: '95%', backgroundColor: appcolor.light, borderRadius: 10, padding: 8, alignSelf: 'center', marginTop: 8 }
    })
    ...
    return (
        <View style={{ flex: 1, backgroundColor: appcolor.homebackground }}>
            {showIMV &&
                <View style={{ width: '100%', height: '100%', marginTop: 30, borderRadius: 5 }}>
            ...
            />
            <LoadingView title="Đang cập nhật dữ liệu" isLoading={refreshing} styles={{ marginTop: 8, zIndex: 100 }} />

            {currentWeek.label !== undefined && (
                <View style={{ flexDirection: 'row', alignItems: 'center', height: 45 }}>
                    <TouchableOpacity onPress={() => handleDisplayBS(WEEK)} style={{ width: '100%', height: '100%', backgroundColor: appcolor.primary, justifyContent: 'center', alignItems: 'center' }}>
...
const RenderExpandBtn = ({ shiftStatus, lateStatus, earlierStatus, expanded, handleExpanded, showState, hideState, isShift, confirmShift, confirmLate, confirmEarlier, colorStatus, appcolor }) => {
    const handlePress = () => {
...
const RenderViewPhotoAttendant = ({ setShowImage, setUrl, imageURL, timeValue, appcolor }) => {
    const showPhoto = () => {
        setShowImage(true);
        setUrl(imageURL.indexOf('https://') === - 1 ? URLDEFAULT + imageURL : imageURL)
    }
    return (
            ...
            <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center', borderRadius: 5, backgroundColor: appcolor.light }}>
                {imageURL ?
                    <TouchableOpacity style={{ width: '90%', height: 120, borderRadius: 5 }} onPress={showPhoto}>
                        <Image source={{ uri: imageURL.indexOf('https://') === - 1 ? URLDEFAULT + imageURL : imageURL }} style={{ width: '100%', height: 120, borderRadius: 5 }} />
                    </TouchableOpacity>
                    :
