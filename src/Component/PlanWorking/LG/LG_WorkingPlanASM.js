...
const LG_WorkingPlanASM = ({ navigation, route }) => {
    ...
    const handleGoBack = () => {
        if (!isDownload)
            navigation.goBack()
        else toastInfo("Thông báo", "Dữ liệu đang được cập nhật, vui lòng chờ trong giây lát")
    }
    // Render View
    ...
    const styles = StyleSheet.create({
        ...
        overflowView: { width: '100%', height: '100%', position: 'absolute', zIndex: 1000, backgroundColor: appcolor.dark, opacity: 0.8, justifyContent: 'center' }
    })
    useEffect(() => {
        LoadDataPlan();
    }, [])
    ...
    return (
        <Modal animationType="slide" visible={dataModal.visibleModal}>
            <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.light }}>
                ...
                <FormGroup
                    containerStyle={styles.filterStyle} useClearAndroid={false}
