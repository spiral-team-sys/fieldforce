...
const LG_WorkingPlanASM = ({ navigation, route }) => {
  ...
  const handleGoBack = () => {
    if (!isDownload) navigation.goBack();
    else
      toastInfo(
        'Thông báo',
        'Dữ liệu đang được cập nhật, vui lòng chờ trong giây lát',
      );
  };
  // Render View
  ...
  return (
    <View style={styles.mainContainer}>
      <HeaderCustom
        title={route?.params?.menuitem.menuNameVN || 'Lịch làm việc ASM'}
        leftFunc={handleGoBack}
        iconRight="cloud-upload-alt"
        rightFunc={handlerSave}
      />
      <ReactNativeCalendarStrip
        key={configDate.dateChoose}
      ...
      {isDownload && (
        <View style={styles.overflowView}>
          <LoadingView
            ...
            title="Đang cập nhật danh sách cửa hàng làm việc"
    ...
    >
      ...
      {item.isSelect == 1 && (
        <TouchableOpacity
          ...
          onPress={clearItem}
        >
          <SpiralIcon name="backspace" size={18} color={appcolor.dark} />
        </TouchableOpacity>
      )}
    </View>
  );
...
export default LG_WorkingPlanASM;
