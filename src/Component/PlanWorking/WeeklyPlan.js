...
export const WeekLyPlan = ({ navigation }) => {
  ...
  const LoadData = async (year, month, weekByYear) => {
    ...
    const calendarResult = await WorkingPlanAPI.GetCalendar(
      yearValue,
      monthValue,
    );
    if (calendarResult.statusId === 200) {
      ...
      if (resultStore.statusId === 200) {
        await countTotalTime(
          calendarResult.data,
          resultStore.data,
          'ON',
          async dataReulst => {
            await setCountTotalWeek(
              dataReulst[calendarResult.data[0]?.isWeekNum].minutePlan || 0,
            );
          },
        );
  ...
  const uploadData = async () => {
    ...
    alertConfirm(
      ...
      async () => {
        const uploadResult = await WorkingPlanAPI.uploadPlanbyWeek(
          dataUploadInfo,
        );
  ...
  const summaryTime = async () => {
    ...
    const lstShopON = dataShop.filter(
      i =>
        i.shiftMode === 'ON' && i.workDate >= fromDate && i.workDate <= toDate,
    );
  ...
  const handlerChangeWeek = async e => {
    ...
    const totalTimeByWeek =
      (await _.filter(planTime, ie => ie.weekNum == i.weekNum)[0]
        ?.minutePlan) || 0;
  ...
  const onDaySelected = item => {
    setFilter({
      ...filter,
      date: item.Date === filter.date ? null : item.Date,
      weekly: item.Weekly,
    });
  ...
  const onViewDay = async () => {
    const dataDate = dataShopMain.filter(
      a => a.workDate === filter.date && a.shiftMode == SHIFT_MODE.ON,
    );
    setPlanDate(dataDate || dataShopMain);
    const imageData = await GetPhotosEvident(
      0,
      moment(filter.date).format('YYYYMMDD'),
      kpiinfo.id,
    );
  ...
  const handlerConfirmTime = async item => {
    if (item.timeIn !== null && item.timeOut !== null) {
      if (
        moment(item.timeOut).hours() == moment(item.timeIn).hours() &&
        moment(item.timeOut).minutes() == moment(item.timeIn).minutes()
      ) {
    ...
    if (item.isCheckNote == 1 && (noteShop == null || noteShop.length < 5)) {
      alertWarning(
        'Vui lòng nhập lí do thay đổi thời gian làm việc (Tối thiểu 5 kí tự)',
      );
      return;
    }
  ...
  const onSelectOFF = item => {
    ...
    _editOff.forEach(e => {
      if (e.ShiftCode === item.ShiftCode) {
        item.Selected === 0
          ? setShiftOFF({ name: item.ShiftNameVN, code: item.ShiftCode })
  ...
  const handlerConfirmOFF = async () => {
    ...
    if (note !== null && note.length < 5) {
      alertWarning(
        'Lý do nghỉ phép quá ngắn, vui lòng nhập lại (Tối thiểu 5 kí tự)',
      );
      return;
    }
    ...
    const lstShopON = dataShop.filter(
      i => i.shiftMode === 'ON' && i.workDate === filter.date,
    );
  ...
  const countTotalTime = async (
    ...
    await calendarList.map((i, _index) => {
      ...
      if (shiftMode == SHIFT_MODE.OFF) {
        let plans = shopData.filter(
          item =>
            item.workDate >= fromDate &&
            item.workDate <= toDate &&
            item.workDate !== filter.date,
        );
        ...
        let plans = shopData.filter(
          item => item.workDate >= fromDate && item.workDate <= toDate,
        );
        const totalItem = _.sumBy(plans, 'totalMinute');
        const itemTime = {
          weekNum: i.weekNum,
  ...
  const itemShop = ({ item, index }) => {
    const workingTime = `${moment(item.timeIn).format('HH:mm')} - ${moment(
      item.timeOut,
    ).format('HH:mm')}`;
    ...
    return (
      item.timeIn !== null && (
        <View key={`shopLists_${index}`} style={{ width: '100%' }}>
          <Text style={{ color: colorStatus, fontSize: 14, fontWeight: '400' }}>
            {statusName}
          </Text>
          <Text
            style={{ color: appcolor.dark, fontSize: 14, fontWeight: '300' }}
          >
            {item.shopName}
          </Text>
          <Text
            style={{
              color: item.timeIn !== null ? appcolor.dark : appcolor.danger,
              fontSize: 13,
              fontWeight: '400',
              fontStyle: 'italic',
            }}
          ...
          {item.isUpdate == 1 && workingUpdateTime !== 0 && (
            <Text
              style={{
                color: item.timeIn !== null ? appcolor.dark : appcolor.danger,
                fontSize: 13,
          ...
          {item.isUpdate == 1 && item.note !== null && (
            <Text
              style={{
  ...
  const rowItemDay = ({ item, index }) => {
    ...
    const itemChooseStyle =
      item.Date === filter.date
        ? { borderRightWidth: 10, borderRightColor: appcolor.warning }
        : {};
    const planList = (
      item.planList !== undefined
        ? JSON.parse(item.planList || '[]')
        : dataShop.filter(i => i.workDate === item.Date)
    ).filter(i => i.shiftMode == SHIFT_MODE.ON);
    const OFFPlan = (
      item.planList !== undefined
        ? JSON.parse(item.planList || '[]')
        : dataShop.filter(i => i.workDate === item.Date)
    ).filter(i => i.shiftMode == SHIFT_MODE.OFF);
    ...
    const workingTime = `${moment(itemOffPlan.timeIn).format(
      'HH:mm',
    )} - ${moment(itemOffPlan.timeOut).format('HH:mm')}`;
    return (
      <View key={`itemday_${index}`} style={styles.dateView}>
        ...
        <TouchableOpacity onPress={onPress} style={{ width: '82%' }}>
          ...
          >
            <Text
              style={{
                color: appcolor.greenBlueLight,
                fontSize: 14,
                fontWeight: '600',
              }}
            >{`${titleItem} (${(totalMinuteItem / 60).toFixed(1)}h)`}</Text>
            {shiftMode === SHIFT_MODE.ON && (
              <FlatList
                key={'planListItem'}
            ...
            {OFFPlan !== null && OFFPlan.length !== 0 && (
              <View>
                ...
                {itemOffPlan?.shiftChange !== undefined &&
                  itemOffPlan?.shiftChange !== null && (
                    <Text
                      style={{
                        color: appcolor.danger,
                        fontSize: 12,
  ...
  const rowItemShop = ({ item, index }) => {
    ...
    const showExpanderTime = () => {
      if (item.isLockDay == 0) {
        item.showTime =
          item.showTime === 0 || item.showTime == undefined ? 1 : 0;
        let edit = {
          ...item,
          timeIn: item.timeIn || DATE,
          timeOut: item.timeOut || DATE,
        };
    ...
    const onDateChangeTimeIn = date => {
      ...
      const edit = {
        ...
        timeIn: dateValue,
        timeOut: timeOut,
        totalMinute: moment.duration(difference).asMinutes(),
      };
      const _list = [...planDate];
      _list[index] = edit;
      setPlanDate(_list);
    ...
    const titleTime = `${moment(timeIn).format('HH:mm')} - ${moment(
      timeOut,
    ).format('HH:mm')}`;
    return (
      <View
        key={`itemshops_${index}`}
        style={{
          width: '100%',
      ...
      >
        ...
        <Text style={{ ...styles.titleView, fontSize: 13, fontWeight: '300' }}>
          {item.address}
        </Text>
        <View
          style={{
        ...
        <TouchableOpacity onPress={showExpanderTime}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Text style={{ color: appcolor.dark, flexGrow: 1, padding: 7 }}>
              Thời gian
            </Text>
            <Text
              style={{
        ...
        {item.showTime === 1 && (
          <View style={{ width: '100%' }}>
            ...
            />
            <TouchableOpacity
              onPress={item.btnConfirm == 1 ? null : onPressConfirm}
            >
              <Text
                style={{
                  fontSize: 14,
    ...
    <View style={styles.mainContainer}>
      ...
      {!loading && (
        <View style={styles.contentView}>
          <TouchableOpacity
            onPress={() => SheetManager.show('sheetYear')}
            style={{
              width: '100%',
              flexDirection: 'row',
              alignItems: 'center',
          ...
          >
            <Text
              style={{ color: appcolor.primary, marginEnd: 7, fontSize: 14 }}
            >
                  ...
                  <View key={`weekData_${idx}`} style={{ width: '100%' }}>
                    ...
                    <Text
                      style={{
                        ...
                        fontSize: 18,
                        fontWeight: '700',
                        color: colorTotalTime,
                        padding: 8,
                      }}
                    >{`Tổng thời gian: ${countTotaByWeek || 0}h`}</Text>
                    <FlatList
                      key={'dayinwekkly'}
                      keyExtractor={(_, index) => index.toString()}
                      data={weekData}
          ...
          <TouchableOpacity
            onPress={onViewDay}
            style={{
              ...styles.actionItemView,
              display: filter.date !== null ? 'flex' : 'none',
            }}
          >
            <SpiralIcon name="add" size={23} color={appcolor.dark} />
          </TouchableOpacity>
        </View>
      )}
      <ActionSheet
      ...
      >
        ...
        >
            ...
            >
              <Text
                style={{
                  color: appcolor.dark,
                  textAlign: 'center',
                  fontWeight: tabActive === 0 ? '900' : 'normal',
                }}
              >
                Cửa hàng
              </Text>
            ...
            >
              <Text
                style={{
                  color: appcolor.red,
                  textAlign: 'center',
                  fontWeight: tabActive === 1 ? '900' : 'normal',
                }}
              >
                Nghỉ
              </Text>
            ...
            >
              <Text
                style={{
                  color: appcolor.info,
                  textAlign: 'center',
                  fontWeight: tabActive === 2 ? '900' : 'normal',
                }}
              >
                Hình ảnh
              </Text>
          ...
          {tabActive === 0 && (
            <PlanON
              key={`ONPLan`}
              props={{ planDate, noteShop, setNoteShop, rowItemShop }}
            />
          )}
          {tabActive === 1 && (
            <PlanOFF
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => {
              return (
                <View
                  key={`pat_${index}`}
                  style={{ flex: 1, alignSelf: 'center', marginRight: 7 }}
                >
                  <Text style={styles.titleAttendant}>
                    {item.isParent ? item.ShopName : ''}
                  </Text>
                  <Text style={styles.titleAttendant}>
                    {item.isParent ? item.ShiftType : ''}
                  </Text>
                  <Text style={styles.titleAttendant}>
                    {item.AttendantType}{' '}
                    {`(${moment(item.AttendentTime).format('HH:mm')})`}
                  </Text>
                  <Text
                    style={{
                      width: '100%',
...
const PlanOFF = ({ props }) => {
  ...
  const { dataShiftList, onSelectOFF, note, setNote, handlerConfirmOFF } =
    props;
  ...
  return (
    <View style={{ backgroundColor: appcolor.light, marginTop: 8 }}>
        ...
        {dataShiftList.length > 0 &&
          dataShiftList?.map((item, index) => {
            return (
              <TouchableOpacity
                key={`iip_off_${index}`}
                onPress={() => onSelectOFF(item)}
              >
                <View
                  style={{
                    padding: 12,
                    backgroundColor:
                      item.Selected === 1 ? appcolor.red : appcolor.light,
                    flexDirection: 'row',
                    alignItems: 'center',
                  }}
                >
                  <Badge
                    ...
                    containerStyle={{ minWidth: 50, marginRight: 10 }}
                  />
                  <View style={{ flexGrow: 1 }} key={`offitem_${index}`}>
                    <Text
                      style={{ fontSize: scaleSize(13), color: appcolor.dark }}
                    >
                      {item.ShiftNameVN}
                    </Text>
                    <Text
                      style={{ fontSize: scaleSize(11), color: appcolor.dark }}
                    >
                      {item.ShiftName}
                    </Text>
                  </View>
      ...
      <TouchableOpacity
        style={{ width: '100%', marginBottom: 16, paddingTop: 8, bottom: 8 }}
        onPress={handlerConfirmOFF}
      >
        <Text
          style={{
            fontSize: 14,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: appcolor.light, marginTop: 8 }}>
        ...
        <View style={{ width: '100%' }}>
          ...
          >
            <SpiralIcon
              ...
              color={appcolor.dark}
            />
            <Text style={{ fontSize: 15, marginLeft: 5, color: appcolor.dark }}>
              Chụp hình
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
      ...
      <View style={{ height: '10%', alignItems: 'center' }}>
        <TouchableOpacity
          onPress={handlerUploadPhotoEvident}
          style={{
            height: 38,
