...
import { View, ScrollView, Text, TextInput, Platform, KeyboardAvoidingView, Keyboard, StatusBar, } from "react-native";
import { Icon, Badge, Button } from '@rneui/themed';
import { Store, InsertItems, UpdateItem, QueryStringSql } from "../../Core/SqliteDbContext";
...
const MarketCreate = ({ navigation }) => {
    ...
    const onLoad = async () => {
        ...
        // Options
        const { res } = await QueryStringSql("SELECT Distinct ref_Id as id,ref_Name as name FROM masterList WHERE listCode='MARKETISSUE' ")
        await setOption(res || [])
    }
    const Save = async () => {
    ...
    return (
        <View style={{ flex: 1, }}>
            <HeaderCustom rightFunc={() => navigation.goBack()} iconRight="close" rightType={"font-awaresome-5"}
                title={"Báo cáo thị trường"} iconLeft={"save"} leftFunc={Save} />
            <View style={{ flex: 12, flexDirection: 'column', backgroundColor: appcolor.surface }}>

                <ScrollView keyboardShouldPersistTaps='always'
                    style={{ height: deviceHeight * .92, padding: 7 }}>
                    <Text style={{ color: appcolor.dark, padding: 7, marginLeft: 12, fontSize: 12, fontWeight: '700' }}>Ngành hàng</Text>
                    ...
                    <Text style={{ color: appcolor.dark, padding: 7, marginLeft: 12, fontSize: 12, fontWeight: '700' }}>Hạng mục</Text>
                    <ListView appcolor={appcolor} list={Options} item={itemSelect?.cate}
                        onSelected={(item) => {
                        ...
                        }} />
                    {
                        dataIssue?.length > 0 &&
                        <View>
                            <Text style={{ color: appcolor.dark, padding: 7, marginLeft: 12, fontSize: 12, fontWeight: '700' }}>Vấn đề</Text>
                            <ListView appcolor={appcolor} list={dataIssue} item={itemSelect?.issue}
                                onSelected={(item) => {
                    ...
                    }
                    {
                        showNote &&
                        <View style={{}}>
                            <Text style={{ color: appcolor.dark, padding: 7, marginLeft: 12, fontSize: 12, fontWeight: '700' }}>Ghi chú vấn đề</Text>
                            <TextInput
                                multiline
                                ...
                                value={Content}
                                style={{
                                    backgroundColor: appcolor.light, fontSize: 10, borderWidth: 0.3,
                                    maxHeight: 150, minHeight: 55
                                }}
                                placeholder='nhập ghi chú'
                            />
                        </View>
                    }
                    <Text style={{ color: appcolor.dark, padding: 7, marginLeft: 12, fontSize: 12, fontWeight: '700' }}>Mật độ khách hàng </Text>
                    <ListView appcolor={appcolor} list={Traffics} item={itemSelect?.traffic}
                        onSelected={(item) => {
                            setItemSelect({ ...itemSelect, traffic: item })
                        }} />

                    <Text style={{ color: appcolor.dark, padding: 7, marginLeft: 12, fontSize: 12, fontWeight: '700' }}>Trạng thái </Text>
                    <ListView appcolor={appcolor} list={dataStatus} item={itemSelect?.status}
                        onSelected={(item) => {
                            setItemSelect({ ...itemSelect, status: item })
                        }} />
                    <View>
                        <Text style={{ color: appcolor.dark, padding: 7, marginLeft: 12, fontSize: 12, fontWeight: '700' }}>Ghi chú mật độ khách hàng</Text>
                        <TextInput
                            multiline
                            onChangeText={text => {
                                setNoteTraffic(text);
                            ...
                            value={NoteTraffic}
                            style={{
                                borderWidth: 0.3, color: appcolor.dark, fontSize: 10, backgroundColor: appcolor.light,
                                borderColor: 'lightgray', maxHeight: 150, minHeight: 55
                            }}
                            placeholder='nhập ghi chú'
                        />
                    </View>
    ...
    return (<View>
        {
            list.map((l, i) => (
                <ListItem containerStyle={{
                    backgroundColor: select?.id === l.id ?
                        appcolor.warning : appcolor.light, padding: 7
                }}
                    onPress={() => onSelectItem(l)} key={i} bottomDivider>
                    <Badge value={i + 1} />
                    <ListItem.Content>
