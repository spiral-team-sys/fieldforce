...
import { TouchableOpacity, View, Text, AppState, VirtualizedList, ScrollView } from "react-native"
...
export const QuickViewHPI = ({ navigation, refreshing }) => {
            ...
            if (index == 0)
                ...
                listCate.push(
                    <View key={`${index}1lak`} style={{
                        padding: 7, justifyContent: 'flex-end',
                    ...
                    }}><TouchableOpacity style={{ flexDirection: 'row', }} onPress={() => showDetail(item.category)}>
                            <Text style={{ textAlign: 'right', fontSize: 10, color: ColorRand(index) }}>{item.category}</Text>
                            <Text style={{ fontSize: 20, color: ColorRand(index) }}>{item.quantity || 0}</Text>
                            {
                                item?.amount &&
    ...
    const rowItem = ({ item, index }) => {
        return (<View style={{ padding: 7, }}>
            ...
            {
                item?.moreinfo &&
                <Text style={{ fontSize: 10, fontWeight: '300', color: appcolor.dark }}>{item?.moreinfo || ''}</Text>
            }
            <View style={{ borderWidth: 1, borderColor: appcolor.surface, width: '100%', marginTop: 7 }} />
        </View>)
    }
    ...
    return (
        <View style={{
            ...
            <View style={{
                ...
                width: deviceHeight * 0.2, height: deviceHeight * 0.26,
                backgroundColor: appcolor.light, position: 'absolute', zIndex: 100,
                margin: 20, borderRadius: 500,
                alignContent: 'flex-end'
            }}>
                <TouchableOpacity onPress={() => showDetail()} style={{
                    width: '100%', height: '100%', padding: 20,
                    justifyContent: 'flex-end'
                }}>
                    <Text style={{ textAlign: 'center', fontSize: 48, color: appcolor.primary }}>
                        {data?.length > 0 ? data[0].quantity : 0}
                    </Text>
                    <Text style={{ textAlign: 'center', color: appcolor.primary, fontSize: 10 }}>
                        {data?.length > 0 ? data[0]?.unit : "Số lượng"}
                    </Text>
                </TouchableOpacity>
            </View>
            <Text style={{
                position: 'absolute', top: 10, zIndex: 122,
                color: appcolor.dark, fontSize: 11, marginLeft: 10
            }}>
                {data?.length > 0 ? data[0].title : ''}</Text>
            <TouchableOpacity
                style={{
                    position: 'absolute', top: 0, right: 10, height: 30, width: 30, zIndex: 10000,
                    justifyContent: 'center', alignItems: 'center', backgroundColor: appcolor.surface, borderRadius: 50
                }}
                onPress={() => reloadDashboard()}>
                <IconAnimation isLoop={isRefresh} sourceIcon={require('../../Themes/lotties/refresh.json')} />
            </TouchableOpacity>

            <View style={{ flexGrow: 1, backgroundColor: appcolor.surface, marginTop: 40 }}>
                {UIByCate()}
            </View>
            <ActionSheet id="details">
                ...
                <TouchableOpacity onPress={() => SheetManager.hide("details")}
                    style={{ position: 'absolute', bottom: 10, alignItems: 'center', alignSelf: 'center' }}>
                    <Icon raised name="close" size={18} color={appcolor.danger} />
                </TouchableOpacity>
            </ActionSheet>
        </View >
    )
