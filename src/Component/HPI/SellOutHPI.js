...
const SellOutHPI = ({ navigation }) => {
    ...
    const SOItem = ({ item, index }) => {
        // console.log(item)
        return (
            <GmailStyleSwipeableRow key={`kj${index}12`} enableRight={false} deleteItem={() => deleteItemSellOut(item)}>
                <TouchableOpacity
                    onPress={() => handleShowItem(item)}
                    key={`${index}as`} style={{ flex: 1, backgroundColor: appcolor.success, margin: 5, borderRadius: 8, flexDirection: "row", alignItems: 'center', justifyContent: 'space-between' }}>
                    <View style={{ width: '90%', borderRadius: 8, padding: 5, backgroundColor: appcolor.surface, marginBottom: 2, }}>
                        <View style={{ padding: 1 }}>
                            {item.contactName && <Text style={{ color: appcolor.dark, fontWeight: '500', fontSize: 12, padding: 1 }}>Khách hàng : {item.contactName || ''}</Text>}
                            {item.phone && <Text style={{ color: appcolor.dark, fontWeight: '500', fontSize: 12, padding: 1 }}>Số điện thoại : {item.phone | ''}</Text>}
                        </View>
                        <View style={{ flexDirection: 'row', alignItems: 'center', padding: 1 }}>
                            {item.productCode && <Text style={{ color: appcolor.dark, fontWeight: '500', fontSize: 12 }}>Mã :
                                {item.productCode}{`${item.category && ` - Ngành hàng : ${item.category}`}`}</Text>}
                        </View>
                        {item?.segment && <Text style={{ color: appcolor.dark, fontWeight: '500', fontSize: 12, padding: 1 }}>{`Loại ${item?.segment}`}</Text>}
                        {item?.subSegment && <Text style={{ color: appcolor.dark, fontWeight: '500', fontSize: 12, padding: 1 }}>{`Model ${item?.subSegment}`}</Text>}
                        {item.imeI1 !== null && item.imeI1 !== "" && <Text style={{ color: appcolor.dark, fontWeight: '500', fontSize: 12, padding: 1 }}>{`IMEI : ${item?.imeI1}`}</Text>}
                        {item.price !== null && item.quantity !== undefined && item.price !== "" && <Text style={{ color: appcolor.dark, fontWeight: '500', fontSize: 12, padding: 1 }}>{`Giá : ${item?.price} VNĐ`}</Text>}
                        {item?.note && <Text style={{ color: appcolor.dark, fontWeight: '500', fontSize: 12, padding: 1 }}>{`Ghi chú ${item?.note}`}</Text>}
                    </View>
                    {
                        item.quantity !== undefined && item.quantity !== null &&
                        <View style={{ width: '10%', height: '100%', justifyContent: 'center', alignItems: 'center' }}>
                            <Text style={{ color: appcolor.white, fontWeight: '500', fontSize: 14 }}>{item.quantity || 0}</Text>
                        </View>
                    }
                </TouchableOpacity>
    ...
    return (
        ...
        <View style={{ height: '100%', width: '100%', backgroundColor: appcolor.light }}>
            <HeaderCustom
                ...
                title={`Báo cáo số bán ${moment(sodate?.toString() || new Date()).format("yyyy/MM/DD")}`}
                iconRight={"filter"}
                rightFunc={onSearch}
            />
            ...
            <Icon
                ...
                onPress={() => navigation.navigate("sellouthpinput", { onLoad: onLoad })}
                size={45}
                name='add-circle-outline'
                containerStyle={{ position: 'absolute', bottom: 10, zIndex: 100, right: 20 }}
                type='ionicon'
            />
            {!showProgress &&
                <FlatList
            ...
            >
                <View style={{ paddingVertical: 10 }}>
                    <Calendar
                        firstDay={1}
                        current={moment(sodate?.toString() || new Date()).format("yyyy-MM-DD")}
                    ...
                    />
                    <View style={{ paddingBottom: 30, justifyContent: 'center', alignItems: 'center', paddingTop: 10 }}>
                        <TouchableOpacity
                            onPress={() => filterByDate()}
                            style={{ justifyContent: 'center', alignItems: 'center', padding: 10, backgroundColor: appcolor.primary, borderRadius: 50, width: '50%' }}>
                            <Text style={{ fontSize: 14, color: appcolor.white, fontWeight: '500' }}>Tìm kiếm</Text>
                        </TouchableOpacity>
                    </View>
                </View>
