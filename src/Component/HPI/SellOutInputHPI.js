...
export const SellOutInputHPI = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (
      (ItemSaved !== null && filterData[0]?.length > 0) ||
      infoSave.isSaveInfo == true
    ) {
      loadItemSave();
  ...
  loadData = async () => {
    ...
    await setDataSheet({
      currentdata: category?.filter(a => a?.id !== 0),
      currentDataF: category?.filter(a => a?.id !== 0),
    });
    // segment
    ...
    if (
      route.params?.ItemSaved !== undefined &&
      Object.keys(route.params?.ItemSaved).length
    ) {
      setItemSaved(route.params?.ItemSaved || null);
    } else if (
      settings.isSaveInfo === true &&
      settings.shopId == workinfo.shopId
    ) {
  ...
  const loadItemSave = async () => {
    const itemCate = filterData[0]?.find(
      it => it.name == ItemSaved.category || it.name == ItemSaved.subCategory,
    );
    const itemSegment = filterData[1]?.find(it => it.name == ItemSaved.segment);
    const itemSubSegment = filterData[2]?.find(
      it => it.name == ItemSaved.subSegment,
    );
    const itemProduct = filterData[3]?.find(it => it.id == ItemSaved.productId);
    let _listtemp = masterList
      .filter(a => a.ref_Id == itemCate?.id)
  ...
  const PostData = async () => {
    if (competitorId === _competitorId) {
      ...
      if (
        ...
        (ItemSaved == null ||
          ItemSaved == undefined ||
          ItemSaved.imeI1 !== IMEI ||
          ItemSaved.productId !== filter.product.productId)
      ) {
        if (IMEI.length < 10) {
      ...
      if (custlist?.custlist?.length > 0) {
        const countGroup = new Set(custlist.custlist?.map(it => it.groupId))
          .size;
        const countSelect = custlist.custlist?.filter(
          it => it.selectValue,
        ).length;
    ...
    const sellOutdetail = {
      ...
      Quantity:
        ItemSaved !== null && ItemSaved !== undefined ? ItemSaved.quantity : 1,
    ...
    const checklist = custlist.custlist.filter(a => {
      if (a.selectValue == true) return { id: a.id, name: a.name };
    });
    const sopost = {
      ShopId: workinfo.shopId,
  ...
  const onSelected = async item => {
    ...
    switch (key) {
      case 'cate':
        _listtemp = _.filter(masterList, { ref_Id: item.id })?.sort(
          (a, b) => a.groupId - b.groupId,
        );
        //Load currentData by cate Id
        currentdata =
          _.filter(filterData[1], { categoryId: item.id }) || filterData[1];
      ...
      case 'segment':
        if (filter?.cate == undefined) {
          ...
          itemFilter = { cate: filter?.cate };
        }
        _listtemp = _.filter(masterList, { ref_Id: item.categoryId })?.sort(
          (a, b) => a.groupId - b.groupId,
        );
        //Load currentData by segment Id
        currentdata =
          _.filter(filterData[2], { segmentId: item.id }) || filterData[2];
        dataSheet.currentdata = currentdata;
        dataSheet.currentDataF = currentdata;
      ...
      case 'subsegment':
        ...
        if (filter?.segment == undefined) {
          ...
          itemFilter = { ...itemFilter, segment: filter?.segment };
        }
        ...
        currentdata =
          _.filter(filterData[3], { subSegment: item.name }) || filterData[3];
        dataSheet.currentdata = currentdata;
        dataSheet.currentDataF = currentdata;
        _listtemp.length > 0 && (custlist.custlist = _listtemp);
      ...
      case 'product':
        ...
        if (filter?.subsegment == undefined) {
          ...
          itemFilter = { ...itemFilter, subsegment: filter?.subsegment };
        }
        _listtemp = _.filter(masterList, { ref_Id: itemFilter.cate.id })?.sort(
          (a, b) => a.groupId - b.groupId,
        );
        _listtemp.length > 0 && (custlist.custlist = _listtemp);
    ...
    >
      ...
      {!showProgress && (
        <KeyboardAvoidingView
          ...
          enabled
          keyboardVerticalOffset={Platform.OS !== 'ios' ? 1 : 50}
        >
          <ScrollView
            style={{ padding: 3, marginBottom: 20 }}
            keyboardShouldPersistTaps="always"
          >
            <View key={`TickSaveInfo`} style={{ alignItems: 'flex-end' }}>
              <CheckBox
                checked={infoSave || false}
            ...
            <DropDownView
              ...
              title={'Loại'}
              onPress={() => showSheetByIndex(1, 'segment')}
            />
            <DropDownView
              key="subsegment"
              title={'Model'}
            ...
            {competitorId === _competitorId && (
              <View key={'sellOut_Price'} style={{ ...styles.containTextBox }}>
                ...
                <TextInput
                  ...
                  value={
                    Price !== undefined && Price !== 'undefined' ? Price : ''
                  }
            ...
            {custlist?.custlist?.length > 0 && (
              <View
                key={'ViewSelloutCustList'}
                style={{ ...styles.containTextBox }}
              >
                <CustomerCheckList
                  key={'selloutCustList'}
                  data={custlist.custlist}
                  callback={onCheckList}
                  reloadView={reload}
                />
              </View>
            )}
            ...
            {competitorId === _competitorId && (
              ...
              >
                ...
                <TextInput
                  ...
                  value={
                    CustAddress !== undefined && CustAddress !== 'undefined'
                      ? CustAddress
                      : ''
                  }
                  placeholder="Địa chỉ"
            ...
            {competitorId === _competitorId && (
              <View key={'sellOut_Phone'} style={{ ...styles.containTextBox }}>
                ...
                <TextInput
                  ...
                  value={
                    CustPhone !== undefined && CustPhone !== 'undefined'
                      ? CustPhone
                      : ''
                  }
...
const ViewItemSheet = ({ onSelected, keySheet, filter, dataSheet }) => {
  ...
  const filterProduct = async text => {
    ...
    if (text !== null && text.length > 0) {
      ...
      if (newDataShow?.length > 0) dataSheet.currentdata = newDataShow;
