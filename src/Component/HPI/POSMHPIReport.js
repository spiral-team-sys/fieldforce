...
export const POSMHPIReport = ({ navigation }) => {
	...
	const LoadData = async () => {
		...
		const dataFilter = {
			shopId: shopinfo.shopId, reportId: kpiinfo.id, typeReport: 'POSM'
		}
	...
	const checkedNoAll = (item) => {
		if (!isUploaded) {
			alertConfirm('Thông báo', 'Bạn muốn chọn "Không" tất cả POSM ?', async () => {
				const dataUpdate = dataPOSM.map(e => (e.CategoryName == item.CategoryName ? { ...e, itemValue: 0 } : e))
				const itemDataMain = dataMain.filter(e => { return e.DisplayId == filter.displayId })
	...
	const styles = StyleSheet.create({
		...
		mainItem: { flex: 1, borderBottomWidth: 0.3, borderColor: appcolor.greylight, paddingTop: 8, paddingBottom: 8 },
		titleHeader: { width: '100%', fontSize: 16, fontWeight: '700', color: appcolor.info, fontStyle: 'italic', padding: 8 },
		titleItem: { width: deviceWidth / 2, fontSize: 14, fontWeight: '500', color: appcolor.dark, padding: 8 },
...
const HeaderItemView = ({ itemHeader, index, handlerCamera, handlerYesAll }) => {
	...
	return (
		<View style={{ width: '100%', flexDirection: 'row', borderRadius: 5, justifyContent: 'center', marginBottom: 8, marginTop: 8 }}>
			...
			<RenderButton iconName='tasks' titleName='Tất cả POSM' onAction={handlerYesAll} />
		</View>
	)
} 