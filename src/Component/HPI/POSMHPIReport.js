...
export const POSMHPIReport = ({ navigation }) => {
  ...
  const LoadData = async () => {
    ...
    await REPORT.GetDataReportByShop(dataFilter, async (mData, mesager) => {
      ...
      if (mData !== null && mData.length > 0) await setupDataView(mData);
    });
  ...
  const setupDataView = async (data, displayId) => {
    ...
    //
    const dataPOSM = data.filter((item, _index) => {
      return item.DisplayId == _id;
    });
    ...
    for (let index = 0; index < dataMain.length; index++) {
      ...
      for (let index = 0; index < dataGroupCheck.length; index++) {
        ...
        if (dataItem.length > 0) {
          const lstPhoto = _.filter(dataPhoto, e => {
            return e.photoType == `${itemMain.DisplayId}_${i.RefId}`;
          });
          const photoConfig = i.ImageList[0] || {};
          if (photoConfig?.numberIMG || 0 > 0) {
            if (lstPhoto.length < (photoConfig?.numberIMG || 0)) {
              ToastError(
                `Vui lòng chụp đủ hình ảnh ${itemMain.DisplayName} - ${i.CategoryName}/${i.RefName} (Tối thiểu ${photoConfig?.numberIMG} tấm)`,
                'Thông báo',
                'top',
              );
              return false;
  ...
  const cameraAction = item => {
    ...
    navigation.navigate('photogroup', {
      Status: isUploaded ? 1 : 0,
      hideIcon: true,
      dataImageList: ImageByList,
    });
  };
  const onCheckedChange = async (item, value) => {
    const _valueChange = value == item.itemValue ? null : value;
    item.itemValue = _valueChange;
    setMutate(e => !e);
    await saveJsonData(shopinfo.shopId, kpiinfo.id, TODAY, dataMain);
  };
  const checkedNoAll = item => {
    if (!isUploaded) {
      alertConfirm(
        'Thông báo',
        'Bạn muốn chọn "Không" tất cả POSM ?',
        async () => {
          const dataUpdate = dataPOSM.map(e =>
            e.CategoryName == item.CategoryName ? { ...e, itemValue: 0 } : e,
          );
          const itemDataMain = dataMain.filter(e => {
            return e.DisplayId == filter.displayId;
          });
          itemDataMain[0].DataDetail = dataUpdate;
          await setDataPOSM(dataUpdate);
          await setDataFilter(dataUpdate);
          await saveJsonData(shopinfo.shopId, kpiinfo.id, TODAY, dataMain);
        },
        () => {},
        'Xác nhận',
  ...
  const searchData = text => {
    const searchList = dataFilter.filter(i => {
      return i.ItemName.toLowerCase().match(text.toLowerCase());
    });
    setDataPOSM(searchList);
  };
  // View
  const renderItem = ({ item, index }) => {
    const handlerCheckedYes = () => {
      onCheckedChange(item, 1);
    };
    const handlerCheckedNo = () => {
      onCheckedChange(item, 0);
    };
    return (
      <View key={`eeq${index}`} style={styles.mainItem}>
        {item.isParent && (
          <Text style={styles.titleHeader}>{`${item.RefName}`}</Text>
        )}
        <View
        ...
        >
          ...
          >
            <CheckBox
              ...
              checked={item.itemValue == 1}
              disabled={isUploaded}
              onPress={handlerCheckedYes}
            />
            <CheckBox
              title="Không"
              containerStyle={styles.checkboxContainer}
              textStyle={{ fontWeight: '500', color: appcolor.dark }}
              checked={item.itemValue == 0}
              disabled={isUploaded}
              onPress={handlerCheckedNo}
            />
          </View>
        </View>
      </View>
                ...
                >
                  <Text
                    style={{
                      fontSize: 15,
                      fontWeight: '700',
                      color: isSelected ? appcolor.red : appcolor.greydark,
                      marginStart: 8,
                      marginEnd: 8,
                    }}
                  >
      ...
      <View style={styles.contentView}>
        {dataGroup.length > 0 && (
          ...
          >
            {dataGroup.length > 0 &&
              dataGroup.map((it, i) => {
                let dataItem = dataPOSM.filter(e => {
                  return (
                    e.DisplayId == it.DisplayId &&
                    e.CategoryName == it.CategoryName
                  );
                });
  ...
  return (
    ...
    >
      <RenderButton
        iconName="camera"
        titleName="Chụp hình"
        onAction={handlerCamera}
      />
      <RenderButton
        iconName="tasks"
        titleName="Tất cả POSM"
        onAction={handlerYesAll}
      />
    </View>
  );
};
