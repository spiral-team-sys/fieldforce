...
const SwipeableRow = ({ item, index, appcolor }) => {
    return (
        <View key={`kja${index}20`} style={{ flex: 1, backgroundColor: appcolor.light, marginBottom: 7 }}>
            <TouchableOpacity style={{ flexDirection: 'column', justifyContent: 'space-between' }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 7 }}>
                    <Badge
                        status={item.upload == 0 ? 'error' : 'success'}
                        containerStyle={{ height: 30, width: 40, position: 'absolute', top: 10, right: 10 }}
                        value={item.upload == 0 ? 'NO' : 'OK'} />
                    <View style={{ padding: 7, flexGrow: 0.2 }}>
                        <Text style={{ fontWeight: '700', color: appcolor.dark, fontSize: 12, marginEnd: 7 }}>{'Ngành hàng: ' + item.categoryName}</Text>
                        <Text style={{ color: appcolor.dark, fontSize: 10, marginEnd: 7 }}>{'Hạng mục: ' + item.optionName}</Text>
                        <Text style={{ color: appcolor.dark, fontSize: 10, marginEnd: 7 }} numberOfLines={2}>
                            {'Vấn đề:' + item.surveyDisplayName}
                        </Text>
                        <Text style={{ color: appcolor.dark, fontSize: 10, marginEnd: 7 }} numberOfLines={4}>{'Ghi chú: ' + item.content}</Text>
                    </View>
                </View>
...
class Market extends PureComponent {
    ...
    async Upload() {
        ...
        const photoList = await getAllPhotosUploaded(kpiinfo.id, workinfo.shopId, workinfo.workDate);
    ...
    render() {
        ...
        return (
            <SafeAreaView style={{ height: '100%', width: '100%', backgroundColor: appcolor.surface }}>
                <HeaderCustom leftFunc={() => this.props.navigation.goBack()}
                ...
                <View style={{ flex: 1, margin: 7, }}>
                    <FlatList
                        ...
                        ItemSeparatorComponent={() => <View style={{ borderColor: appcolor.surface, borderWidth: 1, width: '100%' }} />}
                ...
                <SpeedDial
                    ...
                    containerStyle={{ color: appcolor.dark, backgroundColor: appcolor.dark }}
