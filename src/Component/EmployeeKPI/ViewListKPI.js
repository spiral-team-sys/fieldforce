...
export const ViewListKPI = ({ dataItem, dataGroup, dataKPI, configData }) => {
    ...
    const RenderItemKPI = ({ item, index, currentTab }) => {
        ...
        return (
            <TouchableOpacity key={`iiemwd_${index}`} onPress={item.isLock == 1 ? null : onPressItem}>
                <View style={{ width: '100%', flexDirection: 'row', justifyContent: "space-between", alignItems: 'center', borderBottomWidth: 0.5, borderBottomColor: appcolor.grayLight }}>
                    <Icon
                    ...
                    />
                    <Text style={{ fontSize: 14, fontWeight: '500', color: appcolor.dark, padding: 8, width: '75%' }}>{`${item.KPIName}`}</Text>
                    <View style={{ alignItems: 'center', padding: 4, width: '16%' }}>
                        <Text style={{ color: appcolor.dark, fontSize: 14, fontWeight: '500' }}>{`${item.mPoint}`}</Text>
                    </View>
                </View>
            </TouchableOpacity>
        )
    ...
    return (
        <View style={{ flex: 1 }}>
            < View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 12, padding: 8 }
            }>
                <Text style={{ fontWeight: '500', fontSize: 18, color: appcolor.success }}>Tổng điểm</Text>
                <Text style={{ fontWeight: '500', fontSize: 18, color: appcolor.success }}>{countTotal || 0}</Text>
            </View >
            <ScrollView showsVerticalScrollIndicator={false}>
                {
                    (dataGroup.map((it, idx) => {
                        ...
                        return (
                            <View key={'ViewGroup_' + idx} style={{ width: '100%', padding: 4 }}>

                                {
                                    dataGroup?.length > 1 && <TouchableOpacity onPress={() => onPressShow(it)}
                                        style={{
                                        ...
                                        <View style={{ justifyContent: 'center', width: '80%' }}>
                                            <Text style={{ fontWeight: '500', fontSize: 15, color: appcolor.dark }}>{it.SubGroupName}</Text>
                                            <Text style={{ fontWeight: '500', fontSize: 10, color: appcolor.placeholderText }}>{it.Decription}</Text>
                                        </View>
                                        <View style={{ padding: 10, width: '20%', flexDirection: 'row', justifyContent: 'space-between' }}>
                                            {
                                                configData.byShop == 0 &&
                                                <Text style={{ fontWeight: '500', fontSize: 16, color: appcolor.dark }}>({totalByGroup || 0})</Text>
                                            }
                                            <Icon name={it.isShowDetail ? 'chevron-down' : 'chevron-right'} type='font-awesome-5' size={20} color={appcolor.dark} />
                                        </View>
                                    </TouchableOpacity>
                                }
                                <View style={{ width: '100%', display: (it.isShowDetail || dataGroup?.length == 1) ? 'flex' : 'none', zIndex: 10 }}>
                                    {
