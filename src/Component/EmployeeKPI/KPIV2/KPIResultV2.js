...
export const KPIResultV2 = ({ navigation }) => {
  ...
  const LoadData = async (fromDate, toDate) => {
    ...
    await DataKPIResult(
      ...
      async info => {
        if (info.data !== null && info.data.length > 0) setDataKPI(info.data);
        else {
          setDataKPI([]);
  ...
  const renderItem = ({ item, index }) => {
    ...
    return (
      <View key={`iiemwd_${index}`}>
        {item.isParent && (
          <Text style={styles.headerDate}>{`Ngày ${item.AuditDate}`}</Text>
        )}
        {keyLayer2 && (
          <View style={styles.managerView}>
            <Text
              style={styles.titleManager}
            >{`Quản lí: (${item.ParentCode}) - ${item.ParentName}`}</Text>
          </View>
        )}
        {item.Note?.length > 0 && (
        ...
        >
          <View style={styles.pointView}>
            <Text
              style={{ color: appcolor.red, fontSize: 14, fontWeight: '500' }}
            >{`${item.mPoint}`}</Text>
          </View>
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <View style={styles.contentView}>
        {dataKPI.length > 0 && (
          <Text style={styles.scoreResultKPI}>{dataKPI[0].totalKPI}</Text>
        )}
        {dataKPI.length > 0 && (
          <Tabs.Container
            ref={tabRef}
        ...
        >
          <SafeAreaView
            style={{ width: '100%', height: deviceHeight, padding: 8 }}
          >
            <View style={{ margin: 8 }}>
              <FormGroup
                containerStyle={{
