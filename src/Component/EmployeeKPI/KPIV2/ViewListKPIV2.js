...
import { Pressable, ScrollView, StyleSheet, FlatList, LayoutAnimation, Platform, Text, TouchableOpacity, UIManager, View } from "react-native";
...
export const ViewListKPIV2 = ({ dataItem, dataGroup, dataPhoto, showALbumByItem, settings, dataKPI, dataKPIHistory, configData, countByData, configTableData, handleSelectHistory }) => {
    ...
    const styles = StyleSheet.create({
        root: { flex: 1 },
        totalRow: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 12, padding: 8 },
        totalText: { fontWeight: '500', fontSize: 18, color: appcolor.success },
        groupWrap: { width: '100%', padding: 4 },
        groupHeader: { flexDirection: "row", width: '100%', justifyContent: "space-between", padding: 4, backgroundColor: appcolor.surface, borderRadius: 8, zIndex: 1000, marginBottom: 4 },
        groupHeaderLeft: { justifyContent: 'center', width: '80%' },
        groupHeaderTitle: { fontWeight: '500', fontSize: 15, color: appcolor.dark },
        groupHeaderDesc: { fontWeight: '500', fontSize: 10, color: appcolor.placeholderText },
        groupHeaderRight: { padding: 10, width: '20%', flexDirection: 'row', justifyContent: 'space-between' },
        ...
        groupHistoryBtn: { flexDirection: "row", padding: 8, justifyContent: 'flex-end', backgroundColor: appcolor.primary, borderRadius: 8, zIndex: 1000 },
        groupHistoryText: { fontWeight: '500', fontSize: 15, color: appcolor.white },
        ...
        itemKPIWrap: { flex: 1 },
        itemKPIBtn: {},
        itemKPIContent: { width: '100%', flexDirection: 'row', justifyContent: "space-between", alignItems: 'center', borderBottomWidth: 0.5, borderBottomColor: appcolor.grayLight },
        itemKPIContentLock: { opacity: 0.4 },
        itemKPIIcon: { textAlign: 'center' },
        itemKPIName: { fontSize: 14, fontWeight: '500', color: appcolor.dark, padding: 8, width: '75%' },
        itemKPIPointWrap: { alignItems: 'center', padding: 4, width: '16%' },
        itemKPIPoint: { color: appcolor.dark, fontSize: 14, fontWeight: '500' },
        noteFormItem: { flex: 1, backgroundColor: appcolor.placeholderBody, margin: 8 },
    ...
    const handlerAnswer = (item, index, currentTab) => {
        ...
        if (item.isSingleChoice == 1) {
            for (let i = 0; i < detail.length; i++) {
                const it = detail[i]
                if (it.KPIId == item.KPIId && ((it.SubGroupId || 0) == (currentTab.SubGroupId || 0))) {
                    it.IsChecked = value
    ...
    const actionPhoto = async (photoinfo, type, item) => {
        switch (type) {
            case 'TAKE':
                await NativeCamera.cameraStart(photoinfo, () => { actionCallBackResult(item) })
                break;
            case 'UPLOAD':
                await NativeCamera.imageGalleryLaunch(photoinfo, () => { actionCallBackResult(item) })
                break;
        }
    }
    const actionCallBackResult = async (item) => {
        const listPhoto = await getPhotoByType(photoType + `_${settings.userId}`, item.guid)
        let newPhotoList = []
        listPhoto.map(it => {
    ...
    const RenderItemKPI = ({ item, index, currentTab }) => {
        ...
        return (
            <View key={`item_${index}`} style={styles.itemKPIWrap}>
                <TouchableOpacity key={`iiemwd_${index}`} activeOpacity={item.isLock == 1 ? 1 : 0.4} onPress={item.isLock == 1 ? null : onPressItem} style={styles.itemKPIBtn}>
                    <View style={[styles.itemKPIContent, item.isLock == 1 && styles.itemKPIContentLock, { opacity: undefined }]}>
                        <Icon
                            style={styles.itemKPIIcon}
                            type='font-awesome-5'
                            name={item.IsChecked == 1 ? "check-circle" : "circle"} size={18}
                ...
                {
                    item.IsChecked == 1 && <View style={{ flexDirection: 'row', alignItems: 'center', marginVertical: 8 }}>
                        {
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <View style={{ flexDirection: 'column' }}>
                                    <TouchableOpacity
                                        ...
                                        style={{
                                            width: 50, height: 40, backgroundColor: appcolor.light, borderWidth: 0.5, borderRadius: 5,
                                            borderColor: appcolor.greydark, marginRight: 2, justifyContent: 'center', alignItems: 'center'
                                        }}>
                                        <Icon color={appcolor.dark} name='camera' type='ionicon' size={20} />
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        ...
                                        <Icon color={appcolor.dark} name='attach' type='ionicon' size={20} />
                                    </TouchableOpacity>
                                </View>

                                <TouchableOpacity
                                    onPress={() => dataPhotoByItem?.length > 0 ? showALbumByItem(dataPhotoByItem, item) : null}
                                    style={{
                                        width: 50, height: 80, backgroundColor: appcolor.light, borderWidth: 0.5, borderRadius: 5,
                                        borderColor: appcolor.greydark, marginRight: 2, justifyContent: 'center', alignItems: 'center'
                                    }}>
                                    <Icon color={appcolor.dark} name='images' type='ionicon' size={20} />
                                    <Badge
    ...
    const filterData = (item) => {
        const dataByGroup = dataItem.filter(itemD => itemD.SubGroupId == item.SubGroupId)
        let totalByGroup = 0
        dataByGroup.map(it => {
            if (it.IsChecked == 1) {
    ...
    return (
        <View style={styles.root}>
            <View style={styles.totalRow}>
                <Text style={styles.totalText}>{configTableData?.titlePoint || 'Tổng điểm'}</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                {
                    (dataGroup.map((it, idx) => {
                        ...
                        const dataHistoryByGroup = dataKPIHistory?.length > 0 ? dataKPIHistory?.filter(ith => ith.GroupId == it.GroupId && ith.SubGroupId == it.SubGroupId) : []
                        return (
                            <View key={'ViewGroup_' + idx} style={styles.groupWrap}>
                                {
                                    (dataGroup?.length > 1 || configTableData?.showGroupDetail == 1) &&
                                    <View>
                                        <TouchableOpacity onPress={() => onPressShow(it)} style={styles.groupHeader}>
                                            <View style={styles.groupHeaderLeft}>
                                                <Text style={styles.groupHeaderTitle}>{it.SubGroupName}</Text>
                                                {it.Decription && <Text style={styles.groupHeaderDesc}>{it.Decription}</Text>}
                                            </View>
                                            <View style={styles.groupHeaderRight}>
                                                {
                                                    configData.byShop == 0 &&
                                                    <Text style={styles.groupHeaderTotal}>({totalByGroup || 0})</Text>
                                                }
                                                <Icon name={it.isShowDetail ? 'chevron-down' : 'chevron-right'} type='font-awesome-5' size={20} color={appcolor.dark} />
                                            </View>
                                        </TouchableOpacity>
                                        {
                                            dataHistoryByGroup?.length > 0 &&
                                            <View style={styles.groupHistoryWrap}>
                                                <TouchableOpacity onPress={() => handleSelectHistory(dataHistoryByGroup)} style={styles.groupHistoryBtn}>
                                                    <Text style={styles.groupHistoryText}>Lịch sử chấm điểm</Text>
                                                    {it.Decription && <Text style={styles.groupHeaderDesc}>{it.Decription}</Text>}
                                                </TouchableOpacity>
                                            </View>
                                        }
                                ...
                                <View style={[(it.isShowDetail || (dataGroup?.length == 1 && configTableData?.showGroupDetail !== 1)) ? styles.groupShowDetail : styles.groupShowDetailNone]}>
                                    {
                                    ...
                                    <FlatList
                                        ...
                                        windowSize={7}
                                        renderItem={({ item, index }) => <RenderItemKPI item={item} index={index} currentTab={it} />}
