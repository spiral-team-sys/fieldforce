...
export const HeaderInfoKPI = memo(
  ({ item, dataKPI, configData, configTableData, handleSelectItemTab }) => {
    ...
    const dateTitle =
      itemKPI?.auditDate !== null
        ? `Ngày ${itemKPI?.auditDate}`
        : 'Ngày chấm điểm:';
    const employeeTitle =
      item.userId > 0
        ? `Nhân viên (${item.employeeCode}) - ${item.fullName}`
        : 'Nhân viên:';
    const shopTitle =
      item.shopId > 0
      ...
      <View style={{ flexDirection: 'column', padding: 8 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <View style={{ width: '80%' }}>
            ...
            {configData.byShop !== 0 && (
              <Text style={styles.titleResultView}>
                {itemKPI?.resultItem || ''}
              </Text>
            )}
          </View>
          ...
          >
            ...
            >
              {configTableData?.titlePoint || 'Tổng'}
            </Text>
            <Text
              style={{ fontSize: 14, fontWeight: '700', color: appcolor.dark }}
            >
              {kpiSummary[0]?.totalSummary || 0}
            </Text>
          </View>
        </View>
        <Text
          key={`TotalKPI`}
          style={{ fontSize: 14, fontWeight: '600', color: appcolor.dark }}
        >
        ...
        >
          {kpiSummary?.map((item, index) => {
            return index > 0 ? (
              <TouchableOpacity
                onPress={() => handleSelectItemTab(item)}
                key={'TotalKPI_' + index}
                style={{
                  flexDirection: 'row',
                  alignItems: 'center',
              ...
              >
                ...
                >
                  ...
                  >{`${item.groupName} (${item.totalByGroup})`}</Text>
                </View>
              </TouchableOpacity>
            ) : null;
          })}
