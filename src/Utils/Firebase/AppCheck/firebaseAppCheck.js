...
const getKeyAppCheck = async () => {
    ...
    try {
        if (!AppAttestModule) {
            throw new Error('AppAttestModule chưa được liên kết thành công trong Xcode.');
        }
...
const verifyDeviceSecurity = async () => {
    try {
        ...
        if (Platform.OS === 'ios') {
            ...
            console.log('iOS: Đang ký Challenge tạo chứng chỉ chứng thực (Attestation Object)...');
            const attestationObject = await AppAttestModule.attestKey(keyId, mockChallenge);

            console.log('✅ iOS Bảo mật OK! Dữ liệu sẵn sàng gửi qua .NET:', { keyId, attestationObject });
            Alert.alert("iOS Thành công", "Đã lấy được chuỗi chứng thực App Attest.");
            return { platform: 'ios', keyId, attestationObject, challenge: mockChallenge };
            ...
            console.log('✅ Android Bảo mật OK! Gửi chuỗi Token JWT này qua .NET để bóc tách:', integrityToken);
        ...
        Alert.alert("Lỗi bảo mật hệ thống", error.message || "Thiết bị không vượt qua bài kiểm tra toàn vẹn.");
        return false;
    }
};
