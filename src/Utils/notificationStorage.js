...
export const getNotificationKey = (remoteMessage) => {
    const messageId = remoteMessage?.messageId || remoteMessage?.message_id || remoteMessage?.data?.messageId || remoteMessage?.data?.message_id;
...
export const savePendingNotification = async (remoteMessage) => {
    ...
    try {
        ...
        const current = (currentRaw !== null && currentRaw !== '[]') ? JSON.parse(currentRaw) : [];
