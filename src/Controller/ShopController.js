...
export const getDataStoreList = async (actionResult) => {
    ...
    const sql = `SELECT shopId as id, s.*,
        (SELECT COUNT(*) FROM ${photos.tableName} p WHERE p.shopId=s.shopId AND p.photoDate=s.auditDate AND p.reportId = 1) as finish
...
export const GetDataWorkingStatus = async (actionResult) => {
    ...
    if (res !== null && res.length > 0) {
        ...
        await actionResult({ isStartAction: true, isEndAction: true, isShow: false, isTakePicture: false })
    }
}
//
...
export async function getStoreListSO(search, date) {
    const sql = `SELECT shopId as id,shopName as name,auditDate 
    FROM ${storeList.tableName} st 
    WHERE auditDate=${date}` + (search !== '' ? " AND shopName like '%" + search + "%'" : "")
    const { res } = await QueryStringSql(sql)
    return res || [];
}
export const GetShopManager = async () => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'shops/storemaintant', requestInfo)
...
export async function uploadProfileShop(shopUpload, workinfo, finish, error) {
    try {
        await uploadServer(workinfo, shopUpload, async (result) => {
...
export const GetStoreReport = async (actionResult) => {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'shops/storereport', requestInfo);
...
export const GetDataCreateStore = async (actionResult) => {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'shops/datacreate', requestInfo);
        const result = await responseJson.json();
        if (result.statusId === 200)
            actionResult(result.data)
        else
...
export const GetConfigNewStore = async (actionResult) => {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'shops/confignewstore', requestInfo);
        const result = await responseJson.json();
        if (result.statusId === 200)
            actionResult(result.data)
        else
...
export const GetStoreInfo = async (shopId, actionResult) => {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'shops/getstoreinfo', requestInfo);
        const result = await responseJson.json();
        if (result.statusId === 200)
            actionResult(result.data)
        else
...
export const GetDataAllRegion = async (actionResult) => {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'shops/regionall', requestInfo);
        const result = await responseJson.json();
        if (result.statusId === 200)
            actionResult(result.data)
        else
...
export const CreateItemStore = async (dataStore, contentMassage, actionResult) => {
    try {
        let access_token = await GetToken();
        const requestInfo = {
            method: 'POST',
        ...
        const responseJson = await fetch(URLDEFAULT + 'shops/createstore', requestInfo);
        const result = await responseJson.json();
        actionResult(result)
    } catch (e) {
        alertError('Error: ' + e)
...
export const GetListConfirmStore = async (actionResult) => {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'shops/getlistconfirm', requestInfo);
        const result = await responseJson.json();
...
export const ConfirmNewStore = async (data, listEmployees, actionResult) => {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'shops/confirmnewstore', requestInfo);
        const result = await responseJson.json();
