...
import { URL_GET_ATTENDANT, URL_TIME, URL_UPLOAD_ATTENDANT, URL_UPLOAD_PHOTODATA, URL_UPLOAD_PHOTOS } from '../Core/URLs';
import { SelectItems, SelectItemsClause, Store, _createTable, _saveOrUpdate, UpdateItem, exeSql, Insert, DeleteAll, QueryStringSql } from './../Core/SqliteDbContext';
...
export async function GetPhotosCheckIn(shopId, workDate, reportId) {
    let lst = [];
    await Store().then(async db => {
        const { res, err } = await SelectItemsClause(db, 'photos', "*", { "shopId": shopId, "photoDate": workDate, "reportId": reportId, "photoType": 0 }, "GROUP BY photoType");
        lst = res;
    });
    return lst;
}
export async function GetPhotosINOUT(shopId, workDate, reportId) {
    let lst = [];
    await Store().then(async db => {
        const { res, err } = await SelectItemsClause(db, 'photos', "*", { "shopId": shopId, "photoDate": workDate, "reportId": reportId }, "GROUP BY photoType");
        lst = res;
    });
    return lst;
}
...
export async function UpdatePhotosStatus(photoTime, shopId, workDate, photoType) {
    let result = false;
    await Store().then(async db => {
        const { res, err } = await UpdateItem(db, 'photos', { "dataUpload": 1, "fileUpload": 1, "photoTime": photoTime }, { "shopId": shopId, "photoDate": workDate, "photoType": photoType });
        result = true;
    });
    return result;
}
export async function UpdateTimeOT(timeOT, noteOT, shopId, workDate, photoType) {
    let result = false;
    await Store().then(async db => {
        const { res, err } = await UpdateItem(db, 'photos', { "timeOT": timeOT, "noteOT": noteOT }, { "shopId": shopId, "photoDate": workDate, "photoType": photoType });
        //console.log(res, 'res')
        result = true;
    });
    return result;
...
export async function UpdateLocationStoreList(latitude, longitude, shopId) {
    let result = false;
    await Store().then(async db => {
        const { res, err } = await UpdateItem(db, 'storeList', { "latitude": latitude, "longitude": longitude }, { "shopId": shopId });
        result = true;
    });
...
export async function UpdatePhotosDataStatus(shopId, workDate, photoType) {
    let result = false;
    await Store().then(async db => {
        const { res, err } = await UpdateItem(db, 'photos', { "dataUpload": 1 }, { "shopId": shopId, "photoDate": workDate, "photoType": photoType });
        result = true;
    });
    return result;
}
export async function UpdateStatusPhotoData(id) {
    let result = false;
    await Store().then(async db => {
        const { res, err } = await UpdateItem(db, 'photos', { "dataUpload": 1 }, { "id": id });
        result = true;
    });
    return result;
}
export async function UpdateStatusPhotoFile(id) {
    let result = false;
    await Store().then(async db => {
        const { res, err } = await UpdateItem(db, 'photos', { "fileUpload": 1 }, { "id": id });
        result = true;
    });
    return result;
}
export async function UpdateUrlPhotosShop(shopId, urlImage, latitude, longitude) {
    let result = false;
    await Store().then(async db => {
        const { res, err } = await UpdateItem(db, 'storeList', { "imageUrl": urlImage, "latitude": latitude, "longitude": longitude }, { "shopId": shopId });
        result = true;
    });
    return result;
}
export async function UpdatePhotosFileStatus(shopId, workDate, photoType) {
    let result = false;
    await Store().then(async db => {
        const { res, err } = await UpdateItem(db, 'photos', { "fileUpload": 1 }, { "shopId": shopId, "photoDate": workDate, "photoType": photoType });
        result = true;
    });
    return result;
}
...
export async function GetAttendant(db, shopinfo) {
    const { res, err } = await SelectItems(db, 'photos', "*", { "ShopId": shopinfo.ShopId, "PhotoDate": shopinfo.AuditDate });
...
export async function uploadAttendant(photoInfo, type) {
    ...
    try {
        ...
        if (!isNetwork) {
            ToastError('Không có kết nối. Vui lòng kiểm tra lại kết nối mạng sau đó bấm nút refresh trên tấm hình để gửi lại.');
            return false
        }
        ...
        await fetch(URL_UPLOAD_ATTENDANT, {
            ...
            .then(responseJson => {
                if (responseJson.status == 100) {
                    UpdateAttendantStatus(photoInfo.shopId, photoInfo.photoDate, photoInfo.photoType);
                    type == "TAKE" && ToastSuccess(responseJson.messeger);
                    return true;
                }
                else {
                    // alert(responseJson.messeger+responseJson.status);
                    type == "TAKE" && ToastError('Gửi chấm công không thành công. Vui lòng bấm nút refresh trên tấm hình để gửi lại. Lỗi: ' + responseJson.messeger + responseJson.status);
                    return false;
                }
            })
            .catch(error => {
...
export async function uploadAllDataPhoto(dataPhoto, fileSuccess, fileError) {
    try {
        ...
        for (const photoInfo of (dataPhoto || [])) {
            ...
            if (await RNFS.exists(photoInfo.photoPath)) {
                ...
                if (result.status === 100) {
                    ...
                    typeof (fileError) === 'boolean' ? null : alertToast('Vui lòng vào album để up lại hình, Lỗi: ' + result.messeger);
        ...
        typeof (fileError) === 'boolean' ? null : alertToast('Vui lòng vào album để up lại hình, Lỗi: ' + error);
    }
}
...
export async function getPhotosNotUploadReport(reportId = 0, photoType, shopId, photoDate) {
    const sql = `Select * from photos WHERE fileUpload<>1 ${reportId >= 0 ? `and reportId=${reportId}` : ''}`;
    console.log(sql);
...
export const deletePhotoByList = async (lstDel) => {
    const del = await lstDel.map(v => { return v.id })
...
export const deleteOldPhotosByReport = async (shopId, reportId, photoDate = TODAY) => {
...
export const updatePhotoReportZalo = async (shopId, reportId, data) => {
    const checkSql = `SELECT COUNT(*) AS count FROM ${mobileRaw.tableName} WHERE shopId=${shopId} AND reportId=${reportId}`;
    ...
    if (isExist) {
        sql = `UPDATE ${mobileRaw.tableName} 
               ...
               WHERE shopId=${shopId} AND reportId=${reportId}`;
    } else {
...
export const getPhotoReportZalo = async (shopId, reportId) => {
    const sql = `SELECT * FROM ${mobileRaw.tableName} AS m WHERE m.shopId=${shopId} AND reportId=${reportId}`
    const { res } = await QueryStringSql(sql)
    return res
}

export const clearPhotoReportZalo = async (shopId, reportId, actionResult) => {
    const sql = `DELETE FROM ${mobileRaw.tableName} AS m WHERE m.shopId=${shopId} AND reportId=${reportId}`
    const { res } = await QueryStringSql(sql)
...
// Multiple Data
export const getDataPhotos = async (shopId, photoDate, photoType, reportId, guid, isUpload = false, actionResult) => {
    const sql = `SELECT * FROM ${photos.tableName} AS p 
    ...
    if (isUpload) {
        const dataPhotoUpload = _.map(res, (e) => {
            let imageName = e.photoPath.slice(e.photoPath.lastIndexOf('/', e.photoPath.length) + 1)
