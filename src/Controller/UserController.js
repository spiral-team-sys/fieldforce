...
import { URLLogin, URLLogout, URL_DOWNLOAD_MESSENGER, URLDEFAULT } from "../Core/URLs";
import AsyncStorage from '@react-native-async-storage/async-storage';
import { CreateDb, QueryStringSql, Insert } from "../Core/SqliteDbContext";
import { GetAsyncStorage, GetToken, defaultSetting, TOKEN_KEY, GetEmployeeInfo, ToastError } from "../Core/Helper";
import { getIdMaxMessenger, getManagerNotifi } from '../Controller/WorkController';
...
export const onLogin = async (dataLogin, actionResult, errorResult) => {
    try {
        ...
        if (!isNetwork) {
            Alert.alert("Không có kết nối mạng, vui lòng kiểm tra lại kết nối sau đó thử lại.");
            return
        }
        const requestInfo = {
            method: 'POST',
...
export const saveTokenUser = async (info) => {
    ...
    await json === null && await AsyncStorage.setItem("SETTINGS", JSON.stringify(defaultSetting))
    // await GAppController.SetUserInfo(info);
}
export const saveTokenUserToGapp = async (info, GAppController, isSercurity) => {
    const employee = { ...info, token: null };
    await CreateDb(info.employeeId);
    await AsyncStorage.setItem(TOKEN_KEY, info.token);
    await AsyncStorage.setItem("EmployeeId", info.employeeId.toString());
    await AsyncStorage.setItem("Employee", JSON.stringify(employee));
    isSercurity !== undefined && await AsyncStorage.setItem(SERCURITY.LOCALSECUIRY, isSercurity);
    await GAppController.SetUserInfo(info);
...
export const GetMenuKPI = async (byShop = 0, shopinfo, workinfo) => {
    ...
    const taskDateCondition = taskTable.tableName === taskListHistory.tableName ? ` AND t.workDate=${workDate}` : ''
