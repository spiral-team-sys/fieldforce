...
export async function updateItemsPromotion(item) {
  const sql = `UPDATE ${promotion.tableName}
...
export async function getListPromotion(workinfo, isByType = 0) {
  let sql = `
    INSERT INTO ${
      promotionPrice.tableName
    }(workId,shopId,workDate,productId,promotionType,upload)
    SELECT ${workinfo.workId},${workinfo.shopId},${
    workinfo.workDate
  },p.productId,0,0
    ...
    NOT EXISTS ( SELECT 1 FROM ${
      promotionPrice.tableName
    } AS d WHERE d.workId = ${workinfo.workId} AND d.productId = p.productId)
    `;
  await QueryStringSql(sql);
  ...
  sql = `SELECT d.*,p.type AS divisionId, p.division,p.productName, p.categoryName,p.productCode,p.report, p.categoryId,
    ...
    (CASE WHEN (d.discountPrice >0 AND d.discountPrice < 10000) OR  (d.discountPrice % 1000 > 0) THEN 1 ELSE 0 END)as discountError 
        ...
        AND ${isByType !== 1 ? ` p.type=${_competitorId} AND ` : ''} d.workId=${
    workinfo.workId
  }
...
export const uploadPromotionPrice = async (
  resDisplay,
  workinfo,
  finish,
  error,
      ...
      async result => {
        ...
        if (result.statusId == 200) {
          await Store().then(async db => {
            const sqlData =
              'Update promotionPrice set upload=1 WHERE workId=' +
              workinfo.workId;
