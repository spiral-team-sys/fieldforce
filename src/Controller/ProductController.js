...
export async function getCompetitorSO() {
    const sql = 'Select type as id,division as name'
        + " FROM products WHERE division IS NOT NULL"
        + (AppNameBuild === mitsuApp || AppNameBuild === 'lg' ? ' AND type=' + _competitorId : '')
        + " GROUP BY type"
...
export async function getCategoryPromotion() {
    const sql = 'Select categoryId as id,categoryName as name'
...
export const CountProduct = async () => {
    const { res } = await QueryStringSql(`SELECT COUNT(*) as totalProduct FROM ${products.tableName}`)
    return (res != undefined && res !== null && res.length > 0) ? res[0].totalProduct : 0
}
