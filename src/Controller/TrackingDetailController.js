...
export const getListTrackLG = async trackingId => {
  ...
  await Store().then(async db => {
    let sql =
      'select * from trackingDetail WHERE trackingId=' +
      trackingId +
...
export const getLstISLG = async trackingId => {
  ...
  await Store().then(async db => {
    let sql =
      'select * from trackingDetail WHERE competitorId <> 62 AND trackingId=' +
      trackingId +
      ' order by competitorId';
    const { res, err } = await exeSql(db, sql);
    data = res;
  ...
  if (trackingId) {
    ...
    await Store().then(async db => {
      UpdateItem(
        ...
        'trackingDetailResult',
        { note: note },
        { workId: workInfo.workId, shopId: workInfo.shopId },
      );
    });
  }
}
    ...
    async result => {
      if (result.statusId === 200) {
        await QueryStringSql(
          `UPDATE ${trackingDetailResult.tableName} SET upload=1 WHERE workId=${workinfo.workId}`,
        );
      }
...
export const UploadTrackingDisplayLG = async (
  workinfo,
  res,
  resPhotos,
  finish,
...
export const getListDataTracking = async (workInfo, trackingId) => {
  ...
  const sql = `SELECT t.* ,s.display, s.price, s.upload, s.note, s.isChangeValue, s.isCheckValue,
                ...
                LEFT JOIN ${
                  trackingDetail.tableName
                } as t ON s.itemId = t.id AND t.trackingId in(${trackingId}) 
                LEFT JOIN ${trackingDetailHistory.tableName} AS h ON h.shopId=${
    workInfo.shopId
  } AND h.itemId=s.itemId
                WHERE s.workId = ${workInfo.workId} AND s.shopId = ${
    workInfo.shopId
  } AND s.trackingId in(${trackingId}) 
                ORDER BY t.categoryId,${
                  AppNameBuild !== daikinApp &&
                  `${
                    AppNameBuild === bekoApp
                      ? 't.competitorId, t.subCatId'
                      : 't.subCatId, t.competitorId'
                  }`
                },t.orderBy
            `;
  const { res } = await QueryStringSql(sql);
...
export const updateCheckValueTracking = async (workInfo, item) => {
  const sql = `UPDATE ${trackingDetailResult.tableName}
        SET isChangeValue=${item.isChangeValue || 0},
            ...
            display=${
              item.display === null || item.display === undefined
                ? 'null'
                : item.display
            }
        WHERE itemId=${item.id} AND workId=${workInfo.workId} AND shopId=${
    workInfo.shopId
  }`;
  await QueryStringSql(sql);
};
...
export const dataSurveyResult = async (
  dataUpload,
  workinfo,
  trackingId,
  finish,
...
export const GetListTrackingStore = async (shopId, reportId, actionResult) => {
  try {
    ...
    const response = await fetch(
      URLDEFAULT + 'shops/trackingstore',
      requestInfo,
    );
...
export const uploadTrackingStore = async (shopId, dataStore, actionResult) => {
  try {
    ...
    const responseJson = await fetch(
      URLDEFAULT + 'shops/update/trackingstore',
      requestInfo,
    );
