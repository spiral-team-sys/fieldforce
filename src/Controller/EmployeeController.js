...
const SaveProfileEmployee = async (data) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'employee/profileupdate', requestInfo)
...
export const CheckProfileEmployee = async () => {
    try {
        try {
            ...
            const response = await fetch(URLDEFAULT + 'employee/checkprofile', requestInfo)
            const result = await response.json()
            if (result.statusId === 200) {
...
export const GetEmployeeManager = async () => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'employee/employeemaintant', requestInfo)
...
const GetEmployeeInfo = async () => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'employee/employeeById', requestInfo)
        const result = await response.json()
        if (result.statusId === 200)
            return result.data
        return result.messager;
...
const uploadProfile = async (resProfile, uploadPhoto) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + "employee/update/employeemaintant", requestInfo)
        const result = await response.json();
        if (result.statusId === 200) {
...
export const sendMailEmployeeResign = async (json) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + "employee/sendmailresign", requestInfo)
        const result = await response.json()
        return result;
...
const sendEmployeeResigns = async (resProfile, uploadPhoto, notifyNote, group, userId) => {
    try {
        const token = await GetToken();
        const requestInfo = {
            method: 'POST',
        ...
        const response = await fetch(URLDEFAULT + "employee/update/employeeresign", requestInfo)
        const result = await response.json();
...
export const getResignInfo = async (typeGetResign) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + "employee/employeeresigns", requestInfo)
        const result = await response.json()
        return result;
    } catch (error) {
        return { statusId: 500, messager: "lỗi kết nối" }
    ...
    await EmployeeAPI.validDataEmployeeInfo(async (info, masterData) => {
        if (masterData !== null && masterData.length > 0) {
            ...
            if (strError !== null && strError.length > 0) {
                actionResult(info.requiredData == 1 ? 500 : 200, `Vui lòng cập nhật đầy đủ các thông tin bắt buộc sau trong mục "Thông tin nhân viên":\n${strError}`)
                return
            } else {
...
export const getMaternityInfo = async (typeGetMaternity) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + "employee/employeematernity", requestInfo)
        const result = await response.json()
        return result;
    } catch (error) {
        return { statusId: 500, messager: "lỗi kết nối" }
...
const sendEmployeeMaternity = async (resProfile, uploadPhoto, notifyNote, group, userId) => {
    try {
        const token = await GetToken();
        const requestInfo = {
            method: 'POST',
        ...
        const response = await fetch(URLDEFAULT + "employee/update/employeematernity", requestInfo)
        const result = await response.json();
        if (result.statusId === 200 || (result.statusId === 500 && result.messager.includes('mail'))) {
            uploadPhoto && await UploadController.PostFile()
            return result
...
export const Employee = { GetEmployeeInfo, SaveProfileEmployee, getResignInfo, uploadProfile, sendMailEmployeeResign, sendEmployeeResigns, getEmployeeConfig, getPhotoResignByEmployee, GetPhotosProfile, validData_EmployeeInfo, getMaternityInfo, sendEmployeeMaternity }
