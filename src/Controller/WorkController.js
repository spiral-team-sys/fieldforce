import {
  ...
  QueryStringSql,
  Insert,
} from '../Core/SqliteDbContext'; //"../Core/SqliteDbContext";
import {
  GetToken,
  ToastError,
  ToastSuccess,
...
import {
  URL_UPLOAD_AUDIT_DISPLAY,
  URL_UPLOAD_PHOTOS,
  URL_SAVE_NOTE_ATTENDANT,
  EMPLOYEE_TYPE_PHOTO,
  AppNameBuild,
  epsonApp,
  hafeleApp,
  URL_POST_SELLOUTUPLOAD,
  URL_UPLOAD_STOCK,
  URL_UPLOAD_DISPLAY,
  URL_UPLOAD_OOS,
  URL_UPLOAD_EVIDENT,
  URL_POST_SELLOUTUPLOAD_DB,
  URL_UPLOAD_OT_TIME,
  URLDEFAULT,
} from '../Core/URLs';
...
export async function uploadDisplayData(
  resDisplay,
  resPhotos,
  workinfo,
  finish,
      ...
      .then(responseJson => {
        if (responseJson.status == 200) {
          if (responseJson.messeger === 'Đã gửi báo cáo') {
            ...
            QueryStringSql(
              `UPDATE ${taskList.tableName} set taskDone=1 WHERE shopId=${workinfo.shopId} and reportId=${workinfo.reportId}`,
            );
          }
...
export async function uploadStockoutData(stockData, workinfo, finish) {
  ...
  try {
    ...
    const response = await fetch(URL_UPLOAD_STOCK, {
      method: 'post',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
...
export async function getCategoryPro(comId) {
  ...
  await Store().then(async db => {
    const sql =
      'select categoryName  as label,categoryId as value from products' +
      ' where type = ' +
      comId +
      ' GROUP BY categoryId';
...
export async function uploadSelloutWFH(resupload, workinfo, finish, errorAct) {
  ...
  var Results = await UploadData(
    URL_POST_SELLOUTUPLOAD_DB,
    JSON.stringify(SellInfo),
  );
  if (Results != null && Results.status == 200) {
    alertToast('Gửi dữ liệu thành công');
    await Store().then(async db => {
      const sql =
        'Update Sellout set upload=1 WHERE auditDate=' + workinfo.workDate;
...
export async function uploadSelloutData(resupload, workinfo, finish, errorAct) {
  var details = [];
  let resPhotos =
    (await getAllPhotosUploaded(0, workinfo.shopId, workinfo.workDate)) || [];
  ...
  if (mPhoto !== null && mPhoto.length > 0) {
    try {
      ...
      mPhoto.forEach(photoInfo => {
        ...
        let pathPhoto =
          '/uploaded/' + Moment(new Date()).format('YYYYMMDD') + '/' + ImgName;
        let dataItem = {
          shopId: photoInfo.shopId,
          photoName: ImgName,
          latitude: photoInfo.latitude,
...
export async function uploadFileCMND() {
  ...
  if (dataPhoto == null || dataPhoto.length < 1) return;
  try {
    let access_token = await Token();
      ...
      await fetch(URL_UPLOAD_PHOTOS, {
        ...
        .then(responseJson => {
          if (responseJson.status == 100) {
            ...
            alertError(
              'Vui lòng vào album để up lại hình, Lỗi: ' +
...
export async function saveNoteAttendant(
  shopId,
  workdate,
  note,
  actionResult,
...
export async function uploadTimeOT(
  shopId,
  workdate,
  OTTime,
  note,
  ...
  try {
    ...
    await fetch(URL_UPLOAD_OT_TIME, {
      ...
      .catch(error => {
        actionResult({
          status: 100,
          messeger: 'Không kết nối được với hệ thống',
        });
      });
  } catch (e) {
...
export async function GetLstWorkInfo(shopId, workDate) {
  ...
  await Store().then(async db => {
    const sql =
      'SELECT * FROM workResults WHERE shopId=' +
      shopId +
      ' AND workDate=' +
      workDate;
    const { res, err } = await exeSql(db, sql);
...
export async function GetSelloutBy(id, workId) {
  ...
  await Store().then(async db => {
    const sql =
      'Select * FROM sellOut WHERE competitorId=' +
      id +
      ' AND workId=' +
      workId;
    const { res, err } = await exeSql(db, sql);
...
export async function Check2IMEISellout(
  IMEI,
  IMEI2,
  productId,
  sellId,
...
export async function PromotionUpload(db, workinfo) {
  const sql =
    'Select * FROM promotion Where upload<>1 AND workId=' + workinfo.workId;
...
export async function getIdMaxPhotos(photoDate) {
  ...
  await Store().then(async db => {
    const sql =
      "Select * from photos WHERE photoType NOT IN ('CMND_BEFORE','CMND_AFTER','LEAVE_JOB') AND id in (" +
      'Select Max(id) FROM photos WHERE dataUpload=0 AND fileUpload=0 AND photoDate=' +
      photoDate +
      ')';
    // alert(sql)
    const { res, err } = await exeSql(db, sql);
...
export async function getIdPhotoCMND(photoDate) {
  ...
  await Store().then(async db => {
    const sql =
      "Select * from photos WHERE photoType IN ('CMND_BEFORE','CMND_AFTER') AND id in (" +
      'Select Max(id) FROM photos WHERE dataUpload=0 AND fileUpload=0 AND photoDate=' +
      photoDate +
      ')';
    const { res, err } = await exeSql(db, sql);
...
export async function getIdMaxImage(photoDate) {
  ...
  await Store().then(async db => {
    const sql =
      "Select * from photos WHERE photoType NOT IN ('CMND_BEFORE','CMND_AFTER') AND id in (" +
      'Select Max(id) FROM photos WHERE photoDate=' +
      photoDate +
      " AND photoType='" +
      EMPLOYEE_TYPE_PHOTO +
      "')";
...
export async function getLstMessenger() {
  ...
  await Store().then(async db => {
    const sql =
      "Select *  FROM messenger WHERE typeReport!='Payslip' ORDER BY id DESC";
    const { res, err } = await exeSql(db, sql);
    lst = res;
  });
  return lst;
...
export const getLstMessengerNotSeen = async () => {
  const sql =
    "Select * FROM messenger WHERE (seen IS NULL OR seen = 0)  AND typeReport is not null AND typeReport not in ('Payslip')";
...
export async function addTableSettingApp() {
  await Store().then(async db => {
    try {
      const sql =
        'CREATE TABLE IF NOT EXISTS settingapp (id INTEGER PRIMARY KEY, name NVARCHAR(200), nameVN NVARCHAR(200),status INTEGER);';
      var { res } = await exeSql(db, sql);

      const sqlGet = 'SELECT * FROM settingapp';
      var { res } = await exeSql(db, sqlGet);
      if (res === undefined) {
        await InsertItems(db, 'settingapp', [
          {
            id: 3,
            name: 'sort list MENU',
            nameVN: 'hiển thị danh sách Menu',
            status: 0,
          },
        ]);
        await InsertItems(db, 'settingapp', [
...
export async function updateSetting(id) {
  await Store().then(async db => {
    let sql =
      'Update settingapp set status= case when status = 1 then 0 else 1 end where id=' +
      id;
    await exeSql(db, sql);
  });
}
export async function updateSettingBy(id, status) {
  await Store().then(async db => {
    let sql = 'Update settingapp set status=' + status + ' where id=' + id;
    await exeSql(db, sql);
  });
}
export async function getSettingApp() {
  let lst = [];
  await Store().then(async db => {
    const sql = 'Select * ' + ' FROM settingapp';
    const { res, err } = await exeSql(db, sql);
    lst = res;
  });
  return lst;
...
export async function getCategory() {
  ...
  await Store().then(async db => {
    const sql =
      'Select Distinct categoryid as id, categoryName as name, competitorName as division' +
    ...
    await getCompetitorItemRes(
      ...
      if (lstHave.length > 0) {
        await UpdateItem(
          ...
          'displayCompetitor',
          { quantity: item.quantity },
          { workId: item.workId, divisionId: item.divisionId },
        );
      } else {
        await InsertItems(db, 'displayCompetitor', [item]);
      }
...
export async function deleteCompetitorResult(displayCompetitor) {
  await Store().then(async db => {
    DeleteItem(db, 'displayCompetitor', {
      displayCompetitorId: displayCompetitor.displayCompetitorId,
    });
  });
}
...
export async function updateCompetitorResult(idItem, displayCompetitor) {
  await Store().then(async db => {
    UpdateItem(db, 'displayCompetitor', displayCompetitor, {
      displayCompetitorId: idItem,
    });
  });
}
        ...
        switch (type) {
          ...
          case 3:
            await UpdateItem(
              db,
              'displayAudit',
              { kpi3: display.kpi3, kpi2: display.kpi2 },
              {
                workId: display.workId,
                displayId: display.displayId,
                itemId: display.itemId,
    ...
    await getDisplayRes(info.workId, info.productId).then(async lstHave => {
      if (lstHave.length > 0) {
        await UpdateItem(
          ...
          { workId: info.workId, productId: info.productId },
        );
      } else {
...
export async function insertPriceOfDisplay(display) {
  await Store().then(async db => {
    await getDisplayRes(display.workId, display.productId).then(
      async lstHave => {
        if (lstHave.length > 0) {
          await UpdateItem(
            db,
            'display',
            { price: display.price },
            { workId: display.workId, productId: display.productId },
          );
        } else {
...
export async function updateNoteStock(stock) {
  await Store().then(async db => {
    UpdateItem(
      db,
      'stockout',
      { displayComment: stock.displayComment },
      { workId: stock.workId },
    );
  });
}
...
export async function updateDisplayHPResult(display) {
  await Store().then(async db => {
    UpdateItem(
      db,
      'displayAudit',
      { displayComment: display.displayComment },
      {
        workId: display.workId,
        displayId: display.displayId,
        displayRef: display.displayRef,
        displaySubCat: display.displaySubCat,
      },
    );
  });
}
...
export async function updateNoteDisplayReport(itemDisplay) {
  const sql = `UPDATE ${display.tableName} 
        ...
        ${
          itemDisplay.displayRef !== null
            ? `AND displayRef='${itemDisplay.displayRef}'`
            : ''
        } `;
  await QueryStringSql(sql);
}
...
export async function getDisplayProgramResult(workId, ProgramId) {
  ...
  await Store().then(async db => {
    const sql =
      'Select * from displayAudit WHERE workId=' +
      workId +
      ' AND quanity != -1' +
      ' AND displayId=' +
      ProgramId;
...
export async function deleteAuditDisplayResult(workId, ProgramId) {
  ...
  await Store().then(async db => {
    const sql =
      'Delete from displayAudit WHERE workId=' +
      workId +
      ' AND displayId=' +
      ProgramId;
...
export async function getAllAuditResult(workId, lstId) {
  ...
  await Store().then(async db => {
    const sql =
      'Select displayId from displayAudit WHERE workId=' +
      workId +
      ' AND displayId IN(' +
      lstId +
      ')' +
...
export async function existAuditDisplayResult(
  workId,
  ProgramId,
  itemId,
  displayRef,
...
export async function existAuditData(displayId, itemName, refName) {
  ...
  await Store().then(async db => {
    const sql =
      'Select *' +
      ' FROM auditDisplayItem WHERE displayId=' +
      displayId +
      " AND itemName='" +
      itemName +
      "'" +
...
export async function getDisplayRes(workId, productId) {
  ...
  await Store().then(async db => {
    const sql =
      'Select * from display WHERE workId=' +
      workId +
      ' AND productId=' +
      productId;
    const { res, err } = await exeSql(db, sql);
...
export async function updateAuditDisplayResult(workinfo, displayId) {
  await Store().then(async db => {
    UpdateItem(
      ...
      'displayAudit',
      { upload: 1 },
      { workId: workinfo.workId, displayId: displayId },
    );
  });
}
export async function updateHavePack(workinfo, displayId, havePack, comment) {
  await Store().then(async db => {
    UpdateItem(
      db,
      'displayAudit',
      { havePack: havePack, comment: comment },
      { workId: workinfo.workId, displayId: displayId },
    );
  });
}
...
export async function getSubCategoriesList() {
  ...
  await Store().then(async db => {
    const sql =
      'Select Distinct subCatId as id, subCategory as name FROM products AS p WHERE type=109 ORDER BY categoryId';
    const { res, err } = await exeSql(db, sql);
    lst = res;
  });
  return lst;
...
export async function getSubCatByCateId(competitor, categoryId) {
  ...
  await Store().then(async db => {
    const sql =
      "Select Distinct subCatId as id, subCategory as name FROM products AS p WHERE productCode not in ('OOS','NOSELL') and categoryId='" +
      categoryId +
      "'" +
      (competitor !== '' ? " AND division='" + competitor + "'" : '');
    +"' ORDER BY categoryId";
...
export async function getSubCategoriesAudit(categorySelect) {
  let lst = [];
  await Store().then(async db => {
    const sql = 'Select * FROM masterList';
...
export async function getProductList() {
  ...
  await Store().then(async db => {
    const sql =
      "Select * , productId as id, productName as name FROM products AS p WHERE productCode not in ('OOS','NOSELL') and type=109 ORDER BY categoryId,subCatId";
    const { res, err } = await exeSql(db, sql);
    lst = res;
  });
  return lst;
...
export async function getAllProductList(search) {
  ...
  await Store().then(async db => {
    const sql =
      "Select * , productId as id, productName as name FROM products AS p WHERE productCode not in ('OOS','NOSELL')" +
...
export async function getPOSM(search) {
  ...
  await Store().then(async db => {
    const sql =
      'Select id, itemName as name FROM posm AS p' +
...
export async function getDisplayHistory(workinfo) {
  ...
  await Store().then(async db => {
    const sql =
      'Select * ' + ' FROM historyDisplay' + ' Where shopId=' + workinfo.shopId;
    const { res, err } = await exeSql(db, sql);
    lst = res;
  });
  return lst;
...
export async function getMarketHistory(workinfo) {
  ...
  await Store().then(async db => {
    const sql =
      'Select * ' + ' FROM historyMarket' + ' Where shopId=' + workinfo.shopId;
    const { res, err } = await exeSql(db, sql);
    lst = res;
  });
  return lst;
...
export async function getMarketResult(workinfo) {
  ...
  await Store().then(async db => {
    const sql =
      'Select * FROM market Where workId=' +
      workinfo.workId +
      ' ORDER BY Id DESC';
    const { res, err } = await exeSql(db, sql);
    // alert(sql)
...
export async function getDisplayResult(
  workinfo,
  reportId = 0,
  isNeedShelve = 0,
) {
...
export async function getProgramDisplay(shopId) {
  ...
  await Store().then(async db => {
    const sql =
      'Select displayId as id,displayNameVN as name,fieldSetting,visit,isPhotos FROM auditDisplayItem' +
      ' WHERE shopId=' +
      shopId +
      ' GROUP BY displayId';
    const { res, err } = await exeSql(db, sql);
...
export async function getCategoryAudit(displayId) {
  ...
  await Store().then(async db => {
    const sql =
      'Select Distinct refId as id, refName as name' +
      ' FROM auditDisplayItem WHERE displayId=' +
      displayId +
      ' GROUP BY refName';
    const { res, err } = await exeSql(db, sql);
...
export async function setDataOOS(
  workinfo,
  categoryName,
  keyFilter,
  oosSelected,
...
export async function OOSGetList(
  db,
  workinfo,
  categoryName,
  keyFilter,
  ...
  let lstOOS = [];
  const sql =
    'select o.*,p.productId,p.productName,p.productCode,p.unit,p.categoryId,p.categoryName,p.subCatId,p.subCategory' +
    ' from oos as o' +
    ' join products as p on p.productId = o.productId and o.workId = ' +
    workinfo.workId +
    ' where 1=1 ' +
    (categoryName != null && categoryName != 'Tất cả'
...
export async function insertOOSList(db, workinfo) {
  const sql =
    'insert into oos (workId,productId,oos)' +
    ' select ' +
    workinfo.workId +
    ', p.productId, o.oos from products as p' +
    ' left join oosHistory as o on p.productId = o.productId and o.shopId = ' +
    workinfo.shopId +
    ' and o.workDate = ' +
    workinfo.workDate +
...
export async function updateOOSItem(oosItem, isChecked) {
  await Store().then(async db => {
    ...
    const updateOOSFull =
      'update oos set oosFull = 0 where workId = ' + oosItem.workId;
...
export async function setFullOOS(workId, isFullOOS) {
  await Store().then(async db => {
    const sql =
      'update oos set' +
      (isFullOOS == 1 ? ' oos = ' + isFullOOS : ' oos = null') +
      ', oosFull = ' +
      isFullOOS +
      ' where workId = ' +
      workId;
...
export async function getListCategory() {
  ...
  await Store().then(async db => {
    const sql =
      "select 'Tất cả' as label, 'Tất cả' as value union all" +
...
export async function getCategoryByProduct() {
  ...
  await Store().then(async db => {
    const sql =
      "select distinct categoryId as id, category as name, type from products WHERE productCode not in ('OOS','NOSELL')";
    const { res, err } = await exeSql(db, sql);
    lst = res;
  });
  return lst;
...
export async function getQuestionSurveyCTSS(groupName) {
  ...
  await Store().then(async db => {
    const sql =
      'Select Distinct ref_Id as id, ref_Name as name' +
      " FROM masterList WHERE listCode = 'CTSS' AND groupName='" +
      groupName +
      "' GROUP BY ref_Id,ref_Name";
    const { res, err } = await exeSql(db, sql);
    lst = res;
  });
  return lst;
...
export async function getAllProduct() {
  ...
  await Store().then(async db => {
    var sql = '';
    sql =
      "Select p.* FROM products as p WHERE productName IS NOT NULL AND productCode not in ('OOS','NOSELL')";
...
export async function getItemsProduct(
  comSelect,
  categorySelect,
  subCategorySelect,
) {
  ...
  await Store().then(async db => {
    var sql = '';
    sql =
      'Select productId,categoryId, productName,unit,productCode,catID, category, category as categoryName, marketName, division, [type], subCategory,subCatId,productLine,errorPrice,price,orderBy,segmentId,segment,SubSegmentId,SubSegment,report,isCheck,marketName' +
    ...
    if (categorySelect !== All_Select) {
      sql +=
        categorySelect !== null && categorySelect !== ''
          ? " AND p.category='" + categorySelect + "'"
          : '';
    }
    if (subCategorySelect !== All_Select) {
      sql +=
        subCategorySelect !== null && subCategorySelect !== ''
          ? " AND p.subCategory='" + subCategorySelect + "'"
          : '';
    }
...
export async function getItemsProductLG(
  comSelect,
  categorySelect,
  subCategorySelect,
) {
  ...
  await Store().then(async db => {
    var sql = '';
    sql =
      'Select p.*, p.category as categoryName,p.marketName AS division' +
...
export async function getProductDisplay(comSelect) {
  ...
  await Store().then(async db => {
    var sql = '';
    sql =
      'Select productId,categoryId, productName,unit,productCode,catID, category, category as categoryName, marketName,marketName AS division, [type], subCategory,subCatId,productLine,errorPrice,price,orderBy,segmentId,segment,SubSegmentId,SubSegment,report,isCheck' +
      " FROM products as p WHERE productName IS NOT NULL AND productCode not in ('OOS','NOSELL')";
...
export async function getItemsProgramDisplay(
  DisplayId,
  categorySelect,
  subCategorySelect,
) {
...
export async function checkItemsAuditDisplay(DisplayId, workId) {
  ...
  await Store().then(async db => {
    const sql =
      ...
      ' AND di.id NOT IN (SELECT itemId FROM displayAudit WHERE workId=' +
      workId +
...
export async function checkResAuditDisplay(DisplayId, workId) {
  ...
  await Store().then(async db => {
    const sql =
      ...
      ' AND da.itemId NOT IN (SELECT itemId FROM displayAudit WHERE workId=' +
      workId +
      " AND displayRef = 'PRODUCT'" +
...
export async function getPhotosKTVUpload(shopId, photoDate) {
  ...
  await Store().then(async db => {
    const sql =
      "Select * from photos WHERE photoType='KTV'" +
      ' AND photoDate=' +
      photoDate +
      ' AND shopId=' +
      shopId;
...
export const getPhotosReport = async (
  reportId,
  photoType,
  shopId,
  photoDate,
...
export const getPhotosReportByCate = async (
  reportId,
  photoType,
  shopId,
  photoDate,
...
export async function getPhotosReportByGuiId(
  reportId,
  guiId,
  shopId,
  photoDate,
...
export async function getPhotosByLstGuiId(LstGuiId) {
  ...
  await Store().then(async db => {
    const sql =
      'Select * from photos WHERE guid IN (' +
      LstGuiId +
      ') AND dataUpload = 0';
    const { res, err } = await exeSql(db, sql);
    lst = res;
...
export async function getPhotosAllAuditDisplay(
  reportId,
  shopId,
  photoDate,
  photoType,
...
export async function getPhotosUploaded(
  reportId,
  shopId,
  photoDate,
  photoType,
...
export async function getPhotosAuditUploaded(
  reportId,
  shopId,
  photoDate,
  photoType,
...
export async function getPhotosAuditPack(
  reportId,
  shopId,
  photoDate,
  DisplayId,
...
export async function getPhotosAuditPackBy(
  reportId,
  shopId,
  photoDate,
  photoType,
...
export async function deleteItemPhotosAudit(
  reportId,
  shopId,
  photoDate,
  photoType,
  ...
  await Store().then(async db => {
    const sql =
      'Delete from photos WHERE reportId=' +
      reportId +
      ' AND photoDate=' +
      photoDate +
      ' AND shopId=' +
...
export async function deletePhotosAuditUploaded(
  reportId,
  shopId,
  photoDate,
  DisplayId,
...
export async function getPhotoByCategory(
  reportId,
  shopId,
  photoDate,
  CategoryName,
...
export async function updateStatusFileUploaded(
  reportId,
  shopId,
  photoDate,
  photoType,
...
export async function uploadDataDisplayCompe(
  lstDisplay,
  workinfo,
  finish,
  error,
...
export async function uploadAuditDisplay(
  DisplayId,
  lstDisplay,
  lstPhotos,
  workinfo,
