...
const GetPriceCompetitorDealer = async workDate => {
  try {
    ...
    const response = await fetch(
      URLDEFAULT + 'pricereport/pricecompetitorgetlist',
      requestInfo,
    );
...
const UploadPriceCompetitorDealer = async (jsonData, actionResult) => {
  try {
    ...
    const response = await fetch(
      URLDEFAULT + 'pricereport/pricecompetitorsend',
      requestInfo,
    );
    const result = await response.json();
    actionResult(result);
...
// Market Price
const dataTabMarketPrice = async () => {
  //lấy danh sách Tab theo thể loại mặt hàng
  const sql = `
...
const dataCompetitorMarketPrice = async () => {
...
const GetOnlyCompetitor = async () => {
  //Lấy danh sách đối thủ mà ko có của hãng
  const sql = `SELECT DISTINCT p.division as competitorName, p.type as competitorId, 0 as isSelect 
...
const getListMarketPrice = async (mWork, competitorId) => {
  //lấy danh sách sản phẩm từng Id
...
const getListAllMarketPrice = async mWork => {
  //lấy danh sách sản phẩm từng Id
...
const getListPriceCompetitor = async (mWork, isAllCompetitor = false) => {
  //lấy danh sách sản phẩm đối thủ ngoại trừ hãng
  let sql = `INSERT INTO ${
    marketPriceResult.tableName
  } (workId,productId,isUploaded,priceValue,netValue,fsmValue)
    SELECT ${
      mWork.workId
    } as workId,p.productId,0,h.priceValue,h.netValue,h.fsmValue
    FROM ${products.tableName} AS p
    LEFT JOIN ${
      historyMarketPrice.tableName
    } AS h ON p.productId=h.productId AND h.shopId=${
    mWork.shopId
  } AND h.workDate=${mWork.workDate}
    WHERE ${isAllCompetitor ? '1=1' : `p.type<>${_competitorId}`} 
    AND NOT EXISTS (SELECT 1 FROM ${
      marketPriceResult.tableName
    } AS m WHERE m.workId=${mWork.workId} AND m.productId=p.productId)`;
  await QueryStringSql(sql);
  //
  sql = `SELECT 0 AS indexMain,
        p.productId,p.productName,p.categoryId,p.categoryName,p.subCatId,p.subCategory,p.type AS competitorId,
...
const getListUpload_MarketPrice = async (mWork, listConfig = {}) => {
  const sql = `SELECT ${mWork.shopId} AS shopId,${
    mWork.workDate
  } AS workDate,m.productId,m.priceValue,m.netValue,m.fsmValue,m.isUploaded
            FROM ${marketPriceResult.tableName} AS m 
            WHERE m.workId=${mWork.workId}
            AND (
            ${
              listConfig?.priceValue == 1
                ? `(m.priceValue IS NULL OR m.priceValue BETWEEN 1 AND 999 OR m.priceValue % 1000 <> 0)`
                : `m.priceValue IS NOT NULL`
            } OR
            ${
              listConfig?.netValue == 1
                ? `(m.netValue IS NULL OR m.netValue BETWEEN 1 AND 999 OR m.netValue % 1000 <> 0)`
                : `m.netValue IS NOT NULL`
            } OR
            ${
              listConfig?.fsmValue == 1
                ? `(m.fsmValue IS NULL OR m.fsmValue BETWEEN 1 AND 999 OR m.fsmValue % 1000 <> 0) `
                : `m.fsmValue IS NOT NULL`
            }
            )
        `;
  const { res } = await QueryStringSql(sql);
...
const checkDataUpload = async workId => {
  ...
  await Store().then(async db => {
    const sql =
      ...
      ' where m.netValue>0 or m.fsmValue>0 and workId=' +
      workId +
      ' union all ' +
      " select 1000 as target, ifnull(m.netValue,0) as actual, 'Giá NET sản phẩm : '||p.productName||'\nHãng : '||p.division as message" +
      ' from marketPriceResult as m ' +
      ' left join products as p on m.productId=p.productId' +
      ' where m.priceValue>0 or m.fsmValue>0 and workId=' +
      workId +
...
export {
  // Market Price
  getListAllMarketPrice,
  GetOnlyCompetitor,
  dataTabMarketPrice,
  getListMarketPrice,
  updateItemPrice,
  getListUpload_MarketPrice,
  doneReportMarketPrice,
  uploadMarketPrice,
  dataCompetitorMarketPrice,
  checkDataUpload,
  getHistoryMarketPrice,
  insertHistoryMarketPrice,
  getListPriceCompetitor,
