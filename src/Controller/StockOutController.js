...
export const GetListInvetoryReport = async (comSelect, search) => {
    ...
    sql = "Select p.marketName AS division,p.*"
        + " FROM products as p WHERE p.productName IS NOT NULL AND p.productCode not in ('OOS','NOSELL') AND p.type = '" + _competitorId + "'"
...
export const uploadTotalInventory = async (dataStock, actionResult) => {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'stock/uploadwarehouse', requestInfo);
