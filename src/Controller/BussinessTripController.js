...
import { URL_UPLOAD_WORKING_SCHEDULE, URL_WORKINGPLAN_BYDATE, URL_REGIONS } from '../Core/URLs'
...
export async function UploadBussinessTrip(bussinessInfo, finish) {
    ...
    try {
        ...
        let dataItem = {
            ...
            "fromDate": !isNaN(bussinessInfo?.fromDate) ? bussinessInfo?.fromDate : parseInt(bussinessInfo?.fromDate.replace(/-/gm, '')),
            "toDate": !isNaN(bussinessInfo?.toDate) ? bussinessInfo?.toDate : parseInt(bussinessInfo?.toDate.replace(/-/gm, '')),
...
export async function GetBusinesTrips(fromdate, todate, actionResult) {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'WorkingSchedule/getlist', requestInfo);
        const result = await responseJson.json();
...
export async function GetConfirmTrips(fromdate, todate, actionResult) {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'WorkingSchedule/confirmlist', requestInfo);
        const result = await responseJson.json();
        if (result.statusId === 200)
            actionResult(result.data)
        else
...
export async function GetPlanBusiness(fromDate, toDate, actionResult) {
    ...
    const responseJson = await fetch(URLDEFAULT + 'workingplan/workplanbydate', requestInfo);
    const result = await responseJson.json();
    if (result.statusId === 200)
        actionResult(result.data)
    else
        alertError('Lỗi kết nối API')
}
...
export async function UploadBusiness(typeAction, itemTrips, actionResult) {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'workingschedule/uploadtrips', requestInfo);
        const result = await responseJson.json();
...
export const GetAddressByGeo = async (latlng, actionResult) => {
    ...
    if (await result.statusCode === 200) {
        ...
        actionResult(info.formatted_address || 'Không xác định', info.address_components[3]?.long_name || null)
    } else {
...
export const GetDataTripBills = async (data) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'workingschedule/tripbills', requestInfo)
        const result = await response.json()
        return result
    } catch (err) {
        return err;
...
export const GetDataTripBillsDetail = async (detailId) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'workingschedule/tripbillsDetail', requestInfo)
        const result = await response.json()
        if (result.statusId === 200)
            return result.data
...
export const checkLinkInvoice = async (url, year, month, actionResult) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'upload/downloadinvoice', requestInfo)
        const result = await response.json()
        actionResult(result)
    } catch (err) {
        return err;
...
export const SendInvoice = async (data) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'workingschedule/saveinvoice', requestInfo)
        const result = await response.json()
        return result;
    } catch (err) {
        return err;
...
export const SaveNextBill = async (data) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'workingschedule/savenextbill', requestInfo)
        const result = await response.json()
        return result
    } catch (err) {
        return err;
...
export const GetDataConfirmBusiness = async (fromdate, todate, actionResult) => {
    try {
        let access_token = await GetToken();
        const requestInfo = {
            method: 'GET',
        ...
        const responseJson = await fetch(URLDEFAULT + 'workingschedule/dataconfirm', requestInfo);
        const result = await responseJson.json();
        if (result.statusId === 200)
...
export const UploadConfirmBusiness = async (dataConfirm, actionResult) => {
    try {
        ...
        const responseJson = await fetch(URLDEFAULT + 'workingschedule/confirmtrips', requestInfo);
        const result = await responseJson.json();
        if (result.status === 200)
            actionResult(result)
        else
