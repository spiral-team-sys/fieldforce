import {
  _competitorId,
  URL_SELLOUT_DUPLICATE,
  URL_SELLOUT_DUPLICATE_CONFIRM,
  URL_SELLOUT_UPDATE_PHONE,
  URL_SELLOUT_EVIDENT,
  URL_UPLOAD_EVIDENT,
  URL_UPLOAD_PHOTOS,
  URLDEFAULT,
...
export async function getPhotoEvident(mItem) {
  let photoType =
    mItem.productId !== undefined
      ? 'EVIDENT_' + mItem.productName
      : 'EVIDENT_FULL';
  let mPhoto = [];
  await Store().then(async db => {
    const sql =
      'select * from photos' +
      ' where fileUpload = 0 and reportId = 0 and shopId = ' +
      mItem.shopId +
      " and photoType = '" +
      photoType +
      "'" +
...
export async function getProductNosell() {
  let nosell;
  await Store().then(async db => {
    let lstProduct = await SelectItems(db, 'products', ['productId'], {
      productCode: 'NOSELL',
    });
    if (lstProduct != null) {
...
export async function checkProductNosell(workinfo, IdNosell) {
  let nosell;
  await Store().then(async db => {
    const { res, err } = await SelectItems(db, 'sellOut', ['*'], {
      productId: IdNosell,
...
export async function getCompetiorByTracking() {
  ...
  await Store().then(async db => {
    const sql =
      "select 0 as id, '--chọn--' as name, 0 as orderBy union all select distinct type as id, division as name, 0 as orderBy from products where type = " +
      _competitorId +
      " and division <> 'null'" +
  ...
  await Store().then(async db => {
    const sql = `select 0 as id, '--chọn--' as name, 0 as type 
        ...
        WHERE productCode not in ('OOS','NOSELL') ${
          competitorId != undefined ? ' AND type=' + competitorId : ''
        }`;
    // console.log(sql, "s")
    const { res, err } = await exeSql(db, sql);
    res?.forEach(items => {
...
export async function getSubCategoryByProduct(itemDivision) {
  let lstData = [];
  await Store().then(async db => {
    lstData = await SelectItems(
      db,
      'products',
      ['DISTINCT subCatId as id', 'subCategory as name', 'categoryId', 'type'],
      itemDivision,
    );
  });
  return lstData.res;
...
export async function getListProduct(itemDivision) {
  ...
  await Store().then(async db => {
    lstData = await SelectItems(
      ...
      'products',
      [
        'productId as id,productName as name,type,division,categoryId,categoryName,subCatId,subCategory',
      ],
      itemDivision,
    );
  });
...
export async function reportSellOut_SAVE(item) {
  await Store().then(async db => {
    const { res, err } = await exeSql(
      db,
      'select * from sellOut where upload=0 and workId=' +
        item.workId +
        ' and productId=' +
        item.productId +
        ' and competitorId=' +
    ...
    if (res.length > 0) {
      await exeSql(
        db,
        'update sellOut set quantity=(quantity+' +
          item.quantity +
          ')' +
          ' where workId=' +
          item.workId +
  ...
  await Store().then(async db => {
    ...
    if (res !== undefined && res.length > 0) {
      await UpdateItem(
        db,
        sellOut.tableName,
        { quantity: res[0].quantity + item.quantity },
        { sellId: res[0].sellId },
      );
    } else {
...
export const uploadTrackingSellOut = async (
  lstRes,
  workinfo,
  finish,
  error,
...
export const GetListSelloutVerify = async (month, year) => {
  try {
    ...
    const response = await fetch(
      URLDEFAULT + 'sellouts/listselloutverify',
      requestInfo,
    );
...
export const SendSelloutVerify = async (
  itemUpload,
  listPhoto,
  type,
  noteCancel,
  ...
  try {
    ...
    const response = await fetch(
      URLDEFAULT + 'sellouts/senditemselloutverify',
      requestInfo,
    );
    const result = await response.json();
    return result;
...
export const SendAllVerify = async (
  itemUpload,
  listPhoto,
  type,
  noteCancel,
  ...
  if (type == 'submit') {
    if (itemUpload.status == 4 && res.length == 0) {
      const result = await SendSelloutVerify(
        itemUpload,
        listPhoto,
        type,
        noteCancel,
...
export const getInstoreShareByShop = async dataFilter => {
  try {
    ...
    const response = await fetch(
      URLDEFAULT + 'sellouts/getinstoreshare',
      requestInfo,
    );
    const result = await response.json();
    if (result.data) {
...
export const sendInstoreShareByShop = async (itemUpload, actionResult) => {
  try {
    ...
    const response = await fetch(
      URLDEFAULT + 'sellouts/sendinstoreshare',
      requestInfo,
    );
    const result = await response.json();
    actionResult(result);
