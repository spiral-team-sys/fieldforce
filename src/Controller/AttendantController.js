...
const SyncFromServer = async (workinfo) => {
    const result = await ATTENDANT_API.onlineAttendant(workinfo.shopId, workinfo.workDate)
...
const UpdateAttendanceLocal = async (dataOnline) => {
    await dataOnline?.forEach(async item => {
...
const GetAttendant = async (shopinfo, workDate, actionResult) => {
    const sql = `SELECT *  FROM ${photos.tableName} WHERE reportId=1 AND shopId=${shopinfo.shopId} AND photoDate=${workDate} ORDER BY photoType`
    const { res } = await QueryStringSql(sql)
    actionResult && actionResult(res)
    return res || [];
}
const CreateTemplateAttendance = (workinfo, locationInfo, photoList = [], numberAtt, actionResult) => {
    var template = [];
    var dataSwiper = []
    if (workinfo.shopId) {
        for (let i = 0; i < numberAtt; i++) {
            if (photoList.length > i) {
                template.push(photoList[i])
...
const GetOTSummary = async (shopId, actionResult) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + "attendants/otsummary", requestInfo)
...
export const AttendantController = { GetAttendant, SyncFromServer, UpdateAttendanceLocal, countAttShopNotFinish, GetOTSummary, UploadAttendantError, checkErrorAttendant, checkHaveAttendant, CreateTemplateAttendance, DataWaysFromLocation, DataLocationFromAddress, getShopNotFinish }
