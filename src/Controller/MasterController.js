import { exeSql, QueryStringSql, SelectItemsClause, Store } from "../Core/SqliteDbContext";
...
export async function InsertMasterList(db, MasterList) {
    ...
    await MasterList.forEach(Item => {
        db.transaction(tx => {
            tx.executeSql("INSERT INTO MasterList(ListCode,Code,Id,Name,Ref_Id,Ref_Code,IsLock,isColor,NumberValue,OrderBy) VALUES(?,?,?,?,?,?,?,?,?,?)",
                [Item.ListCode, Item.Code, Item.Id, Item.Name, Item.Ref_Id, Item.Ref_Code, Item.IsLock, Item.isColor, Item.NumberValue, Item.OrderBy]);
        });
    });
}
export async function InsertMobileKPIList(db, MobileKPIList) {
    ...
    await MobileKPIList.forEach(Item => {
        db.transaction(tx => {
            tx.executeSql("INSERT INTO MobileKPIList(KPIId,Function,ListCode,Name,isLock,OrderBy,RefId,RefName) VALUES(?,?,?,?,?,?,?,?)",
                [Item.KPIId, Item.Function, Item.ListCode, Item.Name, Item.isLock, Item.OrderBy, Item.RefId, Item.RefName]);
        }).then((result) => {
            //console.log(result);
        });
    });
...
export async function getMenuList() {
    ...
    await Store().then(async db => {
        const sql = "SELECT kpiId,function,listCode,name,isLock,orderBy,refId,refName,refCode,0 as upload FROM mobileKPIList WHERE listCode='KPI' ORDER BY orderBy"
...
export async function getPhotosDisplayBeko() {
    ...
    await Store().then(async db => {
        const sql = 'Select m.*'
            + " FROM masterList AS m WHERE listCode = 'CAMERA_DISPLAY'"
...
export async function getListHome() {
    let data = []
    await Store().then(async db => {
        const sql = "select * from masterList where listCode='Home' order by orderBy"
        const { res, err } = await exeSql(db, sql)
        data = res;
    })
    return data;
...
export async function GET_ConfigSellOut(isWorking, actionReulst) {
    let lst = [];
    await Store().then(async db => {
        const sql = "SELECT * FROM masterList WHERE listCode='SELLOUT_CONFIG'"
