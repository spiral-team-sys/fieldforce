import { Store, exeSql, InsertItems, DeleteItem, UpdateItem, QueryStringSql, Insert } from "../Core/SqliteDbContext";
...
export const getConfigSellIn = async (actionResult) => {
    await Store().then(async db => {
        const sql = "SELECT * FROM " + masterList.tableName + " WHERE listCode='SELLIN_CONFIG' ORDER BY orderBy";
...
export const getListDealer = async () => {
    let lst = [];
    await Store().then(async db => {
        const sql = "select dealerId as id,dealerName as name from dealer order by dealerName";
        const { res, err } = await exeSql(db, sql);
        // console.log(res, err);
        lst = res
    });
...
export const getCompetitorByProduct = async () => {
    ...
    await Store().then(async db => {
        ...
        if (AppNameBuild === psvApp) {
            sql = "select competitorId as id, competitorName as name from competitor ";
        }
        const { res, err } = await exeSql(db, sql);
        lst = res
    });
...
export const getCategoryByProduct = async () => {
    let lst = [];
    await Store().then(async db => {
        const sql = "select categoryId as id, categoryName as name,type from products WHERE productCode not in ('OOS','NOSELL')"
...
export const getSubCategoryByProduct = async () => {
    let lst = [];
    await Store().then(async db => {
        const sql = "select subCatId as id, subCategory as name,categoryId,type from products WHERE productCode not in ('OOS','NOSELL')"
            + " and type = " + _competitorId
...
export const getSegmentByProduct = async () => {
    let lst = [];
    await Store().then(async db => {
        const sql = "select segmentId as id, segment as name,subCatId,categoryId,type from products WHERE productCode not in ('OOS','NOSELL')"
            + " and segmentId>0 and type = " + _competitorId
...
export const getSubSegmentByProduct = async () => {
    let lst = [];
    await Store().then(async db => {
        const sql = "select subSegmentId as id, subSegment as name, segmentId,subCatId,categoryId,type from products WHERE productCode not in ('OOS','NOSELL')"
            + " and type = " + _competitorId
...
export const getAllDataConfig = async (actionResult) => {
    ...
    actionResult(lstDealer, lstCompetitor, lstCategory, lstSubCategory, lstSegment, lstSubSegment, lstProducts)
}
// Get List SellIn
...
export const dataUploadSellIn = async (mWork) => {
    let dataUpload = []
    await Store().then(async db => {
        const sql = "select * from sellIn as s where s.shopId=" + mWork.shopId + " and s.workDate=" + mWork.workDate
...
export const uploadSellIn = async (mWork, dataSellIn, actionResult) => {
    ...
    await fetch(URLDEFAULT + 'sellin/upload', {
        ...
        if (responseJson.status == 200) {
            ...
            QueryStringSql(`UPDATE ${taskList.tableName} set taskDone=1 WHERE shopId=${mWork.shopId} and reportId=${mWork.reportId}`)
...
export const doneUploadSellIn = async (mWork) => {
    await Store().then(async db => {
        await UpdateItem(db, 'sellIn', { isUploaded: 1 }, { shopId: mWork.shopId, workDate: mWork.workDate })
    })
}
// By Employee
