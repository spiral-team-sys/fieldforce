...
export async function uploadConfirmPOP(
  OrderNo,
  OrderList,
  resPhotos,
  finish,
...
export async function updateWarehouse(POPlist, finish, errorAct) {
  ...
  try {
    ...
    POPlist.map(OrderInfo => {
      if (
        OrderInfo.damagedInWarehouse !== null &&
        OrderInfo.damagedInWarehouse !== 0 &&
        OrderInfo.damagedInWarehouse !== ''
      ) {
        let itemP = {
          WareHouseId: OrderInfo.wareHouseId,
          POPId: OrderInfo.popId,
...
export async function uploadOrderPOP(
  POPlist,
  wareHouseId,
  uuid,
  Address,
  ...
  try {
    ...
    POPlist.map(OrderInfo => {
      if (
        OrderInfo.UserInput !== null &&
        OrderInfo.UserInput !== 0 &&
        OrderInfo.UserInput !== ''
      ) {
        let itemP = {
          WarehouseId: OrderInfo.wareHouseId,
  ...
  await Store().then(async db => {
    ...
    if (lstHave.length > 0) {
      UpdateItem(
        db,
        'installPOP',
        { reasonId: reasonInfo.Id, reasonName: reasonInfo.name },
        {
          workId: workInfo.workId,
          shopId: workInfo.shopId,
          popId: popInfo.popId,
...
export async function uploadPOPSHOP(POPlist, workInfo, finish, errorAct) {
  ...
  try {
    ...
    POPlist.map(OrderInfo => {
      if (
        OrderInfo.installQuantity !== null &&
        OrderInfo.installQuantity !== 0 &&
        OrderInfo.installQuantity !== '' &&
        OrderInfo.installQuantity !== 'null'
      ) {
        let itemP = {
          wareHouseId: OrderInfo.wareHouseId,
...
export const POPTotalWarehouse = async (warehouseId, actionResult) => {
  try {
    ...
    const response = await fetch(
      URLDEFAULT + 'pop/total/warehouse',
      requestInfo,
    );
    if (response.status === 200) {
      const result = await response.json();
