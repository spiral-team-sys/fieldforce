...
export async function getLstAccess() {
  ...
  await Store().then(async db => {
    const sql =
      'Select itemId as id, itemNameVN as name, sp.* FROM shopProfile sp';
    // + ' ORDER BY sortList';
...
export async function getLstAccessResult(workId) {
  ...
  await Store().then(async db => {
    const sql =
      'Select itemId as id, itemNameVN as name, spr.*  FROM shopProfileResult as spr WHERE itemId IS NOT NULL' +
      ' AND workId=' +
      workId +
...
export async function getLstAccessUpload(workId) {
  ...
  await Store().then(async db => {
    const sql =
      'Select itemNameVN as name, spr.*  FROM shopProfileResult as spr WHERE itemId IS NOT NULL AND upload = 0' +
      ' AND workId=' +
      workId +
...
export async function getLstCat() {
  ...
  await Store().then(async db => {
    const sql =
      'Select sp.categoryId as id, sp.categoryName as name FROM shopProfile AS sp GROUP BY sp.categoryId,sp.categoryName';
    // + ' ORDER BY sortList';
    const { res, err } = await exeSql(db, sql);
    lst = res;
  });
...
export async function getLstSubCat(catName) {
  ...
  await Store().then(async db => {
    const sql =
      'Select sp.shopProfileId as id, sp.categoryType as name, sp.* FROM shopProfile AS sp' +
      " WHERE sp.categoryName='" +
      catName +
      "'" +
...
export async function deleteNoteAccess(item, note, workId) {
  await Store().then(async db => {
    const sql = `UPDATE ${shopProfileResult.tableName}
            SET note = ${note}
...
export async function updateItemAceess(ip) {
  await Store().then(async db => {
    ...
    await UpdateItem(db, 'shopProfileResult', itemUpdate, {
      itemId: ip.itemId,
      workId: ip.workId,
    });
  });
