...
import {
  deleteHistoryDisplayByShop,
  getHistoryDisplayByShop,
...
export const downloadAll = async actionResult => {
  ...
  if (connected) {
    try {
      ...
      if (hasUnauthorizedResult(data, dataHistoryDisplay)) {
        await actionResult(
          'Phiên đăng nhập đã hết hạn, vui lòng đăng nhập lại.',
        );
        return;
      ...
      await PostDeviceInfo();
      await Promise.all([data, dataHistoryDisplay, countProduct]).then(
        async result => {
...
export const DeleteTaskList = async TableInfo => {
  try {
    await QueryStringSql(
      `DELETE FROM ${TableInfo.tableName} WHERE reportDate IS NULL OR reportDate=${TODAY}`,
    );
  } catch (err) {
