import { URL_WORKINGPLAN_GETLIST, URL_WORKINGPLAN_CHANGE, URL_GET_SHIFTTYPE, URL_WORKINGPLAN_CHANGESHIFT, URL_REGISTERPLAN_GETLIST, URL_REGISTERPLAN_LISTWEEK, URL_REGISTERPLAN_COPY, URL_GET_LISTWEEK, URL_WORKINGPLAN_UPLOAD_IMAGE, URL_REGISTERPLAN_SAVE, URLDEFAULT, } from '../Core/URLs';
...
export async function V2_REGISTERPLAN_GetByWeek(fromDate, toDate, actionResult) {
    let isNetwork = await checkNetwork();
    if (!isNetwork) {
        alert("Không có kết nối mạng, vui lòng kiểm tra lại kết nối sau đó thử lại.");
        return;
...
export async function V2_REGISTERPLAN_GetCopyPlan(fromDate, toDate, actionResult) {
    let isNetwork = await checkNetwork();
    if (!isNetwork) {
        alert("Không có kết nối mạng, vui lòng kiểm tra lại kết nối sau đó thử lại.");
        return;
...
export async function REGISTERPLAN_SavePlan(weekByYear, PlanChange, actionResult) {
    let isNetwork = await checkNetwork();
    if (!isNetwork) {
        alertWarning("Không có kết nối mạng, vui lòng kiểm tra lại kết nối sau đó thử lại.");
        return;
    ...
    PlanChange.forEach(i => {
        let infoPlan = {
            ...
            "ShopId": i.shopId,
            "WorkDate": i.dateSave,
            "ShiftType": i.shiftChange !== null && i.shiftChange.length > 0 ? i.shiftChange : i.shiftType,
            "Note": i.notes
        }
...
export async function PLAN_uploadData(lstWorkingPlan, actionResult, isProgress) {
    let isNetwork = await checkNetwork();
    if (!isNetwork) {
        alertWarning("Không có kết nối mạng, vui lòng kiểm tra lại kết nối sau đó thử lại.");
        return;
...
export async function PLAN_ChangeShift(data, actionResult) {
    ...
    try {
        ...
        await fetch(URLDEFAULT + 'workingplan/pg/changeshift', {
            ...
            actionResult({ msg: responseJson.messeger, status: responseJson.status });
        }).catch(error => {
...
// SR 
export async function SR_PLAN_ChangeShift(lstWorkingPlan, actionResult, isProgress) {
    let isNetwork = await checkNetwork();
    if (!isNetwork) {
        alertWarning("Không có kết nối mạng, vui lòng kiểm tra lại kết nối sau đó thử lại.");
        return;
...
export async function SR_PLAN_GETLIST(data, actionResult) {
    ...
    try {
        ...
        await fetch(URLDEFAULT + 'workingplan/plansr', {
            ...
            actionResult(responseJson.table1, responseJson.table, responseJson.table2 || [])
        });
    } catch (error) {
        actionResult([], []);
    }
...
export const PLANSR_ACTION = async (data, employeeSend) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'workingplan/sraction', requestInfo);
...
export const GetDataConfirmBySR = async (data, resultAction, errorAction) => {
    ...
    try {
        ...
        await fetch(URLDEFAULT + 'workingplan/confirmsr', {
            ...
            resultAction(responseJson.table1, responseJson.table, responseJson.table2)
        });
    } catch (error) {
        errorAction(error)
    }
...
export const ACTION_CONFIRM_SR = async (dataPlan, listEmployees, resultAction) => {
...
export const ACTION_CONFIRM_PG = async (dataPlan, listEmployees, resultAction) => {
    let jsonBody = JSON.stringify(dataPlan);
    try {
        const token = await Token();
...
// History Confirm
export const GetHistoryConfirm = async (typeConfirm, childEmployee, month, year, resultAction) => {
    let isNetwork = await checkNetwork();
    if (!isNetwork) {
        alert("Không có kết nối mạng, vui lòng kiểm tra lại kết nối sau đó thử lại.");
        return;
    ...
    try {
        ...
        await fetch(URLDEFAULT + 'history/confirmplan', {
            ...
            resultAction(responseJson.data.table, responseJson.data.table1, responseJson.data.table2)
        });
    } catch (error) {
        console.log(error)
    }
...
export const GetConfigPlan = async (fromDate, toDate, resultAction) => {
    ...
    try {
        ...
        const response = await fetch(URLDEFAULT + 'workingplan/configplan', requestInfo)
        const result = await response.json()
