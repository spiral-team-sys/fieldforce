import { Store, exeSql, UpdateItem, QueryStringSql, Insert, exeSqlNoQuery } from "../Core/SqliteDbContext";
import { _competitorId, _competitorName, AppNameBuild, psvApp, URLDEFAULT, URL_DOWNLOAD_HISTORY_DISPLAYCOMPETITOR } from "../Core/URLs";
import { All_Select } from "../Component/common";
import { GetToken, ToastError, ToastSuccess, Token } from "../Core/Helper";
import Moment from 'moment';
import { display, displayAudit, displayByShop, displayCompetitor, historyDisplay, mobileRaw, photos, posmByShop, surveyDisplayItem } from '../Core/TableLocal';
import UploadController, { uploadServer } from "./UploadController";
import { auditDisplayItems, displayTarget, masterList, products } from "../Core/Table";
import AsyncStorage from "@react-native-async-storage/async-storage";
import { insertRawReport } from "./ReportController";
export async function uploadDisplayCompetitor(resDisplay, resPhotos, workinfo, finish, errorAct) {
...
export async function getCompetiorByTracking() {
    ...
    await Store().then(async db => {
        const sql = "select distinct type as value, division as name, 0 as orderBy from products where type = " + _competitorId
            + " and division <> 'null' "
            + (AppNameBuild == psvApp ? " union all select 0, 'Đối thủ', 1 order by orderBy" : "")
...
export async function getSubCatProduct(competitor, categoryName) {
    let lst = [];
    await Store().then(async db => {
        const sql = "select 0 as id, '" + All_Select + "' as name union all"
            + " select distinct subCatId as id, subCategory as name FROM products AS p WHERE productCode not in ('OOS','NOSELL') and category='" + categoryName + "'"
...
export async function dataDisplayResult(mWORK, competitorName, isInput) {
    ...
    if (competitorName !== null && competitorName == _competitorName) {
        ...
        await Store().then(async db => {
            const sql = "select * from displayCompetitor where workId = " + mWORK.workId;
            const { res, err } = await exeSql(db, sql);
            if (res.length < 1) {
...
export async function setDataDisplayResult(mWORK) {
    await Store().then(async db => {
        const sql = "insert into display(workId,productId,division,categoryId,subCatId,subCategory,quanity,price,quanityDisplay,quantitySuggest)"
            + " select " + mWORK.workId + ",p.productId,p.division,p.categoryId,p.subCatId,p.subCategory, ifnull(h.quanityStock,null), ifnull(h.rrp,null), ifnull(h.display,null), ifnull(h.quanitySuggest,null)"
            + " from products as p"
            + " left join displayHistory as h on p.productId = h.productId and h.shopId = " + mWORK.shopId
...
export async function setDataDisplayCompetitor(mWORK) {
    await Store().then(async db => {
        const sql = "insert into displayCompetitor(workId,workDate,divisionId,categoryId)"
            + " select " + mWORK.workId + "," + mWORK.workDate + ",t.competitorId,t.categoryId"
...
export async function getDataDisplayResult(mWORK, competitorName, isInput) {
    let mData = [];
    await Store().then(async db => {
        const sql = "select d.displayId,d.workId,d.productId,d.quanity,d.quanityDisplay,d.quantitySuggest,cast(d.price as varchar(100)) as price,"
            ...
            + " and d.division = '" + competitorName + "'"
            + (isInput ? " and (d.quanity is not null or d.price is not null or d.quanityDisplay is not null or d.quantitySuggest is not null)" : "")
...
export async function getDataDisplayCompetitorResult(mWORK, isInput) {
    let mData = [];
    await Store().then(async db => {
        const sql = "select d.workId,d.divisionId,d.categoryId,d.quantity,t.competitorName,d.displayComment,d.upload"
...
export async function insertDisplay(display) {
    await Store().then(async db => {
        const sql = "update display set quanity = " + display.quanity + ", price = " + display.price
            + " where workId = " + display.workId
            + " and productId = " + display.productId;
        await exeSql(db, sql);
    })
}
export async function insertDisplayBeko(display) {
    await Store().then(async db => {
        const sql = "update display set quanity = " + display.quanity + ", price = " + display.price + ", quanityDisplay = " + display.quanityDisplay + ", quanitySuggest = " + display.quanitySuggest
            + " where workId = " + display.workId
            + " and productId = " + display.productId;
        await exeSql(db, sql);
    })
...
export async function insertDisplayCompetitor(display) {
    await Store().then(async db => {
        const sql = "update displayCompetitor set quantity = " + display.quantity
            + " where workId = " + display.workId
            + " and divisionId = " + display.competitorId + " and categoryId = " + display.categoryId;
        await exeSql(db, sql);
    })
}
export async function getDisplayComment(workId, categoryId, strTable) {
    let mResult = null;
    await Store().then(async db => {
        const sql = "select distinct displayComment from " + strTable + " where workId = " + workId + " and categoryId = " + categoryId;
        const { res, err } = await exeSql(db, sql);
        if (res !== null)
            mResult = res[0].displayComment;
    })
    return mResult;
}
export async function insertNoteDisplay(display, strTable) {
    await Store().then(async db => {
        const sql = "update " + strTable + " set displayComment = '" + display.displayComment + "'"
            + " where workId = " + display.workId
            + " and categoryId = " + display.categoryId;
        await exeSql(db, sql);
    })
}
export async function clearDisplayData(mWORK, type) {
    let mData = [];
    await Store().then(async db => {
        const sql = "update display set quanity = null, price = null where workId = " + mWORK.workId;
        await exeSql(db, sql);
    });
...
export async function getDisplayCompetitorUpload(workId) {
    let lst = [];
    await Store().then(async db => {
        const sql = "Select * from displayCompetitor WHERE workId=" + workId + ' AND quantity != -1';
        const { res, err } = await exeSql(db, sql);
        lst = res;
    });
...
export async function getPhotoUploadByType(reportId, shopId, photoDate, photoType) {
    let lst = [];
    await Store().then(async db => {
...
const displayUpload = async (workinfo, uploadResult) => {
    ...
    const { res } = await QueryStringSql(sql);
    await UploadController.uploadServer(workinfo, res || [], (result) => {
...
export async function getListByCompetitorMD(workInfo) {
    ...
    let sql =
        `INSERT INTO displayCompetitor(workId,workDate,divisionId,division,categoryId, categoryName ,subCatId,subCategory,upload)
...
export async function getDisplayProduct(workinfo, competitorId = _competitorId, lstReport, dealerId = 0, detailProduct = null) {
    const isHaveDetail = detailProduct !== null ? 1 : 0
    await QueryStringSql(
        `
        INSERT INTO ${display.tableName}(workId,productId,quanity,price, fsmValue,quantityStock,quantitySuggest,displayArea,displayRef,upload,division,categoryId,subCategory,subCatId,mockupValue,productComment,posmComment,tagPOPId)
        SELECT ${workinfo.workId} , p.productId, h.hDisplay,h.hPrice,h.hFsmValue,h.hStock,h.hSuggest,h.hArea,p.category,0,p.division,p.categoryId,p.subCategory, p.subCatId,h.hMockup,h.hNoteProduct,h.hNotePOP,h.hTagPOPID
        FROM ${products.tableName} AS p
        LEFT JOIN ${historyDisplay.tableName} AS h ON p.productId = h.productId AND h.shopId = ${workinfo.shopId}
        ...
        (
            (${isHaveDetail}=1 AND instr('${detailProduct}',p.productId) > 0)
            OR
            (${isHaveDetail}=0 AND (${lstReport?.isByDealer || 0} <> 1 OR (${lstReport?.isByDealer || 0} = 1 AND instr(p.details,${dealerId || 0}) > 0)))
        )`
    );
    //
...
export async function getCateSubmit(workinfo) {
    const sql =
        `SELECT distinct d.categoryId, p.category, p.categoryName  
...
export async function updateItemDisplay(item, workinfo) {
    let sql = `
    UPDATE display
    SET quanity=${item.quanity === 0 ? 0 : (item.quanity !== '' ? item.quanity : null)},
        quantityStock=${item.quantityStock === 0 ? 0 : (item.quantityStock !== '' ? item.quantityStock : null)},
        quantitySuggest=${item.quantitySuggest === 0 ? 0 : (item.quantitySuggest !== '' ? item.quantitySuggest : null)},
        price=${item.price || null},
        fsmValue=${item.fsmValue || null},
        displayArea=${item.displayArea ? `'${item.displayArea}'` : null},
        tagPOPId=${item.tagPOPId === 0 ? 0 : (item.tagPOPId !== '' ? item.tagPOPId : null)},
        displayComment=${item.displayComment == null ? null : (`'${item.displayComment}'`)},
        posmComment=${item.posmComment == null ? null : (`'${item.posmComment}'`)},
        productComment=${item.productComment == null ? null : (`'${item.productComment}'`)}
...
export async function getPhotoByType(type, guid = null) {
    let sql = `SELECT s.* FROM ${photos.tableName} s WHERE photoType = '${type}' ${guid !== null ? `AND guid='${guid}'` : ''}`
    const { res } = await QueryStringSql(sql)
    return res || []
}
...
export const DataDisplaySituation = async (resDisplay, workinfo, finish, error) => {
    try {
        await uploadServer(workinfo, resDisplay, async (result) => {
            ToastSuccess(result.messager);
            if (result.statusId == 200) {
                await Store().then(async db => {
                    const sqlData = "Update surveyDisplayItem set upload=1 WHERE workId=" + workinfo.workId;
...
export async function uploadDisplaySurvey(resDisplay, workinfo, finish, error) {
    try {
        await uploadServer(workinfo, resDisplay, async (result) => {
            ...
            if (result.statusId == 200) {
                await Store().then(async db => {
                    const sqlData = "Update displayAudit set upload=1 WHERE workId=" + workinfo.workId;
                    await exeSqlNoQuery(db, sqlData);
...
export async function getCompetitorHistory(workinfo) {
    try {
        ...
        const response = await fetch(URL_DOWNLOAD_HISTORY_DISPLAYCOMPETITOR, requestinfo);
        ...
        if (dataHistoryCompetitor.statusId == 200) {
            await Store().then(async db => {
                await dataHistoryCompetitor.data.forEach(item => {
                    db.transaction(tx => {
                        tx.executeSql("INSERT INTO displayCompetitor(workId,workDate,divisionId, division,categoryId,categoryName,subCatId,subCategory,quantity,priceValue,netValue,fsmValue,modelName,isAddProduct,upload) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                            [workinfo.workId, workDateString, item.competitorId, item.competitorName, item.categoryId, item.category, item.subCateId, item.subCategory, item.hDisplay, item.hPrice, item.hNetValue, item.hFsmValue, item.hModelName, item.productId, 0]);
                    }).then((result) => {
...
export async function uploadDataCompetitorReport(lstDisplay, workinfo, finish, error) {
    // console.log(lstDisplay);
    try {
        await uploadServer(workinfo, lstDisplay, async (result) => {
            ToastSuccess(result.messager);
            if (result.statusId == 200) {
                await Store().then(async db => {
...
export const getCompetitorByModel = async (competitorId) => {
    ...
    Select DISTINCT type as id, marketName as name
            FROM ${products.tableName} WHERE productCode not in ('OOS', 'NOSELL') 
            ${competitorId === _competitorId ? ` AND type = ${competitorId}` : ``}
            ORDER BY type
    `
    const { res } = await QueryStringSql(sql);
...
export async function uploadDataDisplayByModel(lstDisplay, workinfo, finish, error) {
    // console.log(lstDisplay);
    try {
        await uploadServer(workinfo, lstDisplay, async (result) => {
            ToastSuccess(result.messager);
...
export const getHistoryDisplayByShop = async (shopId) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'display/historybyshop', requestInfo)
...
//
export const DisplayContext = { displayUpload, DisplayTargetGetList, DisplayGroup, DTUpdateItem, GetProductMore, AddMore, removeAdd, taskDone, checkFormMode }
