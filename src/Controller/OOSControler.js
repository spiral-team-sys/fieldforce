import { exeSql, Store, DeleteAll, Insert, QueryStringSql } from "../Core/SqliteDbContext";
...
export const getDataTabOOS = async (workId, actionResult) => {
    const sql = "SELECT DISTINCT p.categoryId AS tabId, p.categoryName AS tabName"
        + " FROM products AS p"
        + " LEFT JOIN oos AS o ON p.productId = o.productId AND o.workId=" + workId
...
export const getDataOOS = async (workinfo, actionResult) => {
    ...
    `
    const { res } = await QueryStringSql(sql)
    await res.map((i, idx) => { i.mainIndex = idx })
    await actionResult(res, res[0].oosFull == 1 ? true : false, res[0].isUploaded == 1 ? true : false)
}
