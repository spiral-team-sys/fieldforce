...
const PosmUpdate = async item => {
  const sql = `
    update ${posmByShop.tableName} SET displayValue=${
    item.displayValue
  },posmList='${item.posmList}',posmNote='${
    item.posmNote === null || item.posmNote === 'null' ? '' : item.posmNote
  }'
    WHERE id=${item.id}
    `;
  // console.log(sql);
...
const PosmNote = async (workinfo, note) => {
  const sql = `UPDATE ${posmByShop.tableName} SET posmNote=${
    note !== null ? "'" + note + "'" : null
  }
    WHERE shopId=${workinfo.shopId} AND reportDate=${workinfo.workDate}`;
  await QueryStringSql(sql);
