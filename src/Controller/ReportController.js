...
export const getDataPhotoByGUID = async (
  reportId,
  shopId,
  guid,
  actionResult,
...
export const saveJsonData = async (shopId, reportId, reportDate, data) => {
  ...
  if (res !== null && res.length > 0) {
    const updateList = _.map(res, e => {
      return { ...e, jsonData: JSON.stringify(data) };
    });
...
export const dataUploaded = async (shopinfo, reportId) => {
  ...
  sqlPhoto = await replaceQueryString(sqlPhoto, shopinfo, [
    'shopId',
    'auditDate',
  ]);
  ...
  for (let i = 0, lenData = jsonPhoto.length; i < lenData; i++) {
    let imageName = jsonPhoto[i].photoPath.slice(
      jsonPhoto[i].photoPath.lastIndexOf('/', jsonPhoto[i].photoPath.length) +
        1,
    );
...
export const dataUploaded_Realtime = async (itemUpload, shopinfo, reportId) => {
  const shopId =
    shopinfo.shopId === 0 ? 0 : shopinfo.shopId || itemUpload.shopId;
  let dataUpload = {
    shopId,
    reportId: reportId,
  ...
  sqlPhoto = await replaceQueryString(
    sqlPhoto,
    {
      ...shopinfo,
      shopId,
...
export const getPhotoByReport = async (shopinfo, reportId) => {
  ...
  let jsonPhoto = itemPhoto.res || [];
  for (let i = 0, lenData = jsonPhoto.length; i < lenData; i++) {
    jsonPhoto[i].photoPath = jsonPhoto[i].photoPath.slice(
      jsonPhoto[i].photoPath.lastIndexOf('/', jsonPhoto[i].photoPath.length) +
        1,
    );
  }
