...
export async function checkImageDisplay(mWORK) {
    ...
    await Store().then(async db => {
        const sql = "select 4 as target, COUNT(*) as actual, 'Vui lòng chụp đầy đủ hình ảnh của " + _competitorName + " ('||COUNT(*)||'/4)' as alertMessage"
            + " from photos where shopId = " + mWORK.shopId
            + " and photoDate = " + mWORK.workDate
            + " and photoType like 'DISPLAY_" + _competitorName + "_%'"
...
export function doneSelloutReport(mWORK) {
    let isUploaded = 0;
    Store().then(async db => {
        const sql = "select distinct ifnull(upload,0) as upload from sellOut where workId = " + mWORK.workId;
...
export async function checkImageBeko(shopId, photoDate) {
    let mResult = '';
    await Store().then(async db => {
        const sql = "SELECT m.numberValue AS [target],"
            + "(SELECT COUNT(*) FROM photos AS p WHERE p.shopId=" + shopId + " AND p.photoDate=" + photoDate + " AND p.photoType like '333_'||m.code) AS actual,m.name"
