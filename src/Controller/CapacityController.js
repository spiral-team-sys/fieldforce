...
export const uploadItemCapacity = async (
  itemCapacity,
  workinfo,
  finish,
  error,
    ...
    await uploadServer(
      ...
      async result => {
        await QueryStringSql(
          `update ${capacity.tableName} set isUploaded=1 where shopId=${itemCapacity.shopId} and workDate=${itemCapacity.workDate}`,
        );
...
export const GetItemCapacity = async (item, actionResult) => {
  const { res } = await QueryStringSql(
    `SELECT * FROM ${capacity.tableName} WHERE shopId=${item.shopId} and workDate=${item.workDate}`,
  );
  actionResult(res[0] || {});
};
