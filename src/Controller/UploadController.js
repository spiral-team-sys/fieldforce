...
const DataPhoto = async workinfo => {
  try {
    ...
    if ((await res) !== null && (await res?.length) > 0) {
      ...
      return await {
        statusId: 500,
        messager: 'Không có dữ liệu hình ảnh để gửi',
      };
    }
  } catch (err) {
    console.log(err);
    ...
    await uploadServer(workinfo, res, async result => {
      ...
      if (result.statusId === 200) {
        await QueryStringSql(
          `Update Sellout set upload=1 WHERE WorkId=${workinfo.workId}`,
        );
    ...
    await uploadServer(
      ...
      async result => {
        ...
        if (result.statusId == 200) {
          await Store().then(async db => {
            const sqlData =
              'Update promotion set upload=1 WHERE workId=' + workinfo.workId;
    ...
    await uploadServer(
      ...
      async result => {
        ...
        if (result.statusId == 200) {
          await Store().then(async db => {
            const sqlData =
              'Update display set upload=1 WHERE workId=' + workinfo.workId;
            await exeSqlNoQuery(db, sqlData);
    ...
    await uploadServer(
      ...
      async result => {
        ...
        if (result.statusId == 200) {
          await Store().then(async db => {
            const sqlData =
              'Update ShelfPGResult set upload=1 WHERE workId=' +
              workinfo.workId;
            await exeSqlNoQuery(db, sqlData);
          });
        }
    ...
    await uploadServer(
      ...
      async result => {
        ...
        if (result.statusId == 200) {
          await Store().then(async db => {
            const sqlData =
              'Update shopProfileResult set upload=1 WHERE workId=' +
              workinfo.workId;
            await exeSqlNoQuery(db, sqlData);
          });
        }
    ...
    await uploadServer(
      ...
      async result => {
        ...
        if (result.statusId == 200) {
          await Store().then(async db => {
            const sqlData =
              'Update stockout set upload=1 WHERE workId=' + workinfo.workId;
            await exeSqlNoQuery(db, sqlData);
    ...
    await uploadServer(
      ...
      async result => {
        ...
        if (result.statusId == 200) {
          await Store().then(async db => {
            const sqlData =
              'Update trackingDetailResult set upload=1 WHERE workId=' +
              workinfo.workId +
              ' AND shopId=' +
              workinfo.shopId +
...
export const uploadServer = async (
  workinfo,
  dataInput,
  finishSrv,
  errorSrv,
  ...
  try {
    ...
    if (result.statusId === 200) {
      await QueryStringSql(
        `UPDATE ${photos.tableName} SET dataUpload=1 WHERE shopId=${workinfo.shopId} AND photoDate=${workinfo.workDate} AND reportId=${workinfo.reportId} AND (photoDesc<> 'VERIFY' OR photoDesc IS NULL)`,
      );
      await QueryStringSql(
  ...
  try {
    ...
    for (let index = 0; index < dataUpload.length; index++) {
      ...
      if (
        ...
        !(await RNFS.exists(decodeURIComponent(destinationPath)))
      ) {
      ...
      let FileAsBase64 = await RNFS.readFile(
        decodeURIComponent(destinationPath),
        'base64',
      );
      let dataItem = {
    ...
    typeof fileError === 'boolean'
      ? null
      : alertToast('Xảy ra lỗi khi gửi file : ' + error);
  }
};
