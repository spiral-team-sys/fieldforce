...
export async function getLstResShelfPG(workinfo) {
  ...
  await Store().then(async db => {
    const sql =
      'Select * ' + ' FROM ShelfPGResult' + ' Where workId=' + workinfo.workId;
  ...
  await getItemShelfPg(item.idItem, workId).then(async itemsHave => {
    await Store().then(async db => {
      if (itemsHave && itemsHave.length > 0) {
        let itemUpdate =
          type === 'SHELF'
            ? { shelfvalue: item.shelfvalue }
            : { pgvalue: item.pgvalue };
        await UpdateItem(db, 'ShelfPGResult', itemUpdate, {
          idItem: item.idItem,
          workId: item.workId,
        });
      } else {
