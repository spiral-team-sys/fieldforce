...
export const getListImageAudit = async () => {
  ...
  await Store().then(async db => {
    const sql =
      "select 0 as Id,'' as code,'Tất cả' AS name union all select Id,code,name from masterList where listCode = 'CameraAudit' order by Id";
...
export const getPhotoAudit = async workInfo => {
  ...
  await Store().then(async db => {
    const sql =
      'select * from photos where shopId = ' +
      workInfo.shopId +
      ' and photoDate=' +
      workInfo.workDate;
    const { res, err } = await exeSql(db, sql);
...
export const updateFSMValue = async (workInfo, value) => {
  await Store().then(async db => {
    ...
    exeSql(
      db,
      `UPDATE workResults SET fsmValue=${value} WHERE workId=${workInfo.workId}`,
    );
  });
};
export const dataPhotoUpload = async workInfo => {
  ...
  await Store().then(async db => {
    const sql =
      'select p.* from masterList as m' +
      ' left join photos as p on p.photoType=m.code ' +
      ' where p.dataUpload=0 and p.shopId=' +
      workInfo.shopId +
      ' and p.photoDate=' +
      workInfo.workDate +
      " and m.listCode='CameraAudit'";
    const { res, err } = await exeSql(db, sql);
...
export const checkTakePhotoTraining = async workinfo => {
  ...
  if (dataResult.length == 0) {
    strResult =
      'Vui lòng chụp tối thiểu 1 tấm hình Đào tạo cửa hàng trước khi làm bài kiểm tra';
  } else {
    strResult = '';
  }
  return strResult;
...
export async function doneUploadPhotoTraining(workinfo) {
  ...
  await Store().then(async db => {
    const sql =
      'UPDATE photos Set dataUpload = 1 WHERE shopId=' +
      workinfo.shopId +
      ' and photoDate =' +
      workinfo.photoDate;
    const { res, err } = await exeSql(db, sql);
