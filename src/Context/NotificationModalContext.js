...
import { clearNotificationModalHandler, registerNotificationModalHandler } from '../Component/Notification/Modal/NotificationModalRef';
import { clearPendingNotifications, getPendingNotifications } from '../Utils/notificationStorage';
...
export const NotificationModalProvider = ({ children }) => {
        ...
        try {
            await fetchDataNotify(async () => {
                ...
                if (result.statusId === 200 && result?.data?.length > 0) {
                    ...
                    const lastDismissedInAppId = await AsyncStorage.getItem(LAST_DISMISSED_INAPP_ID_KEY);
    ...
    return (
        <NotificationModalContext.Provider value={{ visible, notifyData, showNotificationModal, hideNotificationModal }}>
            {children}
