...
import { AUTH_SESSION_EXPIRED_EVENT, defaultSetting, GetEmployeeInfo } from '../Core/Helper';
...
export const AppProvider = ({ children }) => {
    ...
    //
    const logout = async () => {
        alertConfirm('Đăng xuất', 'Bạn muốn đăng xuất tài khoản này không?',
            async () => {
                ...
                await onLogout(deviceId, async (result) => {
                    if (result.statusId === 200) {
                        ...
                        toastError("Lỗi đăng xuất", "Lỗi đăng xuất khỏi hệ thống, vui lòng kiểm tra lại mạng")
                    }
                })
            }
        )
    ...
    useEffect(() => {
        const authExpired = DeviceEventEmitter.addListener(AUTH_SESSION_EXPIRED_EVENT, () => {
            setUserInfo({})
            setIsLoggedIn(false)
            dispatch(SetUserInfo({}))
        })
