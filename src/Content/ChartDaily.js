  ...
  constructor(props) {
    ...
    this.state = {
      ...
      description: {
        ...
        positionX:
          Platform.OS == 'ios'
            ? Dimensions.get('screen').width / 2 + 70
            : Dimensions.get('screen').width * 1.3,
        positionY:
          Platform.OS == 'ios' ? 160 : Dimensions.get('screen').height * 0.5,
      },
    };
  }
  async ChartSetup(category) {
    this.setState({ isProgress: true });
    await GetDataChartDaily(
      category !== undefined ? category : '',
      dataChart => {
        this.setState({
          Percent:
            dataChart.percent !== null && dataChart.percent !== undefined
              ? dataChart.percent + '%'
              : '0%',
          data: {
            dataSets: [
              {
                values: [
                  { value: dataChart.actual, label: 'Actual' },
                  { value: dataChart.target, label: 'Target' },
                ],
                label: '',
                config: {
  ...
  render() {
    return (
      ...
      >
        ...
        >
          {'Số bán ngày: ' + moment(new Date()).format('YYYY-MM-DD')}
        </Text>
