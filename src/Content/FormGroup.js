...
const FormGroup = ({
  ...
  const styles = StyleSheet.create({
    container: {
      borderWidth: editable && !nonBorder ? 1 : 0,
      marginBottom: 8,
  ...
  return (
    <View style={[styles.container, containerStyle]}>
      ...
      <View style={styles.inputContainer}>
        ...
        <TextInput
          ref={
            Array.isArray(inputRef) && index !== undefined
              ? inputRef[index]
              : inputRefFull
          }
          autoCorrect={false}
        ...
        {iconRight && (
          <TouchableOpacity
            style={{ padding: 5 }}
            onPress={rightFunc}
            disabled={disabledRightFunc}
          >
            <SpiralIcon
              type={iconType || 'font-awesome-5'}
              color={iconColorRight || appcolor.dark}
              style={[styles.icon, iconRightStyle]}
              name={iconRight}
              size={iconSizeRight || 18}
            />
          </TouchableOpacity>
        )}
        {Platform.OS == 'android' && useClearAndroid && editable && (
          <TouchableOpacity
            onPress={() =>
              onClearTextAndroid !== undefined && onClearTextAndroid('')
            }
          >
            <SpiralIcon
              color={iconColor || appcolor.dark}
      ...
      {isWarning &&
        titleWarning &&
        titleWarning !== null &&
        titleWarning.length > 0 && (
          <View style={styles.warningContent}>
            <Text style={styles.titleWarning}>
              * {titleWarning || 'Nội dung cảnh báo'}
            </Text>
          </View>
        )}
    </View>
