...
export const DetailOrderItem = ({ item, details, setDetails, route }) => {
  ...
  const [damageText, setDamageText] = useState(
    item.QuantityDamaged !== undefined ? item.QuantityDamaged : '',
  );
  const [pickText, setPickText] = useState(
    item.QuantityPickup !== undefined ? item.QuantityPickup : '',
  );
      ...
      >
        ...
        <View style={{ flexDirection: 'column', width: '70%' }}>
          <View style={{ flexDirection: 'row', width: '100%', padding: 5 }}>
            ...
            >
              {item.POPName !== undefined ? item.POPName : item.popName}
            </Text>
          </View>
          ...
          <View style={{ flexDirection: 'row', width: '100%', padding: 5 }}>
            ...
            >
              {item.TotalPOP !== undefined ? item.TotalPOP : item.totalPOP}
            </Text>
          </View>
          <View style={{ flexDirection: 'row', width: '100%', padding: 5 }}>
            <Text
          ...
          {!route.params.orderStatus.includes('chờ xác nhận') && (
            <View>
              <View style={{ flexDirection: 'row', width: '100%', padding: 5 }}>
                ...
                <TextInput
                  editable={
                    route.params.orderStatus.includes('thành công')
                      ? false
                      : true
                  }
                  keyboardType={'numeric'}
