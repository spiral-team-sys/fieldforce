...
class ImageReview extends PureComponent {
  ...
  async componentDidMount() {
    ...
    this.timeoutView = setTimeout(() => {
      this.goBackView();
    }, cameraConfig?.timeOutSave || 30000);
  }
  ...
  calculatorDistance() {
    var distance = getDistance(
      {
        latitude: this.state.photo.latitude,
        longitude: this.state.photo.longitude,
      },
      {
        latitude: this.props.shopinfo.shopLat,
        longitude: this.props.shopinfo.shopLong,
      },
    );
    return distance;
  }
  SavePhoto = async () => {
    ...
    if (!isNetwork) {
      ToastError(
        'Không có kết nối mạng, vui lòng kiểm tra lại kết nối sau đóngó thử lại.',
      );
      return;
    }
    //
    ...
    if (this.state.photo.photoType === EMPLOYEE_TYPE_PHOTO) {
      ...
      if (
        this.props.shopinfo.shopLat != null &&
        this.props.shopinfo.shopLong != null &&
        distanceLimit > 0
      ) {
        let distance = this.calculatorDistance();
        if (AppNameBuild === hpiApp) {
          if (distance > distanceLimit && countDate != 0) {
            // chu nhat
            ToastError(
              ` Vị trí của bạn so với vị trí cửa hàng lớn hơn ${distanceLimit}m vui lòng đến gần cửa hàng hơn, Khoảng cách giữa bạn và cửa hàng hiện là: \n\n${distance} Mét\nHay\n${
                distance / 1000
              } Kilomet`,
            );
            return;
          ...
          if (distance > distanceLimit && statusDistance === 2) {
            // chu nhat
            ToastError(
              ` Vị trí của bạn so với vị trí cửa hàng lớn hơn ${distanceLimit}m vui lòng đến gần cửa hàng hơn, Khoảng cách giữa bạn và cửa hàng hiện là: \n\n${distance} Mét\nHay\n${
                distance / 1000
      ...
      if (count == 1) {
        ...
        if (this.state?.jsonSetting?.exportAttendant === true) {
          if (Object.keys(this.props.userinfo).length === 0) {
            ToastError(
              `Lỗi khi lấy thông tin nhân viên, vui lòng tải lại dữ liệu!!!`,
            );
            await this.goBackView();
            return;
          }
        ...
        await OnTime(async () => {
          await this.setShowProgress(true);
          (await this.state?.jsonSetting?.exportAttendant) === true &&
            (await this.AttendantScreen());
          await InsertPhotosItem(this.state.photo);
      ...
      if (resUploadData) {
        await UpdatePhotosDataStatus(
          this.state.photo.shopId,
          this.state.photo.photoDate,
          this.state.photo.photoType,
        );
          ...
          async () => {
            ...
            if (shopContent.latitude == 0 || shopContent.longitude == 0) {
              await UpdateUrlPhotosShop(
                this.state.photo.shopId,
                this.state.photo.photoPath,
                this.state.photo.latitude,
                this.state.photo.longitude,
              );
    ...
    return (
      <View
        style={{
          ...
          backgroundColor: this.props.appcolor.black,
        }}
      >
        <HeaderCustom
          iconRight="save"
          ...
          rightFunc={
            ...
            (!this.state.showProgress ? () => this.SavePhoto() : null)
          }
        />
        <View style={{ flex: 1, justifyContent: 'center' }}>
          <ViewShot
            ...
            style={{
              width:
                this.state.widthViewShot === 0
                  ? '100%'
                  : this.state.widthViewShot,
              height:
                this.state.heightViewShot === 0
                  ? '80%'
                  : this.state.heightViewShot,
            }}
            options={{ format: 'jpg', quality: 1 }}
          >
            <ImageZoom ImagePath={this.state.photo.photoPath} />
            {this.state.photo.reportId === 1 &&
              this.state.jsonSetting?.exportAttendant === true && (
                <View
                  style={{
                    position: 'absolute',
                    width: '100%',
                ...
                >
                  <View style={{ paddingRight: 10, paddingTop: 5 }}>
                    ...
                    >
                      ...
                      } [${this.props.shopinfo?.shopCode?.toUpperCase()}]`}
                    </Text>
                  </View>
                  ...
                  <View style={{ paddingLeft: 10, paddingBottom: 5 }}>
                    ...
                    >
                      {`${(this.state.photo?.photoType % 2 == 0
                        ? `CheckIn`
                        : 'Check out'
                      ).toUpperCase()}`}
                      _{`${moment().format('DD/MM/YYYY, h:mm:ss A')}`}
                    </Text>
                    <Text
                      style={{
                        color: 'red',
