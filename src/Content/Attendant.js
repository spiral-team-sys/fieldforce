...
const Attendant = ({ navigation }) => {
  ...
  const { userinfo, shopinfo, workinfo, appcolor, isEdit } = useSelector(
  ...
  useEffect(() => {
    if (shopinfo !== undefined && workinfo !== undefined) {
      loadForm();
      const unSyncData = DeviceEventEmitter.addListener(
        SYNC_DATA_ATT,
        async () => {
          await loadData(false);
        },
      );
      const unSyncConfig = DeviceEventEmitter.addListener(
        SYNC_DATA_CONFIG,
        async () => {
          await loadForm();
        },
      );
      const unCameraNote = DeviceEventEmitter.addListener(
        CAMERA_NOTE,
        async note => {
      ...
      await fetch(URL_UPLOAD_ATTENDANT, {
        ...
        .then(responseJson => {
          if (responseJson.status == 100) {
            let isUpdate = UpdateAttendantStatus(
              photoInfo.shopId,
              photoInfo.photoDate,
              photoInfo.photoType,
            );
  ...
  const startCameraAttendant = async notesave => {
    ...
    if (photoInfoClick !== null) {
      ...
      else navigation.navigate('Camera', itemInfo);
    }
  };
  ...
  const checkAcceptToCheckOut = async () => {
    ...
    if (menulist.length > 0) {
      ...
      setMenuList(_tem);
      if (pTask === null || pTask.length === 0) return true;
      else {
        return false;
      }
    } else return true;
  ...
  countTime = async (timeLimit, timeClick, type) => {
    ...
    let diffMs = type === 'IN' ? timeClick - timeLimit : timeLimit - timeClick;
  ...
  const startCamera = async (e, photoInfo) => {
    ...
    const cameraConfig = JSON.parse(shopinfo?.cameraConfig || '{}');
    // Check report Online
    if (config.checkOnline == 1) await checkTaskListOnline();
    //
    const menuHome = await GetMenu(0);
    const _fistDone = await menuHome.filter(
      m => m.fistTask === 1 && m.taskDone === 0,
    );
    if (_fistDone.length > 0) {
      await MessageAcept(
        'Thông báo',
        `${_fistDone[0].menuNameVN} ${_fistDone[0].taskAlter}`,
        () => {
          navigation.navigate(_fistDone[0].pageName, {
            menuitem: _fistDone[0],
          });
        },
      );
      return;
    ...
    if (checkEditDisplay == 1 && PHOTOTYPE === CICO.co) {
      ...
      if (isEdit || isEditStore) {
        if (itemFilter.length > 0) {
          MessageAcept(
            'Thông báo',
            `Vui lòng thoát chế độ "Chỉnh sửa" trong ${itemFilter[0].name} trước khi Checkout`,
            () => {
              dispatch({ type: 'SET_KPI_INFO', payload: itemFilter[0] });
              navigation.navigate(`${itemFilter[0].pageName}`, {
                workinfo: workinfo,
                titlePage: itemFilter[0].name,
                reportId: itemFilter[0].kpiId,
              });
            },
          );
          return;
      ...
      if (noteReport === 1) {
        if (pendingTask === false) {
          ...
          menuList.map(
            it =>
              it.taskDone === 0 &&
              (str += `${str === '' ? it.menuNameVN : ', ' + it.menuNameVN}`),
          );
          dispatch({
            type: 'NOTE_ATTENDANT',
            payload: {
              type: ATTENDANT.NOTEREPORT,
              content: 'Lý do không làm báo cáo ' + str,
            },
          });
          return;
        }
    ...
    if (constraintCOMinute > 0 && PHOTOTYPE === CICO.co) {
      ...
      let timeConvert = moment(photoTimeIn, 'YYYYMMDDHHmmss').format(
        'YYYY-MM-DD HH:mm:ss',
      );
    ...
    if (constraintLate > 0 || constraintEarly > 0) {
      if (checkFirstShop == 1) {
              ...
              if (minuteBetween < 0) {
                Message(
                  ...
                  `Bạn đang "CHECK IN" trễ ${
                    ...
                    'HH:mm',
                  )}. Vui lòng nhập ghi chú và tiếp tục Check In.`,
                  () => {
                    setNoteAttType(0);
        ...
        await ATTENDANT_API.validTimeAttendant(
          ...
          async (itemTime, message) => {
            ...
            if (itemTime.isCheckTimeOut == 0) {
              const defaultTime = moment();
              const validTime = moment(shopinfo.timeOut).add(
                'minute',
                -constraintLate,
              );
              ...
              if (minuteBetween < 0) {
                Message(
                  ...
                  `Bạn đang "CHECK OUT" sớm ${
                    ...
                    'HH:mm',
                  )}. Vui lòng nhập ghi chú và tiếp tục chấm công`,
                  () => {
                    setNoteAttType(1);
        ...
        if (PHOTOTYPE === CICO.ci && e === 0 && constraintLate > 0) {
          ...
          let timeOutMiliLimit =
            timeOutMili - (constraintEarly || constraintLate) * 60 * 1000;
          if (
            (timeClick < timeOutMiliLimit && checkExactTime != 1) ||
    ...
    if (photoInfo.photoTime === null && isPermissLocation === true) {
      ...
      else navigation.navigate('Camera', itemPhotoInfo);
    } else {
  ...
  const renderItem = ({ item, index }) => {
    const onPress = () => {
      ...
      } else ToastError('Bạn không được chấm công ngày cũ');
    };
    return (
      <View
        key={'Asd' + index}
        style={{
          width: deviceSize.dwidth / 1.8,
          marginEnd: 7,
      ...
      >
        <TouchableOpacity onPress={onPress} style={{ flex: 1 }}>
          ...
          >
            {item.photoType % 2 == 0 ? `CheckIn` : 'Check out'}
          </Text>
          {item?.photoTime > 0 && (
            <Text
              style={{
    ...
    >
      ...
      >
        {attendantList !== null && attendantList.length > 0 ? (
          ...
          <View>
            <Text
              style={{ color: appcolor.danger, fontWeight: 'bold', padding: 3 }}
            >
              Ngày{' '}
              {moment(workinfo.workDate, 'YYYYMMDD').format('dddd D MMMM YYYY')}{' '}
              bạn không chấm công
            </Text>
          </View>
        )}
      ...
      <Modal visible={viewImage}>
        <MultipleShowImage
          key={'ShowItemImage'}
          listItem={swiperList}
          closeShowImage={() => setViewImage(false)}
