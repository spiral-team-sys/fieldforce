...
class Profile extends PureComponent {
  ...
  handleChangeForm = async (value, stateName, baseState) => {
    if (stateName === 'identityCardNumber') {
      const template = this.state.isUseCanCuoc
  ...
  async getEmployeeInfo() {
    ...
    await fetch(URL_EMPLOYEEINFO, {
      ...
      .then(async res => {
        ...
        const checkCanCuoc =
          empinfo?.identityCardNumber?.toString()?.length === 12;
        const identityCardNumber = formatByTemplate(
          empinfo.identityCardNumber,
          checkCanCuoc ? templateFormat.cancuoc : templateFormat.cmnd,
        );
        ...
        let province = [];
        try {
          province = JSON.parse(empinfo.province);
        } catch (e) {
          province = [];
        }
        ...
        this.setState({
          ...
          isUpdateCMND:
            !empinfo.cmndAfter || !empinfo.cmndBefore ? false : true,
          urlPage: urlPage,
          titlePage: titlePage,
            ...
            if (photoInfo?.photoType == 'CMND_BEFORE') {
              this.setState({
                employeeData: {
                  ...
                  cmndBefore: photoInfo.photoPath,
                },
              });
            } else {
              this.setState({
  ...
  onFilterBankBS = value => {
    const filterList = this.state.banks.filter(e => {
      const nameFilter = e.bankName
        ? e.bankName.toUpperCase()
        : ''.toUpperCase();
  ...
  onFilterProvinceBS = value => {
    const filterList = this.state.initProvinceList.filter(e => {
      const nameFilter = e.Province
        ? e.Province.toUpperCase()
        : ''.toUpperCase();
      return nameFilter.indexOf(value.toUpperCase()) > -1;
    });
    this.setState({
  ...
  onChangePress = async () => {
    const dataPassword = {
      ...
      confirmNewPassword: this.state.repass,
    };
  ...
  changeEmployeeInfo = async () => {
    ...
    if (identityCardNumber !== null && identityCardNumber !== '') {
      ...
      if (
        (await identityCardDate) === null ||
        (await identityCardDate) === ''
      ) {
    ...
    if (address !== null && address !== '') {
      if (
        (addressProvince == null && addressProvince !== PROVINCE) ||
        (addressDistrict == null && addressDistrict !== DISTRICT) ||
        (addressTown == null && addressTown !== TOWN)
      ) {
        alertWarning(
    ...
    if (permanentAddress !== null && permanentAddress !== '') {
      if (
        (permanentProvince == null && permanentProvince !== PROVINCE) ||
        (permanentDistrict == null && permanentDistrict !== DISTRICT) ||
        (permanentTown == null && permanentTown !== TOWN)
      ) {
        alertWarning(
          'Vui lòng chọn đầy đủ ' +
            PROVINCE +
      ...
      const empinfoUpdate = {
        ...
        photo:
          this.state.AvatarPath !== ''
            ? this.state.AvatarPath
            : this.state.employeeData.photo,
        accountId: this.state.employee.accountId,
      };
  ...
  checkInputEmployee = async employee => {
    ...
    //
    if (gender == null || gender == 0) strValid += '- Giới tính\n';
    if ((birthday || '').length == 0) strValid += '- Ngày sinh\n';
    if ((email || '').length == 0) strValid += '- Email\n';
    if ((mobile || '').length == 0) strValid += '- Số điện thoại\n';
    if ((permanentAddress || '').length == 0)
      strValid += '- Địa chỉ thường trú\n';
    if ((address || '').length == 0) strValid += '- Địa chỉ tạm trú\n';
    if ((accountNumber || '').length == 0) strValid += '- Số tài khoản\n';
    if (bankId == null || bankId == 0) strValid += '- Ngân hàng\n';
    if ((identityCardNumber || '').length == 0) strValid += '- CMND/CCCD\n';
    if ((identityCardDate || '').length == 0)
      strValid += '- Ngày cấp CMND/CCCD\n';
    if ((identityCardBy || '').length == 0) strValid += '- Nơi cấp CMND/CCCD';
    //
  ...
  onSwitchID = () => {
    ...
    this.setState({
      ...
      employeeData: {
        ...this.state.employeeData,
        identityCardNumber:
          checkCanCuoc !== this.state.isUseCanCuoc ? id || '' : '',
      },
    });
  };
          ...
          await assets?.forEach(async res => {
            ...
            if (this.state.photoTypeCMND === 'CMND_AFTER')
              this.setState({
                ...
                CMNDAfterPath: photoinfo.photoPath,
              });
            else
              this.setState({
                employeeData: {
                ...
                CMNDBeforePath: photoinfo.photoPath,
              });
          });
        }
      }
  ...
  getStringPhotoCMND = photoPath => {
    let result = '';
    if (photoPath.substring(0, 4) === 'file') result = photoPath;
    else if (photoPath.substring(0, 7) === 'content') result = photoPath;
    else result = URLDEFAULT + photoPath;
    return result;
  };
  handlerUploadData = () => {
  ...
  onIndexModalAddressChanged = index => {
    switch (index) {
      case 0:
        this.state.dataModalAddress.dataProvince = JSON.parse(
          JSON.stringify(this.state.dataModalAddress.dataProvincePrototype),
        );
        this.state.dataModalAddress.dataProvinceF = JSON.parse(
          JSON.stringify(this.state.dataModalAddress.dataProvincePrototype),
        );
        this.state.dataModalAddress.searchProvince = '';
        break;
      case 1:
        this.state.dataModalAddress.dataDistrict = JSON.parse(
          JSON.stringify(this.state.dataModalAddress.dataDistrictPrototype),
        );
        this.state.dataModalAddress.dataDistrictF = JSON.parse(
          JSON.stringify(this.state.dataModalAddress.dataDistrictPrototype),
        );
        this.state.dataModalAddress.searchDistrict = '';
        break;
      case 2:
        this.state.dataModalAddress.dataTown = JSON.parse(
          JSON.stringify(this.state.dataModalAddress.dataTownPrototype),
        );
        this.state.dataModalAddress.dataTownF = JSON.parse(
          JSON.stringify(this.state.dataModalAddress.dataTownPrototype),
        );
        ...
        case 1:
          ...
          this.setState({
            ...
            employeeData: {
              ...this.state.employeeData,
              permanentProvince:
                isPermanent && TypeSave == PROVINCE
                  ? dropdownItem.name
                  : this.state.employeeData.permanentProvince,
              addressProvince:
                !isPermanent && TypeSave == PROVINCE
                  ? dropdownItem.name
                  : this.state.employeeData.addressProvince,
            },
          });
        ...
        case 2:
          ...
          this.setState({
            ...
            employeeData: {
              ...this.state.employeeData,
              permanentDistrict:
                isPermanent && TypeSave == DISTRICT
                  ? dropdownItem.name
                  : this.state.employeeData.permanentDistrict,
              addressDistrict:
                !isPermanent && TypeSave == DISTRICT
                  ? dropdownItem.name
                  : this.state.employeeData.addressDistrict,
            },
          });
          this.onSwipe(indexAnswer);
          break;
        case 3:
          ...
          this.setState({
            employeeData: {
              ...this.state.employeeData,
              permanentTown:
                isPermanent && TypeSave == TOWN
                  ? dropdownItem.name
                  : this.state.employeeData.permanentTown,
              addressTown:
                !isPermanent && TypeSave == TOWN
                  ? dropdownItem.name
                  : this.state.employeeData.addressTown,
            },
          });
          break;
      }
    ...
    switch (nameField) {
      case 'searchProvince':
        if (value !== null && value !== undefined && value.length > 0) {
          dataFilter = this.state.dataModalAddress?.dataProvinceF.filter(i =>
      ...
      case 'searchDistrict':
        if (value !== null && value !== undefined && value.length > 0) {
          dataFilter = this.state.dataModalAddress?.dataDistrictF.filter(i =>
            i.name.toLowerCase().match(value.toLowerCase()),
          );
        } else {
      ...
      case 'searchTown':
        if (value !== null && value !== undefined && value.length > 0) {
          dataFilter = this.state.dataModalAddress?.dataTownF.filter(i =>
            i.name.toLowerCase().match(value.toLowerCase()),
          );
        } else {
    ...
    if (isConstraint == 1) {
      if (resulCheck.status == 500) {
        ...
        if (
          ...
          this.state.isUpdateAvatar === false ||
          this.state.isUpdateCMND === false
        ) {
          ToastError(
      ...
      >
        ...
        >
          ...
          >
                ...
                >
                  ...
                  this.state.photoNewest !== undefined ? (
                    <Image
                      ...
                      source={{ uri: this.state.photoNewest.photoPath }}
                    />
                    ...
                    <Image
                      ...
                      source={{
                        uri: this.state.employeeData.photo?.includes('uploaded')
                          ? URLDEFAULT + this.state.employeeData.photo
                          : this.state.employeeData.photo || null,
                      }}
                    />
                  ) : (
                    <Avatar
            ...
            {this.state?.employeeData?.showResign === 1 && (
              <TouchableOpacity
                onPress={() =>
                  this.props.navigation.navigate('employeeresigns', {
                    employeeInfo: info,
                    minDateResign: this.state.minDateResign,
                    selectPastDate: this.state.selectPastDate,
                  })
                }
                  ...
                  <Input
                    rightIcon={
                      <SpiralIcon
                        size={20}
                        name={
                          this.state.isShowPassOld ? 'eye' : 'eye-with-line'
                        }
                        type="entypo"
                        onPress={e =>
                          this.setState({
                            isShowPassOld: this.state.isShowPassOld
                              ? false
                              : true,
                          })
                        }
                    ...
                    secureTextEntry={this.state.isShowPassOld ? false : true}
                  ...
                  <Input
                    rightIcon={
                      <SpiralIcon
                        size={20}
                        name={
                          this.state.isShowPassNew ? 'eye' : 'eye-with-line'
                        }
                        type="entypo"
                        onPress={e =>
                          this.setState({
                            isShowPassNew: this.state.isShowPassNew
                              ? false
                              : true,
                          })
                        }
                  ...
                  <Input
                    rightIcon={
                      <SpiralIcon
                        size={20}
                        name={
                          this.state.isShowPassNew1 ? 'eye' : 'eye-with-line'
                        }
                        type="entypo"
                        onPress={e =>
                          this.setState({
                            isShowPassNew1: this.state.isShowPassNew1
                              ? false
                              : true,
                          })
                        }
              ...
              <View width="95%">
                ...
                <View>
                  <RenderFormGroup
                    ...
                    title="Mã nhân viên"
                    editable={false}
                    stateName="employeeCode"
                    baseState={'employeeData1'}
                    state={this.state}
                  />
                  <RenderFormGroup
                    index={-1}
                    title="Tên nhân viên"
                    editable={
                      AppNameBuild === nokiaApp || AppNameBuild === lgApp
                        ? false
                        : true
                    }
                    stateName="fullName"
                    baseState={'employeeData'}
                    state={this.state}
                  ...
                  <RenderFormGroup
                    ...
                    title="Giới tính"
                    editable={false}
                    stateName="genderNameVN"
                    baseState={'employeeData'}
                    state={this.state}
                    iconRight="venus-mars"
                    typeBS="GENDER"
                  ...
                  <RenderFormGroup
                    index={3}
                    title="Địa chỉ thường trú (Nhập số nhà,tên đường)"
                    placeholder={'Số nhà, tên đường'}
                    stateName="permanentAddress"
                    baseState={'employeeData'}
                    state={this.state}
                    handleChangeForm={this.handleChangeForm}
                    onSubmitEditing={this.onSubmitEditing}
                  ...
                  />
                  <RenderAddress
                    item={this.state.employeeData}
                    appcolor={appcolor}
                    onOpenModalAddress={this.onOpenModalAddress}
                    isPermanent={true}
                  />
                  <RenderFormGroup
                    index={4}
                    title="Địa chỉ tạm trú (Nhập số nhà,tên đường)"
                    placeholder={'Số nhà, tên đường'}
                    stateName="address"
                    baseState={'employeeData'}
                    state={this.state}
                  ...
                  >
                    ...
                    <Switch
                      ...
                      thumbColor={
                        this.state.isUseCanCuoc ? '#f5dd4b' : '#f4f3f4'
                      }
                      ios_backgroundColor="#3e3e3e"
                      onValueChange={this.onSwitchID}
                    />
                  </View>
                  <RenderFormGroup
                    index={6}
                  ...
                  <RenderFormGroup
                    ...
                    title="Nơi cấp"
                    editable={false}
                    stateName="identityCardBy"
                    baseState={'employeeData'}
                    state={this.state}
                    iconRight="map-marker-alt"
                    typeBS="PROVINCE"
                    handleDisplayBS={this.handleDisplayBS}
                    multiline={true}
                  />
              ...
              <View width="95%">
                ...
                <View style={{ paddingBottom: deviceHeight / 3 }}>
                  ...
                  <RenderFormGroup
                    ...
                    editable={false}
                    stateName="parentEmail"
                    baseState={'employeeData'}
                    state={this.state}
                    handleDisplayBS={this.handlerSendMail}
                    iconRight="paper-plane"
                  />
                  <RenderFormGroup
                    index={-1}
                    title="SĐT người quản lý"
                    editable={false}
        ...
        {this.state.showProgress === true && (
          ...
          >
            ...
            <Text style={{ color: '#007AFF' }}>Vui lòng chờ...</Text>
          </View>
        )}
        <ActionSheet
          ref={bottomSheet}
          ...
          indicatorColor={appcolor.success}
          defaultOverlayOpacity={0.3}
        >
          <View
            nestedScrollEnabled={true}
            style={{ width: deviceWidth, height: deviceHeight, padding: 12 }}
          >
            {this.state.currentBS?.type === 'DATE' ? (
              <RenderCalendarBS
                appcolor={appcolor}
                currentBS={this.state.currentBS}
                handleDisplayBS={this.handleDisplayBS}
        ...
        <Modal visible={this.state.visibleModalAddress} animationType="slide">
          ...
          >
            <Swiper
              ...
              onIndexChanged={this.onIndexModalAddressChanged}
            >
              <View
                key={1}
                style={{ flex: 1, backgroundColor: appcolor.light, padding: 8 }}
              >
                <View
                  style={{
                    flexDirection: 'row',
                ...
                >
                  <TouchableOpacity
                    onPress={() => this.onOpenModalAddress(false)}
                    style={{ padding: 15 }}
                  >
                    <SpiralIcon
                      type="font-awesome-5"
                ...
                <FormGroup
                  iconName="search"
                  editable={true}
                  value={this.state.dataModalAddress?.searchProvince || ''}
                  placeholder="Tìm kiếm..."
                  handleChangeForm={e =>
                    this.onFilterModalAddress(
                      e,
                      'searchProvince',
                      'dataProvince',
                    )
                ...
                />
                <FlatList
                  data={this.state.dataModalAddress?.dataProvince || []}
              ...
              >
                ...
                <FormGroup
                  iconName="search"
                  editable={true}
                  value={this.state.dataModalAddress?.searchDistrict || ''}
                  placeholder="Tìm kiếm..."
                  handleChangeForm={e =>
                    this.onFilterModalAddress(
                      e,
                ...
                />
                <FlatList
                  data={this.state.dataModalAddress?.dataDistrict || []}
                  keyExtractor={(_, index) => index.toString()}
                  renderItem={({ item, index }) => {
                    const indexAnswer = 2;
                    const answerAddressItem =
              ...
              >
                ...
                <FormGroup
                  iconName="search"
                  editable={true}
                  value={this.state.dataModalAddress?.searchTown || ''}
                  placeholder="Tìm kiếm..."
                  handleChangeForm={e =>
                    this.onFilterModalAddress(e, 'searchTown', 'dataTown')
                  }
                  stateName="search"
                />
                <FlatList
                  data={this.state.dataModalAddress?.dataTown || []}
                  keyExtractor={(_, index) => index.toString()}
                  renderItem={({ item, index }) => {
                    const indexAnswer = 3;
                    const answerAddressItem =
...
const RenderFormGroup = ({
  ...
  const valueItem =
    typeBS == 'DATE' || stateName == 'workingDate'
      ? Moment(state?.[baseState]?.[stateName]?.toString()).format('YYYY-MM-DD')
      : state?.[baseState]?.[stateName]?.toString() || '';
  return (
    <FormGroup
      title={title}
      value={valueItem}
      ...
      iconRight={isDisplayBS ? iconRight : null}
      rightFunc={isDisplayBS ? onRightFunc : null}
      multiline={multiline}
...
const RenderCalendarBS = ({
  ...
  return (
    <View
      style={{ flex: 1, backgroundColor: appcolor.light, overflow: 'hidden' }}
    >
      <View
        style={{
          padding: 10,
...
const RenderGenderBS = ({ appcolor, onSelect, handleDisplayBS }) => {
  ...
  return (
    ...
    >
      ...
      <View>
        <TouchableOpacity
          onPress={() => onSelectGender(2, 'Nam')}
          style={{ padding: 20 }}
        >
          <Text style={{ fontSize: 17, color: appcolor.dark }}>Nam</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => onSelectGender(1, 'Nữ')}
          style={{ padding: 20 }}
        >
  ...
  return (
    ...
    >
      ...
      <FormGroup
        ...
        editable={true}
        placeholder="Tìm kiếm..."
        handleChangeForm={onFilterContentBS}
        stateName="search"
      />
      <FlatList
        scrollEnabled={true}
        ...
        renderItem={({ item, index }) => {
          ...
          return (
            ...
            >
              <Text style={{ color: appcolor.dark, padding: 7 }}>
                {item.bankName}
              </Text>
              {isSelected && (
  ...
  return (
    ...
    >
      ...
      >
        ...
        <TouchableOpacity onPress={() => handleDisplayBS(false)}>
          <SpiralIcon type="font-awesome-5" name="times" size={22} />
        </TouchableOpacity>
      </View>
      <FormGroup
        iconName="search"
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => {
          ...
          return (
            <TouchableOpacity
              key={index}
              onPress={() => onSelectBank(item.Province)}
              style={{
                flexDirection: 'row',
                alignItems: 'center',
                justifyContent: 'space-between',
  ...
  return (
    ...
    >
      <TouchableOpacity
        ...
        onPress={() => onOpenModalAddress(true, 0, isPermanent, PROVINCE, item)}
        style={{
          flexDirection: 'row',
          alignItems: 'center',
          flex: 1.2,
      ...
      <TouchableOpacity
        disabled={isSubmitted}
        onPress={() => onOpenModalAddress(true, 1, isPermanent, DISTRICT, item)}
        style={{
          flexDirection: 'row',
          alignItems: 'center',
          flex: 1,
      ...
      <TouchableOpacity
        disabled={isSubmitted}
        onPress={() => onOpenModalAddress(true, 2, isPermanent, TOWN, item)}
