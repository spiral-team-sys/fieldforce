...
const KPIList = ({}) => {
  const { appcolor, shopinfo, workinfo } = useSelector(
  ...
  const onPressItem = async item => {
    await validateCheckIn().then(async valid => {
      if (
        valid.length === 0 &&
        item.fistTask === 1 &&
        shopinfo?.isAnotherRoute !== 1
      ) {
        ToastError(
          'Vui lòng check in trước khi làm báo cáo.',
          'Chấm công',
          'top',
        );
        return;
      ...
      if (item.pageName !== null) {
        if (item.pageName.includes('traninee')) {
          ...
          SheetManager.show('webview_kpi_sheet', {
            payload: {
              url: item.reportItem + appShare,
              title: item.menuNameVN,
            },
          });
        } else if (
          item.reportItem !== null &&
          item.reportItem.includes('https')
        ) {
          const shareInfo = {
            employeeId: employeeInfo.employeeId,
          ...
          SheetManager.show('webview_kpi_sheet', {
            payload: {
              url: item.reportItem + app_access,
              title: item.menuNameVN,
            },
          });
        } else {
  ...
  //
  useEffect(() => {
    const reload_workload = DeviceEventEmitter.addListener(
      'WORK_LOAD',
      LoadData,
    );
    LoadData();
    ...
    <View style={styles.mainContainer}>
      <View style={styles.headerContainer}>
        <TouchableOpacity
          ...
          activeOpacity={0.8}
          onPress={onChangeDisplayMenu}
        >
          <SpiralIcon
            type="ionicon"
            name={displayKpi === 0 ? 'grid-outline' : 'list-outline'}
            size={24}
            color={appcolor.light}
          />
          <Text numberOfLines={1} style={styles.title}>
            Báo cáo{' '}
            {moment(workinfo?.workDate, 'YYYYMMDD').format('ddd, DD/MM/YYYY')}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <ActionSheet
        id="webview_kpi_sheet"
        onBeforeShow={setTraineeURL}
        closable={false}
