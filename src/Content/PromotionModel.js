...
import { InsertItemsPromotion, updateItemsPromotion } from "../Controller/PromotionController";
...
export function PromotionModel({ Categories, Competitors, guiId, Closed, navigation, loaddata, loading, ItemSaved, loadHistory, listReport }) {
    ...
    const countPhoto = async () => {
        let lst = await getPhotosReportByGuiId(kpiinfo.kpiId, GuiID, workinfo.shopId, workinfo.workDate);
    ...
    const showALbum = async () => {
        const photos = await getPhotosReportByGuiId(kpiinfo.kpiId, GuiID, workinfo.shopId, workinfo.workDate);
    ...
    const takePhoto = async () => {
        ...
        await NativeCamera.cameraStart(item, (result) => { countPhoto(item, result) })
    };
    ...
    const Save = async () => {
        ...
        let fromdateConvert = parseInt(Moment(dataCalendar.startDate.toString()).format('YYYYMMDD')) || parseInt(Moment(itemShow.fromDate.toString()).format('YYYYMMDD'))

        let todateConvert = parseInt(Moment(dataCalendar.endDate.toString()).format('YYYYMMDD')) ||
        ...
        if (listReport.isCheckTitle == 1) {
            if (itemShow.titlePromotion === null || itemShow.titlePromotion === '' || itemShow.titlePromotion === undefined || itemShow.titlePromotion === 'undefined') {
        ...
        if (listReport.isCheckContent == 1) {
            if (itemShow.content === null || itemShow.content === '' || itemShow.content === undefined || itemShow.content === 'undefined') {
        ...
        if (listReport.isCheckImage == 1 && numPhoto == 0) {
            ToastError(`Vui lòng chụp hình ảnh chương trình, ít nhất ${listReport.limitPhoto || 1} tấm hình`);
            return;
        }
    ...
    const onSelectItem = (item, type) => {
        if (type == 'CATEGORY') {
            ...
            setItemShow({ ...itemShow, categoryId: item.id, categoryName: item.name })
        } else if (type == 'COMPE') {
            // setSelectedCompetitor(item);
            setItemShow({ ...itemShow, competitorId: item.id, competitorName: item.name })
        }
    }
    const RenderItemCate = ({ item }) => {
        return (
            <TouchableOpacity
                style={{
                    padding: 8, minWidth: deviceWidth * 0.2, justifyContent: 'center',
                    backgroundColor: itemShow.categoryId === item.id ? appcolor.primary : appcolor.surface,
                    marginBottom: 5, alignItems: 'center', borderRadius: 50, marginHorizontal: 5,
                }}
                onPress={() => { onSelectItem(item, 'CATEGORY') }}
            >
                <Text style={{ color: itemShow.categoryId === item.id ? appcolor.white : appcolor.dark }}>
                    {item.name}
                </Text>
            </TouchableOpacity>
        )
    ...
    const RenderItemCompe = ({ item, index }) => {
        return (
            <TouchableOpacity
                style={{
                    justifyContent: 'center', alignItems: 'center', borderRadius: 50, marginHorizontal: 5,
                    backgroundColor:
                        itemShow.competitorId === item.id ? appcolor.primary : appcolor.surface,
                    marginBottom: 5, padding: 10, minWidth: deviceWidth * 0.2,
                }}
                onPress={() => { onSelectItem(item, 'COMPE'); scrollOnPress(index) }}
            >
                <Text style={{ color: itemShow.competitorId === item.id ? appcolor.white : appcolor.dark }}>
                    {item.name}
                </Text>
            </TouchableOpacity>
        )
        ...
        <View style={{ flex: 1, backgroundColor: appcolor.light }}>
            ...
            <View style={{ margin: 10, flex: 1, backgroundColor: appcolor.light }}>
                <ScrollView showsVerticalScrollIndicator={false}                >
                    <View style={{ marginTop: 10, paddingBottom: 5, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ color: appcolor.dark, margin: 5, fontSize: 14, fontWeight: '600', }} >Hãng : </Text>
                        <FlatList
...

                    {
                        ...
                        <View>
                            <View style={{ flex: 1, flexDirection: "row", alignSelf: 'center' }}>
                                <View style={{ width: '100%', marginTop: 5, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                                    <Text style={{ color: appcolor.dark, fontSize: 14, fontWeight: '600', width: '10%' }}>Ngày</Text>
                                    <View style={{ width: '90%', flexDirection: "row", justifyContent: 'space-between', padding: 5 }}>
                                        <TouchableOpacity
                                            style={{ alignItems: 'center', width: '100%', padding: 8, backgroundColor: appcolor.surface, borderWidth: 0.5, borderColor: '#bbb', borderRadius: 10 }}
                                            onPress={() => selectItem()}
                                        >
                                            <Text style={{ color: appcolor.dark, fontSize: 14 }}>
                                                {Moment((dataCalendar.startDate || itemShow.fromDate)).format('DD/MM/yyyy') + ` - ` +
                                                    Moment((dataCalendar.endDate || (dataCalendar.startDate ? dataCalendar.startDate : itemShow.toDate))).format('DD/MM/yyyy')}
                                            </Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                            </View>
                            {
                                ...
                                <View style={{ marginTop: 5 }}>
                                    <FormGroup rightFunc={() => { }}
                                        ...
                                        handleChangeForm={text => setItemShow({ ...itemShow, titlePromotion: text })}
                                        onClearTextAndroid={() => setItemShow({ ...itemShow, titlePromotion: '' })}
                    ...
                    <View style={{ marginTop: 10 }}>
                        <FormGroup rightFunc={() => { }}
                            ...
                            handleChangeForm={text => setItemShow({ ...itemShow, content: text })}
                            onClearTextAndroid={() => setItemShow({ ...itemShow, content: '' })}
                            placeholderTextColor={appcolor.greydark}
                            editable
                        />
                    </View>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", width: '100%', marginTop: 10, marginBottom: 20 }}>
                        <Button
                            containerStyle={{ width: AppNameBuild !== psvApp ? '33%' : '49%', borderColor: 'gray', borderWidth: 1.0 }}
                            buttonStyle={{ height: 45, backgroundColor: appcolor.surface }}
                            onPress={e => { takePhoto() }}
                            icon={<Icon color={appcolor.dark} name='camera' type='ionicon' size={30} />}
                        />

                        {
                            (listReport.isHideUpload !== 1) &&
                            <Button
                                containerStyle={{ width: '33%', borderColor: 'gray', borderWidth: 1.0 }}
                                buttonStyle={{ height: 45, backgroundColor: appcolor.surface }}
                                onPress={e => { uploadFile() }}
                                icon={<Icon color={appcolor.dark} name='attach' type='ionicon' size={30} />}
                            />
                        }
                        <Button
                            containerStyle={{ width: AppNameBuild !== psvApp ? '33%' : '49%', borderColor: 'gray', borderWidth: 1.0 }}
            ...
            <ActionSheet ref={bottomSheet}
                containerStyle={{ backgroundColor: appcolor.grayLight, padding: 5 }} closeOnPressBack={true}
                gestureEnabled={true} indicatorColor={appcolor.primary} defaultOverlayOpacity={0.3}>
