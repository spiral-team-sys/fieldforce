...
export const HeaderCustom = ({ iconLeft, iconMiddle, iconRight, leftFunc, middleFunc, rightFunc, title, subTitle, titleRight, countNotify, isHome = false, rightType, leftType, middleType, disabled }) => {
    ...
    const styles = StyleSheet.create({
        mainContainer: {
            width: '100%', backgroundColor: isHome ? bgColorHeader : DEFAULT_COLOR,
            justifyContent: 'center', alignItems: 'center',
            minHeight: 52, paddingVertical: 4,
            position: 'relative', zIndex: 1000, elevation: 12
        },
        headerRow: { width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
        leftView: { width: '20%', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', paddingLeft: 8 },
        leftAction: { padding: 10, paddingRight: 15, borderRadius: 20, width: 45 },
        middleView: { width: '60%', display: 'flex', justifyContent: 'center', alignItems: 'center' },
        middleAction: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
        titleMiddle: { textAlign: 'center', fontSize: 15, fontWeight: fontWeightBold, color: colorContent },
        subTitleMiddle: { textAlign: 'center', fontSize: 12, color: colorContent, fontStyle: 'italic' },
        rightView: { width: '20%', display: 'flex', paddingRight: 12, justifyContent: 'center', alignItems: 'flex-end' },
        rightAction: { position: 'relative', padding: 10, paddingLeft: 15, flexDirection: 'row', alignItems: 'center' },
        titleRight: { color: appcolor.dark, fontSize: 15, fontWeight: fontWeightBold },
        badgeContainer: { position: 'absolute', top: -8, right: -16 },
        badgeStyle: { width: 25, borderRadius: 25, backgroundColor: bgColorBadgeMessage, borderColor: bgColorBadgeMessage },
            ...
            <View style={styles.headerRow}>
                ...
                <View style={styles.middleView}>
                    {typeof middleFunc === "function" ? (
                        ...
                        <Text numberOfLines={2} style={styles.titleMiddle}>{title}</Text>
                    )}
                    {(subTitle || null) !== null && <Text style={styles.subTitleMiddle}>{subTitle}</Text>}
                </View>
                <View style={styles.rightView}>
                    {typeof rightFunc === "function" && (
                        <TouchableOpacity disabled={disabled} onPress={rightFunc} style={styles.rightAction}>
                            ...
                            {typeof titleRight === 'string' && <Text style={styles.titleRight}>{titleRight}</Text>}
                        </TouchableOpacity>
                    )}
                </View>
            </View>
