...
import { View, TouchableOpacity, StyleSheet, Dimensions, Text, Platform, Linking, BackHandler, DeviceEventEmitter } from 'react-native';
import { Icon, Button } from '@rneui/themed';
import Moment from 'moment';
import Geolocation from '@react-native-community/geolocation';
import { alertToast } from '../Core/Utility';
import { APPNAME, EMPLOYEE_TYPE_PHOTO, GO_OVERVIEW, SYNC_DATA_ATT } from '../Core/URLs';
import { check, RESULTS, PERMISSIONS, request, openSettings } from 'react-native-permissions'
import { GetEmployeeInfo, MessageAction, MessageAction2, MessageInfo, MessageSetting, OnTime, requestTimeout } from '../Core/Helper';
...
class TakePicture extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      photo: '',
      type: (this.props.route.params.reportId === 1 || this.props.route.params.photoType === EMPLOYEE_TYPE_PHOTO) ? RNCamera.Constants.Type.front : RNCamera.Constants.Type.back,
      ...
      .then(async (result) => {
        switch (result) {
          case RESULTS.UNAVAILABLE:
            MessageSetting('Thông báo', Platform.OS === 'ios' ? 'Vui lòng bật Dịch vụ định vị, Quyền riêng tư -> Dịch vụ định vị -> Dịch vụ định vị (Bật)' : 'Vui lòng cho phép chức năng vị trí trong phần cài đặt của thiết bị', () => {
          ...
          case RESULTS.GRANTED:
            ...
            requestTimeout(10000, this.getLocationUserFast(), async (er) => {
              await this.setState({ isTimeoutLocation: (this.state.latitudePo !== 0 || this.state.longitudePo !== 0) ? false : true })
            })

            break;
          case RESULTS.BLOCKED:
            MessageSetting('Thông báo', Platform.OS === 'ios' ? 'Vui lòng cho phép App sử dụng Dịch vụ định vị, Quyền riêng tư -> Dịch vụ định vị -> tìm đến App ' + APPNAME + ' -> tích chọn khi dùng Ứng dụng & chính xác cao' : 'Vui lòng cho phép chức năng vị trí trong phần cài đặt của thiết bị', () => {
              Platform.OS === 'ios' ? Linking.openURL("App-Prefs:root=Privacy&path=LOCATION") :
                openSettings().catch(() => console.warn('cannot open settings'));;
            })
            break;
  ...
  getLocationUserFast(again) {
    Geolocation.getCurrentPosition(async info => {
      ...
      this.setState({ latitudePo: latitude, longitudePo: longitude, accuracyPo: accuracy, timestamp: timestamp, mocked: mocked, isTimeoutLocation: false });

      (latitude !== 0 && longitude !== 0) && this.checkDistanceVsShop(latitude, longitude)
      if (mocked) {
      ...
      again !== undefined && this.setState({ countGetLoction: this.state.countGetLoction + 1 });
  ...
  getLocationLast() {
    Geolocation.getCurrentPosition(info => {
      ...
      const mocked = info.mocked;
      const timestamp = info.timestamp;
      this.setState({ messageLocationMatch: '', countGetLoction: 0, latitudePo: latitude, longitudePo: longitude, accuracyPo: accuracy, timestamp: timestamp, mocked: mocked, isTimeoutLocation: false });
    }, (error) => alertToast(error.message)
    );
  }
  requestCamera() {
  ...
  checkCamera() {
    check(Platform.OS === 'ios' ? PERMISSIONS.IOS.CAMERA : PERMISSIONS.ANDROID.CAMERA)
      .then(async (result) => {
        switch (result) {
          case RESULTS.UNAVAILABLE:
            await MessageSetting('Thông báo', Platform.OS === 'ios' ? 'Vui lòng bật Dịch vụ định vị, Quyền riêng tư -> camera -> tìm đến App ' + APPNAME + ' -> cho phép (Bật).' : 'Vui lòng cho phép chức năng chụp hình trong phần cài đặt của thiết bị', () => {
              Platform.OS === 'ios' ? Linking.openURL("App-Prefs:root=Privacy&path=LOCATION") :
                openSettings().catch(() => console.warn('cannot open settings'));;
            })
            break;
  ...
  checkDistanceVsShop = async (latitudePo, longitudePo) => {
    if (this.state.photoInfo.shopLat != null && this.state.photoInfo.shopLong != null && this.state.distanceLimit > 0) {
      let distance = await this.calculatorDistance(latitudePo, longitudePo);

      if (this.state.statusDistance === 1 && distance > this.state.distanceLimit) {
        await this.setState({
          messageLocationMatch: 'Vị trí bạn đang Checkin/Checkout cách vị trí cửa hàng lớn hơn '
            + this.state.distanceLimit + 'm'
        })
        if (this.state.countGetLoction > 4) {
          MessageAction2('Hệ thống ghi nhận vị trí bạn chấm công đang cách cửa hàng hơn '
            + this.state.distanceLimit + 'm, bạn muốn tiếp tục chấm công hay lấy lại vị trí ?', () => {
              this.getLocationLast();
            }, () => {
              this.props.navigation.goBack()
              // DeviceEventEmitter.emit(GO_OVERVIEW, null)
              MessageAction('Chọn Đi đến để tới chi tiết cửa hàng, chọn Bản đồ -> Bấm vào biểu tượng góc phải bản đồ để kiểm tra/lấy lại vị trí hiện tại', async () => {
                DeviceEventEmitter.emit('GO_SHOP_MAP', null)
                DeviceEventEmitter.emit(GO_OVERVIEW, null)
              }, 'Không', 'Đi đến')
            }, 'Lấy lại vị trí', 'Chấm công')
        } else {
          await delay(1000);
          await this.getLocationUserFast(true)
        }
      }
      else if (this.state.statusDistance === 2 && distance > this.state.distanceLimit) {
        this.setState({
      ...
      } else if (this.state.statusDistance === 3 && distance > this.state.distanceLimit) {
        this.setState({
          messageLocationMatch: 'Đang định lại vị trí, vui lòng đợi... '
        })
        ...
        if (this.state.countGetLoction > 4) {
          MessageAction2('Hệ thống ghi nhận vị trí bạn chấm công đang cách cửa hàng hơn '
            + this.state.distanceLimit + 'm, bạn muốn lấy lại vị trí ?', () => {
              this.getLocationLast();
              this.props.navigation.goBack()
              MessageAction('Chọn Đi đến để tới chi tiết cửa hàng, chọn Bản đồ -> Bấm vào biểu tượng góc phải bản đồ để kiểm tra/lấy lại vị trí hiện tại', async () => {
                DeviceEventEmitter.emit('GO_SHOP_MAP', null)
  ...
  getConfigServ = () => {
    ...
    if (configInfo != undefined && configInfo != null && configInfo != 'null') {
      ...
      this.setState({ statusDistance: statusDistance, distanceLimit: distanceLimit })
    }
  }
  async componentDidMount() {
    ...
    await (this.props.route.params.QRCode === 1) && await this.setState({ isFullCamera: false, type: RNCamera.Constants.Type.back })
    this.getConfigServ()
  }
  ...
  checkYaw(faces, isOk, isNotOk) {
    faces.map(item => {
      (item.yawAngle.toFixed(0) < - 12 || item.yawAngle.toFixed(0) > 12) ? isNotOk() : isOk();
    })
  }
  checkEyes(faces, isOk, isNotOk) {
    faces.map(item => {
      (item.rightEyeOpenProbability < 0.15 || item.leftEyeOpenProbability < 0.15) ? isNotOk() : isOk();
    })
  }
  checkOutScreen(faces, isOk, isNotOk) {
    faces.map(item => {
      ((item.bounds.origin.y + item.bounds.size.height) > Dimensions.get('screen').height - 100 || (item.bounds.origin.x + item.bounds.size.width) > Dimensions.get('screen').width) ? isNotOk() : isOk();
    })
  }
  ...
  checkFarFace(faces, isOk, isNotOk) {
    faces.map(item => {
      ((item.bounds.origin.x + item.bounds.size.width) * 100 / Dimensions.get('screen').width) < 60 ? isNotOk() : isOk();
    })
  }
  checkNearFace(faces, isOk, isNotOk) {
    faces.map(item => {
      ((item.bounds.origin.x + item.bounds.size.width) * 100 / Dimensions.get('screen').width) > 82 ? isNotOk() : isOk();
    })
  }
  ...
  calculatorDistance(latitudePo, longitudePo) {
    var distance = getDistance(
      ...
      { latitude: this.state.photoInfo?.shopLat, longitude: this.state.photoInfo?.shopLong }
    );
  ...
  takePicture = async () => {
    ...
    try {
      ...
      const isAutoSave = this.state.photoInfo.reportId === 1 && (cameraConfig?.isAutoSave || false) > true
      ...
      const photoinfo = await [{
        ...
        photoType: this.state.photoInfo?.photoType !== undefined ? '' + this.state.photoInfo?.photoType : '',
      ...
      if (this.state.photoInfo?.reportId === 1) {
        if (photoinfo[0].latitude === 0 || photoinfo[0].longitude === 0) {
          alert('Chưa lấy được thông số vị trí của bạn. Vui lòng cấu hình cho phép sử dụng định vị sau đó thử lại.');
          return
        }
      }
      ...
      if (this.state.photoInfo?.reportId === 1) {
        if (this.state.photoInfo?.shopLat != null && this.state.photoInfo?.shopLong != null && this.state.distanceLimit > 0) {
          let distance = this.calculatorDistance(this.state.latitudePo, this.state.longitudePo);
          if (distance > this.state.distanceLimit && this.state.statusDistance === 2) {
            MessageInfo(` Vị trí của bạn so với vị trí cửa hàng lớn hơn ${this.state.distanceLimit}m vui lòng đến gần cửa hàng hơn, Khoảng cách giữa bạn và cửa hàng hiện là: \n\n${distance} Mét\nHay\n${distance / 1000} Kilomet`);
            return
          }
          OnTime(() => {
            this.props.navigation.navigate('Review',
              {
                ...
                shopinfo: { ...this.state.photoInfo?.shopinfo, shopLat: this.state.photoInfo?.shopLat || null, shopLong: this.state.photoInfo?.shopLong || null }
              });
          }, () => {
            this.props.navigation.goBack()
          })
  ...
  getBackData = (data) => {
    if (data === true) {
      if (this.state.photoInfo.reportId == 1 || this.state.photoInfo.reportId == -1 || this.state.photoInfo.photoType === EMPLOYEE_TYPE_PHOTO) {
        this.props.navigation.goBack();
        this.state.photoInfo.reportId == 1 && DeviceEventEmitter.emit(SYNC_DATA_ATT);
      }
      else if (this.props.route.params.closeTakePhoto) {//chụp một tấm 
        (typeof this.props.route.params.callBackReport === 'function') && this.props.route.params.callBackReport();
        this.props.navigation.goBack(null);
      }
    }
    else {
      this.setState({ DoneTakePhoto: false, statusFace: 'Vui lòng đưa máy vào chính diện khuôn mặt bạn để chụp chấm công' })
    }
  }
  loadFlashMode = () => {
  ...
  barcodeRecognized = ({ barcodes }) => { barcodes.length > 0 && this.goBackView(barcodes) };
  ...
  render() {
    const cameraConfig = JSON.parse(this.props.shopinfo?.cameraConfig || '{}')
    const typeCameraId = cameraConfig?.cameraId == 'front' ? RNCamera.Constants.Type.front : cameraConfig?.cameraId == 'back' ? RNCamera.Constants.Type.back : this.state.type
      ...
      <View style={stylesDetect.container}>
        ...
        <View style={{ flex: 1 }}>
          <RNCamera
            ...
            // }}
            style={{ flex: 1 }}
            onGoogleVisionBarcodesDetected={this.props.route.params.QRCode === 1 ? this.barcodeRecognized : null}
          ...
          >
            <View style={{
              ...
              {this.state.photoInfo.reportId === 1 && this.state.statusFace !== null && this.state.statusFace.length > 0 &&
                <Text style={{ color: 'red', fontSize: 15, fontWeight: '600', padding: 5, backgroundColor: 'white', opacity: 0.6 }}>{this.state.statusFace}</Text>
              }
              <View style={{
                padding: cameraConfig?.isFlipCamera ? 15 : 0,
                flexDirection: 'row',
              ...
              >
                {
                  ...
                  <View style={camerestyles.bottomButtonsStart}>
                    <TouchableOpacity onPress={this.flashPress}>
                      <Icon name='bolt' type='font-awesome' size={35} color={flashcolor} />
                    </TouchableOpacity>
                  </View>
                }
                {(this.state.longitudePo !== 0 && this.props.route.params?.QRCode !== 1 && this.state?.statusDistance === null) ?
                  <View style={camerestyles.bottomButtons}>
                    <TouchableOpacity onPress={this.takePicture}>
                      <Icon name='camera' size={60} color="orange" />
                    </TouchableOpacity>
                  </View>
                  :
...

                {
                  cameraConfig?.isFlipCamera ?
                    ...
                    </View> : <View style={camerestyles.bottomButtonsEnd} />
                }
              </View>
            </View>
            {!!canDetectFaces && this.renderFaces()}
            {!!canDetectFaces && this.renderLandmarks()}
            {this.props.route.params.QRCode === 1 && <BarcodeMask width={300} height={200} />}
          </RNCamera>
          <View style={{ height: 0 }} />
          {
            (this.state.longitudePo === 0) &&
            <View style={{ width: '100%', position: 'absolute', alignItems: 'center', alignSelf: "center", marginTop: Dimensions.get('window').height / 2 }}>
              <Progress.Circle thickness={1} size={65} indeterminate={true} />
              <Text style={{ color: '#007AFF' }}>Đang xác định vị trí...</Text>
            </View>
          }
          {
            this.state.isTimeoutLocation === true && <View style={{ width: '100%', position: 'absolute', alignItems: 'center', alignSelf: "center", marginTop: (Dimensions.get('window').height / 2) + 100 }}>
              <Button style={{ color: '#007AFF' }} onPress={async () => this.getLocationUserFast()} title={'Thử lại'} titleStyle={{ fontSize: 15 }} />
              <Text style={{ color: 'orange', fontStyle: 'italic', fontSize: 13 }}>Vị trí chưa được xác định! vui lòng bấm Thử lại!</Text>
            </View>
          }
          {
            (this.state.messageLocationMatch !== '' && this.state.countGetLoction < 6) &&
            <View style={{ width: '100%', position: 'absolute', alignItems: 'center', alignSelf: "center", marginTop: Dimensions.get('window').height / 2 }}>
              <Progress.Circle thickness={1} size={65} indeterminate={true} />
              <Text style={{ color: 'red', fontSize: 20 }}>{this.state.countGetLoction}</Text>
              <Text style={{ color: '#007AFF', padding: 20, textAlign: 'justify' }}>{this.state.messageLocationMatch}</Text>
            </View>
          }
        </View >
