...
export const PhotoCustom = ({
  ...
  const MapData = async () => {
    ...
    Photos.map(async item => {
      const lstPhoto = await getPhotosReport(
        ReportId,
        DisplayId + '_' + item.code,
        Workinfo.shopId,
        Workinfo.workDate,
      );
  ...
  const takePhoto = item => {
    ...
    combine
      ? navigation.navigate('Camera', { ...itemInfo, callBackReport: callBack })
      : Props.navigation.navigate('Camera', {
          ...itemInfo,
          callBackReport: callBack,
        });
  };
  ...
  const viewAlbum = item => {
    ...
    combine
      ? navigation.navigate('AlbumPhoto', itemInfo)
      : Props.navigation.navigate('AlbumPhoto', itemInfo);
  };
  ...
  const styles = StyleSheet.create({
    mainContainer: {
      ...
      width: '100%',
      top: AppNameBuild === 'bk' ? 0 : 60,
      height:
        AppNameBuild === 'bk'
          ? '100%'
          : Platform.OS === 'android'
          ? Dimensions.get('screen').height - (HeightHeader + 160)
          : Dimensions.get('screen').height -
            (HeightHeader + (iPhonex ? 110 : 80)),
    },
    viewMainPhoto: {
  ...
  const renderItem = ({ item, index }) => {
    ...
    const colorStatusUpload =
      item.numberValue > 0
        ? item.numPhoto > 0
          ? item.numPhoto >= item.numberValue
            ? appcolor.green
            : appcolor.warning
          : appcolor.danger
        : appcolor.green;
    ...
    return (
      <View key={index} style={styles.viewMainPhoto}>
        ...
        <View
          style={{ ...styles.viewPhoto, backgroundColor: colorStatusUpload }}
        >
          <TouchableOpacity
            style={{ width: '100%' }}
            onPress={viewPhotoDisplay}
          >
            <Text
              style={{
                fontSize: 14,
            ...
            >
              {item.numPhoto}
              {item.numberValue > 0 ? '/' + item.numberValue : ''} Tấm hình
            </Text>
          </TouchableOpacity>
        </View>
      </View>
