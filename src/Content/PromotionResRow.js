...
export const PromotionResRow = ({ item, index, navigation, loadData, showDetail, listReport }) => {
    ...
    return (
        <>
            <GmailStyleSwipeableRow enableRight={item.upload == 0 ? false : true} deleteItem={() => item.upload == 0 ? deleteItemPromotion(item, loadData) : alert('Không thể xoá dữ liệu đã gửi rồi.')}>
                <TouchableOpacity
                    onPress={() => item.upload == 0 && showDetail(item)}
                    style={{ flexDirection: 'column', justifyContent: 'space-between', marginTop: index > 0 ? 10 : 0 }}
                >
                <View style={{ padding: 8, borderRadius: 5, flexDirection: 'row', justifyContent: 'space-between', backgroundColor: appcolor.surface }}>
                    <Badge
                    ...
                    <View style={{ flexDirection: 'column', width: '85%' }}>
                        <Text style={{ padding: 5, fontSize: 17, fontWeight: 'bold', textAlign: 'left', color: appcolor.dark }}>
                            {`${item.titlePromotion !== null && item.titlePromotion.length > 0 ? item.titlePromotion : 'CTKM'}`}
                        </Text>
                        <Text style={{
                            width: '100%', height: 0.6, backgroundColor: appcolor.greydark, paddingStart: 10, paddingEnd: 10, marginBottom: 4, marginTop: 4, width: '50%'
                        }}></Text>
                        {
                            (item.competitorName !== 'undefined' && item.competitorName !== null && item.competitorName !== undefined) &&
                            <Text style={{ color: appcolor.dark }} >{'Hãng: ' + item.competitorName}</Text>
                        }
                        {
                            (item.categoryName !== 'undefined' && item.categoryName !== null && item.categoryName !== undefined && item.categoryName !== '0') &&
                            <Text style={{ color: appcolor.dark }}>{'Ngành hàng: ' + item.categoryName}</Text>
                        }
                        {
                            (item.content !== 'undefined' && item.content !== null && item.content !== undefined) &&
                            <Text style={{ color: appcolor.dark }} >{'Nội dung: ' + item.content}</Text>
                        }
                        <Text style={{ color: appcolor.dark }}>{'Từ ngày: ' + item.fromDate + ', ' + 'Đến ngày: ' + item.toDate}</Text>
                    </View>
                    {
                        item.upload == 0 && <Icon
                ...
                <View style={{ flexDirection: "row", justifyContent: "space-between", width: '100%', marginTop: 5 }}>
                    <Button
                        containerStyle={{ width: AppNameBuild !== psvApp ? '33%' : '49%' }}
                        buttonStyle={{ height: 45, backgroundColor: appcolor.surface }}
                        onPress={e => {
                            (item.upload == 0) && takePhotoCell(e, item.guiId, workinfo, navigation, kpiinfo.kpiId)
                        }}
                        icon={
                            <Icon
                                color={appcolor.dark}
                    ...
                    />
                    {
                        ...
                        <Button
                            ...
                            buttonStyle={{ height: 45, backgroundColor: appcolor.surface }}
                            onPress={e => {
                                (item.upload == 0) && uploadFileCell(e, item.guiId, workinfo, loadData, kpiinfo.kpiId)
                            }}
                            icon={
                                <Icon
                                    color={appcolor.dark}
