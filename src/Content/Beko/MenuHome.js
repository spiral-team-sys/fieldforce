        ...
        return (
            <TouchableOpacity key={index + "aw"} style={{ alignItems: 'center', backgroundColor: appcolor.light }} onPress={onItemPress}>
                <View style={styles.itemMenu}>
                    <Icon raised type={item.iconType || 'font-awesome'}
                        name={item.iconName} size={21} color={appcolor.primary} solid />
                    <Text style={styles.titleMenu}>{item.menuNameVN}</Text>
                </View>
                <View style={{ borderWidth: 1, borderColor: appcolor.grey, width: '95%', height: 0.5 }} />
            </TouchableOpacity>
        )
    }
