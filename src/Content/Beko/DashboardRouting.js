...
export const DashboardRouting = ({ navigation, data, onTitleResolved }) => {
    ...
    const styles = StyleSheet.create({
        ...
        summaryValue: { fontSize: scaleSize(24), fontWeight: 'bold', color: appcolor.light, padding: 2 },
        summaryLabel: { fontSize: scaleSize(12), color: 'rgba(255,255,255,0.88)', fontWeight: '600' },
        decorativeCircle: {
        ...
        spacer: { flex: 1 },
        listWrap: { height: 136, marginHorizontal: 12, marginTop: 8, marginBottom: 12 },
        listContent: { alignItems: 'center', paddingRight: 4 },
        detailCardPress: { width: deviceWidth * 0.58, height: 120, marginRight: 10 },
        detailCard: { flex: 1, backgroundColor: appcolor.light, padding: 12, borderRadius: 12 },
        detailRow: { flexDirection: 'row', alignItems: 'center' },
        detailDate: { fontWeight: '500', fontSize: scaleSize(18), color: appcolor.dark },
        detailLabel: { fontSize: scaleSize(11), color: appcolor.dark },
        detailValue: { flexGrow: 1, textAlign: 'right', fontSize: scaleSize(18), color: appcolor.dark },
        detailDangerValue: { flexGrow: 1, fontWeight: '700', textAlign: 'right', fontSize: scaleSize(18), color: appcolor.danger },
        moreWrap: { width: 90, height: 120, alignItems: 'center', justifyContent: 'center' },
        moreIconWrap: {
        ...
        moreText: { color: 'rgba(255,255,255,0.9)', fontWeight: '600', fontSize: scaleSize(12) },
        ...
        routingTabWrap: { backgroundColor: appcolor.light, marginTop: 40, padding: 6, width: deviceWidth },
        detailTabWrap: { backgroundColor: appcolor.surface, marginTop: 40, padding: 6, width: deviceWidth },
    })
    ...
    const rowItem = ({ item, index }) => {
        if (item.viewMonth === undefined) {
            return (
                <TouchableOpacity onPress={() => showDetail(item)} key={index + "dasd6aj"} style={styles.detailCardPress}>
        ...
        return (
            <TouchableOpacity onPress={() => navigation.navigate("routingmonth")} style={styles.moreWrap}>
    ...
    return (
        ...
        >
            <TouchableOpacity onPress={() => navigation.navigate("attendanthistory")} style={styles.summaryAction}>
            ...
            <ActionSheet bottomOffset={50} id="shopList" containerStyle={styles.sheetContainer}>
                <View style={styles.sheetContent}>
                    <View style={styles.sheetHeader}>
                        <Text style={styles.sheetTitle}>
                            Cửa hàng đi ngày {selected.workDate}
                        </Text>
                        <TouchableOpacity style={styles.sheetCloseWrap} onPress={() => SheetManager.hide('shopList')}>
