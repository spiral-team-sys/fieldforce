...
const byShopConfig = async (navigation, params) => {
  try {
    ...
    const PHOTOTYPE =
      item.photoType % 2 == 0 ? ATTENDANT_TYPE.IN : ATTENDANT_TYPE.OUT;
    ...
    if (isWrongTimezone) {
      toastError(
        'Sai múi giờ',
        'Vui lòng chỉnh múi giờ ở Việt Nam trong cài đặt của máy',
      );
      return false;
    }
    // Shop Info
    if (isCheckStoreInfo === 1) {
      MessageAction(config.titleCheck || 'Cập nhật dữ liệu cửa hàng', () =>
    ...
    if (!isValid_TaskList) return false;
    // Work
    const isValid_Work = await valid_Work(navigation, {
      workinfo,
      shopinfo,
      item,
      isEdit,
...
const valid_Tasklist = async (navigation, checkOnline = 0) => {
  if (checkOnline == 1) await DOWNLOADAPI.downloadTaskList();
  //
  const menuHome = await menuController.getMenu();
  const fistDone = _.filter(
    menuHome,
    e => e.fistTask === 1 && e.taskDone === 0,
  );
  if (isValidData(fistDone)) {
    const itemDone = fistDone[0] || {};
    MessageAcept(
      'Thông báo',
      `${itemDone.menuNameVN} ${itemDone.taskAlter}`,
      () => {
...
const countTime = (timeLimit, timeClick, type) => {
  ...
  const diffMs =
    type === ATTENDANT_TYPE.IN ? timeClick - timeLimit : timeLimit - timeClick;
...
const getCheckInPhotoTimeValue = async (shopinfo, item) => {
  ...
  const attendantList = shopinfo?.auditDate
    ? await AttendantController.GetAttendant(shopinfo, shopinfo.auditDate)
    : [];
  const checkInPhoto =
    _.find(attendantList, it => Number(it.photoType) === checkInPhotoType) ||
    item;
...
const valid_Work = async (navigation, params) => {
  ...
  if (PHOTOTYPE == ATTENDANT_TYPE.IN) {
    if (shopNotCO > 0) {
      ...
      if ((await lstHave?.length) > 0) {
        MessageInfo(
          `Vui lòng thực hiện checkout cửa hàng ${lstHave[0].shopName} sau đó thực hiện tiếp checkin cửa hàng khác.`,
        );
        return false;
      }
    }
  ...
  const requireOverview = () => {
    MessageAction(
      'Vui lòng chụp hình tổng quan cửa hàng trước khi chấm công.',
      async () => {
  ...
  if (overviewIMG === 1 && PHOTOTYPE === ATTENDANT_TYPE.IN) {
    const isMissingImageToday =
      !shopinfo.imageUrl ||
      (shopinfo.imageUrl && !shopinfo.imageUrl.includes(TODAY));
  ...
  if (PHOTOTYPE == ATTENDANT_TYPE.OUT) {
    if (shopSO > 0) {
      ...
      if (resSellout?.length === 0) {
        toastError(
          'Cảnh báo chưa làm báo cáo sellout',
          'Vui lòng nhập sellout (Không bán được sản phẩm vui lòng chọn NoSell sau đó gửi lên hệ thống)',
        );
        return false;
      } else {
        ...
        if (lstHave.length > 0) {
          toastError(
            'Cảnh báo chưa làm báo cáo sellout',
            'Bạn có nhập sellout nhưng chưa gửi lên hết, Vui lòng vào lại báo cáo đẩy dữ liệu lên.',
          );
          return false;
        }
      }
    ...
    if (checkEditDisplay == 1) {
      ...
      if (isEdit || isEditStore) {
        if (isValidObject(itemMenuDisplay)) {
          MessageAcept(
            'Thông báo',
            `Vui lòng thoát chế độ "Chỉnh sửa" trong ${itemMenuDisplay.name} trước khi Checkout`,
            () => {
    ...
    if (checkReportEdit == 1) {
      ...
      dataAsync.map(it => {
        if (it.isUpdate == 0 && it.isCheck != 1) {
          it.isCheck = 1;
          strErrReport =
            strErrReport + `Bạn chưa gửi báo cáo "${it.menuNameVN}"\n`;
        }
      });
    ...
    if (
      workinfo.workStatus == 1 &&
      (checkNumberAtt !== 1 || (checkNumberAtt == 1 && numberAtt === e + 1))
    ) {
    ...
    if (constraintMaxCOMinute > 0 && PHOTOTYPE === ATTENDANT_TYPE.OUT) {
      ...
      if (!timeCIMili) {
        toastError(
          'Lỗi dữ liệu',
          'Không xác định được thời gian CHECK-IN để kiểm tra thời gian ghé thăm.',
        );
        return false;
      }
      ...
      if (timeClick > timeInMiliLimit) {
        if (noteCO > 0) {
          // không cho phép out dù bắt ki lý do gì
          toastError(
            'Quá thời gian ghé thăm',
            'Tổng thời gian ghé thăm trên ' + constraintMaxCOMinute + ' phút.',
            'Thông báo',
            'top',
          );
          return false;
    ...
    if (constraintCOMinute > 0 && PHOTOTYPE === ATTENDANT_TYPE.OUT) {
      ...
      if (timeClick < timeInMiliLimit) {
        if (noteCO > 0) {
          toastError(
            `Thời gian ghé thăm`,
            `Tổng thời gian ghé thăm dưới ${constraintCOMinute} phút.`,
          );
          return false;
        } else {
          Message(
            'Thông báo',
            `Tổng thời gian ghé thăm dưới ${constraintCOMinute} phút. Vui lòng nhập lý do ở dưới đây sau đó CHECK-OUT`,
            () => {
              SheetManager.show('note-attendance-sheet', {
                payload: {
                  ...shopinfo,
  ...
  if (constraintLate > 0 || constraintEarly > 0) {
    ...
    const onLateAction = (time = 0) => {
      Message(
        'Thông báo',
        `Bạn đang CHECK-IN trễ ${time} phút so với thời gian làm việc là ${moment(
          shopinfo.timeIn,
        ).format('HH:mm')}. Vui lòng nhập ghi chú và tiếp tục CHECK-IN.`,
        () => {
          SheetManager.show('note-attendance-sheet', {
            payload: {
              ...shopinfo,
    ...
    const onEarlyAction = time => {
      Message(
        'Thông báo',
        `Bạn đang CHECK-OUT sớm ${time} phút so với thời gian làm việc là ${moment(
          shopinfo.timeOut,
        ).format('HH:mm')}. Vui lòng nhập ghi chú và tiếp tục CHECK-OUT.`,
        () => {
          SheetManager.show('note-attendance-sheet', {
            payload: {
              ...shopinfo,
    ...
    if (checkFirstShop == 1) {
      ...
      // Validate CHECK IN
      await ATTENDANT_API.validTimeAttendant(
        ATTENDANT_TYPE.IN,
        async (itemTime, message) => {
          if (message) {
          ...
          if (itemTime?.isCheckTimeIn === 0) {
            const validTime = moment(shopinfo.timeIn).add(
              constraintLate,
              'minute',
            );
      ...
      await ATTENDANT_API.validTimeAttendant(
        ...
        async (itemTime, message) => {
          ...
          if (itemTime?.isCheckTimeOut === 0) {
            const validTime = moment(shopinfo.timeOut).subtract(
              constraintLate,
              'minute',
            );
      ...
      if (PHOTOTYPE === ATTENDANT_TYPE.IN) {
        const timeInLimit = moment(shopinfo.timeIn)
          .add(constraintLate, 'minute')
          .utc()
          .valueOf();
        if (timeClick > timeInLimit) {
          const time = countTime(
            moment(shopinfo.timeIn).utc().valueOf(),
            timeClick,
            ATTENDANT_TYPE.IN,
          );
          if (notNoteLate === 0 || notNoteLate == 3) {
            ...
            alertNotify(
              `Bạn đang checkIn trễ ${time} phút so với thời gian làm việc ${moment(
                shopinfo.timeIn,
              ).format('HH:mm')}.`,
            );
            return false;
        ...
        const timeOutLimit = moment(shopinfo.timeOut)
          .subtract(constraintEarly || constraintLate, 'minute')
          .utc()
          .valueOf();
        let timeOut = shopinfo.timeOut;
        let timeOutMili = moment(timeOut).utc().valueOf();
        if (
          (timeClick < timeOutLimit && checkExactTime != 1) ||
          (checkExactTime == 1 && timeClick < timeOutMili)
        ) {
          const time = countTime(
            moment(shopinfo.timeOut).utc().valueOf(),
            timeClick,
            ATTENDANT_TYPE.OUT,
          );
          if (notNoteLate === 0) {
            ...
            alertNotify(
              `Bạn đang checkOut sớm ${time} phút so với thời gian làm việc ${moment(
                shopinfo.timeOut,
              ).format('HH:mm')}.`,
            );
            return false;
  ...
  if (PHOTOTYPE === ATTENDANT_TYPE.IN && shopNote == 1) {
    SheetManager.show('note-attendance-sheet', {
      payload: {
        ...shopinfo,
        item,
