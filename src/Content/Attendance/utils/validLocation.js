...
import { MessageAction, MessageAction2, MessageInfo, MessageSetting, requestTimeout } from "../../../Core/Helper";
...
const checkPermissionLocation = async (callbackGranted, callbackError, callbackTimeout) => {
    ...
    check(Platform.OS === 'ios' ? PERMISSIONS.IOS.LOCATION_WHEN_IN_USE : PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION).then(async (result) => {
        switch (result) {
...
const checkDistance = async (shopinfo, locationinfo, count, callbackMessage, callbackGoback) => {
    ...
    if (shopinfo.latitude && shopinfo.longitude && shopConfig.Distance > 0) {
        ...
        if (distance > shopConfig?.Distance) {
            if (shopConfig.statusDistance == 1) {
                callbackMessage(`Vị trí bạn đang Checkin/Checkout cách vị trí cửa hàng lớn hơn ${shopConfig.Distance}m`)
                if (count > 5) {
                    callbackMessage('')
                    MessageAction2(
            ...
            if (shopConfig.statusDistance == 2) {
                ...
                if (count > 5) {
                    callbackMessage(`Bạn đang cách cửa hàng ${distance}m.\n\n` +
