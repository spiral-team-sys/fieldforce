...
const AttendanceList = ({ navigation }) => {
    const { appcolor, shopinfo, workinfo, isEdit } = useSelector(state => state.GAppState)
    ...
    const LoadData = async (workSelected) => {
        ...
        const attendantDate = currentWork?.workDate || shopinfo.auditDate || shopinfo.auditdate

        await ATTENDANT_API.GetDataAttendance({ shopId: currentWork?.shopId || shopinfo.shopId, attendantDate })
        await deley(1000)

        const data = await AttendantController.GetAttendant({ ...shopinfo, shopId: currentWork?.shopId || shopinfo.shopId }, attendantDate)
        await AttendantController.CreateTemplateAttendance(currentWork, locationInfo, data, config.numberAtt, (mAttendance, mSwiper) => {
            setDataAttendance(mAttendance)
            setDataSwiper(mSwiper)

            // Task Today
            if (mAttendance.length % 2 == 0) {
    ...
    const handlerPressAttendance = async (item) => {
        if (item.photoDate == TODAY) {
            // Valid Data
            const isValid = await VALID_ATTENDANCE.byShopConfig(navigation, { shopinfo, workinfo, isEdit, item })
            if (!isValid)
                return;
            //
            await setWaiting(true)
            await dispatch(setCameraInfo({ ...item, guid: UUIDGenerator() }))
            await navigation.navigate('Camera')
            await setWaiting(false)
        } else {
            alertConfirm('Chấm công', 'Bạn đang chấm công ngày cũ, vui lòng thoát ứng dụng và thử lại', () => { RNRestart.Restart() }, () => { }, 'Đồng ý', 'Đóng')
        }
    }
    ...
    const onReportTo = (itemMenu) => {
        ...
        navigation.navigate(`${itemMenu.pageName}`, { workinfo: workinfo, titlePage: itemMenu.name, reportId: itemMenu.kpiId })
    }
    // Action
    const onAcceptNote = async (itemNoteWork) => {
        const config = JSON.parse(shopinfo?.config || '{}')
        if ((config?.shopNote || 0) == 0) {
            if (!isValidField(itemNoteWork.note) || itemNoteWork.note.length < 10) {
                toastError(itemNoteWork.titleAlert, 'Vui lòng nhập nội dụng ghi chú & tối thiểu 10 kí tự');
                return
            }
        }
        //
        ...
        LOCATION_INFO.getCurrentLocation(async (info) => {
            dispatch(setLocationInfo(info))
            dispatch(setCameraInfo({ ...itemNoteWork.item, photoDesc: itemNoteWork.note, guid: UUIDGenerator() }))
    ...
    //
    useEffect(() => {
        const reload_attendance = DeviceEventEmitter.addListener("RELOAD_ATTENDANCE", handlerReloadAttendance)
        const sync_attendance = DeviceEventEmitter.addListener(SYNC_DATA_ATT, LoadData)
    ...
    const styles = StyleSheet.create({
        ...
        itemMain: {
            width: deviceWidth / 1.8, backgroundColor: appcolor.surface, marginHorizontal: 4, borderWidth: 1, borderRadius: 8, borderColor: appcolor.surface,
            shadowColor: appcolor.grey, shadowOffset: { width: 1, height: 0 }, shadowRadius: 8, shadowOpacity: 0.3, elevation: 3, overflow: 'hidden'
        },
        nonePhotoView: { width: '100%', height: '100%', backgroundColor: appcolor.light, alignSelf: 'center' },
        havePhotoView: { width: '100%', height: '100%', backgroundColor: appcolor.light, alignSelf: 'center' },
        contentImage: { flex: 1 },
        titleName: { position: 'absolute', top: 0, width: '100%', textAlign: 'center', padding: 8, fontSize: 14, fontWeight: fontWeightBold, color: appcolor.primary },
        titleTime: { position: 'absolute', bottom: 0, textAlign: 'center', width: '100%', backgroundColor: appcolor.light, fontSize: 11, fontWeight: fontWeightBold, color: appcolor.dark, padding: 2 },
        iconStatusContainer: { position: 'absolute', top: 8, right: 8 },
        preview: { width: '100%', height: '100%' },
        reuploadView: { width: '100%', height: '100%', position: 'absolute', justifyContent: 'center', backgroundColor: appcolor.white, opacity: 0.7 },
        sheetContainer: { width: '100%', height: 'auto', padding: 8, paddingBottom: 32 },
        titleAlert: { fontSize: 14, fontWeight: fontWeightBold, color: appcolor.dark, padding: 8, textAlign: 'center' }
    })
    const renderItem = ({ item }) => {
        ...
        const nonePhoto = (
            <TouchableOpacity style={styles.nonePhotoView} onPress={onPress}>
                <LottieView autoPlay style={{ height: '100%' }} source={require("../../Themes/lotties/facecico.json")} />
            </TouchableOpacity>
        )
        const havePhoto = (
        ...
        return (
            <View style={styles.itemMain} >
                ...
                <Text style={styles.titleName}>{(item.photoType % 2 == 0 ? `IN` : 'OUT')}</Text>
                <Text style={styles.titleTime}>{item.photoTime ? moment(item.photoTime, 'YYYYMMDDHHmmss').format('HH:mm:ss dddd DD/MM') : ''}</Text>
            </View>
        )
    }
