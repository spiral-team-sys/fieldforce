...
import { View, Text, Platform, KeyboardAvoidingView, TouchableOpacity, SafeAreaView, Dimensions, Modal, FlatList, ScrollView, Image } from "react-native";
import { CheckBox, Icon } from '@rneui/themed';
import { Store, UpdateItem } from "../Core/SqliteDbContext";
import { deviceHeight, scaleSize } from '../Themes/AppsStyle';
import { checkIMEI, checkIMEI2, formatCard, formatNumber, formatPhone, isPhone } from '../Core/Helper';
import { toastError, toastSuccess } from '../Utils/configToast';
import { Check2IMEISellout, CheckIMEISellout, getPhotosByGuiId } from "../Controller/WorkController";
////import { NumericFormat } from "react-number-format";;
import { AppNameBuild, daikinApp, _competitorId, _competitorName, toshibaApp } from '../Core/URLs';
import { alertWarning, ConvertToInt, deviceWidth, findFirstLetterPosition } from "../Core/Utility";
import { Keyboard } from "react-native";
import { deletePhotoByList, InsertPhotosItem } from "../Controller/PhotoController";
...
const SelloutModel = ({ Shops, Competitors, Categories, SubCategories, Segments, SubSegment, Products, Closed, Props, ItemSaved, LoadHistory, Dashboard, MasterList, reload, guiId, lstReport, infoSave }) => {
    ...
    const [dataCalendar, setDataCalendar] = useState({
        "markedDatesDefault": { [Moment(LoadHistory ? ItemSaved.reportDate?.toString() : new Date()).format('YYYY-MM-DD').toString()]: { selected: true, marked: true, selectedColor: appcolor.primary } },
        "markedDates": { [Moment(LoadHistory ? ItemSaved.reportDate?.toString() : new Date()).format('YYYY-MM-DD').toString()]: { selected: true, marked: true, selectedColor: appcolor.primary } },
    ...
    const [modalItem, setModalItem] = useState({ isShowModal: false, itemSelect: {}, dataShow: [], typeItem: '', itemMain: {} })

    const workingDate = Number(userinfo?.workingDate) || 0;
    const firstDateOfMonthInt = Number(Moment().format("YYYYMM01"));
    const minDateInt = workingDate > firstDateOfMonthInt ? workingDate : firstDateOfMonthInt;
    const minDateCalendar = Moment(minDateInt.toString(), "YYYYMMDD").format("YYYY-MM-DD");
    ...
    const filterItem = async (text, type) => {
        const { category, division, competitor, segment, subsegment } = selectedProducts
        switch (type) {
            case 'products':
                const lstProduct = dataProducts.dataFilter.filter(i => (i.division === division || i.division === competitor || i.category === category || i.segment === segment)
    ...
    React.useEffect(() => {
        ...
        if (infoSave?.tickInfoSave == true) {
            ...
            setSellItem({
                ...
                reportDate: infoSave?.dataInfoSave?.reportDate ? infoSave?.dataInfoSave?.reportDate : Moment(workinfo.workDate.toString()).format('YYYY-MM-DD').toString(),
                ...
                deposit: infoSave?.dataInfoSave?.deposit ? JSON.parse(infoSave?.dataInfoSave?.deposit || '[]') : null,
                billCode: infoSave?.dataInfoSave?.billCode
            })
        }
        else {
            setSellItem({ ...sotemplate, reportDate: Moment(workinfo?.workDate?.toString()).format('YYYY-MM-DD').toString() })
        }
        setNameShop()
        defaultLoad();
        loadDataCalendar()
    ...
    const defaultLoad = async () => {
        ...
        if (Competitors !== null && Competitors?.length > 0) {
            ...
            const lstProductByDivision = await Products.filter(i => i.division == _division)
            await setCategory(_tempCate)
            // Category
            if (_tempCate?.length > 0) {
                const _category = lstProductByDivision.length > 300 ? Categories[0].name : null
                const _categoryId = lstProductByDivision.length > 300 ? Categories[0].id : null
                await setSelectProduct({ ...selectedProducts, category: _category, categoryId: _categoryId, competitor: _division, competitorId: _divisionId })
                await setSegment(Segments.filter((seg) => seg.category == _category && seg.division === _division))
                const lstProductFilter = await lstProductByDivision.filter(i => i.division == _division && (i.category == _category || _category == null))
                await setProduct({ dataView: lstProductFilter, dataFilter: lstProductFilter })
            } else {
                const lstProductFilter = await Products.filter(i => i.division == _division)
                await setProduct({ dataView: lstProductFilter, dataFilter: lstProductFilter })
            }
        } else {
        ...
        if (LoadHistory) {
            const filterCompetitor = Competitors.filter(it => it.id == ItemSaved.competitorId)
            const products = await {
                ...ItemSaved,
                "deposit": ItemSaved.deposit ? JSON.parse(ItemSaved.deposit || '[]') : null,
                "id": ItemSaved.productId, "products": ItemSaved.productName, "competitor": filterCompetitor[0]?.name || null
            }
    ...
    const onValidated = async (onFinish) => {
        ...
        for (let index = 0; index < MasterList.length; index++) {
            ...
            if (element.ref_Name === 'billCode' && element.isRequired === 1 && (sellItem.billCode === null || sellItem.billCode === '' || sellItem.billCode === 'null')) {
                isValid = false
                onFinish({ statusId: 500, messager: `Bạn chưa nhập Số hoá đơn.` });
                return;
            }
            if (element.ref_Name === 'reportDate' && sellItem.reportDate && sellItem.reportDate !== "" && parseInt(sellItem.reportDate) > currentDate) {
                isValid = false
                onFinish({ statusId: 500, messager: `Ngày báo cáo không được lớn hơn ngày hiện tại!` });
                return;
            }
            if (element.ref_Name === 'shop' && (nameShop === null || nameShop === undefined)) {
                isValid = false
                onFinish({ statusId: 500, messager: 'Bạn chưa chọn cửa hàng.' });
                return;
            }
            if (element.ref_Name === 'products') {
                if ((lstReport?.isCheckModel == undefined || lstReport?.isCheckModel == 1) && (selectedProducts.id == undefined || selectedProducts.id < 1)
                    && (selectedProducts.competitorId == _competitorId || selectedProducts.competitorId == null || selectedProducts.competitorId == undefined)) {
                    isValid = false
            ...
            if (element.ref_Name === 'customer' && element.isRequired === 1
                && (sellItem.customer == null || sellItem.customer == '' || ((sellItem.customer?.length || 0) < element.numberValue))) {
                if ((sellItem.customer?.length || 0) < element.numberValue) {
                    isValid = false
                    onFinish({ statusId: 500, messager: `Tên khách hàng ngắn, nhập ít nhất ${element.numberValue} ký tự` });
                    return;
                }
            }
            if (element.ref_Name === 'quantity' && (sellItem.quantity == null || sellItem.quantity == "" || ConvertToInt(sellItem.quantity) < 1)) {
                isValid = false
                onFinish({ statusId: 500, messager: `Số lượng không hợp lệ` });
                return;
            } if (element.ref_Name === 'quantity' && sellItem.quantity !== null && sellItem.quantity !== "" && ConvertToInt(sellItem.quantity) > element.numberValue) {
                isValid = false
                onFinish({ statusId: 500, messager: `Số lượng không hợp lệ, số lượng lớn hơn cho phép ${element.numberValue}` });
                return;
            }
            if (element.ref_Name === 'price' && element.isRequired === 1 && (sellItem.price === null || sellItem.price === '' || sellItem.price === 'null')) {
                isValid = false
                onFinish({ statusId: 500, messager: `Giá không được để trống` });
                return;
            }
            if (element.ref_Name === 'statusVerify' && element.isRequired === 1 && (sellItem.statusVerify === null || sellItem.statusVerify === '' || sellItem.statusVerify === 'null')) {
                isValid = false
                onFinish({ statusId: 500, messager: `Bạn chưa chọn trạng thái đơn hàng` });
                return;
            }
            if (element.ref_Name === 'itemClassify' && element.isRequired === 1 && (sellItem.itemClassify === null || sellItem.itemClassify === '' || sellItem.itemClassify === 'null')) {
                isValid = false
                onFinish({ statusId: 500, messager: `Bạn chưa chọn loại hàng hoá` });
                return;
            }
            if (element.ref_Name === 'promotionType' && element.isRequired === 1 && (sellItem.promotionType === null || sellItem.promotionType === '' || sellItem.promotionType === 'null')) {
                isValid = false
                onFinish({ statusId: 500, messager: `Bạn chưa chọn loại khuyến mãi` });
                return;
            }
            if (element.ref_Name === 'deposit' && element.isRequired === 1 && (sellItem.deposit === null || sellItem.deposit === '' || sellItem.deposit === 'null')) {
                isValid = false
                onFinish({ statusId: 500, messager: `Bạn chưa chọn Khách hàng cọc trước.` });
                return;
            }
            if (element.ref_Name === 'taxCode' && element.isRequired === 1 && (sellItem.taxCode === null || sellItem.taxCode === '' || sellItem.taxCode === 'null')) {
                isValid = false
            ...
            if (element.ref_Name === 'email' && element.isRequired === 1) {
                if (sellItem.email === null || sellItem.email === '' || sellItem.email === 'null') {
                    isValid = false
                ...
                if (!regEmail.test(sellItem.email) && sellItem.email) {
                    isValid = false
                    onFinish({ statusId: 500, messager: `Định dạng email không chính xác!` });
                    return;
                }
            }

            if (element.ref_Name === 'paymentMethod' && element.isRequired === 1 && (sellItem.paymentMethod === null || sellItem.paymentMethod === '' || sellItem.paymentMethod === 'null')) {
                isValid = false
                onFinish({ statusId: 500, messager: `Bạn chưa chọn phương thức thanh toán.` });
                return;
            }

            if (element.ref_Name === 'phone' && element.isRequired === 1 && (sellItem.phone === null || sellItem.phone === '') && element.isRequired === 1 && (selectedProducts.competitorId == _competitorId || selectedProducts.competitorId == null || selectedProducts.competitorId == undefined)) {
                isValid = false
                onFinish({ statusId: 500, messager: `Số điện thoại không được để trống` });
                return;
            }
            if (sellItem.phone !== null && sellItem.phone !== '') {
                const checkPhone = isPhone(sellItem.phone)
                if (!checkPhone) {
                    onFinish({ statusId: 500, messager: `Số điện thoại không đúng định dạng` })
                    isValid = false
                    return
                }
            }
            if (element.ref_Name === 'serial' && (element.isRequired === 1 || element.isRequired === 3) && (sellItem.serial === null || sellItem.serial === '')) {
                isValid = false
                // arrItem.push({ statusId: 500, messager: `IMEI không được để trống` })
                onFinish({ statusId: 500, messager: `Số Seri không được để trống` });
                return;
            }
            if (element.ref_Name === 'serial' && (element.isRequired !== 2 && element.isRequired !== 3) && sellItem.serial !== null && sellItem.serial !== "" && sellItem.serial?.length !== element.numberValue) {
                isValid = false
                onFinish({ statusId: 500, messager: `${element.name} chưa đúng định dạng, Seri phải đủ ${element.numberValue} ký tự` });
                return;
            }
            if (element.ref_Name === 'IMEI2' && (element.isRequired !== 2 && element.isRequired !== 3 && sellItem.IMEI2 !== null && sellItem.IMEI2 !== "" && sellItem.IMEI2?.length !== element.numberValue)) {
                isValid = false
                onFinish({ statusId: 500, messager: `${element.name} chưa đúng định dạng, Seri phải đủ ${element.numberValue} ký tự` });
                return;
            }
            ...
            if (element.filterList?.length > 0 && typeof filterList === 'object' && filterList !== null && !Array.isArray(filterList) && sellItem[element.ref_Name] !== null && sellItem[element.ref_Name] !== "") {
                // '{"isCheckCharacters":1,"numCharacters":1,"numDigits":6,"characterPosition":1}'

                if (filterList.isCheckCharacters === 1) {
                    const textPosition = findFirstLetterPosition(sellItem[element.ref_Name])
                    if (textPosition == -1) {
                        isValid = false
                        onFinish({ statusId: 500, messager: `${element.name} chưa đúng định dạng, Seri phải có ký tự chữ` });
                        return
                    } else if (filterList.characterPosition != undefined && filterList.characterPosition > 0 && (textPosition + 1) != filterList.characterPosition) {
                        isValid = false
                        onFinish({ statusId: 500, messager: `${element.name} chưa đúng định dạng, ký tự chữ phải nằm ở vị trí ${filterList.characterPosition == 1 ? 'đầu tiên' : ('thứ' + filterList.characterPosition)}` });
                        return
                    }

                    const lettersOnly = sellItem[element.ref_Name].replace(/[^a-zA-Z]/g, '');
                    const digitsOnly = sellItem[element.ref_Name].match(/\d/g)

                    if (filterList.numCharacters != undefined && filterList.numCharacters != lettersOnly.length) {
                        isValid = false
                        onFinish({ statusId: 500, messager: `${element.name} sai định dạng, yêu cầu : ${filterList.numCharacters} Ký tự chữ cái vị trí đầu tiên và ${filterList.numDigits} ký tự số` });
                        return
                    }
                    if (filterList.numDigits != undefined && filterList.numDigits != digitsOnly.length) {
                        isValid = false
                        onFinish({ statusId: 500, messager: `${element.name} sai định dạng, yêu cầu : ${filterList.numCharacters} Ký tự chữ cái vị trí đầu tiên và ${filterList.numDigits} ký tự số` });
                        return
                    }
            ...
            if (lstReport?.isCheckIMEI !== -1 && ((element.ref_Name === 'serial' && sellItem.serial !== null && sellItem.serial !== "") || (element.ref_Name === 'IMEI2' && sellItem.IMEI2 !== null && sellItem.IMEI2 !== ""))) {
                if (lstReport?.isOnlyNumber == 1 && (sellItem.serial.match(/^[a-zA-Z]+$/) || sellItem.IMEI2.match(/^[a-zA-Z]+$/))) {
                    isValid = false
                    onFinish({ statusId: 500, messager: `Số Seri chỉ chứa số và không chứa ký tự hoặc ký tự đặc biệt!` });
                    return;
                }
                if (lstReport?.isOtherCheck == 1) {
                    if (element.ref_Name === 'IMEI2') {
                        let lstSellout2 = await Check2IMEISellout(sellItem.serial, sellItem.IMEI2, selectedProducts.productId, sellItem.sellId, lstReport?.checkDuplicate);

                        if (lstSellout2?.length > 0) {
                            isValid = false
                            onFinish({ statusId: 500, messager: `số Seri ${sellItem.serial},${sellItem.IMEI2} đã tồn tại ` });
                            return;
                        }
                    }
                } else {
                    let lstSellout = await CheckIMEISellout(element.ref_Name === 'serial' ? sellItem.serial : sellItem.IMEI2, element.ref_Name);
                    if (lstSellout?.length > 0 && LoadHistory === false) {
                        isValid = false
                        onFinish({ statusId: 500, messager: `Số Seri này đã tồn tại ${element.ref_Name === 'serial' ? sellItem.serial : sellItem.IMEI2}` });
                        return;
                    }
                }
                if (lstReport?.isCheckIMEISystem == 1) {
                    if (lstReport?.isDoubleImei == 1 && sellItem.IMEI2 !== null) {
                        const checkIMEIServer = await checkIMEI2(sellItem.serial, sellItem.IMEI2, selectedProducts.productCode)
                        if (checkIMEIServer !== null && checkIMEIServer.length > 0) {
                            isValid = false
                            onFinish({ statusId: 500, messager: checkIMEIServer[0].messager, showModal: true });
                            return;
                        }
                    } else if (lstReport?.isDoubleImei !== 1) {
                        const checkIMEIServer = await checkIMEI(element.ref_Name === 'serial' ? sellItem.serial : sellItem.IMEI2, selectedProducts.productCode)
                        if (checkIMEIServer !== null && checkIMEIServer.length > 0) {
                            isValid = false
                            onFinish({ statusId: 500, messager: checkIMEIServer[0].messager, showModal: true });
                            return;
            ...
            if (lstReport?.isMismatched !== 1 && element.ref_Name === 'IMEI2' && sellItem.serial !== null && sellItem.serial !== "" && sellItem.IMEI2 == sellItem.serial) {
                isValid = false
                onFinish({ statusId: 500, messager: `2 số serial phải khác nhau!` });
                return;
            }
            if (element.ref_Name === 'photo' && element.isRequired === 1 && (selectedProducts.competitorId == _competitorId || selectedProducts.competitorId == null || selectedProducts.competitorId == undefined)) {
                const listPhoto = await getPhotosByGuiId(guiId, workinfo.shopId)
                if (listPhoto.length == undefined || listPhoto.length == 0) {
                    onFinish({ statusId: 500, messager: `Bạn chưa chụp hình ảnh hoá đơn, số lượng cần chụp là ${element.numberValue}` });
                    return;
                }
                if (listPhoto.length < element.numberValue) {
                    onFinish({ statusId: 500, messager: `Số lượng hình ảnh hoá đơn cần chụp không đủ ${listPhoto.length}/${element.numberValue}` });
                    return;
                }
            }
            ...
            await onValidated(async (result) => {
                if (result?.statusId === 500) {
                    ...
                    if (sellItem.serial?.length > 0) {
                        const itemSerial = MasterList.filter(it => it.ref_Name === 'serial')
                        serial = formatCard(sellItem.serial || '', itemSerial || 6)
                    }
                    if (sellItem.IMEI2?.length > 0) {
                        const itemSerial2 = MasterList.filter(it => it.ref_Name === 'IMEI2')
                    ...
                    await Store().then(async db => {
                        const item = {
                            ...
                            serial: serial || '',
                            IMEI2: IMEI2 || '',
                            quantity: serial.length > 0 && serial !== null && serial !== 'null' ? 1 : sellItem.quantity,
                        ...
                        if (LoadHistory === true) {
                            await UpdateItem(db, sellOut.tableName, item, { "sellId": ItemSaved.sellId });
    ...
    const onNextPage = (refName) => {
        ...
        if (selectedProducts.competitorId == _competitorId || (selectedProducts.competitorId !== _competitorId && indexPageProduct !== (indexPage + 1))) {
    ...
    const selectItem = async (item, refName) => {
        switch (refName) {
            case 'competitor':
                const filerCategoryList = Categories.filter((cat) => cat.division == item.name)
            ...
            case 'category':
                ...
                //filter subcate
                const _subcate = SubCategories.filter((sc) => sc.category == item.name && sc.division === item.division)
                await setSubCategory(_subcate)
                //filter segment
                const _segment = Segments.filter((seg) => seg.category == item.name && seg.division === item.division)
                await setSegment(_segment)
                let lstProFil = Products.filter((pro) =>
                    (selectedProducts.competitor === undefined ?
                        (item.division !== undefined ? pro.division === item.division : item.division === undefined)
                        : pro.division === selectedProducts.competitor)
                    && pro.category === item.name
                );
                await setProduct({ dataView: lstProFil, dataFilter: lstProFil })
                setSelectProduct({ ...selectedProducts, [refName]: item.name, categoryId: item.id })
                if ((lstReport?.isCheckModel == undefined || lstReport?.isCheckModel == 1) && selectedProducts.competitorId == _competitorId) {
                    await onNextPage(refName)
                } else {
            ...
            case 'subcategory':
                ...
                const _dataSubCate = await SubCategories.filter((sc) => sc.category == item.category && sc.division === item.division)
                await setSubCategory(_dataSubCate)
                const _subProduct = Products.filter((pro) => {
                    return (selectedProducts.competitor === undefined || pro.division === selectedProducts.competitor)
                        && (selectedProducts.category === undefined || pro.category === selectedProducts.category)
            ...
            case 'segment':
                ...
                setSelectProduct({ ...selectedProducts, [refName]: item.name })
                SubSegment.filter((sseg) => sseg.segment === item.name).forEach(result => {
                    dataSubSegment.push(result);
                });
                let sListProduct = Products.filter((pro) =>
                    (selectedProducts.competitor === undefined || pro.division === selectedProducts.competitor)
                    && (selectedProducts.category === undefined || pro.category === selectedProducts.category)
                    && (selectedProducts.subcategory === undefined || pro.subcategory === selectedProducts.subcategory)
            ...
            case 'subsegment':
                ...
                let product = Products.filter((pro) =>
                    ...
                    && (selectedProducts.category === undefined || pro.category === selectedProducts.category)
                    && (selectedProducts.subcategory === undefined || pro.subCategory === selectedProducts.subcategory)
                    && (selectedProducts.segment === undefined || pro.segment === selectedProducts.segment)
                    && pro.subCategory === item.name)
            ...
            case 'products':
                setSelectProduct({
                    ...
                    subcategory: MasterList.filter(i => i.ref_Name == 'subcategory')?.length ? item.subCategory : undefined,
                    segment: MasterList.filter(i => i.ref_Name == 'segment')?.length ? item.segment : undefined,
                    subsegment: MasterList.filter(i => i.ref_Name == 'subsegment')?.length ? item.subSegment : undefined,
                    "id": item.id,
                    "productId": item.id,
                    "products": item.name,
                ...
                (AppNameBuild === daikinApp || AppNameBuild === toshibaApp || config?.isUsePriceProduct == 1) && setSellItem({ ...sellItem, price: item.price })
                closeBottomSheet()
                break
            case 'gender':
    ...
    const showAttack = async () => {
        ...
        await setMdata([{ name: 'Hình đã đính kèm', id: 1 }, { name: 'Chụp hình', id: 2 }, { name: 'Chọn trong thư viện', id: 3 }])
    ...
    const saveItemEnter = (item, text) => {
        if (item.ref_Name === 'serial' && lstReport?.isOnlyNumber == 1) {
            ...
            if (item.ref_Code === 'phone') {
                if (text !== null) {
                    let textValue = text.replace(/\D+/g, '').replace(/(\d{3})(\d{3})(\d{4})/, '$1$2$3');
                    if (text?.length == 11)
                        textValue = text.replace(/\D+/g, '').replace(/(\d{3})(\d{4})(\d{4})/, '$1$2$3');
                    (!sellItem[item.ref_Name] || textValue == '' || textValue?.length < 12) && setSellItem({ ...sellItem, [item.ref_Name]: textValue })
                } else {
                    setSellItem({ ...sellItem, [item.ref_Name]: null })
                }
            } else {
    ...
    const configDataByItem = async (itemSelect, typeItem, itemMain) => {
        ...
        if (listData.length == 0) {
            listData = await GetByListCode(`"${itemSelect.listByItem}"`, itemSelect.groupName || null)
        }
        let itemConfig = {
    ...
    const ItemViewSelect = ({ itemM }) => {
        return (
            (itemM.ref_Name !== 'products' || (itemM.ref_Name == 'products' && (selectedProducts.competitorId == _competitorId || selectedProducts.competitorId == null || selectedProducts.competitorId == undefined))) ?
            ...
            MasterList?.map((itemM, index) => {
                switch (itemM.ref_Code) {
                    ...
                    case "scan":
                        return <NumericFormat
                            ...
                            renderText={value =>
                                <FormGroup
                                    ...
                                    title={itemM.name}
                                    rightFunc={() => scanQRCODE()} iconRight="barcode"
                                    value={value}
                                    ...
                                    blurOnSubmit={false} handleChangeForm={text => saveItemEnter(itemM, text)}
                        ...
                        />
                    case "number":
                        if (itemM.ref_Name !== 'price' || (itemM.ref_Name == 'price' && (selectedProducts.competitorId == _competitorId || selectedProducts.competitorId == null || selectedProducts.competitorId == undefined))) {
                            return <NumericFormat
                                key={`n${index}`}
                                disabled={true}
                                value={sellItem[itemM.ref_Name] || ''}
                                    ...
                                    <FormGroup
                                        editable={
                                            (itemM.ref_Name === 'price' ?
                                                ((AppNameBuild === daikinApp || AppNameBuild === toshibaApp) ? ((selectedProducts.isHavePrice === 1 || itemM.ref_Id === 1) ? false : true) :
                                                    (itemM.ref_Id === 1 ? false : true)) : (itemM.ref_Id === 1 ? false : true))
                                        }
                                        key={index} inputRef={ref => refInputs.current[index] = ref}
                                        returnKeyType={(Platform.OS === 'ios') ? 'done' : 'next'}
                                        placeholder={itemM.ref_Id === 1 ? itemM.name : 'Nhập ' + itemM.name + ' ở đây'}
                                        value={value} title={itemM.name}
                                        inputStyle={{ textAlign: 'right' }}
                                        keyboardType="numeric" handleChangeForm={text => onNumberChanged(itemM, text)}
                                        onSubmitEditing={() => handlerTextInput(index, MasterList[index + 1] !== undefined ? MasterList[index + 1].ref_Id : 0)}
                    ...
                    case "date":
                        return <FormGroup key={`date${index}`} rightFunc={() => showListSelect(itemM)}
                            iconRight="caret-down" iconRightStyle={{ color: appcolor.primary }}
                            title={itemM.name}
                            value={Moment(sellItem.reportDate?.toString() || new Date()).format("YYYY-MM-DD")}
                            useClearAndroid={false}
                        />
                    case "phone":
                        return <FormGroup title={itemM.name}
                    ...
                    case "template":
                        return <FormGroup
                            ...
                            isRequired={itemM.isRequired == 1}
                            key={itemM.ref_Name}
                            value={lstReport?.isOnlyNumber == 1 ? (sellItem[itemM.ref_Name]?.replace(/\D/g, "") || '') : (sellItem[itemM.ref_Name] || '')}
                            handleChangeForm={text => saveItemEnter(itemM, text)}
                            onClearTextAndroid={() => saveItemEnter(itemM, null)}
                            keyboardType={lstReport?.isOnlyNumber ? 'numeric' : 'default'}
                            placeholder={itemM.textValue}
                            maxLength={itemM.numberValue} editable={itemM.ref_Id === 1 ? false : true} />
                    case "tick":
                        return <View key={`Tick${index}`} style={{ alignItems: 'flex-end' }}>
                            <CheckBox checked={infoSave?.tickInfoSave || false} title={'Giữ thông tin khách hàng'}
                                textStyle={{ color: appcolor.dark, paddingRight: 5 }}
                                containerStyle={{ maxWidth: Dimensions.get('screen').width / 1.6, borderRadius: 10, backgroundColor: appcolor.surface, borderColor: appcolor.light }} onPress={() => onSelectTick()}></CheckBox>
                        </View>
                    case 'itemSelected':
                        const data = JSON.parse(itemM.filterList || '[]')
                        let valueItem = ((sellItem[itemM.ref_Name] != null && typeof sellItem[itemM.ref_Name] == 'object') ? sellItem[itemM.ref_Name].itemName : sellItem[itemM.ref_Name])
                        let subList = []
                        if (sellItem[itemM.ref_Name]?.itemName && sellItem[itemM.ref_Name]?.subList) {
                            valueItem = sellItem[itemM.ref_Name]?.itemName
                            subList = JSON.parse(sellItem[itemM.ref_Name].subList || '[]')
                        }
                        return <View key={`Select${index}`} style={{ flex: 1 }}>
                            < MutipleItemSelected
                            ...
                            />
                            {
                                subList.length > 0 &&
                                <TouchableOpacity
                                    onPress={() => configDataByItem(sellItem[itemM.ref_Name], itemM.ref_Name, itemM)}
                                    style={{ flex: 1, backgroundColor: appcolor.transparent, marginLeft: 40, borderRadius: 8, padding: 8, paddingHorizontal: 16, backgroundColor: appcolor.surface }}>
                                    {
                                        subList.map((it, idx) => {
                                            const value = (it.ref_Code == 'number' ? formatNumber(it.itemValue, ',') : it.itemValue)
                                            return <View key={`subList_${idx}`}
                                                style={{ flexDirection: "row", justifyContent: "space-between", paddingBottom: 4, shadowColor: "#000", shadowOpacity: 0.1, shadowRadius: 4, elevation: 2, }}
                                            >
                                                <Text style={{ fontSize: 12, color: appcolor.dark, flex: 1 }}>{it.name}</Text>
                                                <Text style={{ fontSize: 12, fontWeight: fontWeightBold, color: appcolor.dark }}>{value ?? "-"}</Text>
                                            </View>
                                        })
                                    }
                                </TouchableOpacity>
                    ...
                    case "flag":
                        return <View key={`Tick${index}`} style={{ alignItems: 'flex-end' }}>
                            <CheckBox checked={sellItem?.flag || false} title={itemM.name}
                                textStyle={{ color: appcolor.dark, paddingRight: 5 }}
                                containerStyle={{ maxWidth: Dimensions.get('screen').width / 1.6, borderRadius: 10, backgroundColor: appcolor.surface, borderColor: appcolor.light }}
                                onPress={() => onSelectFlag()}
                            ></CheckBox>
                        </View>
                    case "takePhoto":
                        return <View key={`${index}`} style={{ backgroundColor: appcolor.surface, justifyContent: 'space-between', borderRadius: 10, padding: 10 }}>
                            <ViewPhotoInvoice key={'ViewInvoicephoto'} guiId={guiId} Props={Props} itemM={itemM} handleShowImage={handleShowImage} config={config} />
                        </View>
                    default:
                        return <FormGroup
                            key={`f${index}`}
    ...
    const renderItem = (item, index, refName) => {
        ...
        return (
            <TouchableOpacity
                ...
                style={{ backgroundColor: appcolor.grayLight, flexDirection: 'row', alignItems: 'center', padding: 10, marginBottom: 1 }}
                onPress={onPressItem}>
                <Text style={{ width: '80%', fontSize: scaleSize(15), fontWeight: '500', color: appcolor.dark }} >{index + 1}. {item.name}</Text>
            </TouchableOpacity>
        )
    }
    const ProductView = () => {
        ...
        MasterList.forEach((m, i) => {
            switch (m.ref_Name) {
                case "reportDate":
                    ...
                    uiView.push(
                        <View style={{ flex: 1 }} key={'reportDate'}>
                            <View style={{ flexDirection: 'row', padding: 12, width: '100%' }}>
                                <Text style={{ flexGrow: 1, marginLeft: 12, fontSize: scaleSize(22), color: appcolor.dark }}>{m.name}</Text>
                            </View>
                            <View style={{ borderWidth: 1, borderColor: appcolor.surface, width: '100%' }} />

                            <Calendar
                                firstDay={1}
                                current={Moment(sellItem.reportDate?.toString() || new Date()).format("yyyy-MM-DD")}
                    ...
                    break;
                case "competitor":
                    const dataDivisionlist = dataDivision.filter(it => m.ref_Id == 1 ? it.id == _competitorId : it)
                    _pageTemp = { [m.ref_Name]: i }
                    uiView.push(
                        <View style={{ flex: 1 }} key={0}>
...

                            {
                                <FlatList
                                    ...
                                    renderItem={({ item, index }) => renderItem(item, index, m.ref_Name)}
                                />
                            }
                        </View>)
                    break;
                case "category":
                    ...
                    uiView.push(<View style={{ flex: 1 }} key={1}>
                        <View style={{ flexDirection: 'row', padding: 12, width: '100%' }}>
                            <Text style={{ flexGrow: 1, marginLeft: 12, fontSize: scaleSize(22), color: appcolor.dark }}>{m.name}</Text>
                ...
                case "subcategory":
                    _pageTemp = { ..._pageTemp, [m.ref_Name]: i }
                    uiView.push(<View style={{ flex: 1 }} key={2}>
                        <View style={{ flexDirection: 'row', padding: 12, width: '100%' }}>
                            <Text style={{ flexGrow: 1, marginLeft: 12, fontSize: scaleSize(22), color: appcolor.dark }}>{m.name}</Text>
                        </View>
                        <View style={{ borderWidth: 1, borderColor: appcolor.surface, width: '100%' }} />
                ...
                case "segment":
                    _pageTemp = { ..._pageTemp, [m.ref_Name]: i }
                    uiView.push(<View style={{ flex: 1 }} key={3}>
                        <View style={{ flexDirection: 'row', padding: 12, width: '100%' }}>
                            <Text style={{ flexGrow: 1, marginLeft: 12, fontSize: scaleSize(22), color: appcolor.dark }}>{m.name}</Text>
                        </View>
                        <View style={{ borderWidth: 1, borderColor: appcolor.surface, width: '100%' }} />
                ...
                case "subsegment":
                    _pageTemp = { ..._pageTemp, [m.ref_Name]: i }
                    uiView.push(<View style={{ flex: 1 }} key={4}>
                        <View style={{ flexDirection: 'row', padding: 12, width: '100%' }}>
                            <Text style={{ flexGrow: 1, marginLeft: 12, fontSize: scaleSize(22), color: appcolor.dark }}>{m.name}</Text>
                ...
                case "products":
                    ...
                    uiView.push(<View style={{ flex: 1 }} key={4}>
                        ...
                        <FormGroup
                            containerStyle={{ backgroundColor: appcolor.grayLight, margin: 8, padding: 3, paddingEnd: 8 }}
                            inputStyle={{ fontSize: 13, color: appcolor.dark }}
                            placeholder='Tìm kiếm sản phẩm' iconName='search' editable
    ...
    const handlerSelectCalendar = async (date, refName) => {
        ...
        if (dateString !== null && dateString !== undefined) {
            ...
            if (parseInt(Moment(dateString).format('YYYYMMDD')) > currentDate) {
                alertWarning('Vui lòng chọn ngày báo cáo không được lớn hơn ngày hiện tại!')
                return;
            }
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <LoadingView title={'Đang lưu dữ liệu...'} isLoading={loading} styles={{ marginTop: 8 }} />
            {!loading &&
                <KeyboardAvoidingView
                    ...
                    <ScrollView
                        ...
                        {
                            Dashboard &&
                            <FormGroup value={nameShop !== undefined ? nameShop : HOLDER_TEXT}
                                iconRight="caret-down" title="Cửa hàng" rightFunc={() => showShops()} />
                        }
            ...
            {isVisible &&
                <ModalNotify messager={messager} visible={isVisible} handleVisibleModal={handleVisibleModal} titleConfirm={'Đóng'} />
            }
            <Modal visible={visibleModal} style={{ flex: 1, backgroundColor: appcolor.danger }} >
                <SafeAreaView style={{ flex: 1, backgroundColor: appcolor.light }} >
                    <TouchableOpacity onPress={() => closeBottomSheet()}
                        style={{ zIndex: 100, padding: 20, position: 'absolute', right: 0, top: Platform.OS === 'ios' ? 20 : -10 }}>
            ...
            <ActionSheet
                id={'imageSheet'}
                containerStyle={{ height: deviceHeight * 0.9, width: deviceWidth, backgroundColor: appcolor.light }}
            >
            ...
            <ActionSheet
                ...
                containerStyle={{ height: deviceHeight * 0.85, width: deviceWidth, backgroundColor: appcolor.transparent }}
            >
                <ViewItemByList modalItem={modalItem} handleVisible={handleVisible} handlerShowModal={handlerShowModal} handlerSaveData={handlerSaveData} />
            </ActionSheet>

        </View>
    )
}
const ViewItemByList = ({ modalItem, handleVisible, handlerShowModal, handlerSaveData }) => {
            ...
            if (result) {
                ...
                modalItem.dataShow?.map(it =>
                    dataSave.push({
                        ...
                        itemValue: it.ref_Code == 'date' && !it.itemValue ? Moment(new Date()).format("yyyy-MM-DD") : it.itemValue,
                        name: it.name,
    ...
    const validData = (onFinish) => {
        for (let index = 0; index < modalItem.dataShow?.length || 0; index++) {
            ...
            if (item.isRequired == 1 && (!item.itemValue || item.itemValue?.length == 0)) {
            ...
            <NumericFormat
                ...
                renderText={value =>
                    <FormGroup
                        ...
                        returnKeyType={(Platform.OS === 'ios') ? 'done' : 'next'}
                        placeholder={itemChild.ref_Id === 1 ? itemChild.name : 'Nhập ' + itemChild.name + ' ở đây'}
                        defaultValue={value} title={itemChild.name}
                        inputStyle={{ textAlign: 'right' }}
    ...
    const SelectDate = ({ itemChild, indexChild }) => {
        ...
        const currentDate = (Moment(itemChild.itemValue) || Moment(new Date()).format("yyyy-MM-DD"))
        return (
            <View style={{ flex: 1, paddingBottom: 12 }} key={`Date_${indexChild}`}>
                <Text style={{ color: appcolor.dark, fontSize: 13, padding: 8, fontWeight: fontWeightBold }}>{itemChild.name}</Text>
                <View style={{ backgroundColor: appcolor.light, borderRadius: 8, flexDirection: 'row', alignItems: 'baseline', paddingHorizontal: 20, marginBottom: 10, elevation: 3, shadowColor: appcolor.dark, shadowOpacity: 0.2, shadowRadius: 5, shadowOffset: { width: 0, height: 3 } }}>
                    <Text style={{ color: appcolor.placeholderText, fontSize: 36, fontWeight: '700', marginRight: 8 }}>{currentDate.date()}</Text>
                    <Text style={{ color: appcolor.greylight, fontSize: 16, fontWeight: '500' }}>{currentDate.format('MMM YYYY').toUpperCase()}</Text>
                </View>

                <View style={{ borderRadius: 20, overflow: 'hidden', elevation: 3, shadowColor: appcolor.dark, shadowOpacity: 0.2, shadowRadius: 5, shadowOffset: { width: 0, height: 3 } }}>
                    <Calendar
                        firstDay={1}
                        current={itemChild.itemValue || Moment(new Date()).format("yyyy-MM-DD")}
                        ...
                        markedDates={{
                            [itemChild.itemValue || Moment(new Date()).format("yyyy-MM-DD")]: { selected: true, selectedColor: appcolor.danger, textColor: appcolor.light },
                        }}
    ...
    return (
        <View style={{ width: '100%', height: '100%', backgroundColor: appcolor.surface, borderTopLeftRadius: 50, borderTopRightRadius: 50 }}>
            <View style={{ paddingHorizontal: 16, paddingTop: 20, width: '100%', justifyContent: 'space-between', flexDirection: 'row' }}>
                <TouchableOpacity onPress={() => handleVisible('listByItem')}
                    style={{ width: '15%' }}>
                    <Icon name='close' reverse type='font-asomeware-5' size={20} iconStyle={{ color: appcolor.dark }} color={appcolor.light} />
                </TouchableOpacity>
                <View style={{ flexDirection: 'row', width: '70%', justifyContent: 'center', alignItems: 'center' }}>
                    <Text style={{ fontSize: 16, color: appcolor.dark, fontWeight: fontWeightBold }}>{modalItem.itemMain?.name || 'Thông tin'}</Text>
                </View>
                <TouchableOpacity onPress={() => onSaveData()}
                    style={{ width: '15%' }}>
                    <Icon name='save' reverse type='font-asomeware-5' size={20} iconStyle={{ color: appcolor.dark }} color={appcolor.light} />
                </TouchableOpacity>
            </View>
            <FlatList key="id" data={modalItem.dataShow || []}
                scrollEnabled={true}
...
const ViewImageSheet = ({ dataImage, handleVisible }) => {
    ...
    return (
        <View style={{ width: '100%', height: '100%' }}>
            <TouchableOpacity onPress={() => handleVisible('imageSheet')}
                style={{ position: 'absolute', right: 20, top: Platform.OS == 'ios' ? 40 : 20, zIndex: 100 }}>
                <Icon name='close' type='font-asomeware-5' size={30} color={appcolor.dark} />
            </TouchableOpacity>
            {
    ...
    const RenderItemPhoto = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity key={index} onLongPress={() => onLongPressImage()}
                ...
                style={{ width: deviceWidth / 4, height: deviceWidth / 4, backgroundColor: appcolor.light, margin: 5, borderRadius: 12, justifyContent: 'center', alignItems: "center" }}  >
    ...
    return (
        <View style={{ flex: 1 }}>
            <View style={{ justifyContent: 'space-between', flexDirection: 'row', alignItems: "center" }}>
                <Text style={{ fontSize: 14, color: appcolor.dark, fontWeight: '500' }}>{itemM.name}</Text>
                <TouchableOpacity style={{ width: '30%', justifyContent: 'flex-end', alignItems: 'flex-end' }} onPress={() => isShowDelete ? deletePhotoSelect() : SheetManager.show('ref_takePhoto')}>
                    <Icon iconStyle={{ color: isShowDelete ? appcolor.red : appcolor.dark }} style={{}} size={30} name={isShowDelete ? 'trash' : 'camera-outline'} type='ionicon' />
                </TouchableOpacity>
            </View>
            <View >
                <FlatList
