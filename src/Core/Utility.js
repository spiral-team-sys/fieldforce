...
export function minWidthTab(data) {
    var width = 0;
    (data == undefined || data == null) ? width = deviceHeight :
        width = (data?.length > 4) ? (deviceWidth / 5) : ((data?.length > 0 && data?.length < 5) ? (deviceWidth / data?.length) : 0);
    return width
}
export function removeAccents(str) {
    return str?.normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .replace(/đ/g, 'd').replace(/Đ/g, 'D') || null;
}
...
export async function alertPrint(item, note) {
    alert(note !== undefined ? note + ' ' + JSON.stringify(item) : JSON.stringify(item));
}
...
export function alertConfirm(title, message, actionYes, actionNo, titleYes = 'Có', titleNo = 'Không') {
    Alert.alert(title, message, [{ text: titleNo, onPress: actionNo }, { text: titleYes, onPress: actionYes }]);
}
...
const getDocumentFileName = (fileInfo) => {
    ...
    return fileType && !fileName.endsWith(fileType) ? `${fileName}${fileType}` : fileName
}
const getDocumentDownloadPath = (fileInfo) => {
    const directoryPath = Platform.OS === 'android' && RNFS.DownloadDirectoryPath
...
export const checkFileExist = async (fileInfo) => {
    try {
        ...
        const oldPath = `${RNFS.DocumentDirectoryPath}/${getDocumentFileName(fileInfo)}`;
...
export const openFileViewer = (localPath, actionBack, isNotify = true, actionFalse) => {
    FileViewer.open(localPath, { showOpenWithDialog: true, showAppsSuggestions: true, onDismiss: actionBack }).then(value => {
        console.log(value)
    }).catch(err => {
        if (isNotify) {
...
export const toCurrency = (number) => {
    if (!number) return ''
    return number.toString().match(/[0-9]/g).join('').replace(/\B(?=(\d{3})+(?!\d))/g, ",")
}
export const isValid = (valueCheck) => {
    if (!isNaN(Number(valueCheck))) {
        return valueCheck !== undefined && valueCheck !== null && valueCheck !== ''
    } else {
        return valueCheck !== undefined && valueCheck !== null && valueCheck.length > 0
    }
}
