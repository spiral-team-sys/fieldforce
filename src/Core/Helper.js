    ...
    if (result.statusId === 200) {
      if (Appversion !== result.messager) {
        ...
        if (Linking.canOpenURL(AppStore))
          MessageAcept(
            'Cập nhật phần mềm',
            `Đã có bản cập nhật phần mềm ${result.messager} bạn phải cập nhật trước khi thực hiện công việc.`,
            () => {
    ...
    if (Platform.OS === 'ios') {
      ...
      checkVersion().then(resver => {
        if (resver.needsUpdate) {
          ...
          MessageAcept(
            'Cập nhật phần mềm',
            `Hiện đã có phiên bản cập nhật mới ${resver.version} (${
              resver.notes || ''
            }). Bạn phải cập nhật trước khi thực hiện công việc`,
            () => {
...
export const defaultSetting = {
  ...
  facebook: false,
  exportAttendant:
    AppNameBuild === lgApp || AppNameBuild === bekoApp || toshibaApp
      ? true
      : false,
...
export const GetUrl = urlFile => {
  if (urlFile === undefined || urlFile === null) return null;
  else if (urlFile.indexOf('file://') > -1)
    //Local
    return urlFile;
  else if (urlFile.indexOf('http') > -1)
    //Server
    return urlFile;
...
export const getPhotoUri = PhotoPath => {
  ...
  if (
    ...
    rawPath.toLowerCase() === 'undefined'
  )
    return '';
  const isFull =
    rawPath.indexOf('file://') > -1 ||
    rawPath.indexOf('https://') > -1 ||
    !rawPath.includes('uploaded');
...
export const ColorRand = (index, list) => {
  ...
  if (
    index === undefined ||
    (index !== undefined && index > dataColor.length - 1)
  ) {
...
export const isPhone = phone => {
  ...
  return (phone.length == 10 || phone.length == 11) && phone.indexOf('0') === 0
    ? true
    : false;
};
...
export const CheckTimeServer = async () => {
  ...
  const sameHour =
    now.format('YYYY-MM-DD HH') === online.format('YYYY-MM-DD HH');
  ...
  if (!sameHour || minuteDiff > 4) {
    return {
      success: false,
      message:
        `Thời gian hiện tại là: ${online.format('YYYY-MM-DD HH:mm')}, ` +
...
export async function OnTime(Okey, Error) {
  ...
  if (time !== null) {
    ...
    const onlineTime = moment(time, 'YYYY-MM-DD HH:mm:ss').format(
      'YYYY-MM-DD HH',
    );
    ...
    if (localTime !== onlineTime || accuracyTime > 4) {
      await MessageInfo(
        `Thời gian hiện tại là :${time}, Thời gian thiết bị ${moment().format(
          'YYYY-MM-DD HH:mm',
        )} Bạn vui lòng kiểm tra lại thời gian trong phần cài đặt của máy.`,
      );
      // Error
...
export async function fetchGet(url) {
  try {
    ...
    if (response.status === 401) {
      ...
      return {
        statusId: 401,
        messager: 'Phiên đăng nhập đã hết hạn, vui lòng đăng nhập lại.',
        data: [],
      };
    }
    ...
    if (!responseText) {
      return {
        ...
        messager: 'Không nhận được dữ liệu từ hệ thống',
        data: [],
      };
    }
    ...
    return [
      { statusId: '404', messager: 'Không thể kết nối tới hệ thống', data: [] },
    ];
  }
}
  ...
  try {
    ...
    const mapSubArr = i => {
      ...
      for (let j = 1, lenSubArr = subArr.length; j < lenSubArr; j++) {
        ...
        typeof subFunc === 'function' &&
          subFunc(subArr[j], subAnonymous, j, lenArr);
      }
    };
    for (let i = 0, lenRetArr = arr.length; i < lenRetArr; i++) {
      ...
      if (keyLayer3) {
        if (
          !lastKeyLayer3 ||
          (lastKeyLayer3 &&
            arr[i][keyLayer3] !== lastKeyLayer3 &&
            arr[i][keyLayer2] === lastKeyLayer2)
        ) {
          arr[i][
            `${arr[i][key]}${arr[i][keyLayer2]}${arr[i][keyLayer3]}`
          ] = true;
...
export const confirmAndCall = phoneNumber => {
  alertConfirm('Xác nhận', `Bạn có muốn gọi số "${phoneNumber}" không?`, () => {
...
export function isSameDate(date, dateCompe) {
  ...
  return (
    date.getDate() === dateCompe.getDate() &&
    date.getMonth() === dateCompe.getMonth() &&
    date.getFullYear() === dateCompe.getFullYear()
  );
}
...
export const formatByTemplate = (number, template = '### ###') => {
  try {
    ...
    for (
      let i = 0, idx = 0, lenArr = number.length + lenDelimiter;
      i < lenArr;
      i++
    ) {
...
export const cleanNumberString = (value = '', delimiter = '') => {
  ...
  return value
    .toString()
    .replace(new RegExp(`[\\s${escapedDelimiter}]`, 'g'), '')
    .replace(/\D/g, '');
};
...
export const requestPerrmission = async (permissions, actionResult) => {
  try {
    await request(Platform.select(permissions)).then(result => {
      actionResult(result == RESULTS.GRANTED);
    });
  } catch (e) {
    console.log(e);
  }
...
export const moveFile = async sourcePath => {
  try {
    ...
    const destinationDir = `${RNFS.DocumentDirectoryPath}/${moment().format(
      'YYYYMMDD',
    )}`;
...
export const updateReportEdit = async workinfo => {
  ...
  if (dataByReport.length == 0) {
    ...
    dataAsync.map(it => {
      if (
        it.reportId == workinfo.reportId &&
        (it.isUpdate == 1 || it.isCheck == 1)
      ) {
        it.isUpdate = 0;
        it.isCheck = 0;
      }
...
export const deleteReportEdit = async workinfo => {
  ...
  let json = (await GetAsyncStorage(KeyStore)) || '[]';
  let dataAsync = JSON.parse(json);
  const dataByReport = dataAsync.filter(
    it => it.reportId !== workinfo.reportId,
  );
...
export const getDeviceInfo = async () => {
  ...
  const firstInstallTime = await safeCall(
    () => DeviceInfo.getFirstInstallTime(),
    null,
  );
  const lastUpdateTime = await safeCall(
  ...
  return {
    ...
    getApplicationName: await safeCall(
    ...
    isLocationEnabled: await safeCall(
      () => DeviceInfo.isLocationEnabled(),
      false,
    ),
    getSerialNumber: await safeCall(
      () => DeviceInfo.getSerialNumber(),
      'unknown',
    ),
    getAvailableLocationProviders: await safeCall(
      () => DeviceInfo.getAvailableLocationProviders(),
      {},
    ),
    getFreeDiskStorage: await safeCall(
      () => DeviceInfo.getFreeDiskStorage(),
      null,
    ),
    getIpAddress: await safeCall(() => DeviceInfo.getIpAddress(), '0.0.0.0'),
    getInstallerPackageName: await safeCall(
      () => DeviceInfo.getInstallerPackageName(),
      'unknown',
    ),
    getMaxMemory: await safeCall(() => DeviceInfo.getMaxMemory(), null),
    getTotalDiskCapacity: await safeCall(
    ...
    isBatteryCharging: await safeCall(
      () => DeviceInfo.isBatteryCharging(),
      false,
    ),
    getUserAgent: await safeCall(() => DeviceInfo.getUserAgent(), 'unknown'),
    isHeadphonesConnected: await safeCall(
