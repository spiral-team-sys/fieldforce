...
export const CreateDb = async EmployeeId => {
  try {
    const dbName =
      (AppNameBuild == psvApp ? 'pns_' : 'data_') + EmployeeId + '.db';
    await openDatabase(dbName, '1.0', 'Database', 200000, openCB, errorCB).then(
      async db => {
...
export async function InsertItems(db, tableName, items) {
  try {
    ...
    if (typeof tableName !== 'string')
      throw new Error(
        `Parameter tableName expects string but ${typeof tableName}`,
      );
    ...
    const sqlStrArr = items.map(item => {
      ...
      let sqlStr = columns.reduce(
        (sqlSegment, columnName, index, arr) =>
          `${sqlSegment} ${
            columnsHad.includes(lCFirst(columnName)) ? columnName : ''
          } ${
            index + 1 === arr.length
              ? ')'
              : columnsHad.includes(lCFirst(columns[index + 1]))
              ? ','
              : ''
          }`,
      ...
      sqlStr += columns.reduce(
        (sqlSegment, columnName, index, arr) =>
          `${sqlSegment} ${
            columnsHad.includes(lCFirst(columnName))
              ? typeof item[columnName] !== 'number'
                ? `'${
                    format.test(item[columnName])
                      ? item[columnName].replaceAll("'", "''")
                      : item[columnName]
                  }'`
                : item[columnName]
              : ''
...
export async function SelectItemsClause(
  db,
  tableName,
  columns,
  condition,
...
export async function Store() {
  ...
  const databaseName =
    (AppNameBuild == psvApp ? 'pns_' : 'data_') + dbName + '.db';
  try {
    await openDatabase({ name: databaseName, location: 'default' }).then(db => {
      SQLite = db;
    });
  } catch (error) {
...
export async function UpdateItem(db, tableName, item, condition) {
  try {
    ...
    if (Object.prototype.toString.call(item) !== '[object Object]') {
      throw new Error(
        `Parameter item expects object but ${Object.prototype.toString.call(
          item,
        )}`,
      );
    }
...
export const Insert = async (tableInfo, items) => {
  // Cai thien toc do save
  const db = await Store();

  try {
    if (!Array.isArray(tableInfo.tableFields))
      throw new Error(`Parameter items expects array but ${typeof tableInfo}`);
    if (!Array.isArray(items))
      throw new Error(`Parameter items expects array but ${typeof items}`);
    ...
    await db.transaction(async tx => {
      const insertPromises = items.map(rowdata => {
        let rowItem = columns.map(colName =>
          rowdata[colName] === undefined ? null : rowdata[colName],
        );
...
export const Insert_noUse = async (tableInfo, items) => {
  ...
  try {
    ...
    //Remove UNIQUE Key
    if (columns[columns.length - 1] === 'UNIQUE') columns.pop('UNIQUE');
    ...
    await items.forEach(rowdata => {
      ...
      db.transaction(tx => {
        ...
        console.log(
          err,
          `Insert ${tableInfo.tableName || 'khong tim thay bang'}`,
        );
      });
    });
...
export const Update = async (tableInfo, items, primaryKey) => {
  ...
  try {
    if (!Array.isArray(tableInfo.tableFields))
      throw new Error(
        `Parameter tableFields expects array but ${typeof tableInfo.tableFields}`,
      );
    if (!Array.isArray(items))
      throw new Error(`Parameter items expects array but ${typeof items}`);
    if (typeof primaryKey !== 'string')
      throw new Error(
        `Parameter primaryKey expects string but ${typeof primaryKey}`,
      );
