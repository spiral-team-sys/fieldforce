...
export const AppCreateAction = {
	GetFormNow: () => async (dispatch) => {
		...
		if (result.length > 0) await dispatch({ type: ACTION.SET_FORMNOW, data: result[0] });
	},
...
export const reducer = (state, action) => {
	...
	switch (action.type) {
		...
		case ACTION.SET_LIST_DATA_STORE: {
			console.log(Array.isArray(action.listDataStore), 'Array.isArray(action.listDataStoreArray.isArray(action.listDataStore');
				...
				return {
					...
					listDataStore: isItemInDataStore
						? state.listDataStore.filter(
							(dataStoreItem) => dataStoreItem.shopId !== action.listDataStore.shopId
						)
