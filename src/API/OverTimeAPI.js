...
const GetManagerOT = async actionResult => {
  try {
    ...
    const response = await fetch(
      `${URLDEFAULT}workingplan/managerot/getdata`,
      requestInfo,
    );
...
const SaveManagerOT = async (itemUpload, actionResult) => {
  try {
    ...
    const response = await fetch(
      `${URLDEFAULT}workingplan/managerot/save`,
      requestInfo,
    );
    const result = await response.json();
    actionResult(result);
    ...
    if (result.statusId == 200) {
      await QueryStringSql(
        `UPDATE ${photos.tableName} SET dataUpload=1 WHERE shopId=${itemUpload.shopId} AND reportId=${itemUpload.reportId} AND photoDate=${itemUpload.photoDate}`,
      );
...
const CheckDataManagerOT = async (jsonData, actionResult) => {
  try {
    ...
    const response = await fetch(
      `${URLDEFAULT}workingplan/managerot/checkdetails`,
      requestInfo,
    );
    const result = await response.json();
    actionResult(result);
