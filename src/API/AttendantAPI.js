...
const UploadAttendance = async (photoInfo, type) => {
    try {
        ...
        if (result.status == 100) {
            ...
            await UpdateAttendantStatus(photoInfo.shopId, photoInfo.photoDate, photoInfo.photoType);
            return true;
        } else {
            toastError(`Gửi chấm công không thành công. Lỗi ${result.status}: ${result.messeger}`);
            return false;
        }
    }
    catch (error) {
...
const UploadDataAttendance = async (params, actionResult, errorAction) => {
    try {
        ...
        if (isConnected) {
            ...
            const response = await fetch(`${URLDEFAULT}attendants/uploaddata`, requestInfo)
            const result = await response.json()
            if (result.statusId == 200) {
                await UpdateAttendantStatus(params.shopId, params.photoDate, params.photoType);
                actionResult && actionResult(result.messager)
            } else {
                errorAction && errorAction('Lỗi chấm công', result.messager)
            }
        } else {
            errorAction && errorAction('Lỗi kết nối', `Kết nối dữ liệu Wifi/4G không ổn định, Vui lòng kiểm tra lại`)
        }
    } catch (error) {
...
const validTimeAttendant = async (type, actionResult) => {
    try {
        ...
        const response = await fetch(`${URLDEFAULT}attendants/validTime`, requestInfo)
        const result = await response.json()
        if (result.statusId == 200)
            await actionResult(result.data[0], null)
        else
...
const DataWaysFromLocation = async (jsonLocation, finish) => {
    ...
    const response = await fetch(`${URLDEFAULT}public/distanceaddress`, requestInfo)
    const result = await response.json()
    if (await result.statusCode === 200) {
        const content = await JSON.parse(result.content || '[{}]');
...
const StartStopWork = async (info) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + "attendants/starttopwork", requestInfo)
        const result = await response.json()
        // console.log(result)
        if (result.statusId === 200) {
...
export const ATTENDANT_API = { GetDataAttendance, UploadAttendance, UploadDataAttendance, validTimeAttendant, LocationFromAddress, LocationFromGeoCode, StartStopWork, GetAddressbyGeo, DataWaysFromLocation, DataLocationFromAddress, GetgeoCodeAddress, onlineAttendant }