...
const getEvidentList = async (filterList, actionResult) => {
  try {
    ...
    const response = await fetch(
      URLDEFAULT + 'sellouts/getevidentv2',
      requestInfo,
    );
...
const uploadSellOutEvident = async (reportId, actionResult) => {
  try {
    ...
    if (dataUpload !== null && dataUpload.length > 0) {
      ...
      const response = await fetch(
        URLDEFAULT + 'sellouts/uploadEvident',
        requestInfo,
      );
      const result = await response.json();
      if (result.status == 200) {
        await QueryStringSql(
          `UPDATE ${photos.tableName} SET dataUpload=1 WHERE reportId=${reportId}`,
        );
...
const UploadDataEvident = async (dataUpload, actionResult) => {
  try {
    ...
    const response = await fetch(
      URLDEFAULT + 'sellouts/uploadEvidentV2',
      requestInfo,
    );
    const result = await response.json();
    if (result.status == 200) {
...
export const SelloutAPI = {
  ...
  getEvidentList,
  photoEvident,
  uploadSellOutEvident,
  checkPhotoEvident,
  UploadDataEvident,
  PostFile,
  checkPhotoEvidentUpload,
};
