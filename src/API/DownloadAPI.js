...
import { DeleteAll, Insert, QueryStringSql, UpGradeDB } from "../Core/SqliteDbContext";
import { auditDisplayItems, competitor, dealer, displayHistory, displayTarget, masterList, menulist, mobileKPIList, posm, posmGuid, products, promotionList, regions, shopFormat, shopProfile, stockHistory, storeList, taskList, taskListHistory, trackingCompetitor, trackingDetail } from "../Core/Table";
import { URL_DOWNLOAD, URL_DOWNLOAD_HISTORY_DISPLAY, URL_DOWNLOAD_HISTORY_MARKET, URLDEFAULT } from "../Core/URLs";
...
const saveTaskListHistory = async (workDate) => {
    const { res } = await QueryStringSql(`SELECT COUNT(*) AS total FROM ${taskList.tableName} WHERE workDate IS NOT NULL AND workDate<>${workDate}`)
    ...
    if (connected) {
        try {
            ...
            if (hasUnauthorizedResult(data)) {
                actionResult && await actionResult('Phiên đăng nhập đã hết hạn, vui lòng đăng nhập lại.')
                return false
            }
            ...
            await Promise.all([data, dataHistoryDisplay, dataHistoryMarket, countProduct]).then(async result => {
...
export const DOWNLOADAPI = { downloadAll, downloadTaskList, downloadMenu, uploadDeviceInfo }
