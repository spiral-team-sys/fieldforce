...
const RequestReportNotify = async (data, actionResult) => {
    try {
        ...
        const response = await fetch(`${URLDEFAULT}notify/requestreport`, requestInfo)
        const result = await response.json()
        actionResult(result)
    } catch (e) {
...
const SendNotification = async (data, actionResult) => {
    try {
        ...
        actionResult && actionResult({ statusId: 404, "messager": "can't conected api", data: [] })
    }
}
//
export const NotificationAPI = { GetInApp, GetDataNotify, PostLog, RequestReportNotify, SendNotification }
