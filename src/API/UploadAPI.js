...
const uploadFileBatch = async (dataFiles = [], fileDate, token) => {
    ...
    const response = await fetch(`${URLDEFAULT}upload/file-formdata`, requestInfo);
...
const UploadDataNonReport = async (data, actionResult, progressCallback) => {
    try {
        ...
        if (result.statusId === 200) {
            const uploadResult = await UploadFilesFormData(data.dataFiles || [], data.reportDate, progressCallback)
...
const UploadFilesFormData = async (dataFiles = [], fileDate, progressCallback) => {
    try {
        ...
        progressCallback && progressCallback({ current: currentProgress, total: totalFiles });
