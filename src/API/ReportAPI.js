...
import { checkRawReport, dataUploaded, dataUploaded_Realtime } from "../Controller/ReportController";
import { GetToken, throttling } from "../Core/Helper";
import { Insert, QueryStringSql, replaceQueryString } from "../Core/SqliteDbContext";
...
const GetDataReportByShop = async (dataFilter, actionResult) => {
    const checkSave = await checkRawReport(dataFilter.shopId, dataFilter.reportId);
        ...
        try {
            ...
            if (result.statusId == 200) {
                if (result.data !== null && result.data.length > 0) {
                    ...
                    // LastUdpate 
                    await AsyncStorage.setItem(`lastupdate_${dataFilter.reportId}`, moment().format('HH:mm:ss - DD/MM'))
                    //
...
const GetDataReportByShop_RealTime = async (dataFilter, actionResult) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + `download/reportbyshop/realtime`, requestInfo)
        const result = await response.json()
        if (result.statusId == 200) {
...
const GetDataConfigReport = async (dataFilter, actionResult) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + `download/reportbyshop/getconfig`, requestInfo)
        const result = await response.json()
        if (result.statusId == 200) {
            await actionResult(result.data, null)
        } else {
...
const WorkingCheckReport = async (shopId, reportId, actionResult) => {
    try {
        ...
        const response = await fetch(`${URLDEFAULT}download/reportbyshop/checking`, requestInfo)
        const result = await response.json()
        if (result.statusId == 200) {
            await actionResult(result.data, null)
        } else {
...
const UploadDataRaw = async (shopinfo, reportId) => {
    try {
        ...
        if (result.statusId === 200) {
            ...
            sqlUpdateLoad = await replaceQueryString(sqlUpdateLoad, shopinfo, ["shopId", "auditDate"])
            await QueryStringSql(sqlUpdateLoad)
            await QueryStringSql(`UPDATE ${photos.tableName} SET dataUpload=1 WHERE shopId=${shopinfo.shopId} AND reportId=${reportId} AND photoDate=${shopinfo.auditDate}`)
            await QueryStringSql(`UPDATE ${taskList.tableName} set taskDone=1 WHERE shopId=${shopinfo.shopId} and reportId=${reportId}`)
...
const UploadDataRaw_Realtime = async (itemUpload, shopinfo, reportId) => {
    try {
        ...
        if (result.statusId === 200) {
            ...
            await QueryStringSql(`UPDATE ${taskList.tableName} set taskDone=1 WHERE shopId=${shopinfo.shopId || itemUpload.shopId} and reportId=${reportId}`)
            await UploadFilePhoto();
        }
        return result
    } catch (e) {
...
const GetDataReportZalo = async (workDate, shopId) => {
    try {
        ...
        const response = await fetch(URLDEFAULT + 'employee/reportzalo', requestInfo)
        if (response.status == 204) {
            return { statusId: 204, messager: "Không có dữ liệu", data: result || [] }
        }
        const result = await response.json()
        if (response.status == 200) {
            return { statusId: 200, messager: "Tải dữ liệu thành công", data: result || [] }
        }
    } catch (err) {
        console.log(err)
        return { statusId: 404, messager: "Lỗi truy cập API", data: [] };
...
export const REPORT = { GetDataReportByShop, GetDataReportByShop_RealTime, GetDataConfigReport, WorkingCheckReport, UploadDataRaw, UploadDataRaw_Realtime, GetDecor, UploadFilePhoto, GetDataReportZalo }